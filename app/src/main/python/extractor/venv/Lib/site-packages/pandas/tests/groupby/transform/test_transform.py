...
def test_transform():
    ...
    # GH 9700
    df = DataFrame({"a": range(5, 10), "b": range(5)***REMOVED***)
    msg = "using DataFrameGroupBy.max"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = df.groupby("a").transform(max)
    expected = DataFrame({"b": range(5)***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_transform_fast2():
    ...
    expected = DataFrame(
        {"f": [1.1, 2.1, 2.1, 4.5], "d": dates, "i": [1, 2, 2, 4]***REMOVED***,
        columns=["f", "i", "d"],
    )
    tm.assert_frame_equal(result, expected)
...
def test_transform_broadcast(tsframe, ts):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        grouped = tsframe.groupby({"A": 0, "B": 0, "C": 1, "D": 1***REMOVED***, axis=1)
    msg = "using DataFrameGroupBy.mean"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = grouped.transform(np.mean)
    tm.assert_index_equal(result.index, tsframe.index)
...
def test_transform_bug():
    ...
    df = DataFrame({"A": Timestamp("20130101"), "B": np.arange(5)***REMOVED***)
...
def test_transform_numeric_to_boolean():
    ...
    df = DataFrame({"A": [1.1, 2.2], "B": [1, 2]***REMOVED***)
    result = df.groupby("B").A.transform(lambda x: True)
    tm.assert_series_equal(result, expected)

    df = DataFrame({"A": [1, 2], "B": [1, 2]***REMOVED***)
    result = df.groupby("B").A.transform(lambda x: True)
    tm.assert_series_equal(result, expected)
...
def test_transform_datetime_to_numeric():
    # GH 10972
    # convert dt to float
    df = DataFrame({"a": 1, "b": date_range("2015-01-01", periods=2, freq="D")***REMOVED***)
...
def test_transform_casting():
    ...
    df = DataFrame(
        {
            "A": [f"B-{i***REMOVED***" for i in range(11)],
            "ID3": np.take(
                ["a", "b", "c", "d", "e"], [0, 1, 2, 1, 3, 1, 1, 1, 4, 1, 1]
***REMOVED***,
            "DATETIME": pd.to_datetime([f"2014-10-08 {time***REMOVED***" for time in times]),
        ***REMOVED***,
...
def test_series_fast_transform_date():
    # GH 13191
    df = DataFrame(
        {"grouping": [np.nan, 1, 1, 3], "d": date_range("2014-1-1", "2014-1-4")***REMOVED***
    )
...
def test_transform_length():
    # GH 9697
    df = DataFrame({"col1": [1, 1, 2, 2], "col2": [1, 2, 3, np.nan]***REMOVED***)
...
def test_transform_coercion():
    ...
    # via assignment
    df = DataFrame({"A": ["a", "a", "b", "b"], "B": [0, 1, 3, 4]***REMOVED***)
    g = df.groupby("A")
...
def test_groupby_transform_with_int():
    ...
    expected = DataFrame(
        {"B": np.nan, "C": Series([-1, 0, 1, -1, 0, 1], dtype="float64")***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
    ...
    expected = DataFrame({"B": np.nan, "C": [-1.0, 0.0, 1.0, -1.0, 0.0, 1.0]***REMOVED***)
    tm.assert_frame_equal(result, expected)

    # int that needs float conversion
    s = Series([2, 3, 4, 10, 5, -1])
    df = DataFrame({"A": [1, 1, 1, 2, 2, 2], "B": 1, "C": s, "D": "foo"***REMOVED***)
    with np.errstate(all="ignore"):
        with pytest.raises(TypeError, match="Could not convert"):
            df.groupby("A").transform(lambda x: (x - x.mean()) / x.std())
        result = df.groupby("A")[["B", "C"]].transform(
    ...
    expected = DataFrame({"B": np.nan, "C": concat([s1, s2])***REMOVED***)
    tm.assert_frame_equal(result, expected)

    # int doesn't get downcasted
    result = df.groupby("A")[["B", "C"]].transform(lambda x: x * 2 / 2)
    expected = DataFrame({"B": 1.0, "C": [2.0, 3.0, 4.0, 10.0, 5.0, -1.0]***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_groupby_transform_with_nan_group():
    # GH 9941
    df = DataFrame({"a": range(10), "b": [1, 1, 2, 3, np.nan, 4, 4, 5, 5, 5]***REMOVED***)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"key": ["b"] * 10, "value": np.nan***REMOVED***, Series([np.nan] * 10, name="value")),
        # When there is a single NaN
        (
            {"key": ["b"] * 10 + ["a"] * 2, "value": [3] * 3 + [np.nan] + [3] * 8***REMOVED***,
            {
...
@pytest.mark.parametrize(
    "gb_target",
    [
        {"by": np.random.default_rng(2).integers(0, 50, size=10).astype(float)***REMOVED***,
        {"level": 0***REMOVED***,
        {"by": "string"***REMOVED***,
        pytest.param({"by": "string_missing"***REMOVED***, marks=pytest.mark.xfail),
        {"by": ["int", "string"]***REMOVED***,
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["a", "c"],
            DataFrame({"a": [1, 1, 1], "c": [1, 1, 1]***REMOVED***),
        ),
    ],
)
...
def test_transform_numeric_ret(cols, expected, agg_func):
    ...
    df = DataFrame(
        {"a": date_range("2018-01-01", periods=3), "b": range(3), "c": range(7, 10)***REMOVED***
    )
...
def test_transform_ffill():
    ...
    expected = DataFrame({"values": [0.0, 0.0, 1.0, 1.0]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_group_fill_methods(
    ...
    df = DataFrame({"key": keys, "val": vals***REMOVED***)
    if as_series:
        ...
        exp = DataFrame({"val": _exp_vals***REMOVED***)
...
def test_pct_change(frame_or_series, freq, periods, fill_method, limit):
    ...
    df = DataFrame({"key": key_v, "vals": vals * 2***REMOVED***)
    ...
    msg = (
        ...
        f"{type(gb).__name__***REMOVED***.pct_change are deprecated"
    )
    with tm.assert_produces_warning(FutureWarning, match=msg):
...
def test_ffill_bfill_non_unique_multilevel(func, expected_status):
    ...
    df = DataFrame({"date": date, "symbol": symbol, "status": status***REMOVED***)
...
def test_groupby_transform_rename():
    ...
    def demean_rename(x):
        ...
        result = result.rename(columns={c: f"{c***REMOVED***_demeaned" for c in result.columns***REMOVED***)

        return result

    df = DataFrame({"group": list("ababa"), "value": [1, 1, 1, 2, 2]***REMOVED***)
    expected = DataFrame({"value": [-1.0 / 3, -0.5, -1.0 / 3, 0.5, 2.0 / 3]***REMOVED***)
...
def test_groupby_transform_timezone_column(func):
    ...
    result = DataFrame({"end_time": [ts], "id": [1]***REMOVED***)
...
def test_groupby_transform_with_datetimes(func, values):
    ...
    stocks = DataFrame({"price": np.arange(10.0)***REMOVED***, index=dates)
...
def test_groupby_transform_dtype():
    # GH 22243
    df = DataFrame({"a": [1], "val": [1.35]***REMOVED***)

    result = df["val"].transform(lambda x: x.map(lambda y: f"+{y***REMOVED***"))
    expected1 = Series(["+1.35"], name="val", dtype="object")
    tm.assert_series_equal(result, expected1)

    result = df.groupby("a")["val"].transform(lambda x: x.map(lambda y: f"+{y***REMOVED***"))
    tm.assert_series_equal(result, expected1)
...
def test_transform_absent_categories(func):
    ...
    x_cats = range(2)
    y = [1]
    df = DataFrame({"x": Categorical(x_vals, x_cats), "y": y***REMOVED***)
...
def test_ffill_not_in_axis(func, key, val):
    # GH 21521
    df = DataFrame([[np.nan]])
    result = getattr(df.groupby(**{key: val***REMOVED***), func)()
    expected = df
...
def test_transform_invalid_name_raises():
    # GH#27486
    df = DataFrame({"a": [0, 1, 1, 2]***REMOVED***)
...
def test_transform_agg_by_name(request, reduction_func, frame_or_series):
    ...
    obj = DataFrame(
        {"a": [0, 0, 0, 1, 1, 1], "b": range(6)***REMOVED***,
        index=["A", "B", "C", "D", "E", "F"],
    )
    if frame_or_series is Series:
        obj = obj["a"]
...
def test_transform_fastpath_raises():
    ...
    df = DataFrame({"A": [1, 1, 2, 2], "B": [1, -1, 1, 2]***REMOVED***)
    gb = df.groupby("A")
...
def test_categorical_and_not_categorical_key(observed):
    ...
    df_without_categorical = DataFrame(
        {"A": ["a", "b", "a"], "B": [1, 2, 3], "C": ["a", "b", "a"]***REMOVED***
    )
    ...
    tm.assert_frame_equal(result, expected)
    expected_explicit = DataFrame({"B": [4, 2, 4]***REMOVED***)
...
def test_string_rank_grouping():
    # GH 19354
    df = DataFrame({"A": [1, 1, 2], "B": [1, 2, 3]***REMOVED***)
    result = df.groupby("A").transform("rank")
    expected = DataFrame({"B": [1.0, 2.0, 1.0]***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_transform_cumcount():
    # GH 27472
    df = DataFrame({"a": [0, 0, 0, 1, 1, 1], "b": range(6)***REMOVED***)
...
def test_null_group_lambda_self(sort, dropna, keys):
    ...
    df = DataFrame({"A1": a1, "A2": a2, "B": values***REMOVED***)
...
def test_null_group_str_reducer(request, dropna, reduction_func):
    ...
    df = DataFrame({"A": [1, 1, np.nan, np.nan], "B": [1, 2, 2, 3]***REMOVED***, index=index)
    ...
    if reduction_func == "first":
        expected = DataFrame({"B": [1, 1, 2, 2]***REMOVED***, index=index)
    elif reduction_func == "last":
        expected = DataFrame({"B": [2, 2, 3, 3]***REMOVED***, index=index)
    elif reduction_func == "nth":
        expected = DataFrame({"B": [1, 1, 2, 2]***REMOVED***, index=index)
    elif reduction_func == "size":
        expected = Series([2, 2, 2, 2], index=index)
    elif reduction_func == "corrwith":
        expected = DataFrame({"B": [1.0, 1.0, 1.0, 1.0]***REMOVED***, index=index)
    else:
...
def test_null_group_str_transformer(request, dropna, transformation_func):
    # GH 17093
    df = DataFrame({"A": [1, 1, np.nan], "B": [1, 2, 2]***REMOVED***, index=[1, 2, 3])
    args = get_groupby_method_args(transformation_func, df)
    gb = df.groupby("A", dropna=dropna)
    ...
    for k, (idx, group) in enumerate(gb):
        if transformation_func == "cumcount":
            ...
            res = DataFrame({"B": range(len(group))***REMOVED***, index=group.index)
        elif transformation_func == "ngroup":
...
def test_transform_aligns(func, frame_or_series, expected_values, keys, keys_in_index):
    ...
    df = DataFrame({"a1": [1, 1, 3, 2, 2], "b": [5, 4, 3, 2, 1]***REMOVED***)
    if "a2" in keys:
        df["a2"] = df["a1"]
    if keys_in_index:
    ...
    result = gb.transform(func)
    expected = DataFrame({"b": expected_values***REMOVED***, index=df.index)
    if frame_or_series is Series:
        expected = expected["b"]
    tm.assert_equal(result, expected)
...
def test_idxmin_idxmax_transform_args(how, skipna, numeric_only):
    ...
    df = DataFrame({"a": [1, 1, 1, 2], "b": [3.0, 4.0, np.nan, 6.0], "c": list("abcd")***REMOVED***)
    gb = df.groupby("a")
    msg = f"'axis' keyword in DataFrameGroupBy.{how***REMOVED*** is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = gb.transform(how, 0, skipna, numeric_only)
    warn = None if skipna else FutureWarning
    msg = f"The behavior of DataFrameGroupBy.{how***REMOVED*** with .* any-NA and skipna=False"
    with tm.assert_produces_warning(warn, match=msg):
