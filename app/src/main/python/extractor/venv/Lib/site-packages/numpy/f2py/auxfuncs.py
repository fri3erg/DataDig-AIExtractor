...
def hasassumedshape(rout):
    if rout.get('hasassumedshape'):
        return True
    for a in rout['args']:
        for d in rout['vars'].get(a, {***REMOVED***).get('dimension', []):
            if d == ':':
...
def getcallprotoargument(rout, cb_map={***REMOVED***):
...
def applyrules(rules, d, var={***REMOVED***):
    ret = {***REMOVED***
    ...
    if '_check' in rules and (not rules['_check'](var)):
        return ret
    if 'need' in rules:
        res = applyrules({'needs': rules['need']***REMOVED***, d, var)
        if 'needs' in res:
    ...
    for k in rules.keys():
        ...
        if isinstance(rules[k], str):
            ...
            ret[k] = []
            for i in rules[k]:
                ar = applyrules({k: i***REMOVED***, d, var)
                if k in ar:
                    ret[k].append(ar[k])
        elif k[0] == '_':
            continue
                    ...
                    if isinstance(rules[k][k1], list):
                        for i in rules[k][k1]:
                            if isinstance(i, dict):
                                res = applyrules({'supertext': i***REMOVED***, d, var)
                                if 'supertext' in res:
                                    i = res['supertext']
                                else:
                                    i = ''
                        ...
                        i = rules[k][k1]
                        if isinstance(i, dict):
                            res = applyrules({'supertext': i***REMOVED***, d)
                            if 'supertext' in res:
                                i = res['supertext']
                            else:
                                i = ''
...
def process_f2cmap_dict(f2cmap_all, new_map, c2py_map, verbose = False):
    ...
    new_map_lower = {***REMOVED***
    for k, d1 in new_map.items():
        d1_lower = {k1.lower(): v1 for k1, v1 in d1.items()***REMOVED***
        new_map_lower[k.lower()] = d1_lower

    for k, d1 in new_map_lower.items():
        if k not in f2cmap_all:
            f2cmap_all[k] = {***REMOVED***
        ...
        ***REMOVED***
                ...
                if verbose:
                    errmess(
                        "\tIgnoring map {'%s':{'%s':'%s'***REMOVED******REMOVED***: '%s' must be in %s\n"
