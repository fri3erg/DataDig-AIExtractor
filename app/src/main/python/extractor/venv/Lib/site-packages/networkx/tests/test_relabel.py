...
class TestRelabel:
    ...
    def test_relabel_nodes_copy(self):
        G = nx.empty_graph()
        G.add_edges_from([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")])
        mapping = {"A": "aardvark", "B": "bear", "C": "cat", "D": "dog"***REMOVED***
    ...
    def test_relabel_nodes_multigraph(self):
        ...
        mapping = {"a": "aardvark", "b": "bear"***REMOVED***
    ...
    def test_relabel_copy_name(self):
        G = nx.Graph()
        H = nx.relabel_nodes(G, {***REMOVED***, copy=True)
        assert H.graph == G.graph
        H = nx.relabel_nodes(G, {***REMOVED***, copy=False)
        assert H.graph == G.graph
        G.name = "first"
        H = nx.relabel_nodes(G, {***REMOVED***, copy=True)
        assert H.graph == G.graph
    ...
    def test_relabel_toposort(self):
        ...
        G = nx.relabel_nodes(G, {i: i + 1 for i in range(4)***REMOVED***, copy=False)
    ...
    def test_relabel_selfloop(self):
        ...
        G = nx.relabel_nodes(G, {1: "One", 2: "Two", 3: "Three"***REMOVED***, copy=False)
        assert nodes_equal(G.nodes(), ["One", "Three", "Two"])
        G = nx.MultiDiGraph([(1, 1), (1, 2), (2, 3)])
        G = nx.relabel_nodes(G, {1: "One", 2: "Two", 3: "Three"***REMOVED***, copy=False)
        assert nodes_equal(G.nodes(), ["One", "Three", "Two"])
        G = nx.MultiDiGraph([(1, 1)])
        G = nx.relabel_nodes(G, {1: 0***REMOVED***, copy=False)
    ...
    def test_relabel_multidigraph_inout_merge_nodes(self):
        for MG in (nx.MultiGraph, nx.MultiDiGraph):
            for cc in (True, False):
                ...
                mapping = {0: 9, 1: 9, 2: 9, 3: 9***REMOVED***
                H = nx.relabel_nodes(G, mapping, copy=cc)
                # No ordering on keys enforced
                assert {"value": "a"***REMOVED*** in H[9][4].values()
                assert {"value": "b"***REMOVED*** in H[9][4].values()
                assert {"value": "c"***REMOVED*** in H[4][9].values()
                assert len(H[4][9]) == 3 if G.is_directed() else 6
                assert {"value": "d"***REMOVED*** in H[4][9].values()
                assert {"value": "e"***REMOVED*** in H[9][4].values()
                assert {"value": "f"***REMOVED*** in H[4][9].values()
    ...
    def test_relabel_multigraph_merge_inplace(self):
        ...
        G[0][2][0]["value"] = "b"
        G[0][3][0]["value"] = "c"
        mapping = {1: 4, 2: 4, 3: 4***REMOVED***
        nx.relabel_nodes(G, mapping, copy=False)
        # No ordering on keys enforced
        assert {"value": "a"***REMOVED*** in G[0][4].values()
        assert {"value": "b"***REMOVED*** in G[0][4].values()
        assert {"value": "c"***REMOVED*** in G[0][4].values()
    ...
    def test_relabel_multigraph_merge_copy(self):
        ...
        mapping = {1: 4, 2: 4, 3: 4***REMOVED***
        H = nx.relabel_nodes(G, mapping, copy=True)
        assert {"value": "a"***REMOVED*** in H[0][4].values()
        assert {"value": "b"***REMOVED*** in H[0][4].values()
        assert {"value": "c"***REMOVED*** in H[0][4].values()
    ...
    def test_relabel_preserve_node_order_full_mapping_with_copy_true(self):
        ...
        mapping = {2: "a", 1: "b", 0: "c"***REMOVED***  # dictionary keys out of order on purpose
        H = nx.relabel_nodes(G, mapping, copy=True)
    ...
    def test_relabel_preserve_node_order_partial_mapping_with_copy_true(self):
        G = nx.path_graph(3)
        original_order = list(G)
        mapping = {1: "a", 0: "b"***REMOVED***  # partial mapping and keys out of order on purpose
        H = nx.relabel_nodes(G, mapping, copy=True)
        new_order = list(H)
        assert [mapping.get(i, i) for i in original_order] == new_order
