...
@pytest.mark.parametrize(
    ...
    [((), {***REMOVED***, 0), ((), {"a": 1***REMOVED***, 1), ((2, 3), {***REMOVED***, 32), ((1,), {"c": 2***REMOVED***, 201)],
)
...
def test_agg_mapping_func_deprecated():
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        s.agg({"a": foo1, "b": foo2***REMOVED***, 0, 3, c=4)
...
def test_apply_box_dt64():
    ...
    res = ser.apply(lambda x: f"{type(x).__name__***REMOVED***_{x.day***REMOVED***_{x.tz***REMOVED***", by_row="compat")
...
def test_apply_box_td64():
    ...
    res = ser.apply(lambda x: f"{type(x).__name__***REMOVED***_{x.days***REMOVED***", by_row="compat")
...
def test_apply_box_period():
    ...
    res = ser.apply(lambda x: f"{type(x).__name__***REMOVED***_{x.freqstr***REMOVED***", by_row="compat")
...
def test_transform(string_series, by_row):
    ...
    with np.errstate(all="ignore"):
        ...
        result = string_series.apply({"foo": np.sqrt, "bar": np.abs***REMOVED***, by_row=by_row)
...
def test_transform_partial_failure(op, request):
    ...
    if op in ("ffill", "bfill", "pad", "backfill", "shift"):
        request.applymarker(
            pytest.mark.xfail(reason=f"{op***REMOVED*** is successful on any dtype")
        )
    ...
    with pytest.raises(error, match=msg):
        ser.transform({"A": op, "B": "shift"***REMOVED***)

    with pytest.raises(error, match=msg):
        ser.transform({"A": [op], "B": ["shift"]***REMOVED***)

    with pytest.raises(error, match=msg):
        ser.transform({"A": [op, "shift"], "B": [op]***REMOVED***)
...
def test_transform_partial_failure_valueerror():
    ...
    with pytest.raises(ValueError, match=msg):
        ser.transform({"A": raising_op, "B": noop***REMOVED***)

    with pytest.raises(ValueError, match=msg):
        ser.transform({"A": [raising_op], "B": [noop]***REMOVED***)

    with pytest.raises(ValueError, match=msg):
        ser.transform({"A": [noop, raising_op], "B": [noop]***REMOVED***)
...
def test_with_nested_series(datetime_series, op_name):
    ...
    expected = DataFrame({"x": datetime_series, "x^2": datetime_series**2***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    "how, kwds",
    [("agg", {***REMOVED***), ("apply", {"by_row": "compat"***REMOVED***), ("apply", {"by_row": False***REMOVED***)],
)
def test_non_callable_aggregates(how, kwds):
    ...
    expected = Series({"size": 3.0, "count": 2.0, "mean": 1.5***REMOVED***)
    tm.assert_series_equal(result, expected)

    result = getattr(s, how)({"size": "size", "count": "count", "mean": "mean"***REMOVED***, **kwds)
    tm.assert_series_equal(result, expected)
...
def test_apply_listlike_reducer(string_series, ops, names, how, kwargs):
    # GH 39140
    expected = Series({name: op(string_series) for name, op in zip(names, ops)***REMOVED***)
    expected.name = "series"
    warn = FutureWarning if how == "agg" else None
    msg = f"using Series.[{'|'.join(names)***REMOVED***]"
...
@pytest.mark.parametrize(
    ...
    [
        {"A": np.sum***REMOVED***,
        {"A": np.sum, "B": np.mean***REMOVED***,
        Series({"A": np.sum***REMOVED***),
        Series({"A": np.sum, "B": np.mean***REMOVED***),
    ],
)
@pytest.mark.parametrize(
    "how, kwargs",
...
def test_apply_dictlike_reducer(string_series, ops, how, kwargs, by_row):
    # GH 39140
    expected = Series({name: op(string_series) for name, op in ops.items()***REMOVED***)
...
@pytest.mark.parametrize(
    "ops, expected",
    [
        ([lambda x: x], DataFrame({"<lambda>": [1, 2, 3]***REMOVED***)),
...
@pytest.mark.parametrize(
    ...
    [
        {"A": np.sqrt***REMOVED***,
        {"A": np.sqrt, "B": np.exp***REMOVED***,
        Series({"A": np.sqrt***REMOVED***),
        Series({"A": np.sqrt, "B": np.exp***REMOVED***),
    ],
)
def test_apply_dictlike_transformer(string_series, ops, by_row):
    # GH 39140
    with np.errstate(all="ignore"):
        expected = concat({name: op(string_series) for name, op in ops.items()***REMOVED***)
        expected.name = string_series.name
        result = string_series.apply(ops, by_row=by_row)
        tm.assert_series_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"a": lambda x: x***REMOVED***,
            Series([1, 2, 3], index=MultiIndex.from_arrays([["a"] * 3, range(3)])),
        ),
        ({"a": lambda x: x.sum()***REMOVED***, Series([6], index=["a"])),
    ],
)
...
def test_apply_retains_column_name(by_row):
    # GH 16380
    df = DataFrame({"x": range(3)***REMOVED***, Index(range(3), name="x"))
...
def test_series_apply_unpack_nested_data():
    ...
    expected = DataFrame({0: [1.0, 4.0], 1: [2.0, 5.0], 2: [3.0, 6.0], 3: [np.nan, 7]***REMOVED***)
    tm.assert_frame_equal(result, expected)
