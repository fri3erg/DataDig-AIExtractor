...
***REMOVED***
    ...
    def test_shift_object_non_scalar_fill(self):
        ...
        result = obj_ser.shift(1, fill_value={***REMOVED***)
        assert result[0] == {***REMOVED***

        obj_df = obj_ser.to_frame()
        result = obj_df.shift(1, fill_value={***REMOVED***)
        assert result.iloc[0, 0] == {***REMOVED***
...
***REMOVED***
    ...
    def test_shift_bool(self):
        df = DataFrame({"high": [True, False], "low": [False, False]***REMOVED***)
        rs = df.shift(1)
        xp = DataFrame(
    ...
    def test_shift_categorical(self):
        ...
        df = DataFrame({"one": s1, "two": s2***REMOVED***)
        rs = df.shift(1)
        xp = DataFrame({"one": s1.shift(1), "two": s2.shift(1)***REMOVED***)
        tm.assert_frame_equal(rs, xp)
...
***REMOVED***
    ...
    def test_shift_axis1_multiple_blocks(self, using_array_manager):
        ...
        expected = expected.pipe(
            ...
            .astype({0: "float", 1: "float"***REMOVED***)
        ...
        expected = expected.pipe(
            lambda df: df.set_axis(range(df.shape[1]), axis=1)
            .astype({3: "float", 4: "float"***REMOVED***)
            .set_axis(df.columns, axis=1)
        )
        expected.iloc[:, -2:] = np.nan
        expected.columns = df3.columns
...
***REMOVED***,
    ...
    def test_shift_dt64values_axis1_invalid_fill(self, vals, as_cat):
        ...
        expected = DataFrame({"A": ["foo", "foo"]***REMOVED***)
        tm.assert_frame_equal(result, expected)
        ...
        expected = DataFrame({"A": df2["B"], "B": ["foo", "foo"]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_shift_axis1_categorical_columns(self):
        ...
        df = DataFrame(
            {"a": [1, 3], "b": [2, 4], "c": [5, 6]***REMOVED***, index=ci[:-1], columns=ci
        )
        result = df.shift(axis=1)

        expected = DataFrame(
            {"a": [np.nan, np.nan], "b": [1, 3], "c": [2, 4]***REMOVED***, index=ci[:-1], columns=ci
        )
        tm.assert_frame_equal(result, expected)
        ...
        expected = DataFrame(
            {"a": [np.nan, np.nan], "b": [np.nan, np.nan], "c": [1, 3]***REMOVED***,
            index=ci[:-1],
            columns=ci,
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_shift_with_offsets_freq(self):
        df = DataFrame({"x": [1, 2, 3]***REMOVED***, index=date_range("2000", periods=3))
    ...
    def test_shift_with_iterable_check_other_arguments(self):
        # GH#44424
        data = {"a": [1, 2], "b": [4, 5]***REMOVED***
        shifts = [0, 1]
        df = DataFrame(data)

        # test suffix
        shifted = df[["a"]].shift(shifts, suffix="_suffix")
        expected = DataFrame({"a_suffix_0": [1, 2], "a_suffix_1": [np.nan, 1.0]***REMOVED***)
        tm.assert_frame_equal(shifted, expected)
