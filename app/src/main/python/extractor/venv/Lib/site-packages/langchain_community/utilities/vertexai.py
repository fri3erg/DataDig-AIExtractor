...
def raise_vertex_import_error(minimum_expected_version: str = "1.38.0") -> None:
    ...
    raise ImportError(
        ...
        f"pip install google-cloud-aiplatform>={minimum_expected_version***REMOVED***"
    )
...
def get_client_info(module: Optional[str] = None) -> "ClientInfo":
    ...
    client_library_version = (
        f"{langchain_version***REMOVED***-{module***REMOVED***" if module else langchain_version
    )
    return ClientInfo(
        client_library_version=client_library_version,
        user_agent=f"langchain/{client_library_version***REMOVED***",
    )
...
def load_image_from_gcs(path: str, project: Optional[str] = None) -> "Image":
    ...
    if len(blobs) > 1:
        raise ValueError(f"Found more than one candidate for {path***REMOVED***!")
