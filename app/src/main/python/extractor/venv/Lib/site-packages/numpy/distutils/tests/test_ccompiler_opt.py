        ...
        if regex:
            if isinstance(regex, dict):
                for k, v in regex.items():
                    if v[-1:] not in ')***REMOVED***$?\\.+*':
                        regex[k] = v + '$'
            else:
                assert(isinstance(regex, str))
                if regex[-1:] not in ')***REMOVED***$?\\.+*':
                    regex += '$'
        return regex
...
***REMOVED***
    ...
    def expect_targets(self, targets, groups={***REMOVED***, **kwargs):
        match = self.arg_regex(**kwargs)
        if match is None:
            return
...
***REMOVED***
    ...
    def expect_target_flags(self, targets, groups={***REMOVED***, **kwargs):
    ...
    def test_targets_policies(self):
        ...
        for policy in ("$maxopt", "$autovec"):
            ...
            self.expect_target_flags(
                ...
                gcc={"baseline":".*-O3.*"***REMOVED***, icc={"baseline":".*-O3.*"***REMOVED***,
                iccw={"baseline":".*/O3.*"***REMOVED***, msvc={"baseline":".*/O2.*"***REMOVED***,
                unknown={"baseline":".*"***REMOVED***
***REMOVED***

        # 'werror', force compilers to treat warnings as errors
        self.expect_target_flags(
            "/*@targets baseline $werror */",
            gcc={"baseline":".*-Werror.*"***REMOVED***, icc={"baseline":".*-Werror.*"***REMOVED***,
            iccw={"baseline":".*/Werror.*"***REMOVED***, msvc={"baseline":".*/WX.*"***REMOVED***,
            unknown={"baseline":".*"***REMOVED***
        )
...
def new_test(arch, cc):
    ...
    class TestCCompilerOpt_{class_name***REMOVED***(_Test_CCompilerOpt, unittest.TestCase):
        arch = '{arch***REMOVED***'
        cc   = '{cc***REMOVED***'
    ...
    return textwrap.dedent("""\
    class TestCCompilerOpt_{class_name***REMOVED***(_Test_CCompilerOpt):
        arch = '{arch***REMOVED***'
        cc   = '{cc***REMOVED***'
    """).format(
        class_name=arch + '_' + cc, arch=arch, cc=cc
