...
client operation.  Here are a few examples using ``upload_file``::
    ...
    transfer.upload_file('/tmp/myfile', 'bucket', 'key',
                         extra_args={'ACL': 'public-read'***REMOVED***)

    # Setting metadata
    transfer.upload_file('/tmp/myfile', 'bucket', 'key',
                         extra_args={'Metadata': {'a': 'b', 'c': 'd'***REMOVED******REMOVED***)

    # Setting content type
    transfer.upload_file('/tmp/myfile.json', 'bucket', 'key',
                         extra_args={'ContentType': "application/json"***REMOVED***)
...
def create_transfer_manager(client, config, osutil=None):
    ...
    if _should_use_crt(config):
        ...
        if crt_transfer_manager is not None:
            logger.debug(
                f"Using CRT client. pid: {getpid()***REMOVED***, thread: {threading.get_ident()***REMOVED***"
***REMOVED***
            return crt_transfer_manager

    # If we don't resolve something above, fallback to the default.
    logger.debug(
        f"Using default client. pid: {getpid()***REMOVED***, thread: {threading.get_ident()***REMOVED***"
    )
...
def _should_use_crt(config):
    ...
    logger.debug(
        ...
        f"{pref_transfer_client***REMOVED***, CRT available: {HAS_CRT***REMOVED***, "
        f"Instance Optimized: {is_optimized_instance***REMOVED***."
    )
    return False
    ...
    def upload_file(
        ...
        # client error.
            ...
            raise S3UploadFailedError(
                "Failed to upload {***REMOVED*** to {***REMOVED***: {***REMOVED***".format(
