...
def test_invert_array():
    df = pd.DataFrame({"a": pd.date_range("20190101", periods=3, tz="UTC")***REMOVED***)
...
def test_typical_usecase():
    df = pd.DataFrame(
        [{"var1": "a,b,c", "var2": 1***REMOVED***, {"var1": "d,e,f", "var2": 2***REMOVED***],
        columns=["var1", "var2"],
    )
    exploded = df.var1.str.split(",").explode()
    result = df[["var2"]].join(exploded)
    expected = pd.DataFrame(
        {"var2": [1, 1, 1, 2, 2, 2], "var1": list("abcdef")***REMOVED***,
        columns=["var2", "var1"],
        index=[0, 0, 0, 1, 1, 1],
    )
...
def test_explode_sets():
    ...
    s = pd.Series([{"a", "b", "c"***REMOVED***], index=[1])
