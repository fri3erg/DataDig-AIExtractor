...
def all_simple_paths(G, source, target, cutoff=None):
    ...
    """
    if source not in G:
        raise nx.NodeNotFound(f"source node {source***REMOVED*** not in graph")
    if target in G:
        ...
        try:
            ...
            raise nx.NodeNotFound(f"target node {target***REMOVED*** not in graph") from err
...
def _all_simple_paths_graph(G, source, targets, cutoff):
    ...
    while stack:
        ...
        if child is None:
            ...
            for target in (targets & (set(children) | {child***REMOVED***)) - set(visited.keys()):
...
def shortest_simple_paths(G, source, target, weight=None):
    ...
    if target not in G:
        raise nx.NodeNotFound(f"target node {target***REMOVED*** not in graph")
...
def _bidirectional_dijkstra(
    ...
    if source == target:
        if source not in G:
            raise nx.NodeNotFound(f"Node {source***REMOVED*** not in graph")
        return (0, [source])
    ...
    seen = [{source: 0***REMOVED***, {target: 0***REMOVED***]  # dictionary of distances to
    # nodes seen
    c = count()
