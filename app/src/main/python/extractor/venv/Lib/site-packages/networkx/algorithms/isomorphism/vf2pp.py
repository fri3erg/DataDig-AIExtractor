...
{1: 1, 2: 2, 0: 0, 3: 3***REMOVED***

With node labels:

>>> G1 = nx.path_graph(4)
>>> G2 = nx.path_graph(4)
>>> mapped = {1: 1, 2: 2, 3: 3, 0: 0***REMOVED***
...
@nx._dispatch(graphs={"G1": 0, "G2": 1***REMOVED***, node_attrs={"node_label": "default_label"***REMOVED***)
...
def _matching_order(graph_params):
    ...
    label_rarity = {label: len(nodes) for label, nodes in nodes_of_G2Labels.items()***REMOVED***
    used_degrees = {node: 0 for node in G1***REMOVED***
    node_order = []
...
def _cut_PT(u, v, graph_params, state_params):
    ...
    u_labels_predecessors, v_labels_predecessors = {***REMOVED***, {***REMOVED***
    if G1.is_directed():
        u_labels_predecessors = nx.utils.groups(
            {n1: G1_labels[n1] for n1 in G1.pred[u]***REMOVED***
        )
        v_labels_predecessors = nx.utils.groups(
            {n2: G2_labels[n2] for n2 in G2.pred[v]***REMOVED***
        )

        if set(u_labels_predecessors.keys()) != set(v_labels_predecessors.keys()):
            return True

    u_labels_successors = nx.utils.groups({n1: G1_labels[n1] for n1 in G1[u]***REMOVED***)
    v_labels_successors = nx.utils.groups({n2: G2_labels[n2] for n2 in G2[v]***REMOVED***)
...
***REMOVED*** != G2.number_of_edges(predecessor, v):
                return False
...
def _update_Tinout(new_node1, new_node2, graph_params, state_params):
    ...
    uncovered_successors_G1 = {succ for succ in G1[new_node1] if succ not in mapping***REMOVED***
