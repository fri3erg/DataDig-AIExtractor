...
def test_spectral_bisection():
    ...
    assert C == ({0, 1, 2***REMOVED***, {3, 4, 5***REMOVED***)

    mapping = dict(enumerate("badfec"))
    G = nx.relabel_nodes(G, mapping)
    C = nx.spectral_bisection(G)
    assert C == (
        {mapping[0], mapping[1], mapping[2]***REMOVED***,
        {mapping[3], mapping[4], mapping[5]***REMOVED***,
    )
...
***REMOVED*** == pytest.approx(sigma, abs=1e-7)
...
class TestSpectralOrdering:
    ...
    def test_three_nodes(self, method):
        ...
        assert {1, 3***REMOVED*** in (set(order[:-1]), set(order[1:]))
    ...
    def test_three_nodes_multigraph(self, method):
        ...
        assert set(order) == set(G)
        assert {2, 3***REMOVED*** in (set(order[:-1]), set(order[1:]))
