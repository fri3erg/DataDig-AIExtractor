...
def _format_url(url: str, path_params: dict) -> str:
    expected_path_param = re.findall(r"{(.*?)***REMOVED***", url)
    new_params = {***REMOVED***
    for param in expected_path_param:
        ...
        if isinstance(val, list):
            if param[0] == ".":
                ...
                sep = f"{clean_param***REMOVED***=" if param[-1] == "*" else ","
                new_val = f"{clean_param***REMOVED***=" + sep.join(val)
            else:
                new_val = ",".join(val)
            ...
            if param[0] == ".":
                new_val = f".{val***REMOVED***"
            elif param[0] == ";":
                new_val = f";{clean_param***REMOVED***={val***REMOVED***"
            else:
                new_val = val
...
def _openapi_params_to_json_schema(params: List[Parameter], spec: OpenAPISpec) -> dict:
    ...
    return {"type": "object", "properties": properties, "required": required***REMOVED***
        ...
        ***REMOVED***
            ...
            ***REMOVED***
                if len(media_types) == 1:
                    ...
                    request_args["data"] = {"anyOf": list(media_types.values())***REMOVED***
...
class SimpleRequestChain(Chain):
    ...
    def _call(
        ...
        _text = f"Calling endpoint {_pretty_name***REMOVED*** with arguments:\n" + _pretty_args
        _run_manager.on_text(_text)
        api_response: Response = self.request_method(name, args)
        if api_response.status_code != 200:
            response = (
                f"{api_response.status_code***REMOVED***: {api_response.reason***REMOVED***"
                + f"\nFor {name***REMOVED*** "
                + f"Called with args: {args.get('params','')***REMOVED***"
***REMOVED***
        else:
            try:
        ...
        return {self.output_key: response***REMOVED***
...
def get_openapi_chain(
    ...
    **kwargs: Any,
    ...
    if isinstance(spec, str):
        for conversion in (
            ...
            try:
                ...
                pass
        if isinstance(spec, str):
            raise ValueError(f"Unable to parse spec from source {spec***REMOVED***")
    ...
    llm_chain = LLMChain(
        llm=llm,
        prompt=prompt,
        llm_kwargs={"functions": openai_fns***REMOVED***,
        output_parser=JsonOutputFunctionsParser(args_only=False),
        output_key="function",
        verbose=verbose,
        **(llm_chain_kwargs or {***REMOVED***),
    )
