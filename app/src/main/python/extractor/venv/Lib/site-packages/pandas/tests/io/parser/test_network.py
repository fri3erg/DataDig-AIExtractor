...
class TestS3:
    def test_parse_public_s3_bucket(self, s3_public_bucket_with_data, tips_df, s3so):
        ...
        for ext, comp in [("", None), (".gz", "gzip"), (".bz2", "bz2")]:
            df = read_csv(
                f"s3://{s3_public_bucket_with_data.name***REMOVED***/tips.csv" + ext,
                compression=comp,
...
***REMOVED***
    ...
    def test_parse_private_s3_bucket(self, s3_private_bucket_with_data, tips_df, s3so):
        ...
        pytest.importorskip("s3fs")
        df = read_csv(
            f"s3://{s3_private_bucket_with_data.name***REMOVED***/tips.csv", storage_options=s3so
        )
        assert isinstance(df, DataFrame)
        assert not df.empty
        tm.assert_frame_equal(df, tips_df)
    ...
    def test_parse_public_s3n_bucket(self, s3_public_bucket_with_data, tips_df, s3so):
        ...
        df = read_csv(
            f"s3n://{s3_public_bucket_with_data.name***REMOVED***/tips.csv",
            nrows=10,
            storage_options=s3so,
        )
        assert isinstance(df, DataFrame)
    ...
    def test_parse_public_s3a_bucket(self, s3_public_bucket_with_data, tips_df, s3so):
        ...
        df = read_csv(
            f"s3a://{s3_public_bucket_with_data.name***REMOVED***/tips.csv",
            nrows=10,
            storage_options=s3so,
        )
        assert isinstance(df, DataFrame)
...
***REMOVED***
    ...
    def test_read_s3_fails_private(self, s3_private_bucket, s3so):
        ...
        with pytest.raises(OSError, match=msg):
            read_csv(f"s3://{s3_private_bucket.name***REMOVED***/file.csv")
...
***REMOVED***
    ...
    def test_read_csv_chunked_download(self, s3_public_bucket, caplog, s3so):
        ...
        with BytesIO(df.to_csv().encode("utf-8")) as buf:
            ...
            uri = f"{s3_public_bucket.name***REMOVED***/large-file.csv"
            match_re = re.compile(rf"^Fetch: {uri***REMOVED***, 0-(?P<stop>\d+)$")
    ...
    ***REMOVED***
    ...
    def test_read_s3_with_hash_in_key(self, s3_public_bucket_with_data, tips_df, s3so):
        # GH 25945
        result = read_csv(
            f"s3://{s3_public_bucket_with_data.name***REMOVED***/tips#1.csv", storage_options=s3so
        )
    ...
    def test_read_feather_s3_file_path(
        ...
        res = read_feather(
            f"s3://{s3_public_bucket_with_data.name***REMOVED***/simple_dataset.feather",
            storage_options=s3so,
        )
