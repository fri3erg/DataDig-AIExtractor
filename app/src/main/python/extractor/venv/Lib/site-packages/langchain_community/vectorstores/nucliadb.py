...
***REMOVED***
        ...
        if local:
            ...
            self._config["BACKEND"] = f"{backend***REMOVED***/api/v1"
...
***REMOVED***
    ...
    @property
    def kb_url(self) -> str:
        return f"{self._config['BACKEND']***REMOVED***/kb/{self._kb***REMOVED***"
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        for i, text in enumerate(texts):
            extra: Dict[str, Any] = {"metadata": ""***REMOVED***
            if metadatas:
                extra = {"metadata": metadatas[i]***REMOVED***
            id = factory.create(
                texts={"text": {"body": text***REMOVED******REMOVED***,
                extra=extra,
                url=self.kb_url,
        ...
        for resource in results.resources.values():
            for field in resource.fields.values():
                for paragraph_id, paragraph in field.paragraphs.items():
                    ...
                    if not field_type:
                        continue
                    value = getattr(resource.data, field_type, {***REMOVED***).get(field_id, None)
                    paragraphs.append(
                        {
                            "text": paragraph.text,
                            "metadata": {
                                "extra": getattr(
                                    getattr(resource, "extra", {***REMOVED***), "metadata", None
                    ***REMOVED***,
                                "value": value,
                            ***REMOVED***,
