...
def _warn_on_import(name: str, replacement: Optional[str] = None) -> None:
    ...
    if replacement:
        warnings.warn(
            f"Importing {name***REMOVED*** from langchain root module is no longer supported. "
            f"Please use {replacement***REMOVED*** instead."
        )
    else:
        warnings.warn(
            f"Importing {name***REMOVED*** from langchain root module is no longer supported."
...
***REMOVED***,
        ...
        raise AttributeError(f"Could not find: {name***REMOVED***")
