...
VALID_BUCKET = re.compile(r'^[a-zA-Z0-9.\-_]{1,255***REMOVED***$')
_ACCESSPOINT_ARN = (
    r'^arn:(aws).*:(s3|s3-object-lambda):[a-z\-0-9]*:[0-9]{12***REMOVED***:accesspoint[/:]'
    r'[a-zA-Z0-9\-.]{1,63***REMOVED***$'
)
_OUTPOST_ARN = (
    r'^arn:(aws).*:s3-outposts:[a-z\-0-9]+:[0-9]{12***REMOVED***:outpost[/:]'
    r'[a-zA-Z0-9\-]{1,63***REMOVED***[/:]accesspoint[/:][a-zA-Z0-9\-]{1,63***REMOVED***$'
)
...
def add_recursion_detection_header(params, **kwargs):
    ...
    if has_lambda_name and trace_id:
        ...
        if 'X-Amzn-Trace-Id' not in headers:
            headers['X-Amzn-Trace-Id'] = quote(trace_id, safe='-=;:+&[]{***REMOVED***"\',')
...
def set_operation_specific_signer(context, signing_name, **kwargs):
    ...
    if auth_type.startswith('v4'):
        ...
        if auth_type == 'v4a':
            ...
            # global signature.
            signing = {'region': '*', 'signing_name': signing_name***REMOVED***
        ...
        if signing_name in S3_SIGNING_NAMES:
            signature_version = f's3{signature_version***REMOVED***'
...
def validate_bucket_name(params, **kwargs):
    ...
    if not VALID_BUCKET.search(bucket) and not VALID_S3_ARN.search(bucket):
        error_msg = (
            f'Invalid bucket name "{bucket***REMOVED***": Bucket name must match '
            f'the regex "{VALID_BUCKET.pattern***REMOVED***" or be an ARN matching '
            f'the regex "{VALID_S3_ARN.pattern***REMOVED***"'
        )
...
def document_copy_source_form(section, event_name, **kwargs):
    if 'request-example' in event_name:
        ...
        value_portion.write(
            ...
            "'Key': 'string', 'VersionId': 'string'***REMOVED***"
        )
        ...
        doc_section.write(
            ...
            "string form is {bucket***REMOVED***/{key***REMOVED*** or "
            "{bucket***REMOVED***/{key***REMOVED***?versionId={versionId***REMOVED*** if you want to copy a "
            ...
            "format is: {'Bucket': 'bucket', 'Key': 'key', 'VersionId': 'id'***REMOVED***."
            ...
            " (i.e. {accesspoint_arn***REMOVED***/object/{key***REMOVED***)"
        )
...
def _quote_source_header_from_dict(source_dict):
    try:
        ...
        if VALID_S3_ARN.search(bucket):
            final = f'{bucket***REMOVED***/object/{key***REMOVED***'
        else:
            final = f'{bucket***REMOVED***/{key***REMOVED***'
    except KeyError as e:
        raise ParamValidationError(
            report=f'Missing required parameter: {str(e)***REMOVED***'
        )
...
def _get_cross_region_presigned_url(
    ...
    request_dict_copy['headers'] = {***REMOVED***
...
class HeaderToHostHoister:
    ...
    _VALID_HOSTNAME = re.compile(r'(?!-)[a-z\d-]{1,63***REMOVED***(?<!-)$', re.IGNORECASE)
...
def add_retry_headers(request, **kwargs):
    ...
    sdk_request_headers = [
        f'{key***REMOVED***={retries_context[key]***REMOVED***'
...
def remove_bucket_from_url_paths_from_model(params, model, context, **kwargs):
    """Strips leading `{Bucket***REMOVED***/` from any operations that have it.
    ...
    if req_uri.startswith(bucket_path):
        ...
        model.http['authPath'] = f'{req_uri***REMOVED***/' if needs_slash else req_uri


def remove_accid_host_prefix_from_model(params, model, context, **kwargs):
    """Removes the `{AccountId***REMOVED***.` prefix from the operation model.
    ...
    When the ruleset based endpoint resolver is in effect, both the endpoint
    ruleset AND the service model place the {AccountId***REMOVED***. prefix in the URL.
    ...
    if (
        ...
        and model.endpoint.get('hostPrefix') == '{AccountId***REMOVED***.'
        and has_ctx_param
    ):
