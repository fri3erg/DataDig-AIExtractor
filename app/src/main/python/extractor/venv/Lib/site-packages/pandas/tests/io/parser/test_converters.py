...
"""
    if parser.engine == "pyarrow":
        msg = "The 'converters' option is not supported with the 'pyarrow' engine"
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(StringIO(data), converters={column: converter***REMOVED***)
        return

    result = parser.read_csv(StringIO(data), converters={column: converter***REMOVED***)
...
def test_converters_no_implicit_conv(all_parsers):
    ...
    converters = {0: lambda x: x.strip()***REMOVED***
    ...
    for day_converter in [convert_days, convert_days_sentinel]:
        if parser.engine == "pyarrow":
            ...
            with pytest.raises(ValueError, match=msg):
                parser.read_csv(
                    StringIO(data),
                    converters={"score": convert_score, "days": day_converter***REMOVED***,
                    na_values=["", None],
    ***REMOVED***
            continue
...
def test_converter_index_col_bug(all_parsers, conv_f):
    ...
    if parser.engine == "pyarrow":
        ...
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(
                StringIO(data), sep=";", index_col="A", converters={"A": conv_f***REMOVED***
***REMOVED***
        return
    ...
    xp = DataFrame({"B": [2, 4]***REMOVED***, index=Index(["1", "3"], name="A", dtype="object"))
...
def test_converter_identity_object(all_parsers):
    ...
    if parser.engine == "pyarrow":
        msg = "The 'converters' option is not supported with the 'pyarrow' engine"
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(StringIO(data), converters={"A": lambda x: x***REMOVED***)
        return

    rs = parser.read_csv(StringIO(data), converters={"A": lambda x: x***REMOVED***)

    xp = DataFrame({"A": ["1", "3"], "B": [2, 4]***REMOVED***)
    tm.assert_frame_equal(rs, xp)
