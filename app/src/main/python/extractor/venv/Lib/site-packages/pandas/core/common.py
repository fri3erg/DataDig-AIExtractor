...
def standardize_mapping(into):
    ...
    if not issubclass(into, abc.Mapping):
        raise TypeError(f"unsupported type: {into***REMOVED***")
    if into == defaultdict:
...
def pipe(
    ...
    **kwargs : dict, optional
    ...
    if isinstance(func, tuple):
        func, target = func
        if target in kwargs:
            msg = f"{target***REMOVED*** is both the pipe target and a keyword argument"
...
def fill_missing_names(names: Sequence[Hashable | None]) -> list[Hashable]:
    ...
    """
    return [f"level_{i***REMOVED***" if name is None else name for i, name in enumerate(names)]
