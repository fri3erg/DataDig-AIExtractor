...
class DependencyGraph:
    ...
    def __init__(
        ...
        self.nodes[0].update({"ctag": "TOP", "tag": "TOP", "address": 0***REMOVED***)
    ...
    def to_dot(self):
        ...
        ***REMOVED***
        ...
        for node in sorted(self.nodes.values(), key=lambda v: v["address"]):
            s += '\n{***REMOVED*** [label="{***REMOVED*** ({***REMOVED***)"]'.format(
                node["address"],
                node["address"],
                node["word"],
***REMOVED***
            for rel, deps in node["deps"].items():
                for dep in deps:
                    if rel is not None:
                        s += '\n{***REMOVED*** -> {***REMOVED*** [label="{***REMOVED***"]'.format(node["address"], dep, rel)
                    else:
                        s += "\n{***REMOVED*** -> {***REMOVED*** ".format(node["address"], dep)
        s += "\n***REMOVED***"
    ...
    def __repr__(self):
        return f"<DependencyGraph with {len(self.nodes)***REMOVED*** nodes>"
        ...
        for index, line in enumerate(lines, start=1):
            ...
            if cell_extractor is None:
                try:
                    ...
                    raise ValueError(
                        "Number of tab-delimited fields ({***REMOVED***) not supported by "
...
***REMOVED***
    ...
    def contains_cycle(self):
        ...
        >>> top = {'word': None, 'deps': [1], 'rel': 'TOP', 'address': 0***REMOVED***
        >>> child1 = {'word': None, 'deps': [2], 'rel': 'NTOP', 'address': 1***REMOVED***
        >>> child2 = {'word': None, 'deps': [4], 'rel': 'NTOP', 'address': 2***REMOVED***
        >>> child3 = {'word': None, 'deps': [1], 'rel': 'NTOP', 'address': 3***REMOVED***
        >>> child4 = {'word': None, 'deps': [3], 'rel': 'NTOP', 'address': 4***REMOVED***
        >>> cyclic_dg.nodes = {
        ...     0: top,
        ...     1: child1,
        ...     2: child2,
    ...
    def to_conll(self, style):
        ...
        if style == 3:
            template = "{word***REMOVED***\t{tag***REMOVED***\t{head***REMOVED***\n"
        elif style == 4:
            template = "{word***REMOVED***\t{tag***REMOVED***\t{head***REMOVED***\t{rel***REMOVED***\n"
        elif style == 10:
            template = (
                "{i***REMOVED***\t{word***REMOVED***\t{lemma***REMOVED***\t{ctag***REMOVED***\t{tag***REMOVED***\t{feats***REMOVED***\t{head***REMOVED***\t{rel***REMOVED***\t_\t_\n"
***REMOVED***
        else:
            raise ValueError(
                "Number of tab-delimited fields ({***REMOVED***) not supported by "
...
def dot2img(dot_string, t="svg"):
    ...
    ***REMOVED***
            ...
            raise Exception(
                "Cannot create image representation by running dot from string: {***REMOVED***"
                "".format(dot_string)
***REMOVED***
    except OSError as e:
...
def cycle_finding_demo():
    ...
    cyclic_dg = DependencyGraph()
    cyclic_dg.add_node({"word": None, "deps": [1], "rel": "TOP", "address": 0***REMOVED***)
    cyclic_dg.add_node({"word": None, "deps": [2], "rel": "NTOP", "address": 1***REMOVED***)
    cyclic_dg.add_node({"word": None, "deps": [4], "rel": "NTOP", "address": 2***REMOVED***)
    cyclic_dg.add_node({"word": None, "deps": [1], "rel": "NTOP", "address": 3***REMOVED***)
    cyclic_dg.add_node({"word": None, "deps": [3], "rel": "NTOP", "address": 4***REMOVED***)
