...
def test_at_timezone():
    ...
    result = DataFrame({"foo": [datetime(2000, 1, 1)]***REMOVED***)
    with tm.assert_produces_warning(FutureWarning, match="incompatible dtype"):
        result.at[0, "foo"] = datetime(2000, 1, 2, tzinfo=timezone.utc)
    expected = DataFrame(
        {"foo": [datetime(2000, 1, 2, tzinfo=timezone.utc)]***REMOVED***, dtype=object
    )
...
def test_selection_methods_of_assigned_col():
    # GH 29282
    df = DataFrame(data={"a": [1, 2, 3], "b": [4, 5, 6]***REMOVED***)
    df2 = DataFrame(data={"c": [7, 8, 9]***REMOVED***, index=[2, 1, 0])
    df["c"] = df2["c"]
    df.at[1, "c"] = 11
    result = df
    expected = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [9, 11, 7]***REMOVED***)
    tm.assert_frame_equal(result, expected)
    result = df.at[1, "c"]
    assert result == 11
    ...
    result = df[["c"]]
    expected = DataFrame({"c": [9, 11, 7]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
class TestAtErrors:
    ...
    def test_at_frame_raises_key_error(self, indexer_al):
        ...
        df = DataFrame({0: [1, 2, 3]***REMOVED***, index=[3, 2, 1])
    ...
    def test_at_frame_multiple_columns(self):
        ...
        with pytest.raises(
            InvalidIndexError,
            match=f"You can only assign a scalar value not a \\{type(new_row)***REMOVED***",
        ):
            df.at[5] = new_row
