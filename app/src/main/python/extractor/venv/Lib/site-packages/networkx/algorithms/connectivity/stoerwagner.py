...
def stoer_wagner(G, weight="weight", heap=BinaryHeap):
    ...
    Pairing heap   $O(2^{2 \sqrt{\log \log n***REMOVED******REMOVED*** nm + n^2 \log n)$
    ============== =============================================
    ...
    G = nx.Graph(
        (u, v, {"weight": e.get(weight, 1)***REMOVED***) for u, v, e in G.edges(data=True) if u != v
    )
    ...
    for i in range(n - 1):
        # Pick an arbitrary node u and create a set A = {u***REMOVED***.
