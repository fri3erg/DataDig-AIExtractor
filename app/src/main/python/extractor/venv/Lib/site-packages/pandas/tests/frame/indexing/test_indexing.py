...
class TestDataFrameIndexing:
    ...
    def test_setitem_list(self, float_frame):
        ...
        msg = (
            rf"Length of values \({len(newcolumndata)***REMOVED***\) "
            rf"does not match length of index \({len(data)***REMOVED***\)"
        )
        with pytest.raises(ValueError, match=msg):
    ...
    def test_getitem_boolean(self, mixed_float_frame, mixed_int_frame, datetime_frame):
        ...
        for df in [
            ...
            bif = df[df > 0]
            bifw = DataFrame(
                {c: np.where(data[c] > 0, data[c], np.nan) for c in data.columns***REMOVED***,
                index=data.index,
...
***REMOVED***
    ...
    def test_setattr_column(self):
        df = DataFrame({"foobar": 1***REMOVED***, index=range(10))
    ...
    def test_setitem_corner(self, float_frame, using_infer_string):
        # corner case
        df = DataFrame({"B": [1.0, 2.0, 3.0], "C": ["a", "b", "c"]***REMOVED***, index=np.arange(3))
        del df["B"]
        df["B"] = [1.0, 2.0, 3.0]
        assert "B" in df
    ...
    def test_loc_setitem_boolean_mask_allfalse(self):
        # GH 9596
        df = DataFrame(
            {"a": ["1", "2", "3"], "b": ["11", "22", "33"], "c": ["111", "222", "333"]***REMOVED***
        )
    ...
    def test_loc_duplicates(self):
        ...
        expected = DataFrame(
            {"A": [0, 0, 0, 0, 6, 6], "B": [0, 0, 0, 0, 0, 0]***REMOVED***, index=trange
        )
        tm.assert_frame_equal(df, expected)
    ...
    def test_setitem_with_unaligned_tz_aware_datetime_column(self):
        ...
        df = DataFrame({"dates": column***REMOVED***)
    ...
    def test_getitem_boolean_indexing_mixed(self):
        df = DataFrame(
            {
                0: {35: np.nan, 40: np.nan, 43: np.nan, 49: np.nan, 50: np.nan***REMOVED***,
                1: {
                    35: np.nan,
                ...
                ***REMOVED***,
                2: {
                    ...
                    50: np.nan,
                ***REMOVED***,
                3: {35: np.nan, 40: np.nan, 43: np.nan, 49: np.nan, 50: np.nan***REMOVED***,
                4: {
                    ...
                    50: np.nan,
                ***REMOVED***,
                "y": {35: 0, 40: 0, 43: 0, 49: 0, 50: 1***REMOVED***,
            ***REMOVED***
        )
    ...
    def test_setitem_array_as_cell_value(self):
        ...
        df.loc[0] = {"a": np.zeros((2,)), "b": np.zeros((2, 2))***REMOVED***
        expected = DataFrame({"a": [np.zeros((2,))], "b": [np.zeros((2, 2))]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_setting_mismatched_na_into_nullable_fails(
        ...
        df = DataFrame({"A": [1, 2, 3]***REMOVED***, dtype=any_numeric_ea_dtype)
        ser = df["A"].copy()
        arr = ser._values
        ...
        msg = "|".join(
            [
                ...
                r"Invalid value '.*' for dtype (U?Int|Float)\d{1,2***REMOVED***",
            ]
        )
        with pytest.raises(TypeError, match=msg):
            arr[0] = null
    ...
    def test_loc_expand_empty_frame_keep_index_name(self):
        ...
        df.loc[0] = 1
        expected = DataFrame({"b": [1]***REMOVED***, index=Index([0], name="a"))
        tm.assert_frame_equal(df, expected)
    ...
    def test_loc_setitem_rhs_frame(self, idxr, val):
        ...
        with tm.assert_produces_warning(
            ...
            df.loc[:, idxr] = DataFrame({"a": [val, 11]***REMOVED***, index=[1, 2])
        expected = DataFrame({"a": [np.nan, val]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_loc_internals_not_updated_correctly(self):
        ...
        df = DataFrame(
            {"bool_col": True, "a": 1, "b": 2.5***REMOVED***,
    ...
    def test_iloc_setitem_string_list_na(self, val):
        # GH#45469
        df = DataFrame({"a": ["a", "b", "c"]***REMOVED***, dtype="string")
        df.iloc[[0], :] = val
        expected = DataFrame({"a": [pd.NA, "b", "c"]***REMOVED***, dtype="string")
        tm.assert_frame_equal(df, expected)
    ...
    def test_iloc_setitem_ea_null_slice_length_one_list(self, func):
        ...
        df = DataFrame({"a": [1, 2, 3]***REMOVED***, dtype="Int64")
        df.iloc[:, func([0])] = 5
        expected = DataFrame({"a": [5, 5, 5]***REMOVED***, dtype="Int64")
        tm.assert_frame_equal(df, expected)
...
***REMOVED***
    ...
    @pytest.mark.parametrize("col", [{***REMOVED***, {"b": 1***REMOVED***])
    def test_set_2d_casting_date_to_int(self, col, indexer):
        # GH#49159
        df = DataFrame(
            {"a": [Timestamp("2022-12-29"), Timestamp("2022-12-30")], **col***REMOVED***,
        )
        df.loc[[1], indexer] = df["a"] + pd.Timedelta(days=1)
        expected = DataFrame(
            {"a": [Timestamp("2022-12-29"), Timestamp("2022-12-31")], **col***REMOVED***,
        )
        tm.assert_frame_equal(df, expected)

    @pytest.mark.parametrize("col", [{***REMOVED***, {"name": "a"***REMOVED***])
    def test_loc_setitem_reordering_with_all_true_indexer(self, col):
        # GH#48701
        n = 17
        df = DataFrame({**col, "x": range(n), "y": range(n)***REMOVED***)
        expected = df.copy()
    ...
    def test_loc_bool_multiindex(self, dtype, indexer):
        ...
        df = DataFrame({"c": [1, 2, 3, 4]***REMOVED***, index=midx)
        with tm.maybe_produces_warning(PerformanceWarning, isinstance(indexer, tuple)):
            result = df.loc[indexer]
        expected = DataFrame(
            {"c": [1, 2]***REMOVED***, index=Index([True, False], name="b", dtype=dtype)
        )
        tm.assert_frame_equal(result, expected)
...
class TestDataFrameIndexingUInt64:
    def test_setitem(self):
        df = DataFrame(
            {"A": np.arange(3), "B": [2**63, 2**63 + 5, 2**63 + 10]***REMOVED***,
            dtype=np.uint64,
        )
        idx = df["A"].rename("foo")
...
class TestLocILocDataFrameCategorical:
    ...
    def orig(self):
        ...
        orig = DataFrame({"cats": cats, "values": values***REMOVED***, index=idx)
        return orig
    ...
    def exp_single_row(self):
        ...
        exp_single_row = DataFrame({"cats": cats1, "values": values1***REMOVED***, index=idx1)
    ...
    def exp_parts_cats_col(self):
        ...
        exp_parts_cats_col = DataFrame({"cats": cats3, "values": values3***REMOVED***, index=idx3)
    ...
    def exp_single_cats_value(self):
        ...
        exp_single_cats_value = DataFrame(
            {"cats": cats4, "values": values4***REMOVED***, index=idx4
        )
    ...
    def test_loc_iloc_at_iat_setitem_single_value_in_categories(
        ...
        # "b" is among the categories for df["cat"***REMOVED***]
        indexer(df)[key] = "b"
    ...
    def test_getitem_preserve_object_index_with_dates(self, indexer):
        ...
        df = DataFrame({0: [1, 2, 3]***REMOVED***, index=idx)
...
class TestDeprecatedIndexers:
    @pytest.mark.parametrize(
        "key", [{1***REMOVED***, {1: 1***REMOVED***, ({1***REMOVED***, "a"), ({1: 1***REMOVED***, "a"), (1, {"a"***REMOVED***), (1, {"a": "a"***REMOVED***)]
    )
...
def test_adding_new_conditional_column() -> None:
    ...
    df = DataFrame({"x": [1]***REMOVED***)
    df.loc[df["x"] == 1, "y"] = "1"
    expected = DataFrame({"x": [1], "y": ["1"]***REMOVED***)
    tm.assert_frame_equal(df, expected)
    ...
    expected = DataFrame({"x": [1], "y": [value]***REMOVED***)
    tm.assert_frame_equal(df, expected)
...
def test_adding_new_conditional_column_with_string(dtype, infer_string) -> None:
    ...
    with pd.option_context("future.infer_string", infer_string):
        df.loc[df["a"] == 1, "c"] = "1"
    expected = DataFrame({"a": [1, 2], "b": [3, 4], "c": ["1", float("nan")]***REMOVED***).astype(
        {"a": "int64", "b": "int64", "c": dtype***REMOVED***
    )
    tm.assert_frame_equal(df, expected)
...
def test_add_new_column_infer_string():
    ...
    with pd.option_context("future.infer_string", True):
        df.loc[df["x"] == 1, "y"] = "1"
    expected = DataFrame(
        {"x": [1], "y": Series(["1"], dtype="string[pyarrow_numpy]")***REMOVED***,
...
class TestSetitemValidation:
    ...
    def test_setitem_validation_scalar_bool(self, invalid, indexer):
        df = DataFrame({"a": [True, False, False]***REMOVED***, dtype="bool")
        self._check_setitem_invalid(df, invalid, indexer, FutureWarning)

    @pytest.mark.parametrize("invalid", _invalid_scalars + [True, 1.5, np.float64(1.5)])
    @pytest.mark.parametrize("indexer", _indexers)
    def test_setitem_validation_scalar_int(self, invalid, any_int_numpy_dtype, indexer):
        df = DataFrame({"a": [1, 2, 3]***REMOVED***, dtype=any_int_numpy_dtype)
    ...
    def test_setitem_validation_scalar_float(self, invalid, float_numpy_dtype, indexer):
        df = DataFrame({"a": [1, 2, None]***REMOVED***, dtype=float_numpy_dtype)
        self._check_setitem_invalid(df, invalid, indexer, FutureWarning)
