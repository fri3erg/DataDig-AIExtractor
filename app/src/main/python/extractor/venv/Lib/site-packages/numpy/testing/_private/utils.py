...
IS_MUSL = False
# alternate way is
# from packaging.tags ***REMOVED***_tags
#     _tags = list(sys_tags())
...
if os.name == 'nt':
    ...
    def memusage(_proc_pid_stat=f'/proc/{os.getpid()***REMOVED***/stat'):
        """
...
if sys.platform[:5] == 'linux':
    def jiffies(_proc_pid_stat=f'/proc/{os.getpid()***REMOVED***/stat', _load_time=[]):
        """
    ...
    if verbose:
        for i, a in enumerate(arrays):
            ...
            try:
                ...
                r = f'[repr failed for <{type(a).__name__***REMOVED***>: {exc***REMOVED***]'
            if r.count('\n') > 3:
                r = '\n'.join(r.splitlines()[:3])
                r += '...'
            msg.append(f' {names[i]***REMOVED***: {r***REMOVED***')
    return '\n'.join(msg)
...
def assert_equal(actual, desired, err_msg='', verbose=True):
    ...
    if isinstance(desired, dict):
        ...
        for k, i in desired.items():
            ...
            assert_equal(actual[k], desired[k], f'key={k!r***REMOVED***\n{err_msg***REMOVED***',
                         verbose)
        return
    if isinstance(desired, (list, tuple)) and isinstance(actual, (list, tuple)):
        assert_equal(len(actual), len(desired), err_msg, verbose)
        for k in range(len(desired)):
            assert_equal(actual[k], desired[k], f'item={k!r***REMOVED***\n{err_msg***REMOVED***',
                         verbose)
        return
...
def assert_array_compare(comparison, x, y, err_msg='', verbose=True, header='',
    ...
    try:
        ...
        if not cond:
            if x.shape != y.shape:
                reason = f'\n(shapes {x.shape***REMOVED***, {y.shape***REMOVED*** mismatch)'
            else:
                reason = f'\n(dtypes {x.dtype***REMOVED***, {y.dtype***REMOVED*** mismatch)'
            msg = build_err_msg([x, y],
                                err_msg
                                + reason,
        ...
        if cond != True:
            ...
            remarks = [
                'Mismatched elements: {***REMOVED*** / {***REMOVED*** ({:.3g***REMOVED***%)'.format(
...
def measure(code_str, times=1, label=None):
    ...
    code = compile(code_str, f'Test name: {label***REMOVED*** ', 'exec')
    i = 0
    elapsed = jiffies()
    while i < times:
        i += 1
...
def assert_allclose(actual, desired, rtol=1e-7, atol=0, equal_nan=True,
    ...
    header = f'Not equal to tolerance rtol={rtol:g***REMOVED***, atol={atol:g***REMOVED***'
...
def integer_repr(x):
    ...
    if x.dtype == np.float16:
        ...
        raise ValueError(f'Unsupported dtype {x.dtype***REMOVED***')
...
def _assert_warns_context(warning_class, name=None):
    ...
    with suppress_warnings() as sup:
        ...
        yield
        if not len(l) > 0:
            name_str = f' when calling {name***REMOVED***' if name is not None else ''
...
def _assert_no_warnings_context(name=None):
    ...
    with warnings.catch_warnings(record=True) as l:
        ...
        if len(l) > 0:
            name_str = f' when calling {name***REMOVED***' if name is not None else ''
            raise AssertionError(f'Got warnings{name_str***REMOVED***: {l***REMOVED***')
...
class suppress_warnings:
    ...
    def __init__(self, forwarding_rule="always"):
        ...
        if forwarding_rule not in {"always", "module", "once", "location"***REMOVED***:
...
def _assert_no_gc_cycles_context(name=None):
    ...
    if n_objects_in_cycles:
        ...
        raise AssertionError(
            "Reference cycles were found{***REMOVED***: {***REMOVED*** objects were collected, "
            "of which {***REMOVED*** are shown below:{***REMOVED***"
            .format(
                name_str,
                n_objects_in_cycles,
                len(objects_in_cycles),
                ''.join(
                    "\n  {***REMOVED*** object with id={***REMOVED***:\n    {***REMOVED***".format(
                        type(o).__name__,
                        id(o),
                        pprint.pformat(o).replace('\n', '\n    ')
        ***REMOVED*** for o in objects_in_cycles
    ***REMOVED***
***REMOVED***
        )
...
def check_free_memory(free_bytes):
    ...
    if env_value is not None:
        try:
            ...
            raise ValueError(f'Invalid environment variable {env_var***REMOVED***: {exc***REMOVED***')

        msg = (f'{free_bytes/1e9***REMOVED*** GB memory required, but environment variable '
               f'NPY_AVAILABLE_MEM={env_value***REMOVED*** set')
    else:
        ...
        if mem_free is None:
            ...
            msg = f'{free_bytes/1e9***REMOVED*** GB memory required, but {mem_free/1e9***REMOVED*** GB available'
...
def _parse_size(size_str):
    ...
    suffixes = {'': 1, 'b': 1,
                ...
                'kib': 1024, 'mib': 1024**2, 'gib': 1024**3, 'tib': 1024**4***REMOVED***

    size_re = re.compile(r'^\s*(\d+|\d+\.\d+)\s*({0***REMOVED***)\s*$'.format(
        '|'.join(suffixes.keys())), re.I)

    m = size_re.match(size_str.lower())
    if not m or m.group(2) not in suffixes:
        raise ValueError(f'value {size_str!r***REMOVED*** not a valid size')
