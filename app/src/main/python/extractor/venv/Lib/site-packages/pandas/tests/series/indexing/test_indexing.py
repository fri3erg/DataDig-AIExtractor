...
@pytest.mark.parametrize(
    ...
    [
        [
            Series({1: 12, 2: [1, 2, 2, 3]***REMOVED***),
            Series({1: 313***REMOVED***),
            Series({1: 12***REMOVED***, dtype=object),
        ],
        [
            Series({1: [1, 2, 3], 2: [1, 2, 2, 3]***REMOVED***),
            Series({1: [1, 2, 3]***REMOVED***),
            Series({1: [1, 2, 3]***REMOVED***),
        ],
    ],
)
...
def test_underlying_data_conversion(using_copy_on_write):
    # GH 4080
    df = DataFrame({c: [1, 2, 3] for c in ["a", "b", "c"]***REMOVED***)
    ...
    if using_copy_on_write:
        ...
        with tm.assert_produces_warning(FutureWarning, match="inplace method"):
            df["val"].update(s)
        expected = DataFrame(
            {"a": [1, 2, 3], "b": [1, 2, 3], "c": [1, 2, 3], "val": [0, 1, 0]***REMOVED***
        )
...
def test_loc_setitem_nested_data_enlargement():
    ...
    ser = Series({"label": df***REMOVED***)
    ser.loc["new_label"] = df
    expected = Series({"label": df, "new_label": df***REMOVED***)
    tm.assert_series_equal(ser, expected)
...
class TestDeprecatedIndexers:
    @pytest.mark.parametrize("key", [{1***REMOVED***, {1: 1***REMOVED***])
    ...
    @pytest.mark.parametrize("key", [{1***REMOVED***, {1: 1***REMOVED***, ({1***REMOVED***, 2), ({1: 1***REMOVED***, 2)])
