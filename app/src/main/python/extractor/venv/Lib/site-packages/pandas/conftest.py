...
def ignore_doctest_warning(item: pytest.Item, path: str, message: str) -> None:
    ...
    if item.name.endswith(path):
        item.add_marker(pytest.mark.filterwarnings(f"ignore:{message***REMOVED***"))
...
# Common arguments
# ----------------------------------------------------------------
@pytest.fixture(params=[0, 1, "index", "columns"], ids=lambda x: f"axis={repr(x)***REMOVED***")
def axis(request):
    """
...
@pytest.fixture(params=[1, "columns"], ids=lambda x: f"axis={repr(x)***REMOVED***")
def axis_1(request):
    """
...
indices_dict = {
    "string": Index([f"pandas_{i***REMOVED***" for i in range(100)]),
    ...
    "string-python": Index(
        pd.array([f"pandas_{i***REMOVED***" for i in range(100)], dtype="string[python]")
    ),
***REMOVED***
if has_pyarrow:
    idx = Index(pd.array([f"pandas_{i***REMOVED***" for i in range(100)], dtype="string[pyarrow]"))
...
def string_series() -> Series:
    ...
    return Series(
        ...
        index=Index([f"i_{i***REMOVED***" for i in range(30)], dtype=object),
        name="series",
    )
...
def object_series() -> Series:
    ...
    """
    data = [f"foo_{i***REMOVED***" for i in range(30)]
    index = Index([f"bar_{i***REMOVED***" for i in range(30)], dtype=object)
...
_series = {
    f"series-with-{index_id***REMOVED***-index": _create_series(index)
...
_narrow_series = {
    f"{dtype.__name__***REMOVED***-series": Series(
        range(30), index=[f"i-{i***REMOVED***" for i in range(30)], name="a", dtype=dtype
    )
    for dtype in tm.NARROW_NP_DTYPES
***REMOVED***


_index_or_series_objs = {**indices_dict, **_series, **_narrow_series***REMOVED***
...
_typ_objects_series = {
    f"{dtype.__name__***REMOVED***-series": Series(dtype) for dtype in tm.PYTHON_DATA_TYPES
***REMOVED***
...
def float_frame() -> DataFrame:
    ...
    return DataFrame(
        ...
        index=Index([f"foo_{i***REMOVED***" for i in range(30)]),
    ...
    def deco(*args):
        ...
        if not os.path.exists(path):
            if strict_data_files:
                raise ValueError(
                    f"Could not find file {path***REMOVED*** and --no-strict-data-files is not set."
    ***REMOVED***
            pytest.skip(f"Could not find {path***REMOVED***.")
        return path
...
def tz_naive_fixture(request):
    """
    Fixture for trying timezones including default (None): {0***REMOVED***
    """
    return request.param
...
def tz_aware_fixture(request):
    """
    Fixture for trying explicit timezones: {0***REMOVED***
    """
    return request.param
