...
@nx._dispatch(
    graphs={"G": 0, "residual?": 4***REMOVED***,
    edge_attrs={"capacity": float("inf")***REMOVED***,
...
def boykov_kolmogorov_impl(G, s, t, capacity, residual, cutoff):
    if s not in G:
        raise nx.NetworkXError(f"node {str(s)***REMOVED*** not in graph")
    if t not in G:
        raise nx.NetworkXError(f"node {str(t)***REMOVED*** not in graph")
    if s == t:
    ...
    ***REMOVED***
    ...
    source_tree = {s: None***REMOVED***
    target_tree = {t: None***REMOVED***
    active = deque([s, t])
    orphans = deque()
    flow_value = 0
    # data structures for the marking heuristic
    time = 1
    timestamp = {s: time, t: time***REMOVED***
    dist = {s: 0, t: 0***REMOVED***
