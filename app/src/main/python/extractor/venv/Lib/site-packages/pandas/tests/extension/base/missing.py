...
class BaseMissingTests:
    ...
    def test_dropna_frame(self, data_missing):
        df = pd.DataFrame({"A": data_missing***REMOVED***, columns=pd.Index(["A"], dtype=object))
        ...
        # multiple
        df = pd.DataFrame({"A": data_missing, "B": [1, np.nan]***REMOVED***)
        result = df.dropna()
        expected = df.iloc[:0]
        tm.assert_frame_equal(result, expected)
...
***REMOVED***
    ...
    def test_fillna_frame(self, data_missing):
        ...
        result = pd.DataFrame({"A": data_missing, "B": [1, 2]***REMOVED***).fillna(fill_value)
    ...
    ***REMOVED***,
    ...
    def test_fillna_fill_other(self, data):
        result = pd.DataFrame({"A": data, "B": [np.nan] * len(data)***REMOVED***).fillna({"B": 0.0***REMOVED***)

        expected = pd.DataFrame({"A": data, "B": [0.0] * len(result)***REMOVED***)
