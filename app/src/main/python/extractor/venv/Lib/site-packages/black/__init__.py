...
def read_pyproject_toml(
    ...
    try:
        ...
        raise click.FileError(
            filename=value, hint=f"Error reading configuration file: {e***REMOVED***"
        ) from None
    ...
    default_map: Dict[str, Any] = {***REMOVED***
    if ctx.default_map:
...
def spellcheck_pyproject_toml_keys(
    ...
    available_config_options = {param.name for param in ctx.command.params***REMOVED***
    ...
    if invalid_keys:
        ...
        out(
            f"Invalid config keys detected: {keys_str***REMOVED*** (in {config_file_path***REMOVED***)",
            fg="red",
        )
...
def validate_regex(
    ...
    try:
        ...
        raise click.BadParameter(f"Not a valid regular expression: {e***REMOVED***") from None


@click.command(
    context_settings={"help_option_names": ["-h", "--help"]***REMOVED***,
...
@click.option(
    ...
    help=(
        ...
        f" of known python-magics ({', '.join(sorted(PYTHON_CELL_MAGICS))***REMOVED***)."
...
@click.option(
    ...
    help=(
        ...
        f" value will override all default exclusions. [default: {DEFAULT_EXCLUDES***REMOVED***]"
    ),
    show_default=False,
)
@click.option(
...
@click.version_option(
    version=__version__,
    message=(
        f"%(prog)s, %(version)s (compiled: {'yes' if COMPILED else 'no'***REMOVED***)\n"
        f"Python ({platform.python_implementation()***REMOVED***) {platform.python_version()***REMOVED***"
    ),
)
@click.argument(
    "src",
...
def main(  # noqa: C901
    ...
    if verbose:
        if root:
            out(
                f"Identified `{root***REMOVED***` as project root containing a {method***REMOVED***.",
                fg="blue",
***REMOVED***
...
***REMOVED***:
                ...
                out(f"Using configuration in '{config***REMOVED***'.", fg="blue")
            if ctx.default_map:
                for param, value in ctx.default_map.items():
                    out(f"{param***REMOVED***: {value***REMOVED***")
    ...
    if (
        ...
        err(
            f"{error_msg***REMOVED*** The required version `{required_version***REMOVED***` does not match"
            f" the running version `{__version__***REMOVED***`!"
        )
        ctx.exit(1)
    if ipynb and pyi:
...
***REMOVED***
            if normalized_path is None:
                if verbose:
                    out(f'Skipping invalid source: "{normalized_path***REMOVED***"', fg="red")
                continue

            if is_stdin:
                path = Path(f"{STDIN_PLACEHOLDER***REMOVED***{str(path)***REMOVED***")
...
***REMOVED***:
            ...
            if verbose:
                out(f'Found input source: "{normalized_path***REMOVED***"', fg="blue")
            sources.add(path)
        elif path.is_dir():
            path = root / (path.resolve().relative_to(root))
            if verbose:
                out(f'Found input source directory: "{path***REMOVED***"', fg="blue")
...
def format_file_in_place(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            f"File '{src***REMOVED***' cannot be parsed as valid Jupyter notebook."
        ) from None
    ...
    if write_back == WriteBack.YES:
        ...
        src_name = f"{src***REMOVED***\t{then***REMOVED***"
        dst_name = f"{src***REMOVED***\t{now***REMOVED***"
        if mode.is_ipynb:
    ...
    *,
    ...
    try:
        ...
        if write_back == WriteBack.YES:
            ...
            now = datetime.now(timezone.utc)
            src_name = f"STDIN\t{then***REMOVED***"
            dst_name = f"STDOUT\t{now***REMOVED***"
...
def validate_metadata(nb: MutableMapping[str, Any]) -> None:
    ...
    """
    language = nb.get("metadata", {***REMOVED***).get("language_info", {***REMOVED***).get("name", None)
...
def format_str(
    ...
    ...     mode=black.Mode(
    ...       target_versions={black.TargetVersion.PY36***REMOVED***,
    ...       line_length=10,
...
def _format_str_once(
    ...
    context_manager_features = {
        feature
        for feature in {Feature.PARENTHESIZED_CONTEXT_MANAGERS***REMOVED***
    ...
    ***REMOVED***
    ...
    split_line_features = {
        feature
        for feature in {Feature.TRAILING_COMMA_IN_CALL, Feature.TRAILING_COMMA_IN_DEF***REMOVED***
        if supports_feature(versions, feature)
    ***REMOVED***
...
def get_features_used(  # noqa: C901
    ...
    - f-strings;
    - self-documenting expressions in f-strings (f"{x=***REMOVED***");
    ...
    for n in node.pre_order():
        if is_string_token(n):
            ...
            if value_head in {'f"', 'F"', "f'", "F'", "rf", "fr", "RF", "FR"***REMOVED***:
...
***REMOVED***:
            ...
            n.type in {syms.typedargslist, syms.arglist***REMOVED***
            and n.children
            ...
            n.type in {syms.return_stmt, syms.yield_expr***REMOVED***
...
***REMOVED***:
        ...
        elif n.type in {syms.subscriptlist, syms.trailer***REMOVED*** and any(
...
def assert_equivalent(src: str, dst: str) -> None:
    ...
    try:
        ...
        raise AssertionError(
            f"INTERNAL ERROR: Black produced invalid code: {exc***REMOVED***. "
            "Please report a bug on https://github.com/psf/black/issues.  "
            f"This invalid output might be helpful: {log***REMOVED***"
        ) from None
    ...
    if src_ast_str != dst_ast_str:
        ...
        raise AssertionError(
            ...
            f"https://github.com/psf/black/issues.  This diff might be helpful: {log***REMOVED***"
        ) from None
...
def assert_stable(
    ...
    if dst != newdst:
        ...
        raise AssertionError(
            ...
            f"  This diff might be helpful: {log***REMOVED***"
        ) from None
