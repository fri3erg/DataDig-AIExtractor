...
def dijkstra_path_length(G, source, target, weight="weight"):
    ...
    """
    if source not in G:
        raise nx.NodeNotFound(f"Node {source***REMOVED*** not found in graph")
    if source == target:
        return 0
    ...
    try:
        ...
        raise nx.NetworkXNoPath(f"Node {target***REMOVED*** not reachable from {source***REMOVED***") from err
...
def single_source_dijkstra_path(G, source, cutoff=None, weight="weight"):
    ...
    """
    return multi_source_dijkstra_path(G, {source***REMOVED***, cutoff=cutoff, weight=weight)
...
def single_source_dijkstra_path_length(G, source, cutoff=None, weight="weight"):
    ...
    ...     print(f"{node***REMOVED***: {length[node]***REMOVED***")
    0: 0
    1: 1
    2: 2
    3: 3
    ...
    """
    return multi_source_dijkstra_path_length(G, {source***REMOVED***, cutoff=cutoff, weight=weight)
...
def single_source_dijkstra(G, source, target=None, cutoff=None, weight="weight"):
    ...
    return multi_source_dijkstra(
        G, {source***REMOVED***, cutoff=cutoff, target=target, weight=weight
    )
...
def multi_source_dijkstra_path(G, sources, cutoff=None, weight="weight"):
    ...
    --------
    >>> G = nx.path_graph(5)
    >>> path = nx.multi_source_dijkstra_path(G, {0, 4***REMOVED***)
    >>> path[1]
    [0, 1]
    >>> path[3]
    [4, 3]
...
def multi_source_dijkstra_path_length(G, sources, cutoff=None, weight="weight"):
    ...
    --------
    >>> G = nx.path_graph(5)
    >>> length = nx.multi_source_dijkstra_path_length(G, {0, 4***REMOVED***)
    >>> for node in [0, 1, 2, 3, 4]:
    ...     print(f"{node***REMOVED***: {length[node]***REMOVED***")
    0: 0
    1: 1
    ...
    for s in sources:
        if s not in G:
            raise nx.NodeNotFound(f"Node {s***REMOVED*** not found in graph")
    weight = _weight_function(G, weight)
...
def multi_source_dijkstra(G, sources, target=None, cutoff=None, weight="weight"):
    ...
    --------
    >>> G = nx.path_graph(5)
    >>> length, path = nx.multi_source_dijkstra(G, {0, 4***REMOVED***)
    >>> for node in [0, 1, 2, 3, 4]:
    ...     print(f"{node***REMOVED***: {length[node]***REMOVED***")
    0: 0
    1: 1
    ...
    >>> length, path = nx.multi_source_dijkstra(G, {0, 4***REMOVED***, 1)
    >>> length
    1
    >>> path
    [0, 1]
    ...
    if target in sources:
        return (0, [target])
    weight = _weight_function(G, weight)
    paths = {source: [source] for source in sources***REMOVED***  # dictionary of paths
    ...
    try:
        ...
        raise nx.NetworkXNoPath(f"No path to {target***REMOVED***.") from err
...
def _dijkstra_multisource(
    ...
    push = heappush
    pop = heappop
    dist = {***REMOVED***  # dictionary of final distances
    seen = {***REMOVED***
...
def dijkstra_predecessor_and_distance(G, source, cutoff=None, weight="weight"):
    ...
    """
    if source not in G:
        raise nx.NodeNotFound(f"Node {source***REMOVED*** is not found in the graph")
    weight = _weight_function(G, weight)
    pred = {source: []***REMOVED***  # dictionary of predecessors
...
def all_pairs_dijkstra(G, cutoff=None, weight="weight"):
    ...
    2
    >>> for node in [0, 1, 2, 3, 4]:
    ...     print(f"3 - {node***REMOVED***: {len_path[3][0][node]***REMOVED***")
    3 - 0: 3
    3 - 1: 2
    3 - 2: 1
    3 - 3: 0
...
def all_pairs_dijkstra_path_length(G, cutoff=None, weight="weight"):
    ...
    >>> for node in [0, 1, 2, 3, 4]:
    ...     print(f"1 - {node***REMOVED***: {length[1][node]***REMOVED***")
    1 - 0: 1
    1 - 1: 0
    1 - 2: 1
    1 - 3: 2
...
def _bellman_ford(
    ...
    """
    if pred is None:
        pred = {v: [] for v in source***REMOVED***

    if dist is None:
        dist = {v: 0 for v in source***REMOVED***
...
def _inner_bellman_ford(
    ...
    for s in sources:
        if s not in G:
            raise nx.NodeNotFound(f"Source {s***REMOVED*** not in G")

    if pred is None:
        pred = {v: [] for v in sources***REMOVED***

    if dist is None:
        dist = {v: 0 for v in sources***REMOVED***

    # Heuristic Storage setup. Note: use None because nodes cannot be None
    nonexistent_edge = (None, None)
    pred_edge = {v: None for v in sources***REMOVED***
    recent_update = {v: nonexistent_edge for v in sources***REMOVED***
...
def single_source_bellman_ford(G, source, target=None, weight="weight"):
    ...
    paths = {source: [source]***REMOVED***  # dictionary of paths
    ...
    try:
        ...
        msg = f"Node {target***REMOVED*** not reachable from {source***REMOVED***"
...
def goldberg_radzik(G, source, weight="weight"):
    ...
    if len(G) == 1:
        return {source: None***REMOVED***, {source: 0***REMOVED***

    G_succ = G._adj  # For speed-up (and works for both directed and undirected graphs)

    inf = float("inf")
    d = {u: inf for u in G***REMOVED***
    d[source] = 0
    pred = {source: None***REMOVED***
    ...
    relabeled = {source***REMOVED***

    while relabeled:
        to_scan = topo_sort(relabeled)
        relabeled = relax(to_scan)

    d = {u: d[u] for u in pred***REMOVED***
    return pred, d
...
def bidirectional_dijkstra(G, source, target, weight="weight"):
    ...
    if source not in G or target not in G:
        msg = f"Either source {source***REMOVED*** or target {target***REMOVED*** is not in G"
    ...
    dists = [{***REMOVED***, {***REMOVED***]  # dictionary of final distances
    paths = [{source: [source]***REMOVED***, {target: [target]***REMOVED***]  # dictionary of paths
    fringe = [[], []]  # heap of (distance, node) for choosing node to expand
    seen = [{source: 0***REMOVED***, {target: 0***REMOVED***]  # dict of distances to seen nodes
    c = count()
    ...
    raise nx.NetworkXNoPath(f"No path between {source***REMOVED*** and {target***REMOVED***.")
...
def johnson(G, weight="weight"):
    ...
    """
    dist = {v: 0 for v in G***REMOVED***
    pred = {v: [] for v in G***REMOVED***
    weight = _weight_function(G, weight)
    ...
    return {v: dist_path(v) for v in G***REMOVED***
