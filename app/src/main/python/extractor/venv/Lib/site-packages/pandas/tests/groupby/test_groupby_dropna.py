...
@pytest.mark.parametrize(
    ...
    [
        (
            True,
            [["A", "B"], ["B", "A"]],
            {"c": [13.0, 123.23], "d": [13.0, 123.0], "e": [13.0, 1.0]***REMOVED***,
        ),
        (
            False,
            [["A", "B"], ["A", np.nan], ["B", "A"]],
...
@pytest.mark.parametrize(
    ...
    [
        (
            True,
            [["A", "B"], ["B", "A"]],
            {"c": [12.0, 123.23], "d": [12.0, 123.0], "e": [12.0, 1.0]***REMOVED***,
        ),
        (
            False,
...
@pytest.mark.parametrize(
    "dropna, idx, outputs",
    [
        (True, ["A", "B"], {"b": [123.23, 13.0], "c": [123.0, 13.0], "d": [1.0, 13.0]***REMOVED***),
        (
            False,
            ["A", "B", np.nan],
            {
...
def test_grouper_dropna_propagation(dropna):
    # GH 36604
    df = pd.DataFrame({"A": [0, 0, 1, None], "B": [1, 2, 3, None]***REMOVED***)
...
def test_groupby_dataframe_slice_then_transform(dropna, index):
    # GH35014 & GH35612
    expected_data = {"B": [2, 2, 1, np.nan if dropna else 1]***REMOVED***

    df = pd.DataFrame({"A": [0, 0, 1, None], "B": [1, 2, 3, None]***REMOVED***, index=index)
    gb = df.groupby("A", dropna=dropna)
...
@pytest.mark.parametrize(
    ...
    [
        (
            True,
            [["A", "B"], ["B", "A"]],
            {"c": [13.0, 123.23], "d": [12.0, 123.0], "e": [1.0, 1.0]***REMOVED***,
        ),
        (
            False,
            [["A", "B"], ["A", np.nan], ["B", "A"]],
...
def test_groupby_dropna_multi_index_dataframe_agg(dropna, tuples, outputs):
    ...
    df = pd.DataFrame(df_list, columns=["a", "b", "c", "d", "e"])
    agg_dict = {"c": "sum", "d": "max", "e": "min"***REMOVED***
...
def test_groupby_dropna_datetime_like_data(
    ...
    grouped = df.groupby("dt", dropna=dropna).agg({"values": "sum"***REMOVED***)
    expected = pd.DataFrame({"values": values***REMOVED***, index=pd.Index(indexes, name="dt"))
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            False,
            {"groups": ["a", "a", "b", np.nan], "values": [10, 10, 20, 30]***REMOVED***,
            {"values": [0, 1, 0, 0]***REMOVED***,
            ["a", "b", np.nan],
        ...
        pytest.param(
            ...
            False,
            {"groups": ["a", "a", "b", "c"], "values": [10, 10, 20, 30]***REMOVED***,
            {"values": [0, 1, 0, 0]***REMOVED***,
            None,
...
def test_groupby_apply_with_dropna_for_multi_index(dropna, data, selected_data, levels):
    ...
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
        result = gb.apply(lambda grp: pd.DataFrame({"values": range(len(grp))***REMOVED***))
...
def test_no_sort_keep_na(sequence_index, dtype, test_series, as_index):
    ...
    sequence = "".join(
        [{0: "x", 1: "y", 2: "z"***REMOVED***[sequence_index // (3**k) % 3] for k in range(4)]
    )

    # Unique values to use for grouper, depends on dtype
    if dtype in ("string", "string[pyarrow]"):
        uniques = {"x": "x", "y": "y", "z": pd.NA***REMOVED***
    elif dtype in ("datetime64[ns]", "period[d]"):
        uniques = {"x": "2016-01-01", "y": "2017-01-01", "z": pd.NA***REMOVED***
    else:
        uniques = {"x": 1, "y": 2, "z": np.nan***REMOVED***
...
def test_null_is_null_for_dtype(
    ...
    expected = pd.DataFrame({"a": [3]***REMOVED***, index=index)
    if test_series:
...
def test_categorical_reducers(reduction_func, observed, sort, as_index, index_kind):
    ...
    df = pd.DataFrame(
        {"x": pd.Categorical(values, categories=[1, 2, 3]), "y": range(20)***REMOVED***
    )
    ...
    if reduction_func == "size":
        # size, unlike other methods, has the desired behavior in GH#49519
        expected = expected.rename(columns={0: "size"***REMOVED***)
        if as_index:
...
def test_categorical_transformers(
    ...
    null_group_result = pd.DataFrame({"y": null_group_data***REMOVED***)
...
def test_categorical_head_tail(method, observed, sort, as_index):
    ...
    df = pd.DataFrame(
        {"x": pd.Categorical(values, categories=[1, 2, 3]), "y": range(len(values))***REMOVED***
    )
