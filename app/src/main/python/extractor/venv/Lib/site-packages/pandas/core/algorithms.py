...
def _ensure_arraylike(values, func_name: str) -> ArrayLike:
    ...
    if not isinstance(values, (ABCIndex, ABCSeries, ABCExtensionArray, np.ndarray)):
        ...
        if func_name != "isin-targets":
            ...
            warnings.warn(
                f"{func_name***REMOVED*** with argument that is not not a Series, Index, "
...
def isin(comps: ListLike, values: ListLike) -> npt.NDArray[np.bool_]:
    ...
    if not is_list_like(comps):
        raise TypeError(
            ...
            f"to isin(), you passed a `{type(comps).__name__***REMOVED***`"
        )
    if not is_list_like(values):
        raise TypeError(
            "only list-like objects are allowed to be passed "
            f"to isin(), you passed a `{type(values).__name__***REMOVED***`"
        )
...
def mode(
    ...
    try:
        ...
        warnings.warn(
            f"Unable to sort modes: {err***REMOVED***",
            stacklevel=find_stack_level(),
        )
...
def rank(
    ...
    na_option : {'keep', 'top'***REMOVED***, default 'keep'
...
_diff_special = {"float64", "float32", "int64", "int32", "int16", "int8"***REMOVED***
...
def diff(arr, n: int, axis: AxisInt = 0):
    ...
    if not isinstance(arr, np.ndarray):
        # i.e ExtensionArray
        if hasattr(arr, f"__{op.__name__***REMOVED***__"):
            if axis != 0:
                raise ValueError(f"cannot diff {type(arr).__name__***REMOVED*** on axis={axis***REMOVED***")
            return op(arr, arr.shift(n))
        else:
            raise TypeError(
                f"{type(arr).__name__***REMOVED*** has no 'diff' method. "
...
def map_array(
    ...
    if na_action not in (None, "ignore"):
        msg = f"na_action must either be 'ignore' or None, {na_action***REMOVED*** was passed"
