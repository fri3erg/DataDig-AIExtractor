...
def test_frame():
    return DataFrame(
        {"A": [1] * 20 + [2] * 12 + [3] * 8, "B": np.arange(40)***REMOVED***,
...
def test_getitem_multiple():
    ...
    data = [{"id": 1, "buyer": "A"***REMOVED***, {"id": 2, "buyer": "B"***REMOVED***]
...
def test_groupby_resample_on_api_with_getitem():
    # GH 17813
    df = DataFrame(
        {"id": list("aabbb"), "date": date_range("1-1-2016", periods=5), "data": 1***REMOVED***
    )
...
def test_apply_with_mutated_index():
    ...
    df = DataFrame(
        data={"col1": np.random.default_rng(2).random(len(index))***REMOVED***, index=index
    )
...
def test_apply_columns_multilevel():
    ...
    agg_dict = {col: (np.sum if col[3] == "one" else np.mean) for col in df.columns***REMOVED***
...
def test_resample_groupby_with_label(unit):
    ...
    df = DataFrame(index=index, data={"col0": [0, 0, 1, 1, 2], "col1": [1, 1, 1, 1, 1]***REMOVED***)
    msg = "DataFrameGroupBy.resample operated on the grouping columns"
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
    ...
    expected = DataFrame(
        data={"col0": [0, 0, 2, 2], "col1": [1, 1, 2, 1]***REMOVED***, index=mindex
    )
...
def test_apply_to_one_column_of_df():
    # GH: 36951
    df = DataFrame(
        {"col": range(10), "col1": range(10, 20)***REMOVED***,
...
def test_resample_groupby_agg():
    ...
    result = resampled.agg({"num": "sum"***REMOVED***)
...
def test_resample_groupby_agg_listlike():
    ...
    df = DataFrame({"class": ["beta"], "value": [69]***REMOVED***, index=Index([ts], name="date"))
...
def test_resample_groupby_agg_object_dtype_all_nan(consolidate):
    ...
    df1 = DataFrame({"key": "A", "date": dates, "col1": range(15), "col_object": "val"***REMOVED***)
    df2 = DataFrame({"key": "B", "date": dates, "col1": range(15)***REMOVED***)
...
def test_groupby_resample_size_all_index_same():
    # GH 46826
    df = DataFrame(
        {"A": [1] * 3 + [2] * 3 + [1] * 3 + [2] * 3, "B": np.arange(12)***REMOVED***,
