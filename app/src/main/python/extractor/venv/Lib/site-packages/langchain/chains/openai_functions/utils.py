...
def _resolve_schema_references(schema: Any, definitions: Dict[str, Any]) -> Any:
    ...
    if isinstance(schema, list):
        ...
        if "$ref" in schema:
            ...
            ref = definitions.get(ref_key, {***REMOVED***)
            schema.update(ref)
        else:
...
def _convert_schema(schema: dict) -> dict:
    props = {k: {"title": k, **v***REMOVED*** for k, v in schema["properties"].items()***REMOVED***
    return {
        "type": "object",
        "properties": props,
...
def get_llm_kwargs(function: dict) -> dict:
    ...
    """
    return {"functions": [function], "function_call": {"name": function["name"]***REMOVED******REMOVED***
