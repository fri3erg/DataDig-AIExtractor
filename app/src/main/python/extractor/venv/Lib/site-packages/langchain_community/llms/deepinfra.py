...
class DeepInfra(LLM):
    ...
    def _identifying_params(self) -> Mapping[str, Any]:
        ...
        return {
            **{"model_id": self.model_id***REMOVED***,
    ...
    def _url(self) -> str:
        return f"https://api.deepinfra.com/v1/inference/{self.model_id***REMOVED***"

    def _headers(self) -> Dict:
        return {
            "Authorization": f"bearer {self.deepinfra_api_token***REMOVED***",
            "Content-Type": "application/json",
        ***REMOVED***

    def _body(self, prompt: str, kwargs: Any) -> Dict:
        model_kwargs = self.model_kwargs or {***REMOVED***
        model_kwargs = {**model_kwargs, **kwargs***REMOVED***
    ...
    def _handle_status(self, code: int, text: Any) -> None:
        if code >= 500:
            raise Exception(f"DeepInfra Server: Error {code***REMOVED***")
        elif code >= 400:
            raise ValueError(f"DeepInfra received an invalid payload: {text***REMOVED***")
        elif code != 200:
            raise Exception(
                f"DeepInfra returned an unexpected response with status "
                f"{code***REMOVED***: {text***REMOVED***"
***REMOVED***
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        response = request.post(
            url=self._url(), data=self._body(prompt, {**kwargs, "stream": True***REMOVED***)
        )
...
def _handle_sse_line(line: str) -> Optional[GenerationChunk]:
    try:
        ...
        return GenerationChunk(
            text=obj.get("token", {***REMOVED***).get("text"),
        )
    except Exception:
        return None
