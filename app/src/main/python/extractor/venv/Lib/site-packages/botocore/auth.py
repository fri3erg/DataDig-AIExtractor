...
class SigV2Auth(BaseSigner):
    ...
    def calc_signature(self, request, params):
        ...
        if len(path) == 0:
            path = '/'
        string_to_sign = f"{request.method***REMOVED***\n{split.netloc***REMOVED***\n{path***REMOVED***\n"
        lhmac = hmac.new(
        ...
        for key in sorted(params):
            ...
            pairs.append(f'{quoted_key***REMOVED***={quoted_value***REMOVED***')
        qs = '&'.join(pairs)
        string_to_sign += qs
...
class SigV3Auth(BaseSigner):
    ...
    def add_auth(self, request):
        ...
        signature = (
            f"AWS3-HTTPS AWSAccessKeyId={self.credentials.access_key***REMOVED***,"
            f"Algorithm=HmacSHA256,Signature={encoded_signature.decode('utf-8')***REMOVED***"
        )
...
***REMOVED***
        ...
        for key, value in sorted(key_val_pairs):
            sorted_key_vals.append(f'{key***REMOVED***={value***REMOVED***')
...
***REMOVED***
            headers.append(f'{key***REMOVED***:{ensure_unicode(value)***REMOVED***')
        return '\n'.join(headers)
    ...
    def _is_streaming_checksum_payload(self, request):
        checksum_context = request.context.get('checksum', {***REMOVED***)
...
***REMOVED***
    ...
    def signature(self, string_to_sign, request):
        ...
        k_date = self._sign(
            (f"AWS4{key***REMOVED***").encode(), request.context["timestamp"][0:8]
        )
    ...
    def _inject_signature_to_request(self, request, signature):
        ...
        headers_to_sign = self.headers_to_sign(request)
        auth_str.append(
            f"SignedHeaders={self.signed_headers(headers_to_sign)***REMOVED***"
        )
...
class S3ExpressPostAuth(S3ExpressAuth):
    ...
    def add_auth(self, request):
        ...
        conditions.append({'x-amz-algorithm': 'AWS4-HMAC-SHA256'***REMOVED***)
        conditions.append({'x-amz-credential': self.scope(request)***REMOVED***)
        conditions.append({'x-amz-date': request.context['timestamp']***REMOVED***)

        if self.credentials.token is not None:
            fields['X-Amz-S3session-Token'] = self.credentials.token
            conditions.append(
                {'X-Amz-S3session-Token': self.credentials.token***REMOVED***
***REMOVED***
...
class S3ExpressQueryAuth(S3ExpressAuth):
    ...
    def _modify_request_before_signing(self, request):
        ...
        ***REMOVED***
        ...
        query_dict = {k: v[0] for k, v in query_string_parts.items()***REMOVED***

        if request.params:
            query_dict.update(request.params)
            request.params = {***REMOVED***
        ...
        new_query_string = (
            f"{operation_params***REMOVED***{percent_encode_sequence(auth_params)***REMOVED***"
        )
...
class S3SigV4PostAuth(SigV4Auth):
    ...
    def add_auth(self, request):
        ...
        if self.credentials.token is not None:
            ...
            conditions.append({'x-amz-security-token': self.credentials.token***REMOVED***)
...
class HmacV1Auth(BaseSigner):
    ...
    def canonical_custom_headers(self, headers):
        ...
        ***REMOVED***
        ...
        for key in sorted_header_keys:
            hoi.append(f"{key***REMOVED***:{custom_headers[key]***REMOVED***")
        return '\n'.join(hoi)
    ...
    def _inject_signature(self, request, signature):
        ...
        auth_header = f"AWS {self.credentials.access_key***REMOVED***:{signature***REMOVED***"
...
***REMOVED***:
        ...
        if p[3]:
            ...
            new_query_string = f'{p[3]***REMOVED***&{new_query_string***REMOVED***'
        new_url_parts = (p[0], p[1], p[2], new_query_string, p[4])
        request.url = urlunsplit(new_url_parts)
...
class BearerAuth(TokenSigner):
    ...
    def add_auth(self, request):
        ...
        auth_header = f'Bearer {self.auth_token.token***REMOVED***'
        if 'Authorization' in request.headers:
            del request.headers['Authorization']
        request.headers['Authorization'] = auth_header
