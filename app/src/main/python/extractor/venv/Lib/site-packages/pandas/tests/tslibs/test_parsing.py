...
def test_parse_time_quarter_with_dash_error(dashed):
    msg = f"Unknown datetime string format, unable to parse: {dashed***REMOVED***"
...
def test_parsers_quarter_invalid(date_str):
    if date_str == "6Q-20":
        msg = (
            ...
            f"must be between 1 and 4: {date_str***REMOVED***"
        )
    else:
        msg = f"Unknown datetime string format, unable to parse: {date_str***REMOVED***"
...
def test_guess_datetime_format_no_padding(string, fmt, dayfirst, warning):
    # see gh-11142
    msg = (
        rf"Parsing dates in {fmt***REMOVED*** format when dayfirst=False \(the default\) "
        "was specified. "
