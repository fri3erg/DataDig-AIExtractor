...
def _load_stuff_documents_chain(config: dict, **kwargs: Any) -> StuffDocumentsChain:
    ...
    if not isinstance(llm_chain, LLMChain):
        raise ValueError(f"Expected LLMChain, got {llm_chain***REMOVED***")
...
def load_chain_from_config(config: dict, **kwargs: Any) -> Chain:
    ...
    if config_type not in type_to_loader_dict:
        raise ValueError(f"Loading {config_type***REMOVED*** chain not supported")
...
def load_chain(path: Union[str, Path], **kwargs: Any) -> Chain:
    ...
    if hub_result := try_load_from_hub(
        path, _load_chain_from_file, "chains", {"json", "yaml"***REMOVED***, **kwargs
    ):
        return hub_result
    else:
