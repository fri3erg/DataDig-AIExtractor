...
class TestIndexConcat:
    def test_concat_ignore_index(self, sort):
        frame1 = DataFrame(
            {"test1": ["a", "b", "c"], "test2": [1, 2, 3], "test3": [4.5, 3.2, 1.2]***REMOVED***
        )
        frame2 = DataFrame({"test3": [5.2, 2.2, 4.3]***REMOVED***)
    ...
    def test_concat_same_index_names(self, name_in1, name_in2, name_in3, name_out):
        ...
        frames = [
            DataFrame({c: [0, 1, 2]***REMOVED***, index=i) for i, c in zip(indices, ["x", "y", "z"])
        ]
    ...
    def test_default_index(self):
        ...
        df1 = DataFrame({"A": [1, 2], "B": [5, 6]***REMOVED***)
        df2 = DataFrame({"A": [3, 4], "B": [7, 8]***REMOVED***)
...
class TestMultiIndexConcat:
    ...
    def test_concat_multiindex_with_none_in_index_names(self):
        ...
        df = DataFrame({"col": range(5)***REMOVED***, index=index, dtype=np.int32)

        result = concat([df, df], keys=[1, 2], names=["level2"])
        index = MultiIndex.from_product(
            [[1, 2], [1], range(5)], names=["level2", "level1", None]
        )
        expected = DataFrame({"col": list(range(5)) * 2***REMOVED***, index=index, dtype=np.int32)
        tm.assert_frame_equal(result, expected)
        ...
        expected = DataFrame({"col": no_name***REMOVED***, index=index, dtype=np.int32)
        tm.assert_frame_equal(result, expected)
    ...
    def test_concat_multiindex_dfs_with_deepcopy(self):
        ...
        example_dict = {"s1": example_dataframe1, "s2": example_dataframe2***REMOVED***
...
***REMOVED***
    ...
    def test_concat_multiindex_(self):
        # GitHub #44786
        df = DataFrame({"col": ["a", "b", "c"]***REMOVED***, index=["1", "2", "2"])
        df = concat([df], keys=["X"])
        ...
        expected_df = DataFrame(
            {"col": ["a", "b", "c"]***REMOVED***, index=MultiIndex.from_product(iterables)
        )
        tm.assert_frame_equal(result_df, expected_df)

    def test_concat_with_key_not_unique(self):
        # GitHub #46519
        df1 = DataFrame({"name": [1]***REMOVED***)
        df2 = DataFrame({"name": [2]***REMOVED***)
        df3 = DataFrame({"name": [3]***REMOVED***)
        ...
        df_b = DataFrame(
            {"name": [1, 2, 3]***REMOVED***, index=Index([("x", 0), ("y", 0), ("x", 0)])
        )
        with tm.assert_produces_warning(
            PerformanceWarning, match="indexing past lexsort depth"
        ):
        ...
        df1 = DataFrame({"name": ["a", "a", "b"]***REMOVED***)
        df2 = DataFrame({"name": ["a", "b"]***REMOVED***)
        df3 = DataFrame({"name": ["c", "d"]***REMOVED***)
        df_a = concat([df1, df2, df3], keys=["x", "y", "x"])
        with tm.assert_produces_warning(
            PerformanceWarning, match="indexing past lexsort depth"
        ):
    ...
    def test_concat_with_duplicated_levels(self):
        ...
        df1 = DataFrame({"A": [1]***REMOVED***, index=["x"])
        df2 = DataFrame({"A": [1]***REMOVED***, index=["y"])
    ...
    def test_concat_range_index_result(self):
        # GH#47501
        df1 = DataFrame({"a": [1, 2]***REMOVED***)
        df2 = DataFrame({"b": [1, 2]***REMOVED***)

        result = concat([df1, df2], sort=True, axis=1)
        expected = DataFrame({"a": [1, 2], "b": [1, 2]***REMOVED***)
        tm.assert_frame_equal(result, expected)
