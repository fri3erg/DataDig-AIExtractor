...
def demo_generated_templates():
    ...
    print(
        "Generated {***REMOVED*** templates for transformation-based learning".format(
            len(templates)
        )
    )
...
def postag(
    ...
    :type cache_baseline_tagger: C{string***REMOVED***
...
***REMOVED***
            ...
            print(
                "Trained baseline tagger, pickled it to {***REMOVED***".format(
                    cache_baseline_tagger
    ***REMOVED***
***REMOVED***
        with open(cache_baseline_tagger) as print_rules:
            baseline_tagger = pickle.load(print_rules)
            print(f"Reloaded pickled tagger from {cache_baseline_tagger***REMOVED***")
    else:
        baseline_tagger = UnigramTagger(baseline_data, backoff=baseline_backoff_tagger)
        print("Trained baseline tagger")
    if gold_data:
        print(
            "    Accuracy on test set: {:0.4f***REMOVED***".format(
...
***REMOVED***
    ...
    print(f"Trained tbl tagger in {time.time() - tbrill:0.2f***REMOVED*** seconds")
    if gold_data:
    ...
    if trace == 1:
        ...
        for (ruleno, rule) in enumerate(brill_tagger.rules(), 1):
            print(f"{ruleno:4d***REMOVED*** {rule.format(ruleformat):s***REMOVED***")
...
***REMOVED***
            print(f"Wrote plot of learning curve to {learning_curve_output***REMOVED***")
    else:
    ...
    if error_output is not None:
        ...
        print(f"Wrote tagger errors including context to {error_output***REMOVED***")
    ...
    if serialize_output is not None:
        ...
        print(f"Wrote pickled tagger to {serialize_output***REMOVED***")
        with open(serialize_output) as print_rules:
            brill_tagger_reloaded = pickle.load(print_rules)
        print(f"Reloaded pickled tagger from {serialize_output***REMOVED***")
...
def _demo_prepare_data(
    ...
    print(f"Read testing data ({testseqs:d***REMOVED*** sents/{testtokens:d***REMOVED*** wds)")
    print(f"Read training data ({trainseqs:d***REMOVED*** sents/{traintokens:d***REMOVED*** wds)")
    print(
        "Read baseline data ({:d***REMOVED*** sents/{:d***REMOVED*** wds) {:s***REMOVED***".format(
            bltrainseqs,
            bltraintokens,
