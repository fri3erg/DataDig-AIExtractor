...
def test_iterator_vs_iterable():
    G = nx.empty_graph("a")
    assert list(nx.community.label_propagation_communities(G)) == [{"a"***REMOVED***]
    for community in nx.community.label_propagation_communities(G):
        assert community == {"a"***REMOVED***
...
def test_one_node():
    ...
    ground_truth = {frozenset(["a"])***REMOVED***

    communities = nx.community.label_propagation_communities(test)
    result = {frozenset(c) for c in communities***REMOVED***
...
def test_unconnected_communities():
    ...
    # The expected communities are:
    ground_truth = {frozenset(["a", "c", "d"]), frozenset(["b", "e", "f"])***REMOVED***
...
class TestAsynLpaCommunities:
    ...
    def test_several_communities(self):
        ...
        ground_truth = {frozenset(range(3 * i, 3 * (i + 1))) for i in range(5)***REMOVED***
...
class TestFastLabelPropagationCommunities:
    ...
    def _check_communities(self, G, truth, weight=None, seed=None):
        ...
        assert {frozenset(c) for c in C***REMOVED*** == truth
    ...
    def test_empty_graph(self):
        ...
        truth = {frozenset([i]) for i in G***REMOVED***
        self._check_communities(G, truth)

    def test_star_graph(self):
        G = nx.star_graph(self.N)
        truth = {frozenset(G)***REMOVED***
        self._check_communities(G, truth)
    ...
    def test_disjoin_cliques(self):
        ...
        truth = {frozenset("ab"), frozenset("ABC"), frozenset("1234")***REMOVED***
        self._check_communities(G, truth)
