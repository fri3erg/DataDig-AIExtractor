...
def _get_skiprows(skiprows: int | Sequence[int] | slice | None) -> int | Sequence[int]:
    ...
    if isinstance(skiprows, slice):
        ...
        return 0
    raise TypeError(f"{type(skiprows).__name__***REMOVED*** is not a valid type for skipping rows")
...
def _read(
    ...
    if (
        ...
        raise TypeError(f"Cannot read object of type '{type(obj).__name__***REMOVED***'")
    return text
...
class _HtmlFrameParser:
    ...
    extract_links : {None, "all", "header", "body", "footer"***REMOVED***
...
class _BeautifulSoupHtml5LibFrameParser(_HtmlFrameParser):
    ...
    def _parse_tables(self, document, match, attrs):
        ...
        if not result:
            raise ValueError(f"No tables found matching pattern {repr(match.pattern)***REMOVED***")
        return result
    ...
    def _setup_build_doc(self):
        ...
        if not raw_text:
            raise ValueError(f"No text parsed from document: {self.io***REMOVED***")
        return raw_text
...
def _build_xpath_expr(attrs) -> str:
    ...
    s = " and ".join([f"@{k***REMOVED***={repr(v)***REMOVED***" for k, v in attrs.items()])
    return f"[{s***REMOVED***]"


_re_namespace = {"re": "http://exslt.org/regular-expressions"***REMOVED***
...
class _LxmlFrameParser(_HtmlFrameParser):
    ...
    def _parse_tables(self, document, match, kwargs):
        ...
        # GH 49929
        xpath_expr = f"//table[.//text()[re:test(., {repr(pattern)***REMOVED***)]]"
            ...
            for table in tables:
                ...
                for elem in table.xpath(".//*[@style]"):
                    if "display:none" in elem.attrib.get("style", "").replace(" ", ""):
                        elem.drop_tree()
        if not tables:
            raise ValueError(f"No tables found matching regex {repr(pattern)***REMOVED***")
        return tables
...
def _parser_dispatch(flavor: HTMLFlavors | None) -> type[_HtmlFrameParser]:
    ...
    Parameters
    ----------
    flavor : {{"lxml", "html5lib", "bs4"***REMOVED******REMOVED*** or None
    ...
    if flavor not in valid_parsers:
        raise ValueError(
            f"{repr(flavor)***REMOVED*** is not a valid flavor, valid flavors are {valid_parsers***REMOVED***"
        )
...
def _print_as_set(s) -> str:
    ...
    return f"{{{arg***REMOVED******REMOVED******REMOVED***"
...
def _validate_flavor(flavor):
    if flavor is None:
        ...
        if not all(isinstance(flav, str) for flav in flavor):
            raise TypeError(
                f"Object of type {repr(type(flavor).__name__)***REMOVED*** "
...
***REMOVED***
    ...
    if not flavor_set & valid_flavors:
        raise ValueError(
            f"{_print_as_set(flavor_set)***REMOVED*** is not a valid set of flavors, valid "
            f"flavors are {_print_as_set(valid_flavors)***REMOVED***"
        )
    return flavor
    ...
    for flav in flavor:
        ...
        try:
            ...
            if hasattr(io, "seekable") and io.seekable():
                ...
                raise ValueError(
                    f"The flavor {flav***REMOVED*** failed to parse your input. "
...
def read_html(
    ...
    *,
    ...
    flavor : {{"lxml", "html5lib", "bs4"***REMOVED******REMOVED*** or list-like, optional
    ...
    attrs : dict, optional
        ...
        valid HTML table attributes to work correctly. For example, ::
            ...
            attrs = {{'id': 'table'***REMOVED******REMOVED***

        is a valid attribute dictionary because the 'id' HTML tag attribute is
        a valid HTML attribute for *any* HTML tag as per `this document
        <https://html.spec.whatwg.org/multipage/dom.html#global-attributes>`__. ::

            attrs = {{'asdf': 'table'***REMOVED******REMOVED***
    ...
    extract_links : {{None, "all", "header", "body", "footer"***REMOVED******REMOVED***
        Table elements in the specified section(s) with <a> tags will have their
        href extracted.
    ...
    **after** `skiprows` is applied.
    ...
    if extract_links not in [None, "header", "footer", "body", "all"]:
        raise ValueError(
            ...
            '{None, "header", "footer", "body", "all"***REMOVED***, got '
            f'"{extract_links***REMOVED***"'
        )
