...
cdef extern from "etree_api.h":
    ...
    # return the value of attribute "{ns***REMOVED***name", or the default value
    ...
    # store "{ns***REMOVED***tag" (or None) filter for this matcher or element iterator
    ...
    # split a tag into a (URI, name) tuple, return None as URI for '{***REMOVED***tag'
    cdef tuple getNsTag(object tag)

    # split a tag into a (URI, name) tuple, return b'' as URI for '{***REMOVED***tag'
    cdef tuple getNsTagWithEmptyNs(object tag)

    # get the "{ns***REMOVED***tag" string for a C node
    cdef unicode namespacedName(tree.xmlNode* c_node)

    # get the "{ns***REMOVED***tag" string for a href/tagname pair (c_ns may be NULL)
