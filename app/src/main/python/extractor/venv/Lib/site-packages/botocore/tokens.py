...
***REMOVED***
    ...
    def _load_sso_config(self):
        ...
        if not sso_config:
            error_msg = (
                f'The profile "{self._profile_name***REMOVED***" is configured to use the SSO '
                f'token provider but the "{sso_session_name***REMOVED***" sso_session '
...
***REMOVED***
    ...
    def _refresh_access_token(self, token):
        ...
        if missing_keys:
            msg = f"Unable to refresh SSO token: missing keys: {missing_keys***REMOVED***"
            logger.info(msg)
            return None

        expiry = dateutil.parser.parse(token["registrationExpiresAt"])
        if total_seconds(expiry - self._now()) <= 0:
            logger.info(f"SSO token registration expired at {expiry***REMOVED***")
            return None
    ...
    def _refresher(self):
        ...
        logger.info(f"Loading cached SSO token for {session_name***REMOVED***")
        token_dict = self._token_loader(start_url, session_name=session_name)
        expiration = dateutil.parser.parse(token_dict["expiresAt"])
        logger.debug(f"Cached SSO token expires at {expiration***REMOVED***")
