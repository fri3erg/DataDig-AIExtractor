    ...
    def _handshake(
        ...
        if "websocket" != headers.get(hdrs.UPGRADE, "").lower().strip():
            raise HTTPBadRequest(
                text=(
                    "No WebSocket UPGRADE hdr: {***REMOVED***\n Can "
                    '"Upgrade" only to "WebSocket".'
    ***REMOVED***.format(headers.get(hdrs.UPGRADE))
***REMOVED***

        if "upgrade" not in headers.get(hdrs.CONNECTION, "").lower():
            raise HTTPBadRequest(
                text="No CONNECTION upgrade hdr: {***REMOVED***".format(
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        if version not in ("13", "8", "7"):
            raise HTTPBadRequest(text=f"Unsupported version: {version***REMOVED***")

        # check client handshake for validity
        key = headers.get(hdrs.SEC_WEBSOCKET_KEY)
        try:
            if not key or len(base64.b64decode(key)) != 16:
                raise HTTPBadRequest(text=f"Handshake error: {key!r***REMOVED***")
        except binascii.Error:
            raise HTTPBadRequest(text=f"Handshake error: {key!r***REMOVED***") from None
    ...
    ***REMOVED***
    ...
    async def receive_str(self, *, timeout: Optional[float] = None) -> str:
        ...
        if msg.type != WSMsgType.TEXT:
            raise TypeError(
                "Received message {***REMOVED***:{!r***REMOVED*** is not WSMsgType.TEXT".format(
                    msg.type, msg.data
    ***REMOVED***
***REMOVED***
    ...
    async def receive_bytes(self, *, timeout: Optional[float] = None) -> bytes:
        ...
        if msg.type != WSMsgType.BINARY:
            raise TypeError(f"Received message {msg.type***REMOVED***:{msg.data!r***REMOVED*** is not bytes")
