...
class TestJoin:
    ...
    def target_source(self):
        ...
        ***REMOVED***
        ...
        source = DataFrame(
            {"MergedA": data["A"], "MergedD": data["D"]***REMOVED***, index=data["C"]
        )
    ...
    def test_join_on(self, target_source, infer_string):
        ...
        df = DataFrame({"key": ["a", "a", "b", "b", "c"]***REMOVED***)
        df2 = DataFrame({"value": [0, 1, 2]***REMOVED***, index=["a", "b", "c"])
        joined = df.join(df2, on="key")
        expected = DataFrame(
            {"key": ["a", "a", "b", "b", "c"], "value": [0, 0, 1, 1, 2]***REMOVED***
        )
    ...
    def test_join_on_fails_with_wrong_object_type(self, wrong_type):
        ...
        df = DataFrame({"a": [1, 1]***REMOVED***)
        msg = (
            "Can only merge Series or DataFrame objects, "
            f"a {type(wrong_type)***REMOVED*** was passed"
        )
    ...
    def test_join_on_pass_vector(self, target_source):
        ...
        expected = expected.rename(columns={"C": "key_0"***REMOVED***)
    ...
    def test_join_on_inner(self):
        df = DataFrame({"key": ["a", "a", "d", "b", "b", "c"]***REMOVED***)
        df2 = DataFrame({"value": [0, 1]***REMOVED***, index=["a", "b"])
    ...
    def test_join_on_series_buglet(self):
        ...
        expected = DataFrame({"a": [1, 1], "b": [2, 2]***REMOVED***, index=df.index)
        tm.assert_frame_equal(result, expected)
    ...
    def test_join_index_mixed_overlap(self):
        df1 = DataFrame(
            {"A": 1.0, "B": 2, "C": "foo", "D": True***REMOVED***,
            index=np.arange(10),
            columns=["A", "B", "C", "D"],
        )
    ...
    def test_join_inner_multiindex(self, lexsorted_two_level_string_multiindex):
        ...
        data = DataFrame({"key1": key1, "key2": key2, "data": data***REMOVED***)
    ...
    def test_join_hierarchical_mixed_raises(self):
        ...
        new_df = df.groupby(["a"]).agg({"b": ["mean", "sum"]***REMOVED***)
    ...
    def test_join_float64_float32(self):
        ...
        df = DataFrame({"a": a, "b": b, "c": c***REMOVED***)
        xpdf = DataFrame({"a": a, "b": b, "c": c***REMOVED***)
        s = DataFrame(
    ...
    def test_join_many_non_unique_index(self):
        df1 = DataFrame({"a": [1, 1], "b": [1, 1], "c": [10, 20]***REMOVED***)
        df2 = DataFrame({"a": [1, 1], "b": [1, 2], "d": [100, 200]***REMOVED***)
        df3 = DataFrame({"a": [1, 1], "b": [1, 2], "e": [1000, 2000]***REMOVED***)
        ...
        df1 = DataFrame({"a": [1, 1, 1], "b": [1, 1, 1], "c": [10, 20, 30]***REMOVED***)
        df2 = DataFrame({"a": [1, 1, 1], "b": [1, 1, 2], "d": [100, 200, 300]***REMOVED***)
        df3 = DataFrame({"a": [1, 1, 1], "b": [1, 1, 2], "e": [1000, 2000, 3000]***REMOVED***)
        idf1 = df1.set_index(["a", "b"])
        idf2 = df2.set_index(["a", "b"])
        idf3 = df3.set_index(["a", "b"])
    ...
    def test_join_sort(self, infer_string):
        with option_context("future.infer_string", infer_string):
            left = DataFrame(
                {"key": ["foo", "bar", "baz", "foo"], "value": [1, 2, 3, 4]***REMOVED***
***REMOVED***
            right = DataFrame({"value2": ["a", "b", "c"]***REMOVED***, index=["bar", "baz", "foo"])
...
***REMOVED***
    ...
    def test_join_mixed_non_unique_index(self):
        ...
        df1 = DataFrame({"a": [1, 2, 3, 4]***REMOVED***, index=[1, 2, 3, "a"])
        df2 = DataFrame({"b": [5, 6, 7, 8]***REMOVED***, index=[1, 3, 3, 4])
        result = df1.join(df2)
        expected = DataFrame(
            {"a": [1, 2, 3, 3, 4], "b": [5, np.nan, 6, 7, np.nan]***REMOVED***,
            index=[1, 2, 3, 3, "a"],
        )
        tm.assert_frame_equal(result, expected)

        df3 = DataFrame({"a": [1, 2, 3, 4]***REMOVED***, index=[1, 2, 2, "a"])
        df4 = DataFrame({"b": [5, 6, 7, 8]***REMOVED***, index=[1, 2, 3, 4])
        result = df3.join(df4)
        expected = DataFrame(
            {"a": [1, 2, 3, 4], "b": [5, 6, 6, np.nan]***REMOVED***, index=[1, 2, 2, "a"]
        )
        tm.assert_frame_equal(result, expected)
    ...
    ***REMOVED***,
    ...
    def test_join_multi_to_multi(self, join_type):
        ...
        left = DataFrame({"v1": range(12)***REMOVED***, index=leftindex)

        rightindex = MultiIndex.from_product(
            [list("abc"), list("xy")], names=["abc", "xy"]
        )
        right = DataFrame({"v2": [100 * i for i in range(1, 7)]***REMOVED***, index=rightindex)
    ...
    ***REMOVED***,
    ...
    def test_join_with_categorical_index(self):
        ...
        df1 = DataFrame({"c1": ix***REMOVED***, index=id1)
        df2 = DataFrame({"c2": reversed(ix)***REMOVED***, index=id2)
        result = df1.join(df2)
        expected = DataFrame(
            {"c1": ["a", "b"], "c2": ["a", "b"]***REMOVED***,
        ...
        try:
            ...
            if how in ("left", "inner"):
                raise AssertionError(
                    f"key {group_key***REMOVED*** should not have been in the join"
    ***REMOVED*** from err
...
def _assert_same_contents(join_chunk, source):
    ...
    rows = {tuple(row) for row in jvalues***REMOVED***
...
def test_join_inner_multiindex_deterministic_order():
    ...
    right = DataFrame(
        data={"f": 6***REMOVED***, index=MultiIndex.from_tuples([(2, 3)], names=("b", "c"))
    )
...
def test_join_cross(input_col, output_cols):
    # GH#5401
    left = DataFrame({"a": [1, 3]***REMOVED***)
    right = DataFrame({input_col: [3, 4]***REMOVED***)
    result = left.join(right, how="cross", lsuffix="_x", rsuffix="_y")
    expected = DataFrame({output_cols[0]: [1, 1, 3, 3], output_cols[1]: [3, 4, 3, 4]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_join_multiindex_one_level(join_type):
    # GH#36909
    left = DataFrame(
        data={"c": 3***REMOVED***, index=MultiIndex.from_tuples([(1, 2)], names=("a", "b"))
    )
    right = DataFrame(data={"d": 4***REMOVED***, index=MultiIndex.from_tuples([(2,)], names=("b",)))
...
def test_join_empty(left_empty, how, exp):
    left = DataFrame({"A": [2, 1], "B": [3, 4]***REMOVED***, dtype="int64").set_index("A")
    right = DataFrame({"A": [1], "C": [5]***REMOVED***, dtype="int64").set_index("A")
    ...
    if exp == "left":
        ...
        expected = DataFrame({"B": [np.nan], "A": [1], "C": [5]***REMOVED***)
        expected = expected.set_index("A")
    elif exp == "empty":
