...
class TestFrameAccessor:
    ...
    def test_to_coo(self, colnames):
        ...
        df = pd.DataFrame(
            {colnames[0]: [0, 1, 0], colnames[1]: [1, 0, 0]***REMOVED***, dtype="Sparse[int64, 0]"
        )
    ...
    ***REMOVED***,
    ...
    def test_to_dense(self):
        ...
        expected = pd.DataFrame(
            {"A": [1, 0], "B": [1, 0], "C": [1.0, 0.0]***REMOVED***, index=["b", "a"]
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_with_column_named_sparse(self):
        ...
        df = pd.DataFrame({"sparse": pd.arrays.SparseArray([1, 2])***REMOVED***)
