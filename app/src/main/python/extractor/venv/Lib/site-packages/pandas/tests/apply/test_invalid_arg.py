...
def test_result_type_error(result_type):
    ...
    msg = (
        ...
        "{None, 'reduce', 'broadcast', 'expand'***REMOVED***"
    )
...
def test_map_arg_is_dict_with_invalid_na_action_raises(input_na_action):
    ...
    s = Series([1, 2, 3])
    msg = f"na_action must either be 'ignore' or None, {input_na_action***REMOVED*** was passed"
    with pytest.raises(ValueError, match=msg):
        s.map({1: 2***REMOVED***, na_action=input_na_action)


@pytest.mark.parametrize("method", ["apply", "agg", "transform"])
@pytest.mark.parametrize("func", [{"A": {"B": "sum"***REMOVED******REMOVED***, {"A": {"B": ["sum"]***REMOVED******REMOVED***])
def test_nested_renamer(frame_or_series, method, func):
    # GH 35964
    obj = frame_or_series({"A": [1]***REMOVED***)
...
@pytest.mark.parametrize(
    "renamer",
    [{"foo": ["min", "max"]***REMOVED***, {"foo": ["min", "max"], "bar": ["sum", "mean"]***REMOVED***],
)
...
def test_apply_dict_depr():
    ...
    msg = "nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        tsdf.A.agg({"foo": ["sum", "mean"]***REMOVED***)
...
def test_dict_nested_renaming_depr(method):
    ...
    with pytest.raises(SpecificationError, match=msg):
        getattr(df, method)({"A": {"foo": "min"***REMOVED***, "B": {"bar": "max"***REMOVED******REMOVED***)


@pytest.mark.parametrize("method", ["apply", "agg", "transform"])
@pytest.mark.parametrize("func", [{"B": "sum"***REMOVED***, {"B": ["sum"]***REMOVED***])
def test_missing_column(method, func):
    # GH 40004
    obj = DataFrame({"A": [1]***REMOVED***)
...
def test_transform_mixed_column_name_dtypes():
    ...
    with pytest.raises(KeyError, match=msg):
        df.transform({"a": int, 1: str, "b": int***REMOVED***)
...
def test_apply_str_axis_1_raises(how, args):
    ...
    msg = f"Operation {how***REMOVED*** does not support axis=1"
    with pytest.raises(ValueError, match=msg):
...
def test_agg_none_to_type():
    # GH 40543
    df = DataFrame({"a": [None]***REMOVED***)
    msg = re.escape("int() argument must be a string")
    with pytest.raises(TypeError, match=msg):
        df.agg({"a": lambda x: int(x.iloc[0])***REMOVED***)
...
def test_transform_none_to_type():
    ...
    with pytest.raises(TypeError, match=msg):
        df.transform({"a": lambda x: int(x.iloc[0])***REMOVED***)
...
@pytest.mark.parametrize(
    "op_wrapper", [lambda x: x, lambda x: [x], lambda x: {"A": x***REMOVED***, lambda x: {"A": [x]***REMOVED***]
)
def test_transform_reducer_raises(all_reductions, frame_or_series, op_wrapper):
    # GH 35964
    op = op_wrapper(all_reductions)

    obj = DataFrame({"A": [1, 2, 3]***REMOVED***)
