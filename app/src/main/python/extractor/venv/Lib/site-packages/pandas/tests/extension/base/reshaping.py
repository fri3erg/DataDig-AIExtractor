...
class BaseReshapingTests:
    ...
    def test_concat_all_na_block(self, data_missing, in_frame):
        ...
        if in_frame:
            valid_block = pd.DataFrame({"a": valid_block***REMOVED***)
            na_block = pd.DataFrame({"a": na_block***REMOVED***)
        result = pd.concat([valid_block, na_block])
        if in_frame:
            expected = pd.DataFrame({"a": data_missing.take([1, 1, 0, 0])***REMOVED***)
    ...
    def test_concat_mixed_dtypes(self, data):
        ...
        df1 = pd.DataFrame({"A": data[:3]***REMOVED***)
        df2 = pd.DataFrame({"A": [1, 2, 3]***REMOVED***)
        df3 = pd.DataFrame({"A": ["a", "b", "c"]***REMOVED***).astype("category")
        dfs = [df1, df2, df3]
    ...
    def test_concat_columns(self, data, na_value):
        df1 = pd.DataFrame({"A": data[:3]***REMOVED***)
        df2 = pd.DataFrame({"B": [1, 2, 3]***REMOVED***)

        expected = pd.DataFrame({"A": data[:3], "B": [1, 2, 3]***REMOVED***)
        ...
        # non-aligned
        df2 = pd.DataFrame({"B": [1, 2, 3]***REMOVED***, index=[1, 2, 3])
    ...
    def test_concat_extension_arrays_copy_false(self, data, na_value):
        # GH 20756
        df1 = pd.DataFrame({"A": data[:3]***REMOVED***)
        df2 = pd.DataFrame({"B": data[3:7]***REMOVED***)
        expected = pd.DataFrame(
            {
                "A": data._from_sequence(list(data[:3]) + [na_value], dtype=data.dtype),
                "B": data[3:7],
    ...
    def test_concat_with_reindex(self, data):
        # GH-33027
        a = pd.DataFrame({"a": data[:5]***REMOVED***)
        b = pd.DataFrame({"b": data[:5]***REMOVED***)
    ...
    def test_align_frame(self, data, na_value):
        a = data[:3]
        b = data[2:5]
        r1, r2 = pd.DataFrame({"A": a***REMOVED***).align(pd.DataFrame({"A": b***REMOVED***, index=[1, 2, 3]))

        # Assumes that the ctor can take a list of scalars of the type
        e1 = pd.DataFrame(
            {"A": data._from_sequence(list(a) + [na_value], dtype=data.dtype)***REMOVED***
        )
        e2 = pd.DataFrame(
            {"A": data._from_sequence([na_value] + list(b), dtype=data.dtype)***REMOVED***
        )
    ...
    def test_align_series_frame(self, data, na_value):
        ...
        df = pd.DataFrame({"col": np.arange(len(ser) + 1)***REMOVED***)
        r1, r2 = ser.align(df)
    ...
    def test_set_frame_expand_regular_with_extension(self, data):
        df = pd.DataFrame({"A": [1] * len(data)***REMOVED***)
        df["B"] = data
        expected = pd.DataFrame({"A": [1] * len(data), "B": data***REMOVED***)
        tm.assert_frame_equal(df, expected)

    def test_set_frame_expand_extension_with_regular(self, data):
        df = pd.DataFrame({"A": data***REMOVED***)
        df["B"] = [1] * len(data)
        expected = pd.DataFrame({"A": data, "B": [1] * len(data)***REMOVED***)
        tm.assert_frame_equal(df, expected)

    def test_set_frame_overwrite_object(self, data):
        # https://github.com/pandas-dev/pandas/issues/20555
        df = pd.DataFrame({"A": [1] * len(data)***REMOVED***, dtype=object)
        df["A"] = data
        assert df.dtypes["A"] == data.dtype

    def test_merge(self, data, na_value):
        # GH-20743
        df1 = pd.DataFrame({"ext": data[:3], "int1": [1, 2, 3], "key": [0, 1, 2]***REMOVED***)
        df2 = pd.DataFrame({"int2": [1, 2, 3, 4], "key": [0, 0, 1, 3]***REMOVED***)
    ...
    ***REMOVED***,
    ...
    def test_merge_on_extension_array(self, data):
        ...
        df = pd.DataFrame({"key": key, "val": [1, 2]***REMOVED***)
        result = pd.merge(df, df, on="key")
        expected = pd.DataFrame({"key": key, "val_x": [1, 2], "val_y": [1, 2]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_merge_on_extension_array_duplicates(self, data):
        ...
        df1 = pd.DataFrame({"key": key, "val": [1, 2, 3]***REMOVED***)
        df2 = pd.DataFrame({"key": key, "val": [1, 2, 3]***REMOVED***)
...
***REMOVED***,
    ...
    def test_stack(self, data, columns, future_stack):
        df = pd.DataFrame({"A": data[:5], "B": data[:5]***REMOVED***)
        df.columns = columns
...
***REMOVED***,
    ...
    def test_unstack(self, data, index, obj):
        ...
        if obj == "series":
            ...
            ser = pd.DataFrame({"A": data, "B": data***REMOVED***, index=index)
...
***REMOVED***
    ...
    def test_ravel(self, data):
        ...
        if data.dtype._is_immutable:
            pytest.skip(f"test_ravel assumes mutability and {data.dtype***REMOVED*** is immutable")
    ...
    def test_transpose(self, data):
        ...
        if data.dtype._is_immutable:
            pytest.skip(
                f"test_transpose assumes mutability and {data.dtype***REMOVED*** is immutable"
***REMOVED***
    ...
    def test_transpose_frame(self, data):
        df = pd.DataFrame({"A": data[:4], "B": data[:4]***REMOVED***, index=["a", "b", "c", "d"])
        result = df.T
        expected = pd.DataFrame(
            {
