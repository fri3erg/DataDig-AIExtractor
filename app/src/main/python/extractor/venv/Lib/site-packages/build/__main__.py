...
_COLORS = {
    ...
    'reset': '\33[0m',
***REMOVED***
_NO_COLORS = {color: '' for color in _COLORS***REMOVED***
...
def _showwarning(
    ...
    _cprint('{yellow***REMOVED***WARNING{reset***REMOVED*** {***REMOVED***', str(message))
...
def _error(msg: str, code: int = 1) -> NoReturn:  # pragma: no cover
    ...
    """
    _cprint('{red***REMOVED***ERROR{reset***REMOVED*** {***REMOVED***', msg)
...
class _ProjectBuilder(ProjectBuilder):
    ...
    def log(message: str) -> None:
        _cprint('{bold***REMOVED**** {***REMOVED***{reset***REMOVED***', message)
...
def _build_in_isolated_env(
    ...
    with _DefaultIsolatedEnv() as env:
        ...
        env.install(builder.get_requires_for_build(distribution, config_settings or {***REMOVED***))
        return builder.build(distribution, outdir, config_settings or {***REMOVED***)
...
def _build_in_current_env(
    ...
    if not skip_dependency_check:
        missing = builder.check_dependencies(distribution, config_settings or {***REMOVED***)
        if missing:
            dependencies = ''.join('\n\t' + dep for deps in missing for dep in (deps[0], _format_dep_chain(deps[1:])) if dep)
            _cprint()
            _error(f'Missing dependencies:{dependencies***REMOVED***')
...
***REMOVED***
        ...
        _cprint('\n{dim***REMOVED***{***REMOVED***{reset***REMOVED***\n', tb.strip('\n'))
        _error(str(e))
...
def _natural_language_list(elements: Sequence[str]) -> str:
    if len(elements) == 0:
        ...
        return '{***REMOVED*** and {***REMOVED***'.format(
            ', '.join(elements[:-1]),
            elements[-1],
        )
...
def build_package_via_sdist(
    ...
    if distributions:
        ...
        with TarFile.open(sdist) as t:
            ...
            try:
                _ProjectBuilder.log(f'Building {_natural_language_list(distributions)***REMOVED*** from sdist')
    ...
    parser = argparse.ArgumentParser(
        description=textwrap.indent(
            textwrap.dedent(
                ...
                By default, a source distribution (sdist) is built from {srcdir***REMOVED***
                ...
                artifacts will be built from {srcdir***REMOVED*** (even if you combine
                -w/--wheel with -s/--sdist, the wheel will be built from {srcdir***REMOVED***).
                '''
***REMOVED***.strip(),
            '    ',
        ),
    ...
    parser.add_argument(
        ...
        '-V',
        action='version',
        version=f"build {build.__version__***REMOVED*** ({','.join(build.__path__)***REMOVED***)",
    )
    parser.add_argument(
        '--sdist',
        '-s',
    ...
    parser.add_argument(
        ...
        '-o',
        type=str,
        help=f'output directory (defaults to {{srcdir***REMOVED******REMOVED***{os.sep***REMOVED***dist)',
        metavar='PATH',
    )
    parser.add_argument(
...
***REMOVED***
            artifact_list = _natural_language_list(
                ['{underline***REMOVED***{***REMOVED***{reset***REMOVED***{bold***REMOVED***{green***REMOVED***'.format(artifact, **_STYLES) for artifact in built]
***REMOVED***
            _cprint('{bold***REMOVED***{green***REMOVED***Successfully built {***REMOVED***{reset***REMOVED***', artifact_list)
