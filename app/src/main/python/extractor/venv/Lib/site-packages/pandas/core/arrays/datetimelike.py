...
class DatetimeLikeArrayMixin(  # type: ignore[misc]
    ...
    ***REMOVED***
            ...
            if dtype != np.int64:
                raise TypeError(
                    f"Converting from {self.dtype***REMOVED*** to {dtype***REMOVED*** is not supported. "
    ...
    ***REMOVED***
            ...
            msg = f"Cannot cast {type(self).__name__***REMOVED*** to dtype {dtype***REMOVED***"
            raise TypeError(msg)
        else:
    ...
    def _validation_error_message(self, value, allow_listlike: bool = False) -> str:
        ...
        if hasattr(value, "dtype") and getattr(value, "ndim", 0) > 0:
            msg_got = f"{value.dtype***REMOVED*** array"
        else:
            msg_got = f"'{type(value).__name__***REMOVED***'"
        if allow_listlike:
            msg = (
                f"value should be a '{self._scalar_type.__name__***REMOVED***', 'NaT', "
                f"or array of those. Got {msg_got***REMOVED*** instead."
***REMOVED***
        else:
            msg = (
                f"value should be a '{self._scalar_type.__name__***REMOVED***' or 'NaT'. "
                f"Got {msg_got***REMOVED*** instead."
***REMOVED***
        return msg
    ...
    ***REMOVED***
            ...
            try:
                ...
                warnings.warn(
                    # GH#53111
                    f"The behavior of 'isin' with dtype={self.dtype***REMOVED*** and "
...
***REMOVED***
    ...
    def _add_datetimelike_scalar(self, other) -> DatetimeArray:
        if not lib.is_np_dtype(self.dtype, "m"):
            raise TypeError(
                f"cannot add {type(self).__name__***REMOVED*** and {type(other).__name__***REMOVED***"
***REMOVED***
        ...
        if isna(other):
            ...
            result = self._ndarray + NaT.to_datetime64().astype(f"M8[{self.unit***REMOVED***]")
        ...
        res_values = result.view(f"M8[{self.unit***REMOVED***]")
...
***REMOVED***
    ...
    def _sub_datetimelike_scalar(
        ...
        if self.dtype.kind != "M":
            raise TypeError(f"cannot subtract a datelike from a {type(self).__name__***REMOVED***")
    ...
    def _sub_datetimelike(self, other: Timestamp | DatetimeArray) -> TimedeltaArray:
        ...
        res_m8 = res_values.view(f"timedelta64[{self.unit***REMOVED***]")
    ...
    def _add_period(self, other: Period) -> PeriodArray:
        if not lib.is_np_dtype(self.dtype, "m"):
            raise TypeError(f"cannot add Period to a {type(self).__name__***REMOVED***")
    ...
    def _add_nat(self):
        ...
        if isinstance(self.dtype, PeriodDtype):
            raise TypeError(
                f"Cannot add {type(self).__name__***REMOVED*** and {type(NaT).__name__***REMOVED***"
***REMOVED***
    ...
    def _sub_nat(self):
        ...
        if self.dtype.kind in "mM":
            ...
            self = cast("DatetimeArray| TimedeltaArray", self)
            return result.view(f"timedelta64[{self.unit***REMOVED***]")
        else:
    ...
    def _sub_periodlike(self, other: Period | PeriodArray) -> npt.NDArray[np.object_]:
        ...
        if not isinstance(self.dtype, PeriodDtype):
            raise TypeError(
                f"cannot subtract {type(other).__name__***REMOVED*** from {type(self).__name__***REMOVED***"
***REMOVED***
    ...
    def _addsub_object_array(self, other: npt.NDArray[np.object_], op):
        ...
        op : {operator.add, operator.sub***REMOVED***
        ...
        warnings.warn(
            ...
            f"{type(self).__name__***REMOVED*** not vectorized.",
            PerformanceWarning,
            stacklevel=find_stack_level(),
        )
    ...
    def _accumulate(self, name: str, *, skipna: bool = True, **kwargs) -> Self:
        if name not in {"cummin", "cummax"***REMOVED***:
            raise TypeError(f"Accumulation {name***REMOVED*** not supported for {type(self)***REMOVED***")
    ...
    def __rsub__(self, other):
        ...
        if other_is_dt64 and lib.is_np_dtype(self.dtype, "m"):
            ...
            raise TypeError(
                f"cannot subtract {type(self).__name__***REMOVED*** from {type(other).__name__***REMOVED***"
***REMOVED***
        elif isinstance(self.dtype, PeriodDtype) and lib.is_np_dtype(other_dtype, "m"):
            # TODO: Can we simplify/generalize these cases at all?
            raise TypeError(f"cannot subtract {type(self).__name__***REMOVED*** from {other.dtype***REMOVED***")
    ...
    def mean(self, *, skipna: bool = True, axis: AxisInt | None = 0):
        ...
        if isinstance(self.dtype, PeriodDtype):
            ...
            raise TypeError(
                f"mean is not implemented for {type(self).__name__***REMOVED*** since the "
        ...
        **kwargs,
        ...
        if dtype.kind == "M":
            ...
            if how in ["sum", "prod", "cumsum", "cumprod", "var", "skew"]:
                raise TypeError(f"datetime64 type does not support {how***REMOVED*** operations")
            if how in ["any", "all"]:
                # GH#34479
                warnings.warn(
                    f"'{how***REMOVED***' with datetime64 dtypes is deprecated and will raise in a "
                    f"future version. Use (obj != pd.Timestamp(0)).{how***REMOVED***() instead.",
                    FutureWarning,
                    stacklevel=find_stack_level(),
    ***REMOVED***
            ...
            if how in ["sum", "prod", "cumsum", "cumprod", "var", "skew"]:
                raise TypeError(f"Period type does not support {how***REMOVED*** operations")
            if how in ["any", "all"]:
                # GH#34479
                warnings.warn(
                    f"'{how***REMOVED***' with PeriodDtype is deprecated and will raise in a "
                    f"future version. Use (obj != pd.Period(0, freq)).{how***REMOVED***() instead.",
                    FutureWarning,
                    stacklevel=find_stack_level(),
    ***REMOVED***
        else:
            ...
            if how in ["prod", "cumprod", "skew", "var"]:
                raise TypeError(f"timedelta64 type does not support {how***REMOVED*** operations")
        ...
        if how in ["std", "sem"]:
            ...
            self = cast("DatetimeArray | TimedeltaArray", self)
            new_dtype = f"m8[{self.unit***REMOVED***]"
...
_round_doc = """
    Perform {op***REMOVED*** operation on the data to the specified `freq`.

    Parameters
    ----------
    freq : str or Offset
        The frequency level to {op***REMOVED*** the index to. Must be a fixed
    ...
    Notes
    -----
    If the timestamps have a timezone, {op***REMOVED***ing will take place relative to the
    local ("wall") time and re-localized to the same timezone. When {op***REMOVED***ing
...
class TimelikeOps(DatetimeLikeArrayMixin):
    ...
    def __init__(
        ...
        warnings.warn(
            # GH#55623
            f"{type(self).__name__***REMOVED***.__init__ is deprecated and will be "
        ...
        if isinstance(values, type(self)):
            ...
            if dtype is not None and dtype != values.dtype:
                ...
                raise TypeError(
                    f"dtype={dtype***REMOVED*** does not match data dtype {values.dtype***REMOVED***"
    ***REMOVED***
        ...
        if not isinstance(values, np.ndarray):
            raise ValueError(
                f"Unexpected type '{type(values).__name__***REMOVED***'. 'values' must be a "
                f"{type(self).__name__***REMOVED***, ndarray, or Series or Index "
...
***REMOVED***
        ...
        if values.dtype == "i8":
            ...
            if dtype is None:
                ...
                new_dtype = f"{kind***REMOVED***8[{dtype.unit***REMOVED***]"
        ...
        if freq == "infer":
            raise ValueError(
                f"Frequency inference not allowed in {type(self).__name__***REMOVED***.__init__. "
...
***REMOVED***
            ...
            #  message.
            raise ValueError(
                f"Inferred frequency {inferred***REMOVED*** from passed values "
                f"does not conform to passed frequency {freq.freqstr***REMOVED***"
***REMOVED*** from err
    ...
    def as_unit(self, unit: str, round_ok: bool = True) -> Self:
        ...
        dtype = np.dtype(f"{self.dtype.kind***REMOVED***8[{unit***REMOVED***]")
    ...
    def factorize(
        ...
        if sort:
            ...
            raise NotImplementedError(
                f"The 'sort' keyword in {type(self).__name__***REMOVED***.factorize is "
...
def ensure_arraylike_for_datetimelike(
    ...
    if not hasattr(data, "dtype"):
        ...
        raise TypeError(f"Cannot create a {cls_name***REMOVED*** from a MultiIndex.")
    else:
...
***REMOVED***
            ...
            raise TypeError(f"periods must be a number, got {periods***REMOVED***")
    return periods
...
def _validate_inferred_freq(
    ...
    if inferred_freq is not None:
        if freq is not None and freq != inferred_freq:
            raise ValueError(
                f"Inferred frequency {inferred_freq***REMOVED*** from passed "
                "values does not conform to passed frequency "
                f"{freq.freqstr***REMOVED***"
***REMOVED***
        if freq is None:
            freq = inferred_freq
...
def dtype_to_unit(dtype: DatetimeTZDtype | np.dtype | ArrowDtype) -> str:
    ...
    if isinstance(dtype, DatetimeTZDtype):
        ...
        if dtype.kind not in "mM":
            raise ValueError(f"{dtype=***REMOVED*** does not have a resolution.")
