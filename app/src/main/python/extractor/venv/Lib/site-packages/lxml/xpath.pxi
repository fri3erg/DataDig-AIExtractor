...
cdef object _find_namespaces = re.compile(b'({[^***REMOVED***]+***REMOVED***)').findall

cdef class ETXPath(XPath):
    """ETXPath(self, path, extensions=None, regexp=True, smart_strings=True)
    Special XPath class that supports the ElementTree {uri***REMOVED*** notation for namespaces.
    ...
    cdef _nsextract_path(self, path):
        # replace {namespaces***REMOVED*** by new prefixes
        cdef dict namespaces = {***REMOVED***
        ...
        for namespace_def in _find_namespaces(stripped_path):
            if namespace_def not in namespace_defs:
                ...
                namespace = namespace_def[1:-1] # remove '{***REMOVED***'
                namespace = (<bytes>namespace).decode('utf8')
                namespaces[prefix.decode('utf8')] = namespace
                prefix_str = prefix + b':'
                # FIXME: this also replaces {namespaces***REMOVED*** within strings!
