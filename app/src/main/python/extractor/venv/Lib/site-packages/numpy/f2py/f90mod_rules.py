...
def buildhooks(pymod):
    ...
    for m in findf90modules(pymod):
        ...
        if m['name'] in usenames and not onlyvars:
            outmess(f"\t\t\tSkipping {m['name']***REMOVED*** since it is in 'use'...\n")
            continue
        if onlyvars:
            outmess('\t\t  Variables: %s\n' % (' '.join(onlyvars)))
        chooks = ['']
        ...
        if hasnote(m):
            ...
            dadd(note)
        if onlyvars:
            dadd('\\begin{description***REMOVED***')
        for n in onlyvars:
            var = m['vars'][n]
            modobjs.append(n)
            ...
            cadd('\t{"%s",%s,{{%s***REMOVED******REMOVED***,%s, %s***REMOVED***,' %
                 (undo_rmbadname1(n), dm['rank'], dms, at,
                  capi_maps.get_elsize(var)))
            dadd('\\item[]{{***REMOVED***\\verb@%s@{***REMOVED******REMOVED***' %
        ...
        if onlyvars:
            dadd('\\end{description***REMOVED***')
        if hasbody(m):
            for b in m['body']:
                if not isroutine(b):
                    outmess("f90mod_rules.buildhooks:"
                            f" skipping {b['block']***REMOVED*** {b['name']***REMOVED***\n")
                    continue
                ...
                cadd(('\t{"%s",-1,{{-1***REMOVED******REMOVED***,0,0,NULL,(void *)'
                      'f2py_rout_#modulename#_%s_%s,'
                      'doc_f2py_rout_#modulename#_%s_%s***REMOVED***,')
