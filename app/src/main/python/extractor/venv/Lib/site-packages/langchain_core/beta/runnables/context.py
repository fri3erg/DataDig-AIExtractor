...
def _key_from_id(id_: str) -> str:
    ...
    if wout_prefix.endswith(CONTEXT_CONFIG_SUFFIX_GET):
        ...
        raise ValueError(f"Invalid context config id {id_***REMOVED***")
...
def _config_with_context(
    ...
    if any(k.startswith(CONTEXT_CONFIG_PREFIX) for k in config.get("configurable", {***REMOVED***)):
        return config
    ...
    ***REMOVED***
    ...
    context_funcs: Dict[str, Callable[[], Any]] = {***REMOVED***
    for key, group in grouped_by_key.items():
        ...
        for dep in deps_by_key[key]:
            if key in deps_by_key[dep]:
                raise ValueError(
                    f"Deadlock detected between context keys {key***REMOVED*** and {dep***REMOVED***"
    ***REMOVED***
        if len(setters) != 1:
            raise ValueError(f"Expected exactly one setter for context key {key***REMOVED***")
        setter_idx = setters[0][1]
        if any(getter_idx < setter_idx for _, getter_idx in getters):
            raise ValueError(
                f"Context setter for key {key***REMOVED*** must be defined after all getters."
***REMOVED***
...
class ContextGet(RunnableSerializable):
    ...
    def __str__(self) -> str:
        return f"ContextGet({_print_keys(self.key)***REMOVED***)"
    ...
    def ids(self) -> List[str]:
        ...
        return [
            f"{CONTEXT_CONFIG_PREFIX***REMOVED***{prefix***REMOVED***{k***REMOVED***{CONTEXT_CONFIG_SUFFIX_GET***REMOVED***"
            for k in keys
        ]
...
***REMOVED***
    ...
    def invoke(self, input: Any, config: Optional[RunnableConfig] = None) -> Any:
        ...
        if isinstance(self.key, list):
            return {key: configurable[id_]() for key, id_ in zip(self.key, self.ids)***REMOVED***
        else:
    ...
    async def ainvoke(
        ...
        if isinstance(self.key, list):
            ...
            return {key: value for key, value in zip(self.key, values)***REMOVED***
        else:
...
class ContextSet(RunnableSerializable):
    ...
    def __str__(self) -> str:
        return f"ContextSet({_print_keys(list(self.keys.keys()))***REMOVED***)"

    @property
    def ids(self) -> List[str]:
        prefix = self.prefix + "/" if self.prefix else ""
        return [
            f"{CONTEXT_CONFIG_PREFIX***REMOVED***{prefix***REMOVED***{key***REMOVED***{CONTEXT_CONFIG_SUFFIX_SET***REMOVED***"
        ...
        for spec in mapper_config_specs:
            if spec.id.endswith(CONTEXT_CONFIG_SUFFIX_GET):
                ...
                if getter_key in self.keys:
                    raise ValueError(
                        f"Circular reference in context setter for key {getter_key***REMOVED***"
        ***REMOVED***
        return super().config_specs + [
            ConfigurableFieldSpec(
                id=id_,
...
def _print_keys(keys: Union[str, Sequence[str]]) -> str:
    if isinstance(keys, str):
        return f"'{keys***REMOVED***'"
    else:
        return ", ".join(f"'{k***REMOVED***'" for k in keys)
