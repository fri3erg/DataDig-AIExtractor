...
def _zip_axes_from_type(
    ...
    return {name: new_axes[i] for i, name in enumerate(typ._AXIS_ORDERS)***REMOVED***
        ...
        for axis, items in zip(range(ndim), axes):
            ...
            if hasattr(ti, "reindex"):
                ...
                if ordm >= 1 and reindexer_size >= 10000:
                    w = (
                        f"Alignment difference on axis {axis***REMOVED*** is larger "
                        f"than an order of magnitude on term {repr(terms[i].name)***REMOVED***, "
                        f"by more than {ordm:.4g***REMOVED***; performance may suffer."
        ***REMOVED***
                    warnings.warn(
