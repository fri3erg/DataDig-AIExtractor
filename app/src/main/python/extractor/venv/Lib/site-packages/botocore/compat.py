...
def ensure_bytes(s, encoding='utf-8', errors='strict'):
    ...
    if isinstance(s, bytes):
        return s
    raise ValueError(f"Expected str or bytes, received {type(s)***REMOVED***.")
...
def _windows_shell_split(s):
    ...
    * Backslash is interpreted literally unless it is part of a chain that
    ...
    if is_quoted:
        raise ValueError(f"No closing quotation in string: {s***REMOVED***")
...
IPV4_PAT = r"(?:[0-9]{1,3***REMOVED***\.){3***REMOVED***[0-9]{1,3***REMOVED***"
IPV4_RE = re.compile("^" + IPV4_PAT + "$")
HEX_PAT = "[0-9A-Fa-f]{1,4***REMOVED***"
LS32_PAT = "(?:{hex***REMOVED***:{hex***REMOVED***|{ipv4***REMOVED***)".format(hex=HEX_PAT, ipv4=IPV4_PAT)
_subs = {"hex": HEX_PAT, "ls32": LS32_PAT***REMOVED***
_variations = [
    #                            6( h16 ":" ) ls32
    "(?:%(hex)s:){6***REMOVED***%(ls32)s",
    #                       "::" 5( h16 ":" ) ls32
    "::(?:%(hex)s:){5***REMOVED***%(ls32)s",
    # [               h16 ] "::" 4( h16 ":" ) ls32
    "(?:%(hex)s)?::(?:%(hex)s:){4***REMOVED***%(ls32)s",
    # [ *1( h16 ":" ) h16 ] "::" 3( h16 ":" ) ls32
    "(?:(?:%(hex)s:)?%(hex)s)?::(?:%(hex)s:){3***REMOVED***%(ls32)s",
    # [ *2( h16 ":" ) h16 ] "::" 2( h16 ":" ) ls32
    "(?:(?:%(hex)s:){0,2***REMOVED***%(hex)s)?::(?:%(hex)s:){2***REMOVED***%(ls32)s",
    # [ *3( h16 ":" ) h16 ] "::"    h16 ":"   ls32
    "(?:(?:%(hex)s:){0,3***REMOVED***%(hex)s)?::%(hex)s:%(ls32)s",
    # [ *4( h16 ":" ) h16 ] "::"              ls32
    "(?:(?:%(hex)s:){0,4***REMOVED***%(hex)s)?::%(ls32)s",
    # [ *5( h16 ":" ) h16 ] "::"              h16
    "(?:(?:%(hex)s:){0,5***REMOVED***%(hex)s)?::%(hex)s",
    # [ *6( h16 ":" ) h16 ] "::"
    "(?:(?:%(hex)s:){0,6***REMOVED***%(hex)s)?::",
]
...
ZONE_ID_PAT = "(?:%25|%)(?:[" + UNRESERVED_PAT + "]|%[a-fA-F0-9]{2***REMOVED***)+"
