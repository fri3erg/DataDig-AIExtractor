...
def test_frame(dti, _test_series):
    return DataFrame({"A": _test_series, "B": _test_series, "C": np.arange(len(dti))***REMOVED***)
...
def test_groupby_resample_api():
    ...
    expected = DataFrame({"val": [5] * 7 + [6] + [7] * 7 + [8]***REMOVED***, index=index)
...
def test_resample_group_keys():
    df = DataFrame({"A": 1, "B": 2***REMOVED***, index=date_range("2000", periods=10))
    expected = df.copy()
...
@pytest.mark.parametrize(
    "agg",
    [
        {"func": {"A": np.mean, "B": np.std***REMOVED******REMOVED***,
        {"A": ("A", np.mean), "B": ("B", np.std)***REMOVED***,
        {"A": NamedAgg("A", np.mean), "B": NamedAgg("B", np.std)***REMOVED***,
    ],
)
...
def test_agg_both_mean_std_dict_of_list(cases, a_mean, a_std):
    ...
    result = cases.aggregate({"A": ["mean", "std"]***REMOVED***)
    tm.assert_frame_equal(result, expected)


@pytest.mark.parametrize(
    "agg", [{"func": ["mean", "sum"]***REMOVED***, {"mean": "mean", "sum": "sum"***REMOVED***]
)
...
@pytest.mark.parametrize(
    "agg",
    [
        {"A": {"mean": "mean", "sum": "sum"***REMOVED******REMOVED***,
        {
            "A": {"mean": "mean", "sum": "sum"***REMOVED***,
            "B": {"mean2": "mean", "sum2": "sum"***REMOVED***,
        ***REMOVED***,
    ],
)
...
def test_agg_dict_of_lists(cases, a_mean, a_std, b_mean, b_std):
    ...
    result = cases.aggregate({"A": ["mean", "std"], "B": ["mean", "std"]***REMOVED***)
    tm.assert_frame_equal(result, expected, check_like=True)
...
@pytest.mark.parametrize(
    "agg",
    [
        {"func": {"A": np.sum, "B": lambda x: np.std(x, ddof=1)***REMOVED******REMOVED***,
        {"A": ("A", np.sum), "B": ("B", lambda x: np.std(x, ddof=1))***REMOVED***,
        {"A": NamedAgg("A", np.sum), "B": NamedAgg("B", lambda x: np.std(x, ddof=1))***REMOVED***,
    ],
)
...
@pytest.mark.parametrize(
    "agg",
    [
        {"func": {"result1": np.sum, "result2": np.mean***REMOVED******REMOVED***,
        {"A": ("result1", np.sum), "B": ("result2", np.mean)***REMOVED***,
        {"A": NamedAgg("result1", np.sum), "B": NamedAgg("result2", np.mean)***REMOVED***,
    ],
)
...
@pytest.mark.parametrize(
    "cols, agg",
    [
        [None, {"A": ["sum", "std"], "B": ["mean", "std"]***REMOVED***],
        [
            [
                "A",
                "B",
            ],
            {"A": ["sum", "std"], "B": ["mean", "std"]***REMOVED***,
        ],
    ],
)
...
@pytest.mark.parametrize(
    "agg", [{"A": ["sum", "std"]***REMOVED***, {"A": ["sum", "std"], "B": ["mean", "std"]***REMOVED***]
)
...
def test_agg_specificationerror_invalid_names(cases):
    ...
    with pytest.raises(KeyError, match=msg):
        cases[["A"]].agg({"A": ["sum", "std"], "B": ["mean", "std"]***REMOVED***)


@pytest.mark.parametrize(
    "func", [["min"], ["mean", "max"], {"A": "sum"***REMOVED***, {"A": "prod", "B": "median"***REMOVED***]
)
...
def test_agg_nested_dicts():
    ...
    for t in cases:
        with pytest.raises(pd.errors.SpecificationError, match=msg):
            t.aggregate({"r1": {"A": ["mean", "sum"]***REMOVED***, "r2": {"B": ["mean", "sum"]***REMOVED******REMOVED***)

    for t in cases:
        with pytest.raises(pd.errors.SpecificationError, match=msg):
            t[["A", "B"]].agg(
                {"A": {"ra": ["mean", "std"]***REMOVED***, "B": {"rb": ["mean", "std"]***REMOVED******REMOVED***
***REMOVED***

        with pytest.raises(pd.errors.SpecificationError, match=msg):
            t.agg({"A": {"ra": ["mean", "std"]***REMOVED***, "B": {"rb": ["mean", "std"]***REMOVED******REMOVED***)


def test_try_aggregate_non_existing_column():
    # GH 16766
    data = [
        {"dt": datetime(2017, 6, 1, 0), "x": 1.0, "y": 2.0***REMOVED***,
        {"dt": datetime(2017, 6, 1, 1), "x": 2.0, "y": 2.0***REMOVED***,
        {"dt": datetime(2017, 6, 1, 2), "x": 3.0, "y": 1.5***REMOVED***,
    ]
    ...
    with pytest.raises(KeyError, match=msg):
        df.resample("30min").agg({"x": ["mean"], "y": ["median"], "z": ["sum"]***REMOVED***)


def test_agg_list_like_func_with_args():
    # 50624
    df = DataFrame(
        {"x": [1, 2, 3]***REMOVED***, index=date_range("2020-01-01", periods=3, freq="D")
    )
...
def test_selection_api_validation():
    ...
    df = DataFrame(
        {"date": index, "a": rng***REMOVED***,
        index=pd.MultiIndex.from_arrays([rng, index], names=["v", "d"]),
    )
    df_exp = DataFrame({"a": rng***REMOVED***, index=index)
...
@pytest.mark.parametrize(
    ...
    [
        ("sum", True, {"num": [25]***REMOVED***),
        ("sum", False, {"cat": ["cat_1cat_2"], "num": [25]***REMOVED***),
        ("sum", lib.no_default, {"cat": ["cat_1cat_2"], "num": [25]***REMOVED***),
        ("prod", True, {"num": [100]***REMOVED***),
        ("prod", False, "can't multiply sequence"),
        ("prod", lib.no_default, "can't multiply sequence"),
        ("min", True, {"num": [5]***REMOVED***),
        ("min", False, {"cat": ["cat_1"], "num": [5]***REMOVED***),
        ("min", lib.no_default, {"cat": ["cat_1"], "num": [5]***REMOVED***),
        ("max", True, {"num": [20]***REMOVED***),
        ("max", False, {"cat": ["cat_2"], "num": [20]***REMOVED***),
        ("max", lib.no_default, {"cat": ["cat_2"], "num": [20]***REMOVED***),
        ("first", True, {"num": [5]***REMOVED***),
        ("first", False, {"cat": ["cat_1"], "num": [5]***REMOVED***),
        ("first", lib.no_default, {"cat": ["cat_1"], "num": [5]***REMOVED***),
        ("last", True, {"num": [20]***REMOVED***),
        ("last", False, {"cat": ["cat_2"], "num": [20]***REMOVED***),
        ("last", lib.no_default, {"cat": ["cat_2"], "num": [20]***REMOVED***),
        ("mean", True, {"num": [12.5]***REMOVED***),
        ("mean", False, "Could not convert"),
        ("mean", lib.no_default, "Could not convert"),
        ("median", True, {"num": [12.5]***REMOVED***),
        ("median", False, r"Cannot convert \['cat_1' 'cat_2'\] to numeric"),
        ("median", lib.no_default, r"Cannot convert \['cat_1' 'cat_2'\] to numeric"),
        ("std", True, {"num": [10.606601717798213]***REMOVED***),
        ("std", False, "could not convert string to float"),
        ("std", lib.no_default, "could not convert string to float"),
        ("var", True, {"num": [112.5]***REMOVED***),
        ("var", False, "could not convert string to float"),
        ("var", lib.no_default, "could not convert string to float"),
        ("sem", True, {"num": [7.5]***REMOVED***),
...
def test_frame_downsample_method(method, numeric_only, expected_data):
    ...
    df = DataFrame({"cat": ["cat_1", "cat_2"], "num": [5, 20]***REMOVED***, index=index)
    resampled = df.resample("YE")
    if numeric_only is lib.no_default:
        kwargs = {***REMOVED***
    else:
        kwargs = {"numeric_only": numeric_only***REMOVED***

    func = getattr(resampled, method)
    if isinstance(expected_data, str):
        if method in ("var", "mean", "median", "prod"):
            klass = TypeError
            msg = re.escape(f"agg function failed [how->{method***REMOVED***,dtype->")
        else:
            klass = ValueError
            msg = expected_data
...
def test_series_downsample_method(method, numeric_only, expected_data):
    ...
    resampled = df.resample("YE")
    kwargs = {***REMOVED*** if numeric_only is lib.no_default else {"numeric_only": numeric_only***REMOVED***

    func = getattr(resampled, method)
    if numeric_only and numeric_only is not lib.no_default:
        msg = rf"Cannot use numeric_only=True with SeriesGroupBy\.{method***REMOVED***"
        with pytest.raises(TypeError, match=msg):
            func(**kwargs)
    elif method == "prod":
...
def test_args_kwargs_depr(method, raises):
    ...
    warn_msg = f"Passing additional args to DatetimeIndexResampler.{method***REMOVED***"
