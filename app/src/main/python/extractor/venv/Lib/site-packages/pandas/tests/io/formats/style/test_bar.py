...
def bar_grad(a=None, b=None, c=None, d=None):
    ...
    return ret + [
        (
            "background",
            f"linear-gradient(90deg,{','.join([x for x in [a, b, c, d] if x])***REMOVED***)",
        )
    ]
...
def bar_to(x, color="#d65f5f"):
    return bar_grad(f" {color***REMOVED*** {x:.1f***REMOVED***%", f" transparent {x:.1f***REMOVED***%")


def bar_from_to(x, y, color="#d65f5f"):
    return bar_grad(
        f" transparent {x:.1f***REMOVED***%",
        f" {color***REMOVED*** {x:.1f***REMOVED***%",
        f" {color***REMOVED*** {y:.1f***REMOVED***%",
        f" transparent {y:.1f***REMOVED***%",
    )
...
def test_align_positive_cases(df_pos, align, exp):
    ...
    expected = {(0, 0): exp[0], (1, 0): exp[1], (2, 0): exp[2]***REMOVED***
...
def test_align_mixed_cases(df_mix, align, exp, nans):
    ...
    if nans:
        df_mix.loc[3, :] = np.nan
        expected.update({(3, 0): no_bar()***REMOVED***)
...
def test_vmin_vmax_clipping(df_pos, df_neg, df_mix, values, vmin, vmax, nullify, align):
    ...
    if align == "mid":  # mid acts as left or right in each case
        if values == "positive":
            ...
            align = "right"
    df = {"positive": df_pos, "negative": df_neg, "mixed": df_mix***REMOVED***[values]
...
def test_colors_mixed(align, exp):
    ...
    assert result == {(0, 0): exp[0], (1, 0): exp[1]***REMOVED***
...
def test_bar_value_error_raises():
    df = DataFrame({"A": [-100, -60, -30, -20]***REMOVED***)

    msg = "`align` should be in {'left', 'right', 'mid', 'mean', 'zero'***REMOVED*** or"
...
def test_styler_bar_with_NA_values():
    df1 = DataFrame({"A": [1, 2, NA, 4]***REMOVED***)
