...
class XMLCorpusView(StreamBackedCorpusView):
    ...
    _XML_PIECE = re.compile(
        ...
        (?P<COMMENT>        <!--.*?-->              ***REMOVED***|
        (?P<CDATA>          <![CDATA[.*?]]>         ***REMOVED***|
        (?P<PI>             <\?.*?\?>               ***REMOVED***|
        (?P<DOCTYPE>        <!DOCTYPE\s+[^\[^>]*(\[[^\]]*])?\s*>)|
        # These are the ones we actually care about:
        (?P<EMPTY_ELT_TAG>  <\s*[^>/\?!\s][^>]*/\s*>***REMOVED***|
        (?P<START_TAG>      <\s*[^>/\?!\s][^>]*>    ***REMOVED***|
        (?P<END_TAG>        <\s*/[^>/\?!\s][^>]*>   ***REMOVED***""",
        re.DOTALL | re.VERBOSE,
    )
    ...
    def read_block(self, stream, tagspec=None, elt_handler=None):
        ...
        while elts == [] or elt_start is not None:
            ...
            for piece in self._XML_PIECE.finditer(xml_fragment):
                if self._DEBUG:
                    print("{:>25***REMOVED*** {***REMOVED***".format("/".join(context)[-20:], piece.group()))
                ...
                if piece.group("START_TAG"):
                    ...
                    if name != context[-1]:
                        raise ValueError(f"Unmatched tag <{context[-1]***REMOVED***>...</{name***REMOVED***>")
