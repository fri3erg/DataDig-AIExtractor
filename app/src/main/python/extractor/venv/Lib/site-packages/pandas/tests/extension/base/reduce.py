...
class BaseReduceTests:
    ...
    def check_reduce_frame(self, ser: pd.Series, op_name: str, skipna: bool):
        ...
        arr = ser.array
        df = pd.DataFrame({"a": arr***REMOVED***)

        kwargs = {"ddof": 1***REMOVED*** if op_name in ["var", "std"] else {***REMOVED***
...
***REMOVED***
    ...
    def test_reduce_frame(self, data, all_numeric_reductions, skipna):
        ...
        if not is_numeric_dtype(ser.dtype):
            pytest.skip(f"{ser.dtype***REMOVED*** is not numeric dtype")

        if op_name in ["count", "kurt", "sem"]:
            pytest.skip(f"{op_name***REMOVED*** not an array method")

        if not self._supports_reduction(ser, op_name):
            pytest.skip(f"Reduction {op_name***REMOVED*** not supported for this dtype")
