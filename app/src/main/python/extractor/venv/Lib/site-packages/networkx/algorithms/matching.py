...
def matching_dict_to_set(matching):
    ...
    for edge in matching.items():
        ...
        if (v, u) in edges or edge in edges:
            continue
        if u == v:
            raise nx.NetworkXError(f"Selfloops cannot appear in matchings {edge***REMOVED***")
        edges.add(edge)
    return edges
...
def is_matching(G, matching):
    ...
    --------
    >>> G = nx.Graph([(1, 2), (1, 3), (2, 3), (2, 4), (3, 5), (4, 5)])
    >>> nx.is_maximal_matching(G, {1: 3, 2: 4***REMOVED***)  # using dict to represent matching
    True

    >>> nx.is_matching(G, {(1, 3), (2, 4)***REMOVED***)  # using set to represent matching
    True
    ...
    for edge in matching:
        if len(edge) != 2:
            raise nx.NetworkXError(f"matching has non-2-tuple edge {edge***REMOVED***")
        u, v = edge
        if u not in G or v not in G:
            raise nx.NetworkXError(f"matching contains edge {edge***REMOVED*** with node not in G")
        if u == v:
            return False
...
def is_maximal_matching(G, matching):
    ...
    >>> nx.is_maximal_matching(G, {(1, 2), (3, 4)***REMOVED***)
    True
