...
class TestAstype:
    ...
    def test_astype_str(self):
        ...
        df = DataFrame({"a": a, "b": b, "c": c, "d": d, "e": e***REMOVED***)
    ...
    def test_astype_dict_like(self, dtype_class):
        ...
        df = DataFrame({"a": a, "b": b, "c": c, "d": d***REMOVED***)
        original = df.copy(deep=True)

        # change type of a subset of columns
        dt1 = dtype_class({"b": "str", "d": "float32"***REMOVED***)
        ...
        dt2 = dtype_class({"b": np.float32, "c": "float32", "d": np.float64***REMOVED***)
        ...
        # change all columns
        dt3 = dtype_class({"a": str, "b": str, "c": str, "d": str***REMOVED***)
        tm.assert_frame_equal(df.astype(dt3), df.astype(str))
        tm.assert_frame_equal(df, original)

        # error should be raised when using something other than column labels
        # in the keys of the dtype dict
        dt4 = dtype_class({"b": str, 2: str***REMOVED***)
        dt5 = dtype_class({"e": str***REMOVED***)
        msg_frame = (
            "Only a column name can be used for the key in a dtype mappings argument. "
            "'{***REMOVED***' not found in columns."
        )
        ...
        dt6 = dtype_class({col: df[col].dtype for col in df.columns***REMOVED***)
        equiv = df.astype(dt6)
        ...
        dt7 = dtype_class({***REMOVED***) if dtype_class is dict else dtype_class({***REMOVED***, dtype=object)
        equiv = df.astype(dt7)
        tm.assert_frame_equal(df, equiv)
        tm.assert_frame_equal(df, original)
    ...
    def test_astype_duplicate_col(self):
        ...
        result = df.astype({"a": "str"***REMOVED***)
    ...
    def test_astype_categorical(self, dtype):
        # GH#18099
        d = {"A": list("abbc"), "B": list("bccd"), "C": list("cdde")***REMOVED***
        df = DataFrame(d)
        result = df.astype(dtype)
        expected = DataFrame({k: Categorical(v, dtype=dtype) for k, v in d.items()***REMOVED***)
        tm.assert_frame_equal(result, expected)

    @pytest.mark.parametrize("cls", [CategoricalDtype, DatetimeTZDtype, IntervalDtype])
    def test_astype_categoricaldtype_class_raises(self, cls):
        df = DataFrame({"A": ["a", "a", "b", "c"]***REMOVED***)
        xpr = f"Expected an instance of {cls.__name__***REMOVED***"
        with pytest.raises(TypeError, match=xpr):
            df.astype({"A": cls***REMOVED***)
    ...
    def test_astype_extension_dtypes(self, dtype):
        ...
        expected2 = DataFrame(
            {"a": [1.0, 3.0, 5.0], "b": pd.array([2, 4, 6], dtype=dtype)***REMOVED***
        )
    ...
    def test_astype_extension_dtypes_1d(self, dtype):
        # GH#22578
        df = DataFrame({"a": [1.0, 2.0, 3.0]***REMOVED***)

        expected1 = DataFrame({"a": pd.array([1, 2, 3], dtype=dtype)***REMOVED***)
        tm.assert_frame_equal(df.astype(dtype), expected1)
        tm.assert_frame_equal(df.astype("int64").astype(dtype), expected1)

        df = DataFrame({"a": [1.0, 2.0, 3.0]***REMOVED***)
        df["a"] = df["a"].astype(dtype)
        expected2 = DataFrame({"a": pd.array([1, 2, 3], dtype=dtype)***REMOVED***)
        tm.assert_frame_equal(df, expected2)
    ...
    @pytest.mark.parametrize(
        "dtype", [{100: "float64", 200: "uint64"***REMOVED***, "category", "float64"]
    )
    ...
    def test_astype_from_datetimelike_to_object(self, dtype, unit):
        ...
        # GH#19223 / GH#12425
        dtype = f"{dtype***REMOVED***[{unit***REMOVED***]"
    ...
    def test_astype_to_datetime_unit(self, unit):
        ...
        if unit in ["ns", "us", "ms", "s"]:
            ...
            msg = rf"Cannot cast DatetimeArray to dtype datetime64\[{unit***REMOVED***\]"
    ...
    def test_astype_to_timedelta_unit(self, unit):
        ...
        if unit in ["us", "ms", "s"]:
            ...
            msg = (
                rf"Cannot convert from timedelta64\[s\] to timedelta64\[{unit***REMOVED***\]. "
                "Supported resolutions are 's', 'ms', 'us', 'ns'"
***REMOVED***
            with pytest.raises(ValueError, match=msg):
                df.astype(dtype)
    ...
    def test_astype_to_incorrect_datetimelike(self, unit):
        ...
        # GH#19224
        dtype = f"M8[{unit***REMOVED***]"
        other = f"m8[{unit***REMOVED***]"

        df = DataFrame(np.array([[1, 2, 3]], dtype=dtype))
        msg = "|".join(
            [
                # BlockManager path
                rf"Cannot cast DatetimeArray to dtype timedelta64\[{unit***REMOVED***\]",
                # ArrayManager path
                "cannot astype a datetimelike from "
                rf"\[datetime64\[ns\]\] to \[timedelta64\[{unit***REMOVED***\]\]",
            ]
        )
        with pytest.raises(TypeError, match=msg):
            df.astype(other)
        ...
        msg = "|".join(
            [
                # BlockManager path
                rf"Cannot cast TimedeltaArray to dtype datetime64\[{unit***REMOVED***\]",
                # ArrayManager path
                "cannot astype a timedelta from "
                rf"\[timedelta64\[ns\]\] to \[datetime64\[{unit***REMOVED***\]\]",
            ]
        )
    ...
    def test_astype_invalid_conversion(self):
        # GH#47571
        df = DataFrame({"a": [1, 2, "text"], "b": [1, 2, 3]***REMOVED***)
        ...
        with pytest.raises(ValueError, match=re.escape(msg)):
            df.astype({"a": int***REMOVED***)

    def test_astype_arg_for_errors_dictlist(self):
        # GH#25905
        df = DataFrame(
            [
                {"a": "1", "b": "16.5%", "c": "test"***REMOVED***,
                {"a": "2.2", "b": "15.3", "c": "another_test"***REMOVED***,
            ]
        )
        expected = DataFrame(
            [
                {"a": 1.0, "b": "16.5%", "c": "test"***REMOVED***,
                {"a": 2.2, "b": "15.3", "c": "another_test"***REMOVED***,
            ]
        )
        expected["c"] = expected["c"].astype("object")
        type_dict = {"a": "float64", "b": "float64", "c": "object"***REMOVED***
...
***REMOVED***,
    ...
    def test_astype_tz_conversion(self):
        # GH 35973
        val = {"tz": date_range("2020-08-30", freq="d", periods=2, tz="Europe/London")***REMOVED***
        df = DataFrame(val)
        result = df.astype({"tz": "datetime64[ns, Europe/Berlin]"***REMOVED***)
    ...
    def test_astype_tz_object_conversion(self, tz):
        ...
        result = expected.astype({"tz": f"datetime64[ns, {tz***REMOVED***]"***REMOVED***)
        result = result.astype({"tz": "object"***REMOVED***)

        # do real test: object dtype to a specified tz, different from construction tz.
        result = result.astype({"tz": "datetime64[ns, Europe/London]"***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_astype_retain_attrs(self, any_numpy_dtype):
        # GH#44414
        df = DataFrame({"a": [0, 1, 2], "b": [3, 4, 5]***REMOVED***)
        df.attrs["Location"] = "Michigan"

        result = df.astype({"a": any_numpy_dtype***REMOVED***).attrs
        expected = df.attrs
...
class TestAstypeCategorical:
    def test_astype_from_categorical3(self):
        df = DataFrame({"cats": [1, 2, 3, 4, 5, 6], "vals": [1, 2, 3, 4, 5, 6]***REMOVED***)
        cats = Categorical([1, 2, 3, 4, 5, 6])
        exp_df = DataFrame({"cats": cats, "vals": [1, 2, 3, 4, 5, 6]***REMOVED***)
        df["cats"] = df["cats"].astype("category")
        tm.assert_frame_equal(exp_df, df)

    def test_astype_from_categorical4(self):
        df = DataFrame(
            {"cats": ["a", "b", "b", "a", "a", "d"], "vals": [1, 2, 3, 4, 5, 6]***REMOVED***
        )
    ...
    def test_categorical_astype_to_int(self, any_int_dtype):
        ...
        df = DataFrame(data={"col1": pd.array([2.0, 1.0, 3.0])***REMOVED***)
        df.col1 = df.col1.astype("category")
        df.col1 = df.col1.astype(any_int_dtype)
        expected = DataFrame({"col1": pd.array([2, 1, 3], dtype=any_int_dtype)***REMOVED***)
...
def test_frame_astype_no_copy():
    # GH 42501
    df = DataFrame({"a": [1, 4, None, 5], "b": [6, 7, 8, 9]***REMOVED***, dtype=object)
    result = df.astype({"a": Int16DtypeNoCopy()***REMOVED***, copy=False)
...
def test_astype_copies(dtype):
    ...
    df = DataFrame({"a": [1, 2, 3]***REMOVED***, dtype=dtype)
    result = df.astype("int64[pyarrow]", copy=True)
    df.iloc[0, 0] = 100
    expected = DataFrame({"a": [1, 2, 3]***REMOVED***, dtype="int64[pyarrow]")
    tm.assert_frame_equal(result, expected)
...
def test_astype_to_string_not_modifying_input(string_storage, val):
    # GH#51073
    df = DataFrame({"a": ["a", "b", val]***REMOVED***)
    expected = df.copy()
