...
***REMOVED***
        ...
        if cookies is None:
            cookies = {***REMOVED***
        cookies.update({"BbRouter": bbrouter***REMOVED***)
...
***REMOVED***
    ...
    def load(self) -> List[Document]:
        ...
        if self.load_all_recursively:
            ...
            for path in relative_paths:
                ...
                print(f"Fetching documents from {url***REMOVED***")
                soup_info = self._scrape(url)
                with contextlib.suppress(ValueError):
                    documents.extend(self._get_documents(soup_info))
            return documents
        else:
            print(f"Fetching documents from {self.web_path***REMOVED***")
            soup_info = self.scrape()
            self.folder_path = self._get_folder_path(soup_info)
    ...
    def _get_folder_path(self, soup: Any) -> str:
        ...
        """
        # Get the course name
        course_name = soup.find("span", {"id": "crumb_1"***REMOVED***)
    ...
    def _get_attachments(self, soup: Any) -> List[str]:
        ...
        # Get content list
        content_list = soup.find("ul", {"class": "contentList"***REMOVED***)
        if content_list is None:
            raise ValueError("No content list found.")
        content_list: BeautifulSoup  # type: ignore
        # Get all attachments
        attachments = []
        for attachment in content_list.find_all("ul", {"class": "attachments"***REMOVED***):
    ...
    def _get_paths(self, soup: Any) -> List[str]:
        ...
        relative_paths = []
        course_menu = soup.find("ul", {"class": "courseMenu"***REMOVED***)
    ...
    def _parse_filename_from_url(self, url: str) -> str:
        ...
        if filename_matches:
            ...
            raise ValueError(f"Could not parse filename from {url***REMOVED***")
        if ".pdf" not in filename:
            raise ValueError(f"Incorrect file type: {filename***REMOVED***")
...
***REMOVED***
    ...
    documents = loader.load()
    print(f"Loaded {len(documents)***REMOVED*** pages of PDFs from {loader.web_path***REMOVED***")
