...
DEFAULT_DOCUMENT_PROMPT = PromptTemplate.from_template("{page_content***REMOVED***")


def _validate_prompt(prompt: BasePromptTemplate) -> None:
    if DOCUMENTS_KEY not in prompt.input_variables:
        raise ValueError(
            f"Prompt must accept {DOCUMENTS_KEY***REMOVED*** as an input variable. Received prompt "
            f"with input variables: {prompt.input_variables***REMOVED***"
        )
...
class BaseCombineDocumentsChain(Chain, ABC):
    ...
    def get_input_schema(
        ...
        return create_model(
            ...
            **{self.input_key: (List[Document], None)***REMOVED***,  # type: ignore[call-overload]
        )
    ...
    def get_output_schema(
        ...
        return create_model(
            ...
            **{self.output_key: (str, None)***REMOVED***,  # type: ignore[call-overload]
        )
    ...
    def _call(
        ...
        other_keys = {k: v for k, v in inputs.items() if k != self.input_key***REMOVED***
...
class AnalyzeDocumentChain(Chain):
    ...
    def get_input_schema(
        ...
        return create_model(
            ...
            **{self.input_key: (str, None)***REMOVED***,  # type: ignore[call-overload]
        )
    ...
    def _call(
        ...
        # Other keys are assumed to be needed for LLM prediction
        other_keys: Dict = {k: v for k, v in inputs.items() if k != self.input_key***REMOVED***
