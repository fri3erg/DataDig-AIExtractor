...
def regular():
    return DataFrame(
        {"A": date_range("20130101", periods=5, freq="s"), "B": range(5)***REMOVED***
    ).set_index("A")


@pytest.fixture
def ragged():
    df = DataFrame({"B": range(5)***REMOVED***)
    df.index = [
...
class TestRollingTS:
    ...
    def test_frame_on(self):
        df = DataFrame(
            {"B": range(5), "C": date_range("20130101 09:00:00", periods=5, freq="3s")***REMOVED***
        )
    ...
    def test_frame_on2(self, unit):
        ...
        expected1 = DataFrame(
            {"A": [0.0, 1, 3, 3, 7], "B": [0, 1, 3, np.nan, 4], "C": df["C"]***REMOVED***,
            columns=["A", "C", "B"],
        )
    ...
    def test_regular_min(self):
        df = DataFrame(
            {"A": date_range("20130101", periods=5, freq="s"), "B": [0.0, 1, 2, 3, 4]***REMOVED***
        ).set_index("A")
        ...
        df = DataFrame(
            {"A": date_range("20130101", periods=5, freq="s"), "B": [5, 4, 3, 4, 5]***REMOVED***
        ).set_index("A")
    ...
    def test_perf_min(self):
        ...
        dfp = DataFrame(
            {"B": np.random.default_rng(2).standard_normal(N)***REMOVED***,
    ...
    def test_freqs_ops(self, freq, op, result_data):
        # GH 21096
        index = date_range(start="2018-1-1 01:00:00", freq=f"1{freq***REMOVED***", periods=10)
        # Explicit cast to float to avoid implicit cast when setting nan
        s = Series(data=0, index=index, dtype="float")
        s.iloc[1] = np.nan
        s.iloc[-1] = 2
        result = getattr(s.rolling(window=f"10{freq***REMOVED***"), op)()
    ...
    def test_all2(self, arithmetic_win_operators):
        ...
        df = DataFrame(
            {"B": np.arange(50)***REMOVED***, index=date_range("20130101", periods=50, freq="h")
        )
        # in-range data
    ...
    def test_rolling_on_decreasing_index(self, unit):
        ...
        df = DataFrame({"column": [3, 4, 4, 5, 6]***REMOVED***, index=index)
        result = df.rolling("5s").min()
        expected = DataFrame({"column": [3.0, 3.0, 4.0, 4.0, 6.0]***REMOVED***, index=index)
        tm.assert_frame_equal(result, expected)

    def test_rolling_on_empty(self):
        # GH-32385
        df = DataFrame({"column": []***REMOVED***, index=[])
        result = df.rolling("5s").min()
        expected = DataFrame({"column": []***REMOVED***, index=[])
        tm.assert_frame_equal(result, expected)

    def test_rolling_on_multi_index_level(self):
        # GH-15584
        df = DataFrame(
            {"column": range(6)***REMOVED***,
...
***REMOVED***,
        ...
        expected = DataFrame(
            {"column": [0.0, 1.0, 3.0, 6.0, 10.0, 15.0]***REMOVED***, index=df.index
        )
        tm.assert_frame_equal(result, expected)
...
def test_nat_axis_error(msg, axis):
    ...
    kwargs = {"columns" if axis == 1 else "index": idx***REMOVED***
    ...
    with pytest.raises(ValueError, match=f"{msg***REMOVED*** values must not have NaT"):
