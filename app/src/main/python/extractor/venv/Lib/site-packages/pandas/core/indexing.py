...
class IndexingMixin:
    ...
    def iloc(self) -> _iLocIndexer:
        ...
        Examples
        --------
        >>> mydict = [{'a': 1, 'b': 2, 'c': 3, 'd': 4***REMOVED***,
        ...           {'a': 100, 'b': 200, 'c': 300, 'd': 400***REMOVED***,
        ...           {'a': 1000, 'b': 2000, 'c': 3000, 'd': 4000***REMOVED***]
        ...
        for i, k in enumerate(key):
            try:
                ...
                raise ValueError(
                    ...
                    f"[{self._valid_types***REMOVED***] types"
    ***REMOVED*** from err
        return key
...
class _LocIndexer(_LocationIndexer):
    ...
    def _validate_key(self, key, axis: Axis):
        ...
        if isinstance(key, bool) and not (
            ...
            raise KeyError(
                f"{key***REMOVED***: boolean label can not be used without a boolean index"
***REMOVED***

        if isinstance(key, slice) and (
            isinstance(key.start, bool) or isinstance(key.stop, bool)
        ):
            raise TypeError(f"{key***REMOVED***: boolean values can not be used in a slice")
    ...
    def _getitem_iterable(self, key, axis: AxisInt):
        ...
        return self.obj._reindex_with_indexers(
            {axis: [keyarr, indexer]***REMOVED***, copy=True, allow_dups=True
        )
    ...
    ***REMOVED***
        ...
        if isinstance(key, slice):
            ...
            if not is_numeric_dtype(arr.dtype):
                raise IndexError(f".iloc requires numeric indexers, got {arr***REMOVED***")

            # check that the key does not exceed the maximum size of the index
            if len(arr) and (arr.max() >= len_axis or arr.min() < -len_axis):
                raise IndexError("positional indexers are out-of-bounds")
        else:
            raise ValueError(f"Can only index by location with a [{self._valid_types***REMOVED***]")
    ...
    ***REMOVED***:
        ...
        if isinstance(indexer, tuple):
            ...
            ***REMOVED***
                    ...
                    taker[-1] = -1
                    reindexers = {i: (labels, taker)***REMOVED***
    ...
    ***REMOVED***
                ...
                if dtype not in (np.void, object) and not self.obj.empty:
                    ...
                    warnings.warn(
                        ...
                        f"Value '{value***REMOVED***' has dtype incompatible with {dtype***REMOVED***, "
