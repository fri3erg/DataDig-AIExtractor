...
def langname(tag, typ="full"):
    ...
    if codepattern.fullmatch(code):
        if code in iso639retired:  # retired codes
            ...
            warn(f"Shortening {code!r***REMOVED*** to {code2!r***REMOVED***", stacklevel=2)
        ...
        warn(f"Could not find code in {code!r***REMOVED***", stacklevel=2)
...
def langcode(name, typ=2):
    ...
    if name in bcp47.langcode:
        ...
        warn(f"Could not find language in {name!r***REMOVED***", stacklevel=2)
...
def inverse_dict(dic):
    ...
    if len(dic.keys()) == len(set(dic.values())):
        return {val: key for (key, val) in dic.items()***REMOVED***
    else:
