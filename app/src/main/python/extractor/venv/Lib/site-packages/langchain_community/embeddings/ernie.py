...
class ErnieEmbeddings(BaseModel, Embeddings):
    ...
    def _embedding(self, json: object) -> dict:
        base_url = (
            f"{self.ernie_api_base***REMOVED***/rpc/2.0/ai_custom/v1/wenxinworkshop/embeddings"
        )
        resp = requests.post(
            f"{base_url***REMOVED***/embedding-v1",
            headers={
...
***REMOVED***
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        for chunk in text_in_chunks:
            resp = self._embedding({"input": [text for text in chunk]***REMOVED***)
            if resp.get("error_code"):
                if resp.get("error_code") == 111:
                    self._refresh_access_token_with_lock()
                    resp = self._embedding({"input": [text for text in chunk]***REMOVED***)
                else:
                    raise ValueError(f"Error from Ernie: {resp***REMOVED***")
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        if not self.access_token:
            self._refresh_access_token_with_lock()
        resp = self._embedding({"input": [text]***REMOVED***)
        if resp.get("error_code"):
            if resp.get("error_code") == 111:
                self._refresh_access_token_with_lock()
                resp = self._embedding({"input": [text]***REMOVED***)
