...
def test_mi_hashtable_populated_attribute_error(monkeypatch):
    ...
    r = range(50)
    df = pd.DataFrame({"a": r, "b": r***REMOVED***, index=MultiIndex.from_arrays([r, r]))
...
def test_rangeindex_fallback_coercion_bug():
    ...
    df = pd.concat(
        {"df1": df1.stack(future_stack=True), "df2": df2.stack(future_stack=True)***REMOVED***,
        axis=1,
    )
    df.index.names = ["fizz", "buzz"]

    expected = pd.DataFrame(
        {"df2": np.arange(100), "df1": np.arange(100)***REMOVED***,
