...
class TestDataFrameCov:
    ...
    def test_cov_ddof(self, test_ddof):
        ...
        df = DataFrame({0: np_array1, 1: np_array2***REMOVED***)
    ...
    def test_cov_nullable_integer(self, other_column):
        ...
        data = DataFrame({"a": pd.array([1, 2, None]), "b": other_column***REMOVED***)
        result = data.cov()
    ...
    def test_cov_numeric_only(self, numeric_only):
        ...
        df = DataFrame({"a": [1, 0], "c": ["x", "y"]***REMOVED***)
...
class TestDataFrameCorr:
    ...
    def test_corr_int_and_boolean(self, meth):
        ...
        # so it need to be properly handled
        pytest.importorskip("scipy")
        df = DataFrame({"a": [True, False], "b": [1, 0]***REMOVED***)
    ...
    def test_corr_int(self):
        ...
        df = DataFrame({"a": [1, 2, 3, 4], "b": [1, 2, 3, 4]***REMOVED***)
    ...
    def test_corr_nullable_integer(self, nullable_column, other_column, method):
        # https://github.com/pandas-dev/pandas/issues/33803
        pytest.importorskip("scipy")
        data = DataFrame({"a": nullable_column, "b": other_column***REMOVED***)
    ...
    def test_corr_for_constant_columns(self, length):
        ...
        result = df.corr()
        expected = DataFrame(
            {"A": [np.nan, np.nan], "B": [np.nan, np.nan]***REMOVED***, index=["A", "B"]
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_calc_corr_small_numbers(self):
        # GH: 37452
        df = DataFrame(
            {"A": [1.0e-20, 2.0e-20, 3.0e-20], "B": [1.0e-20, 2.0e-20, 3.0e-20]***REMOVED***
        )
        result = df.corr()
        expected = DataFrame({"A": [1.0, 1.0], "B": [1.0, 1.0]***REMOVED***, index=["A", "B"])
        tm.assert_frame_equal(result, expected)
    ...
    def test_corr_numeric_only(self, meth, numeric_only):
        ...
        # so it need to be properly handled
        pytest.importorskip("scipy")
        df = DataFrame({"a": [1, 0], "b": [1, 0], "c": ["x", "y"]***REMOVED***)
        expected = DataFrame(np.ones((2, 2)), index=["a", "b"], columns=["a", "b"])
        if numeric_only:
...
class TestDataFrameCorrWith:
    ...
    def test_corrwith_mixed_dtypes(self, numeric_only):
        # GH#18570
        df = DataFrame(
            {"a": [1, 4, 3, 2], "b": [4, 6, 7, 3], "c": ["a", "b", "c", "d"]***REMOVED***
        )
        s = Series([0, 6, 7, 3])
        if numeric_only:
...
***REMOVED***:
    ...
    def test_corr_numerical_instabilities(self):
        ...
        result = df.corr()
        expected = DataFrame({0: [1.0, -1.0], 1: [-1.0, 1.0]***REMOVED***)
    ...
    def test_corrwith_spearman_with_tied_data(self):
        ...
        df_bool = DataFrame(
            {"A": [True, True, False, False], "B": [True, False, False, True]***REMOVED***
        )
