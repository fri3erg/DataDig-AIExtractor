...
class TestDataFrameDrop:
    ...
    def test_drop(self):
        simple = DataFrame({"A": [1, 2, 3, 4], "B": [0, 1, 2, 3]***REMOVED***)
    ...
    def test_drop_empty_listlike_non_unique_datetime_index(self, empty_listlike):
        # GH#27994
        data = {"column_a": [5, 10], "column_b": ["one", "two"]***REMOVED***
...
***REMOVED***,
    ...
    def test_inplace_drop_and_operation(self, operation, inplace):
        # GH#30484
        df = DataFrame({"x": range(5)***REMOVED***)
        expected = df.copy()
        df["y"] = range(5)
        y = df["y"]
    ...
    def test_drop_tuple_with_non_unique_multiindex(self, indexer):
        ...
        df = DataFrame({"x": range(len(idx))***REMOVED***, index=idx)
        result = df.drop(index=[("a", "a")])
        expected = DataFrame(
            {"x": [2, 3]***REMOVED***, index=MultiIndex.from_tuples([("b", "a"), ("b", "a")])
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_drop_inplace_no_leftover_column_reference(self):
        # GH 13934
        df = DataFrame({"a": [1, 2, 3]***REMOVED***, columns=Index(["a"], dtype="object"))
        a = df.a
    ...
    def test_drop_index_ea_dtype(self, any_numeric_ea_dtype, idx, level):
        # GH#45860
        df = DataFrame(
            {"a": [1, 2, 2, pd.NA], "b": 100***REMOVED***, dtype=any_numeric_ea_dtype
        ).set_index(idx)
        result = df.drop(Index([2, pd.NA]), level=level)
        expected = DataFrame(
            {"a": [1], "b": 100***REMOVED***, dtype=any_numeric_ea_dtype
        ).set_index(idx)
        tm.assert_frame_equal(result, expected)
    ...
    def test_drop_parse_strings_datetime_index(self):
        ...
        expected = DataFrame({"a": [2], "b": [2]***REMOVED***, index=[Timestamp("2000-01-04")])
        tm.assert_frame_equal(result, expected)
