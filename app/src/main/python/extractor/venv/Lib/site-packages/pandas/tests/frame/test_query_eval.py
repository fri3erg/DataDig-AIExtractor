...
def skip_if_no_pandas_parser(parser):
    if parser != "pandas":
        pytest.skip(f"cannot evaluate with parser={parser***REMOVED***")


class TestCompat:
    @pytest.fixture
    def df(self):
        return DataFrame({"A": [1, 2, 3]***REMOVED***)
...
class TestDataFrameEval:
    ...
    def test_ops(self, op_str, op, rop, n):
        ...
        expected = eval(f"base {op_str***REMOVED*** df")

        # ops as strings
        result = eval(f"m {op_str***REMOVED*** df")
    ...
    def test_dataframe_sub_numexpr_path(self):
        ...
        df = DataFrame({"A": np.random.default_rng(2).standard_normal(25000)***REMOVED***)
        df.iloc[0:5] = np.nan
    ...
    def test_query_non_str(self):
        # GH 11485
        df = DataFrame({"A": [1, 2, 3], "B": ["a", "b", "b"]***REMOVED***)
    ...
    def test_eval_object_dtype_binop(self):
        # GH#24883
        df = DataFrame({"a1": ["Y", "N"]***REMOVED***)
        res = df.eval("c = ((a1 == 'Y') & True)")
        expected = DataFrame({"a1": ["Y", "N"], "c": [True, False]***REMOVED***)
...
class TestDataFrameQueryNumExprPandas:
    ...
    def test_date_query_with_non_date(self, engine, parser):
        n = 10
        df = DataFrame(
            {"dates": date_range("1/1/2012", periods=n), "nondate": np.arange(n)***REMOVED***
        )
        ...
        for op in ["<", ">", "<=", ">="]:
            with pytest.raises(TypeError, match=msg):
                df.query(f"dates {op***REMOVED*** nondate", parser=parser, engine=engine)

    def test_query_syntax_error(self, engine, parser):
        df = DataFrame({"i": range(10), "+": range(3, 13), "r": range(4, 14)***REMOVED***)
        msg = "invalid syntax"
    ...
    def test_at_inside_string(self, engine, parser):
        skip_if_no_pandas_parser(parser)
        c = 1  # noqa: F841
        df = DataFrame({"a": ["a", "a", "b", "b", "@c", "@c"]***REMOVED***)
    ...
    def test_index_resolvers_come_after_columns_with_the_same_name(
        ...
        df = DataFrame(
            {"index": a, "b": np.random.default_rng(2).standard_normal(a.size)***REMOVED***
        )
        df.index.name = "index"
        ...
        df = DataFrame({"a": a, "b": np.random.default_rng(2).standard_normal(a.size)***REMOVED***)
        df.index.name = "a"
...
class TestDataFrameQueryNumExprPython(TestDataFrameQueryNumExprPandas):
    ...
    def test_query_numexpr_with_min_and_max_columns(self):
        df = DataFrame({"min": [1, 2, 3], "max": [4, 5, 6]***REMOVED***)
        regex_to_match = (
...
class TestDataFrameQueryStrings:
            ...
            for lhs, op, rhs in zip(lhs, ops, rhs):
                ...
                with pytest.raises(NotImplementedError, match=msg):
                    df.query(
                        ...
                        engine=engine,
                        parser=parser,
                        local_dict={"strings": df.strings***REMOVED***,
        ***REMOVED***
        else:
    ...
    def test_query_with_nested_strings(self, parser, engine):
        skip_if_no_pandas_parser(parser)
        events = [
            f"page {n***REMOVED*** {act***REMOVED***" for n in range(1, 4) for act in ["load", "exit"]
        ] * 2
    ...
    def test_query_with_nested_special_character(self, parser, engine):
        skip_if_no_pandas_parser(parser)
        df = DataFrame({"a": ["a", "b", "test & test"], "b": [1, 2, 3]***REMOVED***)
    ...
    def test_query_lex_compare_strings(
        ...
        df = DataFrame({"X": a, "Y": b***REMOVED***)

        warning = RuntimeWarning if using_infer_string and engine == "numexpr" else None
        with tm.assert_produces_warning(warning):
            res = df.query(f'X {op***REMOVED*** "d"', engine=engine, parser=parser)
    ...
    def test_query_string_null_elements(self, in_list):
        ...
        parser = "pandas"
        engine = "python"
        expected = {i: value for i, value in enumerate(in_list) if value == "asdf"***REMOVED***

        df_expected = DataFrame({"a": expected***REMOVED***, dtype="string")
        df_expected.index = df_expected.index.astype("int64")
        df = DataFrame({"a": in_list***REMOVED***, dtype="string")
...
class TestDataFrameEvalWithFrame:
    ...
    def test_invalid_type_for_operator_raises(self, parser, engine, op):
        df = DataFrame({"a": [1, 2], "b": ["c", "d"]***REMOVED***)
        msg = r"unsupported operand type\(s\) for .+: '.+' and '.+'|Cannot"

        with pytest.raises(TypeError, match=msg):
            df.eval(f"a {op***REMOVED*** b", engine=engine, parser=parser)
...
class TestDataFrameQueryBacktickQuoting:
    ...
    def test_lots_of_operators_string(self, df):
        res = df.query("`  &^ :!€$?(***REMOVED*** >    <++*''  ` > 4")
        expect = df[df["  &^ :!€$?(***REMOVED*** >    <++*''  "] > 4]
        tm.assert_frame_equal(res, expect)
    ...
    def test_query_ea_dtypes(self, dtype):
        ...
        # GH#50261
        df = DataFrame({"a": Series([1, 2], dtype=dtype)***REMOVED***)
        ref = {2***REMOVED***  # noqa: F841
        warning = RuntimeWarning if dtype == "Int64" and NUMEXPR_INSTALLED else None
        with tm.assert_produces_warning(warning):
            result = df.query("a in @ref")
        expected = DataFrame({"a": Series([2], dtype=dtype, index=[1])***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_query_ea_equality_comparison(self, dtype, engine):
        ...
        if dtype == "int64[pyarrow]":
            pytest.importorskip("pyarrow")
        df = DataFrame(
            {"A": Series([1, 1, 2], dtype="Int64"), "B": Series([1, 2, 2], dtype=dtype)***REMOVED***
        )
        with tm.assert_produces_warning(warning):
