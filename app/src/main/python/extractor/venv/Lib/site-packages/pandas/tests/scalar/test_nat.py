...
def test_nat_methods_raise(method):
    ...
    msg = f"NaTType does not support {method***REMOVED***"
...
@pytest.mark.parametrize(
    ...
    ids=lambda x: f"{x[0].__name__***REMOVED***.{x[1]***REMOVED***",
)
def test_nat_doc_strings(compare):
    ...
    if method == "to_numpy":
        ...
        pytest.skip(f"different docstring for {method***REMOVED*** is intentional")
...
def test_nat_arithmetic_scalar(op_name, value, val_type):
    ...
    invalid_ops = {
        ...
        ***REMOVED***,
        "str": set(_ops.keys()),
        "timedelta": {"left_times_right", "right_times_left"***REMOVED***,
        "timestamp": {
            "left_times_right",
            "right_times_left",
            "left_div_right",
    ...
    ***REMOVED***
    ...
    if op_name in invalid_ops.get(val_type, set()):
        if (
            ...
            msg = rf"unsupported operand type\(s\) for \*: '{typs***REMOVED***' and '{typs***REMOVED***'"
        elif val_type == "str":
...
def test_nat_comparisons_invalid(other_and_type, symbol_and_op):
    ...
    msg = f"'{symbol***REMOVED***' not supported between instances of 'NaTType' and '{other_type***REMOVED***'"
    with pytest.raises(TypeError, match=msg):
        op(NaT, other)

    msg = f"'{symbol***REMOVED***' not supported between instances of '{other_type***REMOVED***' and 'NaTType'"
    with pytest.raises(TypeError, match=msg):
        op(other, NaT)
...
def test_nat_comparisons_invalid_ndarray(other):
    ...
    for symbol, op in [
        ...
        msg = f"'{symbol***REMOVED***' not supported between"
