...
def _assert_not_almost_equal(a, b, **kwargs):
    ...
    **kwargs
    ...
    try:
        tm.assert_almost_equal(a, b, **kwargs)
        msg = f"{a***REMOVED*** and {b***REMOVED*** were approximately equal when they shouldn't have been"
...
def test_assert_almost_equal_sets():
    # GH#51727
    _assert_almost_equal_both({1, 2, 3***REMOVED***, {1, 2, 3***REMOVED***)


def test_assert_almost_not_equal_sets():
    # GH#51727
    msg = r"{1, 2, 3***REMOVED*** != {1, 2, 4***REMOVED***"
    with pytest.raises(AssertionError, match=msg):
        _assert_almost_equal_both({1, 2, 3***REMOVED***, {1, 2, 4***REMOVED***)


def test_assert_almost_equal_dicts():
    _assert_almost_equal_both({"a": 1, "b": 2***REMOVED***, {"a": 1, "b": 2***REMOVED***)


@pytest.mark.parametrize(
    "a,b",
    [
        ({"a": 1, "b": 2***REMOVED***, {"a": 1, "b": 3***REMOVED***),
        ({"a": 1, "b": 2***REMOVED***, {"a": 1, "b": 2, "c": 3***REMOVED***),
        ({"a": 1***REMOVED***, 1),
        ({"a": 1***REMOVED***, "abc"),
        ({"a": 1***REMOVED***, [1]),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DataFrame({"a": [1.0, 1.1]***REMOVED***), DataFrame({"a": [1.0, 1.100001]***REMOVED***)),
    ],
)
...
NESTED_CASES = [
    ...
    ***REMOVED***,
    ...
    # dicts
    (
        np.array([{"f1": 1, "f2": np.array(["a", "b"], dtype=object)***REMOVED***], dtype=object),
        np.array([{"f1": 1, "f2": np.array(["a", "b"], dtype=object)***REMOVED***], dtype=object),
    ),
    (
        np.array([{"f1": 1, "f2": np.array(["a", "b"], dtype=object)***REMOVED***], dtype=object),
        np.array([{"f1": 1, "f2": ["a", "b"]***REMOVED***], dtype=object),
    ),
    # array/list of dicts
    (
        np.array(
            [
                np.array(
                    [{"f1": 1, "f2": np.array(["a", "b"], dtype=object)***REMOVED***], dtype=object
    ***REMOVED***,
