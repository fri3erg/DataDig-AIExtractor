...
doc:degrees="180" doc:sides="3.0"/>
    ...
    output = geom_df.to_xml(
        ...
        namespaces={"doc": "http://example.xom"***REMOVED***,
        prefix="doc",
        parser=parser,
    )
    output = equalize_decl(output)
...
<data xmlns="http://example.com">
  ...
  <row>
    ...
    output = geom_df.to_xml(namespaces={"": "http://example.com"***REMOVED***, parser=parser)
    output = equalize_decl(output)
...
<data xmlns:oth="http://other.org" xmlns:ex="http://example.com">
  ...
  <row>
    ...
    output = geom_df.to_xml(
        namespaces={"oth": "http://other.org", "ex": "http://example.com"***REMOVED***,
        parser=parser,
    )
    output = equalize_decl(output)
...
<doc:data xmlns:doc="http://example.com">
  ...
  <doc:row>
    ...
    output = geom_df.to_xml(
        namespaces={"doc": "http://example.com"***REMOVED***, prefix="doc", parser=parser
    )
    output = equalize_decl(output)
...
def test_missing_prefix_in_nmsp(parser, geom_df):
    with pytest.raises(KeyError, match=("doc is not included in namespaces")):
        geom_df.to_xml(
            namespaces={"": "http://example.com"***REMOVED***, prefix="doc", parser=parser
        )
...
<doc:data xmlns:doc="http://other.org" xmlns="http://example.com">
  ...
  <doc:row>
    ...
    output = geom_df.to_xml(
        namespaces={"": "http://example.com", "doc": "http://other.org"***REMOVED***,
        prefix="doc",
        parser=parser,
    )
    output = equalize_decl(output)
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/data">
        ...
        <xsl:text>***REMOVED***,"degrees":{</xsl:text>
        <xsl:apply-templates select="descendant::row/degrees"/>
        <xsl:text>***REMOVED***,"sides":{</xsl:text>
        <xsl:apply-templates select="descendant::row/sides"/>
        <xsl:text>***REMOVED******REMOVED***</xsl:text>
    </xsl:template>
...
<data>
  <row>
    ...
    df = DataFrame({"a": [NA]***REMOVED***).astype(any_numeric_ea_dtype)
...
def test_s3_permission_output(parser, s3_public_bucket, geom_df):
    ...
    with tm.external_error_raised((PermissionError, FileNotFoundError)):
        ...
        geom_df.to_xml(
            f"s3://{s3_public_bucket.name***REMOVED***/geom.xml", compression="zip", parser=parser
        )
