...
class TestIteration:
    ...
    def test_iterrows_corner(self):
        ...
        expected = Series(
            [datetime.datetime(2015, 1, 1), None, None, "", [], set(), {***REMOVED***],
            index=list("abcdefg"),
            name=0,
            dtype="object",
        )
    ...
    def test_itertuples_index_false(self):
        df = DataFrame(
            {"floats": np.random.default_rng(2).standard_normal(5), "ints": range(5)***REMOVED***,
            columns=["floats", "ints"],
        )
...
***REMOVED***
    ...
    def test_itertuples_disallowed_col_labels(self):
        df = DataFrame(data={"def": [1, 2, 3], "return": [4, 5, 6]***REMOVED***)
    ...
    def test_itertuples_py2_3_field_limit_namedtuple(self, limit, index):
        # GH#28282
        df = DataFrame([{f"foo_{i***REMOVED***": f"bar_{i***REMOVED***" for i in range(limit)***REMOVED***])
    ...
    def test_sequence_like_with_categorical(self):
        ...
        df = DataFrame(
            {"id": [1, 2, 3, 4, 5, 6], "raw_grade": ["a", "b", "b", "a", "a", "e"]***REMOVED***
        )
