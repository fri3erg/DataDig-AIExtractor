...
def _find_typo(dictionary: Mapping[str, str], expected: str) -> None:
    for obj in dictionary:
        if difflib.SequenceMatcher(None, expected, obj).ratio() >= 0.8:
            warnings.warn(
                f"Found '{obj***REMOVED***' in pyproject.toml, did you mean '{expected***REMOVED***'?",
                TypoWarning,
                stacklevel=2,
***REMOVED***
...
def _validate_source_directory(source_dir: PathType) -> None:
    if not os.path.isdir(source_dir):
        msg = f'Source {source_dir***REMOVED*** is not a directory'
    ...
    if not os.path.exists(pyproject_toml) and not os.path.exists(setup_py):
        msg = f'Source {source_dir***REMOVED*** does not appear to be a Python project: no pyproject.toml or setup.py'
        raise BuildException(msg)
...
def _read_pyproject_toml(path: PathType) -> Mapping[str, Any]:
    try:
        ...
        msg = f"{e.strerror***REMOVED***: '{e.filename***REMOVED***' "
        raise BuildException(msg) from None
    except tomllib.TOMLDecodeError as e:
        msg = f'Failed to parse {path***REMOVED***: {e***REMOVED*** '
        raise BuildException(msg) from None
...
def _parse_build_system_table(pyproject_toml: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    unknown_props = build_system_table.keys() - {'requires', 'build-backend', 'backend-path'***REMOVED***
    if unknown_props:
        msg = f'Unknown properties: {", ".join(unknown_props)***REMOVED***'
        raise BuildSystemTableValidationError(msg)
...
def _wrap_subprocess_runner(runner: RunnerType, env: env.IsolatedEnv) -> RunnerType:
    def _invoke_wrapped_runner(cmd: Sequence[str], cwd: str | None, extra_environ: Mapping[str, str] | None) -> None:
        runner(cmd, cwd, {**(env.make_extra_environ() or {***REMOVED***), **(extra_environ or {***REMOVED***)***REMOVED***)
...
class ProjectBuilder:
    ...
    def get_requires_for_build(self, distribution: str, config_settings: ConfigSettingsType | None = None) -> set[str]:
        ...
        """
        self.log(f'Getting build dependencies for {distribution***REMOVED***...')
        hook_name = f'get_requires_for_build_{distribution***REMOVED***'
    ...
    def check_dependencies(self, distribution: str, config_settings: ConfigSettingsType | None = None) -> set[tuple[str, ...]]:
        ...
        return {u for d in dependencies for u in check_dependency(d)***REMOVED***
    ...
    def prepare(
        ...
        """
        self.log(f'Getting metadata for {distribution***REMOVED***...')
        try:
            return self._call_backend(
                f'prepare_metadata_for_build_{distribution***REMOVED***',
                output_directory,
                config_settings,
...
***REMOVED***
    ...
    def build(
        ...
        """
        self.log(f'Building {distribution***REMOVED***...')
        kwargs = {***REMOVED*** if metadata_directory is None else {'metadata_directory': metadata_directory***REMOVED***
        return self._call_backend(f'build_{distribution***REMOVED***', output_directory, config_settings, **kwargs)
    ...
    def metadata_path(self, output_directory: PathType) -> str:
        ...
        distinfo = f"{match['distribution']***REMOVED***-{match['version']***REMOVED***.dist-info"
        member_prefix = f'{distinfo***REMOVED***/'
    ...
    def _call_backend(
        ...
        if os.path.exists(outdir):
            if not os.path.isdir(outdir):
                msg = f"Build path '{outdir***REMOVED***' exists and is not a directory"
                raise BuildException(msg)
        else:
            os.makedirs(outdir)
    ...
    def _handle_backend(self, hook: str) -> Iterator[None]:
        try:
            ...
            raise BuildBackendException(
                exception,
                f"Backend '{self._backend***REMOVED***' is not available.",
                sys.exc_info(),
***REMOVED*** from None
        except subprocess.CalledProcessError as exception:
            raise BuildBackendException(exception, f'Backend subprocess exited when trying to invoke {hook***REMOVED***') from None
