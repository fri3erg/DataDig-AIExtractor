...
class Typed(Descriptor):
    ...
    def __init__(self, *args, **kw):
        ...
        self.__doc__ = f"Values must be of type {self.expected_type***REMOVED***"

    def __set__(self, instance, value):
        if not isinstance(value, self.expected_type):
            if (not self.allow_none
                or (self.allow_none and value is not None)):
                msg = f"{instance.__class__***REMOVED***.{self.name***REMOVED*** should be {self.expected_type***REMOVED*** but value is {type(value)***REMOVED***"
                if DEBUG:
                    msg = f"{instance.__class__***REMOVED***.{self.name***REMOVED*** should be {self.expected_type***REMOVED*** but {value***REMOVED*** is {type(value)***REMOVED***"
                raise TypeError(msg)
...
class Max(Convertible):
    ...
    def __set__(self, instance, value):
        if ((self.allow_none and value is not None)
            ...
            value = _convert(self.expected_type, value)
            if value > self.max:
                raise ValueError('Max value is {0***REMOVED***'.format(self.max))
...
class Min(Convertible):
    ...
    def __set__(self, instance, value):
        if ((self.allow_none and value is not None)
            ...
            value = _convert(self.expected_type, value)
            if value < self.min:
                raise ValueError('Min value is {0***REMOVED***'.format(self.min))
...
class Set(Descriptor):
    ...
    def __init__(self, name=None, **kw):
        ...
        self.__doc__ = "Value must be one of {0***REMOVED***".format(self.values)
...
class Length(Descriptor):
    ...
    def __set__(self, instance, value):
        if len(value) != self.length:
            raise ValueError("Value must be length {0***REMOVED***".format(self.length))
    ...
    def __set__(self, instance, value):
        ...
        if ((self.allow_none and value is not None)
            ...
            if not self.test_pattern.match(value):
                raise ValueError('Value does not match pattern {0***REMOVED***'.format(self.pattern))
