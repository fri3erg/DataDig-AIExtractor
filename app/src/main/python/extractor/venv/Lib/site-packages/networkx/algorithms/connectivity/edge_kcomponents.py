...
def k_edge_components(G, k):
    ...
    >>> # note this returns {1, 4***REMOVED*** unlike k_edge_subgraphs
...
def k_edge_subgraphs(G, k):
    ...
    >>> G.add_nodes_from(it.chain(*paths))
    >>> G.add_edges_from(it.chain(*[pairwise(path) for path in paths]))
    >>> # note this does not return {1, 4***REMOVED*** unlike k_edge_components
...
class EdgeComponentAuxGraph:
    ...
    def construct(EdgeComponentAuxGraph, G):
        ...
        arbitrary node t from N - {s***REMOVED***, and then computes the minimum st-cut
        ...
        def _recursive_build(H, A, source, avail):
            ...
            if {source***REMOVED*** == avail:
                return
            # pick an arbitrary node as the sink
            sink = arbitrary_element(avail - {source***REMOVED***)
...
def general_k_edge_subgraphs(G, k):
    ...
    R0 = {G.subgraph(cc).copy() for cc in find_ccs(G)***REMOVED***
