...
def test_filter_multi_column_df():
    df = DataFrame({"A": [1, 12, 12, 1], "B": [1, 1, 1, 1]***REMOVED***)
    grouper = df["A"].apply(lambda x: x % 2)
    grouped = df.groupby(grouper)
    expected = DataFrame({"A": [12, 12], "B": [1, 1]***REMOVED***, index=[1, 2])
    tm.assert_frame_equal(
...
def test_filter_mixed_df():
    df = DataFrame({"A": [1, 12, 12, 1], "B": "a b c d".split()***REMOVED***)
    grouper = df["A"].apply(lambda x: x % 2)
    grouped = df.groupby(grouper)
    expected = DataFrame({"A": [12, 12], "B": ["b", "c"]***REMOVED***, index=[1, 2])
...
def test_filter_out_all_groups_in_df():
    # GH12768
    df = DataFrame({"a": [1, 1, 2], "b": [1, 2, 0]***REMOVED***)
    res = df.groupby("a")
    res = res.filter(lambda x: x["b"].sum() > 5, dropna=False)
    expected = DataFrame({"a": [np.nan] * 3, "b": [np.nan] * 3***REMOVED***)
    tm.assert_frame_equal(expected, res)

    df = DataFrame({"a": [1, 1, 2], "b": [1, 2, 0]***REMOVED***)
    res = df.groupby("a")
    res = res.filter(lambda x: x["b"].sum() > 5, dropna=True)
    expected = DataFrame({"a": [], "b": []***REMOVED***, dtype="int64")
    tm.assert_frame_equal(expected, res)
...
def test_filter_bad_shapes():
    df = DataFrame({"A": np.arange(8), "B": list("aabbbbcc"), "C": np.arange(8)***REMOVED***)
    s = df["B"]
    g_df = df.groupby("B")
    g_s = s.groupby(s)
...
def test_filter_using_len():
    ...
    expected = DataFrame(
        {"A": np.arange(2, 6), "B": list("bbbb"), "C": np.arange(2, 6)***REMOVED***,
...
def test_filter_maintains_ordering():
    ...
    df = DataFrame(
        {"pid": [1, 1, 1, 2, 2, 3, 3, 3], "tag": [23, 45, 62, 24, 45, 34, 25, 62]***REMOVED***
    )
    s = df["pid"]
...
def test_filter_multiple_timestamp():
    ...
    expected = DataFrame({"A": [2, 8, 2, 8, 8]***REMOVED***)
    tm.assert_frame_equal(result, expected)

    result = grouped.transform(len)
    expected = DataFrame({"A": [2, 3, 2, 3, 3]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_filter_consistent_result_before_after_agg_func():
    # GH 17091
    df = DataFrame({"data": range(6), "key": list("ABCABC")***REMOVED***)
    grouper = df.groupby("key")
    result = grouper.filter(lambda x: True)
    expected = DataFrame({"data": range(6), "key": list("ABCABC")***REMOVED***)
    tm.assert_frame_equal(result, expected)
