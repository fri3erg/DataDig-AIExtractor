...
class TestDataFrameToDict:
    def test_to_dict_timestamp(self):
        ...
        test_data = DataFrame({"A": [tsmp, tsmp], "B": [tsmp, tsmp]***REMOVED***)
        test_data_mixed = DataFrame({"A": [tsmp, tsmp], "B": [1, 2]***REMOVED***)

        expected_records = [{"A": tsmp, "B": tsmp***REMOVED***, {"A": tsmp, "B": tsmp***REMOVED***]
        expected_records_mixed = [{"A": tsmp, "B": 1***REMOVED***, {"A": tsmp, "B": 2***REMOVED***]
    ...
    def test_to_dict_index_not_unique_with_index_orient(self):
        ...
        df = DataFrame({"a": [1, 2], "b": [0.5, 0.75]***REMOVED***, index=["A", "A"])
    ...
    @pytest.mark.parametrize(
        "orient,expected",
        [
            ("list", {"A": [2, 5], "B": [3, 6]***REMOVED***),
            ("dict", {"A": {0: 2, 1: 5***REMOVED***, "B": {0: 3, 1: 6***REMOVED******REMOVED***),
        ],
    )
    ...
    def test_to_dict_box_scalars(self, orient, item_getter):
        ...
        df = DataFrame({"a": [1, 2], "b": [0.1, 0.2]***REMOVED***)
    ...
    def test_to_dict_tz(self):
        ...
        expected = [
            {"d": Timestamp("2017-11-18 21:53:00.219225+0000", tz=pytz.utc)***REMOVED***,
            {"d": Timestamp("2017-11-18 22:06:30.061810+0000", tz=pytz.utc)***REMOVED***,
        ]
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                dict,
                {
                    0: {"int_col": 1, "float_col": 1.0***REMOVED***,
                    1: {"int_col": 2, "float_col": 2.0***REMOVED***,
                    2: {"int_col": 3, "float_col": 3.0***REMOVED***,
                ***REMOVED***,
***REMOVED***,
            (
                OrderedDict,
...
***REMOVED***,
    ...
    def test_to_dict_index_dtypes(self, into, expected):
        ...
        df = DataFrame({"int_col": [1, 2, 3], "float_col": [1.0, 2.0, 3.0]***REMOVED***)
    ...
    def test_to_dict_numeric_names(self):
        # GH#24940
        df = DataFrame({str(i): [i] for i in range(5)***REMOVED***)
    ...
    def test_to_dict_wide(self):
        # GH#24939
        df = DataFrame({(f"A_{i:d***REMOVED***"): [i] for i in range(256)***REMOVED***)
        result = df.to_dict("records")[0]
        expected = {f"A_{i:d***REMOVED***": i for i in range(256)***REMOVED***
        assert result == expected
    ...
    def test_to_dict_scalar_constructor_orient_dtype(self, data, expected_dtype):
        ...
        df = DataFrame({"a": data***REMOVED***, index=[0])
        d = df.to_dict(orient="records")
        result = type(d[0]["a"])
    ...
    def test_to_dict_mixed_numeric_frame(self):
        # GH 12859
        df = DataFrame({"a": [1.0], "b": [9.0]***REMOVED***)
        result = df.reset_index().to_dict("records")
        expected = [{"index": 0, "a": 1.0, "b": 9.0***REMOVED***]
        assert result == expected
...
***REMOVED***
        elif orient in {"split", "tight"***REMOVED***:
            assertion_iterator = (
...
***REMOVED***
    ...
    def test_to_dict_index_false_error(self, orient):
        # GH#46398
        df = DataFrame({"col1": [1, 2], "col2": [3, 4]***REMOVED***, index=["row1", "row2"])
    ...
    @pytest.mark.parametrize(
        "orient, expected",
        [
            ("split", {"columns": ["col1", "col2"], "data": [[1, 3], [2, 4]]***REMOVED***),
            (
                "tight",
                {
                    "columns": ["col1", "col2"],
...
***REMOVED***,
    ...
    @pytest.mark.parametrize(
        "orient, expected",
        [
            ("dict", {"a": {0: 1, 1: None***REMOVED******REMOVED***),
            ("list", {"a": [1, None]***REMOVED***),
            ("split", {"index": [0, 1], "columns": ["a"], "data": [[1], [None]]***REMOVED***),
            (
                "tight",
                {
                    "index": [0, 1],
                ...
                ***REMOVED***,
***REMOVED***,
            ("records", [{"a": 1***REMOVED***, {"a": None***REMOVED***]),
            ("index", {0: {"a": 1***REMOVED***, 1: {"a": None***REMOVED******REMOVED***),
        ],
    )
    def test_to_dict_na_to_none(self, orient, expected):
        # GH#50795
        df = DataFrame({"a": [1, NA]***REMOVED***, dtype="Int64")
        result = df.to_dict(orient=orient)
        assert result == expected

    def test_to_dict_masked_native_python(self):
        # GH#34665
        df = DataFrame({"a": Series([1, 2], dtype="Int64"), "B": 1***REMOVED***)
        result = df.to_dict(orient="records")
        assert isinstance(result[0]["a"], int)

        df = DataFrame({"a": Series([1, NA], dtype="Int64"), "B": 1***REMOVED***)
        result = df.to_dict(orient="records")
        assert isinstance(result[0]["a"], int)
