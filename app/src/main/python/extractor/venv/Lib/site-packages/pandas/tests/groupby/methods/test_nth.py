...
def test_first_last_with_na_object(method, nulls_fixture):
    ...
    groups = DataFrame({"a": [1, 1, 2, 2], "b": [1, 2, 3, nulls_fixture]***REMOVED***).groupby("a")
    ...
    expected = DataFrame({"b": values***REMOVED***, index=idx)
...
def test_nth_with_na_object(index, nulls_fixture):
    # https://github.com/pandas-dev/pandas/issues/32123
    df = DataFrame({"a": [1, 1, 2, 2], "b": [1, 2, 3, nulls_fixture]***REMOVED***)
    groups = df.groupby("a")
...
def test_first_last_with_None(method):
    ...
    df = DataFrame.from_dict({"id": ["a"], "value": [None]***REMOVED***)
...
@pytest.mark.parametrize(
    ...
    [
        (
            DataFrame({"id": "a", "value": [None, "foo", np.nan]***REMOVED***),
            DataFrame({"value": ["foo"]***REMOVED***, index=Index(["a"], name="id")),
        ),
        (
            DataFrame({"id": "a", "value": [np.nan]***REMOVED***, dtype=object),
            DataFrame({"value": [None]***REMOVED***, index=Index(["a"], name="id")),
        ),
    ],
)
...
def test_first_last_nth_nan_dtype():
    # GH 33591
    df = DataFrame({"data": ["A"], "nans": Series([None], dtype=object)***REMOVED***)
...
def test_nth2():
    ...
    df = DataFrame(
        {
            "color": {0: "green", 1: "green", 2: "red", 3: "red", 4: "red"***REMOVED***,
            "food": {0: "ham", 1: "eggs", 2: "eggs", 3: "ham", 4: "pork"***REMOVED***,
            "two": {
...
def test_first_last_extension_array_keeps_dtype(values, function):
    ...
    df = DataFrame({"a": [1, 2], "b": values***REMOVED***)
    grouped = df.groupby("a")
    idx = Index([1, 2], name="a")
    expected_series = Series(values, name="b", index=idx)
    expected_frame = DataFrame({"b": values***REMOVED***, index=idx)

    result_series = getattr(grouped["b"], function)()
    tm.assert_series_equal(result_series, expected_series)

    result_frame = grouped.agg({"b": function***REMOVED***)
...
def test_first_multi_key_groupby_categorical():
    ...
    df = df.astype({"D": "category"***REMOVED***)
...
def test_groupby_nth_interval():
    ...
    df_result = DataFrame({"col": range(len(idx_result))***REMOVED***, index=idx_result)
...
def test_groupby_nth_int_like_precision(data):
    ...
    grouped = df.groupby("a")
    result = grouped.nth(0)
    expected = DataFrame({"a": 1, "b": [data[0]]***REMOVED***)
