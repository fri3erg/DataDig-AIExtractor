...
class TestiLoc:
    ...
    def test_iloc_getitem_int_and_list_int(self, key, kind, col, request):
        obj = request.getfixturevalue(f"{kind***REMOVED***_{col***REMOVED***")
...
class TestiLocBaseIndependent:
    ...
    def test_iloc_setitem_fullcol_categorical(self, indexer, key, using_array_manager):
        frame = DataFrame({0: range(3)***REMOVED***, dtype=object)
        ...
        expected = DataFrame({0: cat***REMOVED***).astype(object)
        if not using_array_manager:
        ...
        frame = DataFrame({0: np.array([0, 1, 2], dtype=object), 1: range(3)***REMOVED***)
        df = frame.copy()
        indexer(df)[key, 0] = cat
        expected = DataFrame({0: Series(cat.astype(object), dtype=object), 1: range(3)***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_iloc_array_not_mutating_negative_indices(self):
        ...
        df = DataFrame(
            {"A": [100, 101, 102], "B": [103, 104, 105], "C": [106, 107, 108]***REMOVED***,
            index=[1, 2, 3],
        )
    ...
    def test_iloc_getitem_neg_int_can_reach_first_index(self):
        ...
        df = DataFrame({"A": [2, 3, 5], "B": [7, 11, 13]***REMOVED***)
        s = df["A"]
    ...
    def test_iloc_getitem_dups(self):
        # GH 6766
        df1 = DataFrame([{"A": None, "B": 1***REMOVED***, {"A": 2, "B": 2***REMOVED***])
        df2 = DataFrame([{"A": 3, "B": 3***REMOVED***, {"A": 4, "B": 4***REMOVED***])
    ...
    def test_iloc_getitem_array(self):
        df = DataFrame(
            [
                {"A": 1, "B": 2, "C": 3***REMOVED***,
                {"A": 100, "B": 200, "C": 300***REMOVED***,
                {"A": 1000, "B": 2000, "C": 3000***REMOVED***,
            ]
        )

        expected = DataFrame([{"A": 1, "B": 2, "C": 3***REMOVED***])
        tm.assert_frame_equal(df.iloc[[0]], expected)

        expected = DataFrame([{"A": 1, "B": 2, "C": 3***REMOVED***, {"A": 100, "B": 200, "C": 300***REMOVED***])
        tm.assert_frame_equal(df.iloc[[0, 1]], expected)

        expected = DataFrame([{"B": 2, "C": 3***REMOVED***, {"B": 2000, "C": 3000***REMOVED***], index=[0, 2])
    ...
    def test_iloc_getitem_bool(self):
        ...
        expected = DataFrame(
            [{"A": 1, "B": 2, "C": 3***REMOVED***, {"A": 1000, "B": 2000, "C": 3000***REMOVED***], index=[0, 2]
        )
    ...
    def test_iloc_getitem_slice(self):
        ...
        expected = DataFrame([{"A": 100, "B": 200***REMOVED***], index=[1])
        result = df.iloc[1:2, 0:2]
        tm.assert_frame_equal(result, expected)

        expected = DataFrame(
            [{"A": 1, "C": 3***REMOVED***, {"A": 100, "C": 300***REMOVED***, {"A": 1000, "C": 3000***REMOVED***]
        )
    ...
    def test_iloc_setitem_dups(self):
        ...
        expected = DataFrame({0: [1, 2], 1: [3, 4]***REMOVED***)
    ...
    def test_iloc_setitem_list_of_lists(self):
        ...
        df = DataFrame(
            {"A": np.arange(5, dtype="int64"), "B": np.arange(5, 10, dtype="int64")***REMOVED***
        )
        df.iloc[2:4] = [[10, 11], [12, 13]]
        expected = DataFrame({"A": [0, 1, 10, 12, 4], "B": [5, 6, 11, 13, 9]***REMOVED***)
        tm.assert_frame_equal(df, expected)

        df = DataFrame(
            {"A": ["a", "b", "c", "d", "e"], "B": np.arange(5, 10, dtype="int64")***REMOVED***
        )
        df.iloc[2:4] = [["x", 11], ["y", 13]]
        expected = DataFrame({"A": ["a", "b", "x", "y", "e"], "B": [5, 6, 11, 13, 9]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_iloc_mask(self):
        ...
        df = DataFrame({"locs": locs, "nums": nums***REMOVED***, reps)
    ...
    ***REMOVED***
                ...
                if r != answer:
                    raise AssertionError(
                        f"[{key***REMOVED***] does not match [{answer***REMOVED***], received [{r***REMOVED***]"
        ***REMOVED***

    def test_iloc_non_unique_indexing(self):
        # GH 4017, non-unique indexing (on the axis)
        df = DataFrame({"A": [0.1] * 3000, "B": [1] * 3000***REMOVED***)
        idx = np.arange(30) * 99
        ...
        df2 = DataFrame({"A": [0.1] * 1000, "B": [1] * 1000***REMOVED***)
    ...
    def test_iloc_setitem_categorical_updates_inplace(self):
        ...
        df = DataFrame({1: cat, 2: [1, 2, 3]***REMOVED***, copy=False)
    ...
    def test_iloc_getitem_singlerow_slice_categoricaldtype_gives_series(self):
        # GH#29521
        df = DataFrame({"x": Categorical("a b c d e".split())***REMOVED***)
        result = df.iloc[0]
    ...
    def test_iloc_setitem_empty_frame_raises_with_3d_ndarray(self):
        ...
        msg = f"Cannot set values with ndim > {obj.ndim***REMOVED***"
        with pytest.raises(ValueError, match=msg):
            obj.iloc[nd3] = 0
    ...
    def test_iloc_getitem_readonly_key(self):
        ...
        df = DataFrame({"data": np.ones(100, dtype="float64")***REMOVED***)
    ...
    def test_iloc_assign_series_to_df_cell(self):
        ...
        expected = DataFrame({"a": [Series([1, 2, 3])]***REMOVED***, columns=["a"], index=[0])
        tm.assert_frame_equal(df, expected)

    @pytest.mark.parametrize("klass", [list, np.array])
    def test_iloc_setitem_bool_indexer(self, klass):
        # GH#36741
        df = DataFrame({"flag": ["x", "y", "z"], "value": [1, 3, 4]***REMOVED***)
        indexer = klass([True, False, False])
        df.iloc[indexer, 1] = df.iloc[indexer, 1] * 2
        expected = DataFrame({"flag": ["x", "y", "z"], "value": [2, 3, 4]***REMOVED***)
        tm.assert_frame_equal(df, expected)

    @pytest.mark.parametrize("indexer", [[1], slice(1, 2)])
    def test_iloc_setitem_pure_position_based(self, indexer):
        # GH#22046
        df1 = DataFrame({"a2": [11, 12, 13], "b2": [14, 15, 16]***REMOVED***)
        df2 = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]***REMOVED***)
        df2.iloc[:, indexer] = df1.iloc[:, [0]]
        expected = DataFrame({"a": [1, 2, 3], "b": [11, 12, 13], "c": [7, 8, 9]***REMOVED***)
        tm.assert_frame_equal(df2, expected)

    def test_iloc_setitem_dictionary_value(self):
        # GH#37728
        df = DataFrame({"x": [1, 2], "y": [2, 2]***REMOVED***)
        rhs = {"x": 9, "y": 99***REMOVED***
        df.iloc[1] = rhs
        expected = DataFrame({"x": [1, 9], "y": [2, 99]***REMOVED***)
        tm.assert_frame_equal(df, expected)

        # GH#38335 same thing, mixed dtypes
        df = DataFrame({"x": [1, 2], "y": [2.0, 2.0]***REMOVED***)
        df.iloc[1] = rhs
        expected = DataFrame({"x": [1, 9], "y": [2.0, 99.0]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_iloc_interval(self):
        # GH#17130
        df = DataFrame({Interval(1, 2): [1, 2]***REMOVED***)

        result = df.iloc[0]
        expected = Series({Interval(1, 2): 1***REMOVED***, name=0)
        tm.assert_series_equal(result, expected)
        ...
        result = df.copy()
        result.iloc[:, 0] += 1
        expected = DataFrame({Interval(1, 2): [2, 3]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_iloc_getitem_slice_negative_step_ea_block(self):
        ...
        df["B"] = "foo"
        res = df.iloc[:, ::-1]
        expected = DataFrame({"B": df["B"], "A": df["A"]***REMOVED***)
        tm.assert_frame_equal(res, expected)

    def test_iloc_setitem_2d_ndarray_into_ea_block(self):
        # GH#44703
        df = DataFrame({"status": ["a", "b", "c"]***REMOVED***, dtype="category")
        df.iloc[np.array([0, 1]), np.array([0])] = np.array([["a"], ["a"]])

        expected = DataFrame({"status": ["a", "a", "c"]***REMOVED***, dtype=df["status"].dtype)
        tm.assert_frame_equal(df, expected)
    ...
    def test_iloc_setitem_multicolumn_to_datetime(self):
        # GH#20511
        df = DataFrame({"A": ["2022-01-01", "2022-01-02"], "B": ["2021", "2022"]***REMOVED***)

        df.iloc[:, [0]] = DataFrame({"A": to_datetime(["2021", "2022"])***REMOVED***)
        expected = DataFrame(
            {
                "A": [
...
***REMOVED***:
    ...
    def test_iloc_frame_indexer(self):
        ...
        indexer = DataFrame({"a": [True, False, True]***REMOVED***)
...
class TestILocSeries:
    ...
    def test_iloc_nullable_int64_size_1_nan(self):
        # GH 31861
        result = DataFrame({"a": ["test"], "b": [np.nan]***REMOVED***)
        with tm.assert_produces_warning(FutureWarning, match="incompatible dtype"):
            result.loc[:, "b"] = result.loc[:, "b"].astype("Int64")
        expected = DataFrame({"a": ["test"], "b": array([NA], dtype="Int64")***REMOVED***)
        tm.assert_frame_equal(result, expected)
