...
def assert_components_edges_equal(x, y):
    sx = {frozenset(frozenset(e) for e in c) for c in x***REMOVED***
    sy = {frozenset(frozenset(e) for e in c) for c in y***REMOVED***
    assert sx == sy


def assert_components_equal(x, y):
    sx = {frozenset(c) for c in x***REMOVED***
    sy = {frozenset(c) for c in y***REMOVED***
    assert sx == sy
...
def test_barbell():
    ...
    assert pts == {7, 8, 9, 10, 11, 12, 20, 21, 22***REMOVED***

    answer = [
        {12, 13, 14, 15, 16, 17, 18, 19***REMOVED***,
...
def test_biconnected_components_cycle():
    G = nx.cycle_graph(3)
    nx.add_cycle(G, [1, 3, 4])
    answer = [{0, 1, 2***REMOVED***, {1, 3, 4***REMOVED***]
    assert_components_equal(list(nx.biconnected_components(G)), answer)
...
def test_biconnected_eppstein():
    ...
    answer_G2 = [{1, 3, 6, 8***REMOVED***, {0, 2, 5***REMOVED***, {2, 3***REMOVED***, {4, 7***REMOVED***]
