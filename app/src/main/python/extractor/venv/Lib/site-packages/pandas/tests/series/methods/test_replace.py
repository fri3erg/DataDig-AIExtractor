...
class TestSeriesReplace:
    def test_replace_explicit_none(self):
        ...
        df = pd.DataFrame(np.zeros((3, 3))).astype({2: object***REMOVED***)
        df.iloc[2, 2] = ""
    ...
    def test_replace_noop_doesnt_downcast(self):
        ...
        res = ser.replace({np.nan: None***REMOVED***)  # should be a no-op
    ...
    def test_replace(self):
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            rs = ser.replace({np.nan: -1, "foo": -2, "bar": -3***REMOVED***)
    ...
    def test_replace_datetime64(self):
        ...
        result = ser.replace({pd.Timestamp("20130103"): pd.Timestamp("20120101")***REMOVED***)
        tm.assert_series_equal(result, expected)
    ...
    def test_replace_timedelta_td64(self):
        ...
        result = ser.replace({ser[1]: ser[3]***REMOVED***)
    ...
    def test_replace_with_dict_with_bool_keys(self):
        s = pd.Series([True, False, True])
        result = s.replace({"asdf": "asdb", True: "yes"***REMOVED***)
    ...
    def test_replace_with_dictlike_and_string_dtype(self, nullable_string_dtype):
        ...
        result = ser.replace({"one": "1", "two": "2"***REMOVED***)
        tm.assert_series_equal(expected, result)

    def test_replace_with_empty_dictlike(self):
        # GH 15289
        s = pd.Series(list("abcd"))
        tm.assert_series_equal(s, s.replace({***REMOVED***))
    ...
    def test_replace_replacer_equals_replacement(self):
        ...
        result = s.replace({"a": "b", "b": "a"***REMOVED***)
        tm.assert_series_equal(expected, result)
    ...
    def test_replace_categorical(self, categorical, numeric):
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = ser.replace({"A": 1, "B": 2***REMOVED***)
    ...
    @pytest.mark.parametrize(
        "ser, to_replace, exp",
        [
            ([1, 2, 3], {1: 2, 2: 3, 3: 4***REMOVED***, [2, 3, 4]),
            (["1", "2", "3"], {"1": "2", "2": "3", "3": "4"***REMOVED***, ["2", "3", "4"]),
        ],
    )
    ...
    def test_replace_with_compiled_regex(self):
        ...
        s = pd.Series(["a", "b", "c"])
        regex = re.compile("^a$")
        result = s.replace({regex: "z"***REMOVED***, regex=True)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("bool", [True, False], {True: False***REMOVED***, [False, False]),
            ("int64", [1, 2], {1: 10, 2: 20***REMOVED***, [10, 20]),
            ("Int64", [1, 2], {1: 10, 2: 20***REMOVED***, [10, 20]),
            ("float64", [1.1, 2.2], {1.1: 10.1, 2.2: 20.5***REMOVED***, [10.1, 20.5]),
            ("Float64", [1.1, 2.2], {1.1: 10.1, 2.2: 20.5***REMOVED***, [10.1, 20.5]),
            ("string", ["one", "two"], {"one": "1", "two": "2"***REMOVED***, ["1", "2"]),
            (
                pd.IntervalDtype("int64"),
                IntervalArray([pd.Interval(1, 2), pd.Interval(2, 3)]),
                {pd.Interval(1, 2): pd.Interval(10, 20)***REMOVED***,
...
***REMOVED***,
            (
                ...
                {pd.Interval(1.0, 2.7): pd.Interval(10.6, 20.8)***REMOVED***,
                IntervalArray([pd.Interval(10.6, 20.8), pd.Interval(2.8, 3.1)]),
***REMOVED***,
            (
                pd.PeriodDtype("M"),
                [pd.Period("2020-05", freq="M")],
                {pd.Period("2020-05", freq="M"): pd.Period("2020-06", freq="M")***REMOVED***,
...
***REMOVED***,
    ...
    def test_replace_string_dtype(self):
        ...
        res = ser.replace({"one": "1", "two": "2"***REMOVED***)
    ...
    def test_replace_nullable_numeric(self):
        ...
        assert floats.replace({1.0: 9***REMOVED***).dtype == floats.dtype
        assert floats.replace(1.0, 9).dtype == floats.dtype
        assert floats.replace({1.0: 9.0***REMOVED***).dtype == floats.dtype
    ...
    def test_replace_different_int_types(self, any_int_numpy_dtype):
        ...
        expected = labs.replace({0: 0, 2: 1, 1: 2***REMOVED***)
        tm.assert_series_equal(result, expected)
    ...
    def test_replace_change_dtype_series(self, using_infer_string):
        # GH#25797
        df = pd.DataFrame.from_dict({"Test": ["0.5", True, "0.6"]***REMOVED***)
        ...
        expected = pd.DataFrame.from_dict({"Test": ["0.5", np.nan, "0.6"]***REMOVED***)
        tm.assert_frame_equal(df, expected)

        df = pd.DataFrame.from_dict({"Test": ["0.5", None, "0.6"]***REMOVED***)
