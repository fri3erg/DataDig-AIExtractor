...
class TestMinEdgeCover:
    ...
    def test_graph_with_loop(self):
        G = nx.Graph()
        G.add_edge(0, 0)
        assert nx.min_edge_cover(G) == {(0, 0)***REMOVED***
    ...
    def test_graph_single_edge(self):
        G = nx.Graph([(0, 1)])
        assert nx.min_edge_cover(G) in ({(0, 1)***REMOVED***, {(1, 0)***REMOVED***)
...
class TestIsEdgeCover:
    ...
    def test_graph_with_loop(self):
        G = nx.Graph()
        G.add_edge(1, 1)
        assert nx.is_edge_cover(G, {(1, 1)***REMOVED***)

    def test_graph_single_edge(self):
        G = nx.Graph()
        G.add_edge(0, 1)
        assert nx.is_edge_cover(G, {(0, 0), (1, 1)***REMOVED***)
        assert nx.is_edge_cover(G, {(0, 1), (1, 0)***REMOVED***)
        assert nx.is_edge_cover(G, {(0, 1)***REMOVED***)
        assert not nx.is_edge_cover(G, {(0, 0)***REMOVED***)
