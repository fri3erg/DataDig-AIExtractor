...
def test_parallel_call_cached_function_defined_in_jupyter(
    ...
    for session_no in [0, 1]:
        ...
        ipython_cell_id = '<ipython-input-{***REMOVED***-000000000000>'.format(session_no)
...
def fast_func_with_conditional_complex_output(complex_output=True):
    complex_obj = {str(i): i for i in range(int(1e5))***REMOVED***
...
def test_filesystem_store_backend_repr(tmpdir):
    ...
    repr_pattern = 'FileSystemStoreBackend(location="{location***REMOVED***")'
...
def test_memory_objects_repr(tmpdir):
    ...
    memorized_func_repr = 'MemorizedFunc(func={func***REMOVED***, location={location***REMOVED***)'
    ...
    memorized_result_repr = ('MemorizedResult(location="{location***REMOVED***", '
                             'func="{func***REMOVED***", args_id="{args_id***REMOVED***")')
    ...
    assert str(memory) == 'Memory(location={location***REMOVED***)'.format(
        location=memory.store_backend.location)
...
@pytest.mark.parametrize('memory_kwargs',
                         [{'compress': 3, 'verbose': 2***REMOVED***,
                          {'mmap_mode': 'r', 'verbose': 5,
                           'backend_options': {'parameter': 'unused'***REMOVED******REMOVED***])
...
class TestCacheValidationCallback:
    ...
    def test_constant_cache_validation_callback(
        ...
        d1, d2 = {"run": False***REMOVED***, {"run": False***REMOVED***
        assert f(2, d1) == 4
        assert f(2, d2) == 4
    ...
    def test_memory_expires_after(self, memory):
        ...
        d1, d2, d3 = {"run": False***REMOVED***, {"run": False***REMOVED***, {"run": False***REMOVED***
        assert f(2, d1) == 4
        assert f(2, d2) == 4
        time.sleep(.5)
        assert f(2, d3) == 4
