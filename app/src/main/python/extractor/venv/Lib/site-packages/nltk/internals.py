...
class ReadError(ValueError):
    ...
    def __str__(self):
        return f"Expected {self.expected***REMOVED*** at {self.position***REMOVED***"
...
def _add_epytext_field(obj, field, message):
    ...
    obj.__doc__ += textwrap.fill(
        f"@{field***REMOVED***: {message***REMOVED***",
...
def deprecated(message):
    ...
    def decorator(func):
        msg = f"Function {func.__name__***REMOVED***() has been deprecated.  {message***REMOVED***"
...
class Deprecated:
    ...
    def __new__(cls, *args, **kwargs):
        ...
        # Put it all together.
        msg = f"{name***REMOVED*** has been deprecated.  {doc***REMOVED***"
        # Wrap it.
        msg = "\n" + textwrap.fill(msg, initial_indent="    ", subsequent_indent="    ")
        warnings.warn(msg, category=DeprecationWarning, stacklevel=2)
...
def find_file_iter(
    ...
    for alternative in file_names:
        ...
        if os.path.isfile(path_to_file):
            if verbose:
                print(f"[Found {filename***REMOVED***: {path_to_file***REMOVED***]")
            yielded = True
            yield path_to_file
        # Check the bare alternatives
        if os.path.isfile(alternative):
            if verbose:
                print(f"[Found {filename***REMOVED***: {alternative***REMOVED***]")
            yielded = True
            yield alternative
        ...
        if env_var in os.environ:
            ...
            for env_dir in os.environ[env_var].split(os.pathsep):
                ...
                if os.path.isfile(env_dir):
                    if verbose:
                        print(f"[Found {filename***REMOVED***: {env_dir***REMOVED***]")
                    yielded = True
                    yield env_dir
    ...
    ***REMOVED***
                ...
                if path.endswith(alternative) and os.path.exists(path):
                    if verbose:
                        print(f"[Found {filename***REMOVED***: {path***REMOVED***]")
                    yielded = True
                    yield path
    ...
    if not yielded:
        ...
        if url:
            msg += f"\n\n  For more information on {filename***REMOVED***, see:\n    <{url***REMOVED***>"
        div = "=" * 75
        raise LookupError(f"\n\n{div***REMOVED***\n{msg***REMOVED***\n{div***REMOVED***")
...
def find_jar_iter(
    ...
    if path_to_jar is not None:
        if os.path.isfile(path_to_jar):
            ...
            raise LookupError(
                f"Could not find {name_pattern***REMOVED*** jar file at {path_to_jar***REMOVED***"
***REMOVED***
    ...
    for env_var in env_vars:
        if env_var in os.environ:
            ...
            ***REMOVED***:
                            if verbose:
                                print(f"[Found {name_pattern***REMOVED***: {cp***REMOVED***]")
                            yielded = True
                            yield cp
    ...
    ***REMOVED***
            ...
            ***REMOVED***:
                            if verbose:
                                print(f"[Found {name_pattern***REMOVED***: {path_to_jar***REMOVED***]")
                            yielded = True
                            yield path_to_jar
            ...
            for filename in os.listdir(directory):
                ...
                if os.path.isfile(path_to_jar):
                    if re.match(name_pattern, filename):
                        if verbose:
                            print(f"[Found {filename***REMOVED***: {path_to_jar***REMOVED***]")
                yielded = True
                yield path_to_jar
        else:
    ...
    if not yielded:
        ...
        if searchpath:
            ...
            msg += "".join("\n    - %s" % d for d in searchpath)
        if url:
            msg += "\n\n  For more information, on {***REMOVED***, see:\n    <{***REMOVED***>".format(
                name_pattern,
                url,
***REMOVED***
        div = "=" * 75
