...
***REMOVED***
    ...
    def _add_vectors(
        ...
        for i in range(0, len(rows), chunk_size):
            ...
            r = self._client.records().bulk_insert(self._table_name, {"records": chunk***REMOVED***)
            if r.status_code != 200:
                raise Exception(f"Error adding vectors to Xata: {r.status_code***REMOVED*** {r***REMOVED***")
    ...
    def similarity_search_with_score(
        ...
        ***REMOVED***
        ...
        if r.status_code != 200:
            raise Exception(f"Error running similarity search: {r.status_code***REMOVED*** {r***REMOVED***")
        hits = r["records"]
        docs_and_scores = [
            (
                Document(
...
***REMOVED***
        ...
        **kwargs: Any,
        ...
        if delete_all:
            ...
            for i in range(0, len(ids), chunk_size):
                ...
                operations = [
                    {"delete": {"table": self._table_name, "id": id***REMOVED******REMOVED*** for id in chunk
                ]
                self._client.records().transaction(payload={"operations": operations***REMOVED***)
        else:
    ...
    def _delete_all(self) -> None:
        ...
        while True:
            r = self._client.data().query(self._table_name, payload={"columns": ["id"]***REMOVED***)
            if r.status_code != 200:
                raise Exception(f"Error running query: {r.status_code***REMOVED*** {r***REMOVED***")
            ids = [rec["id"] for rec in r["records"]]
            if len(ids) == 0:
                break
            operations = [
                {"delete": {"table": self._table_name, "id": id***REMOVED******REMOVED*** for id in ids
            ]
            self._client.records().transaction(payload={"operations": operations***REMOVED***)
    ...
    def wait_for_indexing(self, timeout: float = 5, ndocs: int = 1) -> None:
        ...
        while True:
            r = self._client.data().search_table(
                self._table_name, payload={"query": "", "page": {"size": 0***REMOVED******REMOVED***
***REMOVED***
            if r.status_code != 200:
                raise Exception(f"Error running search: {r.status_code***REMOVED*** {r***REMOVED***")
