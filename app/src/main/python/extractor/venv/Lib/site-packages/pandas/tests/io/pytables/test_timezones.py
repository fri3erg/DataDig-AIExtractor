    ...
    for c in a.columns:
        for i in a.index:
            ...
            if not (a_e == b_e and a_e.tz == b_e.tz):
                raise AssertionError(f"invalid tz comparison [{a_e***REMOVED***] [{b_e***REMOVED***]")
...
def test_roundtrip_tz_aware_index(setup_path, unit):
    ...
    denom = {"ns": 1, "us": 1000, "ms": 10**6, "s": 10**9***REMOVED***[unit]
...
def test_dst_transitions(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        for i in [times, times + pd.Timedelta("10min")]:
            _maybe_remove(store, "df")
            df = DataFrame({"A": range(len(i)), "B": i***REMOVED***, index=i)
            store.append("df", df)
            result = store.select("df")
            tm.assert_frame_equal(result, df)
...
def test_read_with_where_tz_aware_index(tmp_path, setup_path):
    ...
    expected = DataFrame({"MYCOL": 0***REMOVED***, index=mi)
...
def test_py2_created_with_datetimez(datapath):
    ...
    expected = DataFrame({"data": 123***REMOVED***, index=index)
    with ensure_clean_store(
