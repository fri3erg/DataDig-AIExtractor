...
class GoogleDriveLoader(BaseLoader, BaseModel):
    ...
    file_loader_kwargs: Dict["str", Any] = {***REMOVED***
...
***REMOVED***
    ...
    ***REMOVED***
            ...
            ***REMOVED***
            ...
            short_names = ", ".join([f"'{x***REMOVED***'" for x in type_mapping.keys()])
            full_names = ", ".join([f"'{x***REMOVED***'" for x in type_mapping.values()])
            for file_type in file_types:
                if file_type not in allowed_types:
                    raise ValueError(
                        f"Given file type {file_type***REMOVED*** is not supported. "
                        f"Supported values are: {short_names***REMOVED***; and "
                        f"their full-form names: {full_names***REMOVED***"
        ***REMOVED***
    ...
    def validate_credentials_path(cls, v: Any, **kwargs: Any) -> Any:
        ...
        if not v.exists():
            raise ValueError(f"credentials_path {v***REMOVED*** does not exist")
        return v
...
***REMOVED***
            ...
            for i, row in enumerate(values[1:], start=1):
                metadata = {
                    "source": (
                        f"https://docs.google.com/spreadsheets/d/{id***REMOVED***/"
                        f"edit?gid={sheet['properties']['sheetId']***REMOVED***"
        ***REMOVED***,
                    "title": f"{spreadsheet['properties']['title']***REMOVED*** - {sheet_name***REMOVED***",
                    "row": i,
                ***REMOVED***
                content = []
                for j, v in enumerate(row):
                    title = header[j].strip() if len(header) > j else ""
                    content.append(f"{title***REMOVED***: {v.strip()***REMOVED***")
    ...
    def _load_document_from_id(self, id: str) -> Document:
        ...
        try:
            ...
            if e.resp.status == 404:
                print("File not found: {***REMOVED***".format(id))
            else:
                print("An error occurred: {***REMOVED***".format(e))

        text = fh.getvalue().decode("utf-8")
        metadata = {
            "source": f"https://docs.google.com/document/d/{id***REMOVED***/edit",
            "title": f"{file.get('name')***REMOVED***",
            "when": f"{file.get('modifiedTime')***REMOVED***",
        ***REMOVED***
...
***REMOVED***:
    ...
    def _fetch_files_recursive(
        ...
        results = (
            service.files()
            .list(
                q=f"'{folder_id***REMOVED***' in parents",
                pageSize=1000,
...
***REMOVED***
    ...
    def _load_file_from_id(self, id: str) -> List[Document]:
        ...
        if self.file_loader_cls is not None:
            ...
            docs = loader.load()
            for doc in docs:
                doc.metadata["source"] = f"https://drive.google.com/file/d/{id***REMOVED***/view"
                if "title" not in doc.metadata:
                    doc.metadata["title"] = f"{file.get('name')***REMOVED***"
            return docs
            ...
            return [
                Document(
                    ...
                    metadata={
                        "source": f"https://drive.google.com/file/d/{id***REMOVED***/view",
                        "title": f"{file.get('name')***REMOVED***",
                        "page": i,
                    ***REMOVED***,
    ***REMOVED***
