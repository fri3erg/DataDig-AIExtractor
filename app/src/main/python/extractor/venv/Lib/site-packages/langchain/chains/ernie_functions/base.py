...
def _get_python_function_arguments(function: Callable, arg_descriptions: dict) -> dict:
    ...
    for arg, arg_type in annotations.items():
        ...
        if isinstance(arg_type, type) and issubclass(arg_type, BaseModel):
            ...
            properties[arg] = {"type": PYTHON_TO_JSON_TYPES[arg_type.__name__]***REMOVED***
        if arg in arg_descriptions:
            if arg not in properties:
                properties[arg] = {***REMOVED***
...
def _get_python_function_required_args(function: Callable) -> List[str]:
    ...
    required += [k for k in spec.kwonlyargs if k not in (spec.kwonlydefaults or {***REMOVED***)]
...
def convert_to_ernie_function(
    ...
    if isinstance(function, dict):
        ...
        raise ValueError(
            f"Unsupported function type {type(function)***REMOVED***. Functions must be passed in"
    ...
    Example:
        .. code-block:: python
                ...
                prompt = ChatPromptTemplate.from_messages(
                    [
                        ("user", "Make calls to the relevant function to record the entities in the following input: {input***REMOVED***"),
                        ("assistant", "OK!"),
                        ("user", "Tip: Make sure to answer in the correct format"),
                    ]
    ***REMOVED***
                chain = create_ernie_fn_runnable([RecordPerson, RecordDog], llm, prompt)
                chain.invoke({"input": "Harry was a chubby brown beagle who loved chicken"***REMOVED***)
                # -> RecordDog(name="Harry", color="brown", fav_food="chicken")
    """  # noqa: E501
    if not functions:
        raise ValueError("Need to pass in at least one function. Received zero.")
    ernie_functions = [convert_to_ernie_function(f) for f in functions]
    llm_kwargs: Dict[str, Any] = {"functions": ernie_functions, **kwargs***REMOVED***
    if len(ernie_functions) == 1:
        llm_kwargs["function_call"] = {"name": ernie_functions[0]["name"]***REMOVED***
    ...
    **kwargs: Any,
    ...
    Example:
        .. code-block:: python
                ...
                prompt = ChatPromptTemplate.from_messages(
                    [
                        ("user", "Use the given format to extract information from the following input: {input***REMOVED***"),
                        ("assistant", "OK!"),
                        ("user", "Tip: Make sure to answer in the correct format"),
                    ]
    ***REMOVED***
