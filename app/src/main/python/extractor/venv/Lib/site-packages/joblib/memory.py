...
_STORE_BACKENDS = {'local': FileSystemStoreBackend***REMOVED***
...
def _store_backend_factory(backend, location, verbose=0, backend_options=None):
    ...
    if isinstance(location, StoreBackendBase):
        ...
        if obj is None:
            raise TypeError('Unknown location {0***REMOVED*** or backend {1***REMOVED***'.format(
                            location, backend))
        ...
        warnings.warn(
            "Instantiating a backend using a {***REMOVED*** as a location is not "
...
def _format_load_msg(func_id, args_id, timestamp=None, metadata=None):
    ...
    if timestamp is not None:
        ts_string = "{0: <16***REMOVED***".format(format_time(time.time() - timestamp))
    else:
        ts_string = ""
    return '[Memory]{0***REMOVED***: Loading {1***REMOVED***'.format(ts_string, str(signature))
...
class MemorizedResult(Logger):
    ...
    mmap_mode: {None, 'r+', 'r', 'w+', 'c'***REMOVED***
    ...
    def get(self):
        ...
        try:
            ...
            new_exc = KeyError(
                ...
                "It seems that this folder is corrupted : {***REMOVED***".format(
                    os.path.join(
    ...
    ***REMOVED***)
    ...
    def __repr__(self):
        return ('{class_name***REMOVED***(location="{location***REMOVED***", func="{func***REMOVED***", '
                'args_id="{args_id***REMOVED***")'
...
class NotMemorizedResult(object):
    ...
    def __repr__(self):
        if self.valid:
            return ('{class_name***REMOVED***({value***REMOVED***)'
                    .format(class_name=self.__class__.__name__,
    ...
    def __getstate__(self):
        return {"valid": self.valid, "value": self.value***REMOVED***
...
class NotMemorizedFunc(object):
    ...
    def __repr__(self):
        return '{0***REMOVED***(func={1***REMOVED***)'.format(self.__class__.__name__, self.func)
    ...
    def _cached_call(self, args, kwargs, shelving=False):
        ...
        if self._verbose >= 20:
            ...
            self.info(
                dedent(
                    f"""
                        Querying {name***REMOVED*** with signature
                        {signature***REMOVED***.

                        (argument hash {args_id***REMOVED***)

                        The store location is {location***REMOVED***.
                        """
    ***REMOVED***
***REMOVED***
        ...
        if self._is_in_cache_and_valid([func_id, args_id]):
            try:
                ...
                self.warn('Exception while loading results for '
                          '{***REMOVED***\n {***REMOVED***'.format(signature, traceback.format_exc()))

                must_call = True
        else:
            if self._verbose > 10:
                _, name = get_func_name(self.func)
                self.warn('Computing func {0***REMOVED***, argument hash {1***REMOVED*** '
                          'in location {2***REMOVED***'
                          .format(name, args_id,
                                  self.store_backend.
    ...
    def _check_previous_func_code(self, stacklevel=2):
        ...
        if old_first_line == first_line == -1 or func_name == '<lambda>':
            if not first_line == -1:
                func_description = ("{0***REMOVED*** ({1***REMOVED***:{2***REMOVED***)"
                                    .format(func_name, source_file,
                                            first_line))
            else:
                func_description = func_name
            warnings.warn(JobLibCollisionWarning(
                "Cannot detect name collisions for function '{0***REMOVED***'"
        ...
        if self._verbose > 10:
            _, func_name = get_func_name(self.func, resolv_alias=False)
            self.warn("Function {0***REMOVED*** (identified by {1***REMOVED***) has changed"
    ...
    def __repr__(self):
        return '{class_name***REMOVED***(func={func***REMOVED***, location={location***REMOVED***)'.format(
...
class Memory(Logger):
    """ A context object for caching a function's return value each time it
        ...
        bytes_limit: int | str, optional
            ...
            { K | M | G***REMOVED*** for kilo, mega, giga.
    ...
    def cache(self, func=None, ignore=None, verbose=None, mmap_mode=False,
        ...
        if (cache_validation_callback is not None and
            ...
            raise ValueError(
                ...
                f"Got {cache_validation_callback***REMOVED***."
***REMOVED***
        if func is None:
...
***REMOVED***
    ...
    def __repr__(self):
        return '{class_name***REMOVED***(location={location***REMOVED***)'.format(
            class_name=self.__class__.__name__,
