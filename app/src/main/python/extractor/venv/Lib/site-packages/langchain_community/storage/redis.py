...
***REMOVED***
        ...
        if client:
            if not isinstance(client, Redis):
                raise TypeError(
                    f"Expected Redis client, got {type(client).__name__***REMOVED*** instead."
    ***REMOVED***
            _client = client
        else:
            if not redis_url:
    ...
    ***REMOVED***
            _client = get_client(redis_url, **(client_kwargs or {***REMOVED***))

        self.client = _client

        if not isinstance(ttl, int) and ttl is not None:
            raise TypeError(f"Expected int or None, got {type(ttl)***REMOVED*** instead.")
    ...
    def _get_prefixed_key(self, key: str) -> str:
        ...
        delimiter = "/"
        if self.namespace:
            return f"{self.namespace***REMOVED***{delimiter***REMOVED***{key***REMOVED***"
        return key
