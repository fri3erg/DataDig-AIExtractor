...
TEMPLATE_STRING_RE = re.compile(r"\{[a-zA-Z#]+\***REMOVED***")
GET_ATTR_RE = re.compile(r"(\w+)\[(\d+)\]")
VALID_HOST_LABEL_RE = re.compile(
    r"^(?!-)[a-zA-Z\d-]{1,63***REMOVED***(?<!-)$",
)
CACHE_SIZE = 100
...
class RuleSetStandardLibrary:
    ...
    def resolve_template_string(self, value, scope_vars):
        ...
        for literal, reference, _, _ in STRING_FORMATTER.parse(value):
            if reference is not None:
                ...
                result += f"{literal***REMOVED***{template_value***REMOVED***"
            else:
                result += literal
        return result
    ...
    def convert_func_name(self, value):
        ...
        :rtype: str
        """
        normalized_name = f"{xform_name(value)***REMOVED***"
        if normalized_name == "not":
            normalized_name = f"_{normalized_name***REMOVED***"
        ...
        if "assign" in func_signature:
            ...
            if assign in scope_vars:
                raise EndpointResolutionError(
                    msg=f"Assignment {assign***REMOVED*** already exists in "
...
***REMOVED***
    ...
    def string_equals(self, value1, value2):
        ...
        if not all(isinstance(val, str) for val in (value1, value2)):
            msg = f"Both values must be strings, not {type(value1)***REMOVED*** and {type(value2)***REMOVED***."
    ...
    def parse_url(self, value):
        ...
        if not normalized_path.endswith("/"):
            normalized_path = f"{normalized_path***REMOVED***/"
    ...
    def boolean_equals(self, value1, value2):
        ...
        if not all(isinstance(val, bool) for val in (value1, value2)):
            msg = f"Both arguments must be bools, not {type(value1)***REMOVED*** and {type(value2)***REMOVED***."
            raise EndpointResolutionError(msg=msg)
    ...
    def substring(self, value, start, stop, reverse):
        ...
        if not isinstance(value, str):
            msg = f"Input must be a string, not {type(value)***REMOVED***."
            raise EndpointResolutionError(msg=msg)
    ...
    def evaluate(self, scope_vars, rule_lib):
        ...
        if self.evaluate_conditions(scope_vars, rule_lib):
            ...
            properties = self.resolve_properties(
                self.endpoint.get("properties", {***REMOVED***),
                scope_vars,
                rule_lib,
***REMOVED***
...
***REMOVED***
    ...
    def resolve_headers(self, scope_vars, rule_lib):
        ...
        :rtype: dict
        """
        resolved_headers = {***REMOVED***
        headers = self.endpoint.get("headers", {***REMOVED***)
...
class RuleCreator:
    ...
    def create(cls, **kwargs):
        ...
        try:
            ...
            raise EndpointResolutionError(
                msg=f"Unknown rule type: {rule_type***REMOVED***. A rule must "
...
***REMOVED***.value
            ...
            raise EndpointResolutionError(
                msg=f"Unknown parameter type: {parameter_type***REMOVED***. "
...
***REMOVED***
    ...
    def validate_input(self, value):
        ...
        if not isinstance(value, self.parameter_type):
            raise EndpointResolutionError(
                msg=f"Value ({self.name***REMOVED***) is the wrong "
                f"type. Must be {self.parameter_type***REMOVED***."
***REMOVED***
        if self.deprecated is not None:
            depr_str = f"{self.name***REMOVED*** has been deprecated."
            msg = self.deprecated.get("message")
            since = self.deprecated.get("since")
            if msg:
                depr_str += f"\n{msg***REMOVED***"
            if since:
                depr_str += f"\nDeprecated since {since***REMOVED***."
    ...
    def process_input(self, value):
        ...
        if value is None:
            ...
            if self.required:
                raise EndpointResolutionError(
                    f"Cannot find value for required parameter {self.name***REMOVED***"
    ***REMOVED***
...
class EndpointProvider:
    ...
    def resolve_endpoint(self, **input_parameters):
        ...
        if endpoint is None:
            param_string = "\n".join(
                [f"{key***REMOVED***: {value***REMOVED***" for key, value in params_for_error.items()]
***REMOVED***
            raise EndpointResolutionError(
                msg=f"No endpoint found for parameters:\n{param_string***REMOVED***"
***REMOVED***
        return endpoint
