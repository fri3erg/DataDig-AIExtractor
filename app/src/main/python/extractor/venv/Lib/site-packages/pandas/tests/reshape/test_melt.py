    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                DataFrame(
                    {
                        "A": {0: 1.067683, 1: -1.321405, 2: -0.807333***REMOVED***,
                        "CAP": {0: "B", 1: "B", 2: "B"***REMOVED***,
                        "value": {0: -1.110463, 1: 0.368915, 2: 0.08298***REMOVED***,
                    ***REMOVED***
    ***REMOVED***,
***REMOVED***,
            (
...
***REMOVED***,
    ...
    def test_pandas_dtypes(self, col):
        # GH 15785
        df = DataFrame(
            {"klass": range(5), "col": col, "attr1": [1, 0, 0, 0, 0], "attr2": col***REMOVED***
        )
    ...
    def test_preserve_category(self):
        # GH 15853
        data = DataFrame({"A": [1, 2], "B": pd.Categorical(["X", "Y"])***REMOVED***)
        result = melt(data, ["B"], ["A"])
        expected = DataFrame(
            {"B": pd.Categorical(["X", "Y"]), "variable": ["A", "A"], "value": [1, 2]***REMOVED***
        )
    ...
    def test_melt_mixed_int_str_id_vars(self):
        # GH 29718
        df = DataFrame({0: ["foo"], "a": ["bar"], "b": [1], "d": [2]***REMOVED***)
        result = melt(df, id_vars=[0, "a"], value_vars=["b", "d"])
        expected = DataFrame(
            {0: ["foo"] * 2, "a": ["bar"] * 2, "variable": list("bd"), "value": [1, 2]***REMOVED***
        )
        tm.assert_frame_equal(result, expected)

    def test_melt_mixed_int_str_value_vars(self):
        # GH 29718
        df = DataFrame({0: ["foo"], "a": ["bar"]***REMOVED***)
        result = melt(df, value_vars=[0, "a"])
        expected = DataFrame({"variable": [0, "a"], "value": ["foo", "bar"]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_ignore_index(self):
        # GH 17440
        df = DataFrame({"foo": [0], "bar": [1]***REMOVED***, index=["first"])
        result = melt(df, ignore_index=False)
        expected = DataFrame(
            {"variable": ["foo", "bar"], "value": [0, 1]***REMOVED***, index=["first", "first"]
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_ignore_multiindex(self):
        ...
        df = DataFrame({"foo": [0, 1], "bar": [2, 3]***REMOVED***, index=index)
        result = melt(df, ignore_index=False)
        ...
        expected = DataFrame(
            {"variable": ["foo"] * 2 + ["bar"] * 2, "value": [0, 1, 2, 3]***REMOVED***,
    ...
    def test_ignore_index_name_and_type(self):
        ...
        df = DataFrame({"x": [0, 1], "y": [2, 3]***REMOVED***, index=index)
        result = melt(df, ignore_index=False)

        expected_index = Index(["foo", "bar"] * 2, dtype="category", name="baz")
        expected = DataFrame(
            {"variable": ["x", "x", "y", "y"], "value": [0, 1, 2, 3]***REMOVED***,
            index=expected_index,
        )
    ...
    def test_melt_ea_columns(self):
        ...
        df = DataFrame(
            {
                "A": {0: "a", 1: "b", 2: "c"***REMOVED***,
                "B": {0: 1, 1: 3, 2: 5***REMOVED***,
                "C": {0: 2, 1: 4, 2: 6***REMOVED***,
            ***REMOVED***
        )
    ...
    def test_melt_preserves_datetime(self):
        ...
        expected = DataFrame(
            {
                "type": {0: "A0", 1: "A1", 2: "A0", 3: "A1"***REMOVED***,
                "start/end": {
                    0: "start_date",
                    1: "start_date",
                    2: "end_date",
    ...
    def test_melt_allows_non_scalar_id_vars(self):
        ...
        expected = DataFrame({"a": [1, 2, 3], 0: ["b"] * 3, 1: [4, 5, 6]***REMOVED***)
        tm.assert_frame_equal(result, expected)
...
class TestLreshape:
    def test_pairs(self):
        ...
        ***REMOVED***
        ...
        spec = {
            "visitdt": [f"visitdt{i:d***REMOVED***" for i in range(1, 4)],
            "wt": [f"wt{i:d***REMOVED***" for i in range(1, 4)],
        ***REMOVED***
        ...
        ***REMOVED***
        ...
        spec = {
            "visitdt": [f"visitdt{i:d***REMOVED***" for i in range(1, 3)],
            "wt": [f"wt{i:d***REMOVED***" for i in range(1, 4)],
        ***REMOVED***
...
class TestWideToLong:
    def test_simple(self):
        ...
        df = DataFrame(
            {
                "A1970": {0: "a", 1: "b", 2: "c"***REMOVED***,
                "A1980": {0: "d", 1: "e", 2: "f"***REMOVED***,
                "B1970": {0: 2.5, 1: 1.2, 2: 0.7***REMOVED***,
                "B1980": {0: 3.2, 1: 1.3, 2: 0.1***REMOVED***,
    ...
    def test_separating_character(self):
        ...
        df = DataFrame(
            {
                "A.1970": {0: "a", 1: "b", 2: "c"***REMOVED***,
                "A.1980": {0: "d", 1: "e", 2: "f"***REMOVED***,
                "B.1970": {0: 2.5, 1: 1.2, 2: 0.7***REMOVED***,
                "B.1980": {0: 3.2, 1: 1.3, 2: 0.1***REMOVED***,
                "X": dict(zip(range(3), x)),
            ***REMOVED***
        )
        df["id"] = df.index
    ...
    def test_escapable_characters(self):
        ...
        df = DataFrame(
            {
                "A(quarterly)1970": {0: "a", 1: "b", 2: "c"***REMOVED***,
                "A(quarterly)1980": {0: "d", 1: "e", 2: "f"***REMOVED***,
                "B(quarterly)1970": {0: 2.5, 1: 1.2, 2: 0.7***REMOVED***,
                "B(quarterly)1980": {0: 3.2, 1: 1.3, 2: 0.1***REMOVED***,
                "X": dict(zip(range(3), x)),
            ***REMOVED***
        )
        df["id"] = df.index
    ...
    def test_invalid_separator(self):
        ...
        exp_data = {
            ...
            "B": [],
        ***REMOVED***
        expected = DataFrame(exp_data).astype({"year": np.int64***REMOVED***)
        expected = expected.set_index(["id", "year"])[
    ...
    def test_non_unique_idvars(self):
        ...
        df = DataFrame(
            {"A_A1": [1, 2, 3, 4, 5], "B_B1": [1, 2, 3, 4, 5], "x": [1, 1, 1, 1, 1]***REMOVED***
        )
    ...
    def test_col_substring_of_stubname(self):
        ...
        wide_data = {
            "node_id": {0: 0, 1: 1, 2: 2, 3: 3, 4: 4***REMOVED***,
            "A": {0: 0.80, 1: 0.0, 2: 0.25, 3: 1.0, 4: 0.81***REMOVED***,
            "PA0": {0: 0.74, 1: 0.56, 2: 0.56, 3: 0.98, 4: 0.6***REMOVED***,
            "PA1": {0: 0.77, 1: 0.64, 2: 0.52, 3: 0.98, 4: 0.67***REMOVED***,
            "PA3": {0: 0.34, 1: 0.70, 2: 0.52, 3: 0.98, 4: 0.67***REMOVED***,
        ***REMOVED***
    ...
    def test_raise_of_column_name_value(self):
        ...
        df = DataFrame({"col": list("ABC"), "value": range(10, 16, 2)***REMOVED***)
    ...
    def test_missing_stubname(self, dtype):
        # GH46044
        df = DataFrame({"id": ["1", "2"], "a-1": [100, 200], "a-2": [300, 400]***REMOVED***)
        df = df.astype({"id": dtype***REMOVED***)
        result = wide_to_long(
            df,
            stubnames=["a", "b"],
            i="id",
        ...
        expected = DataFrame(
            {"a": [100, 200, 300, 400], "b": [np.nan] * 4***REMOVED***,
            index=index,
        )
