...
def count_rows(conn, table_name: str):
    stmt = f"SELECT count(*) AS count_1 FROM {table_name***REMOVED***"
    adbc = import_optional_dependency("adbc_driver_manager.dbapi", errors="ignore")
    if isinstance(conn, sqlite3.Connection):
        cur = conn.cursor()
...
def drop_table(
    ...
    if isinstance(conn, sqlite3.Connection):
        conn.execute(f"DROP TABLE IF EXISTS {sql._get_valid_sqlite_name(table_name)***REMOVED***")
        conn.commit()
        ...
        if adbc and isinstance(conn, adbc.Connection):
            with conn.cursor() as cur:
                cur.execute(f'DROP TABLE IF EXISTS "{table_name***REMOVED***"')
        else:
            with conn.begin() as con:
...
def drop_view(
    ...
    if isinstance(conn, sqlite3.Connection):
        conn.execute(f"DROP VIEW IF EXISTS {sql._get_valid_sqlite_name(view_name)***REMOVED***")
        conn.commit()
    else:
        adbc = import_optional_dependency("adbc_driver_manager.dbapi", errors="ignore")
        if adbc and isinstance(conn, adbc.Connection):
            with conn.cursor() as cur:
                cur.execute(f'DROP VIEW IF EXISTS "{view_name***REMOVED***"')
        else:
            quoted_view = conn.engine.dialect.identifier_preparer.quote_identifier(
                view_name
***REMOVED***
            stmt = sqlalchemy.text(f"DROP VIEW IF EXISTS {quoted_view***REMOVED***")
            with conn.begin() as con:
...
def mysql_pymysql_engine():
    ...
    engine = sqlalchemy.create_engine(
        ...
        connect_args={"client_flag": pymysql.constants.CLIENT.MULTI_STATEMENTS***REMOVED***,
...
***REMOVED***
    ...
    iris_frame = read_sql_query(
        select(iris), conn, params={"name": "Iris-setosa", "length": 5.1***REMOVED***
    )
    check_iris_frame(iris_frame)
    if isinstance(conn, str):
...
***REMOVED***
    ...
    for db, query in sql_strings["read_parameters"].items():
        ...
        raise KeyError(f"No part of {conn***REMOVED*** found in sql_strings['read_parameters']")
    conn = request.getfixturevalue(conn)
...
def test_read_procedure(conn, request):
    ...
    df = DataFrame({"a": [1, 2, 3], "b": [0.1, 0.2, 0.3]***REMOVED***)
...
def test_copy_from_callable_insertion_method(conn, expected_count, request):
    ...
    def psql_insert_copy(table, conn, keys, data_iter):
        ...
        with dbapi_conn.cursor() as cur:
            ...
            columns = ", ".join([f'"{k***REMOVED***"' for k in keys])
            if table.schema:
                table_name = f"{table.schema***REMOVED***.{table.name***REMOVED***"
            else:
                table_name = table.name

            sql_query = f"COPY {table_name***REMOVED*** ({columns***REMOVED***) FROM STDIN WITH CSV"
            cur.copy_expert(sql=sql_query, file=s_buf)
        return expected_count

    conn = request.getfixturevalue(conn)
    expected = DataFrame({"col1": [1, 2], "col2": [0.1, 0.2], "col3": ["a", "n"]***REMOVED***)
...
def test_read_view_postgres(conn, request):
    ...
    table_name = f"group_{uuid.uuid4().hex***REMOVED***"
    view_name = f"group_view_{uuid.uuid4().hex***REMOVED***"

    sql_stmt = text(
        f"""
    CREATE TABLE {table_name***REMOVED*** (
        group_id INTEGER,
        name TEXT
    );
    INSERT INTO {table_name***REMOVED*** VALUES
        (1, 'name');
    CREATE VIEW {view_name***REMOVED***
    AS
    SELECT * FROM {table_name***REMOVED***;
    """
    )
    if isinstance(conn, Engine):
        with conn.connect() as con:
    ...
    expected = DataFrame({"group_id": [1], "name": "name"***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def flavor(conn_name):
    ...
    raise ValueError(f"unsupported connection: {conn_name***REMOVED***")
...
***REMOVED***
    ...
    params = {"name": "Iris-setosa", "length": 5.1***REMOVED***
    with pandasSQL_builder(conn) as pandasSQL:
        with pandasSQL.run_transaction():
            iris_frame = pandasSQL.read_query(query, params=params)
    check_iris_frame(iris_frame)
...
def test_api_roundtrip(conn, request, test_frame1):
    ...
    if "adbc" in conn_name:
        result = result.rename(columns={"__index_level_0__": "level_0"***REMOVED***)
...
def test_api_date_parsing(conn, request):
    ...
    df = sql.read_sql_query(
        "SELECT * FROM types",
        conn,
        parse_dates={"DateCol": "%Y-%m-%d %H:%M:%S"***REMOVED***,
    )
    assert issubclass(df.DateCol.dtype.type, np.datetime64)
    assert df.DateCol.tolist() == [
        Timestamp(2000, 1, 3, 0, 0, 0),
    ...
    df = sql.read_sql_query(
        "SELECT * FROM types", conn, parse_dates={"IntDateCol": "s"***REMOVED***
    )
    assert issubclass(df.IntDateCol.dtype.type, np.datetime64)
    assert df.IntDateCol.tolist() == [
        Timestamp(1986, 12, 25, 0, 0, 0),
    ...
    df = sql.read_sql_query(
        "SELECT * FROM types",
        conn,
        parse_dates={"IntDateOnlyCol": "%Y%m%d"***REMOVED***,
    )
...
def test_api_custom_dateparsing_error(
    ...
    expected = types_data_frame.astype({"DateCol": "datetime64[ns]"***REMOVED***)

    result = read_sql(
        text,
        con=conn,
        parse_dates={
            "DateCol": {"errors": error***REMOVED***,
        ***REMOVED***,
    )
...
def test_api_complex_raises(conn, request):
    conn_name = conn
    conn = request.getfixturevalue(conn)
    df = DataFrame({"a": [1 + 1j, 2j]***REMOVED***)
...
def test_api_to_sql_index_label(conn, request, index_name, index_label, expected):
    ...
    temp_frame = DataFrame({"col1": range(4)***REMOVED***)
...
***REMOVED***
    ...
    conn_name = conn
    conn = request.getfixturevalue(conn)
    float_frame = DataFrame({"a": [1.1, 1.2], "b": [2.1, 2.2]***REMOVED***)
    ...
    if conn_name == "sqlite_buildin":
        ...
        dtype = Integer
    create_sql = sql.get_schema(float_frame, "test", con=conn, dtype={"b": dtype***REMOVED***)
    assert "CREATE" in create_sql
...
***REMOVED***
    ...
    conn_name = conn
    conn = request.getfixturevalue(conn)
    frame = DataFrame({"Col1": [1.1, 1.2], "Col2": [2.1, 2.2]***REMOVED***)
...
def test_api_escaped_table_name(conn, request):
    ...
    df = DataFrame({"A": [0, 1, 2], "B": [0.2, np.nan, 5.6]***REMOVED***)
...
def test_api_read_sql_duplicate_columns(conn, request):
    ...
    df = DataFrame({"a": [1, 2, 3], "b": [0.1, 0.2, 0.3], "c": 1***REMOVED***)
...
def test_sqlalchemy_type_mapping(conn, request):
    ...
    df = DataFrame(
        {"time": to_datetime(["2014-12-12 01:54", "2014-12-11 02:54"], utc=True)***REMOVED***
    )
    with sql.SQLDatabase(conn) as db:
...
def test_query_by_text_obj(conn, request):
    ...
    if "postgres" in conn_name:
        ...
        name_text = text("select * from iris where name=:name")
    iris_df = sql.read_sql(name_text, conn, params={"name": "Iris-versicolor"***REMOVED***)
    all_names = set(iris_df["Name"])
    assert all_names == {"Iris-versicolor"***REMOVED***
...
def test_query_by_select_obj(conn, request):
    ...
    iris_df = sql.read_sql(name_select, conn, params={"name": "Iris-setosa"***REMOVED***)
    all_names = set(iris_df["Name"])
    assert all_names == {"Iris-setosa"***REMOVED***
...
def test_column_with_percentage(conn, request):
    ...
    conn = request.getfixturevalue(conn)
    df = DataFrame({"A": [0, 1, 2], "%_variation": [3, 4, 5]***REMOVED***)
...
def test_create_table(conn, request):
    ...
    temp_frame = DataFrame({"one": [1.0, 2.0, 3.0, 4.0], "two": [4.0, 3.0, 2.0, 1.0]***REMOVED***)
    with sql.SQLDatabase(conn, need_transaction=True) as pandasSQL:
...
def test_bigint(conn, request):
    ...
    conn = request.getfixturevalue(conn)
    df = DataFrame(data={"i64": [2**62]***REMOVED***)
...
def test_datetime_with_timezone_roundtrip(conn, request):
    ...
    expected = DataFrame(
        {"A": date_range("2013-01-01 09:00:00", periods=3, tz="US/Pacific")***REMOVED***
    )
...
def test_out_of_bounds_datetime(conn, request):
    # GH 26761
    conn = request.getfixturevalue(conn)
    data = DataFrame({"date": datetime(9999, 1, 1)***REMOVED***, index=[0])
...
def test_naive_datetimeindex_roundtrip(conn, request):
    ...
    expected = DataFrame({"nums": range(5)***REMOVED***, index=dates)
...
def test_date_parsing(conn, request):
    ...
    df = sql.read_sql_table("types", conn, parse_dates={"DateCol": "%Y-%m-%d %H:%M:%S"***REMOVED***)
    assert issubclass(df.DateCol.dtype.type, np.datetime64)

    df = sql.read_sql_table(
        "types",
        conn,
        parse_dates={"DateCol": {"format": "%Y-%m-%d %H:%M:%S"***REMOVED******REMOVED***,
    )
    assert issubclass(df.DateCol.dtype.type, np.datetime64)
    ...
    df = sql.read_sql_table("types", conn, parse_dates={"IntDateCol": "s"***REMOVED***)
    assert issubclass(df.IntDateCol.dtype.type, np.datetime64)

    df = sql.read_sql_table("types", conn, parse_dates={"IntDateCol": {"unit": "s"***REMOVED******REMOVED***)
    assert issubclass(df.IntDateCol.dtype.type, np.datetime64)
...
def test_datetime(conn, request):
    ...
    conn = request.getfixturevalue(conn)
    df = DataFrame(
        {"A": date_range("2013-01-01 09:00:00", periods=3), "B": np.arange(3.0)***REMOVED***
    )
...
def test_nan_fullcolumn(conn, request):
    ...
    conn = request.getfixturevalue(conn)
    df = DataFrame({"A": [0, 1, 2], "B": [np.nan, np.nan, np.nan]***REMOVED***)
    assert df.to_sql(name="test_nan", con=conn, index=False) == 3
...
def test_nan_string(conn, request):
    ...
    conn = request.getfixturevalue(conn)
    df = DataFrame({"A": [0, 1, 2], "B": ["a", "b", np.nan]***REMOVED***)
    assert df.to_sql(name="test_nan", con=conn, index=False) == 3
...
***REMOVED***
    ...
    if conn_name in {"sqlite_buildin", "sqlite_str"***REMOVED***:
        ixs = sql.read_sql_query(
            "SELECT * FROM sqlite_master WHERE type = 'index' "
            f"AND tbl_name = '{tbl_name***REMOVED***'",
            conn,
        )
        ix_cols = []
        for ix_name in ixs.name:
            ix_info = sql.read_sql_query(f"PRAGMA index_info({ix_name***REMOVED***)", conn)
...
def test_dtype(conn, request):
    ...
    assert df.to_sql(name="dtype_test2", con=conn, dtype={"B": TEXT***REMOVED***) == 2
    meta = MetaData()
    ...
    with pytest.raises(ValueError, match=msg):
        df.to_sql(name="error", con=conn, dtype={"B": str***REMOVED***)

    # GH9083
    assert df.to_sql(name="dtype_test3", con=conn, dtype={"B": String(10)***REMOVED***) == 2
    meta.reflect(bind=conn)
...
def test_double_precision(conn, request):
    ...
    assert (
        df.to_sql(
            ...
            index=False,
            if_exists="replace",
            dtype={"f64_as_f32": Float(precision=23)***REMOVED***,
        )
        == 1
    )
...
def test_connectable_issue_example(conn, request):
    ...
    assert (
        DataFrame({"test_foo_data": [0, 1, 2]***REMOVED***).to_sql(name="test_foo_data", con=conn)
        == 3
    )
    main(conn)
...
@pytest.mark.parametrize(
    "input",
    [{"foo": [np.inf]***REMOVED***, {"foo": [-np.inf]***REMOVED***, {"foo": [-np.inf], "infe0": ["bar"]***REMOVED***],
)
def test_to_sql_with_negative_npinf(conn, request, input):
    ...
    if "mysql" in conn_name:
        # GH 36465
        # The input {"foo": [-np.inf], "infe0": ["bar"]***REMOVED*** does not raise any error
...
def test_temporary_table(conn, request):
    ...
    expected = DataFrame({"spam": [test_data]***REMOVED***)
    Base = declarative_base()

    class Temporary(Base):
        __tablename__ = "temp_test"
        __table_args__ = {"prefixes": ["TEMPORARY"]***REMOVED***
...
def test_read_sql_dtype_backend(
    ...
    with pd.option_context("mode.string_storage", string_storage):
        result = getattr(pd, func)(
            f"Select * from {table***REMOVED***", conn, dtype_backend=dtype_backend
        )
...
***REMOVED***
        ...
        if "mysql" in conn_name or "sqlite" in conn_name:
            if dtype_backend == "numpy_nullable":
                df = df.astype({"e": "Int64", "f": "Int64"***REMOVED***)
            else:
                df = df.astype({"e": "int64[pyarrow]", "f": "int64[pyarrow]"***REMOVED***)
...
def test_chunksize_empty_dtypes(conn, request):
    ...
    conn = request.getfixturevalue(conn)
    dtypes = {"a": "int64", "b": "object"***REMOVED***
...
def test_read_sql_dtype(conn, request, func, dtype_backend):
    ...
    conn = request.getfixturevalue(conn)
    table = "test"
    df = DataFrame({"a": [1, 2, 3], "b": 5***REMOVED***)
    df.to_sql(name=table, con=conn, index=False, if_exists="replace")

    result = getattr(pd, func)(
        f"Select * from {table***REMOVED***",
        conn,
        dtype={"a": np.float64***REMOVED***,
        dtype_backend=dtype_backend,
    )
    expected = DataFrame(
        {
...
def test_keyword_deprecation(sqlite_engine):
    ...
    df = DataFrame([{"A": 1, "B": 2, "C": 3***REMOVED***, {"A": 1, "B": 2, "C": 3***REMOVED***])
...
def test_bigint_warning(sqlite_engine):
    ...
    df = DataFrame({"a": [1, 2]***REMOVED***, dtype="int64")
...
def test_valueerror_exception(sqlite_engine):
    conn = sqlite_engine
    df = DataFrame({"col1": [1, 2], "col2": [3, 4]***REMOVED***)
...
def test_row_object_is_named_tuple(sqlite_engine):
    ...
    with Session() as session:
        df = DataFrame({"id": [0, 1], "string_column": ["hello", "world"]***REMOVED***)
        assert (
...
def test_read_sql_string_inference(sqlite_engine):
    ...
    expected = DataFrame(
        {"a": ["x", "y"]***REMOVED***, dtype=dtype, columns=Index(["a"], dtype=dtype)
    )
...
def test_roundtripping_datetimes(sqlite_engine):
    conn = sqlite_engine
    # GH#54877
    df = DataFrame({"t": [datetime(2020, 12, 31, 12)]***REMOVED***, dtype="datetime64[ns]")
...
def test_psycopg2_schema_support(postgresql_psycopg2_engine):
    ...
    # mysql/sqlite)
    df = DataFrame({"col1": [1, 2], "col2": [0.1, 0.2], "col3": ["a", "n"]***REMOVED***)
...
def get_sqlite_column_type(conn, table, column):
    recs = conn.execute(f"PRAGMA table_info({table***REMOVED***)")
    for cid, name, ctype, not_null, default, pk in recs:
        if name == column:
            return ctype
    raise ValueError(f"Table {table***REMOVED***, column {column***REMOVED*** not found")
...
def test_sqlite_test_dtype(sqlite_buildin):
    ...
    df = DataFrame(data, columns=cols)
    assert df.to_sql(name="dtype_test", con=conn) == 2
    assert df.to_sql(name="dtype_test2", con=conn, dtype={"B": "STRING"***REMOVED***) == 2
    ...
    with pytest.raises(ValueError, match=msg):
        df.to_sql(name="error", con=conn, dtype={"B": bool***REMOVED***)
...
def test_sqlite_illegal_names(sqlite_buildin):
    ...
    for ndx, weird_name in enumerate(
        ...
        c_tbl = f"test_weird_col_name{ndx:d***REMOVED***"
...
def format_query(sql, *args):
    _formatters = {
        datetime: "'{***REMOVED***'".format,
        str: "'{***REMOVED***'".format,
        np.str_: "'{***REMOVED***'".format,
        bytes: "'{***REMOVED***'".format,
        float: "{:.8f***REMOVED***".format,
        int: "{:d***REMOVED***".format,
        type(None): lambda x: "NULL",
        np.float64: "{:.10f***REMOVED***".format,
        bool: "'{!s***REMOVED***'".format,
    ***REMOVED***
    processed_args = []
    for arg in args:
...
def test_xsqlite_keyword_as_column_names(sqlite_buildin):
    df = DataFrame({"From": np.ones(5)***REMOVED***)
...
def test_xsqlite_if_exists(sqlite_buildin):
    df_if_exists_1 = DataFrame({"col1": [1, 2], "col2": ["A", "B"]***REMOVED***)
    df_if_exists_2 = DataFrame({"col1": [3, 4, 5], "col2": ["C", "D", "E"]***REMOVED***)
    table_name = "table_if_exists"
    sql_select = f"SELECT * FROM {table_name***REMOVED***"
