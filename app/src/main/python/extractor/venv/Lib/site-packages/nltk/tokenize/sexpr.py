...
using the ``parens`` argument to the `SExprTokenizer` constructor:
    ...
    >>> SExprTokenizer(parens='{***REMOVED***').tokenize('{a b {c d***REMOVED******REMOVED*** e f {g***REMOVED***')
    ['{a b {c d***REMOVED******REMOVED***', 'e', 'f', '{g***REMOVED***']
...
class SExprTokenizer(TokenizerI):
    ...
    def __init__(self, parens="()", strict=True):
        ...
        self._paren_regexp = re.compile(
            f"{re.escape(parens[0])***REMOVED***|{re.escape(parens[1])***REMOVED***"
        )
