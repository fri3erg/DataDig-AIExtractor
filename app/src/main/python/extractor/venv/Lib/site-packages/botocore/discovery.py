...
***REMOVED***
    ...
    def discovery_operation_kwargs(self, **kwargs):
        ...
        return {k: v for k, v in kwargs.items() if k in input_keys***REMOVED***
...
class EndpointDiscoveryHandler:
    ...
    def gather_identifiers(self, params, model, context, **kwargs):
        ...
        context['discovery'] = {'identifiers': ids***REMOVED***

    def discover_endpoint(self, request, operation_name, **kwargs):
        ids = request.context.get('discovery', {***REMOVED***).get('identifiers')
        if ids is None:
            return
    ...
    def handle_retries(self, request_dict, response, operation, **kwargs):
        ...
        _, response = response
        status = response.get('ResponseMetadata', {***REMOVED***).get('HTTPStatusCode')
        error_code = response.get('Error', {***REMOVED***).get('Code')
        if status != 421 and error_code != 'InvalidEndpointException':
            return None

        context = request_dict.get('context', {***REMOVED***)
        ids = context.get('discovery', {***REMOVED***).get('identifiers')
        if ids is None:
            return None
