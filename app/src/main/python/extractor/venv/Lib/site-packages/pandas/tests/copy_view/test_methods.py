...
def test_copy(using_copy_on_write):
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [0.1, 0.2, 0.3]***REMOVED***)
    df_copy = df.copy()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda df, copy: df.astype({"b": "int64"***REMOVED***, copy=copy),
...
def test_methods_copy_keyword(
    ...
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [0.1, 0.2, 0.3]***REMOVED***, index=index)
...
@pytest.mark.parametrize(
    "method",
    [
        lambda ser, copy: ser.rename(index={0: 100***REMOVED***, copy=copy),
...
def test_reset_index(using_copy_on_write):
    ...
    # resulting dataframe
    df = DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": [0.1, 0.2, 0.3]***REMOVED***, index=[10, 11, 12]
    )
    df_orig = df.copy()
    df2 = df.reset_index()
...
def test_rename_columns(using_copy_on_write):
    ...
    if using_copy_on_write:
        assert np.shares_memory(get_array(df2, "C"), get_array(df, "c"))
    expected = DataFrame({"A": [0, 2, 3], "B": [4, 5, 6], "C": [0.1, 0.2, 0.3]***REMOVED***)
...
def test_pipe(using_copy_on_write):
    df = DataFrame({"a": [1, 2, 3], "b": 1.5***REMOVED***)
    df_orig = df.copy()
    ...
    if using_copy_on_write:
        ...
        expected = DataFrame({"a": [0, 2, 3], "b": 1.5***REMOVED***)
        tm.assert_frame_equal(df, expected)
...
def test_pipe_modify_df(using_copy_on_write):
    ...
    if using_copy_on_write:
        ...
        expected = DataFrame({"a": [100, 2, 3], "b": 1.5***REMOVED***)
        tm.assert_frame_equal(df, expected)
...
@pytest.mark.parametrize(
    "filter_kwargs", [{"items": ["a"]***REMOVED***, {"like": "a"***REMOVED***, {"regex": "a"***REMOVED***]
)
...
def test_pop(using_copy_on_write, warn_copy_on_write):
    ...
    if using_copy_on_write:
        ...
        expected = DataFrame({"a": [1, 2, 3], "b": [0, 5, 6], "c": [0.1, 0.2, 0.3]***REMOVED***)
...
def test_align_frame(using_copy_on_write, func):
    df = DataFrame({"a": [1, 2, 3], "b": "a"***REMOVED***)
    df_orig = df.copy()
...
def test_swapaxes_single_block(using_copy_on_write):
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]***REMOVED***, index=["x", "y", "z"])
    df_orig = df.copy()
    msg = "'DataFrame.swapaxes' is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
...
def test_swapaxes_read_only_array():
    df = DataFrame({"a": [1, 2], "b": 3***REMOVED***)
    msg = "'DataFrame.swapaxes' is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        df = df.swapaxes(axis1="index", axis2="columns")
    df.iloc[0, 0] = 100
    expected = DataFrame({0: [100, 3], 1: [2, 3]***REMOVED***, index=["a", "b"])
    tm.assert_frame_equal(df, expected)
...
@pytest.mark.parametrize("obj", [Series([1, 2], name="a"), DataFrame({"a": [1, 2]***REMOVED***)])
...
def test_set_index_mutating_parent_does_not_mutate_index():
    df = DataFrame({"a": [1, 2, 3], "b": 1***REMOVED***)
...
def test_add_prefix(using_copy_on_write):
    ...
    expected = DataFrame(
        {"CoW_a": [0, 2, 3], "CoW_b": [4, 5, 6], "CoW_c": [0.1, 0.2, 0.3]***REMOVED***
    )
    tm.assert_frame_equal(df2, expected)
    tm.assert_frame_equal(df, df_orig)
...
def test_add_suffix(using_copy_on_write):
    ...
    expected = DataFrame(
        {"a_CoW": [0, 2, 3], "b_CoW": [4, 5, 6], "c_CoW": [0.1, 0.2, 0.3]***REMOVED***
    )
    tm.assert_frame_equal(df2, expected)
    tm.assert_frame_equal(df, df_orig)
...
def test_dropna(using_copy_on_write, axis, val):
    df = DataFrame({"a": [1, 2, 3], "b": [4, val, 6], "c": "d"***REMOVED***)
    df_orig = df.copy()
...
def test_infer_objects(using_copy_on_write):
    df = DataFrame({"a": [1, 2], "b": "c", "c": 1, "d": "x"***REMOVED***)
    df_orig = df.copy()
...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"before": "a", "after": "b", "axis": 1***REMOVED***,
        {"before": 0, "after": 1, "axis": 0***REMOVED***,
    ],
)
def test_truncate(using_copy_on_write, kwargs):
    df = DataFrame({"a": [1, 2, 3], "b": 1, "c": 2***REMOVED***)
    df_orig = df.copy()
...
@pytest.mark.parametrize("obj", [Series([1, 2]), DataFrame({"a": [1, 2]***REMOVED***)])
...
def test_reindex_like(using_copy_on_write):
    df = DataFrame({"a": [1, 2], "b": "a"***REMOVED***)
    other = DataFrame({"b": "a", "a": [1, 2]***REMOVED***)
...
@pytest.mark.parametrize(
    "obj, kwargs",
    [(Series([1, 2, 3], name="a"), {***REMOVED***), (DataFrame({"a": [1, 2, 3]***REMOVED***), {"by": "a"***REMOVED***)],
)
...
def test_round(using_copy_on_write, warn_copy_on_write, decimals):
    df = DataFrame({"a": [1, 2], "b": "c"***REMOVED***)
    df_orig = df.copy()
...
def test_reorder_levels(using_copy_on_write):
    ...
    df = DataFrame({"a": [1, 2, 3, 4]***REMOVED***, index=index)
    df_orig = df.copy()
...
@pytest.mark.parametrize("obj", [Series([1, 2, 3]), DataFrame({"a": [1, 2, 3]***REMOVED***)])
...
@pytest.mark.parametrize("kwargs", [{"mapper": "test"***REMOVED***, {"index": "test"***REMOVED***])
def test_rename_axis(using_copy_on_write, kwargs):
    df = DataFrame({"a": [1, 2, 3, 4]***REMOVED***, index=Index([1, 2, 3, 4], name="a"))
    df_orig = df.copy()
...
def test_droplevel(using_copy_on_write):
    # GH 49473
    index = MultiIndex.from_tuples([(1, 1), (1, 2), (2, 1)], names=["one", "two"])
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]***REMOVED***, index=index)
    df_orig = df.copy()
    df2 = df.droplevel(0)
...
def test_putmask(using_copy_on_write, dtype, warn_copy_on_write):
    df = DataFrame({"a": [1, 2], "b": 1, "c": 2***REMOVED***, dtype=dtype)
    view = df[:]
    df_orig = df.copy()
    with tm.assert_cow_warning(warn_copy_on_write):
        df[df == df] = 5
...
def test_putmask_aligns_rhs_no_reference(using_copy_on_write, dtype):
    df = DataFrame({"a": [1.5, 2], "b": 1.5***REMOVED***, dtype=dtype)
    arr_a = get_array(df, "a")
    df[df == df] = DataFrame({"a": [5.5, 5]***REMOVED***)
...
def test_putmask_dont_copy_some_blocks(
    ...
    df = DataFrame({"a": [1, 2], "b": 1, "c": 1.5***REMOVED***)
    view = df[:]
    df_orig = df.copy()
    indexer = DataFrame(
...
def test_where_mask_noop_on_single_column(using_copy_on_write, dtype, val, func):
    df = DataFrame({"a": [1, 2, 3], "b": [-4, -5, -6]***REMOVED***, dtype=dtype)
    df_orig = df.copy()
...
def test_chained_where_mask(using_copy_on_write, func):
    df = DataFrame({"a": [1, 4, 2], "b": 1***REMOVED***)
    df_orig = df.copy()
    if using_copy_on_write:
...
def test_iterrows(using_copy_on_write):
    df = DataFrame({"a": 0, "b": 1***REMOVED***, index=[1, 2, 3])
    df_orig = df.copy()
...
def test_interpolate_creates_copy(using_copy_on_write, warn_copy_on_write):
    # GH#51126
    df = DataFrame({"a": [1.5, np.nan, 3]***REMOVED***)
    view = df[:]
    expected = df.copy()
    ...
    if using_copy_on_write:
        ...
        expected = DataFrame({"a": [100.5, 1.5, 3]***REMOVED***)
        tm.assert_frame_equal(view, expected)
...
def test_isetitem_series(using_copy_on_write, dtype):
    df = DataFrame({"a": [1, 2, 3], "b": np.array([4, 5, 6], dtype=dtype)***REMOVED***)
    ser = Series([7, 8, 9])
    ser_orig = ser.copy()
    df.isetitem(0, ser)
    ...
    ser.loc[0] = 0
    expected = DataFrame({"a": [7, 8, 9], "b": np.array([4, 5, 6], dtype=dtype)***REMOVED***)
    tm.assert_frame_equal(df, expected)


def test_isetitem_frame(using_copy_on_write):
    df = DataFrame({"a": [1, 2, 3], "b": 1, "c": 2***REMOVED***)
    rhs = DataFrame({"a": [4, 5, 6], "b": 2***REMOVED***)
    df.isetitem([0, 1], rhs)
    if using_copy_on_write:
...
def test_xs(
    ...
    df = DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": np.array([7, 8, 9], dtype=dtype)***REMOVED***
    )
    df_orig = df.copy()
...
def test_update_frame(using_copy_on_write, warn_copy_on_write):
    df1 = DataFrame({"a": [1.0, 2.0, 3.0], "b": [4.0, 5.0, 6.0]***REMOVED***)
    df2 = DataFrame({"b": [100.0]***REMOVED***, index=[1])
    df1_orig = df1.copy()
    view = df1[:]
    ...
    expected = DataFrame({"a": [1.0, 2.0, 3.0], "b": [4.0, 100.0, 6.0]***REMOVED***)
...
def test_apply_modify_row(using_copy_on_write, warn_copy_on_write):
    ...
    df = DataFrame({"A": [1, 2], "B": [3, 4]***REMOVED***)
    df_orig = df.copy()
    ...
    df = DataFrame({"A": [1, 2], "B": ["b", "c"]***REMOVED***)
    df_orig = df.copy()
