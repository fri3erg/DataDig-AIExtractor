...
***REMOVED***,
    ...
    def test_shift(self, simple_index):
        ...
        msg = (
            ...
            f"TimedeltaIndex; Got type {type(idx).__name__***REMOVED***"
        )
...
***REMOVED***
    ...
    def test_numeric_compat(self, simple_index):
        ...
        lmsg = "|".join(
            [
                rf"unsupported operand type\(s\) for \*: '{typ***REMOVED***' and 'int'",
                "cannot perform (__mul__|__truediv__|__floordiv__) with "
                f"this index type: ({cls***REMOVED***|{typ***REMOVED***)",
            ]
        )
        ...
        rmsg = "|".join(
            [
                rf"unsupported operand type\(s\) for \*: 'int' and '{typ***REMOVED***'",
    ...
    ***REMOVED***
    ...
    def test_repr_roundtrip(self, simple_index):
        if isinstance(simple_index, IntervalIndex):
            pytest.skip(f"Not a valid repr for {type(simple_index).__name__***REMOVED***")
        idx = simple_index
    ...
    ***REMOVED***
    ...
    def test_argsort(self, index):
        if isinstance(index, CategoricalIndex):
            pytest.skip(f"{type(self).__name__***REMOVED*** separately tested")
    ...
    def test_delete_base(self, index):
        ...
        if isinstance(index, RangeIndex):
            # tested in class
            pytest.skip(f"{type(self).__name__***REMOVED*** tested elsewhere")
        ...
        length = len(index)
        msg = f"index {length***REMOVED*** is out of bounds for axis 0 with size {length***REMOVED***"
        with pytest.raises(IndexError, match=msg):
            index.delete(length)
    ...
    def test_equals(self, index):
        if isinstance(index, IntervalIndex):
            pytest.skip(f"{type(index).__name__***REMOVED*** tested elsewhere")
    ...
    def test_fillna(self, index):
        ...
        if len(index) == 0:
            ...
            pytest.skip(f"{index.dtype***REMOVED*** cannot hold NAs")
        elif isinstance(index, Index) and is_integer_dtype(index.dtype):
            pytest.skip(f"Not relevant for Index with {index.dtype***REMOVED***")
    ...
    @pytest.mark.parametrize(
        "mapper",
        [
            lambda values, index: {i: e for e, i in zip(values, index)***REMOVED***,
    ...
    def test_shallow_copy_shares_cache(self, simple_index):
        ...
        assert shallow_copy._cache == {***REMOVED***

    def test_index_groupby(self, simple_index):
        idx = simple_index[:5]
        to_groupby = np.array([1, 2, np.nan, 2, 1])
        tm.assert_dict_equal(
            idx.groupby(to_groupby), {1.0: idx[[0, 4]], 2.0: idx[[1, 3]]***REMOVED***
        )
        ...
        expected = {ex_keys[0]: idx[[0, 4]], ex_keys[1]: idx[[1, 3]]***REMOVED***
    ...
    def test_holds_integer_deprecated(self, simple_index):
        # GH50243
        idx = simple_index
        msg = f"{type(idx).__name__***REMOVED***.holds_integer is deprecated. "
        with tm.assert_produces_warning(FutureWarning, match=msg):
            idx.holds_integer()
    ...
    def test_is_numeric_is_deprecated(self, simple_index):
        ...
        with tm.assert_produces_warning(
            FutureWarning,
            match=f"{type(idx).__name__***REMOVED***.is_numeric is deprecated. ",
        ):
            idx.is_numeric()
