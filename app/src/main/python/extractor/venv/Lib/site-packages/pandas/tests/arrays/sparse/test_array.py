...
***REMOVED***,
    ...
    def test_cumsum(self, data, expected, numpy):
        ...
        if numpy:  # numpy compatibility checks.
            ...
            msg = re.escape(f"axis(={axis***REMOVED***) out of bounds")
            with pytest.raises(ValueError, match=msg):
...
def test_map():
    ...
    # dict
    result = arr.map({0: 10, 1: 11, 2: 12***REMOVED***)
    tm.assert_sp_array_equal(result, expected)

    # series
    result = arr.map(pd.Series({0: 10, 1: 11, 2: 12***REMOVED***))
    tm.assert_sp_array_equal(result, expected)
...
def test_map_missing():
    ...
    result = arr.map({0: 10, 1: 11***REMOVED***)
    tm.assert_sp_array_equal(result, expected)
...
def test_dropna(fill_value):
    ...
    df = pd.DataFrame({"a": [0, 1], "b": arr***REMOVED***)
    expected_df = pd.DataFrame({"a": [1], "b": exp***REMOVED***, index=pd.Index([1]))
...
def test_drop_duplicates_fill_value():
    ...
    expected = pd.DataFrame({i: SparseArray([0.0], fill_value=0) for i in range(5)***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_zero_sparse_column():
    # GH 27781
    df1 = pd.DataFrame({"A": SparseArray([0, 0, 0]), "B": [1, 2, 3]***REMOVED***)
    df2 = pd.DataFrame({"A": SparseArray([0, 1, 0]), "B": [1, 2, 3]***REMOVED***)
    result = df1.loc[df1["B"] != 2]
    expected = df2.loc[df2["B"] != 2]
    tm.assert_frame_equal(result, expected)

    expected = pd.DataFrame({"A": SparseArray([0, 0]), "B": [1, 3]***REMOVED***, index=[0, 2])
    tm.assert_frame_equal(result, expected)
