...
def get_buffer_string(
    ...
    for m in messages:
        if isinstance(m, HumanMessage):
            ...
            raise ValueError(f"Got unsupported message type: {m***REMOVED***")
        message = f"{role***REMOVED***: {m.content***REMOVED***"
        if isinstance(m, AIMessage) and "function_call" in m.additional_kwargs:
            message += f"{m.additional_kwargs['function_call']***REMOVED***"
...
def _message_from_dict(message: dict) -> BaseMessage:
    ...
    if _type == "human":
        ...
        raise ValueError(f"Got unexpected message type: {_type***REMOVED***")
...
def message_chunk_to_message(chunk: BaseMessageChunk) -> BaseMessage:
    ...
    return chunk.__class__.__mro__[1](
        **{k: v for k, v in chunk.__dict__.items() if k != "type"***REMOVED***
    )
