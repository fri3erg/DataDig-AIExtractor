    ...
    def _format_headers(self) -> Dict[str, str]:
        ...
        ***REMOVED***
        ...
        if self.zapier_nla_oauth_access_token:
            headers.update(
                {"Authorization": f"Bearer {self.zapier_nla_oauth_access_token***REMOVED***"***REMOVED***
***REMOVED***
        else:
            headers.update({"X-API-Key": self.zapier_nla_api_key***REMOVED***)
    ...
    def _create_action_payload(  # type: ignore[no-untyped-def]
        ...
        data = params if params else {***REMOVED***
        data.update(
            {
        ...
        if preview_only:
            data.update({"preview_only": True***REMOVED***)
        return data

    def _create_action_url(self, action_id: str) -> str:
        """Create a url for an action."""
        return self.zapier_nla_api_base + f"exposed/{action_id***REMOVED***/execute/"
            ...
            if response.status_code == 401:
                if self.zapier_nla_oauth_access_token:
                    raise requests.HTTPError(
                        ...
                        f"refreshed. Err: {http_err***REMOVED***",
                        response=response,
        ***REMOVED***
                raise requests.HTTPError(
                    f"An unauthorized response occurred. Check that your api "
                    f"key is correct. Err: {http_err***REMOVED***",
                    response=response,
    ***REMOVED***
            raise http_err
    ...
    def preview(
        ...
        session = self._get_session()
        params = params if params else {***REMOVED***
        params.update({"preview_only": True***REMOVED***)
