...
def _response_to_result(
    ...
    for candidate in response.candidates:
        ...
        if author is None:
            raise ChatGooglePalmError(f"ChatResponse must have an author: {candidate***REMOVED***")

        content = _truncate_at_stop_tokens(candidate.get("content", ""), stop)
        if content is None:
            raise ChatGooglePalmError(f"ChatResponse must have a content: {candidate***REMOVED***")
...
class ChatGooglePalm(BaseChatModel, BaseModel):
    ...
    def lc_secrets(self) -> Dict[str, str]:
        return {"google_api_key": "GOOGLE_API_KEY"***REMOVED***
