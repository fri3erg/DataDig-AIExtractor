...
def rot90(m, k=1, axes=(0, 1)):
    ...
    if (axes[0] >= m.ndim or axes[0] < -m.ndim
        ...
        raise ValueError("Axes={***REMOVED*** out of range for array of ndim={***REMOVED***."
            .format(axes, m.ndim))
...
def average(a, axis=None, weights=None, returned=False, *,
    ...
    if keepdims is np._NoValue:
        ...
        keepdims_kw = {'keepdims': keepdims***REMOVED***
...
def piecewise(x, condlist, funclist, *args, **kw):
    ...
    if n == n2 - 1:  # compute the "otherwise" condition.
        ...
        raise ValueError(
            "with {***REMOVED*** condition(s), either {***REMOVED*** or {***REMOVED*** functions are expected"
            .format(n, n, n+1)
        )
...
def select(condlist, choicelist, default=0):
    ...
    try:
        ...
        msg = f'Choicelist elements do not have a common dtype: {e***REMOVED***'
    ...
    try:
        ...
        msg = f'Choicelists and default value do not have a common dtype: {e***REMOVED***'
        raise TypeError(msg) from None
    ...
    for i, cond in enumerate(condlist):
        if cond.dtype.type is not np.bool_:
            raise TypeError(
                'invalid entry {***REMOVED*** in condlist: should be boolean ndarray'.format(i))
...
def gradient(f, *varargs, axis=None, edge_order=1):
    ...
    Notes
    -----
    Assuming that :math:`f\\in C^{3***REMOVED***` (i.e., :math:`f` has at least 3 continuous
    derivatives) and let :math:`h_{****REMOVED***` be a non-homogeneous stepsize, we
    minimize the "consistency error" :math:`\\eta_{i***REMOVED***` between the true gradient
    and its estimate from a linear combination of the neighboring grid-points:

    .. math::

        \\eta_{i***REMOVED*** = f_{i***REMOVED***^{\\left(1\\right)***REMOVED*** -
                    \\left[ \\alpha f\\left(x_{i***REMOVED***\\right) +
                            \\beta f\\left(x_{i***REMOVED*** + h_{d***REMOVED***\\right) +
                            \\gamma f\\left(x_{i***REMOVED***-h_{s***REMOVED***\\right)
                    \\right]

    By substituting :math:`f(x_{i***REMOVED*** + h_{d***REMOVED***)` and :math:`f(x_{i***REMOVED*** - h_{s***REMOVED***)`
    ...
    .. math::
        ...
        \\left\\{
            \\begin{array***REMOVED***{r***REMOVED***
                \\alpha+\\beta+\\gamma=0 \\\\
                \\beta h_{d***REMOVED***-\\gamma h_{s***REMOVED***=1 \\\\
                \\beta h_{d***REMOVED***^{2***REMOVED***+\\gamma h_{s***REMOVED***^{2***REMOVED***=0
            \\end{array***REMOVED***
        \\right.

    The resulting approximation of :math:`f_{i***REMOVED***^{(1)***REMOVED***` is the following:

    .. math::

        \\hat f_{i***REMOVED***^{(1)***REMOVED*** =
            \\frac{
                h_{s***REMOVED***^{2***REMOVED***f\\left(x_{i***REMOVED*** + h_{d***REMOVED***\\right)
                + \\left(h_{d***REMOVED***^{2***REMOVED*** - h_{s***REMOVED***^{2***REMOVED***\\right)f\\left(x_{i***REMOVED***\\right)
                - h_{d***REMOVED***^{2***REMOVED***f\\left(x_{i***REMOVED***-h_{s***REMOVED***\\right)***REMOVED***
                { h_{s***REMOVED***h_{d***REMOVED***\\left(h_{d***REMOVED*** + h_{s***REMOVED***\\right)***REMOVED***
            + \\mathcal{O***REMOVED***\\left(\\frac{h_{d***REMOVED***h_{s***REMOVED***^{2***REMOVED***
                                + h_{s***REMOVED***h_{d***REMOVED***^{2***REMOVED******REMOVED***{h_{d***REMOVED***
                                + h_{s***REMOVED******REMOVED***\\right)

    It is worth noting that if :math:`h_{s***REMOVED***=h_{d***REMOVED***`
    ...
    .. math::
        ...
        \\hat f_{i***REMOVED***^{(1)***REMOVED***=
            \\frac{f\\left(x_{i+1***REMOVED***\\right) - f\\left(x_{i-1***REMOVED***\\right)***REMOVED***{2h***REMOVED***
            + \\mathcal{O***REMOVED***\\left(h^{2***REMOVED***\\right)
...
_DIMENSION_NAME = r'\w+'
_CORE_DIMENSION_LIST = '(?:{0:***REMOVED***(?:,{0:***REMOVED***)*)?'.format(_DIMENSION_NAME)
_ARGUMENT = r'\({***REMOVED***\)'.format(_CORE_DIMENSION_LIST)
_ARGUMENT_LIST = '{0:***REMOVED***(?:,{0:***REMOVED***)*'.format(_ARGUMENT)
_SIGNATURE = '^{0:***REMOVED***->{0:***REMOVED***$'.format(_ARGUMENT_LIST)
...
def _parse_gufunc_signature(signature):
    ...
    if not re.match(_SIGNATURE, signature):
        raise ValueError(
            'not a valid gufunc signature: {***REMOVED***'.format(signature))
...
class vectorize:
    ...
    def __init__(self, pyfunc=np._NoValue, otypes=None, doc=None,
        ...
        self._ufunc = {***REMOVED***    # Caching to improve default performance
        self._doc = None
        self.__doc__ = doc
...
def bartlett(M):
    ...
    .. math:: w(n) = \\frac{2***REMOVED***{M-1***REMOVED*** \\left(
...
def hanning(M):
    ...
    .. math::  w(n) = 0.5 - 0.5\\cos\\left(\\frac{2\\pi{n***REMOVED******REMOVED***{M-1***REMOVED***\\right)
...
def hamming(M):
    ...
    .. math::  w(n) = 0.54 - 0.46\\cos\\left(\\frac{2\\pi{n***REMOVED******REMOVED***{M-1***REMOVED***\\right)
               \\qquad 0 \\leq n \\leq M-1
...
def kaiser(M, beta):
    ...
    .. math::  w(n) = I_0\\left( \\beta \\sqrt{1-\\frac{4n^2***REMOVED***{(M-1)^2***REMOVED******REMOVED***
               \\right)/I_0(\\beta)
...
def percentile(a,
    ...
    axis : {int, tuple of int, None***REMOVED***, optional
...
def _check_interpolation_as_method(method, interpolation, fname):
    ...
    warnings.warn(
        f"the `interpolation=` argument to {fname***REMOVED*** was renamed to "
...
def _quantile(
    ...
    try:
        ...
        raise ValueError(
            f"{method!r***REMOVED*** is not a valid method. Use one of: "
            f"{_QuantileMethods.keys()***REMOVED***") from None
...
def trapz(y, x=None, dx=1.0, axis=-1):
    ...
    \int_t y(t) \left.\frac{dx***REMOVED***{dt***REMOVED***\right|_{x=x(t)***REMOVED*** dt`.
...
def insert(arr, obj, values, axis=None):
    ...
    if indices.size == 1:
        ...
        if index < -N or index > N:
            raise IndexError(f"index {obj***REMOVED*** is out of bounds for axis {axis***REMOVED*** "
                             f"with size {N***REMOVED***")
        if (index < 0):
            index += N
