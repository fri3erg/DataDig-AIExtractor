...
def test_append_series(setup_path):
    with ensure_clean_store(setup_path) as store:
        # basic
        ss = Series(range(20), dtype=np.float64, index=[f"i_{i***REMOVED***" for i in range(20)])
        ts = Series(
...
***REMOVED***
        ...
        # avoid truncation on elements
        df = DataFrame([[123, "asdqwerty"], [345, "dggnhebbsdfbdfb"]])
        store.append("df_big2", df, min_itemsize={"values": 50***REMOVED***)
        ...
        store.append("ss", df["B"], min_itemsize={"index": 4***REMOVED***)
        tm.assert_series_equal(store.select("ss"), df["B"])

        # same as above, with data_columns=True
        store.append("ss2", df["B"], data_columns=True, min_itemsize={"index": 4***REMOVED***)
        tm.assert_series_equal(store.select("ss2"), df["B"])

        # min_itemsize in index without appending (GH 10381)
        store.put("ss3", df, format="table", min_itemsize={"index": 6***REMOVED***)
        ...
        store.put("ss4", df["B"], format="table", min_itemsize={"index": 6***REMOVED***)
    ...
    with ensure_clean_store(setup_path) as store:
        df = DataFrame({"A": "foo", "B": "bar"***REMOVED***, index=range(10))

        # a min_itemsize that creates a data_column
        _maybe_remove(store, "df")
        store.append("df", df, min_itemsize={"A": 200***REMOVED***)
        check_col("df", "A", 200)
        assert store.get_storer("df").data_columns == ["A"]

        # a min_itemsize that creates a data_column2
        _maybe_remove(store, "df")
        store.append("df", df, data_columns=["B"], min_itemsize={"A": 200***REMOVED***)
        check_col("df", "A", 200)
        assert store.get_storer("df").data_columns == ["B", "A"]

        # a min_itemsize that creates a data_column2
        _maybe_remove(store, "df")
        store.append("df", df, data_columns=["B"], min_itemsize={"values": 200***REMOVED***)
        check_col("df", "B", 200)
        ...
        with pytest.raises(ValueError, match=msg):
            store.append("df", df, min_itemsize={"foo": 20, "foobar": 20***REMOVED***)


def test_append_with_empty_string(setup_path):
    with ensure_clean_store(setup_path) as store:
        # with all empty strings (GH 12242)
        df = DataFrame({"x": ["a", "b", "c", "d", "e", "f", ""]***REMOVED***)
        store.append("df", df[:-1], min_itemsize={"x": 1***REMOVED***)
        store.append("df", df[-1:], min_itemsize={"x": 1***REMOVED***)
...
***REMOVED***
    ...
    with ensure_clean_store(setup_path) as store:
        _maybe_remove(store, "df")
        store.append("df", df_new, data_columns=["string"], min_itemsize={"string": 30***REMOVED***)
        check_col("df", "string", 30)
        _maybe_remove(store, "df")
        store.append("df", df_new, data_columns=["string"], min_itemsize=30)
        check_col("df", "string", 30)
        _maybe_remove(store, "df")
        store.append("df", df_new, data_columns=["string"], min_itemsize={"values": 30***REMOVED***)
        check_col("df", "string", 30)
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        store.append(
            ...
            min_itemsize={"string": 30, "string2": 40, "values": 50***REMOVED***,
        )
        check_col("df", "string", 30)
...
def test_append_to_multiple(setup_path):
    ...
    df2 = df1.copy().rename(columns="{***REMOVED***_2".format)
    df2["foo"] = "bar"
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        with pytest.raises(ValueError, match=msg):
            store.append_to_multiple(
                {"df1": ["A", "B"], "df2": None***REMOVED***, df, selector="df3"
***REMOVED***

        with pytest.raises(ValueError, match=msg):
            store.append_to_multiple({"df1": None, "df2": None***REMOVED***, df, selector="df3")
        ...
        # regular operation
        store.append_to_multiple({"df1": ["A", "B"], "df2": None***REMOVED***, df, selector="df1")
...
def test_append_to_multiple_dropna(setup_path):
    ...
    df2 = DataFrame(
        ...
        columns=Index(list("ABCD"), dtype=object),
        index=date_range("2000-01-01", periods=10, freq="B"),
    ).rename(columns="{***REMOVED***_2".format)
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        store.append_to_multiple(
            {"df1": ["A", "B"], "df2": None***REMOVED***, df, selector="df1", dropna=True
        )
...
def test_append_to_multiple_dropna_false(setup_path):
    ...
    with ensure_clean_store(setup_path) as store, pd.option_context(
        ...
        store.append_to_multiple(
            {"df1a": ["A", "B"], "df2a": None***REMOVED***, df, selector="df1a", dropna=False
        )
    ...
    with ensure_clean_store(setup_path) as store:
        store.append_to_multiple(
            ...
            ***REMOVED***,
            df.iloc[[0]],
            "index",
            min_itemsize={"Str": 10, "LongStr": 100, "Num": 2***REMOVED***,
        )
