...
4,5.5"""
    ...
    result = parser.read_csv(StringIO(data), dtype={"one": np.float64, 1: str***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
4,5.5"""
    ...
    with pytest.raises(TypeError, match="data type [\"']foo[\"'] not understood"):
        parser.read_csv(StringIO(data), dtype={"one": "foo", 1: "int"***REMOVED***)
...
2001,106380451,67"""
    ...
    with pytest.raises(ValueError, match=msg):
        parser.read_csv(StringIO(data), dtype={"DOY": np.int64***REMOVED***, skipinitialspace=True)
...
1.2,2.3"""
    ...
    if parser.engine == "pyarrow":
        ...
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(
                StringIO(data), dtype={"a": "i8"***REMOVED***, converters={"a": lambda x: str(x)***REMOVED***
***REMOVED***
        return
    ...
    result = parser.read_csv_check_warnings(
        ...
        StringIO(data),
        dtype={"a": "i8"***REMOVED***,
        converters={"a": lambda x: str(x)***REMOVED***,
    )
    expected = DataFrame({"a": ["1.1", "1.2"], "b": [2.2, 2.3]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_delimiter_with_usecols_and_parse_dates(all_parsers):
    ...
    expected = DataFrame(
        {"col1": [-9.1], "col2": [-9.1], "col3": [Timestamp("2010-10-10")]***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
...
def decimal_number_check(request, parser, numeric_decimal, thousands, float_precision):
    ...
    if thousands is None and value in ("1_,", "1_234,56", "1_234,56e0"):
        request.applymarker(
            pytest.mark.xfail(reason=f"thousands={thousands***REMOVED*** and sep is in {value***REMOVED***")
        )
    df = parser.read_csv(
        StringIO(value),
...
0,aaa
    ...
    result = parser.read_csv(
        ...
        true_values=["yes"],
        false_values=["no"],
        dtype={"a": "boolean"***REMOVED***,
    )
    expected = DataFrame(
        {"a": [True, False, True, False], "b": ["xxx", "yyy", "zzz", "aaa"]***REMOVED***
    )
...
@pytest.mark.usefixtures("pyarrow_xfail")
@pytest.mark.parametrize("dtypes, exp_value", [({***REMOVED***, "1"), ({"a.1": "int64"***REMOVED***, 1)])
def test_dtype_mangle_dup_cols(all_parsers, dtypes, exp_value):
    # GH#35211
    parser = all_parsers
    data = """a,a\n1,1"""
    dtype_dict = {"a": str, **dtypes***REMOVED***
    # GH#42462
    dtype_dict_copy = dtype_dict.copy()
    result = parser.read_csv(StringIO(data), dtype=dtype_dict)
    expected = DataFrame({"a": ["1"], "a.1": [exp_value]***REMOVED***)
...
def test_dtype_mangle_dup_cols_single_dtype(all_parsers):
    ...
    expected = DataFrame({"a": ["1"], "a.1": ["1"]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
"""
    ...
    expected = DataFrame({"a": [1], "b": 2.0***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
"""
    ...
    parser = all_parsers
    result = parser.read_csv(StringIO(data), dtype=dtype)
    expected = DataFrame({"a": [1], "b": [2.0], "a.1": [3], "b.2": [4.0], "b.1": [5]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
"""
    ...
    expected = DataFrame({"a": [1.0, np.nan], "b": [2.5, np.nan]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
"""
    ...
    expected = DataFrame({"a": [1], "b": 2***REMOVED***, dtype="Int64")
    tm.assert_frame_equal(result, expected)
...
"""
    result = parser.read_csv(StringIO(data), dtype={"a": "Int64"***REMOVED***)
    expected = DataFrame(
        {
            "a": IntegerArray(
...
,3"""
    ...
    expected = DataFrame(
        {"a": pd.Series(["x", "y", None], dtype=dtype), "b": [1, 2, 3]***REMOVED***,
...
z,a"""
    ...
    with pd.option_context("future.infer_string", True):
        result = parser.read_csv(StringIO(data), dtype={"a": dtype***REMOVED***)
...
NVDA,20230301181139587,2023552585717889827,2023552585717263361"""
    orders = pd.read_csv(StringIO(data), dtype={"ID_DEAL": pd.Int64Dtype()***REMOVED***)
...
4,5,6"""
    ...
    result = parser.read_csv(StringIO(data), usecols=["a", "c"], dtype={"a": object***REMOVED***)
    if parser.engine == "pyarrow":
        values = [1, 4]
    else:
        values = ["1", "4"]
    expected = DataFrame({"a": pd.Series(values, dtype=object), "c": [3, 6]***REMOVED***)
    tm.assert_frame_equal(result, expected)
