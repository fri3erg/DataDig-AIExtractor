...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"dtype": "float64"***REMOVED***,  # dtype mismatch
        {"index": [1, 2, 4]***REMOVED***,  # index mismatch
        {"name": "foo"***REMOVED***,  # name mismatch
    ],
)
...
categories_dtype={dtype***REMOVED***\\)
\\[right\\]: CategoricalDtype\\(categories=\\['a', 'b', 'c'\\], \
ordered=False, categories_dtype={dtype***REMOVED***\\)"""
...
def test_check_dtype_false_different_reso(dtype):
    # GH 52449
    ser_s = Series([1000213, 2131232, 21312331]).astype(f"{dtype***REMOVED***[s]")
    ser_ms = ser_s.astype(f"{dtype***REMOVED***[ms]")
    ...
    ser_ms -= Series([1, 1, 1]).astype(f"{dtype***REMOVED***[ms]")
