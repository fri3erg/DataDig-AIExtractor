...
def get_context(method=None):
    ...
    try:
        ...
        raise ValueError(
            f"Unknown context '{method***REMOVED***'. Value should be in "
            f"{START_METHODS***REMOVED***."
        )
...
def set_start_method(method, force=False):
    ...
    assert method is None or method in START_METHODS, (
        f"'{method***REMOVED***' is not a valid start_method. It should be in "
        f"{START_METHODS***REMOVED***"
    )
...
***REMOVED***
            ...
            cpu_info = {line for line in cpu_info if not line.startswith("#")***REMOVED***
...
***REMOVED***
            ...
            raise NotImplementedError(f"unsupported platform: {sys.platform***REMOVED***")

        # if cpu_count_physical < 1, we did not find a valid value
        if cpu_count_physical < 1:
            raise ValueError(f"found {cpu_count_physical***REMOVED*** physical cores < 1")
...
class LokyInitMainContext(LokyContext):
    ...
    ``executor`` should be protected by ``***REMOVED***`` and the
