...
def from_pydot(P):
    ...
    try:
        ...
        pass  # N.graph['node']={***REMOVED***
    try:
        N.graph["edge"] = P.get_edge_defaults()[0]
    except (IndexError, TypeError):
        pass  # N.graph['edge']={***REMOVED***
    return N
...
def to_pydot(N):
    ...
    name = N.name
    graph_defaults = N.graph.get("graph", {***REMOVED***)
    if name == "":
        P = pydot.Dot("", graph_type=graph_type, strict=strict, **graph_defaults)
    else:
        P = pydot.Dot(
            f'"{name***REMOVED***"', graph_type=graph_type, strict=strict, **graph_defaults
        )
    try:
    ...
    for n, nodedata in N.nodes(data=True):
        str_nodedata = {str(k): str(v) for k, v in nodedata.items()***REMOVED***
...
***REMOVED***
    ...
    if N.is_multigraph():
        for u, v, key, edgedata in N.edges(data=True, keys=True):
            str_edgedata = {str(k): str(v) for k, v in edgedata.items() if k != "key"***REMOVED***
            u, v = str(u), str(v)
            raise_error = (
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        for u, v, edgedata in N.edges(data=True):
            str_edgedata = {str(k): str(v) for k, v in edgedata.items()***REMOVED***
            u, v = str(u), str(v)
            raise_error = (
                _check_colon_quotes(u)
                or _check_colon_quotes(v)
...
def pydot_layout(G, prog="neato", root=None):
    ...
    if D == "":  # no data returned
        print(f"Graphviz layout with {prog***REMOVED*** failed")
        print()
        print("To debug what happened try:")
        print("P = nx.nx_pydot.to_pydot(G)")
        print('P.write_dot("file.dot")')
        print(f"And then run {prog***REMOVED*** on file.dot")
        return
