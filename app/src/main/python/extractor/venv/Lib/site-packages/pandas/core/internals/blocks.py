...
class Block(PandasObject, libinternals.Block):
    ...
    def __repr__(self) -> str:
        ...
        if self.ndim == 1:
            result = f"{name***REMOVED***: {len(self)***REMOVED*** dtype: {self.dtype***REMOVED***"
        else:
            shape = " x ".join([str(s) for s in self.shape])
            result = f"{name***REMOVED***: {self.mgr_locs.indexer***REMOVED***, {shape***REMOVED***, dtype: {self.dtype***REMOVED***"
...
***REMOVED***
        ...
        if warn_on_upcast:
            warnings.warn(
                ...
                f"Value '{other***REMOVED***' has dtype incompatible with {self.values.dtype***REMOVED***, "
...
***REMOVED***
        if self.values.dtype == new_dtype:
            raise AssertionError(
                f"Did not expect new dtype {new_dtype***REMOVED*** to equal self.dtype "
                f"{self.values.dtype***REMOVED***. Please report a bug at "
                "https://github.com/pandas-dev/pandas/issues."
***REMOVED***
    ...
    ***REMOVED***
    ...
    def astype(
        ...
        copy : bool, default False
            copy if indicated
        errors : str, {'raise', 'ignore'***REMOVED***, default 'raise'
        ...
        if newb.shape != self.shape:
            raise TypeError(
                f"cannot set astype for copy = [{copy***REMOVED***] for dtype "
                f"({self.dtype.name***REMOVED*** [{self.shape***REMOVED***]) to different shape "
                f"({newb.dtype.name***REMOVED*** [{newb.shape***REMOVED***])"
***REMOVED***
        return newb
    ...
    ***REMOVED***
    ...
    def pad_or_backfill(
        ...
        *,
        ...
        kwargs: dict[str, Any] = {"method": method, "limit": limit***REMOVED***
        if "limit_area" in inspect.signature(values._pad_or_backfill).parameters:
            kwargs["limit_area"] = limit_area
        elif limit_area is not None:
            raise NotImplementedError(
                f"{type(values).__name__***REMOVED*** does not implement limit_area "
...
***REMOVED***
    ...
    ***REMOVED***
                ...
                warnings.warn(
                    ...
                    f"{type(self.values).__name__***REMOVED***.",
                    DeprecationWarning,
                    stacklevel=find_stack_level(),
    ***REMOVED***
            else:
    ...
    def iget(self, i: int | tuple[int, int] | tuple[slice, int]):
        ...
        if isinstance(i, tuple):
            ...
            if not com.is_null_slice(col) and col != 0:
                raise IndexError(f"{self***REMOVED*** only contains one item")
...
def check_ndim(values, placement: BlockPlacement, ndim: int) -> None:
    ...
    if values.ndim > ndim:
        ...
        raise ValueError(
            ...
            f"values.ndim > ndim [{values.ndim***REMOVED*** > {ndim***REMOVED***]"
        )
...
***REMOVED***
        if len(placement) != len(values):
            raise ValueError(
                f"Wrong number of items passed {len(values)***REMOVED***, "
                f"placement implies {len(placement)***REMOVED***"
***REMOVED***
