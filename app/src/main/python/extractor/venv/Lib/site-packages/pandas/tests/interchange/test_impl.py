...
def test_categorical_dtype(data, data_categorical):
    df = pd.DataFrame({"A": (data_categorical[data[0]])***REMOVED***)
...
def test_categorical_pyarrow():
    ...
    table = pa.table({"weekday": pa.array(arr).dictionary_encode()***REMOVED***)
    exchange_df = table.__dataframe__()
    result = from_dataframe(exchange_df)
    weekday = pd.Categorical(
        arr, categories=["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
    )
    expected = pd.DataFrame({"weekday": weekday***REMOVED***)
...
def test_empty_categorical_pyarrow():
    ...
    arr = [None]
    table = pa.table({"arr": pa.array(arr, "float64").dictionary_encode()***REMOVED***)
    exchange_df = table.__dataframe__()
    result = pd.api.interchange.from_dataframe(exchange_df)
    expected = pd.DataFrame({"arr": pd.Categorical([np.nan])***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_large_string_pyarrow():
    ...
    arr = ["Mon", "Tue"]
    table = pa.table({"weekday": pa.array(arr, "large_string")***REMOVED***)
    exchange_df = table.__dataframe__()
    result = from_dataframe(exchange_df)
    expected = pd.DataFrame({"weekday": ["Mon", "Tue"]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_bitmasks_pyarrow(offset, length, expected_values):
    ...
    arr = [3.3, None, 2.1]
    table = pa.table({"arr": arr***REMOVED***).slice(offset, length)
    exchange_df = table.__dataframe__()
    result = from_dataframe(exchange_df)
    expected = pd.DataFrame({"arr": expected_values***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_dataframe(data):
    NCOLS, NROWS = 10, 20
    data = {
        f"col{int((i - NCOLS / 2) % NCOLS + 1)***REMOVED***": [data() for _ in range(NROWS)]
...
def test_missing_from_masked():
    ...
    dict_null = {col: rng.integers(low=0, high=len(df)) for col in df.columns***REMOVED***
...
@pytest.mark.parametrize(
    "data",
    [
        {"x": [1.5, 2.5, 3.5], "y": [9.2, 10.5, 11.8]***REMOVED***,
        {"x": [1, 2, 0], "y": [9.2, 10.5, 11.8]***REMOVED***,
        {
...
def test_string(string_data):
    ...
    df = pd.DataFrame({"A": test_str_data***REMOVED***)
    col = df.__dataframe__().get_column_by_name("A")
...
def test_nonstring_object():
    df = pd.DataFrame({"A": ["a", 10, 1.0, ()]***REMOVED***)
    col = df.__dataframe__().get_column_by_name("A")
...
def test_datetime():
    df = pd.DataFrame({"A": [pd.Timestamp("2022-01-01"), pd.NaT]***REMOVED***)
    col = df.__dataframe__().get_column_by_name("A")
...
def test_categorical_to_numpy_dlpack():
    ...
    df = pd.DataFrame({"A": pd.Categorical(["a", "b", "a"])***REMOVED***)
    col = df.__dataframe__().get_column_by_name("A")
...
@pytest.mark.parametrize("data", [{***REMOVED***, {"a": []***REMOVED***])
...
def test_datetimetzdtype(tz, unit):
    ...
    df = pd.DataFrame({"ts_tz": tz_data***REMOVED***)
    tm.assert_frame_equal(df, from_dataframe(df.__dataframe__()))
...
***REMOVED***,
    ...
    table = pa.table({"arr": arr***REMOVED***)
    exchange_df = table.__dataframe__()
    result = from_dataframe(exchange_df)
...
def test_interchange_from_corrected_buffer_dtypes(monkeypatch) -> None:
    ...
    df = pd.DataFrame({"a": ["foo", "bar"]***REMOVED***).__dataframe__()
...
def test_empty_string_column():
    ...
    df = pd.DataFrame({"a": []***REMOVED***, dtype=str)
    df2 = df.__dataframe__()
...
def test_large_string():
    ...
    df = pd.DataFrame({"a": ["x"]***REMOVED***, dtype="large_string[pyarrow]")
    result = pd.api.interchange.from_dataframe(df.__dataframe__())
    expected = pd.DataFrame({"a": ["x"]***REMOVED***, dtype="object")
    tm.assert_frame_equal(result, expected)
