...
class TestEmptyConcat:
    def test_handle_empty_objects(self, sort, using_infer_string):
        ...
        # GH3259
        df = DataFrame(
            {"A": range(10000)***REMOVED***, index=date_range("20130101", periods=10000, freq="s")
        )
        empty = DataFrame()
    ...
    def test_concat_empty_series(self):
        ...
        exp = DataFrame(
            {"x": [1, 2, 3], "y": [np.nan, np.nan, np.nan]***REMOVED***,
            index=RangeIndex(3),
        )
        ...
        res = concat([s1, s2], axis=1)
        exp = DataFrame(
            {"x": [1, 2, 3], 0: [np.nan, np.nan, np.nan]***REMOVED***,
            columns=["x", 0],
            index=RangeIndex(3),
        )
        tm.assert_frame_equal(res, exp)
    ...
    def test_concat_empty_series_dtypes_roundtrips(self, dtype, dtype2):
        ...
        def int_result_type(dtype, dtype2):
            typs = {dtype.kind, dtype2.kind***REMOVED***
            if not len(typs - {"i", "u", "b"***REMOVED***) and (
                dtype.kind == "i" or dtype2.kind == "i"
***REMOVED***:
                return "i"
            elif not len(typs - {"u", "b"***REMOVED***) and (
...
***REMOVED***:
        ...
        def float_result_type(dtype, dtype2):
            typs = {dtype.kind, dtype2.kind***REMOVED***
            if not len(typs - {"f", "i", "u"***REMOVED***) and (
...
***REMOVED***.dtype
    ...
    def test_concat_empty_df_object_dtype(self):
        # GH 9149
        df_1 = DataFrame({"Row": [0, 1, 1], "EmptyCol": np.nan, "NumberCol": [1, 2, 3]***REMOVED***)
    ...
    def test_concat_inner_join_empty(self):
        ...
        df_expected = DataFrame({"a": []***REMOVED***, index=RangeIndex(0), dtype="int64")
    ...
    def test_concat_empty_dataframe_different_dtypes(self, using_infer_string):
        # 39037
        df1 = DataFrame({"a": [1, 2, 3], "b": ["a", "b", "c"]***REMOVED***)
        df2 = DataFrame({"a": [1, 2, 3]***REMOVED***)
    ...
    def test_concat_to_empty_ea(self):
        ...
        df_empty = DataFrame({"a": pd.array([], dtype=pd.Int64Dtype())***REMOVED***)
        df_new = DataFrame({"a": pd.array([1, 2, 3], dtype=pd.Int64Dtype())***REMOVED***)
