...
Simplify
    ...
    ...
    Cannot apply (g -o f) to f. Cannot unify g with f given {***REMOVED***
    >>> print(read_expr(r'(G -o f)(g)').simplify())
    f
...
Test BindingDict
    ...
    >>> d1 = BindingDict({H: h***REMOVED***)
    >>> d2 = BindingDict({F: f, G: F***REMOVED***)
    >>> d12 = d1 + d2
    ...
    >>> BindingDict([(F,f),(G,g),(H,h)]) == BindingDict({F:f, G:g, H:h***REMOVED***)
    True

    >>> d4 = BindingDict({F: f***REMOVED***)
    >>> try: d4[F] = g
...
Test Unify
    ...
    ...
    Cannot unify f with g given {***REMOVED***

    >>> f.unify(G, BindingDict()) == BindingDict({G: f***REMOVED***)
    True
    >>> try: f.unify(G, BindingDict({G: h***REMOVED***))
    ... except UnificationException as e: print(e)
    ...
    Cannot unify f with G given {G: h***REMOVED***
    >>> f.unify(G, BindingDict({G: f***REMOVED***)) == BindingDict({G: f***REMOVED***)
    True
    >>> f.unify(G, BindingDict({H: f***REMOVED***)) == BindingDict({G: f, H: f***REMOVED***)
    True

    >>> G.unify(f, BindingDict()) == BindingDict({G: f***REMOVED***)
    True
    >>> try: G.unify(f, BindingDict({G: h***REMOVED***))
    ... except UnificationException as e: print(e)
    ...
    Cannot unify G with f given {G: h***REMOVED***
    >>> G.unify(f, BindingDict({G: f***REMOVED***)) == BindingDict({G: f***REMOVED***)
    True
    >>> G.unify(f, BindingDict({H: f***REMOVED***)) == BindingDict({G: f, H: f***REMOVED***)
    True
...
Compiled Premises:
    ...
    >>> g3 = ahc[2]
    >>> print(g3)
    \P Q.exists x.(P(x) & Q(x)) : (gr[1] -o (G[2] -o G)) : {3***REMOVED***
    >>> g4 = man.compile(counter)[0]
    >>> print(g4)
    \x.man(x) : (gv -o gr) : {4***REMOVED***
    >>> g5 = walks.compile(counter)[0]
    >>> print(g5)
    \x.walks(x) : (g -o f) : {5***REMOVED***

Derivation:

    >>> g14 = g4.applyto(g1)
    >>> print(g14.simplify())
    man(v1) : gr : {1, 4***REMOVED***
    >>> g134 = g3.applyto(g14)
    >>> print(g134.simplify())
    \Q.exists x.(man(x) & Q(x)) : (G[2] -o G) : {1, 3, 4***REMOVED***
    >>> g25 = g5.applyto(g2)
    >>> print(g25.simplify())
    walks(v2) : f : {2, 5***REMOVED***
    >>> g12345 = g134.applyto(g25)
    >>> print(g12345.simplify())
    exists x.(man(x) & walks(x)) : f : {1, 2, 3, 4, 5***REMOVED***
