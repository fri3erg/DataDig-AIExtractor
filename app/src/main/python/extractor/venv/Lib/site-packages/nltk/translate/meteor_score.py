...
def _generate_enums(
    ...
    if isinstance(hypothesis, str):
        raise TypeError(
            f'"hypothesis" expects pre-tokenized hypothesis (Iterable[str]): {hypothesis***REMOVED***'
        )

    if isinstance(reference, str):
        raise TypeError(
            f'"reference" expects pre-tokenized reference (Iterable[str]): {reference***REMOVED***'
        )
...
***REMOVED***
        ).union({enum_hypothesis_list[i][1]***REMOVED***)
        for j in range(len(enum_reference_list))[::-1]:
