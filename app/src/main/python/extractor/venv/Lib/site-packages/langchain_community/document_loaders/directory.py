...
class DirectoryLoader(BaseLoader):
    ...
    def load_file(
        ...
        if item.is_file():
            if _is_visible(item.relative_to(path)) or self.load_hidden:
                try:
                    logger.debug(f"Processing file: {str(item)***REMOVED***")
                    sub_docs = self.loader_cls(str(item), **self.loader_kwargs).load()
                    docs.extend(sub_docs)
                except Exception as e:
                    if self.silent_errors:
                        logger.warning(f"Error loading file {str(item)***REMOVED***: {e***REMOVED***")
                    else:
                        raise e
                finally:
                    if pbar:
    ...
    def load(self) -> List[Document]:
        ...
        p = Path(self.path)
        if not p.exists():
            raise FileNotFoundError(f"Directory not found: '{self.path***REMOVED***'")
        if not p.is_dir():
            raise ValueError(f"Expected directory, got file: '{self.path***REMOVED***'")
