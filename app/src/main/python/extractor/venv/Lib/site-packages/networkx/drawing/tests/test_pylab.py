...
def test_draw():
    try:
        ...
        options = [{"node_color": "black", "node_size": 100, "width": 3***REMOVED***]
...
def test_edge_color_tuple_interpretation():
    ...
    pos = {n: (n, n) for n in range(len(G))***REMOVED***
...
def test_labels_and_colors():
    ...
    nx.draw_networkx_edge_labels(G, pos, edge_labels={(4, 5): "4-5"***REMOVED***)
    # plt.show()
...
def test_house_with_colors():
    ...
    fig, ax = plt.subplots()
    pos = {0: (0, 0), 1: (1, 0), 2: (0, 1), 3: (1, 1), 4: (0.5, 2.0)***REMOVED***
...
def test_draw_nodes_missing_node_from_position():
    G = nx.path_graph(3)
    pos = {0: (0, 0), 1: (1, 1)***REMOVED***  # No position for node 2
...
def test_draw_edges_min_source_target_margins(node_shape):
    ...
    This test checks that the use of min_{source/target***REMOVED***_margin kwargs result
    ...
    fig, ax = plt.subplots()
    G = nx.DiGraph([(0, 1)])
    pos = {0: (0, 0), 1: (1, 0)***REMOVED***  # horizontal layout
    ...
    # min_{source/target***REMOVED***_margin kwargs
...
def test_nonzero_selfloop_with_single_node():
    ...
    G.add_edge(0, 0)
    # Draw
    patch = nx.draw_networkx_edges(G, {0: (0, 0)***REMOVED***)[0]
...
def test_nonzero_selfloop_with_single_edge_in_edgelist():
    ...
    G.add_edge(1, 1)
    pos = {n: (n, n) for n in G.nodes***REMOVED***
...
def test_draw_networkx_edge_label_empty_dict():
    ...
    G = nx.path_graph(3)
    pos = {n: (n, n) for n in G.nodes***REMOVED***
    assert nx.draw_networkx_edge_labels(G, pos, edge_labels={***REMOVED***) == {***REMOVED***
...
@pytest.mark.parametrize(
    ...
    (
        {"arrowstyle": "-"***REMOVED***,
        {"arrowsize": 20***REMOVED***,
        {"connectionstyle": "arc3,rad=0.2"***REMOVED***,
        {"min_source_margin": 10***REMOVED***,
        {"min_target_margin": 10***REMOVED***,
    ),
)
def test_user_warnings_for_unused_edge_drawing_kwargs(fap_only_kwarg):
    ...
    with pytest.warns(
        UserWarning, match=f"\n\nThe {kwarg_name***REMOVED*** keyword argument is not applicable"
    ):
