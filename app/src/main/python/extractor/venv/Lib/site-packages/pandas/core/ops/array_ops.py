...
def _masked_arith_op(x: np.ndarray, y, op):
    ...
    if isinstance(y, np.ndarray):
        ...
        if not is_scalar(y):
            raise TypeError(
                f"Cannot broadcast np.ndarray with operand of type { type(y) ***REMOVED***"
***REMOVED***
...
def comparison_op(left: ArrayLike, right: Any, op) -> ArrayLike:
    ...
    op : {operator.eq, operator.ne, operator.gt, operator.ge, operator.lt, operator.le***REMOVED***
...
***REMOVED*** as err:
                typ = type(y).__name__
                raise TypeError(
                    f"Cannot perform '{op.__name__***REMOVED***' with a dtyped [{x.dtype***REMOVED***] array "
                    f"and scalar of type [{typ***REMOVED***]"
    ***REMOVED*** from err
...
def logical_op(left: ArrayLike, right: Any, op) -> ArrayLike:
    ...
    right : object
        Cannot be a DataFrame, Series, or Index.
    op : {operator.and_, operator.or_, operator.xor***REMOVED***
        Or one of the reversed variants from roperator.
...
def get_array_op(op):
    ...
    if op_name in {"eq", "ne", "lt", "le", "gt", "ge"***REMOVED***:
        return partial(comparison_op, op=op)
    elif op_name in {"and", "or", "xor", "rand", "ror", "rxor"***REMOVED***:
