...
(?P<date>(?P<year>\d{4***REMOVED***)-(?P<month>\d{2***REMOVED***)-(?P<day>\d{2***REMOVED***))?T?
(?P<time>(?P<hour>\d{2***REMOVED***):(?P<minute>\d{2***REMOVED***)(:(?P<second>\d{2***REMOVED***)(?P<microsecond>\.\d{1,3***REMOVED***)?)?)?Z?''',
                                       re.VERBOSE)
ISO_DURATION = re.compile(r'PT((?P<hours>\d+)H)?((?P<minutes>\d+)M)?((?P<seconds>\d+(\.\d{1,3***REMOVED***)?)S)?')
...
def from_ISO8601(formatted_string):
    ...
    raise ValueError("Invalid datetime value {***REMOVED***".format(formatted_string))
