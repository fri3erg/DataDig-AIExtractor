...
class SparseAccessor(BaseAccessor, PandasDelegate):
    ...
    def to_coo(self, row_levels=(0,), column_levels=(1,), sort_labels: bool = False):
        ...
        (labels) or numbers of the levels. {row_levels, column_levels***REMOVED*** must be
...
class SparseFrameAccessor(BaseAccessor, PandasDelegate):
    ...
    ...                   "b": [3, 0, 0, 4]***REMOVED***, dtype="Sparse[int]")
    >>> df.sparse.density
    0.5
    """
    ...
    def to_dense(self) -> DataFrame:
        ...
        Examples
        --------
        >>> df = pd.DataFrame({"A": pd.arrays.SparseArray([0, 1, 0])***REMOVED***)
        >>> df.sparse.to_dense()
           A
        0  0
        1  1
        ...
        data = {k: v.array.to_dense() for k, v in self._parent.items()***REMOVED***
    ...
    def to_coo(self):
        ...
        Examples
        --------
        >>> df = pd.DataFrame({"A": pd.arrays.SparseArray([0, 1, 0, 1])***REMOVED***)
        >>> df.sparse.to_coo()
    ...
    def _prep_index(data, index, columns):
        ...
        if len(columns) != K:
            raise ValueError(f"Column length mismatch: {len(columns)***REMOVED*** vs. {K***REMOVED***")
        if len(index) != N:
            raise ValueError(f"Index length mismatch: {len(index)***REMOVED*** vs. {N***REMOVED***")
