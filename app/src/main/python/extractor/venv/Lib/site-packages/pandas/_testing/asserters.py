...
def assert_almost_equal(
    ...
    **kwargs,
    ...
    left : object
    right : object
    check_dtype : bool or {'equiv'***REMOVED***, default 'equiv'
...
def _check_isinstance(left, right, cls) -> None:
    ...
    if not isinstance(left, cls):
        raise AssertionError(
            f"{cls_name***REMOVED*** Expected type {cls***REMOVED***, found {type(left)***REMOVED*** instead"
        )
    if not isinstance(right, cls):
        raise AssertionError(
            f"{cls_name***REMOVED*** Expected type {cls***REMOVED***, found {type(right)***REMOVED*** instead"
        )
...
def assert_index_equal(
    ...
    left : Index
    right : Index
    exact : bool or {'equiv'***REMOVED***, default 'equiv'
    ...
    if left.nlevels != right.nlevels:
        msg1 = f"{obj***REMOVED*** levels are different"
        msg2 = f"{left.nlevels***REMOVED***, {left***REMOVED***"
        msg3 = f"{right.nlevels***REMOVED***, {right***REMOVED***"
        raise_assert_detail(obj, msg1, msg2, msg3)

    # length comparison
    if len(left) != len(right):
        msg1 = f"{obj***REMOVED*** length are different"
        msg2 = f"{len(left)***REMOVED***, {left***REMOVED***"
        msg3 = f"{len(right)***REMOVED***, {right***REMOVED***"
        raise_assert_detail(obj, msg1, msg2, msg3)
    ...
    if isinstance(left, MultiIndex):
        ...
        for level in range(left.nlevels):
            lobj = f"MultiIndex level [{level***REMOVED***]"
            try:
    ...
    ***REMOVED***
    ...
    # skip exact index checking when `check_categorical` is False
        ...
        if not left.equals(right):
            ...
            msg = f"{obj***REMOVED*** values are different ({np.round(diff, 5)***REMOVED*** %)"
    ...
    if check_categorical:
        if isinstance(left.dtype, CategoricalDtype) or isinstance(
            ...
            assert_categorical_equal(left._values, right._values, obj=f"{obj***REMOVED*** category")
...
def assert_class_equal(
    ...
    msg = f"{obj***REMOVED*** classes are different"
...
def assert_attr_equal(attr: str, left, right, obj: str = "Attributes") -> None:
    ...
    if not result:
        msg = f'Attribute "{attr***REMOVED***" are different'
    ...
    if isinstance(objs, (Series, np.ndarray)):
        ...
        for el in objs.ravel():
            msg = (
                ...
                f"type encountered {repr(type(el).__name__)***REMOVED***"
***REMOVED***
        ...
        msg = (
            ...
            f"{repr(type(objs).__name__)***REMOVED***"
        )
...
def assert_categorical_equal(
    ...
    if check_category_order:
        assert_index_equal(
            left.categories, right.categories, obj=f"{obj***REMOVED***.categories", exact=exact
        )
        assert_numpy_array_equal(
            left.codes, right.codes, check_dtype=check_dtype, obj=f"{obj***REMOVED***.codes"
        )
    else:
        try:
        ...
        assert_index_equal(lc, rc, obj=f"{obj***REMOVED***.categories", exact=exact)
        assert_index_equal(
            left.categories.take(left.codes),
            right.categories.take(right.codes),
            obj=f"{obj***REMOVED***.values",
            exact=exact,
        )
...
def assert_interval_array_equal(
    ...
    assert_equal(left._left, right._left, obj=f"{obj***REMOVED***.left", **kwargs)
    assert_equal(left._right, right._right, obj=f"{obj***REMOVED***.left", **kwargs)
...
def assert_period_array_equal(left, right, obj: str = "PeriodArray") -> None:
    ...
    assert_numpy_array_equal(left._ndarray, right._ndarray, obj=f"{obj***REMOVED***._ndarray")
    assert_attr_equal("dtype", left, right, obj=obj)
...
def raise_assert_detail(
    ...
    msg = f"""{obj***REMOVED*** are different
...
{message***REMOVED***"""
    ...
    if isinstance(index_values, np.ndarray):
        msg += f"\n[index]: {pprint_thing(index_values)***REMOVED***"
...
def assert_numpy_array_equal(
    ...
    if check_same == "same":
        if left_base is not right_base:
            raise AssertionError(f"{repr(left_base)***REMOVED*** is not {repr(right_base)***REMOVED***")
    elif check_same == "copy":
        if left_base is right_base:
            raise AssertionError(f"{repr(left_base)***REMOVED*** is {repr(right_base)***REMOVED***")

    def _raise(left, right, err_msg) -> NoReturn:
        if err_msg is None:
            if left.shape != right.shape:
                raise_assert_detail(
                    obj, f"{obj***REMOVED*** shapes are different", left.shape, right.shape
    ***REMOVED***
...
def assert_extension_array_equal(
    ...
    if check_dtype:
        assert_attr_equal("dtype", left, right, obj=f"Attributes of {obj***REMOVED***")
...
***REMOVED***:
    ...
    assert_numpy_array_equal(
        left_na, right_na, obj=f"{obj***REMOVED*** NA mask", index_values=index_values
    )
...
def assert_series_equal(
    ...
    *,
    ...
    check_index_type : bool or {'equiv'***REMOVED***, default 'equiv'
        Whether to check the Index class, dtype and inferred_type
        are identical.
    ...
    # length comparison
    if len(left) != len(right):
        msg1 = f"{len(left)***REMOVED***, {left.index***REMOVED***"
        msg2 = f"{len(right)***REMOVED***, {right.index***REMOVED***"
        raise_assert_detail(obj, "Series length are different", msg1, msg2)

    if check_flags:
        assert left.flags == right.flags, f"{repr(left.flags)***REMOVED*** != {repr(right.flags)***REMOVED***"
...
***REMOVED***
        ...
        if not Index(left._values).equals(Index(right._values)):
            msg = (
                f"[datetimelike_compat=True] {left._values***REMOVED*** "
                f"is not equal to {right._values***REMOVED***."
***REMOVED***
            raise AssertionError(msg)
...
def assert_frame_equal(
    ...
    check_index_type : bool or {'equiv'***REMOVED***, default 'equiv'
        Whether to check the Index class, dtype and inferred_type
        are identical.
    check_column_type : bool or {'equiv'***REMOVED***, default 'equiv'
    ...
    >>> df1 = pd.DataFrame({'a': [1, 2], 'b': [3, 4]***REMOVED***)
    >>> df2 = pd.DataFrame({'a': [1, 2], 'b': [3.0, 4.0]***REMOVED***)
    ...
    if left.shape != right.shape:
        raise_assert_detail(
            obj, f"{obj***REMOVED*** shape mismatch", f"{repr(left.shape)***REMOVED***", f"{repr(right.shape)***REMOVED***"
        )
    ...
    # compare by columns
        ...
        for i, col in enumerate(left.columns):
            ...
            assert_series_equal(
                ...
                obj=f'{obj***REMOVED***.iloc[:, {i***REMOVED***] (column name="{col***REMOVED***")',
                rtol=rtol,
                atol=atol,
                check_index=False,
                check_flags=False,
...
def assert_contains_all(iterable, dic) -> None:
    for k in iterable:
        assert k in dic, f"Did not contain item: {repr(k)***REMOVED***"
...
def assert_copy(iter1, iter2, **eql_kwargs) -> None:
    ...
    for elem1, elem2 in zip(iter1, iter2):
        ...
        msg = (
            f"Expected object {repr(type(elem1))***REMOVED*** and object {repr(type(elem2))***REMOVED*** to be "
