    ...
    def download_documents(self, query: str) -> List[Document]:
        ...
        return [
            Document(
                ...
                metadata={"title": item.get("title"), "link": item.get("url")***REMOVED***,
***REMOVED***
            for item in results
        ]
    ...
    def _search_request(self, query: str) -> List[dict]:
        ...
        ***REMOVED***
        ...
        params = {**self.search_kwargs, **{"q": query***REMOVED******REMOVED***
        ...
        if not response.ok:
            raise Exception(f"HTTP error {response.status_code***REMOVED***")

        return response.json().get("web", {***REMOVED***).get("results", [])
