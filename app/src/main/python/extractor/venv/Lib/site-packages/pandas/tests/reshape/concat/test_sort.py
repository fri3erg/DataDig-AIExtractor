...
class TestConcatSort:
    def test_concat_sorts_columns(self, sort):
        ...
        df2 = DataFrame({"a": [3, 4], "c": [5, 6]***REMOVED***)

        # for sort=True/None
        expected = DataFrame(
            {"a": [1, 2, 3, 4], "b": [1, 2, None, None], "c": [None, None, 5, 6]***REMOVED***,
            columns=["a", "b", "c"],
        )
    ...
    def test_concat_sorts_index(self, sort):
        df1 = DataFrame({"a": [1, 2, 3]***REMOVED***, index=["c", "a", "b"])
        df2 = DataFrame({"b": [1, 2]***REMOVED***, index=["a", "b"])

        # For True/None
        expected = DataFrame(
            {"a": [2, 3, 1], "b": [1, 2, None]***REMOVED***,
            index=["a", "b", "c"],
            columns=["a", "b"],
        )
        if sort is False:
    ...
    def test_concat_inner_sort(self, sort):
        ...
        df1 = DataFrame(
            {"a": [1, 2], "b": [1, 2], "c": [1, 2]***REMOVED***, columns=["b", "a", "c"]
        )
        df2 = DataFrame({"a": [1, 2], "b": [3, 4]***REMOVED***, index=[3, 4])
        ...
        expected = DataFrame({"b": [1, 2, 3, 4], "a": [1, 2, 1, 2]***REMOVED***, columns=["b", "a"])
        if sort is True:
    ...
    def test_concat_aligned_sort(self):
        # GH-4588
        df = DataFrame({"c": [1, 2], "b": [3, 4], "a": [5, 6]***REMOVED***, columns=["c", "b", "a"])
        result = pd.concat([df, df], sort=True, ignore_index=True)
        expected = DataFrame(
            {"a": [5, 6, 5, 6], "b": [3, 4, 3, 4], "c": [1, 2, 1, 2]***REMOVED***,
            columns=["a", "b", "c"],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_concat_aligned_sort_does_not_raise(self):
        ...
        df = DataFrame({1: [1, 2], "a": [3, 4]***REMOVED***, columns=[1, "a"])
        expected = DataFrame({1: [1, 2, 1, 2], "a": [3, 4, 3, 4]***REMOVED***, columns=[1, "a"])
    ...
    def test_concat_frame_with_sort_false(self):
        # GH 43375
        result = pd.concat(
            [DataFrame({i: i***REMOVED***, index=[i]) for i in range(2, 0, -1)], sort=False
        )
        ...
        # GH 37937
        df1 = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]***REMOVED***, index=[1, 2, 3])
        df2 = DataFrame({"c": [7, 8, 9], "d": [10, 11, 12]***REMOVED***, index=[3, 1, 6])
    ...
    def test_concat_sort_none_raises(self):
        # GH#41518
        df = DataFrame({1: [1, 2], "a": [3, 4]***REMOVED***)
