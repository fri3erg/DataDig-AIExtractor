...
- Tgrep compatibility: Using "@" for "!", "{" for "<", "***REMOVED***" for ">" are
  not implemented.
...
def _tgrep_macro_use_action(_s, _l, tokens):
    ...
    def macro_use(n, m=None, l=None):
        if m is None or macro_name not in m:
            raise TgrepException(f"macro {macro_name***REMOVED*** not defined")
...
***REMOVED***
            ...
            raise TgrepException(f'cannot interpret tgrep operator "{operator***REMOVED***"')
...
def _tgrep_segmented_pattern_action(_s, _l, tokens):
    ...
    def pattern_segment_pred(n, m=None, l=None):
        ...
        if l is None or node_label not in l:
            raise TgrepException(f"node_label ={node_label***REMOVED*** not bound in pattern")
        node = l[node_label]
            ...
            if node_pred(n, m, l):
                ...
                if l is None:
                    raise TgrepException(
                        "cannot bind node_label {***REMOVED***: label_dict is None".format(
                            node_label
            ***REMOVED***
        ***REMOVED***
                l[node_label] = n
...
def _macro_defn_action(_s, _l, tokens):
    ...
    assert len(tokens) == 3
    assert tokens[0] == "@"
    return {tokens[1]: tokens[2]***REMOVED***
...
def _tgrep_exprs_action(_s, _l, tokens):
    ...
    """
    if len(tokens) == 1:
        return lambda n, m=None, l=None: tokens[0](n, None, {***REMOVED***)
