...
class TestAstypeAPI:
    ...
    def test_astype_dict_like(self, dtype_class):
        ...
        dt1 = dtype_class({"abc": str***REMOVED***)
        result = ser.astype(dt1)
        expected = Series(["0", "2", "4", "6", "8"], name="abc", dtype=object)
        tm.assert_series_equal(result, expected)

        dt2 = dtype_class({"abc": "float64"***REMOVED***)
        result = ser.astype(dt2)
        expected = Series([0.0, 2.0, 4.0, 6.0, 8.0], dtype="float64", name="abc")
        tm.assert_series_equal(result, expected)

        dt3 = dtype_class({"abc": str, "def": str***REMOVED***)
        msg = (
        ...
        if dtype_class is Series:
            dt5 = dtype_class({***REMOVED***, dtype=object)
        else:
            dt5 = dtype_class({***REMOVED***)
...
class TestAstype:
    ...
    def test_astype_generic_timestamp_no_frequency(self, dtype, request):
        ...
        msg = (
            rf"The '{dtype.__name__***REMOVED***' dtype has no unit\. "
            rf"Please pass in '{dtype.__name__***REMOVED***\[ns\]' instead."
        )
        with pytest.raises(ValueError, match=msg):
            ser.astype(dtype)
    ...
    def test_astype_datetime(self, unit):
        ser = Series(iNaT, dtype=f"M8[{unit***REMOVED***]", index=range(5))
        ...
        ser = Series(
            [datetime(2001, 1, 2, 0, 0) for i in range(3)], dtype=f"M8[{unit***REMOVED***]"
        )

        ser[1] = np.nan
        assert ser.dtype == f"M8[{unit***REMOVED***]"
...
class TestAstypeCategorical:
    def test_astype_categorical_to_other(self):
        cat = Categorical([f"{i***REMOVED*** - {i + 499***REMOVED***" for i in range(0, 10000, 500)])
