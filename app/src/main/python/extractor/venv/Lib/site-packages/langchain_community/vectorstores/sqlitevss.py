...
***REMOVED***
    ...
    def create_table_if_not_exists(self) -> None:
        self._connection.execute(
            f"""
            CREATE TABLE IF NOT EXISTS {self._table***REMOVED***
            (
...
***REMOVED***
        ...
        self._connection.execute(
            f"""
                CREATE VIRTUAL TABLE IF NOT EXISTS vss_{self._table***REMOVED*** USING vss0(
                  text_embedding({self.get_dimensionality()***REMOVED***)
    ***REMOVED***;
            """
        )
        self._connection.execute(
            f"""
                ...
                AFTER INSERT ON {self._table***REMOVED***
                BEGIN
                    INSERT INTO vss_{self._table***REMOVED***(rowid, text_embedding)
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        max_id = self._connection.execute(
            f"SELECT max(rowid) as rowid FROM {self._table***REMOVED***"
        ).fetchone()["rowid"]
        ...
        self._connection.executemany(
            f"INSERT INTO {self._table***REMOVED***(text, metadata, text_embedding) "
            f"VALUES (?,?,?)",
            data_input,
        )
        self._connection.commit()
        ...
        results = self._connection.execute(
            f"SELECT rowid FROM {self._table***REMOVED*** WHERE rowid > {max_id***REMOVED***"
        )
    ...
    def similarity_search_with_score_by_vector(
        ...
        sql_query = f"""
            SELECT 
                ...
                metadata,
                distance
            FROM {self._table***REMOVED*** e
            INNER JOIN vss_{self._table***REMOVED*** v on v.rowid = e.rowid  
            WHERE vss_search(
              v.text_embedding,
              vss_search_params('{json.dumps(embedding)***REMOVED***', {k***REMOVED***)
***REMOVED***
        """
        ...
        documents = []
        for row in results:
            metadata = json.loads(row["metadata"]) or {***REMOVED***
