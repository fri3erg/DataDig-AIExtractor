...
def get_io_paths(fname_inp, mname="untitled"):
    ...
    return PPaths(
        ...
        wrap77=bpath.with_name(f"{mname***REMOVED***-f2pywrappers.f"),
        wrap90=bpath.with_name(f"{mname***REMOVED***-f2pywrappers2.f90"),
        cmodf=bpath.with_name(f"{mname***REMOVED***module.c"),
    )
...
def test_gh22819_cli(capfd, gh22819_cli, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", f"f2py -m blah {ipath***REMOVED***".split())
...
def test_gh22819_many_pyf(capfd, gh22819_cli, monkeypatch):
    ...
    """
    ipath = Path(gh22819_cli)
    monkeypatch.setattr(sys, "argv", f"f2py -m blah {ipath***REMOVED*** hello.pyf".split())
    with util.switchdir(ipath.parent):
...
def test_gh23598_warn(capfd, gh23598_warn, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        f'f2py {ipath***REMOVED*** -m test'.split())
...
def test_gen_pyf(capfd, hello_world_f90, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", f'f2py -h {opath***REMOVED*** {ipath***REMOVED***'.split())

    with util.switchdir(ipath.parent):
        f2pycli()  # Generate wrappers
        out, _ = capfd.readouterr()
        assert "Saving signatures to file" in out
        assert Path(f'{opath***REMOVED***').exists()
...
def test_gen_pyf_stdout(capfd, hello_world_f90, monkeypatch):
    ...
    """
    ipath = Path(hello_world_f90)
    monkeypatch.setattr(sys, "argv", f'f2py -h stdout {ipath***REMOVED***'.split())
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
        assert "Saving signatures to file" in out
...
def test_gen_pyf_no_overwrite(capfd, hello_world_f90, monkeypatch):
    ...
    """
    ipath = Path(hello_world_f90)
    monkeypatch.setattr(sys, "argv", f'f2py -h faker.pyf {ipath***REMOVED***'.split())
...
def test_untitled_cli(capfd, hello_world_f90, monkeypatch):
    ...
    """
    ipath = Path(hello_world_f90)
    monkeypatch.setattr(sys, "argv", f"f2py --backend meson -c {ipath***REMOVED***".split())
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
...
def test_no_py312_distutils_fcompiler(capfd, hello_world_f90, monkeypatch):
    ...
    ipath = foutl.f90inp
    monkeypatch.setattr(
        sys, "argv", f"f2py {ipath***REMOVED*** -c --fcompiler=gfortran -m {MNAME***REMOVED***".split()
    )
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
    ...
    monkeypatch.setattr(
        sys, "argv", f"f2py {ipath***REMOVED*** -c -m {MNAME***REMOVED*** --backend distutils".split()
    )
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
...
def test_f2py_skip(capfd, retreal_f77, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        f'f2py {ipath***REMOVED*** -m test skip: {toskip***REMOVED***'.split())
    ...
    with util.switchdir(ipath.parent):
        ...
        for skey in toskip.split():
            assert (
                f'buildmodule: Could not found the body of interfaced routine "{skey***REMOVED***". Skipping.'
                in err)
        for rkey in remaining.split():
            assert f'Constructing wrapper function "{rkey***REMOVED***"' in out
...
def test_f2py_only(capfd, retreal_f77, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        f'f2py {ipath***REMOVED*** -m test only: {tokeep***REMOVED***'.split())
    ...
    with util.switchdir(ipath.parent):
        ...
        for skey in toskip.split():
            assert (
                f'buildmodule: Could not find the body of interfaced routine "{skey***REMOVED***". Skipping.'
                in err)
...
def test_file_processing_switch(capfd, hello_world_f90, retreal_f77,
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py {ipath***REMOVED*** -m {mname***REMOVED*** only: {tokeep***REMOVED*** : {ipath2***REMOVED***'.split(
        ),
    )
...
def test_mod_gen_f77(capfd, hello_world_f90, monkeypatch):
    ...
    foutl = get_io_paths(hello_world_f90, mname=MNAME)
    ipath = foutl.f90inp
    monkeypatch.setattr(sys, "argv", f'f2py {ipath***REMOVED*** -m {MNAME***REMOVED***'.split())
    with util.switchdir(ipath.parent):
        f2pycli()
...
def test_mod_gen_gh25263(capfd, hello_world_f77, monkeypatch):
    ...
    ipath = foutl.finp
    monkeypatch.setattr(sys, "argv", f'f2py {ipath***REMOVED*** -m {MNAME***REMOVED*** -h hi.pyf'.split())
    with util.switchdir(ipath.parent):
        f2pycli()
...
def test_lower_cmod(capfd, hello_world_f77, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", f'f2py {ipath***REMOVED*** -m test --lower'.split())
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
    ...
    monkeypatch.setattr(sys, "argv",
                        f'f2py {ipath***REMOVED*** -m test --no-lower'.split())
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
...
def test_lower_sig(capfd, hello_world_f77, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py {ipath***REMOVED*** -h {foutl.pyf***REMOVED*** -m test --overwrite-signature'.split(),
    )
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py {ipath***REMOVED*** -h {foutl.pyf***REMOVED*** -m test --overwrite-signature --no-lower'
        .split(),
    )
...
def test_build_dir(capfd, hello_world_f90, monkeypatch):
    ...
    odir = "tttmp"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --build-dir {odir***REMOVED***'.split())

    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
        assert f"Wrote C/API module \"{mname***REMOVED***\"" in out
...
def test_overwrite(capfd, hello_world_f90, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        f'f2py -h faker.pyf {ipath***REMOVED*** --overwrite-signature'.split())
...
def test_latexdoc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = "blah"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --latex-doc'.split())

    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
        assert "Documentation is saved to file" in out
        with Path(f"{mname***REMOVED***module.tex").open() as otex:
...
def test_nolatexdoc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = "blah"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --no-latex-doc'.split())
...
def test_shortlatex(capfd, hello_world_f90, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --latex-doc --short-latex'.split(),
    )
...
def test_restdoc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = "blah"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --rest-doc'.split())

    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
        assert "ReST Documentation is saved to file" in out
        with Path(f"./{mname***REMOVED***module.rest").open() as orst:
...
def test_norestexdoc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = "blah"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --no-rest-doc'.split())
...
def test_debugcapi(capfd, hello_world_f90, monkeypatch):
    ...
    mname = "blah"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --debug-capi'.split())

    with util.switchdir(ipath.parent):
        f2pycli()
        with Path(f"./{mname***REMOVED***module.c").open() as ocmod:
...
def test_debugcapi_bld(hello_world_f90, monkeypatch):
    ...
    mname = "blah"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** -c --debug-capi'.split())
...
def test_wrapfunc_def(capfd, hello_world_f90, monkeypatch):
    ...
    ipath = Path(hello_world_f90)
    mname = "blah"
    monkeypatch.setattr(sys, "argv", f'f2py -m {mname***REMOVED*** {ipath***REMOVED***'.split())
    ...
    # Explicit
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --wrap-functions'.split())
...
def test_nowrapfunc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = "blah"
    monkeypatch.setattr(sys, "argv",
                        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** --no-wrap-functions'.split())
...
def test_inclheader(capfd, hello_world_f90, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py -m {mname***REMOVED*** {ipath***REMOVED*** -include<stdbool.h> -include<stdio.h> '.
        split(),
    )
...
def test_f2cmap(capfd, f2cmap_f90, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", f'f2py -m blah {ipath***REMOVED*** --f2cmap mapfile'.split())
...
def test_quiet(capfd, hello_world_f90, monkeypatch):
    ...
    """
    ipath = Path(hello_world_f90)
    monkeypatch.setattr(sys, "argv", f'f2py -m blah {ipath***REMOVED*** --quiet'.split())
...
def test_verbose(capfd, hello_world_f90, monkeypatch):
    ...
    """
    ipath = Path(hello_world_f90)
    monkeypatch.setattr(sys, "argv", f'f2py -m blah {ipath***REMOVED*** --verbose'.split())
...
def test_npdistop(hello_world_f90, monkeypatch):
    ...
    """
    ipath = Path(hello_world_f90)
    monkeypatch.setattr(sys, "argv", f'f2py -m blah {ipath***REMOVED*** -c'.split())
