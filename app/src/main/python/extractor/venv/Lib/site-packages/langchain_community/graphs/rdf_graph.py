...
cls_query_rdf = prefixes["rdfs"] + (
    ...
    """WHERE { \n"""
    """    ?instance a ?cls . \n"""
    """    OPTIONAL { ?cls rdfs:comment ?com ***REMOVED*** \n"""
    """***REMOVED***"""
)
...
rel_query_rdf = prefixes["rdfs"] + (
    ...
    """WHERE { \n"""
    """    ?subj ?rel ?obj . \n"""
    """    OPTIONAL { ?rel rdfs:comment ?com ***REMOVED*** \n"""
    """***REMOVED***"""
)
...
op_query_owl = (
    ...
    + (
        ...
        """    OPTIONAL { ?op rdfs:comment ?com ***REMOVED*** \n"""
        """***REMOVED***"""
    )
)
...
dp_query_owl = (
    ...
    + (
        ...
        """    OPTIONAL { ?dp rdfs:comment ?com ***REMOVED*** \n"""
        """***REMOVED***"""
    )
)
...
***REMOVED***
        if self.standard not in (supported_standards := ("rdf", "rdfs", "owl")):
            raise ValueError(
                f"Invalid standard. Supported standards are: {supported_standards***REMOVED***."
***REMOVED***
...
***REMOVED***
    ...
    def _get_local_name(iri: str) -> str:
        if "#" in iri:
            ...
            raise ValueError(f"Unexpected IRI '{iri***REMOVED***', contains neither '#' nor '/'.")
        return local_name
    ...
    def load_schema(self) -> None:
        ...
        def _rdf_s_schema(
            ...
            return (
                ...
                f'{", ".join([self._res_to_str(r, "cls") for r in classes])***REMOVED***\n'
                f"The RDF graph supports the following relationships:\n"
                f'{", ".join([self._res_to_str(r, "rel") for r in relationships])***REMOVED***\n'
***REMOVED***
        ...
        if self.standard == "rdf":
            ...
            self.schema = (
                ...
                f'{", ".join([self._res_to_str(r, "cls") for r in clss])***REMOVED***\n'
                f"The OWL graph supports the following object properties, "
                f"i.e., relationships between objects:\n"
                f'{", ".join([self._res_to_str(r, "op") for r in ops])***REMOVED***\n'
                f"The OWL graph supports the following data properties, "
                f"i.e., relationships between objects and literals:\n"
                f'{", ".join([self._res_to_str(r, "dp") for r in dps])***REMOVED***\n'
***REMOVED***
        else:
            raise ValueError(f"Mode '{self.standard***REMOVED***' is currently not supported.")
