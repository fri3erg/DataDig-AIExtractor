...
typedef struct {
     ...
     * this line
     */
***REMOVED*** PyDataMemAllocator;

typedef struct {
    char name[127];  /* multiple of 64 to keep the struct aligned */
    uint8_t version; /* currently 1 */
    PyDataMemAllocator allocator;
***REMOVED*** PyDataMem_Handler;
...
#define NPY_BEGIN_THREADS do {_save = PyEval_SaveThread();***REMOVED*** while (0);
#define NPY_END_THREADS   do { if (_save) \
                { PyEval_RestoreThread(_save); _save = NULL;***REMOVED*** ***REMOVED*** while (0);
#define NPY_BEGIN_THREADS_THRESHOLDED(loop_size) do { if ((loop_size) > 500) \
                { _save = PyEval_SaveThread();***REMOVED*** ***REMOVED*** while (0);

#define NPY_BEGIN_THREADS_DESCR(dtype) \
        do {if (!(PyDataType_FLAGCHK((dtype), NPY_NEEDS_PYAPI))) \
                NPY_BEGIN_THREADS;***REMOVED*** while (0);

#define NPY_END_THREADS_DESCR(dtype) \
        do {if (!(PyDataType_FLAGCHK((dtype), NPY_NEEDS_PYAPI))) \
                NPY_END_THREADS; ***REMOVED*** while (0);

#define NPY_ALLOW_C_API_DEF  PyGILState_STATE __save__;
#define NPY_ALLOW_C_API      do {__save__ = PyGILState_Ensure();***REMOVED*** while (0);
#define NPY_DISABLE_C_API    do {PyGILState_Release(__save__);***REMOVED*** while (0);
#else
