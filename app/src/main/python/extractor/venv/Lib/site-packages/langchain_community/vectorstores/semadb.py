...
class SemaDB(VectorStore):
    ...
    def _get_internal_distance_strategy(self) -> str:
        ...
        if self.distance_strategy == DistanceStrategy.EUCLIDEAN_DISTANCE:
            ...
            raise ValueError(f"Unknown distance strategy {self.distance_strategy***REMOVED***")
    ...
    def delete_collection(self) -> bool:
        ...
        response = requests.delete(
            SemaDB.BASE_URL + f"/collections/{self.collection_name***REMOVED***",
            headers=self.headers,
        )
        return response.status_code == 200
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        if len(embeddings[0]) != self.vector_size:
            raise ValueError(
                f"Embedding size mismatch {len(embeddings[0])***REMOVED*** != {self.vector_size***REMOVED***"
***REMOVED***
        # Normalise if needed
...
***REMOVED***
                ...
                points.append(
                    {
                        "id": new_id,
                        "vector": embedding,
                        "metadata": {**metadata, **{"text": text***REMOVED******REMOVED***,
                    ***REMOVED***
    ***REMOVED***
        else:
    ...
    ***REMOVED***
        ...
        for i in range(0, len(points), batch_size):
            ...
            response = requests.post(
                SemaDB.BASE_URL + f"/collections/{self.collection_name***REMOVED***/points",
                json={"points": batch***REMOVED***,
                headers=self.headers,
***REMOVED***
            if response.status_code != 200:
                print("HERE--", batch)
                raise ValueError(f"Error adding points: {response.text***REMOVED***")
            failed_ranges = response.json()["failedRanges"]
            if len(failed_ranges) > 0:
                raise ValueError(f"Error adding points: {failed_ranges***REMOVED***")
        # Return ids
        return ids
    ...
    def _search_points(self, embedding: List[float], k: int = 4) -> List[dict]:
        ...
        ***REMOVED***
        response = requests.post(
            SemaDB.BASE_URL + f"/collections/{self.collection_name***REMOVED***/points/search",
            json=payload,
            headers=self.headers,
        )
        if response.status_code != 200:
            raise ValueError(f"Error searching: {response.text***REMOVED***")
