...
***REMOVED***,
    ...
    def test_to_datetime_format_microsecond(self, cache):
        ...
        val = f"01-{month_abbr***REMOVED***-2011 00:00:01.978"
    ...
    def test_to_datetime_parse_timezone_malformed(self, offset):
        ...
        msg = "|".join(
            [
                ...
                f"{PARSING_ERR_MSG***REMOVED***$",
                r'^unconverted data remains when parsing with format ".*": ".*", '
                f"at position 0. {PARSING_ERR_MSG***REMOVED***$",
            ]
        )
...
***REMOVED***
    ...
    ***REMOVED***,
    ...
    def test_datetime_bool_arrays_mixed(self, cache):
        msg = f"{type(cache)***REMOVED*** is not convertible to datetime"
        with pytest.raises(TypeError, match=msg):
        ...
        with pytest.raises(
            ...
            match=(
                ...
                f"at position 1. {PARSING_ERR_MSG***REMOVED***$"
***REMOVED***,
        ):
...
class TestToDatetimeUnit:
    ...
    def test_to_datetime_month_or_year_unit_non_round_float(self, cache, unit):
        ...
        msg = f"Conversion of non-round float with unit={unit***REMOVED*** is ambiguous"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_to_datetime_unit_invalid(self, bad_val):
        msg = f"{bad_val***REMOVED*** with the unit 'D'"
        with pytest.raises(ValueError, match=msg):
...
class TestToDatetimeDataFrame:
    ...
    def test_dataframe(self, df, cache):
        result = to_datetime(
            {"year": df["year"], "month": df["month"], "day": df["day"]***REMOVED***, cache=cache
        )
        expected = Series(
    ...
    def test_dataframe_coerce(self, cache):
        # passing coerce
        df2 = DataFrame({"year": [2015, 2016], "month": [2, 20], "day": [4, 5]***REMOVED***)
    ...
    def test_dataframe_duplicate_columns_raises(self, cache):
        ...
        df2 = DataFrame(
            {"year": [2015, 2016], "month": [2, 20], "day": [4, 5], "hour": [4, 5]***REMOVED***
        )
    ...
    def test_dataframe_int16(self, cache):
        # GH#13451
        df = DataFrame({"year": [2015, 2016], "month": [2, 3], "day": [4, 5]***REMOVED***)
    ...
    def test_dataframe_float(self, cache):
        # float
        df = DataFrame({"year": [2000, 2001], "month": [1.5, 1], "day": [1, 1]***REMOVED***)
        msg = (
...
class TestToDatetimeMisc:
    ...
    def test_to_datetime_iso8601_fails(self, input, format, exact):
        ...
        with pytest.raises(
            ValueError,
            match=(
                rf"time data \"{input***REMOVED***\" doesn't match format "
                rf"\"{format***REMOVED***\", at position 0"
***REMOVED***,
        ):
...
***REMOVED***,
    ...
    def test_to_datetime_with_space_in_series(self, cache):
        ...
        msg = (
            ...
            rf"at position 2. {PARSING_ERR_MSG***REMOVED***$"
        )
        with pytest.raises(ValueError, match=msg):
    ...
    def test_dti_constructor_numpy_timeunits(self, cache, unit):
        ...
        if unit in ["h", "m"]:
            ...
            unit = "s"
        exp_dtype = np.dtype(f"M8[{unit***REMOVED***]")
    ...
    def test_dayfirst_warnings_invalid_input(self):
        ...
        with pytest.raises(
            ...
            match=(
                ...
                rf'"%d/%m/%Y", at position 1. {PARSING_ERR_MSG***REMOVED***$'
***REMOVED***,
        ):
...
class TestToDatetimeInferFormat:
    ...
    def test_to_datetime_inconsistent_format(self, cache):
        ...
        msg = (
            ...
            rf'"%m/%d/%Y %H:%M:%S", at position 1. {PARSING_ERR_MSG***REMOVED***$'
        )
        with pytest.raises(ValueError, match=msg):
...
***REMOVED***
    ...
    def test_infer_datetime_format_tz_name(self, tz_name, offset):
        # GH 33133
        ser = Series([f"2019-02-02 08:07:13 {tz_name***REMOVED***"])
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "2015-02-29",
                "%Y-%m-%d",
                f"^day is out of range for month, at position 0. {PARSING_ERR_MSG***REMOVED***$",
***REMOVED***,
            (
                "2015-29-02",
                "%Y-%d-%m",
...
def epochs(epoch_1960, request):
    ...
    * str
    """
    assert request.param in {"timestamp", "pydatetime", "datetime64", "str_1960"***REMOVED***
...
class TestOrigin:
    ...
    def test_invalid_origins(self, origin, exc, units, units_from_epochs):
        msg = "|".join(
            [
                f"origin {origin***REMOVED*** is Out of Bounds",
                f"origin {origin***REMOVED*** cannot be converted to a Timestamp",
...
def test_from_numeric_arrow_dtype(any_numeric_ea_dtype):
    ...
    ser = Series([1, 2], dtype=f"{any_numeric_ea_dtype.lower()***REMOVED***[pyarrow]")
    result = to_datetime(ser)
