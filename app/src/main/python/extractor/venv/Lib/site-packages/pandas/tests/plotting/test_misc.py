...
def test_get_accessor_args():
    ...
    with pytest.raises(TypeError, match=msg):
        func(backend_name="", data=[], args=[], kwargs={***REMOVED***)

    msg = "should not be called with positional arguments"
    with pytest.raises(TypeError, match=msg):
        func(backend_name="", data=Series(dtype=object), args=["line", None], kwargs={***REMOVED***)

    x, y, kind, kwargs = func(
        backend_name="",
        data=DataFrame(),
        args=["x"],
        kwargs={"y": "y", "kind": "bar", "grid": False***REMOVED***,
    )
    assert x == "x"
    assert y == "y"
    assert kind == "bar"
    assert kwargs == {"grid": False***REMOVED***
...
def test_savefig(kind, data, index):
    ...
    if kind in ["hexbin", "scatter", "pie"]:
        if isinstance(data, Series):
            pytest.skip(f"{kind***REMOVED*** not supported with Series")
        kwargs = {"x": 0, "y": 1***REMOVED***
...
class TestSeriesPlots:
    ...
    @pytest.mark.parametrize("kwargs", [{***REMOVED***, {"lag": 5***REMOVED***])
...
***REMOVED***,
    ...
    def test_andrews_curves_handle(self):
        ...
        colors = ["b", "g", "r"]
        df = DataFrame({"A": [1, 2, 3], "B": [1, 2, 3], "C": [1, 2, 3], "Name": colors***REMOVED***)
    ...
    def test_radviz_colors_handles(self):
        ...
        df = DataFrame(
            {"A": [1, 2, 3], "B": [2, 1, 3], "C": [3, 2, 1], "Name": ["b", "g", "r"]***REMOVED***
        )
    ...
    def test_dictionary_color(self, kind):
        ...
        dic_color = {"b": (0.3, 0.7, 0.7), "a": (0.5, 0.24, 0.6)***REMOVED***
    ...
    def test_barh_plot_labels_mixed_integer_string(self):
        ...
        df = DataFrame([{"word": 1, "value": 0***REMOVED***, {"word": "knowledge", "value": 2***REMOVED***])
