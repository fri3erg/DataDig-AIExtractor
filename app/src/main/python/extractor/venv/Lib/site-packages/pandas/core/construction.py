...
def array(
    ...
    if lib.is_scalar(data):
        msg = f"Cannot pass scalar '{data***REMOVED***' to 'pandas.array'."
...
def _sanitize_non_ordered(data) -> None:
    ...
    if isinstance(data, (set, frozenset)):
        raise TypeError(f"'{type(data).__name__***REMOVED***' type is unordered")
    ...
    if result.ndim == 1:
        ...
        if isinstance(data, np.ndarray):
            if allow_2d:
                return result
            raise ValueError(
                f"Data must be 1-dimensional, got ndarray of shape {data.shape***REMOVED*** instead"
***REMOVED***
