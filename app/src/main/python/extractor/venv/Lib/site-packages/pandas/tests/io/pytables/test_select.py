...
def test_select_dtypes(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        for v in [True, "true", 1]:
            result = store.select("df", f"boolv == {v***REMOVED***", columns=["A", "boolv"])
            tm.assert_frame_equal(expected, result)
    ...
    with ensure_clean_store(setup_path) as store:
        # floats w/o NaN
        df = DataFrame({"cols": range(11), "values": range(11)***REMOVED***, dtype="float64")
...
def test_select_with_many_inputs(setup_path):
    with ensure_clean_store(setup_path) as store:
        df = DataFrame(
            {
                ...
                + ["b"] * 50
                + ["c"] * 100
                + [f"a{i:03d***REMOVED***" for i in range(100)],
            ***REMOVED***
        )
        _maybe_remove(store, "df")
        ...
        selector = ["a", "b", "c"] + [f"a{i:03d***REMOVED***" for i in range(60)]
...
def test_select_iterator_complete_8014(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        # of range, works
        where = f"index >= '{beg_dt***REMOVED***'"
        result = store.select("df", where=where)
        tm.assert_frame_equal(expected, result)

        # select w/o iterator and where clause, single term, end
        # of range, works
        where = f"index <= '{end_dt***REMOVED***'"
        result = store.select("df", where=where)
        tm.assert_frame_equal(expected, result)

        # select w/o iterator and where clause, inclusive range,
        # works
        where = f"index >= '{beg_dt***REMOVED***' & index <= '{end_dt***REMOVED***'"
        result = store.select("df", where=where)
        tm.assert_frame_equal(expected, result)
...
def test_frame_select_complex2(tmp_path):
    ...
    params = DataFrame({"A": [1, 1, 2, 2, 3]***REMOVED***)
...
def test_nan_selection_bug_4858(setup_path):
    with ensure_clean_store(setup_path) as store:
        df = DataFrame({"cols": range(6), "values": range(6)***REMOVED***, dtype="float64")
        df["cols"] = (df["cols"] + 10).apply(str)
        df.iloc[0] = np.nan

        expected = DataFrame(
            {"cols": ["13.0", "14.0", "15.0"], "values": [3.0, 4.0, 5.0]***REMOVED***,
            index=[3, 4, 5],
        )
...
def test_query_long_float_literal(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        result = store.select("test", f"A < {cutoff:.4f***REMOVED***")
        assert result.empty
        ...
        result = store.select("test", f"A == {exact:.4f***REMOVED***")
        expected = df.loc[[1], :]
        tm.assert_frame_equal(expected, result)
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        for op in ["<", ">", "=="]:
            ...
            for v in [2.1, True, Timestamp("2014-01-01"), pd.Timedelta(1, "s")]:
                query = f"date {op***REMOVED*** v"
                msg = f"Cannot compare {v***REMOVED*** of type {type(v)***REMOVED*** to string column"
                with pytest.raises(TypeError, match=msg):
            ...
            for col in ["int", "float", "real_date"]:
                query = f"{col***REMOVED*** {op***REMOVED*** v"
                if col == "real_date":
