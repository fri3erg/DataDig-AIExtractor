...
def _parse_token_or_url(url_or_token: str, api_url: str) -> Tuple[str, str]:
    ...
    if len(path_parts) >= 2:
        ...
        raise ls_utils.LangSmithUserError(f"Invalid public dataset URL: {url_or_token***REMOVED***")
...
def _serialize_json(obj: Any) -> Union[str, dict]:
    if isinstance(obj, datetime.datetime):
        ...
        try:
            ...
            logger.debug(f"Failed to serialize obj of type {type(obj)***REMOVED*** to JSON")
            return str(obj)
        ...
        try:
            ...
            logger.debug(f"Failed to json serialize {type(obj)***REMOVED*** to JSON")
            return repr(obj)
    else:
        return str(obj)
...
def _as_uuid(value: ID_TYPE, var: str) -> uuid.UUID:
    try:
        ...
        raise ls_utils.LangSmithUserError(
            f"{var***REMOVED*** must be a valid UUID or UUID string. Got {value***REMOVED***"
        ) from e
...
class Client:
    ...
    def _repr_html_(self) -> str:
        ...
        """
        link = self._host_url
        return f'<a href="{link***REMOVED***", target="_blank" rel="noopener">LangSmith Client</a>'
    ...
    def __repr__(self) -> str:
        ...
        """
        return f"Client (API URL: {self.api_url***REMOVED***)"
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        """
        headers = {"User-Agent": f"langsmith-py/{langsmith.__version__***REMOVED***"***REMOVED***
        if self.api_key:
    ...
    def request_with_retries(
                    ...
                    if response is not None:
                        if response.status_code == 500:
                            raise ls_utils.LangSmithAPIError(
                                f"Server error caused failure to {request_method***REMOVED***"
                                f" {url***REMOVED*** in"
                                f" LangSmith API. {repr(e)***REMOVED***"
                ***REMOVED***
                        elif response.status_code == 429:
                            raise ls_utils.LangSmithRateLimitError(
                                f"Rate limit exceeded for {url***REMOVED***. {repr(e)***REMOVED***"
                ***REMOVED***
                        elif response.status_code == 401:
                            raise ls_utils.LangSmithAuthError(
                                f"Authentication failed for {url***REMOVED***. {repr(e)***REMOVED***"
                ***REMOVED***
                        elif response.status_code == 404:
                            raise ls_utils.LangSmithNotFoundError(
                                f"Resource not found for {url***REMOVED***. {repr(e)***REMOVED***"
                ***REMOVED***
                        else:
                            raise ls_utils.LangSmithError(
                                f"Failed to {request_method***REMOVED*** {url***REMOVED*** in LangSmith"
                                f" API. {repr(e)***REMOVED***"
                ***REMOVED***

                    else:
                        raise ls_utils.LangSmithUserError(
                            f"Failed to {request_method***REMOVED*** {url***REMOVED*** in LangSmith API."
                            f" {repr(e)***REMOVED***"
            ***REMOVED***
                except requests.ConnectionError as e:
                    raise ls_utils.LangSmithConnectionError(
                        f"Connection error caused failure to {request_method***REMOVED*** {url***REMOVED***"
        ...
        ***REMOVED*** from e
                    ...
                    raise ls_utils.LangSmithError(
                        f"Failed to {request_method***REMOVED*** {url***REMOVED*** in LangSmith API. {emsg***REMOVED***"
        ***REMOVED*** from e
    ...
    def _get_with_retries(
        ...
        return self.request_with_retries(
            "get",
            f"{self.api_url***REMOVED***{path***REMOVED***",
            request_kwargs={
                "params": params,
    ...
    def _get_paginated_list(
        ...
        """
        params_ = params.copy() if params else {***REMOVED***
    ...
    def _get_cursor_paginated_list(
        ...
        *,
        ...
        dict
            The items in the paginated list.
        """
        params_ = body.copy() if body else {***REMOVED***
        while True:
...
***REMOVED***
        ...
        if "detail" in result and "already exists" in result["detail"]:
            ...
            raise ValueError(f"Dataset {file_name***REMOVED*** already exists")
...
***REMOVED***,
        ...
        ***REMOVED***
        ...
        run_extra = cast(dict, run_create.setdefault("extra", {***REMOVED***))
        runtime = run_extra.setdefault("runtime", {***REMOVED***)
        metadata: dict = run_extra.setdefault("metadata", {***REMOVED***)
        runtime_env = ls_env.get_runtime_and_metrics()
        langchain_metadata = ls_env.get_langchain_env_var_metadata()
        metadata.update(
            {k: v for k, v in langchain_metadata.items() if k not in metadata***REMOVED***
        )
        run_extra["runtime"] = {**runtime_env, **runtime***REMOVED***
        headers = {
            **self._headers,
    ...
    def update_run(
        ...
        ***REMOVED***
        ...
        self.request_with_retries(
            "patch",
            f"{self.api_url***REMOVED***/runs/{_as_uuid(run_id, 'run_id')***REMOVED***",
            request_kwargs={
    ...
    def _load_child_runs(self, run: ls_schemas.Run) -> ls_schemas.Run:
        ...
        runs: Dict[uuid.UUID, ls_schemas.Run] = {***REMOVED***
        for child_run in sorted(
            ...
            if child_run.parent_run_id is None:
                raise ls_utils.LangSmithError(f"Child run {child_run.id***REMOVED*** has no parent")
    ...
    def read_run(
        ...
        Run
            The run.
        """
        response = self._get_with_retries(f"/runs/{_as_uuid(run_id, 'run_id')***REMOVED***")
    ...
    def list_runs(
        ...
        **kwargs : Any
        ...
        body_query: Dict[str, Any] = {
            ...
            **kwargs,
        ***REMOVED***
        body_query = {k: v for k, v in body_query.items() if v is not None***REMOVED***
        yield from (
...
***REMOVED***
    ...
    def get_run_url(
        ...
        *,
        ...
        return (
            f"{self._host_url***REMOVED***/o/{self._get_tenant_id()***REMOVED***/projects/p/{session_id_***REMOVED***/"
            f"r/{run.id***REMOVED***?poll=true"
        )
    ...
    def share_run(self, run_id: ID_TYPE, *, share_id: Optional[ID_TYPE] = None) -> str:
        ...
        ***REMOVED***
        response = self.session.put(
            f"{self.api_url***REMOVED***/runs/{run_id_***REMOVED***/share",
            headers=self._headers,
            json=data,
        )
        ls_utils.raise_for_status_with_text(response)
        share_token = response.json()["share_token"]
        return f"{self._host_url***REMOVED***/public/{share_token***REMOVED***/r"

    def unshare_run(self, run_id: ID_TYPE) -> None:
        """Delete share link for a run."""
        response = self.session.delete(
            f"{self.api_url***REMOVED***/runs/{_as_uuid(run_id, 'run_id')***REMOVED***/share",
            headers=self._headers,
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def read_run_shared_link(self, run_id: ID_TYPE) -> Optional[str]:
        ...
        if result is None or "share_token" not in result:
            return None
        return f"{self._host_url***REMOVED***/public/{result['share_token']***REMOVED***/r"
    ...
    def list_shared_runs(
        ...
        """Get shared runs."""
        params = {"id": run_ids, "share_token": str(share_token)***REMOVED***
        response = self.session.get(
            f"{self.api_url***REMOVED***/public/{_as_uuid(share_token, 'share_token')***REMOVED***/runs",
            headers=self._headers,
            params=params,
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def read_dataset_shared_schema(
        ...
        *,
        ...
        response = self.session.get(
            f"{self.api_url***REMOVED***/datasets/{_as_uuid(dataset_id, 'dataset_id')***REMOVED***/share",
            headers=self._headers,
        )
        ls_utils.raise_for_status_with_text(response)
        d = response.json()
        return cast(
            ...
            {
                **d,
                "url": f"{self._host_url***REMOVED***/public/"
                f"{_as_uuid(d['share_token'], 'response.share_token')***REMOVED***/d",
            ***REMOVED***,
        )
    ...
    def share_dataset(
        ...
        ***REMOVED***
        ...
        return cast(
            ls_schemas.DatasetShareSchema,
            {**d, "url": f"{self._host_url***REMOVED***/public/{d['share_token']***REMOVED***/d"***REMOVED***,
        )
    ...
    def read_shared_dataset(
        ...
        """Get shared datasets."""
        response = self.session.get(
            f"{self.api_url***REMOVED***/public/{_as_uuid(share_token, 'share_token')***REMOVED***/datasets",
            headers=self._headers,
        )
        ls_utils.raise_for_status_with_text(response)
        return ls_schemas.Dataset(
            **response.json(),
            ...
            _public_path=f"/public/{share_token***REMOVED***/d",
        )
    ...
    def list_shared_examples(
        ...
        response = self.session.get(
            f"{self.api_url***REMOVED***/public/{_as_uuid(share_token, 'share_token')***REMOVED***/examples",
            headers=self._headers,
            params=params,
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def list_shared_projects(
        ...
        *,
        ...
        params = {"id": project_ids, "name": name, "name_contains": name_contains***REMOVED***
        ...
        yield from [
            ...
            for project in self._get_paginated_list(
                f"/public/{share_token***REMOVED***/datasets/sessions",
                params=params,
***REMOVED***
        ]
    ...
    def create_project(
        ...
        *,
        ...
        TracerSession
            The created project.
        """
        endpoint = f"{self.api_url***REMOVED***/sessions"
        extra = project_extra
        if metadata:
            extra = {**(extra or {***REMOVED***), "metadata": metadata***REMOVED***
        body: Dict[str, Any] = {
            "name": project_name,
            "extra": extra,
            "description": description,
        ***REMOVED***
        ...
        response = self.session.post(
            endpoint,
            headers={**self._headers, "Content-Type": "application/json"***REMOVED***,
    ...
    def update_project(
        ...
        *,
        ...
        TracerSession
            The updated project.
        """
        endpoint = f"{self.api_url***REMOVED***/sessions/{_as_uuid(project_id, 'project_id')***REMOVED***"
        extra = project_extra
        if metadata:
            extra = {**(extra or {***REMOVED***), "metadata": metadata***REMOVED***
        body: Dict[str, Any] = {
    ...
    def _get_tenant_id(self) -> uuid.UUID:
        ...
        response = self._get_with_retries("/sessions", params={"limit": 1***REMOVED***)
        result = response.json()
...
***REMOVED***
    ...
    def read_project(
        ...
        """
        path = "/sessions"
        params: Dict[str, Any] = {"limit": 1***REMOVED***
        if project_id is not None:
            path += f"/{_as_uuid(project_id, 'project_id')***REMOVED***"
        elif project_name is not None:
        ...
        if isinstance(result, list):
            if len(result) == 0:
                raise ls_utils.LangSmithNotFoundError(
                    f"Project {project_name***REMOVED*** not found"
    ***REMOVED***
        ...
        *,
        ...
        for r in runs:
            row = {
                ...
                **{f"input.{k***REMOVED***": v for k, v in r.inputs.items()***REMOVED***,
                **{f"outputs.{k***REMOVED***": v for k, v in (r.outputs or {***REMOVED***).items()***REMOVED***,
            ***REMOVED***
            if r.feedback_stats:
                for k, v in r.feedback_stats.items():
                    row[f"feedback.{k***REMOVED***"] = v.get("avg")
            row.update(
                {
                ...
                example_outputs.append(
                    {
                        ...
                        **{
                            f"reference.{k***REMOVED***": v
                            for k, v in (example.outputs or {***REMOVED***).items()
                        ***REMOVED***,
                    ***REMOVED***
    ***REMOVED***
        if example_outputs:
...
***REMOVED***.id
    ...
    def delete_project(
        ...
        if project_name is not None:
            ...
            raise ValueError("Must provide project_name or project_id")
        response = self.session.delete(
            self.api_url + f"/sessions/{_as_uuid(project_id, 'project_id')***REMOVED***",
            headers=self._headers,
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def read_dataset(
        ...
        *,
        ...
        if dataset_id is not None:
            path += f"/{_as_uuid(dataset_id, 'dataset_id')***REMOVED***"
        ...
        if isinstance(result, list):
            if len(result) == 0:
                raise ls_utils.LangSmithNotFoundError(
                    f"Dataset {dataset_name***REMOVED*** not found"
    ***REMOVED***
            return ls_schemas.Dataset(
...
***REMOVED***
    ...
    def read_dataset_openai_finetuning(
        ...
        if dataset_id is not None:
            ...
            raise ValueError("Must provide dataset_name or dataset_id")
        response = self._get_with_retries(
            f"{path***REMOVED***/{_as_uuid(dataset_id, 'dataset_id')***REMOVED***/openai_ft",
        )
...
***REMOVED***
    ...
    def delete_dataset(
        ...
        *,
        ...
        response = self.session.delete(
            f"{self.api_url***REMOVED***/datasets/{_as_uuid(dataset_id, 'dataset_id')***REMOVED***",
            headers=self._headers,
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def clone_public_dataset(
        ...
        *,
        ...
        if self.has_dataset(dataset_name=dataset_name):
            logger.info(
                f"Dataset {dataset_name***REMOVED*** already exists in your tenant. Skipping."
***REMOVED***
            return
        try:
...
***REMOVED***
    ...
    ***REMOVED***
                ...
                logger.error(
                    f"An error occurred while creating dataset {dataset_name***REMOVED***. "
    ...
    ***REMOVED***
    ...
    def create_llm_example(
        ...
        return self.create_example(
            inputs={"input": prompt***REMOVED***,
            outputs={"output": generation***REMOVED***,
    ...
    ***REMOVED***
        ...
        return self.create_example(
            inputs={"input": final_input***REMOVED***,
            outputs={"output": final_generations***REMOVED***
    ...
    def create_example_from_run(
        ...
        if dataset_type == ls_schemas.DataType.llm:
            if run.run_type != "llm":
                raise ValueError(
                    f"Run type {run.run_type***REMOVED*** is not supported"
    ...
    ***REMOVED***
            try:
                ...
                raise ValueError(
                    ...
                    f" {run.id***REMOVED*** with inputs {run.inputs***REMOVED***"
    ***REMOVED***
            inputs: Dict[str, Any] = {"input": prompt***REMOVED***
            if not run.outputs:
                ...
                try:
                    ...
                    raise ValueError(
                        ...
                        f" {run.id***REMOVED*** with outputs {run.outputs***REMOVED***"
        ***REMOVED***
                outputs = {"output": generation***REMOVED***
    ...
    ***REMOVED***
            try:
                inputs = {"input": ls_utils.get_messages_from_inputs(run.inputs)***REMOVED***
            except ValueError:
                raise ValueError(
            ...
            ***REMOVED***
                    ***REMOVED***
                    ...
                    raise ValueError(
                        ...
                        f" for run {run.id***REMOVED*** with outputs {run.outputs***REMOVED***"
        ***REMOVED***
            ...
            raise ValueError(f"Dataset type {dataset_type***REMOVED*** not recognized.")
        return self.create_example(
            inputs=inputs,
            outputs=outputs,
            dataset_id=dataset_id,
    ...
    ***REMOVED***
    ...
    def create_example(
        ...
        ***REMOVED***
        ...
        response = self.session.post(
            f"{self.api_url***REMOVED***/examples",
            headers={**self._headers, "Content-Type": "application/json"***REMOVED***,
            data=example.json(),
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def read_example(self, example_id: ID_TYPE) -> ls_schemas.Example:
        ...
        """
        response = self._get_with_retries(
            f"/examples/{_as_uuid(example_id, 'example_id')***REMOVED***",
        )
        return ls_schemas.Example(
            **response.json(),
            _host_url=self._host_url,
...
***REMOVED***
    ...
    def update_example(
        ...
        *,
        ...
        response = self.session.patch(
            f"{self.api_url***REMOVED***/examples/{_as_uuid(example_id, 'example_id')***REMOVED***",
            headers={**self._headers, "Content-Type": "application/json"***REMOVED***,
...
***REMOVED***
    ...
    def _select_eval_results(
        ...
        if isinstance(results, ls_evaluator.EvaluationResult):
            ...
            raise TypeError(
                f"Invalid evaluation result type {type(results)***REMOVED***."
...
***REMOVED***
    ...
    def _log_evaluation_feedback(
        ...
        for res in results:
            source_info_ = source_info or {***REMOVED***
            if res.evaluator_info:
                source_info_ = {**res.evaluator_info, **source_info_***REMOVED***
...
***REMOVED***
            ...
            raise ValueError(f"Unknown feedback source type {feedback_source_type***REMOVED***")
        feedback_source.metadata = (
            feedback_source.metadata if feedback_source.metadata is not None else {***REMOVED***
        )
        if source_run_id is not None and "__run" not in feedback_source.metadata:
            feedback_source.metadata["__run"] = {"run_id": str(source_run_id)***REMOVED***
    ...
    ***REMOVED***,
    ...
    def update_feedback(
        ...
        *,
        ...
        """
        feedback_update: Dict[str, Any] = {***REMOVED***
        ...
        response = self.session.patch(
            self.api_url + f"/feedback/{_as_uuid(feedback_id, 'feedback_id')***REMOVED***",
            headers={**self._headers, "Content-Type": "application/json"***REMOVED***,
    ...
    def read_feedback(self, feedback_id: ID_TYPE) -> ls_schemas.Feedback:
        ...
        """
        response = self._get_with_retries(
            f"/feedback/{_as_uuid(feedback_id, 'feedback_id')***REMOVED***",
        )
    ...
    def delete_feedback(self, feedback_id: ID_TYPE) -> None:
        ...
        """
        response = self.session.delete(
            f"{self.api_url***REMOVED***/feedback/{_as_uuid(feedback_id, 'feedback_id')***REMOVED***",
            headers=self._headers,
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def list_annotation_queues(
        ...
        *,
        ...
        params: dict = {
            "ids": [_as_uuid(id_, f"queue_ids[{i***REMOVED***]") for i, id_ in enumerate(queue_ids)]
    ...
    def create_annotation_queue(
        ...
        ***REMOVED***
        response = self.request_with_retries(
            "post",
            f"{self.api_url***REMOVED***/annotation-queues",
            {
                "json": {k: v for k, v in body.items() if v is not None***REMOVED***,
                "headers": self._headers,
            ***REMOVED***,
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def update_annotation_queue(
        ...
        response = self.request_with_retries(
            "patch",
            f"{self.api_url***REMOVED***/annotation-queues/{_as_uuid(queue_id, 'queue_id')***REMOVED***",
            {
                "json": {
                    "name": name,
                    "description": description,
    ...
    def add_runs_to_annotation_queue(
        ...
        response = self.request_with_retries(
            "post",
            f"{self.api_url***REMOVED***/annotation-queues/{_as_uuid(queue_id, 'queue_id')***REMOVED***/runs",
            {
                "json": [
                    str(_as_uuid(id_, f"run_ids[{i***REMOVED***]")) for i, id_ in enumerate(run_ids)
                ],
                "headers": self._headers,
            ***REMOVED***,
        )
    ...
    def list_runs_from_annotation_queue(
        ...
        path = f"/annotation-queues/{_as_uuid(queue_id, 'queue_id')***REMOVED***/runs"
        yield from (
            ls_schemas.RunWithAnnotationQueueInfo(**run)
            for run in self._get_paginated_list(path, params={"headers": self._headers***REMOVED***)
        )
        ...
        *,
        ...
        Args:
            ...
            project_name: Name of the project to store the traces in.
                Defaults to {dataset_name***REMOVED***-{chain class name***REMOVED***-{datetime***REMOVED***.
        ...
        .. code-block:: python
            ...
            def construct_chain():
                ...
                chain = LLMChain.from_string(
                    llm,
                    "What's the answer to {your_input_key***REMOVED***"
    ***REMOVED***
                return chain
        ...
        .. code-block:: python
            ...
            class MyStringEvaluator(StringEvaluator):
                ...
                def _evaluate_strings(self, prediction, reference=None, input=None, **kwargs) -> dict:
                    return {"score": prediction == reference***REMOVED***
