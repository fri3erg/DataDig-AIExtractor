...
class TestDataFrameUnaryOperators:
    ...
    @pytest.mark.parametrize(
        "df,expected",
        [
            (pd.DataFrame({"a": [-1, 1]***REMOVED***), pd.DataFrame({"a": [1, -1]***REMOVED***)),
            (pd.DataFrame({"a": [False, True]***REMOVED***), pd.DataFrame({"a": [True, False]***REMOVED***)),
            (
                pd.DataFrame({"a": pd.Series(pd.to_timedelta([-1, 1]))***REMOVED***),
                pd.DataFrame({"a": pd.Series(pd.to_timedelta([1, -1]))***REMOVED***),
***REMOVED***,
        ],
    )
    ...
    def test_neg_object(self, df, expected):
        # GH#21380
        df = pd.DataFrame({"a": df***REMOVED***)
        expected = pd.DataFrame({"a": expected***REMOVED***)
        tm.assert_frame_equal(-df, expected)
        tm.assert_series_equal(-df["a"], expected["a"])
    ...
    @pytest.mark.parametrize(
        "df",
        [
            pd.DataFrame({"a": ["a", "b"]***REMOVED***),
            pd.DataFrame({"a": pd.to_datetime(["2017-01-22", "1970-01-01"])***REMOVED***),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "df",
        [
            pd.DataFrame({"a": [-1, 1]***REMOVED***),
            pd.DataFrame({"a": [False, True]***REMOVED***),
            pd.DataFrame({"a": pd.Series(pd.to_timedelta([-1, 1]))***REMOVED***),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "df",
        [
            pd.DataFrame({"a": np.array([-1, 2], dtype=object)***REMOVED***),
            pd.DataFrame({"a": [Decimal("-1.0"), Decimal("2.0")]***REMOVED***),
        ],
    )
...
***REMOVED***:
    ...
    @pytest.mark.parametrize(
        "df", [pd.DataFrame({"a": pd.to_datetime(["2017-01-22", "1970-01-01"])***REMOVED***)]
    )
