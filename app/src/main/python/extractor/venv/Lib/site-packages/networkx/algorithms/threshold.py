...
def degree_correlation(creation_sequence):
    ...
    if denom == 0:
        if numer == 0:
            return 1
        raise ValueError(f"Zero Denominator but Numerator is {numer***REMOVED***")
    return numer / denom
...
def shortest_path(creation_sequence, u, v):
    ...
    if v not in verts:
        raise ValueError(f"Vertex {v***REMOVED*** not in graph from creation_sequence")
    if u not in verts:
        raise ValueError(f"Vertex {u***REMOVED*** not in graph from creation_sequence")
    # Done checking
    if u == v:
        return [u]
...
def eigenvalues(creation_sequence):
    ...
    See::
      ...
      @Article{degree-merris-1994,
       author = {Russel Merris***REMOVED***,
       title = {Degree maximal graphs are Laplacian integral***REMOVED***,
       journal = {Linear Algebra Appl.***REMOVED***,
       year = {1994***REMOVED***,
       volume = {199***REMOVED***,
       pages = {381--389***REMOVED***,
      ***REMOVED***
