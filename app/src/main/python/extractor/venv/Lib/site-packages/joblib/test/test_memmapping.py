...
def test_resource_tracker_retries_when_permissionerror(tmpdir):
    ...
    memmap = np.memmap(r"{filename***REMOVED***", dtype=np.float64, shape=10, mode='w+')
    memmap[:] = np.arange(10).astype(np.int8).data
    memmap.flush()
    assert os.path.exists(r"{filename***REMOVED***")
    del memmap

    # Create a np.memmap backed by this file
    memmap = np.memmap(r"{filename***REMOVED***", dtype=np.float64, shape=10, mode='w+')
    resource_tracker.register(r"{filename***REMOVED***", "file")
    ...
    resource_tracker.maybe_unlink(r"{filename***REMOVED***", "file")
    ...
    assert out == b''
    msg = 'tried to unlink {***REMOVED***, got PermissionError'.format(filename)
...
def test_permission_error_windows_reference_cycle(backend):
    ...
    cmd = """if 1:
        ...
        ***REMOVED***
            results = Parallel(n_jobs=2, backend="{b***REMOVED***")(
...
def test_permission_error_windows_memmap_sent_to_parent(backend):
    ...
    cmd = '''if 1:
        ...
        if __name__ == '__main__':
            ...
            _ = Parallel(n_jobs=2, verbose=5, backend='{b***REMOVED***')(
                delayed(id)(i) for i in range(20))

            time.sleep(0.5)

            slice_of_data = Parallel(n_jobs=2, verbose=5, backend='{b***REMOVED***')(
...
def test_multithreaded_parallel_termination_resource_tracker_silent():
    ...
    cmd = '''if 1:
        ...
        ***REMOVED***
        ...
        future_1 = executor.submit({f1***REMOVED***, array, temp_dirs_thread_1)
        future_2 = executor.submit({f2***REMOVED***, array, temp_dirs_thread_2)
        ...
        if __name__ == '__main__':
            for i in range(5):
                slice_of_data = Parallel(
                    n_jobs=2, max_nbytes=1, backend='{b***REMOVED***')(
    ...
    try:
        ...
        for i in range(10):
            ...
            raise AssertionError(
                'temporary folder {***REMOVED*** was not deleted'.format(p._temp_folder)
***REMOVED***
        del p
...
def test_child_raises_parent_exits_cleanly(backend):
    ...
    cmd = """if 1:
        ...
        ***REMOVED***
            try:
                with Parallel(n_jobs=2, backend="{b***REMOVED***", max_nbytes=100) as p:
                    temp_folder = get_temp_folder(p, "{b***REMOVED***")
...
@parametrize(
    ...
    ids=['{***REMOVED***, {***REMOVED***'.format(x, y) for x, y in itertools.product(
