...
def test_union_attributes():
    ...
    assert set(gh.nodes()) == {"h0", "h1", "g0", "g1"***REMOVED***
    for n in gh:
        graph, node = n
...
def test_intersection():
    ...
    assert set(I.nodes()) == {1, 2, 3, 4***REMOVED***
    ...
    assert set(I2.nodes()) == {1, 2, 3, 4***REMOVED***
...
def test_difference():
    ...
    H.add_edge(3, 4)
    D = nx.difference(G, H)
    assert set(D.nodes()) == {1, 2, 3, 4***REMOVED***
...
def test_union_and_compose():
    ...
    G = nx.Graph()
    H = nx.Graph()
    G.add_nodes_from([(1, {"a1": 1***REMOVED***)])
    H.add_nodes_from([(1, {"b1": 1***REMOVED***)])
    R = nx.compose(G, H)
    assert R.nodes == {1: {"a1": 1, "b1": 1***REMOVED******REMOVED***
...
def test_full_join_graph():
    ...
    assert set(U) == {"g0", "g1", "g2", "h3", "h4"***REMOVED***
    assert len(U) == len(G) + len(H)
    assert len(U.edges()) == len(G.edges()) + len(H.edges()) + len(G) * len(H)
    ...
    U = nx.full_join(G, H, rename=("g", "h"))
    assert set(U) == {"ga", "gb", "gc", "hd", "he"***REMOVED***
    assert len(U) == len(G) + len(H)
    assert len(U.edges()) == len(G.edges()) + len(H.edges()) + len(G) * len(H)
