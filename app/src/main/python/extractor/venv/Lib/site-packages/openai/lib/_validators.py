...
def num_examples_validator(df: pd.DataFrame) -> Remediation:
    ...
    immediate_msg = f"\n- Your file contains {len(df)***REMOVED*** prompt-completion pairs{optional_suggestion***REMOVED***"
...
def necessary_column_validator(df: pd.DataFrame, necessary_column: str) -> Remediation:
    ...
    def lower_case_column(df: pd.DataFrame, column: Any) -> pd.DataFrame:
        ...
        df.rename(columns={cols[0]: column.lower()***REMOVED***, inplace=True)
        return df
    ...
    if necessary_column not in df.columns:
        if necessary_column in [str(c).lower() for c in df.columns]:
            ...
            immediate_msg = f"\n- The `{necessary_column***REMOVED***` column/key should be lowercase"
            necessary_msg = f"Lower case column name to `{necessary_column***REMOVED***`"
        else:
            error_msg = f"`{necessary_column***REMOVED***` column/key is missing. Please make sure you name your columns/keys appropriately, then retry"
...
def additional_column_validator(df: pd.DataFrame, fields: list[str] = ["prompt", "completion"]) -> Remediation:
    ...
    if len(df.columns) > 2:
        ...
        for ac in additional_columns:
            ...
            if len(dups) > 0:
                warn_message += f"\n  WARNING: Some of the additional columns/keys contain `{ac***REMOVED***` in their name. These will be ignored, and the column/key `{ac***REMOVED***` will be used instead. This could also result from a duplicate column/key in the provided file."
        immediate_msg = f"\n- The input file should contain exactly two columns/keys per row. Additional columns/keys present are: {additional_columns***REMOVED***{warn_message***REMOVED***"
        necessary_msg = f"Remove additional columns/keys: {additional_columns***REMOVED***"
...
def non_empty_field_validator(df: pd.DataFrame, field: str = "completion") -> Remediation:
    ...
    if df[field].apply(lambda x: x == "").any() or df[field].isnull().any():
        ...
        immediate_msg = f"\n- `{field***REMOVED***` column/key should not contain empty strings. These are rows: {empty_indexes***REMOVED***"

        def necessary_fn(x: Any) -> Any:
            return x[x[field] != ""].dropna(subset=[field])

        necessary_msg = f"Remove {len(empty_indexes)***REMOVED*** rows with empty {field***REMOVED***s"

    return Remediation(
        name=f"empty_{field***REMOVED***",
        immediate_msg=immediate_msg,
        necessary_msg=necessary_msg,
        necessary_fn=necessary_fn,
    )
...
def duplicated_rows_validator(df: pd.DataFrame, fields: list[str] = ["prompt", "completion"]) -> Remediation:
    ...
    if len(duplicated_indexes) > 0:
        immediate_msg = f"\n- There are {len(duplicated_indexes)***REMOVED*** duplicated {'-'.join(fields)***REMOVED*** sets. These are rows: {duplicated_indexes***REMOVED***"
        optional_msg = f"Remove {len(duplicated_indexes)***REMOVED*** duplicate rows"
...
def long_examples_validator(df: pd.DataFrame) -> Remediation:
    ...
    if ft_type != "open-ended generation":
        ...
        if len(long_indexes) > 0:
            immediate_msg = f"\n- There are {len(long_indexes)***REMOVED*** examples that are very long. These are rows: {long_indexes***REMOVED***\nFor conditional generation, and for classification the examples shouldn't be longer than 2048 tokens."
            optional_msg = f"Remove {len(long_indexes)***REMOVED*** long examples"

            def optional_fn(x: Any) -> Any:
                long_indexes_to_drop = get_long_indexes(x)
                if long_indexes != long_indexes_to_drop:
                    sys.stdout.write(
                        f"The indices of the long examples has changed as a result of a previously applied recommendation.\nThe {len(long_indexes_to_drop)***REMOVED*** long examples to be dropped are now at the following indices: {long_indexes_to_drop***REMOVED***\n"
        ***REMOVED***
...
def common_prompt_suffix_validator(df: pd.DataFrame) -> Remediation:
    ...
    if (df.prompt == common_suffix).all():
        error_msg = f"All prompts are identical: `{common_suffix***REMOVED***`\nConsider leaving the prompts blank if you want to do open-ended generation, otherwise ensure prompts are different"
        return Remediation(name="common_suffix", error_msg=error_msg)

    if common_suffix != "":
        common_suffix_new_line_handled = common_suffix.replace("\n", "\\n")
        immediate_msg = f"\n- All prompts end with suffix `{common_suffix_new_line_handled***REMOVED***`"
        if len(common_suffix) > 10:
            immediate_msg += f". This suffix seems very long. Consider replacing with a shorter suffix, such as `{display_suggested_suffix***REMOVED***`"
        if df.prompt.str[: -len(common_suffix)].str.contains(common_suffix, regex=False).any():
            immediate_msg += f"\n  WARNING: Some of your prompts contain the suffix `{common_suffix***REMOVED***` more than once. We strongly suggest that you review your prompts and add a unique suffix"

    else:
        immediate_msg = "\n- Your data does not contain a common separator at the end of your prompts. Having a separator string appended to the end of the prompt makes it clearer to the fine-tuned model where the completion should begin. See https://platform.openai.com/docs/guides/fine-tuning/preparing-your-dataset for more detail and examples. If you intend to do open-ended generation, then you should leave the prompts empty"

    if common_suffix == "":
        optional_msg = f"Add a suffix separator `{display_suggested_suffix***REMOVED***` to all prompts"
...
def common_prompt_prefix_validator(df: pd.DataFrame) -> Remediation:
    ...
    if common_prefix != "":
        immediate_msg = f"\n- All prompts start with prefix `{common_prefix***REMOVED***`"
        if MAX_PREFIX_LEN < len(common_prefix):
            ...
            optional_msg = f"Remove prefix `{common_prefix***REMOVED***` from all prompts"
...
def common_completion_prefix_validator(df: pd.DataFrame) -> Remediation:
    ...
    def remove_common_prefix(x: Any, prefix: Any, ws_prefix: Any) -> Any:
        ...
        if ws_prefix:
            ...
            x["completion"] = f" {x['completion']***REMOVED***"
        return x
    ...
    immediate_msg = f"\n- All completions start with prefix `{common_prefix***REMOVED***`. Most of the time you should only add the output data into the completion, without any prefix"
    optional_msg = f"Remove prefix `{common_prefix***REMOVED***` from all completions"
...
def common_completion_suffix_validator(df: pd.DataFrame) -> Remediation:
    ...
    if (df.completion == common_suffix).all():
        error_msg = f"All completions are identical: `{common_suffix***REMOVED***`\nEnsure completions are different, otherwise the model will just repeat `{common_suffix***REMOVED***`"
        return Remediation(name="common_suffix", error_msg=error_msg)
    ...
    if common_suffix != "":
        common_suffix_new_line_handled = common_suffix.replace("\n", "\\n")
        immediate_msg = f"\n- All completions end with suffix `{common_suffix_new_line_handled***REMOVED***`"
        if len(common_suffix) > 10:
            immediate_msg += f". This suffix seems very long. Consider replacing with a shorter suffix, such as `{display_suggested_suffix***REMOVED***`"
        if df.completion.str[: -len(common_suffix)].str.contains(common_suffix, regex=False).any():
            immediate_msg += f"\n  WARNING: Some of your completions contain the suffix `{common_suffix***REMOVED***` more than once. We suggest that you review your completions and add a unique ending"

    else:
        immediate_msg = "\n- Your data does not contain a common ending at the end of your completions. Having a common ending string appended to the end of the completion makes it clearer to the fine-tuned model where the completion should end. See https://platform.openai.com/docs/guides/fine-tuning/preparing-your-dataset for more detail and examples."

    if common_suffix == "":
        optional_msg = f"Add a suffix ending `{display_suggested_suffix***REMOVED***` to all completions"
...
def lower_case_validator(df: pd.DataFrame, column: Any) -> Remediation | None:
    ...
    if count_upper * 2 > count_lower:
        return Remediation(
            name="lower_case",
            immediate_msg=f"\n- More than a third of your `{column***REMOVED***` column/key is uppercase. Uppercase {column***REMOVED***s tends to perform worse than a mixture of case encountered in normal language. We recommend to lower case the data if that makes sense in your domain. See https://platform.openai.com/docs/guides/fine-tuning/preparing-your-dataset for more details",
            optional_msg=f"Lowercase all your data in column/key `{column***REMOVED***`",
...
def read_any_format(
    ...
    if os.path.isfile(fname):
        try:
            if fname.lower().endswith(".csv") or fname.lower().endswith(".tsv"):
                ...
                immediate_msg = (
                    f"\n- Based on your file extension, your file is formatted as a {file_extension_str***REMOVED*** file"
    ***REMOVED***
                necessary_msg = f"Your format `{file_extension_str***REMOVED***` will be converted to `JSONL`"
    ...
    ***REMOVED***
                if "." in fname:
                    error_msg += f" Your file `{fname***REMOVED***` ends with the extension `.{fname.split('.')[-1]***REMOVED***` which is not supported."
                else:
                    error_msg += f" Your file `{fname***REMOVED***` is missing a file extension."

        except (ValueError, TypeError):
            file_extension_str = fname.split(".")[-1].upper()
            error_msg = f"Your file `{fname***REMOVED***` does not appear to be in valid {file_extension_str***REMOVED*** format. Please ensure your file is formatted as a valid {file_extension_str***REMOVED*** file."

    else:
        error_msg = f"File {fname***REMOVED*** does not exist."
...
def format_inferrer_validator(df: pd.DataFrame) -> Remediation:
    ...
    if ft_type == "classification":
        immediate_msg = f"\n- Based on your data it seems like you're trying to fine-tune a model for {ft_type***REMOVED***\n- For classification, we recommend you try one of the faster and cheaper models, such as `ada`\n- For classification, you can estimate the expected model performance by keeping a held out dataset, which is not used for training"
    return Remediation(name="num_examples", immediate_msg=immediate_msg)
...
def apply_necessary_remediation(df: OptionalDataFrameT, remediation: Remediation) -> OptionalDataFrameT:
    ...
    if remediation.error_msg is not None:
        sys.stderr.write(f"\n\nERROR in {remediation.name***REMOVED*** validator: {remediation.error_msg***REMOVED***\n\nAborting...")
        sys.exit(1)
...
def apply_optional_remediation(
    ...
    input_text = f"- [Recommended] {remediation.optional_msg***REMOVED*** [Y/n]: "
    ...
    if remediation.necessary_msg is not None:
        sys.stdout.write(f"- [Necessary] {remediation.necessary_msg***REMOVED***\n")
...
def estimate_fine_tuning_time(df: pd.DataFrame) -> None:
    ...
    def format_time(time: float) -> str:
        if time < 60:
            return f"{round(time, 2)***REMOVED*** seconds"
        elif time < 3600:
            return f"{round(time / 60, 2)***REMOVED*** minutes"
        elif time < 86400:
            return f"{round(time / 3600, 2)***REMOVED*** hours"
        else:
            return f"{round(time / 86400, 2)***REMOVED*** days"

    time_string = format_time(expected_time + 140)
    sys.stdout.write(
        f"Once your model starts training, it'll approximately take {time_string***REMOVED*** to train a `curie` model, and less for `ada` and `babbage`. Queue will approximately take half an hour per job ahead of you.\n"
    )
...
def get_outfnames(fname: str, split: bool) -> list[str]:
    ...
    i = 0
    while True:
        index_suffix = f" ({i***REMOVED***)" if i > 0 else ""
        candidate_fnames = [f"{os.path.splitext(fname)[0]***REMOVED***_prepared{suffix***REMOVED***{index_suffix***REMOVED***.jsonl" for suffix in suffixes]
...
def write_out_file(df: pd.DataFrame, fname: str, any_remediations: bool, auto_accept: bool) -> None:
    ...
    optional_ending_string = (
        f' Make sure to include `stop=["{common_completion_suffix_new_line_handled***REMOVED***"]` so that the generated texts ends at the expected place.'
    ...
    if not any_remediations and not split:
        sys.stdout.write(
            f'\nYou can use your file for fine-tuning:\n> openai api fine_tunes.create -t "{fname***REMOVED***"{additional_params***REMOVED***\n\nAfter you’ve fine-tuned a model, remember that your prompt has to end with the indicator string `{common_prompt_suffix_new_line_handled***REMOVED***` for the model to start generating completions, rather than continuing with the prompt.{optional_ending_string***REMOVED***\n'
        )
...
***REMOVED***
            ...
            if n_classes == 2:
                additional_params += f' --classification_positive_class "{pos_class***REMOVED***"'
            else:
                additional_params += f" --classification_n_classes {n_classes***REMOVED***"
        else:
            assert len(fnames) == 1
        ...
        valid_string = f' -v "{fnames[1]***REMOVED***"' if split else ""
        separator_reminder = (
            ""
            if len(common_prompt_suffix_new_line_handled) == 0
            else f"After you’ve fine-tuned a model, remember that your prompt has to end with the indicator string `{common_prompt_suffix_new_line_handled***REMOVED***` for the model to start generating completions, rather than continuing with the prompt."
        )
        sys.stdout.write(
            f'\nWrote modified file{files_string***REMOVED***`\nFeel free to take a look!\n\nNow use that file when fine-tuning:\n> openai api fine_tunes.create -t "{fnames[0]***REMOVED***"{valid_string***REMOVED***{additional_params***REMOVED***\n\n{separator_reminder***REMOVED***{optional_ending_string***REMOVED***\n'
        )
        estimate_fine_tuning_time(df)
    else:
