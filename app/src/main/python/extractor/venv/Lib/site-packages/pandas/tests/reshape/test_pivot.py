...
class TestPivotTable:
    ...
    def test_pivot_table_categorical_observed_equal(self, observed):
        # issue #24923
        df = DataFrame(
            {"col1": list("abcde"), "col2": list("fghij"), "col3": [1, 2, 3, 4, 5]***REMOVED***
        )
    ...
    def test_pivot_table_nocols(self):
        df = DataFrame(
            {"rows": ["a", "b", "c"], "cols": ["x", "y", "z"], "values": [1, 2, 3]***REMOVED***
        )
        rs = df.pivot_table(columns="cols", aggfunc="sum")
        xp = df.pivot_table(index="cols", aggfunc="sum").T
        tm.assert_frame_equal(rs, xp)

        rs = df.pivot_table(columns="cols", aggfunc={"values": "mean"***REMOVED***)
        xp = df.pivot_table(index="cols", aggfunc={"values": "mean"***REMOVED***).T
        tm.assert_frame_equal(rs, xp)

    def test_pivot_table_dropna(self):
        df = DataFrame(
            {
                "amount": {0: 60000, 1: 100000, 2: 50000, 3: 30000***REMOVED***,
                "customer": {0: "A", 1: "A", 2: "B", 3: "C"***REMOVED***,
                "month": {0: 201307, 1: 201309, 2: 201308, 3: 201310***REMOVED***,
                "product": {0: "a", 1: "b", 2: "c", 3: "d"***REMOVED***,
                "quantity": {0: 2000000, 1: 500000, 2: 1000000, 3: 1000000***REMOVED***,
            ***REMOVED***
        )
    ...
    def test_pivot_table_categorical(self):
        ...
        expected = DataFrame({"values": [1.0, 2.0, 3.0, 4.0]***REMOVED***, index=exp_index)
        tm.assert_frame_equal(result, expected)
...
***REMOVED***,
    ...
    def test_pivot_with_interval_index(self, interval_values, dropna):
        # GH 25814
        df = DataFrame({"A": interval_values, "B": 1***REMOVED***)

        msg = "The default value of observed=False is deprecated"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = df.pivot_table(index="A", values="B", dropna=dropna)
        expected = DataFrame(
            {"B": 1.0***REMOVED***, index=Index(interval_values.unique(), name="A")
        )
        if not dropna:
...
***REMOVED***
    ...
    def test_pivot_preserve_dtypes(self, columns, values):
        ...
        df = DataFrame(
            {"float1": v, "float2": v + 2.0, "bool1": v <= 2, "bool2": v <= 3***REMOVED***
        )
        ...
        expected = {col: np.dtype("float64") for col in df_res***REMOVED***
    ...
    def test_pivot_no_values(self):
        ...
        df = DataFrame({"A": [1, 2, 3, 4, 5]***REMOVED***, index=idx)
        ...
        df = DataFrame(
            {
                "dt1": pd.DatetimeIndex(
                    ...
                    dtype=f"M8[{unit***REMOVED***, US/Pacific]",
    ***REMOVED***,
                "dt2": pd.DatetimeIndex(
                    ...
                    dtype=f"M8[{unit***REMOVED***, Asia/Tokyo]",
    ***REMOVED***,
...
***REMOVED***,
    ...
    def test_pivot_columns_none_raise_error(self):
        # GH 30924
        df = DataFrame({"col1": ["a", "b", "c"], "col2": [1, 2, 3], "col3": [1, 2, 3]***REMOVED***)
...
***REMOVED***,
    ...
    def test_margins_dtype(self, data):
        ...
        expected = DataFrame(
            {"dull": [12, 21, 3, 9, 45], "shiny": [33, 0, 36, 51, 120]***REMOVED***, index=mi
    ...
    def test_margins_dtype_len(self, data):
        ...
        mi = MultiIndex.from_tuples(mi_val, names=("A", "B"))
        expected = DataFrame(
            {"dull": [1, 1, 2, 1, 5], "shiny": [2, 0, 2, 2, 6]***REMOVED***, index=mi
        ).rename_axis("C", axis=1)
        expected["All"] = expected["dull"] + expected["shiny"]
    ...
    def test_pivot_table_multiindex_only(self, cols):
        # GH 17038
        df2 = DataFrame({cols[0]: [1, 2, 3], cols[1]: [1, 2, 3], "v": [4, 5, 6]***REMOVED***)
...
***REMOVED***
    ...
    def test_pivot_table_with_margins_set_margin_name(self, margin_name, data):
        # see gh-3335
        msg = (
            f'Conflicting name "{margin_name***REMOVED***" in margins|'
...
***REMOVED***,
    ...
    def test_pivot_dtaccessor(self):
        ...
        expected = DataFrame(
            {7: [0.0, 3.0], 8: [1.0, 4.0], 9: [2.0, 5.0]***REMOVED***,
            index=exp_idx,
...
***REMOVED***,
    ...
    def test_categorical_margins(self, observed):
        # GH 10989
        df = DataFrame(
            {"x": np.arange(8), "y": np.arange(8) // 4, "z": np.arange(8) % 2***REMOVED***
        )
    ...
    def test_margins_casted_to_float(self):
        ...
        expected = DataFrame(
            {"A": [3.0, 7.0, 5], "B": [2.5, 6.5, 4.5], "C": [2.0, 5.0, 3.5]***REMOVED***,
    ...
    def test_categorical_aggfunc(self, observed):
        # GH 9534
        df = DataFrame(
            {"C1": ["A", "B", "C", "C"], "C2": ["a", "a", "b", "b"], "V": [1, 2, 3, 4]***REMOVED***
        )
...
***REMOVED***
    ...
    def test_pivot_table_not_series(self):
        ...
        df = DataFrame({"col1": [3, 4, 5], "col2": ["C", "D", "E"], "col3": [1, 3, 9]***REMOVED***)
    ...
    def test_pivot_margins_name_unicode(self):
        # issue #13292
        greek = "\u0394\u03bf\u03ba\u03b9\u03bc\u03ae"
        frame = DataFrame({"foo": [1, 2, 3]***REMOVED***, columns=Index(["foo"], dtype=object))
        table = pivot_table(
    ...
    def test_pivot_string_as_func(self):
        ...
        expected = DataFrame(
            {("C", "one"): {"bar": 15, "foo": 13***REMOVED***, ("C", "two"): {"bar": 7, "foo": 20***REMOVED******REMOVED***,
            columns=mi,
        ).rename_axis("A")
        tm.assert_frame_equal(result, expected)
        ...
        expected = DataFrame(
            {
                ("mean", "C", "one"): {"bar": 5.0, "foo": 3.25***REMOVED***,
                ("mean", "C", "two"): {"bar": 7.0, "foo": 6.666666666666667***REMOVED***,
                ("sum", "C", "one"): {"bar": 15, "foo": 13***REMOVED***,
                ("sum", "C", "two"): {"bar": 7, "foo": 20***REMOVED***,
            ***REMOVED***,
            columns=mi,
        ).rename_axis("A")
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_string_func_vs_func(self, f, f_numpy, data):
        ...
        msg = f"using DataFrameGroupBy.[{ops***REMOVED***]"
        with tm.assert_produces_warning(FutureWarning, match=msg):
    ...
    def test_pivot_number_of_levels_larger_than_int32(self, monkeypatch):
        ...
        with monkeypatch.context() as m:
            ...
            df = DataFrame(
                {"ind1": np.arange(2**16), "ind2": np.arange(2**16), "count": 0***REMOVED***
***REMOVED***
    ...
    def test_pivot_table_aggfunc_scalar_dropna(self, dropna):
        # GH 22159
        df = DataFrame(
            {"A": ["one", "two", "one"], "x": [3, np.nan, 2], "y": [1, np.nan, np.nan]***REMOVED***
        )
    ...
    def test_pivot_table_no_column_raises(self):
        ...
        df = DataFrame({"X": [0, 0, 1, 1], "Y": [0, 1, 0, 1], "Z": [10, 20, 30, 40]***REMOVED***)
    ...
    def test_pivot_table_multiindex_columns_doctest_case(self):
        ...
        table = pivot_table(
            ...
            values=["D", "E"],
            index=["A", "C"],
            aggfunc={"D": "mean", "E": ["min", "max", "mean"]***REMOVED***,
        )
...
***REMOVED***,
    ...
    def test_pivot_table_nullable_margins(self):
        # GH#48681
        df = DataFrame(
            {"a": "A", "b": [1, 2], "sales": Series([10, 11], dtype="Int64")***REMOVED***
        )
...
***REMOVED***,
    ...
    def test_pivot_ea_dtype_dropna(self, dropna):
        # GH#47477
        df = DataFrame({"x": "a", "y": "b", "age": Series([20, 40], dtype="Int64")***REMOVED***)
        result = df.pivot_table(
...
class TestPivot:
    def test_pivot(self):
        ...
        ***REMOVED***
        ...
        expected = DataFrame(
            {
                "One": {"A": 1.0, "B": 2.0, "C": 3.0***REMOVED***,
                "Two": {"A": 1.0, "B": 2.0, "C": 3.0***REMOVED***,
            ***REMOVED***
        )
...
***REMOVED***,
    ...
    def test_pivot_columns_is_none(self):
        # GH#48293
        df = DataFrame({None: [1], "b": 2, "c": 3***REMOVED***)
        result = df.pivot(columns=None)
        expected = DataFrame({("b", 1): [2], ("c", 1): 3***REMOVED***)
        tm.assert_frame_equal(result, expected)

        result = df.pivot(columns=None, index="b")
        expected = DataFrame({("c", 1): 3***REMOVED***, index=Index([2], name="b"))
        tm.assert_frame_equal(result, expected)

        result = df.pivot(columns=None, index="b", values="c")
        expected = DataFrame({1: 3***REMOVED***, index=Index([2], name="b"))
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_index_is_none(self):
        ...
        expected = DataFrame({("c", 2): 3***REMOVED***, index=[1])
    ...
    def test_pivot_not_changing_index_name(self):
        # GH#52692
        df = DataFrame({"one": ["a"], "two": 0, "three": 1***REMOVED***)
    ...
    def test_pivot_table_handles_explicit_datetime_types(self):
        ...
        df = DataFrame(
            [
                {"a": "x", "date_str": "2023-01-01", "amount": 1***REMOVED***,
                {"a": "y", "date_str": "2023-01-02", "amount": 2***REMOVED***,
                {"a": "z", "date_str": "2023-01-03", "amount": 3***REMOVED***,
            ]
        )
