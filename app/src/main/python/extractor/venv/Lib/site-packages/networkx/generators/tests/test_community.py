...
def test_random_partition_graph():
    ...
    C = G.graph["partition"]
    assert C == [{0, 1, 2***REMOVED***, {3, 4, 5***REMOVED***, {6, 7, 8***REMOVED***]
    assert len(G) == 9
    ...
    C = G.graph["partition"]
    assert C == [{0***REMOVED***, {1, 2***REMOVED***, {3, 4, 5***REMOVED***, {6, 7, 8, 9***REMOVED***, {10, 11, 12, 13, 14***REMOVED***]
    assert len(G) == 15
...
def test_generator():
    ...
    assert len(G) == 250
    C = {frozenset(G.nodes[v]["community"]) for v in G***REMOVED***
