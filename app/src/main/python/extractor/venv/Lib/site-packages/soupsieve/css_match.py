...
RE_NUM = re.compile(r"^(?P<value>-?(?:[0-9]{1,***REMOVED***(\.[0-9]+)?|\.[0-9]+))$")
RE_TIME = re.compile(r'^(?P<hour>[0-9]{2***REMOVED***):(?P<minutes>[0-9]{2***REMOVED***)$')
RE_MONTH = re.compile(r'^(?P<year>[0-9]{4,***REMOVED***)-(?P<month>[0-9]{2***REMOVED***)$')
RE_WEEK = re.compile(r'^(?P<year>[0-9]{4,***REMOVED***)-W(?P<week>[0-9]{2***REMOVED***)$')
RE_DATE = re.compile(r'^(?P<year>[0-9]{4,***REMOVED***)-(?P<month>[0-9]{2***REMOVED***)-(?P<day>[0-9]{2***REMOVED***)$')
RE_DATETIME = re.compile(
    r'^(?P<year>[0-9]{4,***REMOVED***)-(?P<month>[0-9]{2***REMOVED***)-(?P<day>[0-9]{2***REMOVED***)T(?P<hour>[0-9]{2***REMOVED***):(?P<minutes>[0-9]{2***REMOVED***)$'
)
...
class _DocumentNav:
    ...
    def assert_valid_input(cls, tag: Any) -> None:
        ...
        if not cls.is_tag(tag):
            raise TypeError(f"Expected a BeautifulSoup 'Tag', but instead received type {type(tag)***REMOVED***")
...
class Inputs:
    ...
    def validate_week(year: int, week: int) -> bool:
        ...
        max_week = datetime.strptime(f"{12***REMOVED***-{31***REMOVED***-{year***REMOVED***", "%m-%d-%Y").isocalendar()[1]
        if max_week == 1:
            max_week = 53
...
class CSSMatch(_DocumentNav):
    ...
    def __init__(
        ...
        self.namespaces = {***REMOVED*** if namespaces is None else namespaces  # type: ct.Namespaces | dict[str, str]
        self.flags = flags
...
***REMOVED***
    ...
    def match_selectors(self, el: bs4.Tag, selectors: ct.SelectorList) -> bool:
        ...
        if is_html:
            ...
            self.namespaces = {'html': NS_XHTML***REMOVED***
...
class SoupSieve(ct.Immutable):
    ...
    def __repr__(self) -> str:  # pragma: no cover
        ...
        return (
            f"SoupSieve(pattern={self.pattern!r***REMOVED***, namespaces={self.namespaces!r***REMOVED***, "
            f"custom={self.custom!r***REMOVED***, flags={self.flags!r***REMOVED***)"
        )
