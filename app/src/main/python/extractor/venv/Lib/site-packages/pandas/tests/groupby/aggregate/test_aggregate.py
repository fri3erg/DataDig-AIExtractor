...
def test_groupby_agg_no_extra_calls():
    # GH#31760
    df = DataFrame({"key": ["a", "b", "c", "c"], "value": [1, 2, 3, 4]***REMOVED***)
...
def test_groupby_aggregation_multi_level_column():
    ...
    expected = DataFrame({0: [2.0, True, True, True], 1: [1, 0, 1, 1]***REMOVED***)
...
def test_aggregate_str_func(tsframe, groupbyfunc):
    ...
    result = grouped.agg({"A": "var", "B": "std", "C": "mean", "D": "sem"***REMOVED***)
    expected = DataFrame(
        {
            "A": grouped["A"].var(),
            "B": grouped["B"].std(),
...
def test_std_masked_dtype(any_numeric_ea_dtype):
    ...
    expected = DataFrame(
        {"b": [0.57735, 0]***REMOVED***, index=Index([1, 2], name="a"), dtype="Float64"
    )
    tm.assert_frame_equal(result, expected)
...
def test_agg_str_with_kwarg_axis_1_raises(df, reduction_func):
    ...
    warn_msg = f"DataFrameGroupBy.{reduction_func***REMOVED*** with axis=1 is deprecated"
    if reduction_func in ("idxmax", "idxmin"):
        ...
        error = ValueError
        msg = f"Operation {reduction_func***REMOVED*** does not support axis=1"
        warn = None
...
@pytest.mark.parametrize(
    ...
    [
        ("sum", [5, 7, 9], "int64", {***REMOVED***),
        ("std", [4.5**0.5] * 3, int, {"i": float, "j": float, "k": float***REMOVED***),
        ("var", [4.5] * 3, int, {"i": float, "j": float, "k": float***REMOVED***),
        ("sum", [5, 7, 9], "Int64", {"j": "int64"***REMOVED***),
        ("std", [4.5**0.5] * 3, "Int64", {"i": float, "j": float, "k": float***REMOVED***),
        ("var", [4.5] * 3, "Int64", {"i": "float64", "j": "float64", "k": "float64"***REMOVED***),
    ],
)
def test_multiindex_groupby_mixed_cols_axis1(func, expected, dtype, result_dtype_dict):
    ...
    ).astype({("a", "j"): dtype, ("b", "j"): dtype***REMOVED***)
...
@pytest.mark.parametrize(
    ...
    [
        ("sum", [[2, 4], [10, 12], [18, 20]], {10: "int64", 20: "int64"***REMOVED***),
        # std should ideally return Int64 / Float64 #43330
        ("std", [[2**0.5] * 2] * 3, "float64"),
        ("var", [[2] * 2] * 3, {10: "float64", 20: "float64"***REMOVED***),
    ],
)
...
def test_agg_multiple_functions_same_name():
    ...
    result = df.resample("3min").agg(
        {"A": [partial(np.quantile, q=0.9999), partial(np.quantile, q=0.1111)]***REMOVED***
    )
...
def test_agg_multiple_functions_same_name_with_ohlc_present():
    ...
    result = df.resample("3min").agg(
        {"A": ["ohlc", partial(np.quantile, q=0.9999), partial(np.quantile, q=0.1111)]***REMOVED***
    )
...
def test_more_flexible_frame_multi_function(df):
    ...
    exmean = grouped.agg({"C": "mean", "D": "mean"***REMOVED***)
    exstd = grouped.agg({"C": "std", "D": "std"***REMOVED***)

    expected = concat([exmean, exstd], keys=["mean", "std"], axis=1)
    expected = expected.swaplevel(0, 1, axis=1).sort_index(level=0, axis=1)

    d = {"C": ["mean", "std"], "D": ["mean", "std"]***REMOVED***
    result = grouped.aggregate(d)

    tm.assert_frame_equal(result, expected)

    # be careful
    result = grouped.aggregate({"C": "mean", "D": ["mean", "std"]***REMOVED***)
    expected = grouped.aggregate({"C": "mean", "D": ["mean", "std"]***REMOVED***)
    tm.assert_frame_equal(result, expected)
    ...
    with pytest.raises(SpecificationError, match=msg):
        d = {"C": "mean", "D": {"foo": "mean", "bar": "std"***REMOVED******REMOVED***
        grouped.aggregate(d)

    # But without renaming, these functions are OK
    d = {"C": ["mean"], "D": [numpymean, numpystd]***REMOVED***
    grouped.aggregate(d)
...
def test_multi_function_flexible_mix(df):
    ...
    # Expected
    d = {"C": {"foo": "mean", "bar": "std"***REMOVED***, "D": {"sum": "sum"***REMOVED******REMOVED***
    # this uses column selection & renaming
    msg = r"nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        grouped.aggregate(d)
    ...
    # Test 1
    d = {"C": {"foo": "mean", "bar": "std"***REMOVED***, "D": "sum"***REMOVED***
    # this uses column selection & renaming
    with pytest.raises(SpecificationError, match=msg):
        grouped.aggregate(d)
...
def test_groupby_agg_coercing_bools():
    # issue 14873
    dat = DataFrame({"a": [1, 1, 2, 2], "b": [0, 1, 2, 3], "c": [None, None, 1, 1]***REMOVED***)
    gp = dat.groupby("a")
...
def test_groupby_agg_dict_with_getitem():
    # issue 25471
    dat = DataFrame({"A": ["A", "A", "B", "B", "B"], "B": [1, 2, 1, 1, 2]***REMOVED***)
    result = dat.groupby("A")[["B"]].agg({"B": "sum"***REMOVED***)

    expected = DataFrame({"B": [3, 4]***REMOVED***, index=["A", "B"]).rename_axis("A", axis=0)
...
def test_groupby_agg_dict_dup_columns():
    ...
    gb = df.groupby("a")
    result = gb.agg({"b": "sum"***REMOVED***)
    expected = DataFrame({"b": [5, 4]***REMOVED***, index=Index([1, 2], name="a"))
    tm.assert_frame_equal(result, expected)
...
def test_bool_agg_dtype(op):
    ...
    df = DataFrame({"a": [1, 1], "b": [False, True]***REMOVED***)
    s = df.set_index("a")["b"]
...
def test_callable_result_dtype_frame(
    ...
    # GH 21240
    df = DataFrame({"a": [1], "b": [2], "c": [True]***REMOVED***)
    ...
    expected = DataFrame({"c": [df["c"].iloc[0]]***REMOVED***, index=expected_index).astype(
        result_dtype
    )
    if method == "apply":
...
def test_callable_result_dtype_series(keys, agg_index, input, dtype, method):
    # GH 21240
    df = DataFrame({"a": [1], "b": [2], "c": [input]***REMOVED***)
    op = getattr(df.groupby(keys)["c"], method)
...
def test_order_aggregate_multiple_funcs():
    # GH 25692
    df = DataFrame({"A": [1, 1, 2, 2], "B": [1, 2, 3, 4]***REMOVED***)
...
def test_ohlc_ea_dtypes(any_numeric_ea_dtype):
    # GH#37493
    df = DataFrame(
        {"a": [1, 1, 2, 3, 4, 4], "b": [22, 11, pd.NA, 10, 20, pd.NA]***REMOVED***,
...
def test_uint64_type_handling(dtype, how):
    # GH 26310
    df = DataFrame({"x": 6903052872240755750, "y": [1, 2]***REMOVED***)
    expected = df.groupby("y").agg({"x": how***REMOVED***)
    df.x = df.x.astype(dtype)
    result = df.groupby("y").agg({"x": how***REMOVED***)
...
def test_func_duplicates_raises():
    # GH28426
    msg = "Function names"
    df = DataFrame({"A": [0, 0, 1, 1], "B": [1, 2, 3, 4]***REMOVED***)
    with pytest.raises(SpecificationError, match=msg):
...
def test_agg_index_has_complex_internals(index):
    # GH 31223
    df = DataFrame({"group": [1, 1, 2], "value": [0, 1, 0]***REMOVED***, index=index)
    result = df.groupby("group").agg({"value": Series.nunique***REMOVED***)
    expected = DataFrame({"group": [1, 2], "value": [2, 1]***REMOVED***).set_index("group")
    tm.assert_frame_equal(result, expected)
...
def test_agg_split_block():
    ...
    expected = DataFrame(
        {"key2": ["one", "one"], "key3": ["six", "six"]***REMOVED***,
...
class TestNamedAggregationSeries:
    def test_series_named_agg(self):
        ...
        expected = DataFrame(
            {"a": [3, 7], "b": [1, 3]***REMOVED***, columns=["a", "b"], index=np.array([0, 1])
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_series_named_agg_duplicates_no_raises(self):
        ...
        expected = DataFrame({"a": [3, 3], "b": [3, 3]***REMOVED***, index=np.array([0, 1]))
        tm.assert_frame_equal(expected, grouped)

    def test_mangled(self):
        gr = Series([1, 2, 3]).groupby([0, 0, 1])
        result = gr.agg(a=lambda x: 0, b=lambda x: 1)
        expected = DataFrame({"a": [0, 0], "b": [1, 1]***REMOVED***, index=np.array([0, 1]))
        tm.assert_frame_equal(result, expected)
    ...
    def test_named_agg_nametuple(self, inp):
        ...
        msg = f"func is expected but received {type(inp).__name__***REMOVED***"
...
class TestNamedAggregationDataFrame:
    def test_agg_relabel(self):
        df = DataFrame(
            {"group": ["a", "a", "b", "b"], "A": [0, 1, 2, 3], "B": [5, 6, 7, 8]***REMOVED***
        )
        result = df.groupby("group").agg(a_max=("A", "max"), b_max=("B", "max"))
        expected = DataFrame(
            {"a_max": [1, 3], "b_max": [6, 8]***REMOVED***,
    ...
    def test_agg_relabel_non_identifier(self):
        ...
        result = df.groupby("group").agg(**{"my col": ("A", "max")***REMOVED***)
        expected = DataFrame({"my col": [1, 3]***REMOVED***, index=Index(["a", "b"], name="group"))
        tm.assert_frame_equal(result, expected)
    ...
    def test_duplicate_no_raises(self):
        ...
        expected = DataFrame({"a": [1, 3], "b": [1, 3]***REMOVED***, index=Index([0, 1], name="A"))
        ...
        test = DataFrame({"col1": ["a", "a", "b", "b", "b"], "col2": [1, 2, 3, 4, 5]***REMOVED***)

        grouped = test.groupby("col1").agg(
            quantile_50=("col2", quant50), quantile_70=("col2", quant70)
        )
        expected = DataFrame(
            {"quantile_50": [1.5, 4.0], "quantile_70": [1.7, 4.4]***REMOVED***,
    ...
    def test_agg_relabel_with_level(self):
        ...
        expected = DataFrame(
            {"aa": [0, 1], "bb": [0, 1], "cc": [1.5, 3.5]***REMOVED***, index=["A", "B"]
        )
        tm.assert_frame_equal(result, expected)

    def test_agg_relabel_other_raises(self):
        df = DataFrame({"A": [0, 0, 1], "B": [1, 2, 3]***REMOVED***)
        grouped = df.groupby("A")
        match = "Must provide"
    ...
    def test_missing_raises(self):
        df = DataFrame({"A": [0, 1], "B": [1, 2]***REMOVED***)
    ...
    def test_mangled(self):
        df = DataFrame({"A": [0, 1], "B": [1, 2], "C": [3, 4]***REMOVED***)
        result = df.groupby("A").agg(b=("B", lambda x: 0), c=("C", lambda x: 1))
        expected = DataFrame({"b": [0, 0], "c": [1, 1]***REMOVED***, index=Index([0, 1], name="A"))
        tm.assert_frame_equal(result, expected)
...
def test_agg_relabel_multiindex_column(
    ...
    expected = DataFrame({"a_max": [1, 3]***REMOVED***, index=idx)
    tm.assert_frame_equal(result, expected)
    ...
    expected = DataFrame(
        {"col_1": agg_result1, "col_2": agg_result2, "col_3": agg_result3***REMOVED***, index=idx
    )
    tm.assert_frame_equal(result, expected)
...
def test_agg_relabel_multiindex_duplicates():
    ...
    idx = Index(["a", "b"], name=("x", "group"))
    expected = DataFrame({"a": [0, 2], "b": [0, 2]***REMOVED***, index=idx)
    tm.assert_frame_equal(result, expected)


@pytest.mark.parametrize("kwargs", [{"c": ["min"]***REMOVED***, {"b": [], "c": ["min"]***REMOVED***])
def test_groupby_aggregate_empty_key(kwargs):
    # GH: 32580
    df = DataFrame({"a": [1, 1, 2], "b": [1, 2, 3], "c": [1, 2, 4]***REMOVED***)
...
def test_groupby_aggregate_empty_key_empty_return():
    ...
    df = DataFrame({"a": [1, 1, 2], "b": [1, 2, 3], "c": [1, 2, 4]***REMOVED***)
    result = df.groupby("a").agg({"b": []***REMOVED***)
...
def test_grouby_agg_loses_results_with_as_index_false_relabel():
    ...
    df = DataFrame(
        {"key": ["x", "y", "z", "x", "y", "z"], "val": [1.0, 0.8, 2.0, 3.0, 3.6, 0.75]***REMOVED***
    )

    grouped = df.groupby("key", as_index=False)
    result = grouped.agg(min_val=pd.NamedAgg(column="val", aggfunc="min"))
    expected = DataFrame({"key": ["x", "y", "z"], "min_val": [1.0, 0.8, 0.75]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_grouby_agg_loses_results_with_as_index_false_relabel_multiindex():
    ...
    result = grouped.agg(min_val=pd.NamedAgg(column="val", aggfunc="min"))
    expected = DataFrame(
        {"key": ["x", "x", "y"], "key1": ["a", "c", "b"], "min_val": [1.0, 0.75, 0.8]***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
...
def test_multiindex_custom_func(func):
    ...
    expected_dict = {
        (1, 3): {0: 1.0, 1: 5.0***REMOVED***,
        (1, 4): {0: 4.0, 1: 7.0***REMOVED***,
        (2, 3): {0: 2.0, 1: 1.0***REMOVED***,
    ***REMOVED***
...
def test_aggregate_udf_na_extension_type():
    ...
    df = DataFrame({"A": pd.array([1, 2, 3])***REMOVED***)
    result = df.groupby([1, 1, 2]).agg(aggfunc)
    expected = DataFrame({"A": pd.array([1, pd.NA], dtype="Int64")***REMOVED***, index=[1, 2])
    tm.assert_frame_equal(result, expected)
...
class TestLambdaMangling:
    def test_basic(self):
        df = DataFrame({"A": [0, 0, 1, 1], "B": [1, 2, 3, 4]***REMOVED***)
        result = df.groupby("A").agg({"B": [lambda x: 0, lambda x: 1]***REMOVED***)

        expected = DataFrame(
            {("B", "<lambda_0>"): [0, 0], ("B", "<lambda_1>"): [1, 1]***REMOVED***,
    ...
    def test_mangle_series_groupby(self):
        ...
        exp_data = {"<lambda_0>": [0, 0], "<lambda_1>": [1, 1]***REMOVED***
    ...
    def test_with_kwargs(self):
        ...
        expected = DataFrame({"<lambda_0>": [4], "<lambda_1>": [6]***REMOVED***)
        tm.assert_frame_equal(result, expected)

        result = Series([1, 2]).groupby([0, 0]).agg([f1, f2], 0, b=10)
        expected = DataFrame({"<lambda_0>": [13], "<lambda_1>": [30]***REMOVED***)
        tm.assert_frame_equal(result, expected)
...
def test_groupby_get_by_index():
    # GH 33439
    df = DataFrame({"A": ["S", "W", "W"], "B": [1.0, 1.0, 2.0]***REMOVED***)
    res = df.groupby("A").agg({"B": lambda x: x.get(x.index[-1])***REMOVED***)
    expected = DataFrame({"A": ["S", "W"], "B": [1.0, 2.0]***REMOVED***).set_index("A")
...
@pytest.mark.parametrize(
    ...
    [
        ({"nr": "min", "cat_ord": "min"***REMOVED***, {"nr": [1, 5], "cat_ord": ["a", "c"]***REMOVED***),
        ({"cat_ord": "min"***REMOVED***, {"cat_ord": ["a", "c"]***REMOVED***),
        ({"nr": "min"***REMOVED***, {"nr": [1, 5]***REMOVED***),
    ],
)
def test_groupby_single_agg_cat_cols(grp_col_dict, exp_data):
    ...
    input_df = input_df.astype({"cat": "category", "cat_ord": "category"***REMOVED***)
...
@pytest.mark.parametrize(
    "grp_col_dict, exp_data",
    [
        ({"nr": ["min", "max"], "cat_ord": "min"***REMOVED***, [(1, 4, "a"), (5, 8, "c")]),
        ({"nr": "min", "cat_ord": ["min", "max"]***REMOVED***, [(1, "a", "b"), (5, "c", "d")]),
        ({"cat_ord": ["min", "max"]***REMOVED***, [("a", "b"), ("c", "d")]),
    ],
)
...
def test_nonagg_agg():
    ...
    df = DataFrame({"a": [1, 1, 2, 2], "b": [1, 2, 2, 1]***REMOVED***)
    g = df.groupby("a")
...
def test_groupby_index_object_dtype():
    # GH 40014
    df = DataFrame({"c0": ["x", "x", "x"], "c1": ["x", "x", "y"], "p": [0, 1, 2]***REMOVED***)
...
def test_groupby_agg_precision(any_real_numeric_dtype):
    ...
    expected = DataFrame(
        {"key3": pd.array([max_value], dtype=any_real_numeric_dtype)***REMOVED***, index=index
    )
...
def test_groupby_aggregate_directory(reduction_func):
    ...
    result_reduced_frame = obj.groupby(0).agg({1: reduction_func***REMOVED***)
...
@pytest.mark.parametrize(
    "func", [["min"], ["mean", "max"], {"b": "sum"***REMOVED***, {"b": "prod", "c": "median"***REMOVED***]
)
def test_multi_axis_1_raises(func):
    # GH#46995
    df = DataFrame({"a": [1, 1, 2], "b": [3, 4, 5], "c": [6, 7, 8]***REMOVED***)
    msg = "DataFrame.groupby with axis=1 is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
...
@pytest.mark.parametrize(
    "test, constant",
    [
        ([[20, "A"], [20, "B"], [10, "C"]], {0: [10, 20], 1: ["C", ["A", "B"]]***REMOVED***),
        ([[20, "A"], [20, "B"], [30, "C"]], {0: [20, 30], 1: [["A", "B"], "C"]***REMOVED***),
        ([["a", 1], ["a", 1], ["b", 2], ["b", 3]], {0: ["a", "b"], 1: [1, [2, 3]]***REMOVED***),
        pytest.param(
            [["a", 1], ["a", 2], ["b", 3], ["b", 3]],
            {0: ["a", "b"], 1: [[1, 2], 3]***REMOVED***,
...
def test_dataframe_groupy_agg_list_like_func_with_args():
    # GH#50624
    df = DataFrame({"x": [1, 2, 3], "y": ["a", "b", "c"]***REMOVED***)
    gb = df.groupby("y")
...
def test_agg_groupings_selection():
    ...
    df = DataFrame({"a": [1, 1, 2], "b": [3, 3, 4], "c": [5, 6, 7]***REMOVED***)
    gb = df.groupby(["a", "b"])
    selected_gb = gb[["b", "c"]]
    ...
    expected = DataFrame({"b": [6, 4], "c": [11, 7]***REMOVED***, index=index)
    tm.assert_frame_equal(result, expected)


def test_agg_multiple_with_as_index_false_subset_to_a_single_column():
    # GH#50724
    df = DataFrame({"a": [1, 1, 2], "b": [3, 4, 5]***REMOVED***)
    gb = df.groupby("a", as_index=False)["b"]
    result = gb.agg(["sum", "mean"])
    expected = DataFrame({"a": [1, 2], "sum": [7, 5], "mean": [3.5, 5.0]***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_agg_with_as_index_false_with_list():
    # GH#52849
    df = DataFrame({"a1": [0, 0, 1], "a2": [2, 3, 3], "b": [4, 5, 6]***REMOVED***)
...
def test_groupby_aggregation_empty_group():
    ...
    df = DataFrame(
        {"A": pd.Categorical(["a", "a"], categories=["a", "b", "c"]), "B": [1, 1]***REMOVED***
    )
