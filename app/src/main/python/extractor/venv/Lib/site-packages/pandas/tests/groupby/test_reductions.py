...
def test_groupby_bool_aggs(skipna, agg_func, vals):
    df = DataFrame({"key": ["a"] * 3 + ["b"] * 3, "val": vals * 2***REMOVED***)
...
def test_masked_mixed_types(dtype1, dtype2, exp_col1, exp_col2):
    ...
    data = [1.0, np.nan]
    df = DataFrame(
        {"col1": pd.array(data, dtype=dtype1), "col2": pd.array(data, dtype=dtype2)***REMOVED***
    )
    result = df.groupby([1, 1]).agg("all", skipna=False)

    expected = DataFrame({"col1": exp_col1, "col2": exp_col2***REMOVED***, index=np.array([1]))
    tm.assert_frame_equal(result, expected)
...
def test_empty(frame_or_series, bool_agg_func):
    # GH 45231
    kwargs = {"columns": ["a"]***REMOVED*** if frame_or_series is DataFrame else {"name": "a"***REMOVED***
...
@pytest.mark.parametrize(
    "func, values",
    [
        ("idxmin", {"c_int": [0, 2], "c_float": [1, 3], "c_date": [1, 2]***REMOVED***),
        ("idxmax", {"c_int": [1, 3], "c_float": [0, 2], "c_date": [0, 3]***REMOVED***),
    ],
)
...
def test_groupby_non_arithmetic_agg_int_like_precision(method, data):
    # GH#6620, GH#9311
    df = DataFrame({"a": [1, 1], "b": data***REMOVED***)
    ...
    expected = DataFrame({"b": [expected_value]***REMOVED***, index=pd.Index([1], name="a"))
...
def test_mean_on_timedelta():
    # GH 17382
    df = DataFrame({"time": pd.to_timedelta(range(10)), "cat": ["A", "B"] * 5***REMOVED***)
...
def test_max_min_non_numeric():
    # #2700
    aa = DataFrame({"nn": [11, 11, 22, 22], "ii": [1, 2, 3, 4], "ss": 4 * ["mama"]***REMOVED***)
...
def test_max_min_object_multiple_columns(using_array_manager):
    ...
    expected = DataFrame({"C": ["b", "d", "e"]***REMOVED***, index=ei)
    tm.assert_frame_equal(result, expected)

    result = gb[["C"]].min()
    # "min" is valid for column "C" but not for "B"
    ei = pd.Index([1, 2, 3], name="A")
    expected = DataFrame({"C": ["a", "c", "e"]***REMOVED***, index=ei)
    tm.assert_frame_equal(result, expected)
...
def test_min_date_with_nans():
    ...
    df = DataFrame({"a": [np.nan, "1", np.nan], "b": [0, 1, 1], "c": dates***REMOVED***)
...
def test_max_inat():
    ...
    expected = Series({1: 1***REMOVED***, dtype=np.int64)
    tm.assert_series_equal(result, expected, check_exact=True)

    result = gb.min(min_count=2)
    expected = Series({1: iNaT***REMOVED***, dtype=np.int64)
    tm.assert_series_equal(result, expected, check_exact=True)

    # not enough entries -> gets masked to NaN
    result = gb.min(min_count=3)
    expected = Series({1: np.nan***REMOVED***)
    tm.assert_series_equal(result, expected, check_exact=True)
...
def test_max_inat_not_all_na():
    ...
    expected = Series({1: np.nan, 2: np.nan, 3: iNaT + 1***REMOVED***)
...
def test_groupby_aggregate_period_column(func):
    ...
    df = DataFrame({"a": groups, "b": periods***REMOVED***)
...
def test_groupby_aggregate_period_frame(func):
    ...
    idx = pd.Index([1, 2], name="a")
    expected = DataFrame({"b": periods***REMOVED***, index=idx)
...
def test_aggregate_numeric_object_dtype():
    ...
    df = DataFrame(
        {"key": ["A", "A", "B", "B"], "col1": list("abcd"), "col2": [np.nan] * 4***REMOVED***,
    ).astype(object)
    result = df.groupby("key").min()
    expected = (
        DataFrame(
            {"key": ["A", "B"], "col1": ["a", "c"], "col2": [np.nan, np.nan]***REMOVED***,
        )
        .set_index("key")
        .astype(object)
    )
    ...
    df = DataFrame(
        {"key": ["A", "A", "B", "B"], "col1": list("abcd"), "col2": range(4)***REMOVED***,
    ).astype(object)
    result = df.groupby("key").min()
    expected = (
        DataFrame({"key": ["A", "B"], "col1": ["a", "c"], "col2": [0, 2]***REMOVED***)
        .set_index("key")
        .astype(object)
    )
    tm.assert_frame_equal(result, expected)
...
def test_aggregate_categorical_lost_index(func: str):
    ...
    df = DataFrame({"A": [1997], "B": ds***REMOVED***)
    result = df.groupby("A").agg({"B": func***REMOVED***)
    expected = DataFrame({"B": ["b"]***REMOVED***, index=pd.Index([1997], name="A"))
...
def test_groupby_min_max_nullable(dtype):
    ...
    df = DataFrame({"id": [2, 2], "ts": [ts, ts + 1]***REMOVED***)
    ...
    expected2 = DataFrame({"ts": [pd.NA]***REMOVED***, index=expected.index, dtype=dtype)
    ...
    # Case with NA values
    df2 = DataFrame({"id": [2, 2, 2], "ts": [ts, pd.NA, ts + 1]***REMOVED***)
...
def test_min_max_nullable_uint64_empty_group():
    ...
    df = DataFrame({"A": cat, "B": pd.array(np.arange(10, dtype=np.uint64))***REMOVED***)
    gb = df.groupby("A", observed=False)

    res = gb.min()

    idx = pd.CategoricalIndex([0, 1], dtype=cat.dtype, name="A")
    expected = DataFrame({"B": pd.array([0, pd.NA], dtype="UInt64")***REMOVED***, index=idx)
...
def test_min_empty_string_dtype(func):
    ...
    df = DataFrame({"a": ["a"], "b": "a", "c": "a"***REMOVED***, dtype=dtype).iloc[:0]
    result = getattr(df.groupby("a"), func)()
    expected = DataFrame(
...
def test_series_groupby_nunique(sort, dropna, as_index, with_nan, keys):
    ...
    if with_nan:
        df = df.astype({"julie": float***REMOVED***)  # Explicit cast to avoid implicit cast below
...
def test_nunique():
    df = DataFrame({"A": list("abbacc"), "B": list("abxacc"), "C": list("abbacx")***REMOVED***)

    expected = DataFrame({"A": list("abc"), "B": [1, 2, 1], "C": [1, 1, 2]***REMOVED***)
    ...
    # with na
    result = df.replace({"x": None***REMOVED***).groupby("A").nunique(dropna=False)
    tm.assert_frame_equal(result, expected)

    # dropna
    expected = DataFrame({"B": [1] * 3, "C": [1] * 3***REMOVED***, index=list("abc"))
    expected.index.name = "A"
    result = df.replace({"x": None***REMOVED***).groupby("A").nunique()
    tm.assert_frame_equal(result, expected)
...
def test_nunique_with_NaT(key, data, dropna, expected):
    # GH 27951
    df = DataFrame({"key": key, "data": data***REMOVED***)
...
def test_groupby_sum_mincount_boolean(min_count):
    ...
    df = DataFrame({"A": [1, 1, 2, 2, 3, 3, 1], "B": dfg***REMOVED***)
    result = df.groupby("A").sum(min_count=min_count)
    if min_count == 0:
        expected = DataFrame(
            {"B": pd.array([3, 0, 0], dtype="Int64")***REMOVED***,
            index=pd.Index([1, 2, 3], name="A"),
        )
        tm.assert_frame_equal(result, expected)
    else:
        expected = DataFrame(
            {"B": pd.array([pd.NA] * 3, dtype="Int64")***REMOVED***,
            index=pd.Index([1, 2, 3], name="A"),
        )
        tm.assert_frame_equal(result, expected)
...
def test_groupby_sum_below_mincount_nullable_integer():
    ...
    df = DataFrame({"a": [0, 1, 2], "b": [0, 1, 2], "c": [0, 1, 2]***REMOVED***, dtype="Int64")
    grouped = df.groupby("a")
    ...
    expected = DataFrame({"b": [pd.NA] * 3, "c": [pd.NA] * 3***REMOVED***, dtype="Int64", index=idx)
    tm.assert_frame_equal(result, expected)
...
def test_groupby_sum_timedelta_with_nat():
    ...
    res = gb.sum()
    expected = DataFrame({"b": [td3, td3]***REMOVED***, index=pd.Index([1, 2], name="a"))
    tm.assert_frame_equal(res, expected)
...
@pytest.mark.parametrize(
    "method,data",
    [
        ("first", {"df": [{"a": 1, "b": 1***REMOVED***, {"a": 2, "b": 3***REMOVED***]***REMOVED***),
        ("last", {"df": [{"a": 1, "b": 2***REMOVED***, {"a": 2, "b": 4***REMOVED***]***REMOVED***),
        ("min", {"df": [{"a": 1, "b": 1***REMOVED***, {"a": 2, "b": 3***REMOVED***]***REMOVED***),
        ("max", {"df": [{"a": 1, "b": 2***REMOVED***, {"a": 2, "b": 4***REMOVED***]***REMOVED***),
        ("count", {"df": [{"a": 1, "b": 2***REMOVED***, {"a": 2, "b": 2***REMOVED***], "out_type": "int64"***REMOVED***),
    ],
)
def test_groupby_non_arithmetic_agg_types(dtype, method, data):
    # GH9311, GH6620
    df = DataFrame(
        [{"a": 1, "b": 1***REMOVED***, {"a": 1, "b": 2***REMOVED***, {"a": 2, "b": 3***REMOVED***, {"a": 2, "b": 4***REMOVED***]
    )
...
def test_ops_general(op, targop):
    ...
    msg = f"using DataFrameGroupBy.{op***REMOVED***"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ***REMOVED***,
        {"a": [1, 1, 2, 2, 3, 3], "b": [1, 2, 1, 2, 1, 2]***REMOVED***,
    ],
)
...
def test_apply_to_nullable_integer_returns_float(values, function):
    ...
    expected = DataFrame({"b": arr***REMOVED***, index=idx).astype("Float64")
...
def test_groupby_prod_with_int64_dtype():
    ...
    expected = DataFrame({"A": [1], "B": [180970905912331920]***REMOVED***, dtype="int64")
    tm.assert_frame_equal(result, expected)
