...
WHITESPACE: Final = {token.DEDENT, token.INDENT, token.NEWLINE***REMOVED***
STATEMENT: Final = {
    syms.if_stmt,
    syms.while_stmt,
    syms.for_stmt,
...
LOGIC_OPERATORS: Final = {"and", "or"***REMOVED***
COMPARATORS: Final = {
    token.LESS,
    token.GREATER,
    token.EQEQUAL,
...
MATH_OPERATORS: Final = {
    ...
    token.DOUBLESTAR,
***REMOVED***
STARS: Final = {token.STAR, token.DOUBLESTAR***REMOVED***
VARARGS_SPECIALS: Final = STARS | {token.SLASH***REMOVED***
...
TEST_DESCENDANTS: Final = {
    ...
    syms.namedexpr_test,
***REMOVED***
TYPED_NAMES: Final = {syms.tname, syms.tname_star***REMOVED***
ASSIGNMENTS: Final = {
    "=",
    "+=",
    "-=",
...
IMPLICIT_TUPLE: Final = {syms.testlist, syms.testlist_star_expr, syms.exprlist***REMOVED***
BRACKET: Final = {
...
ALWAYS_NO_SPACE: Final = CLOSING_BRACKETS | {token.COMMA, STANDALONE_COMMENT***REMOVED***
...
class Visitor(Generic[T]):
    ...
    def visit(self, node: LN) -> Iterator[T]:
        ...
        visitf = getattr(self, f"visit_{name***REMOVED***", None)
        if visitf:
...
def whitespace(leaf: Leaf, *, complex_subscript: bool, mode: Mode) -> str:  # noqa: C901
    ...
    assert p is not None, f"INTERNAL ERROR: hand-made leaf without parent: {leaf!r***REMOVED***"
    ...
    ***REMOVED***:
    ...
    if not prev:
        ...
        if prevp.type == token.EQUAL:
            ...
            if prevp.parent and prevp.parent.type in {syms.subscript, syms.sliceop***REMOVED***:
    ...
    if p.type in {syms.parameters, syms.arglist***REMOVED***:
        ...
        elif prev.type in {token.EQUAL***REMOVED*** | VARARGS_SPECIALS:
            return NO
    ...
    elif p.type in {syms.subscript, syms.sliceop***REMOVED***:
        # indexing
        if not prev:
    ...
    elif p.type in {syms.factor, syms.star_expr***REMOVED***:
        # unary ops
        if not prev:
            prevp = preceding_leaf(p)
            if not prevp or prevp.type in OPENING_BRACKETS:
        ...
        elif t in {token.NAME, token.NUMBER, token.STRING***REMOVED***:
            return NO
...
def is_parent_function_or_class(node: Node) -> bool:
    assert node.type in {syms.suite, syms.simple_stmt***REMOVED***
    assert node.parent is not None
    # Note this works for suites / simple_stmts in async def as well
    return node.parent.type in {syms.funcdef, syms.classdef***REMOVED***


def is_function_or_class(node: Node) -> bool:
    return node.type in {syms.funcdef, syms.classdef, syms.async_funcdef***REMOVED***
...
def is_async_stmt_or_funcdef(leaf: Leaf) -> bool:
    ...
    return bool(
        leaf.type == token.ASYNC
        and leaf.parent
        and leaf.parent.type in {syms.async_stmt, syms.async_funcdef***REMOVED***
    )
...
def is_type_comment(leaf: Leaf) -> bool:
    ...
    t = leaf.type
    v = leaf.value
    return t in {token.COMMENT, STANDALONE_COMMENT***REMOVED*** and v.startswith("# type:")
...
def is_type_ignore_comment(leaf: Leaf) -> bool:
    ...
    t = leaf.type
    v = leaf.value
    return t in {token.COMMENT, STANDALONE_COMMENT***REMOVED*** and is_type_ignore_comment_string(v)
