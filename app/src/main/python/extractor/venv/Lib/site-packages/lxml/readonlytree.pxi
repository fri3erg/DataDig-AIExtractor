...
cdef class _ReadOnlyProxy:
    ...
    cdef int _raise_unsupported_type(self) except -1:
        raise TypeError(f"Unsupported node type: {self._c_node.type***REMOVED***")
...
cdef _ReadOnlyProxy _newReadOnlyProxy(
    ...
    if c_node.type == tree.XML_ELEMENT_NODE:
        ...
        raise TypeError(f"Unsupported element type: {c_node.type***REMOVED***")
    el._c_node = c_node
...
cdef class _OpaqueDocumentWrapper(_OpaqueNodeWrapper):
    ...
    cpdef append(self, other_element):
        ...
        if c_node.type == tree.XML_ELEMENT_NODE:
            ...
            raise TypeError, f"unsupported element type for top-level node: {c_node.type***REMOVED***"
    ...
    property name:
        ...
        def __set__(self, value):
            ...
            assert '&' not in value and ';' not in value, \
                f"Invalid entity name '{value***REMOVED***'"
            c_text = _xcstr(value)
            tree.xmlNodeSetName(self._c_node, c_text)
...
cdef xmlNode* _roNodeOf(element) except NULL:
    ...
    if isinstance(element, _Element):
        ...
        raise TypeError, f"invalid argument type {type(element)***REMOVED***"
