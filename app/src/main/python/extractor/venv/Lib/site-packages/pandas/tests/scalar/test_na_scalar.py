...
def test_format():
    ...
    assert f"{NA***REMOVED***" == "<NA>"
    assert f"{NA:>10***REMOVED***" == "      <NA>"
    assert f"{NA:xxx***REMOVED***" == "<NA>"
...
def test_arithmetic_ops(all_arithmetic_functions, other):
    ...
    if op.__name__ in ("pow", "rpow", "rmod") and isinstance(other, (str, bytes)):
        pytest.skip(reason=f"{op.__name__***REMOVED*** with NA and {other***REMOVED*** not defined.")
...
def test_integer_hash_collision_dict():
    # GH 30013
    result = {NA: "foo", hash(NA): "bar"***REMOVED***
