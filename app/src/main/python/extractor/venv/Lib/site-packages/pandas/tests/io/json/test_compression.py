...
def test_with_s3_url(compression, s3_public_bucket, s3so):
    ...
    df = pd.read_json(StringIO('{"a": [1, 2, 3], "b": [4, 5, 6]***REMOVED***'))
    ...
    roundtripped_df = pd.read_json(
        f"s3://{s3_public_bucket.name***REMOVED***/test-1",
...
def test_chunksize_with_compression(compression):
    with tm.ensure_clean() as path:
        df = pd.read_json(StringIO('{"a": ["foo", "bar", "baz"], "b": [4, 5, 6]***REMOVED***'))
        df.to_json(path, orient="records", lines=True, compression=compression)
...
def test_to_json_compression_mode(compression):
    ...
    expected = pd.DataFrame({"A": [1]***REMOVED***)
