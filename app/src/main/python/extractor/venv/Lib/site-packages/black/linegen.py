...
***REMOVED***:
    ...
    ***REMOVED***:
    ...
    def __post_init__(self) -> None:
        ...
        v = self.visit_stmt
        Ø: Set[str] = set()
        self.visit_assert_stmt = partial(v, keywords={"assert"***REMOVED***, parens={"assert", ","***REMOVED***)
        self.visit_if_stmt = partial(
            v, keywords={"if", "else", "elif"***REMOVED***, parens={"if", "elif"***REMOVED***
        )
        self.visit_while_stmt = partial(v, keywords={"while", "else"***REMOVED***, parens={"while"***REMOVED***)
        self.visit_for_stmt = partial(v, keywords={"for", "else"***REMOVED***, parens={"for", "in"***REMOVED***)
        self.visit_try_stmt = partial(
            v, keywords={"try", "except", "else", "finally"***REMOVED***, parens=Ø
        )
        self.visit_except_clause = partial(v, keywords={"except"***REMOVED***, parens={"except"***REMOVED***)
        self.visit_with_stmt = partial(v, keywords={"with"***REMOVED***, parens={"with"***REMOVED***)
        self.visit_classdef = partial(v, keywords={"class"***REMOVED***, parens=Ø)

        self.visit_expr_stmt = partial(v, keywords=Ø, parens=ASSIGNMENTS)
        self.visit_return_stmt = partial(v, keywords={"return"***REMOVED***, parens={"return"***REMOVED***)
        self.visit_import_from = partial(v, keywords=Ø, parens={"import"***REMOVED***)
        self.visit_del_stmt = partial(v, keywords=Ø, parens={"del"***REMOVED***)
...
def transform_line(
    ...
    if (
        ...
        rhs = type("rhs", (), {"__call__": _rhs***REMOVED***)()
...
def _maybe_split_omitting_optional_parens(
    ...
    if (
        ...
        omit = {id(rhs.closing_bracket), *omit***REMOVED***
        try:
...
def bracket_split_succeeded_or_raise(head: Line, body: Line, tail: Line) -> None:
    ...
    if not body:
        if tail_len == 0:
            ...
            raise CannotSplit(
                f"Splitting brackets on an empty body to save {tail_len***REMOVED*** characters is"
                " not worth it"
***REMOVED***
...
def delimiter_split(
    ...
    try:
        delimiter_priority = bt.max_delimiter_priority(exclude={id(last_leaf)***REMOVED***)
    except ValueError:
...
***REMOVED***
    ...
    for leaf_idx, leaf in enumerate(line.leaves):
        ...
        if leaf.bracket_depth == lowest_depth:
            if is_vararg(leaf, within={syms.typedargslist***REMOVED***):
                trailing_comma_safe = (
                    trailing_comma_safe and Feature.TRAILING_COMMA_IN_DEF in features
    ***REMOVED***
            elif is_vararg(leaf, within={syms.arglist, syms.argument***REMOVED***):
                trailing_comma_safe = (
...
***REMOVED***
        ...
        if isinstance(child, Node) and child.type == syms.case_block:
            normalize_invisible_parens(
                child, parens_after={"case"***REMOVED***, mode=mode, features=features
***REMOVED***
...
def should_split_line(line: Line, opening_bracket: Leaf) -> bool:
    ...
    return max_priority == COMMA_PRIORITY and (
        ...
        or opening_bracket.parent.type in {syms.atom, syms.import_from***REMOVED***
    )
...
def run_transformer(
    ...
    *,
    ...
    features_fop = features_set | {Feature.FORCE_OPTIONAL_PARENTHESES***REMOVED***
