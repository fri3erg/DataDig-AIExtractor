...
***REMOVED***.T,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ["rolling", {"window": 3, "min_periods": 0***REMOVED***],
            ["expanding", {***REMOVED***],
        ],
    )
...
class TestEWM:
    ...
    def test_invalid_engine(self, grouper, method):
        df = DataFrame({"A": ["a", "b", "a", "b"], "B": range(4)***REMOVED***)
    ...
    def test_invalid_engine_kwargs(self, grouper, method):
        ...
        with pytest.raises(ValueError, match="cython engine does not"):
            getattr(grouper(df).ewm(com=1.0), method)(
                engine="cython", engine_kwargs={"nopython": True***REMOVED***
***REMOVED***
    ...
    def test_cython_vs_numba(
        ...
        df = DataFrame({"B": range(4)***REMOVED***)
        if grouper == "None":
            grouper = lambda x: x
        else:
            df["A"] = ["a", "b", "a", "b"]
    ...
    def test_cython_vs_numba_times(self, grouper, nogil, parallel, nopython, ignore_na):
        ...
        df = DataFrame({"B": [0, 0, 1, 1, 2, 2]***REMOVED***)
        if grouper == "None":
            grouper = lambda x: x
        else:
            grouper = lambda x: x.groupby("A")
...
def test_invalid_kwargs_nopython():
    with pytest.raises(NumbaUtilError, match="numba does not support kwargs with"):
        Series(range(1)).rolling(1).apply(
            lambda x: x, kwargs={"a": 1***REMOVED***, engine="numba", raw=True
        )
...
***REMOVED***
    ...
    def test_table_method_rolling_methods(
        ...
        if method in ("var", "std"):
            with pytest.raises(NotImplementedError, match=f"{method***REMOVED*** not supported"):
...
def test_npfunc_no_warnings():
    df = DataFrame({"col1": [1, 2, 3, 4, 5]***REMOVED***)
