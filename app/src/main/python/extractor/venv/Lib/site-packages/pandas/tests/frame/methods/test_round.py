...
class TestDataFrameRound:
    def test_round(self):
        ...
        df = DataFrame({"col1": [1.123, 2.123, 3.123], "col2": [1.234, 2.234, 3.234]***REMOVED***)

        # Default round to integer (i.e. decimals=0)
        expected_rounded = DataFrame({"col1": [1.0, 2.0, 3.0], "col2": [1.0, 2.0, 3.0]***REMOVED***)
        tm.assert_frame_equal(df.round(), expected_rounded)

        # Round with an integer
        decimals = 2
        expected_rounded = DataFrame(
            {"col1": [1.12, 2.12, 3.12], "col2": [1.23, 2.23, 3.23]***REMOVED***
        )
        ...
        expected_rounded = DataFrame(
            {"col1": [1.1, 2.1, 3.1], "col2": [1.23, 2.23, 3.23]***REMOVED***
        )
        round_dict = {"col1": 1, "col2": 2***REMOVED***
        tm.assert_frame_equal(df.round(round_dict), expected_rounded)

        # Incomplete dict
        expected_partially_rounded = DataFrame(
            {"col1": [1.123, 2.123, 3.123], "col2": [1.2, 2.2, 3.2]***REMOVED***
        )
        partial_round_dict = {"col2": 1***REMOVED***
        tm.assert_frame_equal(df.round(partial_round_dict), expected_partially_rounded)

        # Dict with unknown elements
        wrong_round_dict = {"col3": 2, "col2": 1***REMOVED***
        tm.assert_frame_equal(df.round(wrong_round_dict), expected_partially_rounded)

        # float input to `decimals`
        non_int_round_dict = {"col1": 1, "col2": 0.5***REMOVED***
        msg = "Values in decimals must be integers"
        with pytest.raises(TypeError, match=msg):
            df.round(non_int_round_dict)

        # String input
        non_int_round_dict = {"col1": 1, "col2": "foo"***REMOVED***
        with pytest.raises(TypeError, match=msg):
            df.round(non_int_round_dict)
        ...
        # List input
        non_int_round_dict = {"col1": 1, "col2": [1, 2]***REMOVED***
        with pytest.raises(TypeError, match=msg):
            df.round(non_int_round_dict)
        ...
        # Negative numbers
        negative_round_dict = {"col1": -1, "col2": -2***REMOVED***
        big_df = df * 100
        expected_neg_rounded = DataFrame(
            {"col1": [110.0, 210, 310], "col2": [100.0, 200, 300]***REMOVED***
        )
        tm.assert_frame_equal(big_df.round(negative_round_dict), expected_neg_rounded)

        # nan in Series round
        nan_round_Series = Series({"col1": np.nan, "col2": 1***REMOVED***)
    ...
    def test_round_mixed_type(self):
        ...
        tm.assert_frame_equal(df.round({"col1": 1***REMOVED***), df)
        tm.assert_frame_equal(df.round({"col1": 0***REMOVED***), round_0)
        tm.assert_frame_equal(df.round({"col1": 0, "col2": 1***REMOVED***), round_0)
        tm.assert_frame_equal(df.round({"col3": 1***REMOVED***), df)
