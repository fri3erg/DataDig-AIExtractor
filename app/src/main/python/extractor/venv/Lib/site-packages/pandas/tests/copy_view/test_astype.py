...
def test_astype_single_dtype(using_copy_on_write):
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": 1.5***REMOVED***)
    df_orig = df.copy()
...
def test_astype_string_and_object(using_copy_on_write, dtype, new_dtype):
    df = DataFrame({"a": ["a", "b", "c"]***REMOVED***, dtype=dtype)
    df_orig = df.copy()
    df2 = df.astype(new_dtype)
...
def test_astype_dict_dtypes(using_copy_on_write):
    df = DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": Series([1.5, 1.5, 1.5], dtype="float64")***REMOVED***
    )
    df_orig = df.copy()
    df2 = df.astype({"a": "float64", "c": "float64"***REMOVED***)
...
def test_astype_different_datetime_resos(using_copy_on_write):
    df = DataFrame({"a": date_range("2019-12-31", periods=2, freq="D")***REMOVED***)
...
def test_astype_different_timezones(using_copy_on_write):
    df = DataFrame(
        {"a": date_range("2019-12-31", periods=5, freq="D", tz="US/Pacific")***REMOVED***
    )
...
def test_convert_dtypes(using_copy_on_write):
    df = DataFrame({"a": ["a", "b"], "b": [1, 2], "c": [1.5, 2.5], "d": [True, False]***REMOVED***)
    df_orig = df.copy()
