...
def to_pandas_adjacency(
    ...
    multigraph_weight : {sum, min, max***REMOVED***, optional
...
def from_pandas_adjacency(df, create_using=None):
    ...
    ```
    df_nodes = pd.DataFrame({"node_id": [1, 2, 3], "attribute1": ["A", "B", "C"]***REMOVED***)
    ...
    try:
        ...
        msg = f"{missing***REMOVED*** not in columns"
...
def to_pandas_edgelist(
    ...
    >>> G = nx.Graph(
    ...     [
    ...         ("A", "B", {"cost": 1, "weight": 7***REMOVED***),
    ...         ("C", "E", {"cost": 9, "weight": 10***REMOVED***),
    ...     ]
    ... )
    ...
    >>> G = nx.MultiGraph([('A', 'B', {'cost': 1***REMOVED***), ('A', 'B', {'cost': 9***REMOVED***)])
    ...
    if source in all_attrs:
        raise nx.NetworkXError(f"Source name {source!r***REMOVED*** is an edge attr name")
    if target in all_attrs:
        raise nx.NetworkXError(f"Target name {target!r***REMOVED*** is an edge attr name")

    nan = float("nan")
    edge_attr = {k: [d.get(k, nan) for _, _, d in edgelist] for k in all_attrs***REMOVED***

    if G.is_multigraph() and edge_key is not None:
        if edge_key in all_attrs:
            raise nx.NetworkXError(f"Edge key name {edge_key!r***REMOVED*** is an edge attr name")
        edge_keys = [k for _, _, k in G.edges(keys=True)]
        edgelistdict = {source: source_nodes, target: target_nodes, edge_key: edge_keys***REMOVED***
    else:
        edgelistdict = {source: source_nodes, target: target_nodes***REMOVED***
...
def from_pandas_edgelist(
    ...
    ... )
    >>> G[0][2]
    AtlasView({'A': {'weight': 3, 'color': 'red'***REMOVED***, 'D': {'weight': 6, 'color': 'blue'***REMOVED******REMOVED***)
    ...
    if len(attr_col_headings) == 0:
        raise nx.NetworkXError(
            f"Invalid edge_attr argument: No columns found with name: {attr_col_headings***REMOVED***"
        )

    try:
        attribute_data = zip(*[df[col] for col in attr_col_headings])
    except (KeyError, TypeError) as err:
        msg = f"Invalid edge_attr argument: {edge_attr***REMOVED***"
    ...
    if g.is_multigraph():
        ...
        if edge_key is not None:
            try:
                ...
                msg = f"Invalid edge_key argument: {edge_key***REMOVED***"
                raise nx.NetworkXError(msg) from err
...
def to_scipy_sparse_array(G, nodelist=None, dtype=None, weight="weight", format="csr"):
    ...
    format : str in {'bsr', 'csr', 'csc', 'coo', 'lil', 'dia', 'dok'***REMOVED***
    ...
    if nodelist is None:
        ...
        if nlen != len(nodeset):
            for n in nodelist:
                if n not in G:
                    raise nx.NetworkXError(f"Node {n***REMOVED*** in nodelist is not in G")
    ...
    try:
        ...
        raise nx.NetworkXError(f"Unknown sparse matrix format: {format***REMOVED***") from err
...
def from_scipy_sparse_array(
    ...
    >>> G[1][1]
    AtlasView({0: {'weight': 2***REMOVED******REMOVED***)
    ...
    ... )
    >>> G[1][1]
    AtlasView({0: {'weight': 1***REMOVED***, 1: {'weight': 1***REMOVED******REMOVED***)

    """
    G = nx.empty_graph(0, create_using)
    n, m = A.shape
    if n != m:
        raise nx.NetworkXError(f"Adjacency matrix not square: nx,ny={A.shape***REMOVED***")
...
def to_numpy_array(
    ...
    if nodeset - set(G):
        raise nx.NetworkXError(f"Nodes {nodeset - set(G)***REMOVED*** in nodelist is not in G")
    if len(nodeset) < nlen:
        raise nx.NetworkXError("nodelist contains duplicates.")
...
def from_numpy_array(A, parallel_edges=False, create_using=None, edge_attr="weight"):
    ...
    >>> G.edges(data=True)
    EdgeDataView([(0, 0, {'weight': 1***REMOVED***), (0, 1, {'weight': 2***REMOVED***), (1, 1, {'weight': 1***REMOVED***)])
    ...
    ***REMOVED***
    G = nx.empty_graph(0, create_using)
    if A.ndim != 2:
        raise nx.NetworkXError(f"Input array must be 2D, not {A.ndim***REMOVED***")
    n, m = A.shape
    if n != m:
        raise nx.NetworkXError(f"Adjacency matrix not square: nx,ny={A.shape***REMOVED***")
    dt = A.dtype
    try:
        ...
        raise TypeError(f"Unknown numpy data type: {dt***REMOVED***") from err
...
***REMOVED***
    ...
    # weight of that edge to be the entry in the matrix.
        ...
        #
        if edge_attr in [False, None]:
            triples = chain(((u, v, {***REMOVED***) for d in range(A[u, v])) for (u, v) in edges)
        else:
            triples = chain(
                ((u, v, {edge_attr: 1***REMOVED***) for d in range(A[u, v])) for (u, v) in edges
***REMOVED***
    else:  # basic data type
        if edge_attr in [False, None]:
            triples = ((u, v, {***REMOVED***) for u, v in edges)
        else:
            triples = ((u, v, {edge_attr: python_type(A[u, v])***REMOVED***) for u, v in edges)
    # If we are creating an undirected multigraph, only add the edges from the
    # upper triangle of the matrix. Otherwise, add all the edges. This relies
    # on the fact that the vertices created in the
    # `_generated_weighted_edges()` function are actually the row/column
