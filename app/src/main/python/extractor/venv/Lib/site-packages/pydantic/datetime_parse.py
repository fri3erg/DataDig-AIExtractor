...
date_expr = r'(?P<year>\d{4***REMOVED***)-(?P<month>\d{1,2***REMOVED***)-(?P<day>\d{1,2***REMOVED***)'
time_expr = (
...
date_re = re.compile(f'{date_expr***REMOVED***$')
time_re = re.compile(time_expr)
datetime_re = re.compile(f'{date_expr***REMOVED***[T ]{time_expr***REMOVED***')
...
standard_duration_re = re.compile(
    ...
    r'(?:(?P<minutes>-?\d+):)?'
    r'(?P<seconds>-?\d+)'
    r'(?:\.(?P<microseconds>\d{1,6***REMOVED***)\d{0,6***REMOVED***)?'
    r'$'
)
...
def get_numeric(value: StrBytesIntFloat, native_expected_type: str) -> Union[None, int, float]:
    ...
    try:
        ...
        raise TypeError(f'invalid type; expected {native_expected_type***REMOVED***, string, bytes, int or float')
...
def parse_time(value: Union[time, StrBytesIntFloat]) -> time:
    ...
    kw_: Dict[str, Union[None, int, timezone]] = {k: int(v) for k, v in kw.items() if v is not None***REMOVED***
    kw_['tzinfo'] = tzinfo
...
def parse_duration(value: StrBytesIntFloat) -> timedelta:
    ...
    kw_ = {k: float(v) for k, v in kw.items() if v is not None***REMOVED***
