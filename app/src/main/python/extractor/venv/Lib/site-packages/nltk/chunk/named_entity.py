...
class NEChunkParserTagger(ClassifierBasedTagger):
    ...
    def _feature_detector(self, tokens, index, history):
        ...
        features = {
            ...
            "prevword": prevword,
            "nextword": nextword,
            "word+nextpos": f"{word.lower()***REMOVED***+{nextpos***REMOVED***",
            "pos+prevtag": f"{pos***REMOVED***+{prevtag***REMOVED***",
            "shape+prevtag": f"{prevshape***REMOVED***+{prevtag***REMOVED***",
        ***REMOVED***
        ...
        for child in sent:
            if isinstance(child, Tree):
                if len(child) == 0:
                    ...
                    continue
                toks.append((child[0], f"B-{child.label()***REMOVED***"))
                for tok in child[1:]:
                    toks.append((tok, f"I-{child.label()***REMOVED***"))
            else:
                toks.append((child, "O"))
        return toks
...
def load_ace_file(textfile, fmt):
    print(f"  - {os.path.split(textfile)[1]***REMOVED***")
    ...
    entity_types = {typ for (s, e, typ) in entities***REMOVED***
...
def cmp_chunks(correct, guessed):
    ...
    for (w, ct), (w, gt) in zip(correct, guessed):
        if ct == gt == "O":
            if not ellipsis:
                print(f"  {ct:15***REMOVED*** {gt:15***REMOVED*** {w***REMOVED***")
                print("  {:15***REMOVED*** {:15***REMOVED*** {2***REMOVED***".format("...", "...", "..."))
                ellipsis = True
        else:
            ellipsis = False
            print(f"  {ct:15***REMOVED*** {gt:15***REMOVED*** {w***REMOVED***")
...
def build_model(fmt="binary"):
    ...
    outfilename = f"/tmp/ne_chunker_{fmt***REMOVED***.pickle"
    print(f"Saving chunker to {outfilename***REMOVED***...")
