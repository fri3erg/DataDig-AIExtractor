    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        response = requests.post(
            url=f"{base_url***REMOVED***/{self.model***REMOVED***/complete",
            headers={"Authorization": f"Bearer {self.ai21_api_key.get_secret_value()***REMOVED***"***REMOVED***,
            json={"prompt": prompt, "stopSequences": stop, **params***REMOVED***,
        )
        if response.status_code != 200:
            optional_detail = response.json().get("error")
            raise ValueError(
                f"AI21 /complete call failed with status code {response.status_code***REMOVED***."
