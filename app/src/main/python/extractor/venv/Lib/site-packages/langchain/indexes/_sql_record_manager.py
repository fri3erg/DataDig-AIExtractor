...
class SQLRecordManager(RecordManager):
    ...
    def __init__(
        ...
        *,
        ...
        if db_url:
            if async_mode:
                _engine = create_async_engine(db_url, **(engine_kwargs or {***REMOVED***))
            else:
                _engine = create_engine(db_url, **(engine_kwargs or {***REMOVED***))
        elif engine:
            _engine = engine
    ...
    def get_time(self) -> float:
        ...
        with self._make_session() as session:
            ...
            if self.dialect == "sqlite":
                ...
                raise NotImplementedError(f"Not implemented for dialect {self.dialect***REMOVED***")

            dt = session.execute(query).scalar()
            if isinstance(dt, decimal.Decimal):
                dt = float(dt)
            if not isinstance(dt, float):
                raise AssertionError(f"Unexpected type for datetime: {type(dt)***REMOVED***")
            return dt
    ...
    def update(
        ...
        *,
        ...
        if len(keys) != len(group_ids):
            raise ValueError(
                f"Number of keys ({len(keys)***REMOVED***) does not match number of "
                f"group_ids ({len(group_ids)***REMOVED***)"
***REMOVED***
        ...
        if time_at_least and update_time < time_at_least:
            ...
            raise AssertionError(f"Time sync issue: {update_time***REMOVED*** < {time_at_least***REMOVED***")
    ...
    ***REMOVED***
                ...
                raise NotImplementedError(f"Unsupported dialect {self.dialect***REMOVED***")
        ...
        with self._make_session() as session:
            ...
            session.query(UpsertionRecord).filter(
                and_(
                    ...
                    UpsertionRecord.namespace == self.namespace,
    ***REMOVED***
***REMOVED***.delete()  # type: ignore[attr-defined]
            session.commit()
