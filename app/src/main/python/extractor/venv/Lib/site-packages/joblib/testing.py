...
def check_subprocess_call(cmd, timeout=5, stdout_regex=None,
    ...
    def terminate_process():  # pragma: no cover
        ...
        """
        warnings.warn(f"Timeout running {cmd***REMOVED***")
        proc.terminate()
    ...
    try:
        ...
        if (stdout_regex is not None and
            ...
            raise ValueError(
                "Unexpected stdout: {!r***REMOVED*** does not match:\n{!r***REMOVED***".format(
                    stdout_regex, stdout))
        if (stderr_regex is not None and
                not re.search(stderr_regex, stderr)):
            raise ValueError(
                "Unexpected stderr: {!r***REMOVED*** does not match:\n{!r***REMOVED***".format(
                    stderr_regex, stderr))
