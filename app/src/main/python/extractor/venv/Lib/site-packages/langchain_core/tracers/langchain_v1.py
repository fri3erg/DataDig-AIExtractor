...
def get_headers() -> Dict[str, Any]:
    ...
    headers: Dict[str, Any] = {"Content-Type": "application/json"***REMOVED***
    ...
    def _convert_to_v1_run(self, run: Run) -> Union[LLMRun, ChainRun, ToolRun]:
        ...
        if not isinstance(session, TracerSessionV1):
            raise ValueError(
                ...
                f" session of type {type(session)***REMOVED***"
***REMOVED***
...
***REMOVED***
        if run.run_type == "tool":
            ...
            return ToolRun(
                ...
                child_llm_runs=[run for run in child_runs if isinstance(run, LLMRun)],
***REMOVED***
        raise ValueError(f"Unknown run type: {run.run_type***REMOVED***")
    ...
    def _persist_run(self, run: Union[Run, LLMRun, ChainRun, ToolRun]) -> None:
        ...
        if isinstance(v1_run, LLMRun):
            endpoint = f"{self._endpoint***REMOVED***/llm-runs"
        elif isinstance(v1_run, ChainRun):
            endpoint = f"{self._endpoint***REMOVED***/chain-runs"
        else:
            endpoint = f"{self._endpoint***REMOVED***/tool-runs"
...
***REMOVED***
            ...
            logger.warning(f"Failed to persist run: {e***REMOVED***")
    ...
    def _persist_session(
        ...
        try:
            r = requests.post(
                f"{self._endpoint***REMOVED***/sessions",
                data=session_create.json(),
                headers=self._headers,
***REMOVED***
            session = TracerSessionV1(id=r.json()["id"], **session_create.dict())
        except Exception as e:
            logger.warning(f"Failed to create session, using default session: {e***REMOVED***")
    ...
    def _load_session(self, session_name: Optional[str] = None) -> TracerSessionV1:
        ...
        try:
            url = f"{self._endpoint***REMOVED***/sessions"
            if session_name:
                url += f"?name={session_name***REMOVED***"
            ...
            logger.warning(
                f"Failed to load {session_type***REMOVED*** session, using empty session: {e***REMOVED***"
***REMOVED***
