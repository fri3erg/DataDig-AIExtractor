...
try:
    ...
    def _get_memory_usage(pid, force_gc=False):
        ...
        mp.util.debug(f"psutil return memory size: {mem_size***REMOVED***")
        return mem_size
...
def _python_exit():
    ...
    if len(items) > 0:
        mp.util.debug(
            "Interpreter shutting down. Waking up {len(items)***REMOVED***"
...
class _CallItem:
    ...
    def __repr__(self):
        return (
            f"CallItem({self.work_id***REMOVED***, {self.fn***REMOVED***, {self.args***REMOVED***, {self.kwargs***REMOVED***)"
        )
...
def _process_worker(
    ...
    mp.util.debug(f"Worker started with timeout={timeout***REMOVED***")
    while True:
        try:
            ...
            mp.util.info(f"Shutting down worker after timeout {timeout:0.3f***REMOVED***s")
...
class _ExecutorManagerThread(threading.Thread):
    ...
    ***REMOVED***
        ...
        #     {5: <_WorkItem...>, 6: <_WorkItem...>, ...***REMOVED***
    ...
    ***REMOVED***
            ...
            if sys.platform != "win32":
                ...
                exit_codes = (
                    ...
                    f"{get_exitcodes_terminated_worker(self.processes)***REMOVED***"
    ***REMOVED***
            mp.util.debug(
    ...
    def process_result_item(self, result_item):
        ...
        if isinstance(result_item, int):
            ...
            if p is not None:
                ...
                mp.util.debug(
                    f"joining {p.name***REMOVED*** when processing {p.pid***REMOVED*** as result_item"
    ***REMOVED***
                p.join()
                del p
    ...
    ***REMOVED***
    ...
    def kill_workers(self, reason=""):
        ...
        while self.processes:
            ...
            mp.util.debug(f"terminate process {p.name***REMOVED***, reason: {reason***REMOVED***")
            try:
                kill_process_tree(p)
    ...
    def shutdown_workers(self):
        ...
        with self.processes_management_lock:
            ...
            for p in list(self.processes.values()):
                mp.util.debug(f"releasing worker exit lock on {p.name***REMOVED***")
                p._worker_exit_lock.release()
                n_children_to_stop += 1

        mp.util.debug(f"found {n_children_to_stop***REMOVED*** processes to stop")
                ...
                try:
                    ...
                    if cooldown_time > 5.0:
                        mp.util.info(
                            ...
                            f"\ncall_queue size={self.call_queue._maxsize***REMOVED***; "
                            f" full is {self.call_queue.full()***REMOVED***; "
            ***REMOVED***
                        raise e
                    mp.util.info(
        ...
        ***REMOVED***
        ...
        mp.util.debug(f"sent {n_sentinels_sent***REMOVED*** sentinels to the call queue")
    ...
    def join_executor_internals(self):
        ...
        with self.processes_management_lock:
            mp.util.debug(f"joining {len(self.processes)***REMOVED*** processes")
            n_joined_processes = 0
            while True:
                try:
                    pid, p = self.processes.popitem()
                    mp.util.debug(f"joining process {p.name***REMOVED*** with pid {pid***REMOVED***")
                    p.join()
            ...
            mp.util.debug(
                ...
                f"{n_joined_processes***REMOVED*** workers"
***REMOVED***
...
def _check_system_limits():
    ...
    if nsems_max >= 256:
        ...
        return
    _system_limited = (
        f"system provides too few semaphores ({nsems_max***REMOVED*** available, "
        "256 necessary)"
    )
    raise NotImplementedError(_system_limited)
...
def _check_max_depth(context):
    ...
    if 0 < MAX_DEPTH and _CURRENT_DEPTH + 1 > MAX_DEPTH:
        raise LokyRecursionError(
            "Could not spawn extra nested processes at depth superior to "
            f"MAX_DEPTH={MAX_DEPTH***REMOVED***. If this is intendend, you can change "
...
class ProcessPoolExecutor(Executor):
    ...
    def __init__(
        ...
        if (
            ...
            warnings.warn(
                f"On Windows, max_workers cannot exceed {_MAX_WINDOWS_WORKERS***REMOVED*** "
...
***REMOVED***
        ...
        self._processes = {***REMOVED***

        # Internal variables of the ProcessPoolExecutor
        self._processes = {***REMOVED***
        self._queue_count = 0
        self._pending_work_items = {***REMOVED***
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        mp.util.debug(
            f"Adjusted process count to {self._max_workers***REMOVED***: "
            f"{[(p.name, pid) for pid, p in self._processes.items()]***REMOVED***"
        )
    ...
    ***REMOVED***
    ...
    def shutdown(self, wait=True, kill_workers=False):
        mp.util.debug(f"shutting down executor {self***REMOVED***")
