...
class TestReductions:
    ...
    def test_nan_int_timedelta_sum(self):
        ...
        expected = Series({"A": Timedelta(3), "B": 3***REMOVED***)
        result = df.sum()
        tm.assert_series_equal(result, expected)
...
class TestIndexReductions:
    ...
    def test_invalid_td64_reductions(self, opname):
        ...
        msg = "|".join(
            [
                f"reduction operation '{opname***REMOVED***' not allowed for this dtype",
                rf"cannot perform {opname***REMOVED*** with type timedelta64\[ns\]",
                f"does not support reduction '{opname***REMOVED***'",
            ]
        )
...
***REMOVED***
    ...
    def test_idxmin_dt64index(self, unit):
        ...
        assert res.dtype == f"M8[{unit***REMOVED***]"
        assert res.isna().all()
    ...
    def test_any_axis1_bool_only(self):
        # GH#32432
        df = DataFrame({"A": [True, False], "B": [1, 2]***REMOVED***)
    ...
    def test_idxminmax_object_decimals(self):
        ...
        res = df.idxmax()
        exp = Series({"idx": 1, "x": 1, "y": 1***REMOVED***)
        tm.assert_series_equal(res, exp)
...
class TestCategoricalSeriesReductions:
    ...
    def test_min_max_unordered_raises(self, function):
        ...
        msg = f"Categorical is not ordered for operation {function***REMOVED***"
        with pytest.raises(TypeError, match=msg):
...
***REMOVED***
    ...
    def test_mode_boolean_with_na(self):
        ...
        result = ser.mode()
        expected = Series({0: True***REMOVED***, dtype="boolean")
        tm.assert_series_equal(result, expected)
