...
class TestHITS:
    ...
    def test_hits(self, hits_alg):
        ...
        for n in G:
            assert a[n] == pytest.approx(G.a[n], abs=1e-4)
        nstart = {i: 1.0 / 2 for i in G***REMOVED***
    ...
    def test_empty(self):
        G = nx.Graph()
        assert nx.hits(G) == ({***REMOVED***, {***REMOVED***)
        assert _hits_numpy(G) == ({***REMOVED***, {***REMOVED***)
        assert _hits_python(G) == ({***REMOVED***, {***REMOVED***)
        assert _hits_scipy(G) == ({***REMOVED***, {***REMOVED***)
