...
def _astype_nansafe(
    ...
    if dtype.name in ("datetime64", "timedelta64"):
        msg = (
            f"The '{dtype.name***REMOVED***' dtype has no unit. Please pass in "
            f"'{dtype.name***REMOVED***[ns]' instead."
        )
...
def _astype_float_to_int_nansafe(
    ...
    if dtype.kind == "u":
        # GH#45151
        if not (values >= 0).all():
            raise ValueError(f"Cannot losslessly cast from {values.dtype***REMOVED*** to {dtype***REMOVED***")
...
def astype_array_safe(
    ...
    if errors not in errors_legal_values:
        invalid_arg = (
            ...
            f"{list(errors_legal_values)***REMOVED***. Supplied value is '{errors***REMOVED***'"
        )
        raise ValueError(invalid_arg)

    if inspect.isclass(dtype) and issubclass(dtype, ExtensionDtype):
        msg = (
            f"Expected an instance of {dtype.__name__***REMOVED***, "
