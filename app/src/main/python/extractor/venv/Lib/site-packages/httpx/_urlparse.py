...
PERCENT_ENCODED_REGEX = re.compile("%[A-Fa-f0-9]{2***REMOVED***")


# {scheme***REMOVED***:      (optional)
# //{authority***REMOVED***  (optional)
# {path***REMOVED***
# ?{query***REMOVED***       (optional)
# #{fragment***REMOVED***    (optional)
URL_REGEX = re.compile(
    (
        r"(?:(?P<scheme>{scheme***REMOVED***):)?"
        r"(?://(?P<authority>{authority***REMOVED***))?"
        r"(?P<path>{path***REMOVED***)"
        r"(?:\?(?P<query>{query***REMOVED***))?"
        r"(?:#(?P<fragment>{fragment***REMOVED***))?"
    ).format(
...
# {userinfo***REMOVED***@    (optional)
# {host***REMOVED***
# :{port***REMOVED***        (optional)
AUTHORITY_REGEX = re.compile(
    (
        r"(?:(?P<userinfo>{userinfo***REMOVED***)@)?" r"(?P<host>{host***REMOVED***)" r":?(?P<port>{port***REMOVED***)?"
    ).format(
...
class ParseResult(typing.NamedTuple):
    ...
    def authority(self) -> str:
        return "".join(
            [
                f"{self.userinfo***REMOVED***@" if self.userinfo else "",
                f"[{self.host***REMOVED***]" if ":" in self.host else self.host,
                f":{self.port***REMOVED***" if self.port is not None else "",
            ]
        )
    ...
    def __str__(self) -> str:
        ...
        return "".join(
            [
                f"{self.scheme***REMOVED***:" if self.scheme else "",
                f"//{authority***REMOVED***" if authority else "",
                self.path,
                f"?{self.query***REMOVED***" if self.query is not None else "",
                f"#{self.fragment***REMOVED***" if self.fragment is not None else "",
            ]
        )
...
def urlparse(url: str = "", **kwargs: typing.Optional[str]) -> ParseResult:
    ...
    if "username" in kwargs or "password" in kwargs:
        ...
        kwargs["userinfo"] = f"{username***REMOVED***:{password***REMOVED***" if password else username
    ...
    for key, value in kwargs.items():
        if value is not None:
            if len(value) > MAX_URL_LENGTH:
                raise InvalidURL(f"URL component '{key***REMOVED***' too long")

            # If a component includes any ASCII control characters including \t, \r, \n,
            # then treat it as invalid.
            if any(char.isascii() and not char.isprintable() for char in value):
                raise InvalidURL(
                    f"Invalid non-printable ASCII character in URL component '{key***REMOVED***'"
    ***REMOVED***

            # Ensure that keyword arguments match as a valid regex.
            if not COMPONENT_REGEX[key].fullmatch(value):
                raise InvalidURL(f"Invalid URL component '{key***REMOVED***'")
...
def encode_host(host: str) -> str:
    if not host:
        ...
        try:
            ...
            raise InvalidURL(f"Invalid IPv4 address: {host!r***REMOVED***")
        return host
        ...
        try:
            ...
            raise InvalidURL(f"Invalid IPv6 address: {host!r***REMOVED***")
        return host[1:-1]
    ...
    try:
        ...
        raise InvalidURL(f"Invalid IDNA hostname: {host!r***REMOVED***")
...
def normalize_port(
    ...
    try:
        ...
        raise InvalidURL(f"Invalid port: {port!r***REMOVED***")

    # See https://url.spec.whatwg.org/#url-miscellaneous
    default_port = {"ftp": 21, "http": 80, "https": 443, "ws": 80, "wss": 443***REMOVED***.get(
        scheme
    )
...
def percent_encode(char: str) -> str:
    ...
    For example:
        ...
        percent_encode(" ") == "%20"
    """
    return "".join([f"%{byte:02x***REMOVED***" for byte in char.encode("utf-8")]).upper()
