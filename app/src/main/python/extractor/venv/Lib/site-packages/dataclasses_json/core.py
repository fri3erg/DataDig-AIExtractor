...
def _user_overrides_or_exts(cls):
    ...
    try:
        cls_config = (cls.dataclass_json_config
                      if cls.dataclass_json_config is not None else {***REMOVED***)
    except AttributeError:
        cls_config = {***REMOVED***

    overrides = {***REMOVED***
    for field in fields(cls):
        field_config = {***REMOVED***
        ...
        field_config.update(field.metadata.get('dataclasses_json', {***REMOVED***))
...
def _encode_json_type(value, default=_ExtendedEncoder().default):
    if isinstance(value, Json.__args__):  # type: ignore
        if isinstance(value, list):
            ...
            return {k: _encode_json_type(v) for k, v in value.items()***REMOVED***
        else:
            return value
        ...
        if k in overrides:
            ...
            if k in override_kvs:
                raise ValueError(
                    ...
                    f"key after letter case encoding: {k***REMOVED***"
    ***REMOVED***
...
def _decode_dataclass(cls, kvs, infer_missing):
    ...
    kvs = {***REMOVED*** if kvs is None and infer_missing else kvs
    field_names = [field.name for field in fields(cls)]
    decode_names = _decode_letter_case_overrides(field_names, overrides)
    kvs = {decode_names.get(k, k): v for k, v in kvs.items()***REMOVED***
    missing_fields = {field for field in fields(cls) if field.name not in kvs***REMOVED***
    ...
    for field in fields(cls):
        ...
        if field_value is None:
            if not _is_optional(field_type):
                warning = (
                    f"value of non-optional type {field.name***REMOVED*** detected "
                    f"when decoding {cls.__name__***REMOVED***"
    ***REMOVED***
                if infer_missing:
                    warnings.warn(
                        f"Missing {warning***REMOVED*** and was defaulted to None by "
        ...
        ***REMOVED***
                    ...
                    warnings.warn(
                        f"'NoneType' object {warning***REMOVED***.", RuntimeWarning
        ***REMOVED***
    ...
    # FIXME this is a hack to fix a deeper underlying issue. A refactor is due.
        ...
        if _args is _NO_ARGS:
            ...
            if type(value) is dict and dict not in type_options:
                ...
                if res == value:
                    warnings.warn(
                        f"Failed to decode {value***REMOVED*** Union dataclasses."
...
def _decode_dict_keys(key_type, xs, infer_missing):
    ...
    elif _get_type_origin(key_type) in {tuple, Tuple***REMOVED***:
...
def _decode_items(type_args, xs, infer_missing):
    ...
    def handle_pep0673(pre_0673_hint: str) -> Union[Type, str]:
        ...
        warnings.warn(f"Could not resolve self-reference for type {pre_0673_hint***REMOVED***, "
    ...
    if _isinstance_safe(type_args, Collection) and not _issubclass_safe(type_args, Enum):
        if len(type_args) == len(xs):
            ...
            raise TypeError(f"Number of types specified in the collection type {str(type_args)***REMOVED*** "
