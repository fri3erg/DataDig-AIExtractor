...
class ContextTagger(SequentialBackoffTagger):
    ...
    def __init__(self, context_to_tag, backoff=None):
        ...
        self._context_to_tag = context_to_tag if context_to_tag else {***REMOVED***
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__***REMOVED***: size={self.size()***REMOVED***>"
    ...
    ***REMOVED***:
        ...
        if verbose:
            ...
            print(
                "size={***REMOVED***, backoff={:.2f***REMOVED***%, pruning={:.2f***REMOVED***%]".format(
...
class DefaultTagger(SequentialBackoffTagger):
    ...
    def __repr__(self):
        return f"<DefaultTagger: tag={self._tag***REMOVED***>"
...
class NgramTagger(ContextTagger):
    ...
    def encode_json_obj(self):
        _context_to_tag = {repr(k): v for k, v in self._context_to_tag.items()***REMOVED***
    ...
    def decode_json_obj(cls, obj):
        ...
        _context_to_tag = {ast.literal_eval(k): v for k, v in _context_to_tag.items()***REMOVED***
...
class UnigramTagger(NgramTagger):
    ...
    corpus, and then uses that information to assign tags to new tokens.
        ...
        ...     print("({***REMOVED***, {***REMOVED***), ".format(tok, tag)) # doctest: +NORMALIZE_WHITESPACE
    ...
    def __init__(
        ...
        for regexp, tag in regexps:
            try:
                ...
                raise Exception(
                    f"Invalid RegexpTagger regexp: {e***REMOVED***\n- regexp: {regexp!r***REMOVED***\n- tag: {tag!r***REMOVED***"
    ***REMOVED*** from e
    ...
    def __repr__(self):
        return f"<Regexp Tagger: size={len(self._regexps)***REMOVED***>"
...
***REMOVED***
    ...
    def _train(self, tagged_corpus, classifier_builder, verbose):
        ...
        *tagged_corpus*.
        ...
        if verbose:
            print(f"Training classifier ({len(classifier_corpus)***REMOVED*** instances)")
        self._classifier = classifier_builder(classifier_corpus)

    def __repr__(self):
        return f"<ClassifierBasedTagger: {self._classifier***REMOVED***>"
...
class ClassifierBasedPOSTagger(ClassifierBasedTagger):
    ...
    def feature_detector(self, tokens, index, history):
        ...
        features = {
            ...
            "prevword": prevword,
            "prevtag+word": f"{prevtag***REMOVED***+{word.lower()***REMOVED***",
            "prevprevtag+word": f"{prevprevtag***REMOVED***+{word.lower()***REMOVED***",
            "prevword+word": f"{prevword***REMOVED***+{word.lower()***REMOVED***",
            "shape": shape,
        ***REMOVED***
        return features
