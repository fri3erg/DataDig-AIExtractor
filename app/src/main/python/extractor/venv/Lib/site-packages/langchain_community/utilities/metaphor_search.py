...
class MetaphorSearchAPIWrapper(BaseModel):
    ...
    def _metaphor_search_results(
        ...
        headers = {"X-Api-Key": self.metaphor_api_key***REMOVED***
        params = {
        ...
        ***REMOVED***
        response = requests.post(
            # type: ignore
            f"{METAPHOR_API_URL***REMOVED***/search",
            headers=headers,
            json=params,
        )
            ...
            ***REMOVED***
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{METAPHOR_API_URL***REMOVED***/search", json=params, headers=headers
    ***REMOVED*** as res:
                    if res.status == 200:
                        data = await res.text()
                        return data
                    else:
                        raise Exception(f"Error {res.status***REMOVED***: {res.reason***REMOVED***")
