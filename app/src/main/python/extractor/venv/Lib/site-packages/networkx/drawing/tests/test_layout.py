...
class TestLayout:
    ...
    def test_smoke_planar_layout_embedding_input(self):
        ...
        embedding.set_data({0: [1, 2], 1: [0, 2], 2: [0, 1]***REMOVED***)
    ...
    def test_rescale_layout_dict(self):
        G = nx.empty_graph()
        vpos = nx.random_layout(G, center=(1, 1))
        assert nx.rescale_layout_dict(vpos) == {***REMOVED***

        G = nx.empty_graph(2)
        vpos = {0: (0.0, 0.0), 1: (1.0, 1.0)***REMOVED***
        s_vpos = nx.rescale_layout_dict(vpos)
        assert np.linalg.norm([sum(x) for x in zip(*s_vpos.values())]) < 1e-6

        G = nx.empty_graph(3)
        vpos = {0: (0, 0), 1: (1, 1), 2: (0.5, 0.5)***REMOVED***
        s_vpos = nx.rescale_layout_dict(vpos)
