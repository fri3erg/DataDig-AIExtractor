...
def test_numba_vs_python_indexing():
    frame = DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": [7.0, 8.0, 9.0]***REMOVED***,
        index=Index(["A", "B", "C"]),
    )
...
def test_numba_parallel_unsupported(float_frame):
    ...
    with pytest.raises(
        ...
        float_frame.apply(f, engine="numba", engine_kwargs={"parallel": True***REMOVED***)


def test_numba_nonunique_unsupported(apply_axis):
    f = lambda x: x
    df = DataFrame({"a": [1, 2]***REMOVED***, index=Index(["a", "a"]))
    with pytest.raises(
        NotImplementedError,
...
def test_numba_unsupported_dtypes(apply_axis):
    f = lambda x: x
    df = DataFrame({"a": [1, 2], "b": ["a", "b"], "c": [4, 5]***REMOVED***)
