...
class TestGetitemListLike:
    def test_getitem_list_missing_key(self):
        ...
        # column is accessed
        df = DataFrame({"x": [1.0], "y": [2.0], "z": [3.0]***REMOVED***)
        df.columns = ["x", "x", "z"]
...
***REMOVED***
    ...
    def test_getitem_iloc_generator(self):
        ...
        expected = DataFrame({"a": [2, 3], "b": [5, 6]***REMOVED***, index=[1, 2])
        tm.assert_frame_equal(result, expected)
...
***REMOVED***,
    ...
    def test_getitem_bool_mask_duplicate_columns_mixed_dtypes(
        ...
        exdict = {i: np.array(col) for i, col in enumerate(expected_data)***REMOVED***
        expected = DataFrame(exdict).rename(columns={2: 0, 3: 1***REMOVED***)
        tm.assert_frame_equal(result, expected)
...
class TestGetitemDeprecatedIndexers:
    @pytest.mark.parametrize("key", [{"a", "b"***REMOVED***, {"a": "a"***REMOVED***])
