...
class MaxComputeLoader(BaseLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        for row in self.api_wrapper.query(self.query):
            ...
            page_content = "\n".join(f"{k***REMOVED***: {v***REMOVED***" for k, v in page_content_data.items())
            if self.metadata_columns:
                metadata = {k: v for k, v in row.items() if k in self.metadata_columns***REMOVED***
            else:
                metadata = {k: v for k, v in row.items() if k not in page_content_data***REMOVED***
