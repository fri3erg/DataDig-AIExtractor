...
def test_ewm_with_times_getitem(halflife_with_times):
    ...
    df = DataFrame({"A": data, "B": data***REMOVED***)
...
def test_ewm_getitem_attributes_retained(arg, adjust, ignore_na):
    # GH 40164
    kwargs = {arg: 1, "adjust": adjust, "ignore_na": ignore_na***REMOVED***
    ewm = DataFrame({"A": range(1), "B": range(1)***REMOVED***).ewm(**kwargs)
    expected = {attr: getattr(ewm, attr) for attr in ewm._attributes***REMOVED***
    ewm_slice = ewm["A"]
    result = {attr: getattr(ewm, attr) for attr in ewm_slice._attributes***REMOVED***
...
def test_float_dtype_ewma(func, expected, float_numpy_dtype):
    ...
    df = DataFrame(
        {0: range(5), 1: range(6, 11), 2: range(10, 20, 2)***REMOVED***, dtype=float_numpy_dtype
    )
...
def test_times_string_col_raises():
    # GH 43265
    df = DataFrame(
        {"A": np.arange(10.0), "time_col": date_range("2000", freq="D", periods=10)***REMOVED***
    )
...
def test_numeric_only_series(arithmetic_win_operators, numeric_only, dtype):
    ...
    if numeric_only and dtype is object:
        msg = f"ExponentialMovingWindow.{kernel***REMOVED*** does not implement numeric_only"
