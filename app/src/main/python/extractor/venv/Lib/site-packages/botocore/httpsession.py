...
def get_cert_path(verify):
    ...
    cert_path = where()
    logger.debug(f"Certificate path: {cert_path***REMOVED***")
...
class ProxyConfiguration:
    ...
    def _construct_basic_auth(self, username, password):
        auth_str = f'{username***REMOVED***:{password***REMOVED***'
        encoded_str = b64encode(auth_str.encode('ascii')).strip().decode()
        return f'Basic {encoded_str***REMOVED***'
...
***REMOVED***,
            **kwargs,
        ***REMOVED***
        return {k: v for k, v in proxies_kwargs.items() if v is not None***REMOVED***
