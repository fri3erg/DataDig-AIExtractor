...
def test_cythonized_aggers(op_name):
    ...
    ***REMOVED***
    ...
    exp = {cat: op(group["C"]) for cat, group in grouped***REMOVED***
    exp = DataFrame({"C": exp***REMOVED***)
    exp.index.name = "A"
    result = op(grouped)
    ...
    for (cat1, cat2), group in grouped:
        expd.setdefault(cat1, {***REMOVED***)[cat2] = op(group["C"])
...
def test_cython_agg_nothing_to_agg():
    frame = DataFrame(
        {"a": np.random.default_rng(2).integers(0, 5, 50), "b": ["foo", "bar"] * 25***REMOVED***
    )
...
def test_cython_agg_frame_columns():
    # #2113
    df = DataFrame({"x": [1, 2, 3], "y": [3, 4, 5]***REMOVED***)
...
def test_cython_agg_return_dict():
    ...
    expected = Series(
        [{"two": 1, "one": 1, "three": 1***REMOVED***, {"two": 2, "one": 2, "three": 1***REMOVED***],
...
def test_cython_with_timestamp_and_nat(op, data):
    ...
    df = DataFrame({"a": [0, 1], "b": [data, NaT]***REMOVED***)
    index = Index([0, 1], name="a")

    # We will group by a and test the cython aggregations
    expected = DataFrame({"b": [data, NaT]***REMOVED***, index=index)
...
def test_read_only_buffer_source_agg(agg):
    ...
    result = df.groupby(["species"]).agg({"sepal_length": agg***REMOVED***)
    expected = df.copy().groupby(["species"]).agg({"sepal_length": agg***REMOVED***)
...
def test_cython_agg_EA_known_dtypes(data, op_name, action, with_na):
    ...
    df = DataFrame({"key": ["a", "a", "b", "b"], "col": data***REMOVED***)
