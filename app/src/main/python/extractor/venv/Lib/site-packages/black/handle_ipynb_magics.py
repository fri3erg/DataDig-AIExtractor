    ...
    while token in src:
        ...
        if counter > 100:
            raise AssertionError(
                ...
                "Please report a bug on https://github.com/psf/black/issues.  "
                f"The magic might be helpful: {magic***REMOVED***"
***REMOVED*** from None
    if len(token) + 2 < len(magic):
        token = f"{token***REMOVED***."
    return f'"{token***REMOVED***"'
...
def replace_cell_magics(src: str) -> Tuple[str, List[Replacement]]:
    ...
    return f"{mask***REMOVED***\n{cell_magic_finder.cell_magic.body***REMOVED***", replacements
    ...
    for i, line in enumerate(src.splitlines(), start=1):
        if i in magic_finder.magics:
            ...
            if len(offsets_and_magics) != 1:  # pragma: nocover
                raise AssertionError(
                    f"Expecting one magic per line, got: {offsets_and_magics***REMOVED***\n"
                    "Please report a bug on https://github.com/psf/black/issues."
    ***REMOVED***
            col_offset, magic = (
...
class CellMagic:
    ...
    def header(self) -> str:
        if self.params:
            return f"%%{self.name***REMOVED*** {self.params***REMOVED***"
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        ...
        if isinstance(node.value, ast.Call) and _is_ipython_magic(node.value.func):
            ...
            if node.value.func.attr == "getoutput":
                src = f"!{args[0]***REMOVED***"
            elif node.value.func.attr == "run_line_magic":
                src = f"%{args[0]***REMOVED***"
                if args[1]:
                    src += f" {args[1]***REMOVED***"
            else:
                raise AssertionError(
                    f"Unexpected IPython magic {node.value.func.attr!r***REMOVED*** found. "
                    "Please report a bug on https://github.com/psf/black/issues."
    ***REMOVED*** from None
