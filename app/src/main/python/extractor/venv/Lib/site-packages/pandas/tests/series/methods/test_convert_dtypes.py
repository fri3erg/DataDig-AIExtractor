...
@pytest.fixture(
    params=[
        (
            ...
            {("convert_integer", False): np.dtype("int32")***REMOVED***,
        ),
        (
            [1, 2, 3],
            np.dtype("int64"),
            "Int64",
            {("convert_integer", False): np.dtype("int64")***REMOVED***,
        ),
        (
            ["x", "y", "z"],
            np.dtype("O"),
            pd.StringDtype(),
            {("convert_string", False): np.dtype("O")***REMOVED***,
        ),
        (
            [True, False, np.nan],
            np.dtype("O"),
            pd.BooleanDtype(),
            {("convert_boolean", False): np.dtype("O")***REMOVED***,
        ),
        (
            ["h", "i", np.nan],
            np.dtype("O"),
    ...
    ***REMOVED***,
        ...
        (
            ...
            np.dtype("float"),
            "Float64",
            {("convert_floating", False): np.dtype("float")***REMOVED***,
        ),
        (
            [3, 4, 5],
            "Int8",
        ...
        (
            ...
            np.dtype("uint32"),
            "UInt32",
            {("convert_integer", False): np.dtype("uint32")***REMOVED***,
        ),
        (
            [-10, 12, 13],
            np.dtype("i1"),
            "Int8",
            {("convert_integer", False): np.dtype("i1")***REMOVED***,
        ),
        (
            [1.2, 1.3],
            np.dtype("float32"),
            "Float32",
            {("convert_floating", False): np.dtype("float32")***REMOVED***,
        ),
        (
            [1, 2.0],
            object,
    ...
    ***REMOVED***,
        ...
        (["a", "b"], pd.CategoricalDtype(), pd.CategoricalDtype(), {***REMOVED***),
        (
        ...
        (
            ...
            object,
            np.dtype("datetime64[ns]"),
            {("infer_objects", False): np.dtype("object")***REMOVED***,
        ),
        (
...
class TestSeriesConvertDtypes:
    ...
    def test_convert_string_dtype(self, nullable_string_dtype):
        ...
        df = pd.DataFrame(
            {"A": ["a", "b", pd.NA], "B": ["ä", "ö", "ü"]***REMOVED***, dtype=nullable_string_dtype
        )
    ...
    def test_convert_bool_dtype(self):
        # GH32287
        df = pd.DataFrame({"A": pd.array([True])***REMOVED***)
    ...
    def test_convert_byte_string_dtype(self):
        ...
        df = pd.DataFrame(data={"A": byte_str***REMOVED***, index=[0])
        result = df.convert_dtypes()
        expected = df
