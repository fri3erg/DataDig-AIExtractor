...
@pytest.fixture
def df_compat():
    return pd.DataFrame({"A": [1, 2, 3], "B": "foo"***REMOVED***)
...
def check_round_trip(
    ...
    """
    write_kwargs = write_kwargs or {"compression": None***REMOVED***
    read_kwargs = read_kwargs or {***REMOVED***
...
def test_get_engine_auto_error_message():
    ...
    if not have_usable_pa and not have_usable_fp:
        ...
        if have_pa_bad_version:
            match = f"Pandas requires version .{pa_min_ver***REMOVED***. or newer of .pyarrow."
        ...
        if have_fp_bad_version:
            match = f"Pandas requires version .{fp_min_ver***REMOVED***. or newer of .fastparquet."
            with pytest.raises(ImportError, match=match):
                get_engine("auto")
        else:
...
class TestBasic(Base):
    ...
    def test_columns_dtypes(self, engine):
        df = pd.DataFrame({"string": list("abc"), "int": list(range(1, 4))***REMOVED***)
    ...
    def test_compression(self, engine, compression):
        df = pd.DataFrame({"A": [1, 2, 3]***REMOVED***)
        check_round_trip(df, engine, write_kwargs={"compression": compression***REMOVED***)

    def test_read_columns(self, engine):
        # GH18154
        df = pd.DataFrame({"string": list("abc"), "int": list(range(1, 4))***REMOVED***)

        expected = pd.DataFrame({"string": list("abc")***REMOVED***)
        check_round_trip(
            df, engine, expected=expected, read_kwargs={"columns": ["string"]***REMOVED***
        )
    ...
    def test_read_filters(self, engine, tmp_path):
        ...
        expected = pd.DataFrame({"int": [0, 1]***REMOVED***)
        check_round_trip(
            df,
            engine,
            path=tmp_path,
            expected=expected,
            write_kwargs={"partition_cols": ["part"]***REMOVED***,
            read_kwargs={"filters": [("part", "==", "a")], "columns": ["int"]***REMOVED***,
            repeat=1,
        )
...
***REMOVED***
    ...
    def test_multiindex_with_columns(self, pa):
        ...
        for index in [index1, index2]:
            ...
            check_round_trip(df, engine)
            check_round_trip(
                df, engine, read_kwargs={"columns": ["A", "B"]***REMOVED***, expected=df[["A", "B"]]
***REMOVED***

    def test_write_ignoring_index(self, engine):
        # ENH 20768
        # Ensure index=False omits the index from the written Parquet file.
        df = pd.DataFrame({"a": [1, 2, 3], "b": ["q", "r", "s"]***REMOVED***)

        write_kwargs = {"compression": None, "index": False***REMOVED***
        ...
        # Ignore custom index
        df = pd.DataFrame(
            {"a": [1, 2, 3], "b": ["q", "r", "s"]***REMOVED***, index=["zyx", "wvu", "tsr"]
        )
        ...
        df = pd.DataFrame(
            {"one": list(range(8)), "two": [-i for i in range(8)]***REMOVED***, index=arrays
        )
...
***REMOVED***
        check_round_trip(
            df, pa, read_kwargs={"dtype_backend": "numpy_nullable"***REMOVED***, expected=expected
        )
...
class TestParquetPyArrow(Base):
    ...
    def test_basic_subset_columns(self, pa, df_full):
        ...
        check_round_trip(
            ...
            read_kwargs={"columns": ["string", "int"]***REMOVED***,
        )
    ...
    def test_timedelta(self, pa):
        df = pd.DataFrame({"a": pd.timedelta_range("1 day", periods=3)***REMOVED***)
        check_round_trip(df, pa)

    def test_unsupported(self, pa):
        # mixed python objects
        df = pd.DataFrame({"a": ["a", 1, 2.0]***REMOVED***)
    ...
    def test_s3_roundtrip_explicit_fs(self, df_compat, s3_public_bucket, pa, s3so):
        ...
        kw = {"filesystem": s3***REMOVED***
        check_round_trip(
            df_compat,
            pa,
            path=f"{s3_public_bucket.name***REMOVED***/pyarrow.parquet",
            read_kwargs=kw,
            write_kwargs=kw,
        )
    ...
    def test_s3_roundtrip(self, df_compat, s3_public_bucket, pa, s3so):
        # GH #19134
        s3so = {"storage_options": s3so***REMOVED***
        check_round_trip(
            df_compat,
            pa,
            path=f"s3://{s3_public_bucket.name***REMOVED***/pyarrow.parquet",
            read_kwargs=s3so,
            write_kwargs=s3so,
        )
...
***REMOVED***
        ...
        check_round_trip(
            ...
            path=f"s3://{s3_public_bucket.name***REMOVED***/parquet_dir",
            read_kwargs={"storage_options": s3so***REMOVED***,
            write_kwargs={
    ...
    def test_write_with_schema(self, pa):
        ...
        df = pd.DataFrame({"x": [0, 1]***REMOVED***)
        schema = pyarrow.schema([pyarrow.field("x", type=pyarrow.bool_())])
        out_df = df.astype(bool)
        check_round_trip(df, pa, write_kwargs={"schema": schema***REMOVED***, expected=out_df)
    ...
    def test_additional_extension_arrays(self, pa):
        ...
        df = pd.DataFrame({"a": pd.Series([1, 2, 3, None], dtype="Int64")***REMOVED***)
        check_round_trip(df, pa)

    def test_pyarrow_backed_string_array(self, pa, string_storage):
        # test ArrowStringArray supported through the __arrow_array__ protocol
        pytest.importorskip("pyarrow")
        df = pd.DataFrame({"a": pd.Series(["a", None, "c"], dtype="string[pyarrow]")***REMOVED***)
        with pd.option_context("string_storage", string_storage):
            check_round_trip(df, pa, expected=df.astype(f"string[{string_storage***REMOVED***]"))
    ...
    ***REMOVED***,
    ...
    def test_timestamp_nanoseconds(self, pa):
        ...
        ver = "2.6"
        df = pd.DataFrame({"a": pd.date_range("2017-01-01", freq="1ns", periods=10)***REMOVED***)
        check_round_trip(df, pa, write_kwargs={"version": ver***REMOVED***)
...
***REMOVED***
        ...
        df = pd.DataFrame(index=idx, data={"index_as_col": idx***REMOVED***)
    ...
    def test_filter_row_groups(self, pa):
        ...
        pytest.importorskip("pyarrow")
        df = pd.DataFrame({"a": list(range(3))***REMOVED***)
        with tm.ensure_clean() as path:
...
***REMOVED***
        ...
        check_round_trip(
            df,
            engine=pa,
            read_kwargs={"dtype_backend": "pyarrow"***REMOVED***,
            expected=expected,
        )

    def test_read_dtype_backend_pyarrow_config_index(self, pa):
        df = pd.DataFrame(
            {"a": [1, 2]***REMOVED***, index=pd.Index([3, 4], name="test"), dtype="int64[pyarrow]"
        )
        expected = df.copy()
        import pyarrow
    ...
    def test_df_attrs_persistence(self, tmp_path, pa):
        ...
        df = pd.DataFrame(data={1: [1]***REMOVED***)
        df.attrs = {"test_attribute": 1***REMOVED***
        df.to_parquet(path, engine=pa)
    ...
    def test_string_inference(self, tmp_path, pa):
        ...
        df = pd.DataFrame(data={"a": ["x", "y"]***REMOVED***, index=["a", "b"])
    ...
    def test_roundtrip_decimal(self, tmp_path, pa):
        ...
        df = pd.DataFrame({"a": [Decimal("123.00")]***REMOVED***, dtype="string[pyarrow]")
        df.to_parquet(path, schema=pa.schema([("a", pa.decimal128(5))]))
        result = read_parquet(path)
        expected = pd.DataFrame({"a": ["123"]***REMOVED***, dtype="string[python]")
        tm.assert_frame_equal(result, expected)
    ...
    def test_infer_string_large_string_type(self, tmp_path, pa):
        ...
        table = pa.table({"a": pa.array([None, "b", "c"], pa.large_string())***REMOVED***)
        pq.write_table(table, path)

        with pd.option_context("future.infer_string", True):
            result = read_parquet(path)
        expected = pd.DataFrame(
            data={"a": [None, "b", "c"]***REMOVED***,
            dtype="string[pyarrow_numpy]",
    ...
    #     v1 = b"x" * 100000000
    #     v2 = b"x" * 147483646
    #     df = pd.DataFrame({"strings": [v1] * 20 + [v2] + ["x"] * 20***REMOVED***, dtype="string")
...
class TestParquetFastParquet(Base):
    ...
    def test_bool_with_none(self, fp):
        df = pd.DataFrame({"a": [True, None, False]***REMOVED***)
        expected = pd.DataFrame({"a": [1.0, np.nan, 0.0]***REMOVED***, dtype="float16")
    ...
    def test_unsupported(self, fp):
        # period
        df = pd.DataFrame({"a": pd.period_range("2013", freq="M", periods=3)***REMOVED***)
    ...
    def test_categorical(self, fp):
        df = pd.DataFrame({"a": pd.Categorical(list("abc"))***REMOVED***)
        check_round_trip(df, fp)

    def test_filter_row_groups(self, fp):
        d = {"a": list(range(3))***REMOVED***
        df = pd.DataFrame(d)
        with tm.ensure_clean() as path:
    ...
    def test_s3_roundtrip(self, df_compat, s3_public_bucket, fp, s3so):
        ...
        check_round_trip(
            df_compat,
            fp,
            path=f"s3://{s3_public_bucket.name***REMOVED***/fastparquet.parquet",
            read_kwargs={"storage_options": s3so***REMOVED***,
            write_kwargs={"compression": None, "storage_options": s3so***REMOVED***,
        )
...
***REMOVED***
    ...
    def test_bytes_file_name(self, engine):
        # GH#48944
        df = pd.DataFrame(data={"A": [0, 1], "B": [1, 0]***REMOVED***)
        with tm.ensure_clean("test.parquet") as path:
...
***REMOVED***:
                df.to_parquet(
                    ...
                    storage_options={"foo": "bar"***REMOVED***,
    ***REMOVED***
...
***REMOVED***:
    ...
    ***REMOVED***
    ...
    def test_invalid_dtype_backend(self, engine):
        ...
        df = pd.DataFrame({"int": list(range(1, 4))***REMOVED***)
