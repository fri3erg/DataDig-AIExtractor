...
frame_data = ({"A": [1]***REMOVED***,)
frame_mi_data = ({"A": [1, 2, 3, 4]***REMOVED***, mi)
...
_all_methods = [
    ...
    (pd.Series, ([0, 0],), operator.methodcaller("replace", {0: 1***REMOVED***)),
    ...
    (pd.DataFrame, ({("A", "a"): [1]***REMOVED***,), operator.methodcaller("__getitem__", ["A"])),
    ...
    (pd.DataFrame, frame_data, operator.methodcaller("rename", columns={"A": "a"***REMOVED***)),
    ...
    (
        pd.DataFrame,
        ({"A": ["a", "b", "c"], "B": [1, 3, 5], "C": [2, 4, 6]***REMOVED***,),
    ...
    pytest.param(
        (
            pd.DataFrame,
            frame_data,
            operator.methodcaller("merge", pd.DataFrame({"A": [1]***REMOVED***)),
        ),
        marks=not_implemented_mark,
    ),
    ...
    (
        pd.DataFrame,
        ({"A": [pd.Timedelta(days=1), pd.Timedelta(days=2)]***REMOVED***,),
        operator.methodcaller("quantile", numeric_only=False),
    ),
    (
        pd.DataFrame,
        ({"A": [np.datetime64("2022-01-01"), np.datetime64("2022-01-02")]***REMOVED***,),
        operator.methodcaller("quantile", numeric_only=True),
    ),
    (
        pd.DataFrame,
        ({"A": [1]***REMOVED***, [pd.Period("2000", "D")]),
        operator.methodcaller("to_timestamp"),
    ),
    (
        pd.DataFrame,
        ({"A": [1]***REMOVED***, [pd.Timestamp("2000")]),
    ...
    (
        pd.DataFrame,
        frame_mi_data,
        operator.methodcaller("isin", pd.DataFrame({"A": [1]***REMOVED***)),
    ),
    ...
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("reindex_like", pd.DataFrame({"A": [1, 2, 3]***REMOVED***)),
    ),
    (
        pd.Series,
        frame_data,
    ...
    (pd.DataFrame, ({"A": [1] * 10***REMOVED***,), operator.methodcaller("head")),
    (pd.Series, ([1] * 10,), operator.methodcaller("tail")),
    (pd.DataFrame, ({"A": [1] * 10***REMOVED***,), operator.methodcaller("tail")),
    ...
    (
        pd.DataFrame,
        ({"A": np.array([1, 2], dtype=object)***REMOVED***,),
    ...
    (pd.DataFrame, ({"A": [1, None, 3]***REMOVED***,), operator.methodcaller("interpolate")),
    ...
    (
        pd.DataFrame,
        ({"A": [1, 1, 1, 1]***REMOVED***, pd.date_range("2000", periods=4)),
        operator.methodcaller("asfreq", "h"),
    ),
    (
        pd.Series,
    ...
    (
        pd.DataFrame,
        ({"A": [1, 1, 1, 1]***REMOVED***, pd.date_range("2000", periods=4, tz="UTC")),
        operator.methodcaller("tz_convert", "CET"),
    ),
    (
        pd.Series,
...
def test_finalize_called(ndframe_method):
    ...
    ndframe.attrs = {"a": 1***REMOVED***
    result = method(ndframe)

    assert result.attrs == {"a": 1***REMOVED***
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pd.DataFrame({"A": [1, 1, 1, 1]***REMOVED***, pd.date_range("2000", periods=4)),
    ],
)
def test_finalize_first(data):
    deprecated_msg = "first is deprecated"

    data.attrs = {"a": 1***REMOVED***
...
@pytest.mark.parametrize(
    ...
    [
        (1, pd.Series([1])),
        (1, pd.DataFrame({"A": [1]***REMOVED***)),
        (pd.Series([1]), 1),
        (pd.DataFrame({"A": [1]***REMOVED***), 1),
        (pd.Series([1]), pd.Series([1])),
        (pd.DataFrame({"A": [1]***REMOVED***), pd.DataFrame({"A": [1]***REMOVED***)),
        (pd.Series([1]), pd.DataFrame({"A": [1]***REMOVED***)),
        (pd.DataFrame({"A": [1]***REMOVED***), pd.Series([1])),
    ],
    ids=lambda x: f"({type(x[0]).__name__***REMOVED***,{type(x[1]).__name__***REMOVED***)",
)
    ...
    if not (isinstance(left, int) or isinstance(right, int)) and annotate != "both":
        if not all_binary_operators.__name__.startswith("r"):
            if annotate == "right" and isinstance(left, type(right)):
                request.applymarker(
                    pytest.mark.xfail(
                        reason=f"{all_binary_operators***REMOVED*** doesn't work when right has "
                        f"attrs and both are {type(left)***REMOVED***"
        ***REMOVED***
    ***REMOVED***
            if not isinstance(left, type(right)):
                if annotate == "left" and isinstance(left, pd.Series):
                    request.applymarker(
                        pytest.mark.xfail(
                            reason=f"{all_binary_operators***REMOVED*** doesn't work when the "
        ...
        ***REMOVED***
            ...
            if annotate == "left" and isinstance(left, type(right)):
                request.applymarker(
                    pytest.mark.xfail(
                        reason=f"{all_binary_operators***REMOVED*** doesn't work when left has "
                        f"attrs and both are {type(left)***REMOVED***"
        ***REMOVED***
    ***REMOVED***
            if not isinstance(left, type(right)):
        ...
        ***REMOVED***
            ...
            ***REMOVED***
        ***REMOVED***
    if annotate in {"left", "both"***REMOVED*** and not isinstance(left, int):
        left.attrs = {"a": 1***REMOVED***
    if annotate in {"right", "both"***REMOVED*** and not isinstance(right, int):
        right.attrs = {"a": 1***REMOVED***
...
@pytest.mark.parametrize(
    ...
    [
        ...
        operator.methodcaller("translate", {"a": "b"***REMOVED***),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        operator.methodcaller("rename_categories", {"a": "A", "b": "B"***REMOVED***),
...
@pytest.mark.parametrize(
    "obj", [pd.Series([0, 0]), pd.DataFrame({"A": [0, 1], "B": [1, 2]***REMOVED***)]
)
@pytest.mark.parametrize(
    "method",
    [
...
def test_finalize_frame_series_name():
    ...
    df = pd.DataFrame({"name": [1, 2]***REMOVED***)
