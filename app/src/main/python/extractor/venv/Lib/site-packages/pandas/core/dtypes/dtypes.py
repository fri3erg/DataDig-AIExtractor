...
class PandasExtensionDtype(ExtensionDtype):
    ...
    isbuiltin = 0
    isnative = 0
    _cache_dtypes: dict[str_type, PandasExtensionDtype] = {***REMOVED***
    ...
    def __getstate__(self) -> dict[str_type, Any]:
        ...
        return {k: getattr(self, k, None) for k in self._metadata***REMOVED***

    @classmethod
    def reset_cache(cls) -> None:
        """clear the cache"""
        cls._cache_dtypes = {***REMOVED***
        ...
        if dtype is not None:
            ...
            if isinstance(dtype, str):
                if dtype == "category":
                    ...
                    raise ValueError(f"Unknown dtype {repr(dtype)***REMOVED***")
            elif categories is not None or ordered is not None:
                raise ValueError(
                    "Cannot specify `categories` or `ordered` together with `dtype`."
    ***REMOVED***
            elif not isinstance(dtype, CategoricalDtype):
                raise ValueError(f"Cannot not construct CategoricalDtype from {dtype***REMOVED***")
...
***REMOVED***
    ...
    def construct_from_string(cls, string: str_type) -> CategoricalDtype:
        ...
        if not isinstance(string, str):
            raise TypeError(
                f"'construct_from_string' expects a string, got {type(string)***REMOVED***"
***REMOVED***
        if string != cls.name:
            raise TypeError(f"Cannot construct a 'CategoricalDtype' from '{string***REMOVED***'")
    ...
    def __eq__(self, other: object) -> bool:
        ...
        5) A CDT with ordered={False, None***REMOVED*** is only equal to another CDT with
           ordered={False, None***REMOVED*** and identical categories, but same order is
...
***REMOVED***
    ...
    def __repr__(self) -> str_type:
        ...
        return (
            f"CategoricalDtype(categories={data***REMOVED***, ordered={self.ordered***REMOVED***, "
            f"categories_dtype={dtype***REMOVED***)"
        )
    ...
    def _hash_categories(self) -> int:
        ...
        if len(categories) and isinstance(categories[0], tuple):
            ...
            if categories.dtype == "O" and len({type(x) for x in categories***REMOVED***) != 1:
                # TODO: hash_array doesn't handle mixed types. It casts
                # everything to a str first, which means we treat
                # {'1', '2'***REMOVED*** the same as {'1', 2***REMOVED***
...
***REMOVED***
    ...
    def validate_categories(categories, fastpath: bool = False) -> Index:
        ...
        if not fastpath and not is_list_like(categories):
            raise TypeError(
                f"Parameter 'categories' must be list-like, was {repr(categories)***REMOVED***"
***REMOVED***
    ...
    def __init__(self, unit: str_type | DatetimeTZDtype = "ns", tz=None) -> None:
        ...
        if unit != "ns":
            if isinstance(unit, str) and tz is None:
                ...
                tz = result.tz
                msg = (
                    f"Passing a dtype alias like 'datetime64[ns, {tz***REMOVED***]' "
...
***REMOVED***
        ...
        msg = f"Cannot construct a 'DatetimeTZDtype' from '{string***REMOVED***'"
    ...
    def __str__(self) -> str_type:
        return f"datetime64[{self.unit***REMOVED***, {self.tz***REMOVED***]"
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, str):
            if other.startswith("M8["):
                other = f"datetime64[{other[3:]***REMOVED***"
            return other == self.name
...
class PeriodDtype(PeriodDtypeBase, PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    _cache_dtypes: dict[BaseOffset, int] = {***REMOVED***  # type: ignore[assignment]
...
***REMOVED***
    ...
    def _parse_dtype_strict(cls, freq: str_type) -> BaseOffset:
        ...
        raise TypeError(
            ...
            f"got {type(freq).__name__***REMOVED***"
        )
    ...
    def construct_from_string(cls, string: str_type) -> PeriodDtype:
        ...
        if isinstance(string, str):
            msg = f"Cannot construct a 'PeriodDtype' from '{string***REMOVED***'"
        else:
            msg = f"'construct_from_string' expects a string, got {type(string)***REMOVED***"
        raise TypeError(msg)
    ...
    @property
    def name(self) -> str_type:
        return f"period[{self._freqstr***REMOVED***]"
...
class IntervalDtype(PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    def __init__(self, subtype=None, closed: IntervalClosedType | None = None) -> None:
        ...
        if closed is not None and closed not in {"right", "left", "both", "neither"***REMOVED***:
...
***REMOVED***
        ...
        msg = (
            f"Cannot construct a 'IntervalDtype' from '{string***REMOVED***'.\n\n"
    ...
    def __str__(self) -> str_type:
        ...
        if self.closed is None:
            ...
            return f"interval[{self.subtype***REMOVED***]"
        return f"interval[{self.subtype***REMOVED***, {self.closed***REMOVED***]"
...
class NumpyEADtype(ExtensionDtype):
    ...
    def __repr__(self) -> str:
        return f"NumpyEADtype({repr(self.name)***REMOVED***)"
    ...
    def construct_from_string(cls, string: str) -> NumpyEADtype:
        try:
            ...
            if not isinstance(string, str):
                ...
                msg = f"Cannot construct a 'NumpyEADtype' from '{string***REMOVED***'"
            raise TypeError(msg) from err
        return cls(dtype)
    ...
    ***REMOVED***
    ...
    def _check_fill_value(self) -> None:
        if not lib.is_scalar(self._fill_value):
            raise ValueError(
                f"fill_value must be a scalar. Got {self._fill_value***REMOVED*** instead"
***REMOVED***
    ...
    ***REMOVED***
    ...
    @property
    def name(self) -> str:
        return f"Sparse[{self.subtype.name***REMOVED***, {repr(self.fill_value)***REMOVED***]"
    ...
    def construct_from_string(cls, string: str) -> SparseDtype:
        ...
        if not isinstance(string, str):
            raise TypeError(
                f"'construct_from_string' expects a string, got {type(string)***REMOVED***"
***REMOVED***
        msg = f"Cannot construct a 'SparseDtype' from '{string***REMOVED***'"
        if string.startswith("Sparse"):
            try:
                ...
                msg = (
                    f"Cannot construct a 'SparseDtype' from '{string***REMOVED***'.\n\nIt "
    ...
    ***REMOVED***
    ...
    def _parse_subtype(dtype: str) -> tuple[str, bool]:
        ...
        if m:
            ...
            raise ValueError(f"Cannot parse {dtype***REMOVED***")
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        ...
        if not (len(set(fill_values)) == 1 or isna(fill_values).all()):
            warnings.warn(
                ...
                f"values: '{fill_values***REMOVED***'. Picking the first and "
                "converting the rest.",
                PerformanceWarning,
                stacklevel=find_stack_level(),
***REMOVED***
...
class ArrowDtype(StorageExtensionDtype):
    ...
    def __init__(self, pyarrow_dtype: pa.DataType) -> None:
        ...
        if not isinstance(pyarrow_dtype, pa.DataType):
            raise ValueError(
                f"pyarrow_dtype ({pyarrow_dtype***REMOVED***) must be an instance "
                f"of a pyarrow.DataType. Got {type(pyarrow_dtype)***REMOVED*** instead."
***REMOVED***
    ...
    def name(self) -> str:  # type: ignore[override]
        ...
        """
        return f"{str(self.pyarrow_dtype)***REMOVED***[{self.storage***REMOVED***]"
    ...
    def numpy_dtype(self) -> np.dtype:
        ...
        if pa.types.is_timestamp(self.pyarrow_dtype):
            ...
            return np.dtype(f"datetime64[{self.pyarrow_dtype.unit***REMOVED***]")
        if pa.types.is_duration(self.pyarrow_dtype):
            ...
            # This can be removed if/when pyarrow addresses it:
            # https://github.com/apache/arrow/issues/34462
            return np.dtype(f"timedelta64[{self.pyarrow_dtype.unit***REMOVED***]")
    ...
    def construct_from_string(cls, string: str) -> ArrowDtype:
        ...
        ----------
        string : str
            string should follow the format f"{pyarrow_type***REMOVED***[pyarrow]"
            e.g. int64[pyarrow]
        """
        if not isinstance(string, str):
            raise TypeError(
...
***REMOVED***
        if not string.endswith("[pyarrow]"):
            raise TypeError(f"'{string***REMOVED***' must end with '[pyarrow]'")
        ...
        try:
            ...
            if has_parameters:
                ...
                raise NotImplementedError(
                    ...
                    f"({has_parameters.group()***REMOVED***) in the string is not supported. "
                    "Please construct an ArrowDtype object with a pyarrow_dtype "
                    "instance with specific parameters."
    ***REMOVED*** from err
            raise TypeError(f"'{base_type***REMOVED***' is not a valid pyarrow data type.") from err
        return cls(pa_dtype)
