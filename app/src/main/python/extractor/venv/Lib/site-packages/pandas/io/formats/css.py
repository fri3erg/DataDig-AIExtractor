...
def _side_expander(prop_fmt: str) -> Callable:
    ...
    Returns
    -------
        function: Return to call when a 'border(-{side***REMOVED***): {value***REMOVED***' string is encountered
    """
    ...
    def expand(self, prop, value: str) -> Generator[tuple[str, str], None, None]:
        ...
        try:
            ...
            warnings.warn(
                f'Could not expand "{prop***REMOVED***: {value***REMOVED***"',
                CSSWarning,
...
def _border_expander(side: str = "") -> Callable:
    ...
    def expand(self, prop, value: str) -> Generator[tuple[str, str], None, None]:
        ...
        if len(tokens) == 0 or len(tokens) > 3:
            warnings.warn(
                f'Too many tokens provided to "{prop***REMOVED***" (expected 1-3)',
                CSSWarning,
                stacklevel=find_stack_level(),
***REMOVED***
        ...
        border_declarations = {
            f"border{side***REMOVED***-color": "black",
            f"border{side***REMOVED***-style": "none",
            f"border{side***REMOVED***-width": "medium",
        ***REMOVED***
        for token in tokens:
            if token.lower() in self.BORDER_STYLES:
                border_declarations[f"border{side***REMOVED***-style"] = token
            elif any(ratio in token.lower() for ratio in self.BORDER_WIDTH_RATIOS):
                border_declarations[f"border{side***REMOVED***-width"] = token
            else:
                border_declarations[f"border{side***REMOVED***-color"] = token
...
class CSSResolver:
    ...
    ***REMOVED***
    ...
    MARGIN_RATIOS.update({"none": ("pt", 0)***REMOVED***)
    ...
    ***REMOVED***
    ...
    CSS_EXPANSIONS = {
        **{
            (f"border-{prop***REMOVED***" if prop else "border"): _border_expander(prop)
            for prop in ["", "top", "right", "bottom", "left"]
        ***REMOVED***,
        **{
            f"border-{prop***REMOVED***": _side_expander(f"border-{{:s***REMOVED******REMOVED***-{prop***REMOVED***")
            for prop in ["color", "style", "width"]
        ***REMOVED***,
        "margin": _side_expander("margin-{:s***REMOVED***"),
        "padding": _side_expander("padding-{:s***REMOVED***"),
    ***REMOVED***
    ...
    def __call__(
        ...
        declarations_str : str | Iterable[tuple[str, str]]
            ...
            {("font-weight", "bold"), ("background", "blue")***REMOVED***
        ...
        --------
        >>> resolve = CSSResolver()
        >>> inherited = {'font-family': 'serif', 'font-weight': 'bold'***REMOVED***
        >>> out = resolve('''
...
***REMOVED***
    ...
    def _update_other_units(self, props: dict[str, str]) -> dict[str, str]:
        ...
        for side in self.SIDES:
            prop = f"border-{side***REMOVED***-width"
            if prop in props:
    ...
    ***REMOVED***
            ...
            for prop in [f"margin-{side***REMOVED***", f"padding-{side***REMOVED***"]:
                if prop in props:
                    # TODO: support %
                    props[prop] = self.size_to_pt(
                        props[prop],
    ...
    def size_to_pt(self, in_val, em_pt=None, conversions=UNIT_RATIOS) -> str:
        def _error():
            warnings.warn(
                f"Unhandled size: {repr(in_val)***REMOVED***",
                CSSWarning,
                stacklevel=find_stack_level(),
***REMOVED***
        ...
        val = round(val, 5)
        if int(val) == val:
            size_fmt = f"{int(val):d***REMOVED***pt"
        else:
            size_fmt = f"{val:f***REMOVED***pt"
        return size_fmt
    ...
    def parse(self, declarations_str: str) -> Iterator[tuple[str, str]]:
        ...
        for decl in declarations_str.split(";"):
            ...
            if sep:
                ...
                warnings.warn(
                    f"Ill-formatted attribute: expected a colon in {repr(decl)***REMOVED***",
                    CSSWarning,
                    stacklevel=find_stack_level(),
    ***REMOVED***
