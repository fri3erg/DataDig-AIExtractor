...
def is_split_before_delimiter(leaf: Leaf, previous: Optional[Leaf] = None) -> Priority:
    ...
    if (
        leaf.type == token.DOT
        and leaf.parent
        and leaf.parent.type not in {syms.import_from, syms.dotted_name***REMOVED***
    ...
    if (
        ...
        and leaf.parent
        and leaf.parent.type not in {syms.factor, syms.star_expr***REMOVED***
    ):
    ...
    if leaf.type not in {token.NAME, token.ASYNC***REMOVED***:
        return 0

    if (
        leaf.value == "for"
        and leaf.parent
        and leaf.parent.type in {syms.comp_for, syms.old_comp_for***REMOVED***
    ...
    if (
        leaf.value == "if"
        and leaf.parent
        and leaf.parent.type in {syms.comp_if, syms.old_comp_if***REMOVED***
    ):
        return COMPREHENSION_PRIORITY

    if leaf.value in {"if", "else"***REMOVED*** and leaf.parent and leaf.parent.type == syms.test:
    ...
    if (
        leaf.value == "in"
        and leaf.parent
        and leaf.parent.type in {syms.comp_op, syms.comparison***REMOVED***
        and not (
