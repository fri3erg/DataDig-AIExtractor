...
def find_pyproject_toml(
    ...
    try:
        ...
        err(f"Ignoring user configuration directory due to {e!r***REMOVED***")
        return None
...
def parse_pyproject_toml(path_config: str) -> Dict[str, Any]:
    ...
    config: Dict[str, Any] = pyproject_toml.get("tool", {***REMOVED***).get("black", {***REMOVED***)
    config = {k.replace("--", "").replace("-", "_"): v for k, v in config.items()***REMOVED***
...
def infer_target_version(
    ...
    """
    project_metadata = pyproject_toml.get("project", {***REMOVED***)
...
def parse_req_python_specifier(requires_python: str) -> Optional[List[TargetVersion]]:
    ...
    target_version_map = {f"3.{v.value***REMOVED***": v for v in TargetVersion***REMOVED***
...
def strip_specifier_set(specifier_set: SpecifierSet) -> SpecifierSet:
    ...
    for s in specifier_set:
        if "*" in str(s):
            ...
            stripped = Specifier(f"{s.operator***REMOVED***{version.major***REMOVED***.{version.minor***REMOVED***")
            specifiers.append(stripped)
        elif s.operator == ">":
            version = Version(s.version)
            if len(version.release) > 2:
                s = Specifier(f">={version.major***REMOVED***.{version.minor***REMOVED***")
            specifiers.append(s)
        else:
            specifiers.append(s)
...
def get_gitignore(root: Path) -> PathSpec:
    ...
    try:
        ...
        err(f"Could not parse {gitignore***REMOVED***: {e***REMOVED***")
        raise
...
def normalize_path_maybe_ignore(
    ...
    try:
        ...
        if report:
            report.path_ignored(path, f"cannot be read because {e***REMOVED***")
        return None
...
def get_root_relative_path(
    ...
    try:
        ...
        if report:
            report.path_ignored(path, f"is a symbolic link that points outside {root***REMOVED***")
        return None
    return root_relative_path
...
def gen_python_files(
    ...
    *,
    ...
    assert root.is_absolute(), f"INTERNAL ERROR: `root` must be absolute but is {root***REMOVED***"
    for child in paths:
