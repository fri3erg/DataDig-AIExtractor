...
def test_multiindex_group_all_columns_when_empty(groupby_func):
    # GH 32464
    df = DataFrame({"a": [], "b": [], "c": []***REMOVED***).set_index(["a", "b", "c"])
...
def test_duplicate_columns(request, groupby_func, as_index):
    ...
    if groupby_func not in ("size", "ngroup", "cumcount"):
        expected = expected.rename(columns={"c": "b"***REMOVED***)
...
def test_dup_labels_output_shape(groupby_func, idx):
    if groupby_func in {"size", "ngroup", "cumcount"***REMOVED***:
        pytest.skip(f"Not applicable for {groupby_func***REMOVED***")
