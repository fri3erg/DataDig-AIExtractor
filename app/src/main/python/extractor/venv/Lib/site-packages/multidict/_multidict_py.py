...
class _Base:
    ...
    def __repr__(self):
        body = ", ".join("'{***REMOVED***': {!r***REMOVED***".format(k, v) for k, v in self.items())
        return "<{***REMOVED***({***REMOVED***)>".format(self.__class__.__name__, body)
    ...
    def __init__(self, arg):
        if not isinstance(arg, (MultiDict, MultiDictProxy)):
            raise TypeError(
                ...
                ", not {***REMOVED***".format(type(arg))
***REMOVED***

        self._impl = arg._impl

    def __reduce__(self):
        raise TypeError("can't pickle {***REMOVED*** objects".format(self.__class__.__name__))
...
***REMOVED***
    ...
    def _extend(self, args, kwargs, name, method):
        if len(args) > 1:
            raise TypeError(
                "{***REMOVED*** takes at most 1 positional argument"
                " ({***REMOVED*** given)".format(name, len(args))
***REMOVED***
        if args:
            arg = args[0]
            if isinstance(args[0], (MultiDict, MultiDictProxy)) and not kwargs:
                ...
                for item in arg:
                    if not len(item) == 2:
                        raise TypeError(
                            "{***REMOVED*** takes either dict or list of (key, value) "
                            "tuples".format(name)
            ***REMOVED***
...
class _ItemsView(_ViewBase, abc.ItemsView):
    ...
    def __repr__(self):
        ...
        for item in self._impl._items:
            lst.append("{!r***REMOVED***: {!r***REMOVED***".format(item[1], item[2]))
        body = ", ".join(lst)
        return "{***REMOVED***({***REMOVED***)".format(self.__class__.__name__, body)
...
class _ValuesView(_ViewBase, abc.ValuesView):
    ...
    def __repr__(self):
        lst = []
        for item in self._impl._items:
            lst.append("{!r***REMOVED***".format(item[2]))
        body = ", ".join(lst)
        return "{***REMOVED***({***REMOVED***)".format(self.__class__.__name__, body)
...
class _KeysView(_ViewBase, abc.KeysView):
    ...
    def __repr__(self):
        lst = []
        for item in self._impl._items:
            lst.append("{!r***REMOVED***".format(item[1]))
        body = ", ".join(lst)
        return "{***REMOVED***({***REMOVED***)".format(self.__class__.__name__, body)
