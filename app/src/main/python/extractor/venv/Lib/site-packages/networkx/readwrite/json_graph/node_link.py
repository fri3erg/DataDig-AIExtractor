...
def node_link_data(
    ...
    *,
    ...
    >>> data1
    {'directed': False, 'multigraph': False, 'graph': {***REMOVED***, 'nodes': [{'id': 'A'***REMOVED***, {'id': 'B'***REMOVED***], 'links': [{'source': 'A', 'target': 'B'***REMOVED***]***REMOVED***
    ...
    >>> data2
    {'directed': True, 'multigraph': False, 'graph': {***REMOVED***, 'nodes': [{'id': 0***REMOVED***, {'id': 1***REMOVED***], 'edges': [{'from': 1, 'to': 0***REMOVED***]***REMOVED***
    ...
    if len({source, target, key***REMOVED***) < 3:
        raise nx.NetworkXError("Attribute names are not unique.")
    data = {
        "directed": G.is_directed(),
        "multigraph": multigraph,
        "graph": G.graph,
        "nodes": [{**G.nodes[n], name: n***REMOVED*** for n in G],
    ***REMOVED***
    if multigraph:
        data[link] = [
            {**d, source: u, target: v, key: k***REMOVED***
            for u, v, k, d in G.edges(keys=True, data=True)
        ]
    else:
        data[link] = [{**d, source: u, target: v***REMOVED*** for u, v, d in G.edges(data=True)]
    return data
...
def node_link_graph(
    ...
    *,
    ...
    # Allow 'key' to be omitted from attrs if the graph is not a multigraph.
    key = None if not multigraph else key
    graph.graph = data.get("graph", {***REMOVED***)
    c = count()
    for d in data["nodes"]:
        node = _to_tuple(d.get(name, next(c)))
        nodedata = {str(k): v for k, v in d.items() if k != name***REMOVED***
        graph.add_node(node, **nodedata)
    for d in data[link]:
        src = tuple(d[source]) if isinstance(d[source], list) else d[source]
        tgt = tuple(d[target]) if isinstance(d[target], list) else d[target]
        if not multigraph:
            edgedata = {str(k): v for k, v in d.items() if k != source and k != target***REMOVED***
