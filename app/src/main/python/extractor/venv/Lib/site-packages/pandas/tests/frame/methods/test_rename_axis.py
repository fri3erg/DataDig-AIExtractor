...
class TestDataFrameRenameAxis:
    ...
    def test_rename_axis_raises(self):
        ...
        with pytest.raises(ValueError, match="Use `.rename`"):
            df.rename_axis({0: 10, 1: 20***REMOVED***, axis=0)
    ...
    def test_rename_axis_mapper(self):
        ...
        df = DataFrame(
            {"x": list(range(len(mi))), "y": [i * 10 for i in range(len(mi))]***REMOVED***, index=mi
        )
        ...
        result = result.rename_axis(columns={"cols": "new"***REMOVED***, axis=1)
        tm.assert_index_equal(result.columns, Index(["x", "y"], name="new"))

        # Test for renaming index using dict
        result = df.rename_axis(index={"ll": "foo"***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({"mapper": None, "axis": 0***REMOVED***, True, False),
            ({"mapper": None, "axis": 1***REMOVED***, False, True),
            ({"index": None***REMOVED***, True, False),
            ({"columns": None***REMOVED***, False, True),
            ({"index": None, "columns": None***REMOVED***, True, True),
            ({***REMOVED***, False, False),
        ],
    )
