...
class Boxer:
    ...
    def interpret(self, input, discourse_id=None, question=False, verbose=False):
        ...
        if not d:
            raise Exception(f'Unable to interpret: "{input***REMOVED***"')
        return d
    ...
    def _call_candc(self, inputs, discourse_ids, question, verbose=False):
        ...
        return self._call(
            "\n".join(
                sum(
                    ([f"<META>'{id***REMOVED***'"] + d for d, id in zip(inputs, discourse_ids)),
                    [],
    ***REMOVED***
***REMOVED***,
            self._candc_bin,
...
***REMOVED***
    ...
    def _call(self, input_str, binary, args=[], verbose=False):
        ...
        if input_str is None:
            ...
            cmd = 'echo "{***REMOVED***" | {***REMOVED*** {***REMOVED***'.format(input_str, binary, " ".join(args))
            p = subprocess.Popen(
    ...
    def _parse_to_drs_dict(self, boxer_out, use_disc_id):
        ...
        while i < len(lines):
            ...
            if line.startswith("id("):
                ...
                i += 1
                line = lines[i]
                assert line.startswith(f"sem({drs_id***REMOVED***,")
                if line[-4:] == "').'":
                    line = line[:-4] + ")."
                assert line.endswith(")."), f"can't parse line: {line***REMOVED***"

                search_start = len(f"sem({drs_id***REMOVED***,[")
                brace_count = 1
                drs_start = -1
...
***REMOVED***
    ...
    def _sent_and_word_indices(self, indices):
        ...
        """
        sent_indices = {(i / 1000) - 1 for i in indices if i >= 0***REMOVED***
        if sent_indices:
            pairs = []
...
class BoxerDrs(AbstractBoxerDrs):
    ...
    def __repr__(self):
        s = "drs([{***REMOVED***], [{***REMOVED***])".format(
            ", ".join("%s" % r for r in self.refs),
            ", ".join("%s" % c for c in self.conds),
        )
        if self.consequent is not None:
            s = f"imp({s***REMOVED***, {self.consequent***REMOVED***)"
        return s
...
class BoxerIndexed(AbstractBoxerDrs):
    ...
    def __repr__(self):
        s = "{***REMOVED***({***REMOVED***, {***REMOVED***, [{***REMOVED***]".format(
            self._pred(),
            self.discourse_id,
            self.sent_index,
...
class BoxerPred(BoxerIndexed):
    ...
    def _variables(self):
        return ({self.var***REMOVED***, set(), set())
...
class BoxerRel(BoxerIndexed):
    ...
    def _variables(self):
        return ({self.var1, self.var2***REMOVED***, set(), set())
...
class BoxerProp(BoxerIndexed):
    ...
    def _variables(self):
        return tuple(
            map(operator.or_, (set(), set(), {self.var***REMOVED***), self.drs._variables())
        )

    def referenced_labels(self):
        return {self.drs***REMOVED***
...
***REMOVED***
            ...
            pred = self._add_occur_indexing(f"{ex.pos***REMOVED***_{ex.name***REMOVED***", ex)
            return self._make_atom(pred, ex.var)
        elif isinstance(ex, BoxerNamed):
            pred = self._add_occur_indexing(f"ne_{ex.type***REMOVED***_{ex.name***REMOVED***", ex)
            return self._make_atom(pred, ex.var)
...
***REMOVED***
            ...
            pred = self._add_occur_indexing(f"card_{ex.type***REMOVED***_{ex.value***REMOVED***", ex)
            return self._make_atom(pred, ex.var)
        ...
        assert False, f"{ex.__class__.__name__***REMOVED***: {ex***REMOVED***"
