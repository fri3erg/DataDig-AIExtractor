...
def to_json(
    ...
    if mode not in ["a", "w"]:
        msg = (
            f"mode={mode***REMOVED*** is not a valid option."
...
class SeriesWriter(Writer):
    ...
    def obj_to_write(self) -> NDFrame | Mapping[IndexLabel, Any]:
        if not self.index and self.orient == "split":
            return {"name": self.obj.name, "data": self.obj.values***REMOVED***
        else:
            return self.obj

    def _format_axes(self) -> None:
        if not self.obj.index.is_unique and self.orient == "index":
            raise ValueError(f"Series index must be unique for orient='{self.orient***REMOVED***'")
...
class FrameWriter(Writer):
    ...
    def _format_axes(self) -> None:
        ...
        if not self.obj.index.is_unique and self.orient in ("index", "columns"):
            raise ValueError(
                f"DataFrame index must be unique for orient='{self.orient***REMOVED***'."
***REMOVED***
        if not self.obj.columns.is_unique and self.orient in (
            ...
            raise ValueError(
                f"DataFrame columns must be unique for orient='{self.orient***REMOVED***'."
***REMOVED***
    ...
    def __init__(
        ...
        if date_format != "iso":
            msg = (
                ...
                f"`date_format='{date_format***REMOVED***'`. Table Schema requires dates "
...
***REMOVED***
    ...
    @property
    def obj_to_write(self) -> NDFrame | Mapping[IndexLabel, Any]:
        return {"schema": self.schema, "data": self.obj***REMOVED***
...
def read_json(
    ...
    *,
    ...
    orient : str, optional
        ...
        - ``'split'`` : dict like
          ``{{index -> [index], columns -> [columns], data -> [values]***REMOVED******REMOVED***``
        - ``'records'`` : list like
          ``[{{column -> value***REMOVED******REMOVED***, ... , {{column -> value***REMOVED******REMOVED***]``
        ...
        * when ``typ == 'series'``,
          ...
          - allowed orients are ``{{'split','records','index'***REMOVED******REMOVED***``
          - default is ``'index'``
        ...
        * when ``typ == 'frame'``,
          ...
          - allowed orients are ``{{'split','records','index',
            'columns','values', 'table'***REMOVED******REMOVED***``
          - default is ``'columns'``
    ...
    typ : {{'frame', 'series'***REMOVED******REMOVED***, default 'frame'
    ...
    engine : {{"ujson", "pyarrow"***REMOVED******REMOVED***, default "ujson"
...
'
    ...
    '{{"row 1":{{"col 1":"a","col 2":"b"***REMOVED******REMOVED***,"row 2":{{"col 1":"c","col 2":"d"***REMOVED******REMOVED******REMOVED******REMOVED***'
    ...
    '[{{"col 1":"a","col 2":"b"***REMOVED******REMOVED***,{{"col 1":"c","col 2":"d"***REMOVED******REMOVED***]'
    ...
    >>> df.to_json(orient='table')
        '\
{{"schema":{{"fields":[\
{{"name":"index","type":"string"***REMOVED******REMOVED***,\
{{"name":"col 1","type":"string"***REMOVED******REMOVED***,\
{{"name":"col 2","type":"string"***REMOVED******REMOVED***],\
"primaryKey":["index"],\
"pandas_version":"1.4.0"***REMOVED******REMOVED***,\
"data":[\
{{"index":"row 1","col 1":"a","col 2":"b"***REMOVED******REMOVED***,\
{{"index":"row 2","col 1":"c","col 2":"d"***REMOVED******REMOVED***]\
***REMOVED******REMOVED***\
'

    The following example uses ``dtype_backend="numpy_nullable"``

    >>> data = '''{{"index": {{"0": 0, "1": 1***REMOVED******REMOVED***,
    ...        "a": {{"0": 1, "1": null***REMOVED******REMOVED***,
    ...        "b": {{"0": 2.5, "1": 4.5***REMOVED******REMOVED***,
    ...        "c": {{"0": true, "1": false***REMOVED******REMOVED***,
    ...        "d": {{"0": "a", "1": "b"***REMOVED******REMOVED***,
    ...        "e": {{"0": 1577.2, "1": 1577.1***REMOVED******REMOVED******REMOVED******REMOVED***'''
...
class JsonReader(abc.Iterator, Generic[FrameSeriesStrT]):
    ...
    def __init__(
        ...
        if self.engine not in {"pyarrow", "ujson"***REMOVED***:
            raise ValueError(
                f"The engine type {self.engine***REMOVED*** is currently not supported."
***REMOVED***
...
***REMOVED***
            ...
            and filepath_or_buffer.lower().endswith(
                (".json",) + tuple(f".json{c***REMOVED***" for c in extension_to_compression)
***REMOVED***
            and not file_exists(filepath_or_buffer)
        ):
            raise FileNotFoundError(f"File {filepath_or_buffer***REMOVED*** does not exist")
        else:
            warnings.warn(
                "Passing literal json to 'read_json' is deprecated and "
                "will be removed in a future version. To read from a "
...
***REMOVED***
    ...
    def _combine_lines(self, lines) -> str:
        ...
        """
        return (
            f'[{",".join([line for line in (line.strip() for line in lines) if line])***REMOVED***]'
        )
    ...
    ***REMOVED***
    ...
    def __init__(
        ...
        if date_unit is not None:
            ...
            if date_unit not in self._STAMP_UNITS:
                raise ValueError(f"date_unit must be one of {self._STAMP_UNITS***REMOVED***")
    ...
    def check_keys_split(self, decoded: dict) -> None:
        ...
        if bad_keys:
            ...
            raise ValueError(f"JSON data had unexpected key(s): {bad_keys_joined***REMOVED***")
...
class SeriesParser(Parser):
    ...
    def _parse(self) -> None:
        ...
        if self.orient == "split":
            decoded = {str(k): v for k, v in data.items()***REMOVED***
