...
***REMOVED***
    ...
    def _get_post_headers(self) -> dict:
        ...
        return {"X-API-KEY": self._api_key***REMOVED***
        ...
        **kwargs: Any,
        ...
        for text in texts:
            ...
            response = self._session.post(
                f"{self.base_url***REMOVED***/datastores/{self._datastore_id***REMOVED***/text",
                json={"name": doc_name, "text": text***REMOVED***,
                verify=True,
...
***REMOVED***
            ...
            if response.status_code != 200:
                logging.error(
                    f"Create request failed for doc_name = {doc_name***REMOVED*** with status code "
        ...
        **kwargs: Any,
        ...
        for file in files_list:
            if not os.path.exists(file):
                logging.error(f"File {file***REMOVED*** does not exist, skipping")
                continue

            files.append(("file", (os.path.basename(file), open(file, "rb"))))

        response = self._session.post(
            f"{self.base_url***REMOVED***/datastores/{self._datastore_id***REMOVED***/file",
            files=files,
            verify=True,
            headers=self._get_post_headers(),
        )
        ...
        if response.status_code != 200:
            logging.error(
                f"Create request failed for datastore = {self._datastore_id***REMOVED*** "
                f"with status code {response.status_code***REMOVED***, reason {response.reason***REMOVED***, "
                f"text {response.text***REMOVED***"
***REMOVED***
    ...
    def similarity_search_with_score(
        ...
        response = self._session.post(
            headers=self._get_post_headers(),
            url=f"{self.base_url***REMOVED***/datastores/{self._datastore_id***REMOVED***/search",
            data=json.dumps({"k": k, "text": query***REMOVED***),
            timeout=10,
        )
...
class LLMRailsRetriever(VectorStoreRetriever):
    ...
    vectorstore: LLMRails
    search_kwargs: dict = Field(default_factory=lambda: {"k": 5***REMOVED***)
    """Search params.
        k: Number of Documents to return. Defaults to 5.
        alpha: parameter for hybrid search .
    """
