...
For example::
    ...
    entry_points={'networkx.backends': 'sparse = networkx_backend_sparse'***REMOVED***
...
def _get_backends(group, *, load_and_call=False):
    ...
    for ep in items:
        if ep.name in rv:
            warnings.warn(
                f"networkx backend defined more than once: {ep.name***REMOVED***",
                RuntimeWarning,
                stacklevel=2,
***REMOVED***
        elif load_and_call:
            try:
                ...
                warnings.warn(
                    f"Error encountered when loading info for backend {ep.name***REMOVED***: {exc***REMOVED***",
                    RuntimeWarning,
                    stacklevel=2,
    ***REMOVED***
        else:
...
_loaded_backends = {***REMOVED***  # type: ignore[var-annotated]
...
_registered_algorithms = {***REMOVED***
...
class _dispatch:
    ...
    graphs : str or dict or None, default "G"
        ...
        For example, ``@_dispatch(graphs={"G": 0, "auxiliary?": 4***REMOVED***)``
    ...
    preserve_edge_attrs : bool or str or dict, optional
        ...
        when converting.
        For dict of ``{graph_name: {attr: default***REMOVED******REMOVED***``, indicate pre-determined
    ...
    #  - @_dispatch(graphs={"G": 0, "H": 1***REMOVED***, edge_attrs={"weight": "default"***REMOVED***)
...
***REMOVED***
        ...
        if func.__kwdefaults__:
            self.__kwdefaults__ = {**func.__kwdefaults__, "backend": None***REMOVED***
        else:
            self.__kwdefaults__ = {"backend": None***REMOVED***
        ...
        if edge_attrs is not None and not isinstance(edge_attrs, (str, dict)):
            raise TypeError(
                f"Bad type for edge_attrs: {type(edge_attrs)***REMOVED***. Expected str or dict."
***REMOVED*** from None
        if node_attrs is not None and not isinstance(node_attrs, (str, dict)):
            raise TypeError(
                f"Bad type for node_attrs: {type(node_attrs)***REMOVED***. Expected str or dict."
***REMOVED*** from None
        if not isinstance(self.preserve_edge_attrs, (bool, str, dict)):
            raise TypeError(
                f"Bad type for preserve_edge_attrs: {type(self.preserve_edge_attrs)***REMOVED***."
                " Expected bool, str, or dict."
***REMOVED*** from None
        if not isinstance(self.preserve_node_attrs, (bool, str, dict)):
            raise TypeError(
                f"Bad type for preserve_node_attrs: {type(self.preserve_node_attrs)***REMOVED***."
                " Expected bool, str, or dict."
***REMOVED*** from None
        if not isinstance(self.preserve_graph_attrs, (bool, set)):
            raise TypeError(
                f"Bad type for preserve_graph_attrs: {type(self.preserve_graph_attrs)***REMOVED***."
                " Expected bool or set."
***REMOVED*** from None

        if isinstance(graphs, str):
            graphs = {graphs: 0***REMOVED***
        elif graphs is None:
            pass
        elif not isinstance(graphs, dict):
            raise TypeError(
                f"Bad type for graphs: {type(graphs)***REMOVED***. Expected str or dict."
***REMOVED*** from None
        elif len(graphs) == 0:
        ...
        # self.optional_graphs = {k[:-1] for k in graphs if k[-1] == "?"***REMOVED***
        # self.list_graphs = {k[1:-1] for k in graphs if k[-1] == "]"***REMOVED***
        # self.graphs = {k[:-1] if k[-1] == "?" else k: v for k, v in graphs.items()***REMOVED***
        ...
        ***REMOVED***
        ...
        if name in _registered_algorithms:
            raise KeyError(
                f"Algorithm already exists in dispatch registry: {name***REMOVED***"
***REMOVED*** from None
    ...
    ***REMOVED***
    ...
    def __call__(self, /, *args, backend=None, **kwargs):
        ...
        if backend_name is not None and backend_name not in backends:
            raise ImportError(f"Unable to load backend: {backend_name***REMOVED***")

        graphs_resolved = {***REMOVED***
        for gname, pos in self.graphs.items():
            if pos < len(args):
                if gname in kwargs:
                    raise TypeError(f"{self.name***REMOVED***() got multiple values for {gname!r***REMOVED***")
                val = args[pos]
            elif gname in kwargs:
                val = kwargs[gname]
            elif gname not in self.optional_graphs:
                raise TypeError(
                    f"{self.name***REMOVED***() missing required graph argument: {gname***REMOVED***"
    ***REMOVED***
            else:
                continue
            if val is None:
                if gname not in self.optional_graphs:
                    raise TypeError(
                        f"{self.name***REMOVED***() required graph argument {gname!r***REMOVED*** is None; must be a graph"
        ***REMOVED***
            else:
...
***REMOVED***
        ...
        ***REMOVED***
        ...
        if has_backends:
            ...
            backend_names = graph_backend_names - {"networkx"***REMOVED***
            if len(backend_names) != 1:
                # Future work: convert between backends and run if multiple backends found
                raise TypeError(
                    f"{self.name***REMOVED***() graphs must all be from the same backend, found {backend_names***REMOVED***"
    ***REMOVED***
            ...
            if backend_name is not None and backend_name != graph_backend_name:
                ...
                raise TypeError(
                    f"{self.name***REMOVED***() is unable to convert graph from backend {graph_backend_name!r***REMOVED*** "
                    f"to the specified backend {backend_name!r***REMOVED***."
    ***REMOVED***
            if graph_backend_name not in backends:
                raise ImportError(f"Unable to load backend: {graph_backend_name***REMOVED***")
            if (
...
***REMOVED***:
                ...
                raise TypeError(
                    f"Unable to convert inputs and run {self.name***REMOVED***. "
                    f"{self.name***REMOVED***() has networkx and {graph_backend_name***REMOVED*** graphs, but NetworkX is not "
                    f"configured to automatically convert graphs from networkx to {graph_backend_name***REMOVED***."
    ***REMOVED***
        ...
        ***REMOVED***
            ...
            raise NetworkXNotImplemented(
                f"'{self.name***REMOVED***' not implemented by {graph_backend_name***REMOVED***"
***REMOVED***
...
***REMOVED***:
                ...
                preserve_edge_attrs = False
        # Else: e.g. `preserve_edge_attrs={"G": {"weight": 1***REMOVED******REMOVED***`
        ...
        if edge_attrs is None:
            ...
            if edge_attrs[0] == "[":
                ...
                ***REMOVED***
                ...
                edge_attrs = {bound.arguments[edge_attrs]: 1***REMOVED***
...
***REMOVED***:
            ...
            # e.g. `edge_attrs={"attr": "default"***REMOVED***` and `func(attr="foo", default=7)`
            # e.g. `edge_attrs={"attr": 0***REMOVED***` and `func(attr="foo")`
            edge_attrs = {
...
***REMOVED***:
                ...
                preserve_node_attrs = False
        # Else: e.g. `preserve_node_attrs={"G": {"pos": None***REMOVED******REMOVED***`
        ...
        if node_attrs is None:
            ...
            if node_attrs[0] == "[":
                ...
                ***REMOVED***
                ...
                node_attrs = {bound.arguments[node_attrs]: None***REMOVED***
            else:
                # e.g. `node_attrs="weight"` and `func(weight=None)`
                node_attrs = None
        else:
            # e.g. `node_attrs={"attr": "default"***REMOVED***` and `func(attr="foo", default=7)`
            # e.g. `node_attrs={"attr": 0***REMOVED***` and `func(attr="foo")`
            node_attrs = {
        ...
        ***REMOVED***
                ...
                if graph is None:
                    if gname in self.optional_graphs:
                        continue
                    raise TypeError(
                        f"Missing required graph argument `{gname***REMOVED***` in {self.name***REMOVED*** function"
        ***REMOVED***
    ...
    ***REMOVED***:
    ...
    def _convert_and_call(self, backend_name, args, kwargs, *, fallback_to_nx=False):
        ...
        if not self._can_backend_run(backend_name, *args, **kwargs):
            if fallback_to_nx:
                return self.orig_func(*args, **kwargs)
            msg = f"'{self.name***REMOVED***' not implemented by {backend_name***REMOVED***"
            if hasattr(backend, self.name):
...
***REMOVED***
            ...
            pytest.xfail(
                exc.args[0] if exc.args else f"{self.name***REMOVED*** raised {type(exc).__name__***REMOVED***"
***REMOVED***
    ...
    def _make_doc(self):
        ...
        for backend in sorted(self.backends):
            ...
            if "short_summary" in info:
                lines.append(f"{backend***REMOVED*** : {info['short_summary']***REMOVED***")
            else:
            ...
            if "extra_docstring" in func_info:
                lines.extend(
                    f"  {line***REMOVED***" if line else line
    ...
    ***REMOVED***
            ...
            if "extra_parameters" in func_info:
                ...
                for param in sorted(extra_parameters):
                    lines.append(f"    {param***REMOVED***")
                    if desc := extra_parameters[param]:
                        lines.append(f"      {desc***REMOVED***")
                    lines.append("")
            else:
                lines.append("")
        ...
        to_add = "\n    ".join(lines)
        return f"{self._orig_doc.rstrip()***REMOVED***\n\n    {to_add***REMOVED***"
