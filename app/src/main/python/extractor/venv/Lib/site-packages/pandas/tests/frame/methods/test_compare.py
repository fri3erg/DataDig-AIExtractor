...
def test_compare_axis(align_axis):
    # GH#30429
    df = pd.DataFrame(
        {"col1": ["a", "b", "c"], "col2": [1.0, 2.0, np.nan], "col3": [1.0, 2.0, 3.0]***REMOVED***,
...
def test_compare_result_names():
    ...
    expected = pd.DataFrame(
        {
            ("col1", "left"): {0: "a", 2: np.nan***REMOVED***,
            ("col1", "right"): {0: "c", 2: np.nan***REMOVED***,
            ("col3", "left"): {0: np.nan, 2: 3.0***REMOVED***,
            ("col3", "right"): {0: np.nan, 2: np.nan***REMOVED***,
        ***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
...
def test_compare_ea_and_np_dtype(val1, val2):
    ...
    df1 = pd.DataFrame({"a": arr, "b": [1.0, 2]***REMOVED***)
    df2 = pd.DataFrame({"a": ser, "b": [1.0, 2]***REMOVED***)
    expected = pd.DataFrame(
        {
            ("a", "self"): arr,
            ("a", "other"): ser,
...
def test_compare_nullable_int64_dtype(df1_val, df2_val, diff_self, diff_other):
    # GH 48966
    df1 = pd.DataFrame({"a": pd.Series([df1_val, pd.NA], dtype="Int64"), "b": [1.0, 2]***REMOVED***)
    df2 = df1.copy()
    df2.loc[0, "a"] = df2_val
