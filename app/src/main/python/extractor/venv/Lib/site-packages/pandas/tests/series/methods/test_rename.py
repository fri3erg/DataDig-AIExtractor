...
class TestRename:
    ...
    def test_rename_partial_dict(self):
        ...
        renamed = ser.rename({"b": "foo", "d": "bar"***REMOVED***)
    ...
    def test_rename_retain_index_name(self):
        ...
        renamed = renamer.rename({***REMOVED***)
        assert renamed.index.name == renamer.index.name

    def test_rename_by_series(self):
        ser = Series(range(5), name="foo")
        renamer = Series({1: 10, 2: 20***REMOVED***)
    ...
    def test_rename_axis_supported(self):
        ...
        ser = Series(range(5))
        ser.rename({***REMOVED***, axis=0)
        ser.rename({***REMOVED***, axis="index")

        with pytest.raises(ValueError, match="No axis named 5"):
            ser.rename({***REMOVED***, axis=5)
    ...
    def test_rename_series_with_multiindex(self):
        ...
        result = ser.rename(index={"one": "yes"***REMOVED***, level="second", errors="raise")
    ...
    def test_rename_series_with_multiindex_keeps_ea_dtypes(self):
        ...
        result = ser.rename({1: 4***REMOVED***, level=1)
    ...
    def test_rename_error_arg(self):
        ...
        with pytest.raises(KeyError, match=match):
            ser.rename({2: 9***REMOVED***, errors="raise")

    def test_rename_copy_false(self, using_copy_on_write, warn_copy_on_write):
        # GH 46889
        ser = Series(["foo", "bar"])
        ser_orig = ser.copy()
        shallow_copy = ser.rename({1: 9***REMOVED***, copy=False)
