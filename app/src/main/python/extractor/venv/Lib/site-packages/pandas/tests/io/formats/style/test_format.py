...
def test_display_format(styler):
    ctx = styler.format("{:0.1f***REMOVED***")._translate(True, True)
...
def test_display_format_index(styler, index, columns):
    ...
    exp_columns = ["A", "B"]
    if columns:
        styler.format_index("*{***REMOVED****", axis=1)  # test string
        exp_columns = ["*A*", "*B*"]
...
def test_format_dict(styler):
    ctx = styler.format({"A": "{:0.1f***REMOVED***", "B": "{0:.2%***REMOVED***"***REMOVED***)._translate(True, True)
    assert ctx["body"][0][1]["display_value"] == "0.0"
    assert ctx["body"][0][2]["display_value"] == "-60.90%"


def test_format_index_dict(styler):
    ctx = styler.format_index({0: lambda v: v.upper()***REMOVED***)._translate(True, True)
    for i, val in enumerate(["X", "Y"]):
        assert ctx["body"][i][0]["display_value"] == val


def test_format_string(styler):
    ctx = styler.format("{:.2f***REMOVED***")._translate(True, True)
...
def test_format_with_na_rep():
    ...
    ctx = df.style.format("{:.2%***REMOVED***", na_rep="-")._translate(True, True)
    assert ctx["body"][0][1]["display_value"] == "-"
    assert ctx["body"][0][2]["display_value"] == "-"
    assert ctx["body"][1][1]["display_value"] == "110.00%"
    assert ctx["body"][1][2]["display_value"] == "120.00%"

    ctx = df.style.format("{:.2%***REMOVED***", na_rep="-", subset=["B"])._translate(True, True)
    assert ctx["body"][0][2]["display_value"] == "-"
    assert ctx["body"][1][2]["display_value"] == "120.00%"
...
@pytest.mark.parametrize(
    "func, attr, kwargs",
    [
        ("format", "_display_funcs", {***REMOVED***),
        ("format_index", "_display_funcs_index", {"axis": 0***REMOVED***),
        ("format_index", "_display_funcs_columns", {"axis": 1***REMOVED***),
    ],
)
def test_format_clear(styler, func, attr, kwargs):
    assert (0, 0) not in getattr(styler, attr)  # using default
    getattr(styler, func)("{:.2f***REMOVED***", **kwargs)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "latex",
            '<>\\&"\\%\\$\\#\\_\\{\\***REMOVED***\\textasciitilde \\textasciicircum '
...
def test_format_escape_html(escape, exp):
    ...
    s = Styler(df, uuid_len=0).format("&{0***REMOVED***&", escape=None)
    expected = f'<td id="T__row0_col0" class="data row0 col0" >&{chars***REMOVED***&</td>'
    assert expected in s.to_html()

    # only the value should be escaped before passing to the formatter
    s = Styler(df, uuid_len=0).format("&{0***REMOVED***&", escape=escape)
    expected = f'<td id="T__row0_col0" class="data row0 col0" >&{exp***REMOVED***&</td>'
    assert expected in s.to_html()

    # also test format_index()
    styler = Styler(DataFrame(columns=[chars]), uuid_len=0)
    styler.format_index("&{0***REMOVED***&", escape=None, axis=1)
    assert styler._translate(True, True)["head"][0][1]["display_value"] == f"&{chars***REMOVED***&"
    styler.format_index("&{0***REMOVED***&", escape=escape, axis=1)
    assert styler._translate(True, True)["head"][0][1]["display_value"] == f"&{exp***REMOVED***&"
...
@pytest.mark.parametrize(
    ...
    [
        (
            r"$ \$&%#_{***REMOVED***~^\ $ &%#_{***REMOVED***~^\ $",
            "".join(
                [
                    r"$ \$&%#_{***REMOVED***~^\ $ ",
                    r"\&\%\#\_\{\***REMOVED***\textasciitilde \textasciicircum ",
...
***REMOVED***,
        ...
        (
            r"$\&%#_{***REMOVED***^\$",
            r"\$\textbackslash \&\%\#\_\{\***REMOVED***\textasciicircum \textbackslash \$",
        ),
        (
            r"$ \frac{1***REMOVED***{2***REMOVED*** $ \( \frac{1***REMOVED***{2***REMOVED*** \)",
            "".join(
                [
                    r"$ \frac{1***REMOVED***{2***REMOVED*** $",
                    r" \textbackslash ( \textbackslash frac\{1\***REMOVED***\{2\***REMOVED*** \textbackslash )",
                ]
***REMOVED***,
        ),
    ],
...
def test_format_escape_latex_math(chars, expected):
    ...
    df = DataFrame([[chars]])
    s = df.style.format("{0***REMOVED***", escape="latex-math")
...
def test_format_escape_na_rep():
    ...
    df = DataFrame([['<>&"', None]])
    s = Styler(df, uuid_len=0).format("X&{0***REMOVED***>X", escape="html", na_rep="&")
    ...
    styler.format_index("X&{0***REMOVED***>X", escape="html", na_rep="&", axis=1)
    ctx = styler._translate(True, True)
...
def test_format_escape_floats(styler):
    ...
    s = styler.format("{:.1f***REMOVED***", escape="html")
...
def test_format_subset():
    ...
    ctx = df.style.format(
        {"a": "{:0.1f***REMOVED***", "b": "{0:.2%***REMOVED***"***REMOVED***, subset=IndexSlice[0, :]
    ...
    ctx = df.style.format("{:0.1f***REMOVED***", subset=IndexSlice[0, :])._translate(True, True)
    assert ctx["body"][0][1]["display_value"] == expected
    assert ctx["body"][1][1]["display_value"] == raw_11

    ctx = df.style.format("{:0.1f***REMOVED***", subset=IndexSlice["a"])._translate(True, True)
    assert ctx["body"][0][1]["display_value"] == expected
    assert ctx["body"][0][2]["display_value"] == "0.123400"

    ctx = df.style.format("{:0.1f***REMOVED***", subset=IndexSlice[0, "a"])._translate(True, True)
    assert ctx["body"][0][1]["display_value"] == expected
    assert ctx["body"][1][1]["display_value"] == raw_11

    ctx = df.style.format("{:0.1f***REMOVED***", subset=IndexSlice[[0, 1], ["a"]])._translate(
        True, True
    )
    assert ctx["body"][0][1]["display_value"] == expected
...
@pytest.mark.parametrize("formatter", [None, "{:,.1f***REMOVED***"])
...
@pytest.mark.parametrize("formatter", [None, "{:,.4f***REMOVED***"])
...
def test_str_escape_error():
    msg = "`escape` only permitted in {'html', 'latex', 'latex-math'***REMOVED***, got "
...
def test_format_options():
    df = DataFrame({"int": [2000, 1], "float": [1.009, None], "str": ["&<", "&~"]***REMOVED***)
    ...
    with option_context("styler.format.formatter", {"int": "{:,.2f***REMOVED***"***REMOVED***):
        ctx_with_op = df.style._translate(True, True)
...
@pytest.mark.parametrize(
    "formatter, exp",
    [
        (lambda x: f"{x:.3f***REMOVED***", "9.000"),
        ("{:.2f***REMOVED***", "9.00"),
        ({0: "{:.1f***REMOVED***"***REMOVED***, "9.0"),
        (None, "9"),
    ],
)
def test_formatter_options_validator(formatter, exp):
    ...
    with option_context("styler.format.formatter", formatter):
        assert f" {exp***REMOVED*** " in df.style.to_latex()
...
def test_relabel_index(styler_multi):
    ...
    assert {"value": "X", "display_value": 1***REMOVED***.items() <= ctx["body"][0][0].items()
    assert {"value": "y", "display_value": 2***REMOVED***.items() <= ctx["body"][0][1].items()
    assert {"value": "Y", "display_value": 3***REMOVED***.items() <= ctx["body"][1][0].items()
    assert {"value": "x", "display_value": 4***REMOVED***.items() <= ctx["body"][1][1].items()
...
def test_relabel_columns(styler_multi):
    ...
    ctx = styler_multi._translate(True, True)
    assert {"value": "A", "display_value": 1***REMOVED***.items() <= ctx["head"][0][3].items()
    assert {"value": "B", "display_value": 3***REMOVED***.items() <= ctx["head"][0][4].items()
    assert {"value": "b", "display_value": 2***REMOVED***.items() <= ctx["head"][1][3].items()
    assert {"value": "a", "display_value": 4***REMOVED***.items() <= ctx["head"][1][4].items()


def test_relabel_roundtrip(styler):
    styler.relabel_index(["{***REMOVED***", "{***REMOVED***"])
    ctx = styler._translate(True, True)
    assert {"value": "x", "display_value": "x"***REMOVED***.items() <= ctx["body"][0][0].items()
    assert {"value": "y", "display_value": "y"***REMOVED***.items() <= ctx["body"][1][0].items()
