...
class BasePDFLoader(BaseLoader, ABC):
    ...
    def __init__(self, file_path: str, *, headers: Optional[Dict] = None):
        ...
        if not os.path.isfile(self.file_path) and self._is_valid_url(self.file_path):
            ...
            temp_pdf = os.path.join(self.temp_dir.name, f"tmp{suffix***REMOVED***")
...
***REMOVED***
    ...
    def load(self, **kwargs: Any) -> List[Document]:
        ...
        if kwargs:
            logger.warning(
                f"Received runtime arguments {kwargs***REMOVED***. Passing runtime args to `load`"
                f" is deprecated. Please pass arguments during initialization instead."
***REMOVED***

        text_kwargs = {**self.text_kwargs, **kwargs***REMOVED***
...
class MathpixPDFLoader(BasePDFLoader):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.extra_request_data = (
            extra_request_data if extra_request_data is not None else {***REMOVED***
        )
    ...
    def _mathpix_headers(self) -> Dict[str, str]:
        return {"app_id": self.mathpix_api_id, "app_key": self.mathpix_api_key***REMOVED***
    ...
    def data(self) -> dict:
        options = {
            "conversion_formats": {self.processed_file_format: True***REMOVED***,
            **self.extra_request_data,
        ***REMOVED***
        return {"options_json": json.dumps(options)***REMOVED***
...
***REMOVED***
        ...
        if "error" in response_data:
            raise ValueError(f"Mathpix request failed: {response_data['error']***REMOVED***")
    ...
    def wait_for_processing(self, pdf_id: str) -> None:
        ...
        for _ in range(0, self.max_wait_time_seconds, 5):
            ...
            if error is not None:
                error_msg = f"Unable to retrieve PDF from Mathpix: {error***REMOVED***"

                if error_info is not None:
                    error_msg += f" ({error_info['id']***REMOVED***)"
            ...
            if status == "completed":
                ...
                print(f"Status: {status***REMOVED***, waiting for processing to complete")
                time.sleep(5)
        raise TimeoutError

    def get_processed_pdf(self, pdf_id: str) -> str:
        self.wait_for_processing(pdf_id)
        url = f"{self.url***REMOVED***/{pdf_id***REMOVED***.{self.processed_file_format***REMOVED***"
        response = requests.get(url, headers=self._mathpix_headers)
    ...
    def clean_pdf(self, contents: str) -> str:
        ...
        # replace \section{Title***REMOVED*** with # Title
        contents = contents.replace("\\section{", "# ").replace("***REMOVED***", "")
    ...
    def load(self) -> List[Document]:
        ...
        metadata = {"source": self.source, "file_path": self.source, "pdf_id": pdf_id***REMOVED***
...
***REMOVED***
        ...
        super().__init__(file_path, headers=headers)
        self.text_kwargs = text_kwargs or {***REMOVED***
        self.dedupe = dedupe
        self.extract_images = extract_images
    ...
    def lazy_load(
        ...
        if self.web_path and self._is_s3_url(self.web_path):
            ...
            if AmazonTextractPDFLoader._get_number_of_pages(blob) > 1:
                raise ValueError(
                    f"the file {blob.path***REMOVED*** is a multi-page document, \
...
***REMOVED***
        if blob.mimetype == "application/pdf":
            ...
            raise ValueError(f"unsupported mime type: {blob.mimetype***REMOVED***")
