...
class PydanticPluginConfig:
    ...
    def __init__(self, options: Options) -> None:
        ...
        if toml_config is not None:
            config = toml_config.get('tool', {***REMOVED***).get('pydantic-mypy', {***REMOVED***)
            for key in self.__slots__:
                setting = config.get(key, False)
                if not isinstance(setting, bool):
                    raise ValueError(f'Configuration value must be a boolean for key: {key***REMOVED***')
...
def from_orm_callback(ctx: MethodContext) -> Type:
    ...
    if isinstance(ctx.type, CallableType) and isinstance(ctx.type.ret_type, Instance):
        ...
        detail = f'ctx.type: {ctx.type***REMOVED*** (of type {ctx.type.__class__.__name__***REMOVED***)'
        error_unexpected_behavior(detail, ctx.api, ctx.context)
        return ctx.default_return_type
    pydantic_metadata = model_type.type.metadata.get(METADATA_KEY)
    if pydantic_metadata is None:
        return ctx.default_return_type
    orm_mode = pydantic_metadata.get('config', {***REMOVED***).get('orm_mode')
    ...
    def transform(self) -> None:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        info.metadata[METADATA_KEY] = {
            'fields': {field.name: field.serialize() for field in fields***REMOVED***,
...
***REMOVED***
    ...
    def add_construct_method(self, fields: List['PydanticModelField']) -> None:
        ...
        """
        ctx = self._ctx
        set_str = ctx.api.named_type(f'{BUILTINS_NAME***REMOVED***.set', [ctx.api.named_type(f'{BUILTINS_NAME***REMOVED***.str')])
        ...
        obj_type = ctx.api.named_type(f'{BUILTINS_NAME***REMOVED***.object')
    ...
    def get_config_update(self, substmt: AssignmentStmt) -> Optional['ModelConfigData']:
        ...
        if isinstance(substmt.rvalue, NameExpr) and substmt.rvalue.fullname in ('builtins.True', 'builtins.False'):
            return ModelConfigData(**{lhs.name: substmt.rvalue.fullname == 'builtins.True'***REMOVED***)
        error_invalid_config_value(lhs.name, self._ctx.api, substmt)
        return None
...
class ModelConfigData:
    ...
    def set_values_dict(self) -> Dict[str, Any]:
        return {k: v for k, v in self.__dict__.items() if v is not None***REMOVED***
...
def error_from_orm(model_name: str, api: CheckerPluginInterface, context: Context) -> None:
    api.fail(f'"{model_name***REMOVED***" does not have orm_mode=True', context, code=ERROR_ORM)


def error_invalid_config_value(name: str, api: SemanticAnalyzerPluginInterface, context: Context) -> None:
    api.fail(f'Invalid value for "Config.{name***REMOVED***"', context, code=ERROR_CONFIG)
...
def error_unexpected_behavior(detail: str, api: CheckerPluginInterface, context: Context) -> None:  # pragma: no cover
    ...
    full_message = f'The pydantic mypy plugin ran into unexpected behavior: {detail***REMOVED***\n'
    full_message += f'Please consider reporting this bug at {link***REMOVED*** so we can try to fix it!'
...
def add_method(
    ...
    function_type = ctx.api.named_type(f'{BUILTINS_NAME***REMOVED***.function')
