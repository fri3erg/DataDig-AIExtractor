...
class ReSTStyle(BaseStyle):
    ...
    def ref(self, title, link=None):
        if link is None:
            link = title
        self.doc.write(f':doc:`{title***REMOVED*** <{link***REMOVED***>`')

    def _heading(self, s, border_char):
        border = border_char * len(s)
        self.new_paragraph()
        self.doc.write(f'{border***REMOVED***\n{s***REMOVED***\n{border***REMOVED***')
        self.new_paragraph()
    ...
    def link_target_definition(self, refname, link):
        self.doc.writeln(f'.. _{refname***REMOVED***: {link***REMOVED***')

    def sphinx_reference_label(self, label, text=None):
        if text is None:
            text = label
        if self.doc.target == 'html':
            self.doc.write(f':ref:`{text***REMOVED*** <{label***REMOVED***>`')
        else:
            self.doc.write(text)
    ...
    def _clean_link_text(self):
        ...
        if last_write != '':
            ...
            if last_write.startswith('` '):
                last_write = f'`{last_write[1:].lstrip(" ")***REMOVED***'
    ...
    def external_link(self, title, link):
        if self.doc.target == 'html':
            self.doc.write(f'`{title***REMOVED*** <{link***REMOVED***>`_')
        else:
            self.doc.write(title)

    def internal_link(self, title, page):
        if self.doc.target == 'html':
            self.doc.write(f':doc:`{title***REMOVED*** <{page***REMOVED***>`')
        else:
            self.doc.write(title)
