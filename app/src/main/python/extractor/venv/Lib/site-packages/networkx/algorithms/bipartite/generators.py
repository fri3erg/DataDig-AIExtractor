...
def complete_bipartite_graph(n1, n2, create_using=None):
    """Returns the complete bipartite graph `K_{n_1,n_2***REMOVED***`.
    ...
    G.graph["name"] = f"complete_bipartite_graph({n1***REMOVED***, {n2***REMOVED***)"
    return G
...
def configuration_model(aseq, bseq, create_using=None, seed=None):
    ...
    if not suma == sumb:
        raise nx.NetworkXError(
            f"invalid degree sequences, sum(aseq)!=sum(bseq),{suma***REMOVED***,{sumb***REMOVED***"
        )
...
def preferential_attachment_graph(aseq, p, create_using=None, seed=None):
    ...
    if p > 1:
        raise nx.NetworkXError(f"probability {p***REMOVED*** > 1")
...
def random_graph(n, m, p, seed=None, directed=False):
    ...
    if directed:
        G = nx.DiGraph(G)
    G.name = f"fast_gnp_random_graph({n***REMOVED***,{m***REMOVED***,{p***REMOVED***)"
...
def gnmk_random_graph(n, m, k, seed=None, directed=False):
    """Returns a random bipartite graph G_{n,m,k***REMOVED***.
    ...
    This graph is a bipartite version of the `G_{nm***REMOVED***` random graph model.
    ...
    if directed:
        G = nx.DiGraph(G)
    G.name = f"bipartite_gnm_random_graph({n***REMOVED***,{m***REMOVED***,{k***REMOVED***)"
    if n == 1 or m == 1:
        return G
