...
def stack(arrays, axis=0, out=None, *, dtype=None, casting="same_kind"):
    ...
    shapes = {arr.shape for arr in arrays***REMOVED***
    if len(shapes) != 1:
...
def _block_format_index(index):
    ...
    """
    idx_str = ''.join('[{***REMOVED***]'.format(i) for i in index if i is not None)
        ...
        for index, ndim, size in idxs_ndims:
            ...
            if len(index) != len(first_index):
                raise ValueError(
                    ...
                    "{***REMOVED***, but there is an element at depth {***REMOVED*** ({***REMOVED***)".format(
                        len(first_index),
                        len(index),
...
def _concatenate_shapes(shapes, axis):
    ...
    if any(shape[:axis] != first_shape_pre or
        ...
        raise ValueError(
            'Mismatched array shapes in block along axis {***REMOVED***.'.format(axis))
...
def _block_setup(arrays):
    ...
    if bottom_index and bottom_index[-1] is None:
        raise ValueError(
            'List at {***REMOVED*** cannot be empty'.format(
