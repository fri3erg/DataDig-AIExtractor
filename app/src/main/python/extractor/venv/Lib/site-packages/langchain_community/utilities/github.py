...
***REMOVED***
        ...
        if not installation:
            raise ValueError(
                ...
                f"{github_app_id***REMOVED*** on the repo: {github_repository***REMOVED***"
...
***REMOVED***
    ...
    def parse_issues(self, issues: List[Issue]) -> List[dict]:
        ...
        for issue in issues:
            ...
            issue_dict = {"title": title, "number": number***REMOVED***
...
***REMOVED***
            ...
            if files:
                files_str = "\n".join(files)
                return f"Found {len(files)***REMOVED*** files in the main branch:\n{files_str***REMOVED***"
            else:
    ...
    def set_active_branch(self, branch_name: str) -> str:
        ...
        if branch_name in curr_branches:
            ...
            return f"Switched to branch `{branch_name***REMOVED***`"
        else:
            return (
                f"Error {branch_name***REMOVED*** does not exist,"
                f"in repo with current branches: {str(curr_branches)***REMOVED***"
***REMOVED***
    ...
    def list_branches_in_repo(self) -> str:
        ...
        try:
            ...
            if branches:
                branches_str = "\n".join(branches)
                return (
                    f"Found {len(branches)***REMOVED*** branches in the repository:"
                    f"\n{branches_str***REMOVED***"
    ***REMOVED***
            else:
        ...
        for i in range(1000):
            try:
                self.github_repo_instance.create_git_ref(
                    ref=f"refs/heads/{new_branch_name***REMOVED***", sha=base_branch.commit.sha
    ***REMOVED***
                self.active_branch = new_branch_name
                return (
                    f"Branch '{new_branch_name***REMOVED***' "
                    "created successfully, and set as current active branch."
    ***REMOVED***
            except GithubException as e:
                if e.status == 422 and "Reference already exists" in e.data["message"]:
                    i += 1
                    new_branch_name = f"{proposed_branch_name***REMOVED***_v{i***REMOVED***"
                else:
                    # Handle any other exceptions
                    print(f"Failed to create branch. Error: {e***REMOVED***")
                    raise Exception(
                        "Unable to create branch name from proposed_branch_name: "
                        f"{proposed_branch_name***REMOVED***"
        ***REMOVED***
        return (
            "Unable to create branch. "
            "At least 1000 branches exist with named derived from "
            f"proposed_branch_name: `{proposed_branch_name***REMOVED***`"
        )
...
***REMOVED***
            ...
            if files:
                files_str = "\n".join(files)
                return (
                    f"Found {len(files)***REMOVED*** files in branch `{self.active_branch***REMOVED***`:\n"
                    f"{files_str***REMOVED***"
    ***REMOVED***
            else:
                return f"No files found in branch: `{self.active_branch***REMOVED***`"
        except Exception as e:
            return f"Error: {e***REMOVED***"
...
***REMOVED***
            ...
            return f"Error: status code {e.status***REMOVED***, {e.message***REMOVED***"
    ...
    def get_issue(self, issue_number: int) -> Dict[str, Any]:
        ...
        while len(comments) <= 10:
            ...
            for comment in comments_page:
                comments.append({"body": comment.body, "user": comment.user.login***REMOVED***)
            page += 1
    ...
    def list_pull_request_files(self, pr_number: int) -> List[Dict[str, Any]]:
            ...
            for file in files_page:
                try:
                    ...
                    if file_metadata_response.status_code == 200:
                        ...
                        print(f"Failed to download file: {file.contents_url***REMOVED***, skipping")
                        continue
                    ...
                    if file_content_response.status_code == 200:
                        ...
                        print(
                            ...
                            f"(Error {file_content_response.status_code***REMOVED***). Skipping"
            ***REMOVED***
                        continue
        ...
        ***REMOVED***
            ...
            ***REMOVED***
                    ...
                    print(f"Error when reading files from a PR on github. {e***REMOVED***")
            page += 1
        return pr_files
    ...
    def get_pull_request(self, pr_number: int) -> Dict[str, Any]:
        ...
        response_dict: Dict[str, str] = {***REMOVED***
        ...
        while len(comments) <= 10:
            ...
            if len(comments_page) == 0:
                break
            for comment in comments_page:
                comment_str = str({"body": comment.body, "user": comment.user.login***REMOVED***)
        ...
        while len(commits) <= 10:
            ...
            for commit in commits_page:
                commit_str = str({"message": commit.commit.message***REMOVED***)
    ...
    ***REMOVED***
                return f"Successfully created PR number {pr.number***REMOVED***"
            except Exception as e:
    ...
    def create_file(self, file_query: str) -> str:
        ...
        if self.active_branch == self.github_base_branch:
            return (
                ...
                f"{self.github_base_branch***REMOVED*** branch, which is protected. "
...
***REMOVED***
    ...
    ***REMOVED***
                if file:
                    return (
                        f"File already exists at `{file_path***REMOVED***` "
                        f"on branch `{self.active_branch***REMOVED***`. You must use "
...
***REMOVED***
            ...
            return (
                f"File not found `{file_path***REMOVED***` on branch"
                f"`{self.active_branch***REMOVED***`. Error: {str(e)***REMOVED***"
***REMOVED***
    ...
    def update_file(self, file_query: str) -> str:
        ...
        if self.active_branch == self.github_base_branch:
            return (
                ...
                f"to the {self.github_base_branch***REMOVED*** branch, which is protected. "
                "Please create a new branch and try again."
***REMOVED***
        try:
...
***REMOVED***
    ...
    def search_issues_and_prs(self, query: str) -> str:
        ...
        results = [f"Top {max_items***REMOVED*** results:"]
        for issue in search_result[:max_items]:
            results.append(
                f"Title: {issue.title***REMOVED***, Number: {issue.number***REMOVED***, State: {issue.state***REMOVED***"
***REMOVED***
        return "\n".join(results)
    ...
    def search_code(self, query: str) -> str:
        ...
        results = [f"Showing top {max_results***REMOVED*** of {search_result.totalCount***REMOVED*** results:"]
        count = 0
...
***REMOVED***.decoded_content.decode()
            results.append(
                f"Filepath: `{code.path***REMOVED***`\nFile contents: "
                f"{file_content***REMOVED***\n<END OF FILE>"
***REMOVED***
            count += 1
        return "\n".join(results)
    ...
    def create_review_request(self, reviewer_username: str) -> str:
        ...
        if pr is None:
            return (
                ...
                f"current branch `{self.active_branch***REMOVED***`"
***REMOVED***

        try:
            pr.create_review_request(reviewers=[reviewer_username])
            return (
                f"Review request created for user {reviewer_username***REMOVED*** "
                f"on PR #{pr.number***REMOVED***"
***REMOVED***
        except Exception as e:
            return f"Failed to create a review request with error {e***REMOVED***"
