...
def cycle_basis(G, root=None):
    ...
    while gnodes:  # loop over connected components
        ...
        stack = [root]
        pred = {root: root***REMOVED***
        used = {root: set()***REMOVED***
...
def _bounded_cycle_search(G, path, length_bound):
    ...
    """
    G = _NeighborhoodCache(G)
    lock = {v: 0 for v in path***REMOVED***
    B = defaultdict(set)
    start = path[0]
    stack = [iter(G[path[-1]])]
    blen = [length_bound]
...
def chordless_cycles(G, length_bound=None):
    ...
    while components:
        ...
        components.extend(c for c in separate(F.subgraph(c - {v***REMOVED***)) if len(c) > 2)
...
def find_cycle(G, source=None, orientation=None):
    ...
    for start_node in G.nbunch_iter(source):
        ...
        seen = {start_node***REMOVED***
        # Nodes in active path.
        active_nodes = {start_node***REMOVED***
        previous_head = None
...
def _min_cycle_basis(G, weight):
    ...
    chords = G.edges - tree_edges - {(v, u) for u, v in tree_edges***REMOVED***

    # We maintain a set of vectors orthogonal to sofar found cycles
    set_orth = [{edge***REMOVED*** for edge in chords]
    while set_orth:
        base = set_orth.pop()
        ...
        set_orth = [
            (
                {e for e in orth if e not in base if e[::-1] not in base***REMOVED***
                | {e for e in base if e not in orth if e[::-1] not in orth***REMOVED***
***REMOVED***
...
def _min_cycle(G, orth, weight):
    ...
    lift = {n: spl(Gi, source=n, target=(n, 1), weight="Gi_weight") for n in G***REMOVED***
