...
_empty_metadata_singleton = types.MappingProxyType({***REMOVED***)
...
def _make_method(name, script, filename, globs):
    ...
    while True:
        ...
        filename = f"{base_filename[:-1]***REMOVED***-{count***REMOVED***>"
        count += 1
...
def _make_attr_tuple_class(cls_name, attr_names):
    ...
    """
    attr_class_name = f"{cls_name***REMOVED***Attributes"
    attr_class_template = [
        f"class {attr_class_name***REMOVED***(tuple):",
        "    __slots__ = ()",
    ]
    if attr_names:
        for i, attr_name in enumerate(attr_names):
            attr_class_template.append(
                f"    {attr_name***REMOVED*** = _attrs_property(_attrs_itemgetter({i***REMOVED***))"
***REMOVED***
    else:
        attr_class_template.append("    pass")
    globs = {"_attrs_itemgetter": itemgetter, "_attrs_property": property***REMOVED***
...
def _collect_base_attrs(cls, taken_attr_names):
    ...
    """
    base_attrs = []
    base_attr_map = {***REMOVED***  # A dictionary of base attrs to their classes.
...
***REMOVED***,
    ...
    if collect_by_mro:
        base_attrs, base_attr_map = _collect_base_attrs(
            cls, {a.name for a in own_attrs***REMOVED***
        )
    else:
    ...
    for a in (a for a in attrs if a.init is not False and a.kw_only is False):
        if had_default is True and a.default is NOTHING:
            msg = f"No mandatory attributes allowed after an attribute with a default value or factory.  Attribute in question: {a!r***REMOVED***"
            raise ValueError(msg)
    ...
    if original_getattr is not None:
        ...
        lines.extend(
            [
                ...
                "         original_error = f\"'{self.__class__.__name__***REMOVED***' object has no attribute '{item***REMOVED***'\"",
...
class _ClassBuilder:
    ...
    def __init__(
        ...
        self._cls = cls
        self._cls_dict = dict(cls.__dict__) if slots else {***REMOVED***
        self._attrs = attrs
        self._base_names = {a.name for a in base_attrs***REMOVED***
...
***REMOVED*** = self._make_getstate_setstate()

    def __repr__(self):
        return f"<_ClassBuilder(cls={self._cls.__name__***REMOVED***)>"
...
***REMOVED***
    ...
    def _make_getstate_setstate(self):
        ...
        def slots_getstate(self):
            ...
            """
            return {name: getattr(self, name) for name in state_attr_names***REMOVED***
...
***REMOVED***
    ...
    def _add_method_dunders(self, method):
        ...
        with contextlib.suppress(AttributeError):
            method.__qualname__ = f"{self._cls.__qualname__***REMOVED***.{method.__name__***REMOVED***"

        with contextlib.suppress(AttributeError):
            method.__doc__ = (
                "Method generated by attrs for class "
                f"{self._cls.__qualname__***REMOVED***."
***REMOVED***
...
def _generate_unique_filename(cls, func_name):
    ...
    """
    return (
        f"<attrs generated {func_name***REMOVED*** {cls.__module__***REMOVED***."
        f"{getattr(cls, '__qualname__', cls.__name__)***REMOVED***>"
    )
    ...
    def append_hash_computation_lines(prefix, indent):
        ...
        method_lines.extend(
            [
                ...
                indent + f"        {type_hash***REMOVED***,",
            ]
        )

        for a in attrs:
            if a.eq_key:
                cmp_name = f"_{a.name***REMOVED***_key"
                globs[cmp_name] = a.eq_key
                method_lines.append(
                    indent + f"        {cmp_name***REMOVED***(self.{a.name***REMOVED***),"
    ***REMOVED***
            else:
                method_lines.append(indent + f"        self.{a.name***REMOVED***,")

        method_lines.append(indent + "    " + closing_braces)

    if cache_hash:
        method_lines.append(tab + f"if self.{_hash_cache_field***REMOVED*** is None:")
        if frozen:
            append_hash_computation_lines(
                f"object.__setattr__(self, '{_hash_cache_field***REMOVED***', ", tab * 2
***REMOVED***
            method_lines.append(tab * 2 + ")")  # close __setattr__
        else:
            append_hash_computation_lines(
                f"self.{_hash_cache_field***REMOVED*** = ", tab * 2
***REMOVED***
        method_lines.append(tab + f"return self.{_hash_cache_field***REMOVED***")
    else:
    ...
    if attrs:
        ...
        for a in attrs:
            if a.eq_key:
                ...
                globs[cmp_name] = a.eq_key
                lines.append(f"        {cmp_name***REMOVED***(self.{a.name***REMOVED***),")
                others.append(f"        {cmp_name***REMOVED***(other.{a.name***REMOVED***),")
            else:
                lines.append(f"        self.{a.name***REMOVED***,")
                others.append(f"        other.{a.name***REMOVED***,")
...
def _make_repr(attrs, ns, cls):
    ...
    ***REMOVED***
    ...
    for name, r, i in attr_names_with_reprs:
        ...
        fragment = (
            "%s={%s!r***REMOVED***" % (name, accessor)
            if r == repr
            else "%s={%s_repr(%s)***REMOVED***" % (name, name, accessor)
        )
        attribute_fragments.append(fragment)
    repr_fragment = ", ".join(attribute_fragments)

    if ns is None:
        cls_name_fragment = '{self.__class__.__qualname__.rsplit(">.", 1)[-1]***REMOVED***'
    else:
        cls_name_fragment = ns + ".{self.__class__.__name__***REMOVED***"
    ...
    lines = [
        ...
        "    already_repring = {id(self),***REMOVED***",
        ...
        "  try:",
        f"    return f'{cls_name_fragment***REMOVED***({repr_fragment***REMOVED***)'",
        "  finally:",
    ...
    if attrs is None:
        if generic_base is not None:
            ...
            if attrs is not None:
                ...
                return attrs
        msg = f"{cls!r***REMOVED*** is not an attrs-decorated class."
...
def fields_dict(cls):
    ...
    if attrs is None:
        msg = f"{cls!r***REMOVED*** is not an attrs-decorated class."
        raise NotAnAttrsClassError(msg)
    return {a.name: a for a in attrs***REMOVED***
...
def _make_init(
    ...
    globs.update({"NOTHING": NOTHING, "attr_dict": attr_dict***REMOVED***)
...
def _setattr(attr_name, value_var, has_on_setattr):
    ...
    """
    return f"_setattr('{attr_name***REMOVED***', {value_var***REMOVED***)"
...
def _assign(attr_name, value, has_on_setattr):
    ...
    return f"self.{attr_name***REMOVED*** = {value***REMOVED***"
    ...
    if frozen is True:
        if slots is True:
            ...
            def fmt_setter(attr_name, value_var, has_on_setattr):
                ...
                return f"_inst_dict['{attr_name***REMOVED***'] = {value_var***REMOVED***"
...
***REMOVED***:
    ...
    ***REMOVED***
    ...
    names_for_globals = {***REMOVED***
    annotations = {"return": None***REMOVED***
    ...
    for a in attrs:
                ...
                if a.converter is not None:
                    lines.append(
                        fmt_setter_with_converter(
                            attr_name,
                            init_factory_name + f"({maybe_self***REMOVED***)",
                            has_on_setattr,
            ***REMOVED***
        ***REMOVED***
        ...
        ***REMOVED***
                ...
                lines.append(
                    fmt_setter_with_converter(
                        attr_name,
                        f"attr_dict['{attr_name***REMOVED***'].default",
                        has_on_setattr,
        ***REMOVED***
    ***REMOVED***
                conv_name = _init_converter_pat % (a.name,)
    ...
    ***REMOVED***
            ...
            arg = f"{arg_name***REMOVED***=attr_dict['{attr_name***REMOVED***'].default"
            if a.kw_only:
    ...
    ***REMOVED***
            ...
            arg = f"{arg_name***REMOVED***=NOTHING"
            if a.kw_only:
                kw_only_args.append(arg)
            else:
                args.append(arg)
            lines.append(f"if {arg_name***REMOVED*** is not NOTHING:")
    ...
    ***REMOVED***
    ...
    if attrs_to_validate:  # we can skip this if there are no validators.
        ...
        for a in attrs_to_validate:
            ...
            lines.append(f"    {val_name***REMOVED***(self, {attr_name***REMOVED***, self.{a.name***REMOVED***)")
    ...
    # initialization.
    if is_exc:
        vals = ",".join(f"self.{a.name***REMOVED***" for a in attrs if a.init)

        lines.append(f"BaseException.__init__(self, {vals***REMOVED***)")
    ...
    def from_counting_attr(cls, name, ca, type=None):
        ...
        inst_dict = {
            ...
            not in (
                ...
                "type",
                "inherited",
***REMOVED***  # exclude methods and deprecated alias
        ***REMOVED***
        return cls(
            name=name,
...
def make_class(
    ...
    if isinstance(attrs, dict):
        ...
        cls_dict = {a: attrib() for a in attrs***REMOVED***
    else:
    ...
    type_ = types.new_class(name, bases, {***REMOVED***, lambda ns: ns.update(body))
...
def pipe(*converters):
    ...
    *last* value.
    ...
    if not converters:
        ...
        A = typing.TypeVar("A")
        pipe_converter.__annotations__ = {"val": A, "return": A***REMOVED***
    else:
