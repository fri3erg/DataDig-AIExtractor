    ...
    def test_bad_param(self):
        ...
        assert_raises(ValueError, np.dtype,
                        {'names':['f0', 'f1'],
                         'formats':['i4', 'i1'],
                         'offsets':[0, 4],
                         'itemsize':9***REMOVED***, align=True)
        # If alignment is enabled, the individual fields must be aligned
        assert_raises(ValueError, np.dtype,
                        {'names':['f0', 'f1'],
                         'formats':['i1', 'f4'],
                         'offsets':[0, 2]***REMOVED***, align=True)
...
class TestRecord:
    ...
    def test_different_titles(self):
        ...
        b = np.dtype({'names': ['r', 'b'],
                      'formats': ['u1', 'u1'],
                      'titles': ['RRed pixel', 'Blue pixel']***REMOVED***)
        assert_dtype_not_equal(a, b)
    ...
    def test_refcount_dictionary_setting(self):
        ...
        refcounts = {k: sys.getrefcount(i) for k, i in d.items()***REMOVED***
        np.dtype(d)
        refcounts_new = {k: sys.getrefcount(i) for k, i in d.items()***REMOVED***
    ...
    def test_not_lists(self):
        ...
        assert_raises(TypeError, np.dtype,
                      dict(names={'A', 'B'***REMOVED***, formats=['f8', 'i4']))
        assert_raises(TypeError, np.dtype,
                      dict(names=['A', 'B'], formats={'f8', 'i4'***REMOVED***))
    ...
    def test_aligned_size(self):
        ...
        dt = np.dtype({'names':['f0', 'f1'],
                       'formats':['i4', 'u1'],
                       'offsets':[0, 4]***REMOVED***, align=True)
        assert_equal(dt.itemsize, 8)
        dt = np.dtype({'f0': ('i4', 0), 'f1':('u1', 4)***REMOVED***, align=True)
        assert_equal(dt.itemsize, 8)
        ...
        dt2 = np.dtype({'names':['f0', 'f1', 'f2'],
                       'formats':['i4',
                                  [('f1', 'i1'), ('f2', 'i4'), ('f3', 'i1')],
                                  'i1'],
                       'offsets':[0, 4, 16]***REMOVED***, align=True)
        assert_equal(dt2.itemsize, 20)
        dt3 = np.dtype({'f0': ('i4', 0),
                       'f1': ([('f1', 'i1'), ('f2', 'i4'), ('f3', 'i1')], 4),
                       'f2': ('i1', 16)***REMOVED***, align=True)
        ...
        dt2 = np.dtype({'names':['f0', 'f1', 'f2'],
                       'formats':['i4',
                                  [('f1', 'i1'), ('f2', 'i4'), ('f3', 'i1')],
                                  'i1'],
                       'offsets':[0, 4, 10]***REMOVED***, align=False)
        assert_equal(dt2.itemsize, 11)
        dt3 = np.dtype({'f0': ('i4', 0),
                       'f1': ([('f1', 'i1'), ('f2', 'i4'), ('f3', 'i1')], 4),
                       'f2': ('i1', 10)***REMOVED***, align=False)
    ...
    def test_union_struct(self):
        ...
        dt = np.dtype({'names':['f0', 'f1', 'f2'], 'formats':['<u4', '<u2', '<u2'],
                        'offsets':[0, 0, 2]***REMOVED***, align=True)
        ...
        dt = np.dtype({'names':['f0', 'f1', 'f2'], 'formats':['<u4', '<u2', '<u2'],
                        'offsets':[4, 0, 2]***REMOVED***, align=True)
        assert_equal(dt.itemsize, 8)
        ...
        dt = np.dtype({'names':['f0', 'f1'],
                       'formats':['i1', 'O'],
                       'offsets':[np.dtype('intp').itemsize, 0]***REMOVED***)
    ...
    def test_from_dictproxy(self):
        # Tests for PR #5920
        dt = np.dtype({'names': ['a', 'b'], 'formats': ['i4', 'f4']***REMOVED***)
    ...
    def test_from_dict_with_zero_width_field(self):
        ...
        dt2 = np.dtype({'names': ['val1', 'val2'],
                        'formats': [(np.float32, (0,)), int]***REMOVED***)
    ...
    def test_partial_dict(self):
        # 'names' is missing
        assert_raises(ValueError, np.dtype,
                {'formats': ['i4', 'i4'], 'f0': ('i4', 0), 'f1':('i4', 4)***REMOVED***)
...
class TestSubarray:
    ...
    def test_aligned_empty(self):
        ...
        assert dt == np.dtype([])
        dt = np.dtype({"names": [], "formats": [], "itemsize": 0***REMOVED***, align=True)
        assert dt == np.dtype([])
...
class TestStructuredDtypeSparseFields:
    ...
    dtype = np.dtype([('a', {'names':['aa', 'ab'], 'formats':['f', 'f'],
                             'offsets':[0, 4]***REMOVED***, (2, 3))])
    sparse_dtype = np.dtype([('a', {'names':['ab'], 'formats':['f'],
                                    'offsets':[4]***REMOVED***, (2, 3))])
...
class TestMetadata:
    ...
    def test_metadata_takes_dict(self):
        d = np.dtype(int, metadata={'datum': 1***REMOVED***)
        assert_(d.metadata == {'datum': 1***REMOVED***)
    ...
    def test_nested_metadata(self):
        d = np.dtype([('a', np.dtype(int, metadata={'datum': 1***REMOVED***))])
        assert_(d['a'].metadata == {'datum': 1***REMOVED***)

    def test_base_metadata_copied(self):
        d = np.dtype((np.void, np.dtype('i4,i4', metadata={'datum': 1***REMOVED***)))
        assert_(d.metadata == {'datum': 1***REMOVED***)
...
class TestString:
    def test_complex_dtype_str(self):
        ...
        dt = np.dtype({'names': ['r', 'g', 'b'], 'formats': ['u1', 'u1', 'u1'],
                        'offsets': [0, 1, 2],
                        'titles': ['Red pixel', 'Green pixel', 'Blue pixel']***REMOVED***)
        assert_equal(str(dt),
                    "[(('Red pixel', 'r'), 'u1'), "
                    "(('Green pixel', 'g'), 'u1'), "
                    "(('Blue pixel', 'b'), 'u1')]")
    ...
    def test_repr_structured_not_packed(self):
        dt = np.dtype({'names': ['rgba', 'r', 'g', 'b'],
                       ...
                       'offsets': [0, 0, 1, 2],
                       'titles': ['Color', 'Red pixel',
                                  'Green pixel', 'Blue pixel']***REMOVED***, align=True)
        assert_equal(repr(dt),
...
class TestPickling:
    ...
    def test_metadata(self):
        dt = np.dtype(int, metadata={'datum': 1***REMOVED***)
        self.check_pickling(dt)
...
class TestDTypeClasses:
    ...
    def test_integer_alias_names(self, int_, size):
        DType = getattr(numpy.dtypes, f"{int_***REMOVED***{size***REMOVED***DType")
        sctype = getattr(numpy, f"{int_.lower()***REMOVED***{size***REMOVED***")
...
class TestFromCTypes:
    ...
    def test_pairs(self, pair):
        ...
        pair_type = np.dtype('{***REMOVED***,{***REMOVED***'.format(*pair))
