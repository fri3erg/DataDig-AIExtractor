...
class TestCopy:
    ...
    def test_copy_cache(self):
        ...
        tm.assert_frame_equal(df, DataFrame({"a": [-1], "x": [0]***REMOVED***))

        df["y"] = [0]

        assert df["a"].values[0] == -1
        tm.assert_frame_equal(df, DataFrame({"a": [-1], "x": [0], "y": [0]***REMOVED***))
    ...
    def test_copy_consolidates(self):
        ...
        for i in range(10):
            df.loc[:, f"n_{i***REMOVED***"] = np.random.default_rng(2).integers(0, 100, size=55)
