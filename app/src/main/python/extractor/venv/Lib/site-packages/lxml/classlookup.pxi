    ...
    def __init__(self, *children, attrib=None, nsmap=None, **_extra):
        ...
        for child in children:
            if _isString(child):
                ...
                raise TypeError, f"Invalid child type: {type(child)!r***REMOVED***"
...
cdef int _validateNodeClass(xmlNode* c_node, cls) except -1:
    if c_node.type == tree.XML_ELEMENT_NODE:
        ...
        assert False, f"Unknown node type: {c_node.type***REMOVED***"

    if not (isinstance(cls, type) and issubclass(cls, expected)):
        raise TypeError(
            f"result of class lookup must be subclass of {type(expected)***REMOVED***, got {type(cls)***REMOVED***")
    return 0
...
cdef class AttributeBasedElementClassLookup(FallbackElementClassLookup):
    ...
    Arguments:
      - attribute name - '{ns***REMOVED***name' style string
