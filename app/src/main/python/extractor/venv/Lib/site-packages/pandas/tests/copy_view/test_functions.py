...
def test_concat_frames(using_copy_on_write):
    df = DataFrame({"b": ["a"] * 3***REMOVED***)
    df2 = DataFrame({"a": ["a"] * 3***REMOVED***)
    df_orig = df.copy()
...
def test_concat_frames_chained(using_copy_on_write):
    df1 = DataFrame({"a": [1, 2, 3], "b": [0.1, 0.2, 0.3]***REMOVED***)
    df2 = DataFrame({"c": [4, 5, 6]***REMOVED***)
    df3 = DataFrame({"d": [4, 5, 6]***REMOVED***)
...
def test_concat_mixed_series_frame(using_copy_on_write):
    df = DataFrame({"a": [1, 2, 3], "c": 1***REMOVED***)
...
def test_concat_copy_keyword(using_copy_on_write, copy):
    ...
    df2 = DataFrame({"b": [1.5, 2.5]***REMOVED***)
...
def test_merge_on_key(using_copy_on_write, func):
    df1 = DataFrame({"key": ["a", "b", "c"], "a": [1, 2, 3]***REMOVED***)
    df2 = DataFrame({"key": ["a", "b", "c"], "b": [4, 5, 6]***REMOVED***)
    df1_orig = df1.copy()
    df2_orig = df2.copy()
...
def test_merge_on_key_enlarging_one(using_copy_on_write, func, how):
    df1 = DataFrame({"key": ["a", "b", "c"], "a": [1, 2, 3]***REMOVED***)
    df2 = DataFrame({"key": ["a", "b"], "b": [4, 5]***REMOVED***)
    df1_orig = df1.copy()
    df2_orig = df2.copy()
...
def test_join_on_key(using_copy_on_write):
    ...
    df1 = DataFrame({"a": [1, 2, 3]***REMOVED***, index=df_index.copy(deep=True))
    df2 = DataFrame({"b": [4, 5, 6]***REMOVED***, index=df_index.copy(deep=True))
...
def test_join_multiple_dataframes_on_key(using_copy_on_write):
    ...
    df1 = DataFrame({"a": [1, 2, 3]***REMOVED***, index=df_index.copy(deep=True))
    dfs_list = [
        DataFrame({"b": [4, 5, 6]***REMOVED***, index=df_index.copy(deep=True)),
        DataFrame({"c": [7, 8, 9]***REMOVED***, index=df_index.copy(deep=True)),
    ]
