...
class TestDataFramePctChange:
    ...
    def test_pct_change_with_nas(
        ...
        msg = (
            ...
            f"{type(obj).__name__***REMOVED***.pct_change are deprecated"
        )
    ...
    def test_pct_change_shift_over_nas(self):
        ...
        edf = DataFrame({"a": expected, "b": expected***REMOVED***)
...
def test_pct_change_with_duplicated_indices(fill_method):
    # GH30463
    data = DataFrame(
        {0: [np.nan, 1, 2, 3, 9, 18], 1: [0, 1, np.nan, 3, 9, 18]***REMOVED***, index=["a", "b"] * 3
    )
    ...
    expected = DataFrame(
        {0: [np.nan, np.nan, 1.0, 0.5, 2.0, 1.0], 1: second_column***REMOVED***,
        index=["a", "b"] * 3,
    )
    tm.assert_frame_equal(result, expected)
...
def test_pct_change_none_beginning_no_warning():
    ...
    expected = DataFrame(
        {0: [np.nan, 1, 0.5, 1 / 3, 0.25], 1: [np.nan, np.nan, 1, 0.5, 1 / 3]***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
