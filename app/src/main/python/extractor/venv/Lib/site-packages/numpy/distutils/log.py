...
def set_verbosity(v, force=False):
    ...
    return {FATAL:-2,ERROR:-1,WARN:0,INFO:1,DEBUG:2***REMOVED***.get(prev_level, 1)
...
def error(msg, *a, **kw):
    _error(f"ERROR: {msg***REMOVED***", *a, **kw)


def warn(msg, *a, **kw):
    _warn(f"WARN: {msg***REMOVED***", *a, **kw)


def info(msg, *a, **kw):
    _info(f"INFO: {msg***REMOVED***", *a, **kw)


def debug(msg, *a, **kw):
    _debug(f"DEBUG: {msg***REMOVED***", *a, **kw)
