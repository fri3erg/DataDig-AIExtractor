...
class _Test_Utility:
    ...
    def _x2(self, intrin_name):
        return getattr(self.npyv, f"{intrin_name***REMOVED***_{self.sfx***REMOVED***x2")
    ...
    def _to_unsigned(self, vector):
        if isinstance(vector, (list, tuple)):
            ...
            if sfx[0] == "b":
                cvt_intrin = "cvt_u{0***REMOVED***_b{0***REMOVED***"
            else:
                cvt_intrin = "reinterpret_u{0***REMOVED***_{1***REMOVED***"
...
class _SIMD_BOOL(_Test_Utility):
    ...
    def _load_b(self, data):
        ...
        cvt = getattr(self.npyv, f"cvt_b{len_str***REMOVED***_u{len_str***REMOVED***")
    ...
    def test_pack(self):
        ...
        pack_simd = getattr(self.npyv, f"pack_b8_{self.sfx***REMOVED***")
...
class _SIMD_FP(_Test_Utility):
    ...
    def test_max_min(self, intrin):
        ...
        """
        pinf, ninf, nan = self._pinfinity(), self._ninfinity(), self._nan()
        chk_nan = {"xp": 1, "np": 1, "nn": 2, "xn": 2***REMOVED***.get(intrin[-2:], 0)
        func = eval(intrin[:3])
    ...
    ***REMOVED***
    ...
    def test_lut(self, intrin, table_size, elsize):
        ...
        if elsize != self._scalar_size():
            return
        intrin = eval(intrin)
        idx_itrin = getattr(self.npyv, f"setall_u{elsize***REMOVED***")
    ...
    def test_conversion_expand(self):
        ...
        expand = getattr(self.npyv, f"expand_{totype***REMOVED***_{self.sfx***REMOVED***")
...
for target_name, npyv in targets.items():
    ...
    if len(pretty_name) > 1:
        # multi-target
        pretty_name = f"({' '.join(pretty_name)***REMOVED***)"
    else:
    ...
    skip_sfx = dict()
    if not npyv:
        skip = f"target '{pretty_name***REMOVED***' isn't supported by current machine"
    elif not npyv.simd:
        skip = f"target '{pretty_name***REMOVED***' isn't supported by NPYV"
    else:
        if not npyv.simd_f32:
            skip_sfx["f32"] = f"target '{pretty_name***REMOVED***' "\
                               "doesn't support single-precision"
        if not npyv.simd_f64:
            skip_sfx["f64"] = f"target '{pretty_name***REMOVED***' doesn't"\
    ...
    for sfxes, cls in tests_registry.items():
        for sfx in sfxes:
            ...
            tcls = type(f"Test{cls.__name__***REMOVED***_{simd_width***REMOVED***_{target_name***REMOVED***_{sfx***REMOVED***", inhr, attr)
            if skip_m:
