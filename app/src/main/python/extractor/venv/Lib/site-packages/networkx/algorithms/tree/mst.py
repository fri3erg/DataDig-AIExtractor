    ...
    def best_edge(component):
        ...
        for e in nx.edge_boundary(G, component, data=True):
            ...
            if isnan(wt):
                if ignore_nan:
                    continue
                msg = f"NaN found as an edge weight. Edge {e***REMOVED***"
...
@nx._dispatch(
    edge_attrs={"weight": None, "partition": None***REMOVED***, preserve_edge_attrs="data"
)
    ...
    * Excluded edges
    ...
    for e in edges:
        ...
        if isnan(wt):
            if ignore_nan:
                continue
            raise ValueError(f"NaN found as an edge weight. Edge {e***REMOVED***")
        ...
        if is_multigraph:
            for v, keydict in G.adj[u].items():
                for k, d in keydict.items():
                    ...
                    if isnan(wt):
                        if ignore_nan:
                            continue
                        msg = f"NaN found as an edge weight. Edge {(u, v, k, d)***REMOVED***"
                        raise ValueError(msg)
            ...
            for v, d in G.adj[u].items():
                ...
                if isnan(wt):
                    if ignore_nan:
                        continue
                    msg = f"NaN found as an edge weight. Edge {(u, v, d)***REMOVED***"
                    raise ValueError(msg)
            ...
            if is_multigraph:
                for w, keydict in G.adj[v].items():
                    ...
                    for k2, d2 in keydict.items():
                        ...
                        if isnan(new_weight):
                            if ignore_nan:
                                continue
                            msg = f"NaN found as an edge weight. Edge {(v, w, k2, d2)***REMOVED***"
                            raise ValueError(msg)
                ...
                for w, d2 in G.adj[v].items():
                    ...
                    if isnan(new_weight):
                        if ignore_nan:
                            continue
                        msg = f"NaN found as an edge weight. Edge {(v, w, d2)***REMOVED***"
                        raise ValueError(msg)
...
def minimum_spanning_edges(
    ...
    try:
        ...
        msg = f"{algorithm***REMOVED*** is not a valid choice for an algorithm."
...
def minimum_spanning_tree(G, weight="weight", algorithm="kruskal", ignore_nan=False):
    ...
    [(0, 1, {***REMOVED***), (1, 2, {***REMOVED***), (2, 3, {***REMOVED***)]
...
def maximum_spanning_tree(G, weight="weight", algorithm="kruskal", ignore_nan=False):
    ...
    >>> sorted(T.edges(data=True))
    [(0, 1, {***REMOVED***), (0, 3, {'weight': 2***REMOVED***), (1, 2, {***REMOVED***)]
...
***REMOVED***
    ...
    ***REMOVED*** * nx.total_spanning_tree_weight(prepared_G_e) + G_e_total_tree_weight
                denominator = (
    ...
    ***REMOVED***
    ...
    raise Exception(f"Something went wrong! Only {len(U)***REMOVED*** edges in the spanning tree!")
...
***REMOVED***
    ...
    def __iter__(self):
        ...
        self.partition_queue.put(
            self.Partition(mst_weight if self.minimum else -mst_weight, {***REMOVED***)
        )
