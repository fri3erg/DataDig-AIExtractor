...
@pytest.mark.parametrize(
    "val", [1, 1.1, 1 + 1j, True, "abc", [1, 2], (1, 2), {1, 2***REMOVED***, {"a": 1***REMOVED***, None]
)
...
def test_array_equivalent_nested_dicts(strict_nan):
    left = np.array([{"f1": 1, "f2": np.array(["a", "b"], dtype=object)***REMOVED***], dtype=object)
    right = np.array(
    ...
    right2 = np.array([{"f1": 1, "f2": ["a", "b"]***REMOVED***], dtype=object)
