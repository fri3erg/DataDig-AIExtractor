...
def test_apply_categorical_func():
    # GH 9573
    df = DataFrame({"c0": ["A", "A", "B", "B"], "c1": ["C", "C", "D", "D"]***REMOVED***)
...
def test_apply_axis1_with_ea():
    # GH#36785
    expected = DataFrame({"A": [Timestamp("2013-01-01", tz="UTC")]***REMOVED***)
...
def test_apply_broadcast_scalars_axis1(float_frame):
    ...
    expected = DataFrame({c: m for c in float_frame.columns***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_apply_broadcast_lists_index(float_frame):
    ...
    expected = DataFrame(
        {c: m for c in float_frame.columns***REMOVED***,
        dtype="float64",
        index=float_frame.index,
    )
    tm.assert_frame_equal(result, expected)
...
def test_apply_mixed_dtype_corner():
    df = DataFrame({"A": ["foo"], "B": [1.0]***REMOVED***)
...
def test_apply_reduce_to_dict():
    ...
    expected = Series([{"i0": 1, "i1": 3***REMOVED***, {"i0": 2, "i1": 4***REMOVED***], index=data.columns)
    tm.assert_series_equal(result, expected)

    result = data.apply(dict, axis=1)
    expected = Series([{"c0": 1, "c1": 2***REMOVED***, {"c0": 3, "c1": 4***REMOVED***], index=data.index)
    tm.assert_series_equal(result, expected)
...
def test_apply_differently_indexed():
    ...
    expected = DataFrame({i: v.describe() for i, v in df.items()***REMOVED***, columns=df.columns)
    tm.assert_frame_equal(result, expected)

    result = df.apply(Series.describe, axis=1)
    expected = DataFrame({i: v.describe() for i, v in df.T.items()***REMOVED***, columns=df.index).T
    tm.assert_frame_equal(result, expected)
...
def test_apply_multi_index():
    ...
    result = s.apply(lambda x: Series({"min": min(x), "max": max(x)***REMOVED***), 1)
...
@pytest.mark.parametrize(
    ...
    [
        [
            ...
            Series([{0: "foo", 1: "spam"***REMOVED***, {0: "bar", 1: "eggs"***REMOVED***]),
        ],
        [DataFrame([[0, 1], [2, 3]]), Series([{0: 0, 1: 2***REMOVED***, {0: 1, 1: 3***REMOVED***])],
    ],
)
...
def test_apply_non_numpy_dtype():
    # GH 12244
    df = DataFrame({"dt": date_range("2015-01-01", periods=3, tz="Europe/Brussels")***REMOVED***)
    result = df.apply(lambda x: x)
    tm.assert_frame_equal(result, df)

    result = df.apply(lambda x: x + pd.Timedelta("1day"))
    expected = DataFrame(
        {"dt": date_range("2015-01-02", periods=3, tz="Europe/Brussels")***REMOVED***
    )
    tm.assert_frame_equal(result, expected)


def test_apply_non_numpy_dtype_category():
    df = DataFrame({"dt": ["a", "b", "c", "a"]***REMOVED***, dtype="category")
    result = df.apply(lambda x: x)
    tm.assert_frame_equal(result, df)
...
def test_apply_noreduction_tzaware_object():
    ...
    expected = DataFrame(
        {"foo": [Timestamp("2020", tz="UTC")]***REMOVED***, dtype="datetime64[ns, UTC]"
    )
...
def test_apply_category_equalness(val):
    ...
    df = DataFrame({"a": df_values***REMOVED***, dtype="category")
...
@pytest.mark.parametrize(
    "ops, by_row, expected",
    [
        ({"a": lambda x: x + 1***REMOVED***, "compat", DataFrame({"a": [2, 3]***REMOVED***)),
        ({"a": lambda x: x + 1***REMOVED***, False, DataFrame({"a": [2, 3]***REMOVED***)),
        ({"a": lambda x: x.sum()***REMOVED***, "compat", Series({"a": 3***REMOVED***)),
        ({"a": lambda x: x.sum()***REMOVED***, False, Series({"a": 3***REMOVED***)),
        (
            {"a": ["sum", np.sum, lambda x: x.sum()]***REMOVED***,
            "compat",
            DataFrame({"a": [3, 3, 3]***REMOVED***, index=["sum", "sum", "<lambda>"]),
        ),
        (
            {"a": ["sum", np.sum, lambda x: x.sum()]***REMOVED***,
            False,
        ...
        ({"a": lambda x: 1***REMOVED***, "compat", DataFrame({"a": [1, 1]***REMOVED***)),
        ({"a": lambda x: 1***REMOVED***, False, Series({"a": 1***REMOVED***)),
    ],
)
...
@pytest.mark.parametrize(
    "ops",
    [
        {"a": lambda x: x + 1***REMOVED***,
        {"a": lambda x: x.sum()***REMOVED***,
        {"a": ["sum", np.sum, lambda x: x.sum()]***REMOVED***,
        {"a": lambda x: 1***REMOVED***,
    ],
)
...
def test_with_dictlike_columns():
    ...
    result = df.apply(lambda x: {"s": x["a"] + x["b"]***REMOVED***, axis=1)
    expected = Series([{"s": 3***REMOVED*** for t in df.itertuples()])
    tm.assert_series_equal(result, expected)
    ...
    # compose a series
    result = (df["a"] + df["b"]).apply(lambda x: {"s": x***REMOVED***)
    expected = Series([{"s": 3***REMOVED***, {"s": 3***REMOVED***])
    tm.assert_series_equal(result, expected)
...
def test_with_dictlike_columns_with_datetime():
    ...
    result = df.apply(lambda x: {***REMOVED***, axis=1)
    expected = Series([{***REMOVED***, {***REMOVED***, {***REMOVED***])
    tm.assert_series_equal(result, expected)
...
def test_with_dictlike_columns_with_infer():
    # GH 17602
    df = DataFrame([[1, 2], [1, 2]], columns=["a", "b"])
    result = df.apply(lambda x: {"s": x["a"] + x["b"]***REMOVED***, axis=1, result_type="expand")
    expected = DataFrame({"s": [3, 3]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    "ops, by_row, expected",
    [
        ([lambda x: x + 1], "compat", DataFrame({("a", "<lambda>"): [2, 3]***REMOVED***)),
        ([lambda x: x + 1], False, DataFrame({("a", "<lambda>"): [2, 3]***REMOVED***)),
        ([lambda x: x.sum()], "compat", DataFrame({"a": [3]***REMOVED***, index=["<lambda>"])),
        ([lambda x: x.sum()], False, DataFrame({"a": [3]***REMOVED***, index=["<lambda>"])),
        (
        ...
        (
            ...
            False,
            DataFrame({"a": [2, 3]***REMOVED***, ["<lambda>", "<lambda>"]),
        ),
    ],
)
...
def test_with_listlike_columns_returning_list():
    # GH 18919
    df = DataFrame({"x": Series([["a", "b"], ["q"]]), "y": Series([["z"], ["q", "t"]])***REMOVED***)
...
def test_infer_output_shape_listlike_columns_np_func(val):
    # GH 17970
    df = DataFrame({"a": [1, 2, 3]***REMOVED***, index=list("abc"))
...
def test_consistent_names(int_frame_const_col):
    ...
    expected = int_frame_const_col.rename(
        columns={"A": "test", "B": "other", "C": "cols"***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
...
def test_consistency_for_boxed(box, int_frame_const_col):
    ...
    expected = int_frame_const_col[["A", "B"]].rename(columns={"A": 0, "B": 1***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_agg_transform(axis, float_frame):
    other_axis = 1 if axis in {0, "index"***REMOVED*** else 0
...
def test_demo():
    # demonstration tests
    df = DataFrame({"A": range(5), "B": 5***REMOVED***)

    result = df.agg(["min", "max"])
    expected = DataFrame(
        {"A": [0, 4], "B": [5, 5]***REMOVED***, columns=["A", "B"], index=["min", "max"]
    )
    tm.assert_frame_equal(result, expected)
...
def test_demo_dict_agg():
    # demonstration tests
    df = DataFrame({"A": range(5), "B": 5***REMOVED***)
    result = df.agg({"A": ["min", "max"], "B": ["sum", "max"]***REMOVED***)
    expected = DataFrame(
        {"A": [4.0, 0.0, np.nan], "B": [5.0, np.nan, 25.0]***REMOVED***,
        columns=["A", "B"],
        index=["max", "min", "sum"],
    )
...
def test_agg_with_name_as_column_name():
    ...
    data = {"name": ["foo", "bar"]***REMOVED***
    df = DataFrame(data)

    # result's name should be None
    result = df.agg({"name": "count"***REMOVED***)
    expected = Series({"name": 2***REMOVED***)
    tm.assert_series_equal(result, expected)

    # Check if name is still preserved when aggregating series instead
    result = df["name"].agg({"name": "count"***REMOVED***)
    expected = Series({"name": 2***REMOVED***, name="name")
    tm.assert_series_equal(result, expected)
...
def test_agg_reduce(axis, float_frame):
    ...
    expected = expected.T if axis in {0, "index"***REMOVED*** else expected

    result = float_frame.agg(["mean", "max", "sum"], axis=axis)
    tm.assert_frame_equal(result, expected)

    # dict input with scalars
    func = {name1: "mean", name2: "sum"***REMOVED***
    ...
    expected = expected.T if axis in {1, "columns"***REMOVED*** else expected
    tm.assert_frame_equal(result, expected)

    # dict input with lists with multiple
    func = {name1: ["mean", "sum"], name2: ["sum", "max"]***REMOVED***
    result = float_frame.agg(func, axis=axis)
    expected = pd.concat(
        {
            name1: Series(
...
def test_non_callable_aggregates(how):
    ...
    df = DataFrame(
        {"A": [None, 2, 3], "B": [1.0, np.nan, 3.0], "C": ["foo", None, "bar"]***REMOVED***
    )

    # Function aggregate
    result = getattr(df, how)({"A": "count"***REMOVED***)
    expected = Series({"A": 2***REMOVED***)

    tm.assert_series_equal(result, expected)

    # Non-function aggregate
    result = getattr(df, how)({"A": "size"***REMOVED***)
    expected = Series({"A": 3***REMOVED***)
    ...
    result2 = getattr(df, how)(
        {"A": ["count", "size"], "B": ["count", "size"], "C": ["count", "size"]***REMOVED***
    )
    expected = DataFrame(
        {
            "A": {"count": 2, "size": 3***REMOVED***,
            "B": {"count": 2, "size": 3***REMOVED***,
            "C": {"count": 2, "size": 3***REMOVED***,
        ***REMOVED***
    )
...
def test_agg_listlike_result():
    ...
    df = DataFrame({"A": [2, 2, 3], "B": [1.5, np.nan, 1.5], "C": ["foo", None, "bar"]***REMOVED***)
...
@pytest.mark.parametrize("df", [DataFrame({"A": ["a", None], "B": ["c", "d"]***REMOVED***)])
...
def test_apply_mutating(using_array_manager, using_copy_on_write, warn_copy_on_write):
    ...
    df = DataFrame({"a": range(100), "b": range(100, 200)***REMOVED***)
    df_orig = df.copy()
...
def test_apply_empty_list_reduce():
    ...
    expected = Series({"a": [], "b": []***REMOVED***, dtype=object)
    tm.assert_series_equal(result, expected)
...
def test_apply_no_suffix_index(engine, request):
    ...
    expected = DataFrame(
        {"A": [12, 12, 12], "B": [27, 27, 27]***REMOVED***, index=["sum", "<lambda>", "<lambda>"]
    )
...
def test_apply_getitem_axis_1(engine, request):
    ...
    if engine == "numba":
        ...
        request.node.add_marker(mark)
    df = DataFrame({"a": [0, 1, 2], "b": [1, 2, 3]***REMOVED***)
    result = df[["a", "a"]].apply(
...
def test_apply_type():
    # GH 46719
    df = DataFrame(
        {"col1": [3, "string", float], "col2": [0.25, datetime(2020, 1, 1), np.nan]***REMOVED***,
        index=["a", "b", "c"],
    )

    # axis=0
    result = df.apply(type, axis=0)
    expected = Series({"col1": Series, "col2": Series***REMOVED***)
    tm.assert_series_equal(result, expected)

    # axis=1
    result = df.apply(type, axis=1)
    expected = Series({"a": Series, "b": Series, "c": Series***REMOVED***)
    tm.assert_series_equal(result, expected)


def test_apply_on_empty_dataframe(engine):
    # GH 39111
    df = DataFrame({"a": [1, 2], "b": [3, 0]***REMOVED***)
...
def test_apply_return_list():
    df = DataFrame({"a": [1, 2], "b": [2, 3]***REMOVED***)
    result = df.apply(lambda x: [x.values])
    expected = DataFrame({"a": [[1, 2]], "b": [[2, 3]]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    "test, constant",
    [
        ({"a": [1, 2, 3], "b": [1, 1, 1]***REMOVED***, {"a": [1, 2, 3], "b": [1]***REMOVED***),
        ({"a": [2, 2, 2], "b": [1, 1, 1]***REMOVED***, {"a": [2], "b": [1]***REMOVED***),
    ],
)
def test_unique_agg_type_is_series(test, constant):
    ...
    aggregation = {"a": "unique", "b": "unique"***REMOVED***
...
def test_any_apply_keyword_non_zero_axis_regression():
    ...
    df = DataFrame({"A": [1, 2, 0], "B": [0, 2, 0], "C": [0, 0, 0]***REMOVED***)
...
def test_agg_mapping_func_deprecated():
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = df.agg({"x": foo1***REMOVED***, 0, 3, c=4)
...
def test_agg_std():
    ...
    expected = Series({"A": 2.0, "B": 2.0***REMOVED***, dtype=float)
    tm.assert_series_equal(result, expected)

    with tm.assert_produces_warning(FutureWarning, match="using Series.std"):
        result = df.agg([np.std])
    expected = DataFrame({"A": 2.0, "B": 2.0***REMOVED***, index=["std"])
    tm.assert_frame_equal(result, expected)
