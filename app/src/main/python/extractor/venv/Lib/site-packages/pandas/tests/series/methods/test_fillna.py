...
class TestSeriesFillNA:
    def test_fillna_nat(self):
        ...
        expected = DataFrame({"A": expected***REMOVED***)
    ...
    def test_fillna_nonscalar(self):
        ...
        result = s1.fillna({***REMOVED***)
        tm.assert_series_equal(result, s1)
        result = s1.fillna(Series((), dtype=object))
        tm.assert_series_equal(result, s1)
        result = s2.fillna(s1)
        tm.assert_series_equal(result, s2)
        result = s1.fillna({0: 1***REMOVED***)
        tm.assert_series_equal(result, expected)
        result = s1.fillna({1: 1***REMOVED***)
        tm.assert_series_equal(result, Series([np.nan]))
        result = s1.fillna({0: 1, 1: 1***REMOVED***)
        tm.assert_series_equal(result, expected)
        result = s1.fillna(Series({0: 1, 1: 1***REMOVED***))
        tm.assert_series_equal(result, expected)
        result = s1.fillna(Series({0: 1, 1: 1***REMOVED***, index=[4, 5]))
        tm.assert_series_equal(result, s1)
    ...
    def test_fillna_downcast(self):
        ...
        ser = Series([1.0, np.nan])
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = ser.fillna({1: 0***REMOVED***, downcast="infer")
        expected = Series([1, 0])
        tm.assert_series_equal(result, expected)
...
***REMOVED***,
    ...
    def test_datetime64_tz_fillna(self, tz, unit):
        ...
        result = ser.fillna(
            {1: Timestamp("2011-01-02 10:00"), 3: Timestamp("2011-01-04 10:00")***REMOVED***
        )
        expected = Series(
            [
                Timestamp("2011-01-01 10:00"),
        ...
        ser = Series(idx)
        assert ser.dtype == f"datetime64[{unit***REMOVED***, {tz***REMOVED***]"
        tm.assert_series_equal(isna(ser), null_loc)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("a", ["a", "a", "b", "a", "a"]),
            ({1: "a", 3: "b", 4: "b"***REMOVED***, ["a", "a", "b", "b", "b"]),
            ({1: "a"***REMOVED***, ["a", "a", "b", np.nan, np.nan]),
            ({1: "a", 3: "b"***REMOVED***, ["a", "a", "b", "b", np.nan]),
            (Series("a"), ["a", np.nan, "b", np.nan, np.nan]),
            (Series("a", index=[1]), ["a", "a", "b", np.nan, np.nan]),
            (Series({1: "a", 3: "b"***REMOVED***), ["a", "a", "b", "b", np.nan]),
...
***REMOVED***,
    ...
    def test_fillna_categorical_raises(self):
        ...
        with pytest.raises(TypeError, match=msg):
            ser.fillna(DataFrame({1: ["a"], 3: ["b"]***REMOVED***))
