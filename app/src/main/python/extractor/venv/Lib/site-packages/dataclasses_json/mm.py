...
class _UnionField(fields.Field):
    ...
    def _serialize(self, value, attr, obj, **kwargs):
        ...
        for type_, schema_ in self.desc.items():
            ...
            warnings.warn(
                f'The type "{type(value).__name__***REMOVED***" (value: "{value***REMOVED***") '
                f'is not in the list of possible types of typing.Union '
                f'(dataclass: {self.cls.__name__***REMOVED***, field: {self.field.name***REMOVED***). '
    ...
    def _deserialize(self, value, attr, data, **kwargs):
        ...
        if isinstance(tmp_value, dict) and '__type' in tmp_value:
            ...
            warnings.warn(
                f'Attempting to deserialize "dict" (value: "{tmp_value***REMOVED***) '
...
***REMOVED***
            ...
            for type_, schema_ in self.desc.items():
                ...
                warnings.warn(
                    f'The type "{type(tmp_value).__name__***REMOVED***" (value: "{tmp_value***REMOVED***") '
                    f'is not in the list of possible types of typing.Union '
                    f'(dataclass: {self.cls.__name__***REMOVED***, field: {self.field.name***REMOVED***). '
    ...
    def inner(type_, options):
        ...
        if is_dataclass(type_):
            if _issubclass_safe(type_, mixin):
                ...
                warnings.warn(f"Nested dataclass field {field.name***REMOVED*** of type "
                              f"{field.type***REMOVED*** detected in "
                              f"{cls.__name__***REMOVED*** that is not an instance of "
                              f"dataclass_json. Did you mean to recursively "
                              f"serialize this field? If so, make sure to "
                              f"augment {type_***REMOVED*** with either the "
                              f"`dataclass_json` decorator or mixin.")
                return fields.Field(**options)

        origin = getattr(type_, '__origin__', type_)
        args = [inner(a, {***REMOVED***) for a in getattr(type_, '__args__', []) if
                a is not type(None)]
        ...
        warnings.warn(
            f"Unknown type {type_***REMOVED*** at {cls.__name__***REMOVED***.{field.name***REMOVED***: {field.type***REMOVED*** "
...
def schema(cls, mixin, infer_missing):
    ...
    for field in dc_fields(cls):
        ...
        if metadata.mm_field is not None:
            ...
            type_ = field.type
            options: typing.Dict[str, typing.Any] = {***REMOVED***
            ...
            if field.metadata.get('dataclasses_json', {***REMOVED***).get('decoder'):
    ...
    def dump(self, obj, *, many=None):
        ...
        if many:
            for i, _obj in enumerate(obj):
                dumped[i].update(
                    _handle_undefined_parameters_safe(cls=_obj, kvs={***REMOVED***,
                                                      usage="dump"))
        else:
            dumped.update(_handle_undefined_parameters_safe(cls=obj, kvs={***REMOVED***,
                                                            usage="dump"))
        return dumped

    schema_ = schema(cls, mixin, infer_missing)
    DataClassSchema: typing.Type["SchemaType[A]"] = type(
        f'{cls.__name__.capitalize()***REMOVED***Schema',
        (Schema,),
        {'Meta': Meta,
         f'make_{cls.__name__.lower()***REMOVED***': make_instance,
         'dumps': dumps,
         'dump': dump,
         **schema_***REMOVED***)
