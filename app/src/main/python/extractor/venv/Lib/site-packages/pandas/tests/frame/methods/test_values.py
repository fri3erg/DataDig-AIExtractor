...
class TestDataFrameValues:
    ...
    def test_values_mixed_dtypes(self, float_frame, float_string_frame):
        ...
        df = DataFrame({"complex": [1j, 2j, 3j], "real": [1, 2, 3]***REMOVED***)
        arr = df.values
        assert arr[0, 0] == 1j
    ...
    def test_values_casts_datetimelike_to_object(self, constructor):
        ...
        df = DataFrame(
            {"a": series, "b": np.random.default_rng(2).standard_normal(len(series))***REMOVED***
        )

        result = df.values.squeeze()
        assert (result[:, 0] == expected.values).all()

        df = DataFrame({"a": series, "b": ["foo"] * len(series)***REMOVED***)
    ...
    def test_frame_values_with_tz(self):
        tz = "US/Central"
        df = DataFrame({"A": date_range("2000", periods=4, tz=tz)***REMOVED***)
        result = df.values
        expected = np.array(
            [
...
class TestPrivateValues:
    ...
    def test_private_values_dt64_multiblock(self):
        ...
        df = DataFrame({"A": dta[:4]***REMOVED***, copy=False)
        df["B"] = dta[4:]
