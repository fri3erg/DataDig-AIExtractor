...
class TestDataFrameConcat:
    ...
    def test_concat_named_keys(self):
        # GH#14252
        df = DataFrame({"foo": [1, 2], "bar": [0.1, 0.2]***REMOVED***)
        index = Index(["a", "b"], name="baz")
        concatted_named_from_keys = concat([df, df], keys=index)
        expected_named = DataFrame(
            {"foo": [1, 2, 1, 2], "bar": [0.1, 0.2, 0.1, 0.2]***REMOVED***,
    ...
    def test_concat_axis_parameter(self):
        # GH#14369
        df1 = DataFrame({"A": [0.1, 0.2]***REMOVED***, index=range(2))
        df2 = DataFrame({"A": [0.3, 0.4]***REMOVED***, index=range(2))

        # Index/row/0 DataFrame
        expected_index = DataFrame({"A": [0.1, 0.2, 0.3, 0.4]***REMOVED***, index=[0, 1, 0, 1])
...
***REMOVED***,
    ...
    def test_concat_dataframe_keys_bug(self, sort):
        t1 = DataFrame(
            {"value": Series([1, 2, 3], index=Index(["a", "b", "c"], name="id"))***REMOVED***
        )
        t2 = DataFrame({"value": Series([7, 8], index=Index(["a", "b"], name="id"))***REMOVED***)
    ...
    def test_outer_sort_columns(self):
        # GH#47127
        df1 = DataFrame({"A": [0], "B": [1], 0: 1***REMOVED***)
        df2 = DataFrame({"A": [100]***REMOVED***)
        result = concat([df1, df2], ignore_index=True, join="outer", sort=True)
        expected = DataFrame({0: [1.0, np.nan], "A": [0, 100], "B": [1.0, np.nan]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_inner_sort_columns(self):
        # GH#47127
        df1 = DataFrame({"A": [0], "B": [1], 0: 1***REMOVED***)
        df2 = DataFrame({"A": [100], 0: 2***REMOVED***)
        result = concat([df1, df2], ignore_index=True, join="inner", sort=True)
        expected = DataFrame({0: [1, 2], "A": [0, 100]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_sort_columns_one_df(self):
        # GH#47127
        df1 = DataFrame({"A": [100], 0: 2***REMOVED***)
        result = concat([df1], ignore_index=True, join="inner", sort=True)
        expected = DataFrame({0: [2], "A": [100]***REMOVED***)
        tm.assert_frame_equal(result, expected)
