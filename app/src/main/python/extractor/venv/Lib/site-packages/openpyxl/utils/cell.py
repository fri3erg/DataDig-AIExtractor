...
# constants
COORD_RE = re.compile(r'^[$]?([A-Za-z]{1,3***REMOVED***)[$]?(\d+)$')
COL_RANGE = """[A-Z]{1,3***REMOVED***:[A-Z]{1,3***REMOVED***:"""
ROW_RANGE = r"""\d+:\d+:"""
RANGE_EXPR = r"""
[$]?(?P<min_col>[A-Za-z]{1,3***REMOVED***)?
[$]?(?P<min_row>\d+)?
(:[$]?(?P<max_col>[A-Za-z]{1,3***REMOVED***)?
[$]?(?P<max_row>\d+)?)?
"""
ABSOLUTE_RE = re.compile('^' + RANGE_EXPR +'$', re.VERBOSE)
SHEET_TITLE = r"""
(('(?P<quoted>([^']|'')*)')|(?P<notquoted>[^'^ ^!]*))!"""
SHEETRANGE_RE = re.compile("""{0***REMOVED***(?P<cells>{1***REMOVED***)(?=,?)""".format(
...
def coordinate_from_string(coord_string):
    ...
    if not match:
        msg = f"Invalid cell coordinates ({coord_string***REMOVED***)"
        raise CellCoordinatesException(msg)
    column, row = match.groups()
    row = int(row)
    if not row:
        msg = f"There is no row 0 ({coord_string***REMOVED***)"
        raise CellCoordinatesException(msg)
    return column, row
...
def absolute_coordinate(coord_string):
    ...
    if not m:
        raise ValueError(f"{coord_string***REMOVED*** is not a valid coordinate range")
    ...
    if d['max_col'] or d['max_row']:
        fmt = "{min_col***REMOVED***{min_row***REMOVED***:{max_col***REMOVED***{max_row***REMOVED***"
    else:
        fmt = "{min_col***REMOVED***{min_row***REMOVED***"
    return fmt.format(**d)
...
def _get_column_letter(col_idx):
    ...
    if not 1 <= col_idx <= 18278:
        raise ValueError("Invalid column index {0***REMOVED***".format(col_idx))
    letters = []
    while col_idx > 0:
...
_COL_STRING_CACHE = {***REMOVED***
_STRING_COL_CACHE = {***REMOVED***
...
def get_column_letter(idx,):
    ...
    try:
        ...
        raise ValueError("Invalid column index {0***REMOVED***".format(idx))
...
def column_index_from_string(str_col):
    ...
    try:
        ...
        raise ValueError("{0***REMOVED*** is not a valid column name".format(str_col))
...
def range_boundaries(range_string):
    ...
    """
    msg = "{0***REMOVED*** is not a valid coordinate or range".format(range_string)
...
def rows_from_range(range_string):
    ...
    for row in rows:
        yield tuple('{0***REMOVED***{1***REMOVED***'.format(col, row) for col in cols)
...
def cols_from_range(range_string):
    ...
    cols = (get_column_letter(col) for col in range(min_col, max_col+1))
    for col in cols:
        yield tuple('{0***REMOVED***{1***REMOVED***'.format(col, row) for row in rows)
...
def quote_sheetname(sheetname):
    ...
    sheetname = u"'{0***REMOVED***'".format(sheetname)
    return sheetname
