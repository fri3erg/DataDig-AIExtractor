...
def guard_import(
    ...
    try:
        ...
        raise ImportError(
            f"Could not import {module_name***REMOVED*** python package. "
            f"Please install it with `pip install {pip_name or module_name***REMOVED***`."
        )
    return module
...
def check_package_version(
    ...
    if lt_version is not None and imported_version >= parse(lt_version):
        raise ValueError(
            f"Expected {package***REMOVED*** version to be < {lt_version***REMOVED***. Received "
            f"{imported_version***REMOVED***."
        )
    if lte_version is not None and imported_version > parse(lte_version):
        raise ValueError(
            f"Expected {package***REMOVED*** version to be <= {lte_version***REMOVED***. Received "
            f"{imported_version***REMOVED***."
        )
    if gt_version is not None and imported_version <= parse(gt_version):
        raise ValueError(
            f"Expected {package***REMOVED*** version to be > {gt_version***REMOVED***. Received "
            f"{imported_version***REMOVED***."
        )
    if gte_version is not None and imported_version < parse(gte_version):
        raise ValueError(
            f"Expected {package***REMOVED*** version to be >= {gte_version***REMOVED***. Received "
            f"{imported_version***REMOVED***."
        )
