...
class TestEmptyFrameSetitemExpansion:
    def test_empty_frame_setitem_index_name_retained(self):
        ...
        df = DataFrame({***REMOVED***, index=pd.RangeIndex(0, name="df_index"))
        series = Series(1.23, index=pd.RangeIndex(4, name="series_index"))

        df["series"] = series
        expected = DataFrame(
            {"series": [1.23] * 4***REMOVED***,
    ...
    def test_partial_set_empty_frame_no_index(self):
        # no index to start
        expected = DataFrame({0: Series(1, index=range(4))***REMOVED***, columns=["A", "B", 0])
    ...
    def test_partial_set_empty_frame_row(self):
        ...
        df = DataFrame({"A": [1, 2, 3], "B": [1.2, 4.2, 5.2]***REMOVED***)
        y = df[df.A > 5]
        y["New"] = np.nan
    ...
    def test_partial_set_empty_frame_set_series(self):
        ...
        expected = DataFrame({0: Series(dtype=object)***REMOVED***)
        tm.assert_frame_equal(df, expected)

        df = DataFrame(Series(name="foo", dtype=object))
        expected = DataFrame({"foo": Series(dtype=object)***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_partial_set_empty_frame_empty_consistencies(self, using_infer_string):
        ...
        df["x"] = [1, 2]
        expected = DataFrame({"x": [1, 2], "y": [np.nan, np.nan]***REMOVED***)
    ...
    ***REMOVED***,
        ...
        df = DataFrame(columns=["x", "y"])
        df.loc[0, "x"] = 1
        expected = DataFrame({"x": [1], "y": [np.nan]***REMOVED***)
        tm.assert_frame_equal(df, expected, check_dtype=False)
...
class TestPartialSetting:
    ...
    def test_partial_setting_frame(self, using_array_manager):
        ...
        expected = DataFrame({"A": [0, 4, 4], "B": [1, 5, 5]***REMOVED***)
        df = df_orig.copy()
        df.iloc[1] = df.iloc[2]
        tm.assert_frame_equal(df, expected)
        ...
        expected = DataFrame({"A": [0, 2, 4, 4], "B": [1, 3, 5, 5]***REMOVED***)
        df = df_orig.copy()
        df.loc[3] = df.loc[2]
        tm.assert_frame_equal(df, expected)

        # single dtype frame, overwrite
        expected = DataFrame({"A": [0, 2, 4], "B": [0, 2, 4]***REMOVED***)
        df = df_orig.copy()
        df.loc[:, "B"] = df.loc[:, "A"]
        tm.assert_frame_equal(df, expected)

        # mixed dtype frame, overwrite
        expected = DataFrame({"A": [0, 2, 4], "B": Series([0.0, 2.0, 4.0])***REMOVED***)
        df = df_orig.copy()
    ...
    def test_partial_setting2(self):
        ...
        expected = pd.concat(
            [df_orig, DataFrame({"A": 7***REMOVED***, index=dates[-1:] + dates.freq)], sort=True
        )
        df = df_orig.copy()
        ...
        exp_other = DataFrame({0: 7***REMOVED***, index=dates[-1:] + dates.freq)
    ...
    def test_series_partial_set(self):
        ...
        msg = (
            rf"\"None of \[Index\(\[3, 3, 3\], dtype='{np.dtype(int)***REMOVED***'\)\] "
            r"are in the \[index\]\""
        )
