...
def maximal_independent_set(G, nodes=None, seed=None):
    ...
    """
    if not nodes:
        nodes = {seed.choice(list(G))***REMOVED***
    else:
        nodes = set(nodes)
    if not nodes.issubset(G):
        raise nx.NetworkXUnfeasible(f"{nodes***REMOVED*** is not a subset of the nodes of G")
    neighbors = set.union(*[set(G.adj[v]) for v in nodes])
    if set.intersection(neighbors, nodes):
        raise nx.NetworkXUnfeasible(f"{nodes***REMOVED*** is not an independent set of G")
