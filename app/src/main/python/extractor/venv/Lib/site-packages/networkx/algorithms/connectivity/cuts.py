...
@nx._dispatch(
    graphs={"G": 0, "auxiliary?": 4, "residual?": 5***REMOVED***,
    preserve_edge_attrs={
        "auxiliary": {"capacity": float("inf")***REMOVED***,
        "residual": {"capacity": float("inf")***REMOVED***,
    ***REMOVED***,
    preserve_graph_attrs={"auxiliary", "residual"***REMOVED***,
)
def minimum_st_edge_cut(G, s, t, flow_func=None, auxiliary=None, residual=None):
    ...
    kwargs = {"capacity": "capacity", "flow_func": flow_func, "residual": residual***REMOVED***
...
@nx._dispatch(
    graphs={"G": 0, "auxiliary?": 4, "residual?": 5***REMOVED***,
    preserve_edge_attrs={"residual": {"capacity": float("inf")***REMOVED******REMOVED***,
    preserve_node_attrs={"auxiliary": {"id": None***REMOVED******REMOVED***,
    preserve_graph_attrs={"auxiliary", "residual"***REMOVED***,
)
def minimum_st_node_cut(G, s, t, flow_func=None, auxiliary=None, residual=None):
    ...
    if G.has_edge(s, t) or G.has_edge(t, s):
        return {***REMOVED***
    kwargs = {"flow_func": flow_func, "residual": residual, "auxiliary": H***REMOVED***

    # The edge cut in the auxiliary digraph corresponds to the node cut in the
    # original graph.
    edge_cut = minimum_st_edge_cut(H, f"{mapping[s]***REMOVED***B", f"{mapping[t]***REMOVED***A", **kwargs)
    # Each node in the original graph maps to two nodes of the auxiliary graph
    node_cut = {H.nodes[node]["id"] for edge in edge_cut for node in edge***REMOVED***
    return node_cut - {s, t***REMOVED***
...
def minimum_node_cut(G, s=None, t=None, flow_func=None):
    ...
    if s is not None and t is not None:
        if s not in G:
            raise nx.NetworkXError(f"node {s***REMOVED*** not in graph")
        if t not in G:
            raise nx.NetworkXError(f"node {t***REMOVED*** not in graph")
    ...
    H = build_auxiliary_node_connectivity(G)
    R = build_residual_network(H, "capacity")
    kwargs = {"flow_func": flow_func, "auxiliary": H, "residual": R***REMOVED***
    ...
    for w in set(G) - set(neighbors(v)) - {v***REMOVED***:
...
def minimum_edge_cut(G, s=None, t=None, flow_func=None):
    ...
    H = build_auxiliary_edge_connectivity(G)
    R = build_residual_network(H, "capacity")
    kwargs = {"flow_func": flow_func, "residual": R, "auxiliary": H***REMOVED***
