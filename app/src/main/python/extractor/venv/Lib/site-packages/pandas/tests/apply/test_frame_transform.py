...
def unpack_obj(obj, klass, axis):
    ...
    if klass is not DataFrame:
        obj = obj["A"]
        if axis != 0:
            pytest.skip(f"Test is only for DataFrame with axis={axis***REMOVED***")
    return obj
...
def test_transform_dictlike(axis, float_frame, box):
    ...
    result = float_frame.transform(box({e: np.abs***REMOVED***), axis=axis)
    tm.assert_frame_equal(result, expected)


def test_transform_dictlike_mixed():
    # GH 40018 - mix of lists and non-lists in values of a dictionary
    df = DataFrame({"a": [1, 2], "b": [1, 4], "c": [1, 4]***REMOVED***)
    result = df.transform({"b": ["sqrt", "abs"], "c": "sqrt"***REMOVED***)
    expected = DataFrame(
        [[1.0, 1, 1.0], [2.0, 4, 2.0]],
...
def test_transform_bad_dtype(op, frame_or_series, request):
    ...
    obj = DataFrame({"A": 3 * [object]***REMOVED***)  # DataFrame that will fail on most transforms
...
def test_transform_failure_typeerror(request, op):
    ...
    df = DataFrame({"A": 3 * [object], "B": [1, 2, 3]***REMOVED***)
    error = TypeError
    msg = "|".join(
        [
            "not supported between instances of 'type' and 'type'",
    ...
    with pytest.raises(error, match=msg):
        df.transform({"A": op, "B": op***REMOVED***)

    with pytest.raises(error, match=msg):
        df.transform({"A": [op], "B": [op]***REMOVED***)

    with pytest.raises(error, match=msg):
        df.transform({"A": [op, "shift"], "B": [op]***REMOVED***)
...
def test_transform_failure_valueerror():
    ...
    df = DataFrame({"A": [1, 2, 3], "B": [400, 500, 600]***REMOVED***)
