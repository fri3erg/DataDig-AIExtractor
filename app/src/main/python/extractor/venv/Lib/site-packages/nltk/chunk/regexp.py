...
class ChunkString:
    ...
    by braces.  An example of this encoding is::
        ...
        {<DT><JJ><NN>***REMOVED***<VBN><IN>{<DT><NN>***REMOVED***<.>{<DT><NN>***REMOVED***<VBD><.>
    ...
    CHUNK_TAG_CHAR = r"[^\{\***REMOVED***<>]"
    CHUNK_TAG = r"(<%s+?>)" % CHUNK_TAG_CHAR

    IN_CHUNK_PATTERN = r"(?=[^\{]*\***REMOVED***)"
    IN_STRIP_PATTERN = r"(?=[^\***REMOVED***]*(\{|$))"

    # These are used by _verify
    _CHUNK = r"(\{%s+?\***REMOVED***)+?" % CHUNK_TAG
    _STRIP = r"(%s+?)+?" % CHUNK_TAG
    _VALID = re.compile(r"^(\{?%s\***REMOVED***?)*?$" % CHUNK_TAG)
    _BRACKETS = re.compile(r"[^\{\***REMOVED***]+")
    _BALANCED_BRACKETS = re.compile(r"(\{\***REMOVED***)*$")
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        tags1 = (re.split(r"[\{\***REMOVED***<>]+", s))[1:-1]
...
***REMOVED***
    ...
    def to_chunkstruct(self, chunk_label="CHUNK"):
        ...
        index = 0
        piece_in_chunk = 0
        for piece in re.split("[{***REMOVED***]", self._str):
    ...
    def xform(self, regexp, repl):
        ...
        # (substrings of the form "{***REMOVED***").  Remove them, so they don't
        # interfere with other transformations.
        s = re.sub(r"\{\***REMOVED***", "", s)
    ...
    def __repr__(self):
        ...
        It has the form::
            ...
            <ChunkString: '{<DT><JJ><NN>***REMOVED***<VBN><IN>{<DT><NN>***REMOVED***'>
    ...
    def __str__(self):
        ...
        str = re.sub(r">(?!\***REMOVED***)", r"> ", self._str)
...
class RegexpChunkRule:
    ...
    def __repr__(self):
        ...
        Return a string representation of this rule.  It has the form::
            ...
            <RegexpChunkRule: '{<IN|VB.*>***REMOVED***'->'<IN>'>
    ...
    def fromstring(s):
        ...
        Currently, the following formats are supported::
          ...
          {regexp***REMOVED***         # chunk rule
          ***REMOVED***regexp{         # strip rule
          regexp***REMOVED***{regexp   # split rule
          regexp{***REMOVED***regexp   # merge rule
        ...
        >>> RegexpChunkRule.fromstring('{<DT>?<NN.*>+***REMOVED***')
        <ChunkRule: '<DT>?<NN.*>+'>
        """
        ...
        try:
            ...
            if rule[0] == "{" and rule[-1] == "***REMOVED***":
                return ChunkRule(rule[1:-1], comment)
            elif rule[0] == "***REMOVED***" and rule[-1] == "{":
                return StripRule(rule[1:-1], comment)
            elif "***REMOVED***{" in rule:
                left, right = rule.split("***REMOVED***{")
                return SplitRule(left, right, comment)
            elif "{***REMOVED***" in rule:
                left, right = rule.split("{***REMOVED***")
                return MergeRule(left, right, comment)
            elif re.match("[^{***REMOVED***]*{[^{***REMOVED***]****REMOVED***[^{***REMOVED***]*", rule):
                left, chunk, right = re.split("[{***REMOVED***]", rule)
...
class ChunkRule(RegexpChunkRule):
    ...
    def __init__(self, tag_pattern, descr):
        ...
        RegexpChunkRule.__init__(self, regexp, r"{\g<chunk>***REMOVED***", descr)
...
class StripRule(RegexpChunkRule):
    ...
    def __init__(self, tag_pattern, descr):
        ...
        RegexpChunkRule.__init__(self, regexp, r"***REMOVED***\g<strip>{", descr)
...
class UnChunkRule(RegexpChunkRule):
    ...
    def __init__(self, tag_pattern, descr):
        ...
        """
        self._pattern = tag_pattern
        regexp = re.compile(r"\{(?P<chunk>%s)\***REMOVED***" % tag_pattern2re_pattern(tag_pattern))
...
***REMOVED***
        ...
        RegexpChunkRule.__init__(self, regexp, r"\g<left>***REMOVED***{", descr)
...
class ExpandRightRule(RegexpChunkRule):
    ...
    def __init__(self, left_tag_pattern, right_tag_pattern, descr):
        ...
        self._right_tag_pattern = right_tag_pattern
        regexp = re.compile(
            r"(?P<left>%s)\***REMOVED***(?P<right>%s)"
            % (
                tag_pattern2re_pattern(left_tag_pattern),
                tag_pattern2re_pattern(right_tag_pattern),
***REMOVED***
        )
        RegexpChunkRule.__init__(self, regexp, r"\g<left>\g<right>***REMOVED***", descr)
...
***REMOVED***
        ...
        replacement = r"\g<left>{\g<chunk>***REMOVED***\g<right>"
    ...
    def __repr__(self):
        ...
        :rtype: str
        """
        return "<ChunkRuleWithContext:  {!r***REMOVED***, {!r***REMOVED***, {!r***REMOVED***>".format(
...
CHUNK_TAG_PATTERN = re.compile(
    r"^(({***REMOVED***|<{***REMOVED***>)*)$".format(r"([^\{\***REMOVED***<>]|\{\d+,?\***REMOVED***|\{\d*,\d+\***REMOVED***)+", r"[^\{\***REMOVED***<>]+")
)
...
def tag_pattern2re_pattern(tag_pattern):
    ...
    expression patterns and tag patterns are:
        ...
        - In tag patterns, ``'.'`` is equivalent to ``'[^{***REMOVED***<>]'``; so
    ...
    # We have to do this after, since it adds {***REMOVED***[]<>s, which would
...
class RegexpParser(ChunkParserI):
    ...
    A grammar contains one or more clauses in the following form::
     ...
     NP:
       {<DT|JJ>***REMOVED***          # chunk determiners and adjectives
       ***REMOVED***<[\.VI].*>+{      # strip any tag beginning with V, I, or .
       <.*>***REMOVED***{<DT>         # split a chunk at a determiner
       <DT|JJ>{***REMOVED***<NN.*>    # merge chunk ending with det/adj
...
def demo():
    ...
    grammar = r"""
    NP:                   # NP stage
      {<DT>?<JJ>*<NN>***REMOVED***    # chunk determiners, adjectives and nouns
      {<NNP>+***REMOVED***            # chunk proper nouns
    """
    ...
    grammar = r"""
    NP:
      {<.*>***REMOVED***              # start by chunking each tag
      ***REMOVED***<[\.VI].*>+{       # unchunk any verbs, prepositions or periods
      <DT|JJ>{***REMOVED***<NN.*>     # merge det/adj with nouns
    """
    cp = chunk.RegexpParser(grammar)
    demo_eval(cp, text)

    grammar = r"""
    NP: {<DT>?<JJ>*<NN>***REMOVED***    # chunk determiners, adjectives and nouns
    VP: {<TO>?<VB.*>***REMOVED***       # VP = verb words
    """
    cp = chunk.RegexpParser(grammar)
    demo_eval(cp, text)

    grammar = r"""
    NP: {<.*>****REMOVED***             # start by chunking everything
        ***REMOVED***<[\.VI].*>+{       # strip any verbs, prepositions or periods
        <.*>***REMOVED***{<DT>          # separate on determiners
    PP: {<IN><NP>***REMOVED***          # PP = preposition + noun phrase
    VP: {<VB.*><NP|PP>****REMOVED***    # VP = verb words + NPs and PPs
    """
    cp = chunk.RegexpParser(grammar)
    demo_eval(cp, text)
