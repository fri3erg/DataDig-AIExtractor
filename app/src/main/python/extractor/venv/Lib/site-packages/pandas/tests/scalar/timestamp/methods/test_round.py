...
***REMOVED***,
    ...
    def test_round_dst_border_ambiguous(self, method, unit):
        ...
        assert result == ts
        assert result._creso == getattr(NpyDatetimeUnit, f"NPY_FR_{unit***REMOVED***").value
    ...
    def test_round_int64(self, timestamp, freq):
        ...
        assert result._value % unit == 0, f"floor not a {freq***REMOVED*** multiple"
        assert 0 <= dt._value - result._value < unit, "floor error"

        # test ceil
        result = dt.ceil(freq)
        assert result._value % unit == 0, f"ceil not a {freq***REMOVED*** multiple"
        assert 0 <= result._value - dt._value < unit, "ceil error"

        # test round
        result = dt.round(freq)
        assert result._value % unit == 0, f"round not a {freq***REMOVED*** multiple"
