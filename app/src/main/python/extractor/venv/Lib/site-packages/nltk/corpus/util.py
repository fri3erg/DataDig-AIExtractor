...
class LazyCorpusLoader:
    ...
    def __load(self):
        ...
        if TRY_ZIPFILE_FIRST:
            try:
                root = nltk.data.find(f"{self.subdir***REMOVED***/{zip_name***REMOVED***")
            except LookupError as e:
                try:
                    root = nltk.data.find(f"{self.subdir***REMOVED***/{self.__name***REMOVED***")
                except LookupError:
                    raise e
        else:
            try:
    ...
    def __repr__(self):
        return "<{***REMOVED*** in {!r***REMOVED*** (not loaded yet)>".format(
