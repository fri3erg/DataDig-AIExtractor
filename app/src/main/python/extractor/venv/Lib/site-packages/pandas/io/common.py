...
def _get_filepath_or_buffer(
    ...
    if (
        ...
        warnings.warn(
            f"{compression***REMOVED*** will not write the byte order mark for {encoding***REMOVED***",
            UnicodeWarning,
            stacklevel=find_stack_level(),
        )
    ...
    if isinstance(filepath_or_buffer, str) and is_url(filepath_or_buffer):
        ...
        storage_options = storage_options or {***REMOVED***
        ...
        with urlopen(req_info) as req:
            ...
            if content_encoding == "gzip":
                ...
                compression = {"method": "gzip"***REMOVED***
...
***REMOVED***
        ...
        try:
            file_obj = fsspec.open(
                filepath_or_buffer, mode=fsspec_mode, **(storage_options or {***REMOVED***)
***REMOVED***.open()
        # GH 34626 Reads from Public Buckets without Credentials needs anon=True
        except tuple(err_types_to_retry_with_anon):
            if storage_options is None:
                storage_options = {"anon": True***REMOVED***
            else:
...
***REMOVED***.open()
    ...
    if not (
        ...
        msg = f"Invalid file path or buffer object type: {type(filepath_or_buffer)***REMOVED***"
...
def get_compression_method(
    ...
    Returns
    -------
    tuple of ({compression method***REMOVED***, Optional[str]
              {compression arguments***REMOVED***, Dict[str, Any])
...
def infer_compression(
    ...
    msg = (
        f"Unrecognized compression type: {compression***REMOVED***\n"
        f"Valid compression types are {valid***REMOVED***"
    )
    raise ValueError(msg)
...
def check_parent_directory(path: Path | str) -> None:
    ...
    if not parent.is_dir():
        raise OSError(rf"Cannot save file into a non-existent directory: '{parent***REMOVED***'")
...
***REMOVED***
            if handle.buffer.mode == "r":
                ...
                if len(zip_names) == 1:
                    ...
                    raise ValueError(f"Zero files found in ZIP file {path_or_buf***REMOVED***")
                else:
                    raise ValueError(
                        "Multiple files found in ZIP file. "
                        f"Only one file per ZIP: {zip_names***REMOVED***"
        ***REMOVED***
    ...
    ***REMOVED***
            ...
            if "r" in handle.buffer.mode:
                ...
                if len(files) == 1:
                    ...
                    raise ValueError(f"Zero files found in TAR archive {path_or_buf***REMOVED***")
                else:
                    raise ValueError(
                        "Multiple files found in TAR archive. "
                        f"Only one file per TAR archive: {files***REMOVED***"
        ***REMOVED***
...
***REMOVED***
        ...
        # Zstd Compression
            ...
            if "r" in ioargs.mode:
                open_args = {"dctx": zstd.ZstdDecompressor(**compression_args)***REMOVED***
            else:
                open_args = {"cctx": zstd.ZstdCompressor(**compression_args)***REMOVED***
            handle = zstd.open(
                handle,
                mode=ioargs.mode,
                **open_args,
***REMOVED***
        ...
        # Unrecognized Compression
            ...
            msg = f"Unrecognized compression type: {compression***REMOVED***"
            raise ValueError(msg)
...
***REMOVED***
    ...
    if "r" in ioargs.mode and not hasattr(handle, "read"):
        raise TypeError(
            ...
            f"got {type(ioargs.filepath_or_buffer)***REMOVED*** type"
        )
    ...
    def extend_mode(self, mode: str) -> str:
        ...
        if self.name is not None:
            ...
            if suffix in (".gz", ".xz", ".bz2"):
                mode = f"{mode***REMOVED***:{suffix[1:]***REMOVED***"
        return mode
    ...
    for i, col in enumerate(names):
        ...
        while cur_count > 0:
            ...
            if is_potential_multiindex:
                ...
                col = col[:-1] + (f"{col[-1]***REMOVED***.{cur_count***REMOVED***",)
            else:
                col = f"{col***REMOVED***.{cur_count***REMOVED***"
            cur_count = counts[col]
