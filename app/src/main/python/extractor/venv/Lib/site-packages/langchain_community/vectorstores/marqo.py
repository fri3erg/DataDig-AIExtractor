...
***REMOVED***
        if not isinstance(client, marqo.Client):
            raise ValueError(
                f"client should be an instance of marqo.Client, got {type(client)***REMOVED***"
***REMOVED***
        self._client = client
        self._index_name = index_name
        self._add_documents_settings = (
            {***REMOVED*** if add_documents_settings is None else add_documents_settings
        )
...
***REMOVED***
        ...
        for i, text in enumerate(texts):
            doc = {
                "text": text,
                "metadata": json.dumps(metadatas[i]) if metadatas else json.dumps({***REMOVED***),
            ***REMOVED***
...
***REMOVED***
            if response["errors"]:
                err_msg = (
                    f"Error in upload for documents in index range [{i***REMOVED***,"
                    f"{i + self._document_batch_size***REMOVED***], "
                    f"check Marqo logs."
    ***REMOVED***
    ...
    def _construct_documents_from_results_with_score(
        ...
        for res in results["hits"]:
            ...
            metadata = json.loads(res.get("metadata", "{***REMOVED***"))
            documents.append(
...
***REMOVED***
        ...
        try:
            client.create_index(index_name, settings_dict=index_settings or {***REMOVED***)
            if verbose:
                print(f"Created {index_name***REMOVED*** successfully.")
        except Exception:
            if verbose:
                print(f"Index {index_name***REMOVED*** exists.")

        instance: Marqo = cls(
            client,
            index_name,
            searchable_attributes=searchable_attributes,
            add_documents_settings=add_documents_settings or {***REMOVED***,
