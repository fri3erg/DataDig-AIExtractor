...
class TestSymbolic(util.F2PyTest):
    ...
    def test_sanity(self):
        ...
        assert repr(t) == "Expr(Op.TERMS, {Expr(Op.SYMBOL, 'x'): 1***REMOVED***)"
        assert t == t
        assert t != u
        ...
        assert repr(v) == "Expr(Op.FACTORS, {Expr(Op.SYMBOL, 'x'): 1***REMOVED***)"
        assert v == v
        assert w != v
    ...
    def test_tostring_fortran(self):
        ...
        assert str(Expr(Op.TERMS, {x: 1***REMOVED***)) == "x"
        assert str(Expr(Op.TERMS, {x: 2***REMOVED***)) == "2 * x"
        assert str(Expr(Op.TERMS, {x: -1***REMOVED***)) == "-x"
        assert str(Expr(Op.TERMS, {x: -2***REMOVED***)) == "-2 * x"
        assert str(Expr(Op.TERMS, {x: 1, y: 1***REMOVED***)) == "x + y"
        assert str(Expr(Op.TERMS, {x: -1, y: -1***REMOVED***)) == "-x - y"
        assert str(Expr(Op.TERMS, {x: 2, y: 3***REMOVED***)) == "2 * x + 3 * y"
        assert str(Expr(Op.TERMS, {x: -2, y: 3***REMOVED***)) == "-2 * x + 3 * y"
        assert str(Expr(Op.TERMS, {x: 2, y: -3***REMOVED***)) == "2 * x - 3 * y"

        assert str(Expr(Op.FACTORS, {x: 1***REMOVED***)) == "x"
        assert str(Expr(Op.FACTORS, {x: 2***REMOVED***)) == "x ** 2"
        assert str(Expr(Op.FACTORS, {x: -1***REMOVED***)) == "x ** -1"
        assert str(Expr(Op.FACTORS, {x: -2***REMOVED***)) == "x ** -2"
        assert str(Expr(Op.FACTORS, {x: 1, y: 1***REMOVED***)) == "x * y"
        assert str(Expr(Op.FACTORS, {x: 2, y: 3***REMOVED***)) == "x ** 2 * y ** 3"

        v = Expr(Op.FACTORS, {x: 2, Expr(Op.TERMS, {x: 1, y: 1***REMOVED***): 3***REMOVED***)
        assert str(v) == "x ** 2 * (x + y) ** 3", str(v)
        v = Expr(Op.FACTORS, {x: 2, Expr(Op.FACTORS, {x: 1, y: 1***REMOVED***): 3***REMOVED***)
        assert str(v) == "x ** 2 * (x * y) ** 3", str(v)

        assert str(Expr(Op.APPLY, ("f", (), {***REMOVED***))) == "f()"
        assert str(Expr(Op.APPLY, ("f", (x, ), {***REMOVED***))) == "f(x)"
        assert str(Expr(Op.APPLY, ("f", (x, y), {***REMOVED***))) == "f(x, y)"
    ...
    def test_tostring_c(self):
        ...
        assert Expr(Op.FACTORS, {x: 2***REMOVED***).tostring(language=language) == "x * x"
        assert (Expr(Op.FACTORS, {
            x + y: 2
        ***REMOVED***).tostring(language=language) == "(x + y) * (x + y)")
        assert Expr(Op.FACTORS, {
            x: 12
        ***REMOVED***).tostring(language=language) == "pow(x, 12)"
    ...
    def test_operations(self):
        ...
        assert x + x == Expr(Op.TERMS, {x: 2***REMOVED***)
        assert x - x == Expr(Op.INTEGER, (0, 4))
        assert x + y == Expr(Op.TERMS, {x: 1, y: 1***REMOVED***)
        assert x - y == Expr(Op.TERMS, {x: 1, y: -1***REMOVED***)
        assert x * x == Expr(Op.FACTORS, {x: 2***REMOVED***)
        assert x * y == Expr(Op.FACTORS, {x: 1, y: 1***REMOVED***)

        assert +x == x
        assert -x == Expr(Op.TERMS, {x: -1***REMOVED***), repr(-x)
        assert 2 * x == Expr(Op.TERMS, {x: 2***REMOVED***)
        assert 2 + x == Expr(Op.TERMS, {x: 1, as_number(1): 2***REMOVED***)
        assert 2 * x + 3 * y == Expr(Op.TERMS, {x: 2, y: 3***REMOVED***)
        assert (x + y) * 2 == Expr(Op.TERMS, {x: 2, y: 2***REMOVED***)

        assert x**2 == Expr(Op.FACTORS, {x: 2***REMOVED***)
        assert (x + y)**2 == Expr(
            Op.TERMS,
            {
                Expr(Op.FACTORS, {x: 2***REMOVED***): 1,
                Expr(Op.FACTORS, {y: 2***REMOVED***): 1,
                Expr(Op.FACTORS, {
                    x: 1,
                    y: 1
                ***REMOVED***): 2,
        ...
        assert (x / 2.0) == Expr(Op.TERMS, {x: 0.5***REMOVED***)
    ...
    def test_substitute(self):
        ...
        assert x.substitute({x: y***REMOVED***) == y
        assert (x + y).substitute({x: z***REMOVED***) == y + z
        assert (x * y).substitute({x: z***REMOVED***) == y * z
        assert (x**4).substitute({x: z***REMOVED***) == z**4
        assert (x / y).substitute({x: z***REMOVED***) == z / y
        assert x.substitute({x: y + z***REMOVED***) == y + z
        assert a.substitute({x: y + z***REMOVED***) == as_array((y + z, y))

        assert as_ternary(x, y,
                          z).substitute({x: y + z***REMOVED***) == as_ternary(y + z, y, z)
        assert as_eq(x, y).substitute({x: y + z***REMOVED***) == as_eq(y + z, y)
...
***REMOVED***
    ...
    def test_traverse(self):
        ...
        assert function_symbols == {f***REMOVED***
        assert symbols == {x, y, z***REMOVED***
        ...
        assert symbols == {x, y, z, f***REMOVED***
    ...
    def test_polynomial_atoms(self):
        ...
        assert x.polynomial_atoms() == {x***REMOVED***
        assert n.polynomial_atoms() == set()
        assert (y[x]).polynomial_atoms() == {y[x]***REMOVED***
        assert (y(x)).polynomial_atoms() == {y(x)***REMOVED***
        assert (y(x) + x).polynomial_atoms() == {y(x), x***REMOVED***
        assert (y(x) * x[y]).polynomial_atoms() == {y(x), x[y]***REMOVED***
        assert (y(x)**x).polynomial_atoms() == {y(x)***REMOVED***
