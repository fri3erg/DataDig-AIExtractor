    ...
    def _validate_func(self, func: Union[Operator, Comparator]) -> None:
        if isinstance(func, Operator) and self.allowed_operators is not None:
            if func not in self.allowed_operators:
                raise ValueError(
                    f"Received disallowed operator {func***REMOVED***. Allowed "
                    f"comparators are {self.allowed_operators***REMOVED***"
    ***REMOVED***
        if isinstance(func, Comparator) and self.allowed_comparators is not None:
            if func not in self.allowed_comparators:
                raise ValueError(
                    f"Received disallowed comparator {func***REMOVED***. Allowed "
...
class Expr(BaseModel):
    ...
    def accept(self, visitor: Visitor) -> Any:
        ...
        Returns:
            result of visiting
        """
        return getattr(visitor, f"visit_{_to_snake_case(self.__class__.__name__)***REMOVED***")(
            self
        )
