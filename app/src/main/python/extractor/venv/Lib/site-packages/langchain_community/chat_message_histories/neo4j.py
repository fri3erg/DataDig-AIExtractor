...
***REMOVED***
        ...
        self._driver.execute_query(
            f"MERGE (s:`{self._node_label***REMOVED***` {{id:$session_id***REMOVED******REMOVED***)",
            {"session_id": self._session_id***REMOVED***,
        ).summary
    ...
    def messages(self) -> List[BaseMessage]:  # type: ignore
        ...
        query = (
            f"MATCH (s:`{self._node_label***REMOVED***`)-[:LAST_MESSAGE]->(last_message) "
            "WHERE s.id = $session_id MATCH p=(last_message)<-[:NEXT*0.."
            f"{self._window*2***REMOVED***]-() WITH p, length(p) AS length "
            "ORDER BY length DESC LIMIT 1 UNWIND reverse(nodes(p)) AS node "
            "RETURN {data:{content: node.content***REMOVED***, type:node.type***REMOVED*** AS result"
        )
        records, _, _ = self._driver.execute_query(
            query, {"session_id": self._session_id***REMOVED***
        )
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        query = (
            f"MATCH (s:`{self._node_label***REMOVED***`) WHERE s.id = $session_id "
            "OPTIONAL MATCH (s)-[lm:LAST_MESSAGE]->(last_message) "
            "CREATE (s)-[:LAST_MESSAGE]->(new:Message) "
            "SET new += {type:$type, content:$content***REMOVED*** "
    ...
    def clear(self) -> None:
        ...
        self._driver.execute_query(query, {"session_id": self._session_id***REMOVED***).summary
