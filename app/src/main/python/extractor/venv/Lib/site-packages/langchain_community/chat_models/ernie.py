...
class ErnieBotChat(BaseChatModel):
    ...
    def _chat(self, payload: object) -> dict:
        base_url = f"{self.ernie_api_base***REMOVED***/rpc/2.0/ai_custom/v1/wenxinworkshop/chat"
        model_paths = {
        ...
        ***REMOVED***
        if self.model_name in model_paths:
            url = f"{base_url***REMOVED***/{model_paths[self.model_name]***REMOVED***"
        else:
            raise ValueError(f"Got unknown model_name {self.model_name***REMOVED***")
        ...
        resp = requests.post(
            ...
            ***REMOVED***,
            params={"access_token": self.access_token***REMOVED***,
            json=payload,
        )
        return resp.json()
    ...
    def _refresh_access_token_with_lock(self) -> None:
        with self._lock:
            ...
            base_url: str = f"{self.ernie_api_base***REMOVED***/oauth/2.0/token"
            resp = requests.post(
                base_url,
                timeout=10,
                headers={
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        payload = {
            ...
            **kwargs,
        ***REMOVED***
        logger.debug(f"Payload for ernie api is {payload***REMOVED***")
        ...
        if resp.get("error_code"):
            if resp.get("error_code") == 111:
                ...
                raise ValueError(f"Error from ErnieChat api response: {resp***REMOVED***")
        return self._create_chat_result(resp)

    def _create_chat_result(self, response: Mapping[str, Any]) -> ChatResult:
        if "function_call" in response:
            additional_kwargs = {
                "function_call": dict(response.get("function_call", {***REMOVED***))
            ***REMOVED***
        else:
        ...
        generations = [
            ChatGeneration(
                message=AIMessage(
                    ...
                    additional_kwargs={**additional_kwargs***REMOVED***,
    ***REMOVED***
***REMOVED***
        ]
