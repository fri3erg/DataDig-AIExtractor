...
class TestCategoricalDtypes:
    ...
    def test_codes_dtypes(self):
        ...
        result = Categorical([f"foo{i:05d***REMOVED***" for i in range(400)])
        assert result.codes.dtype == "int16"

        result = Categorical([f"foo{i:05d***REMOVED***" for i in range(40000)])
        assert result.codes.dtype == "int32"

        # adding cats
        result = Categorical(["foo", "bar", "baz"])
        assert result.codes.dtype == "int8"
        result = result.add_categories([f"foo{i:05d***REMOVED***" for i in range(400)])
        assert result.codes.dtype == "int16"

        # removing cats
        result = result.remove_categories([f"foo{i:05d***REMOVED***" for i in range(300)])
        assert result.codes.dtype == "int8"
