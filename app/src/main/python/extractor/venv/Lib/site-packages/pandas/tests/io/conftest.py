...
def compression_to_extension():
    return {value: key for key, value in icom.extension_to_compression.items()***REMOVED***
...
***REMOVED***
        ...
        endpoint_port = f"555{worker_id***REMOVED***"
        endpoint_uri = f"http://127.0.0.1:{endpoint_port***REMOVED***/"

        # pipe to null to avoid logging in terminal
        with subprocess.Popen(
            shlex.split(f"moto_server s3 -p {endpoint_port***REMOVED***"),
...
@pytest.fixture
def s3so(s3_base):
    return {"client_kwargs": {"endpoint_url": s3_base***REMOVED******REMOVED***
...
def s3_public_bucket(s3_resource):
    bucket = s3_resource.Bucket(f"pandas-test-{uuid.uuid4()***REMOVED***")
    bucket.create()
    yield bucket
...
def s3_private_bucket(s3_resource):
    bucket = s3_resource.Bucket(f"cant_get_it-{uuid.uuid4()***REMOVED***")
