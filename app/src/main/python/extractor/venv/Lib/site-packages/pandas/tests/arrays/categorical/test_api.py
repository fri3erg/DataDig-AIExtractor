...
class TestCategoricalAPI:
    ...
    def test_rename_categories_dict(self):
        ...
        res = cat.rename_categories({"a": 4, "b": 3, "c": 2, "d": 1***REMOVED***)
        expected = Index([4, 3, 2, 1])
        tm.assert_index_equal(res.categories, expected)

        # Test for dicts of smaller length
        cat = Categorical(["a", "b", "c", "d"])
        res = cat.rename_categories({"a": 1, "c": 3***REMOVED***)
        ...
        cat = Categorical(["a", "b", "c", "d"])
        res = cat.rename_categories({"a": 1, "b": 2, "c": 3, "d": 4, "e": 5, "f": 6***REMOVED***)
        expected = Index([1, 2, 3, 4])
        tm.assert_index_equal(res.categories, expected)

        # Test for dicts with no items from old categories
        cat = Categorical(["a", "b", "c", "d"])
        res = cat.rename_categories({"f": 1, "g": 3***REMOVED***)
    ...
    def test_add_categories_existing_raises(self):
        ...
        msg = re.escape("new categories must not include old categories: {'d'***REMOVED***")
        with pytest.raises(ValueError, match=msg):
    ...
    def test_remove_categories_raises(self, removals):
        ...
        message = re.escape("removals must all be in old categories: {'c'***REMOVED***")
...
class TestCategoricalAPIWithFactor:
    def test_describe(self):
        ...
        expected = DataFrame(
            {"counts": [3, 2, 3], "freqs": [3 / 8.0, 2 / 8.0, 3 / 8.0]***REMOVED***, index=exp_index
        )
        ...
        expected = DataFrame(
            {"counts": [3, 2, 3, 0], "freqs": [3 / 8.0, 2 / 8.0, 3 / 8.0, 0]***REMOVED***,
            index=exp_index,
        )
        tm.assert_frame_equal(desc, expected)
        ...
        expected = DataFrame(
            {"counts": [5, 3, 3], "freqs": [5 / 11.0, 3 / 11.0, 3 / 11.0]***REMOVED***,
            index=exp_index,
        )
        tm.assert_frame_equal(desc, expected)
        ...
        desc = cat.describe()
        expected = DataFrame(
            {"counts": [1, 2, 1], "freqs": [1 / 4.0, 2 / 4.0, 1 / 4.0]***REMOVED***,
