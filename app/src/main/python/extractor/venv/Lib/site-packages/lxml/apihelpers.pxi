...
cdef _Document _documentOrRaise(object input):
    ...
    if isinstance(input, _ElementTree):
        ...
        raise TypeError, f"Invalid input object: {python._fqtypename(input).decode('utf8')***REMOVED***"
    if doc is None:
        raise ValueError, f"Input object has no document: {python._fqtypename(input).decode('utf8')***REMOVED***"
    _assertValidDoc(doc)
    return doc
...
cdef _Element _rootNodeOrRaise(object input):
    ...
    if (node is None or not node._c_node or
        ...
        raise ValueError, f"Input object is not an XML element: {python._fqtypename(input).decode('utf8')***REMOVED***"
...
cdef _initNodeAttributes(xmlNode* c_node, _Document doc, attrib, dict extra):
    ...
    if attrib is not None and not hasattr(attrib, 'items'):
        raise TypeError, f"Invalid attribute dictionary: {python._fqtypename(attrib).decode('utf8')***REMOVED***"
...
cdef int _replaceSlice(_Element parent, xmlNode* c_node,
    ...
    if step != 1 or not left_to_right:
        ...
        if seqlength != slicelength:
            raise ValueError, f"attempt to assign sequence of size {seqlength***REMOVED*** " \
                f"to extended slice of size {slicelength***REMOVED***"
...
cdef tuple __getNsTag(tag, bint empty_ns):
    ...
    if c_tag[0] == c'{':
        c_tag += 1
        c_ns_end = cstring_h.strchr(c_tag, c'***REMOVED***')
        if c_ns_end is NULL:
...
cdef int _tagValidOrRaise(tag_utf) except -1:
    if not _pyXmlNameIsValid(tag_utf):
        raise ValueError(f"Invalid tag name {(<bytes>tag_utf).decode('utf8')!r***REMOVED***")
    return 0

cdef int _htmlTagValidOrRaise(tag_utf) except -1:
    if not _pyHtmlNameIsValid(tag_utf):
        raise ValueError(f"Invalid HTML tag name {(<bytes>tag_utf).decode('utf8')!r***REMOVED***")
    return 0

cdef int _attributeValidOrRaise(name_utf) except -1:
    if not _pyXmlNameIsValid(name_utf):
        raise ValueError(f"Invalid attribute name {(<bytes>name_utf).decode('utf8')!r***REMOVED***")
    return 0

cdef int _prefixValidOrRaise(tag_utf) except -1:
    if not _pyXmlNameIsValid(tag_utf):
        raise ValueError(f"Invalid namespace prefix {(<bytes>tag_utf).decode('utf8')!r***REMOVED***")
    return 0

cdef int _uriValidOrRaise(uri_utf) except -1:
    cdef uri.xmlURI* c_uri = uri.xmlParseURI(_cstr(uri_utf))
    if c_uri is NULL:
        raise ValueError(f"Invalid namespace URI {(<bytes>uri_utf).decode('utf8')!r***REMOVED***")
...
cdef unicode _namespacedNameFromNsName(const_xmlChar* c_href, const_xmlChar* c_name):
    ...
    return f"{{{href***REMOVED******REMOVED******REMOVED***{name***REMOVED***"
