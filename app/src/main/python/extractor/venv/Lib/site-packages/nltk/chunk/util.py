...
class ChunkScore:
    ...
    def __str__(self):
        ...
        return (
            "ChunkParse score:\n"
            + (f"    IOB Accuracy: {self.accuracy() * 100:5.1f***REMOVED***%%\n")
            + (f"    Precision:    {self.precision() * 100:5.1f***REMOVED***%%\n")
            + (f"    Recall:       {self.recall() * 100:5.1f***REMOVED***%%\n")
            + (f"    F-Measure:    {self.f_measure() * 100:5.1f***REMOVED***%%")
        )
...
def tagstr2tree(
    ...
    for match in WORD_OR_BRACKET.finditer(s):
        ...
        if text[0] == "[":
            if len(stack) != 1:
                raise ValueError(f"Unexpected [ at char {match.start():d***REMOVED***")
    ...
    if len(stack) != 1:
        raise ValueError(f"Expected ] at char {len(s):d***REMOVED***")
    return stack[0]
...
def conllstr2tree(s, chunk_types=("NP", "PP", "VP"), root_label="S"):
    ...
    for lineno, line in enumerate(s.split("\n")):
        ...
        if match is None:
            raise ValueError(f"Error on line {lineno:d***REMOVED***")
...
***REMOVED***:
            ...
            raise ValueError(f"Bad conll tag {chunktag!r***REMOVED***")
    return tree
...
def _ieer_read_text(s, root_label):
    ...
    for piece_m in re.finditer(r"<[^>]+>|[^\s<]+", s):
        ...
        try:
            ...
            raise ValueError(
                f"Bad IEER string (error at character {piece_m.start():d***REMOVED***)"
***REMOVED*** from e
    if len(stack) != 1:
