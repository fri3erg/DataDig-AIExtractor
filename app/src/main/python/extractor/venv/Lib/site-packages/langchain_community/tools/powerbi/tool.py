...
***REMOVED***
            ...
            self.session_cache[tool_input] = f"Error on call to LLM: {exc***REMOVED***"
            return self.session_cache[tool_input]
        ...
        logger.debug(f"PBI Query duration: {end_time - start_time:0.6f***REMOVED***")
...
***REMOVED***
    ...
    async def _arun(
        ...
        **kwargs: Any,
        ...
        if cache := self._check_cache(tool_input):
            logger.debug("Found cached result for %s: %s", tool_input, cache)
            return f"{cache***REMOVED***, from cache, you have already asked this question."
        try:
            logger.info("Running PBI Query Tool with input: %s", tool_input)
...
***REMOVED***
    ...
    ***REMOVED***
            ...
            if too_long:
                return (
                    f"Result too large, please try to be more specific or use the `TOPN` function. The result is {length***REMOVED*** tokens long, the limit is {self.output_token_limit***REMOVED*** tokens.",  # noqa: E501
                    None,
    ***REMOVED***
            return result, None
