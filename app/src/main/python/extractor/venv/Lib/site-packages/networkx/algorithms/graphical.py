...
def is_valid_degree_sequence_havel_hakimi(deg_sequence):
    ...
    .. math::
        |d| >= \frac{(\max(d) + \min(d) + 1)^2***REMOVED***{4*\min(d)***REMOVED***
...
def is_valid_degree_sequence_erdos_gallai(deg_sequence):
    ...
    sum of the sequence is even and for all strong indices k in the sequence,
     ...
     .. math::
       ...
       \sum_{i=1***REMOVED***^{k***REMOVED*** d_i \leq k(k-1) + \sum_{j=k+1***REMOVED***^{n***REMOVED*** \min(d_i,k)
             = k(n-1) - ( k \sum_{j=0***REMOVED***^{k-1***REMOVED*** n_j - \sum_{j=0***REMOVED***^{k-1***REMOVED*** j n_j )
