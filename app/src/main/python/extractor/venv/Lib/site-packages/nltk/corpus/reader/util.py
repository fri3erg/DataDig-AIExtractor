...
class StreamBackedCorpusView(AbstractLazySequence):
    ...
    def __init__(self, fileid, block_reader=None, startpos=0, encoding="utf8"):
        ...
        try:
            ...
            raise ValueError(f"Unable to open or access {fileid!r***REMOVED*** -- {exc***REMOVED***") from exc
...
***REMOVED***
            ...
            assert (
                new_filepos > filepos
***REMOVED***, "block reader %s() should consume at least 1 byte (filepos=%d)" % (
                self.read_block.__name__,
                filepos,
***REMOVED***
        ...
        ***REMOVED***, "inconsistent block reader (num chars read)"
                    assert (
                        toknum + num_toks == self._toknum[block_index]
        ***REMOVED***, "inconsistent block reader (num tokens returned)"
...
def concat(docs):
    ...
    types = {d.__class__ for d in docs***REMOVED***
