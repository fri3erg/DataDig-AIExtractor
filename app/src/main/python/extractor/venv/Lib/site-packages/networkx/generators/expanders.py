...
def margulis_gabber_galil_graph(n, create_using=None):
    ...
    is at most `5 \sqrt{2***REMOVED***`, regardless of `n`.
    ...
    for x, y in itertools.product(range(n), repeat=2):
        for u, v in (
            ...
            G.add_edge((x, y), (u, v))
    G.graph["name"] = f"margulis_gabber_galil_graph({n***REMOVED***)"
    return G
...
def chordal_cycle_graph(p, create_using=None):
    ...
    for x in range(p):
        ...
        for y in (left, right, chord):
            G.add_edge(x, y)
    G.graph["name"] = f"chordal_cycle_graph({p***REMOVED***)"
    return G


@nx._dispatch(graphs=None)
def paley_graph(p, create_using=None):
    r"""Returns the Paley $\frac{(p-1)***REMOVED***{2***REMOVED***$ -regular graph on $p$ nodes.

    The returned graph is a graph on $\mathbb{Z***REMOVED***/p\mathbb{Z***REMOVED***$ with edges between $x$ and $y$
    if and only if $x-y$ is a nonzero square in $\mathbb{Z***REMOVED***/p\mathbb{Z***REMOVED***$.

    If $p \equiv 1  \pmod 4$, $-1$ is a square in $\mathbb{Z***REMOVED***/p\mathbb{Z***REMOVED***$ and therefore $x-y$ is a square if and
    only if $y-x$ is also a square, i.e the edges in the Paley graph are symmetric.

    If $p \equiv 3 \pmod 4$, $-1$ is not a square in $\mathbb{Z***REMOVED***/p\mathbb{Z***REMOVED***$ and therefore either $x-y$ or $y-x$
    is a square in $\mathbb{Z***REMOVED***/p\mathbb{Z***REMOVED***$ but not both.

    Note that a more general definition of Paley graphs extends this construction
    to graphs over $q=p^n$ vertices, by using the finite field $F_q$ instead of $\mathbb{Z***REMOVED***/p\mathbb{Z***REMOVED***$.
    ...
    # when is prime).
    square_set = {(x**2) % p for x in range(1, p) if (x**2) % p != 0***REMOVED***

    for x in range(p):
        for x2 in square_set:
            G.add_edge(x, (x + x2) % p)
    G.graph["name"] = f"paley({p***REMOVED***)"
    return G
