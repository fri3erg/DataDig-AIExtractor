...
def tz_to_dtype(
    ...
    """
    if tz is None:
        return np.dtype(f"M8[{unit***REMOVED***]")
    else:
...
***REMOVED***
        ...
        validate_kwds = {"ambiguous": ambiguous***REMOVED***
...
***REMOVED***
        ...
        dt64_values = i8values.view(f"datetime64[{unit***REMOVED***]")
...
***REMOVED***
        ...
        if self.tz is not None:
            ...
            new_dates = tzconversion.tz_localize_to_utc(
                ...
                creso=self._creso,
***REMOVED***
        new_dates_dt64 = new_dates.view(f"M8[{self.unit***REMOVED***]")
        dtype = tz_to_dtype(tz, unit=self.unit)
    ...
    def to_period(self, freq=None) -> PeriodArray:
        ...
        Examples
        --------
        >>> df = pd.DataFrame({"y": [1, 2, 3]***REMOVED***,
...
***REMOVED***:
    ...
    ***REMOVED***
    ...
    _is_month_doc = """
        Indicates whether the date is the {first_or_last***REMOVED*** day of the month.
...
def _sequence_to_dt64(
    ...
    *,
    ...
    if out_unit is None:
        out_unit = "ns"
    out_dtype = np.dtype(f"M8[{out_unit***REMOVED***]")
...
def objects_to_datetime64(
    ...
    errors : {'raise', 'ignore', 'coerce'***REMOVED***
    allow_object : bool
...
def maybe_convert_dtype(data, copy: bool, tz: tzinfo | None = None):
    ...
    if is_float_dtype(data.dtype):
        ...
        raise TypeError(f"dtype {data.dtype***REMOVED*** cannot be converted to datetime64[ns]")
...
def _maybe_infer_tz(tz: tzinfo | None, inferred_tz: tzinfo | None) -> tzinfo | None:
    ...
    if tz is None:
        ...
        raise TypeError(
            f"data is already tz-aware {inferred_tz***REMOVED***, unable to "
            f"set specified tz: {tz***REMOVED***"
        )
    return tz
...
***REMOVED***
        ...
        if (
            ...
            raise ValueError(
                f"Unexpected value for 'dtype': '{dtype***REMOVED***'. "
...
def _maybe_localize_point(
    ...
    if ts is not None and ts.tzinfo is None:
        ...
        localize_args = {"ambiguous": ambiguous, "nonexistent": nonexistent, "tz": None***REMOVED***
    ...
    * If both start and end are specified, the returned dates will
    ...
    if offset.n >= 0:
        while cur <= end:
            ...
            if next_date <= cur:
                raise ValueError(f"Offset {offset***REMOVED*** did not increment date")
            cur = next_date
    else:
        while cur >= end:
            yield cur
            ...
            next_date = next_date.as_unit(unit)
            if next_date >= cur:
                raise ValueError(f"Offset {offset***REMOVED*** did not decrement date")
            cur = next_date
