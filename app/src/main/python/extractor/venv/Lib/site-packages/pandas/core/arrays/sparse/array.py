...
_sparray_doc_kwargs = {"klass": "SparseArray"***REMOVED***
...
def _sparse_array_op(
    ...
    if left.sp_index.ngaps == 0 or right.sp_index.ngaps == 0:
        ...
        if name in ("and", "or", "xor") and dtype == "bool":
            opname = f"sparse_{name***REMOVED***_uint8"
            # to make template simple, cast here
            left_sp_values = left.sp_values.view(np.uint8)
            right_sp_values = right.sp_values.view(np.uint8)
            result_dtype = bool
        else:
            opname = f"sparse_{name***REMOVED***_{dtype***REMOVED***"
        ...
        if (
            ...
            opname = f"sparse_{name***REMOVED***_float64"
...
class SparseArray(OpsMixin, PandasObject, ExtensionArray):
    ...
    def __init__(
        ...
        if is_scalar(data):
            warnings.warn(
                f"Constructing {type(self).__name__***REMOVED*** with scalar data is deprecated "
...
***REMOVED***
            ...
            if not isinstance(data, np.ndarray):
                ...
                if isinstance(data.dtype, DatetimeTZDtype):
                    warnings.warn(
                        f"Creating SparseArray from {data.dtype***REMOVED*** data "
...
***REMOVED***
            ...
            if len(sparse_values) != sparse_index.npoints:
                raise AssertionError(
                    f"Non array-like type {type(sparse_values)***REMOVED*** must "
    ...
    ***REMOVED***
    ...
    def from_spmatrix(cls, data: spmatrix) -> Self:
        ...
        if ncol != 1:
            raise ValueError(f"'data' must have a single column, not '{ncol***REMOVED***'")
    ...
    def kind(self) -> SparseIndexKind:
        """
        The kind of sparse index for this array. One of {'integer', 'block'***REMOVED***.
        """
...
***REMOVED***
            ...
            if com.is_bool_indexer(key):
                ...
                raise ValueError(f"Cannot slice with '{key***REMOVED***'")
    ...
    def take(self, indices, *, allow_fill: bool = False, fill_value=None) -> Self:
        if is_scalar(indices):
            raise ValueError(f"'indices' must be an array, not a scalar '{indices***REMOVED***'.")
...
***REMOVED***
    ...
    def map(self, mapper, na_action=None) -> Self:
        ...
        >>> arr.map({0: 10, 1: 11, 2: 12***REMOVED***)
        [10, 11, 12]
        Fill: 10
        IntIndex
        Indices: array([1, 2], dtype=int32)
    ...
    def _reduce(
        ...
        if method is None:
            raise TypeError(f"cannot perform {name***REMOVED*** with type {self.dtype***REMOVED***")
    ...
    def cumsum(self, axis: AxisInt = 0, *args, **kwargs) -> SparseArray:
        ...
        if axis is not None and axis >= self.ndim:  # Mimic ndarray behaviour.
            raise ValueError(f"axis(={axis***REMOVED***) out of bounds")
    ...
    ***REMOVED***
            ...
            with np.errstate(all="ignore"):
                if len(self) != len(other):
                    raise AssertionError(
                        f"length mismatch: {len(self)***REMOVED*** vs. {len(other)***REMOVED***"
        ***REMOVED***
    ...
    def _cmp_method(self, other, op) -> SparseArray:
        ...
        if isinstance(other, SparseArray):
            if len(self) != len(other):
                raise ValueError(
                    f"operands have mismatched length {len(self)***REMOVED*** and {len(other)***REMOVED***"
    ***REMOVED***
...
***REMOVED***
    ...
    def __repr__(self) -> str:
        ...
        return f"{pp_str***REMOVED***\nFill: {pp_fill***REMOVED***\n{pp_index***REMOVED***"
