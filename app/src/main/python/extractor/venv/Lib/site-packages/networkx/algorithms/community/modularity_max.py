...
def _greedy_modularity_communities_generator(G, weight=None, resolution=1):
    ...
    if directed:
        a = {node: deg_out * q0 for node, deg_out in G.out_degree(weight=weight)***REMOVED***
        b = {node: deg_in * q0 for node, deg_in in G.in_degree(weight=weight)***REMOVED***
    else:
        a = b = {node: deg * q0 * 0.5 for node, deg in G.degree(weight=weight)***REMOVED***
    ...
    dq_heap = {u: MappedQueue({(u, v): -dq for v, dq in dq_dict[u].items()***REMOVED***) for u in G***REMOVED***
    ...
    communities = {n: frozenset([n]) for n in G***REMOVED***
    ...
    while len(H) > 1:
        ...
        all_nbrs = (u_nbrs | v_nbrs) - {u, v***REMOVED***
...
def greedy_modularity_communities(
    ...
    if (cutoff < 1) or (cutoff > G.number_of_nodes()):
        raise ValueError(f"cutoff must be between 1 and {len(G)***REMOVED***. Got {cutoff***REMOVED***.")
    if best_n is not None:
        if (best_n < 1) or (best_n > G.number_of_nodes()):
            raise ValueError(f"best_n must be between 1 and {len(G)***REMOVED***. Got {best_n***REMOVED***.")
        if best_n < cutoff:
            raise ValueError(f"Must have best_n >= cutoff. Got {best_n***REMOVED*** < {cutoff***REMOVED***")
        if best_n == 1:
            return [set(G)]
    else:
