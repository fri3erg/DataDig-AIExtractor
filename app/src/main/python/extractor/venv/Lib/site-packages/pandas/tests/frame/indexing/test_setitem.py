...
class TestDataFrameSetItem:
    def test_setitem_str_subclass(self):
        ...
        df = DataFrame({"a": [1]***REMOVED***, index=index)
        df["b"] = 2
        df[mystring("c")] = 3
        expected = DataFrame({"a": [1], "b": [2], mystring("c"): [3]***REMOVED***, index=index)
    ...
    def test_setitem_empty_columns(self):
        ...
        df["X"] = df.index
        df["X"] = ["x", "y", "z"]
        exp = DataFrame(data={"X": ["x", "y", "z"]***REMOVED***, index=["A", "B", "C"])
    ...
    def test_setitem_wrong_length_categorical_dtype_raises(self):
        ...
        msg = (
            rf"Length of values \({len(cat)***REMOVED***\) "
            rf"does not match length of index \({len(df)***REMOVED***\)"
        )
        with pytest.raises(ValueError, match=msg):
            df["foo"] = cat
    ...
    def test_setitem_with_sparse_value(self):
        # GH#8131
        df = DataFrame({"c_1": ["a", "b", "c"], "n_1": [1.0, 2.0, 3.0]***REMOVED***)
    ...
    def test_setitem_period_preserves_dtype(self):
        ...
        expected = DataFrame({"a": data***REMOVED***)
...
***REMOVED***,
    ...
    def test_setitem_extension_types(self, obj, dtype):
        # GH: 34832
        expected = DataFrame({"idx": [1, 2, 3], "obj": Series([obj] * 3, dtype=dtype)***REMOVED***)

        df = DataFrame({"idx": [1, 2, 3]***REMOVED***)
        df["obj"] = obj
    ...
    def test_setitem_with_ea_name(self, ea_name):
        ...
        result = DataFrame([0])
        result[ea_name] = [1]
        expected = DataFrame({0: [0], ea_name: [1]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_setitem_dt64_ndarray_with_NaT_and_diff_time_units(self):
        ...
        result["new"] = data_ns
        expected = DataFrame({0: [1, None], "new": [1, None]***REMOVED***, dtype="datetime64[ns]")
        tm.assert_frame_equal(result, expected)
    ...
    def test_frame_setitem_datetime64_col_other_units(self, unit):
        ...
        dtype = np.dtype(f"M8[{unit***REMOVED***]")
        ...
        df = DataFrame({"ints": np.arange(n)***REMOVED***, index=np.arange(n))
        df[unit] = vals
    ...
    def test_setitem_complete_column_with_array(self):
        # GH#37954
        df = DataFrame({"a": ["one", "two", "three"], "b": [1, 2, 3]***REMOVED***)
    ...
    def test_setitem_period_d_dtype(self):
        ...
        expected = DataFrame(
            {"A": ["NaT", "NaT", "NaT", "NaT", "NaT", "NaT", "NaT", "NaT", "NaT"]***REMOVED***,
            dtype="period[D]",
        )
    ...
    def test_setitem_frame_length_0_str_key(self, indexer):
        ...
        other = DataFrame({"B": [1, 2]***REMOVED***)
        df[indexer] = other
        expected = DataFrame({"A": [np.nan] * 2, "B": [1, 2]***REMOVED***)
    ...
    def test_setitem_intervals(self):
        df = DataFrame({"A": range(10)***REMOVED***)
        ser = cut(df["A"], 5)
    ...
    def test_setitem_categorical(self):
        # GH#35369
        df = DataFrame({"h": Series(list("mn")).astype("category")***REMOVED***)
        df.h = df.h.cat.reorder_categories(["n", "m"])
        expected = DataFrame(
            {"h": Categorical(["m", "n"]).reorder_categories(["n", "m"])***REMOVED***
        )
        tm.assert_frame_equal(df, expected)
...
***REMOVED***,
    ...
    def test_setitem_iloc_generator(self):
        ...
        df.iloc[indexer] = 1
        expected = DataFrame({"a": [1, 1, 1], "b": [4, 1, 1]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_setitem_iloc_two_dimensional_generator(self):
        ...
        indexer = (x for x in [1, 2])
        df.iloc[indexer, 1] = 1
        expected = DataFrame({"a": [1, 2, 3], "b": [4, 1, 1]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_boolean_mask_nullable_int64(self):
        # GH 28928
        result = DataFrame({"a": [3, 4], "b": [5, 6]***REMOVED***).astype(
            {"a": "int64", "b": "Int64"***REMOVED***
        )
        mask = Series(False, index=result.index)
        result.loc[mask, "a"] = result["a"]
        result.loc[mask, "b"] = result["b"]
        expected = DataFrame({"a": [3, 4], "b": [5, 6]***REMOVED***).astype(
            {"a": "int64", "b": "Int64"***REMOVED***
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_setitem_ea_dtype_rhs_series(self):
        ...
        expected = DataFrame({"a": [1, 2]***REMOVED***, dtype="Int64")
        tm.assert_frame_equal(df, expected)
    ...
    def test_setitem_npmatrix_2d(self):
        ...
        expected = DataFrame(
            {"np-array": np.ones(10), "np-matrix": np.ones(10)***REMOVED***, index=np.arange(10)
        )
    ...
    @pytest.mark.parametrize("vals", [{***REMOVED***, {"d": "a"***REMOVED***])
    def test_setitem_aligning_dict_with_index(self, vals):
        # GH#47216
        df = DataFrame({"a": [1, 2], "b": [3, 4], **vals***REMOVED***)
        df.loc[:, "a"] = {1: 100, 0: 200***REMOVED***
        df.loc[:, "c"] = {0: 5, 1: 6***REMOVED***
        df.loc[:, "e"] = {1: 5***REMOVED***
        expected = DataFrame(
            {"a": [200, 100], "b": [3, 4], **vals, "c": [5, 6], "e": [np.nan, 5]***REMOVED***
        )
        tm.assert_frame_equal(df, expected)
    ...
    def test_setitem_rhs_dataframe(self):
        # GH#47578
        df = DataFrame({"a": [1, 2]***REMOVED***)
        df["a"] = DataFrame({"a": [10, 11]***REMOVED***, index=[1, 2])
        expected = DataFrame({"a": [np.nan, 10]***REMOVED***)
        tm.assert_frame_equal(df, expected)

        df = DataFrame({"a": [1, 2]***REMOVED***)
        df.isetitem(0, DataFrame({"a": [10, 11]***REMOVED***, index=[1, 2]))
        tm.assert_frame_equal(df, expected)

    def test_setitem_frame_overwrite_with_ea_dtype(self, any_numeric_ea_dtype):
        # GH#46896
        df = DataFrame(columns=["a", "b"], data=[[1, 2], [3, 4]])
        df["a"] = DataFrame({"a": [10, 11]***REMOVED***, dtype=any_numeric_ea_dtype)
        expected = DataFrame(
            {
    ...
    def test_setitem_string_option_object_index(self):
        ...
        expected = DataFrame({"a": [1, 2], "b": Series(["a", "b"], dtype=object)***REMOVED***)
        tm.assert_frame_equal(df, expected)

    def test_setitem_frame_midx_columns(self):
        # GH#49121
        df = DataFrame({("a", "b"): [10]***REMOVED***)
        expected = df.copy()
        col_name = ("a", "b")
    ...
    def test_loc_setitem_ea_dtype(self):
        # GH#55604
        df = DataFrame({"a": np.array([10], dtype="i8")***REMOVED***)
        df.loc[:, "a"] = Series([11], dtype="Int64")
        expected = DataFrame({"a": np.array([11], dtype="i8")***REMOVED***)
        tm.assert_frame_equal(df, expected)
...
class TestDataFrameSetItemWithExpansion:
    def test_setitem_listlike_views(self, using_copy_on_write, warn_copy_on_write):
        # GH#38148
        df = DataFrame({"a": [1, 2, 3], "b": [4, 4, 6]***REMOVED***)
    ...
    ***REMOVED***
        ...
        labels = Categorical([f"{i***REMOVED*** - {i + 499***REMOVED***" for i in range(0, 10000, 500)])
    ...
    def test_frame_setitem_rangeindex_into_new_col(self):
        ...
        result = df.loc[[1], :]
        expected = DataFrame({"a": ["b"], "b": [100]***REMOVED***, index=[1])
        tm.assert_frame_equal(result, expected)

    def test_setitem_frame_keep_ea_dtype(self, any_numeric_ea_dtype):
        # GH#46896
        df = DataFrame(columns=["a", "b"], data=[[1, 2], [3, 4]])
        df["c"] = DataFrame({"a": [10, 11]***REMOVED***, dtype=any_numeric_ea_dtype)
        expected = DataFrame(
            {
                "a": [1, 3],
                "b": [2, 4],
...
class TestDataFrameSetItemCallable:
    def test_setitem_callable(self):
        # GH#12533
        df = DataFrame({"A": [1, 2, 3, 4], "B": [5, 6, 7, 8]***REMOVED***)
        df[lambda x: "A"] = [11, 12, 13, 14]

        exp = DataFrame({"A": [11, 12, 13, 14], "B": [5, 6, 7, 8]***REMOVED***)
        tm.assert_frame_equal(df, exp)
...
class TestDataFrameSetItemBooleanMask:
    ...
    def test_setitem_loc_empty_indexer_raises_with_non_empty_value(self, box):
        # GH#37672
        df = DataFrame({"a": ["a"], "b": [1], "c": [1]***REMOVED***)
        if box == Series:
    ...
    def test_setitem_loc_only_false_indexer_dtype_changed(self, box):
        ...
        expected = DataFrame({"a": ["a"], "b": [1], "c": [1]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_setitem_boolean_mask_aligning(self, indexer):
        # GH#39931
        df = DataFrame({"a": [1, 4, 2, 3], "b": [5, 6, 7, 8]***REMOVED***)
        expected = df.copy()
        mask = df["a"] >= 3
    ...
    def test_setitem_mask_categorical(self):
        ...
        exp_multi_row = DataFrame({"cats": cats2, "values": values2***REMOVED***, index=idx2)
        ...
        df = DataFrame({"cats": catsf, "values": valuesf***REMOVED***, index=idxf)
    ...
    @pytest.mark.parametrize("kwargs", [{***REMOVED***, {"index": [1]***REMOVED***, {"columns": ["A"]***REMOVED***])
...
***REMOVED***,
    ...
    def test_loc_setitem_all_false_boolean_two_blocks(self):
        # GH#40885
        df = DataFrame({"a": [1, 2], "b": [3, 4], "c": "a"***REMOVED***)
        expected = df.copy()
        indexer = Series([False, False], name="c")
        df.loc[indexer, ["b"]] = DataFrame({"b": [5, 6]***REMOVED***, index=[0, 1])
        tm.assert_frame_equal(df, expected)
...
class TestDataFrameSetitemCopyViewSemantics:
    ...
    def test_setitem_partial_column_inplace(
        ...
        df = DataFrame(
            {"x": [1.1, 2.1, 3.1, 4.1], "y": [5.1, 6.1, 7.1, 8.1]***REMOVED***, index=[0, 1, 2, 3]
        )
...
***REMOVED***,
    ...
    def test_setitem_not_operating_inplace(self, value, set_value, indexer):
        # GH#43406
        df = DataFrame({"a": value***REMOVED***, index=[0, 1])
        expected = df.copy()
        view = df[:]
    ...
    def test_setitem_column_update_inplace(
        ...
        labels = [f"c{i***REMOVED***" for i in range(10)]
        df = DataFrame({col: np.zeros(len(labels)) for col in labels***REMOVED***, index=labels)
    ...
    def test_setitem_iloc_with_numpy_array(self, dtype):
        # GH-33828
        df = DataFrame({"a": np.ones(3)***REMOVED***, dtype=dtype)
        df.iloc[np.array([0]), np.array([0])] = np.array([[2]])

        expected = DataFrame({"a": [2, 1, 1]***REMOVED***, dtype=dtype)
        tm.assert_frame_equal(df, expected)
    ...
    def test_frame_setitem_empty_dataframe(self):
        ...
        df = DataFrame({"date": dti***REMOVED***).set_index("date")
        df = df[0:0].copy()
...
def test_full_setter_loc_incompatible_dtype():
    ...
    with tm.assert_produces_warning(FutureWarning, match="incompatible dtype"):
        df.loc[:, "a"] = True
    expected = DataFrame({"a": [True, True]***REMOVED***)
    tm.assert_frame_equal(df, expected)

    df = DataFrame({"a": [1, 2]***REMOVED***)
    with tm.assert_produces_warning(FutureWarning, match="incompatible dtype"):
        df.loc[:, "a"] = {0: 3.5, 1: 4.5***REMOVED***
    expected = DataFrame({"a": [3.5, 4.5]***REMOVED***)
    tm.assert_frame_equal(df, expected)

    df = DataFrame({"a": [1, 2]***REMOVED***)
    df.loc[:, "a"] = {0: 3, 1: 4***REMOVED***
    expected = DataFrame({"a": [3, 4]***REMOVED***)
    tm.assert_frame_equal(df, expected)
