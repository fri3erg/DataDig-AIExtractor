...
def test_subset_column_selection(backend, using_copy_on_write):
    ...
    expected = DataFrame({"a": [0, 2, 3], "c": [0.1, 0.2, 0.3]***REMOVED***)
...
def test_subset_column_selection_modify_parent(backend, using_copy_on_write):
    ...
    expected = DataFrame({"a": [1, 2, 3], "c": [0.1, 0.2, 0.3]***REMOVED***)
    tm.assert_frame_equal(subset, expected)
...
def test_subset_row_slice(backend, using_copy_on_write, warn_copy_on_write):
    ...
    expected = DataFrame({"a": [0, 3], "b": [5, 6], "c": [0.2, 0.3]***REMOVED***, index=range(1, 3))
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
...
def test_subset_column_slice(
    ...
    expected = DataFrame({"b": [0, 5, 6], "c": np.array([7, 8, 9], dtype=dtype)***REMOVED***)
    tm.assert_frame_equal(subset, expected)
...
***REMOVED***
    ...
    expected = DataFrame(
        {"b": [0, 6], "c": np.array([8, 9], dtype=dtype)***REMOVED***, index=range(1, 3)
    )
    tm.assert_frame_equal(subset, expected)
    if mutate_parent:
        df_orig.iloc[1, 1] = 0
...
def test_subset_set_with_row_indexer(
    ...
    _, DataFrame, _ = backend
    df = DataFrame({"a": [1, 2, 3, 4], "b": [4, 5, 6, 7], "c": [0.1, 0.2, 0.3, 0.4]***REMOVED***)
    df_orig = df.copy()
    subset = df[1:4]
    ...
    expected = DataFrame(
        {"a": [0, 0, 4], "b": [0, 0, 7], "c": [0.0, 0.0, 0.4]***REMOVED***, index=range(1, 4)
    )
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
        # original parent dataframe is not modified (CoW)
...
def test_subset_set_with_mask(backend, using_copy_on_write, warn_copy_on_write):
    ...
    expected = DataFrame(
        {"a": [2, 3, 0], "b": [0, 0, 0], "c": [0.20, 0.3, 0.4]***REMOVED***, index=range(1, 4)
    )
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
        # original parent dataframe is not modified (CoW)
...
def test_subset_set_column(backend, using_copy_on_write, warn_copy_on_write):
    ...
    subset._mgr._verify_integrity()
    expected = DataFrame(
        {"a": [10, 11], "b": [5, 6], "c": [0.2, 0.3]***REMOVED***, index=range(1, 3)
    )
    tm.assert_frame_equal(subset, expected)
    tm.assert_frame_equal(df, df_orig)
...
***REMOVED***:
    ...
    subset._mgr._verify_integrity()
    expected = DataFrame(
        {"a": [10, 11], "b": [5, 6], "c": np.array([8, 9], dtype=dtype)***REMOVED***,
        index=range(1, 3),
    )
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
...
***REMOVED***:
    ...
    subset._mgr._verify_integrity()
    expected = DataFrame({"a": [0, 0]***REMOVED***, index=range(1, 3))
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
        # original parent dataframe is not modified (CoW)
        tm.assert_frame_equal(df, df_orig)
...
def test_subset_set_columns(backend, using_copy_on_write, warn_copy_on_write, dtype):
    ...
    expected = DataFrame({"a": [0, 0], "b": [5, 6], "c": [0, 0]***REMOVED***, index=range(1, 3))
...
def test_subset_set_with_column_indexer(
    ...
    _, DataFrame, _ = backend
    df = DataFrame({"a": [1, 2, 3], "b": [0.1, 0.2, 0.3], "c": [4, 5, 6]***REMOVED***)
    df_orig = df.copy()
    subset = df[1:3]
    ...
    subset._mgr._verify_integrity()
    expected = DataFrame({"a": [0, 0], "b": [0.0, 0.0], "c": [5, 6]***REMOVED***, index=range(1, 3))
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
        tm.assert_frame_equal(df, df_orig)
    else:
...
def test_subset_chained_getitem(
    ...
    with tm.assert_cow_warning(warn_copy_on_write and subset_is_view):
        df.iloc[0, 0] = 0
    expected = DataFrame({"a": [1, 2], "b": [4, 5]***REMOVED***)
    if using_copy_on_write or not subset_is_view:
        tm.assert_frame_equal(subset, expected)
    else:
...
def test_dataframe_add_column_from_series(backend, using_copy_on_write):
    ...
    s[0] = 0
    expected = DataFrame({"a": [1, 2, 3], "b": [0.1, 0.2, 0.3], "new": [10, 11, 12]***REMOVED***)
...
def test_set_value_copy_only_necessary_column(
    ...
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": col***REMOVED***)
    df_orig = df.copy()
    view = df[:]
...
def test_getitem_midx_slice(
    ...
    df = DataFrame({("a", "x"): [1, 2], ("a", "y"): 1, ("b", "x"): 2***REMOVED***)
    df_orig = df.copy()
    new_df = df[("a",)]
...
def test_midx_read_only_bool_indexer():
    # GH#56635
    def mklbl(prefix, n):
        return [f"{prefix***REMOVED***{i***REMOVED***" for i in range(n)]
...
def test_loc_enlarging_with_dataframe(using_copy_on_write):
    df = DataFrame({"a": [1, 2, 3]***REMOVED***)
    rhs = DataFrame({"b": [1, 2, 3], "c": [4, 5, 6]***REMOVED***)
    rhs_orig = rhs.copy()
    df.loc[:, ["b", "c"]] = rhs
    if using_copy_on_write:
