...
class Base64Binary(MatchPattern):
    ...
    pattern = "^(?:[A-Za-z0-9+/]{4***REMOVED***)*(?:[A-Za-z0-9+/]{2***REMOVED***==|[A-Za-z0-9+/]{3***REMOVED***=|[A-Za-z0-9+/]{4***REMOVED***)$"


class Guid(MatchPattern):
    # https://msdn.microsoft.com/en-us/library/dd946381(v=office.12).aspx
    pattern = r"{[0-9A-F]{8***REMOVED***-[0-9A-F]{4***REMOVED***-[0-9A-F]{4***REMOVED***-[0-9A-F]{4***REMOVED***-[0-9A-F]{12***REMOVED***\***REMOVED***"


class CellRange(MatchPattern):

    pattern = r"^[$]?([A-Za-z]{1,3***REMOVED***)[$]?(\d+)(:[$]?([A-Za-z]{1,3***REMOVED***)[$]?(\d+)?)?$|^[A-Za-z]{1,3***REMOVED***:[A-Za-z]{1,3***REMOVED***$"
    allow_none = True
...
def _explicit_none(tagname, value, namespace=None):
    ...
    if namespace is not None:
        tagname = "{%s***REMOVED***%s" % (namespace, tagname)
