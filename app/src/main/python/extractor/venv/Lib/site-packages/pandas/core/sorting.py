...
def get_indexer_indexer(
    ...
    kind : {'quicksort', 'mergesort', 'heapsort', 'stable'***REMOVED***
    na_position : {'first', 'last'***REMOVED***
    sort_remaining : bool
...
def nargminmax(values: ExtensionArray, method: str, axis: AxisInt = 0):
    ...
    int
    """
    assert method in {"argmax", "argmin"***REMOVED***
...
def ensure_key_mapped(
    ...
    try:
        ...
        raise TypeError(
            f"User-provided `key` function returned an invalid type {type(result)***REMOVED*** \
            which could not be converted to {type(values)***REMOVED***."
        )
