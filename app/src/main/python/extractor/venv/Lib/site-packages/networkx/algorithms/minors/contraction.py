...
def equivalence_classes(iterable, relation):
    ...
    The equivalence classes of this relation are `{0, 3, 6, 9***REMOVED***`, `{1, 4, 7***REMOVED***`,
    `{2, 5, 8***REMOVED***`: `0`, `3`, `6`, `9` are all divisible by `3` and leave zero
    ...
    {frozenset({1, 4, 7***REMOVED***), frozenset({8, 2, 5***REMOVED***), frozenset({0, 9, 3, 6***REMOVED***)***REMOVED***
    """
    ...
    for y in iterable:
        ...
        for block in blocks:
            ...
            # class for it.
            blocks.append([y])
    return {frozenset(block) for block in blocks***REMOVED***
...
def quotient_graph(
    ...
    *`Strongly connected component`_*.
    ...
    >>> G = nx.path_graph(6)
    >>> partition = [{0, 1***REMOVED***, {2, 3***REMOVED***, {4, 5***REMOVED***]
    ...
    >>> G = nx.path_graph(6)
    >>> partition = {0: {0, 1***REMOVED***, 2: {2, 3***REMOVED***, 4: {4, 5***REMOVED******REMOVED***
    >>> M = nx.quotient_graph(G, partition, relabel=True)
    >>> list(M.edges())
    [(0, 1), (1, 2)]
...
***REMOVED***
            return {"weight": sum(d.get(weight, 1) for d in edgedata)***REMOVED***
    ...
    if H.is_multigraph():
        edges = chaini(
            (
                (b, c, G.get_edge_data(u, v, default={***REMOVED***))
...
***REMOVED***
    ...
    if relabel:
        ...
        labels = {b: i for i, b in enumerate(partition)***REMOVED***
...
def contracted_nodes(G, u, v, self_loops=True, copy=True):
    ...
    for prev_w, prev_x, d in edges_to_remap:
        ...
        if ({prev_w, prev_x***REMOVED*** == {u, v***REMOVED***) and not self_loops:
            continue
        ...
        if not H.has_edge(w, x) or G.is_multigraph():
            ...
            if "contraction" in H.edges[(w, x)]:
                ...
                H.edges[(w, x)]["contraction"] = {(prev_w, prev_x): d***REMOVED***

    if "contraction" in H.nodes[u]:
        H.nodes[u]["contraction"][v] = v_data
    else:
        H.nodes[u]["contraction"] = {v: v_data***REMOVED***
    return H
...
def contracted_edge(G, edge, self_loops=True, copy=True):
    ...
    if not G.has_edge(u, v):
        raise ValueError(f"Edge {edge***REMOVED*** does not exist in graph G; cannot contract it")
