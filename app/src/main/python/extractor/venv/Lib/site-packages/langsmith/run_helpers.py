...
def _container_end(
    ...
    if run_tree is None:
        # Tracing disabled
        return
    outputs_ = outputs if isinstance(outputs, dict) else {"output": outputs***REMOVED***
...
def _collect_extra(extra_outer: dict, langsmith_extra: LangSmithExtra) -> dict:
    ...
    if run_extra:
        extra_inner = {**extra_outer, **run_extra***REMOVED***
    else:
...
def _setup_run(
    ...
    metadata_ = {
        **(langsmith_extra.get("metadata") or {***REMOVED***),
        **(outer_metadata or {***REMOVED***),
    ***REMOVED***
    _METADATA.set(metadata_)
    metadata_.update(metadata or {***REMOVED***)
    metadata_["ls_method"] = "traceable"
    extra_inner["metadata"] = metadata_
    try:
        inputs = _get_inputs(signature, *args, **kwargs)
    except TypeError as e:
        logger.debug(f"Failed to infer inputs for {name_***REMOVED***: {e***REMOVED***")
        inputs = {"args": args, "kwargs": kwargs***REMOVED***
...
def trace(
    ...
    *,
    ...
    _TAGS.set(tags_)
    metadata = {**(metadata or {***REMOVED***), **(outer_metadata or {***REMOVED***), "ls_method": "trace"***REMOVED***
...
def as_runnable(traceable_fn: Callable) -> Runnable:
    ...
    if not is_traceable_function(traceable_fn):
        ...
        raise ValueError(
            ...
            f" @traceable decorator. Got {traceable_fn***REMOVED*** defined as:\n{fn_src***REMOVED***"
        )
    ...
    class RunnableTraceable(RunnableLambda):
        ...
        ***REMOVED***
            ...
            if wrapped is None:
                raise ValueError(
                    f"{self.__class__.__name__***REMOVED*** expects a function wrapped by"
                    " the LangSmith"
                    f" @traceable decorator. Got {func***REMOVED***"
    ***REMOVED***
...
***REMOVED***
            ...
            if isinstance(callback_manager, (CallbackManager, AsyncCallbackManager)):
                ...
                if lc_tracers:
                    ...
                    run_tree = run_trees.RunTree(
                        ...
                        extra={"metadata": callback_manager.metadata***REMOVED***,
        ***REMOVED***
            return run_tree
    ...
    ***REMOVED***
                return func(**inputs, langsmith_extra={"run_tree": run_tree***REMOVED***)
        ...
        def _wrap_async(
            ...
            if not is_traceable_function(afunc):
                raise ValueError(
                    ...
                    f" @traceable decorator. Got {afunc***REMOVED***"
    ***REMOVED***
            ...
            async def awrap_traceable(inputs: dict, config: RunnableConfig) -> Any:
                run_tree = RunnableTraceable._configure_run_tree(
                    config.get("callbacks")
    ***REMOVED***
                return await afunc_(**inputs, langsmith_extra={"run_tree": run_tree***REMOVED***)
