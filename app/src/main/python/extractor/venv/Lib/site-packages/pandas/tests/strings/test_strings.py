...
def test_startswith_endswith_non_str_patterns(pattern):
    # GH3485
    ser = Series(["foo", "bar"])
    msg = f"expected a string or tuple, not {type(pattern).__name__***REMOVED***"
...
def test_zfill_with_non_integer_argument():
    value = Series(["-2", "+5"])
    wid = "a"
    msg = f"width must be of integer type, not {type(wid).__name__***REMOVED***"
    with pytest.raises(TypeError, match=msg):
        value.str.zfill(wid)
...
def test_get_with_dict_label():
    ...
    s = Series(
        [
            {"name": "Hello", "value": "World"***REMOVED***,
