    ...
    def __from(
        ...
        **kwargs: Any,
        ...
        if metric not in INDEX_METRICS:
            raise ValueError(
                (
                    f"Unsupported distance metric: {metric***REMOVED***. "
                    f"Expected one of {list(INDEX_METRICS)***REMOVED***"
    ***REMOVED***
***REMOVED***
        ...
        index_to_id = {i: str(uuid.uuid4()) for i in range(len(documents))***REMOVED***
        docstore = InMemoryDocstore(
            {index_to_id[i]: doc for i, doc in enumerate(documents)***REMOVED***
        )
