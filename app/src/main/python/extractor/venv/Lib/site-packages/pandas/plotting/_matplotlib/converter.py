...
_mpl_units = {***REMOVED***  # Cache for units overwritten by us
...
def deregister() -> None:
    ...
    for unit, formatter in _mpl_units.items():
        if type(formatter) not in {DatetimeConverter, PeriodConverter, TimeConverter***REMOVED***:
...
def get_datevalue(date, freq):
    if isinstance(date, Period):
        ...
        return None
    raise ValueError(f"Unrecognizable date '{date***REMOVED***'")
    ...
    def __call__(self):
        ...
        if estimate > self.MAXTICKS * 2:
            raise RuntimeError(
                ...
                f"{estimate:d***REMOVED*** ticks from {dmin***REMOVED*** to {dmax***REMOVED***: exceeds Locator.MAXTICKS"
                f"* 2 ({self.MAXTICKS * 2:d***REMOVED***) "
***REMOVED***

        interval = self._get_interval()
        freq = f"{interval***REMOVED***ms"
...
def _get_periods_per_ymd(freq: BaseOffset) -> tuple[int, int, int]:
    ...
    if dtype_code >= FreqGroup.FR_HR.value:
        ...
        raise NotImplementedError(f"Unsupported frequency: {dtype_code***REMOVED***")
...
class TimeSeries_DateLocator(Locator):
    ...
    minor_locator : {False, True***REMOVED***, optional
        Whether the locator is for minor ticks (True) or not.
    dynamic_mode : {True, False***REMOVED***, optional
        Whether the locator should work in dynamic mode.
    base : {int***REMOVED***, optional
    quarter : {int***REMOVED***, optional
    month : {int***REMOVED***, optional
    day : {int***REMOVED***, optional
    """
...
class TimeSeries_DateFormatter(Formatter):
    ...
    def _set_default_format(self, vmin, vmax):
        ...
        self.formatdict = {x: f for (x, _, _, f) in format***REMOVED***
...
class TimeSeries_TimedeltaFormatter(Formatter):
    ...
    def format_timedelta_ticks(x, pos, n_decimals: int) -> str:
        ...
        s = f"{int(h):02d***REMOVED***:{int(m):02d***REMOVED***:{int(s):02d***REMOVED***"
        if n_decimals > 0:
            s += f".{decimals:0{n_decimals***REMOVED***d***REMOVED***"
        if d != 0:
            s = f"{int(d):d***REMOVED*** days {s***REMOVED***"
        return s
