...
***REMOVED***.grid(column=i * 2 + 1, row=0)
    ...
    def show_trace(self, *e):
        ...
        ***REMOVED***
        self.linenum = {i: i * 2 + 2 for i in range(len(rules) + 1)***REMOVED***
    ...
    ***REMOVED***
                ...
                C = "1.0 + %d chars"
                for (tag, params) in self.HELP_AUTOTAG:
                    pattern = f"(?s)(<{tag***REMOVED***>)(.*?)(</{tag***REMOVED***>)"
    ...
    def _view_history(self, index):
        ...
        if self._history_index < len(self._history) - 1:
            self.grammarlabel["text"] = "Grammar {***REMOVED***/{***REMOVED***:".format(
...
***REMOVED***
    ...
    def _devset_scroll(self, command, *args):
        ...
        if command == "scroll" and args[1].startswith("unit"):
            ...
            assert 0, f"bad scroll command {command***REMOVED*** {args***REMOVED***"
        if showing_trace:
            self.show_trace()
    ...
    def show_devset(self, index=None):
        ...
        self.charnum = {***REMOVED***
        self.linenum = {0: 1***REMOVED***
        for sentnum, sent in enumerate(sample):
            ...
            for wordnum, (word, pos) in enumerate(sent.leaves()):
                ...
                linestr += f"{word***REMOVED***/{pos***REMOVED*** "
    ...
    def _syntax_highlight_grammar(self, grammar):
        ...
        for lineno, line in enumerate(grammar.split("\n")):
            ...
            for m in re.finditer("[<>{***REMOVED***]", line):
    ...
    def _chunkparse(self, words):
        try:
            ...
            # E.g., this is caused by: "({<NN>***REMOVED***)"
    ...
    def _color_chunk(self, sentnum, chunk, tag):
        ...
        self.devsetbox.tag_add(
            tag,
            f"{self.linenum[sentnum]***REMOVED***.{self.charnum[sentnum, start]***REMOVED***",
            f"{self.linenum[sentnum]***REMOVED***.{self.charnum[sentnum, end] - 1***REMOVED***",
        )
