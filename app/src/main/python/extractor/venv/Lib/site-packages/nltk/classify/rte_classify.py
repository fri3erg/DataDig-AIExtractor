...
class RTEFeatureExtractor:
    ...
    def __init__(self, rtepair, stop=True, use_lemmatize=False):
        ...
        ***REMOVED***
        ...
        self.negwords = {"no", "not", "never", "failed", "rejected", "denied"***REMOVED***
        ...
        if use_lemmatize:
            self.text_words = {self._lemmatize(token) for token in self.text_tokens***REMOVED***
            self.hyp_words = {self._lemmatize(token) for token in self.hyp_tokens***REMOVED***
    ...
    def overlap(self, toktype, debug=False):
        ...
        """
        ne_overlap = {token for token in self._overlap if self._ne(token)***REMOVED***
        if toktype == "ne":
            if debug:
    ...
    def hyp_extra(self, toktype, debug=True):
        ...
        :type toktype: 'ne' or 'word'
        """
        ne_extra = {token for token in self._hyp_extra if self._ne(token)***REMOVED***
        if toktype == "ne":
            return ne_extra
        elif toktype == "word":
