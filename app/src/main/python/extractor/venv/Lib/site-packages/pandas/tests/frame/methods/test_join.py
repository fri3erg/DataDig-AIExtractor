...
def left_w_dups(left_no_dup):
    return concat(
        [left_no_dup, DataFrame({"a": ["a"], "b": ["cow"]***REMOVED***, index=[3])], sort=True
    )
...
def right_w_dups(right_no_dup):
    return concat(
        [right_no_dup, DataFrame({"a": ["e"], "c": ["moo"]***REMOVED***, index=[3])]
    ).set_index("a")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "right",
            False,
            DataFrame({"a": [np.nan, 10, 20], "b": [300, 100, 200]***REMOVED***, index=[3, 1, 2]),
        ),
        (
            "right",
            True,
            DataFrame({"a": [10, 20, np.nan], "b": [100, 200, 300]***REMOVED***, index=[1, 2, 3]),
        ),
        (
            "outer",
            False,
...
def test_suffix_on_list_join():
    first = DataFrame({"key": [1, 2, 3, 4, 5]***REMOVED***)
    second = DataFrame({"key": [1, 8, 3, 2, 5], "v1": [1, 2, 3, 4, 5]***REMOVED***)
    third = DataFrame({"keys": [5, 2, 3, 4, 1], "v2": [1, 2, 3, 4, 5]***REMOVED***)
...
def test_join_period_index(frame_with_period_index):
    other = frame_with_period_index.rename(columns=lambda key: f"{key***REMOVED***{key***REMOVED***")
...
def test_join_left_sequence_non_unique_index():
    ...
    df1 = DataFrame({"a": [0, 10, 20]***REMOVED***, index=[1, 2, 3])
    df2 = DataFrame({"b": [100, 200, 300]***REMOVED***, index=[4, 3, 2])
    df3 = DataFrame({"c": [400, 500, 600]***REMOVED***, index=[2, 2, 4])
...
def test_suppress_future_warning_with_sort_kw(sort_kw):
    a = DataFrame({"col1": [1, 2]***REMOVED***, index=["c", "a"])

    b = DataFrame({"col2": [4, 5]***REMOVED***, index=["b", "a"])

    c = DataFrame({"col3": [7, 8]***REMOVED***, index=["a", "b"])

    expected = DataFrame(
        {
            "col1": {"a": 2.0, "b": float("nan"), "c": 1.0***REMOVED***,
            "col2": {"a": 5.0, "b": 4.0, "c": float("nan")***REMOVED***,
            "col3": {"a": 7.0, "b": 8.0, "c": float("nan")***REMOVED***,
        ***REMOVED***
    )
    if sort_kw is False:
...
class TestDataFrameJoin:
    ...
    def test_join_segfault(self):
        # GH#1532
        df1 = DataFrame({"a": [1, 1], "b": [1, 2], "x": [1, 2]***REMOVED***)
        df2 = DataFrame({"a": [2, 2], "b": [1, 2], "y": [1, 2]***REMOVED***)
