...
class YouRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(
        ...
        headers = {"X-API-Key": self.ydc_api_key***REMOVED***
        if self.endpoint_type == "web":
            results = requests.get(
                f"https://api.ydc-index.io/search?query={query***REMOVED***",
                headers=headers,
***REMOVED***.json()
            ...
            results = requests.get(
                f"https://api.ydc-index.io/snippet_search?query={query***REMOVED***",
                headers=headers,
***REMOVED***.json()
            return [Document(page_content=snippet) for snippet in results]
        else:
            raise RuntimeError(f"Invalid endpoint type provided {self.endpoint_type***REMOVED***")
