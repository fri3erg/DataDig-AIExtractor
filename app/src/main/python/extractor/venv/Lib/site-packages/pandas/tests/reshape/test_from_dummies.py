...
def test_error_no_prefix_contains_unassigned():
    dummies = DataFrame({"a": [1, 0, 0], "b": [0, 1, 0]***REMOVED***)
    with pytest.raises(
        ValueError,
        match=(
...
def test_error_no_prefix_wrong_default_category_type():
    dummies = DataFrame({"a": [1, 0, 1], "b": [0, 1, 1]***REMOVED***)
    with pytest.raises(
        TypeError,
        match=(
...
def test_error_no_prefix_contains_nan():
    dummies = DataFrame({"a": [1, 0, 0], "b": [0, 1, np.nan]***REMOVED***)
    with pytest.raises(
...
def test_error_contains_non_dummies():
    dummies = DataFrame(
        {"a": [1, 6, 3, 1], "b": [0, 1, 0, 2], "c": ["c1", "c2", "c3", "c4"]***REMOVED***
    )
    with pytest.raises(
        TypeError,
...
def test_error_with_prefix_default_category_dict_not_complete(
    ...
    with pytest.raises(
        ...
        from_dummies(dummies_with_unassigned, sep="_", default_category={"col1": "x"***REMOVED***)
...
def test_roundtrip_series_to_dataframe():
    ...
    expected = DataFrame({"": ["a", "b", "c", "a"]***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_roundtrip_single_column_dataframe():
    categories = DataFrame({"": ["a", "b", "c", "a"]***REMOVED***)
    dummies = get_dummies(categories)
...
def test_roundtrip_with_prefixes():
    categories = DataFrame({"col1": ["a", "b", "a"], "col2": ["b", "a", "c"]***REMOVED***)
    dummies = get_dummies(categories)
    result = from_dummies(dummies, sep="_")
    expected = categories
    tm.assert_frame_equal(result, expected)
...
def test_no_prefix_string_cats_basic():
    dummies = DataFrame({"a": [1, 0, 0, 1], "b": [0, 1, 0, 0], "c": [0, 0, 1, 0]***REMOVED***)
    expected = DataFrame({"": ["a", "b", "c", "a"]***REMOVED***)
    result = from_dummies(dummies)
    tm.assert_frame_equal(result, expected)
...
def test_no_prefix_string_cats_basic_mixed_bool_values():
    dummies = DataFrame(
        {"a": [1, 0, 0, 1], "b": [False, True, False, False], "c": [0, 0, 1, 0]***REMOVED***
    )
    expected = DataFrame({"": ["a", "b", "c", "a"]***REMOVED***)
    result = from_dummies(dummies)
    tm.assert_frame_equal(result, expected)
...
def test_no_prefix_int_cats_basic():
    dummies = DataFrame(
        {1: [1, 0, 0, 0], 25: [0, 1, 0, 0], 2: [0, 0, 1, 0], 5: [0, 0, 0, 1]***REMOVED***
    )
    expected = DataFrame({"": [1, 25, 2, 5]***REMOVED***)
    result = from_dummies(dummies)
    tm.assert_frame_equal(result, expected)
...
def test_no_prefix_float_cats_basic():
    dummies = DataFrame(
        {1.0: [1, 0, 0, 0], 25.0: [0, 1, 0, 0], 2.5: [0, 0, 1, 0], 5.84: [0, 0, 0, 1]***REMOVED***
    )
    expected = DataFrame({"": [1.0, 25.0, 2.5, 5.84]***REMOVED***)
    result = from_dummies(dummies)
    tm.assert_frame_equal(result, expected)
...
def test_no_prefix_mixed_cats_basic():
    ...
    expected = DataFrame({"": [1.23, "c", 2, False, None]***REMOVED***, dtype="object")
    result = from_dummies(dummies)
    tm.assert_frame_equal(result, expected)


def test_no_prefix_string_cats_contains_get_dummies_NaN_column():
    dummies = DataFrame({"a": [1, 0, 0], "b": [0, 1, 0], "NaN": [0, 0, 1]***REMOVED***)
    expected = DataFrame({"": ["a", "b", "NaN"]***REMOVED***)
    result = from_dummies(dummies)
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "c",
            DataFrame({"": ["a", "b", "c"]***REMOVED***),
            id="default_category is a str",
        ),
        pytest.param(
            1,
            DataFrame({"": ["a", "b", 1]***REMOVED***),
            id="default_category is a int",
        ),
        pytest.param(
            1.25,
            DataFrame({"": ["a", "b", 1.25]***REMOVED***),
            id="default_category is a float",
        ),
        pytest.param(
            0,
            DataFrame({"": ["a", "b", 0]***REMOVED***),
            id="default_category is a 0",
        ),
        pytest.param(
            False,
            DataFrame({"": ["a", "b", False]***REMOVED***),
            id="default_category is a bool",
        ),
        pytest.param(
            (1, 2),
            DataFrame({"": ["a", "b", (1, 2)]***REMOVED***),
...
def test_with_prefix_basic(dummies_basic):
    expected = DataFrame({"col1": ["a", "b", "a"], "col2": ["b", "a", "c"]***REMOVED***)
...
def test_with_prefix_contains_get_dummies_NaN_column():
    ...
    expected = DataFrame({"col1": ["a", "b", "NaN"], "col2": ["NaN", "a", "c"]***REMOVED***)
    result = from_dummies(dummies, sep="_")
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "x",
            DataFrame({"col1": ["a", "b", "x"], "col2": ["x", "a", "c"]***REMOVED***),
            id="default_category is a str",
        ),
        pytest.param(
            0,
            DataFrame({"col1": ["a", "b", 0], "col2": [0, "a", "c"]***REMOVED***),
            id="default_category is a 0",
        ),
        pytest.param(
            False,
            DataFrame({"col1": ["a", "b", False], "col2": [False, "a", "c"]***REMOVED***),
            id="default_category is a False",
        ),
        pytest.param(
            {"col2": 1, "col1": 2.5***REMOVED***,
            DataFrame({"col1": ["a", "b", 2.5], "col2": [1, "a", "c"]***REMOVED***),
            id="default_category is a dict with int and float values",
        ),
        pytest.param(
            {"col2": None, "col1": False***REMOVED***,
            DataFrame({"col1": ["a", "b", False], "col2": [None, "a", "c"]***REMOVED***),
            id="default_category is a dict with bool and None values",
        ),
        pytest.param(
            {"col2": (1, 2), "col1": [1.25, False]***REMOVED***,
            DataFrame({"col1": ["a", "b", [1.25, False]], "col2": [(1, 2), "a", "c"]***REMOVED***),
...
def test_ea_categories():
    # GH 54300
    df = DataFrame({"a": [1, 0, 0, 1], "b": [0, 1, 0, 0], "c": [0, 0, 1, 0]***REMOVED***)
    df.columns = df.columns.astype("string[python]")
    result = from_dummies(df)
    expected = DataFrame({"": Series(list("abca"), dtype="string[python]")***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_maintain_original_index():
    # GH 54300
    df = DataFrame(
        {"a": [1, 0, 0, 1], "b": [0, 1, 0, 0], "c": [0, 0, 1, 0]***REMOVED***, index=list("abcd")
    )
    result = from_dummies(df)
    expected = DataFrame({"": list("abca")***REMOVED***, index=list("abcd"))
    tm.assert_frame_equal(result, expected)
