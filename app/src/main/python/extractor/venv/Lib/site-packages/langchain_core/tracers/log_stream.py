...
class RunLogPatch:
    ...
    def __add__(self, other: Union[RunLogPatch, Any]) -> RunLog:
        ...
        raise TypeError(
            f"unsupported operand type(s) for +: '{type(self)***REMOVED***' and '{type(other)***REMOVED***'"
        )
    ...
    def __repr__(self) -> str:
        ...
        return f"RunLogPatch({pformat(self.ops)[1:-1]***REMOVED***)"
...
class RunLog(RunLogPatch):
    ...
    def __repr__(self) -> str:
        ...
        return f"RunLog({pformat(self.state)***REMOVED***)"
...
class LogStreamCallbackHandler(BaseTracer):
    ...
    def __init__(
        ...
        *,
        ...
        self._key_map_by_run_id: Dict[UUID, str] = {***REMOVED***
                ...
                if key := self._key_map_by_run_id.get(run_id):
                    self.send_stream.send_nowait(
                        RunLogPatch(
                            {
                                "op": "add",
                                "path": f"/logs/{key***REMOVED***/streamed_output/-",
                                "value": chunk,
                            ***REMOVED***
            ***REMOVED***
        ***REMOVED***
...
***REMOVED***
        ...
        with self.lock:
            ...
            self._key_map_by_run_id[run.id] = (
                run.name if count == 1 else f"{run.name***REMOVED***:{count***REMOVED***"
***REMOVED***
        ...
        self.send_stream.send_nowait(
            RunLogPatch(
                {
                    "op": "add",
                    "path": f"/logs/{self._key_map_by_run_id[run.id]***REMOVED***",
                    "value": LogEntry(
                        id=str(run.id),
                        name=run.name,
                        type=run.run_type,
                        tags=run.tags or [],
                        metadata=(run.extra or {***REMOVED***).get("metadata", {***REMOVED***),
    ...
    def _on_llm_new_token(
        ...
        self.send_stream.send_nowait(
            RunLogPatch(
                {
                    "op": "add",
                    "path": f"/logs/{index***REMOVED***/streamed_output_str/-",
                    "value": token,
                ***REMOVED***,
                {
                    "op": "add",
                    "path": f"/logs/{index***REMOVED***/streamed_output/-",
                    "value": chunk.message
