...
CREATE TABLE IF NOT EXISTS {self.config.database***REMOVED***.{self.config.table***REMOVED***(    
    ...
    {self.config.column_map['document']***REMOVED*** string,
    {self.config.column_map['embedding']***REMOVED*** array<float>,
    {self.config.column_map['metadata']***REMOVED*** string
...
"""
    ...
    def _build_insert_sql(self, transac: Iterable, column_names: Iterable[str]) -> str:
        ...
        for n in transac:
            n = ",".join(
                [
                    f"'{self.escape_str(str(_n))***REMOVED***'"
                    if idx != embed_tuple_index
                    else f"array<float>{str(_n)***REMOVED***"
...
***REMOVED***:
    ...
    ***REMOVED***
    ...
    def __repr__(self) -> str:
        ...
        _repr += f"|\033[94m{columns[0]:24s***REMOVED***\033[0m|\033[96m{columns[1]:24s***REMOVED***"
        _repr += f"\033[0m|\033[96m{columns[2]:24s***REMOVED***\033[0m|\n"
        _repr += "-" * (width * fields + 1) + "\n"
        q_str = f"DESC {self.config.database***REMOVED***.{self.config.table***REMOVED***"
        debug_output(q_str)
        rs = get_named_result(self.connection, q_str)
        for r in rs:
            _repr += f"|\033[94m{r['Field']:24s***REMOVED***\033[0m|\033[96m{r['Type']:24s***REMOVED***"
            _repr += f"\033[0m|\033[96m{r['Key']:24s***REMOVED***\033[0m|\n"
        _repr += "-" * (width * fields + 1) + "\n"
        return _repr
    ...
    def _build_query_sql(
        ...
        if where_str:
            where_str = f"WHERE {where_str***REMOVED***"
        else:
            where_str = ""

        q_str = f"""
            SELECT {self.config.column_map['document']***REMOVED***, 
                {self.config.column_map['metadata']***REMOVED***, 
                cosine_similarity_norm(array<float>[{q_emb_str***REMOVED***],
                  {self.config.column_map['embedding']***REMOVED***) as dist
            FROM {self.config.database***REMOVED***.{self.config.table***REMOVED***
            {where_str***REMOVED***
            ORDER BY dist {self.dist_order***REMOVED***
            LIMIT {topk***REMOVED***
            """
