...
# ***REMOVED*** (end orthographic context constants)
######################################################################
...
# ***REMOVED*** (end decision reasons for debugging)
######################################################################
...
class PunktLanguageVars:
    ...
    re_boundary_realignment = re.compile(r'["\')\]***REMOVED***]+?(?:\s+|(?=--)|$)', re.MULTILINE)
    """Used to realign punctuation that should be included in a sentence
    although it follows the period (or ?, !)."""

    _re_word_start = r"[^\(\"\`{\[:;&\#\*@\)***REMOVED***\]\-,]"
    """Excludes some characters from starting word tokens"""

    @property
    def _re_non_word_chars(self):
        return r"(?:[)\";***REMOVED***\]\*:@\'\({\[%s])" % re.escape(
            "".join(set(self.sent_end_chars) - {"."***REMOVED***)
        )

    """Characters that cannot appear within words"""

    _re_multi_char_punct = r"(?:\-{2,***REMOVED***|\.{2,***REMOVED***|(?:\.\s){2,***REMOVED***\.)"
...
class PunktToken:
    ...
    def __repr__(self):
        ...
        propvals = ", ".join(
            f"{p***REMOVED***={repr(getattr(self, p))***REMOVED***"
...
class PunktTrainer(PunktBaseClass):
        ...
        for abbr, score, is_add in self._reclassify_abbrev_types(unique_types):
            if score >= self.ABBREV:
                if is_add:
                    ...
                    if verbose:
                        print(f"  Abbreviation: [{score:6.4f***REMOVED***] {abbr***REMOVED***")
            else:
                if not is_add:
                    self._params.abbrev_types.remove(abbr)
                    if verbose:
                        print(f"  Removed abbreviation: [{score:6.4f***REMOVED***] {abbr***REMOVED***")
    ...
    def _unique_types(self, tokens):
        return {aug_tok.type for aug_tok in tokens***REMOVED***
    ...
    def finalize_training(self, verbose=False):
        ...
        for typ, log_likelihood in self._find_sent_starters():
            ...
            if verbose:
                print(f"  Sent Starter: [{log_likelihood:6.4f***REMOVED***] {typ!r***REMOVED***")
        ...
        for (typ1, typ2), log_likelihood in self._find_collocations():
            ...
            if verbose:
                print(f"  Collocation: [{log_likelihood:6.4f***REMOVED***] {typ1!r***REMOVED***+{typ2!r***REMOVED***")
...
DEBUG_DECISION_FMT = """Text: {text!r***REMOVED*** (at offset {period_index***REMOVED***)
Sentence break? {break_decision***REMOVED*** ({reason***REMOVED***)
Collocation? {collocation***REMOVED***
{type1!r***REMOVED***:
    known abbreviation: {type1_in_abbrs***REMOVED***
    is initial: {type1_is_initial***REMOVED***
{type2!r***REMOVED***:
    known sentence starter: {type2_is_sent_starter***REMOVED***
    orthographic heuristic suggests is a sentence starter? {type2_ortho_heuristic***REMOVED***
    orthographic contexts in training: {type2_ortho_contexts***REMOVED***
"""
