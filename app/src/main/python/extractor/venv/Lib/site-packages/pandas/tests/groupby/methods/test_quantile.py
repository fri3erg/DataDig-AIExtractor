...
***REMOVED***
    ...
    df = DataFrame({"key": ["a"] * len(a_vals) + ["b"] * len(b_vals), "val": all_vals***REMOVED***)
...
def test_quantile_array():
    ...
    df = DataFrame({"A": [0, 1, 2, 3, 4]***REMOVED***)
    key = np.array([0, 0, 1, 1, 1], dtype=np.int64)
    result = df.groupby(key).quantile([0.25])

    index = pd.MultiIndex.from_product([[0, 1], [0.25]])
    expected = DataFrame({"A": [0.25, 2.50]***REMOVED***, index=index)
    tm.assert_frame_equal(result, expected)

    df = DataFrame({"A": [0, 1, 2, 3], "B": [4, 5, 6, 7]***REMOVED***)
    ...
    expected = DataFrame(
        {"A": [0.25, 0.75, 2.25, 2.75], "B": [4.25, 4.75, 6.25, 6.75]***REMOVED***, index=index
    )
    tm.assert_frame_equal(result, expected)
...
def test_quantile_array_no_sort():
    ...
    expected = DataFrame(
        {"A": [0.5, 1.0, 1.5, 1.0, 1.0, 1.0], "B": [3.5, 4.0, 4.5, 4.0, 4.0, 4.0]***REMOVED***,
    ...
    expected = DataFrame(
        {"A": [1.5, 0.5, 1.0, 1.0], "B": [4.5, 3.5, 4.0, 4.0]***REMOVED***,
...
def test_quantile_array_multiple_levels():
    df = DataFrame(
        {"A": [0, 1, 2], "B": [3, 4, 5], "c": ["a", "a", "a"], "d": ["a", "a", "b"]***REMOVED***
    )
    ...
    expected = DataFrame(
        {"A": [0.25, 0.75, 2.0, 2.0], "B": [3.25, 3.75, 5.0, 5.0]***REMOVED***, index=index
    )
    tm.assert_frame_equal(result, expected)
...
def test_quantile_missing_group_values_no_segfaults():
    ...
    df = DataFrame({"key": data, "val": range(3)***REMOVED***)
...
def test_quantile_missing_group_values_correct_results(
    ...
    df = DataFrame({"key": key, "val": val***REMOVED***)
...
def test_groupby_quantile_nullable_array(values, q):
    ...
    df = DataFrame({"a": ["x"] * 3 + ["y"] * 3, "b": values***REMOVED***)
...
def test_groupby_quantile_raises_on_invalid_dtype(q, numeric_only):
    df = DataFrame({"a": [1], "b": [2.0], "c": ["x"]***REMOVED***)
    if numeric_only:
...
def test_groupby_quantile_NA_float(any_float_dtype):
    # GH#42849
    df = DataFrame({"x": [1, 1], "y": [0.2, np.nan]***REMOVED***, dtype=any_float_dtype)
...
def test_groupby_quantile_NA_int(any_int_ea_dtype):
    # GH#42849
    df = DataFrame({"x": [1, 1], "y": [2, 5]***REMOVED***, dtype=any_int_ea_dtype)
    result = df.groupby("x")["y"].quantile(0.5)
    expected = pd.Series(
        [3.5],
        dtype="Float64",
    ...
    expected = DataFrame(
        {"y": 3.5***REMOVED***, dtype="Float64", index=Index([1], name="x", dtype=any_int_ea_dtype)
    )
    tm.assert_frame_equal(result, expected)
...
def test_groupby_quantile_all_na_group_masked(
    ...
    # GH#37493
    df = DataFrame(
        {"a": [1, 1, 1, 2], "b": [1, 2, 3, pd.NA]***REMOVED***, dtype=any_numeric_ea_dtype
    )
    result = df.groupby("a").quantile(q=[0.5, 0.7], interpolation=interpolation)
    expected = DataFrame(
        {"b": [val1, val2, pd.NA, pd.NA]***REMOVED***,
...
def test_groupby_quantile_allNA_column(dtype):
    # GH#42849
    df = DataFrame({"x": [1, 1], "y": [pd.NA] * 2***REMOVED***, dtype=dtype)
    result = df.groupby("x")["y"].quantile(0.5)
    expected = pd.Series(
...
def test_groupby_timedelta_quantile():
    # GH: 29485
    df = DataFrame(
        {"value": pd.to_timedelta(np.arange(4), unit="s"), "group": [1, 1, 2, 2]***REMOVED***
    )
...
def test_timestamp_groupby_quantile(unit):
    ...
    expected = DataFrame(
        [
            {"category": 12.8, "value": 112.8***REMOVED***,
            {"category": 48.2, "value": 148.2***REMOVED***,
            {"category": 68.8, "value": 168.8***REMOVED***,
            {"category": 92.2, "value": 192.2***REMOVED***,
        ],
        index=mi,
    )
...
def test_groupby_quantile_dt64tz_period():
    ...
    exp = {i: df.iloc[i::5].quantile(0.5) for i in range(5)***REMOVED***
