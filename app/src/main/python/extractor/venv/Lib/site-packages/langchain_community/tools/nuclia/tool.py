...
class NucliaUnderstandingAPI(BaseTool):
    ...
    _results: Dict[str, Any] = {***REMOVED***
    _config: Dict[str, Any] = {***REMOVED***

    def __init__(self, enable_ml: bool = False) -> None:
        zone = os.environ.get("NUCLIA_ZONE", "europe-1")
        self._config["BACKEND"] = f"https://{zone***REMOVED***.nuclia.cloud/api/v1"
    ...
    def _pushText(self, id: str, text: str) -> str:
        field = {
            "textfield": {"text": {"body": text, "format": 0***REMOVED******REMOVED***,
            "processing_options": {"ml_text": self._config["enable_ml"]***REMOVED***,
        ***REMOVED***
...
***REMOVED***
            if response.status_code != 200:
                logger.info(
                    f"Error uploading {content_path***REMOVED***: "
                    f"{response.status_code***REMOVED*** {response.text***REMOVED***"
    ***REMOVED***
                return ""
            else:
                field = {
                    "filefield": {"file": f"{response.text***REMOVED***"***REMOVED***,
                    "processing_options": {"ml_text": self._config["enable_ml"]***REMOVED***,
                ***REMOVED***
                return self._pushField(id, field)

    def _pushField(self, id: str, field: Any) -> str:
        logger.info(f"Pushing {id***REMOVED*** in queue")
        response = requests.post(
        ...
        if response.status_code != 200:
            logger.info(
                f"Error pushing field {id***REMOVED***:" f"{response.status_code***REMOVED*** {response.text***REMOVED***"
***REMOVED***
            raise ValueError("Error pushing field")
        else:
            uuid = response.json()["uuid"]
            logger.info(f"Field {id***REMOVED*** pushed in queue, uuid: {uuid***REMOVED***")
            self._results[id] = {"uuid": uuid, "status": "pending"***REMOVED***
            return uuid
    ...
    def _pull(self, id: str) -> str:
        ...
        if not result:
            logger.info(f"{id***REMOVED*** not in queue")
            return ""
        elif result["status"] == "pending":
            logger.info(f'Waiting for {result["uuid"]***REMOVED*** to be processed')
            return ""
        else:
            return result["data"]
...
***REMOVED*** from e
        ...
        if res["status"] == "empty":
            ...
            uuid = pb.uuid
            logger.info(f"Pulled {uuid***REMOVED*** from queue")
            matching_id = self._find_matching_id(uuid)
            if not matching_id:
                logger.info(f"No matching id for {uuid***REMOVED***")
            else:
