...
class CubeSemanticLoader(BaseLoader):
    ...
    def _get_dimension_values(self, dimension_name: str) -> List[str]:
        ...
        """
        logger.info("Loading dimension values for: {dimension_name***REMOVED***...")
        ...
        ***REMOVED***
        ...
        while retries < self.dimension_values_max_retries:
            response = requests.request(
                "POST",
                f"{self.cube_api_url***REMOVED***/load",
                headers=headers,
...
***REMOVED***
    ...
    ***REMOVED***:
    ...
    def load(self) -> List[Document]:
        ...
        ***REMOVED***
        ...
        logger.info(f"Loading metadata from {self.cube_api_url***REMOVED***...")
        response = requests.get(f"{self.cube_api_url***REMOVED***/meta", headers=headers)
        response.raise_for_status()
        raw_meta_json = response.json()
        cube_data_objects = raw_meta_json.get("cubes", [])

        logger.info(f"Found {len(cube_data_objects)***REMOVED*** cube data objects in metadata.")
        ...
        for cube_data_obj in cube_data_objects:
            ...
            logger.info(f"Processing {cube_data_obj_name***REMOVED***...")

            if not cube_data_obj_is_public:
                logger.info(f"Skipping {cube_data_obj_name***REMOVED*** because it is not public.")
                continue
    ...
    ***REMOVED***
                ...
                page_content = f"{str(item.get('title'))***REMOVED***, "
                page_content += f"{str(item.get('description'))***REMOVED***"
