...
class TestScalar:
    ...
    def test_iat_set_ints(self, kind, col, request):
        f = request.getfixturevalue(f"{kind***REMOVED***_{col***REMOVED***")
...
class TestAtAndiAT:
    ...
    def test_iat_setter_incompatible_assignment(self):
        # GH 23236
        result = DataFrame({"a": [0.0, 1.0], "b": [4, 5]***REMOVED***)
        result.iat[0, 0] = None
        expected = DataFrame({"a": [None, 1], "b": [4, 5]***REMOVED***)
...
def test_iat_dont_wrap_object_datetimelike():
    ...
    df = DataFrame({"A": ser, "B": ser2***REMOVED***)
...
def test_at_with_tuple_index_get():
    ...
    df = DataFrame({"a": [1, 2]***REMOVED***, index=[(1, 2), (3, 4)])
