...
def build_auxiliary_node_connectivity(G):
    ...
    for i, node in enumerate(G):
        mapping[node] = i
        H.add_node(f"{i***REMOVED***A", id=node)
        H.add_node(f"{i***REMOVED***B", id=node)
        H.add_edge(f"{i***REMOVED***A", f"{i***REMOVED***B", capacity=1)

    edges = []
    for source, target in G.edges():
        edges.append((f"{mapping[source]***REMOVED***B", f"{mapping[target]***REMOVED***A"))
        if not directed:
            edges.append((f"{mapping[target]***REMOVED***B", f"{mapping[source]***REMOVED***A"))
