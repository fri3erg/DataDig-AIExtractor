...
def set_pytype_attribute_tag(attribute_tag=None):
    ...
    Default: "{http://codespeak.net/lxml/objectify/pytype***REMOVED***pytype"
    """
...
cdef object XML_SCHEMA_INSTANCE_NIL_ATTR = "{%s***REMOVED***nil" % XML_SCHEMA_INSTANCE_NS
cdef object XML_SCHEMA_INSTANCE_TYPE_ATTR = "{%s***REMOVED***type" % XML_SCHEMA_INSTANCE_NS
...
cdef class ObjectifiedElement(ElementBase):
    ...
    def __dict__(self):
        ...
        tag = "{%s***REMOVED****" % pyunicode(c_ns) if c_ns is not NULL else None
        children = {***REMOVED***
    ...
    def __setattr__(self, tag, value):
        ...
        if tag == 'text' or tag == 'pyval':
            # read-only !
            raise TypeError, f"attribute '{tag***REMOVED***' of '{_typename(self)***REMOVED***' objects is not writable"
        elif tag == 'tail':
...
cdef _setSlice(sliceobject, _Element target, items):
    ...
    if c_step != 1 and len(del_items) != len(new_items):
        raise ValueError, \
            f"attempt to assign sequence of size {len(new_items)***REMOVED*** to extended slice of size {len(del_items)***REMOVED***"
...
cdef bint _parseBool(s) except -1:
    ...
    value = __parseBoolAsInt(s)
    if value == -1:
        raise ValueError, f"Invalid boolean value: '{s***REMOVED***'"
    return value
...
cdef dict _PYTYPE_DICT = {***REMOVED***
cdef dict _SCHEMA_TYPE_DICT = {***REMOVED***
...
cdef class ElementMaker:
    ...
    def __init__(self, *, namespace=None, nsmap=None, annotate=True,
                 makeelement=None):
        if nsmap is None:
            nsmap = _DEFAULT_NSMAP if annotate else {***REMOVED***
        self._nsmap = nsmap
        self._namespace = None if namespace is None else "{%s***REMOVED***" % namespace
        self._annotate = annotate
        if makeelement is not None:
            if not callable(makeelement):
                raise TypeError(
                    f"argument of 'makeelement' parameter must be callable, got {type(makeelement)***REMOVED***")
...
cdef object _dump(_Element element, int indent):
    ...
    if isinstance(element, ObjectifiedDataElement):
        ...
        if value is not None:
            if not value.strip():
                ...
                value = repr(value)
    result = f"{indentstr***REMOVED***{element.tag***REMOVED*** = {value***REMOVED*** [{_typename(element)***REMOVED***]\n"
    xsi_ns    = "{%s***REMOVED***" % XML_SCHEMA_INSTANCE_NS
    pytype_ns = "{%s***REMOVED***" % PYTYPE_NAMESPACE
    for name, value in sorted(cetree.iterattributes(element, 3)):
        ...
        result += f"{indentstr***REMOVED***  * {name***REMOVED*** = {value!r***REMOVED***\n"
