...
def _format_parameters_helper(http_request, params):
    ...
    if query:
        ...
        existing_params = {p[0]: p[-1] for p in [p.partition("=") for p in query.split("&")]***REMOVED***
    ...
    for k, v in params.items():
        if isinstance(v, list):
            for w in v:
                if w is None:
                    raise ValueError("Query parameter {***REMOVED*** cannot be None".format(k))
                query_params.append("{***REMOVED***={***REMOVED***".format(k, w))
        else:
            if v is None:
                raise ValueError("Query parameter {***REMOVED*** cannot be None".format(k))
            query_params.append("{***REMOVED***={***REMOVED***".format(k, v))
...
def _pad_attr_name(attr: str, backcompat_attrs: Sequence[str]) -> str:
    ...
    """
    return "_{***REMOVED***".format(attr) if attr in backcompat_attrs else attr
