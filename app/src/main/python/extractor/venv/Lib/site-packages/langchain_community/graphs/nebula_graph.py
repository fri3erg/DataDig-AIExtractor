...
***REMOVED***
            ...
            raise ValueError(f"Error initializing session pool. Error: {e***REMOVED***")
    ...
    def __del__(self) -> None:
        try:
            ...
            logger.warning(f"Could not close session pool. Error: {e***REMOVED***")
        ...
        try:
            ...
            if not result.is_succeeded():
                logger.warning(
                    ...
                    f"Error: {result.error_msg()***REMOVED***\n"
                    f"Query: {query***REMOVED*** \n"
    ***REMOVED***
            return result
            ...
            logger.warning(
                ...
                f"Current size: {self.session_pool_size***REMOVED***"
***REMOVED***
            raise ValueError(
                f"No valid session found in session pool. "
                f"Please consider increasing the session pool size. "
...
***REMOVED***
            ...
            if retry < RETRY_TIMES:
                ...
                logger.warning(
                    f"Error executing query to NebulaGraph. "
                    f"Retrying ({retry***REMOVED***/{RETRY_TIMES***REMOVED***)...\n"
                    f"query: {query***REMOVED*** \n"
                    f"Error: {e***REMOVED***"
    ***REMOVED***
                return self.execute(query, params, retry)
            else:
                raise ValueError(f"Error executing query to NebulaGraph. Error: {e***REMOVED***")
    ...
    ***REMOVED***
    ...
    def refresh_schema(self) -> None:
        ...
        for tag in self.execute("SHOW TAGS").column_values("Name"):
            tag_name = tag.cast()
            tag_schema = {"tag": tag_name, "properties": []***REMOVED***
            r = self.execute(f"DESCRIBE TAG `{tag_name***REMOVED***`")
        ...
        for edge_type in self.execute("SHOW EDGES").column_values("Name"):
            ...
            edge_schema = {"edge": edge_type_name, "properties": []***REMOVED***
            r = self.execute(f"DESCRIBE EDGE `{edge_type_name***REMOVED***`")
            props, types = r.column_values("Field"), r.column_values("Type")
            for i in range(r.row_size()):
...
***REMOVED***.column_values("rels")
            if len(r) > 0:
                relationships.append(r[0].cast())

        self.schema = (
            f"Node properties: {tags_schema***REMOVED***\n"
            f"Edge properties: {edge_types_schema***REMOVED***\n"
