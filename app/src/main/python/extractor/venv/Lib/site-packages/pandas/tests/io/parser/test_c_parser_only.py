...
"""
    ...
    result = parser.read_csv(
        StringIO(data), sep=r"\s+", header=None, names=["a", "b"], dtype={"a": np.int32***REMOVED***
    )
...
***REMOVED***,
            {"dtype": {"A": "datetime64", "B": "float64"***REMOVED******REMOVED***,
        ),
        (
            (
                "the dtype datetime64 is not supported for parsing, "
                "pass this column using parse_dates instead"
***REMOVED***,
            {"dtype": {"A": "datetime64", "B": "float64"***REMOVED***, "parse_dates": ["B"]***REMOVED***,
        ),
        (
            "the dtype timedelta64 is not supported for parsing",
            {"dtype": {"A": "timedelta64", "B": "float64"***REMOVED******REMOVED***,
        ),
        (
            f"the dtype {tm.ENDIAN***REMOVED***U8 is not supported for parsing",
            {"dtype": {"A": "U8"***REMOVED******REMOVED***,
        ),
    ],
    ids=["dt64-0", "dt64-1", "td64", f"{tm.ENDIAN***REMOVED***U8"],
)
...
def test_precise_conversion(c_parser_only, num):
    ...
    def error(val: float, actual_val: Decimal) -> Decimal:
        return abs(Decimal(f"{val:.100***REMOVED***") - actual_val)
...
def test_grow_boundary_at_cap(c_parser_only, count):
    ...
    with StringIO("," * count) as s:
        expected = DataFrame(columns=[f"Unnamed: {i***REMOVED***" for i in range(count + 1)])
...
def test_float_precision_round_trip_with_text(c_parser_only):
    ...
    tm.assert_frame_equal(df, DataFrame({0: ["a"]***REMOVED***))
...
def test_data_after_quote(c_parser_only):
    ...
    expected = DataFrame({"a": ["1", "ba"]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_file_like_no_next(c_parser_only):
    ...
    expected = DataFrame({"a": [1]***REMOVED***)
...
def test_1000_sep_with_decimal(
    ...
    parser = c_parser_only
    expected = DataFrame({"A": [1, 10], "B": [2334.01, 13], "C": [5, 10.0]***REMOVED***)
