    ...
    def create_table_if_not_exists(self) -> None:
        ...
        with self.engine.connect() as conn:
            with conn.begin():
                ...
                index_name = f"{self.collection_name***REMOVED***_embedding_idx"
                index_query = text(
                    f"""
                    SELECT 1
                    FROM pg_indexes
                    WHERE indexname = '{index_name***REMOVED***';
                """
    ***REMOVED***
                ...
                if not result:
                    index_statement = text(
                        f"""
                        CREATE INDEX {index_name***REMOVED***
                        ON {self.collection_name***REMOVED*** USING ann(embedding)
                        WITH (
                            "dim" = {self.embedding_dimension***REMOVED***,
                            "hnsw_m" = 100
            ***REMOVED***;
                    """
        ***REMOVED***
    ...
    def delete_collection(self) -> None:
        ...
        drop_statement = text(f"DROP TABLE IF EXISTS {self.collection_name***REMOVED***;")
        with self.engine.connect() as conn:
            with conn.begin():
...
***REMOVED***
        ...
        if filter is not None:
            conditions = [
                f"metadata->>{key!r***REMOVED*** = {value!r***REMOVED***" for key, value in filter.items()
            ]
            filter_condition = f"WHERE {' AND '.join(conditions)***REMOVED***"

        # Define the base query
        sql_query = f"""
            SELECT *, l2_distance(embedding, :embedding) as distance
            FROM {self.collection_name***REMOVED***
            {filter_condition***REMOVED***
            ORDER BY embedding <-> :embedding
            LIMIT :k
        """

        # Set up the query parameters
        params = {"embedding": embedding, "k": k***REMOVED***
...
***REMOVED***
    ...
    def connection_string_from_db_params(
        ...
        return f"postgresql+{driver***REMOVED***://{user***REMOVED***:{password***REMOVED***@{host***REMOVED***:{port***REMOVED***/{database***REMOVED***"
