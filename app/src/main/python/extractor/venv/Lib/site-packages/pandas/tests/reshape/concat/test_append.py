...
class TestAppend:
    ...
    def test_append_new_columns(self):
        ...
        df = DataFrame({"a": {"x": 1, "y": 2***REMOVED***, "b": {"x": 3, "y": 4***REMOVED******REMOVED***)
        row = Series([5, 6, 7], index=["a", "b", "c"], name="z")
        expected = DataFrame(
            {
                "a": {"x": 1, "y": 2, "z": 5***REMOVED***,
                "b": {"x": 3, "y": 4, "z": 6***REMOVED***,
                "c": {"z": 7***REMOVED***,
            ***REMOVED***
        )
    ...
    def test_append_sorts(self, sort):
        df1 = DataFrame({"a": [1, 2], "b": [1, 2]***REMOVED***, columns=["b", "a"])
        df2 = DataFrame({"a": [1, 2], "c": [3, 4]***REMOVED***, index=[2, 3])

        result = df1._append(df2, sort=sort)

        # for None / True
        expected = DataFrame(
            {"b": [1, 2, None, None], "a": [1, 2, 1, 2], "c": [None, None, 3, 4]***REMOVED***,
            columns=["a", "b", "c"],
        )
        if sort is False:
    ...
    ***REMOVED***,
    ...
    def test_append_missing_column_proper_upcast(self, sort):
        df1 = DataFrame({"A": np.array([1, 2, 3, 4], dtype="i8")***REMOVED***)
        df2 = DataFrame({"B": np.array([True, False, True, False], dtype=bool)***REMOVED***)
    ...
    def test_append_empty_frame_to_series_with_dateutil_tz(self):
        ...
        ser = Series({"a": 1.0, "b": 2.0, "date": date***REMOVED***)
    ...
    def test_append_empty_tz_frame_with_datetime64ns(self, using_array_manager):
        ...
        result = df._append({"a": pd.NaT***REMOVED***, ignore_index=True)
        if using_array_manager:
            expected = DataFrame({"a": [pd.NaT]***REMOVED***, dtype=object)
        else:
            expected = DataFrame({"a": [np.nan]***REMOVED***, dtype=object)
        tm.assert_frame_equal(result, expected)

        # also test with typed value to append
        df = DataFrame(columns=["a"]).astype("datetime64[ns, UTC]")
        other = Series({"a": pd.NaT***REMOVED***, dtype="datetime64[ns]")
        result = df._append(other, ignore_index=True)
        tm.assert_frame_equal(result, expected)

        # mismatched tz
        other = Series({"a": pd.NaT***REMOVED***, dtype="datetime64[ns, US/Pacific]")
        result = df._append(other, ignore_index=True)
        expected = DataFrame({"a": [pd.NaT]***REMOVED***).astype(object)
        tm.assert_frame_equal(result, expected)
    ...
    def test_append_empty_frame_with_timedelta64ns_nat(
        ...
        other = DataFrame({"a": [np.timedelta64(val, "ns")]***REMOVED***)
        result = df._append(other, ignore_index=True)
    ...
    def test_append_frame_with_timedelta64ns_nat(self, dtype_str, val):
        # https://github.com/pandas-dev/pandas/issues/35460
        df = DataFrame({"a": pd.array([1], dtype=dtype_str)***REMOVED***)

        other = DataFrame({"a": [np.timedelta64(val, "ns")]***REMOVED***)
        result = df._append(other, ignore_index=True)

        expected = DataFrame({"a": [df.iloc[0, 0], other.iloc[0, 0]]***REMOVED***, dtype=object)
        tm.assert_frame_equal(result, expected)
