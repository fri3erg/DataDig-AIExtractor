...
def test_37477():
    # fixed by GH#45121
    orig = DataFrame({"A": [1, 2, 3], "B": [3, 4, 5]***REMOVED***)
    expected = DataFrame({"A": [1, 2, 3], "B": [3, 1.2, 5]***REMOVED***)
...
def test_26395(indexer_al):
    ...
    indexer_al(df)["C", "D"] = 2
    expected = DataFrame({"D": [0, 0, 2]***REMOVED***, index=["A", "B", "C"], dtype=np.int64)
    tm.assert_frame_equal(df, expected)
    ...
    expected = DataFrame({"D": [0, 0, 44.5]***REMOVED***, index=["A", "B", "C"], dtype=np.float64)
    tm.assert_frame_equal(df, expected)
    ...
    expected = DataFrame({"D": [0, 0, "hello"]***REMOVED***, index=["A", "B", "C"], dtype=object)
    tm.assert_frame_equal(df, expected)
...
def test_15231():
    ...
    df.loc[2] = Series({"a": 5, "b": 6***REMOVED***)
    assert (df.dtypes == np.int64).all()

    df.loc[3] = Series({"a": 7***REMOVED***)
...
def test_12499():
    ...
    data = [{"one": 0, "two": ts***REMOVED***]
    orig = DataFrame(data)
    df = orig.copy()
    df.loc[1] = [np.nan, NaT]

    expected = DataFrame(
        {"one": [0, np.nan], "two": Series([ts, NaT], dtype="datetime64[ns, UTC]")***REMOVED***
    )
    tm.assert_frame_equal(df, expected)
