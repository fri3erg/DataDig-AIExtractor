...
***REMOVED*** from e
    ...
    def tabulate(self, *args, **kwargs):
        ...
        width = max(len(f"{s***REMOVED***") for s in samples)
    ...
    def pformat(self, maxlen=10):
        ...
        :rtype: string
        """
        items = ["{!r***REMOVED***: {!r***REMOVED***".format(*item) for item in self.most_common(maxlen)]
        if len(self) > maxlen:
            items.append("...")
        return "FreqDist({{{0***REMOVED******REMOVED******REMOVED***)".format(", ".join(items))
...
***REMOVED***
    ...
    def unirand(cls, samples):
        ...
        return {s: randrow[i] for i, s in enumerate(samples)***REMOVED***
...
class DictionaryProbDist(ProbDistI):
    ...
    def __init__(self, prob_dict=None, log=False, normalize=False):
        ...
        self._prob_dict = prob_dict.copy() if prob_dict is not None else {***REMOVED***
        self._log = log
...
class MutableProbDist(ProbDistI):
    ...
    def __init__(self, prob_dist, samples, store_logs=True):
        ...
        self._sample_dict = {samples[i]: i for i in range(len(samples))***REMOVED***
...
class KneserNeyProbDist(ProbDistI):
    ...
    def __repr__(self):
        ...
        :rtype: str
        """
        return f"<KneserNeyProbDist based on {self._trigrams.N()***REMOVED*** trigrams"
...
class ConditionalFreqDist(defaultdict):
    ...
    the indexing operator:
        ...
        >>> cfdist[3]
        FreqDist({'the': 3, 'dog': 2, 'not': 1***REMOVED***)
        >>> cfdist[3].freq('the')
        0.5
        >>> cfdist[3]['dog']
        2
...
***REMOVED*** from e
        ...
        if not samples:
            samples = sorted({v for c in conditions for v in self[c]***REMOVED***)
        if "linewidth" not in kwargs:
            kwargs["linewidth"] = 2
        ax = plt.gca()
        if conditions:
    ...
    def tabulate(self, *args, **kwargs):
        ...
        samples = _get_kwarg(
            kwargs,
            "samples",
            sorted({v for c in conditions if c in self for v in self[c]***REMOVED***),
...
def demo(numsamples=6, numoutcomes=500):
    ...
    for pdist in pdists:
        ...
        print("{:>20***REMOVED*** {***REMOVED***".format(pdist.__class__.__name__[:20], ("%s" % fdist)[:55]))
    print()
...
def gt_demo():
    ...
    print("{:>18***REMOVED*** {:>8***REMOVED***  {:>14***REMOVED***".format("word", "frequency", "SimpleGoodTuring"))
    fd_keys_sorted = (
