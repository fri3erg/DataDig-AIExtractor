...
BOOL_FALSE = {0, '0', 'off', 'f', 'false', 'n', 'no'***REMOVED***
BOOL_TRUE = {1, '1', 'on', 't', 'true', 'y', 'yes'***REMOVED***
...
def make_literal_validator(type_: Any) -> Callable[[Any], Any]:
    ...
    allowed_choices = {v: v for v in permitted_choices***REMOVED***
...
class IfConfig:
    ...
    def check(self, config: Type['BaseConfig']) -> bool:
        return any(getattr(config, name) not in {None, self.ignored_value***REMOVED*** for name in self.config_attr_names)
...
def find_validators(  # noqa: C901 (ignore complexity)
    ...
    for val_type, validators in _VALIDATORS:
        try:
            ...
            raise RuntimeError(f'error checking inheritance of {type_!r***REMOVED*** (type: {display_as_type(type_)***REMOVED***)')

    if config.arbitrary_types_allowed:
        yield make_arbitrary_type_validator(type_)
    else:
        raise RuntimeError(f'no validator found for {type_***REMOVED***, see `arbitrary_types_allowed` in Config')
