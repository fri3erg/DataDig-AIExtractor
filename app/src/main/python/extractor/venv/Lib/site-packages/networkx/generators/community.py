...
def _generate_communities(degree_seq, community_sizes, mu, max_iters, seed):
    ...
    ..., *n* - 1***REMOVED***, where *n* is the length of ``degree_seq``.
...
def LFR_benchmark_graph(
    ...
    1) Find a degree sequence with a power law distribution, and minimum
       ...
       ``n``. Each node *u* will have $\mu \mathrm{deg***REMOVED***(u)$ edges
       joining it to nodes in communities other than its own and $(1 -
       \mu) \mathrm{deg***REMOVED***(u)$ edges joining it to nodes in its own
       community.
    ...
    3) Each node will be randomly assigned a community with the
       ...
       intra-community degree, $(1 - \mu) \mathrm{deg***REMOVED***(u)$ as
    ...
    4) Each node *u* then adds $(1 - \mu) \mathrm{deg***REMOVED***(u)$
       intra-community edges and $\mu \mathrm{deg***REMOVED***(u)$ inter-community
       edges.
    ...
    NetworkXError
        ...
        - ``mu`` must be in [0, 1].
        - ``max_degree`` must be in {1, ..., *n****REMOVED***.
    ...
    node attributes of the graph::
        ...
        >>> communities = {frozenset(G.nodes[v]["community"]) for v in G***REMOVED***
