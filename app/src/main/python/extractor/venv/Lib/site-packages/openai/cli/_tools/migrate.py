...
def _debug(message: str) -> None:
    ...
    sys.stdout.write(f"[DEBUG]: {message***REMOVED***\n")
...
def install() -> Path:
    ...
    if target_path.exists():
        _debug(f"{target_path***REMOVED*** already exists")
        sys.stdout.flush()
        return target_path

    _debug(f"Using Grit CLI path: {target_path***REMOVED***")
    ...
    arch = _get_arch()
    _debug(f"Using architecture {arch***REMOVED***")

    file_name = f"marzano-{platform***REMOVED***-{arch***REMOVED***"
    meta_url = f"https://api.keygen.sh/v1/accounts/{KEYGEN_ACCOUNT***REMOVED***/artifacts/{file_name***REMOVED***"

    sys.stdout.write(f"Retrieving Grit CLI metadata from {meta_url***REMOVED***\n")
    with httpx.Client() as client:
        ...
        if errors:
            for error in errors:
                sys.stdout.write(f"{error***REMOVED***\n")
        ...
        _debug(f"Redirect URL {link***REMOVED***")
