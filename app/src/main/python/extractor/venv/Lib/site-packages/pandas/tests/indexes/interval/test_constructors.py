...
class TestFromArrays(ConstructorTests):
    ...
    def get_kwargs_from_breaks(self, breaks, closed="right"):
        ...
        """
        return {"left": breaks[:-1], "right": breaks[1:]***REMOVED***
...
class TestFromTuples(ConstructorTests):
    ...
    def get_kwargs_from_breaks(self, breaks, closed="right"):
        ...
        if is_unsigned_integer_dtype(breaks):
            pytest.skip(f"{breaks.dtype***REMOVED*** not relevant IntervalIndex.from_tuples tests")

        if len(breaks) == 0:
            return {"data": breaks***REMOVED***

        tuples = list(zip(breaks[:-1], breaks[1:]))
        if isinstance(breaks, (list, tuple)):
            return {"data": tuples***REMOVED***
        elif isinstance(getattr(breaks, "dtype", None), CategoricalDtype):
            return {"data": breaks._constructor(tuples)***REMOVED***
        return {"data": com.asarray_tuplesafe(tuples)***REMOVED***
    ...
    def test_constructor_errors(self):
        ...
        # too few/many items
        tuples = [(0, 1), (2,), (3, 4)]
        msg = "IntervalIndex.from_tuples requires tuples of length 2, got {t***REMOVED***"
...
class TestClassConstructors(ConstructorTests):
    ...
    def get_kwargs_from_breaks(self, breaks, closed="right"):
        ...
        """
        if is_unsigned_integer_dtype(breaks):
            pytest.skip(f"{breaks.dtype***REMOVED*** not relevant for class constructor tests")
        ...
        if isinstance(breaks, list):
            return {"data": ivs***REMOVED***
        elif isinstance(getattr(breaks, "dtype", None), CategoricalDtype):
            return {"data": breaks._constructor(ivs)***REMOVED***
        return {"data": np.array(ivs, dtype=object)***REMOVED***
...
def test_interval_index_subtype(timezone, inclusive_endpoints_fixture):
    ...
    dtype = f"interval[datetime64[ns, {timezone***REMOVED***], {inclusive_endpoints_fixture***REMOVED***]"
