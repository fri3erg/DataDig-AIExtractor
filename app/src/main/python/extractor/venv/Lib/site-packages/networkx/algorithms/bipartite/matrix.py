...
def biadjacency_matrix(
    ...
    `U = u_{1***REMOVED***,...,u_{r***REMOVED***` and `V = v_{1***REMOVED***,...,v_{s***REMOVED***`. The biadjacency
    matrix [1]_ is the `r` x `s` matrix `B` in which `b_{i,j***REMOVED*** = 1`
...
***REMOVED***
    ...
    try:
        ...
        raise nx.NetworkXError(f"Unknown sparse array format: {format***REMOVED***") from err
