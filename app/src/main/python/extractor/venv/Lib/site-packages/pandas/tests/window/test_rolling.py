...
def test_doc_string():
    df = DataFrame({"B": [0, 1, 2, np.nan, 4]***REMOVED***)
    df
    df.rolling(2).sum()
...
def test_constructor_timedelta_window_and_minperiods(window, raw):
    ...
    expected = DataFrame(
        {"value": np.append([np.nan, 1.0], np.arange(3.0, 27.0, 3))***REMOVED***,
        index=date_range("2017-08-08", periods=n, freq="D"),
    )
...
def test_closed_fixed(closed, arithmetic_win_operators):
    ...
    df_fixed = DataFrame({"A": [0, 1, 2, 3, 4]***REMOVED***)
    df_time = DataFrame({"A": [0, 1, 2, 3, 4]***REMOVED***, index=date_range("2020", periods=5))
...
def test_datetimelike_centered_selections(
    ...
    func_name = arithmetic_win_operators
    df_time = DataFrame(
        {"A": [0.0, 1.0, 2.0, 3.0, 4.0]***REMOVED***, index=date_range("2020", periods=5)
    )

    expected = DataFrame(
        {"A": [getattr(df_time["A"].iloc[s], func_name)() for s in window_selections]***REMOVED***,
        index=date_range("2020", periods=5),
    )

    if func_name == "sem":
        kwargs = {"ddof": 0***REMOVED***
    else:
        kwargs = {***REMOVED***
...
def test_rolling_axis_sum(axis_frame):
    ...
    if axis == 0:
        ...
        expected = DataFrame({i: [np.nan] * 2 + [3.0] * 8 for i in range(20)***REMOVED***)
    else:
        # axis == 1
...
def test_rolling_axis_count(axis_frame):
    # see gh-26055
    df = DataFrame({"x": range(3), "y": range(3)***REMOVED***)

    axis = df._get_axis_number(axis_frame)

    if axis in [0, "index"]:
        msg = "The 'axis' keyword in DataFrame.rolling"
        expected = DataFrame({"x": [1.0, 2.0, 2.0], "y": [1.0, 2.0, 2.0]***REMOVED***)
    else:
        msg = "Support for axis=1 in DataFrame.rolling is deprecated"
        expected = DataFrame({"x": [1.0, 1.0, 1.0], "y": [2.0, 2.0, 2.0]***REMOVED***)
...
def test_rolling_datetime(axis_frame, tz_naive_fixture):
    ...
    tz = tz_naive_fixture
    df = DataFrame(
        {i: [1] * 2 for i in date_range("2019-8-01", "2019-08-03", freq="D", tz=tz)***REMOVED***
    )
...
def test_rolling_window_as_string(center):
    ...
    df = DataFrame({"DateCol": days, "metric": data***REMOVED***)
...
@pytest.mark.parametrize(
    ...
    [
        (
            DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]***REMOVED***),
            [
                ({"A": [1], "B": [4]***REMOVED***, [0]),
                ({"A": [1, 2], "B": [4, 5]***REMOVED***, [0, 1]),
                ({"A": [1, 2, 3], "B": [4, 5, 6]***REMOVED***, [0, 1, 2]),
            ],
            3,
            None,
        ),
        ...
        (DataFrame({"A": [1], "B": [4]***REMOVED***), [], 2, None),
        (DataFrame({"A": [1], "B": [4]***REMOVED***), [], 2, 1),
        (DataFrame(), [({***REMOVED***, [])], 2, None),
        (
            DataFrame({"A": [1, np.nan, 3], "B": [np.nan, 5, 6]***REMOVED***),
            [
                ({"A": [1.0], "B": [np.nan]***REMOVED***, [0]),
                ({"A": [1, np.nan], "B": [np.nan, 5]***REMOVED***, [0, 1]),
                ({"A": [1, np.nan, 3], "B": [np.nan, 5, 6]***REMOVED***, [0, 1, 2]),
            ],
            3,
            2,
        ),
...
def test_iter_rolling_on_dataframe_unordered():
    # GH 43386
    df = DataFrame({"a": ["x", "y", "x"], "b": [0, 1, 2]***REMOVED***)
...
def test_rolling_positional_argument(grouping, _index, raw):
    ...
    df = DataFrame(data={"X": range(5)***REMOVED***, index=[0, 0, 1, 1, 1])

    expected = DataFrame(data={"X": [0.0, 0.5, 1.0, 1.5, 2.0]***REMOVED***, index=_index)
    # GH 40341
    if "by" in grouping:
...
def test_rolling_numerical_accuracy_kahan_mean(add, unit):
    ...
    df = DataFrame(
        {"A": [3002399751580331.0 + add, -0.0, -0.0]***REMOVED***,
        index=dti,
    )
    result = (
...
def test_rolling_numerical_accuracy_jump():
    ...
    df = DataFrame({"data": data***REMOVED***, index=index)
...
def test_rolling_mixed_dtypes_axis_1(func, value):
    ...
    expected = DataFrame(
        {"a": [1.0, 1.0], "b": [value, value], "c": [value, value]***REMOVED***,
        index=[1, 2],
    )
    tm.assert_frame_equal(result, expected)
...
def test_rolling_axis_1_non_numeric_dtypes(value):
    ...
    expected = DataFrame({"a": [1.0, 2.0]***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_rolling_on_df_transposed():
    # GH: 32724
    df = DataFrame({"A": [1, None], "B": [4, 5], "C": [7, 8]***REMOVED***)
    expected = DataFrame({"A": [1.0, np.nan], "B": [5.0, 5.0], "C": [11.0, 13.0]***REMOVED***)
    msg = "Support for axis=1 in DataFrame.rolling is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
...
def test_timeoffset_as_window_parameter_for_corr(unit):
    ...
    df = DataFrame(
        {"B": [0, 1, 2, 4, 3], "A": [7, 4, 6, 9, 3]***REMOVED***,
        index=dti,
    )
...
def test_rolling_decreasing_indices(method):
    ...
    GH 36933
    """
    df = DataFrame({"values": np.arange(-15, 10) ** 2***REMOVED***)
    df_reverse = DataFrame({"values": df["values"][::-1]***REMOVED***, index=df.index[::-1])
...
def test_rolling_non_monotonic(method, expected):
    ...
    df = DataFrame({"values": np.arange(len(use_expanding)) ** 2***REMOVED***)
    ...
    expected = DataFrame({"values": expected***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_groupby_rolling_nan_included():
    # GH 35542
    data = {"group": ["g1", np.nan, "g1", "g2", np.nan], "B": [0, 1, 2, 3, 4]***REMOVED***
    df = DataFrame(data)
    result = df.groupby("group", dropna=False).rolling(1, min_periods=1).mean()
    expected = DataFrame(
        {"B": [0.0, 2.0, 3.0, 1.0, 4.0]***REMOVED***,
...
def test_rolling_float_dtype(float_numpy_dtype):
    # GH#42452
    df = DataFrame({"A": range(5), "B": range(10, 15)***REMOVED***, dtype=float_numpy_dtype)
    expected = DataFrame(
        {"A": [np.nan] * 5, "B": range(10, 20, 2)***REMOVED***,
...
def test_numeric_only_corr_cov_frame(kernel, numeric_only, use_arg):
    # GH#46560
    df = DataFrame({"a": [1, 2, 3], "b": 2, "c": 3***REMOVED***)
    df["c"] = df["c"].astype(object)
...
def test_numeric_only_series(arithmetic_win_operators, numeric_only, dtype):
    ...
    if numeric_only and dtype is object:
        msg = f"Rolling.{kernel***REMOVED*** does not implement numeric_only"
...
def test_rolling_timedelta_window_non_nanoseconds(unit, tz):
    # Test Sum, GH#55106
    df_time = DataFrame(
        {"A": range(5)***REMOVED***, index=date_range("2013-01-01", freq="1s", periods=5, tz=tz)
    )
