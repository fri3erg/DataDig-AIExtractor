...
def interesting_binop_operands(val1, val2, dtype):
    ...
    * Oddly strided arrays which may not be SIMD compatible
    ...
    for pos in [0, 1, 2, 3, 4, 5, -1, -2, -3, -4]:
        ...
        yield arr1, arr2, extractor, f"off-{pos***REMOVED***"
        yield arr1, arr2[pos], extractor, f"off-{pos***REMOVED***-with-scalar"
    ...
    for stride in [-1, 113]:
        ...
        yield op1, op2, extractor, f"stride-{stride***REMOVED***"
...
class TestComparisons:
    ...
    def test_comparison_functions(self, dtype, py_comp, np_comp):
        ...
        assert_(comp_b.tolist() == comp_b_list,
            f"Failed comparison ({py_comp.__name__***REMOVED***)")
...
class TestPower:
    ...
    def test_zero_power_nonzero(self):
        # Testing 0^{Non-zero***REMOVED*** issue 18378
...
class TestSpecialMethods:
        ...
        def do_test(f_call, f_expected):
            ...
            try:
                ...
                raise AssertionError("\n".join([
                    ...
                    " expected:              {***REMOVED***".format(expected),
                    " __array_prepare__ got: {***REMOVED***".format(p),
                    " __array_wrap__ got:    {***REMOVED***".format(w)
                ]))
    ...
    def test_ufunc_override(self):
        ...
        assert_equal(res0[4], {***REMOVED***)
        assert_equal(res1[4], {'out': (a,)***REMOVED***)
    ...
    def test_ufunc_override_methods(self):
        ...
        assert_equal(res[4], {'subok': 'bar', 'where': 42***REMOVED***)
        ...
        assert_equal(res[4], {'dtype':'dtype0',
                              ...
                              'axis': 'axis0',
                              'initial': 'init0',
                              'where': 'where0'***REMOVED***)

        # reduce, output equal to None removed, but not other explicit ones,
        # even if they are at their default value.
        res = np.multiply.reduce(a, 0, None, None, False)
        assert_equal(res[4], {'axis': 0, 'dtype': None, 'keepdims': False***REMOVED***)
        res = np.multiply.reduce(a, out=None, axis=0, keepdims=True)
        assert_equal(res[4], {'axis': 0, 'keepdims': True***REMOVED***)
        res = np.multiply.reduce(a, None, out=(None,), dtype=None)
        assert_equal(res[4], {'axis': None, 'dtype': None***REMOVED***)
        res = np.multiply.reduce(a, 0, None, None, False, 2, True)
        assert_equal(res[4], {'axis': 0, 'dtype': None, 'keepdims': False,
                              'initial': 2, 'where': True***REMOVED***)
        ...
        assert_equal(res[4], {'axis': 0, 'dtype': None, 'keepdims': False,
                              'initial': None, 'where': True***REMOVED***)
        ...
        assert_equal(res[4], {'axis': 0, 'dtype': None***REMOVED***)
        res = np.multiply.accumulate(a, out=None, axis=0, dtype='dtype1')
        assert_equal(res[4], {'axis': 0, 'dtype': 'dtype1'***REMOVED***)
        ...
        assert_equal(res[4], {'axis': None, 'dtype': 'dt'***REMOVED***)
    ...
    def test_ufunc_override_out(self):
        ...
        assert_(np.sin(a, None) == {***REMOVED***)
        assert_(np.sin(a, out=None) == {***REMOVED***)
        assert_(np.sin(a, out=(None,)) == {***REMOVED***)
        assert_(np.modf(a, None) == {***REMOVED***)
        assert_(np.modf(a, None, None) == {***REMOVED***)
        assert_(np.modf(a, out=(None, None)) == {***REMOVED***)
        with assert_raises(TypeError):
            # Out argument must be tuple, since there are multiple outputs.
            np.modf(a, out=None)
    ...
    def test_gufunc_override(self):
        ...
        assert_equal(res[4], {'out': (a,)***REMOVED***)
    ...
    def test_ufunc_override_with_super(self):
        ...
        assert_equal(b.info, {'inputs': [0]***REMOVED***)
        b = np.sin(d, out=(a,))
        assert_(np.all(check == b))
        assert_equal(b.info, {'outputs': [0]***REMOVED***)
        assert_(b is a)
        a = np.arange(5.).view(A)
        b = np.sin(a, out=a)
        assert_(np.all(check == b))
        assert_equal(b.info, {'inputs': [0], 'outputs': [0]***REMOVED***)

        # 1 input, 2 outputs
        a = np.arange(5.).view(A)
        b1, b2 = np.modf(a)
        assert_equal(b1.info, {'inputs': [0]***REMOVED***)
        b1, b2 = np.modf(d, out=(None, a))
        assert_(b2 is a)
        assert_equal(b1.info, {'outputs': [1]***REMOVED***)
        a = np.arange(5.).view(A)
        b = np.arange(5.).view(A)
        c1, c2 = np.modf(a, out=(a, b))
        assert_(c1 is a)
        assert_(c2 is b)
        assert_equal(c1.info, {'inputs': [0], 'outputs': [0, 1]***REMOVED***)
        ...
        c = np.add(a, b, out=a)
        assert_(c is a)
        assert_equal(c.info, {'inputs': [0, 1], 'outputs': [0]***REMOVED***)
        ...
        assert_(c.info, {'inputs': [0]***REMOVED***)
        c = a.max()
        check = d.max()
        assert_equal(c, check)
        assert_(c.info, {'inputs': [0]***REMOVED***)
        ...
        assert_equal(c, check)
        assert_(c is b)
        assert_(c.info, {'inputs': [0], 'outputs': [0]***REMOVED***)
        check = a.max(axis=0)
        ...
        assert_(a.info, {'inputs': [0]***REMOVED***)
        b = np.array(1.).view(A)
        a = d.copy().view(A)
        np.add.at(a, ([0, 1], [0, 2]), b)
        assert_equal(a, check)
        assert_(a.info, {'inputs': [0, 2]***REMOVED***)
