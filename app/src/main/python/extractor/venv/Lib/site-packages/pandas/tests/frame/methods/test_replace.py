...
def mix_ab() -> dict[str, list[int | str]]:
    return {"a": list(range(4)), "b": list("ab..")***REMOVED***


@pytest.fixture
def mix_abc() -> dict[str, list[float | str]]:
    return {"a": list(range(4)), "b": list("ab.."), "c": ["a", "b", np.nan, "d"]***REMOVED***
...
***REMOVED***,
    ...
    def test_regex_replace_list_obj(
        ...
        df = DataFrame({"a": list("ab.."), "b": list("efgh"), "c": list("helo")***REMOVED***)
    ...
    def test_regex_replace_list_mixed(self, mix_ab):
        ...
        values = [np.nan, "crap"]
        mix2 = {"a": list(range(4)), "b": list("ab.."), "c": list("halo")***REMOVED***
        ...
        expec = DataFrame({"a": mix_ab["a"], "b": ["a_crap", "b_crap", "..", ".."]***REMOVED***)
        tm.assert_frame_equal(res, expec)
        ...
        res = dfmix.replace(to_replace_res, values, regex=True)
        expec = DataFrame({"a": mix_ab["a"], "b": ["crap", "b_crap", "..", ".."]***REMOVED***)
        tm.assert_frame_equal(res, expec)
    ...
    def test_regex_replace_list_mixed_inplace(self, mix_ab):
        ...
        assert return_value is None
        expec = DataFrame({"a": mix_ab["a"], "b": ["crap", "b", np.nan, np.nan]***REMOVED***)
        tm.assert_frame_equal(res, expec)
    ...
    def test_regex_replace_dict_mixed(self, mix_abc):
        ...
        # dicts
        # single dict {re1: v1***REMOVED***, search the whole frame
        # need test for this...

        # list of dicts {re1: v1, re2: v2, ..., re3: v3***REMOVED***, search the whole
        # frame
        res = dfmix.replace({"b": r"\s*\.\s*"***REMOVED***, {"b": np.nan***REMOVED***, regex=True)
        res2 = dfmix.copy()
        return_value = res2.replace(
            {"b": r"\s*\.\s*"***REMOVED***, {"b": np.nan***REMOVED***, inplace=True, regex=True
        )
        assert return_value is None
        expec = DataFrame(
            {"a": mix_abc["a"], "b": ["a", "b", np.nan, np.nan], "c": mix_abc["c"]***REMOVED***
        )
        tm.assert_frame_equal(res, expec)
        tm.assert_frame_equal(res2, expec)

        # list of dicts {re1: re11, re2: re12, ..., reN: re1N***REMOVED***, search the
        # whole frame
        res = dfmix.replace({"b": r"\s*(\.)\s*"***REMOVED***, {"b": r"\1ty"***REMOVED***, regex=True)
        res2 = dfmix.copy()
        return_value = res2.replace(
            {"b": r"\s*(\.)\s*"***REMOVED***, {"b": r"\1ty"***REMOVED***, inplace=True, regex=True
        )
        assert return_value is None
        expec = DataFrame(
            {"a": mix_abc["a"], "b": ["a", "b", ".ty", ".ty"], "c": mix_abc["c"]***REMOVED***
        )
        tm.assert_frame_equal(res, expec)
        tm.assert_frame_equal(res2, expec)

        res = dfmix.replace(regex={"b": r"\s*(\.)\s*"***REMOVED***, value={"b": r"\1ty"***REMOVED***)
        res2 = dfmix.copy()
        return_value = res2.replace(
            regex={"b": r"\s*(\.)\s*"***REMOVED***, value={"b": r"\1ty"***REMOVED***, inplace=True
        )
        assert return_value is None
        expec = DataFrame(
            {"a": mix_abc["a"], "b": ["a", "b", ".ty", ".ty"], "c": mix_abc["c"]***REMOVED***
        ...
        # scalar -> dict
        # to_replace regex, {value: value***REMOVED***
        expec = DataFrame(
            {"a": mix_abc["a"], "b": [np.nan, "b", ".", "."], "c": mix_abc["c"]***REMOVED***
        )
        res = dfmix.replace("a", {"b": np.nan***REMOVED***, regex=True)
        res2 = dfmix.copy()
        return_value = res2.replace("a", {"b": np.nan***REMOVED***, regex=True, inplace=True)
        assert return_value is None
        tm.assert_frame_equal(res, expec)
        tm.assert_frame_equal(res2, expec)
        ...
        res = dfmix.replace("a", {"b": np.nan***REMOVED***, regex=True)
        res2 = dfmix.copy()
        return_value = res2.replace(regex="a", value={"b": np.nan***REMOVED***, inplace=True)
        assert return_value is None
        expec = DataFrame(
            {"a": mix_abc["a"], "b": [np.nan, "b", ".", "."], "c": mix_abc["c"]***REMOVED***
        )
    ...
    def test_regex_replace_dict_nested(self, mix_abc):
        ...
        dfmix = DataFrame(mix_abc)
        res = dfmix.replace({"b": {r"\s*\.\s*": np.nan***REMOVED******REMOVED***, regex=True)
        res2 = dfmix.copy()
        res4 = dfmix.copy()
        return_value = res2.replace(
            {"b": {r"\s*\.\s*": np.nan***REMOVED******REMOVED***, inplace=True, regex=True
        )
        assert return_value is None
        res3 = dfmix.replace(regex={"b": {r"\s*\.\s*": np.nan***REMOVED******REMOVED***)
        return_value = res4.replace(regex={"b": {r"\s*\.\s*": np.nan***REMOVED******REMOVED***, inplace=True)
        assert return_value is None
        expec = DataFrame(
            {"a": mix_abc["a"], "b": ["a", "b", np.nan, np.nan], "c": mix_abc["c"]***REMOVED***
        )
    ...
    def test_regex_replace_dict_nested_non_first_character(
        ...
        df = DataFrame({"first": ["abc", "bca", "cab"]***REMOVED***, dtype=dtype)
        if using_infer_string and any_string_dtype == "object":
            with tm.assert_produces_warning(FutureWarning, match="Downcasting"):
                result = df.replace({"a": "."***REMOVED***, regex=True)
            expected = DataFrame({"first": [".bc", "bc.", "c.b"]***REMOVED***)

        else:
            result = df.replace({"a": "."***REMOVED***, regex=True)
            expected = DataFrame({"first": [".bc", "bc.", "c.b"]***REMOVED***, dtype=dtype)
        tm.assert_frame_equal(result, expected)
    ...
    def test_regex_replace_dict_nested_gh4115(self):
        df = DataFrame({"Type": ["Q", "T", "Q", "Q", "T"], "tmp": 2***REMOVED***)
        expected = DataFrame({"Type": [0, 1, 0, 0, 1], "tmp": 2***REMOVED***)
        msg = "Downcasting behavior in `replace`"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = df.replace({"Type": {"Q": 0, "T": 1***REMOVED******REMOVED***)
        tm.assert_frame_equal(result, expected)
...
***REMOVED***
    ...
    def test_regex_replace_str_to_numeric(self, mix_abc):
        ...
        assert return_value is None
        expec = DataFrame({"a": mix_abc["a"], "b": ["a", "b", 0, 0], "c": mix_abc["c"]***REMOVED***)
        tm.assert_frame_equal(res, expec)
        tm.assert_frame_equal(res2, expec)
        tm.assert_frame_equal(res3, expec)
    ...
    def test_regex_replace_regex_list_to_numeric(self, mix_abc):
        ...
        assert return_value is None
        expec = DataFrame(
            {"a": mix_abc["a"], "b": ["a", 0, 0, 0], "c": ["a", 0, np.nan, "d"]***REMOVED***
        )
        tm.assert_frame_equal(res, expec)
        tm.assert_frame_equal(res2, expec)
        tm.assert_frame_equal(res3, expec)
    ...
    def test_regex_replace_series_of_regexes(self, mix_abc):
        df = DataFrame(mix_abc)
        s1 = Series({"b": r"\s*\.\s*"***REMOVED***)
        s2 = Series({"b": np.nan***REMOVED***)
    ...
    def test_regex_replace_numeric_to_object_conversion(self, mix_abc):
        df = DataFrame(mix_abc)
        expec = DataFrame({"a": ["a", 1, 2, 3], "b": mix_abc["b"], "c": mix_abc["c"]***REMOVED***)
        res = df.replace(0, "a")
        tm.assert_frame_equal(res, expec)
        assert res.a.dtype == np.object_

    @pytest.mark.parametrize(
        "to_replace", [{"": np.nan, ",": ""***REMOVED***, {",": "", "": np.nan***REMOVED***]
    )
    ...
    def test_replace_regex_metachar(self, metachar):
        df = DataFrame({"a": [metachar, "else"]***REMOVED***)
        result = df.replace({"a": {metachar: "paren"***REMOVED******REMOVED***)
        expected = DataFrame({"a": ["paren", "else"]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    @pytest.mark.parametrize(
        "data,to_replace,expected",
        [
            (["xax", "xbx"], {"a": "c", "b": "d"***REMOVED***, ["xcx", "xdx"]),
            (["d", "", ""], {r"^\s*$": pd.NA***REMOVED***, ["d", pd.NA, pd.NA]),
        ],
    )
    ...
    ***REMOVED***
    ...
    def test_replace_list(self):
        obj = {"a": list("ab.."), "b": list("efgh"), "c": list("helo")***REMOVED***
        dfobj = DataFrame(obj)
    ...
    def test_replace_with_empty_list(self, frame_or_series):
        ...
        # GH 19266
        msg = (
            "NumPy boolean array indexing assignment cannot assign {size***REMOVED*** "
            "input values to the 1 output values where the mask is true"
        )
        with pytest.raises(ValueError, match=msg.format(size=0)):
            obj.replace({np.nan: []***REMOVED***)
        with pytest.raises(ValueError, match=msg.format(size=2)):
            obj.replace({np.nan: ["dummy", "alt"]***REMOVED***)

    def test_replace_series_dict(self):
        # from GH 3064
        df = DataFrame({"zero": {"a": 0.0, "b": 1***REMOVED***, "one": {"a": 2.0, "b": 0***REMOVED******REMOVED***)
        result = df.replace(0, {"zero": 0.5, "one": 1.0***REMOVED***)
        expected = DataFrame({"zero": {"a": 0.5, "b": 1***REMOVED***, "one": {"a": 2.0, "b": 1.0***REMOVED******REMOVED***)
        tm.assert_frame_equal(result, expected)
        ...
        df = DataFrame({"zero": {"a": 0.0, "b": 1***REMOVED***, "one": {"a": 2.0, "b": 0***REMOVED******REMOVED***)
        s = Series({"zero": 0.0, "one": 2.0***REMOVED***)
        result = df.replace(s, {"zero": 0.5, "one": 1.0***REMOVED***)
        expected = DataFrame({"zero": {"a": 0.5, "b": 1***REMOVED***, "one": {"a": 1.0, "b": 0.0***REMOVED******REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_replace_mixed3(self):
        # test case from
        df = DataFrame(
            {"A": Series([3, 0], dtype="int64"), "B": Series([0, 3], dtype="int64")***REMOVED***
        )
    ...
    def test_replace_nullable_int_with_string_doesnt_cast(self):
        ...
        df = DataFrame({"a": [1, 2, 3, np.nan], "b": ["some", "strings", "here", "he"]***REMOVED***)
    ...
    def test_replace_with_nullable_column(self, dtype):
        ...
        df = DataFrame({"A": ["A", "B", "x"], "B": nullable_ser***REMOVED***)
        result = df.replace("x", "X")
        expected = DataFrame({"A": ["A", "B", "X"], "B": nullable_ser***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_replace_simple_nested_dict(self):
        df = DataFrame({"col": range(1, 5)***REMOVED***)
        expected = DataFrame({"col": ["a", 2, 3, "b"]***REMOVED***)

        result = df.replace({"col": {1: "a", 4: "b"***REMOVED******REMOVED***)
        tm.assert_frame_equal(expected, result)

        # in this case, should be the same as the not nested version
        result = df.replace({1: "a", 4: "b"***REMOVED***)
        tm.assert_frame_equal(expected, result)
    ...
    def test_replace_simple_nested_dict_with_nonexistent_value(self):
        ...
        result = df.replace({-1: "-", 1: "a", 4: "b"***REMOVED***)
        tm.assert_frame_equal(expected, result)

        result = df.replace({"col": {-1: "-", 1: "a", 4: "b"***REMOVED******REMOVED***)
        tm.assert_frame_equal(expected, result)

    def test_replace_NA_with_None(self):
        # gh-45601
        df = DataFrame({"value": [42, None]***REMOVED***).astype({"value": "Int64"***REMOVED***)
        result = df.replace({pd.NA: None***REMOVED***)
        expected = DataFrame({"value": [42, None]***REMOVED***, dtype=object)
        tm.assert_frame_equal(result, expected)

    def test_replace_NAT_with_None(self):
        # gh-45836
        df = DataFrame([pd.NaT, pd.NaT])
        result = df.replace({pd.NaT: None, np.nan: None***REMOVED***)
    ...
    def test_replace_with_None_keeps_categorical(self):
        ...
        result = df.replace({3: None***REMOVED***)
    ...
    def test_replace_value_is_none(self, datetime_frame):
        ...
        result = datetime_frame.replace(to_replace={np.nan: 0***REMOVED***)
        expected = datetime_frame.T.replace(to_replace={np.nan: 0***REMOVED***).T
        tm.assert_frame_equal(result, expected)

        result = datetime_frame.replace(to_replace={np.nan: 0, 1: -1e8***REMOVED***)
        tsframe = datetime_frame.copy()
        tsframe.iloc[0, 0] = 0
    ...
    @pytest.mark.parametrize(
        ...
        [
            (DataFrame({"ints": [1, 2, 3]***REMOVED***), 1, 0, DataFrame({"ints": [0, 2, 3]***REMOVED***)),
            (
                DataFrame({"ints": [1, 2, 3]***REMOVED***, dtype=np.int32),
                1,
                0,
                DataFrame({"ints": [0, 2, 3]***REMOVED***, dtype=np.int32),
***REMOVED***,
            (
                DataFrame({"ints": [1, 2, 3]***REMOVED***, dtype=np.int16),
                1,
                0,
                DataFrame({"ints": [0, 2, 3]***REMOVED***, dtype=np.int16),
***REMOVED***,
            (
                DataFrame({"bools": [True, False, True]***REMOVED***),
                False,
                True,
                DataFrame({"bools": [True, True, True]***REMOVED***),
***REMOVED***,
            (
                DataFrame({"complex": [1j, 2j, 3j]***REMOVED***),
                1j,
                0,
                DataFrame({"complex": [0j, 2j, 3j]***REMOVED***),
***REMOVED***,
            (
                DataFrame(
                    {
    ...
    ***REMOVED***,
                datetime(2018, 5, 28),
                datetime(2018, 7, 28),
                DataFrame({"datetime64": Index([datetime(2018, 7, 28)] * 3)***REMOVED***),
***REMOVED***,
            # GH 20380
            (
                DataFrame({"dt": [datetime(3017, 12, 20)], "str": ["foo"]***REMOVED***),
                "foo",
                "bar",
                DataFrame({"dt": [datetime(3017, 12, 20)], "str": ["bar"]***REMOVED***),
***REMOVED***,
            # GH 36782
            (
                DataFrame({"dt": [datetime(2920, 10, 1)]***REMOVED***),
                datetime(2920, 10, 1),
                datetime(2020, 10, 1),
                DataFrame({"dt": [datetime(2020, 10, 1)]***REMOVED***),
***REMOVED***,
            (
                DataFrame(
                    {
...
***REMOVED***,
    ...
    def test_replace_input_formats_listlike(self):
        # both dicts
        to_rep = {"A": np.nan, "B": 0, "C": ""***REMOVED***
        values = {"A": 0, "B": -1, "C": "missing"***REMOVED***
        df = DataFrame(
            {"A": [np.nan, 0, np.inf], "B": [0, 2, 5], "C": ["", "asdf", "fd"]***REMOVED***
        )
        filled = df.replace(to_rep, values)
        expected = {k: v.replace(to_rep[k], values[k]) for k, v in df.items()***REMOVED***
        tm.assert_frame_equal(filled, DataFrame(expected))

        result = df.replace([0, 2, 5], [5, 2, 0])
        expected = DataFrame(
            {"A": [np.nan, 5, np.inf], "B": [5, 2, 0], "C": ["", "asdf", "fd"]***REMOVED***
        )
        tm.assert_frame_equal(result, expected)
        ...
        values = {"A": 0, "B": -1, "C": "missing"***REMOVED***
        df = DataFrame(
            {"A": [np.nan, 0, np.nan], "B": [0, 2, 5], "C": ["", "asdf", "fd"]***REMOVED***
        )
        filled = df.replace(np.nan, values)
        expected = {k: v.replace(np.nan, values[k]) for k, v in df.items()***REMOVED***
        tm.assert_frame_equal(filled, DataFrame(expected))
    ...
    def test_replace_input_formats_scalar(self):
        ...
        expected = {k: v.replace(to_rep[k], 0) for k, v in df.items()***REMOVED***
        tm.assert_frame_equal(filled, DataFrame(expected))
    ...
    def test_replace_dict_no_regex(self):
        ...
        weights = {
            ...
            "Strongly Disagree": 1,
        ***REMOVED***
        expected = Series({0: 5, 1: 4, 2: 3, 3: 2, 4: 1***REMOVED***)
        msg = "Downcasting behavior in `replace` "
        with tm.assert_produces_warning(FutureWarning, match=msg):
    ...
    def test_replace_dict_tuple_list_ordering_remains_the_same(self):
        df = DataFrame({"A": [np.nan, 1]***REMOVED***)
        res1 = df.replace(to_replace={np.nan: 0, 1: -1e8***REMOVED***)
        res2 = df.replace(to_replace=(1, np.nan), value=[-1e8, 0])
        res3 = df.replace(to_replace=[1, np.nan], value=[-1e8, 0])

        expected = DataFrame({"A": [0, -1e8]***REMOVED***)
    ...
    def test_replace_doesnt_replace_without_regex(self):
        ...
        res = df.replace({r"\D": 1***REMOVED***)
        tm.assert_frame_equal(df, res)

    def test_replace_bool_with_string(self):
        df = DataFrame({"a": [True, False], "b": list("ab")***REMOVED***)
        result = df.replace(True, "a")
        expected = DataFrame({"a": ["a", False], "b": df.b***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_replace_with_dict_with_bool_keys(self):
        df = DataFrame({0: [True, False], 1: [False, True]***REMOVED***)
        result = df.replace({"asdf": "asdb", True: "yes"***REMOVED***)
        expected = DataFrame({0: ["yes", False], 1: [False, "yes"]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_replace_dict_strings_vs_ints(self):
        # GH#34789
        df = DataFrame({"Y0": [1, 2], "Y1": [3, 4]***REMOVED***)
        result = df.replace({"replace_string": "test"***REMOVED***)

        tm.assert_frame_equal(result, df)

        result = df["Y0"].replace({"replace_string": "test"***REMOVED***)
        tm.assert_series_equal(result, df["Y0"])

    def test_replace_truthy(self):
        df = DataFrame({"a": [True, True]***REMOVED***)
    ...
    def test_nested_dict_overlapping_keys_replace_int(self):
        ...
        df = DataFrame({"a": list(range(1, 5))***REMOVED***)

        result = df.replace({"a": dict(zip(range(1, 5), range(2, 6)))***REMOVED***)
    ...
    def test_nested_dict_overlapping_keys_replace_str(self):
        ...
        df = DataFrame({"a": astr***REMOVED***)
        result = df.replace(dict(zip(astr, bstr)))
        expected = df.replace({"a": dict(zip(astr, bstr))***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_replace_swapping_bug(self, using_infer_string):
        df = DataFrame({"a": [True, False, True]***REMOVED***)
        res = df.replace({"a": {True: "Y", False: "N"***REMOVED******REMOVED***)
        expect = DataFrame({"a": ["Y", "N", "Y"]***REMOVED***)
        tm.assert_frame_equal(res, expect)

        df = DataFrame({"a": [0, 1, 0]***REMOVED***)
        res = df.replace({"a": {0: "Y", 1: "N"***REMOVED******REMOVED***)
        expect = DataFrame({"a": ["Y", "N", "Y"]***REMOVED***)
        tm.assert_frame_equal(res, expect)
    ...
    ***REMOVED***,
        ...
        ***REMOVED***
        ...
        expected = DataFrame({"fname": [d["fname"][k] for k in df.fname.values]***REMOVED***)
    ...
    def test_replace_datetimetz(self):
        ...
        result = result.replace({"A": pd.NaT***REMOVED***, Timestamp("20130104", tz="US/Eastern"))
        tm.assert_frame_equal(result, expected)

        # pre-2.0 this would coerce to object with mismatched tzs
        result = df.copy()
        result.iloc[1, 0] = np.nan
        result = result.replace({"A": pd.NaT***REMOVED***, Timestamp("20130104", tz="US/Pacific"))
        expected = DataFrame(
            {
                "A": [
                    Timestamp("20130101", tz="US/Eastern"),
        ...
        result = df.copy()
        result.iloc[1, 0] = np.nan
        result = result.replace({"A": np.nan***REMOVED***, Timestamp("20130104"))
        expected = DataFrame(
            {
                "A": [
                    Timestamp("20130101", tz="US/Eastern"),
    ...
    def test_replace_with_empty_dictlike(self, mix_abc):
        # GH 15289
        df = DataFrame(mix_abc)
        tm.assert_frame_equal(df, df.replace({***REMOVED***))
        tm.assert_frame_equal(df, df.replace(Series([], dtype=object)))

        tm.assert_frame_equal(df, df.replace({"b": {***REMOVED******REMOVED***))
        tm.assert_frame_equal(df, df.replace(Series({"b": {***REMOVED******REMOVED***)))

    @pytest.mark.parametrize(
        "to_replace, method, expected",
        [
            (0, "bfill", {"A": [1, 1, 2], "B": [5, np.nan, 7], "C": ["a", "b", "c"]***REMOVED***),
            (
                np.nan,
                "bfill",
                {"A": [0, 1, 2], "B": [5.0, 7.0, 7.0], "C": ["a", "b", "c"]***REMOVED***,
***REMOVED***,
            ("d", "ffill", {"A": [0, 1, 2], "B": [5, np.nan, 7], "C": ["a", "b", "c"]***REMOVED***),
            (
                [0, 2],
                "bfill",
                {"A": [1, 1, 2], "B": [5, np.nan, 7], "C": ["a", "b", "c"]***REMOVED***,
***REMOVED***,
            (
                [1, 2],
                "pad",
                {"A": [0, 0, 0], "B": [5, np.nan, 7], "C": ["a", "b", "c"]***REMOVED***,
***REMOVED***,
            (
                (1, 2),
                "bfill",
                {"A": [0, 2, 2], "B": [5, np.nan, 7], "C": ["a", "b", "c"]***REMOVED***,
***REMOVED***,
            (
                ["b", "c"],
                "ffill",
                {"A": [0, 1, 2], "B": [5, np.nan, 7], "C": ["a", "a", "a"]***REMOVED***,
***REMOVED***,
        ],
    )
    def test_replace_method(self, to_replace, method, expected):
        # GH 19632
        df = DataFrame({"A": [0, 1, 2], "B": [5, np.nan, 7], "C": ["a", "b", "c"]***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [({"a": 1, "b": 1***REMOVED***, [[3, 3], [2, 2]]), ({"a": 1, "b": 2***REMOVED***, [[3, 1], [2, 3]])],
    )
    def test_categorical_replace_with_dict(self, replace_dict, final_data):
        ...
        expected = DataFrame({"a": a, "b": b***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"col1": [1, 2, 3], "col2": [4, 5, 6]***REMOVED***,
                {4: 5, 5: 6, 6: 7***REMOVED***,
                {"col1": [1, 2, 3], "col2": [5, 6, 7]***REMOVED***,
***REMOVED***,
            (
                {"col1": [1, 2, 3], "col2": ["4", "5", "6"]***REMOVED***,
                {"4": "5", "5": "6", "6": "7"***REMOVED***,
...
***REMOVED***,
    ...
    def test_replace_replacer_dtype(self, replacer):
        ...
        msg = "Downcasting behavior in `replace` "
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = df.replace({"a": replacer, "b": replacer***REMOVED***)
        expected = DataFrame([replacer])
        tm.assert_frame_equal(result, expected)

    def test_replace_after_convert_dtypes(self):
        # GH31517
        df = DataFrame({"grp": [1, 2, 3, 4, 5]***REMOVED***, dtype="Int64")
        result = df.replace(1, 10)
        expected = DataFrame({"grp": [10, 2, 3, 4, 5]***REMOVED***, dtype="Int64")
        tm.assert_frame_equal(result, expected)

    def test_replace_invalid_to_replace(self):
        # GH 18634
        # API: replace() should raise an exception if invalid argument is given
        df = DataFrame({"one": ["a", "b ", "c"], "two": ["d ", "e ", "f "]***REMOVED***)
        msg = (
    ...
    def test_replace_with_duplicate_columns(self, replacement):
        # GH 24798
        result = DataFrame({"A": [1, 2, 3], "A1": [4, 5, 6], "B": [7, 8, 9]***REMOVED***)
        result.columns = list("AAB")

        expected = DataFrame(
            {"A": [1, 2, 3], "A1": [4, 5, 6], "B": [replacement, 8, 9]***REMOVED***
        )
    ...
    def test_replace_ea_ignore_float(self, frame_or_series, value):
        # GH#34871
        obj = DataFrame({"Per": [value] * 3***REMOVED***)
        obj = tm.get_obj(obj, frame_or_series)
    ...
    def test_replace_value_category_type(self):
        ...
        ***REMOVED***
        ...
        input_df = DataFrame(data=input_dict).astype(
            {"col2": "category", "col4": "category"***REMOVED***
        )
    ...
    def test_replace_dict_category_type(self):
        ...
        input_dict = {"col1": ["a"], "col2": ["obj1"], "col3": ["cat1"]***REMOVED***
        # explicitly cast columns as category
        input_df = DataFrame(data=input_dict).astype(
            {"col1": "category", "col2": "category", "col3": "category"***REMOVED***
        )

        # create expected dataframe
        expected_dict = {"col1": ["z"], "col2": ["obj9"], "col3": ["catX"]***REMOVED***
        # explicitly cast columns as category
        expected = DataFrame(data=expected_dict).astype(
            {"col1": "category", "col2": "category", "col3": "category"***REMOVED***
        )
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = input_df.replace({"a": "z", "obj1": "obj9", "cat1": "catX"***REMOVED***)
    ...
    def test_replace_with_compiled_regex(self):
        ...
        df = DataFrame(["a", "b", "c"])
        regex = re.compile("^a$")
        result = df.replace({regex: "z"***REMOVED***, regex=True)
        expected = DataFrame(["z", "b", "c"])
        tm.assert_frame_equal(result, expected)

    def test_replace_intervals(self, using_infer_string):
        # https://github.com/pandas-dev/pandas/issues/35931
        df = DataFrame({"a": [pd.Interval(0, 1), pd.Interval(0, 1)]***REMOVED***)
        warning = FutureWarning if using_infer_string else None
        with tm.assert_produces_warning(warning, match="Downcasting"):
            result = df.replace({"a": {pd.Interval(0, 1): "x"***REMOVED******REMOVED***)
        expected = DataFrame({"a": ["x", "x"]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_replace_unicode(self):
        # GH: 16784
        columns_values_map = {"positive": {"正面": 1, "中立": 1, "负面": 0***REMOVED******REMOVED***
        df1 = DataFrame({"positive": np.ones(3)***REMOVED***)
        result = df1.replace(columns_values_map)
        expected = DataFrame({"positive": np.ones(3)***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_replace_bytes(self, frame_or_series):
        ...
        expected = obj.copy()
        obj = obj.replace({None: np.nan***REMOVED***)
    ...
    def test_replace_value_none_dtype_numeric(self, val):
        # GH#48231
        df = DataFrame({"a": [1, val]***REMOVED***)
        result = df.replace(val, None)
        expected = DataFrame({"a": [1, None]***REMOVED***, dtype=object)
        tm.assert_frame_equal(result, expected)

        df = DataFrame({"a": [1, val]***REMOVED***)
        result = df.replace({val: None***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_replace_with_nil_na(self):
        # GH 32075
        ser = DataFrame({"a": ["nil", pd.NA]***REMOVED***)
        expected = DataFrame({"a": ["anything else", pd.NA]***REMOVED***, index=[0, 1])
...
class TestDataFrameReplaceRegex:
    @pytest.mark.parametrize(
        "data",
        [
            {"a": list("ab.."), "b": list("efgh")***REMOVED***,
            {"a": list("ab.."), "b": list(range(4))***REMOVED***,
        ],
    )
    @pytest.mark.parametrize(
    ...
    def test_replace_regex_dtype_frame(self, regex):
        # GH-48644
        df1 = DataFrame({"A": ["0"], "B": ["0"]***REMOVED***)
        expected_df1 = DataFrame({"A": [1], "B": [1]***REMOVED***)
        msg = "Downcasting behavior in `replace`"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result_df1 = df1.replace(to_replace="0", value=1, regex=regex)
        tm.assert_frame_equal(result_df1, expected_df1)

        df2 = DataFrame({"A": ["0"], "B": ["1"]***REMOVED***)
        expected_df2 = DataFrame({"A": [1], "B": ["1"]***REMOVED***)
        with tm.assert_produces_warning(FutureWarning, match=msg):
    ...
    def test_replace_with_value_also_being_replaced(self):
        # GH46306
        df = DataFrame({"A": [0, 1, 2], "B": [1, 0, 2]***REMOVED***)
        result = df.replace({0: 1, 1: np.nan***REMOVED***)
        expected = DataFrame({"A": [1, np.nan, 2], "B": [np.nan, 1, 2]***REMOVED***)
        tm.assert_frame_equal(result, expected)
