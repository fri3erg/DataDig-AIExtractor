...
class Feature(metaclass=ABCMeta):
    ...
    The property-computing method, M{extract_property(tokens, index)***REMOVED***,
    ...
    methods may consider the entire sequence M{tokens***REMOVED*** and
    ...
    def __init__(self, positions, end=None):
        """
        Construct a Feature which may apply at C{positions***REMOVED***.
        ...
        if end is None:
            self.positions = tuple(sorted({int(i) for i in positions***REMOVED***))
            ...
            try:
                ...
                raise ValueError(
                    "illegal interval specification: (start={***REMOVED***, end={***REMOVED***)".format(
                        positions, end
        ***REMOVED***
    ***REMOVED*** from e
    ...
    def __repr__(self):
        return f"{self.__class__.__name__***REMOVED***({list(self.positions)!r***REMOVED***)"
    ...
    def expand(cls, starts, winlens, excludezero=False):
        ...
        if not all(x > 0 for x in winlens):
            raise ValueError(f"non-positive window length in {winlens***REMOVED***")
