...
def timer(method):
    ...
    def timed(*args, **kw):
        ...
        if hours == 0 and mins == 0 and secs < 10:
            print(f"[TIMER] {method.__name__***REMOVED***(): {method.__name__:.3f***REMOVED*** seconds")
        else:
            print(f"[TIMER] {method.__name__***REMOVED***(): {hours***REMOVED***h {mins***REMOVED***m {secs***REMOVED***s")
        return result
...
def extract_unigram_feats(document, unigrams, handle_negation=False):
    ...
    :return: a dictionary of unigram features {unigram : boolean***REMOVED***.
    ...
    for word in unigrams:
        features[f"contains({word***REMOVED***)"] = word in set(document)
    return features
...
def extract_bigram_feats(document, bigrams):
    ...
    :param unigrams: a list of bigrams whose presence/absence has to be
        checked in `document`.
    :return: a dictionary of bigram features {bigram : boolean***REMOVED***.
    ...
    features = {***REMOVED***
    for bigr in bigrams:
        features[f"contains({bigr[0]***REMOVED*** - {bigr[1]***REMOVED***)"] = bigr in nltk.bigrams(document)
    return features
        ...
        for k in sorted(kwargs):
            if isinstance(kwargs[k], dict):
                ...
                for entry in sorted(dictionary):
                    text += f"    - {entry***REMOVED***: {dictionary[entry]***REMOVED*** \n"
            elif isinstance(kwargs[k], list):
                text += f"  - **{k***REMOVED***:**\n"
                for entry in kwargs[k]:
                    text += f"    - {entry***REMOVED***\n"
            else:
                text += f"  - **{k***REMOVED***:** {kwargs[k]***REMOVED*** \n"
        outfile.write(text)
...
def parse_tweets_set(
    ...
    with codecs.open(filename, "rt") as csvfile:
        ...
        for tweet_id, text in reader:
            # text = text[1]
            i += 1
            sys.stdout.write(f"Loaded {i***REMOVED*** tweets\r")
    ...
    print(f"Loaded {i***REMOVED*** tweets")
    return tweets
...
def demo_vader_tweets(n_instances=None, output=None):
    ...
    for label in labels:
        ...
        metrics_results[f"Precision [{label***REMOVED***]"] = precision_score
        recall_score = eval_recall(gold_results[label], test_results[label])
        metrics_results[f"Recall [{label***REMOVED***]"] = recall_score
        f_measure_score = eval_f_measure(gold_results[label], test_results[label])
        metrics_results[f"F-measure [{label***REMOVED***]"] = f_measure_score

    for result in sorted(metrics_results):
        print(f"{result***REMOVED***: {metrics_results[result]***REMOVED***")
