...
def test_concat_with_all_na():
    ...
    df1 = pd.DataFrame({"col": arr, "key": [0, 1]***REMOVED***)
    df2 = pd.DataFrame({"key": [0, 1], "col2": [1, 2]***REMOVED***)
    result = pd.merge(df1, df2, on="key")
    expected = pd.DataFrame({"col": arr, "key": [0, 1], "col2": [1, 2]***REMOVED***)
    tm.assert_frame_equal(result, expected)
    assert result["col"].array.attr == "test"

    df1 = pd.DataFrame({"col": arr, "key": [0, 1]***REMOVED***)
    df2 = pd.DataFrame({"key": [0, 2], "col2": [1, 2]***REMOVED***)
    result = pd.merge(df1, df2, on="key")
    expected = pd.DataFrame({"col": arr.take([0]), "key": [0], "col2": [1]***REMOVED***)
    tm.assert_frame_equal(result, expected)
    assert result["col"].array.attr == "test"

    result = pd.concat([df1.set_index("key"), df2.set_index("key")], axis=1)
    expected = pd.DataFrame(
        {"col": arr.take([0, 1, -1]), "col2": [1, np.nan, 2], "key": [0, 1, 2]***REMOVED***
    ).set_index("key")
    tm.assert_frame_equal(result, expected)
    assert result["col"].array.attr == "test"
