...
def usage(obj):
    ...
    print(f"{obj.__name__***REMOVED*** supports the following operations:")
    for (name, method) in sorted(pydoc.allmethods(obj).items()):
        ...
        if isinstance(meth, (classmethod, staticmethod)):
            name = f"cls.{name***REMOVED***"
        elif args and args[0] == "self":
            name = f"self.{name***REMOVED***"
            args.pop(0)
        print(
            textwrap.fill(
                f"{name***REMOVED***({', '.join(args)***REMOVED***)",
                initial_indent="  - ",
...
def re_show(regexp, string, left="{", right="***REMOVED***"):
    """
...
def edge_closure(tree, children=iter, maxdepth=-1, verbose=False):
    ...
    >>> print(list(edge_closure('A', lambda node:{'A':['B','C'], 'B':'C', 'C':'B'***REMOVED***[node])))
    [('A', 'B'), ('A', 'C'), ('B', 'C'), ('C', 'B')]
    """
    traversed = set()
    edges = set()
            ...
            try:
                for child in children(node):
                    if child not in traversed:
                        ...
                        if verbose:
                            warnings.warn(
                                f"Discarded redundant search for {child***REMOVED*** at depth {depth + 1***REMOVED***",
                                stacklevel=2,
                ***REMOVED***
                    edge = (node, child)
...
def edges2dot(edges, shapes=None, attr=None):
    ...
    ***REMOVED***
    ...
    for pair in attr.items():
        dot_string += f"{pair[0]***REMOVED*** = {pair[1]***REMOVED***;\n"

    for edge in edges:
        for shape in shapes.items():
            for node in range(2):
                if shape[0] in repr(edge[node]):
                    dot_string += f'"{edge[node]***REMOVED***" [shape = {shape[1]***REMOVED***];\n'
        dot_string += f'"{edge[0]***REMOVED***" -> "{edge[1]***REMOVED***";\n'
            ...
            try:
                for child in children(node):
                    if child not in traversed:
                        ...
                        warnings.warn(
                            "Discarded redundant search for {***REMOVED*** at depth {***REMOVED***".format(
                                child, depth + 1
                ***REMOVED***,
                            stacklevel=2,
            ***REMOVED***
...
def acyclic_depth_first(tree, children=iter, depth=-1, cut_mark=None, traversed=None):
    ...
    if depth != 0:
        ...
        ***REMOVED***
                    if cut_mark:
                        out_tree += [f"Cycle({child***REMOVED***,{depth - 1***REMOVED***,{cut_mark***REMOVED***)"]
        except TypeError:
            pass
    elif cut_mark:
        out_tree += [cut_mark]
...
def unweighted_minimum_spanning_dict(tree, children=iter):
    ...
    {Synset('bound.a.01'): [Synset('unfree.a.02')],
     ...
     Synset('unfree.a.02'): [Synset('confined.a.02'),
                             Synset('dependent.a.01'),
                             Synset('restricted.a.01')]***REMOVED***

    """
    traversed = set()  # Empty set of traversed nodes
    queue = deque([tree])  # Initialize queue
    agenda = {tree***REMOVED***  # Set of all nodes ever queued
    mstdic = {***REMOVED***  # Empty MST dictionary
    while queue:
...
def transitive_closure(graph, reflexive=False):
    ...
    """
    if reflexive:
        base_set = lambda k: {k***REMOVED***
    else:
        base_set = lambda k: set()
    # The graph U_i in the article:
    agenda_graph = {k: graph[k].copy() for k in graph***REMOVED***
    # The graph M_i in the article:
    closure_graph = {k: base_set(k) for k in graph***REMOVED***
    for i in graph:
...
def set_proxy(proxy, user=None, password=""):
    ...
    proxy_handler = ProxyHandler({"https": proxy, "http": proxy***REMOVED***)
