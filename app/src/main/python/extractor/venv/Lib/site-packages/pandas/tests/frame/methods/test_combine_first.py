...
class TestDataFrameCombineFirst:
    def test_combine_first_mixed(self):
        ...
        f = DataFrame({"A": a, "B": b***REMOVED***)

        a = Series(["a", "b"], index=range(5, 7))
        b = Series(range(2), index=range(5, 7))
        g = DataFrame({"A": a, "B": b***REMOVED***)

        exp = DataFrame({"A": list("abab"), "B": [0, 1, 0, 1]***REMOVED***, index=[0, 1, 5, 6])
    ...
    def test_combine_first(self, float_frame, using_infer_string):
        ...
        # #2525
        df = DataFrame({"a": [1]***REMOVED***, index=[datetime(2012, 1, 1)])
    ...
    def test_combine_first_mixed_bug(self):
        ...
        frame1 = DataFrame({"col0": ser1, "col2": ser2, "col3": ser3***REMOVED***)
        ...
        frame2 = DataFrame({"col1": ser1, "col2": ser2, "col5": ser3***REMOVED***)
    ...
    def test_combine_first_doc_example(self):
        # doc example
        df1 = DataFrame(
            {"A": [1.0, np.nan, 3.0, 5.0, np.nan], "B": [np.nan, 2.0, 3.0, np.nan, 6.0]***REMOVED***
        )
        ...
        expected = DataFrame({"A": [1, 2, 3, 5, 3, 7.0], "B": [np.nan, 2, 3, 4, 6, 8]***REMOVED***)
...
***REMOVED***,
    ...
    def test_combine_first_convert_datatime_correctly(
        ...
        df1, df2 = DataFrame({"a": data1***REMOVED***), DataFrame({"a": data2***REMOVED***)
        result = df1.combine_first(df2)
        expected = DataFrame({"a": data_expected***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_combine_first_align_nan(self):
        ...
        exp = DataFrame(
            {"a": [pd.Timestamp("2011-01-01"), pd.NaT], "b": [2, 5]***REMOVED***,
            columns=["a", "b"],
        )
        ...
        exp = DataFrame({"a": [np.nan, np.nan], "b": [4, 5]***REMOVED***, columns=["a", "b"])
        tm.assert_frame_equal(res, exp)
    ...
    def test_combine_first_timezone(self, unit):
        ...
        exp = DataFrame(
            ...
            ***REMOVED***,
            ...
            dtype=f"datetime64[{unit***REMOVED***, UTC]",
        )
        assert res["UTCdatetime"].dtype == f"datetime64[{unit***REMOVED***, UTC]"
        assert res["abc"].dtype == f"datetime64[{unit***REMOVED***, UTC]"
    ...
    def test_combine_first_timezone3(self, unit):
        ...
        exp = DataFrame({"DATE": exp_dts***REMOVED***, index=[1, 2, 3, 4, 5, 7])
        tm.assert_frame_equal(res, exp)
    ...
    def test_combine_first_int(self):
        ...
        df1 = DataFrame({"a": [0, 1, 3, 5]***REMOVED***, dtype="int64")
        df2 = DataFrame({"a": [1, 4]***REMOVED***, dtype="int64")

        result_12 = df1.combine_first(df2)
        expected_12 = DataFrame({"a": [0, 1, 3, 5]***REMOVED***)
        tm.assert_frame_equal(result_12, expected_12)

        result_21 = df2.combine_first(df1)
        expected_21 = DataFrame({"a": [1, 4, 3, 5]***REMOVED***)
        tm.assert_frame_equal(result_21, expected_21)

    @pytest.mark.parametrize("val", [1, 1.0])
    def test_combine_first_with_asymmetric_other(self, val):
        # see gh-20699
        df1 = DataFrame({"isNum": [val]***REMOVED***)
        df2 = DataFrame({"isBool": [True]***REMOVED***)

        res = df1.combine_first(df2)
        exp = DataFrame({"isBool": [True], "isNum": [val]***REMOVED***)
    ...
    def test_combine_first_string_dtype_only_na(self, nullable_string_dtype):
        # GH: 37519
        df = DataFrame(
            {"a": ["962", "85"], "b": [pd.NA] * 2***REMOVED***, dtype=nullable_string_dtype
        )
        df2 = DataFrame({"a": ["85"], "b": [pd.NA]***REMOVED***, dtype=nullable_string_dtype)
...
def test_combine_first_with_nan_multiindex():
    ...
    df = DataFrame({"c": [1, 1, 1, 1, 1, 1]***REMOVED***, index=mi1)
    mi2 = MultiIndex.from_arrays(
        [["a", "b", "c", "a", "b", "d"], [1, 1, 1, 1, 1, 1]], names=["a", "b"]
    )
    s = Series([1, 2, 3, 4, 5, 6], index=mi2)
    res = df.combine_first(DataFrame({"d": s***REMOVED***))
...
def test_combine_preserve_dtypes():
    ...
    df1 = DataFrame({"A": a_column, "B": b_column***REMOVED***)

    c_column = Series(["a", "b"], index=range(5, 7))
    b_column = Series(range(-1, 1), index=range(5, 7))
    df2 = DataFrame({"B": b_column, "C": c_column***REMOVED***)
...
def test_combine_first_int64_not_cast_to_float64():
    # GH 28613
    df_1 = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]***REMOVED***)
    df_2 = DataFrame({"A": [1, 20, 30], "B": [40, 50, 60], "C": [12, 34, 65]***REMOVED***)
    result = df_1.combine_first(df_2)
    expected = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6], "C": [12, 34, 65]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_midx_losing_dtype():
    ...
    df1 = DataFrame({"a": [None, 4]***REMOVED***, index=midx)
    df2 = DataFrame({"a": [3, 3]***REMOVED***, index=midx2)
    result = df1.combine_first(df2)
    expected_midx = MultiIndex.from_arrays(
        [[0, 0, 1, 1], [np.nan, np.nan, np.nan, np.nan]]
    )
    expected = DataFrame({"a": [np.nan, 4, 3, 3]***REMOVED***, index=expected_midx)
    tm.assert_frame_equal(result, expected)
