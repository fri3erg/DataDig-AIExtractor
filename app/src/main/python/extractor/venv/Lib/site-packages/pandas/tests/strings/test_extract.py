...
def test_extract_expand_True_single_capture_group(index_or_series, any_string_dtype):
    ...
    expected = DataFrame({"uno": ["A", "A"]***REMOVED***, dtype=any_string_dtype)
    tm.assert_frame_equal(result, expected)
...
def test_extract_series(name, any_string_dtype):
    ...
    expected = DataFrame({"letter": ["A", "B", np.nan]***REMOVED***, dtype=any_string_dtype)
    tm.assert_frame_equal(result, expected)
...
def test_extract_dataframe_capture_groups_index(index, any_string_dtype):
    ...
    if len(index) < len(data):
        pytest.skip(f"Index needs more than {len(data)***REMOVED*** values")
...
def test_extract_single_group_returns_frame(any_string_dtype):
    ...
    expected = DataFrame({"letter": ["a", "b", "c"]***REMOVED***, dtype=any_string_dtype)
    tm.assert_frame_equal(result, expected)
...
def test_extractall_single_group(any_string_dtype):
    ...
    expected = DataFrame(
        {"letter": ["a", "b", "d", "c"]***REMOVED***, index=expected_index, dtype=any_string_dtype
    )
    tm.assert_frame_equal(result, expected)
