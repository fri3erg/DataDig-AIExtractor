...
>>> pos = {n: (n, n) for n in G***REMOVED***  # nodes set on a line
...
>>> node_color = {0: "red", 1: "orange", 2: "blue", 3: "gray!90"***REMOVED***
>>> edge_width = {e: "line width=1.5" for e in H3.edges***REMOVED***
>>> pos = nx.circular_layout(H3)
>>> latex_code = nx.to_latex(H3, pos, node_options=node_color, edge_options=edge_width)
>>> print(latex_code)
\documentclass{report***REMOVED***
\usepackage{tikz***REMOVED***
\usepackage{subcaption***REMOVED***
<BLANKLINE>
\begin{document***REMOVED***
\begin{figure***REMOVED***
  \begin{tikzpicture***REMOVED***
      \draw
        (1.0, 0.0) node[red] (0){0***REMOVED***
        (0.707, 0.707) node[orange] (1){1***REMOVED***
        (-0.0, 1.0) node[blue] (2){2***REMOVED***
        (-0.707, 0.707) node[gray!90] (3){3***REMOVED***
        (-1.0, -0.0) node (4){4***REMOVED***
        (-0.707, -0.707) node (5){5***REMOVED***
        (0.0, -1.0) node (6){6***REMOVED***
        (0.707, -0.707) node (7){7***REMOVED***;
      \begin{scope***REMOVED***[-]
...
def to_latex_raw(
    ...
    if not pos:
        ...
        pos = {n: f"({round(360.0 * i / len(G), 3)***REMOVED***:2)" for i, n in enumerate(G)***REMOVED***
    for node in G:
        if node not in pos:
            raise nx.NetworkXError(f"node {node***REMOVED*** has no specified pos {pos***REMOVED***")
        posnode = pos[node]
        if not isinstance(posnode, str):
            try:
                posx, posy = posnode
                pos[node] = f"({round(posx, 3)***REMOVED***, {round(posy, 3)***REMOVED***)"
            except (TypeError, ValueError):
                msg = f"position pos[{node***REMOVED***] is not 2-tuple or a string: {posnode***REMOVED***"
    ...
    topts = "" if tikz_options == "" else f"[{tikz_options.strip('[]')***REMOVED***]"
    defn = "" if default_node_options == "" else f"[{default_node_options.strip('[]')***REMOVED***]"
    linestyle = f"{'->' if G.is_directed() else '-'***REMOVED***"
    if default_edge_options == "":
        defe = "[" + linestyle + "]"
    elif "-" in default_edge_options:
        defe = default_edge_options
    else:
        defe = f"[{linestyle***REMOVED***,{default_edge_options.strip('[]')***REMOVED***]"

    # Construct the string line by line
    result = "  \\begin{tikzpicture***REMOVED***" + topts
    result += i4 + "  \\draw" + defn
    # load the nodes
    for n in G:
        # node options goes inside square brackets
        nopts = f"[{node_options[n].strip('[]')***REMOVED***]" if n in node_options else ""
        # node text goes inside curly brackets {***REMOVED***
        ntext = f"{{{node_label[n]***REMOVED******REMOVED******REMOVED***" if n in node_label else f"{{{n***REMOVED******REMOVED******REMOVED***"

        result += i8 + f"{pos[n]***REMOVED*** node{nopts***REMOVED*** ({n***REMOVED***){ntext***REMOVED***"
    result += ";\n"

    # load the edges
    result += "      \\begin{scope***REMOVED***" + defe
    for edge in G.edges:
        u, v = edge[:2]
        e_opts = f"{edge_options[edge]***REMOVED***".strip("[]") if edge in edge_options else ""
        # add loop options for selfloops if not present
        if u == v and "loop" not in e_opts:
            e_opts = "loop," + e_opts
        e_opts = f"[{e_opts***REMOVED***]" if e_opts != "" else ""
        ...
        els = f"[{els.strip('[]')***REMOVED***]"
        # edge text is drawn using the TikZ node command inside curly brackets {***REMOVED***
        e_label = f" node{els***REMOVED*** {{{edge_label[edge]***REMOVED******REMOVED******REMOVED***" if edge in edge_label else ""

        result += i8 + f"\\draw{e_opts***REMOVED*** ({u***REMOVED***) to{e_label***REMOVED*** ({v***REMOVED***);"

    result += "\n      \\end{scope***REMOVED***\n    \\end{tikzpicture***REMOVED***\n"
    return result


_DOC_WRAPPER_TIKZ = r"""\documentclass{{report***REMOVED******REMOVED***
\usepackage{{tikz***REMOVED******REMOVED***
\usepackage{{subcaption***REMOVED******REMOVED***

\begin{{document***REMOVED******REMOVED***
{content***REMOVED***
\end{{document***REMOVED******REMOVED***"""


_FIG_WRAPPER = r"""\begin{{figure***REMOVED******REMOVED***
{content***REMOVED***{caption***REMOVED***{label***REMOVED***
\end{{figure***REMOVED******REMOVED***"""


_SUBFIG_WRAPPER = r"""  \begin{{subfigure***REMOVED******REMOVED***{{{size***REMOVED***\textwidth***REMOVED******REMOVED***
{content***REMOVED***{caption***REMOVED***{label***REMOVED***
  \end{{subfigure***REMOVED******REMOVED***"""
...
***REMOVED***
        ...
        for G, pos, subcap, sublbl in zip(Gbunch, pos, sub_captions, sub_labels):
            subraw = to_latex_raw(
                ...
                edge_label_options,
***REMOVED***
            cap = f"    \\caption{{{subcap***REMOVED******REMOVED******REMOVED***" if subcap else ""
            lbl = f"\\label{{{sublbl***REMOVED******REMOVED******REMOVED***" if sublbl else ""
    ...
    raw = raw[:-1]
    cap = f"\n  \\caption{{{caption***REMOVED******REMOVED******REMOVED***" if caption else ""
    lbl = f"\\label{{{latex_label***REMOVED******REMOVED******REMOVED***" if latex_label else ""
