...
def _dispatched_lookup(words, vocab):
    raise TypeError(f"Unsupported type for looking up in vocabulary: {type(words)***REMOVED***")
...
class Vocabulary:
    ...
    def __init__(self, counts=None, unk_cutoff=1, unk_label="<UNK>"):
        ...
        if unk_cutoff < 1:
            raise ValueError(f"Cutoff value cannot be less than 1. Got: {unk_cutoff***REMOVED***")
    ...
    def __str__(self):
        return "<{***REMOVED*** with cutoff={***REMOVED*** unk_label='{***REMOVED***' and {***REMOVED*** items>".format(
