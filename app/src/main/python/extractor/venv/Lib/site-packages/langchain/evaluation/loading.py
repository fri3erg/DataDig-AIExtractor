...
def load_dataset(uri: str) -> List[Dict]:
    ...
    **Prerequisites**
    ...
    dataset = load_dataset(f"LangChainDatasets/{uri***REMOVED***")
...
def load_evaluator(
    ...
    **kwargs : Any
    ...
    if evaluator not in _EVALUATOR_MAP:
        raise ValueError(
            f"Unknown evaluator type: {evaluator***REMOVED***"
            f"\nValid types are: {list(_EVALUATOR_MAP.keys())***REMOVED***"
        )
    evaluator_cls = _EVALUATOR_MAP[evaluator]
    if issubclass(evaluator_cls, LLMEvalChain):
        try:
            llm = llm or ChatOpenAI(
                model="gpt-4", model_kwargs={"seed": 42***REMOVED***, temperature=0
***REMOVED***
        except Exception as e:
            raise ValueError(
                f"Evaluation with the {evaluator_cls***REMOVED*** requires a "
...
def load_evaluators(
    ...
    **kwargs : Any
    ...
    for evaluator in evaluators:
        _kwargs = config.get(evaluator, {***REMOVED***) if config else {***REMOVED***
        loaded.append(load_evaluator(evaluator, llm=llm, **{**kwargs, **_kwargs***REMOVED***))
    return loaded
