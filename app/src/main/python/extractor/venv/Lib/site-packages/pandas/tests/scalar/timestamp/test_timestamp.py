...
***REMOVED***
    ...
    def test_dow_parametric(self, ts, sign):
        # GH 53738
        ts = (
            f"{sign***REMOVED***{str(ts.year).zfill(4)***REMOVED***"
            f"-{str(ts.month).zfill(2)***REMOVED***"
            f"-{str(ts.day).zfill(2)***REMOVED***"
        )
...
***REMOVED***
    ...
    def test_roundtrip(self):
        ...
        assert result == Timestamp(f"{base***REMOVED***.005000")
        assert result.microsecond == 5000

        result = Timestamp(base._value + Timedelta("5us")._value)
        assert result == Timestamp(f"{base***REMOVED***.000005")
        assert result.microsecond == 5

        result = Timestamp(base._value + Timedelta("5ns")._value)
        assert result == Timestamp(f"{base***REMOVED***.000000005")
        assert result.nanosecond == 5
        assert result.microsecond == 0

        result = Timestamp(base._value + Timedelta("6ms 5us")._value)
        assert result == Timestamp(f"{base***REMOVED***.006005")
        assert result.microsecond == 5 + 6 * 1000

        result = Timestamp(base._value + Timedelta("200ms 5us")._value)
        assert result == Timestamp(f"{base***REMOVED***.200005")
        assert result.microsecond == 5 + 200 * 1000

    def test_hash_equivalent(self):
        d = {datetime(2011, 1, 1): 5***REMOVED***
