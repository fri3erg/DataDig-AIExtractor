...
def test_compression_size(obj, method, compression_only):
    if compression_only == "tar":
        compression_only = {"method": "tar", "mode": "w:gz"***REMOVED***
...
@pytest.mark.parametrize(
    ...
    [
        ("to_csv", {"index": False***REMOVED***, pd.read_csv),
        ("to_json", {***REMOVED***, pd.read_json),
        ("to_pickle", {***REMOVED***, pd.read_pickle),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ("to_csv", {"index": False, "header": True***REMOVED***, pd.read_csv, {"squeeze": True***REMOVED***),
        ("to_json", {***REMOVED***, pd.read_json, {"typ": "series"***REMOVED***),
        ("to_pickle", {***REMOVED***, pd.read_pickle, {***REMOVED***),
    ],
)
...
def test_gzip_reproducibility_file_name():
    ...
    compression_options = {"method": "gzip", "mtime": 1***REMOVED***
...
def test_gzip_compression_level(obj, method):
    ...
    with tm.ensure_clean() as path:
        ...
        getattr(obj, method)(path, compression={"method": "gzip", "compresslevel": 1***REMOVED***)
...
def test_xz_compression_level_read(obj, method):
    with tm.ensure_clean() as path:
        ...
        compressed_size_default = os.path.getsize(path)
        getattr(obj, method)(path, compression={"method": "xz", "preset": 1***REMOVED***)
        compressed_size_fast = os.path.getsize(path)
        assert compressed_size_default < compressed_size_fast
        if method == "to_csv":
...
def test_bzip_compression_level(obj, method):
    ...
    """
    with tm.ensure_clean() as path:
        getattr(obj, method)(path, compression={"method": "bz2", "compresslevel": 1***REMOVED***)
...
def test_tar_gz_to_different_filename():
    with tm.ensure_clean(filename=".foo") as file:
        pd.DataFrame(
            [["1", "2"]],
            columns=["foo", "bar"],
        ).to_csv(file, compression={"method": "tar", "mode": "w:gz"***REMOVED***, index=False)
