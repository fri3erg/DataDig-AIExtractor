...
class LoggingCallbackHandler(FunctionCallbackHandler):
    ...
    def on_text(
        ...
        **kwargs: Any,  # noqa: ARG002
        ...
        try:
            crumbs_str = f"[{self.get_breadcrumbs(run=self._get_run(run_id=run_id))***REMOVED***] "
        except TracerException:
            crumbs_str = ""
        self.function_callback(
            f'{get_colored_text("[text]", color="blue")***REMOVED***'
            f' {get_bolded_text(f"{crumbs_str***REMOVED***New text:")***REMOVED***\n{text***REMOVED***'
        )
