...
def _get_search_index_query(search_type: SearchType) -> str:
    type_to_query_map = {
        ...
        SearchType.HYBRID: (
            ...
            "YIELD node, score "
            "WITH collect({node:node, score:score***REMOVED***) AS nodes, max(score) AS max "
            "UNWIND nodes AS n "
            # We use 0 as min
            "RETURN n.node AS node, (n.score / max) AS score UNION "
            "CALL db.index.fulltext.queryNodes($keyword_index, $query, {limit: $k***REMOVED***) "
            "YIELD node, score "
            "WITH collect({node:node, score:score***REMOVED***) AS nodes, max(score) AS max "
            "UNWIND nodes AS n "
            # We use 0 as min
...
def check_if_not_null(props: List[str], values: List[Any]) -> None:
    ...
    for prop, value in zip(props, values):
        if not value:
            raise ValueError(f"Parameter `{prop***REMOVED***` must not be None or empty string")
...
***REMOVED***
        ...
        if pre_delete_collection:
            ...
            self.query(
                f"MATCH (n:`{self.node_label***REMOVED***`) "
                "CALL { WITH n DETACH DELETE n ***REMOVED*** "
                "IN TRANSACTIONS OF 10000 ROWS;"
***REMOVED***
            # Delete index
            try:
                self.query(f"DROP INDEX {self.index_name***REMOVED***")
    ...
    def query(
        ...
        Args:
            ...
            params (dict, optional): Dictionary of query parameters. Defaults to {***REMOVED***.
...
***REMOVED***
    ...
    def create_new_keyword_index(self, text_node_properties: List[str] = []) -> None:
        ...
        fts_index_query = (
            f"CREATE FULLTEXT INDEX {self.keyword_index_name***REMOVED*** "
            f"FOR (n:`{self.node_label***REMOVED***`) ON EACH "
            f"[{', '.join(['n.`' + el + '`' for el in node_props])***REMOVED***]"
        )
    ...
    def __from(
        ...
        **kwargs: Any,
        ...
        if not metadatas:
            metadatas = [{***REMOVED*** for _ in texts]
        ...
        # If the index already exists, check if embedding dimensions match
            ...
            raise ValueError(
                f"Index with name {store.index_name***REMOVED*** already exists."
                "The provided embedding function and vector index "
                "dimensions do not match.\n"
                f"Embedding function dimension: {store.embedding_dimension***REMOVED***\n"
                f"Vector index dimension: {embedding_dimension***REMOVED***"
***REMOVED***
        ...
        ***REMOVED***
        ...
        if create_id_index:
            store.query(
                ...
                f"FOR (n:`{store.node_label***REMOVED***`) REQUIRE n.id IS UNIQUE;"
***REMOVED***
    ...
    def add_embeddings(
        ...
        **kwargs: Any,
        ...
        import_query = (
            "UNWIND $data AS row "
            "CALL { WITH row "
            f"MERGE (c:`{self.node_label***REMOVED***` {{id: row.id***REMOVED******REMOVED***) "
            "WITH c, row "
            f"CALL db.create.setVectorProperty(c, "
            f"'{self.embedding_node_property***REMOVED***', row.embedding) "
            "YIELD node "
            f"SET c.`{self.text_node_property***REMOVED***` = row.text "
            "SET c += row.metadata ***REMOVED*** IN TRANSACTIONS OF 1000 ROWS"
        )

        parameters = {
            "data": [
                {"text": text, "metadata": metadata, "embedding": embedding, "id": id***REMOVED***
    ...
    ***REMOVED***
    ...
    def similarity_search_with_score_by_vector(
        ...
        """
        default_retrieval = (
            f"RETURN node.`{self.text_node_property***REMOVED***` AS text, score, "
            f"node {{.*, `{self.text_node_property***REMOVED***`: Null, "
            f"`{self.embedding_node_property***REMOVED***`: Null, id: Null ***REMOVED******REMOVED*** AS metadata"
        )
...
***REMOVED***
        ...
        if not retrieval_query:
            retrieval_query = (
                f"RETURN reduce(str='', k IN {text_node_properties***REMOVED*** |"
                " str + '\\n' + k + ': ' + coalesce(node[k], '')) AS text, "
                "node {.*, `"
                + embedding_node_property
                + "`: Null, id: Null, "
                + ", ".join([f"`{prop***REMOVED***`: Null" for prop in text_node_properties])
                + "***REMOVED*** AS metadata, score"
***REMOVED***
        store = cls(
            embedding=embedding,
            index_name=index_name,
...
***REMOVED***
        ...
        ***REMOVED***
        ...
        while True:
            fetch_query = (
                f"MATCH (n:`{node_label***REMOVED***`) "
                f"WHERE n.{embedding_node_property***REMOVED*** IS null "
                "AND any(k in $props WHERE n[k] IS NOT null) "
                f"RETURN elementId(n) AS id, reduce(str='',"
                "k IN $props | str + '\\n' + k + ':' + coalesce(n[k], '')) AS text "
                "LIMIT 1000"
***REMOVED***
            data = store.query(fetch_query, params={"props": text_node_properties***REMOVED***)
            text_embeddings = embedding.embed_documents([el["text"] for el in data])

            params = {
                "data": [
                    {"id": el["id"], "embedding": embedding***REMOVED***
            ...
            ***REMOVED***
            ...
            store.query(
                ...
                f"'{embedding_node_property***REMOVED***', row.embedding) "
    ...
    def _select_relevance_score_fn(self) -> Callable[[float], float]:
        ...
        if self._distance_strategy == DistanceStrategy.COSINE:
            ...
            raise ValueError(
                ...
                f" for distance_strategy of {self._distance_strategy***REMOVED***."
