...
class TestDataFramePlots:
    ...
    def test_plot_invalid_arg(self):
        df = DataFrame({"x": [1, 2], "y": [3, 4]***REMOVED***)
    ...
    @pytest.mark.parametrize(
        "kwargs",
        [
            {"yticks": [1, 5, 10]***REMOVED***,
            {"xticks": [1, 5, 10]***REMOVED***,
            {"ylim": (-100, 100), "xlim": (-100, 100)***REMOVED***,
            {"default_axes": True, "subplots": True, "title": "blah"***REMOVED***,
        ],
    )
    ...
    def test_plot_single_column_bar(self, layout):
        ...
        df = DataFrame({"x": np.random.default_rng(2).random(10)***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ["hist", {***REMOVED***],
            ["pie", {"y": "y"***REMOVED***],
            ["scatter", {"x": "x", "y": "y"***REMOVED***],
            ["hexbin", {"x": "x", "y": "y"***REMOVED***],
        ],
    )
    def test_integer_array_plot_df(self, plot, kwargs):
        # GH 25587
        arr = pd.array([1, 2, 3, 4], dtype="UInt32")
        df = DataFrame({"x": arr, "y": arr***REMOVED***)
        _check_plot_works(getattr(df.plot, plot), **kwargs)

    def test_nonnumeric_exclude(self):
        df = DataFrame({"A": ["x", "y", "z"], "B": [1, 2, 3]***REMOVED***)
        ax = df.plot()
    ...
    def test_logscales(self, input_log, expected_log):
        df = DataFrame({"a": np.arange(100)***REMOVED***, index=np.arange(100))
    ...
    def test_invalid_logscale(self, input_param):
        ...
        msg = f"keyword '{input_param***REMOVED***' should be bool, None, or 'sym', not 'sm'"
        with pytest.raises(ValueError, match=msg):
            df.plot(**{input_param: "sm"***REMOVED***)

        msg = f"PiePlot ignores the '{input_param***REMOVED***' keyword"
        with tm.assert_produces_warning(UserWarning, match=msg):
            df.plot.pie(subplots=True, **{input_param: True***REMOVED***)
    ...
    @pytest.mark.parametrize(
        "df",
        [
            DataFrame({"y": [0.0, 1.0, 2.0, 3.0]***REMOVED***, index=[1.0, 0.0, 3.0, 2.0]),
            DataFrame(
                {"y": [0.0, 1.0, np.nan, 3.0, 4.0, 5.0, 6.0]***REMOVED***,
...
***REMOVED***,
    ...
    def test_unsorted_index_lims_x_y(self):
        df = DataFrame({"y": [0.0, 1.0, 2.0, 3.0], "z": [91.0, 90.0, 93.0, 92.0]***REMOVED***)
        ax = df.plot(x="z", y="y")
        xmin, xmax = ax.get_xlim()
        lines = ax.get_lines()
        assert xmin <= np.nanmin(lines[0].get_data()[0])
    ...
    def test_line_area_nan_df(self, idx):
        ...
        df = DataFrame({"a": values1, "b": values2***REMOVED***, index=idx)
    ...
    @pytest.mark.parametrize("kwargs", [{***REMOVED***, {"stacked": False***REMOVED***])
    ...
    @pytest.mark.parametrize("kwargs", [{***REMOVED***, {"secondary_y": True***REMOVED***])
    ...
    def test_bar_nan(self):
        df = DataFrame({"A": [10, np.nan, 20], "B": [5, 10, 20], "C": [1, 2, 3]***REMOVED***)
        ax = df.plot.bar()
    ...
    def test_scatterplot_datetime_data(self, x, y):
        ...
        df = DataFrame({"dates": dates, "vals": vals***REMOVED***)
    ...
    def test_scatterplot_object_data(self, b_col, x, y, infer_string):
        ...
        with option_context("future.infer_string", infer_string):
            df = DataFrame({"a": ["A", "B", "C"], "b": b_col***REMOVED***)
    ...
    def test_plot_scatter_with_categorical_data(self, x, y):
        ...
        df = DataFrame({"x": [1, 2, 3, 4], "y": pd.Categorical(["a", "b", "a", "c"])***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"default_axes": True, "subplots": True***REMOVED***,
            {"stacked": True***REMOVED***,
        ],
    )
    ...
    def test_plot_bar_ticks(self):
        df = DataFrame({"a": [0, 1], "b": [1, 0]***REMOVED***)
    ...
    def test_boxplot_return_type_invalid(self):
        ...
        msg = "return_type must be {None, 'axes', 'dict', 'both'***REMOVED***"
        with pytest.raises(ValueError, match=msg):
    ...
    ***REMOVED***,
    ...
    def test_specified_props_kwd_plot_box(self, props, expected):
        # GH 30346
        df = DataFrame({k: np.random.default_rng(2).random(100) for k in "ABC"***REMOVED***)
        kwd = {props: {"color": "C1"***REMOVED******REMOVED***
    ...
    @pytest.mark.parametrize(
        "kwargs, expected",
        [
            ({***REMOVED***, "BuGn"),  # default cmap
            ({"colormap": "cubehelix"***REMOVED***, "cubehelix"),
            ({"cmap": "YlGn"***REMOVED***, "YlGn"),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            {"logy": True***REMOVED***,
            {"logx": True, "logy": True***REMOVED***,
            {"loglog": True***REMOVED***,
        ],
    )
    def test_errorbar_plot(self, kwargs):
        d = {"x": np.arange(12), "y": np.arange(12, 0, -1)***REMOVED***
        df = DataFrame(d)
        d_err = {"x": np.ones(12) * 0.2, "y": np.ones(12) * 0.4***REMOVED***
    ...
    def test_errorbar_plot_external_typeerror(self):
        d = {"x": np.arange(12), "y": np.arange(12, 0, -1)***REMOVED***
        df = DataFrame(d)
        df_err = DataFrame({"x": ["zzz"] * 12, "y": ["zzz"] * 12***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            DataFrame({"x": np.ones(12) * 0.2, "y": np.ones(12) * 0.4***REMOVED***),
        ],
    )
    def test_errorbar_plot_different_yerr(self, kind, y_err):
        df = DataFrame({"x": np.arange(12), "y": np.arange(12, 0, -1)***REMOVED***)
...
***REMOVED***,
    ...
    def test_errorbar_plot_different_yerr_xerr_subplots(self, kind):
        df = DataFrame({"x": np.arange(12), "y": np.arange(12, 0, -1)***REMOVED***)
        df_err = DataFrame({"x": np.ones(12) * 0.2, "y": np.ones(12) * 0.4***REMOVED***)
        axes = _check_plot_works(
            df.plot,
            default_axes=True,
            yerr=df_err,
    ...
    def test_errorbar_with_partial_columns_box(self, err_box):
        d = {"x": np.arange(12), "y": np.arange(12, 0, -1)***REMOVED***
        df = DataFrame(d)
        err = err_box({"x": np.ones(12) * 0.2, "z": np.ones(12) * 0.4***REMOVED***)
    ...
    def test_scatter_unknown_colormap(self):
        # GH#48726
        df = DataFrame({"a": [1, 2, 3], "b": 4***REMOVED***)
...
***REMOVED***
    ...
    def test_df_grid_settings(self):
        ...
        _check_grid_settings(
            DataFrame({"a": [1, 2, 3], "b": [2, 3, 4]***REMOVED***),
...
***REMOVED***
        ...
        kwargs = {"secondary_y": sy, "fontsize": fontsize, "mark_right": True***REMOVED***
    ...
    def test_group_subplot_bad_input(self, subplots, expected_msg):
        ...
        d = {"a": np.arange(10), "b": np.arange(10)***REMOVED***
        df = DataFrame(d)
    ...
    def test_group_subplot_duplicated_column(self):
        d = {"a": np.arange(10), "b": np.arange(10), "c": np.arange(10)***REMOVED***
        df = DataFrame(d)
