...
@pytest.mark.parametrize(
    ...
    [
        ({***REMOVED***, {"dtype": np.int64***REMOVED***),
        ({"errors": "coerce", "downcast": "integer"***REMOVED***, {"dtype": np.int8***REMOVED***),
    ],
)
...
@pytest.mark.parametrize(
    "data,arr_kwargs",
    [
        ([1, 3, 4, 5], {"dtype": np.int64***REMOVED***),
        ([1.0, 3.0, 4.0, 5.0], {***REMOVED***),
...
@pytest.mark.parametrize("kwargs", [{"dtype": "O"***REMOVED***, {***REMOVED***])
...
def test_numeric_df_columns(columns):
    ...
    expected = DataFrame({"a": [1.2, 3.14, np.inf, 0.1], "b": [1.0, 2.0, 3.0, 4.0]***REMOVED***)
...
def test_numeric_embedded_arr_likes(data, exp_data):
    ...
    expected = DataFrame({"a": exp_data***REMOVED***)
...
def test_type_check(errors):
    # see gh-11776
    df = DataFrame({"a": [1, -3.14, 7], "b": ["4", "5", "6"]***REMOVED***)
    kwargs = {"errors": errors***REMOVED*** if errors is not None else {***REMOVED***
...
def test_really_large_in_arr_consistent(large_val, signed, multiple_elts, errors):
    ...
    if errors in (None, "raise"):
        ...
        msg = f"Integer out of range. at position {index***REMOVED***"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({***REMOVED***, np.int64),
        ({"downcast": None***REMOVED***, np.int64),
        # Support below np.float32 is rare and far between.
        ({"downcast": "float"***REMOVED***, np.dtype(np.float32).char),
        # Basic dtype support.
        ({"downcast": "unsigned"***REMOVED***, np.dtype(np.typecodes["UnsignedInteger"][0])),
    ],
)
