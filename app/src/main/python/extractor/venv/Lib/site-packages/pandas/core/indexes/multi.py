...
_index_doc_kwargs.update(
    {"klass": "MultiIndex", "target_klass": "MultiIndex or list of tuples"***REMOVED***
)
...
***REMOVED***
        ...
        for i in levels_to_verify:
            ...
            if len(level_codes) != codes_length:
                raise ValueError(
                    f"Unequal code lengths: {[len(code_) for code_ in codes]***REMOVED***"
    ***REMOVED***
            if len(level_codes) and level_codes.max() >= len(level):
                raise ValueError(
                    f"On level {i***REMOVED***, code max ({level_codes.max()***REMOVED***) >= length of "
                    f"level ({len(level)***REMOVED***). NOTE: this index is in an "
                    "inconsistent state"
    ***REMOVED***
            if len(level_codes) and level_codes.min() < -1:
                raise ValueError(f"On level {i***REMOVED***, code value ({level_codes.min()***REMOVED***) < -1")
            if not level.is_unique:
                raise ValueError(
                    f"Level values must be unique: {list(level)***REMOVED*** on level {i***REMOVED***"
    ***REMOVED***
        if self.sortorder is not None:
            if self.sortorder > _lexsort_depth(self.codes, self.nlevels):
                raise ValueError(
                    "Value for sortorder must be inferior or equal to actual "
                    f"lexsort_depth: sortorder {self.sortorder***REMOVED*** "
                    f"with lexsort_depth {_lexsort_depth(self.codes, self.nlevels)***REMOVED***"
    ***REMOVED***
    ...
    ***REMOVED***
        ...
        for lev, name in zip(level, names):
            if name is not None:
                ...
                if not is_hashable(name):
                    raise TypeError(
                        f"{type(self).__name__***REMOVED***.name must be a hashable type"
        ***REMOVED***
    ...
    def _get_level_number(self, level) -> int:
        ...
        if (count > 1) and not is_integer(level):
            raise ValueError(
                f"The name {level***REMOVED*** occurs multiple times, use a level number"
***REMOVED***
        try:
            level = self.names.index(level)
        except ValueError as err:
            if not is_integer(level):
                raise KeyError(f"Level {level***REMOVED*** not found") from err
            if level < 0:
                level += self.nlevels
                if level < 0:
                    orig_level = level - self.nlevels
                    raise IndexError(
                        f"Too many levels: Index has only {self.nlevels***REMOVED*** levels, "
                        f"{orig_level***REMOVED*** is not a valid level number"
        ***REMOVED*** from err
            # Note: levels are zero-based
            elif level >= self.nlevels:
                raise IndexError(
                    f"Too many levels: Index has only {self.nlevels***REMOVED*** levels, "
                    f"not {level + 1***REMOVED***"
    ***REMOVED*** from err
        return level
...
***REMOVED***
        ...
        result = DataFrame(
            {level: self._get_level_values(level) for level in range(len(self.levels))***REMOVED***,
            copy=False,
        )
...
***REMOVED***
    ...
    def drop(  # type: ignore[override]
        ...
        for level_codes in codes:
            ...
            ***REMOVED***
                    ...
                    msg = f"unsupported indexer of type {type(loc)***REMOVED***"
    ...
    def _drop_from_level(
        ...
        if len(not_found) != 0 and errors != "ignore":
            raise KeyError(f"labels {not_found***REMOVED*** not found in level")
    ...
    def _reorder_ilevels(self, order) -> MultiIndex:
        if len(order) != self.nlevels:
            raise AssertionError(
                f"Length of order must be same as number of levels ({self.nlevels***REMOVED***), "
                f"got {len(order)***REMOVED***"
***REMOVED***
    ...
    def _recode_for_new_levels(
        ...
        if len(new_levels) > self.nlevels:
            raise AssertionError(
                f"Length of new_levels ({len(new_levels)***REMOVED***) "
                f"must be <= self.nlevels ({self.nlevels***REMOVED***)"
***REMOVED***
    ...
    def _raise_if_missing(self, key, indexer, axis_name: str) -> None:
        ...
        if len(keyarr) and not isinstance(keyarr[0], tuple):
            ...
            if mask.any():
                ...
                cmask = check == -1
                if cmask.any():
                    raise KeyError(f"{keyarr[cmask]***REMOVED*** not in index")
                # We get here when levels still contain values which are not
                # actually in Index anymore
                raise KeyError(f"{keyarr***REMOVED*** not in index")
        else:
    ...
    def _partial_tup_index(self, tup: tuple, side: Literal["left", "right"] = "left"):
        if len(tup) > self._lexsort_depth:
            raise UnsortedIndexError(
                f"Key length ({len(tup)***REMOVED***) was greater than MultiIndex lexsort depth "
                f"({self._lexsort_depth***REMOVED***)"
***REMOVED***
        ...
        for k, (lab, lev, level_codes) in enumerate(zipped):
            ...
            if lab not in lev and not isna(lab):
                ...
                try:
                    ...
                    raise TypeError(f"Level type mismatch: {lab***REMOVED***") from err
                if not is_integer(loc):
                    # non-comparable level, e.g. test_groupby_example
                    raise TypeError(f"Level type mismatch: {lab***REMOVED***")
    ...
    ***REMOVED***
    ...
    def get_loc(self, key):
        ...
        if self.nlevels < keylen:
            raise KeyError(
                f"Key length ({keylen***REMOVED***) exceeds index depth ({self.nlevels***REMOVED***)"
***REMOVED***
        ...
        if isinstance(key, tuple) and level == 0:
            ...
            ***REMOVED***
                ...
                ***REMOVED***:
                        ...
                        raise TypeError(f"Expected label or tuple of labels, got {key***REMOVED***")
    ...
    def get_locs(self, seq) -> npt.NDArray[np.intp]:
        ...
        if true_slices and true_slices[-1] >= self._lexsort_depth:
            raise UnsortedIndexError(
                ...
                f"on levels {true_slices***REMOVED***, lexsort depth {self._lexsort_depth***REMOVED***"
***REMOVED***
...
def _get_na_rep(dtype: DtypeObj) -> str:
    if isinstance(dtype, ExtensionDtype):
        return f"{dtype.na_value***REMOVED***"
    else:
        dtype_type = dtype.type

    return {np.datetime64: "NaT", np.timedelta64: "NaT"***REMOVED***.get(dtype_type, "NaN")
...
def _require_listlike(level, arr, arrname: str):
    ...
    if level is not None and not is_list_like(level):
        if not is_list_like(arr):
            raise TypeError(f"{arrname***REMOVED*** must be list-like")
        ...
        if not is_list_like(arr) or not is_list_like(arr[0]):
            raise TypeError(f"{arrname***REMOVED*** must be list of lists-like")
    return level, arr
