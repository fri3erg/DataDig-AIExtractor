        ...
        **kwargs: Any,
        ...
        if metadatas is not None and len(texts) != len(metadatas):
            raise ValueError(
                ...
                f"{len(texts)***REMOVED*** texts and {len(metadatas)***REMOVED*** metadatas."
***REMOVED***
        ...
        for idx, (embedding, text) in enumerate(zip(embeddings, texts)):
            id = str(uuid.uuid4())
            ids.append(id)
            json_: dict = {"id": id, "embedding": embedding***REMOVED***
            if metadatas is not None:
                json_["metadata"] = metadatas[idx]
            jsons.append(json_)
            self._upload_to_gcs(text, f"documents/{id***REMOVED***")

        logger.debug(f"Uploaded {len(ids)***REMOVED*** documents to GCS.")

        # Creating json lines from the embedded documents.
        result_str = "\n".join([json.dumps(x) for x in jsons])

        filename_prefix = f"indexes/{uuid.uuid4()***REMOVED***"
        filename = f"{filename_prefix***REMOVED***/{time.time()***REMOVED***.json"
        self._upload_to_gcs(result_str, filename)
        logger.debug(
            f"Uploaded updated json with embeddings to "
            f"{self.gcs_bucket_name***REMOVED***/{filename***REMOVED***."
        )

        self.index = self.index.update_embeddings(
            contents_delta_uri=f"gs://{self.gcs_bucket_name***REMOVED***/{filename_prefix***REMOVED***/"
        )
    ...
    def similarity_search_with_score(
        ...
        """
        logger.debug(f"Embedding query {query***REMOVED***.")
...
***REMOVED***
        ...
        logger.debug(f"Found {len(response)***REMOVED*** matches.")
        ...
        for result in response[0]:
            page_content = self._download_from_gcs(f"documents/{result.id***REMOVED***")
...
***REMOVED***
    ...
    def _get_index_id(self) -> str:
        ...
        raise ValueError(
            f"No index with id {self.index.resource_name***REMOVED*** "
            f"deployed on endpoint "
            f"{self.endpoint.display_name***REMOVED***."
        )
    ...
    def _validate_gcs_bucket(cls, gcs_bucket_name: str) -> str:
        ...
        if "/" in gcs_bucket_name:
            raise ValueError(
                ...
                f"the bucket name. Received {gcs_bucket_name***REMOVED***"
***REMOVED***
...
***REMOVED***
    ...
    def _create_index_by_id(
        ...
        logger.debug(f"Creating matching engine index with id {index_id***REMOVED***.")
    ...
    def _create_endpoint_by_id(
        ...
        logger.debug(f"Creating endpoint with id {endpoint_id***REMOVED***.")
    ...
    def _init_aiplatform(
        ...
        logger.debug(
            f"Initializing AI Platform for project {project_id***REMOVED*** on "
            f"{region***REMOVED*** and for {gcs_bucket_name***REMOVED***."
        )
        aiplatform.init(
            project=project_id,
            location=region,
