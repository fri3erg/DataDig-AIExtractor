...
class TestDatetimeConcat:
    ...
    def test_concat_datetime_datetime64_frame(self):
        ...
        df1 = DataFrame({"date": ind, "test": range(10)***REMOVED***)
    ...
    def test_concat_datetime_timezone(self):
        ...
        df1 = DataFrame({"a": [1, 2, 3]***REMOVED***, index=idx1)
        df2 = DataFrame({"b": [1, 2, 3]***REMOVED***, index=idx2)
        ...
        df3 = DataFrame({"b": [1, 2, 3]***REMOVED***, index=idx3)
        ...
        expected = DataFrame(
            {"a": [1, 2, 3] + [np.nan] * 3, "b": [np.nan] * 3 + [1, 2, 3]***REMOVED***,
...
class TestTimezoneConcat:
    ...
    def test_concat_tz_series3(self, unit, unit2):
        ...
        first = DataFrame([[datetime(2016, 1, 1)]], dtype=f"M8[{unit***REMOVED***]")
        first[0] = first[0].dt.tz_localize("UTC")

        second = DataFrame([[datetime(2016, 1, 2)]], dtype=f"M8[{unit2***REMOVED***]")
        second[0] = second[0].dt.tz_localize("UTC")

        result = concat([first, second])
        exp_unit = tm.get_finest_unit(unit, unit2)
        assert result[0].dtype == f"datetime64[{exp_unit***REMOVED***, UTC]"
    ...
    def test_concat_tz_series4(self, unit, unit2):
        ...
        result = concat([first, second])
        exp_unit = tm.get_finest_unit(unit, unit2)
        assert result[0].dtype == f"datetime64[{exp_unit***REMOVED***, Europe/London]"

    def test_concat_tz_series5(self, unit, unit2):
        # Concatenating 2+1 London times
        first = DataFrame(
            [[datetime(2016, 1, 1)], [datetime(2016, 1, 2)]], dtype=f"M8[{unit***REMOVED***]"
        )
        first[0] = first[0].dt.tz_localize("Europe/London")

        second = DataFrame([[datetime(2016, 1, 3)]], dtype=f"M8[{unit2***REMOVED***]")
        second[0] = second[0].dt.tz_localize("Europe/London")
    ...
    def test_concat_tz_series6(self, unit, unit2):
        ...
        second = DataFrame(
            [[datetime(2016, 1, 2)], [datetime(2016, 1, 3)]], dtype=f"M8[{unit2***REMOVED***]"
        )
        second[0] = second[0].dt.tz_localize("Europe/London")
    ...
    def test_concat_multiple_tzs(self):
        ...
        df1 = DataFrame({"time": [ts1]***REMOVED***)
        df2 = DataFrame({"time": [ts2]***REMOVED***)
        df3 = DataFrame({"time": [ts3]***REMOVED***)

        results = concat([df1, df2]).reset_index(drop=True)
        expected = DataFrame({"time": [ts1, ts2]***REMOVED***, dtype=object)
        tm.assert_frame_equal(results, expected)

        results = concat([df1, df3]).reset_index(drop=True)
        expected = DataFrame({"time": [ts1, ts3]***REMOVED***, dtype=object)
        tm.assert_frame_equal(results, expected)

        results = concat([df2, df3]).reset_index(drop=True)
        expected = DataFrame({"time": [ts2, ts3]***REMOVED***)
        tm.assert_frame_equal(results, expected)
    ...
    ***REMOVED***,
        ...
        expected = DataFrame(
            {"c": [1, 2, 3] * 2, "d": [4, 5, 6] * 2***REMOVED***, index=exp_idx, columns=["c", "d"]
        )
    ...
    def test_concat_tz_not_aligned(self):
        ...
        a = DataFrame({"A": ts***REMOVED***)
        b = DataFrame({"A": ts, "B": ts***REMOVED***)
        result = concat([a, b], sort=True, ignore_index=True)
        expected = DataFrame(
            {"A": list(ts) + list(ts), "B": [pd.NaT, pd.NaT] + list(ts)***REMOVED***
        )
        tm.assert_frame_equal(result, expected)
...
def test_concat_multiindex_datetime_nat():
    # GH#44900
    left = DataFrame({"a": 1***REMOVED***, index=MultiIndex.from_tuples([(1, pd.NaT)]))
    right = DataFrame(
        {"b": 2***REMOVED***, index=MultiIndex.from_tuples([(1, pd.NaT), (2, pd.NaT)])
    )
    result = concat([left, right], axis="columns")
    expected = DataFrame(
        {"a": [1.0, np.nan], "b": 2***REMOVED***, MultiIndex.from_tuples([(1, pd.NaT), (2, pd.NaT)])
    )
    tm.assert_frame_equal(result, expected)
...
def test_concat_float_datetime64(using_array_manager):
    # GH#32934
    df_time = DataFrame({"A": pd.array(["2000"], dtype="datetime64[ns]")***REMOVED***)
    df_float = DataFrame({"A": pd.array([1.0], dtype="float64")***REMOVED***)
    ...
    expected = DataFrame({"A": pd.array([], dtype="object")***REMOVED***)
    result = concat([df_time.iloc[:0], df_float.iloc[:0]])
    tm.assert_frame_equal(result, expected)

    expected = DataFrame({"A": pd.array([1.0], dtype="object")***REMOVED***)
    result = concat([df_time.iloc[:0], df_float])
    tm.assert_frame_equal(result, expected)

    if not using_array_manager:
        expected = DataFrame({"A": pd.array(["2000"], dtype="datetime64[ns]")***REMOVED***)
        msg = "The behavior of DataFrame concatenation with empty or all-NA entries"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = concat([df_time, df_float.iloc[:0]])
        tm.assert_frame_equal(result, expected)
    else:
        expected = DataFrame({"A": pd.array(["2000"], dtype="datetime64[ns]")***REMOVED***).astype(
            {"A": "object"***REMOVED***
        )
        result = concat([df_time, df_float.iloc[:0]])
        tm.assert_frame_equal(result, expected)
