...
two are written in Python and included in the ``nltk.inference`` package.
    ...
    [2] {man(socrates)***REMOVED***         A
    [3] {-man(z2), mortal(z2)***REMOVED***  A
    [4] {-man(socrates)***REMOVED***        (1, 3)
...
a call to ``prove()`` will execute the theorem prover.
    ...
    >>> print(prover.proof())
    [1] {-mortal(socrates)***REMOVED***     A
    [2] {-man(z6), mortal(z6)***REMOVED***  A
    [3] {-man(socrates)***REMOVED***        (1, 2)
    <BLANKLINE>
...
Thus, the following interaction shows how both ``{a, c1***REMOVED***`` and ``{a, c2***REMOVED***``
are consistent sets, since Mace succeeds in a building a
model for each of them, while ``{c1, c2***REMOVED***`` is inconsistent.
...
is logically entailed by assumptions ``S = {s1, s2, ..., sn***REMOVED***``.
...
the set ``S' = {s1, s2, ..., sn, (not g)***REMOVED***``. If ``g`` fails to follow
...
The property ``valuation`` accesses the stored ``Valuation``.
    ...
    >>> print(mb.valuation)
    {'john': 'a', 'mary': 'b', 'see': {('b', 'a')***REMOVED******REMOVED***
