...
class InMemoryEntityStore(BaseEntityStore):
    ...
    store: Dict[str, Optional[str]] = {***REMOVED***
...
***REMOVED***
    ...
    def full_key_prefix(self) -> str:
        return f"{self.key_prefix***REMOVED***:{self.session_id***REMOVED***"

    def get(self, key: str, default: Optional[str] = None) -> Optional[str]:
        res = (
            self.redis_client.getex(f"{self.full_key_prefix***REMOVED***:{key***REMOVED***", ex=self.recall_ttl)
            or default
            or ""
        )
        logger.debug(f"Upstash Redis MEM get '{self.full_key_prefix***REMOVED***:{key***REMOVED***': '{res***REMOVED***'")
        return res

    def set(self, key: str, value: Optional[str]) -> None:
        if not value:
            return self.delete(key)
        self.redis_client.set(f"{self.full_key_prefix***REMOVED***:{key***REMOVED***", value, ex=self.ttl)
        logger.debug(
            f"Redis MEM set '{self.full_key_prefix***REMOVED***:{key***REMOVED***': '{value***REMOVED***' EX {self.ttl***REMOVED***"
        )

    def delete(self, key: str) -> None:
        self.redis_client.delete(f"{self.full_key_prefix***REMOVED***:{key***REMOVED***")

    def exists(self, key: str) -> bool:
        return self.redis_client.exists(f"{self.full_key_prefix***REMOVED***:{key***REMOVED***") == 1

    def clear(self) -> None:
        def scan_and_delete(cursor: int) -> int:
            cursor, keys_to_delete = self.redis_client.scan(
                cursor, f"{self.full_key_prefix***REMOVED***:*"
***REMOVED***
...
***REMOVED***
    ...
    def get(self, key: str, default: Optional[str] = None) -> Optional[str]:
        ...
        logger.debug(f"REDIS MEM get '{self.full_key_prefix***REMOVED***:{key***REMOVED***': '{res***REMOVED***'")
        return res
    ...
    def clear(self) -> None:
        ...
        for keybatch in batched(
            self.redis_client.scan_iter(f"{self.full_key_prefix***REMOVED***:*"), 500
        ):
...
***REMOVED***
    ...
    def full_table_name(self) -> str:
        return f"{self.table_name***REMOVED***_{self.session_id***REMOVED***"

    def _create_table_if_not_exists(self) -> None:
        create_table_query = f"""
            CREATE TABLE IF NOT EXISTS {self.full_table_name***REMOVED*** (
...
***REMOVED***
    ...
    def get(self, key: str, default: Optional[str] = None) -> Optional[str]:
        query = f"""
            SELECT value
            FROM {self.full_table_name***REMOVED***
            WHERE key = ?
        """
    ...
    def set(self, key: str, value: Optional[str]) -> None:
        if not value:
            return self.delete(key)
        query = f"""
            INSERT OR REPLACE INTO {self.full_table_name***REMOVED*** (key, value)
            VALUES (?, ?)
        """
        with self.conn:
    ...
    def delete(self, key: str) -> None:
        query = f"""
            DELETE FROM {self.full_table_name***REMOVED***
            WHERE key = ?
        """
        with self.conn:
