...
class BaseGetitemTests:
    ...
    def test_iloc_frame(self, data):
        df = pd.DataFrame({"A": data, "B": np.arange(len(data), dtype="int64")***REMOVED***)
        expected = pd.DataFrame({"A": data[:4]***REMOVED***)
    ...
    def test_getitem_invalid(self, data):
        ...
        msg = "|".join(
            [
                ...
                f"loc must be an integer between -{ub***REMOVED*** and {ub***REMOVED***",  # Sparse
                f"index {ub+1***REMOVED*** is out of bounds for axis 0 with size {ub***REMOVED***",
                f"index -{ub+1***REMOVED*** is out of bounds for axis 0 with size {ub***REMOVED***",
            ]
        )
        with pytest.raises(IndexError, match=msg):
    ...
    def test_getitem_mask_raises(self, data):
        ...
        msg = f"Boolean index has wrong length: 2 instead of {len(data)***REMOVED***"
        with pytest.raises(IndexError, match=msg):
            data[mask]
