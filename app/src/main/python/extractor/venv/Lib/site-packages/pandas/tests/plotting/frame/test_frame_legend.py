...
class TestFrameLegend:
    ...
    def test_mixed_yerr(self):
        ...
        df = DataFrame([{"x": 1, "a": 1, "b": 1***REMOVED***, {"x": 2, "a": 2, "b": 3***REMOVED***])
    ...
    def test_legend_false(self):
        ...
        df = DataFrame({"a": [1, 1], "b": [2, 3]***REMOVED***)
        df2 = DataFrame({"d": [2.5, 2.5]***REMOVED***)

        ax = df.plot(legend=True, color={"a": "blue", "b": "green"***REMOVED***, secondary_y="b")
        df2.plot(legend=True, color={"d": "red"***REMOVED***, ax=ax)
        legend = ax.get_legend()
        if Version(mpl.__version__) < Version("3.7"):
