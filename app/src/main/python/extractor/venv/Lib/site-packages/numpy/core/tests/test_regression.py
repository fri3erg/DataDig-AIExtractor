...
class TestRegression:
    ...
    def test_mem_dtype_align(self):
        ...
        assert_raises(TypeError, np.dtype,
                              {'names':['a'], 'formats':['foo']***REMOVED***, align=1)
    ...
    def test_format_on_flex_array_element(self):
        ...
        formatted = '{0***REMOVED***'.format(arr[0])
    ...
    def test_invalid_structured_dtypes(self):
        ...
        assert_raises(ValueError, np.dtype,
                      ('i', {'name': ('i', 0, 'title', 'oops')***REMOVED***))
        assert_raises(ValueError, np.dtype,
                      ('i', {'name': ('i', 'wrongtype', 'title')***REMOVED***))
    ...
    def test_correct_hash_dict(self):
        ...
        all_types = set(np.sctypeDict.values()) - {np.void***REMOVED***
        for t in all_types:
            val = t()
    ...
    def test_ufunc_order(self, operation, order):
        ...
        def get_idx(string, str_lst):
            for i, s in enumerate(str_lst):
                if string in s:
                    return i
            raise ValueError(f"{string***REMOVED*** not in list")
        types = getattr(np, operation).types
        assert get_idx(order[0], types) < get_idx(order[1], types), (
                f"Unexpected types order of ufunc in {operation***REMOVED***"
                f"for {order***REMOVED***. Possible fix: Use signed before unsigned"
