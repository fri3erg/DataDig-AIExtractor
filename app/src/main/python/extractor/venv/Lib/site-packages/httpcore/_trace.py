...
class Trace:
    ...
    def trace(self, name: str, info: Dict[str, Any]) -> None:
        if self.trace_extension is not None:
            prefix_and_name = f"{self.prefix***REMOVED***.{name***REMOVED***"
    ...
    ***REMOVED***
        ...
        if self.debug:
            if not info or "return_value" in info and info["return_value"] is None:
                ...
                args = " ".join([f"{key***REMOVED***={value!r***REMOVED***" for key, value in info.items()])
                message = f"{name***REMOVED*** {args***REMOVED***"
            self.logger.debug(message)

    def __enter__(self) -> "Trace":
        if self.should_trace:
            info = self.kwargs
            self.trace(f"{self.name***REMOVED***.started", info)
        return self
    ...
    def __exit__(
        ...
        if self.should_trace:
            if exc_value is None:
                info = {"return_value": self.return_value***REMOVED***
                self.trace(f"{self.name***REMOVED***.complete", info)
            else:
                info = {"exception": exc_value***REMOVED***
                self.trace(f"{self.name***REMOVED***.failed", info)
    ...
    ***REMOVED***
    ...
    async def __aenter__(self) -> "Trace":
        if self.should_trace:
            info = self.kwargs
            await self.atrace(f"{self.name***REMOVED***.started", info)
        return self
    ...
    async def __aexit__(
        ...
        if self.should_trace:
            if exc_value is None:
                info = {"return_value": self.return_value***REMOVED***
                await self.atrace(f"{self.name***REMOVED***.complete", info)
            else:
                info = {"exception": exc_value***REMOVED***
                await self.atrace(f"{self.name***REMOVED***.failed", info)
