...
def data_missing():
    ...
    return JSONArray([{***REMOVED***, {"a": 10***REMOVED***])


@pytest.fixture
def data_for_sorting():
    return JSONArray([{"b": 1***REMOVED***, {"c": 4***REMOVED***, {"a": 2, "c": 3***REMOVED***])


@pytest.fixture
def data_missing_for_sorting():
    return JSONArray([{"b": 1***REMOVED***, {***REMOVED***, {"a": 4***REMOVED***])
...
class TestJSONArray(base.ExtensionTests):
    ...
    def test_stack(self):
        ...
        rows since we consider `{***REMOVED***` NA, but `.astype(object)` doesn't.
        """
        super().test_stack()
...
def custom_assert_frame_equal(left, right, *args, **kwargs):
    ...
    tm.assert_index_equal(
        ...
        obj=f"{obj_type***REMOVED***.columns",
    )
...
def test_custom_asserts():
    ...
    data = JSONArray(
        [
            collections.UserDict({"a": 1***REMOVED***),
            collections.UserDict({"b": 2***REMOVED***),
            collections.UserDict({"c": 3***REMOVED***),
        ]
    )
    a = pd.Series(data)
