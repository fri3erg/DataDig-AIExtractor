...
@nx._dispatch(
    graphs={"G1": 0, "G2": 1***REMOVED***, preserve_edge_attrs=True, preserve_node_attrs=True
)
...
@nx._dispatch(graphs={"G1": 0, "G2": 1***REMOVED***)
...
***REMOVED***
    ...
    ***REMOVED***
    ...
    # debug_print(f"Cv: {m***REMOVED*** x {n***REMOVED***")
    # debug_print(Cv.C)
    ...
    # debug_print(f'Ce: {m***REMOVED*** x {n***REMOVED***')
    # debug_print(Ce.C)
    # debug_print()
...
def simrank_similarity(
    ...
    {0: {0: 1.0, 1: 0.0***REMOVED***, 1: {0: 0.0, 1: 1.0***REMOVED******REMOVED***
    ...
    if isinstance(x, np.ndarray):
        if x.ndim == 1:
            return dict(zip(G, x))
        # else x.ndim == 2
        return {u: dict(zip(G, row)) for u, row in zip(G, x)***REMOVED***
    return x
...
def _simrank_similarity_python(
    ...
    {0: {0: 1, 1: 0.0***REMOVED***, 1: {0: 0.0, 1: 1***REMOVED******REMOVED***
    ...
    newsim = {u: {v: 1 if u == v else 0 for v in G***REMOVED*** for u in G***REMOVED***
    ...
    for its in range(max_iterations):
        oldsim = newsim
        newsim = {u: {v: sim(u, v) if u != v else 1 for v in G***REMOVED*** for u in G***REMOVED***
        is_close = all(
            all(
...
***REMOVED***
    ...
    if its + 1 == max_iterations:
        raise nx.ExceededMaxIterations(
            f"simrank did not converge after {max_iterations***REMOVED*** iterations."
        )
...
def panther_similarity(
    ...
    if num_nodes < k:
        warnings.warn(
            f"Number of nodes is {num_nodes***REMOVED***, but requested k is {k***REMOVED***. "
    ...
    inv_node_map = {name: index for index, name in enumerate(G.nodes)***REMOVED***
    node_map = np.array(G)
    ...
    for path_index in range(sample_size):
        ...
        if index_map is not None:
            if node in index_map:
                ...
                index_map[node] = {path_index***REMOVED***
...
***REMOVED***
            ...
            if index_map is not None:
                if neighbor_node in index_map:
                    ...
                    index_map[neighbor_node] = {path_index***REMOVED***
