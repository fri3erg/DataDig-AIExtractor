...
@pytest.mark.parametrize(
    "test_data",
    [
        {"a": ["foo", "bar"], "b": ["baz", "qux"]***REMOVED***,
        {"a": [1.5, 2.5, 3.5], "b": [9.2, 10.5, 11.8]***REMOVED***,
        {"A": [1, 2, 3, 4], "B": [1, 2, 3, 4]***REMOVED***,
    ],
...
def test_mixed_dtypes(df_from_dict):
    ...
    columns = {"a": 0, "b": 0, "c": 2, "d": 0, "e": 20, "f": 21***REMOVED***
...
def test_na_float(df_from_dict):
    df = df_from_dict({"a": [1.0, math.nan, 2.0]***REMOVED***)
    dfX = df.__dataframe__()
...
def test_noncategorical(df_from_dict):
    df = df_from_dict({"a": [1, 2, 3]***REMOVED***)
    dfX = df.__dataframe__()
    colX = dfX.get_column_by_name("a")
...
def test_categorical(df_from_dict):
    df = df_from_dict(
        {"weekday": ["Mon", "Tue", "Mon", "Wed", "Mon", "Thu", "Fri", "Sat", "Sun"]***REMOVED***,
        is_categorical=True,
    )
...
def test_dataframe(df_from_dict):
    df = df_from_dict(
        {"x": [True, True, False], "y": [1, 2, 0], "z": [9.2, 10.5, 11.8]***REMOVED***
    )
    dfX = df.__dataframe__()
...
def test_df_get_chunks(size, n_chunks, df_from_dict):
    df = df_from_dict({"x": list(range(size))***REMOVED***)
    dfX = df.__dataframe__()
...
def test_get_columns(df_from_dict):
    df = df_from_dict({"a": [0, 1], "b": [2.5, 3.5]***REMOVED***)
    dfX = df.__dataframe__()
    ...
    if device == 1:  # CPU-only as we're going to directly read memory here
        ...
        ***REMOVED***[bitwidth]
        ...
        for idx, truth in enumerate(arr):
            ...
            assert val == truth, f"Buffer at index {idx***REMOVED*** mismatch"
