...
def load_f2cmap_file(f2cmap_file):
    ...
    # example, {'real':{'low':'float'***REMOVED******REMOVED*** means that Fortran 'real(low)' is
    # interpreted as C 'float'. This feature is useful for F90/95 users if
    # they use PARAMETERS in type specifications.
    try:
        outmess('Reading f2cmap from {!r***REMOVED*** ...\n'.format(f2cmap_file))
        with open(f2cmap_file) as f:
            d = eval(f.read().lower(), {***REMOVED***, {***REMOVED***)
    ...
    if isintent_out(var):
        for k in var['intent']:
            if k[:4] == 'out=':
                out_a = k[4:]
                break
    ret = {'varname': a, 'outvarname': out_a, 'ctype': getctype(var)***REMOVED***
    intent_flags = []
...
def modsign2map(m):
    ...
    if ismodule(m):
        ret = {'f90modulename': m['name'],
               ...
               'texf90modulename': m['name'].replace('_', '\\_')***REMOVED***
    else:
        ret = {'modulename': m['name'],
               'MODULENAME': m['name'].upper(),
               'texmodulename': m['name'].replace('_', '\\_')***REMOVED***
...
def common_sign2map(a, var):  # obsolute
    ret = {'varname': a, 'ctype': getctype(var)***REMOVED***
