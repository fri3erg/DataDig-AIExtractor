...
class TestPreserves:
    ...
    def test_preserved_frame(self):
        df = pd.DataFrame({"A": [1, 2], "B": [3, 4]***REMOVED***, index=["a", "b"]).set_flags(
    ...
    def test_preserve_getitem(self):
        df = pd.DataFrame({"A": [1, 2]***REMOVED***).set_flags(allows_duplicate_labels=False)
    ...
    def test_ndframe_getitem_caching_issue(
        ...
        df = pd.DataFrame({"A": [0]***REMOVED***).set_flags(allows_duplicate_labels=False)
        assert df["A"].flags.allows_duplicate_labels is False
...
***REMOVED***,
            ...
            (
                [
                    pd.DataFrame({"A": [1, 2]***REMOVED***, index=["a", "b"]),
                    pd.DataFrame({"A": [1, 2]***REMOVED***, index=["c", "d"]),
                ],
                {***REMOVED***,
***REMOVED***,
            (
...
***REMOVED***,
            (
                [
                    pd.DataFrame({"A": [1, 2]***REMOVED***, index=["a", "b"]),
                    pd.DataFrame({"B": [1, 2]***REMOVED***, index=["a", "b"]),
                ],
                {"axis": 1***REMOVED***,
***REMOVED***,
            # Series / Frame
...
***REMOVED***,
    ...
    @pytest.mark.parametrize(
        ...
        [
            # false false false
            pytest.param(
                pd.DataFrame({"A": [0, 1]***REMOVED***, index=["a", "b"]).set_flags(
                    allows_duplicate_labels=False
    ***REMOVED***,
                pd.DataFrame({"B": [0, 1]***REMOVED***, index=["a", "d"]).set_flags(
                    allows_duplicate_labels=False
    ***REMOVED***,
                False,
...
***REMOVED***,
            ...
            pytest.param(
                pd.DataFrame({"A": [0, 1]***REMOVED***, index=["a", "b"]).set_flags(
                    allows_duplicate_labels=False
    ***REMOVED***,
                pd.DataFrame({"B": [0, 1]***REMOVED***, index=["a", "d"]),
                False,
                marks=not_implemented,
***REMOVED***,
            # true true true
            (
                pd.DataFrame({"A": [0, 1]***REMOVED***, index=["a", "b"]),
                pd.DataFrame({"B": [0, 1]***REMOVED***, index=["a", "d"]),
                True,
***REMOVED***,
        ],
    ...
    def test_groupby(self):
        ...
        df = pd.DataFrame({"A": [1, 2, 3]***REMOVED***).set_flags(allows_duplicate_labels=False)
...
class TestRaises:
    @pytest.mark.parametrize(
        "cls, axes",
        [
            (pd.Series, {"index": ["a", "a"], "dtype": float***REMOVED***),
            (pd.DataFrame, {"index": ["a", "a"]***REMOVED***),
            (pd.DataFrame, {"index": ["a", "a"], "columns": ["b", "b"]***REMOVED***),
            (pd.DataFrame, {"columns": ["b", "b"]***REMOVED***),
        ],
    )
...
***REMOVED***
    ...
    def test_merge_raises(self):
        a = pd.DataFrame({"A": [0, 1, 2]***REMOVED***, index=["a", "b", "c"]).set_flags(
            allows_duplicate_labels=False
        )
        b = pd.DataFrame({"B": [0, 1, 2]***REMOVED***, index=["a", "b", "b"])
        msg = "Index has duplicates."
        with pytest.raises(pd.errors.DuplicateLabelError, match=msg):
...
def test_raises_basic(idx):
    ...
    with pytest.raises(pd.errors.DuplicateLabelError, match=msg):
        pd.DataFrame({"A": [1, 1]***REMOVED***, index=idx).set_flags(allows_duplicate_labels=False)
...
def test_format_duplicate_labels_message():
    ...
    expected = pd.DataFrame(
        {"positions": [[0, 2], [1, 3]]***REMOVED***, index=pd.Index(["a", "b"], name="label")
    )
...
def test_inplace_raises(method, frame_only):
    df = pd.DataFrame({"A": [0, 0], "B": [1, 2]***REMOVED***).set_flags(
        allows_duplicate_labels=False
    )
    s = df["A"]
...
def test_pickle():
    ...
    a = pd.DataFrame({"A": []***REMOVED***).set_flags(allows_duplicate_labels=False)
    b = tm.round_trip_pickle(a)
