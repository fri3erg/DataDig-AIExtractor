...
class TestDataFrameMissingData:
    def test_dropEmptyRows(self, float_frame):
        ...
        frame = DataFrame({"foo": mat***REMOVED***, index=float_frame.index)
    ...
    def test_drop_and_dropna_caching(self):
        ...
        df = DataFrame({"A": original.values.copy()***REMOVED***)
        df2 = df.copy()
        df["A"].dropna()
    ...
    def test_dropna_tz_aware_datetime(self):
        ...
        expected = DataFrame({"Time": [dt1]***REMOVED***)
        tm.assert_frame_equal(result, expected)

        # Ex2
        df = DataFrame({"Time": [dt1, None, np.nan, dt2]***REMOVED***)
        result = df.dropna(axis=0)
    ...
    def test_dropna_categorical_interval_index(self):
        ...
        df = DataFrame({"A": list("abc")***REMOVED***, index=ci)
    ...
    def test_set_single_column_subset(self):
        # GH 41021
        df = DataFrame({"A": [1, 2, 3], "B": list("abc"), "C": [4, np.nan, 5]***REMOVED***)
        expected = DataFrame(
            {"A": [1, 3], "B": list("ac"), "C": [4.0, 5.0]***REMOVED***, index=[0, 2]
        )
    ...
    def test_subset_is_nparray(self):
        # GH 41021
        df = DataFrame({"A": [1, 2, np.nan], "B": list("abc"), "C": [4, np.nan, 5]***REMOVED***)
        expected = DataFrame({"A": [1.0], "B": ["a"], "C": [4.0]***REMOVED***)
    ...
    def test_dropna_ignore_index(self, val):
        # GH#31725
        df = DataFrame({"a": [1, 2, val]***REMOVED***, index=[3, 2, 1])
        result = df.dropna(ignore_index=True)
        expected = DataFrame({"a": [1, 2, val]***REMOVED***)
        tm.assert_frame_equal(result, expected)
