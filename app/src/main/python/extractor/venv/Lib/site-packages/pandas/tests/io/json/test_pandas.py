...
def test_literal_json_deprecation():
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = read_json('{"a": 1, "b": 2***REMOVED***\n{"b":2, "a" :1***REMOVED***\n', lines=True)
...
class TestPandasContainer:
    ...
    def test_frame_non_unique_index_raises(self, orient):
        df = DataFrame([["a", "b"], ["c", "d"]], index=[1, 1], columns=["x", "y"])
        msg = f"DataFrame index must be unique for orient='{orient***REMOVED***'"
    ...
    def test_frame_non_unique_columns_raises(self, orient):
        ...
        msg = f"DataFrame columns must be unique for orient='{orient***REMOVED***'"
        with pytest.raises(ValueError, match=msg):
            df.to_json(orient=orient)
    ...
    def test_roundtrip_categorical(
        ...
        if orient in ("index", "columns"):
            request.applymarker(
                pytest.mark.xfail(
                    reason=f"Can't have duplicate index values for orient '{orient***REMOVED***')"
    ***REMOVED***
***REMOVED***
    ...
    @pytest.mark.parametrize(
        "data,msg,orient",
        [
            ('{"key":b:a:d***REMOVED***', "Expected object or value", "columns"),
            # too few indices
            (
                '{"columns":["A","B"],'
                '"index":["2","3"],'
                '"data":[[1.0,"1"],[2.0,"2"],[null,"3"]]***REMOVED***',
                "|".join(
                    [
...
***REMOVED***,
    ...
    def test_frame_to_json_float_precision(self, value, precision, expected_val):
        df = DataFrame([{"a_float": value***REMOVED***])
        encoded = df.to_json(double_precision=precision)
        assert encoded == f'{{"a_float":{{"0":{expected_val***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***'
    ...
    def test_frame_empty_to_json(self):
        # GH 7445
        df = DataFrame({"test": []***REMOVED***, index=[])
        result = df.to_json(orient="columns")
        expected = '{"test":{***REMOVED******REMOVED***'
    ...
    def test_frame_nonprintable_bytes(self):
        ...
        df_printable = DataFrame({"A": [binthing.hexed]***REMOVED***)
        assert df_printable.to_json() == f'{{"A":{{"0":"{hexed***REMOVED***"***REMOVED******REMOVED******REMOVED******REMOVED***'

        # check if non-printable content throws appropriate Exception
        df_nonprintable = DataFrame({"A": [binthing]***REMOVED***)
        msg = "Unsupported UTF-8 sequence length when encoding string"
        with pytest.raises(OverflowError, match=msg):
            df_nonprintable.to_json()

        # the same with multiple columns threw segfaults
        df_mixed = DataFrame({"A": [binthing], "B": [1]***REMOVED***, columns=["A", "B"])
        with pytest.raises(OverflowError, match=msg):
            df_mixed.to_json()

        # default_handler should resolve exceptions for non-string types
        result = df_nonprintable.to_json(default_handler=str)
        expected = f'{{"A":{{"0":"{hexed***REMOVED***"***REMOVED******REMOVED******REMOVED******REMOVED***'
        assert result == expected
        assert (
            df_mixed.to_json(default_handler=str)
            == f'{{"A":{{"0":"{hexed***REMOVED***"***REMOVED******REMOVED***,"B":{{"0":1***REMOVED******REMOVED******REMOVED******REMOVED***'
        )

    def test_label_overflow(self):
        # GH14256: buffer length not checked when writing label
        result = DataFrame({"bar" * 100000: [1], "foo": [1337]***REMOVED***).to_json()
        expected = f'{{"{"bar" * 100000***REMOVED***":{{"0":1***REMOVED******REMOVED***,"foo":{{"0":1337***REMOVED******REMOVED******REMOVED******REMOVED***'
        assert result == expected
...
***REMOVED***,
    ...
    def test_reconstruction_index(self):
        ...
        df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]***REMOVED***, index=["A", "B", "C"])
        result = read_json(StringIO(df.to_json()))
        tm.assert_frame_equal(result, df)
    ...
    def test_date_index_and_values(self, date_format, as_object, date_typ):
        ...
        if date_format == "epoch":
            expected = '{"1577836800000":1577836800000,"null":null***REMOVED***'
        else:
            expected = (
                '{"2020-01-01T00:00:00.000":"2020-01-01T00:00:00.000","null":null***REMOVED***'
***REMOVED***

        if as_object:
            expected = expected.replace("***REMOVED***", ',"a":"a"***REMOVED***')
    ...
    def test_convert_dates_infer(self, infer_word):
        ...
        data = [{"id": 1, infer_word: 1036713600000***REMOVED***, {"id": 2***REMOVED***]
        expected = DataFrame(
    ...
    ***REMOVED***,
    ...
    def test_doc_example(self):
        ...
        result = read_json(json, dtype={"ints": np.int64, "bools": np.bool_***REMOVED***)
    ...
    def test_url(self, field, dtype, httpserver):
        data = '{"created_at": ["2023-06-23T18:21:36Z"], "closed_at": ["2023-06-23T18:21:36"], "updated_at": ["2023-06-23T18:21:36Z"]***REMOVED***\n'  # noqa: E501
    ...
    def test_mixed_timedelta_datetime(self):
        ...
        frame = DataFrame({"a": [td, ts]***REMOVED***, dtype=object)

        expected = DataFrame(
            {"a": [pd.Timedelta(td).as_unit("ns")._value, ts.as_unit("ns")._value]***REMOVED***
        )
        data = StringIO(frame.to_json(date_unit="ns"))
        result = read_json(data, dtype={"a": "int64"***REMOVED***)
    ...
    def test_timedelta_to_json(self, as_object, date_format, timedelta_typ):
        ...
        if date_format == "iso":
            expected = (
                '{"P1DT0H0M0S":"P1DT0H0M0S","P2DT0H0M0S":"P2DT0H0M0S","null":null***REMOVED***'
***REMOVED***
        else:
            expected = '{"86400000":86400000,"172800000":172800000,"null":null***REMOVED***'
    ...
    def test_timedelta_to_json_fractional_precision(self, as_object, timedelta_typ):
        ...
        result = ser.to_json()
        expected = '{"42":42***REMOVED***'
        assert result == expected

    def test_default_handler(self):
        value = object()
        frame = DataFrame({"a": [7, value]***REMOVED***)
        expected = DataFrame({"a": [7, str(value)]***REMOVED***)
    ...
    def test_default_handler_indirect(self):
        ...
        df_list = [
            9,
            DataFrame(
                {"a": [1, "STR", complex(4, -5)], "b": [float("nan"), None, "N/A"]***REMOVED***,
                columns=["a", "b"],
***REMOVED***,
        ]
        expected = (
    ...
    def test_default_handler_numpy_unsupported_dtype(self):
        ...
        df = DataFrame(
            {"a": [1, 2.3, complex(4, -5)], "b": [float("nan"), None, complex(1.2, 0)]***REMOVED***,
            columns=["a", "b"],
        )
        expected = (
            '[["(1+0j)","(nan+0j)"],'
    ...
    def test_default_handler_raises(self):
        ...
        with pytest.raises(TypeError, match=msg):
            DataFrame({"a": [1, 2, object()]***REMOVED***).to_json(
                default_handler=my_handler_raises
***REMOVED***
        with pytest.raises(TypeError, match=msg):
            DataFrame({"a": [1, 2, complex(4, -5)]***REMOVED***).to_json(
                default_handler=my_handler_raises
***REMOVED***

    def test_categorical(self):
        # GH4377 df.to_json segfaults with non-ndarray blocks
        df = DataFrame({"A": ["a", "b", "c", "a", "b", "b", "a"]***REMOVED***)
        df["B"] = df["A"]
        expected = df.to_json()
    ...
    def test_datetime_tz(self):
        ...
        df = DataFrame({"A": tz_range, "B": date_range("20130101", periods=3)***REMOVED***)
    ...
    def test_tz_range_is_utc(self, tz_range):
        ...
        dfexp = (
            ...
            '"1":"2013-01-02T05:00:00.000Z"***REMOVED******REMOVED***'
        )
        ...
        df = DataFrame({"DT": dti***REMOVED***)
        result = ujson_dumps(df, iso_dates=True)
        assert result == dfexp
        assert ujson_dumps(df.astype({"DT": object***REMOVED***), iso_dates=True)
    ...
    def test_tz_range_is_naive(self):
        ...
        dfexp = '{"DT":{"0":"2013-01-01T05:00:00.000","1":"2013-01-02T05:00:00.000"***REMOVED******REMOVED***'
    ...
    def test_read_inline_jsonl(self):
        ...
        result = read_json(StringIO('{"a": 1, "b": 2***REMOVED***\n{"b":2, "a" :1***REMOVED***\n'), lines=True)
        expected = DataFrame([[1, 2], [1, 2]], columns=["a", "b"])
        tm.assert_frame_equal(result, expected)
    ...
    def test_read_s3_jsonl(self, s3_public_bucket_with_data, s3so):
        ...
        result = read_json(
            f"s3n://{s3_public_bucket_with_data.name***REMOVED***/items.jsonl",
            lines=True,
    ...
    def test_read_jsonl_unicode_chars(self):
        ...
        # simulate string
        json = StringIO('{"a": "foo”", "b": "bar"***REMOVED***\n{"a": "foo", "b": "bar"***REMOVED***\n')
        result = read_json(json, lines=True)
        expected = DataFrame([["foo\u201d", "bar"], ["foo", "bar"]], columns=["a", "b"])
        tm.assert_frame_equal(result, expected)
    ...
    def test_to_json_large_numbers(self, bigNum):
        ...
        json = series.to_json()
        expected = '{"articleId":' + str(bigNum) + "***REMOVED***"
        assert json == expected

        df = DataFrame(bigNum, dtype=object, index=["articleId"], columns=[0])
        json = df.to_json()
        expected = '{"0":{"articleId":' + str(bigNum) + "***REMOVED******REMOVED***"
        assert json == expected

    @pytest.mark.parametrize("bigNum", [-(2**63) - 1, 2**64])
    def test_read_json_large_numbers(self, bigNum):
        # GH20599, 26068
        json = StringIO('{"articleId":' + str(bigNum) + "***REMOVED***")
        msg = r"Value is too small|Value is too big"
        with pytest.raises(ValueError, match=msg):
            read_json(json)

        json = StringIO('{"0":{"articleId":' + str(bigNum) + "***REMOVED******REMOVED***")
        with pytest.raises(ValueError, match=msg):
            read_json(json)

    def test_read_json_large_numbers2(self):
        # GH18842
        json = '{"articleId": "1404366058080022500245"***REMOVED***'
        json = StringIO(json)
        result = read_json(json, typ="series")
        expected = Series(1.404366e21, index=["articleId"])
        tm.assert_series_equal(result, expected)

        json = '{"0": {"articleId": "1404366058080022500245"***REMOVED******REMOVED***'
        json = StringIO(json)
        result = read_json(json)
    ...
    def test_to_jsonl(self):
        ...
        expected = '{"a":1,"b":2***REMOVED***\n{"a":1,"b":2***REMOVED***\n'
        assert result == expected

        df = DataFrame([["foo***REMOVED***", "bar"], ['foo"', "bar"]], columns=["a", "b"])
        result = df.to_json(orient="records", lines=True)
        expected = '{"a":"foo***REMOVED***","b":"bar"***REMOVED***\n{"a":"foo\\"","b":"bar"***REMOVED***\n'
        assert result == expected
    ...
    def test_from_json_to_json_table_dtypes(self):
        # GH21345
        expected = DataFrame({"a": [1, 2], "b": [3.0, 4.0], "c": ["5", "6"]***REMOVED***)
        dfjson = expected.to_json(orient="table")
        result = read_json(StringIO(dfjson), orient="table")
        tm.assert_frame_equal(result, expected)
    ...
    ***REMOVED***,
    ...
    @pytest.mark.parametrize("dtype", [True, {"b": int, "c": int***REMOVED***])
    def test_read_json_table_dtype_raises(self, dtype):
        # GH21345
        df = DataFrame({"a": [1, 2], "b": [3.0, 4.0], "c": ["5", "6"]***REMOVED***)
    ...
    def test_read_json_table_empty_axes_dtype(self, orient):
        ...
        expected = DataFrame()
        result = read_json(StringIO("{***REMOVED***"), orient=orient, convert_axes=True)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                {"columns": ["a", "b"], "data": [[1, 2], [4, 5]]***REMOVED***,
***REMOVED***,
            (
...
***REMOVED***,
    ...
    ***REMOVED***,
                {"columns": ["a", "b"], "data": [[1, 2], [4, 5]]***REMOVED***,
***REMOVED***,
            (Series([1, 2, 3], name="A"), {"name": "A", "data": [1, 2, 3]***REMOVED***),
            (
                Series([1, 2, 3], name="A").rename_axis("foo"),
                {"name": "A", "data": [1, 2, 3]***REMOVED***,
***REMOVED***,
            (
                Series([1, 2], name="A", index=[["a", "b"], ["c", "d"]]),
                {"name": "A", "data": [1, 2]***REMOVED***,
***REMOVED***,
        ],
    )
...
***REMOVED***,
    ...
    def test_read_timezone_information(self):
        # GH 25546
        result = read_json(
            StringIO('{"2019-01-01T11:00:00.000Z":88***REMOVED***'), typ="series", orient="index"
        )
    ...
    def test_read_json_with_url_value(self, url):
        # GH 36271
        result = read_json(StringIO(f'{{"url":{{"0":"{url***REMOVED***"***REMOVED******REMOVED******REMOVED******REMOVED***'))
        expected = DataFrame({"url": [url]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_read_json_with_very_long_file_path(self, compression):
        # GH 46718
        long_json_path = f'{"a" * 1000***REMOVED***.json{compression***REMOVED***'
        with pytest.raises(
            FileNotFoundError, match=f"File {long_json_path***REMOVED*** does not exist"
        ):
    ...
    def test_timedelta_as_label(self, date_format, key):
        ...
        expected = f'{{"{key***REMOVED***":{{"0":1***REMOVED******REMOVED******REMOVED******REMOVED***'
    ...
    @pytest.mark.parametrize(
        "orient,expected",
        [
            ("index", "{\"('a', 'b')\":{\"('c', 'd')\":1***REMOVED******REMOVED***"),
            ("columns", "{\"('c', 'd')\":{\"('a', 'b')\":1***REMOVED******REMOVED***"),
...
***REMOVED***,
    ...
    def test_to_json_indent(self, indent):
        ...
        expected = f"""{{
{spaces***REMOVED***"a":{{
{spaces***REMOVED***{spaces***REMOVED***"0":"foo",
{spaces***REMOVED***{spaces***REMOVED***"1":"baz"
{spaces***REMOVED******REMOVED******REMOVED***,
{spaces***REMOVED***"b":{{
{spaces***REMOVED***{spaces***REMOVED***"0":"bar",
{spaces***REMOVED***{spaces***REMOVED***"1":"qux"
{spaces***REMOVED******REMOVED******REMOVED***
***REMOVED******REMOVED***"""
...
***REMOVED***,
    ...
    def test_frame_int_overflow(self):
        # GH 30320
        encoded_json = json.dumps([{"col": "31900441201190696999"***REMOVED***, {"col": "Text"***REMOVED***])
        expected = DataFrame({"col": ["31900441201190696999", "Text"]***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                DataFrame({"x": [1, 2, 3], "y": ["a", "b", "c"]***REMOVED***),
                '{"(0, \'x\')":1,"(0, \'y\')":"a","(1, \'x\')":2,'
                '"(1, \'y\')":"b","(2, \'x\')":3,"(2, \'y\')":"c"***REMOVED***',
***REMOVED***
        ],
    )
    ...
    def test_to_s3(self, s3_public_bucket, s3so):
        ...
        df = DataFrame({"x": [1, 2, 3], "y": [2, 4, 6]***REMOVED***)
        df.to_json(f"s3://{mock_bucket_name***REMOVED***/{target_file***REMOVED***", storage_options=s3so)
        timeout = 5
        while True:
    ...
    def test_json_pandas_nulls(self, nulls_fixture, request):
        ...
        assert result == '{"0":{"0":null***REMOVED******REMOVED***'
    ...
    def test_to_json_multiindex_escape(self):
        ...
        expected = (
            ...
            "\"(Timestamp('2017-01-23 00:00:00'), 'bar')\":true***REMOVED***"
        )
        assert result == expected
    ...
    def test_to_json_series_of_objects(self):
        ...
        assert json.loads(series.to_json()) == {"0": {"a": 1, "b": 2, "d": 4***REMOVED******REMOVED***

    @pytest.mark.parametrize(
        "data,expected",
        [
            (
                Series({0: -6 + 8j, 1: 0 + 1j, 2: 9 - 5j***REMOVED***),
                '{"0":{"imag":8.0,"real":-6.0***REMOVED***,'
                '"1":{"imag":1.0,"real":0.0***REMOVED***,'
                '"2":{"imag":-5.0,"real":9.0***REMOVED******REMOVED***',
***REMOVED***,
            (
                Series({0: -9.39 + 0.66j, 1: 3.95 + 9.32j, 2: 4.03 - 0.17j***REMOVED***),
                '{"0":{"imag":0.66,"real":-9.39***REMOVED***,'
                '"1":{"imag":9.32,"real":3.95***REMOVED***,'
                '"2":{"imag":-0.17,"real":4.03***REMOVED******REMOVED***',
***REMOVED***,
            (
                DataFrame([[-2 + 3j, -1 - 0j], [4 - 3j, -0 - 10j]]),
                '{"0":{"0":{"imag":3.0,"real":-2.0***REMOVED***,'
                '"1":{"imag":-3.0,"real":4.0***REMOVED******REMOVED***,'
                '"1":{"0":{"imag":0.0,"real":-1.0***REMOVED***,'
                '"1":{"imag":-10.0,"real":0.0***REMOVED******REMOVED******REMOVED***',
***REMOVED***,
            (
                DataFrame(
                    [[-0.28 + 0.34j, -1.08 - 0.39j], [0.41 - 0.34j, -0.78 - 1.35j]]
    ***REMOVED***,
                '{"0":{"0":{"imag":0.34,"real":-0.28***REMOVED***,'
                '"1":{"imag":-0.34,"real":0.41***REMOVED******REMOVED***,'
                '"1":{"0":{"imag":-0.39,"real":-1.08***REMOVED***,'
                '"1":{"imag":-1.35,"real":-0.78***REMOVED******REMOVED******REMOVED***',
***REMOVED***,
        ],
    )
    ...
    def test_json_uint64(self):
        ...
        expected = (
            ...
            '"data":[[13342205958987758245],[12388075603347835679]]***REMOVED***'
        )
        df = DataFrame(data={"col1": [13342205958987758245, 12388075603347835679]***REMOVED***)
