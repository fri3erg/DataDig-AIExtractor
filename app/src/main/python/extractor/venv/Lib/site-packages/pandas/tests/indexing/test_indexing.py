...
***REMOVED***
    ...
    def test_setitem_dtype_upcast(self):
        # GH3216
        df = DataFrame([{"a": 1***REMOVED***, {"a": 3, "b": 2***REMOVED***])
        df["c"] = np.nan
        ...
        with tm.assert_produces_warning(
            ...
            df.loc[0, "c"] = "foo"
        expected = DataFrame(
            {"a": [1, 3], "b": [np.nan, 2], "c": Series(["foo", np.nan], dtype=object)***REMOVED***
        )
    ...
    def test_dups_fancy_indexing_not_in_order(self):
        ...
        df = DataFrame(
            {"test": [5, 7, 9, 11], "test1": [4.0, 5, 6, 7], "other": list("abcd")***REMOVED***,
            index=["A", "A", "B", "C"],
        )
        rows = ["C", "B"]
        expected = DataFrame(
            {"test": [11, 9], "test1": [7.0, 6], "other": ["d", "c"]***REMOVED***, index=rows
        )
        result = df.loc[rows]
...
***REMOVED***:
    ...
    def test_dups_fancy_indexing_missing_label(self, vals):
        ...
        df = DataFrame({"A": vals***REMOVED***)
        with pytest.raises(KeyError, match="not in index"):
            df.loc[[0, 8, 0]]

    def test_dups_fancy_indexing_non_unique(self):
        # non unique with non unique selector
        df = DataFrame({"test": [5, 7, 9, 11]***REMOVED***, index=["A", "A", "B", "C"])
        with pytest.raises(KeyError, match="not in index"):
            df.loc[["A", "A", "E"]]
    ...
    def test_indexing_mixed_frame_bug(self):
        # GH3492
        df = DataFrame(
            {"a": {1: "aaa", 2: "bbb", 3: "ccc"***REMOVED***, "b": {1: 111, 2: 222, 3: 333***REMOVED******REMOVED***
        )
...
class TestMisc:
    def test_float_index_to_mixed(self):
        ...
        expected = DataFrame({0.0: df[0.0], 1.0: df[1.0], "a": [10] * 10***REMOVED***)
        tm.assert_frame_equal(expected, df)

    def test_float_index_non_scalar_assignment(self):
        df = DataFrame({"a": [1, 2, 3], "b": [3, 4, 5]***REMOVED***, index=[1.0, 2.0, 3.0])
        df.loc[df.index[:2]] = 1
        expected = DataFrame({"a": [1, 1, 3], "b": [1, 1, 5]***REMOVED***, index=df.index)
        tm.assert_frame_equal(expected, df)
    ...
    def test_rhs_alignment(self):
        ...
        for frame in [df, rhs, right_loc, right_iloc]:
            ...
            frame["jolie"] = frame["jolie"].map(lambda x: f"@{x***REMOVED***")
    ...
    def test_loc_setitem_indexing_assignment_dict_already_exists(self):
        ...
        df = DataFrame({"x": [1, 2, 6], "y": [2, 2, 8]***REMOVED***, index=index)
        expected = df.copy()
        ...
        df = DataFrame({"x": [1, 2, 6], "y": [2.0, 2.0, 8.0]***REMOVED***, index=index)
        df.loc[5] = rhs
        expected = DataFrame({"x": [1, 2, 9], "y": [2.0, 2.0, 99.0]***REMOVED***, index=index)
        tm.assert_frame_equal(df, expected)

    def test_iloc_getitem_indexing_dtypes_on_empty(self):
        # Check that .iloc returns correct dtypes GH9983
        df = DataFrame({"a": [1, 2, 3], "b": ["b", "b2", "b3"]***REMOVED***)
        df2 = df.iloc[[], :]
    ...
    def test_no_reference_cycle(self):
        df = DataFrame({"a": [0, 1], "b": [2, 3]***REMOVED***)
    ...
    def test_label_indexing_on_nan(self, nulls_fixture):
        # GH 32431
        df = Series([1, "{1,2***REMOVED***", 1, nulls_fixture])
...
class TestDataframeNoneCoercion:
    ...
    def test_coercion_with_loc(self, expected):
        ...
        start_dataframe = DataFrame({"foo": start_data***REMOVED***)
        start_dataframe.loc[0, ["foo"]] = None

        expected_dataframe = DataFrame({"foo": expected_result***REMOVED***)
...
def test_extension_array_cross_section_converts():
    ...
    df = DataFrame(
        {"A": pd.array([1, 2], dtype="Int64"), "B": np.array(["a", "b"])***REMOVED***,
        index=["a", "b"],
    )
    result = df.loc["a"]
...
def test_scalar_setitem_with_nested_value(value):
    ...
    df = DataFrame({"A": [1, 2, 3], "B": np.array([1, "a", "b"], dtype=object)***REMOVED***)
...
def test_scalar_setitem_with_nested_value_length1(value):
    ...
    df = DataFrame({"A": [1, 2, 3]***REMOVED***)
    df.loc[0, "B"] = value
    expected = DataFrame({"A": [1, 2, 3], "B": [0.0, np.nan, np.nan]***REMOVED***)
    tm.assert_frame_equal(df, expected)
