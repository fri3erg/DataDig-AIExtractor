...
def _DEFAULT_COMPOSER(op_name: str) -> Callable:
    ...
    def f(*args: Any) -> str:
        ...
        return f" {op_name***REMOVED*** ".join(args_)
...
def _FUNCTION_COMPOSER(op_name: str) -> Callable:
    ...
    def f(*args: Any) -> str:
        args_: map[str] = map(str, args)
        return f"{op_name***REMOVED***({','.join(args_)***REMOVED***)"
    ...
    ***REMOVED***
    ...
    def visit_comparison(self, comparison: Comparison) -> Dict:
        ...
        if matched:
            attr = re.sub(
                regex,
                f"({self.metadata_key***REMOVED***.{matched.group(0)[1:-1]***REMOVED***)",
                comparison.attribute,
***REMOVED***
        else:
            attr = f"{self.metadata_key***REMOVED***.{comparison.attribute***REMOVED***"
        value = comparison.value
        comp = comparison.comparator

        value = f"'{value***REMOVED***'" if isinstance(value, str) else value

        # convert timestamp for datetime objects
        if isinstance(value, dict) and value.get("type") == "date":
            attr = f"parseDateTime32BestEffort({attr***REMOVED***)"
            value = f"parseDateTime32BestEffort('{value['date']***REMOVED***')"

        # string pattern match
        if comp is Comparator.LIKE:
            value = f"'%{value[1:-1]***REMOVED***%'"
    ...
    def visit_structured_query(
        ...
        if structured_query.filter is None:
            ...
            kwargs = {"where_str": structured_query.filter.accept(self)***REMOVED***
