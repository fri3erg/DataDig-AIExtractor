...
***REMOVED***
    ...
    def norm_cdf(x):
        """Return the area under the normal distribution from M{-∞..x***REMOVED***."""
...
def align_log_prob(i, j, source_sents, target_sents, alignment, params):
    """Returns the log probability of the two sentences C{source_sents[i]***REMOVED***, C{target_sents[j]***REMOVED***
    being aligned with a specific C{alignment***REMOVED***.
...
def split_at(it, split_value):
    """Splits an iterator C{it***REMOVED*** at values of C{split_value***REMOVED***.

    Each instance of C{split_value***REMOVED*** is swallowed. The iterator produces
...
def parse_token_stream(stream, soft_delimiter, hard_delimiter):
    """Parses a stream of tokens and splits it into sentences (using C{soft_delimiter***REMOVED*** tokens)
    and blocks (using C{hard_delimiter***REMOVED*** tokens) for use with the L{align_texts***REMOVED*** function.
    """
    return [
        [
