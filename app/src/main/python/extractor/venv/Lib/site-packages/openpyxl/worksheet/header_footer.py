...
FONT_PATTERN = '&"(?P<font>.+)"'
COLOR_PATTERN  = "&K(?P<color>[A-F0-9]{6***REMOVED***)"
SIZE_REGEX = r"&(?P<size>\d+\s?)"
FORMAT_REGEX = re.compile("{0***REMOVED***|{1***REMOVED***|{2***REMOVED***".format(FONT_PATTERN, COLOR_PATTERN,
                                               SIZE_REGEX)
              ***REMOVED***
...
def _split_string(text):
    ...
    try:
        ...
        parts = {'left':'', 'right':'', 'center':''***REMOVED***
    return parts
...
class _HeaderFooterPart(Strict):
    ...
    * &nn   Selects the specified 2-digit font point size
    ...
    def __str__(self):
        ...
        fmt = []
        if self.font:
            fmt.append(u'&"{0***REMOVED***"'.format(self.font))
        if self.size:
            fmt.append("&{0***REMOVED*** ".format(self.size))
        if self.color:
            fmt.append("&K{0***REMOVED***".format(self.color))
...
class HeaderFooterItem(Strict):
    ...
    def __str__(self):
        ...
        SUBS_REGEX = re.compile("|".join(["({0***REMOVED***)".format(re.escape(k))
                                          for k in TRANSFORM]))
        ...
        for key, part in zip(
            ...
            if part.text is not None:
                txt.append(u"&{0***REMOVED***{1***REMOVED***".format(key, str(part)))
        txt = "".join(txt)
