...
***REMOVED***
        ...
        index_params = (
            ", " + ",".join([f"'{k***REMOVED***={v***REMOVED***'" for k, v in self.config.index_param.items()])
            if self.config.index_param
            else ""
        )
        schema_ = f"""
            CREATE TABLE IF NOT EXISTS {self.config.database***REMOVED***.{self.config.table***REMOVED***(
                {self.config.column_map['id']***REMOVED*** String,
                {self.config.column_map['text']***REMOVED*** String,
                {self.config.column_map['vector']***REMOVED*** Array(Float32),
                {self.config.column_map['metadata']***REMOVED*** JSON,
                CONSTRAINT cons_vec_len CHECK length(\
                    {self.config.column_map['vector']***REMOVED***) = {dim***REMOVED***,
                VECTOR INDEX vidx {self.config.column_map['vector']***REMOVED*** \
                    TYPE {self.config.index_type***REMOVED***(\
                        'metric_type={self.config.metric***REMOVED***'{index_params***REMOVED***)
***REMOVED*** ENGINE = MergeTree ORDER BY {self.config.column_map['id']***REMOVED***
        """
        self.dim = dim
        self.BS = "\\"
        self.must_escape = ("\\", "'")
    ...
    def escape_str(self, value: str) -> str:
        return "".join(f"{self.BS***REMOVED***{c***REMOVED***" if c in self.must_escape else c for c in value)

    def _build_istr(self, transac: Iterable, column_names: Iterable[str]) -> str:
        ks = ",".join(column_names)
        _data = []
        for n in transac:
            n = ",".join([f"'{self.escape_str(str(_n))***REMOVED***'" for _n in n])
            _data.append(f"({n***REMOVED***)")
        i_str = f"""
                INSERT INTO TABLE 
                    {self.config.database***REMOVED***.{self.config.table***REMOVED***({ks***REMOVED***)
                VALUES
                {','.join(_data)***REMOVED***
                """
        return i_str
    ...
    def add_texts(
        ...
        ***REMOVED***
        metadatas = metadatas or [{***REMOVED*** for _ in texts]
...
***REMOVED***:
            ...
            logger.error(f"\033[91m\033[1m{type(e)***REMOVED***\033[0m \033[95m{str(e)***REMOVED***\033[0m")
            return []
    ...
    def __repr__(self) -> str:
        ...
        """
        _repr = f"\033[92m\033[1m{self.config.database***REMOVED***.{self.config.table***REMOVED*** @ "
        _repr += f"{self.config.host***REMOVED***:{self.config.port***REMOVED***\033[0m\n\n"
        _repr += f"\033[1musername: {self.config.username***REMOVED***\033[0m\n\nTable Schema:\n"
        _repr += "-" * 51 + "\n"
        for r in self.client.query(
            f"DESC {self.config.database***REMOVED***.{self.config.table***REMOVED***"
        ).named_results():
            _repr += (
                f"|\033[94m{r['name']:24s***REMOVED***\033[0m|\033[96m{r['type']:24s***REMOVED***\033[0m|\n"
***REMOVED***
        _repr += "-" * 51 + "\n"
        return _repr
    ...
    def _build_qstr(
        ...
        if where_str:
            where_str = f"PREWHERE {where_str***REMOVED***"
        else:
            where_str = ""

        q_str = f"""
            SELECT {self.config.column_map['text']***REMOVED***, 
                {self.config.column_map['metadata']***REMOVED***, dist
            FROM {self.config.database***REMOVED***.{self.config.table***REMOVED***
            {where_str***REMOVED***
            ORDER BY distance({self.config.column_map['vector']***REMOVED***, [{q_emb_str***REMOVED***]) 
                AS dist {self.dist_order***REMOVED***
            LIMIT {topk***REMOVED***
            """
        return q_str
    ...
    def similarity_search(
        ...
        Args:
            ...
            NOTE: Please do not let end-user to fill this and always be aware
                  ...
                  use `{self.metadata_column***REMOVED***.attribute` instead of `attribute`
    ...
    ***REMOVED***
    ...
    def drop(self) -> None:
        ...
        """
        self.client.command(
            f"DROP TABLE IF EXISTS {self.config.database***REMOVED***.{self.config.table***REMOVED***"
        )
    ...
    def delete(
        ...
        **kwargs: Any,
        ...
        if ids and len(ids) > 0:
            id_list = ", ".join([f"'{id***REMOVED***'" for id in ids])
            conds.append(f"{self.config.column_map['id']***REMOVED*** IN ({id_list***REMOVED***)")
        if where_str:
            conds.append(where_str)
        assert len(conds) > 0
        where_str_final = " AND ".join(conds)
        qstr = (
            f"DELETE FROM {self.config.database***REMOVED***.{self.config.table***REMOVED*** "
            f"WHERE {where_str_final***REMOVED***"
        )
        try:
...
class MyScaleWithoutJSON(MyScale):
    ...
    def _build_qstr(
        ...
        q_str = f"""
            SELECT {self.config.column_map['text']***REMOVED***, dist, 
                {','.join(self.must_have_cols)***REMOVED***
            FROM {self.config.database***REMOVED***.{self.config.table***REMOVED***
            {where_str***REMOVED***
            ORDER BY distance({self.config.column_map['vector']***REMOVED***, [{q_emb_str***REMOVED***]) 
                AS dist {self.dist_order***REMOVED***
        ...
        **kwargs: Any,
        ...
        try:
            return [
                Document(
                    page_content=r[self.config.column_map["text"]],
                    metadata={k: r[k] for k in self.must_have_cols***REMOVED***,
    ***REMOVED***
                for r in self.client.query(q_str).named_results()
            ]
        except Exception as e:
