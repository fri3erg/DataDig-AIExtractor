...
***REMOVED***:
    ...
    ***REMOVED***
    ...
    if typ == "block":
        ...
        raise ValueError(f"'typ' needs to be one of {{'block', 'array'***REMOVED******REMOVED***, got '{typ***REMOVED***'")
...
def _ensure_2d(values: np.ndarray) -> np.ndarray:
    ...
    if values.ndim == 1:
        ...
        raise ValueError(f"Must pass 2-d input. shape={values.shape***REMOVED***")
    return values
...
***REMOVED***
        ...
        if have_series:
            if lengths[0] != len(index):
                msg = (
                    f"array length {lengths[0]***REMOVED*** does not match index "
                    f"length {len(index)***REMOVED***"
    ***REMOVED***
...
def _get_names_from_index(data) -> Index:
    ...
    for i, s in enumerate(data):
        ...
        if n is not None:
            ...
            index[i] = f"Unnamed {count***REMOVED***"
            count += 1
...
def dataclasses_to_dicts(data):
    ...
    [{'x': 1, 'y': 2***REMOVED***, {'x': 2, 'y': 3***REMOVED***]
...
def _list_of_series_to_arrays(
    ...
    indexer_cache: dict[int, np.ndarray] = {***REMOVED***
    ...
    if columns is None:
        ...
        if not is_mi_list and len(columns) != len(content):  # pragma: no cover
            ...
            raise AssertionError(
                f"{len(columns)***REMOVED*** columns passed, passed data had "
                f"{len(content)***REMOVED*** columns"
***REMOVED***
        if is_mi_list:
            # check if nested list column, length of each sub-list should be equal
            if len({len(col) for col in columns***REMOVED***) > 1:
                raise ValueError(
    ...
    ***REMOVED***
            ...
            if columns and len(columns[0]) != len(content):
                raise ValueError(
                    f"{len(columns[0])***REMOVED*** columns passed, passed data had "
                    f"{len(content)***REMOVED*** columns"
    ***REMOVED***
    return columns
