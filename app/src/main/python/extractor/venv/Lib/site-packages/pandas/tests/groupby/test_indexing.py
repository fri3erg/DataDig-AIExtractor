...
def multiindex_data():
    ...
    items = [f"item {i***REMOVED***" for i in range(nitems)]
...
def _make_df_from_data(data):
    ...
    for date in data:
        for level in data[date]:
            rows[(date, level[0])] = {"A": level[1], "B": level[2]***REMOVED***
...
def test_multiindex(multiindex_data):
    ...
    sliced = {date: multiindex_data[date][3:-3] for date in multiindex_data***REMOVED***
...
def test_against_head_and_tail(arg, method, simulated):
    ...
    data = {
        "group": [
            f"group {g***REMOVED***" for j in range(n_rows_per_group) for g in range(n_groups)
        ],
        "value": [
            f"group {g***REMOVED*** row {j***REMOVED***"
...
def test_step(step):
    ...
    data = [["x", f"x{i***REMOVED***"] for i in range(5)]
    data += [["y", f"y{i***REMOVED***"] for i in range(4)]
    data += [["z", f"z{i***REMOVED***"] for i in range(3)]
    ...
    data = [["x", f"x{i***REMOVED***"] for i in range(0, 5, step)]
    data += [["y", f"y{i***REMOVED***"] for i in range(0, 4, step)]
    data += [["z", f"z{i***REMOVED***"] for i in range(0, 3, step)]
...
def test_columns_on_iter():
    # GitHub issue #44821
    df = pd.DataFrame({k: range(10) for k in "ABC"***REMOVED***)
