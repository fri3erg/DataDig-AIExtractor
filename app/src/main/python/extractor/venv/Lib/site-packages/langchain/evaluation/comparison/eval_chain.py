...
def resolve_pairwise_criteria(
    ...
    if criteria is None:
        ...
        return {k.value: _SUPPORTED_CRITERIA[k] for k in _default_criteria***REMOVED***
    elif isinstance(criteria, Criteria):
        criteria_ = {criteria.value: _SUPPORTED_CRITERIA[criteria]***REMOVED***
    elif isinstance(criteria, str):
        if criteria in _SUPPORTED_CRITERIA:
            criteria_ = {criteria: _SUPPORTED_CRITERIA[Criteria(criteria)]***REMOVED***
        else:
            criteria_ = {criteria: ""***REMOVED***
    elif isinstance(criteria, ConstitutionalPrinciple):
        criteria_ = {criteria.name: criteria.critique_request***REMOVED***
...
class PairwiseStringResultOutputParser(BaseOutputParser[dict]):
    ...
    def parse(self, text: str) -> Dict[str, Any]:
        ...
        if not match or verdict not in {"A", "B", "C"***REMOVED***:
            raise ValueError(
                f"Invalid output: {text***REMOVED***. "
...
class PairwiseStringEvalChain(PairwiseStringEvaluator, LLMEvalChain, LLMChain):
    ...
    Example:
        ...
        >>> llm = ChatOpenAI(temperature=0, model_name="gpt-4", model_kwargs={"random_seed": 42***REMOVED***)
    ...
    def _skip_reference_warning(self) -> str:
        ...
        """
        return (
            f"Ignoring reference in {self.__class__.__name__***REMOVED***, as it is not expected."
...
***REMOVED***
        ...
        expected_input_vars = {"prediction", "prediction_b", "input", "criteria"***REMOVED***
        prompt_ = prompt or COMPARISON_TEMPLATE.partial(reference="")
        if expected_input_vars != set(prompt_.input_variables):
            raise ValueError(
                f"Input variables should be {expected_input_vars***REMOVED***, "
                f"but got {prompt_.input_variables***REMOVED***"
***REMOVED***
        criteria_ = resolve_pairwise_criteria(criteria)
        criteria_str = "\n".join(f"{k***REMOVED***: {v***REMOVED***" if v else k for k, v in criteria_.items())
...
***REMOVED***
        criteria_ = resolve_pairwise_criteria(criteria)
        criteria_str = "\n".join(f"{k***REMOVED***: {v***REMOVED***" for k, v in criteria_.items())
        criteria_str = CRITERIA_INSTRUCTIONS + criteria_str if criteria_str else ""
        return cls(llm=llm, prompt=prompt_.partial(criteria=criteria_str), **kwargs)
