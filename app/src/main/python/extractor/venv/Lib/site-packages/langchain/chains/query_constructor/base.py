    ...
    def parse(self, text: str) -> StructuredQuery:
        try:
            ...
            return StructuredQuery(
                **{k: v for k, v in parsed.items() if k in allowed_keys***REMOVED***
***REMOVED***
...
def _format_attribute_info(info: Sequence[Union[AttributeInfo, dict]]) -> str:
    ...
    return json.dumps(info_dicts, indent=4).replace("{", "{{").replace("***REMOVED***", "***REMOVED******REMOVED***")
...
def construct_examples(input_output_pairs: Sequence[Tuple[str, dict]]) -> List[dict]:
    ...
    for i, (_input, output) in enumerate(input_output_pairs):
        structured_request = (
            json.dumps(output, indent=4).replace("{", "{{").replace("***REMOVED***", "***REMOVED******REMOVED***")
        )
        example = {
            "i": i + 1,
            "user_query": _input,
