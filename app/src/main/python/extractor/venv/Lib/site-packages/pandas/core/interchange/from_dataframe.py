...
_NP_DTYPES: dict[DtypeKind, dict[int, Any]] = {
    DtypeKind.INT: {8: np.int8, 16: np.int16, 32: np.int32, 64: np.int64***REMOVED***,
    DtypeKind.UINT: {8: np.uint8, 16: np.uint16, 32: np.uint32, 64: np.uint64***REMOVED***,
    DtypeKind.FLOAT: {32: np.float32, 64: np.float64***REMOVED***,
    DtypeKind.BOOL: {1: bool, 8: bool***REMOVED***,
***REMOVED***
...
def protocol_df_chunk_to_pandas(df: DataFrameXchg) -> pd.DataFrame:
    ...
    columns: dict[str, Any] = {***REMOVED***
    buffers = []  # hold on to buffers, keeps memory alive
    for name in df.column_names():
        if not isinstance(name, str):
            raise ValueError(f"Column {name***REMOVED*** is not a string")
        if name in columns:
            raise ValueError(f"Column {name***REMOVED*** is not unique")
        ...
        if dtype in (
            ...
            raise NotImplementedError(f"Data type {dtype***REMOVED*** not handled yet")
...
def string_column_to_ndarray(col: Column) -> tuple[np.ndarray, Any]:
    ...
    if null_kind not in (
        ...
        raise NotImplementedError(
            f"{null_kind***REMOVED*** null kind is not yet supported for string columns."
        )
...
def parse_datetime_format_str(format_str, data) -> pd.Series | np.ndarray:
    ...
    if timestamp_meta:
        ...
        if unit != "s":
            ...
            # 'm' -> 'ms', 'u' -> 'us', 'n' -> 'ns'
            unit += "s"
        data = data.astype(f"datetime64[{unit***REMOVED***]")
        if tz != "":
            data = pd.Series(data).dt.tz_localize("UTC").dt.tz_convert(tz)
        return data

    # date 'td{Days/Ms***REMOVED***'
    ...
    if date_meta:
        ...
        if unit == "D":
            ...
            raise NotImplementedError(f"Date unit is not supported: {unit***REMOVED***")
        return data

    raise NotImplementedError(f"DateTime kind is not supported: {format_str***REMOVED***")
...
def datetime_column_to_ndarray(col: Column) -> tuple[np.ndarray | pd.Series, Any]:
    ...
    data = buffer_to_ndarray(
        ...
        (
            DtypeKind.INT,
            col_bit_width,
            getattr(ArrowCTypes, f"INT{col_bit_width***REMOVED***"),
            Endianness.NATIVE,
        ),
        offset=col.offset,
        length=col.size(),
...
def buffer_to_ndarray(
    ...
    *,
    ...
    column_dtype = _NP_DTYPES.get(kind, {***REMOVED***).get(bit_width, None)
    if column_dtype is None:
        raise NotImplementedError(f"Conversion for {dtype***REMOVED*** is not yet supported.")
...
def set_nulls(
    ...
    if null_kind == ColumnNullType.USE_SENTINEL:
        ...
        raise NotImplementedError(f"Null kind {null_kind***REMOVED*** is not yet supported.")
