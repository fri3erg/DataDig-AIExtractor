...
Each call to ``parse()`` returns a dict has this form::
    ...
    {
      "ResponseMetadata": {"RequestId": <requestid>***REMOVED***
      <response keys>
    ***REMOVED***
...
***REMOVED***
    ...
    def parse(self, response, shape):
        ...
        if isinstance(parsed, dict):
            response_metadata = parsed.get('ResponseMetadata', {***REMOVED***)
    ...
    def _add_checksum_response_metadata(self, response, response_metadata):
        checksum_context = response.get('context', {***REMOVED***).get('checksum', {***REMOVED***)
    ...
    ***REMOVED***,
    ...
    def _do_error_parse(self, response, shape):
        raise NotImplementedError(f"{self.__class__.__name__***REMOVED***._do_error_parse")

    def _do_modeled_error_parse(self, response, shape, parsed):
        raise NotImplementedError(
            f"{self.__class__.__name__***REMOVED***._do_modeled_error_parse"
        )

    def _parse_shape(self, shape, node):
        handler = getattr(
            self, f'_handle_{shape.type_name***REMOVED***', self._default_handle
        )
    ...
    ***REMOVED***
    ...
    def _handle_unknown_tagged_union_member(self, tag):
        return {'SDK_UNKNOWN_MEMBER': {'name': tag***REMOVED******REMOVED***


class BaseXMLResponseParser(ResponseParser):
    def __init__(self, timestamp_parser=None, blob_parser=None):
        super().__init__(timestamp_parser, blob_parser)
        self._namespace_re = re.compile('{.****REMOVED***')
    ...
    def _build_name_to_xml_node(self, parent_node):
        ...
        for item in parent_node:
            ...
            if key in xml_dict:
                ...
                # <foo>1</foo><foo>2</foo> -> {'foo': [Node(1), Node(2)]***REMOVED***
...
class QueryParser(BaseXMLResponseParser):
    def _do_error_parse(self, response, shape):
        ...
        # 1. {"Errors": {"Error": {...***REMOVED******REMOVED******REMOVED*** -> {"Error": {...***REMOVED******REMOVED***
        # 2. {"RequestId": "id"***REMOVED*** -> {"ResponseMetadata": {"RequestId": "id"***REMOVED******REMOVED***
        if 'Errors' in parsed:
            parsed.update(parsed.pop('Errors'))
        if 'RequestId' in parsed:
            parsed['ResponseMetadata'] = {'RequestId': parsed.pop('RequestId')***REMOVED***
        return parsed
...
class EC2QueryParser(QueryParser):
    def _inject_response_metadata(self, node, inject_into):
        ...
        if child_node is not None:
            inject_into['ResponseMetadata'] = {'RequestId': child_node.text***REMOVED***
...
class BaseJSONParser(ResponseParser):
    ...
    def _do_error_parse(self, response, shape):
        ...
        error = {"Error": {"Message": '', "Code": ''***REMOVED***, "ResponseMetadata": {***REMOVED******REMOVED***
        headers = response['headers']
        # Error responses can have slightly different structures for json.
        # The basic structure is:
        #
        # {"__type":"ConnectClientException",
        #  "message":"The error message."***REMOVED***
    ...
    ***REMOVED***
    ...
    def _inject_response_metadata(self, parsed, headers):
        if 'x-amzn-requestid' in headers:
            parsed.setdefault('ResponseMetadata', {***REMOVED***)['RequestId'] = headers[
                'x-amzn-requestid'
            ]
    ...
    ***REMOVED***,
    ...
    def _parse_error_from_body(self, response):
        ...
        if root.tag == 'Error':
            ...
            parsed.pop('HostId', '')
            return {'Error': parsed, 'ResponseMetadata': metadata***REMOVED***
        elif 'RequestId' in parsed:
            # Other rest-xml services:
            parsed['ResponseMetadata'] = {'RequestId': parsed.pop('RequestId')***REMOVED***
        default = {'Error': {'Message': '', 'Code': ''***REMOVED******REMOVED***
