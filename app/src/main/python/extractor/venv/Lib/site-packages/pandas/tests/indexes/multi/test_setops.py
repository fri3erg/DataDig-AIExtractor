...
def test_union_duplicates(index, request):
    ...
    if index.empty or isinstance(index, (IntervalIndex, CategoricalIndex)):
        pytest.skip(f"No duplicates in an empty {type(index).__name__***REMOVED***")
...
def test_intersection_with_non_lex_sorted_categories(a, b):
    ...
    df1 = DataFrame({"x": a, "y": other***REMOVED***)
    df2 = DataFrame({"x": b, "y": other***REMOVED***)
...
def test_union_with_na_when_constructing_dataframe():
    ...
    expected = DataFrame({(np.nan, np.nan): [1.0, 10.0], ("a", "b"): [np.nan, 20.0]***REMOVED***)
