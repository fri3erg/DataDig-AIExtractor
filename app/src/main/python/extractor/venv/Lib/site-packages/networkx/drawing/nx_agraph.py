...
def from_agraph(A, create_using=None):
    ...
    for n in A.nodes():
        str_attr = {str(k): v for k, v in n.attr.items()***REMOVED***
    ...
    for e in A.edges():
        u, v = str(e[0]), str(e[1])
        attr = dict(e.attr)
        str_attr = {str(k): v for k, v in attr.items()***REMOVED***
...
def to_agraph(N):
    ...
    for node in N:
        if "pos" in N.nodes[node]:
            N.nodes[node]["pos"] = "{***REMOVED***,{***REMOVED***!".format(
...
***REMOVED***
    ...
    A.graph_attr.update(N.graph.get("graph", {***REMOVED***))
    A.node_attr.update(N.graph.get("node", {***REMOVED***))
    A.edge_attr.update(N.graph.get("edge", {***REMOVED***))
    ...
    for n, nodedata in N.nodes(data=True):
        ...
        # Add node data
        a = A.get_node(n)
        a.attr.update({k: str(v) for k, v in nodedata.items()***REMOVED***)

    # loop over edges
    if N.is_multigraph():
        for u, v, key, edgedata in N.edges(data=True, keys=True):
            str_edgedata = {k: str(v) for k, v in edgedata.items() if k != "key"***REMOVED***
        ...
        for u, v, edgedata in N.edges(data=True):
            str_edgedata = {k: str(v) for k, v in edgedata.items()***REMOVED***
            A.add_edge(u, v)
            # Add edge data
            a = A.get_edge(u, v)
            a.attr.update(str_edgedata)
...
def pygraphviz_layout(G, prog="neato", root=None, args=""):
    ...
    for the layout computation using something similar to::
        ...
        >>> G_layout = {H.nodes[n]["node_label"]: p for n, p in H_layout.items()***REMOVED***
...
def view_pygraphviz(
    ...
    for attr in attrs:
        if attr not in G.graph:
            G.graph[attr] = {***REMOVED***

    # These are the default values.
    edge_attrs = {"fontsize": "10"***REMOVED***
    node_attrs = {
        "style": "filled",
        "fillcolor": "#0000FF40",
        "height": "0.75",
    ...
    ***REMOVED***
    ...
    if path is None:
        ext = "png"
        if suffix:
            suffix = f"_{suffix***REMOVED***.{ext***REMOVED***"
        else:
            suffix = f".{ext***REMOVED***"
