...
class XMLOutputParser(BaseTransformOutputParser):
    ...
    def parse(self, text: str) -> Dict[str, List[Any]]:
        ...
        if (text.startswith("<") or text.startswith("\n<")) and (
            ...
            raise ValueError(f"Could not parse output: {text***REMOVED***")
    ...
    def _root_to_dict(self, root: ET.Element) -> Dict[str, List[Any]]:
        ...
        result: Dict[str, List[Any]] = {root.tag: []***REMOVED***
        for child in root:
            if len(child) == 0:
                result[root.tag].append({child.tag: child.text***REMOVED***)
            else:
...
def nested_element(path: List[str], elem: ET.Element) -> Any:
    ...
    if len(path) == 0:
        return AddableDict({elem.tag: elem.text***REMOVED***)
    else:
        return AddableDict({path[0]: [nested_element(path[1:], elem)]***REMOVED***)
