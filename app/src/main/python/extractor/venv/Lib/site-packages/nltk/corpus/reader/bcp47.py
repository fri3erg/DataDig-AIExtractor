...
***REMOVED***
    ...
    def subdiv_dict(self, subdivs):
        ...
        return {sub.attrib["type"]: sub.text for sub in subdivs***REMOVED***
    ...
    def morphology(self):
        ...
        ***REMOVED***
        ...
        alnum = "[a-zA-Z0-9]"
        self.format = {
            "language": re.compile(f"{low*3***REMOVED***?"),
            "extlang": re.compile(f"{low*3***REMOVED***"),
            "script": re.compile(f"{up***REMOVED***{low*3***REMOVED***"),
            "region": re.compile(f"({up*2***REMOVED***)|({dig*3***REMOVED***)"),
            "variant": re.compile(f"{alnum*4***REMOVED***{(alnum+'?')*4***REMOVED***"),
            "singleton": re.compile(f"{low***REMOVED***"),
        ***REMOVED***
            ...
            while labels:
                ...
                if self.format[label].fullmatch(subtag):
                    if subtag in self.db[label]:
                        ...
                        found = True
                        note = f"The {subtag!r***REMOVED*** {label***REMOVED*** code is deprecated"
                        if "Preferred-Value" in self.db["deprecated"][label][subtag]:
                            prefer = self.db["deprecated"][label][subtag][
                                "Preferred-Value"
                            ]
                            note += f"', prefer '{self.val2str(prefer)***REMOVED***'"
            ...
            if not found:
                if subtag == "u" and subtags[0] == "sd":  # CLDR regional subdivisions
                    ...
                    if sd in self.subdiv:
                        ...
                        ext = f"<Unknown subdivision: {ext***REMOVED***>"
                else:  # other extension subtags are not supported yet
                    ext = f"{subtag***REMOVED***{''.join(['-'+ext for ext in subtags])***REMOVED***".lower()
                    if not self.format["singleton"].fullmatch(subtag):
                        ext = f"<Invalid extension: {ext***REMOVED***>"
                        warn(ext)
                lang["extension"] = ext
                subtags = []
        return lang
    ...
    def name(self, tag):
        ...
        for label in ["redundant", "grandfathered"]:
            ...
            if tag in self.db[label]:
                val = f"{self.db[label][tag]['Description']***REMOVED***"
                note = f"The {tag!r***REMOVED*** code is {label***REMOVED***"
            elif tag in self.db["deprecated"][label]:
                val = f"{self.db['deprecated'][label][tag]['Description']***REMOVED***"
                note = f"The {tag!r***REMOVED*** code is {label***REMOVED*** and deprecated"
                if "Preferred-Value" in self.db["deprecated"][label][tag]:
                    prefer = self.db["deprecated"][label][tag]["Preferred-Value"]
                    note += f", prefer {self.val2str(prefer)!r***REMOVED***"
            if val:
                warn(note)
                return val
        try:
            ...
            warn(f"Tag {tag!r***REMOVED*** was not recognized")
            return None
