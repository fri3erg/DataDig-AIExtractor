...
***REMOVED***
    ...
    def show_most_informative_features(self, n=10, show="all"):
        ...
        if show == "pos":
            ...
            fids = [fid for fid in fids if self._weights[fid] < 0]
        for fid in fids[:n]:
            print(f"{self._weights[fid]:8.3f***REMOVED*** {self._encoding.describe(fid)***REMOVED***")
...
***REMOVED***
        ...
        if unseen_features:
            fnames = {fname for (fname, fval, label) in mapping***REMOVED***
            self._unseen = {fname: i + self._length for (i, fname) in enumerate(fnames)***REMOVED***
    ...
    def describe(self, f_id):
        ...
        if f_id < len(self._mapping):
            ...
            return f"{fname***REMOVED***=={fval!r***REMOVED*** and label is {label!r***REMOVED***"
    ...
    def train(cls, train_toks, count_cutoff=0, labels=None, **options):
        ...
        """
        mapping = {***REMOVED***  # maps (fname, fval, label) -> fid
...
class GISEncoding(BinaryMaxentFeatureEncoding):
    ...
    def __init__(
        ...
        if C is None:
            C = len({fname for (fname, fval, label) in mapping***REMOVED***) + 1
        self._C = C
...
class TypedMaxentFeatureEncoding(MaxentFeatureEncodingI):
    ...
    def __init__(self, labels, mapping, unseen_features=False, alwayson_features=False):
        ...
        :param mapping: A dictionary mapping from ``(fname,fval,label)``
            ...
            indexes must be the set of integers from 0...len(mapping).
            If ``mapping[fname,fval,label]=id``, then
            ``self.encode({..., fname:fval, ...``, label)[id]***REMOVED*** is 1;
            otherwise, it is 0.
...
***REMOVED***
    ...
    if trace > 2:
        ...
        print(f"         Final    {ll:14.5f***REMOVED***    {acc:9.3f***REMOVED***")
...
def calculate_nfmap(train_toks, encoding):
    ...
    *feature_vector*.
    ...
    7, then ``_nfmap`` might return the dictionary ``{3:0, 5:1, 7:2***REMOVED***``.
    ...
    return {nf: i for (i, nf) in enumerate(nfset)***REMOVED***
...
***REMOVED***
    ...
    try:
        ...
        print(f"Warning: unable to delete {trainfile_name***REMOVED***: {e***REMOVED***")
