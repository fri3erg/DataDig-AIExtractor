...
def test_merge_cross_mixed_dtypes():
    ...
    result = merge(left, right, how="cross")
    expected = DataFrame({"A": ["a", "a", "b", "b", "c", "c"], "B": [0, 1, 0, 1, 0, 1]***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_merge_cross_more_than_one_column():
    # GH#5401
    left = DataFrame({"A": list("ab"), "B": [2, 1]***REMOVED***)
    right = DataFrame({"C": range(2), "D": range(4, 6)***REMOVED***)
    result = merge(left, right, how="cross")
    expected = DataFrame(
        {
            "A": ["a", "a", "b", "b"],
...
def test_merge_cross_null_values(nulls_fixture):
    # GH#5401
    left = DataFrame({"a": [1, nulls_fixture]***REMOVED***)
    right = DataFrame({"b": ["a", "b"], "c": [1.0, 2.0]***REMOVED***)
    result = merge(left, right, how="cross")
    expected = DataFrame(
        {
