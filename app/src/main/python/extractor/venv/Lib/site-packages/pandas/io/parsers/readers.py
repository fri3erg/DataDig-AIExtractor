...
sep : str, default {_default_sep***REMOVED***
...
dtype : dtype or dict of {{Hashable : dtype***REMOVED******REMOVED***, optional
    Data type(s) to apply to either the whole dataset or individual columns.
    E.g., ``{{'a': np.float64, 'b': np.int32, 'c': 'Int64'***REMOVED******REMOVED***``
    ...
    .. versionadded:: 1.5.0
        ...
        listed.
engine : {{'c', 'python', 'pyarrow'***REMOVED******REMOVED***, optional
...
converters : dict of {{Hashable : Callable***REMOVED******REMOVED***, optional
...
na_values : Hashable, Iterable of Hashable or dict of {{Hashable : Iterable***REMOVED******REMOVED***, optional
...
parse_dates : bool, list of Hashable, list of lists or dict of {{Hashable : list***REMOVED******REMOVED***, \
default False
    ...
    * ``dict``, e.g. ``{{'foo' : [1, 3]***REMOVED******REMOVED***`` -> parse columns 1, 3 as date and call
...
3 or csv.QUOTE_NONE***REMOVED******REMOVED***, default csv.QUOTE_MINIMAL
...
on_bad_lines : {{'error', 'warn', 'skip'***REMOVED******REMOVED*** or Callable, default 'error'
...
float_precision : {{'high', 'legacy', 'round_trip'***REMOVED******REMOVED***, optional
...
dtype_backend : {{'numpy_nullable', 'pyarrow'***REMOVED******REMOVED***, default 'numpy_nullable'
...
{see_also_func_name***REMOVED*** : {see_also_func_summary***REMOVED***
read_fwf : Read a table of fixed-width formatted lines into DataFrame.

Examples
--------
>>> pd.{func_name***REMOVED***('data.csv')  # doctest: +SKIP
"""
)
...
_fwf_defaults: _Fwf_Defaults = {"colspecs": "infer", "infer_nrows": 100, "widths": None***REMOVED***
_c_unsupported = {"skipfooter"***REMOVED***
_python_unsupported = {"low_memory", "float_precision"***REMOVED***
...
def validate_integer(
    ...
    msg = f"'{name:s***REMOVED***' must be an integer >={min_val:d***REMOVED***"
    if is_float(val):
        if int(val) != val:
...
***REMOVED***:
                raise ValueError(
                    f"The {repr(argname)***REMOVED*** option is not supported with the "
                    f"'pyarrow' engine"
    ***REMOVED***
...
***REMOVED***
    ...
    def _clean_options(
        ...
        if sep is None and not delim_whitespace:
            if engine in ("c", "pyarrow"):
                fallback_reason = (
                    f"the '{engine***REMOVED***' engine does not support "
    ...
    ***REMOVED***
            ...
            if not encodeable and engine not in ("python", "python-fwf"):
                fallback_reason = (
                    f"the separator encoded in {encoding***REMOVED*** "
                    f"is > 1 char long, and the '{engine***REMOVED***' engine "
...
***REMOVED***:
                fallback_reason = (
                    ...
                    f"and the '{engine***REMOVED***' engine does not support such quotechars"
    ***REMOVED***
                engine = "python"
            ...
            for arg in _python_unsupported:
                if fallback_reason and result[arg] != _c_parser_defaults.get(arg):
                    raise ValueError(
                        ...
                        f"{fallback_reason***REMOVED***, but this causes {repr(arg)***REMOVED*** to be "
        ...
        ***REMOVED***
        ...
        if fallback_reason:
            warnings.warn(
                (
                    "Falling back to the 'python' engine because "
                    f"{fallback_reason***REMOVED***; you can avoid this warning by specifying "
                    "engine='python'."
    ***REMOVED***,
                ParserWarning,
                stacklevel=find_stack_level(),
        ...
        if converters is not None:
            if not isinstance(converters, dict):
                raise TypeError(
                    ...
                    f"input was a {type(converters).__name__***REMOVED***"
    ***REMOVED***
        else:
            converters = {***REMOVED***
    ...
    def _make_engine(
        ...
        ***REMOVED***
        if engine not in mapping:
            raise ValueError(
                f"Unknown engine: {engine***REMOVED*** (valid options are {mapping.keys()***REMOVED***)"
***REMOVED***
...
***REMOVED***
            ...
            msg = f"Invalid file path or buffer object type: {type(f)***REMOVED***"
            raise ValueError(msg)
        ...
        if self.engine == "pyarrow":
            ...
            try:
                ...
                (
                    ...
                    columns,
                    col_dict,
    ***REMOVED*** = self._engine.read(  # type: ignore[attr-defined]
                    nrows
    ***REMOVED***
            except Exception:
                self.close()
...
def _clean_na_values(na_values, keep_default_na: bool = True, floatify: bool = True):
    ...
    if na_values is None:
        ...
        na_values = {***REMOVED***  # Prevent aliasing.
        ...
        for k, v in old_na_values.items():
            ...
            na_values[k] = v
        na_fvalues = {k: _floatify_na_values(v) for k, v in na_values.items()***REMOVED***
    else:
...
def _refine_defaults_read(
    ...
    delim_whitespace : bool
        ...
        .. deprecated:: 2.2.0
            Use ``sep="\\s+"`` instead.
    engine : {{'c', 'python'***REMOVED******REMOVED***
...
***REMOVED***
        ...
        raise ValueError(f"Argument {on_bad_lines***REMOVED*** is invalid for on_bad_lines")
...
def _validate_dialect(dialect: csv.Dialect) -> None:
    ...
    for param in MANDATORY_DIALECT_ATTRS:
        if not hasattr(dialect, param):
            raise ValueError(f"Invalid dialect {dialect***REMOVED*** provided")
...
def _merge_with_dialect_properties(
    ...
    for param in MANDATORY_DIALECT_ATTRS:
        ...
        if provided not in (parser_default, dialect_val):
            msg = (
                f"Conflicting values for '{param***REMOVED***': '{provided***REMOVED***' was "
                f"provided, but the dialect specifies '{dialect_val***REMOVED***'. "
