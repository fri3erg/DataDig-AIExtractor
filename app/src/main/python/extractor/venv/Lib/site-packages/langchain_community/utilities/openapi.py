...
class HTTPVerb(str, Enum):
    ...
    def from_str(cls, verb: str) -> HTTPVerb:
        ...
        try:
            ...
            raise ValueError(f"Invalid HTTP verb. Valid values are {cls.__members__***REMOVED***")
...
class OpenAPISpec(OpenAPI):
    ...
    def _get_path_strict(self, path: str) -> PathItem:
        ...
        if not path_item:
            raise ValueError(f"No path found for {path***REMOVED***")
        return path_item
    ...
    def _get_referenced_parameter(self, ref: Reference) -> Union[Parameter, Reference]:
        ...
        if ref_name not in parameters:
            raise ValueError(f"No parameter found for {ref_name***REMOVED***")
    ...
    def get_referenced_schema(self, ref: Reference) -> Schema:
        ...
        if ref_name not in schemas:
            raise ValueError(f"No schema found for {ref_name***REMOVED***")
    ...
    def _get_referenced_request_body(
        ...
        if ref_name not in request_bodies:
            raise ValueError(f"No request body found for {ref_name***REMOVED***")
    ...
    def _alert_unsupported_spec(obj: dict) -> None:
        ...
        if isinstance(openapi_version, str):
            if openapi_version != "3.1.0":
                logger.warning(
                    f"Attempting to load an OpenAPI {openapi_version***REMOVED***"
                    f" spec. {warning_message***REMOVED***"
    ***REMOVED***
            else:
                pass
        elif isinstance(swagger_version, str):
            logger.warning(
                f"Attempting to load a Swagger {swagger_version***REMOVED***"
                f" spec. {warning_message***REMOVED***"
***REMOVED***
        else:
            raise ValueError(
                "Attempting to load an unsupported spec:"
                f"\n\n{obj***REMOVED***\n{warning_message***REMOVED***"
***REMOVED***
    ...
    def from_file(cls, path: Union[str, Path]) -> OpenAPISpec:
        ...
        if not path_.exists():
            raise FileNotFoundError(f"{path***REMOVED*** does not exist")
    ...
    def get_operation(self, path: str, method: str) -> Operation:
        ...
        if not isinstance(operation_obj, Operation):
            raise ValueError(f"No {method***REMOVED*** method found for {path***REMOVED***")
    ...
    def get_cleaned_operation_id(operation: Operation, path: str, method: str) -> str:
        ...
        if operation_id is None:
            ...
            operation_id = f"{path***REMOVED***_{method***REMOVED***"
