...
def _log_and_unlink(filename):
    ...
    util.debug(
        "[FINALIZER CALL] object mapping to {***REMOVED*** about to be deleted,"
        " decrementing the refcount of the file (pid: {***REMOVED***)".format(
...
def add_maybe_unlink_finalizer(memmap):
    util.debug(
        "[FINALIZER ADD] adding finalizer to {***REMOVED*** (id {***REMOVED***, filename {***REMOVED***, pid  {***REMOVED***)"
...
def unlink_file(filename):
    ...
    for retry_no in range(1, NUM_RETRIES + 1):
        try:
            ...
            util.debug(
                '[ResourceTracker] tried to unlink {***REMOVED***, got '
...
def _reduce_memmap_backed(a, m):
    ...
    util.debug('[MEMMAP REDUCE] reducing a memmap-backed array '
               '(shape, {***REMOVED***, pid: {***REMOVED***)'.format(a.shape, os.getpid()))
    ...
    def __call__(self, a):
        ...
        if (not a.dtype.hasobject and self._max_nbytes is not None and
            ...
            try:
                ...
                basename = "{***REMOVED***-{***REMOVED***-{***REMOVED***.pkl".format(
            ...
            if not os.path.exists(filename):
                util.debug(
                    "[ARRAY DUMP] Pickling new array (shape={***REMOVED***, dtype={***REMOVED***) "
                    "creating a new memmap at {***REMOVED***".format(
                ...
                util.debug(
                    "[ARRAY DUMP] Pickling known array (shape={***REMOVED***, dtype={***REMOVED***) "
                    "reusing memmap file: {***REMOVED***".format(
...
***REMOVED***
            ...
            util.debug(
                '[ARRAY DUMP] Pickling array (NO MEMMAPPING) (shape={***REMOVED***, '
                ' dtype={***REMOVED***).'.format(a.shape, a.dtype))
...
class TemporaryResourcesManager(object):
    ...
    def register_new_context(self, context_id):
        ...
        if context_id in self._cached_temp_folders:
            ...
            new_folder_name = (
                "joblib_memmapping_folder_{***REMOVED***_{***REMOVED***_{***REMOVED***".format(
...
***REMOVED***.delete_folder
            try:
                ...
                warnings.warn("Failed to delete temporary folder: {***REMOVED***"
