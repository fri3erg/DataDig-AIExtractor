...
class VectaraQueryConfig:
    ...
    filter Dictionary of argument(s) to filter on metadata. For example a
        filter can be "doc.rating > 3.0 and part.lang = 'deu'"***REMOVED*** see
...
***REMOVED***
            ...
            logger.debug(f"Using corpus id {self._vectara_corpus_id***REMOVED***")
        self._source = source
    ...
    def _delete_doc(self, doc_id: str) -> bool:
        ...
        ***REMOVED***
        ...
        if response.status_code != 200:
            logger.error(
                f"Delete request failed for doc_id = {doc_id***REMOVED*** with status code "
                f"{response.status_code***REMOVED***, reason {response.reason***REMOVED***, text "
                f"{response.text***REMOVED***"
***REMOVED***
            return False
        return True

    def _index_doc(self, doc: dict, use_core_api: bool = False) -> str:
        request: dict[str, Any] = {***REMOVED***
        ...
        **kwargs: Any,
        ...
        for inx, file in enumerate(files_list):
            if not os.path.exists(file):
                logger.error(f"File {file***REMOVED*** does not exist, skipping")
                continue
            md = metadatas[inx] if metadatas else {***REMOVED***
            files: dict = {
            ...
            ***REMOVED***
            ...
            response = self._session.post(
                f"https://api.vectara.io/upload?c={self._vectara_customer_id***REMOVED***&o={self._vectara_corpus_id***REMOVED***&d=True",
                files=files,
                verify=True,
                headers=headers,
                timeout=self.vectara_api_timeout,
***REMOVED***
            ...
            if response.status_code == 409:
                ...
                logger.info(
                    f"File {file***REMOVED*** already exists on Vectara (doc_id={doc_id***REMOVED***), skipping"
    ***REMOVED***
            elif response.status_code == 200:
                doc_id = response.json()["document"]["documentId"]
                doc_ids.append(doc_id)
            else:
                logger.info(f"Error indexing file {file***REMOVED***: {response.json()***REMOVED***")
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        if doc_metadata:
            ...
            doc_metadata = {"source": "langchain"***REMOVED***
        ...
        doc = {
            ...
            section_key: [
                {"text": text, "metadataJson": json.dumps(md)***REMOVED***
                    ...
                    "corpusKey": [
                        {
                            ...
                            "lexicalInterpolationConfig": {"lambda": config.lambda_val***REMOVED***,
                        ***REMOVED***
                    ],
                ***REMOVED***
            ]
        ***REMOVED***
        if config.mmr_config.is_enabled:
            data["query"][0]["rerankingConfig"] = {
                ...
                "mmrConfig": {"diversityBias": config.mmr_config.diversity_bias***REMOVED***,
            ***REMOVED***
        ...
        if response.status_code != 200:
            logger.error(
                "Query failed %s",
                f"(code {response.status_code***REMOVED***, reason {response.reason***REMOVED***, details "
                f"{response.text***REMOVED***)",
***REMOVED***
            return [], ""
        ...
        metadatas = []
        for x in responses:
            md = {m["name"]: m["value"] for m in x["metadata"]***REMOVED***
            doc_num = x["documentIndex"]
            doc_md = {m["name"]: m["value"] for m in documents[doc_num]["metadata"]***REMOVED***
...
***REMOVED***
        ...
        if config.summary_config.is_enabled:
            ...
            res.append(
                (Document(page_content=summary, metadata={"summary": True***REMOVED***), 0.0)
***REMOVED***
    ...
    ***REMOVED***
        ...
        doc_metadata = kwargs.pop("doc_metadata", {***REMOVED***)
        vectara = cls(**kwargs)
...
class VectaraRetriever(VectorStoreRetriever):
    ...
    def add_texts(
        ...
        """
        self.vectorstore.add_texts(texts, metadatas, doc_metadata or {***REMOVED***)
