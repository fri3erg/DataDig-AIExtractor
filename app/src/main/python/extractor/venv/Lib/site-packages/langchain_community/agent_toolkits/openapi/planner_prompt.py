...
POST /users/{{id***REMOVED******REMOVED***/cart to add products to a user's cart
PATCH /users/{{id***REMOVED******REMOVED***/cart to update a user's cart
PUT /users/{{id***REMOVED******REMOVED***/coupon to apply idempotent coupon to a user's cart
DELETE /users/{{id***REMOVED******REMOVED***/cart to delete a user's cart
...
3. POST /users/{{id***REMOVED******REMOVED***/cart to add a couch to the user's cart

User query: I want to add a lamp to my cart
Plan: 1. GET /products with a query param to search for lamps
2. GET /user to find the user's id
3. PATCH /users/{{id***REMOVED******REMOVED***/cart to add a lamp to the user's cart

User query: I want to add a coupon to my cart
Plan: 1. GET /user to find the user's id
2. PUT /users/{{id***REMOVED******REMOVED***/coupon to apply the coupon

User query: I want to delete my cart
Plan: 1. GET /user to find the user's id
2. DELETE required. Did user specify DELETE or previously authorize? Yes, proceed.
3. DELETE /users/{{id***REMOVED******REMOVED***/cart to delete the user's cart
...
User query: {query***REMOVED***
Plan:"""
API_PLANNER_TOOL_NAME = "api_planner"
API_PLANNER_TOOL_DESCRIPTION = f"Can be used to generate the right API calls to assist with a user query, like {API_PLANNER_TOOL_NAME***REMOVED***(query). Should always be called before trying to call the API controller."
...
Base url: {api_url***REMOVED***
Endpoints:
{api_docs***REMOVED***


Here are tools to execute requests against the API: {tool_descriptions***REMOVED***
...
Action: the action to take, should be one of the tools [{tool_names***REMOVED***]
...
API_CONTROLLER_TOOL_DESCRIPTION = f"Can be used to execute a plan of API calls, like {API_CONTROLLER_TOOL_NAME***REMOVED***(plan)."
...
Here are the tools to plan and execute API requests: {tool_descriptions***REMOVED***
...
Your task is to extract some information according to these instructions: {instructions***REMOVED***
