...
class TestDataFrameConstructors:
    ...
    def test_constructor_from_2d_datetimearray(self, using_array_manager):
        ...
        df = DataFrame(dta)
        expected = DataFrame({0: dta[:, 0], 1: dta[:, 1]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_constructor_dict_with_tzaware_scalar(self):
        ...
        df = DataFrame({"dt": dt***REMOVED***, index=[0])
        expected = DataFrame({"dt": [dt]***REMOVED***)
        tm.assert_frame_equal(df, expected)

        # Non-homogeneous
        df = DataFrame({"dt": dt, "value": [1]***REMOVED***)
        expected = DataFrame({"dt": [dt], "value": [1]***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_columns_with_leading_underscore_work_with_to_dict(self):
        col_underscore = "_b"
        df = DataFrame({"a": [1, 2], col_underscore: [3, 4]***REMOVED***)
        d = df.to_dict(orient="records")

        ref_d = [{"a": 1, col_underscore: 3***REMOVED***, {"a": 2, col_underscore: 4***REMOVED***]

        assert ref_d == d

    def test_columns_with_leading_number_and_underscore_work_with_to_dict(self):
        col_with_num = "1_b"
        df = DataFrame({"a": [1, 2], col_with_num: [3, 4]***REMOVED***)
        d = df.to_dict(orient="records")

        ref_d = [{"a": 1, col_with_num: 3***REMOVED***, {"a": 2, col_with_num: 4***REMOVED***]
    ...
    @pytest.mark.parametrize(
        "constructor",
        [
            lambda: DataFrame({***REMOVED***),
            lambda: DataFrame(data={***REMOVED***),
        ],
    )
    ...
    def test_constructor_cast_failure(self):
        ...
        with pytest.raises(ValueError, match=msg):
            DataFrame({"a": ["a", "b", "c"]***REMOVED***, dtype=np.float64)
    ...
    def test_constructor_dtype_copy(self):
        orig_df = DataFrame({"col1": [1.0], "col2": [2.0], "col3": [3.0]***REMOVED***)
    ...
    def test_constructor_complex_dtypes(self):
        ...
        df = DataFrame({"a": a, "b": b***REMOVED***)
        assert a.dtype == df.a.dtype
        assert b.dtype == df.b.dtype

    def test_constructor_dtype_str_na_values(self, string_dtype):
        # https://github.com/pandas-dev/pandas/issues/21083
        df = DataFrame({"A": ["x", None]***REMOVED***, dtype=string_dtype)
        result = df.isna()
        expected = DataFrame({"A": [False, True]***REMOVED***)
        tm.assert_frame_equal(result, expected)
        assert df.iloc[1, 0] is None

        df = DataFrame({"A": ["x", np.nan]***REMOVED***, dtype=string_dtype)
    ...
    def test_constructor_bool(self):
        df = DataFrame({0: np.ones(10, dtype=bool), 1: np.zeros(10, dtype=bool)***REMOVED***)
    ...
    def test_constructor_overflow_int64(self):
        ...
        result = DataFrame({"a": values***REMOVED***)
    ...
    def test_constructor_ordereddict(self):
        ...
        expected = [f"A{i:d***REMOVED***" for i in nums]
    ...
    def test_constructor_dict(self):
        ...
        frame = DataFrame({"col1": datetime_series, "col2": datetime_series_short***REMOVED***)
        ...
        frame = DataFrame(
            {"col1": datetime_series, "col2": datetime_series_short***REMOVED***,
    ...
    def test_constructor_dict_of_empty_lists(self):
        ...
        frame = DataFrame({"A": [], "B": []***REMOVED***, columns=["A", "B"])
        tm.assert_index_equal(frame.index, RangeIndex(0), exact=True)

    def test_constructor_dict_with_none(self):
        # GH 14381
        # Dict with None value
        frame_none = DataFrame({"a": None***REMOVED***, index=[0])
        frame_none_list = DataFrame({"a": [None]***REMOVED***, index=[0])
    ...
    def test_constructor_dict_errors(self):
        ...
        with pytest.raises(ValueError, match=msg):
            DataFrame({"a": 0.7***REMOVED***)

        with pytest.raises(ValueError, match=msg):
            DataFrame({"a": 0.7***REMOVED***, columns=["a"])

    @pytest.mark.parametrize("scalar", [2, np.nan, None, "D"])
    def test_constructor_invalid_items_unused(self, scalar):
        # No error if invalid (scalar) value is in fact not used:
        result = DataFrame({"a": scalar***REMOVED***, columns=["b"])
    ...
    def test_constructor_dict_nan_key(self, value):
        ...
        idx = ["a", value]
        values = [[0, 3], [1, 4], [2, 5]]
        data = {cols[c]: Series(values[c], index=idx) for c in range(3)***REMOVED***
    ...
    def test_constructor_dict_order_insertion(self):
        ...
        d = {"b": datetime_series_short, "a": datetime_series***REMOVED***
    ...
    def test_constructor_dict_nan_key_and_columns(self):
        # GH 16894
        result = DataFrame({np.nan: [1, 2], 2: [2, 3]***REMOVED***, columns=[np.nan, 2])
...
***REMOVED***
        ...
        msg = "If using all scalar values, you must pass an index"
        with pytest.raises(ValueError, match=msg):
            DataFrame({"a": False, "b": True***REMOVED***)
    ...
    def test_constructor_subclass_dict(self, dict_subclass):
        ...
        ***REMOVED***
        df = DataFrame(data)
        refdf = DataFrame({col: dict(val.items()) for col, val in data.items()***REMOVED***)
    ...
    def test_constructor_dict_block(self):
        ...
        df = DataFrame(
            {"d": [4.0], "c": [3.0], "b": [2.0], "a": [1.0]***REMOVED***,
            columns=["d", "c", "b", "a"],
        )
    ...
    def test_constructor_dict_cast(self, using_infer_string):
        # cast float tests
        test_data = {"A": {"1": 1, "2": 2***REMOVED***, "B": {"1": "1", "2": "2", "3": "3"***REMOVED******REMOVED***
    ...
    def test_constructor_dict_cast2(self):
        # can't cast to float
        test_data = {
            "A": dict(zip(range(20), [f"word_{i***REMOVED***" for i in range(20)])),
    ...
    def test_constructor_dict_dont_upcast(self):
        d = {"Col1": {"Row1": "A String", "Row2": np.nan***REMOVED******REMOVED***
        df = DataFrame(d)
    ...
    def test_constructor_dict_of_tuples(self):
        # GH #1491
        data = {"a": (1, 2, 3), "b": (4, 5, 6)***REMOVED***

        result = DataFrame(data)
        expected = DataFrame({k: list(v) for k, v in data.items()***REMOVED***)
        tm.assert_frame_equal(result, expected, check_dtype=False)

    def test_constructor_dict_of_ranges(self):
        # GH 26356
        data = {"a": range(3), "b": range(3, 6)***REMOVED***

        result = DataFrame(data)
        expected = DataFrame({"a": [0, 1, 2], "b": [3, 4, 5]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_constructor_dict_of_iterators(self):
        # GH 26349
        data = {"a": iter(range(3)), "b": reversed(range(3))***REMOVED***

        result = DataFrame(data)
        expected = DataFrame({"a": [0, 1, 2], "b": [2, 1, 0]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_constructor_dict_of_generators(self):
        # GH 26349
        data = {"a": (i for i in (range(3))), "b": (i for i in reversed(range(3)))***REMOVED***
        result = DataFrame(data)
        expected = DataFrame({"a": [0, 1, 2], "b": [2, 1, 0]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_dict_multiindex(self):
        d = {
            ("a", "a"): {("i", "i"): 0, ("i", "j"): 1, ("j", "i"): 2***REMOVED***,
            ("b", "a"): {("i", "i"): 6, ("i", "j"): 5, ("j", "i"): 4***REMOVED***,
            ("b", "c"): {("i", "i"): 7, ("i", "j"): 8, ("j", "i"): 9***REMOVED***,
        ***REMOVED***
        _d = sorted(d.items())
        df = DataFrame(d)
        expected = DataFrame(
        ...
        d["z"] = {"y": 123.0, ("i", "i"): 111, ("i", "j"): 111, ("j", "i"): 111***REMOVED***
        _d.insert(0, ("z", d["z"]))
        expected = DataFrame(
    ...
    def test_constructor_dict_datetime64_index(self):
        ...
        def create_data(constructor):
            return {i: {constructor(s): 2 * i***REMOVED*** for i, s in enumerate(dates_as_str)***REMOVED***
        ...
        expected = DataFrame(
            [
                {0: 0, 1: None, 2: None, 3: None***REMOVED***,
                {0: None, 1: 2, 2: None, 3: None***REMOVED***,
                {0: None, 1: None, 2: 4, 3: None***REMOVED***,
                {0: None, 1: None, 2: None, 3: 6***REMOVED***,
            ],
    ...
    def test_constructor_dict_timedelta64_index(self, klass, name):
        ...
        data = {i: {klass(s): 2 * i***REMOVED*** for i, s in enumerate(td_as_int)***REMOVED***
    ...
    def test_constructor_period_dict(self):
        ...
        # list of periods
        df = DataFrame({"a": a.astype(object).tolist(), "b": b.astype(object).tolist()***REMOVED***)
        assert df["a"].dtype == a.dtype
        assert df["b"].dtype == b.dtype

    def test_constructor_dict_extension_scalar(self, ea_scalar_and_dtype):
        ea_scalar, ea_dtype = ea_scalar_and_dtype
        df = DataFrame({"a": ea_scalar***REMOVED***, index=[0])
...
***REMOVED***,
    ...
    def test_constructor_extension_scalar_data(self, data, dtype):
        ...
        expected = DataFrame({"a": arr, "b": arr***REMOVED***)
    ...
    def test_nested_dict_frame_constructor(self):
        ...
        for col in df.columns:
            for row in df.index:
                data.setdefault(col, {***REMOVED***)[row] = df._get_value(row, col)
        ...
        for col in df.columns:
            for row in df.index:
                data.setdefault(row, {***REMOVED***)[col] = df._get_value(row, col)
...
***REMOVED***
    ...
    def test_constructor_maskedarray_hardened(self):
        ...
        expected = DataFrame(
            {"A": [np.nan, np.nan], "B": [np.nan, np.nan]***REMOVED***,
            columns=["A", "B"],
            index=[1, 2],
            dtype=float,
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({***REMOVED***, None, ["foo", "bar"], None, np.object_),
            ({"b": 1***REMOVED***, list(range(10)), list("abc"), int, np.dtype("float64")),
        ],
    )
    ...
    def test_constructor_dtype_nullable_extension_arrays(
        ...
        df = DataFrame({"a": data***REMOVED***, dtype=input_dtype)
        assert df["a"].dtype == expected_dtype()

    def test_constructor_scalar_inference(self, using_infer_string):
        data = {"int": 1, "bool": True, "float": 3.0, "complex": 4j, "object": "foo"***REMOVED***
    ...
    def test_constructor_arrays_and_scalars(self):
        df = DataFrame({"a": np.random.default_rng(2).standard_normal(10), "b": True***REMOVED***)
        exp = DataFrame({"a": df["a"].values, "b": [True] * 10***REMOVED***)
    ...
    def test_constructor_more(self, float_frame):
        ...
        # int cast
        dm = DataFrame(
            {"A": np.ones(10, dtype=int), "B": np.ones(10, dtype=np.float64)***REMOVED***,
            index=np.arange(10),
        )
    ...
    def test_constructor_empty_list(self):
        ...
        expected = DataFrame({***REMOVED***, columns=["A", "B"])
        tm.assert_frame_equal(df, expected)
    ...
    def test_constructor_list_of_lists(self, using_infer_string):
        ...
        expected = DataFrame({0: np.arange(10)***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [{"x": Timestamp("2021-01-01")***REMOVED***],
            {"x": [Timestamp("2021-01-01")]***REMOVED***,
            {"x": Timestamp("2021-01-01").as_unit("ns")***REMOVED***,
        ],
    )
    def test_constructor_one_element_data_list(self, data):
        # GH#42810
        result = DataFrame(data, index=[0, 1, 2], columns=["x"])
        expected = DataFrame({"x": [Timestamp("2021-01-01")] * 3***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_stdlib_array(self):
        # GH 4297
        # support Array
        result = DataFrame({"A": array.array("i", range(10))***REMOVED***)
        expected = DataFrame({"A": list(range(10))***REMOVED***)
        tm.assert_frame_equal(result, expected, check_dtype=False)
    ...
    def test_constructor_generator(self):
        ...
        expected = DataFrame({0: range(10), 1: "a"***REMOVED***)
        tm.assert_frame_equal(result, expected, check_dtype=False)

    def test_constructor_list_of_dicts(self):
        result = DataFrame([{***REMOVED***])
    ...
    def test_constructor_ordered_dict_nested_preserve_order(self):
        ...
        result = DataFrame(data)
        data = {"col2": [1, 2], "col1": [2, 5]***REMOVED***
    ...
    def test_constructor_list_of_series_aligned_index(self):
        ...
        expected = DataFrame(
            {"b": [0, 1, 2], "a": [0, 1, 2], "c": [0, 1, 2]***REMOVED***,
            columns=["b", "a", "c"],
            index=["0", "1", "2"],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_scalar(self):
        idx = Index(range(3))
        df = DataFrame({"a": 0***REMOVED***, index=idx)
        expected = DataFrame({"a": [0, 0, 0]***REMOVED***, index=idx)
    ...
    def test_constructor_mixed_dict_and_Series(self):
        data = {***REMOVED***
        data["A"] = {"foo": 1, "bar": 2, "baz": 3***REMOVED***
        ...
        with pytest.raises(ValueError, match="ambiguous ordering"):
            DataFrame({"A": ["a", "b"], "B": {"a": "a", "b": "b"***REMOVED******REMOVED***)

        # this is OK though
        result = DataFrame({"A": ["a", "b"], "B": Series(["a", "b"], index=["a", "b"])***REMOVED***)
        expected = DataFrame({"A": ["a", "b"], "B": ["a", "b"]***REMOVED***, index=["a", "b"])
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_list_of_tuples(self):
        result = DataFrame({"A": [(1, 2), (3, 4)]***REMOVED***)
        expected = DataFrame({"A": Series([(1, 2), (3, 4)])***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_list_of_namedtuples(self):
        ...
        expected = DataFrame({"a": [1, 2], "b": [3, 4]***REMOVED***)
        result = DataFrame(tuples)
        tm.assert_frame_equal(result, expected)

        # with columns
        expected = DataFrame({"y": [1, 2], "z": [3, 4]***REMOVED***)
    ...
    def test_constructor_list_of_dataclasses(self):
        ...
        expected = DataFrame({"x": [0, 1], "y": [3, 3]***REMOVED***)
        result = DataFrame(data)
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_list_of_dataclasses_with_varying_types(self):
        ...
        expected = DataFrame(
            {"x": [0, np.nan], "y": [3, 3], "x0": [np.nan, 1], "x1": [np.nan, 3]***REMOVED***
        )
        result = DataFrame(data)
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_list_of_dataclasses_error_thrown(self):
        ...
        with pytest.raises(TypeError, match=re.escape(msg)):
            DataFrame([Point(0, 0), {"x": 1, "y": 0***REMOVED***])

    def test_constructor_list_of_dict_order(self):
        # GH10056
        data = [
            {"First": 1, "Second": 4, "Third": 7, "Fourth": 10***REMOVED***,
            {"Second": 5, "First": 2, "Fourth": 11, "Third": 8***REMOVED***,
            {"Second": 6, "First": 3, "Fourth": 12, "Third": 9, "YYY": 14, "XXX": 13***REMOVED***,
        ]
        expected = DataFrame(
            {
                "First": [1, 2, 3],
    ...
    def test_constructor_Series_named(self):
        ...
        s = Series(arr, name="x")
        df = DataFrame(s)
        expected = DataFrame({"x": s***REMOVED***)
        tm.assert_frame_equal(df, expected)

        s = Series(arr, index=range(3, 13))
        df = DataFrame(s)
        expected = DataFrame({0: s***REMOVED***)
        tm.assert_frame_equal(df, expected)
        ...
        expected = DataFrame({"x": s1, "Unnamed 0": arr***REMOVED***, columns=["x", "Unnamed 0"])
        tm.assert_frame_equal(df, expected)

        # this is a bit non-intuitive here; the series collapse down to arrays
        df = DataFrame([arr, s1]).T
        expected = DataFrame({1: s1, 0: arr***REMOVED***, columns=[0, 1])
        tm.assert_frame_equal(df, expected)
    ...
    def test_constructor_mix_series_nonseries(self, float_frame):
        df = DataFrame(
            {"A": float_frame["A"], "B": list(float_frame["B"])***REMOVED***, columns=["A", "B"]
        )
        tm.assert_frame_equal(df, float_frame.loc[:, ["A", "B"]])

        msg = "does not match index length"
        with pytest.raises(ValueError, match=msg):
            DataFrame({"A": float_frame["A"], "B": list(float_frame["B"])[:-2]***REMOVED***)
...
***REMOVED***,
    ...
    def test_constructor_with_datetimes3(self):
        ...
        df = DataFrame({"End Date": dt***REMOVED***, index=[0])
        assert df.iat[0, 0] == dt
        tm.assert_series_equal(
            df.dtypes, Series({"End Date": "datetime64[us, US/Eastern]"***REMOVED***, dtype=object)
        )

        df = DataFrame([{"End Date": dt***REMOVED***])
        assert df.iat[0, 0] == dt
        tm.assert_series_equal(
            df.dtypes, Series({"End Date": "datetime64[ns, US/Eastern]"***REMOVED***, dtype=object)
        )
    ...
    def test_constructor_with_datetimes5(self):
        ...
        expected = DataFrame({"a": i.to_series().reset_index(drop=True)***REMOVED***)
        df = DataFrame()
        df["a"] = i
        tm.assert_frame_equal(df, expected)

        df = DataFrame({"a": i***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_constructor_with_datetimes6(self):
        ...
        df = DataFrame({"a": i, "b": i_no_tz***REMOVED***)
        expected = DataFrame({"a": i.to_series().reset_index(drop=True), "b": i_no_tz***REMOVED***)
        tm.assert_frame_equal(df, expected)
    ...
    def test_constructor_timedelta_non_ns(self, order, unit):
        dtype = f"timedelta64[{unit***REMOVED***]"
        na = np.array(
            [
        ...
        if unit in ["D", "h", "m"]:
            ...
            exp_unit = unit
        exp_dtype = np.dtype(f"m8[{exp_unit***REMOVED***]")
        expected = DataFrame(
            [
    ...
    def test_constructor_for_list_with_dtypes(self, using_infer_string):
        ...
        df = DataFrame({"a": [2**31, 2**31 + 1]***REMOVED***)
        ...
        df = DataFrame({"a": [1, 2]***REMOVED***)
        assert df.dtypes.iloc[0] == np.dtype("int64")

        df = DataFrame({"a": [1.0, 2.0]***REMOVED***)
        assert df.dtypes.iloc[0] == np.dtype("float64")

        df = DataFrame({"a": 1***REMOVED***, index=range(3))
        assert df.dtypes.iloc[0] == np.dtype("int64")

        df = DataFrame({"a": 1.0***REMOVED***, index=range(3))
        assert df.dtypes.iloc[0] == np.dtype("float64")
    ...
    def test_constructor_series_copy(self, float_frame):
        ...
        df = DataFrame({"A": series["A"]***REMOVED***, copy=True)
...
***REMOVED***,
    ...
    def test_constructor_lists_to_object_dtype(self):
        # from #1074
        d = DataFrame({"a": [np.nan, False]***REMOVED***)
    ...
    def test_constructor_ndarray_categorical_dtype(self):
        ...
        expected = DataFrame({0: cat, 1: cat, 2: cat, 3: cat***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_categorical(self):
        ...
        # dict creation
        df = DataFrame({"A": list("abc")***REMOVED***, dtype="category")
    ...
    def test_construct_from_1item_list_of_categorical(self):
        # pre-2.0 this behaved as DataFrame({0: cat***REMOVED***), in 2.0 we remove
    ...
    def test_constructor_categorical_series(self):
        ...
        expected = DataFrame({"x": expected***REMOVED***)
        df = DataFrame({"x": Series(["a", "b", "c"], dtype="category")***REMOVED***, index=index)
        tm.assert_frame_equal(df, expected)
    ...
    def test_check_dtype_empty_numeric_column(self, dtype):
        ...
        data = DataFrame({"a": [1, 2]***REMOVED***, columns=["b"], dtype=dtype)
    ...
    def test_to_frame_with_falsey_names(self):
        ...
        expected = Series({0: object***REMOVED***)
        tm.assert_series_equal(result, expected)
    ...
    def test_constructor_range_dtype(self, dtype):
        expected = DataFrame({"A": [0, 1, 2, 3, 4]***REMOVED***, dtype=dtype or "int64")
        ...
        # GH 16804
        result = DataFrame({"A": range(5)***REMOVED***, dtype=dtype)
        tm.assert_frame_equal(result, expected)
    ...
    def test_datetime_date_tuple_columns_from_dict(self):
        ...
        v = date.today()
        tup = v, v
        result = DataFrame({tup: Series(range(3), index=range(3))***REMOVED***, columns=[tup])
    ...
    def test_constructor_series_nonexact_categoricalindex(self):
        ...
        result = DataFrame({"1": ser1, "2": ser2***REMOVED***)
        ...
        expected = DataFrame(
            {"1": [10] * 5 + [np.nan] * 5, "2": [np.nan] * 5 + [10] * 5***REMOVED***, index=index
        )
    ...
    def test_from_datetime_subclass(self):
        ...
        data = DataFrame({"datetime": [DatetimeSubclass(2020, 1, 1, 1, 1)]***REMOVED***)
    ...
    def test_frame_ctor_datetime64_column(self):
        ...
        df = DataFrame(
            {"A": np.random.default_rng(2).standard_normal(len(rng)), "B": dates***REMOVED***
        )
    ...
    def test_constructor_list_str(self, input_vals, string_dtype):
        ...
        result = DataFrame({"A": input_vals***REMOVED***, dtype=string_dtype)
        expected = DataFrame({"A": input_vals***REMOVED***).astype({"A": string_dtype***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_constructor_list_str_na(self, string_dtype):
        result = DataFrame({"A": [1.0, 2.0, None]***REMOVED***, dtype=string_dtype)
        expected = DataFrame({"A": ["1.0", "2.0", None]***REMOVED***, dtype=object)
        tm.assert_frame_equal(result, expected)
    ...
    def test_dict_nocopy(
        ...
        if b.dtype.kind in ["S", "U"]:
            ...
            pytest.skip(f"{b.dtype***REMOVED*** get cast, making the checks below more cumbersome")

        c = pd.array([1, 2], dtype=any_numeric_ea_dtype)
        c_orig = c.copy()
        df = DataFrame({"a": a, "b": b, "c": c***REMOVED***, copy=copy)
...
***REMOVED***
    ...
    def test_from_2d_object_array_of_periods_or_intervals(self):
        ...
        df3 = DataFrame(data3)
        expected = DataFrame({0: pi, 1: ii, 2: pi, 3: ii***REMOVED***)
        tm.assert_frame_equal(df3, expected)
    ...
    def test_error_from_2darray(self, col_a, col_b):
        ...
        with pytest.raises(ValueError, match=msg):
            DataFrame({"a": col_a, "b": col_b***REMOVED***)
    ...
    def test_construct_with_strings_and_none(self):
        ...
        expected = DataFrame({"a": ["1", "2", None]***REMOVED***, dtype="str")
        tm.assert_frame_equal(df, expected)
    ...
    def test_frame_string_inference(self):
        ...
        expected = DataFrame(
            {"a": ["a", "b"]***REMOVED***, dtype=dtype, columns=Index(["a"], dtype=dtype)
        )
        with pd.option_context("future.infer_string", True):
            df = DataFrame({"a": ["a", "b"]***REMOVED***)
        tm.assert_frame_equal(df, expected)
        ...
        with pd.option_context("future.infer_string", True):
            df = DataFrame({"a": ["a", "b"]***REMOVED***, index=["x", "y"])
        tm.assert_frame_equal(df, expected)

        expected = DataFrame(
            {"a": ["a", 1]***REMOVED***, dtype="object", columns=Index(["a"], dtype=dtype)
        )
        with pd.option_context("future.infer_string", True):
            df = DataFrame({"a": ["a", 1]***REMOVED***)
        tm.assert_frame_equal(df, expected)

        expected = DataFrame(
            {"a": ["a", "b"]***REMOVED***, dtype="object", columns=Index(["a"], dtype=dtype)
        )
        with pd.option_context("future.infer_string", True):
            df = DataFrame({"a": ["a", "b"]***REMOVED***, dtype="object")
        tm.assert_frame_equal(df, expected)
    ...
    def test_frame_string_inference_array_string_dtype(self):
        ...
        with pd.option_context("future.infer_string", True):
            df = DataFrame({"a": np.array(["a", "b"])***REMOVED***)
        tm.assert_frame_equal(df, expected)

        expected = DataFrame({0: ["a", "b"], 1: ["c", "d"]***REMOVED***, dtype=dtype)
        with pd.option_context("future.infer_string", True):
    ...
    def test_inference_on_pandas_objects(self):
        ...
        result = DataFrame({"a": idx***REMOVED***)
        assert result.dtypes.iloc[0] == np.object_
        ...
        assert result.dtypes.iloc[0] != np.object_
        result = DataFrame({"a": ser***REMOVED***)
        assert result.dtypes.iloc[0] == np.object_
...
class TestDataFrameConstructorIndexInference:
    def test_frame_from_dict_of_series_overlapping_monthly_period_indexes(self):
        ...
        df = DataFrame({"s1": s1, "s2": s2***REMOVED***)
    ...
    def test_frame_from_dict_with_mixed_tzaware_indexes(self):
        ...
        with pytest.raises(TypeError, match=msg):
            DataFrame({"A": ser2, "B": ser3, "C": ser4, "D": ser1***REMOVED***)
        with pytest.raises(TypeError, match=msg):
            DataFrame({"A": ser2, "B": ser3, "D": ser1***REMOVED***)
        with pytest.raises(TypeError, match=msg):
            DataFrame({"D": ser1, "A": ser2, "B": ser3***REMOVED***)
    ...
    def test_dict_data_arrow_column_expansion(self, key_val, col_vals, col_type):
        ...
        result = DataFrame({key_val: [1, 2]***REMOVED***, columns=cols)
...
class TestDataFrameConstructorWithDatetimeTZ:
    ...
    def test_construction_preserves_tzaware_dtypes(self, tz):
        ...
        df = DataFrame({"A": "foo", "B": dr_tz***REMOVED***, index=dr)
    ...
    def test_constructor_data_aware_dtype_naive(self, tz_aware_fixture, pydt):
        ...
        with pytest.raises(ValueError, match=msg):
            DataFrame({0: [ts]***REMOVED***, dtype="datetime64[ns]")

        msg2 = "Cannot unbox tzaware Timestamp to tznaive dtype"
        with pytest.raises(TypeError, match=msg2):
            DataFrame({0: ts***REMOVED***, index=[0], dtype="datetime64[ns]")
    ...
    def test_from_dict(self):
        ...
        # construction
        df = DataFrame({"A": idx, "B": dr***REMOVED***)
    ...
    def test_frame_dict_constructor_datetime64_1680(self):
        ...
        # it works!
        DataFrame({"a": "foo", "b": s***REMOVED***, index=dr)
        DataFrame({"a": "foo", "b": s.values***REMOVED***, index=dr)
    ...
    def test_frame_datetime64_mixed_index_ctor_1681(self):
        ...
        # it works!
        d = DataFrame({"A": "foo", "B": ts***REMOVED***, index=dr)
        assert d["B"].isna().all()
    ...
    def test_nested_dict_construction(self):
        ...
        columns = ["Nevada", "Ohio"]
        pop = {
            "Nevada": {2001: 2.4, 2002: 2.9***REMOVED***,
            "Ohio": {2000: 1.5, 2001: 1.7, 2002: 3.6***REMOVED***,
        ***REMOVED***
    ...
    def test_construction_from_set_raises(self, typ):
        ...
        values = typ({1, 2, 3***REMOVED***)
        msg = f"'{typ.__name__***REMOVED***' type is unordered"
        with pytest.raises(TypeError, match=msg):
            DataFrame({"a": values***REMOVED***)
    ...
    def test_columns_indexes_raise_on_sets(self):
        ...
        with pytest.raises(ValueError, match="index cannot be a set"):
            DataFrame(data, index={"a", "b"***REMOVED***)
        with pytest.raises(ValueError, match="columns cannot be a set"):
            DataFrame(data, columns={"a", "b", "c"***REMOVED***)
        ...
        if box is None:
            ...
            if frame_or_series is Series:
                return lambda x, **kwargs: frame_or_series(
                    {0: x, 1: x***REMOVED***, **extra, **kwargs
    ***REMOVED***
            else:
                return lambda x, **kwargs: frame_or_series({"A": x***REMOVED***, **extra, **kwargs)
        elif frame_or_series is Series:
            return lambda x, **kwargs: frame_or_series([x, x], **extra, **kwargs)
        else:
            return lambda x, **kwargs: frame_or_series({"A": [x, x]***REMOVED***, **extra, **kwargs)
    ...
    def test_from_scalar_datetimelike_mismatched(self, constructor, cls):
        scalar = cls("NaT", "ns")
        dtype = {np.datetime64: "m8[ns]", np.timedelta64: "M8[ns]"***REMOVED***[cls]
