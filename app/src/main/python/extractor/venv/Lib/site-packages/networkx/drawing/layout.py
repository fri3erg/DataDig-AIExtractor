...
def circular_layout(G, scale=1, center=None, dim=2):
    ...
    if len(G) == 0:
        ...
        pos = {nx.utils.arbitrary_element(G): center***REMOVED***
    else:
...
def shell_layout(G, nlist=None, rotate=None, scale=1, center=None, dim=2):
    ...
    if len(G) == 0:
        return {***REMOVED***
    if len(G) == 1:
        return {nx.utils.arbitrary_element(G): center***REMOVED***
...
def spring_layout(
    ...
    if fixed is not None:
        ...
        for node in fixed:
            if node not in pos:
                raise ValueError("nodes are fixed without positions given")
        nfixed = {node: i for i, node in enumerate(G)***REMOVED***
    ...
    if len(G) == 0:
        return {***REMOVED***
    if len(G) == 1:
        return {nx.utils.arbitrary_element(G.nodes()): center***REMOVED***
...
def arf_layout(
    ...
    node_order = {node: i for i, node in enumerate(G)***REMOVED***
    for x, y in G.edges():
        if x != y:
...
def rescale_layout_dict(pos, scale=1):
    ...
    --------
    >>> import numpy as np
    >>> pos = {0: np.array((0, 0)), 1: np.array((1, 1)), 2: np.array((0.5, 0.5))***REMOVED***
    >>> nx.rescale_layout_dict(pos)
    {0: array([-1., -1.]), 1: array([1., 1.]), 2: array([0., 0.])***REMOVED***

    >>> pos = {0: np.array((0, 0)), 1: np.array((-1, 1)), 2: np.array((-0.5, 0.5))***REMOVED***
    >>> nx.rescale_layout_dict(pos, scale=2)
    {0: array([ 2., -2.]), 1: array([-2.,  2.]), 2: array([0., 0.])***REMOVED***
