...
class TestUnique:
    ...
    def test_order_of_appearance_dt64(self, unit):
        ...
        result = pd.unique(ser)
        expected = np.array(["2016-01-01T00:00:00.000000000"], dtype=f"M8[{unit***REMOVED***]")
        tm.assert_numpy_array_equal(result, expected)
    ...
    def test_order_of_appearance_dt64tz(self, unit):
        ...
        expected = DatetimeIndex(
            ["2016-01-01 00:00:00"], dtype=f"datetime64[{unit***REMOVED***, US/Eastern]", freq=None
        )
        tm.assert_index_equal(result, expected)
...
class TestIsin:
    ...
    def test_basic(self):
        ...
        result = algos.isin(Series([1, 2]), {1***REMOVED***)
        expected = np.array([True, False])
        tm.assert_numpy_array_equal(result, expected)
        ...
        result = algos.isin(Series(["a", "b"]), {"a"***REMOVED***)
        expected = np.array([True, False])
        tm.assert_numpy_array_equal(result, expected)
    ...
    def test_isin_int_df_string_search(self):
        ...
        df = DataFrame({"values": [1, 2]***REMOVED***)
        result = df.isin(["1"])
        expected_false = DataFrame({"values": [False, False]***REMOVED***)
    ...
    def test_isin_nan_df_string_search(self):
        ...
        df = DataFrame({"values": [np.nan, 2]***REMOVED***)
    ...
    def test_isin_float_df_string_search(self):
        ...
        df = DataFrame({"values": [1.4245, 2.32441]***REMOVED***)
...
class TestValueCounts:
    ...
    def test_value_counts_nat(self):
        ...
        exp_dt = Series({Timestamp("2014-01-01 00:00:00"): 1***REMOVED***, name="count")
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result_dt = algos.value_counts(dt)
        tm.assert_series_equal(result_dt, exp_dt)

        exp_td = Series({np.timedelta64(10000): 1***REMOVED***, name="count")
        with tm.assert_produces_warning(FutureWarning, match=msg):
...
class TestHashTable:
    @pytest.mark.parametrize(
        "htable, data",
        [
            (ht.PyObjectHashTable, [f"foo_{i***REMOVED***" for i in range(1000)]),
            (ht.StringHashTable, [f"foo_{i***REMOVED***" for i in range(1000)]),
