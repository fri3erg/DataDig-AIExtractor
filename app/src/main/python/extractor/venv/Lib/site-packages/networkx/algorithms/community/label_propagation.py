...
def fast_label_propagation_communities(G, *, weight=None, seed=None):
    ...
    comms = {node: i for i, node in enumerate(G)***REMOVED***
...
def asyn_lpa_communities(G, weight=None, seed=None):
    ...
    labels = {n: i for i, n in enumerate(G)***REMOVED***
    cont = True
...
def label_propagation_communities(G):
    ...
    labeling = {v: k for k, v in enumerate(G)***REMOVED***
...
def _color_network(G):
    ...
    """
    coloring = {***REMOVED***  # color => set(node)
    colors = nx.coloring.greedy_color(G)
    for node, color in colors.items():
        if color in coloring:
            coloring[color].add(node)
        else:
            coloring[color] = {node***REMOVED***
    return coloring
...
def _most_frequent_labels(node, labeling, G):
    ...
    if not G[node]:
        ...
        return {labeling[node]***REMOVED***

    # Compute the frequencies of all neighbours of node
    freqs = Counter(labeling[q] for q in G[node])
    max_freq = max(freqs.values())
    return {label for label, freq in freqs.items() if freq == max_freq***REMOVED***
