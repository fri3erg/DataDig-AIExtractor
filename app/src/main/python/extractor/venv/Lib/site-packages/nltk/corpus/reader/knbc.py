    ...
    def _parse(self, t):
        ...
        for line in t.splitlines():
            if line[0] in "*+":
                ...
                node = dg.nodes[i]
                node.update({"address": i, "rel": m.group(2), "word": []***REMOVED***)
...
def demo():
    ...
    knbc.morphs2str = lambda morphs: "/".join(
        "{***REMOVED***({***REMOVED***)".format(m[0], m[1].split(" ")[2]) for m in morphs if m[0] != "EOS"
    ).encode("utf-8")
    ...
    print(
        "\n".join(
            " ".join("{***REMOVED***/{***REMOVED***".format(w[0], w[1].split(" ")[2]) for w in sent)
