...
class TestSeriesConcat:
    ...
    def test_concat_series_axis1_preserves_series_names(self):
        ...
        expected = DataFrame({"A": s, "B": s2***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_concat_series_axis1_with_reindex(self, sort):
        ...
        expected = DataFrame({"A": s, "B": s2***REMOVED***, index=["c", "a", "b", "d"])
        if sort:
    ...
    def test_concat_series_name_npscalar_tuple(self, s1name, s2name):
        # GH21015
        s1 = Series({"a": 1, "b": 2***REMOVED***, name=s1name)
        s2 = Series({"c": 5, "d": 6***REMOVED***, name=s2name)
        result = concat([s1, s2])
        expected = Series({"a": 1, "b": 2, "c": 5, "d": 6***REMOVED***)
        tm.assert_series_equal(result, expected)
    ...
    def test_concat_series_partial_columns_names(self):
        ...
        expected = DataFrame(
            {"foo": [1, 2], 0: [1, 2], 1: [4, 5]***REMOVED***, columns=["foo", 0, 1]
        )
        tm.assert_frame_equal(result, expected)
        ...
        expected = DataFrame(
            {"red": [1, 2], "blue": [1, 2], "yellow": [4, 5]***REMOVED***,
        ...
        expected = DataFrame({0: [1, 2], 1: [1, 2], 2: [4, 5]***REMOVED***)
        tm.assert_frame_equal(result, expected)
