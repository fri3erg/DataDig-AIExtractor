...
class TestFillNA:
    def test_fillna_dict_inplace_nonunique_columns(
        ...
        df = DataFrame(
            {"A": [np.nan] * 3, "B": [NaT, Timestamp(1), NaT], "C": [np.nan, "foo", 2]***REMOVED***
        )
        df.columns = ["A", "A", "A"]
        orig = df[:]
        ...
        with tm.assert_cow_warning(warn_copy_on_write):
            df.fillna({"A": 2***REMOVED***, inplace=True)
        # The first and third columns can be set inplace, while the second cannot.

        expected = DataFrame(
            {"A": [2.0] * 3, "B": [2, Timestamp(1), 2], "C": [2, "foo", 2]***REMOVED***
        )
    ...
    def test_fillna_different_dtype(self, using_infer_string):
        ...
        if using_infer_string:
            with tm.assert_produces_warning(FutureWarning, match="Downcasting"):
                result = df.fillna({2: "foo"***REMOVED***)
        else:
            result = df.fillna({2: "foo"***REMOVED***)
        expected = DataFrame(
        ...
        if using_infer_string:
            with tm.assert_produces_warning(FutureWarning, match="Downcasting"):
                return_value = df.fillna({2: "foo"***REMOVED***, inplace=True)
        else:
            return_value = df.fillna({2: "foo"***REMOVED***, inplace=True)
        tm.assert_frame_equal(df, expected)
    ...
    def test_fillna_datelike(self):
        ...
        result = df.fillna(value={"Date": df["Date2"]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    def test_fillna_tzaware(self):
        # with timezone
        # GH#15855
        df = DataFrame({"A": [Timestamp("2012-11-11 00:00:00+01:00"), NaT]***REMOVED***)
        exp = DataFrame(
            {
                "A": [
        ...
        df = DataFrame({"A": [NaT, Timestamp("2012-11-11 00:00:00+01:00")]***REMOVED***)
        exp = DataFrame(
            {
                "A": [
                    Timestamp("2012-11-11 00:00:00+01:00"),
    ...
    def test_na_actions_categorical(self):
        ...
        vals = ["a", "b", np.nan, "d"]
        df = DataFrame({"cats": cat, "vals": vals***REMOVED***)
        cat2 = Categorical([1, 2, 3, 3], categories=[1, 2, 3])
        vals2 = ["a", "b", "b", "d"]
        df_exp_fill = DataFrame({"cats": cat2, "vals": vals2***REMOVED***)
        cat3 = Categorical([1, 2, 3], categories=[1, 2, 3])
        vals3 = ["a", "b", np.nan]
        df_exp_drop_cats = DataFrame({"cats": cat3, "vals": vals3***REMOVED***)
        cat4 = Categorical([1, 2], categories=[1, 2, 3])
        vals4 = ["a", "b"]
        df_exp_drop_all = DataFrame({"cats": cat4, "vals": vals4***REMOVED***)

        # fillna
        res = df.fillna(value={"cats": 3, "vals": "b"***REMOVED***)
        tm.assert_frame_equal(res, df_exp_fill)

        msg = "Cannot setitem on a Categorical with a new category"
        with pytest.raises(TypeError, match=msg):
            df.fillna(value={"cats": 4, "vals": "c"***REMOVED***)
        ...
        df = DataFrame({"cats": c, "vals": [1, 2, 3]***REMOVED***)

        cat_exp = Categorical(["a", "b", "a"], categories=["a", "b"])
        df_exp = DataFrame({"cats": cat_exp, "vals": [1, 2, 3]***REMOVED***)
    ...
    def test_fillna_categorical_nan(self):
        ...
        df = DataFrame({"cats": cat, "vals": val***REMOVED***)

        # GH#32950 df.median() is poorly behaved because there is no
        #  Categorical.median
        median = Series({"cats": 2.0, "vals": np.nan***REMOVED***)

        res = df.fillna(median)
        v_exp = [np.nan, np.nan, np.nan]
        df_exp = DataFrame({"cats": [2, 2, 2], "vals": v_exp***REMOVED***, dtype="category")
        tm.assert_frame_equal(res, df_exp)
        ...
        df = DataFrame({"a": Categorical(idx)***REMOVED***)
    ...
    def test_fillna_downcast(self):
        ...
        df = DataFrame({"a": [1.0, np.nan]***REMOVED***)
        msg = "The 'downcast' keyword in fillna is deprecated"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = df.fillna(0, downcast="infer")
        expected = DataFrame({"a": [1, 0]***REMOVED***)
        tm.assert_frame_equal(result, expected)

        # infer int64 from float64 when fillna value is a dict
        df = DataFrame({"a": [1.0, np.nan]***REMOVED***)
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = df.fillna({"a": 0***REMOVED***, downcast="infer")
        expected = DataFrame({"a": [1, 0]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_fillna_dtype_conversion_equiv_replace(self, val):
        df = DataFrame({"A": [1, np.nan], "B": [1.0, 2.0]***REMOVED***)
    ...
    def test_fillna_inplace(self):
        ...
        expected = df.fillna(value={0: 0***REMOVED***, inplace=True)
    ...
    def test_fillna_dict_series(self):
        ...
        result = df.fillna({"a": 0, "b": 5***REMOVED***)
        ...
        # it works
        result = df.fillna({"a": 0, "b": 5, "d": 7***REMOVED***)
    ...
    def test_fillna_invalid_value(self, float_frame):
        # list
        msg = '"value" parameter must be a scalar or dict, but you passed a "{***REMOVED***"'
    ...
    def test_fillna_downcast_dict(self):
        # GH#40809
        df = DataFrame({"col1": [1, np.nan]***REMOVED***)

        msg = "The 'downcast' keyword in fillna"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = df.fillna({"col1": 2***REMOVED***, downcast={"col1": "int64"***REMOVED***)
        expected = DataFrame({"col1": [1, 2]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    @pytest.mark.parametrize("val", [-1, {"x": -1, "y": -1***REMOVED***])
    def test_inplace_dict_update_view(
        self, val, using_copy_on_write, warn_copy_on_write
    ):
        # GH#47188
        df = DataFrame({"x": [np.nan, 2], "y": [np.nan, 2]***REMOVED***)
        df_orig = df.copy()
        result_view = df[:]
        with tm.assert_cow_warning(warn_copy_on_write):
            df.fillna(val, inplace=True)
        expected = DataFrame({"x": [-1, 2.0], "y": [-1.0, 2]***REMOVED***)
        tm.assert_frame_equal(df, expected)
        if using_copy_on_write:
    ...
    def test_fillna_with_multi_index_frame(self):
        ...
        tm.assert_frame_equal(pdf.fillna({"x": -1***REMOVED***), expected)
        tm.assert_frame_equal(pdf.fillna({"x": -1, ("x", "b"): -2***REMOVED***), expected)
        ...
        tm.assert_frame_equal(pdf.fillna({("x", "b"): -2, "x": -1***REMOVED***), expected)
...
def test_fillna_nones_inplace():
    ...
    msg = "Downcasting object dtype arrays"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        df.fillna(value={"A": 1, "B": 2***REMOVED***, inplace=True)
...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            "ffill",
            {"limit_area": "inside"***REMOVED***,
        ),
        (
            [np.nan, np.nan, 3, np.nan, np.nan, np.nan, 7, np.nan, np.nan],
            [np.nan, np.nan, 3.0, 3.0, np.nan, np.nan, 7.0, np.nan, np.nan],
            "ffill",
            {"limit_area": "inside", "limit": 1***REMOVED***,
        ),
        (
            [np.nan, np.nan, 3, np.nan, np.nan, np.nan, 7, np.nan, np.nan],
            [np.nan, np.nan, 3.0, np.nan, np.nan, np.nan, 7.0, 7.0, 7.0],
            "ffill",
            {"limit_area": "outside"***REMOVED***,
        ),
        (
            [np.nan, np.nan, 3, np.nan, np.nan, np.nan, 7, np.nan, np.nan],
            [np.nan, np.nan, 3.0, np.nan, np.nan, np.nan, 7.0, 7.0, np.nan],
            "ffill",
            {"limit_area": "outside", "limit": 1***REMOVED***,
        ),
        (
