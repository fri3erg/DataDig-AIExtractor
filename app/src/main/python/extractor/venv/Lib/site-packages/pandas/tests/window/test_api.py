...
def tests_skip_nuisance(step):
    df = DataFrame({"A": range(5), "B": range(5, 10), "C": "foo"***REMOVED***)
    r = df.rolling(window=3, step=step)
    result = r[["A", "B"]].sum()
    expected = DataFrame(
        {"A": [np.nan, np.nan, 3, 6, 9], "B": [np.nan, np.nan, 18, 21, 24]***REMOVED***,
        columns=list("AB"),
    )[::step]
...
def test_agg(step):
    df = DataFrame({"A": range(5), "B": range(0, 10, 2)***REMOVED***)
    ...
    with tm.assert_produces_warning(FutureWarning, match="using Rolling.[mean|std]"):
        result = r.aggregate({"A": np.mean, "B": np.std***REMOVED***)

    expected = concat([a_mean, b_std], axis=1)
    tm.assert_frame_equal(result, expected, check_like=True)

    result = r.aggregate({"A": ["mean", "std"]***REMOVED***)
    ...
    with pytest.raises(SpecificationError, match=msg):
        ...
        r.aggregate({"A": {"mean": "mean", "sum": "sum"***REMOVED******REMOVED***)

    with pytest.raises(SpecificationError, match=msg):
        r.aggregate(
            {"A": {"mean": "mean", "sum": "sum"***REMOVED***, "B": {"mean2": "mean", "sum2": "sum"***REMOVED******REMOVED***
        )

    result = r.aggregate({"A": ["mean", "std"], "B": ["mean", "std"]***REMOVED***)
    expected = concat([a_mean, a_std, b_mean, b_std], axis=1)
...
def test_agg_apply(raw):
    ...
    with tm.assert_produces_warning(FutureWarning, match="using Rolling.[sum|std]"):
        result = r.agg({"A": np.sum, "B": lambda x: np.std(x, ddof=1)***REMOVED***)
...
def test_agg_consistency(step):
    ...
    with tm.assert_produces_warning(FutureWarning, match="using Rolling.[sum|mean]"):
        result = r.agg({"A": [np.sum, np.mean]***REMOVED***).columns
...
def test_agg_nested_dicts():
    ...
    msg = "nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        r.aggregate({"r1": {"A": ["mean", "sum"]***REMOVED***, "r2": {"B": ["mean", "sum"]***REMOVED******REMOVED***)
    ...
    with pytest.raises(SpecificationError, match=msg):
        r[["A", "B"]].agg({"A": {"ra": ["mean", "std"]***REMOVED***, "B": {"rb": ["mean", "std"]***REMOVED******REMOVED***)

    with pytest.raises(SpecificationError, match=msg):
        r.agg({"A": {"ra": ["mean", "std"]***REMOVED***, "B": {"rb": ["mean", "std"]***REMOVED******REMOVED***)
...
def test_multiple_agg_funcs(func, window_size, expected_vals):
    ...
    result = window.agg({"low": ["mean", "max"], "high": ["mean", "min"]***REMOVED***)
...
def test_dont_modify_attributes_after_methods(
    ...
    expected = {attr: getattr(roll_obj, attr) for attr in roll_obj._attributes***REMOVED***
    getattr(roll_obj, arithmetic_win_operators)()
    result = {attr: getattr(roll_obj, attr) for attr in roll_obj._attributes***REMOVED***
