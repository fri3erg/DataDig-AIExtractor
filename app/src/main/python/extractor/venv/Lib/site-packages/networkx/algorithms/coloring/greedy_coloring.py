...
def strategy_saturation_largest_first(G, colors):
    ...
    """
    distinct_colors = {v: set() for v in G***REMOVED***
    ...
    while len(G) != len(colors):
        ...
        saturation = {v: len(c) for v, c in distinct_colors.items() if v not in colors***REMOVED***
...
def greedy_color(G, strategy="largest_first", interchange=False):
    ...
    >>> d in [{0: 0, 1: 1, 2: 0, 3: 1***REMOVED***, {0: 1, 1: 0, 2: 1, 3: 0***REMOVED***]
    True
    ...
    if not callable(strategy):
        raise nx.NetworkXError(
            "strategy must be callable or a valid string. " f"{strategy***REMOVED*** not valid."
        )
    ...
    for u in nodes:
        ...
        neighbour_colors = {colors[v] for v in G[u] if v in colors***REMOVED***
...
class _Node:
    ...
    def __repr__(self):
        return (
            f"Node_id: {self.node_id***REMOVED***, Color: {self.color***REMOVED***, "
            f"Adj_list: ({self.adj_list***REMOVED***), adj_color: ({self.adj_color***REMOVED***)"
        )
...
class _AdjEntry:
    ...
    def __repr__(self):
        ...
        return (
            f"Node_id: {self.node_id***REMOVED***, Next: ({self.next***REMOVED***), "
            f"Mate: ({self.mate.node_id***REMOVED***), "
            f"col_next: ({col_next***REMOVED***), col_prev: ({col_prev***REMOVED***)"
        )
...
def _greedy_coloring_with_interchange(G, nodes):
    ...
    graph = {node: _Node(node, n) for node in G***REMOVED***
    ...
    for node in nodes:
        ...
        col_used = {graph[adj_node.node_id].color for adj_node in neighbors***REMOVED***
        col_used.discard(-1)
    ...
    return {node.node_id: node.color for node in graph.values()***REMOVED***
