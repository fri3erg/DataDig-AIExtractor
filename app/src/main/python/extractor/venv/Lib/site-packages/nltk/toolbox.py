...
class ToolboxData(StandardFormat):
    ...
    def _record_parse(self, key=None, **kwargs):
        ...
        """
        builder = TreeBuilder()
        builder.start("toolbox_data", {***REMOVED***)
        builder.start("header", {***REMOVED***)
        in_records = False
        for mkr, value in self.fields(**kwargs):
            ...
            if mkr == key:
                if in_records:
                    ...
                    builder.end("header")
                    in_records = True
                builder.start("record", {***REMOVED***)
            builder.start(mkr, {***REMOVED***)
            builder.data(value)
            builder.end(mkr)
        if in_records:
            builder.end("record")
        ...
        for field in rec:
            ...
            if encoding is not None:
                ...
                if re.search(_is_value, value):
                    l.append((f"\\{mkr***REMOVED*** {value***REMOVED***\n").encode(cur_encoding, errors))
                else:
                    l.append((f"\\{mkr***REMOVED***{value***REMOVED***\n").encode(cur_encoding, errors))
            else:
                if re.search(_is_value, value):
...
def _to_settings_string(node, l, **kwargs):
    ...
    if len(node) == 0:
        if text:
            l.append(f"\\{tag***REMOVED*** {text***REMOVED***\n")
        else:
            l.append("\\%s\n" % tag)
    else:
        if text:
