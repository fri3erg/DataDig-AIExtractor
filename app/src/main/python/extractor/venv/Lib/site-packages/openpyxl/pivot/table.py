...
class ConditionalFormatList(Serialisable):
    ...
    def _dedupe(self):
        ...
        * Data fields are referenced by the 0-index reference.x.v value
        ...
        fmts = {field:fmt for (field, priority), fmt in sorted(fmts.items(), reverse=True)***REMOVED***
        #fmts = {field:fmt for (field, priority), fmt in fmts.items()***REMOVED***
        if fmts:
...
class TableDefinition(Serialisable):
    ...
    _id = 1
    _path = "/xl/pivotTables/pivotTable{0***REMOVED***.xml"
    ...
    ***REMOVED***:
    ...
    def summary(self):
        ...
        return f"{self.name***REMOVED*** {dict(self.location)***REMOVED***"
