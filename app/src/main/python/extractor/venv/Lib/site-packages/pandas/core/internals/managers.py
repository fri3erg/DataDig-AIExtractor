...
class BaseBlockManager(DataManager):
    ...
    def apply(
        ...
        **kwargs
        ...
        aligned_args = {k: kwargs[k] for k in align_keys***REMOVED***
    ...
    def setitem(self, indexer, value, warn: bool = True) -> Self:
        ...
        if isinstance(indexer, np.ndarray) and indexer.ndim > self.ndim:
            raise ValueError(f"Cannot set values with ndim > {self.ndim***REMOVED***")
            ...
            for block in blocks:
                if self.ndim != block.ndim:
                    raise AssertionError(
                        f"Number of Block dimensions ({block.ndim***REMOVED***) must equal "
                        f"number of axes ({self.ndim***REMOVED***)"
        ***REMOVED***
    ...
    def _verify_integrity(self) -> None:
        ...
        if len(self.items) != tot_items:
            raise AssertionError(
                ...
                f"block items\n# manager items: {len(self.items)***REMOVED***, # "
                f"tot_items: {tot_items***REMOVED***"
***REMOVED***
...
***REMOVED***
        ...
        if value.ndim == 2:
            ...
            if len(value) > 1:
                raise ValueError(
                    f"Expected a 1D array, got an array with shape {value.T.shape***REMOVED***"
    ***REMOVED***
        else:
...
***REMOVED***
    ...
    def to_dict(self) -> dict[str, Self]:
        ...
        bd: dict[str, list[Block]] = {***REMOVED***
        for b in self.blocks:
            bd.setdefault(str(b.dtype), []).append(b)

        # TODO(EA2D): the combine will be unnecessary with 2D EAs
        return {dtype: self._combine(blocks) for dtype, blocks in bd.items()***REMOVED***
    ...
    def __getstate__(self):
        ...
        extra_state = {
            "0.14.1": {
                "axes": axes_array,
                "blocks": [
                    {"values": b.values, "mgr_locs": b.mgr_locs.indexer***REMOVED***
                    for b in self.blocks
                ],
            ***REMOVED***
        ***REMOVED***
...
def raise_construction_error(
    ...
    raise ValueError(f"Shape of passed values is {passed***REMOVED***, indices imply {implied***REMOVED***")
...
def make_na_array(dtype: DtypeObj, shape: Shape, fill_value) -> ArrayLike:
    if isinstance(dtype, DatetimeTZDtype):
        ...
        dt64values = i8values.view(f"M8[{dtype.unit***REMOVED***]")
