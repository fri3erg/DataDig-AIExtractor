...
try:
    ...
    raise ImportError(
        f"aiohttp dependency is not installed: {ie***REMOVED***. "
...
@click.command(context_settings={"help_option_names": ["-h", "--help"]***REMOVED***)
@click.option(
    "--bind-host",
    type=str,
...
def main(bind_host: str, bind_port: int) -> None:
    ...
    black.out(f"blackd version {ver***REMOVED*** listening on {bind_host***REMOVED*** port {bind_port***REMOVED***")
...
async def handle(request: web.Request, executor: Executor) -> web.Response:
    headers = {BLACK_VERSION_HEADER: __version__***REMOVED***
    try:
...
***REMOVED***
        ...
        if only_diff:
            ...
            src_name = f"In\t{then***REMOVED***"
            dst_name = f"Out\t{now***REMOVED***"
            loop = asyncio.get_event_loop()
            formatted_str = await loop.run_in_executor(
                executor,
...
def parse_mode(headers: MultiMapping[str]) -> black.Mode:
    ...
    if PYTHON_VARIANT_HEADER in headers:
        ...
        try:
            ...
            raise HeaderError(
                f"Invalid value for {PYTHON_VARIANT_HEADER***REMOVED***: {e.args[0]***REMOVED***",
***REMOVED*** from None
    else:
        pyi = False
        versions = set()
    ...
    for piece in enable_unstable_features:
        ...
        if piece:
            try:
                ...
                raise HeaderError(
                    f"Invalid value for {ENABLE_UNSTABLE_FEATURE***REMOVED***: {piece***REMOVED***",
    ***REMOVED*** from None
    ...
    if value == "pyi":
        ...
        for version in value.split(","):
            ...
            try:
                ...
                version_str = f"PY{major***REMOVED***{minor***REMOVED***"
                if major == 3 and not hasattr(black.TargetVersion, version_str):
                    raise InvalidVariantHeader(f"3.{minor***REMOVED*** is not supported")
