    ...
    ***REMOVED***
    ...
    def get_xml_type(self, key):
        ...
        try:
            ...
            raise TypeError(
                f"GraphML does not support type {type(key)***REMOVED*** as data values."
***REMOVED*** from err
...
class GraphMLWriter(GraphML):
    ...
    def attr_type(self, name, scope, value):
        ...
        if self.infer_numeric_types:
            ...
            if len(types) > 1:
                types = {self.get_xml_type(t) for t in types***REMOVED***
                if "string" in types:
                    return str
    ...
    def get_key(self, name, attr_type, scope, default):
        ...
        try:
            ...
            if self.named_key_ids:
                ...
                new_id = f"d{len(list(self.keys))***REMOVED***"
    ...
    def add_data(self, name, element_type, value, scope="all", default=None):
        ...
        if element_type not in self.xml_type:
            raise nx.NetworkXError(
                f"GraphML writer does not support {element_type***REMOVED*** as data values."
***REMOVED***
...
***REMOVED***
        ...
        graphdata = {
            ...
            if k not in ("node_default", "edge_default")
        ***REMOVED***
        node_default = G.graph.get("node_default", {***REMOVED***)
        edge_default = G.graph.get("edge_default", {***REMOVED***)
        # Graph attributes
        ...
        with graph_element:
            self.add_attributes("graph", incremental_writer, graphdata, {***REMOVED***)
...
class GraphMLReader(GraphML):
    ...
    def __init__(self, node_type=str, edge_key_type=int, force_multigraph=False):
        ...
        self.edge_ids = {***REMOVED***  # dict mapping (u,v) tuples to edge id attributes
    ...
    def __call__(self, path=None, string=None):
        ...
        for g in self.xml.findall(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***graph"):
    ...
    def make_graph(self, graph_xml, graphml_keys, defaults, G=None):
        ...
        G.graph["node_default"] = {***REMOVED***
        G.graph["edge_default"] = {***REMOVED***
        for key_id, value in defaults.items():
            key_for = graphml_keys[key_id]["for"]
            name = graphml_keys[key_id]["name"]
            python_type = graphml_keys[key_id]["type"]
            if key_for == "node":
                G.graph["node_default"].update({name: python_type(value)***REMOVED***)
            if key_for == "edge":
                G.graph["edge_default"].update({name: python_type(value)***REMOVED***)
        # hyperedges are not supported
        hyperedge = graph_xml.find(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***hyperedge")
        if hyperedge is not None:
            raise nx.NetworkXError("GraphML reader doesn't support hyperedges")
        # add nodes
        for node_xml in graph_xml.findall(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***node"):
            self.add_node(G, node_xml, graphml_keys, defaults)
        # add edges
        for edge_xml in graph_xml.findall(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***edge"):
    ...
    def add_node(self, G, node_xml, graphml_keys, defaults):
        ...
        ports = node_xml.find(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***port")
        ...
        if node_xml.attrib.get("yfiles.foldertype") == "group":
            graph_xml = node_xml.find(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***graph")
            self.make_graph(graph_xml, graphml_keys, defaults, G)

    def add_edge(self, G, edge_element, graphml_keys):
        """Add an edge to the graph."""
        # warn on finding unsupported ports tag
        ports = edge_element.find(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***port")
        if ports is not None:
            warnings.warn("GraphML port tag not supported.")
    ...
    def decode_data_elements(self, graphml_keys, obj_xml):
        ...
        data = {***REMOVED***
        for data_element in obj_xml.findall(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***data"):
            key = data_element.get("key")
            try:
                data_name = graphml_keys[key]["name"]
                data_type = graphml_keys[key]["type"]
            except KeyError as err:
                raise nx.NetworkXError(f"Bad GraphML data: no key {key***REMOVED***") from err
            ...
            if text is not None and len(list(data_element)) == 0:
                ...
                gn = data_element.find(f"{{{self.NS_Y***REMOVED******REMOVED******REMOVED***GenericNode")
                if gn:
                    data["shape_type"] = gn.get("configuration")
                for node_type in ["GenericNode", "ShapeNode", "SVGNode", "ImageNode"]:
                    pref = f"{{{self.NS_Y***REMOVED******REMOVED******REMOVED***{node_type***REMOVED***/{{{self.NS_Y***REMOVED******REMOVED******REMOVED***"
                    geometry = data_element.find(f"{pref***REMOVED***Geometry")
                    if geometry is not None:
                        data["x"] = geometry.get("x")
                        data["y"] = geometry.get("y")
                    if node_label is None:
                        node_label = data_element.find(f"{pref***REMOVED***NodeLabel")
                    shape = data_element.find(f"{pref***REMOVED***Shape")
                ...
                for edge_type in [
                    ...
                    pref = f"{{{self.NS_Y***REMOVED******REMOVED******REMOVED***{edge_type***REMOVED***/{{{self.NS_Y***REMOVED******REMOVED******REMOVED***"
                    edge_label = data_element.find(f"{pref***REMOVED***EdgeLabel")
    ...
    def find_graphml_keys(self, graph_element):
        ...
        graphml_keys = {***REMOVED***
        graphml_key_defaults = {***REMOVED***
        for k in graph_element.findall(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***key"):
            attr_id = k.get("id")
            ...
            if attr_type is None:
                attr_type = "string"
                warnings.warn(f"No key type for id {attr_id***REMOVED***. Using string")
            if attr_name is None:
                raise nx.NetworkXError(f"Unknown key for id {attr_id***REMOVED***.")
            ...
            ***REMOVED***
            ...
            default = k.find(f"{{{self.NS_GRAPHML***REMOVED******REMOVED******REMOVED***default")
            if default is not None:
