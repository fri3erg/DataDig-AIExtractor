...
class Expr:
    ...
    def __init__(self, op, data):
        ...
        if op is Op.INTEGER:
            ...
            # data is {<term|base>:<coeff|exponent>***REMOVED*** where dict values
            ...
            raise NotImplementedError(
                f'unknown op or missing sanity check: {op***REMOVED***')
    ...
    def __repr__(self):
        return f'{type(self).__name__***REMOVED***({self.op***REMOVED***, {self.data!r***REMOVED***)'
    ...
    def tostring(self, parent_precedence=Precedence.NONE,
        ...
        if self.op in (Op.INTEGER, Op.REAL):
            ...
            r = str(self.data[0]) + (f'_{self.data[1]***REMOVED***'
            ...
            for term, coeff in sorted(self.data.items()):
                ...
                if coeff == 1:
                    ...
                    if term == as_number(1):
                        ...
                        term = f'{coeff***REMOVED*** * ' + term.tostring(
                ...
                if exp == 1:
                    ...
                    if exp in range(2, 10):
                        ...
                        factor = base.tostring(Precedence.TUPLE,
                                               language=language)
                        factor = f'pow({factor***REMOVED***, {exp***REMOVED***)'
                else:
                    factor = base.tostring(Precedence.POWER,
                                           language=language) + f' ** {exp***REMOVED***'
                if factors:
                    factors.append(op)
            ...
            if name is ArithOp.DIV and language is Language.C:
                numer, denom = [arg.tostring(Precedence.PRODUCT,
                                             language=language)
                                for arg in args]
                r = f'{numer***REMOVED*** / {denom***REMOVED***'
                ...
                r = f'{name***REMOVED***({", ".join(args)***REMOVED***)'
                precedence = Precedence.ATOM
            ...
            if language is Language.C:
                r = f'({cond***REMOVED***?{expr1***REMOVED***:{expr2***REMOVED***)'
            elif language is Language.Python:
                r = f'({expr1***REMOVED*** if {cond***REMOVED*** else {expr2***REMOVED***)'
            elif language is Language.Fortran:
                r = f'merge({expr1***REMOVED***, {expr2***REMOVED***, {cond***REMOVED***)'
            else:
                raise NotImplementedError(
                    f'tostring for {self.op***REMOVED*** and {language***REMOVED***')
            precedence = Precedence.ATOM
            ...
            r = f'{left***REMOVED*** {rop***REMOVED*** {right***REMOVED***'
        else:
            raise NotImplementedError(f'tostring for op {self.op***REMOVED***')
    ...
    def __pow__(self, other):
        ...
        if isinstance(other, Expr):
            if other.op is Op.INTEGER:
                ...
                return Expr(Op.FACTORS, {self: exponent***REMOVED***)
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(index, tuple):
            index = index,
        if len(index) > 1:
            ewarn(f'C-index should be a single expression but got `{index***REMOVED***`')
    ...
    def substitute(self, symbols_map):
        ...
        raise NotImplementedError(f'substitute method for {self.op***REMOVED***: {self!r***REMOVED***')
    ...
    def traverse(self, visit, *args, **kwargs):
        ...
        if self.op in (Op.INTEGER, Op.REAL, Op.STRING, Op.SYMBOL):
            ...
            return normalize(Expr(self.op, (rop, left, right)))
        raise NotImplementedError(f'traverse method for {self.op***REMOVED***')
    ...
    def linear_solve(self, symbol):
        ...
        """
        b = self.substitute({symbol: as_number(0)***REMOVED***)
        ax = self - b
        a = ax.substitute({symbol: as_number(1)***REMOVED***)

        zero, _ = as_numer_denom(a * symbol - ax)

        if zero != as_number(0):
            raise RuntimeError(f'not a {symbol***REMOVED***-linear equation:'
                               f' {a***REMOVED*** * {symbol***REMOVED*** + {b***REMOVED*** == {self***REMOVED***')
        return a, b
...
def normalize(obj):
    ...
    if obj.op is Op.FACTORS:
        ...
        if len(d) == 0 or coeff == 0:
            ...
            if coeff == 1:
                return t
            return Expr(Op.TERMS, {t: coeff***REMOVED***)
        elif coeff == 1:
            return Expr(Op.FACTORS, d)
        else:
            return Expr(Op.TERMS, {Expr(Op.FACTORS, d): coeff***REMOVED***)
...
def as_number(obj, kind=4):
    ...
    if isinstance(obj, Expr):
        if obj.op in (Op.INTEGER, Op.REAL):
            return obj
    raise OpError(f'cannot convert {obj***REMOVED*** to INTEGER or REAL constant')
...
def as_integer(obj, kind=4):
    ...
    if isinstance(obj, Expr):
        if obj.op is Op.INTEGER:
            return obj
    raise OpError(f'cannot convert {obj***REMOVED*** to INTEGER constant')
...
def as_real(obj, kind=4):
    ...
    raise OpError(f'cannot convert {obj***REMOVED*** to REAL constant')
...
def as_terms(obj):
    ...
    if isinstance(obj, Expr):
        ...
        if obj.op is Op.TERMS:
            return obj
        if obj.op is Op.INTEGER:
            return Expr(Op.TERMS, {as_integer(1, obj.data[1]): obj.data[0]***REMOVED***)
        if obj.op is Op.REAL:
            return Expr(Op.TERMS, {as_real(1, obj.data[1]): obj.data[0]***REMOVED***)
        return Expr(Op.TERMS, {obj: 1***REMOVED***)
    raise OpError(f'cannot convert {type(obj)***REMOVED*** to terms Expr')
    ...
    if isinstance(obj, Expr):
        ...
        if obj.op is Op.TERMS:
            if len(obj.data) == 1:
                ...
                if coeff == 1:
                    return Expr(Op.FACTORS, {term: 1***REMOVED***)
                return Expr(Op.FACTORS, {term: 1, Expr.number(coeff): 1***REMOVED***)
        if ((obj.op is Op.APPLY
             and obj.data[0] is ArithOp.DIV
             and not obj.data[2])):
            return Expr(Op.FACTORS, {obj.data[1][0]: 1, obj.data[1][1]: -1***REMOVED***)
        return Expr(Op.FACTORS, {obj: 1***REMOVED***)
    raise OpError(f'cannot convert {type(obj)***REMOVED*** to terms Expr')
...
def as_term_coeff(obj):
    ...
    if isinstance(obj, Expr):
        ...
        return obj, 1
    raise OpError(f'cannot convert {type(obj)***REMOVED*** to term and coeff')
        ...
        if obj.op in (Op.INTEGER, Op.REAL, Op.COMPLEX, Op.SYMBOL,
            ...
            for b, e in obj.data.items():
                ...
                if e > 0:
                    ...
                    denom *= bnumer ** (-e)
            return numer, denom
    raise OpError(f'cannot convert {type(obj)***REMOVED*** to numer and denom')
...
def eliminate_quotes(s):
    ...
    def repl(m):
        ...
        if kind:
            ...
            kind = kind[:-1]
        p = {"'": "SINGLE", '"': "DOUBLE"***REMOVED***[value[0]]
        k = f'{kind***REMOVED***@__f2py_QUOTES_{p***REMOVED***_{COUNTER.__next__()***REMOVED***@'
        d[k] = value
        return k

    new_s = re.sub(r'({kind***REMOVED***_|)({single_quoted***REMOVED***|{double_quoted***REMOVED***)'.format(
        kind=r'\w[\w\d_]*',
        single_quoted=r"('([^'\\]|(\\.))*')",
        double_quoted=r'("([^"\\]|(\\.))*")'),
        repl, s)
...
def replace_parenthesis(s):
    ...
    for left_, right_ in (('(/', '/)'),
                          '()',
                          '{***REMOVED***',  # to support C literal structs
                          '[]'):
        i = s.find(left_)
        if i == -1:
            continue
    ...
    while s.count(left, i + 1, j) != s.count(right, i + 1, j):
        j = s.find(right, j + 1)
        if j == -1:
            raise ValueError(f'Mismatch of {left+right***REMOVED*** parenthesis in {s!r***REMOVED***')

    p = {'(': 'ROUND', '[': 'SQUARE', '{': 'CURLY', '(/': 'ROUNDDIV'***REMOVED***[left]

    k = f'@__f2py_PARENTHESIS_{p***REMOVED***_{COUNTER.__next__()***REMOVED***@'
    v = s[i+len(left):j]
...
def unreplace_parenthesis(s, d):
    ...
    for k, v in d.items():
        ...
        right = dict(ROUND=')', SQUARE=']', CURLY='***REMOVED***', ROUNDDIV='/)')[p]
...
def fromstring(s, language=Language.C):
    ...
    if isinstance(r, Expr):
        return r
    raise ValueError(f'failed to parse `{s***REMOVED***` to Expr instance: got `{r***REMOVED***`')
...
class _Pair:
    ...
    def __repr__(self):
        return f'{type(self).__name__***REMOVED***({self.left***REMOVED***, {self.right***REMOVED***)'
...
class _FromStringWorker:
    ...
    def process(self, s, context='expr'):
        ...
        if ',' in r:
            ...
            raise NotImplementedError(
                f'parsing comma-separated list (context={context***REMOVED***): {r***REMOVED***')
        ...
        if r.startswith('*') or r.startswith('&'):
            op = {'*': Op.DEREF, '&': Op.REF***REMOVED***[r[0]]
        ...
        m = re.match(r'\A({digit_string***REMOVED***)({kind***REMOVED***|)\Z'.format(
            digit_string=r'\d+',
            kind=r'_(\d+|\w[\w\d_]*)'), r)
        if m:
        ...
        m = re.match(r'\A({significant***REMOVED***({exponent***REMOVED***|)|\d+{exponent***REMOVED***)({kind***REMOVED***|)\Z'
                     .format(
                         significant=r'[.]\d+|\d+[.]\d*',
                         exponent=r'[edED][+-]?\d+',
                         kind=r'_(\d+|\w[\w\d_]*)'), r)
        ...
        ewarn(
            f'fromstring: treating {r!r***REMOVED*** as symbol (original={self.original***REMOVED***)')
        return as_symbol(r)
