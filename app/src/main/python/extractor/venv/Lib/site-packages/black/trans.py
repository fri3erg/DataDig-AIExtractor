...
def hug_power_op(
    ...
    def is_simple_lookup(index: int, step: Literal[1, -1]) -> bool:
        ...
        if step == -1:
            disallowed = {token.RPAR, token.RSQB***REMOVED***
        else:
            disallowed = {token.LPAR, token.LSQB***REMOVED***

        while 0 <= index < len(line.leaves):
            current = line.leaves[index]
            if current.type in disallowed:
                return False
            if current.type not in {token.NAME, token.DOT***REMOVED*** or current.value == "for":
    ...
    def is_simple_operand(index: int, kind: Literal["base", "exponent"]) -> bool:
        ...
        if start.type in {token.NAME, token.NUMBER***REMOVED***:
            return is_simple_lookup(index, step=(1 if kind == "exponent" else -1))

        if start.type in {token.PLUS, token.MINUS, token.TILDE***REMOVED***:
            if line.leaves[index + 1].type in {token.NAME, token.NUMBER***REMOVED***:
    ...
    def __call__(
        ...
        if isinstance(match_result, Err):
            ...
            raise CannotTransform(
                f"The string transformer {self.__class__.__name__***REMOVED*** does not recognize"
                " this line as one that it can transform."
***REMOVED*** from cant_transform
...
***REMOVED***
    ...
    def _merge_string_group(
        ...
        # A dict of {string_idx: tuple[num_of_strings, string_leaf]***REMOVED***.
        merged_string_idx_dict: Dict[int, Tuple[int, Leaf]] = {***REMOVED***
        for string_idx in string_indices:
...
***REMOVED***:
    ...
    ***REMOVED***
                debug_expressions_contain_visible_quotes = any(
                    re.search(r".*[\'\"].*(?<![!:=])={1***REMOVED***(?!=)(?![^\s:])", expression)
...
***REMOVED***
        ...
        for has_prefix in prefix_tracker:
            ...
            assert (
                mark_idx >= 0
***REMOVED***, "Logic error while filling the custom string breakpoint cache."
    ...
    def _validate_msg(line: Line, string_idx: int) -> TResult[None]:
        ...
        Returns:
            ...
            * Err(CannotTransform), if any of the following are true:
                ...
                - The set of all string prefixes in the string group is of
                  length greater than one and is not equal to {"", "f"***REMOVED***.
        ...
        ***REMOVED***
        ...
        if num_of_strings < 2:
            return TErr(
                f"Not enough strings to merge (num_of_strings={num_of_strings***REMOVED***)."
***REMOVED***

        if num_of_inline_string_comments > 1:
            return TErr(
                f"Too many inline string comments ({num_of_inline_string_comments***REMOVED***)."
***REMOVED***

        if len(set_of_prefixes) > 1 and set_of_prefixes != {"", "f"***REMOVED***:
            return TErr(f"Too many different prefixes ({set_of_prefixes***REMOVED***).")
...
***REMOVED***:
            ...
            if is_valid_index(idx - 2):
                ...
                if token.PERCENT in {leaf.type for leaf in LL[idx - 1 : next_idx]***REMOVED*** and (
                    (
                        before_lpar.type
                        in {
                            token.STAR,
        ...
        ***REMOVED***
                    or (
                        ...
                        and (before_lpar.type in {token.PLUS, token.MINUS***REMOVED***)
        ***REMOVED***
    ***REMOVED***:
                    continue
...
class BaseStringSplitter(StringTransformer):
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        assert len(string_indices) == 1, (
            f"{self.__class__.__name__***REMOVED*** should only find one match at a time, found"
            f" {len(string_indices)***REMOVED***"
        )
...
***REMOVED***
        ...
        if not string_leaf.parent or [L.type for L in string_leaf.parent.children] == [
            ...
            return TErr(
                f"This string ({string_leaf.value***REMOVED***) appears to be pointless (i.e. has"
                " no parent)."
***REMOVED***
...
class StringSplitter(BaseStringSplitter, CustomSplitMapMixin):
    ...
    Transformations:
        ...
        f-expression (e.g. in f"FooBar: {foo() if x else bar()***REMOVED***", {foo() if x
        else bar()***REMOVED*** is an f-expression).
    ...
    def do_transform(
        ...
        if max_break_width < 0:
            yield TErr(
                f"Unable to split {LL[string_idx].value***REMOVED*** at such high of a line depth:"
                f" {line.depth***REMOVED***"
***REMOVED***
            return
...
***REMOVED***:
    ...
    def _iter_nameescape_slices(self, string: str) -> Iterator[Tuple[Index, Index]]:
        ...
        Yields:
            ...
            would result in the splitting of an \\N{...***REMOVED*** expression (which is NOT
            allowed).
        """
        ...
        for idx, c in it:
            ...
            begin = idx - 1  # the position of backslash before \N{...***REMOVED***
            for idx, c in it:
                if c == "***REMOVED***":
                    end = idx
                    break
                ...
                raise RuntimeError(f"{self.__class__.__name__***REMOVED*** LOGIC ERROR!")
            yield begin, end
    ...
    def _normalize_f_string(self, string: str, prefix: str) -> str:
        ...
        Returns:
            * If @string is an f-string that contains no f-expressions, we
            ...
            has been stripped and all double braces (i.e. '{{' or '***REMOVED******REMOVED***') have
            been normalized (i.e. turned into '{' or '***REMOVED***').
                OR
        ...
        if "f" in prefix and not fstring_contains_expr(string):
            ...
            temp = re.sub(r"\{\{", "{", temp)
            temp = re.sub(r"\***REMOVED***\***REMOVED***", "***REMOVED***", temp)
            new_string = temp

            return f"{new_prefix***REMOVED***{new_string***REMOVED***"
        else:
            return string
...
class StringParser:
    ...
    Examples:
        ...
        ```
        x = "Some {***REMOVED***.".format("String") + some_other_string
        ```

        Furthermore, we will assume that `string_idx` is some index such that:
        ```
        assert line.leaves[string_idx].value == "Some {***REMOVED***."
        ```
