...
class Array:
    ...
    def _new(cls, x, /):
        ...
        if x.dtype not in _all_dtypes:
            raise TypeError(
                f"The array_api namespace does not support the dtype '{x.dtype***REMOVED***'"
***REMOVED***
        obj._array = x
        return obj
    ...
    def __repr__(self: Array, /) -> str:
        ...
        """
        suffix = f", dtype={self.dtype.name***REMOVED***)"
        if 0 in self.shape:
            prefix = "empty("
            mid = str(self.shape)
        else:
    ...
    def _check_allowed_dtypes(self, other: bool | int | float | Array, dtype_category: str, op: str) -> Array:
        ...
        if self.dtype not in _dtype_categories[dtype_category]:
            raise TypeError(f"Only {dtype_category***REMOVED*** dtypes are allowed in {op***REMOVED***")
        ...
        if op.startswith("__i"):
            ...
            if res_dtype != self.dtype:
                raise TypeError(
                    f"Cannot perform {op***REMOVED*** with dtypes {self.dtype***REMOVED*** and {other.dtype***REMOVED***"
    ***REMOVED***
...
***REMOVED***:
                raise IndexError(
                    f"Single-axes index {i***REMOVED*** has {type(i)=***REMOVED***, but only "
    ...
    ***REMOVED***
        ...
        if n_ellipsis > 1:
            ...
            if not key_has_mask and n_single_axes < self.ndim:
                raise IndexError(
                    f"{self.ndim=***REMOVED***, but the multi-axes index only specifies "
                    f"{n_single_axes***REMOVED*** dimensions. If this was intentional, "
        ...
        for i, side in zip(single_axes, indexed_shape):
            if isinstance(i, slice):
                if side == 0:
                    ...
                    f_range = f"between -{side***REMOVED*** and {side - 1***REMOVED*** (or None)"
                if i.start is not None:
                    try:
                        ...
                        if not (-side <= start <= side):
                            raise IndexError(
                                f"Slice {i***REMOVED*** contains {start=***REMOVED***, but should be "
                                f"{f_range***REMOVED*** for an axis of size {side***REMOVED*** "
                ...
                if i.stop is not None:
                    try:
                        ...
                        if not (-side <= stop <= side):
                            raise IndexError(
                                f"Slice {i***REMOVED*** contains {stop=***REMOVED***, but should be "
                                f"{f_range***REMOVED*** for an axis of size {side***REMOVED*** "
                ...
                ***REMOVED***
                ...
                if i.dtype in _boolean_dtypes and len(_key) != 1:
                    ...
                    raise IndexError(
                        f"Single-axes index {i***REMOVED*** is a boolean array and "
                        f"{len(key)=***REMOVED***, but masking is only specified in the "
                        "Array API when the array is the sole index."
        ***REMOVED***
                elif i.dtype in _integer_dtypes and i.ndim != 0:
                    raise IndexError(
                        f"Single-axes index {i***REMOVED*** is a non-zero-dimensional "
                        "integer array, but advanced integer indexing is not "
                        "specified in the Array API."
        ***REMOVED***
            elif isinstance(i, tuple):
                raise IndexError(
                    f"Single-axes index {i***REMOVED*** is a tuple, but nested tuple "
    ...
    ***REMOVED***
    ...
    def __array_namespace__(
        ...
        if api_version is not None and not api_version.startswith("2021."):
            raise ValueError(f"Unrecognized array API version: {api_version!r***REMOVED***")
        return array_api
    ...
    def to_device(self: Array, device: Device, /, stream: None = None) -> Array:
        ...
        if device == 'cpu':
            return self
        raise ValueError(f"Unsupported device {device!r***REMOVED***")
