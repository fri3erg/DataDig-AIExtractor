...
class TestFeather:
    ...
    def check_round_trip(self, df, expected=None, write_kwargs={***REMOVED***, **read_kwargs):
        if expected is None:
            expected = df.copy()
    ...
    ***REMOVED***,
    ...
    def test_read_columns_different_order(self):
        # GH 33878
        df = pd.DataFrame({"A": [1, 2], "B": ["x", "y"], "C": [True, False]***REMOVED***)
        expected = df[["B", "A"]]
    ...
    def test_rw_use_threads(self):
        df = pd.DataFrame({"A": np.arange(100000)***REMOVED***)
    ...
    def test_passthrough_keywords(self):
        ...
        self.check_round_trip(df, write_kwargs={"version": 1***REMOVED***)
...
***REMOVED***
    ...
    def test_int_columns_and_index(self):
        df = pd.DataFrame({"a": [1, 2, 3]***REMOVED***, index=pd.Index([3, 4, 5], name="test"))
    ...
    def test_string_inference(self, tmp_path):
        ...
        df = pd.DataFrame(data={"a": ["x", "y"]***REMOVED***)
        df.to_feather(path)
        with pd.option_context("future.infer_string", True):
            result = read_feather(path)
        expected = pd.DataFrame(data={"a": ["x", "y"]***REMOVED***, dtype="string[pyarrow_numpy]")
        tm.assert_frame_equal(result, expected)
