    ...
    def applyto(self, arg):
        ...
        if self.indices & arg.indices:  # if the sets are NOT disjoint
            raise linearlogic.LinearLogicApplicationException(
                f"'{self***REMOVED***' applied to '{arg***REMOVED***'.  Indices are not disjoint."
***REMOVED***
...
***REMOVED***
            ...
            raise linearlogic.LinearLogicApplicationException(
                f"'{self.simplify()***REMOVED***' applied to '{arg.simplify()***REMOVED***'"
***REMOVED*** from e
    ...
    ***REMOVED***
    ...
    def compile(self, counter=None):
        ...
        return new_forms + [
            self.__class__(self.meaning, compiled_glue, {counter.get()***REMOVED***)
        ]
    ...
    def __str__(self):
        ...
        accum = f"{self.meaning***REMOVED*** : {self.glue***REMOVED***"
        if self.indices:
            accum += (
                " : {" + ", ".join(str(index) for index in sorted(self.indices)) + "***REMOVED***"
***REMOVED***
        return accum
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        for line in lines:  # example: 'n : (\\x.(<word> x), (v-or))'
            ...
            parts = line.split(
                " : ", 2
***REMOVED***  # ['verb', '(\\x.(<word> x), ( subj -o f ))', '[subj]']
            ...
            if len(parts) > 1:
                ...
                ***REMOVED***  # add the GlueFormula to the list
                    elif c == ",":
                        if (
                            paren_count == 1
            ***REMOVED***:  # if it's a comma separating the parts of the tuple
                            tuple_comma = i  # then save the index
                    elif c == "#":  # skip comments at the ends of lines
                        if (
                            paren_count != 0
            ***REMOVED***:  # if the line hasn't parsed correctly so far
                            raise RuntimeError(
...
***REMOVED***:  # if not specified for a specific relationship set
                ...
                if supertype:
                    for rels in self[supertype]:
                        ...
                        self[sem][rels].extend(
                            glue_formulas
            ***REMOVED***  # add the glue formulas to every rel entry
                else:
        ...
        ***REMOVED***  # add the glue formulas to every rel entry
                ...
                self[sem][relationships].extend(
                    glue_formulas
    ***REMOVED***  # add the glue entry to the dictionary
...
***REMOVED***
    ...
    def _lookup_semtype_option(self, semtype, node, depgraph):
        ...
        try:
            ...
            for relset_option in set(semtype) - {None***REMOVED***:
                if (
    ...
    def get_glueformulas_from_semtype_entry(
        ...
        for meaning, glue in lookup:
            ...
            if not len(glueformulas):
                ...
                gf.word = f"{word***REMOVED***{len(glueformulas) + 1***REMOVED***"
...
***REMOVED***
    ...
    ***REMOVED***
            ...
            if name == "f":
                ...
                return f"{lbl.upper()***REMOVED***{unique_index***REMOVED***"
            elif name == "a":
    ...
    def lookup_unique(self, rel, node, depgraph):
        ...
        if len(deps) == 0:
            raise KeyError(
                "'{***REMOVED***' doesn't contain a feature '{***REMOVED***'".format(node["word"], rel)
***REMOVED***
        elif len(deps) > 1:
            raise KeyError(
                "'{***REMOVED***' should only have one feature '{***REMOVED***'".format(node["word"], rel)
***REMOVED***
        else:
            return deps[0]
...
***REMOVED***:  # if cur.glue is non-atomic
                for key in atomics:
                    try:
                ...
                ***REMOVED***:  # if the sets of indices are disjoint
                                try:
...
***REMOVED***,
    ...
    for (i, sentence) in enumerate(examples):
        if i == show_example or show_example == -1:
            print(f"[[[Example {i***REMOVED***]]]  {sentence***REMOVED***")
