...
def test_dataframe_from_dict_of_series(
    ...
    s1_orig = s1.copy()
    expected = DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6]***REMOVED***, index=index, columns=columns, dtype=dtype
    )

    result = DataFrame(
        {"a": s1, "b": s2***REMOVED***, index=index, columns=columns, dtype=dtype, copy=False
    )
...
def test_dataframe_from_dict_of_series_with_reindex(dtype):
    ...
    s1 = Series([1, 2, 3])
    s2 = Series([4, 5, 6])
    df = DataFrame({"a": s1, "b": s2***REMOVED***, index=[1, 2, 3], dtype=dtype, copy=False)
...
def test_dataframe_from_dict_of_series_with_dtype(index):
    ...
    s2 = Series([4, 5, 6])
    df = DataFrame({"a": s1, "b": s2***REMOVED***, index=index, dtype="int64", copy=False)
...
def test_frame_from_dict_of_index(using_copy_on_write):
    idx = Index([1, 2, 3])
    expected = idx.copy(deep=True)
    df = DataFrame({"a": idx***REMOVED***, copy=False)
