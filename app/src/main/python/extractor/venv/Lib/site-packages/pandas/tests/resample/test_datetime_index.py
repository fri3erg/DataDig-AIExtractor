...
def test_resample_string_kwargs(series, keyword, value, unit):
    ...
    msg = f"Unsupported value {value***REMOVED*** for `{keyword***REMOVED***`"
    with pytest.raises(ValueError, match=msg):
        series.resample("5min", **({keyword: value***REMOVED***))
...
def test_resample_how_callables(unit):
    ...
    df = DataFrame({"A": data, "B": data***REMOVED***, index=ind)
...
def test_resample_rounding(unit):
    ...
    df = DataFrame({"value": [1] * 19***REMOVED***, index=pd.to_datetime(ts))
    df.index = df.index.as_unit(unit)

    result = df.resample("6s").sum()
    expected = DataFrame(
        {"value": [4, 9, 4, 2]***REMOVED***,
...
def test_resample_extra_index_point(unit):
    ...
    expected = DataFrame({"A": Series([21, 41, 63], index=index)***REMOVED***)

    index = date_range(start="20150101", end="20150331", freq="B").as_unit(unit)
    df = DataFrame({"A": Series(range(len(index)), index=index)***REMOVED***, dtype="int64")
...
@pytest.mark.parametrize(
    ...
    [
        ["YE-DEC", {"start": "1990", "end": "2000", "freq": "Y-DEC"***REMOVED***],
        ["YE-JUN", {"start": "1990", "end": "2000", "freq": "Y-JUN"***REMOVED***],
        ["ME", {"start": "1990-01", "end": "2000-01", "freq": "M"***REMOVED***],
    ],
)
...
def test_resample_single_group(end, unit):
    ...
    rng = date_range("2000-1-1", f"2000-{end***REMOVED***-10", freq="D").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"origin": "1999-12-31 23:57:00"***REMOVED***,
        {"origin": Timestamp("1970-01-01 00:02:00")***REMOVED***,
        {"origin": "epoch", "offset": "2m"***REMOVED***,
        # origin of '1999-31-12 12:02:00' should be equivalent for this case
        {"origin": "1999-12-31 12:02:00"***REMOVED***,
        {"offset": "-3m"***REMOVED***,
    ],
)
...
def test_resample_bad_offset(offset, unit):
    rng = date_range("2000-01-01 00:00:00", "2000-01-01 02:00", freq="s").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
    msg = f"'offset' should be a Timedelta convertible type. Got '{offset***REMOVED***' instead."
    with pytest.raises(ValueError, match=msg):
...
def test_resample_anchored_multiday(label, sec):
    ...
    assert result.index[-1] == Timestamp(f"2014-10-15 23:00:{sec***REMOVED***00")
...
def test_how_lambda_functions(simple_date_range_series, unit):
    ...
    result = ts.resample("ME").aggregate(
        {"foo": lambda x: x.mean(), "bar": lambda x: x.std(ddof=1)***REMOVED***
    )
...
def test_resample_unequal_times(unit):
    ...
    df = DataFrame({"close": 1***REMOVED***, index=bad_ind)
...
def test_resample_timegrouper(dates, unit):
    ...
    df = DataFrame({"A": dates, "B": np.arange(len(dates))***REMOVED***)
    ...
    expected = DataFrame({"B": [1, 0, 2, 2, 1]***REMOVED***, index=exp_idx)
    if df["A"].isna().any():
...
def test_resample_timegrouper2(dates, unit):
    ...
    df = DataFrame({"A": dates, "B": np.arange(len(dates)), "C": np.arange(len(dates))***REMOVED***)
    result = df.set_index("A").resample("ME").count()
    ...
    expected = DataFrame(
        {"B": [1, 0, 2, 2, 1], "C": [1, 0, 2, 2, 1]***REMOVED***,
        index=exp_idx,
        columns=["B", "C"],
    )
    if df["A"].isna().any():
...
def test_resample_dst_anchor2(unit):
    ...
    df = DataFrame({"a": values, "b": values, "c": values***REMOVED***, index=dti, dtype="int64")
    how = {"a": "min", "b": "max", "c": "count"***REMOVED***
    ...
    expected3 = DataFrame(
        {"a": [0, 48, 1538], "b": [47, 1537, 1586], "c": [48, 1490, 49]***REMOVED***,
    ...
    expected4 = DataFrame(
        {"a": [0, 1538], "b": [1537, 1586], "c": [1538, 49]***REMOVED***,
        index=date_range(
    ...
    result_d = rs_d.agg({"a": "min", "b": "max", "c": "count"***REMOVED***)[["a", "b", "c"]]
...
def test_resample_datetime_values(unit):
    ...
    df = DataFrame({"timestamp": dates***REMOVED***, index=dates)
    df.index = df.index.as_unit(unit)
...
def test_resample_apply_with_additional_args2():
    ...
    df = DataFrame({"A": 1, "B": 2***REMOVED***, index=date_range("2017", periods=10))
    msg = "DataFrameGroupBy.resample operated on the grouping columns"
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
...
def test_resample_apply_product(duplicates, unit):
    ...
    df = DataFrame({"A": ts, "B": ts + 2***REMOVED***)
    if duplicates:
        df.columns = ["A", "A"]
...
def test_resample_empty_series_with_tz():
    # GH#53664
    df = DataFrame({"ts": [], "values": []***REMOVED***).astype(
        {"ts": "datetime64[ns, Atlantic/Faroe]"***REMOVED***
    )
...
def test_resample_M_Q_Y_A_deprecated(freq, freq_depr):
    # GH#9586
    depr_msg = f"'{freq_depr[1:]***REMOVED***' is deprecated and will be removed "
    f"in a future version, please use '{freq[1:]***REMOVED***' instead."
...
def test_resample_ms_closed_right(unit):
    ...
    df = DataFrame({"ts": dti***REMOVED***, index=dti)
    ...
    expected = DataFrame(
        {"ts": [datetime(2020, 2, 1), datetime(2020, 2, 4, 3, 59)]***REMOVED***,
        index=exp_dti,
    ).astype(f"M8[{unit***REMOVED***]")
    tm.assert_frame_equal(result, expected)
