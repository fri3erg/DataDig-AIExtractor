...
def get_identifier_values_for_example(identifier_names):
    return ','.join([f'\'{identifier***REMOVED***\'' for identifier in identifier_names])
...
def get_identifier_description(resource_name, identifier_name):
    return (
        f"The {resource_name***REMOVED***'s {identifier_name***REMOVED*** identifier. "
        f"This **must** be set."
    )
...
def add_resource_type_overview(
    ...
    if intro_link is not None:
        section.write(
            f'For more information about {resource_type.lower()***REMOVED*** refer to the '
            f':ref:`Resources Introduction Guide<{intro_link***REMOVED***>`.'
        )
        section.style.new_line()
...
***REMOVED***
            ...
            if type_section.getvalue().decode('utf-8').startswith(':type'):
                ...
                type_section.write(f':type {section.name***REMOVED***: {self._new_type***REMOVED***')
            else:
                type_section.clear_text()
                type_section.style.italics(f'({self._new_type***REMOVED***) -- ')
