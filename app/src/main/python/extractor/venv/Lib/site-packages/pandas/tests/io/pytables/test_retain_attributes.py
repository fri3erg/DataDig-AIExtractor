...
def test_retain_index_attributes(setup_path, unit):
    ...
    df = DataFrame({"A": Series(range(3), index=dti)***REMOVED***)
    ...
    ***REMOVED***
        ...
        with tm.assert_produces_warning(errors.AttributeConflictWarning):
            df2 = DataFrame({"A": Series(range(3), index=dti2)***REMOVED***)
        ...
        dti3 = DatetimeIndex(
            ["2001-01-01", "2001-01-02", "2002-01-01"], dtype=f"M8[{unit***REMOVED***]"
        )
        df2 = DataFrame(
            {
                "A": Series(
    ...
    ***REMOVED***
        ...
        df3 = DataFrame({"A": Series(range(3), index=dti4)***REMOVED***)
...
def test_retain_index_attributes2(tmp_path, setup_path):
    ...
    with tm.assert_produces_warning(errors.AttributeConflictWarning):
        df = DataFrame(
            {"A": Series(range(3), index=date_range("2000-1-1", periods=3, freq="h"))***REMOVED***
        )
        df.to_hdf(path, key="data", mode="w", append=True)
        df2 = DataFrame(
            {"A": Series(range(3), index=date_range("2002-1-1", periods=3, freq="D"))***REMOVED***
        )
        ...
        idx.name = "foo"
        df = DataFrame({"A": Series(range(3), index=idx)***REMOVED***)
        df.to_hdf(path, key="data", mode="w", append=True)
    ...
    with tm.assert_produces_warning(errors.AttributeConflictWarning):
        ...
        idx2.name = "bar"
        df2 = DataFrame({"A": Series(range(3), index=idx2)***REMOVED***)
        df2.to_hdf(path, key="data", append=True)
