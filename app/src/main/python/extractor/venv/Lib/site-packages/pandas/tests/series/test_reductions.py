...
@pytest.mark.parametrize("kwargs", [{"keepdims": True***REMOVED***, {"out": object()***REMOVED***])
def test_validate_any_all_out_keepdims_raises(kwargs, func):
    ...
    msg = (
        f"the '{param***REMOVED***' parameter is not "
        "supported in the pandas "
        rf"implementation of {name***REMOVED***\(\)"
    )
    with pytest.raises(ValueError, match=msg):
        func(ser, **kwargs)
...
def test_mean_dont_convert_j_to_complex(using_array_manager):
    # GH#36703
    df = pd.DataFrame([{"db": "J", "numeric": 123***REMOVED***])
