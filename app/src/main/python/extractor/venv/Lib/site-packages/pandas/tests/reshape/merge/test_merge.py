...
def dfs_for_indicator():
    df1 = DataFrame({"col1": [0, 1], "col_conflict": [1, 2], "col_left": ["a", "b"]***REMOVED***)
    df2 = DataFrame(
        {
            "col1": [1, 2, 3, 4, 5],
...
class TestMerge:
    ...
    def right(self):
        return DataFrame(
            {"v2": np.random.default_rng(2).standard_normal(4)***REMOVED***,
            index=["d", "b", "c", "a"],
        )
    ...
    def test_merge_inner_join_empty(self):
        # GH 15328
        df_empty = DataFrame()
        df_a = DataFrame({"a": [1, 2]***REMOVED***, index=[0, 1], dtype="int64")
        result = merge(df_empty, df_a, left_index=True, right_index=True)
        expected = DataFrame({"a": []***REMOVED***, dtype="int64")
    ...
    def test_merge_non_string_columns(self):
        ...
        left = DataFrame(
            {0: [1, 0, 1, 0], 1: [0, 1, 0, 0], 2: [0, 0, 2, 0], 3: [1, 0, 0, 3]***REMOVED***
        )
...
***REMOVED***
    ...
    def test_merge_different_column_key_names(self):
        left = DataFrame({"lkey": ["foo", "bar", "baz", "foo"], "value": [1, 2, 3, 4]***REMOVED***)
        right = DataFrame({"rkey": ["foo", "bar", "qux", "foo"], "value": [5, 6, 7, 8]***REMOVED***)
    ...
    def test_merge_copy(self):
        left = DataFrame({"a": 0, "b": 1***REMOVED***, index=range(10))
        right = DataFrame({"c": "foo", "d": "bar"***REMOVED***, index=range(10))
    ...
    def test_intelligently_handle_join_key(self):
        ...
        left = DataFrame(
            {"key": [1, 1, 2, 2, 3], "value": list(range(5))***REMOVED***, columns=["value", "key"]
        )
        right = DataFrame({"key": [1, 1, 2, 3, 4, 5], "rvalue": list(range(6))***REMOVED***)
    ...
    def test_merge_join_key_dtype_cast(self):
        ...
        df1 = DataFrame({"key": [1], "v1": [10]***REMOVED***)
        df2 = DataFrame({"key": [2], "v1": [20]***REMOVED***)
        df = merge(df1, df2, how="outer")
        assert df["key"].dtype == "int64"

        df1 = DataFrame({"key": [True], "v1": [1]***REMOVED***)
        df2 = DataFrame({"key": [False], "v1": [0]***REMOVED***)
        df = merge(df1, df2, how="outer")
        ...
        df1 = DataFrame({"val": [1]***REMOVED***)
        df2 = DataFrame({"val": [2]***REMOVED***)
        lkey = np.array([1])
        rkey = np.array([2])
    ...
    def test_handle_join_key_pass_array(self):
        left = DataFrame(
            {"key": [1, 1, 2, 2, 3], "value": np.arange(5)***REMOVED***,
            columns=["value", "key"],
            dtype="int64",
        )
        right = DataFrame({"rvalue": np.arange(6)***REMOVED***, dtype="int64")
        ...
        left = DataFrame({"value": np.arange(5)***REMOVED***, columns=["value"])
        right = DataFrame({"rvalue": np.arange(6)***REMOVED***)
        ...
        left = DataFrame({"value": np.arange(3)***REMOVED***)
        right = DataFrame({"rvalue": np.arange(6)***REMOVED***)
    ...
    def test_no_overlap_more_informative_error(self):
        dt = datetime.now()
        df1 = DataFrame({"x": ["a"]***REMOVED***, index=[dt])

        df2 = DataFrame({"y": ["b", "c"]***REMOVED***, index=[dt, dt])

        msg = (
            "No common columns to perform merge on. "
            f"Merge options: left_on={None***REMOVED***, right_on={None***REMOVED***, "
            f"left_index={False***REMOVED***, right_index={False***REMOVED***"
        )
    ...
    def test_merge_non_unique_indexes(self):
        ...
        # Not monotonic
        df1 = DataFrame({"x": ["a", "b", "q"]***REMOVED***, index=[dt2, dt, dt4])
        df2 = DataFrame(
            {"y": ["c", "d", "e", "f", "g", "h"]***REMOVED***, index=[dt3, dt3, dt2, dt2, dt, dt]
        )
        _check_merge(df1, df2)

        df1 = DataFrame({"x": ["a", "b"]***REMOVED***, index=[dt, dt])
        df2 = DataFrame({"y": ["c", "d"]***REMOVED***, index=[dt, dt])
        _check_merge(df1, df2)
    ...
    def test_merge_non_unique_index_many_to_many(self):
        ...
        dt2 = datetime(2012, 5, 2)
        dt3 = datetime(2012, 5, 3)
        df1 = DataFrame({"x": ["a", "b", "c", "d"]***REMOVED***, index=[dt2, dt2, dt, dt])
        df2 = DataFrame(
            {"y": ["e", "f", "g", " h", "i"]***REMOVED***, index=[dt2, dt2, dt3, dt, dt]
        )
        _check_merge(df1, df2)

    def test_left_merge_empty_dataframe(self):
        left = DataFrame({"key": [1], "value": [2]***REMOVED***)
        right = DataFrame({"key": []***REMOVED***)
    ...
    @pytest.mark.parametrize(
        "kwarg",
        [
            {"left_index": True, "right_index": True***REMOVED***,
            {"left_index": True, "right_on": "x"***REMOVED***,
            {"left_on": "a", "right_index": True***REMOVED***,
            {"left_on": "a", "right_on": "x"***REMOVED***,
        ],
    )
    ...
    def test_merge_left_empty_right_notempty(self):
        ...
        kwarg = {"left_on": "a", "right_index": True***REMOVED***
        check1(exp_in, kwarg)
        exp_out["a"] = [0, 1, 2]
        check2(exp_out, kwarg)

        kwarg = {"left_on": "a", "right_on": "x"***REMOVED***
        check1(exp_in, kwarg)
    ...
    def test_merge_empty_frame(self, series_of_dtype, series_of_dtype2):
        # GH 25183
        df = DataFrame(
            {"key": series_of_dtype, "value": series_of_dtype2***REMOVED***,
            columns=["key", "value"],
        )
        df_empty = df[:0]
        expected = DataFrame(
    ...
    def test_merge_all_na_column(self, series_of_dtype, series_of_dtype_all_na):
        # GH 25183
        df_left = DataFrame(
            {"key": series_of_dtype, "value": series_of_dtype_all_na***REMOVED***,
            columns=["key", "value"],
        )
        df_right = DataFrame(
            {"key": series_of_dtype, "value": series_of_dtype_all_na***REMOVED***,
    ...
    def test_merge_nosort(self):
        ...
        ***REMOVED***
        ...
        new = DataFrame.from_dict(
            {"var3": var3, "var8": np.random.default_rng(2).random(7)***REMOVED***
        )
    ...
    def test_merge_nan_right(self):
        df1 = DataFrame({"i1": [0, 1], "i2": [0, 1]***REMOVED***)
        df2 = DataFrame({"i1": [0], "i3": [0]***REMOVED***)
        result = df1.join(df2, on="i1", rsuffix="_")
        expected = (
            DataFrame(
                {
                    "i1": {0: 0.0, 1: 1***REMOVED***,
                    "i2": {0: 0, 1: 1***REMOVED***,
                    "i1_": {0: 0, 1: np.nan***REMOVED***,
                    "i3": {0: 0.0, 1: np.nan***REMOVED***,
                    None: {0: 0, 1: 0***REMOVED***,
                ***REMOVED***,
...
***REMOVED***
    ...
    def test_merge_nan_right2(self):
        df1 = DataFrame({"i1": [0, 1], "i2": [0.5, 1.5]***REMOVED***)
        df2 = DataFrame({"i1": [0], "i3": [0.7]***REMOVED***)
        result = df1.join(df2, rsuffix="_", on="i1")
        expected = DataFrame(
            {
                "i1": {0: 0, 1: 1***REMOVED***,
                "i1_": {0: 0.0, 1: np.nan***REMOVED***,
                "i2": {0: 0.5, 1: 1.5***REMOVED***,
                "i3": {0: 0.69999999999999996, 1: np.nan***REMOVED***,
            ***REMOVED***
        )[["i1", "i2", "i1_", "i3"]]
        tm.assert_frame_equal(result, expected)
    ...
    def test_join_append_timedeltas(self, using_array_manager):
        ...
        d = DataFrame.from_dict(
            {"d": [datetime(2013, 11, 5, 5, 56)], "t": [timedelta(0, 22500)]***REMOVED***
        )
    ...
    def test_other_datetime_unit(self, unit):
        # GH 13389
        df1 = DataFrame({"entity_id": [101, 102]***REMOVED***)
        ser = Series([None, None], index=[101, 102], name="days")

        dtype = f"datetime64[{unit***REMOVED***]"
    ...
    def test_other_timedelta_unit(self, unit):
        ...
        exp = DataFrame(
            {"entity_id": [101, 102], "days": np.array(["nat", "nat"], dtype=dtype)***REMOVED***,
            columns=["entity_id", "days"],
        )
        tm.assert_frame_equal(result, exp)

    def test_overlapping_columns_error_message(self):
        df = DataFrame({"key": [1, 2, 3], "v1": [4, 5, 6], "v2": [7, 8, 9]***REMOVED***)
        df2 = DataFrame({"key": [1, 2, 3], "v1": [4, 5, 6], "v2": [7, 8, 9]***REMOVED***)
    ...
    ***REMOVED***,
    ...
    def test_merge_on_periods(self):
        left = DataFrame(
            {"key": pd.period_range("20151010", periods=2, freq="D"), "value": [1, 2]***REMOVED***
        )
        right = DataFrame(
            {
    ...
    def test_merge_period_values(self):
        left = DataFrame(
            {"key": [1, 2], "value": pd.period_range("20151010", periods=2, freq="D")***REMOVED***
        )
        right = DataFrame(
            {"key": [2, 3], "value": pd.period_range("20151011", periods=2, freq="D")***REMOVED***
        )
    ...
    def test_indicator(self, dfs_for_indicator):
        ...
        df_result_custom_name = df_result_custom_name.rename(
            columns={"_merge": "custom_name"***REMOVED***
        )
    ...
    def test_merge_indicator_invalid(self, dfs_for_indicator):
        ...
        for i in ["_right_indicator", "_left_indicator", "_merge"]:
            df_badcolumn = DataFrame({"col1": [1, 2], i: [2, 2]***REMOVED***)

            msg = (
                "Cannot use `indicator=True` option when data contains a "
                f"column named {i***REMOVED***|"
...
***REMOVED***
        ...
        df_badcolumn = DataFrame({"col1": [1, 2], "custom_column_name": [2, 2]***REMOVED***)
...
***REMOVED***
    ...
    def test_merge_indicator_multiple_columns(self):
        ...
        df3 = DataFrame({"col1": [0, 1], "col2": ["a", "b"]***REMOVED***)

        df4 = DataFrame({"col1": [1, 1, 3], "col2": ["b", "x", "y"]***REMOVED***)

        hand_coded_result = DataFrame(
            {"col1": [0, 1, 1, 3], "col2": ["a", "b", "x", "y"]***REMOVED***
        )
    ...
    def test_validation(self):
        left = DataFrame(
            {"a": ["a", "b", "c", "d"], "b": ["cat", "dog", "weasel", "horse"]***REMOVED***,
            index=range(4),
        )
        ...
        # Dups on right
        right_w_dups = concat([right, DataFrame({"a": ["e"], "c": ["moo"]***REMOVED***, index=[4])])
        merge(
            left,
            right_w_dups,
            left_index=True,
...
***REMOVED***
        ...
        # Dups on left
        left_w_dups = concat(
            [left, DataFrame({"a": ["a"], "c": ["cow"]***REMOVED***, index=[3])], sort=True
        )
        merge(
            left_w_dups,
            right,
...
***REMOVED***,
    ...
    def test_merge_on_index_with_more_values(self, how, index, expected_index):
        ...
        df1 = DataFrame({"a": [0, 1, 2], "key": [0, 1, 2]***REMOVED***, index=index)
        df2 = DataFrame({"b": [0, 1, 2, 3, 4, 5]***REMOVED***)
    ...
    def test_merge_right_index_right(self):
        ...
        left = DataFrame({"a": [1, 2, 3], "key": [0, 1, 1]***REMOVED***)
        right = DataFrame({"b": [1, 2, 3]***REMOVED***)

        expected = DataFrame(
            {"a": [1, 2, 3, None], "key": [0, 1, 1, 2], "b": [1, 2, 2, 3]***REMOVED***,
            columns=["a", "key", "b"],
            index=[0, 1, 2, np.nan],
        )
    ...
    def test_merge_preserves_row_order(self, how):
        # GH 27453
        left_df = DataFrame({"animal": ["dog", "pig"], "max_speed": [40, 11]***REMOVED***)
        right_df = DataFrame({"animal": ["quetzal", "pig"], "max_speed": [80, 11]***REMOVED***)
        result = left_df.merge(right_df, on=["animal", "max_speed"], how=how)
        if how == "right":
            expected = DataFrame({"animal": ["quetzal", "pig"], "max_speed": [80, 11]***REMOVED***)
        else:
            expected = DataFrame({"animal": ["dog", "pig"], "max_speed": [40, 11]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_merge_take_missing_values_from_index_of_other_dtype(self):
        ...
        right = DataFrame({"b": [1, 2, 3]***REMOVED***, index=CategoricalIndex(["a", "b", "c"]))
        result = left.merge(right, left_on="key", right_index=True, how="right")
        expected = DataFrame(
            {
                "a": [1, 2, 3, None],
...
class TestMergeDtypes:
    ...
    def test_different(self, right_vals):
        ...
        right = DataFrame({"A": right_vals***REMOVED***)
    ...
    def test_join_multi_dtypes(self, d1, d2):
        ...
        right = DataFrame({"v2": np.array([5, 7], dtype=dtype2)***REMOVED***, index=index)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ([1, 2, 3], [1.0, 2.0, 3.0], {"X": [1, 2, 3], "Y": [1.0, 2.0, 3.0]***REMOVED***),
            ([1, 2, 3], [1.0, 3.0], {"X": [1, 3], "Y": [1.0, 3.0]***REMOVED***),
            ([1, 2], [1.0, 2.0, 3.0], {"X": [1, 2], "Y": [1.0, 2.0]***REMOVED***),
        ],
    )
    def test_merge_on_ints_floats(self, int_vals, float_vals, exp_vals):
        ...
        A = DataFrame({"X": int_vals***REMOVED***)
        B = DataFrame({"Y": float_vals***REMOVED***)
    ...
    def test_merge_key_dtype_cast(self):
        # GH 17044
        df1 = DataFrame({"key": [1.0, 2.0], "v1": [10, 20]***REMOVED***, columns=["key", "v1"])
        df2 = DataFrame({"key": [2], "v2": [200]***REMOVED***, columns=["key", "v2"])
        result = df1.merge(df2, on="key", how="left")
        expected = DataFrame(
            {"key": [1.0, 2.0], "v1": [10, 20], "v2": [np.nan, 200.0]***REMOVED***,
            columns=["key", "v1", "v2"],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_merge_on_ints_floats_warning(self):
        ...
        A = DataFrame({"X": [1, 2, 3]***REMOVED***)
        B = DataFrame({"Y": [1.1, 2.5, 3.0]***REMOVED***)
        expected = DataFrame({"X": [3], "Y": [3.0]***REMOVED***)
        ...
        B = DataFrame({"Y": [np.nan, np.nan, 3.0]***REMOVED***)
    ...
    def test_merge_incompat_infer_boolean_object(self):
        ...
        df1 = DataFrame({"key": Series([True, False], dtype=object)***REMOVED***)
        df2 = DataFrame({"key": [True, False]***REMOVED***)

        expected = DataFrame({"key": [True, False]***REMOVED***, dtype=object)
    ...
    def test_merge_incompat_infer_boolean_object_with_missing(self):
        # GH21119: bool + object bool merge OK
        # with missing value
        df1 = DataFrame({"key": Series([True, False, np.nan], dtype=object)***REMOVED***)
        df2 = DataFrame({"key": [True, False]***REMOVED***)
    ...
    def test_merge_incompat_dtypes_are_ok(self, df1_vals, df2_vals):
        ...
        df1 = DataFrame({"A": df1_vals***REMOVED***)
        df2 = DataFrame({"A": df2_vals***REMOVED***)
...
***REMOVED***,
    ...
    def test_merge_incompat_dtypes_error(self, df1_vals, df2_vals):
        ...
        msg = (
            f"You are trying to merge on {df1['A'].dtype***REMOVED*** and {df2['A'].dtype***REMOVED*** "
        ...
        msg = (
            f"You are trying to merge on {df2['A'].dtype***REMOVED*** and {df1['A'].dtype***REMOVED*** "
            "columns for key 'A'. If you wish to proceed you should use pd.concat"
        )
        msg = re.escape(msg)
        with pytest.raises(ValueError, match=msg):
        ...
        if len(df1_vals) == len(df2_vals):
            ...
            df3 = DataFrame({"A": df2_vals, "B": df1_vals, "C": df1_vals***REMOVED***)
            df4 = DataFrame({"A": df2_vals, "B": df2_vals, "C": df2_vals***REMOVED***)

            # Check that error raised correctly when merging all columns A, B, and C
            # The error message should mention key 'B'
            msg = (
                f"You are trying to merge on {df3['B'].dtype***REMOVED*** and {df4['B'].dtype***REMOVED*** "
...
***REMOVED***
            ...
            msg = (
                f"You are trying to merge on {df3['C'].dtype***REMOVED*** and {df4['C'].dtype***REMOVED*** "
...
***REMOVED***
    ...
    def test_merge_bool_dtype(self, how, expected_data):
        # GH#40073
        df1 = DataFrame({"A": [True, False], "B": [1, 5]***REMOVED***)
        df2 = DataFrame({"A": [False, True], "C": [3, 4]***REMOVED***)
...
***REMOVED***,
    ...
    def test_merge_empty(self, left_empty, how, exp):
        left = DataFrame({"A": [2, 1], "B": [3, 4]***REMOVED***)
        right = DataFrame({"A": [1], "C": [5]***REMOVED***, dtype="int64")
        ...
        if exp == "left":
            expected = DataFrame({"A": [2, 1], "B": [3, 4], "C": [np.nan, np.nan]***REMOVED***)
        elif exp == "right":
            expected = DataFrame({"A": [1], "B": [np.nan], "C": [5]***REMOVED***)
        elif exp == "empty":
...
***REMOVED***.astype(CategoricalDtype(["foo", "bar"])),
...
class TestMergeCategorical:
    ...
    def test_merge_categorical(self):
        ...
        right = DataFrame(
            {
                "c": {0: "a", 1: "b", 2: "c", 3: "d", 4: "e"***REMOVED***,
                "d": {0: "null", 1: "null", 2: "null", 3: "null", 4: "null"***REMOVED***,
            ***REMOVED***
        )
        left = DataFrame(
            {
                "a": {0: "f", 1: "f", 2: "f", 3: "f", 4: "f"***REMOVED***,
                "b": {0: "g", 1: "g", 2: "g", 3: "g", 4: "g"***REMOVED***,
            ***REMOVED***
        )
    ...
    def test_merging_with_bool_or_int_cateorical_column(
        ...
        df1 = DataFrame({"id": [1, 2, 3, 4], "cat": category_column***REMOVED***)
        df1["cat"] = df1["cat"].astype(CategoricalDtype(categories, ordered=ordered))
        df2 = DataFrame({"id": [2, 4], "num": [1, 9]***REMOVED***)
        result = df1.merge(df2)
        expected = DataFrame({"id": [2, 4], "cat": expected_categories, "num": [1, 9]***REMOVED***)
    ...
    def test_merge_on_int_array(self):
        # GH 23020
        df = DataFrame({"A": Series([1, 2, np.nan], dtype="Int64"), "B": 1***REMOVED***)
        result = merge(df, df, on="A")
        expected = DataFrame(
            {"A": Series([1, 2, np.nan], dtype="Int64"), "B_x": 1, "B_y": 1***REMOVED***
        )
        tm.assert_frame_equal(result, expected)
...
@pytest.fixture
def left_df():
    return DataFrame({"a": [20, 10, 0]***REMOVED***, index=[2, 1, 0])


@pytest.fixture
def right_df():
    return DataFrame({"b": [300, 100, 200]***REMOVED***, index=[3, 1, 2])
...
class TestMergeOnIndexes:
    @pytest.mark.parametrize(
        "how, sort, expected",
        [
            ("inner", False, DataFrame({"a": [20, 10], "b": [200, 100]***REMOVED***, index=[2, 1])),
            ("inner", True, DataFrame({"a": [10, 20], "b": [100, 200]***REMOVED***, index=[1, 2])),
            (
                "left",
                False,
                DataFrame({"a": [20, 10, 0], "b": [200, 100, np.nan]***REMOVED***, index=[2, 1, 0]),
***REMOVED***,
            (
                "left",
                True,
                DataFrame({"a": [0, 10, 20], "b": [np.nan, 100, 200]***REMOVED***, index=[0, 1, 2]),
***REMOVED***,
            (
                "right",
                False,
                DataFrame(
                    {"a": [np.nan, 10, 20], "b": [300, 100, 200]***REMOVED***, index=[3, 1, 2]
    ***REMOVED***,
***REMOVED***,
            (
                "right",
                True,
                DataFrame(
                    {"a": [10, 20, np.nan], "b": [100, 200, 300]***REMOVED***, index=[1, 2, 3]
    ***REMOVED***,
***REMOVED***,
            (
                "outer",
                False,
                DataFrame(
                    {"a": [0, 10, 20, np.nan], "b": [np.nan, 100, 200, 300]***REMOVED***,
                    index=[0, 1, 2, 3],
    ***REMOVED***,
***REMOVED***,
            (
...
@pytest.mark.parametrize(
    ...
    ids=lambda x: f"{type(x).__name__***REMOVED***[{x.dtype***REMOVED***]",
)
def test_merge_index_types(index):
    # gh-20777
    # assert key access is consistent across index types
    left = DataFrame({"left_data": [1, 2]***REMOVED***, index=index)
    right = DataFrame({"right_data": [1.0, 2.0]***REMOVED***, index=index)

    result = left.merge(right, on=["index_col"])

    expected = DataFrame({"left_data": [1, 2], "right_data": [1.0, 2.0]***REMOVED***, index=index)
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        (0, 0, {"suffixes": ("", "_dup")***REMOVED***, ["0", "0_dup"]),
        (0, 0, {"suffixes": (None, "_dup")***REMOVED***, [0, "0_dup"]),
        (0, 0, {"suffixes": ("_x", "_y")***REMOVED***, ["0_x", "0_y"]),
        (0, 0, {"suffixes": ["_x", "_y"]***REMOVED***, ["0_x", "0_y"]),
        ("a", 0, {"suffixes": (None, "_y")***REMOVED***, ["a", 0]),
        (0.0, 0.0, {"suffixes": ("_x", None)***REMOVED***, ["0.0_x", 0.0]),
        ("b", "b", {"suffixes": (None, "_y")***REMOVED***, ["b", "b_y"]),
        ("a", "a", {"suffixes": ("_x", None)***REMOVED***, ["a_x", "a"]),
        ("a", "b", {"suffixes": ("_x", None)***REMOVED***, ["a", "b"]),
        ("a", "a", {"suffixes": (None, "_x")***REMOVED***, ["a", "a_x"]),
        (0, 0, {"suffixes": ("_a", None)***REMOVED***, ["0_a", 0]),
        ("a", "a", {***REMOVED***, ["a_x", "a_y"]),
        (0, 0, {***REMOVED***, ["0_x", "0_y"]),
    ],
)
def test_merge_suffix(col1, col2, kwargs, expected_cols):
    # issue: 24782
    a = DataFrame({col1: [1, 2, 3]***REMOVED***)
    b = DataFrame({col2: [4, 5, 6]***REMOVED***)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "right",
            DataFrame(
                {"A": [100, 200, 300], "B1": [60, 70, np.nan], "B2": [600, 700, 800]***REMOVED***
***REMOVED***,
        ),
        (
            "outer",
...
def test_merge_duplicate_suffix(how, expected):
    left_df = DataFrame({"A": [100, 200, 1], "B": [60, 70, 80]***REMOVED***)
    right_df = DataFrame({"A": [100, 200, 300], "B": [600, 700, 800]***REMOVED***)
...
def test_merge_suffix_error(col1, col2, suffixes):
    # issue: 24782
    a = DataFrame({col1: [1, 2, 3]***REMOVED***)
    b = DataFrame({col2: [3, 4, 5]***REMOVED***)
...
@pytest.mark.parametrize("suffixes", [{"left", "right"***REMOVED***, {"left": 0, "right": 0***REMOVED***])
def test_merge_suffix_raises(suffixes):
    a = DataFrame({"a": [1, 2, 3]***REMOVED***)
    b = DataFrame({"b": [3, 4, 5]***REMOVED***)
...
def test_merge_equal_cat_dtypes(cat_dtype, reverse):
    ...
    ***REMOVED***
    ...
    df1 = DataFrame(
        {"foo": Series(["a", "b", "c"]).astype(cat_dtypes["one"]), "left": [1, 2, 3]***REMOVED***
    ).set_index("foo")
    ...
    df2 = DataFrame(
        {"foo": Series(data_foo).astype(cat_dtypes[cat_dtype]), "right": data_right***REMOVED***
    ).set_index("foo")
...
def test_merge_equal_cat_dtypes2():
    ...
    # Test Data
    df1 = DataFrame(
        {"foo": Series(["a", "b"]).astype(cat_dtype), "left": [1, 2]***REMOVED***
    ).set_index("foo")

    df2 = DataFrame(
        {"foo": Series(["a", "b", "c"]).astype(cat_dtype), "right": [3, 2, 1]***REMOVED***
    ).set_index("foo")

    result = df1.merge(df2, left_index=True, right_index=True)

    expected = DataFrame(
        {"left": [1, 2], "right": [3, 2], "foo": Series(["a", "b"]).astype(cat_dtype)***REMOVED***
    ).set_index("foo")
...
def test_merge_on_cat_and_ext_array():
    # GH 28668
    right = DataFrame(
        {"a": Series([pd.Interval(0, 1), pd.Interval(1, 2)], dtype="interval")***REMOVED***
    )
    left = right.copy()
...
def test_merge_datetime_upcast_dtype():
    ...
    df1 = DataFrame({"x": ["a", "b", "c"], "y": ["1", "2", "4"]***REMOVED***)
    df2 = DataFrame(
        {"y": ["1", "2", "3"], "z": pd.to_datetime(["2000", "2001", "2002"])***REMOVED***
    )
...
@pytest.mark.parametrize(
    ("kwargs", "err_msg"),
    [
        ({"left_on": "a", "left_index": True***REMOVED***, ["left_on", "left_index"]),
        ({"right_on": "a", "right_index": True***REMOVED***, ["right_on", "right_index"]),
    ],
)
def test_merge_join_cols_error_reporting_duplicates(func, kwargs, err_msg):
    # GH: 16228
    left = DataFrame({"a": [1, 2], "b": [3, 4]***REMOVED***)
    right = DataFrame({"a": [1, 1], "c": [5, 6]***REMOVED***)
    msg = rf'Can only pass argument "{err_msg[0]***REMOVED***" OR "{err_msg[1]***REMOVED***" not both\.'
    with pytest.raises(MergeError, match=msg):
...
@pytest.mark.parametrize(
    ("kwargs", "err_msg"),
    [
        ({"left_on": "a"***REMOVED***, ["right_on", "right_index"]),
        ({"right_on": "a"***REMOVED***, ["left_on", "left_index"]),
    ],
)
def test_merge_join_cols_error_reporting_missing(func, kwargs, err_msg):
    ...
    left = DataFrame({"a": [1, 2], "b": [3, 4]***REMOVED***)
    right = DataFrame({"a": [1, 1], "c": [5, 6]***REMOVED***)
    msg = rf'Must pass "{err_msg[0]***REMOVED***" OR "{err_msg[1]***REMOVED***"\.'
    with pytest.raises(MergeError, match=msg):
        getattr(pd, func)(left, right, **kwargs)
...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"right_index": True***REMOVED***,
        {"left_index": True***REMOVED***,
    ],
)
...
def test_merge_right_left_index():
    # GH#38616
    left = DataFrame({"x": [1, 1], "z": ["foo", "foo"]***REMOVED***)
    right = DataFrame({"x": [1, 1], "z": ["foo", "foo"]***REMOVED***)
...
def test_merge_result_empty_index_and_on():
    # GH#33814
    df1 = DataFrame({"a": [1], "b": [2]***REMOVED***).set_index(["a", "b"])
    df2 = DataFrame({"b": [1]***REMOVED***).set_index(["b"])
    expected = DataFrame({"a": [], "b": []***REMOVED***, dtype=np.int64).set_index(["a", "b"])
...
def test_merge_suffixes_produce_dup_columns_raises():
    ...
    left = DataFrame({"a": [1, 2, 3], "b": 1, "b_x": 2***REMOVED***)
    right = DataFrame({"a": [1, 2, 3], "b": 2***REMOVED***)
...
def test_merge_duplicate_columns_with_suffix_no_warning():
    ...
    right = DataFrame({"a": [1, 3], "b": 2***REMOVED***)
    result = merge(left, right, on="a")
...
def test_merge_on_left_categoricalindex():
    ...
    right = DataFrame({"A": ci, "B": range(3)***REMOVED***)
    left = DataFrame({"C": range(3, 6)***REMOVED***)
...
def test_merge_outer_with_NaN(dtype):
    # GH#43550
    left = DataFrame({"key": [1, 2], "col1": [1, 2]***REMOVED***, dtype=dtype)
    right = DataFrame({"key": [np.nan, np.nan], "col2": [3, 4]***REMOVED***, dtype=dtype)
    result = merge(left, right, on="key", how="outer")
    expected = DataFrame(
        {
...
def test_merge_different_index_names():
    # GH#45094
    left = DataFrame({"a": [1]***REMOVED***, index=Index([1], name="c"))
    right = DataFrame({"a": [1]***REMOVED***, index=Index([1], name="d"))
    result = merge(left, right, left_on="c", right_on="d")
    expected = DataFrame({"a_x": [1], "a_y": 1***REMOVED***)
    tm.assert_frame_equal(result, expected)


def test_merge_ea(any_numeric_ea_dtype, join_type):
    # GH#44240
    left = DataFrame({"a": [1, 2, 3], "b": 1***REMOVED***, dtype=any_numeric_ea_dtype)
    right = DataFrame({"a": [1, 2, 3], "c": 2***REMOVED***, dtype=any_numeric_ea_dtype)
    result = left.merge(right, how=join_type)
    expected = DataFrame({"a": [1, 2, 3], "b": 1, "c": 2***REMOVED***, dtype=any_numeric_ea_dtype)
    tm.assert_frame_equal(result, expected)
...
def test_merge_ea_and_non_ea(any_numeric_ea_dtype, join_type):
    # GH#44240
    left = DataFrame({"a": [1, 2, 3], "b": 1***REMOVED***, dtype=any_numeric_ea_dtype)
    right = DataFrame({"a": [1, 2, 3], "c": 2***REMOVED***, dtype=any_numeric_ea_dtype.lower())
    result = left.merge(right, how=join_type)
    expected = DataFrame(
        {
...
def test_merge_arrow_and_numpy_dtypes(dtype):
    ...
    df = DataFrame({"a": [1, 2]***REMOVED***, dtype=dtype)
    df2 = DataFrame({"a": [1, 2]***REMOVED***, dtype="int64[pyarrow]")
    result = df.merge(df2)
    expected = df.copy()
    tm.assert_frame_equal(result, expected)
...
def test_merge_datetime_different_resolution(tz, how):
    ...
    df1 = DataFrame({"t": vals[:2], "a": [1.0, 2.0]***REMOVED***)
    df1["t"] = df1["t"].dt.as_unit("ns")
    df2 = DataFrame({"t": vals[1:], "b": [1.0, 2.0]***REMOVED***)
    df2["t"] = df2["t"].dt.as_unit("s")

    expected = DataFrame({"t": vals, "a": [1.0, 2.0, np.nan], "b": [np.nan, 1.0, 2.0]***REMOVED***)
...
def test_merge_multiindex_single_level():
    # GH52331
    df = DataFrame({"col": ["A", "B"]***REMOVED***)
    df2 = DataFrame(
        data={"b": [100]***REMOVED***,
        index=MultiIndex.from_tuples([("A",), ("C",)], names=["col"]),
    )
    expected = DataFrame({"col": ["A", "B"], "b": [100, np.nan]***REMOVED***)
...
def test_merge_combinations(
    ...
    left = DataFrame({"key": left***REMOVED***)
    right = DataFrame({"key": right***REMOVED***)

    if on_index:
        left = left.set_index("key")
        right = right.set_index("key")
        on_kwargs = {"left_index": True, "right_index": True***REMOVED***
    else:
        on_kwargs = {"on": "key"***REMOVED***
    ...
    if how in ["left", "right", "inner"]:
        ...
        expected = DataFrame({"key": expected***REMOVED***)
        expected = expected.sort_values("key")
...
def test_merge_arrow_string_index(any_string_dtype):
    # GH#54894
    pytest.importorskip("pyarrow")
    left = DataFrame({"a": ["a", "b"]***REMOVED***, dtype=any_string_dtype)
    right = DataFrame({"b": 1***REMOVED***, index=Index(["a", "c"], dtype=any_string_dtype))
    result = left.merge(right, left_on="a", right_index=True, how="left")
    expected = DataFrame(
        {"a": Series(["a", "b"], dtype=any_string_dtype), "b": [1, np.nan]***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
...
def test_merge_datetime_and_timedelta(how):
    left = DataFrame({"key": Series([1, None], dtype="datetime64[ns]")***REMOVED***)
    right = DataFrame({"key": Series([1], dtype="timedelta64[ns]")***REMOVED***)

    msg = (
        f"You are trying to merge on {left['key'].dtype***REMOVED*** and {right['key'].dtype***REMOVED*** "
    ...
    msg = (
        f"You are trying to merge on {right['key'].dtype***REMOVED*** and {left['key'].dtype***REMOVED*** "
        "columns for key 'key'. If you wish to proceed you should use pd.concat"
    )
    with pytest.raises(ValueError, match=re.escape(msg)):
