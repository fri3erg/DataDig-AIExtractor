...
def is_k_edge_connected(G, k):
    ...
    """
    if k < 1:
        raise ValueError(f"k must be positive, not {k***REMOVED***")
...
def k_edge_augmentation(G, k, avail=None, weight=None, partial=False):
    ...
    >>> avail = [(1, 5, {"weight": 11***REMOVED***), (2, 5, {"weight": 10***REMOVED***)]
    ...
    >>> avail = {(1, 5): 11, (2, 5): 10, (4, 3): 1, (4, 5): 51***REMOVED***
    >>> sorted(nx.k_edge_augmentation(G, k=2, avail=avail))
    [(1, 5), (2, 5), (4, 5)]
    >>> # If augmentation is infeasible, then a partial solution can be found
    >>> avail = {(1, 5): 11***REMOVED***
    >>> sorted(nx.k_edge_augmentation(G, k=2, avail=avail, partial=True))
    [(1, 5)]
    """
    try:
        if k <= 0:
            raise ValueError(f"k must be a positive integer, not {k***REMOVED***")
        elif G.number_of_nodes() < k + 1:
            msg = f"impossible to {k***REMOVED*** connect in graph with less than {k + 1***REMOVED*** nodes"
...
***REMOVED***
            ...
            # raise NotImplementedError(f'not implemented for k>2. k={k***REMOVED***')
...
def partial_k_edge_augmentation(G, k, avail, weight=None):
    ...
    def _edges_between_disjoint(H, only1, only2):
        ...
        only1_adj = {u: set(H.adj[u]) for u in only1***REMOVED***
    ...
    H.add_edges_from(
        (
            (u, v, {"weight": w, "generator": (u, v)***REMOVED***)
...
def _lightest_meta_edges(mapping, avail_uv, avail_w):
    ...
    >>> groups = ([1, 2, 3], [4, 5], [6])
    >>> mapping = {n: meta_n for meta_n, ns in enumerate(groups) for n in ns***REMOVED***
...
def weighted_one_edge_augmentation(G, avail, weight=None, partial=False):
    ...
    C.add_edges_from(
        (mu, mv, {"weight": w, "generator": uv***REMOVED***)
...
def collapse(G, grouped_nodes):
    ...
    >>> G.add_node("A")
    >>> grouped_nodes = [{0, 1, 2, 3***REMOVED***, {5, 6, 7***REMOVED***]
    >>> C = collapse(G, grouped_nodes)
    >>> members = nx.get_node_attributes(C, "members")
    >>> sorted(members.keys())
    [0, 1, 2, 3]
    >>> member_values = set(map(frozenset, members.values()))
    >>> assert {0, 1, 2, 3***REMOVED*** in member_values
    >>> assert {4***REMOVED*** in member_values
    >>> assert {5, 6, 7***REMOVED*** in member_values
    >>> assert {"A"***REMOVED*** in member_values
    """
    mapping = {***REMOVED***
    members = {***REMOVED***
    C = G.__class__()
...
def greedy_k_edge_augmentation(G, k, avail=None, weight=None, seed=None):
    ...
    []
    >>> G = nx.path_graph((1, 2, 3, 4, 5, 6, 7))
    >>> avail = {(u, v): 1 for (u, v) in complement_edges(G)***REMOVED***
