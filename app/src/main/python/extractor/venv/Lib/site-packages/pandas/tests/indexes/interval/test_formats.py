...
***REMOVED***,
    ...
    def test_timestamp_with_timezone(self, unit):
        # GH 55035
        left = DatetimeIndex(["2020-01-01"], dtype=f"M8[{unit***REMOVED***, UTC]")
        right = DatetimeIndex(["2020-01-02"], dtype=f"M8[{unit***REMOVED***, UTC]")
        index = IntervalIndex.from_arrays(left, right)
        result = repr(index)
        expected = (
            "IntervalIndex([(2020-01-01 00:00:00+00:00, 2020-01-02 00:00:00+00:00]], "
            f"dtype='interval[datetime64[{unit***REMOVED***, UTC], right]')"
        )
        assert result == expected
