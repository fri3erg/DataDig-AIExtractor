...
CELL_TAG = '{%s***REMOVED***c' % SHEET_MAIN_NS
VALUE_TAG = '{%s***REMOVED***v' % SHEET_MAIN_NS
FORMULA_TAG = '{%s***REMOVED***f' % SHEET_MAIN_NS
MERGE_TAG = '{%s***REMOVED***mergeCells' % SHEET_MAIN_NS
INLINE_STRING = "{%s***REMOVED***is" % SHEET_MAIN_NS
COL_TAG = '{%s***REMOVED***col' % SHEET_MAIN_NS
ROW_TAG = '{%s***REMOVED***row' % SHEET_MAIN_NS
CF_TAG = '{%s***REMOVED***conditionalFormatting' % SHEET_MAIN_NS
LEGACY_TAG = '{%s***REMOVED***legacyDrawing' % SHEET_MAIN_NS
PROT_TAG = '{%s***REMOVED***sheetProtection' % SHEET_MAIN_NS
EXT_TAG = "{%s***REMOVED***extLst" % SHEET_MAIN_NS
HYPERLINK_TAG = "{%s***REMOVED***hyperlinks" % SHEET_MAIN_NS
TABLE_TAG = "{%s***REMOVED***tableParts" % SHEET_MAIN_NS
PRINT_TAG = '{%s***REMOVED***printOptions' % SHEET_MAIN_NS
MARGINS_TAG = '{%s***REMOVED***pageMargins' % SHEET_MAIN_NS
PAGE_TAG = '{%s***REMOVED***pageSetup' % SHEET_MAIN_NS
HEADER_TAG = '{%s***REMOVED***headerFooter' % SHEET_MAIN_NS
FILTER_TAG = '{%s***REMOVED***autoFilter' % SHEET_MAIN_NS
VALIDATION_TAG = '{%s***REMOVED***dataValidations' % SHEET_MAIN_NS
PROPERTIES_TAG = '{%s***REMOVED***sheetPr' % SHEET_MAIN_NS
VIEWS_TAG = '{%s***REMOVED***sheetViews' % SHEET_MAIN_NS
FORMAT_TAG = '{%s***REMOVED***sheetFormatPr' % SHEET_MAIN_NS
ROW_BREAK_TAG = '{%s***REMOVED***rowBreaks' % SHEET_MAIN_NS
COL_BREAK_TAG = '{%s***REMOVED***colBreaks' % SHEET_MAIN_NS
SCENARIOS_TAG = '{%s***REMOVED***scenarios' % SHEET_MAIN_NS
DATA_TAG = '{%s***REMOVED***sheetData' % SHEET_MAIN_NS
DIMENSION_TAG = '{%s***REMOVED***dimension' % SHEET_MAIN_NS
CUSTOM_VIEWS_TAG = '{%s***REMOVED***customSheetViews' % SHEET_MAIN_NS
...
class WorkSheetParser(object):
    ...
    def parse_cell(self, element):
        ...
        if not self.data_only and element.find(FORMULA_TAG) is not None:
            ...
            ***REMOVED***
                        ...
                        msg = f"""Cell {coordinate***REMOVED*** is marked as a date but the serial value {value***REMOVED*** is outside the limits for dates. The cell will be treated as an error."""
                        warn(msg)
                        data_type = "e"
                        value = "#VALUE!"
            elif data_type == 's':
        ...
        return {'row':row, 'column':column, 'value':value, 'data_type':data_type, 'style_id':style_id***REMOVED***
        ...
        if "r" in attrs:
            try:
                ...
                if val.is_integer():
                    ...
                    raise ValueError(f"{attrs['r']***REMOVED*** is not a valid row number")
        else:
            self.row_counter += 1
        self.col_counter = 0

        keys = {k for k in attrs if not k.startswith('{')***REMOVED***
        if keys - {'r', 'spans'***REMOVED***:
    ...
    def parse_formatting(self, element):
        try:
            ...
            msg = f"Failed to load a conditional formatting rule. It will be discarded. Cause: {e***REMOVED***"
            warn(msg)
    ...
    def parse_extensions(self, element):
        ...
        for e in extLst.ext:
            ...
            msg = "{0***REMOVED*** extension is not supported and will be removed".format(ext_type)
            warn(msg)
