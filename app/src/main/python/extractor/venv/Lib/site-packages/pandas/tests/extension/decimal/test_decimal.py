...
def test_dataframe_constructor_with_dtype():
    ...
    result = pd.DataFrame({"A": arr***REMOVED***, dtype=DecimalDtype())
    expected = pd.DataFrame({"A": arr***REMOVED***)
    tm.assert_frame_equal(result, expected)

    arr = DecimalArray([decimal.Decimal("10.0")])
    result = pd.DataFrame({"A": arr***REMOVED***, dtype="int64")
    expected = pd.DataFrame({"A": [10]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_groupby_agg():
    ...
    # multiple columns
    expected = pd.DataFrame({"id2": [0, 1], "decimals": to_decimal([data[0], data[3]])***REMOVED***)
...
def test_groupby_agg_ea_method(monkeypatch):
    ...
    data = make_data()[:5]
    df = pd.DataFrame({"id": [0, 0, 0, 1, 1], "decimals": DecimalArray(data)***REMOVED***)
...
def test_indexing_no_materialize(monkeypatch):
    ...
    data = make_data()
    s = pd.Series(DecimalArray(data))
    df = pd.DataFrame({"a": s, "b": range(len(s))***REMOVED***)
...
def test_array_copy_on_write(using_copy_on_write):
    df = pd.DataFrame({"a": [decimal.Decimal(2), decimal.Decimal(3)]***REMOVED***, dtype="object")
    df2 = df.astype(DecimalDtype())
    df.iloc[0, 0] = 0
    if using_copy_on_write:
        expected = pd.DataFrame(
            {"a": [decimal.Decimal(2), decimal.Decimal(3)]***REMOVED***, dtype=DecimalDtype()
        )
