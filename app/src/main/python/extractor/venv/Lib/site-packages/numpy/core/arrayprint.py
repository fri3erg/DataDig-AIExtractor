...
def _make_options_dict(precision=None, threshold=None, edgeitems=None,
    ...
    *legacy* and sanity checks.
    ...
    options = {k: v for k, v in locals().items() if v is not None***REMOVED***
    ...
    if floatmode not in modes + [None]:
        raise ValueError("floatmode option must be one of " +
                         ", ".join('"{***REMOVED***"'.format(m) for m in modes))
...
def set_printoptions(precision=None, threshold=None, edgeitems=None,
    ...
    >>> np.set_printoptions(formatter={'all':lambda x: 'int: '+str(-x)***REMOVED***)
    >>> x = np.arange(3)
    >>> x
...
def _get_formatdict(data, *, precision, floatmode, suppress, sign, legacy,
    ...
    formatdict = {
        ...
        'numpystr': lambda: repr_format***REMOVED***
...
def array2string(a, max_line_width=None, precision=None,
    ...
    >>> x  = np.arange(3.)
    >>> np.array2string(x, formatter={'float_kind':lambda x: "%.2f" % x***REMOVED***)
    '[0.00 1.00 2.00]'

    >>> x  = np.arange(3)
    >>> np.array2string(x, formatter={'int':lambda x: hex(x)***REMOVED***)
    '[0x0 0x1 0x2]'
...
def _none_or_positive_arg(x, name):
    if x is None:
        return -1
    if x < 0:
        raise ValueError("{***REMOVED*** must be >= 0".format(name))
    return x
...
class IntegerFormat:
    def __init__(self, data):
        if data.size > 0:
            ...
            max_str_len = 0
        self.format = '%{***REMOVED***d'.format(max_str_len)
...
class _TimelikeFormat:
    def __init__(self, data):
        ...
        self._format = '%{***REMOVED***s'.format(max_str_len)
...
class StructuredVoidFormat:
    ...
    def __call__(self, x):
        ...
        if len(str_fields) == 1:
            return "({***REMOVED***,)".format(str_fields[0])
        else:
            return "({***REMOVED***)".format(", ".join(str_fields))
...
def _array_repr_implementation(
    ...
    dtype_str = "dtype={***REMOVED***)".format(dtype_short_repr(arr.dtype))
