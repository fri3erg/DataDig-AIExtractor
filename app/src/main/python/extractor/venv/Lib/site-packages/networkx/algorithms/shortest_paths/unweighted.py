...
def single_source_shortest_path_length(G, source, cutoff=None):
    ...
    """
    if source not in G:
        raise nx.NodeNotFound(f"Source {source***REMOVED*** is not in G")
    if cutoff is None:
        cutoff = float("inf")
    nextlevel = [source]
...
def single_target_shortest_path_length(G, target, cutoff=None):
    ...
    """
    if target not in G:
        raise nx.NodeNotFound(f"Target {target***REMOVED*** is not in G")
...
def _bidirectional_pred_succ(G, source, target):
    ...
    if target == source:
        return ({target: None***REMOVED***, {source: None***REMOVED***, source)
...
def single_source_shortest_path(G, source, cutoff=None):
    ...
    """
    if source not in G:
        raise nx.NodeNotFound(f"Source {source***REMOVED*** not in G")
    ...
    if cutoff is None:
        cutoff = float("inf")
    nextlevel = {source: 1***REMOVED***  # list of nodes to check at next level
    paths = {source: [source]***REMOVED***  # paths dictionary  (paths to key from source)
...
def _single_shortest_path(adj, firstlevel, paths, cutoff, join):
    ...
    ----------
        adj : dict
            Adjacency dict or view
        firstlevel : dict
            starting nodes, e.g. {source: 1***REMOVED*** or {target: 1***REMOVED***
        paths : dict
            paths for starting nodes, e.g. {source: [source]***REMOVED***
        cutoff : int or float
            level at which we stop the process
        join : function
...
def single_target_shortest_path(G, target, cutoff=None):
    ...
    """
    if target not in G:
        raise nx.NodeNotFound(f"Target {target***REMOVED*** not in G")
    ...
    if cutoff is None:
        cutoff = float("inf")
    nextlevel = {target: 1***REMOVED***  # list of nodes to check at next level
    paths = {target: [target]***REMOVED***  # paths dictionary  (paths to key from source)
...
def predecessor(G, source, target=None, cutoff=None, return_seen=None):
    ...
    ({0: [], 1: [0], 2: [1], 3: [2]***REMOVED***, {0: 0, 1: 1, 2: 2, 3: 3***REMOVED***)
    ...
    seen = {source: level***REMOVED***  # level (number of hops) when seen in BFS
    pred = {source: []***REMOVED***  # predecessor dictionary
    while nextlevel:
        level = level + 1
        thislevel = nextlevel
        nextlevel = []
