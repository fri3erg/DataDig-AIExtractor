...
def test_resample_with_nat():
    ...
    result = DataFrame({"value": [2, 3, 5]***REMOVED***, index).resample("1s").mean()
    expected = DataFrame(
        {"value": [2.5, np.nan, 5.0]***REMOVED***,
...
def test_resample_as_freq_with_subperiod():
    ...
    df = DataFrame(data={"value": [1, 5, 10]***REMOVED***, index=index)
    result = df.resample("2min").asfreq()
    expected_data = {"value": [1, np.nan, np.nan, np.nan, np.nan, 10]***REMOVED***
    expected = DataFrame(
...
def test_resample_with_timedeltas():
    expected = DataFrame({"A": np.arange(1480)***REMOVED***)
    expected = expected.groupby(expected.index // 30).sum()
    expected.index = timedelta_range("0 days", freq="30min", periods=50)

    df = DataFrame(
        {"A": np.arange(1480)***REMOVED***, index=pd.to_timedelta(np.arange(1480), unit="min")
    )
...
def test_resample_categorical_data_with_timedeltaindex():
    # GH #12169
    df = DataFrame({"Group_obj": "A"***REMOVED***, index=pd.to_timedelta(list(range(20)), unit="s"))
    ...
    expected = DataFrame(
        {"Group_obj": ["A", "A"], "Group": ["A", "A"]***REMOVED***,
        index=exp_tdi,
    )
...
def test_resample_timedelta_values():
    ...
    df = DataFrame({"time": times***REMOVED***, index=times)
...
def test_resample_quantile_timedelta(unit):
    ...
    df = DataFrame(
        {"value": pd.to_timedelta(np.arange(4), unit="s").astype(dtype)***REMOVED***,
