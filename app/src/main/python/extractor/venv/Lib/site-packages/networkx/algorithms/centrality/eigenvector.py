...
def eigenvector_centrality(G, max_iter=100, tol=1.0e-6, nstart=None, weight=None):
    ...
    .. math::
        ...
        \lambda x_i = \sum_{j\to i***REMOVED***x_j.
    ...
    >>> sorted((v, f"{c:0.2f***REMOVED***") for v, c in centrality.items())
    ...
    * tol`` or after ``max_iter`` iterations, but in the second case it
    ...
    if nstart is None:
        nstart = {v: 1 for v in G***REMOVED***
    if all(v == 0 for v in nstart.values()):
        raise nx.NetworkXError("initial vector cannot have all zero values")
    # Normalize the initial vector so that each entry is in [0, 1]. This is
    # guaranteed to never have a divide-by-zero error by the previous line.
    nstart_sum = sum(nstart.values())
    x = {k: v / nstart_sum for k, v in nstart.items()***REMOVED***
    ...
    for _ in range(max_iter):
        ...
        x = {k: v / norm for k, v in x.items()***REMOVED***
...
def eigenvector_centrality_numpy(G, weight=None, max_iter=50, tol=0):
    ...
    >>> print([f"{node***REMOVED*** {centrality[node]:0.2f***REMOVED***" for node in centrality])
    ['0 0.37', '1 0.60', '2 0.60', '3 0.37']
