...
class StreamingChecksumBody(StreamingBody):
    ...
    def _validate_checksum(self):
        if self._checksum.digest() != base64.b64decode(self._expected):
            error_msg = (
                f"Expected checksum {self._expected***REMOVED*** did not match calculated "
                f"checksum: {self._checksum.b64digest()***REMOVED***"
***REMOVED***
        ...
        if algorithm_name not in supported_algorithms:
            if not HAS_CRT and algorithm_name in _CRT_CHECKSUM_ALGORITHMS:
                raise MissingDependencyException(
                    msg=(
                        f"Using {algorithm_name.upper()***REMOVED*** requires an "
...
def apply_request_checksum(request):
    checksum_context = request.get("context", {***REMOVED***).get("checksum", {***REMOVED***)
...
def handle_checksum_body(http_response, response, context, operation_model):
    ...
    checksum_context = context.get("checksum", {***REMOVED***)
...
***REMOVED***
        ...
        checksum_context = response["context"].get("checksum", {***REMOVED***)
    ...
    logger.info(
        ...
        f'following algorithms: {algorithms***REMOVED***.'
    )
