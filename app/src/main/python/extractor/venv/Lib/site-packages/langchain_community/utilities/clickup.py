...
def load_query(
    ...
    try:
        ...
        if fault_tolerant:
            return (
                None,
                f"""Input must be a valid JSON. Got the following error: {str(e)***REMOVED***. 
...
def fetch_first_id(data: dict, key: str) -> Optional[int]:
    ...
    if key in data and len(data[key]) > 0:
        if len(data[key]) > 1:
            warnings.warn(f"Found multiple {key***REMOVED***: {data[key]***REMOVED***. Defaulting to first.")
...
def fetch_data(url: str, access_token: str, query: Optional[dict] = None) -> dict:
    ...
    headers = {"Authorization": access_token***REMOVED***
...
def fetch_team_id(access_token: str) -> Optional[int]:
    """Fetch the team id."""
    url = f"{DEFAULT_URL***REMOVED***/team"
...
def fetch_space_id(team_id: int, access_token: str) -> Optional[int]:
    """Fetch the space id."""
    url = f"{DEFAULT_URL***REMOVED***/team/{team_id***REMOVED***/space"
    data = fetch_data(url, access_token, query={"archived": "false"***REMOVED***)
    return fetch_first_id(data, "spaces")


def fetch_folder_id(space_id: int, access_token: str) -> Optional[int]:
    """Fetch the folder id."""
    url = f"{DEFAULT_URL***REMOVED***/space/{space_id***REMOVED***/folder"
    data = fetch_data(url, access_token, query={"archived": "false"***REMOVED***)
...
def fetch_list_id(space_id: int, folder_id: int, access_token: str) -> Optional[int]:
    """Fetch the list id."""
    if folder_id:
        url = f"{DEFAULT_URL***REMOVED***/folder/{folder_id***REMOVED***/list"
    else:
        url = f"{DEFAULT_URL***REMOVED***/space/{space_id***REMOVED***/list"
...
class ClickupAPIWrapper(BaseModel):
    ...
    def get_access_code_url(
        ...
        url = f"https://app.clickup.com/api?client_id={oauth_client_id***REMOVED***"
        return f"{url***REMOVED***&redirect_uri={redirect_uri***REMOVED***"
    ...
    def get_access_token(
        ...
        url = f"{DEFAULT_URL***REMOVED***/oauth/token"
    ...
    ***REMOVED***
    ...
    def attempt_parse_teams(self, input_dict: dict) -> Dict[str, List[dict]]:
        ...
        parsed_teams: Dict[str, List[dict]] = {"teams": []***REMOVED***
        for team in input_dict["teams"]:
            try:
                ...
                warnings.warn(f"Error parsing a team {e***REMOVED***")
    ...
    def get_headers(
        ...
        if not isinstance(self.access_token, str):
            raise TypeError(f"Access Token: {self.access_token***REMOVED***, must be str.")
    ...
    def get_folders(self) -> Dict:
        ...
        """
        url = f"{DEFAULT_URL***REMOVED***/team/" + str(self.team_id) + "/space"
    ...
    def get_task(self, query: str, fault_tolerant: bool = True) -> Dict:
        ...
        url = f"{DEFAULT_URL***REMOVED***/task/{params['task_id']***REMOVED***"
        params = {
    ...
    def get_lists(self) -> Dict:
        ...
        url = f"{DEFAULT_URL***REMOVED***/folder/{self.folder_id***REMOVED***/list"
        params = self.get_default_params()
        response = requests.get(url, headers=self.get_headers(), params=params)
        return {"response": response***REMOVED***
    ...
    def query_tasks(self, query: str) -> Dict:
        ...
        url = f"{DEFAULT_URL***REMOVED***/list/{params['list_id']***REMOVED***/task"
    ...
    def get_spaces(self) -> Dict:
        ...
        """
        url = f"{DEFAULT_URL***REMOVED***/team/{self.team_id***REMOVED***/space"
    ...
    def get_task_attribute(self, query: str) -> Dict:
        ...
        if params["attribute_name"] not in task:
            return {
                "Error": f"""attribute_name = {params['attribute_name']***REMOVED*** was not 
found in task keys {task.keys()***REMOVED***. Please call again with one of the key names."""
            ***REMOVED***

        return {params["attribute_name"]: task[params["attribute_name"]]***REMOVED***
    ...
    def update_task(self, query: str) -> Dict:
        ...
        url = f"{DEFAULT_URL***REMOVED***/task/{query_dict['task_id']***REMOVED***"
        params = {
            "custom_task_ids": "true",
            "team_id": self.team_id,
            "include_subtasks": "true",
        ***REMOVED***
        ...
        payload = {query_dict["attribute_name"]: query_dict["value"]***REMOVED***
...
"Got user {user***REMOVED*** if type {type(user)***REMOVED***"""
                ***REMOVED***
        ...
        if query_dict["operation"] == "add":
            assigne_payload = {"add": query_dict["users"], "rem": []***REMOVED***
        elif query_dict["operation"] == "rem":
            assigne_payload = {"add": [], "rem": query_dict["users"]***REMOVED***
        else:
            raise ValueError(
                f"Invalid operation ({query_dict['operation']***REMOVED***). ",
...
***REMOVED***
        ...
        ***REMOVED***
        ...
        payload = {"assignees": assigne_payload***REMOVED***
        response = requests.put(url, headers=headers, params=params, json=payload)
        return {"response": response***REMOVED***
    ...
    def create_task(self, query: str) -> Dict:
        ...
        url = f"{DEFAULT_URL***REMOVED***/list/{list_id***REMOVED***/task"
        params = {"custom_task_ids": "true", "team_id": self.team_id***REMOVED***
    ...
    def create_list(self, query: str) -> Dict:
        ...
        url = f"{DEFAULT_URL***REMOVED***/folder/{location***REMOVED***/list"
    ...
    def run(self, mode: str, query: str) -> str:
        ...
        if mode == "get_task":
            ...
            output = {"ModeError": f"Got unexpected mode {mode***REMOVED***."***REMOVED***
