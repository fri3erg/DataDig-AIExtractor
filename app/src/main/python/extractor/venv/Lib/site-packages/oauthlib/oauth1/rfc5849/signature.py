...
def base_string_uri(uri: str, host: str = None) -> str:
    ...
    if host is not None:
        ...
        host = host.lower()
        host = f"{scheme***REMOVED***://{host***REMOVED***"
    ...
    if (scheme, port) in (('http', 80), ('https', 443)):
        ...
        netloc = f"{hostname***REMOVED***:{port***REMOVED***"  # use hostname:port
    else:
        netloc = hostname
...
def collect_parameters(uri_query='', body=None, headers=None,
    ...
    if headers:
        headers_lower = {k.lower(): v for k, v in headers.items()***REMOVED***
...
def normalize_parameters(params) -> str:
    ...
    parameter_parts = ['{***REMOVED***={***REMOVED***'.format(k, v) for k, v in key_values]
...
_jwt_rsa = {***REMOVED***  # cache of RSA-hash implementations from PyJWT jwt.algorithms
