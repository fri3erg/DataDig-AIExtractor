...
cdef class DocInfo:
    ...
    property public_id:
        ...
        def __set__(self, value):
            ...
            if value is not None:
                ...
                if match:
                    raise ValueError, f'Invalid character(s) {match.group(0)!r***REMOVED*** in public_id.'
                value = _utf8(value)
    ...
    def doctype(self):
        ...
        if system_url:
            ...
            if '"' in system_url:
                quoted_system_url = f"'{system_url***REMOVED***'"
            else:
                quoted_system_url = f'"{system_url***REMOVED***"'
        if public_id:
            if system_url:
                return f'<!DOCTYPE {root_name***REMOVED*** PUBLIC "{public_id***REMOVED***" {quoted_system_url***REMOVED***>'
            else:
                return f'<!DOCTYPE {root_name***REMOVED*** PUBLIC "{public_id***REMOVED***">'
        elif system_url:
            return f'<!DOCTYPE {root_name***REMOVED*** SYSTEM {quoted_system_url***REMOVED***>'
        elif self._doc.hasdoctype():
            return f'<!DOCTYPE {root_name***REMOVED***>'
        else:
            return ''
...
cdef public class _Element [ type LxmlElementType, object LxmlElement ]:
    ...
    def __delitem__(self, x):
        ...
        if isinstance(x, slice):
            ...
            c_node = _findChild(self._c_node, x)
            if c_node is NULL:
                raise IndexError, f"index out of range: {x***REMOVED***"
            _removeNode(self._doc, c_node)
...
***REMOVED***
    ...
    def iter(self, tag=None, *tags):
        ...
        Can be restricted to find only elements with specific tags:
        pass ``"{ns***REMOVED***localname"`` as tag. Either or both of ``ns`` and
        ``localname`` can be ``*`` for a wildcard; ``ns`` can be empty
        for no namespace. ``"localname"`` is equivalent to ``"{***REMOVED***localname"``
        (i.e. no namespace) but ``"*"`` is ``"{****REMOVED****"`` (any or no namespace),
        not ``"{***REMOVED****"``.
...
cdef class _ProcessingInstruction(__ContentOnlyElement):
    ...
    def attrib(self):
        ...
        return { attr : (value1 or value2)
                 for attr, value1, value2 in _FIND_PI_ATTRIBUTES(' ' + self.text) ***REMOVED***
...
cdef class _Entity(__ContentOnlyElement):
    ...
    property name:
        ...
        def __set__(self, value):
            ...
            if b'&' in value_utf or b';' in value_utf:
                raise ValueError, f"Invalid entity name '{value***REMOVED***'"
    ...
    def text(self):
        ...
        # entity value ?
        _assertValidNode(self)
        return f'&{funicode(self._c_node.name)***REMOVED***;'
...
cdef class QName:
    ...
    ``{namespace***REMOVED***tagname`` notation.  The ``namespace`` and
    ...
    def __init__(self, text_or_uri_or_element, tag=None):
        ...
        if not _isString(text_or_uri_or_element):
            if isinstance(text_or_uri_or_element, _Element):
                ...
                if not _isString(text_or_uri_or_element):
                    raise ValueError, f"Invalid input tag of type {type(text_or_uri_or_element)!r***REMOVED***"
        ...
        if tag is not None:
            # either ('ns', 'tag') or ('{ns***REMOVED***oldtag', 'newtag')
            if ns_utf is None:
        ...
        if ns_utf is None:
            ...
            self.text = "{%s***REMOVED***%s" % (self.namespace, self.localname)
    def __str__(self):
        return self.text
    def __hash__(self):
    ...
    def getelementpath(self, _Element element not None):
        ...
        while c_element is not root._c_node:
            ...
            while c_node is not NULL:
                ...
                c_node = c_node.prev
            if count:
                tag = f'{tag***REMOVED***[{count+1***REMOVED***]'
            else:
...
cdef class _Attrib:
    ...
    def pop(self, key, *default):
        if len(default) > 1:
            raise TypeError, f"pop expected at most 2 arguments, got {len(default)+1***REMOVED***"
        _assertValidNode(self._element)
...
def ProcessingInstruction(target, text=None):
    ...
    if target.lower() == b'xml':
        raise ValueError, f"Invalid PI name '{target***REMOVED***'"
...
def Entity(name):
    ...
    if c_name[0] == c'#':
        if not _characterReferenceIsValid(c_name + 1):
            raise ValueError, f"Invalid character reference: '{name***REMOVED***'"
    elif not _xmlNameIsValid(c_name):
        raise ValueError, f"Invalid entity reference: '{name***REMOVED***'"
    c_doc = _newXMLDoc()
    doc = _documentFactory(c_doc, None)
...
def indent(tree, space="  ", *, Py_ssize_t level=0):
    ...
    *level* is the initial indentation level. Setting this to a higher
    ...
    if level < 0:
        raise ValueError(f"Initial indentation level must be >= 0, got {level***REMOVED***")
 ...
 ***REMOVED***:
    ...
    if isinstance(element_or_tree, _Element):
        ...
        raise TypeError, f"Type '{python._fqtypename(element_or_tree).decode('utf8')***REMOVED***' cannot be serialized."
...
def tounicode(element_or_tree, *, method="xml", bint pretty_print=False,
    ...
    if isinstance(element_or_tree, _Element):
        ...
        raise TypeError, f"Type '{type(element_or_tree)***REMOVED***' cannot be serialized."
