...
def test_groupby_sample_balanced_groups_shape(n, frac):
    values = [1] * 10 + [2] * 10
    df = DataFrame({"a": values, "b": values***REMOVED***)

    result = df.groupby("a").sample(n=n, frac=frac)
    values = [1] * 2 + [2] * 2
    expected = DataFrame({"a": values, "b": values***REMOVED***, index=result.index)
...
def test_groupby_sample_index_value_spans_groups():
    values = [1] * 3 + [2] * 3
    df = DataFrame({"a": values, "b": values***REMOVED***, index=[1, 2, 2, 2, 2, 2])
...
def test_groupby_sample_without_n_or_frac():
    ...
    expected = DataFrame({"a": [1, 2], "b": [1, 2]***REMOVED***, index=result.index)
    tm.assert_frame_equal(result, expected)
...
def test_groupby_sample_with_weights(index, expected_index):
    ...
    values = [1] * 2 + [2] * 2
    df = DataFrame({"a": values, "b": values***REMOVED***, index=Index(index))

    result = df.groupby("a").sample(n=2, replace=True, weights=[1, 0, 1, 0])
    expected = DataFrame({"a": values, "b": values***REMOVED***, index=Index(expected_index))
    tm.assert_frame_equal(result, expected)
...
def test_groupby_sample_with_selections():
    # GH 39928
    values = [1] * 10 + [2] * 10
    df = DataFrame({"a": values, "b": values, "c": values***REMOVED***)

    result = df.groupby("a")[["b", "c"]].sample(n=None, frac=None)
    expected = DataFrame({"b": [1, 2], "c": [1, 2]***REMOVED***, index=result.index)
    tm.assert_frame_equal(result, expected)
