...
def test_default_na_values(all_parsers):
    ...
    ***REMOVED***
    ...
    def f(i, v):
        ...
        buf = f"{buf***REMOVED***{v***REMOVED***"

        if i < nv - 1:
            joined = "".join([","] * (nv - i - 1))
            buf = f"{buf***REMOVED***{joined***REMOVED***"
...
bar,foo,foo"""
    ...
    if parser.engine == "pyarrow":
        ...
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(StringIO(data), na_values={"A": ["foo"], "B": ["bar"]***REMOVED***)
        return

    df = parser.read_csv(StringIO(data), na_values={"A": ["foo"], "B": ["bar"]***REMOVED***)
    expected = DataFrame(
        {
...
@pytest.mark.parametrize(
    ...
    [
        (
            [0],
            DataFrame({"b": [np.nan], "c": [1], "d": [5]***REMOVED***, index=Index([0], name="a")),
        ),
        (
            [0, 2],
            DataFrame(
                {"b": [np.nan], "d": [5]***REMOVED***,
...
***REMOVED***,
        ...
        (
            {"na_values": {"A": [], "C": []***REMOVED***, "keep_default_na": False***REMOVED***,
            DataFrame(
                {
                    "A": ["a", "b", "", "d", "e", "nan", "g"],
                    "B": [1, 2, 3, 4, 5, 6, 7],
...
***REMOVED***,
        ...
        (
            {"na_values": ["a"], "keep_default_na": False***REMOVED***,
            DataFrame(
                {
...
def test_no_keep_default_na_dict_na_values(all_parsers):
    ...
    if parser.engine == "pyarrow":
        ...
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(
                StringIO(data), na_values={"b": ["2"]***REMOVED***, keep_default_na=False
***REMOVED***
        return
    ...
    expected = DataFrame({"a": [""], "b": [np.nan]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_no_keep_default_na_dict_na_scalar_values(all_parsers):
    ...
    if parser.engine == "pyarrow":
        msg = "The pyarrow engine doesn't support passing a dict for na_values"
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(StringIO(data), na_values={"b": 2***REMOVED***, keep_default_na=False)
        return

    df = parser.read_csv(StringIO(data), na_values={"b": 2***REMOVED***, keep_default_na=False)
    expected = DataFrame({"a": [1], "b": [np.nan]***REMOVED***)
    tm.assert_frame_equal(df, expected)
    ...
    if parser.engine == "pyarrow":
        ...
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(
                ...
                na_values={2: "", 6: "214.008", 1: "blah", 0: col_zero_na_values***REMOVED***,
***REMOVED***
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"a": 2, "b": 1***REMOVED***, [[1.0, 2.0], [np.nan, np.nan]]),
    ],
)
...
def test_na_values_dict_col_index(all_parsers):
    ...
    result = parser.read_csv(StringIO(data), na_values=na_values)
    expected = DataFrame({"a": [np.nan, 1]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        (
            str(2**63) + "\n" + str(2**63 + 1),
            {"na_values": [2**63]***REMOVED***,
            DataFrame([str(2**63), str(2**63 + 1)]),
        ),
        (str(2**63) + ",1" + "\n,2", {***REMOVED***, DataFrame([[str(2**63), 1], ["", 2]])),
        (str(2**63) + "\n1", {"na_values": [2**63]***REMOVED***, DataFrame([np.nan, 1])),
    ],
)
...
def test_empty_na_values_no_default_with_index(all_parsers):
    ...
    data = "a,1\nb,2"
    parser = all_parsers
    expected = DataFrame({"1": [2]***REMOVED***, index=Index(["b"], name="a"))
...
def test_no_na_filter_on_index(all_parsers, na_filter, index_data, request):
    ...
    expected = DataFrame({"a": [1, 4], "c": [3, 6]***REMOVED***, index=Index(index_data, name="b"))
...
def test_inf_na_values_with_int_index(all_parsers):
    ...
    expected = DataFrame(
        {"col1": [3, np.nan], "col2": [4, np.nan]***REMOVED***, index=Index([1, 2], name="idx")
    )
...
def test_na_values_with_dtype_str_and_na_filter(all_parsers, na_filter):
    ...
    expected = DataFrame({"a": ["1", "4"], "b": [empty, "5"], "c": ["3", "6"]***REMOVED***)
...
01001000155,4530,898"""
    ...
    result = parser.read_csv(
        ...
        dtype={"col1": str, "col2": str, "col3": str***REMOVED***,
    ).dropna()
...
def test_nan_multi_index(all_parsers):
    ...
    if parser.engine == "pyarrow":
        ...
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(
                StringIO(data), header=list(range(2)), na_values={("B", "Z"): "inf"***REMOVED***
***REMOVED***
        return
...
"""
    ...
    expected = DataFrame.from_dict({"0": [np.nan, 1.0, 0.0]***REMOVED***)
    tm.assert_frame_equal(result, expected)
