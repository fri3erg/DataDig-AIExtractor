...
def graph_example_1():
    ...
    labels = {v: k for k, v in rlabels.items()***REMOVED***
    ...
    while True:
        ...
        if nx.is_connected(G) and not nx.is_biconnected(G):
            ...
            if attempts >= max_attempts:
                msg = f"Tried {attempts***REMOVED*** times: no suitable Graph."
                raise Exception(msg)
            else:
                attempts += 1
...
def test_articulation_points():
    ...
    for i in range(1):  # change 1 to 3 or more for more realizations.
        G = next(Ggen)
        articulation_points = [{a***REMOVED*** for a in nx.articulation_points(G)]
...
def test_grid_2d_graph():
    ...
    solution = [{(0, 1), (1, 0)***REMOVED***, {(3, 0), (4, 1)***REMOVED***, {(3, 4), (4, 3)***REMOVED***, {(0, 3), (1, 4)***REMOVED***]
    for cut in nx.all_node_cuts(G):
...
def test_is_separating_set_complete_graph():
    ...
    assert _is_separating_set(G, {0, 1, 2, 3***REMOVED***)
...
def test_is_separating_set():
    for i in [5, 10, 15]:
        ...
        assert _is_separating_set(G, {max_degree_node***REMOVED***)


def test_non_repeated_cuts():
    # The algorithm was repeating the cut {0, 1***REMOVED*** for the giant biconnected
    # component of the Karate club graph.
    K = nx.karate_club_graph()
    bcc = max(list(nx.biconnected_components(K)), key=len)
    G = K.subgraph(bcc)
    solution = [{32, 33***REMOVED***, {2, 33***REMOVED***, {0, 3***REMOVED***, {0, 1***REMOVED***, {29, 33***REMOVED***]
    cuts = list(nx.all_node_cuts(G))
    if len(solution) != len(cuts):
        print(f"Solution: {solution***REMOVED***")
        print(f"Result: {cuts***REMOVED***")
...
def test_cycle_graph():
    G = nx.cycle_graph(5)
    solution = [{0, 2***REMOVED***, {0, 3***REMOVED***, {1, 3***REMOVED***, {1, 4***REMOVED***, {2, 4***REMOVED***]
    cuts = list(nx.all_node_cuts(G))
    assert len(solution) == len(cuts)
    for cut in cuts:
        assert cut in solution
...
def test_complete_graph():
    G = nx.complete_graph(5)
    solution = [{0, 1, 2, 3***REMOVED***, {0, 1, 2, 4***REMOVED***, {0, 1, 3, 4***REMOVED***, {0, 2, 3, 4***REMOVED***, {1, 2, 3, 4***REMOVED***]
    cuts = list(nx.all_node_cuts(G))
    assert len(solution) == len(cuts)
    for cut in cuts:
        assert cut in solution
