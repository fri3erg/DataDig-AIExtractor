...
def louvain_communities(
    ...
    .. math::
        \Delta Q = \frac{k_{i,in***REMOVED******REMOVED***{2m***REMOVED*** - \gamma\frac{ \Sigma_{tot***REMOVED*** \cdot k_i***REMOVED***{2m^2***REMOVED***

    where $m$ is the size of the graph, $k_{i,in***REMOVED***$ is the sum of the weights of the links
    from $i$ to nodes in $C$, $k_i$ is the sum of the weights of the links incident to node $i$,
    $\Sigma_{tot***REMOVED***$ is the sum of the weights of the links incident to nodes in $C$ and $\gamma$
    is the resolution parameter.

    For the directed case the modularity gain can be computed using this formula according to [3]_

    .. math::
        \Delta Q = \frac{k_{i,in***REMOVED******REMOVED***{m***REMOVED***
        - \gamma\frac{k_i^{out***REMOVED*** \cdot\Sigma_{tot***REMOVED***^{in***REMOVED*** + k_i^{in***REMOVED*** \cdot \Sigma_{tot***REMOVED***^{out***REMOVED******REMOVED***{m^2***REMOVED***

    where $k_i^{out***REMOVED***$, $k_i^{in***REMOVED***$ are the outer and inner weighted degrees of node $i$ and
    $\Sigma_{tot***REMOVED***^{in***REMOVED***$, $\Sigma_{tot***REMOVED***^{out***REMOVED***$ are the sum of in-going and out-going links incident
    to nodes in $C$.
    ...
    [{0, 4, 5, 7, 9***REMOVED***, {1, 2, 3, 6, 8***REMOVED***]
...
def louvain_partitions(
    ...
    partition = [{u***REMOVED*** for u in G.nodes()]
    if nx.is_empty(G):
        yield partition
        return
...
def _one_level(G, m, partition, resolution=1, is_directed=False, seed=None):
    ...
    """
    node2com = {u: i for i, u in enumerate(G.nodes())***REMOVED***
    inner_partition = [{u***REMOVED*** for u in G.nodes()]
    if is_directed:
        ...
        nbrs = {u: {v: data["weight"] for v, data in G[u].items() if v != u***REMOVED*** for u in G***REMOVED***
    ...
    ***REMOVED*** / (2 * m**2)
        ...
        ***REMOVED***
            ...
            if best_com != node2com[u]:
                com = G.nodes[u].get("nodes", {u***REMOVED***)
...
def _gen_graph(G, partition):
    ...
    for i, part in enumerate(partition):
        ...
        for node in part:
            node2com[node] = i
            nodes.update(G.nodes[node].get("nodes", {node***REMOVED***))
    ...
    for node1, node2, wt in G.edges(data=True):
        ...
        com1 = node2com[node1]
        com2 = node2com[node2]
        temp = H.get_edge_data(com1, com2, {"weight": 0***REMOVED***)["weight"]
