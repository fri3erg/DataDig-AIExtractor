...
def assert_framelist_equal(list1, list2, *args, **kwargs):
    assert len(list1) == len(list2), (
        ...
        f"len(list1) == {len(list1)***REMOVED***, "
        f"len(list2) == {len(list2)***REMOVED***"
    )
...
def test_invalid_flavor():
    ...
    msg = r"\{" + flavor + r"\***REMOVED*** is not a valid set of flavors"
...
***REMOVED***
            ...
            .map("{:.3f***REMOVED***".format).astype(float)
        )
        out = df.to_html()
        res = flavor_read_html(
            StringIO(out), attrs={"class": "dataframe"***REMOVED***, index_col=0
        )[0]
    ...
    def test_banklist_url(self, httpserver, banklist_data, flavor_read_html):
        with open(banklist_data, encoding="utf-8") as f:
            ...
            df1 = flavor_read_html(
                ...
                httpserver.url,
                match="First Federal Bank of Florida",  # attrs={"class": "dataTable"***REMOVED***
***REMOVED***
            # lxml cannot find attrs leave out for now
            df2 = flavor_read_html(
                httpserver.url,
                match="Metcalf Bank",
***REMOVED***  # attrs={"class": "dataTable"***REMOVED***)
    ...
    def test_banklist(self, banklist_data, flavor_read_html):
        df1 = flavor_read_html(
            banklist_data, match=".*Florida.*", attrs={"id": "table"***REMOVED***
        )
        df2 = flavor_read_html(
            banklist_data, match="Metcalf Bank", attrs={"id": "table"***REMOVED***
        )
    ...
    def test_banklist_no_match(self, banklist_data, flavor_read_html):
        dfs = flavor_read_html(banklist_data, attrs={"id": "table"***REMOVED***)
        for df in dfs:
            assert isinstance(df, DataFrame)
    ...
    def test_skiprows_set(self, spam_data, flavor_read_html):
        df1 = flavor_read_html(spam_data, match=".*Water.*", skiprows={1, 2***REMOVED***)
        df2 = flavor_read_html(spam_data, match="Unit", skiprows={2, 1***REMOVED***)
    ...
    def test_file_url(self, banklist_data, flavor_read_html):
        ...
        dfs = flavor_read_html(
            file_path_to_url(os.path.abspath(url)), match="First", attrs={"id": "table"***REMOVED***
        )
    ...
    def test_invalid_table_attrs(self, banklist_data, flavor_read_html):
        ...
        with pytest.raises(ValueError, match="No tables found"):
            flavor_read_html(
                url, match="First Federal Bank of Florida", attrs={"id": "tasdfable"***REMOVED***
***REMOVED***

    @pytest.mark.slow
    def test_multiindex_header(self, banklist_data, flavor_read_html):
        df = flavor_read_html(
            banklist_data, match="Metcalf", attrs={"id": "table"***REMOVED***, header=[0, 1]
        )[0]
    ...
    def test_multiindex_index(self, banklist_data, flavor_read_html):
        df = flavor_read_html(
            banklist_data, match="Metcalf", attrs={"id": "table"***REMOVED***, index_col=[0, 1]
        )[0]
...
***REMOVED***
        ...
        expected = DataFrame(data={"Header": "first"***REMOVED***, index=[0])
...
***REMOVED***,
    ...
    def test_banklist_header(self, banklist_data, datapath, flavor_read_html):
        ...
        df = flavor_read_html(banklist_data, match="Metcalf", attrs={"id": "table"***REMOVED***)[0]
        ground_truth = read_csv(
            datapath("io", "data", "csv", "banklist.csv"),
            converters={"Updated Date": Timestamp, "Closing Date": Timestamp***REMOVED***,
        )
    ...
    def test_gold_canyon(self, banklist_data, flavor_read_html):
        ...
        df = flavor_read_html(
            banklist_data, match="Gold Canyon", attrs={"id": "table"***REMOVED***
        )[0]
...
***REMOVED***
    ...
    def test_parse_dates_list(self, flavor_read_html):
        df = DataFrame({"date": date_range("1/1/2001", periods=10)***REMOVED***)
        expected = df.to_html()
    ...
    def test_parse_dates_combine(self, flavor_read_html):
        ...
        res = flavor_read_html(
            StringIO(df.to_html()), parse_dates={"datetime": [1, 2]***REMOVED***, index_col=1
        )
        newdf = DataFrame({"datetime": raw_dates***REMOVED***)
        tm.assert_frame_equal(newdf, res[0])

    def test_wikipedia_states_table(self, datapath, flavor_read_html):
        data = datapath("io", "data", "html", "wikipedia_states.html")
        assert os.path.isfile(data), f"{repr(data)***REMOVED*** is not a file"
        assert os.path.getsize(data), f"{repr(data)***REMOVED*** is an empty file"
...
***REMOVED***,
        ...
        expected = DataFrame(data={"Header": 1100.101***REMOVED***, index=[0])
...
***REMOVED***,
            converters={"a": str***REMOVED***,
        )[0]

        expected = DataFrame({"a": ["0.763", "0.244"]***REMOVED***)
...
***REMOVED***,
        ...
        expected = DataFrame({"a": [0.763, np.nan]***REMOVED***)
    ...
    def test_keep_default_na(self, flavor_read_html):
        ...
        expected_df = DataFrame({"a": ["N/A", "NA"]***REMOVED***)
        html_df = flavor_read_html(StringIO(html_data), keep_default_na=False)[0]
        tm.assert_frame_equal(expected_df, html_df)

        expected_df = DataFrame({"a": [np.nan, np.nan]***REMOVED***)
...
***REMOVED***
    ...
    def test_displayed_only_with_many_elements(self, displayed_only, flavor_read_html):
        ...
        expected = DataFrame({"A": [1, 4], "B": [2, 5]***REMOVED***)
        tm.assert_frame_equal(result, expected)
...
***REMOVED***
    ...
    def test_extract_links_bad(self, spam_data):
        msg = (
            ...
            '{None, "header", "footer", "body", "all"***REMOVED***, got "incorrect"'
        )
        with pytest.raises(ValueError, match=msg):
