...
class Authenticate:
    ...
    ***REMOVED***
        ...
        if not os.path.isfile(self.creds_fullpath):
            raise OSError(f"Cannot find file {self.creds_fullpath***REMOVED***")

        with open(self.creds_fullpath) as infile:
            if verbose:
                print(f"Reading credentials file {self.creds_fullpath***REMOVED***")
    ...
    def _validate_creds_file(self, verbose=False):
        ...
        if not (oauth1 or oauth2):
            msg = f"Missing or incorrect entries in {self.creds_file***REMOVED***\n"
            msg += pprint.pformat(self.oauth)
            raise ValueError(msg)
        elif verbose:
            print(f'Credentials file "{self.creds_file***REMOVED***" looks good')
...
def add_access_token(creds_file=None):
    ...
    tok = f"access_token={access_token***REMOVED***\n"
