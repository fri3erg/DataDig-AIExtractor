        ...
        if _is_complete_graph(sg):
            ...
            if current_treewidth > treewidth_bound:
                raise nx.NetworkXTreewidthBoundExceeded(
                    f"treewidth_bound exceeded: {current_treewidth***REMOVED***"
    ***REMOVED***
        else:
            # sg is not a clique,
...
def complete_to_chordal_graph(G):
    ...
    """
    H = G.copy()
    alpha = {node: 0 for node in H***REMOVED***
    if nx.is_chordal(H):
        return H, alpha
    chords = set()
    weight = {node: 0 for node in H.nodes()***REMOVED***
