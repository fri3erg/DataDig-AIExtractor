...
def _checknames(descr, names=None):
    ...
    if names is None:
        ...
        if isinstance(names, (tuple, list)):
            ...
            raise NameError(f'illegal input names {names!r***REMOVED***')
...
class MaskedRecords(MaskedArray):
    ...
    def __getattribute__(self, attr):
        ...
        try:
            ...
            raise AttributeError(
                f'record array has no attribute {attr***REMOVED***') from e
        # So far, so good
    ...
    def __setattr__(self, attr, val):
        ...
        try:
            ...
            fielddict = ndarray.__getattribute__(self, 'dtype').fields or {***REMOVED***
            optinfo = ndarray.__getattribute__(self, '_optinfo') or {***REMOVED***
    ...
    def __str__(self):
        ...
        """
        if self.size > 1:
            mstr = [f"({','.join([str(i) for i in s])***REMOVED***)"
                    for s in zip(*[getattr(self, f) for f in self.dtype.names])]
            return f"[{', '.join(mstr)***REMOVED***]"
        else:
            mstr = [f"{','.join([str(i) for i in s])***REMOVED***"
                    for s in zip([getattr(self, f) for f in self.dtype.names])]
            return f"({', '.join(mstr)***REMOVED***)"
...
def openfile(fname):
    ...
    try:
        ...
        raise FileNotFoundError(f"No such file: '{fname***REMOVED***'") from e
    if f.readline()[:2] != "\\x":
        f.seek(0, 0)
        return f
    f.close()
...
def fromtextfile(fname, delimiter=None, commentchar='#', missingchar='',
    ...
    delimiter : {None, string***REMOVED***, optional
