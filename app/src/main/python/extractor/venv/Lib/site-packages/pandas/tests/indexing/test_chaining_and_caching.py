...
class TestCaching:
    def test_slice_consolidate_invalidate_item_cache(self, using_copy_on_write):
        ...
        with option_context("chained_assignment", None):
            # #3970
            df = DataFrame({"aa": np.arange(5), "bb": [2.2] * 5***REMOVED***)
    ...
    def test_setitem_cache_updating(self, do_ref):
        ...
        df = DataFrame({"a": cont, "b": cont[3:] + cont[:3], "c": np.arange(7)***REMOVED***)
    ...
    def test_setitem_cache_updating_slices(
        ...
        expected = DataFrame(
            {"A": [600, 600, 600]***REMOVED***, index=date_range("5/7/2014", "5/9/2014")
        )
        out = DataFrame({"A": [0, 0, 0]***REMOVED***, index=date_range("5/7/2014", "5/9/2014"))
        df = DataFrame({"C": ["A", "A", "A"], "D": [100, 200, 300]***REMOVED***)
...
class TestChaining:
    def test_setitem_chained_setfault(self, using_copy_on_write):
        ...
        df = DataFrame({"response": np.array(data)***REMOVED***)
        mask = df.response == "timeout"
        with tm.raises_chained_assignment_error():
            df.response[mask] = "none"
        if using_copy_on_write:
            tm.assert_frame_equal(df, DataFrame({"response": data***REMOVED***))
        else:
            tm.assert_frame_equal(df, DataFrame({"response": mdata***REMOVED***))
        ...
        df = DataFrame({"response": data, "response1": data***REMOVED***)
        df_original = df.copy()
        mask = df.response == "timeout"
        with tm.raises_chained_assignment_error():
            df.response[mask] = "none"
        if using_copy_on_write:
            ...
            tm.assert_frame_equal(df, DataFrame({"response": mdata, "response1": data***REMOVED***))

        # GH 6056
        expected = DataFrame({"A": [np.nan, "bar", "bah", "foo", "bar"]***REMOVED***)
        df = DataFrame({"A": np.array(["foo", "bar", "bah", "foo", "bar"])***REMOVED***)
        with tm.raises_chained_assignment_error():
            df["A"].iloc[0] = np.nan
        if using_copy_on_write:
            expected = DataFrame({"A": ["foo", "bar", "bah", "foo", "bar"]***REMOVED***)
        else:
            expected = DataFrame({"A": [np.nan, "bar", "bah", "foo", "bar"]***REMOVED***)
        result = df.head()
...
***REMOVED***
    ...
    def test_detect_chained_assignment_object_dtype(
        ...
        expected = DataFrame({"A": [111, "bbb", "ccc"], "B": [1, 2, 3]***REMOVED***)
        df = DataFrame(
            {"A": Series(["aaa", "bbb", "ccc"], dtype=object), "B": [1, 2, 3]***REMOVED***
        )
        df_original = df.copy()
    ...
    def test_detect_chained_assignment_is_copy(self):
        ...
        df = DataFrame({"a": [1]***REMOVED***).dropna()
        assert df._is_copy is None
        df["a"] += 1
    ...
    def test_detect_chained_assignment_false_positives(self):
        ...
        df = DataFrame({"column1": ["a", "a", "a"], "column2": [4, 8, 9]***REMOVED***)
        str(df)
    ...
    def test_setting_with_copy_bug(self, using_copy_on_write, warn_copy_on_write):
        # operating on a copy
        df = DataFrame(
            {"a": list(range(4)), "b": list("ab.."), "c": ["a", "b", np.nan, "d"]***REMOVED***
        )
        df_original = df.copy()
        mask = pd.isna(df.c)
    ...
    def test_setting_with_copy_bug_no_warning(self):
        ...
        # GH 8730
        df1 = DataFrame({"x": Series(["a", "b", "c"]), "y": Series(["d", "e", "f"])***REMOVED***)
        df2 = df1[["x"]]
    ...
    def test_detect_chained_assignment_warnings_errors(
        ...
        df = DataFrame({"A": ["aaa", "bbb", "ccc"], "B": [1, 2, 3]***REMOVED***)
        if using_copy_on_write or warn_copy_on_write:
            with tm.raises_chained_assignment_error():
                df.loc[0]["A"] = 111
            return
    ...
    @pytest.mark.parametrize("rhs", [3, DataFrame({0: [1, 2, 3, 4]***REMOVED***)])
    ...
    def test_chained_getitem_with_lists(self):
        ...
        df = DataFrame({"A": 5 * [np.zeros(3)], "B": 5 * [np.ones(3)]***REMOVED***)
    ...
    def test_iloc_setitem_chained_assignment(self, using_copy_on_write):
        # GH#3970
        with option_context("chained_assignment", None):
            df = DataFrame({"aa": range(5), "bb": [2.2] * 5***REMOVED***)
            df["cc"] = 0.0
    ...
    def test_getitem_loc_assignment_slice_state(self):
        # GH 13569
        df = DataFrame({"a": [10, 20, 30]***REMOVED***)
        with tm.raises_chained_assignment_error():
            df["a"].loc[4] = 40
        tm.assert_frame_equal(df, DataFrame({"a": [10, 20, 30]***REMOVED***))
