...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            None,
            Series({0: ["split", "once"], 1: ["split", "once too!"]***REMOVED***),
        ),
        (
            ["split_once", "split_once_too!"],
            "_",
            Series({0: ["split", "once"], 1: ["split", "once_too!"]***REMOVED***),
        ),
    ],
)
...
def test_split_to_dataframe_no_splits(any_string_dtype):
    ...
    exp = DataFrame({0: Series(["nosplit", "alsonosplit"], dtype=any_string_dtype)***REMOVED***)
    tm.assert_frame_equal(result, exp)
...
def test_split_to_dataframe(any_string_dtype):
    ...
    result = s.str.split("_", expand=True)
    exp = DataFrame(
        {0: ["some", "with"], 1: ["equal", "no"], 2: ["splits", "nans"]***REMOVED***,
        dtype=any_string_dtype,
    )
    tm.assert_frame_equal(result, exp)
...
def test_split_to_dataframe_with_index(any_string_dtype):
    ...
    result = s.str.split("_", expand=True)
    exp = DataFrame(
        {0: ["some", "with"], 1: ["splits", "index"]***REMOVED***,
        index=["preserve", "me"],
        dtype=any_string_dtype,
    )
    tm.assert_frame_equal(result, exp)
...
def test_rsplit_to_dataframe_expand_no_splits(any_string_dtype):
    ...
    exp = DataFrame({0: Series(["nosplit", "alsonosplit"])***REMOVED***, dtype=any_string_dtype)
    tm.assert_frame_equal(result, exp)
...
def test_rsplit_to_dataframe_expand(any_string_dtype):
    ...
    exp = DataFrame(
        {0: ["some_equal", "with_no"], 1: ["splits", "nans"]***REMOVED***, dtype=any_string_dtype
    )
    tm.assert_frame_equal(result, exp)
...
def test_partition_with_name(any_string_dtype):
    ...
    expected = DataFrame(
        {0: ["a", "c"], 1: [",", ","], 2: ["b", "d"]***REMOVED***, dtype=any_string_dtype
    )
    tm.assert_frame_equal(result, expected)
...
def test_get_complex(idx, exp):
    ...
    ser = Series([(1, 2, 3), [1, 2, 3], {1, 2, 3***REMOVED***, {1: "a", 2: "b", 3: "c"***REMOVED***])
