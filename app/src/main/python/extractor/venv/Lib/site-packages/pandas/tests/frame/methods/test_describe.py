...
class TestDataFrameDescribe:
    def test_describe_bool_in_mixed_frame(self):
        ...
        result = df.describe()
        expected = DataFrame(
            {"int_data": [5, 30, df.int_data.std(), 10, 20, 30, 40, 50]***REMOVED***,
        ...
        expected = DataFrame(
            {"bool_data": [5, 2, False, 3]***REMOVED***, index=["count", "unique", "top", "freq"]
        )
        tm.assert_frame_equal(result, expected)

    def test_describe_empty_object(self):
        # GH#27183
        df = DataFrame({"A": [None, None]***REMOVED***, dtype=object)
        result = df.describe()
        expected = DataFrame(
            {"A": [0, 0, np.nan, np.nan]***REMOVED***,
            dtype=object,
    ...
    def test_describe_bool_frame(self):
        ...
        result = df.describe()
        expected = DataFrame(
            {"bool_data_1": [4, 2, False, 2], "bool_data_2": [4, 2, True, 3]***REMOVED***,
            index=["count", "unique", "top", "freq"],
        )
        tm.assert_frame_equal(result, expected)
        ...
        result = df.describe()
        expected = DataFrame(
            {"int_data": [5, 2, df.int_data.std(), 0, 1, 2, 3, 4]***REMOVED***,
            index=["count", "mean", "std", "min", "25%", "50%", "75%", "max"],
        )
        tm.assert_frame_equal(result, expected)

        df = DataFrame(
            {"bool_data": [False, False, True, True], "str_data": ["a", "b", "c", "a"]***REMOVED***
        )
        result = df.describe()
        expected = DataFrame(
            {"bool_data": [4, 2, False, 2], "str_data": [4, 3, "a", 2]***REMOVED***,
            index=["count", "unique", "top", "freq"],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_describe_categorical(self):
        ...
        df3 = DataFrame({"cat": cat, "s": ["a", "b", "c", "c"]***REMOVED***)
        result = df3.describe()
    ...
    def test_describe_empty_categorical_column(self):
        ...
        df = DataFrame({"empty_col": Categorical([])***REMOVED***)
        result = df.describe()
        expected = DataFrame(
            {"empty_col": [0, 0, np.nan, np.nan]***REMOVED***,
            index=["count", "unique", "top", "freq"],
            dtype="object",
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_describe_timedelta_values(self):
        ...
        df = DataFrame({"t1": t1, "t2": t2***REMOVED***)
    ...
    def test_datetime_is_numeric_includes_datetime(self):
        df = DataFrame({"a": date_range("2012", periods=3), "b": [1, 2, 3]***REMOVED***)
        result = df.describe()
        expected = DataFrame(
            {
                "a": [
    ...
    def test_describe_percentiles_integer_idx(self):
        ...
        expected = DataFrame(
            {"x": [1.0, 1.0, np.nan, 1.0, *(1.0 for _ in pct), 1.0]***REMOVED***,
            index=[
                "count",
                "mean",
                "std",
    ...
    def test_describe_does_not_raise_error_for_dictlike_elements(self):
        # GH#32409
        df = DataFrame([{"test": {"a": "1"***REMOVED******REMOVED***, {"test": {"a": "2"***REMOVED******REMOVED***])
        expected = DataFrame(
            {"test": [2, 2, {"a": "1"***REMOVED***, 1]***REMOVED***, index=["count", "unique", "top", "freq"]
        )
        result = df.describe()
        tm.assert_frame_equal(result, expected)
    ...
    def test_describe_when_include_all_exclude_not_allowed(self, exclude):
        ...
        """
        df = DataFrame({"x": [1], "y": [2], "z": [3]***REMOVED***)
    ...
    def test_ea_with_na(self, any_numeric_ea_dtype):
        ...
        df = DataFrame({"a": [1, pd.NA, pd.NA], "b": pd.NA***REMOVED***, dtype=any_numeric_ea_dtype)
        result = df.describe()
        expected = DataFrame(
            {"a": [1.0, 1.0, pd.NA] + [1.0] * 5, "b": [0.0] + [pd.NA] * 7***REMOVED***,
            index=["count", "mean", "std", "min", "25%", "50%", "75%", "max"],
            dtype="Float64",
        )
        tm.assert_frame_equal(result, expected)
