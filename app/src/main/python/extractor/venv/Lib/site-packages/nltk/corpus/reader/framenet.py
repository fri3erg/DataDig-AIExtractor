...
def _pretty_any(obj):
    ...
    for k in obj:
        if isinstance(obj[k], str) and len(obj[k]) > 65:
            outstr += f"[{k***REMOVED***]\n"
            outstr += "{***REMOVED***".format(_pretty_longstring(obj[k], prefix="  "))
            outstr += "\n"
        else:
            outstr += f"[{k***REMOVED***] {obj[k]***REMOVED***\n"
...
def _pretty_semtype(st):
    ...
    outstr = ""
    outstr += "semantic type ({0.ID***REMOVED***): {0.name***REMOVED***\n".format(st)
    if "abbrev" in semkeys:
        outstr += f"[abbrev] {st.abbrev***REMOVED***\n"
    if "definition" in semkeys:
        outstr += "[definition]\n"
        outstr += _pretty_longstring(st.definition, "  ")
    outstr += f"[rootType] {st.rootType.name***REMOVED***({st.rootType.ID***REMOVED***)\n"
    if st.superType is None:
        outstr += "[superType] <None>\n"
    else:
        outstr += f"[superType] {st.superType.name***REMOVED***({st.superType.ID***REMOVED***)\n"
    outstr += f"[subTypes] {len(st.subTypes)***REMOVED*** subtypes\n"
    outstr += (
        "  "
        + ", ".join(f"{x.name***REMOVED***({x.ID***REMOVED***)" for x in st.subTypes)
        + "\n" * (len(st.subTypes) > 0)
    )
    return outstr
...
def _pretty_frame_relation_type(freltyp):
    ...
    :rtype: str
    """
    outstr = "<frame relation type ({0.ID***REMOVED***): {0.superFrameName***REMOVED*** -- {0.name***REMOVED*** -> {0.subFrameName***REMOVED***>".format(
        freltyp
    )
    return outstr
...
def _pretty_frame_relation(frel):
    ...
    :rtype: str
    """
    outstr = "<{0.type.superFrameName***REMOVED***={0.superFrameName***REMOVED*** -- {0.type.name***REMOVED*** -> {0.type.subFrameName***REMOVED***={0.subFrameName***REMOVED***>".format(
        frel
    )
    return outstr
...
def _pretty_fe_relation(ferel):
    ...
    :rtype: str
    """
    outstr = "<{0.type.superFrameName***REMOVED***={0.frameRelation.superFrameName***REMOVED***.{0.superFEName***REMOVED*** -- {0.type.name***REMOVED*** -> {0.type.subFrameName***REMOVED***={0.frameRelation.subFrameName***REMOVED***.{0.subFEName***REMOVED***>".format(
        ferel
    )
    return outstr
...
def _pretty_lu(lu):
    ...
    lukeys = lu.keys()
    outstr = ""
    outstr += "lexical unit ({0.ID***REMOVED***): {0.name***REMOVED***\n\n".format(lu)
    if "definition" in lukeys:
        outstr += "[definition]\n"
        outstr += _pretty_longstring(lu.definition, "  ")
    if "frame" in lukeys:
        outstr += f"\n[frame] {lu.frame.name***REMOVED***({lu.frame.ID***REMOVED***)\n"
    if "incorporatedFE" in lukeys:
        outstr += f"\n[incorporatedFE] {lu.incorporatedFE***REMOVED***\n"
    if "POS" in lukeys:
        outstr += f"\n[POS] {lu.POS***REMOVED***\n"
    if "status" in lukeys:
        outstr += f"\n[status] {lu.status***REMOVED***\n"
    if "totalAnnotated" in lukeys:
        outstr += f"\n[totalAnnotated] {lu.totalAnnotated***REMOVED*** annotated examples\n"
    if "lexemes" in lukeys:
        outstr += "\n[lexemes] {***REMOVED***\n".format(
            " ".join(f"{lex.name***REMOVED***/{lex.POS***REMOVED***" for lex in lu.lexemes)
        )
    if "semTypes" in lukeys:
        outstr += f"\n[semTypes] {len(lu.semTypes)***REMOVED*** semantic types\n"
        outstr += (
            "  " * (len(lu.semTypes) > 0)
            + ", ".join(f"{x.name***REMOVED***({x.ID***REMOVED***)" for x in lu.semTypes)
            + "\n" * (len(lu.semTypes) > 0)
        )
    if "URL" in lukeys:
        outstr += f"\n[URL] {lu.URL***REMOVED***\n"
    if "subCorpus" in lukeys:
        ...
        outstr += f"\n[subCorpus] {len(lu.subCorpus)***REMOVED*** subcorpora\n"
        for line in textwrap.fill(", ".join(sorted(subc)), 60).split("\n"):
            outstr += f"  {line***REMOVED***\n"
    if "exemplars" in lukeys:
        outstr += "\n[exemplars] {***REMOVED*** sentences across all subcorpora\n".format(
            len(lu.exemplars)
        )
...
def _pretty_exemplars(exemplars, lu):
    ...
    outstr = ""
    outstr += "exemplar sentences for {0.name***REMOVED*** in {0.frame.name***REMOVED***:\n\n".format(lu)
    for i, sent in enumerate(exemplars):
        outstr += f"[{i***REMOVED***] {sent.text***REMOVED***\n"
    outstr += "\n"
    return outstr
...
def _pretty_fulltext_sentences(sents):
    ...
    outstr = ""
    outstr += "full-text document ({0.ID***REMOVED***) {0.name***REMOVED***:\n\n".format(sents)
    outstr += "[corpid] {0.corpid***REMOVED***\n[corpname] {0.corpname***REMOVED***\n[description] {0.description***REMOVED***\n[URL] {0.URL***REMOVED***\n\n".format(
        sents
    )
    outstr += f"[sentence]\n"
...
def _pretty_fulltext_sentence(sent):
    ...
    outstr = ""
    outstr += "full-text sentence ({0.ID***REMOVED***) in {1***REMOVED***:\n\n".format(
        sent, sent.doc.get("name", sent.doc.description)
    )
    outstr += f"\n[POS] {len(sent.POS)***REMOVED*** tags\n"
    outstr += f"\n[POS_tagset] {sent.POS_tagset***REMOVED***\n\n"
...
def _pretty_pos(aset):
    ...
    outstr = ""
    outstr += "POS annotation set ({0.ID***REMOVED***) {0.POS_tagset***REMOVED*** in sentence {0.sent.ID***REMOVED***:\n\n".format(
        aset
    )
...
***REMOVED***  # '~' to prevent line wrapping
...
def _pretty_annotation(sent, aset_level=False):
    ...
    outstr += f" ({sent.ID***REMOVED***):\n"
    if aset_level:  # TODO: any UNANN exemplars?
        outstr += f"\n[status] {sent.status***REMOVED***\n"
    for k in ("corpID", "docID", "paragNo", "sentNo", "aPos"):
        if k in sentkeys:
            outstr += f"[{k***REMOVED***] {sent[k]***REMOVED***\n"
    outstr += (
        "\n[LU] ({0.ID***REMOVED***) {0.name***REMOVED*** in {0.frame.name***REMOVED***\n".format(sent.LU)
        if sent.LU
        else "\n[LU] Not found!"
    )
    outstr += "\n[frame] ({0.ID***REMOVED***) {0.name***REMOVED***\n".format(
        sent.frame
    )  # redundant with above, but .frame is convenient
    if not aset_level:
        outstr += "\n[annotationSet] {***REMOVED*** annotation sets\n".format(
            len(sent.annotationSet)
        )
        outstr += f"\n[POS] {len(sent.POS)***REMOVED*** tags\n"
        outstr += f"\n[POS_tagset] {sent.POS_tagset***REMOVED***\n"
    outstr += "\n[GF] {***REMOVED*** relation{***REMOVED***\n".format(
        len(sent.GF), "s" if len(sent.GF) != 1 else ""
    )
    outstr += "\n[PT] {***REMOVED*** phrase{***REMOVED***\n".format(
...
***REMOVED***:  # same target, same frame
...
def _annotation_ascii_FEs(sent):
    ...
    for lyr in ("Verb", "Noun", "Adj", "Adv", "Prep", "Scon", "Art"):
        if lyr in sent and sent[lyr]:
            for a, b, lbl in sent[lyr]:
                if (
                    lbl == "X"
    ***REMOVED***:  # skip this, which covers an entire phrase typically containing the target and all its FEs
    ...
    ***REMOVED***  # lowercase Cop=>cop, Non-Asp=>nonasp, etc. to distinguish from FE names
    if posspec_separate:
        POSSPEC = _annotation_ascii_FE_layer(posspec, {***REMOVED***, feAbbrevs)
...
def _pretty_fe(fe):
    ...
    fekeys = fe.keys()
    outstr = ""
    outstr += "frame element ({0.ID***REMOVED***): {0.name***REMOVED***\n    of {1.name***REMOVED***({1.ID***REMOVED***)\n".format(
        fe, fe.frame
    )
    ...
    if "abbrev" in fekeys:
        outstr += f"[abbrev] {fe.abbrev***REMOVED***\n"
    if "coreType" in fekeys:
        outstr += f"[coreType] {fe.coreType***REMOVED***\n"
    if "requiresFE" in fekeys:
        outstr += "[requiresFE] "
        if fe.requiresFE is None:
            outstr += "<None>\n"
        else:
            outstr += f"{fe.requiresFE.name***REMOVED***({fe.requiresFE.ID***REMOVED***)\n"
    if "excludesFE" in fekeys:
        outstr += "[excludesFE] "
        if fe.excludesFE is None:
            outstr += "<None>\n"
        else:
            outstr += f"{fe.excludesFE.name***REMOVED***({fe.excludesFE.ID***REMOVED***)\n"
    if "semType" in fekeys:
        outstr += "[semType] "
        if fe.semType is None:
            outstr += "<None>\n"
        else:
            outstr += "\n  " + f"{fe.semType.name***REMOVED***({fe.semType.ID***REMOVED***)" + "\n"
...
def _pretty_frame(frame):
    ...
    outstr = ""
    outstr += "frame ({0.ID***REMOVED***): {0.name***REMOVED***\n\n".format(frame)
    outstr += f"[URL] {frame.URL***REMOVED***\n\n"
    outstr += "[definition]\n"
    outstr += _pretty_longstring(frame.definition, "  ") + "\n"

    outstr += f"[semTypes] {len(frame.semTypes)***REMOVED*** semantic types\n"
    outstr += (
        "  " * (len(frame.semTypes) > 0)
        + ", ".join(f"{x.name***REMOVED***({x.ID***REMOVED***)" for x in frame.semTypes)
        + "\n" * (len(frame.semTypes) > 0)
    )

    outstr += "\n[frameRelations] {***REMOVED*** frame relations\n".format(
        len(frame.frameRelations)
    )
    outstr += "  " + "\n  ".join(repr(frel) for frel in frame.frameRelations) + "\n"

    outstr += f"\n[lexUnit] {len(frame.lexUnit)***REMOVED*** lexical units\n"
    lustrs = []
    for luName, lu in sorted(frame.lexUnit.items()):
        tmpstr = f"{luName***REMOVED*** ({lu.ID***REMOVED***)"
        lustrs.append(tmpstr)
    outstr += "{***REMOVED***\n".format(_pretty_longstring(", ".join(lustrs), prefix="  "))

    outstr += f"\n[FE] {len(frame.FE)***REMOVED*** frame elements\n"
    fes = {***REMOVED***
    for feName, fe in sorted(frame.FE.items()):
        try:
            fes[fe.coreType].append(f"{feName***REMOVED*** ({fe.ID***REMOVED***)")
        except KeyError:
            fes[fe.coreType] = []
            fes[fe.coreType].append(f"{feName***REMOVED*** ({fe.ID***REMOVED***)")
    for ct in sorted(
        ...
        outstr += "{:>16***REMOVED***: {***REMOVED***\n".format(ct, ", ".join(sorted(fes[ct])))

    outstr += "\n[FEcoreSets] {***REMOVED*** frame element core sets\n".format(
...
class AttrDict(dict):
    ...
    def _short_repr(self):
        if "_type" in self:
            ...
            try:
                return "<{***REMOVED*** ID={***REMOVED*** name={***REMOVED***>".format(
                    self["_type"], self["ID"], self["name"]
    ***REMOVED***
            except KeyError:
                try:  # no ID--e.g., for _type=lusubcorpus
                    return "<{***REMOVED*** name={***REMOVED***>".format(self["_type"], self["name"])
                except KeyError:  # no name--e.g., for _type=lusentence
                    return "<{***REMOVED*** ID={***REMOVED***>".format(self["_type"], self["ID"])
        else:
            return self.__repr__()
    ...
    def __repr__(self):
        ...
        for k, v in sorted(self.items()):
            ...
            try:
                ...
                kv += repr(v)
            parts.append(kv)
        return "{" + (",\n " if self._BREAK_LINES else ", ").join(parts) + "***REMOVED***"
...
class PrettyList(list):
    ...
    def __repr__(self):
        ...
        for elt in self:
            pieces.append(
                elt._short_repr()
***REMOVED***  # key difference from inherited version: call to _short_repr()
...
class FramenetCorpusReader(XMLCorpusReader):
    ...
    def __init__(self, root, fileids):
        ...
        self._cached_frames = {***REMOVED***  # name -> ID
        self._lu_idx = None
...
warnings(True) to display corpus consistency warnings when loading data
    ...
    def _buildrelationindex(self):
        ...
        self._freltyp_idx = {***REMOVED***
        self._frel_idx = {***REMOVED***
        self._frel_f_idx = defaultdict(set)
        self._ferel_idx = {***REMOVED***
    ...
    def doc(self, fn_docid):
        ...
        try:
            ...
            raise FramenetError(f"Unknown document id: {fn_docid***REMOVED***") from e

        # construct the path name for the xml file containing the document info
        locpath = os.path.join(f"{self._root***REMOVED***", self._fulltext_dir, xmlfname)
    ...
    def frame_by_id(self, fn_fid, ignorekeys=[]):
        ...
        try:
            ...
            raise FramenetError(f"Unknown frame id: {fn_fid***REMOVED***") from e
    ...
    def frame_by_name(self, fn_fname, ignorekeys=[], check_cache=True):
        ...
        locpath = os.path.join(f"{self._root***REMOVED***", self._frame_dir, fn_fname + ".xml")
        ...
        try:
            ...
            raise FramenetError(f"Unknown frame: {fn_fname***REMOVED***") from e
        ...
        ***REMOVED***:  # identity containment check
    ...
    def lu_basic(self, fn_luid):
        ...
        {'ID': 256,
         ...
         'lemmaID': 15082,
         'lexemes': [{'POS': 'V', 'breakBefore': 'false', 'headword': 'false', 'name': 'foresee', 'order': 1***REMOVED***],
         'name': 'foresee.v',
         'semTypes': [],
         'sentenceCount': {'annotated': ..., 'total': ...***REMOVED***,
         'status': 'FN1_Sent'***REMOVED***
    ...
    def lu(self, fn_luid, ignorekeys=[], luName=None, frameID=None, frameName=None):
        ...
        [{'POS': 'V', 'breakBefore': 'false', 'headword': 'false', 'name': 'foresee', 'order': 1***REMOVED***]
        ...
        if luinfo is OOV:
            ...
            self._warn(
                "LU ID not found: {***REMOVED*** ({***REMOVED***) in {***REMOVED*** ({***REMOVED***)".format(
...
***REMOVED***
        ...
        if ignorekeys:
            return AttrDict({k: v for k, v in luinfo.items() if k not in ignorekeys***REMOVED***)
    ...
    def _lu_file(self, lu, ignorekeys=[]):
        ...
        fname = f"lu{fn_luid***REMOVED***.xml"
        locpath = os.path.join(f"{self._root***REMOVED***", self._lu_dir, fname)
        # print(locpath, file=sys.stderr)
        if not self._lu_idx:
            self._buildluindex()
        ...
        try:
            ...
            raise FramenetError(f"Unknown LU id: {fn_luid***REMOVED***") from e
...
***REMOVED***
            if frame is not None:
                if isinstance(frame, int):
                    frameIDs = {frame***REMOVED***
                elif isinstance(frame, str):
                    frameIDs = {f.ID for f in self.frames(frame)***REMOVED***
                else:
                    frameIDs = {frame.ID***REMOVED***
    ...
    def exemplars(self, luNamePattern=None, frame=None, fe=None, fe2=None):
        ...
        ***REMOVED***
        ...
        if frame is not None or luNamePattern is not None:
            if frame is None or isinstance(frame, str):
                ...
                if luNamePattern is not None:
                    lusByFrame = {frame.name: self.lus(luNamePattern, frame=frame)***REMOVED***
        ...
        ***REMOVED***
            ...
            ***REMOVED***
            ...
            if fe is not None:
                frames = {ffe.frame.ID for ffe in self.fes(fe)***REMOVED***
                if fe2 is not None:
                    frames2 = {ffe.frame.ID for ffe in self.fes(fe2)***REMOVED***
        ...
        def _matching_exs():
            for f in frames:
                ...
                if fe is not None:
                    fes = (
                        {ffe for ffe in f.FE.keys() if re.search(fe, ffe, re.I)***REMOVED***
                        if isinstance(fe, str)
                        else {fe.name***REMOVED***
        ***REMOVED***
                    if fe2 is not None:
                        fes2 = (
                            {ffe for ffe in f.FE.keys() if re.search(fe2, ffe, re.I)***REMOVED***
                            if isinstance(fe2, str)
                            else {fe2.name***REMOVED***
            ***REMOVED***
...
***REMOVED***
    ...
    def fe_relations(self):
        ...
        'type': <framerelationtype ID=1 name=Inheritance>***REMOVED***
...
***REMOVED***
    ...
    def _handle_fulltextindex_elt(self, elt, tagspec=None):
        ...
        for sub in elt:
            if sub.tag.endswith("document"):
                ...
                doc.filename = f"{corpname***REMOVED***__{docname***REMOVED***.xml"
                doc.URL = (
    ...
    ***REMOVED***
                ...
                luentry["URL"] = (
                    ...
                    + self._lu_dir
                    + "/"
                    + "lu{***REMOVED***.xml".format(luentry["ID"])
    ***REMOVED***
                ...
                if "Target" in a:
                    for tspan in a.Target:
                        if tspan in target_spans:
                            self._warn(
                                'Duplicate target span "{***REMOVED***"'.format(
    ...
    def _handle_luannotationset_elt(self, elt, is_pos=False):
        ...
        for sub in elt:
            if sub.tag.endswith("layer"):
                ...
                if l is not None:
                    overt = []
                    ni = {***REMOVED***  # null instantiations
                    ...
                    for lbl in l.label:
                        if "start" in lbl:
                            ...
                            if l.name not in (
                                "Sent",
                                "Other",
                ***REMOVED***:  # 'Sent' and 'Other' layers sometimes contain accidental duplicate spans
                    ...
                    ***REMOVED***
                    ...
                    if l.name == "Target":
                        if not overt:
                            self._warn(
                                "Skipping empty Target layer in annotation set ID={***REMOVED***".format(
                                    info.ID
                    ***REMOVED***
                ***REMOVED***
                            continue
                        ...
                        if "Target" in info:
                            self._warn(
                                "Annotation set {***REMOVED*** has multiple Target layers".format(
                                    info.ID
                    ***REMOVED***
                ***REMOVED***
                        else:
        ...
        if not is_pos and "cxnID" not in info:
            if "Target" not in info:
                self._warn(f"Missing target in annotation set ID={info.ID***REMOVED***")
            assert "FE" in info
            if "FE3" in info:
                assert "FE2" in info
...
def demo():
    ...
    print(f'Found {len(medframes)***REMOVED*** Frames whose name matches "(?i)medical":')
    ...
    #
    print(
        '\nNumber of frame relations for the "{***REMOVED***" ({***REMOVED***) frame:'.format(
    ...
    #
    print(
        f'\nNumber of Frame Elements in the "{m_frame.name***REMOVED***" frame:',
        len(m_frame.FE),
    )
    ...
    #
    print(f'\nThe "core" Frame Elements in the "{m_frame.name***REMOVED***" frame:')
    ...
    #
    print(
        f'\nNumber of Lexical Units in the "{m_frame.name***REMOVED***" frame:',
        len(m_frame.lexUnit),
    )
    ...
    print(f"\nInformation on the LU: {luinfo.name***REMOVED***")
    pprint(luinfo)
    ...
    allcorpora = {x.corpname for x in fn.docs_metadata()***REMOVED***
    ...
    print(f'\nNames of the annotated documents in the "{firstcorp***REMOVED***" corpus:')
