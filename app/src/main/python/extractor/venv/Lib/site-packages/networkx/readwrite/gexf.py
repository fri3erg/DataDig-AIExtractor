...
def write_gexf(G, path, encoding="utf-8", prettyprint=True, version="1.2draft"):
    ...
    # visualization data
    >>> G.nodes[0]["viz"] = {"size": 54***REMOVED***
    >>> G.nodes[0]["viz"]["position"] = {"x": 0, "y": 1***REMOVED***
    >>> G.nodes[0]["viz"]["color"] = {"r": 0, "g": 0, "b": 256***REMOVED***
...
***REMOVED***,
    ...
    ***REMOVED***
    ...
    def set_version(self, version):
        ...
        if d is None:
            raise nx.NetworkXError(f"Unknown GEXF version {version***REMOVED***.")
...
class GEXFWriter(GEXF):
    ...
    def __init__(
        ...
        subelement_text = f"NetworkX {nx.__version__***REMOVED***"
        ...
        self.attr = {***REMOVED***
        self.attr["node"] = {***REMOVED***
        self.attr["edge"] = {***REMOVED***
        self.attr["node"]["dynamic"] = {***REMOVED***
        self.attr["node"]["static"] = {***REMOVED***
        self.attr["edge"]["dynamic"] = {***REMOVED***
        self.attr["edge"]["static"] = {***REMOVED***
    ...
    def add_nodes(self, G, graph_element):
        ...
        for node, data in G.nodes(data=True):
            ...
            default = G.graph.get("node_default", {***REMOVED***)
    ...
    def add_edges(self, G, graph_element):
        ...
        for u, v, key, edge_data in edge_key_data(G):
            ...
            default = G.graph.get("edge_default", {***REMOVED***)
            if self.VERSION == "1.1":
    ...
    def add_attributes(self, node_or_edge, xml_obj, data, default):
        ...
        for k, v in data.items():
            ...
            if val_type not in self.xml_type:
                raise TypeError(f"attribute value type is not allowed: {val_type***REMOVED***")
    ...
    ***REMOVED***
    ...
    def get_attr_id(self, title, attr_type, edge_or_node, default, mode):
        ...
        try:
            ...
            attr_kwargs = {"id": new_id, "title": title, "type": attr_type***REMOVED***
            ...
            if attributes_element is None:
                ...
                attr_kwargs = {"mode": mode, "class": edge_or_node***REMOVED***
    ...
    def add_viz(self, element, node_data):
        ...
        if viz:
            ...
            if color is not None:
                if self.VERSION == "1.1":
                    e = Element(
                        f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***color",
                        r=str(color.get("r")),
                        g=str(color.get("g")),
                        b=str(color.get("b")),
        ***REMOVED***
        ...
        ***REMOVED***
            ...
            size = viz.get("size")
            if size is not None:
                e = Element(f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***size", value=str(size))
                element.append(e)

            thickness = viz.get("thickness")
            if thickness is not None:
                e = Element(f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***thickness", value=str(thickness))
                element.append(e)
            ...
            if shape is not None:
                if shape.startswith("http"):
                    e = Element(
                        f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***shape", value="image", uri=str(shape)
        ***REMOVED***
                else:
                    e = Element(f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***shape", value=str(shape))
                element.append(e)

            position = viz.get("position")
            if position is not None:
                e = Element(
                    f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***position",
...
class GEXFReader(GEXF):
    ...
    def __call__(self, stream):
        ...
        g = self.xml.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***graph")
        if g is not None:
    ...
    def make_graph(self, graph_xml):
        ...
        attributes_elements = graph_xml.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***attributes")
        # dictionaries to hold attributes and attribute defaults
        node_attr = {***REMOVED***
        node_default = {***REMOVED***
        edge_attr = {***REMOVED***
        edge_default = {***REMOVED***
        ...
        ea = {"weight": {"type": "double", "mode": "static", "title": "weight"***REMOVED******REMOVED***
        ed = {***REMOVED***
        edge_attr.update(ea)
        edge_default.update(ed)
        G.graph["edge_default"] = edge_default
        ...
        # add nodes
        nodes_element = graph_xml.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***nodes")
        if nodes_element is not None:
            for node_xml in nodes_element.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***node"):
                self.add_node(G, node_xml, node_attr)

        # add edges
        edges_element = graph_xml.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***edges")
        if edges_element is not None:
            for edge_xml in edges_element.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***edge"):
    ...
    def add_node(self, G, node_xml, node_attr, node_pid=None):
        ...
        subnodes = node_xml.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***nodes")
        if subnodes is not None:
            for node_xml in subnodes.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***node"):
    ...
    def add_viz(self, data, node_xml):
        ...
        viz = {***REMOVED***
        color = node_xml.find(f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***color")
        if color is not None:
            if self.VERSION == "1.1":
                viz["color"] = {
                    "r": int(color.get("r")),
        ...
        size = node_xml.find(f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***size")
        if size is not None:
            viz["size"] = float(size.get("value"))

        thickness = node_xml.find(f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***thickness")
        if thickness is not None:
            viz["thickness"] = float(thickness.get("value"))

        shape = node_xml.find(f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***shape")
        if shape is not None:
            viz["shape"] = shape.get("shape")
            if viz["shape"] == "image":
                viz["shape"] = shape.get("uri")

        position = node_xml.find(f"{{{self.NS_VIZ***REMOVED******REMOVED******REMOVED***position")
        if position is not None:
            viz["position"] = {
    ...
    def add_parents(self, data, node_xml):
        parents_element = node_xml.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***parents")
        if parents_element is not None:
            data["parents"] = []
            for p in parents_element.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***parent"):
                parent = p.get("for")
                data["parents"].append(parent)
        return data

    def add_slices(self, data, node_or_edge_xml):
        slices_element = node_or_edge_xml.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***slices")
        if slices_element is not None:
            data["slices"] = []
            for s in slices_element.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***slice"):
                start = s.get("start")
                end = s.get("end")
    ...
    def add_spells(self, data, node_or_edge_xml):
        spells_element = node_or_edge_xml.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***spells")
        if spells_element is not None:
            data["spells"] = []
            ttype = self.timeformat
            for s in spells_element.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***spell"):
    ...
    def decode_attr_elements(self, gexf_keys, obj_xml):
        ...
        attr_element = obj_xml.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***attvalues")
        if attr_element is not None:
            # loop over <attvalue> elements
            for a in attr_element.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***attvalue"):
                key = a.get("for")  # for is required
                try:  # should be in our gexf_keys dictionary
                    title = gexf_keys[key]["title"]
                except KeyError as err:
                    raise nx.NetworkXError(f"No attribute defined for={key***REMOVED***.") from err
    ...
    def find_gexf_attributes(self, attributes_element):
        ...
        for k in attributes_element.findall(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***attribute"):
            attr_id = k.get("id")
            title = k.get("title")
            atype = k.get("type")
            attrs[attr_id] = {"title": title, "type": atype, "mode": mode***REMOVED***
            # check for the 'default' subelement of key element and add
            default = k.find(f"{{{self.NS_GEXF***REMOVED******REMOVED******REMOVED***default")
