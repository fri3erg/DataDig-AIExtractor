...
def test_basic(using_infer_string):  # TODO: split this test
    ...
    data = DataFrame({"a": [1, 1, 1, 2, 2, 2, 3, 4, 5], "b": cats***REMOVED***)

    exp_index = CategoricalIndex(list("abcd"), name="b", ordered=True)
    expected = DataFrame({"a": [1, 2, 4, np.nan]***REMOVED***, index=exp_index)
    result = data.groupby("b", observed=False).mean()
    tm.assert_frame_equal(result, expected)

    cat1 = Categorical(["a", "a", "b", "b"], categories=["a", "b", "z"], ordered=True)
    cat2 = Categorical(["c", "d", "c", "d"], categories=["c", "d", "y"], ordered=True)
    df = DataFrame({"A": cat1, "B": cat2, "values": [1, 2, 3, 4]***REMOVED***)

    # single grouper
    gb = df.groupby("A", observed=False)
    exp_idx = CategoricalIndex(["a", "b", "z"], name="A", ordered=True)
    expected = DataFrame({"values": Series([3, 7, 0], index=exp_idx)***REMOVED***)
    ...
    # GH 9921
    # Monotonic
    df = DataFrame({"a": [5, 15, 25]***REMOVED***)
    ...
    # Non-monotonic
    df = DataFrame({"a": [5, 15, 25, -5]***REMOVED***)
    ...
    # GH 9603
    df = DataFrame({"a": [1, 0, 0, 0]***REMOVED***)
...
def test_apply(ordered):
    ...
    df = DataFrame({"missing": missing, "dense": dense, "values": values***REMOVED***)
...
def test_observed(observed):
    ...
    expected = DataFrame({"values": Series([1, 2, 3, 4], index=exp_index)***REMOVED***).sort_index()
    result = gb.sum()
    if not observed:
    ...
    expected = DataFrame(
        {"values": [1, 2, 3, 4], "C": ["foo", "bar", "foo", "bar"]***REMOVED***, index=exp_index
    )
    result = gb.sum()
    if not observed:
        expected = cartesian_product_for_groupers(
    ...
    ***REMOVED***
    ...
    expected = DataFrame({"ints": [1.5, 1.5], "val": [20.0, 30]***REMOVED***, index=exp_index)
    if not observed:
...
def test_observed_codes_remap(observed):
    d = {"C1": [3, 3, 4, 5], "C2": [1, 2, 3, 4], "C3": [10, 100, 200, 34]***REMOVED***
    df = DataFrame(d)
    ...
    expected = DataFrame(
        {"C1": [3.0, 3.0, 4.0, 5.0], "C3": [10.0, 100.0, 200.0, 34.0]***REMOVED***, index=idx
    )
    if not observed:
        expected = cartesian_product_for_groupers(
...
def test_observed_groups(observed):
    ...
    df = DataFrame({"cat": cat, "vals": [1, 2, 3]***REMOVED***)
    g = df.groupby("cat", observed=observed)

    result = g.groups
    if observed:
        expected = {"a": Index([0, 2], dtype="int64"), "c": Index([1], dtype="int64")***REMOVED***
    else:
        expected = {
...
def test_unobserved_in_index(keys, expected_values, expected_index_levels, test_series):
    ...
    expected = DataFrame({"c": expected_values***REMOVED***, index=index)
    if test_series:
        expected = expected["c"]
...
def test_observed_groups_with_nan(observed):
    ...
    result = g.groups
    if observed:
        expected = {"a": Index([0, 2], dtype="int64")***REMOVED***
    else:
        expected = {
            "a": Index([0, 2], dtype="int64"),
            "b": Index([], dtype="int64"),
...
def test_observed_nth():
    ...
    ser = Series([1, 2, 3])
    df = DataFrame({"cat": cat, "ser": ser***REMOVED***)
...
def test_dataframe_categorical_with_nan(observed):
    ...
    if observed:
        expected = DataFrame(
            {"s1": Categorical(["a"], categories=["a", "b", "c"]), "s2": [2]***REMOVED***
        )
    else:
        expected = DataFrame(
            {
...
def test_dataframe_categorical_ordered_observed_sort(ordered, observed, sort):
    ...
    df = DataFrame({"label": label, "val": val***REMOVED***)
    ...
    if not all(label == aggr):
        msg = (
            ...
            f"for (ordered={ordered***REMOVED***, observed={observed***REMOVED***, sort={sort***REMOVED***)\n"
            f"Result:\n{result***REMOVED***"
        )
        assert False, msg
...
def test_unstack_categorical():
    ...
    df = DataFrame(
        {"a": range(10), "medium": ["A", "B"] * 5, "artist": list("XYXXY") * 2***REMOVED***
    )
...
def test_preserve_categories():
    ...
    # ordered=True
    df = DataFrame({"A": Categorical(list("ba"), categories=categories, ordered=True)***REMOVED***)
    ...
    # ordered=False
    df = DataFrame({"A": Categorical(list("ba"), categories=categories, ordered=False)***REMOVED***)
...
def test_preserve_on_ordered_ops(func, values):
    ...
    df = DataFrame({"payload": [-1, -2, -1, -2], "col": c***REMOVED***)
    g = df.groupby("payload")
    result = getattr(g, func)()
    expected = DataFrame(
        {"payload": [-2, -1], "col": Series(values, dtype=c.dtype)***REMOVED***
    ).set_index("payload")
    tm.assert_frame_equal(result, expected)
...
def test_groupby_empty_with_category():
    ...
    df = DataFrame({"A": [None] * 3, "B": Categorical(["train", "train", "test"])***REMOVED***)
...
def test_sort():
    ...
    df = DataFrame({"value": np.random.default_rng(2).integers(0, 10000, 100)***REMOVED***)
    labels = [f"{i***REMOVED*** - {i+499***REMOVED***" for i in range(0, 10000, 500)]
...
def test_empty_sum():
    ...
    df = DataFrame(
        {"A": Categorical(["a", "a", "b"], categories=["a", "b", "c"]), "B": [1, 2, 1]***REMOVED***
    )
...
***REMOVED***,
    ...
    expected = DataFrame({"values": [0, 4, 8, 3, 4, 5, 6, np.nan, 2]***REMOVED***, index=idx)
    tm.assert_frame_equal(result, expected)
...
def test_groupby_agg_observed_true_single_column(as_index, expected):
    # GH-23970
    df = DataFrame(
        {"a": Series([1, 1, 2], dtype="category"), "b": [1, 2, 2], "x": [1, 2, 3]***REMOVED***
    )
...
def test_seriesgroupby_observed_apply_dict(df_cat, observed, index, data):
    ...
    result = df_cat.groupby(["A", "B"], observed=observed)["C"].apply(
        lambda x: {"min": x.min(), "max": x.max()***REMOVED***
    )
    tm.assert_series_equal(result, expected)
...
def test_groupby_categorical_axis_1(code):
    # GH 13420
    df = DataFrame({"a": [1, 2, 3, 4], "b": [-1, -2, -3, -4], "c": [5, 6, 7, 8]***REMOVED***)
...
def test_groupby_cat_preserves_structure(observed, ordered):
    # GH 28787
    df = DataFrame(
        {"Name": Categorical(["Bob", "Greg"], ordered=ordered), "Item": [1, 2]***REMOVED***,
        columns=["Name", "Item"],
    )
    expected = df.copy()
...
def test_get_nonexistent_category():
    ...
    df = DataFrame({"var": ["a", "a", "b", "b"], "val": range(4)***REMOVED***)
    with pytest.raises(KeyError, match="'vau'"):
        df.groupby("var").apply(
            lambda rows: DataFrame(
                {"var": [rows.iloc[-1]["var"]], "val": [rows.iloc[-1]["vau"]]***REMOVED***
***REMOVED***
        )
...
def test_series_groupby_categorical_aggregation_getitem():
    # GH 8870
    d = {"foo": [10, 8, 4, 1], "bar": [10, 20, 30, 40], "baz": ["d", "c", "d", "c"]***REMOVED***
    df = DataFrame(d)
...
def test_groupby_agg_categorical_columns(func, expected_values):
    ...
    expected = DataFrame(
        {"value": expected_values***REMOVED***, index=Index([0, 1, 2], name="groups")
    )
    tm.assert_frame_equal(result, expected)


def test_groupby_agg_non_numeric():
    df = DataFrame({"A": Categorical(["a", "a", "b"], categories=["a", "b", "c"])***REMOVED***)
    expected = DataFrame({"A": [2, 1]***REMOVED***, index=np.array([1, 2]))
...
def test_groupby_first_returned_categorical_instead_of_dataframe(func):
    ...
    df = DataFrame({"A": [1997], "B": Series(["b"], dtype="category").cat.as_ordered()***REMOVED***)
...
def test_read_only_category_no_sort():
    ...
    df = DataFrame(
        {"a": [1, 3, 5, 7], "b": Categorical([1, 1, 2, 2], categories=Index(cats))***REMOVED***
    )
    expected = DataFrame(data={"a": [2.0, 6.0]***REMOVED***, index=CategoricalIndex(cats, name="b"))
...
def test_sorted_missing_category_values():
    ...
    expected = DataFrame(
        {
            "tiny": {"A": 0, "C": 0***REMOVED***,
            "small": {"A": 0, "C": 1***REMOVED***,
            "medium": {"A": 1, "C": 1***REMOVED***,
            "large": {"A": 3, "C": 2***REMOVED***,
        ***REMOVED***
    )
...
def test_agg_cython_category_not_implemented_fallback():
    ...
    df = DataFrame({"col_num": [1, 1, 2, 3]***REMOVED***)
    ...
    result = df.groupby("col_num").agg({"col_cat": "first"***REMOVED***)
...
def test_aggregate_categorical_with_isnan():
    ...
    df = df.astype({"categorical_col": "category"***REMOVED***)
...
def test_series_groupby_first_on_categorical_col_grouped_on_2_categoricals(
    ...
    val = [0, 1, 1, 0]
    df = DataFrame({"a": cat, "b": cat, "c": val***REMOVED***)
...
def test_groupby_categorical_observed_nunique():
    # GH#45128
    df = DataFrame({"a": [1, 2], "b": [1, 2], "c": [10, 11]***REMOVED***)
    df = df.astype(dtype={"a": "category", "b": "category"***REMOVED***)
...
def test_groupby_categorical_aggregate_functions():
    ...
    ).astype({"description": dtype***REMOVED***)
...
def test_groupby_categorical_dropna(observed, dropna):
    ...
    df = DataFrame({"x": Categorical([1, 2], categories=[1, 2, 3]), "y": [3, 4]***REMOVED***)
    gb = df.groupby("x", observed=observed, dropna=dropna)
    result = gb.sum()

    if observed:
        expected = DataFrame({"y": [3, 4]***REMOVED***, index=cat)
    else:
        index = CategoricalIndex([1, 2, 3], [1, 2, 3])
        expected = DataFrame({"y": [3, 4, 0]***REMOVED***, index=index)
...
def test_many_categories(as_index, sort, index_kind, ordered):
    ...
    df = DataFrame({"a": grouper, "b": range(4)***REMOVED***)
    if index_kind == "range":
        keys = ["a"]
    elif index_kind == "single":
        keys = ["a"]
    ...
    if as_index:
        expected = DataFrame({"b": data***REMOVED***)
        if index_kind == "multi":
            expected.index = MultiIndex.from_frame(DataFrame({"a": index, "a2": index***REMOVED***))
        else:
            expected.index = index
    elif index_kind == "multi":
        expected = DataFrame({"a": Series(index), "a2": Series(index), "b": data***REMOVED***)
    else:
        expected = DataFrame({"a": Series(index), "b": data***REMOVED***)
...
def test_groupby_default_depr(cat_columns, keys):
    # GH#43999
    df = DataFrame({"a": [1, 1, 2, 3], "b": [4, 5, 6, 7]***REMOVED***)
...
def test_agg_list(request, as_index, observed, reduction_func, test_series, keys):
    ...
    df = df.astype({"a1": "category", "a2": "category"***REMOVED***)
    if "a2" not in keys:
