...
def document_attribute(
    ...
    if include_signature:
        full_attr_name = f"{section.context.get('qualifier', '')***REMOVED***{attr_name***REMOVED***"
...
def document_identifier(
    ...
    if include_signature:
        full_identifier_name = (
            f"{section.context.get('qualifier', '')***REMOVED***{identifier_model.name***REMOVED***"
        )
        section.style.start_sphinx_py_attr(full_identifier_name)
    description = get_identifier_description(
        resource_name, identifier_model.name
    )
    section.write(f'*(string)* {description***REMOVED***')


def document_reference(section, reference_model, include_signature=True):
    if include_signature:
        full_reference_name = (
            f"{section.context.get('qualifier', '')***REMOVED***{reference_model.name***REMOVED***"
        )
        section.style.start_sphinx_py_attr(full_reference_name)
    reference_type = f'(:py:class:`{reference_model.resource.type***REMOVED***`) '
    section.write(reference_type)
    section.include_doc_string(
        f'The related {reference_model.name***REMOVED*** if set, otherwise ``None``.'
    )
