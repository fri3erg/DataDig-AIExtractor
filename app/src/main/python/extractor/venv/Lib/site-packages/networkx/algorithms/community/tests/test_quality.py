...
class TestPerformance:
    ...
    def test_bad_partition(self):
        ...
        G = barbell_graph(3, 0)
        partition = [{0, 1, 4***REMOVED***, {2, 3, 5***REMOVED***]
...
def test_modularity():
    ...
    C = [{0, 1, 4***REMOVED***, {2, 3, 5***REMOVED***]
    assert (-16 / (14**2)) == pytest.approx(modularity(G, C), abs=1e-7)
    C = [{0, 1, 2***REMOVED***, {3, 4, 5***REMOVED***]
...
def test_modularity_resolution():
    ...
    C = [{0, 1, 2, 3***REMOVED***, {9, 10, 11, 12***REMOVED***, {5, 6, 7***REMOVED***, {4***REMOVED***, {8***REMOVED***]
    gamma = 1
    result = modularity(G, C, resolution=gamma)
...
def test_inter_community_edges_with_digraphs():
    ...
    partition = [{0***REMOVED***, {1***REMOVED***]
    assert inter_community_edges(G, partition) == 2

    G = nx.complete_graph(10, create_using=nx.DiGraph())
    partition = [{0***REMOVED***, {1, 2***REMOVED***, {3, 4, 5***REMOVED***, {6, 7, 8, 9***REMOVED***]
    assert inter_community_edges(G, partition) == 70

    G = nx.cycle_graph(4, create_using=nx.DiGraph())
    partition = [{0, 1***REMOVED***, {2, 3***REMOVED***]
    assert inter_community_edges(G, partition) == 2
