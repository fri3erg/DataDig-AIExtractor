...
class Tokenizer(object):
    ...
    ***REMOVED***
    ...
    TOKEN_ENDERS = ',;***REMOVED***) +-*/^&=><%'  # Each of these characters, marks the
    ...
    def _parse(self):
        ...
        consumers = (
            ...
            ('{(', self._parse_opener),
            (')***REMOVED***', self._parse_closer),
            (';,', self._parse_separator),
        )
        dispatcher = {***REMOVED***  # maps chars to the specific parsing function
    ...
    def _parse_string(self):
        ...
        if match is None:
            ...
            raise TokenizerError(f"Reached end of formula while parsing {subtype***REMOVED*** in {self.formula***REMOVED***")
        match = match.group(0)
        if delim == '"':
    ...
    def _parse_brackets(self):
        ...
        raise TokenizerError(f"Encountered unmatched '[' in {self.formula***REMOVED***")
    ...
    def _parse_error(self):
        ...
        for err in self.ERROR_CODES:
            if subformula.startswith(err):
                ...
                del self.token[:]
                return len(err)
        raise TokenizerError(f"Invalid error code at position {self.offset***REMOVED*** in '{self.formula***REMOVED***'")
...
***REMOVED***
    ...
    def _parse_closer(self):
        """
        Consumes a ***REMOVED*** or ) character.

        Returns the number of characters consumed. (Does not update
        self.offset)

        """
        assert self.formula[self.offset] in (')', '***REMOVED***')
    ...
    def assert_empty_token(self, can_follow=()):
        ...
        if self.token and self.token[-1] not in can_follow:
            raise TokenizerError(f"Unexpected character at position {self.offset***REMOVED*** in '{self.formula***REMOVED***'")
...
class Token(object):
    ...
    * `subtype`: A string identifying subtype of the token (optional, and
    ...
    def __repr__(self):
        return u"{0***REMOVED*** {1***REMOVED*** {2***REMOVED***:".format(self.type, self.subtype, self.value)
    ...
    # and 'CLOSE' is used when parsing the closing expression token ('***REMOVED***' or
    # ')').
    ...
    def make_subexp(cls, value, func=False):
        ...
        if func:
            ...
            type_ = Token.FUNC
        subtype = cls.CLOSE if value in ')***REMOVED***' else cls.OPEN
    ...
    def get_closer(self):
        ...
        value = "***REMOVED***" if self.type == self.ARRAY else ")"
