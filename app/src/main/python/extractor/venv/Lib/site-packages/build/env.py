...
class DefaultIsolatedEnv(IsolatedEnv):
    ...
    def make_extra_environ(self) -> dict[str, str]:
        ...
        return {'PATH': os.pathsep.join([self._scripts_dir, path]) if path is not None else self._scripts_dir***REMOVED***
    ...
    def install(self, requirements: Collection[str]) -> None:
        ...
        self.log(f'Installing packages in isolated environment... ({", ".join(sorted(requirements))***REMOVED***)')
...
def _create_isolated_env_venv(path: str) -> tuple[str, str]:
    ...
    if current_pip_version < packaging.version.Version(minimum_pip_version):
        _subprocess([executable, '-m', 'pip', 'install', f'pip>={minimum_pip_version***REMOVED***'])
...
def _find_executable_and_scripts(path: str) -> tuple[str, str, str]:
    ...
    if not os.path.exists(executable):
        msg = f'Virtual environment creation failed, executable {executable***REMOVED*** missing'
