...
def ensure_python_int(value: int | np.integer) -> int:
    ...
    if not (is_integer(value) or is_float(value)):
        if not is_scalar(value):
            raise TypeError(
                f"Value needs to be a scalar value, was type {type(value).__name__***REMOVED***"
***REMOVED***
        raise TypeError(f"Wrong type {type(value)***REMOVED*** for value {value***REMOVED***")
    try:
        new_value = int(value)
        assert new_value == value
    except (TypeError, ValueError, AssertionError) as err:
        raise TypeError(f"Wrong type {type(value)***REMOVED*** for value {value***REMOVED***") from err
    return new_value
...
def _validate_date_like_dtype(dtype) -> None:
    ...
    if typ not in ["generic", "ns"]:
        raise ValueError(
            f"{repr(dtype.name)***REMOVED*** is too specific of a frequency, "
            f"try passing {repr(dtype.type.__name__)***REMOVED***"
        )
...
def validate_all_hashable(*args, error_name: str | None = None) -> None:
    ...
    *args
    ...
    if not all(is_hashable(arg) for arg in args):
        if error_name:
            raise TypeError(f"{error_name***REMOVED*** must be a hashable type")
...
def pandas_dtype(dtype) -> DtypeObj:
    ...
    if result is not None:
        if isinstance(result, type):
            # GH 31356, GH 54592
            warnings.warn(
                f"Instantiating {result.__name__***REMOVED*** without any arguments."
                f"Pass a {result.__name__***REMOVED*** instance to silence this warning.",
                UserWarning,
...
***REMOVED***
    ...
    try:
        ...
        raise TypeError(f"data type '{dtype***REMOVED***' not understood") from err
    ...
    if is_hashable(dtype) and dtype in [
        ...
        raise TypeError(f"dtype '{dtype***REMOVED***' not understood")
