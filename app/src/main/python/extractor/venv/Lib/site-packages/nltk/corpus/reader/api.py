...
class CorpusReader:
    ...
    def __repr__(self):
        if isinstance(self._root, ZipFilePathPointer):
            path = f"{self._root.zipfile.filename***REMOVED***/{self._root.entry***REMOVED***"
        else:
            path = "%s" % self._root.path
        return f"<{self.__class__.__name__***REMOVED*** in {path!r***REMOVED***>"
