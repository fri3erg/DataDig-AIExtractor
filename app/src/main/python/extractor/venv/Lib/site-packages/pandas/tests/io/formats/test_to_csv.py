...
"""
    ...
    def test_to_csv_default_encoding(self):
        # GH17097
        df = DataFrame({"col": ["AAAAA", "ÄÄÄÄÄ", "ßßßßß", "聞聞聞聞聞"]***REMOVED***)
...
"""
    ...
    def test_to_csv_doublequote(self):
        df = DataFrame({"col": ['a"a', '"bb"']***REMOVED***)
        expected = '''\
"","col"
"0","a""a"
"1","""bb"""
...
"""
    ...
    def test_to_csv_decimal(self):
        # see gh-781
        df = DataFrame({"col1": [1], "col2": ["a"], "col3": [10.1]***REMOVED***)
        ...
        df = DataFrame({"a": [0, 1.1], "b": [2.2, 3.3], "c": 1***REMOVED***)
    ...
    def test_to_csv_float_format(self):
        ...
        # GH 11553
        df = DataFrame({"a": [0, 1], "b": [2.2, 3.3], "c": 1***REMOVED***)
    ...
    def test_to_csv_na_rep(self):
        ...
        df = DataFrame({"a": [0, np.nan], "b": [0, 1], "c": [2, 3]***REMOVED***)
        ...
        df = DataFrame({"a": np.nan, "b": [0, 1], "c": [2, 3]***REMOVED***)
        ...
        df = DataFrame({"a": 0, "b": [0, 1], "c": [2, 3]***REMOVED***)
    ...
    def test_to_csv_date_format(self):
        # GH 10209
        df_sec = DataFrame({"A": pd.date_range("20130101", periods=5, freq="s")***REMOVED***)
        df_day = DataFrame({"A": pd.date_range("20130101", periods=5, freq="d")***REMOVED***)
...
***REMOVED***.append(pd.DatetimeIndex([pd.NaT]))
        )
        ser = ser.astype("category")
    ...
    def test_to_csv_float_ea_float_format(self):
        # GH#45991
        df = DataFrame({"a": [1.1, 2.02, pd.NA, 6.000006], "b": "c"***REMOVED***)
...
***REMOVED***,
    ...
    def test_to_csv_string_array_ascii(self):
        # GH 10813
        str_array = [{"names": ["foo", "bar"]***REMOVED***, {"names": ["baz", "qux"]***REMOVED***]
...
"""
    ...
    def test_to_csv_compression_dict(self, compression_only):
        ...
        df = DataFrame({"ABC": [1]***REMOVED***)
        filename = "to_csv_compress_as_dict."
        extension = {
            "gzip": "gz",
            "zstd": "zst",
        ***REMOVED***.get(method, method)
        filename += extension
        with tm.ensure_clean(filename) as path:
            df.to_csv(path, compression={"method": method***REMOVED***)
    ...
    def test_to_csv_compression_dict_no_method_raises(self):
        # GH 26023
        df = DataFrame({"ABC": [1]***REMOVED***)
        compression = {"some_option": True***REMOVED***
    ...
    def test_to_csv_zip_arguments(self, compression, archive_name):
        ...
        with tm.ensure_clean("to_csv_archive_name.zip") as path:
            df.to_csv(
                path, compression={"method": compression, "archive_name": archive_name***REMOVED***
***REMOVED***
    ...
    def test_to_csv_na_rep_long_string(self, df_new_type):
        # see gh-25099
        df = DataFrame({"c": [float("nan")] * 3***REMOVED***)
