    ...
    def func_call(self, func_name: Any, args: list) -> FilterDirective:
        ...
        if isinstance(func, Comparator):
            if self.allowed_attributes and args[0] not in self.allowed_attributes:
                raise ValueError(
                    f"Received invalid attributes {args[0]***REMOVED***. Allowed attributes are "
                    f"{self.allowed_attributes***REMOVED***"
    ***REMOVED***
        ...
        if func_name in set(Comparator):
            if self.allowed_comparators is not None:
                if func_name not in self.allowed_comparators:
                    raise ValueError(
                        f"Received disallowed comparator {func_name***REMOVED***. Allowed "
                        f"comparators are {self.allowed_comparators***REMOVED***"
        ***REMOVED***
            ...
            if self.allowed_operators is not None:
                if func_name not in self.allowed_operators:
                    raise ValueError(
                        f"Received disallowed operator {func_name***REMOVED***. Allowed operators"
                        f" are {self.allowed_operators***REMOVED***"
        ***REMOVED***
            return Operator(func_name)
        else:
            raise ValueError(
                f"Received unrecognized function {func_name***REMOVED***. Valid functions are "
                f"{list(Operator) + list(Comparator)***REMOVED***"
***REMOVED***
    ...
    def date(self, item: Any) -> ISO8601Date:
        ...
        try:
            ...
            warnings.warn(
                ...
                "(YYYY-MM-DD)."
***REMOVED***
        return {"date": item, "type": "date"***REMOVED***
