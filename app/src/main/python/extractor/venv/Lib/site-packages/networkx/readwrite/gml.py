...
def literal_destringizer(rep):
    ...
    if isinstance(rep, str):
        ...
        try:
            ...
            raise ValueError(f"{orig_rep!r***REMOVED*** is not a valid Python literal") from err
    else:
        raise ValueError(f"{rep!r***REMOVED*** is not a string")
...
def parse_gml_lines(lines, label, destringizer):
    ...
    def tokenize():
        ...
        tokens = re.compile("|".join(f"({pattern***REMOVED***)" for pattern in patterns))
        lineno = 0
        ...
        for line in lines:
            ...
            while pos < length:
                ...
                if match is None:
                    m = f"cannot tokenize {line[pos:]***REMOVED*** at ({lineno + 1***REMOVED***, {pos + 1***REMOVED***)"
    ...
    def unexpected(curr_token, expected):
        ...
        raise NetworkXError(f"expected {expected***REMOVED***, found {value***REMOVED*** at ({lineno***REMOVED***, {pos***REMOVED***)")
    ...
    def parse_kv(curr_token):
        ...
        while curr_token.category == Pattern.KEYS:
            ...
            ***REMOVED***
                ...
                elif curr_token.value in {"NAN", "INF"***REMOVED***:
        ...
        dct = {key: clean_dict_value(value) for key, value in dct.items()***REMOVED***
    ...
    graph_attr = {k: v for k, v in graph.items() if k not in ("node", "edge")***REMOVED***
    ...
    def pop_attr(dct, category, attr, i):
        try:
            ...
            raise NetworkXError(f"{category***REMOVED*** #{i***REMOVED*** has no {attr!r***REMOVED*** attribute") from err
    ...
    for i, node in enumerate(nodes if isinstance(nodes, list) else [nodes]):
        ...
        if id in G:
            raise NetworkXError(f"node id {id!r***REMOVED*** is duplicated")
        if label is not None and label != "id":
            node_label = pop_attr(node, "node", label, i)
            if node_label in node_labels:
                raise NetworkXError(f"node label {node_label!r***REMOVED*** is duplicated")
    ...
    for i, edge in enumerate(edges if isinstance(edges, list) else [edges]):
        ...
        if source not in G:
            raise NetworkXError(f"edge #{i***REMOVED*** has undefined source {source!r***REMOVED***")
        if target not in G:
            raise NetworkXError(f"edge #{i***REMOVED*** has undefined target {target!r***REMOVED***")
        if not multigraph:
            if not G.has_edge(source, target):
                G.add_edge(source, target, **edge)
            else:
                arrow = "->" if directed else "--"
                msg = f"edge #{i***REMOVED*** ({source!r***REMOVED***{arrow***REMOVED***{target!r***REMOVED***) is duplicated"
                raise nx.NetworkXError(msg)
        else:
            key = edge.pop("key", None)
            if key is not None and G.has_edge(source, target, key):
                arrow = "->" if directed else "--"
                msg = f"edge #{i***REMOVED*** ({source!r***REMOVED***{arrow***REMOVED***{target!r***REMOVED***, {key!r***REMOVED***)"
...
def literal_stringizer(value):
    ...
    def stringize(value):
        if isinstance(value, (int, bool)) or value is None:
            ...
            msg = f"{value!r***REMOVED*** cannot be converted into a Python literal"
...
def generate_gml(G, stringizer=None):
    ...
    def stringize(key, value, ignored_keys, indent, in_list=False):
        if not isinstance(key, str):
            raise NetworkXError(f"{key!r***REMOVED*** is not a string")
        if not valid_keys.match(key):
            raise NetworkXError(f"{key!r***REMOVED*** is not a valid key")
        if not isinstance(key, str):
            key = str(key)
        if key not in ignored_keys:
            if isinstance(value, (int, bool)):
                ...
                yield indent + key + f" \"({','.join(repr(v) for v in value)***REMOVED***)\""
            elif isinstance(value, (list, tuple)) and key != "label" and not in_list:
                if len(value) == 0:
                    yield indent + key + " " + f'"{value!r***REMOVED***"'
                if len(value) == 1:
                    yield indent + key + " " + f'"{LIST_START_VALUE***REMOVED***"'
                for val in value:
                ...
                if stringizer:
                    try:
                        ...
                        raise NetworkXError(
                            f"{value!r***REMOVED*** cannot be converted into a string"
            ***REMOVED*** from err
                if not isinstance(value, str):
                    raise NetworkXError(f"{value!r***REMOVED*** is not a string")
    ...
    if multigraph:
        yield "  multigraph 1"
    ignored_keys = {"directed", "multigraph", "node", "edge"***REMOVED***
    for attr, value in G.graph.items():
        yield from stringize(attr, value, ignored_keys, "  ")

    # Output node data
    node_id = dict(zip(G, range(len(G))))
    ignored_keys = {"id", "label"***REMOVED***
    ...
    # Output edge data
    ignored_keys = {"source", "target"***REMOVED***
    kwargs = {"data": True***REMOVED***
    if multigraph:
