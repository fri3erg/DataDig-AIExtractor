...
def import_string(dotted_path: str) -> Any:
    ...
    try:
        ...
        raise ImportError(f'"{dotted_path***REMOVED***" doesn\'t look like a module path') from e

    module = import_module(module_path)
    try:
        return getattr(module, class_name)
    except AttributeError as e:
        raise ImportError(f'Module "{module_path***REMOVED***" does not define a "{class_name***REMOVED***" attribute') from e
...
def validate_field_name(bases: List[Type['BaseModel']], field_name: str) -> None:
    ...
    for base in bases:
        if getattr(base, field_name, None):
            raise NameError(
                f'Field name "{field_name***REMOVED***" shadows a BaseModel attribute; '
                f'use a different field name with "alias=\'{field_name***REMOVED***\'".'
***REMOVED***
...
def update_not_none(mapping: Dict[Any, Any], **update: Any) -> None:
    mapping.update({k: v for k, v in update.items() if v is not None***REMOVED***)
...
def generate_model_signature(
    ...
    merged_params: Dict[str, Parameter] = {***REMOVED***
    var_kw = None
    use_var_kw = False
    ...
    if var_kw:  # if custom init has no var_kw, fields which are not declared in it cannot be passed through
        ...
        for field_name, field in fields.items():
            ...
            kwargs = {'default': field.default***REMOVED*** if not field.required else {***REMOVED***
...
class Representation:
    ...
    def __repr_str__(self, join_str: str) -> str:
        return join_str.join(repr(v) if a is None else f'{a***REMOVED***={v!r***REMOVED***' for a, v in self.__repr_args__())
    ...
    def __repr__(self) -> str:
        return f'{self.__repr_name__()***REMOVED***({self.__repr_str__(", ")***REMOVED***)'
...
class GetterDict(Representation):
    ...
    def __repr_name__(self) -> str:
        return f'GetterDict[{display_as_type(self._obj)***REMOVED***]'
...
class ValueItems(Representation):
    ...
    def _normalize_indexes(self, items: 'MappingIntStrAny', v_length: int) -> 'DictIntStrAny':
        ...
        >>> self._normalize_indexes({0: True, -2: True, -1: True***REMOVED***, 4)
        {0: True, 2: True, 3: True***REMOVED***
        >>> self._normalize_indexes({'__all__': True***REMOVED***, 4)
        {0: True, 1: True, 2: True, 3: True***REMOVED***
        """

        normalized_items: 'DictIntStrAny' = {***REMOVED***
        all_items = None
        for i, v in items.items():
            if not (isinstance(v, Mapping) or isinstance(v, AbstractSet) or self.is_true(v)):
                raise TypeError(f'Unexpected type of exclude value for index "{i***REMOVED***" {v.__class__***REMOVED***')
            if i == '__all__':
    ...
    ***REMOVED***
        ...
        if self.is_true(all_items):
            ...
            return normalized_items
        for i in range(v_length):
            normalized_item = normalized_items.setdefault(i, {***REMOVED***)
    ...
    def _coerce_items(items: Union['AbstractSetIntStr', 'MappingIntStrAny']) -> 'MappingIntStrAny':
        if isinstance(items, Mapping):
            ...
            assert_never(
                items,
                f'Unexpected type of exclude value {class_name***REMOVED***',
***REMOVED***
        return items
...
class ClassAttribute:
    ...
    def __get__(self, instance: Any, owner: Type[Any]) -> None:
        if instance is None:
            return self.value
        raise AttributeError(f'{self.name!r***REMOVED*** attribute of {owner.__name__!r***REMOVED*** is class-only')
...
def get_unique_discriminator_alias(all_aliases: Collection[str], discriminator_key: str) -> str:
    ...
    if len(unique_aliases) > 1:
        raise ConfigError(
            f'Aliases for discriminator {discriminator_key!r***REMOVED*** must be the same (got {", ".join(sorted(all_aliases))***REMOVED***)'
        )
...
def get_discriminator_alias_and_values(tp: Any, discriminator_key: str) -> Tuple[str, Tuple[str, ...]]:
    ...
    if is_union(get_origin(tp)):
        ...
        if len(set(all_values)) > 1:
            raise ConfigError(
                f'Field {discriminator_key!r***REMOVED*** is not the same for all submodels of {display_as_type(tp)!r***REMOVED***'
***REMOVED***
        ...
        try:
            ...
            raise TypeError(f'Type {tp.__name__!r***REMOVED*** is not a valid `BaseModel` or `dataclass`') from e
        except KeyError as e:
            raise ConfigError(f'Model {tp.__name__!r***REMOVED*** needs a discriminator field for key {discriminator_key!r***REMOVED***') from e

        if not is_literal_type(t_discriminator_type):
            raise ConfigError(f'Field {discriminator_key!r***REMOVED*** of model {tp.__name__!r***REMOVED*** needs to be a `Literal`')
