...
def test_indexer_constructor_arg():
    ...
    df = DataFrame({"values": range(5)***REMOVED***)
    ...
    expected = DataFrame({"values": [0.0, 1.0, 3.0, 3.0, 10.0]***REMOVED***)
...
def test_indexer_accepts_rolling_args():
    ...
    ***REMOVED***:
    ...
    expected = DataFrame({"values": [0.0, 1.0, 10.0, 3.0, 4.0]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        ("count", len, [3.0, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0, 2.0, np.nan], {***REMOVED***),
        ("min", np.min, [0.0, 1.0, 2.0, 3.0, 4.0, 6.0, 6.0, 7.0, 8.0, np.nan], {***REMOVED***),
        (
            "max",
            np.max,
...
def test_fixed_forward_indexer_count(step):
    # GH: 35579
    df = DataFrame({"b": [None, None, None, 7]***REMOVED***)
    indexer = FixedForwardWindowIndexer(window_size=2)
    result = df.rolling(window=indexer, min_periods=0, step=step).count()
    expected = DataFrame({"b": [0.0, 0.0, 1.0, 1.0]***REMOVED***)[::step]
    tm.assert_frame_equal(result, expected)
...
def test_indexer_quantile_sum(end_value, values, func, args):
    ...
    expected = DataFrame({"values": values***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        {"a": [1, 1], "b": [0, 1]***REMOVED***,
        {"a": [1, 2], "b": [0, 1]***REMOVED***,
        {"a": [1] * 16, "b": [np.nan, 1, 2, np.nan] + list(range(4, 16))***REMOVED***,
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            DataFrame({"b": [0, 1, 2], "a": [1, 2, 2]***REMOVED***),
            2,
            Series(
                [0, 1.5, 2.0],
