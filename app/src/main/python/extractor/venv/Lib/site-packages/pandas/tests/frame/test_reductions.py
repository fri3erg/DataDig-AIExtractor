...
def assert_stat_op_calc(
    ...
    if check_dates:
        df = DataFrame({"b": date_range("1/1/2001", periods=2)***REMOVED***)
...
def bool_frame_with_na():
    ...
    df = DataFrame(
        ...
        index=Index([f"foo_{i***REMOVED***" for i in range(30)], dtype=object),
                ...
                if axis == 0:
                    ...
                    msg = "|".join(
                        [
                            ...
                            "Could not convert string '(bar){30***REMOVED***' to numeric",
                        ]
        ***REMOVED***
                else:
                    msg = r"unsupported operand type\(s\) for \+: 'float' and 'str'"
        ...
        [
            DataFrame(
                ...
                ***REMOVED***,
                ...
                dtype="O",
***REMOVED***,
            DataFrame({0: [np.nan, 2], 1: [np.nan, 3], 2: [np.nan, 4]***REMOVED***, dtype=object),
        ],
    )
...
***REMOVED***
    ...
    def test_nunique(self):
        df = DataFrame({"A": [1, 1, 1], "B": [1, 2, 3], "C": [1, np.nan, 3]***REMOVED***)
        tm.assert_series_equal(df.nunique(), Series({"A": 1, "B": 3, "C": 2***REMOVED***))
        tm.assert_series_equal(
            df.nunique(dropna=False), Series({"A": 1, "B": 3, "C": 3***REMOVED***)
        )
        tm.assert_series_equal(df.nunique(axis=1), Series({0: 1, 1: 2, 2: 2***REMOVED***))
        tm.assert_series_equal(
            df.nunique(axis=1, dropna=False), Series({0: 1, 1: 3, 2: 2***REMOVED***)
        )

    @pytest.mark.parametrize("tz", [None, "UTC"])
    def test_mean_mixed_datetime_numeric(self, tz):
        # https://github.com/pandas-dev/pandas/issues/24752
        df = DataFrame({"A": [1, 1], "B": [Timestamp("2000", tz=tz)] * 2***REMOVED***)
        result = df.mean()
    ...
    def test_mean_includes_datetimes(self, tz):
        ...
        df = DataFrame({"A": [Timestamp("2000", tz=tz)] * 2***REMOVED***)
        result = df.mean()
    ...
    def test_mean_mixed_string_decimal(self):
        ...
        d = [
            {"A": 2, "B": None, "C": Decimal("628.00")***REMOVED***,
            {"A": 1, "B": None, "C": Decimal("383.00")***REMOVED***,
            {"A": 3, "B": None, "C": Decimal("651.00")***REMOVED***,
            {"A": 2, "B": None, "C": Decimal("575.00")***REMOVED***,
            {"A": 4, "B": None, "C": Decimal("1114.00")***REMOVED***,
            {"A": 1, "B": "TEST", "C": Decimal("241.00")***REMOVED***,
            {"A": 2, "B": None, "C": Decimal("572.00")***REMOVED***,
            {"A": 4, "B": None, "C": Decimal("609.00")***REMOVED***,
            {"A": 3, "B": None, "C": Decimal("820.00")***REMOVED***,
            {"A": 5, "B": None, "C": Decimal("1223.00")***REMOVED***,
        ]
    ...
    def test_numeric_only_flag(self, meth):
        ...
        df1 = df1.astype({"foo": object***REMOVED***)
        ...
        df2 = df2.astype({"foo": object***REMOVED***)
...
***REMOVED***,
    ...
    ***REMOVED***,
    ...
    def test_mode_sortwarning(self, using_infer_string):
        ...
        df = DataFrame({"A": [np.nan, np.nan, "a", "a"]***REMOVED***)
        expected = DataFrame({"A": ["a", np.nan]***REMOVED***)
    ...
    def test_operators_timedelta64(self):
        ...
        diffs = DataFrame({"A": df["A"] - df["C"], "B": df["A"] - df["B"]***REMOVED***)
        ...
        result = diffs.abs()
        result2 = abs(diffs)
        expected = DataFrame({"A": df["A"] - df["C"], "B": df["B"] - df["A"]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_std_timedelta64_skipna_false(self):
        ...
        df = DataFrame({"A": tdi, "B": tdi***REMOVED***, copy=True)
        df.iloc[-2, -1] = pd.NaT
...
***REMOVED***
        ...
        df = DataFrame({"a": dti***REMOVED***)
        result = df.std(skipna=skipna)
        if not skipna or all(value is pd.NaT for value in values):
            expected = Series({"a": pd.NaT***REMOVED***, dtype=f"timedelta64[{unit***REMOVED***]")
        else:
            # 86400000000000ns == 1 day
            expected = Series({"a": 86400000000000***REMOVED***, dtype=f"timedelta64[{unit***REMOVED***]")
        tm.assert_series_equal(result, expected)
    ...
    def test_sum_prod_nanops(self, method, unit, numeric_only):
        idx = ["a", "b", "c"]
        df = DataFrame({"a": [unit, unit], "b": [unit, np.nan], "c": [np.nan, np.nan]***REMOVED***)
        # The default
        ...
        # min_count > 1
        df = DataFrame({"A": [unit] * 10, "B": [unit] * 5 + [np.nan] * 5***REMOVED***)
    ...
    def test_sum_nanops_timedelta(self):
        ...
        idx = ["a", "b", "c"]
        df = DataFrame({"a": [0, 0], "b": [0, np.nan], "c": [np.nan, np.nan]***REMOVED***)
    ...
    def test_sum_nanops_min_count(self):
        ...
        df = DataFrame({"x": [1, 2, 3], "y": [4, 5, 6]***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({"axis": 1, "min_count": 2***REMOVED***, [3.2, 5.3, np.nan]),
            ({"axis": 1, "min_count": 3***REMOVED***, [np.nan, np.nan, np.nan]),
            ({"axis": 1, "skipna": False***REMOVED***, [3.2, 5.3, np.nan]),
        ],
    )
    def test_sum_nanops_dtype_min_count(self, float_type, kwargs, expected_result):
        # GH#46947
        df = DataFrame({"a": [1.0, 2.3, 4.4], "b": [2.2, 3, np.nan]***REMOVED***, dtype=float_type)
    ...
    @pytest.mark.parametrize(
        "kwargs, expected_result",
        [
            ({"axis": 1, "min_count": 2***REMOVED***, [2.0, 4.0, np.nan]),
            ({"axis": 1, "min_count": 3***REMOVED***, [np.nan, np.nan, np.nan]),
            ({"axis": 1, "skipna": False***REMOVED***, [2.0, 4.0, np.nan]),
        ],
    )
    def test_prod_nanops_dtype_min_count(self, float_type, kwargs, expected_result):
        # GH#46947
        df = DataFrame(
            {"a": [1.0, 2.0, 4.4], "b": [2.0, 2.0, np.nan]***REMOVED***, dtype=float_type
        )
    ...
    def test_sum_mixed_datetime(self):
        # GH#30886
        df = DataFrame({"A": date_range("2000", periods=4), "B": [1, 2, 3, 4]***REMOVED***).reindex(
            [2, 3, 4]
        )
    ...
    def test_mean_datetimelike_numeric_only_false(self):
        ...
        expected = Series({"A": 1, "B": df.loc[1, "B"], "C": df.loc[1, "C"]***REMOVED***)
        tm.assert_series_equal(result, expected)
    ...
    def test_idxmin_numeric_only(self, numeric_only):
        df = DataFrame({"a": [2, 3, 1], "b": [2, 1, 1], "c": list("xyx")***REMOVED***)
    ...
    def test_idxmax_arrow_types(self):
        ...
        df = DataFrame({"a": [2, 3, 1], "b": [2, 1, 1]***REMOVED***, dtype="int64[pyarrow]")
        result = df.idxmax()
        expected = Series([1, 0], index=["a", "b"])
        tm.assert_series_equal(result, expected)
        ...
        df = DataFrame({"a": ["b", "c", "a"]***REMOVED***, dtype="string[pyarrow]")
    ...
    def test_idxmax_mixed_dtype(self):
        ...
        df = DataFrame({1: [0, 2, 1], 2: range(3)[::-1], 3: dti.copy(deep=True)***REMOVED***)
    ...
    def test_idxmax_dt64_multicolumn_axis1(self):
        dti = date_range("2016-01-01", periods=3)
        df = DataFrame({3: dti, 4: dti[::-1]***REMOVED***, copy=True)
        df.iloc[0, 0] = pd.NaT
    ...
    def test_any_datetime(self):
        ...
        df = DataFrame({"A": float_data, "B": datetime_data***REMOVED***)
    ...
    def test_any_all_bool_only(self):
        # GH 25101
        df = DataFrame(
            {"col1": [1, 2, 3], "col2": [4, 5, 6], "col3": [None, None, None]***REMOVED***,
        ...
        result = df.all(bool_only=True)
        expected = Series({"col4": False***REMOVED***)
        tm.assert_series_equal(result, expected)

    @pytest.mark.parametrize(
        "func, data, expected",
        [
            (np.any, {***REMOVED***, False),
            (np.all, {***REMOVED***, True),
            (np.any, {"A": []***REMOVED***, False),
            (np.all, {"A": []***REMOVED***, True),
            (np.any, {"A": [False, False]***REMOVED***, False),
            (np.all, {"A": [False, False]***REMOVED***, False),
            (np.any, {"A": [True, False]***REMOVED***, True),
            (np.all, {"A": [True, False]***REMOVED***, False),
            (np.any, {"A": [True, True]***REMOVED***, True),
            (np.all, {"A": [True, True]***REMOVED***, True),
            (np.any, {"A": [False], "B": [False]***REMOVED***, False),
            (np.all, {"A": [False], "B": [False]***REMOVED***, False),
            (np.any, {"A": [False, False], "B": [False, True]***REMOVED***, True),
            (np.all, {"A": [False, False], "B": [False, True]***REMOVED***, False),
            # other types
            (np.all, {"A": Series([0.0, 1.0], dtype="float")***REMOVED***, False),
            (np.any, {"A": Series([0.0, 1.0], dtype="float")***REMOVED***, True),
            (np.all, {"A": Series([0, 1], dtype=int)***REMOVED***, False),
            (np.any, {"A": Series([0, 1], dtype=int)***REMOVED***, True),
            pytest.param(np.all, {"A": Series([0, 1], dtype="M8[ns]")***REMOVED***, False),
            pytest.param(np.all, {"A": Series([0, 1], dtype="M8[ns, UTC]")***REMOVED***, False),
            pytest.param(np.any, {"A": Series([0, 1], dtype="M8[ns]")***REMOVED***, True),
            pytest.param(np.any, {"A": Series([0, 1], dtype="M8[ns, UTC]")***REMOVED***, True),
            pytest.param(np.all, {"A": Series([1, 2], dtype="M8[ns]")***REMOVED***, True),
            pytest.param(np.all, {"A": Series([1, 2], dtype="M8[ns, UTC]")***REMOVED***, True),
            pytest.param(np.any, {"A": Series([1, 2], dtype="M8[ns]")***REMOVED***, True),
            pytest.param(np.any, {"A": Series([1, 2], dtype="M8[ns, UTC]")***REMOVED***, True),
            pytest.param(np.all, {"A": Series([0, 1], dtype="m8[ns]")***REMOVED***, False),
            pytest.param(np.any, {"A": Series([0, 1], dtype="m8[ns]")***REMOVED***, True),
            pytest.param(np.all, {"A": Series([1, 2], dtype="m8[ns]")***REMOVED***, True),
            pytest.param(np.any, {"A": Series([1, 2], dtype="m8[ns]")***REMOVED***, True),
            ...
            (np.all, {"A": Series([0, 1], dtype="category")***REMOVED***, True),
            (np.any, {"A": Series([0, 1], dtype="category")***REMOVED***, False),
            (np.all, {"A": Series([1, 2], dtype="category")***REMOVED***, True),
            (np.any, {"A": Series([1, 2], dtype="category")***REMOVED***, False),
            # Mix GH#21484
            pytest.param(
                np.all,
                {
...
***REMOVED***:
    ...
    def test_any_all_object_bool_only(self):
        df = DataFrame({"A": ["foo", 2], "B": [True, False]***REMOVED***).astype(object)
        df._consolidate_inplace()
    ...
    def test_series_broadcasting(self):
        ...
        df_nan = DataFrame({"A": [np.nan, 2.0, np.nan]***REMOVED***)
        s = Series([1, 1, 1])
...
class TestDataFrameReductions:
    def test_min_max_dt64_with_NaT(self):
        ...
        df = DataFrame({"foo": [pd.NaT, pd.NaT, Timestamp("2012-05-01")]***REMOVED***)
        ...
        df = DataFrame({"foo": [pd.NaT, pd.NaT]***REMOVED***)
...
***REMOVED***
    ...
    def test_min_max_dt64_api_consistency_with_NaT(self):
        ...
        # information
        df = DataFrame({"x": to_datetime([])***REMOVED***)
    ...
    def test_minmax_tzaware_skipna_axis_1(self, method, skipna):
        ...
        df = DataFrame(
            {"a": Series([pd.NaT, pd.NaT, val]), "b": Series([pd.NaT, val, val])***REMOVED***
        )
...
class TestNuisanceColumns:
    ...
    def test_median_categorical_dtype_nuisance_column(self):
        ...
        df = DataFrame({"A": Categorical([1, 2, 2, 2, 3])***REMOVED***)
        ser = df["A"]
...
class TestEmptyDataFrameReductions:
    ...
    def test_df_empty_min_count_0(self, opname, dtype, exp_value, exp_dtype):
        df = DataFrame({0: [], 1: []***REMOVED***, dtype=dtype)
...
def test_minmax_extensionarray(method, numeric_only):
    ...
    df = DataFrame({"Int64": ser***REMOVED***)
    result = getattr(df, method)(numeric_only=numeric_only)
    expected = Series(
...
def test_frame_mixed_numeric_object_with_timestamp(ts_value):
    # GH 13912
    df = DataFrame({"a": [1], "b": [1.1], "c": ["foo"], "d": [ts_value]***REMOVED***)
    with pytest.raises(TypeError, match="does not support reduction"):
        df.sum()
...
def test_reduction_axis_none_returns_scalar(method, numeric_only, dtype):
    ...
    if method in {"skew", "kurt"***REMOVED***:
...
def test_fails_on_non_numeric(kernel):
    # GH#46852
    df = DataFrame({"a": [1, 2, 3], "b": object***REMOVED***)
...
def test_numeric_ea_axis_1(method, skipna, min_count, any_numeric_ea_dtype):
    ...
    if not skipna and method in ("idxmax", "idxmin"):
        warn = FutureWarning
        msg = f"The behavior of DataFrame.{method***REMOVED*** with all-NA values"
    with tm.assert_produces_warning(warn, match=msg):
