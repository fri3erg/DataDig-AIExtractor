...
class TestGetNumericData:
    def test_get_numeric_data_preserve_dtype(self):
        ...
        obj = DataFrame({"A": [1, "2", 3.0]***REMOVED***, columns=Index(["A"], dtype="object"))
    ...
    def test_get_numeric_data(self, using_infer_string):
        ...
        df = DataFrame(
            {"a": 1.0, "b": 2, "c": "foo", "f": Timestamp("20010102")***REMOVED***,
            index=np.arange(10),
        )
        result = df.dtypes
        expected = Series(
        ...
        df = DataFrame.from_dict({"a": [1, 2], "b": ["foo", "bar"], "c": [np.pi, np.e]***REMOVED***)
        result = df._get_numeric_data()
        expected = DataFrame.from_dict({"a": [1, 2], "c": [np.pi, np.e]***REMOVED***)
        tm.assert_frame_equal(result, expected)
