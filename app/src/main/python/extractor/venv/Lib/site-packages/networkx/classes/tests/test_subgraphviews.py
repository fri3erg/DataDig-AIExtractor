...
class TestSubGraphView:
    ...
    def setup_class(cls):
        ...
        cls.hide_edges_w_hide_nodes = {(3, 4), (4, 5), (5, 6)***REMOVED***
    ...
    def test_hidden_nodes(self):
        ...
        assert self.G.nodes - G.nodes == {4, 5***REMOVED***
    ...
    def test_hidden_edges(self):
        ...
        if G.is_directed():
            assert self.G.edges - G.edges == {(2, 3)***REMOVED***
            assert list(G[2]) == []
            assert list(G.pred[3]) == []
            assert list(G.pred[2]) == [1]
            assert G.size() == 7
        else:
            assert self.G.edges - G.edges == {(2, 3), (7, 8)***REMOVED***
            assert list(G[2]) == [1]
            assert G.size() == 6
        assert list(G[3]) == [4]
    ...
    def test_shown_node(self):
        ...
        assert set(G.nodes) == {2, 3***REMOVED***
        if G.is_directed():
            assert list(G[3]) == []
        else:
            assert list(G[3]) == [2]
    ...
    def test_shown_edges(self):
        ...
        if G.is_directed():
            ...
            assert G.edges == {(2, 3), (7, 8)***REMOVED***
            assert list(G[3]) == [2]
            assert list(G[2]) == [3]
            assert G.size() == 2
        pytest.raises(KeyError, G.__getitem__, 221)
...
class TestSubDiGraphView(TestSubGraphView):
    ...
    hide_edges = [(2, 3), (8, 7), (222, 223)]
    excluded = {(2, 3), (3, 4), (4, 5), (5, 6)***REMOVED***
...
class TestMultiGraphView(TestSubGraphView):
    ...
    def setup_class(cls):
        cls.G = nx.path_graph(9, create_using=cls.graph())
        multiedges = {(2, 3, 4), (2, 3, 5)***REMOVED***
        cls.G.add_edges_from(multiedges)
        cls.hide_edges_w_hide_nodes = {(3, 4, 0), (4, 5, 0), (5, 6, 0)***REMOVED***
    ...
    def test_hidden_edges(self):
        ...
        assert self.G.nodes == G.nodes
        if G.is_directed():
            assert self.G.edges - G.edges == {(2, 3, 4)***REMOVED***
            assert list(G[3]) == [4]
            assert list(G[2]) == [3]
            ...
            assert self.G.edges - G.edges == {(2, 3, 4), (7, 8, 0)***REMOVED***
            assert list(G[3]) == [2, 4]
            assert list(G[2]) == [1, 3]
            assert G.size() == 8
        assert G.degree(3) == 3
    ...
    def test_shown_edges(self):
        ...
        assert self.G.nodes == G.nodes
        if G.is_directed():
            assert G.edges == {(2, 3, 4)***REMOVED***
            assert list(G[3]) == []
            assert list(G.pred[3]) == [2]
            assert list(G.pred[2]) == []
            assert G.size() == 1
        else:
            assert G.edges == {(2, 3, 4), (7, 8, 0)***REMOVED***
            assert G.size() == 2
            assert list(G[3]) == [2]
        assert G.degree(3) == 1
        assert list(G[2]) == [3]
...
class TestMultiDiGraphView(TestMultiGraphView, TestSubDiGraphView):
    ...
    excluded = {(2, 3, 0), (3, 4, 0), (4, 5, 0), (5, 6, 0)***REMOVED***
...
class TestInducedSubGraph:
    ...
    def test_partial_subgraph(self):
        ...
        assert dict(H.adj) == {0: {***REMOVED******REMOVED***
        assert dict(G.adj) != {0: {***REMOVED******REMOVED***

        H = nx.induced_subgraph(G, [0, 1])
        assert dict(H.adj) == {0: {1: {***REMOVED******REMOVED***, 1: {0: {***REMOVED******REMOVED******REMOVED***
