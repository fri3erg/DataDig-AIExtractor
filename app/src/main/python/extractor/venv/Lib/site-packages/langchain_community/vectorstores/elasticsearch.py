...
***REMOVED***
        ...
        # RRF has two optional parameters: {'rank_constant':int, 'window_size':int***REMOVED***
        # https://www.elastic.co/guide/en/elasticsearch/reference/current/rrf.html
        if self.hybrid:
            query_body = {
                "knn": knn,
            ...
            ***REMOVED***
            ...
            if isinstance(self.rrf, dict):
                query_body["rank"] = {"rrf": self.rrf***REMOVED***
            elif isinstance(self.rrf, bool) and self.rrf is True:
                query_body["rank"] = {"rrf": {***REMOVED******REMOVED***

            return query_body
        else:
            return {"knn": knn***REMOVED***
    ...
    def index(
        ...
        if similarity is DistanceStrategy.COSINE:
            ...
            raise ValueError(f"Similarity {similarity***REMOVED*** not supported.")
...
class ExactRetrievalStrategy(BaseRetrievalStrategy):
    ...
    def query(
        ...
        if similarity is DistanceStrategy.COSINE:
            similarityAlgo = (
                f"cosineSimilarity(params.query_vector, '{vector_query_field***REMOVED***') + 1.0"
***REMOVED***
        elif similarity is DistanceStrategy.EUCLIDEAN_DISTANCE:
            similarityAlgo = (
                f"1 / (1 + l2norm(params.query_vector, '{vector_query_field***REMOVED***'))"
***REMOVED***
        elif similarity is DistanceStrategy.DOT_PRODUCT:
            similarityAlgo = f"""
            double value = dotProduct(params.query_vector, '{vector_query_field***REMOVED***');
            return sigmoid(1, Math.E, -value);
            """
        else:
            raise ValueError(f"Similarity {similarity***REMOVED*** not supported.")

        queryBool: Dict = {"match_all": {***REMOVED******REMOVED***
        if filter:
            queryBool = {"bool": {"filter": filter***REMOVED******REMOVED***
            ...
            "query": {
                "script_score": {
                    ...
                    "script": {
                        ...
                        "params": {"query_vector": query_vector***REMOVED***,
                    ***REMOVED***,
                ***REMOVED***,
            ***REMOVED***
        ***REMOVED***
...
class SparseRetrievalStrategy(BaseRetrievalStrategy):
            ...
            "query": {
                "bool": {
                    "must": [
                        {
                            "text_expansion": {
                                f"{vector_query_field***REMOVED***.tokens": {
    ...
    def _get_pipeline_name(self) -> str:
        return f"{self.model_id***REMOVED***_sparse_embedding"
                ...
                processors=[
                    {
                        "inference": {
                            ...
                            "field_map": {text_field: "text_field"***REMOVED***,
                            "inference_config": {
                                "text_expansion": {"results_field": "tokens"***REMOVED***
                            ***REMOVED***,
                        ***REMOVED***
                    ***REMOVED***
                ],
    ...
    def index(
        ...
        return {
            "mappings": {
                "properties": {
                    vector_query_field: {
                        "properties": {"tokens": {"type": "rank_features"***REMOVED******REMOVED***
                    ***REMOVED***
                ***REMOVED***
            ***REMOVED***,
            "settings": {"default_pipeline": self._get_pipeline_name()***REMOVED***,
        ***REMOVED***
        ...
        *,
        ...
        if es_connection is not None:
            self.client = es_connection.options(
                headers={"user-agent": self.get_user_agent()***REMOVED***
***REMOVED***
...
***REMOVED***
    ...
    def get_user_agent() -> str:
        ...
        return f"langchain-py-vs/{__version__***REMOVED***"
...
***REMOVED***
        ...
        connection_params: Dict[str, Any] = {***REMOVED***
        ...
        es_client = elasticsearch.Elasticsearch(
            **connection_params,
            headers={"user-agent": ElasticsearchStore.get_user_agent()***REMOVED***,
        )
        try:
            es_client.info()
        except Exception as e:
            logger.error(f"Error connecting to Elasticsearch: {e***REMOVED***")
            raise e
    ...
    def _search(
        ...
        **kwargs: Any,
        ...
        logger.debug(f"Query body: {query_body***REMOVED***")

        if custom_query is not None:
            query_body = custom_query(query_body, query)
            logger.debug(f"Calling custom_query, Query body now: {query_body***REMOVED***")
...
***REMOVED***
        ...
        for _id in ids:
            body.append({"_op_type": "delete", "_index": self.index_name, "_id": _id***REMOVED***)

        if len(body) > 0:
            try:
                bulk(self.client, body, refresh=refresh_indices, ignore_status=404)
                logger.debug(f"Deleted {len(body)***REMOVED*** texts from index")

                return True
            except BulkIndexError as e:
                logger.error(f"Error deleting texts: {e***REMOVED***")
                firstError = e.errors[0].get("index", {***REMOVED***).get("error", {***REMOVED***)
                logger.error(f"First error reason: {firstError.get('reason')***REMOVED***")
                raise e
    ...
    def _create_index_if_not_exists(
        ...
        if self.client.indices.exists(index=index_name):
            logger.debug(f"Index {index_name***REMOVED*** already exists. Skipping creation.")
...
***REMOVED***
            logger.debug(
                f"Creating index {index_name***REMOVED*** with mappings {indexSettings['mappings']***REMOVED***"
***REMOVED***
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ImportError(
                ...
                "Please install it with `pip install elasticsearch`."
***REMOVED***
        bulk_kwargs = bulk_kwargs or {***REMOVED***
...
***REMOVED***
        ...
        for i, text in enumerate(texts):
            metadata = metadatas[i] if metadatas else {***REMOVED***
    ...
    ***REMOVED***
                logger.debug(
                    f"Added {success***REMOVED*** and failed to add {failed***REMOVED*** texts to index"
    ***REMOVED***

                logger.debug(f"added texts {ids***REMOVED*** to index")
                return ids
            except BulkIndexError as e:
                logger.error(f"Error adding texts: {e***REMOVED***")
                firstError = e.errors[0].get("index", {***REMOVED***).get("error", {***REMOVED***)
                logger.error(f"First error reason: {firstError.get('reason')***REMOVED***")
                raise e
    ...
    def ApproxRetrievalStrategy(
        ...
        Args:
            ...
            rrf: Optional. rrf is Reciprocal Rank Fusion.
                 When `hybrid` is True,
                    and `rrf` is True, then rrf: {***REMOVED***.
