...
def split_resource_url(resource_url):
    ...
    if protocol == "nltk":
        ...
        path_ = re.sub(r"^/{0,2***REMOVED***", "", path_)
...
class ZipFilePathPointer(PathPointer):
        ...
        if entry:
            ...
            try:
                ...
                if entry.endswith("/") and [
                    ...
                    # Otherwise, complain.
                    raise OSError(
                        f"Zipfile {zipfile.filename!r***REMOVED*** does not contain {entry!r***REMOVED***"
        ***REMOVED*** from e
    ...
    def join(self, fileid):
        entry = f"{self._entry***REMOVED***/{fileid***REMOVED***"
        return ZipFilePathPointer(self._zipfile, entry)

    def __repr__(self):
        return f"ZipFilePathPointer({self._zipfile.filename!r***REMOVED***, {self._entry!r***REMOVED***)"
...
def find(resource_name, paths=None):
    ...
    msg = str(
        "Resource \33[93m{resource***REMOVED***\033[0m not found.\n"
        "Please use the NLTK Downloader to obtain the resource:\n\n"
        "\33[31m"  # To display red text in terminal.
        ">>> import nltk\n"
        ">>> nltk.download('{resource***REMOVED***')\n"
        "\033[0m"
    ...
    msg += "\n  Attempted to load \33[93m{resource_name***REMOVED***\033[0m\n".format(
        resource_name=resource_name
    )
...
def retrieve(resource_url, filename=None, verbose=True):
    ...
    if verbose:
        print(f"Retrieving {resource_url!r***REMOVED***, saving to {filename!r***REMOVED***")
...
***REMOVED***
    ...
    if format not in FORMATS:
        raise ValueError(f"Unknown format type: {format***REMOVED***!")
    ...
    if cache:
        ...
        if resource_val is not None:
            if verbose:
                print(f"<<Using cached copy of {resource_url***REMOVED***>>")
            return resource_val

    # Let the user know what's going on.
    if verbose:
        print(f"<<Loading {resource_url***REMOVED***>>")
