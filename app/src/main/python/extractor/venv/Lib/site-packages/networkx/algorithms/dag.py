...
def descendants(G, source):
    ...
    >>> sorted(nx.descendants(DG, 2) | {2***REMOVED***)
    [2, 3, 4]
    ...
    ancestors
    """
    return {child for parent, child in nx.bfs_edges(G, source)***REMOVED***
...
def ancestors(G, source):
    ...
    >>> sorted(nx.ancestors(DG, 2) | {2***REMOVED***)
    [0, 1, 2]
    ...
    descendants
    """
    return {child for parent, child in nx.bfs_edges(G, source, reverse=True)***REMOVED***
...
def topological_generations(G):
    ...
    indegree_map = {v: d for v, d in G.in_degree() if d > 0***REMOVED***
...
def lexicographical_topological_sort(G, key=None):
    ...
    nodeid_map = {n: i for i, n in enumerate(G)***REMOVED***
...
def transitive_closure(G, reflexive=False):
    ...
    ------
    NetworkXError
        If `reflexive` not in `{None, True, False***REMOVED***`
    ...
    for v in G:
        if reflexive is None:
            ...
            TC.add_edges_from(
                (v, u) for u in nx.descendants(G, v) | {v***REMOVED*** if u not in TC[v]
***REMOVED***
...
def transitive_reduction(G):
    ...
    [(1, 2, {'color': 'red'***REMOVED***), (2, 3, {'color': 'red'***REMOVED***)]
        ...
        for v in G[u]:
            if v in u_nbrs:
                if v not in descendants:
                    descendants[v] = {y for x, y in nx.dfs_edges(G, v)***REMOVED***
...
@not_implemented_for("undirected")
@nx._dispatch(edge_attrs={"weight": "default_weight"***REMOVED***)
def dag_longest_path(G, weight="weight", default_weight=1, topo_order=None):
    ...
    Examples
    --------
    >>> DG = nx.DiGraph([(0, 1, {'cost':1***REMOVED***), (1, 2, {'cost':1***REMOVED***), (0, 2, {'cost':42***REMOVED***)])
    ...
    dist = {***REMOVED***  # stores {v : (length, u)***REMOVED***
    for v in topo_order:
        us = [
            (
                dist[u][0]
    ...
    ***REMOVED***.get(weight, default_weight),
                u,
***REMOVED***
