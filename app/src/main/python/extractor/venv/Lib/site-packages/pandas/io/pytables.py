...
# formats
_FORMAT_MAP = {"f": "fixed", "fixed": "fixed", "t": "table", "table": "table"***REMOVED***

# axes map
_AXES_MAP = {DataFrame: [0]***REMOVED***
...
def read_hdf(
    ...
    **kwargs
    ...
    if mode not in ["r", "r+", "a"]:
        raise ValueError(
            f"mode {mode***REMOVED*** is not allowed while performing a read. "
...
***REMOVED***
        ...
        if not exists:
            raise FileNotFoundError(f"File {path_or_buf***REMOVED*** does not exist")
...
class HDFStore:
    ...
    mode : {'a', 'w', 'r', 'r+'***REMOVED***, default 'a'
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if complib is not None and complib not in tables.filters.all_complibs:
            raise ValueError(
                f"complib only supports {tables.filters.all_complibs***REMOVED*** compression."
***REMOVED***
    ...
    def __getattr__(self, name: str):
        ...
        raise AttributeError(
            f"'{type(self).__name__***REMOVED***' object has no attribute '{name***REMOVED***'"
        )
    ...
    def __repr__(self) -> str:
        ...
        return f"{type(self)***REMOVED***\nFile path: {pstr***REMOVED***\n"
    ...
    def keys(self, include: str = "pandas") -> list[str]:
        ...
        raise ValueError(
            f"`include` should be either 'pandas' or 'native' but is '{include***REMOVED***'"
        )
            ...
            if self._mode in ["a", "w"] and mode in ["r", "r+"]:
                ...
                if self.is_open:
                    raise PossibleDataLossError(
                        f"Re-opening the file [{self._path***REMOVED***] with mode [{self._mode***REMOVED***] "
...
***REMOVED***
    ...
    def get(self, key: str):
        ...
        with patch_pickle():
            ...
            if group is None:
                raise KeyError(f"No object named {key***REMOVED*** in the file")
...
***REMOVED***
        ...
        for t, k in itertools.chain([(s, selector)], zip(tbls, keys)):
            if t is None:
                raise KeyError(f"Invalid table [{k***REMOVED***]")
            if not t.is_table:
                raise TypeError(
                    f"object [{t.pathname***REMOVED***] is not a table, and cannot be used in all "
                    "select as multiple"
    ***REMOVED***
        ...
        axis = {t.non_index_axes[0][0] for t in _tbls***REMOVED***.pop()
    ...
    def put(
        ...
        ----------
        key : str
        value : {Series, DataFrame***REMOVED***
...
***REMOVED***
        ...
        ***REMOVED***
        ...
        for k, v in d.items():
            ...
            filtered = (
                {key: value for (key, value) in min_itemsize.items() if key in v***REMOVED***
...
***REMOVED***
    ...
    def info(self) -> str:
        ...
        output = f"{type(self)***REMOVED***\nFile path: {path***REMOVED***\n"
            ...
            if len(lkeys):
                ...
                for k in lkeys:
                    try:
                        ...
                        values.append(f"[invalid_HDFStore node: {dstr***REMOVED***]")
    ...
    def _check_if_open(self) -> None:
        if not self.is_open:
            raise ClosedFileError(f"{self._path***REMOVED*** file is not open!")
    ...
    def _validate_format(self, format: str) -> str:
        ...
        try:
            ...
            raise TypeError(f"invalid HDFStore format specified [{format***REMOVED***]") from err
    ...
    ***REMOVED***:
        ...
        ***REMOVED***
        ...
        # a storer node
        if "table" not in pt:
            _STORER_MAP = {"series": SeriesFixed, "frame": FrameFixed***REMOVED***
            try:
                cls = _STORER_MAP[pt]
            except KeyError as err:
                raise TypeError(
                    f"cannot properly create the storer for: [_STORER_MAP] [group->"
                    f"{group***REMOVED***,value->{type(value)***REMOVED***,format->{format***REMOVED***"
    ***REMOVED*** from err
...
class IndexCol:
    ...
    def kind_attr(self) -> str:
        return f"{self.name***REMOVED***_kind"
        ...
        if _ensure_decoded(self.kind) == "string":
            ...
            if c is not None:
                ...
                if c.itemsize < itemsize:
                    raise ValueError(
                        f"Trying to store a string with len [{itemsize***REMOVED***] in "
                        f"[{self.cname***REMOVED***] column but\nthis column has a limit of "
                        f"[{c.itemsize***REMOVED***]!\nConsider using min_itemsize to "
    ...
    def validate_attr(self, append: bool) -> None:
        ...
        if append:
            ...
            if existing_kind is not None and existing_kind != self.kind:
                raise TypeError(
                    f"incompatible kind in col [{existing_kind***REMOVED*** - {self.kind***REMOVED***]"
    ***REMOVED***
    ...
    def update_info(self, info) -> None:
        ...
        for key in self._info_fields:
            ...
            idx = info.setdefault(self.name, {***REMOVED***)
        ...
        ***REMOVED***
                    ...
                    raise ValueError(
                        f"invalid info for [{self.name***REMOVED***] for [{key***REMOVED***], "
                        f"existing_value [{existing_value***REMOVED***] conflicts with "
                        f"new value [{value***REMOVED***]"
        ***REMOVED***
...
class DataCol(IndexCol):
    ...
    def dtype_attr(self) -> str:
        return f"{self.name***REMOVED***_dtype"

    @property
    def meta_attr(self) -> str:
        return f"{self.name***REMOVED***_meta"
...
***REMOVED***
    ...
    def get_atom_coltype(cls, kind: str) -> type[Col]:
        ...
        if kind.startswith("uint"):
            k4 = kind[4:]
            col_name = f"UInt{k4***REMOVED***Col"
        elif kind.startswith("period"):
            # we store as integer
            col_name = "Int64Col"
        else:
            kcap = kind.capitalize()
            col_name = f"{kcap***REMOVED***Col"
    ...
    def __repr__(self) -> str:
        ...
        if s is not None:
            if isinstance(s, (list, tuple)):
                ...
                s = f"[{jshape***REMOVED***]"
            return f"{self.pandas_type:12.12***REMOVED*** (shape->{s***REMOVED***)"
...
class GenericFixed(Fixed):
    ...
    _index_type_map = {DatetimeIndex: "datetime", PeriodIndex: "period"***REMOVED***
    _reverse_index_map = {v: k for k, v in _index_type_map.items()***REMOVED***
    attributes: list[str] = []
...
***REMOVED***
    ...
    def read_index(
        ...
        variety = _ensure_decoded(getattr(self.attrs, f"{key***REMOVED***_variety"))
        ...
        if variety == "multi":
            ...
            raise TypeError(f"unrecognized index variety: {variety***REMOVED***")

    def write_index(self, key: str, index: Index) -> None:
        if isinstance(index, MultiIndex):
            setattr(self.attrs, f"{key***REMOVED***_variety", "multi")
            self.write_multi_index(key, index)
        else:
            setattr(self.attrs, f"{key***REMOVED***_variety", "regular")
    ...
    def write_multi_index(self, key: str, index: MultiIndex) -> None:
        setattr(self.attrs, f"{key***REMOVED***_nlevels", index.nlevels)
    ...
    ***REMOVED***
            level_key = f"{key***REMOVED***_level{i***REMOVED***"
            ...
            # write the name
            setattr(node._v_attrs, f"{key***REMOVED***_name{name***REMOVED***", name)

            # write the labels
            label_key = f"{key***REMOVED***_label{i***REMOVED***"
            self.write_array(label_key, level_codes)

    def read_multi_index(
        self, key: str, start: int | None = None, stop: int | None = None
    ) -> MultiIndex:
        nlevels = getattr(self.attrs, f"{key***REMOVED***_nlevels")
...
***REMOVED***
            ...
            node._v_attrs.value_type = f"datetime64[{value.dtype.unit***REMOVED***]"
        elif lib.is_np_dtype(value.dtype, "m"):
            self._handle.create_array(self.group, key, value.view("i8"))
...
class BlockManagerFixed(GenericFixed):
    ...
    def shape(self) -> Shape | None:
        try:
            ...
            for i in range(self.nblocks):
                node = getattr(self.group, f"block{i***REMOVED***_items")
    ...
    def read(
        ...
        for i in range(self.ndim):
            ...
            ax = self.read_index(f"axis{i***REMOVED***", start=_start, stop=_stop)
            axes.append(ax)
        ...
        for i in range(self.nblocks):
            blk_items = self.read_index(f"block{i***REMOVED***_items")
            values = self.read_array(f"block{i***REMOVED***_values", start=_start, stop=_stop)
    ...
    def write(self, obj, **kwargs) -> None:
        ...
        for i, ax in enumerate(data.axes):
            ...
            self.write_index(f"axis{i***REMOVED***", ax)
        ...
        for i, blk in enumerate(data.blocks):
            ...
            self.write_array(f"block{i***REMOVED***_values", blk.values, items=blk_items)
            self.write_index(f"block{i***REMOVED***_items", blk_items)
...
class Table(Fixed):
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"{self.pandas_type:12.12***REMOVED***{ver***REMOVED*** "
            f"(typ->{self.table_type_short***REMOVED***,nrows->{self.nrows***REMOVED***,"
            f"ncols->{self.ncols***REMOVED***,indexers->[{jindex_axes***REMOVED***]{dc***REMOVED***)"
        )
    ...
    def validate(self, other) -> None:
        ...
        if other.table_type != self.table_type:
            raise TypeError(
                ...
                f"[{other.table_type***REMOVED*** - {self.table_type***REMOVED***]"
***REMOVED***
        ...
        for c in ["index_axes", "non_index_axes", "values_axes"]:
            ...
            if sv != ov:
                ...
                for i, sax in enumerate(sv):  # type: ignore[arg-type]
                    ...
                    if sax != oax:
                        raise ValueError(
                            f"invalid combination of [{c***REMOVED***] on appending data "
                            f"[{sax***REMOVED***] vs current table [{oax***REMOVED***]"
            ***REMOVED***

                # should never get here
                raise Exception(
                    f"invalid combination of [{c***REMOVED***] on appending data [{sv***REMOVED***] vs "
                    f"current table [{ov***REMOVED***]"
    ***REMOVED***
...
***REMOVED***
    ...
    def queryables(self) -> dict[str, Any]:
        ...
        axis_names = {0: "index", 1: "columns"***REMOVED***
    ...
    def _get_metadata_path(self, key: str) -> str:
        ...
        return f"{group***REMOVED***/meta/{key***REMOVED***/meta"
    ...
    def get_attrs(self) -> None:
        ...
        self.info = getattr(self.attrs, "info", None) or {***REMOVED***
    ...
    ***REMOVED***
    ...
    def validate_min_itemsize(self, min_itemsize) -> None:
        ...
        for k in min_itemsize:
            ...
            if k not in q:
                raise ValueError(
                    f"min_itemsize has the key [{k***REMOVED***] which is not an axis or "
                    "data_column"
    ***REMOVED***
    ...
    def indexables(self):
        ...
        for i, (axis, name) in enumerate(self.attrs.index_cols):
            ...
            kind_attr = f"{name***REMOVED***_kind"
...
***REMOVED***
        ...
        def f(i, c):
            ...
            values = getattr(table_attrs, f"{adj_name***REMOVED***_kind", None)
            dtype = getattr(table_attrs, f"{adj_name***REMOVED***_dtype", None)
            ...
            #  meta = "category" if md is not None else None
            meta = getattr(table_attrs, f"{adj_name***REMOVED***_meta", None)
    ...
    def create_index(
        ...
        for c in columns:
            ...
            ***REMOVED***
                ...
                # GH 28156
                raise AttributeError(
                    f"column {c***REMOVED*** is not a data_column.\n"
                    f"In order to read column {c***REMOVED*** you must reload the dataframe \n"
                    f"into HDFStore and include {c***REMOVED*** with the data_columns argument."
    ***REMOVED***
...
***REMOVED***
    ...
    def validate_data_columns(self, data_columns, min_itemsize, non_index_axes):
        ...
        info = self.info.get(axis, {***REMOVED***)
        if info.get("type") == "MultiIndex" and data_columns:
            raise ValueError(
                f"cannot use a multi-index on axis [{axis***REMOVED***] with "
                f"data_columns {data_columns***REMOVED***"
***REMOVED***
...
***REMOVED***
    ...
    def _create_axes(
        ...
        if not isinstance(obj, DataFrame):
            ...
            raise TypeError(
                f"cannot properly create the storer for: [group->{group***REMOVED***,"
                f"value->{type(obj)***REMOVED***]"
***REMOVED***
...
***REMOVED***:
    ...
    ***REMOVED***:
        ...
        info = new_info.setdefault(idx, {***REMOVED***)
        ...
        for i, (blk, b_items) in enumerate(zip(blocks, blk_items)):
            ...
            if table_exists and validate:
                try:
                    ...
                    raise ValueError(
                        f"Incompatible appended table [{blocks***REMOVED***]"
                        f"with existing table [{self.values_axes***REMOVED***]"
        ***REMOVED*** from err
            else:
                existing_col = None

            new_name = name or f"values_block_{i***REMOVED***"
...
***REMOVED***
            ...
            ***REMOVED***
            ...
            for ea in values_axes:
                ...
                try:
                    ...
                    raise ValueError(
                        f"cannot match existing table structure for [{jitems***REMOVED***] "
                        "on appending data"
        ***REMOVED*** from err
            blocks = new_blocks
    ...
    def process_axes(self, obj, selection: Selection, columns=None) -> DataFrame:
        ...
        for axis, labels in self.non_index_axes:
            ...
            def process_filter(field, filt, op):
                ...
                raise ValueError(f"cannot find the field [{field***REMOVED***] for filtering!")
    ...
    def create_description(
        ...
        d = {"name": "table", "expectedrows": expectedrows***REMOVED***

        # description from the axes & values
        d["description"] = {a.cname: a.typ for a in self.axes***REMOVED***
...
***REMOVED***
        ...
        for a in self.axes:
            if column == a.name:
                if not a.is_data_indexable:
                    raise ValueError(
                        f"column [{column***REMOVED***] can not be extracted individually; "
    ...
    ***REMOVED***
        ...
        raise KeyError(f"column [{column***REMOVED***] not found in the table")
...
class AppendableFrameTable(AppendableTable):
    ...
    def read(
        ...
        info = (
            self.info.get(self.non_index_axes[0][0], {***REMOVED***)
...
def _unconvert_index(data, kind: str, encoding: str, errors: str) -> np.ndarray | Index:
    ...
    if kind.startswith("datetime64"):
        ...
        raise ValueError(f"unrecognized index type {kind***REMOVED***")
    return index
...
def _maybe_convert_for_string_atom(
    ...
    if inferred_type != "string":
        ...
        for i in range(data.shape[0]):
            ...
            if inferred_type != "string":
                error_column_label = columns[i] if len(columns) > i else f"No.{i***REMOVED***"
                raise TypeError(
                    f"Cannot serialize the column [{error_column_label***REMOVED***]\n"
                    f"because its data contents are not [string] but "
                    f"[{inferred_type***REMOVED***] object dtype"
    ***REMOVED***
    ...
    data_converted = data_converted.astype(f"|S{itemsize***REMOVED***", copy=False)
...
def _convert_string_array(data: np.ndarray, encoding: str, errors: str) -> np.ndarray:
    ...
    data = np.asarray(data, dtype=f"S{itemsize***REMOVED***")
    return data
...
def _get_converter(kind: str, encoding: str, errors: str):
    if kind == "datetime64":
        ...
        raise ValueError(f"invalid kind {kind***REMOVED***")
...
def _dtype_to_kind(dtype_str: str) -> str:
    ...
    if dtype_str.startswith(("string", "bytes")):
        ...
        raise ValueError(f"cannot interpret dtype of [{dtype_str***REMOVED***]")
...
def _get_data_and_dtype_name(data: ArrayLike):
    ...
    if isinstance(data.dtype, DatetimeTZDtype):
        ...
        dtype_name = f"datetime64[{data.dtype.unit***REMOVED***]"
    else:
...
class Selection:
    ...
    def generate(self, where):
        ...
        try:
            ...
            msg = dedent(
                f"""\
                The passed where expression: {where***REMOVED***
                            contains an invalid variable reference
                            all of the variable references must be a reference to
                            an axis (e.g. 'index' or 'columns'), or a data_column
                            The currently defined references are: {qkeys***REMOVED***
                """
***REMOVED***
