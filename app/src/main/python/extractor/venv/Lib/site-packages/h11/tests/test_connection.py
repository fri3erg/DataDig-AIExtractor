...
def test__body_framing() -> None:
    ...
    for kwargs in [{***REMOVED***, {"cl": 100***REMOVED***, {"te": True***REMOVED***, {"cl": 100, "te": True***REMOVED***]:
    ...
    # Transfer-encoding
    for kwargs in [{"te": True***REMOVED***, {"cl": 100, "te": True***REMOVED***]:
        kwargs = cast(Dict[str, Any], kwargs)
...
def test_Connection_basics_and_content_length() -> None:
    ...
    for conn in p.conns:
        assert conn.states == {CLIENT: SEND_BODY, SERVER: SEND_RESPONSE***REMOVED***
    ...
    for conn in p.conns:
        assert conn.states == {CLIENT: SEND_BODY, SERVER: SEND_BODY***REMOVED***
    ...
    for conn in p.conns:
        assert conn.states == {CLIENT: DONE, SERVER: SEND_BODY***REMOVED***
    ...
    for conn in p.conns:
        assert conn.states == {CLIENT: DONE, SERVER: DONE***REMOVED***
...
***REMOVED***,
    ...
    for conn in p.conns:
        assert conn.states == {CLIENT: MUST_CLOSE, SERVER: MUST_CLOSE***REMOVED***
...
***REMOVED***,
        ...
        for conn in p.conns:
            assert conn.states == {CLIENT: SWITCHED_PROTOCOL, SERVER: SWITCHED_PROTOCOL***REMOVED***
...
def test_close_different_states() -> None:
    ...
    for conn in p.conns:
        assert conn.states == {CLIENT: CLOSED, SERVER: MUST_CLOSE***REMOVED***
    ...
    p.send(CLIENT, ConnectionClosed())
    for conn in p.conns:
        assert conn.states == {CLIENT: CLOSED, SERVER: SEND_RESPONSE***REMOVED***
    ...
    for conn in p.conns:
        assert conn.states == {CLIENT: MUST_CLOSE, SERVER: CLOSED***REMOVED***
...
def test_pipelined_close() -> None:
    ...
    assert c.states == {CLIENT: CLOSED, SERVER: SEND_RESPONSE***REMOVED***
    c.send(Response(status_code=200, headers=[]))  # type: ignore[arg-type]
    c.send(EndOfMessage())
    assert c.states == {CLIENT: CLOSED, SERVER: MUST_CLOSE***REMOVED***
    c.send(ConnectionClosed())
    assert c.states == {CLIENT: CLOSED, SERVER: CLOSED***REMOVED***
