...
***REMOVED***
    ...
    def get(
        ...
        if status_code != 200:
            logger.error(f"Failed to get records: {response['message']***REMOVED***")
            raise Exception("Error: {***REMOVED***.".format(response["message"]))
    ...
    def _create_collection(
        ...
        fields: List[dict] = [
            {"name": "id", "dataType": "INT"***REMOVED***,
            {"name": "text", "dataType": "STRING"***REMOVED***,
            {"name": "embeddings", "dataType": "VECTOR_FLOAT", "dimensions": dim***REMOVED***,
        ]
            ...
            for metadata in metadatas:
                for key, value in metadata.items():
                    ...
                    if isinstance(value, str):
                        ...
                        raise ValueError(f"Unsupported data type for {key***REMOVED***.")
                    fields.append({"name": key, "dataType": d_type***REMOVED***)
        ...
        if status_code != 200:
            if status_code == 409:
                logger.info(f"Continuing with the existing table {table_name***REMOVED***.")
            else:
                logger.error(
                    f"Failed to create collection {table_name***REMOVED***: {response['message']***REMOVED***"
    ***REMOVED***
                raise Exception("Error: {***REMOVED***.".format(response["message"]))
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        if status_code != 200:
            logger.error(
                f"Failed to add records to {collection_name***REMOVED***: {response['message']***REMOVED***"
***REMOVED***
            raise Exception("Error: {***REMOVED***.".format(response["message"]))
    ...
    def similarity_search(
        ...
        if status_code != 200:
            logger.error(f"Search failed: {response['message']***REMOVED***.")
            raise Exception("Error: {***REMOVED***.".format(response["message"]))
