...
TRICODE_TO_NAME = {i: TRIAD_NAMES[code - 1] for i, code in enumerate(TRICODES)***REMOVED***
...
def triadic_census(G, nodelist=None):
    ...
    ...     print(f"{key***REMOVED***: {value***REMOVED***")
    ...
    003: 0
    012: 0
    102: 0
    ...
    m = {n: i for i, n in enumerate(nodeset)***REMOVED***
    if Nnot:
    ...
    # nbrs = {n: G._pred[n].keys() | G._succ[n].keys() for n in G***REMOVED***
    nbrs = {n: G.pred[n].keys() | G.succ[n].keys() for n in G***REMOVED***
    dbl_nbrs = {n: G.pred[n].keys() & G.succ[n].keys() for n in G***REMOVED***

    if Nnot:
        sgl_nbrs = {n: G.pred[n].keys() ^ G.succ[n].keys() for n in not_nodeset***REMOVED***
    ...
    census = {name: 0 for name in TRIAD_NAMES***REMOVED***
    ...
    for v in nodeset:
        ...
        for u in vnbrs:
            if m[u] <= m[v]:
                continue
            unbrs = nbrs[u]
            neighbors = (vnbrs | unbrs) - {u, v***REMOVED***
...
def triad_type(G):
    ...
    {m***REMOVED***{a***REMOVED***{n***REMOVED***{type***REMOVED*** (for example: 111D, 210, 102)

    Here:

    {m***REMOVED***     = number of mutual ties (takes 0, 1, 2, 3); a mutual tie is (0,1)
              AND (1,0)
    {a***REMOVED***     = number of asymmetric ties (takes 0, 1, 2, 3); an asymmetric tie
              is (0,1) BUT NOT (1,0) or vice versa
    {n***REMOVED***     = number of null ties (takes 0, 1, 2, 3); a null tie is NEITHER
              (0,1) NOR (1,0)
    {type***REMOVED***  = a letter (takes U, D, C, T) corresponding to up, down, cyclical
    ...
    if num_edges == 0:
        ...
        for e1, e2, e3 in permutations(G.edges(), 3):
            if set(e1) == set(e2):
                ...
                if {e1[0], e2[0], e3[0]***REMOVED*** == {e1[0], e2[0], e3[0]***REMOVED*** == set(G.nodes()):
                    return "030C"
        ...
        for e1, e2, e3, e4 in permutations(G.edges(), 4):
            if set(e1) == set(e2):
                ...
                if set(e3) == set(e4):
                    return "201"
                if {e3[0]***REMOVED*** == {e4[0]***REMOVED*** == set(e3).intersection(set(e4)):
                    return "120D"
                if {e3[1]***REMOVED*** == {e4[1]***REMOVED*** == set(e3).intersection(set(e4)):
                    return "120U"
                if e3[1] == e4[0]:
                    return "120C"
    elif num_edges == 5:
...
def random_triad(G, seed=None):
    ...
    if len(G) < 3:
        raise nx.NetworkXError(
            f"G needs at least 3 nodes to form a triad; (it has {len(G)***REMOVED*** nodes)"
        )
