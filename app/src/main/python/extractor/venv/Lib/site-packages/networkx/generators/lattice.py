...
def triangular_lattice_graph(
    ...
    if periodic:
        if n < 5 or m < 3:
            msg = f"m > 2 and n > 4 required for periodic. m={m***REMOVED***, n={n***REMOVED***"
    ...
    if with_positions:
        ...
        pos = {(i, j): (x, y) for i, j, x, y in zip(ii, jj, xx, yy) if (i, j) in H***REMOVED***
...
def hexagonal_lattice_graph(
    ...
    pos = {(i, j): (x, y) for i, j, x, y in zip(ii, jj, xx, yy) if (i, j) in G***REMOVED***
