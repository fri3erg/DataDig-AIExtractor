...
def _check_arg_length(fname, args, max_fname_arg_count, compat_args) -> None:
    ...
    if len(args) > len(compat_args):
        ...
        raise TypeError(
            f"{fname***REMOVED***() takes at most {max_arg_count***REMOVED*** {argument***REMOVED*** "
            f"({actual_arg_count***REMOVED*** given)"
        )
...
def _check_for_default_values(fname, arg_val_dict, compat_args) -> None:
    ...
    for key in arg_val_dict:
        ...
        if not match:
            raise ValueError(
                f"the '{key***REMOVED***' parameter is not supported in "
                f"the pandas implementation of {fname***REMOVED***()"
***REMOVED***
...
def _check_for_invalid_keys(fname, kwargs, compat_args) -> None:
    ...
    if diff:
        ...
        raise TypeError(f"{fname***REMOVED***() got an unexpected keyword argument '{bad_arg***REMOVED***'")
...
def validate_args_and_kwargs(
    ...
    for key in args_dict:
        if key in kwargs:
            raise TypeError(
                f"{fname***REMOVED***() got multiple values for keyword argument '{key***REMOVED***'"
***REMOVED***
...
def validate_bool_kwarg(
    ...
    if not good_value:
        raise ValueError(
            f'For argument "{arg_name***REMOVED***" expected type bool, received '
            f"type {type(value).__name__***REMOVED***."
        )
    ...
    if value is None and method is not None:
        ...
        if validate_scalar_dict_value and isinstance(value, (list, tuple)):
            raise TypeError(
                ...
                f'you passed a "{type(value).__name__***REMOVED***"'
***REMOVED***
...
def validate_ascending(
    ...
    kwargs = {"none_allowed": False, "int_allowed": True***REMOVED***
...
def validate_endpoints(closed: str | None) -> tuple[bool, bool]:
    ...
    Parameters
    ----------
    closed : {None, "left", "right"***REMOVED***
...
def validate_inclusive(inclusive: str | None) -> tuple[bool, bool]:
    """
    Check that the `inclusive` argument is among {"both", "neither", "left", "right"***REMOVED***.
...
def validate_insert_loc(loc: int, length: int) -> int:
    ...
    if not is_integer(loc):
        raise TypeError(f"loc must be an integer between -{length***REMOVED*** and {length***REMOVED***")

    if loc < 0:
        loc += length
    if not 0 <= loc <= length:
        raise IndexError(f"loc must be an integer between -{length***REMOVED*** and {length***REMOVED***")
...
def check_dtype_backend(dtype_backend) -> None:
    if dtype_backend is not lib.no_default:
        if dtype_backend not in ["numpy_nullable", "pyarrow"]:
            raise ValueError(
                f"dtype_backend {dtype_backend***REMOVED*** is invalid, only 'numpy_nullable' and "
