...
class BaseArrayManager(DataManager):
    ...
    def __repr__(self) -> str:
        ...
        output += f"\nIndex: {self._axes[0]***REMOVED***"
        if self.ndim == 2:
            output += f"\nColumns: {self._axes[1]***REMOVED***"
        output += f"\n{len(self.arrays)***REMOVED*** arrays:"
    ...
    def insert(self, loc: int, item: Hashable, value: ArrayLike, refs=None) -> None:
        ...
        if value.ndim == 2:
            if value.shape[0] == 1:
                ...
                raise ValueError(
                    f"Expected a 1D array, got an array with shape {value.shape***REMOVED***"
    ***REMOVED***
        value = maybe_coerce_values(value)
...
def concat_arrays(to_concat: list) -> ArrayLike:
    ...
    dtypes = {x.dtype for x in to_concat_no_proxy***REMOVED***
    ...
    if len(result) == 0:
        ...
        kinds = {obj.dtype.kind for obj in to_concat_no_proxy***REMOVED***
        if len(kinds) != 1:
            if "b" in kinds:
