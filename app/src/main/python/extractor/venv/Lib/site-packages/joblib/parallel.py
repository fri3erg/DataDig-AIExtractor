...
MAYBE_AVAILABLE_BACKENDS = {'multiprocessing', 'loky'***REMOVED***
...
def _get_active_backend(
    ...
    if prefer not in VALID_BACKEND_HINTS:
        raise ValueError(
            f"prefer={prefer***REMOVED*** is not a valid backend hint, "
            f"expected one of {VALID_BACKEND_HINTS***REMOVED***"
        )
    if require not in VALID_BACKEND_CONSTRAINTS:
        raise ValueError(
            f"require={require***REMOVED*** is not a valid backend constraint, "
            f"expected one of {VALID_BACKEND_CONSTRAINTS***REMOVED***"
        )
    ...
    if force_threads:
        ...
        if verbose >= 10 and explicit_backend:
            print(
                f"Using {sharedmem_backend.__class__.__name__***REMOVED*** as "
                f"joblib backend instead of {backend.__class__.__name__***REMOVED*** "
...
class parallel_config:
    ...
    mmap_mode: {None, 'r+', 'r', 'w+', 'c'***REMOVED***, default='r'
        Memmapping mode for numpy arrays passed to workers. None will
        disable memmapping, other modes defined in the numpy.memmap doc:
        https://numpy.org/doc/stable/reference/generated/numpy.memmap.html
        Also, see 'max_nbytes' parameter documentation for more details.

    prefer: str in {'processes', 'threads'***REMOVED*** or None, default=None
        ...
        if isinstance(backend, str):
            ...
            if backend not in BACKENDS:
                if backend in EXTERNAL_BACKENDS:
                    ...
                    warnings.warn(
                        f"joblib backend '{backend***REMOVED***' is not available on "
                        f"your system, falling back to {DEFAULT_BACKEND***REMOVED***.",
                        UserWarning,
                        stacklevel=2
        ***REMOVED***
                    ...
                    raise ValueError(
                        f"Invalid backend: {backend***REMOVED***, expected one of "
                        f"{sorted(BACKENDS.keys())***REMOVED***"
        ***REMOVED***
        ...
        if inner_max_num_threads is not None:
            msg = (
                f"{backend.__class__.__name__***REMOVED*** does not accept setting the "
...
class BatchedCalls(object):
    ...
    def __init__(self, iterator_slice, backend_and_jobs, reducer_callback=None,
        ...
        self._pickle_cache = pickle_cache if pickle_cache is not None else {***REMOVED***
...
class Parallel(Logger):
    ''' Helper class for readable parallel mapping.
        ...
        return_as: str in {'list', 'generator'***REMOVED***, default: 'list'
        ...
        pre_dispatch: {'all', integer, or expression, as in '3*n_jobs'***REMOVED***
    ...
    def __init__(
        ...
        if return_as not in {"list", "generator"***REMOVED***:
            raise ValueError(
                'Expected `return_as` parameter to be a string equal to "list"'
                f' or "generator", but got {return_as***REMOVED*** instead'
***REMOVED***
...
***REMOVED***
        ...
        if not isinstance(backend, SequentialBackend):
            if self.return_generator and not backend.supports_return_generator:
                raise ValueError(
                    "Backend {***REMOVED*** does not support "
                    "return_as={***REMOVED***".format(backend, return_as)
    ***REMOVED***
    ...
    def _initialize_backend(self):
        ...
        try:
            ...
            if self.timeout is not None and not self._backend.supports_timeout:
                warnings.warn(
                    'The backend class {!r***REMOVED*** does not support timeout. '
                    "You have set 'timeout={***REMOVED***' in Parallel but "
    ...
    def _print(self, msg):
        ...
        writer(f"[{self***REMOVED***]: {msg***REMOVED***\n")
    ...
    def print_progress(self):
        ...
        if self._is_completed():
            ...
            self._print(
                f"Done {self.n_completed_tasks:3d***REMOVED*** out of "
                f"{self.n_completed_tasks:3d***REMOVED*** | elapsed: "
                f"{short_format_time(elapsed_time)***REMOVED*** finished"
***REMOVED***
            return
        ...
        # tasks.
            ...
            if _verbosity_filter(self.n_dispatched_batches, self.verbose):
                return
            self._print(
                f"Done {self.n_completed_tasks:3d***REMOVED*** tasks      | elapsed: "
                f"{short_format_time(elapsed_time)***REMOVED***"
***REMOVED***
        else:
            ...
            self._print(
                f"Done {index:3d***REMOVED*** out of {total_tasks:3d***REMOVED*** | elapsed: "
                f"{short_format_time(elapsed_time)***REMOVED*** remaining: "
                f"{short_format_time(remaining_time)***REMOVED***"
***REMOVED***
    ...
    ***REMOVED***.start()
    ...
    def _warn_exit_early(self):
        ...
        if ready_outputs:
            msg += (
                f"{ready_outputs***REMOVED*** tasks have been successfully executed "
                " but not used."
***REMOVED***
            if not is_completed:
                msg += " Additionally, "
        ...
        if not is_completed:
            msg += (
                f"{self.n_dispatched_tasks - self.n_completed_tasks***REMOVED*** tasks "
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        self._print(
            f"Using backend {backend_name***REMOVED*** with {n_jobs***REMOVED*** concurrent workers."
        )
