...
def test_map(na_action):
    ...
    def f(x):
        return {"A": 10, "B": 20, "C": 30***REMOVED***.get(x)
    ...
    result = cat.map({"A": 10, "B": 20, "C": 30***REMOVED***, na_action=na_action)
    tm.assert_categorical_equal(result, exp)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ([1, 1, np.nan], {1: False***REMOVED***, Categorical([False, False, np.nan])),
        ([1, 2, np.nan], {1: False, 2: False***REMOVED***, Index([False, False, np.nan])),
        (
            [1, 1, np.nan],
            Series([False, False]),
