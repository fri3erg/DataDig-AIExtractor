...
def test_table_index_incompatible_dtypes(setup_path):
    ...
    df2 = DataFrame({"a": [4, 5, 6]***REMOVED***, index=date_range("1/1/2000", periods=3))
...
***REMOVED***
            df[n] = f
            msg = re.escape(f"[{n***REMOVED***] is not implemented as a table column")
            with pytest.raises(TypeError, match=msg):
                store.append(f"df1_{n***REMOVED***", df)
...
def test_append_with_diff_col_name_types_raises_value_error(setup_path):
    ...
    df2 = DataFrame({"a": np.random.default_rng(2).standard_normal(10)***REMOVED***)
    df3 = DataFrame({(1, 2): np.random.default_rng(2).standard_normal(10)***REMOVED***)
    df4 = DataFrame({("1", 2): np.random.default_rng(2).standard_normal(10)***REMOVED***)
    df5 = DataFrame({("1", 2, object): np.random.default_rng(2).standard_normal(10)***REMOVED***)
