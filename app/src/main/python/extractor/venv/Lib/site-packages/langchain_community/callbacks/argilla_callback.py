...
class ArgillaCallbackHandler(BaseCallbackHandler):
    ...
    ISSUES_URL: str = f"{REPO_URL***REMOVED***/issues"
...
***REMOVED***
        ...
        if parse(self.ARGILLA_VERSION) < parse("1.8.0"):
            raise ImportError(
                f"The installed `argilla` version is {self.ARGILLA_VERSION***REMOVED*** but "
        ...
        if api_url is None and os.getenv("ARGILLA_API_URL") is None:
            warnings.warn(
                (
                    ...
                    f" set, it will default to `{self.DEFAULT_API_URL***REMOVED***`, which is the"
...
***REMOVED***
            ...
            warnings.warn(
                (
                    ...
                    f" set, it will default to `{self.DEFAULT_API_KEY***REMOVED***`, which is the"
...
***REMOVED***
        ...
        try:
            ...
            raise ConnectionError(
                f"Could not connect to Argilla with exception: '{e***REMOVED***'.\n"
                "Please check your `api_key` and `api_url`, and make sure that "
                "the Argilla server is up and running. If the problem persists "
                f"please report it to {self.ISSUES_URL***REMOVED*** as an `integration` issue."
***REMOVED*** from e
        ...
        try:
            ...
            if parse(self.ARGILLA_VERSION) < parse("1.14.0"):
                warnings.warn(
                    f"You have Argilla {self.ARGILLA_VERSION***REMOVED***, but Argilla 1.14.0 or"
                    " higher is recommended.",
                    UserWarning,
    ***REMOVED***
                extra_args = {"with_records": False***REMOVED***
...
***REMOVED***
            ...
            raise FileNotFoundError(
                f"`FeedbackDataset` retrieval from Argilla failed with exception `{e***REMOVED***`."
                f"\nPlease check that the dataset with name={self.dataset_name***REMOVED*** in the"
                f" workspace={self.workspace_name***REMOVED*** exists in advance. If you need help"
                " on how to create a `langchain`-compatible `FeedbackDataset` in"
                f" Argilla, please visit {self.BLOG_URL***REMOVED***. If the problem persists"
                f" please report it to {self.ISSUES_URL***REMOVED*** as an `integration` issue."
***REMOVED*** from e
        ...
        if supported_fields != [field.name for field in self.dataset.fields]:
            raise ValueError(
                f"`FeedbackDataset` with name={self.dataset_name***REMOVED*** in the workspace="
                f"{self.workspace_name***REMOVED*** had fields that are not supported yet for the"
                f"`langchain` integration. Supported fields are: {supported_fields***REMOVED***,"
                f" and the current `FeedbackDataset` fields are {[field.name for field in self.dataset.fields]***REMOVED***."  # noqa: E501
                " For more information on how to create a `langchain`-compatible"
                f" `FeedbackDataset` in Argilla, please visit {self.BLOG_URL***REMOVED***."
***REMOVED***

        self.prompts: Dict[str, List[str]] = {***REMOVED***
        ...
        warnings.warn(
            (
                ...
                f" {self.ISSUES_URL***REMOVED*** as an `integration` issue."
***REMOVED***,
        )
    ...
    def on_llm_start(
        ...
        self.prompts.update({str(kwargs["parent_run_id"] or kwargs["run_id"]): prompts***REMOVED***)
