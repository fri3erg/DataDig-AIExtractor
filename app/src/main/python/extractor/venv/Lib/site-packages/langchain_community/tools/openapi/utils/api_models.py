...
class APIPropertyLocation(Enum):
    ...
    def from_str(cls, location: str) -> "APIPropertyLocation":
        ...
        try:
            ...
            raise ValueError(
                f"Invalid APIPropertyLocation. Valid values are {cls.__members__***REMOVED***"
***REMOVED***
...
INVALID_LOCATION_TEMPL = (
    'Unsupported APIPropertyLocation "{location***REMOVED***"'
    " for parameter {name***REMOVED***. "
    + f"Valid values are {[loc.value for loc in SUPPORTED_LOCATIONS]***REMOVED***"
)
...
class APIProperty(APIPropertyBase):
    ...
    def _get_schema_type_for_enum(parameter: Parameter, schema: Schema) -> Enum:
        ...
        param_name = f"{parameter.name***REMOVED***Enum"
        return Enum(param_name, {str(v): v for v in schema.enum***REMOVED***)
    ...
    def _get_schema_type_for_array(
        ...
        if isinstance(items, Schema):
            ...
            raise ValueError(f"Unsupported array items: {items***REMOVED***")
    ...
    def _get_schema_type(parameter: Parameter, schema: Optional[Schema]) -> SCHEMA_TYPE:
        ...
        if schema_type == "array":
            ...
            raise NotImplementedError(f"Unsupported type: {schema_type***REMOVED***")
...
***REMOVED***
    ...
    def _get_schema(parameter: Parameter, spec: OpenAPISpec) -> Optional[Schema]:
        ...
        if isinstance(schema, Reference):
            ...
            raise ValueError(f"Error dereferencing schema: {schema***REMOVED***")
...
class APIRequestBodyProperty(APIPropertyBase):
    ...
    def _process_object_schema(
        ...
        if schema.properties is None:
            raise ValueError(
                f"No properties found when processing object schema: {schema***REMOVED***"
***REMOVED***
        ...
        if items is not None:
            if isinstance(items, Reference):
                ...
                if ref_name not in references_used:
                    ...
                    pass
                return f"Array<{ref_name***REMOVED***>"
            else:
                pass
            ...
            if isinstance(items, Schema):
                array_type = cls.from_schema(
                    schema=items,
                    name=f"{name***REMOVED***Item",
                    required=True,  # TODO: Add required
                    spec=spec,
                    references_used=references_used,
    ***REMOVED***
                return f"Array<{array_type.type***REMOVED***>"
...
***REMOVED***
            ...
            raise ValueError(f"Unsupported type: {schema_type***REMOVED***")
...
class APIRequestBody(BaseModel):
    ...
    def _process_supported_media_type(
        ...
        if schema is None:
            raise ValueError(
                f"Could not resolve schema for media type: {media_type_obj***REMOVED***"
***REMOVED***
    ...
    ***REMOVED***
    ...
    def ts_type_from_python(type_: SCHEMA_TYPE) -> str:
        if type_ is None:
            ...
            return {
                ...
                "float": "number",
                "date-time": "string",
            ***REMOVED***.get(type_, type_)
        elif isinstance(type_, tuple):
            return f"Array<{APIOperation.ts_type_from_python(type_[0])***REMOVED***>"
        elif isinstance(type_, type) and issubclass(type_, Enum):
            return " | ".join([f"'{e.value***REMOVED***'" for e in type_])
        else:
            return str(type_)
    ...
    def _format_nested_properties(
        ...
        for prop in properties:
            ...
            prop_desc = f"/* {prop.description***REMOVED*** */" if prop.description else ""

            if prop.properties:
                nested_props = self._format_nested_properties(
                    prop.properties, indent + 2
    ***REMOVED***
                prop_type = f"{{\n{nested_props***REMOVED***\n{' ' * indent***REMOVED******REMOVED******REMOVED***"

            formatted_props.append(
                f"{prop_desc***REMOVED***\n{' ' * indent***REMOVED***{prop_name***REMOVED***"
                f"{prop_required***REMOVED***: {prop_type***REMOVED***,"
***REMOVED***
...
***REMOVED***
        ...
        for prop in self.properties:
            ...
            prop_required = "" if prop.required else "?"
            prop_desc = f"/* {prop.description***REMOVED*** */" if prop.description else ""
            params.append(f"{prop_desc***REMOVED***\n\t\t{prop_name***REMOVED***{prop_required***REMOVED***: {prop_type***REMOVED***,")

        formatted_params = "\n".join(params).strip()
        description_str = f"/* {self.description***REMOVED*** */" if self.description else ""
        typescript_definition = f"""
{description_str***REMOVED***
type {operation_name***REMOVED*** = (_: {{
{formatted_params***REMOVED***
***REMOVED******REMOVED***) => any;
"""
