...
***REMOVED***
    ...
    def test_empty(self, interp_method):
        interpolation, method = interp_method
        q = DataFrame({"x": [], "y": []***REMOVED***).quantile(
            0.1, axis=0, numeric_only=True, interpolation=interpolation, method=method
        )
    ...
    def test_non_numeric_exclusion(self, interp_method, request, using_array_manager):
        interpolation, method = interp_method
        df = DataFrame({"col1": ["A", "A", "B", "B"], "col2": [1, 2, 3, 4]***REMOVED***)
        rs = df.quantile(
    ...
    def test_axis(self, interp_method, request, using_array_manager):
        # axis
        interpolation, method = interp_method
        df = DataFrame({"A": [1, 2, 3], "B": [2, 3, 4]***REMOVED***, index=[1, 2, 3])
        ...
        expected = DataFrame(
            {1: [1.5, 1.75], 2: [2.5, 2.75], 3: [3.5, 3.75]***REMOVED***, index=[0.5, 0.75]
        )
        if interpolation == "nearest":
            expected.iloc[0, :] -= 0.5
    ...
    def test_quantile_interpolation(self):
        ...
        # float
        df = DataFrame({"A": [1.0, 2.0, 3.0], "B": [2.0, 3.0, 4.0]***REMOVED***, index=[1, 2, 3])
        result = df.quantile(0.5, axis=1, interpolation="nearest")
        ...
        expected = DataFrame(
            {1: [1.0, 1.0], 2: [2.0, 2.0], 3: [3.0, 3.0]***REMOVED***, index=[0.5, 0.75]
        )
        tm.assert_frame_equal(result, expected)

        # test degenerate case
        df = DataFrame({"x": [], "y": []***REMOVED***)
    ...
    def test_quantile_datetime(self, unit):
        ...
        result = df.quantile([0.5], numeric_only=False)
        expected = DataFrame(
            {"a": Timestamp("2010-07-02 12:00:00").as_unit(unit), "b": 2.5***REMOVED***,
            index=[0.5],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_quantile_nan(self, interp_method, request, using_array_manager):
        ...
        df = DataFrame({"a": np.arange(1, 6.0), "b": np.arange(1, 6.0)***REMOVED***)
        df.iloc[-1, 1] = np.nan
        ...
        res = df.quantile([0.5, 0.75], interpolation=interpolation, method=method)
        exp = DataFrame({"a": [3.0, 4.0], "b": [np.nan, np.nan]***REMOVED***, index=[0.5, 0.75])
        tm.assert_frame_equal(res, exp)
    ...
    def test_quantile_nat(self, interp_method, request, using_array_manager, unit):
        ...
        if method == "table" and using_array_manager:
            request.applymarker(pytest.mark.xfail(reason="Axis name incorrectly set."))
        # full NaT column
        df = DataFrame({"a": [pd.NaT, pd.NaT, pd.NaT]***REMOVED***, dtype=f"M8[{unit***REMOVED***]")

        res = df.quantile(
            0.5, numeric_only=False, interpolation=interpolation, method=method
        )
        exp = Series([pd.NaT], index=["a"], name=0.5, dtype=f"M8[{unit***REMOVED***]")
        tm.assert_series_equal(res, exp)

        res = df.quantile(
            [0.5], numeric_only=False, interpolation=interpolation, method=method
        )
        exp = DataFrame({"a": [pd.NaT]***REMOVED***, index=[0.5], dtype=f"M8[{unit***REMOVED***]")
        tm.assert_frame_equal(res, exp)
...
***REMOVED***,
    ...
    def test_quantile_ea_scalar(self, request, obj, index):
        ...
        expected = Series({"A": index[4]***REMOVED***, dtype=exp_dtype, name=0.5)
        if isinstance(obj, Series):
            expected = expected["A"]
