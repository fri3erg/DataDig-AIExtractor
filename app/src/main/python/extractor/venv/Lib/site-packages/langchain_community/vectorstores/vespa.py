...
***REMOVED***
        if not isinstance(app, Vespa):
            raise ValueError(
                f"app should be an instance of vespa.application.Vespa, got {type(app)***REMOVED***"
***REMOVED***
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        if ids is None:
            ids = [str(f"{i+1***REMOVED***") for i, _ in enumerate(texts)]

        batch = []
        for i, text in enumerate(texts):
            fields: Dict[str, Union[str, List[float]]] = {***REMOVED***
            ...
            batch.append({"id": ids[i], "fields": fields***REMOVED***)
        ...
        for result in results:
            if not (str(result.status_code).startswith("2")):
                raise RuntimeError(
                    ...
                    f"Error code: {result.status_code***REMOVED***. "
                    f"Message: {result.json['message']***REMOVED***"
    ***REMOVED***
        return ids
    ...
    def delete(self, ids: Optional[List[str]] = None, **kwargs: Any) -> Optional[bool]:
        if ids is None:
            return False
        batch = [{"id": id***REMOVED*** for id in ids]
    ...
    def _create_query(
        ...
        yql += f"{{targetHits: {hits***REMOVED***, approximate: {approximate***REMOVED******REMOVED******REMOVED***"
        yql += f"nearestNeighbor({doc_embedding_field***REMOVED***, {input_embedding_field***REMOVED***)"
        if filter is not None:
            yql += f" and {filter***REMOVED***"

        query = {
            "yql": yql,
            f"input.query({input_embedding_field***REMOVED***)": query_embedding,
    ...
    def similarity_search_by_vector_with_score(
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"{e.args[0][0]['summary']***REMOVED***. "
                f"Error: {e.args[0][0]['message']***REMOVED***"
***REMOVED***
        if not str(response.status_code).startswith("2"):
            raise RuntimeError(
                f"Could not retrieve data from Vespa. "
                f"Error code: {response.status_code***REMOVED***. "
                f"Message: {response.json['message']***REMOVED***"
***REMOVED***
