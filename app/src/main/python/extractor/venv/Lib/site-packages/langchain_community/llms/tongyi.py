...
def check_response(resp: Any) -> Any:
    ...
    if resp.status_code == 200:
        ...
        raise ValueError(
            f"status_code: {resp.status_code***REMOVED*** \n "
            f"code: {resp.code***REMOVED*** \n message: {resp.message***REMOVED***"
        )
    else:
        raise HTTPError(
            f"HTTP error occurred: status_code: {resp.status_code***REMOVED*** \n "
            f"code: {resp.code***REMOVED*** \n message: {resp.message***REMOVED***",
            response=resp,
        )
...
***REMOVED***
    ...
    def _identifying_params(self) -> Mapping[str, Any]:
        return {"model_name": self.model_name, **super()._identifying_params***REMOVED***
        ...
        if self.streaming:
            ...
            for prompt in prompts:
                completion = await asyncio.get_running_loop().run_in_executor(
                    None,
                    functools.partial(
                        generate_with_retry, **{"llm": self, "prompt": prompt, **params***REMOVED***
        ***REMOVED***,
    ***REMOVED***
                generations.append(
                    [Generation(**self._generation_from_qwen_resp(completion))]
