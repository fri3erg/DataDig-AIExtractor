            ...
            if env is None:
                if field.has_alias:
                    warnings.warn(
                        ...
                        FutureWarning,
        ***REMOVED***
                env_names = {cls.env_prefix + field.name***REMOVED***
            elif isinstance(env, str):
                env_names = {env***REMOVED***
            elif isinstance(env, (set, frozenset)):
                env_names = env
            elif sequence_like(env):
                env_names = list(env)
            else:
                raise TypeError(f'invalid field env: {env!r***REMOVED*** ({display_as_type(env)***REMOVED***); should be string, list or set')
...
class InitSettingsSource:
    ...
    def __repr__(self) -> str:
        return f'InitSettingsSource(init_kwargs={self.init_kwargs!r***REMOVED***)'
...
class EnvSettingsSource:
    ...
    def __call__(self, settings: BaseSettings) -> Dict[str, Any]:  # noqa C901
        ...
        if settings.__config__.case_sensitive:
            ...
            env_vars = {k.lower(): v for k, v in os.environ.items()***REMOVED***

        dotenv_vars = self._read_env_files(settings.__config__.case_sensitive)
        if dotenv_vars:
            env_vars = {**dotenv_vars, **env_vars***REMOVED***
                ...
                if env_val is None:
                    ...
                    try:
                        ...
                        if not allow_parse_failure:
                            raise SettingsError(f'error parsing env var "{env_name***REMOVED***"') from e
    ...
    ***REMOVED***
    ...
    def explode_env_vars(self, field: ModelField, env_vars: Mapping[str, Optional[str]]) -> Dict[str, Any]:
        ...
        """
        prefixes = [f'{env_name***REMOVED***{self.env_nested_delimiter***REMOVED***' for env_name in field.field_info.extra['env_names']]
        ...
        for env_name, env_val in env_vars.items():
            ...
            env_var = result
            for key in keys:
                env_var = env_var.setdefault(key, {***REMOVED***)
    ...
    def __repr__(self) -> str:
        return (
            f'EnvSettingsSource(env_file={self.env_file!r***REMOVED***, env_file_encoding={self.env_file_encoding!r***REMOVED***, '
            f'env_nested_delimiter={self.env_nested_delimiter!r***REMOVED***)'
        )
...
class SecretsSettingsSource:
    ...
    def __call__(self, settings: BaseSettings) -> Dict[str, Any]:
        ...
        """
        secrets: Dict[str, Optional[str]] = {***REMOVED***
        ...
        if not secrets_path.exists():
            warnings.warn(f'directory "{secrets_path***REMOVED***" does not exist')
            return secrets

        if not secrets_path.is_dir():
            raise SettingsError(f'secrets_dir must reference a directory, not a {path_type(secrets_path)***REMOVED***')
        ...
        for field in settings.__fields__.values():
            for env_name in field.field_info.extra['env_names']:
                ...
                if path.is_file():
                    ...
                    warnings.warn(
                        f'attempted to load secret file "{path***REMOVED***" but found a {path_type(path)***REMOVED*** instead.',
                        stacklevel=4,
        ***REMOVED***
        return secrets

    def __repr__(self) -> str:
        return f'SecretsSettingsSource(secrets_dir={self.secrets_dir!r***REMOVED***)'
...
def read_env_file(
    ...
    if not case_sensitive:
        return {k.lower(): v for k, v in file_vars.items()***REMOVED***
    else:
        return file_vars
