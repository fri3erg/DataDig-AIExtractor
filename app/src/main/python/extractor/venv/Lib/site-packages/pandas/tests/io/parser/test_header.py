...
"""
    ...
    data_gen_f = lambda r, c: f"R{r***REMOVED***C{c***REMOVED***"

    data = [[data_gen_f(r, c) for c in range(3)] for r in range(5)]
    index = MultiIndex.from_arrays(
        [[f"R_l0_g{i***REMOVED***" for i in range(5)], [f"R_l1_g{i***REMOVED***" for i in range(5)]],
        names=["R0", "R1"],
    )
    columns = MultiIndex.from_arrays(
        [
            [f"C_l0_g{i***REMOVED***" for i in range(3)],
            [f"C_l1_g{i***REMOVED***" for i in range(3)],
            [f"C_l2_g{i***REMOVED***" for i in range(3)],
            [f"C_l3_g{i***REMOVED***" for i in range(3)],
        ],
        names=["C0", "C1", "C2", "C3"],
    )
...
***REMOVED***,
        ...
        (
            {"index_col": [0, 1], "names": ["foo", "bar"]***REMOVED***,
            ("cannot specify names when specifying a multi-index header"),
        ),
        (
            {"index_col": [0, 1], "usecols": ["foo", "bar"]***REMOVED***,
...
@pytest.mark.parametrize("kwargs", [{***REMOVED***, {"index_col": False***REMOVED***])
...
@pytest.mark.parametrize(
    ...
    [
        ({***REMOVED***, [0, 1, 2, 3, 4]),
        (
            {"names": ["foo", "bar", "baz", "quux", "panda"]***REMOVED***,
...
def test_singleton_header(all_parsers):
    ...
    expected = DataFrame({"a": [0, 1], "b": [1, 2], "c": [2, 3]***REMOVED***)
...
def test_multi_index_unnamed(all_parsers, index_col, columns):
    ...
    for i, col in enumerate(columns):
        if not col:  # Unnamed.
            col = f"Unnamed: {i if index_col is None else i + 1***REMOVED***_level_0"
...
"""
    ...
    expected = DataFrame({"A": [1, 5], "B": [2, 6], "C": [3, 4]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_header_none_and_implicit_index(all_parsers):
    ...
    expected = DataFrame(
        {"a": [1, 2, 3], "b": [5, np.nan, np.nan]***REMOVED***, index=["x", "y", "z"]
    )
    tm.assert_frame_equal(result, expected)
...
def test_header_none_and_on_bad_lines_skip(all_parsers):
    ...
    expected = DataFrame({"a": ["x", "z"], "b": [1, 3]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_header_multiple_whitespaces(all_parsers):
    ...
    expected = DataFrame({"aa": [0], "bb(1,1)": 2, "cc(1,1)": 3.5***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
3,4
    ...
    expected = DataFrame({"a,b": ["1,2", "3,4"]***REMOVED***)
    tm.assert_frame_equal(result, expected)
