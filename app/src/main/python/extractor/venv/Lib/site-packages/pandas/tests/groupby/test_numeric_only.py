        ...
        if method in ("min", "max", "cummin", "cummax", "cumsum", "cumprod"):
            ...
            msg = "|".join(
                [
                    ...
                    f"Cannot perform {method***REMOVED*** with non-ordered Categorical",
                    re.escape(f"agg function failed [how->{method***REMOVED***,dtype->object]"),
...
***REMOVED***
            ...
            msg = "|".join(
                [
                    ...
                    re.escape(f"agg function failed [how->{method***REMOVED***,dtype->object]"),
                    re.escape(f"agg function failed [how->{method***REMOVED***,dtype->string]"),
                ]
***REMOVED***
            with pytest.raises(exception, match=msg):
                getattr(gb, method)()
...
def test_axis1_numeric_only(request, groupby_func, numeric_only, using_infer_string):
    ...
    warn_msg = f"DataFrameGroupBy.{groupby_func***REMOVED*** with axis=1 is deprecated"
    ...
    # fillna and shift are successful even on object dtypes
        ...
        if using_infer_string:
            ...
            errs = TypeError
        with pytest.raises(errs, match=f"({'|'.join(msgs)***REMOVED***)"):
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                method(*args, **kwargs)
    else:
        with tm.assert_produces_warning(FutureWarning, match=warn_msg):
...
def test_numeric_only(kernel, has_arg, numeric_only, keys):
    ...
    df = DataFrame({"a1": [1, 1], "a2": [2, 2], "a3": [5, 6], "b": 2 * [object]***REMOVED***)
    ...
    if has_arg and numeric_only is True:
        ...
        msg = "|".join(
            [
                ...
                "unsupported operand type",
                "function is not implemented for this dtype",
                re.escape(f"agg function failed [how->{kernel***REMOVED***,dtype->object]"),
            ]
        )
        if kernel == "idxmin":
