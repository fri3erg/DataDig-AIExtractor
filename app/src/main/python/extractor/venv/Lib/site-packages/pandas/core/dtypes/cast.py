...
def _disallow_mismatched_datetimelike(value, dtype: DtypeObj):
    ...
    if vdtype is None:
        ...
        raise TypeError(f"Cannot cast {repr(value)***REMOVED*** to {dtype***REMOVED***")
...
***REMOVED***
        ...
        size_tols = {4: 5e-4, 8: 5e-8, 16: 5e-16***REMOVED***
...
def dict_compat(d: dict[Scalar, Scalar]) -> dict[Scalar, Scalar]:
    ...
    dict
    """
    return {maybe_box_datetimelike(key): value for key, value in d.items()***REMOVED***
...
def _ensure_nanosecond_dtype(dtype: DtypeObj) -> None:
    ...
    """  # noqa: E501
    msg = (
        f"The '{dtype.name***REMOVED***' dtype has no unit. "
        f"Please pass in '{dtype.name***REMOVED***[ns]' instead."
    )
    ...
    if not isinstance(dtype, np.dtype):
        ...
        if not is_supported_dtype(dtype):
            ...
            raise TypeError(
                f"dtype={dtype***REMOVED*** is not supported. Supported resolutions are 's', "
                "'ms', 'us', and 'ns'"
***REMOVED***
...
***REMOVED***:
                ...
                right = np.dtype(f"i{right_dtype.itemsize***REMOVED***")
            else:
                right = right_dtype
...
def construct_2d_arraylike_from_scalar(
    ...
    try:
        ...
        raise TypeError(
            f"DataFrame constructor called with incompatible data and dtype: {err***REMOVED***"
        ) from err
...
def maybe_cast_to_integer_array(arr: list | np.ndarray, dtype: np.dtype) -> np.ndarray:
    ...
    try:
        ...
        ***REMOVED***
        ...
        raise OverflowError(
            ...
            f"casted to the dtype {dtype***REMOVED***"
        ) from err
    ...
    if np.issubdtype(arr.dtype, str):
        if (casted.astype(str) == arr).all():
            return casted
        raise ValueError(f"string values cannot be losslessly cast to {dtype***REMOVED***")
...
***REMOVED***
    ...
    if casted.dtype < arr.dtype:
        ...
        raise ValueError(
            f"Values are too large to be losslessly converted to {dtype***REMOVED***. "
            f"To cast anyway, use pd.Series(values).astype({dtype***REMOVED***)"
        )

    if arr.dtype.kind in "mM":
        # test_constructor_maskedarray_nonfloat
        raise TypeError(
            f"Constructing a Series or DataFrame from {arr.dtype***REMOVED*** values and "
            f"dtype={dtype***REMOVED*** is not supported. Use values.view({dtype***REMOVED***) instead."
        )

    # No known cases that get here, but raising explicitly to cover our bases.
    raise ValueError(f"values cannot be losslessly cast to {dtype***REMOVED***")
