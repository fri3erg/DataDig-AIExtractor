...
class TestMultiIndexLoc:
    ...
    def test_loc_getitem_general(self, any_real_numpy_dtype):
        ...
        data = {
            "amount": {0: 700, 1: 600, 2: 222, 3: 333, 4: 444***REMOVED***,
            "col": {0: 3.5, 1: 3.5, 2: 4.0, 3: 4.0, 4: 4.0***REMOVED***,
            "num": {0: 12, 1: 11, 2: 12, 3: 12, 4: 12***REMOVED***,
        ***REMOVED***
        df = DataFrame(data)
        df = df.astype({"col": dtype, "num": dtype***REMOVED***)
        ...
        expected = DataFrame({"amount": [222, 333, 444]***REMOVED***, index=index)
    ...
    def test_sorted_multiindex_after_union(self):
        ...
        df = DataFrame({0: ser1, 1: ser2***REMOVED***)
        result = df.loc["2011-01-01":"2011-01-02"]
        tm.assert_frame_equal(result, expected)
...
def test_loc_with_nan():
    # GH: 27104
    df = DataFrame(
        {"col": [1, 2, 5], "ind1": ["a", "d", np.nan], "ind2": [1, 4, 5]***REMOVED***
    ).set_index(["ind1", "ind2"])
    result = df.loc[["a"]]
    expected = DataFrame(
        {"col": [1]***REMOVED***, index=MultiIndex.from_tuples([("a", 1)], names=["ind1", "ind2"])
    )
    tm.assert_frame_equal(result, expected)

    result = df.loc["a"]
    expected = DataFrame({"col": [1]***REMOVED***, index=Index([1], name="ind2"))
    tm.assert_frame_equal(result, expected)
...
def test_loc_getitem_drops_levels_for_one_row_dataframe():
    ...
    df = DataFrame({"d": [0]***REMOVED***, index=mi)
...
def test_loc_get_scalar_casting_to_float():
    # GH#41369
    df = DataFrame(
        {"a": 1.0, "b": 2***REMOVED***, index=MultiIndex.from_arrays([[3], [4]], names=["c", "d"])
    )
    result = df.loc[(3, 4), "b"]
    assert result == 2
