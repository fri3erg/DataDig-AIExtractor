    ...
    def test_parse_gml(self):
        ...
        assert sorted(G.edges(data=True)) == [
            (
                ...
                "Node 2",
                {
                    "color": {"line": "blue", "thickness": 3***REMOVED***,
                    "label": "Edge from node 1 to node 2",
                ***REMOVED***,
***REMOVED***,
            ("Node 2", "Node 3", {"label": "Edge from node 2 to node 3"***REMOVED***),
            ("Node 3", "Node 1", {"label": "Edge from node 3 to node 1"***REMOVED***),
        ]
...
"""
  ...
  node [
    ...
    def test_special_float_label(self):
        ...
        edges = list(G.edges)
        attrs = {edges[i]: value for i, value in enumerate(special_floats)***REMOVED***
...
"""
    ...
    def test_data_types(self):
        data = [
            ...
            "'",
            '"&&amp;&&#34;"',
            [{(b"\xfd",): "\x7f", chr(0x4444): (1, 2)***REMOVED***, (2, "3")],
        ]
        data.append(chr(0x14444))
        data.append(literal_eval("{2.3j, 1 - 2.3j, ()***REMOVED***"))
        G = nx.Graph()
        G.name = data
        G.graph["data"] = data
        G.add_node(0, int=-1, data={"data": data***REMOVED***)
        G.add_edge(0, 0, float=-2.5, data=data)
        gml = "\n".join(nx.generate_gml(G, stringizer=literal_stringizer))
        G = nx.parse_gml(gml, destringizer=literal_destringizer)
        assert data == G.name
        assert {"name": data, "data": data***REMOVED*** == G.graph
        assert list(G.nodes(data=True)) == [(0, {"int": -1, "data": {"data": data***REMOVED******REMOVED***)]
        assert list(G.edges(data=True)) == [(0, 0, {"float": -2.5, "data": data***REMOVED***)]
        G = nx.Graph()
...
class TestPropertyLists:
    ...
    def test_reading_graph_with_list_property(self):
        with byte_file() as f:
            ...
            """
    ***REMOVED***.encode("ascii")
***REMOVED***
            f.seek(0)
            graph = nx.read_gml(f)
        assert graph.nodes(data=True)["n1"] == {"properties": ["element", 0, 1, 2.5]***REMOVED***
...
***REMOVED***
            f.seek(0)
            graph = nx.read_gml(f)
        assert graph.nodes(data=True)["n1"] == {"properties": ["element"]***REMOVED***
...
def test_stringize_empty_list_tuple(coll):
    ...
    H = nx.relabel_nodes(H, {"0": 0, "1": 1***REMOVED***)
