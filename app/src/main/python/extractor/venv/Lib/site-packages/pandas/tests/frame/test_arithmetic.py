...
class DummyElement:
    ...
    def __str__(self) -> str:
        return f"DummyElement({self.value***REMOVED***, {self.dtype***REMOVED***)"
...
class TestFrameComparisons:
    ...
    def test_comparison_with_categorical_dtype(self):
        ...
        df = DataFrame({"A": ["foo", "bar", "baz"]***REMOVED***)
        exp = DataFrame({"A": [True, False, False]***REMOVED***)
    ...
    def test_comparison_invalid(self, arg, arg2):
        ...
        result = x == y
        expected = DataFrame(
            {col: x[col] == y[col] for col in x.columns***REMOVED***,
            index=x.index,
            columns=x.columns,
        )
...
***REMOVED***
    ...
    def test_df_string_comparison(self):
        df = DataFrame([{"a": 1, "b": "foo"***REMOVED***, {"a": 2, "b": "bar"***REMOVED***])
        mask_a = df.a > 1
...
class TestFrameFlexComparisons:
    ...
    def test_bool_flex_frame_complex_dtype(self):
        ...
        df = DataFrame({"a": arr***REMOVED***)
        df2 = DataFrame({"a": arr2***REMOVED***)
    ...
    def test_bool_flex_frame_object_dtype(self):
        # corner, dtype=object
        df1 = DataFrame({"col": ["foo", np.nan, "bar"]***REMOVED***, dtype=object)
        df2 = DataFrame({"col": ["foo", datetime.now(), "bar"]***REMOVED***, dtype=object)
        result = df1.ne(df2)
        exp = DataFrame({"col": [False, True, False]***REMOVED***)
    ...
    def test_df_flex_cmp_constant_return_types(self, opname):
        ...
        df = DataFrame({"x": [1, 2, 3], "y": [1.0, 2.0, 3.0]***REMOVED***)
        const = 2
    ...
    def test_df_flex_cmp_ea_dtype_with_ndarray_series(self):
        ...
        df = DataFrame({"A": ii, "B": ii***REMOVED***)

        ser = Series([0, 0])
        res = df.eq(ser, axis=0)

        expected = DataFrame({"A": [False, False], "B": [False, False]***REMOVED***)
...
class TestFrameFlexArithmetic:
    def test_floordiv_axis0(self):
        ...
        arr = np.arange(3)
        ser = Series(arr)
        df = DataFrame({"A": ser, "B": ser***REMOVED***)

        result = df.floordiv(ser, axis=0)

        expected = DataFrame({col: df[col] // ser for col in df.columns***REMOVED***)
    ...
    def test_df_add_td64_columnwise(self):
        ...
        tser = Series(tdi)
        df = DataFrame({0: dti, 1: tdi***REMOVED***)

        result = df.add(tser, axis=0)
        expected = DataFrame({0: dti + tdi, 1: tdi + tdi***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_df_add_flex_filled_mixed_dtypes(self):
        ...
        df = DataFrame({"A": dti, "B": ser***REMOVED***)
        other = DataFrame({"A": ser, "B": ser***REMOVED***)
    ...
    ***REMOVED***,
    ...
    def test_arith_flex_frame(
        ...
        tm.assert_frame_equal(result, expected)
        _check_mixed_float(result, dtype={"C": None***REMOVED***)
    ...
    def test_arith_flex_frame_mixed(
        ...
        dtype = None
        if op in ["__sub__"]:
            dtype = {"B": "uint64", "C": None***REMOVED***
        elif op in ["__add__", "__mul__"]:
            dtype = {"C": None***REMOVED***
    ...
    def test_flex_add_scalar_fill_value(self):
        ...
        df = DataFrame({"foo": dat***REMOVED***, index=range(6))
    ...
    def test_arithmetic_with_duplicate_columns(self, op):
        # operations
        df = DataFrame({"A": np.arange(10), "B": np.random.default_rng(2).random(10)***REMOVED***)
    ...
    def test_broadcast_multiindex(self, level):
        # GH34388
        df1 = DataFrame({"A": [0, 1, 2], "B": [1, 2, 3]***REMOVED***)
        df1.columns = df1.columns.set_names("L1")

        df2 = DataFrame({("A", "C"): [0, 0, 0], ("A", "D"): [0, 0, 0]***REMOVED***)
        df2.columns = df2.columns.set_names(["L1", "L2"])

        result = df1.add(df2, level=level)
        expected = DataFrame({("A", "C"): [0, 1, 2], ("A", "D"): [0, 1, 2]***REMOVED***)
...
***REMOVED***,
        ...
        expected = DataFrame(
            {2010: [1.4, 2.4, 3.4], 2020: [3.4, 4.4, 5.4]***REMOVED***,
            index=MultiIndex.from_product(
                [["a"], ["b"], [0, 1, 2]], names=["scen", "mod", "id"]
***REMOVED***,
        )
...
***REMOVED***,
        ...
        expected = DataFrame(
            {2010: [11.0, 21, 31.0], 2020: [13.0, 23.0, 33.0]***REMOVED***,
            index=MultiIndex.from_product(
                [["a"], ["b"], [0, 1, 2]], names=["scen", "mod", "id"]
***REMOVED***,
        )
...
***REMOVED***,
        ...
        expected = DataFrame(
            {2010: np.nan, 2020: np.nan***REMOVED***,
...
***REMOVED***,
        ...
        expected = DataFrame(
            {2010: [1.4, 2.4, np.nan], 2020: [3.4, 4.4, np.nan]***REMOVED***,
            index=MultiIndex.from_tuples(
                [
                    ("a", "b", 0),
                    ("a", "b", 1),
...
class TestFrameArithmetic:
    def test_td64_op_nat_casting(self):
        ...
        result = df * ser
        expected = DataFrame({0: ser, 1: ser***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_arith_mixed(self):
        left = DataFrame({"A": ["a", "b", "c"], "B": [1, 2, 3]***REMOVED***)

        result = left + left
        expected = DataFrame({"A": ["aa", "bb", "cc"], "B": [2, 4, 6]***REMOVED***)
        tm.assert_frame_equal(result, expected)

    @pytest.mark.parametrize("col", ["A", "B"])
    def test_arith_getitem_commute(self, all_arithmetic_functions, col):
        df = DataFrame({"A": [1.1, 3.3], "B": [2.5, -3.9]***REMOVED***)
    ...
    def test_arith_alignment_non_pandas_object(self, values):
        # GH#17901
        df = DataFrame({"A": [1, 1], "B": [1, 1]***REMOVED***)
        expected = DataFrame({"A": [2, 2], "B": [3, 3]***REMOVED***)
        result = df + values
        tm.assert_frame_equal(result, expected)
    ...
    def test_frame_with_frame_reindex(self):
        ...
        expected = DataFrame(
            {"foo": [pd.Timedelta(0), pd.Timedelta(0)], "bar": [np.nan, np.nan]***REMOVED***,
            columns=["bar", "foo"],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_binop_other(self, op, value, dtype, switch_numexpr_min_elements):
        ...
        ***REMOVED***
        ...
        df = DataFrame({"A": [elem.value, elem.value]***REMOVED***, dtype=elem.dtype)
...
***REMOVED***:
    ...
    ***REMOVED***:
                ...
                msg = (
                    f"cannot perform __{op.__name__***REMOVED***__ with this "
...
class TestFrameArithmeticUnsorted:
    ...
    def test_operators_none_as_na(self, op):
        df = DataFrame(
            {"col1": [2, 5.0, 123, None], "col2": [1, 2, 3, 4]***REMOVED***, dtype=object
        )
    ...
    def test_add_with_dti_mismatched_tzs(self):
        ...
        df1 = DataFrame({"A": [1, 2]***REMOVED***, index=idx1)
        df2 = DataFrame({"A": [1, 1]***REMOVED***, index=idx2)
        exp = DataFrame({"A": [np.nan, 3, np.nan]***REMOVED***, index=base)
    ...
    def test_combineFrame(self, float_frame, mixed_float_frame, mixed_int_frame):
        ...
        _check_mixed_float(added, dtype={"C": None***REMOVED***)
    ...
    def test_combine_series(self, float_frame, mixed_float_frame, mixed_int_frame):
        ...
        _check_mixed_int(
            added, dtype={"A": "int64", "B": "float64", "C": "int64", "D": "int64"***REMOVED***
        )
        added = mixed_int_frame + (100 * series).astype("int32")
        _check_mixed_int(
            added, dtype={"A": "int32", "B": "float64", "C": "int32", "D": "int64"***REMOVED***
        )
    ...
    def test_strings_to_numbers_comparisons_raises(self, compare_operators_no_eq_ne):
        # GH 11565
        df = DataFrame(
            {x: {"x": "foo", "y": "bar", "z": "baz"***REMOVED*** for x in ["a", "b", "c"]***REMOVED***
        )
    ...
    def test_inplace_ops_identity(self):
        ...
        df_orig = DataFrame({"A": arr.copy(), "B": "foo"***REMOVED***)
        df = df_orig.copy()
        df2 = df
        df["A"] += 1
        expected = DataFrame({"A": arr.copy() + 1, "B": "foo"***REMOVED***)
        ...
        df2 = df
        df["A"] += 1.5
        expected = DataFrame({"A": arr.copy() + 1.5, "B": "foo"***REMOVED***)
        tm.assert_frame_equal(df, expected)
        tm.assert_frame_equal(df2, expected)
        assert df._mgr is df2._mgr
...
***REMOVED***,
    ...
    def test_inplace_ops_identity2(self, op):
        df = DataFrame({"a": [1.0, 2.0, 3.0], "b": [1, 2, 3]***REMOVED***)
        ...
        df_copy = df.copy()
        iop = f"__i{op***REMOVED***__"
        op = f"__{op***REMOVED***__"
    ...
    def test_alignment_non_pandas(self, val):
        ...
        expected = DataFrame({"X": val, "Y": val, "Z": val***REMOVED***, index=df.index)
        tm.assert_frame_equal(align(df, val, axis=0)[1], expected)

        expected = DataFrame(
            {"X": [1, 1, 1], "Y": [2, 2, 2], "Z": [3, 3, 3]***REMOVED***, index=df.index
        )
    ...
    def test_no_warning(self, all_arithmetic_operators):
        df = DataFrame({"A": [0.0, 0.0], "B": [0.0, None]***REMOVED***)
        b = df["B"]
        with tm.assert_produces_warning(None):
...
def test_pow_with_realignment():
    ...
    left = DataFrame({"A": [0, 1, 2]***REMOVED***)
    right = DataFrame(index=[0, 1, 2])

    result = left**right
    expected = DataFrame({"A": [np.nan, 1.0, np.nan]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_dataframe_blockwise_slicelike():
    ...
    df2 = df1.copy().astype({1: "float", 3: "float", 7: "float"***REMOVED***)
    df2.iloc[0, [1, 3, 7]] = np.nan

    # Explicit cast to float to avoid implicit cast when setting nan
    df3 = df1.copy().astype({5: "float"***REMOVED***)
    df3.iloc[0, [5]] = np.nan

    # Explicit cast to float to avoid implicit cast when setting nan
    df4 = df1.copy().astype({2: "float", 3: "float", 4: "float"***REMOVED***)
    df4.iloc[0, np.arange(2, 5)] = np.nan
    # Explicit cast to float to avoid implicit cast when setting nan
    df5 = df1.copy().astype({4: "float", 5: "float", 6: "float"***REMOVED***)
    df5.iloc[0, np.arange(4, 7)] = np.nan

    for left, right in [(df1, df2), (df2, df3), (df4, df5)]:
        res = left + right

        expected = DataFrame({i: left[i] + right[i] for i in left.columns***REMOVED***)
        tm.assert_frame_equal(res, expected)
...
def test_dataframe_operation_with_non_numeric_types(df, col_dtype):
    ...
    expected = expected.astype({"b": col_dtype***REMOVED***)
...
def test_arith_list_of_arraylike_raise(to_add):
    ...
    df = DataFrame({"x": [1, 2], "y": [1, 2]***REMOVED***)

    msg = f"Unable to coerce list of {type(to_add[0])***REMOVED*** to Series/DataFrame"
    with pytest.raises(ValueError, match=msg):
        df + to_add
    with pytest.raises(ValueError, match=msg):
        to_add + df
...
def test_inplace_arithmetic_series_update(using_copy_on_write, warn_copy_on_write):
    ...
    if using_copy_on_write:
        ...
        expected = DataFrame({"A": [2, 3, 4]***REMOVED***)
        tm.assert_frame_equal(df, expected)
...
def test_frame_op_subclass_nonclass_constructor():
    ...
    sdf = SubclassedDataFrame("some_data", {"A": [1, 2, 3], "B": [4, 5, 6]***REMOVED***)
    result = sdf * 2
    expected = SubclassedDataFrame("some_data", {"A": [2, 4, 6], "B": [8, 10, 12]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_enum_column_equality():
    ...
    q1 = DataFrame({Cols.col1: [1, 2, 3]***REMOVED***)
    q2 = DataFrame({Cols.col1: [1, 2, 3]***REMOVED***)
