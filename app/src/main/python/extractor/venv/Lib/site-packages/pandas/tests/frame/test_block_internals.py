...
class TestDataFrameBlockInternals:
    ...
    def test_constructor_with_convert(self):
        ...
        # #2845
        df = DataFrame({"A": [2**63 - 1]***REMOVED***)
        result = df["A"]
        expected = Series(np.asarray([2**63 - 1], np.int64), name="A")
        tm.assert_series_equal(result, expected)

        df = DataFrame({"A": [2**63]***REMOVED***)
        result = df["A"]
        expected = Series(np.asarray([2**63], np.uint64), name="A")
        tm.assert_series_equal(result, expected)

        df = DataFrame({"A": [datetime(2005, 1, 1), True]***REMOVED***)
        result = df["A"]
        expected = Series(
    ...
    def test_construction_with_mixed(self, float_string_frame, using_infer_string):
        ...
        # check dtypes
        result = df.dtypes
        expected = Series({"datetime64[us]": 3***REMOVED***)
    ...
    def test_construction_with_conversions(self):
        ...
        df["A"] = arr
        expected = DataFrame(
            {"A": pd.timedelta_range("00:00:01", periods=3, freq="s")***REMOVED***, index=range(3)
        )
...
***REMOVED***
    ...
    def test_constructor_no_pandas_array(self):
        ...
        result = DataFrame({"A": arr***REMOVED***)
    ...
    def test_add_column_with_pandas_array(self):
        # GH 26390
        df = DataFrame({"a": [1, 2, 3, 4], "b": ["a", "b", "c", "d"]***REMOVED***)
...
def test_update_inplace_sets_valid_block_values(using_copy_on_write):
    ...
    df = DataFrame({"a": Series([1, 2, None], dtype="category")***REMOVED***)
...
def test_nonconsolidated_item_cache_take():
    ...
    expected = DataFrame({"col1": ["A"], "col2": [0]***REMOVED***, dtype=object)
    tm.assert_frame_equal(df, expected)
