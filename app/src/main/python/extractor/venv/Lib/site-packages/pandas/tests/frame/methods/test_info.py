...
def test_info_categorical_column_smoke_test():
    n = 2500
    df = DataFrame({"int64": np.random.default_rng(2).integers(100, size=n, dtype=int)***REMOVED***)
    df["category"] = Series(
...
def test_info_memory():
    ...
    df = DataFrame({"a": Series([1, 2], dtype="i8")***REMOVED***)
    buf = StringIO()
    df.info(buf=buf)
    result = buf.getvalue()
    ...
    dtypes: int64(1)
    memory usage: {bytes***REMOVED*** bytes
    """
    )
    assert result == expected
...
def test_info_shows_column_dtypes():
    ...
    assert header in res
    for i, dtype in enumerate(dtypes):
        name = f" {i:d***REMOVED***   {i:d***REMOVED***       {n:d***REMOVED*** non-null     {dtype***REMOVED***"
        assert name in res
...
def test_info_memory_usage():
    ...
    df_with_object_index = DataFrame({"a": [1]***REMOVED***, index=["foo"])
...
def test_info_memory_usage_deep_not_pypy():
    ...
    df_object = DataFrame({"a": ["a"]***REMOVED***)
...
def test_info_memory_usage_bug_on_multiindex():
    ...
    df = DataFrame(
        {"value": np.random.default_rng(2).standard_normal(N * M)***REMOVED***, index=index
    )
...
def test_info_int_columns():
    # GH#37245
    df = DataFrame({1: [1, 2], 2: [2, 3]***REMOVED***, index=["A", "B"])
    buf = StringIO()
...
def test_info_show_counts(row, columns, show_counts, result):
    ...
    df = DataFrame(1, columns=range(10), index=range(10)).astype({1: "float"***REMOVED***)
    df.iloc[1, 1] = np.nan
