...
***REMOVED***
        ...
        if async_client is not None and not isinstance(
            ...
            raise ValueError(
                ...
                f"got {type(async_client)***REMOVED***"
***REMOVED***
        ...
        Args:
            ...
            prefix:
                ...
                Example: service/v1 will result in
                    http://localhost:6333/service/v1/{qdrant-endpoint***REMOVED*** for REST API.
                Default: None
            timeout:
...
***REMOVED***
            ...
            if isinstance(current_vector_config, dict) and vector_name is not None:
                if vector_name not in current_vector_config:
                    raise QdrantException(
                        f"Existing Qdrant collection {collection_name***REMOVED*** does not "
                        f"contain vector named {vector_name***REMOVED***. Did you mean one of the "
                        f"existing vectors: {', '.join(current_vector_config.keys())***REMOVED***? "
        ...
        ***REMOVED***
                ...
                raise QdrantException(
                    f"Existing Qdrant collection {collection_name***REMOVED*** uses named vectors. "
                    f"If you want to reuse it, please set `vector_name` to any of the "
                    f"existing named vectors: "
                    f"{', '.join(current_vector_config.keys())***REMOVED***."  # noqa
                    f"If you want to recreate the collection, set `force_recreate` "
                    f"parameter to `True`."
    ***REMOVED***
            elif (
...
***REMOVED***:
                raise QdrantException(
                    f"Existing Qdrant collection {collection_name***REMOVED*** doesn't use named "
    ...
    ***REMOVED***
            ...
            if current_vector_config.size != vector_size:  # type: ignore[union-attr]
                raise QdrantException(
                    ...
                    f"{current_vector_config.size***REMOVED*** "  # type: ignore[union-attr]
                    f"dimensions. Selected embeddings are {vector_size***REMOVED***-dimensional. "
                    f"If you want to recreate the collection, set `force_recreate` "
                    f"parameter to `True`."
    ***REMOVED***
...
***REMOVED***
            if current_distance_func != distance_func:
                raise QdrantException(
                    ...
                    f"{current_distance_func***REMOVED*** similarity, but requested "
                    f"{distance_func***REMOVED***. Please set `distance_func` parameter to "
                    f"`{current_distance_func***REMOVED***` if you want to reuse it. "
                    f"If you want to recreate the collection, set `force_recreate` "
                    f"parameter to `True`."
    ***REMOVED***
...
***REMOVED***
            if current_distance_func != distance_func:
                raise QdrantException(
                    f"Existing Qdrant collection is configured for "
                    f"{current_vector_config.distance***REMOVED*** "  # type: ignore[union-attr]
                    f"similarity. Please set `distance_func` parameter to "
                    f"`{distance_func***REMOVED***` if you want to reuse it. If you want to "
...
***REMOVED***
    ...
    def _document_from_scored_point(
        ...
        return Document(
            ...
            metadata=scored_point.payload.get(metadata_payload_key) or {***REMOVED***,
        )
    ...
    def _build_condition(self, key: str, value: Any) -> List[rest.FieldCondition]:
        ...
        if isinstance(value, dict):
            for _key, value in value.items():
                out.extend(self._build_condition(f"{key***REMOVED***.{_key***REMOVED***", value))
        elif isinstance(value, list):
            for _value in value:
                if isinstance(_value, dict):
                    out.extend(self._build_condition(f"{key***REMOVED***[]", _value))
                else:
                    out.extend(self._build_condition(f"{key***REMOVED***", _value))
        else:
            out.append(
                rest.FieldCondition(
                    key=f"{self.metadata_payload_key***REMOVED***.{key***REMOVED***",
        ...
        while batch_texts := list(islice(texts_iterator, batch_size)):
            ...
            points = [
                rest.PointStruct(
                    ...
                    else {self.vector_name: vector***REMOVED***,
                    payload=payload,
    ***REMOVED***
