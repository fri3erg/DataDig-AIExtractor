...
***REMOVED*** -> T:
        ...
        if not old_doc:
            ...
            new_doc = f"[*Deprecated*]  {old_doc***REMOVED***"

        # Modify the docstring to include a deprecation notice.
        notes_header = "\nNotes\n-----"
        components = [
            message,
            f"Use {alternative***REMOVED*** instead." if alternative else "",
            addendum,
        ]
        details = " ".join([component.strip() for component in components if component])
        new_doc += (
            f"[*Deprecated*] {old_doc***REMOVED***\n"
            f"{notes_header if notes_header not in old_doc else ''***REMOVED***\n"
            f".. deprecated:: {since***REMOVED***\n"
            f"   {details***REMOVED***"
        )
...
def warn_deprecated(
    ...
    *,
    ...
    if not pending:
        if not removal:
            removal = f"in {removal***REMOVED***" if removal else "within ?? minor releases"
            raise NotImplementedError(
                f"Need to determine which default deprecation schedule to use. "
                f"{removal***REMOVED***"
***REMOVED***
        else:
            removal = f"in {removal***REMOVED***"

    if not message:
        message = ""

        if obj_type:
            message += f"The {obj_type***REMOVED*** `{name***REMOVED***`"
        else:
        ...
        if pending:
            ...
            message += f" was deprecated in LangChain {since***REMOVED***"

            if removal:
                message += f" and will be removed {removal***REMOVED***"

        if alternative:
            message += f". Use {alternative***REMOVED*** instead."

        if addendum:
            message += f" {addendum***REMOVED***"
