...
def test_map_keeps_dtype(na_action):
    ...
    df = DataFrame(data={"a": arr, "b": sparse_arr***REMOVED***)
    ...
    expected = DataFrame({"a": expected_arr, "b": expected_sparse***REMOVED***)
...
def test_frame_map_dont_convert_datetime64():
    df = DataFrame({"x1": [datetime(1996, 1, 1)]***REMOVED***)
...
def test_map_type():
    ...
    result = df.map(type)
    expected = DataFrame(
        {"col1": [int, str, type], "col2": [float, datetime, float]***REMOVED***,
        index=["a", "b", "c"],
    )
    tm.assert_frame_equal(result, expected)
