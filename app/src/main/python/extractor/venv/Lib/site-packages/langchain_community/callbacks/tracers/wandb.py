        ...
        try:
            ...
            if PRINT_WARNINGS:
                self.wandb.termwarn(
                    ...
                    f"into W&B Trace due to: {e***REMOVED***"
    ***REMOVED***
            return None
    ...
    def _convert_run_to_wb_span(self, run: Run) -> "Span":
        ...
        """
        attributes = {**run.extra***REMOVED*** if run.extra else {***REMOVED***
    ...
    def _convert_llm_run_to_wb_span(self, run: Run) -> "Span":
        ...
        if base_span.attributes is None:
            base_span.attributes = {***REMOVED***
        base_span.attributes["llm_output"] = (run.outputs or {***REMOVED***).get("llm_output", {***REMOVED***)

        base_span.results = [
            self.trace_tree.Result(
                inputs={"prompt": prompt***REMOVED***,
                outputs={
                    f"gen_{g_i***REMOVED***": gen["text"]
...
***REMOVED***
            ...
            if PRINT_WARNINGS:
                self.wandb.termwarn(f"WARNING: Failed to serialize model: {e***REMOVED***")
            return None
    ...
    def modify_serialized_iterative(
        ...
        def transform_run(run: Dict[str, Any]) -> Dict[str, Any]:
            ...
            output_dict = {
                f"{exec_ord***REMOVED***_{name***REMOVED***": transformed_dict,
            ***REMOVED***
            return output_dict
...
***REMOVED*** from e
    ...
    def _log_trace_from_run(self, run: Run) -> None:
        ...
        if self._wandb.run is not None:
            self._wandb.run.log({"langchain_trace": model_trace***REMOVED***)
    ...
    def _ensure_run(self, should_print_url: bool = False) -> None:
        ...
        if self._wandb.run is None:
            run_args = self._run_args or {***REMOVED***  # type: ignore
            run_args: dict = {**run_args***REMOVED***  # type: ignore

            if "settings" not in run_args:  # type: ignore
                run_args["settings"] = {"silent": True***REMOVED***  # type: ignore
        ...
        if self._wandb.run is not None:
            if should_print_url:
                ...
                self._wandb.termlog(
                    f"Streaming LangChain activity to W&B at {run_url***REMOVED***\n"
