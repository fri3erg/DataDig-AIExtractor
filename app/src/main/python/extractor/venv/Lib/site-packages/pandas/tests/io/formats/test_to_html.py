...
def test_to_html_with_column_specific_col_space_raises():
    ...
    msg = "unknown column"
    with pytest.raises(ValueError, match=msg):
        df.to_html(col_space={"a": "foo", "b": 23, "d": 34***REMOVED***)
...
def test_to_html_with_column_specific_col_space():
    ...
    result = df.to_html(col_space={"a": "2em", "b": 23***REMOVED***)
...
def test_to_html_with_empty_string_label():
    ...
    data = {"c1": ["a", "b"], "c2": ["a", ""], "data": [1, 2]***REMOVED***
...
@pytest.mark.parametrize(
    "df,expected",
    [
        (DataFrame({"\u03c3": np.arange(10.0)***REMOVED***), "unicode_1"),
        (DataFrame({"A": ["\u03c3"]***REMOVED***), "unicode_2"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ({***REMOVED***, "<type 'str'>", "escaped"),
        ({"escape": False***REMOVED***, "<b>bold</b>", "escape_disabled"),
    ],
)
def test_to_html_escaped(kwargs, string, expected, datapath):
    ...
    test_dict = {"co<l1": {a: string, b: string***REMOVED***, "co>l2": {a: string, b: string***REMOVED******REMOVED***
...
def test_to_html_multiindex_index_false(index_is_named, datapath):
    # GH 8452
    df = DataFrame(
        {"a": range(2), "b": range(3, 5), "c": range(5, 7), "d": range(3, 5)***REMOVED***
    )
...
def test_to_html_multiindex_odd_even_truncate(max_rows, expected, datapath):
    ...
    df = DataFrame({"n": range(len(index))***REMOVED***, index=index)
    ...
    [
        (
            DataFrame(
                ...
                index=np.arange(4),
***REMOVED***,
            {"__index__": lambda x: "abcd"[x]***REMOVED***,
            "index_formatter",
        ),
        (
            DataFrame({"months": [datetime(2016, 1, 1), datetime(2016, 2, 2)]***REMOVED***),
            {"months": lambda x: x.strftime("%Y-%m")***REMOVED***,
        ...
        ***REMOVED***
                ***REMOVED***
***REMOVED***,
            {"hod": lambda x: x.strftime("%H:%M")***REMOVED***,
...
def test_html_invalid_formatters_arg_raises(size):
    ...
    msg = "Formatters length({***REMOVED***) should match DataFrame number of columns(3)"
    with pytest.raises(ValueError, match=re.escape(msg.format(size))):
        df.to_html(formatters=["{***REMOVED***".format] * size)
...
def test_to_html_truncate_formatter(datapath):
    ...
    data = [
        ...
        {"A": 5, "B": 6, "C": 7, "D": 8***REMOVED***,
        {"A": 9, "B": 10, "C": 11, "D": 12***REMOVED***,
        {"A": 13, "B": 14, "C": 15, "D": 16***REMOVED***,
    ]
...
def test_to_html_border(option, result, expected):
    ...
    if option is None:
        ...
        with option_context("display.html.border", option):
            result = result(df)
    expected = f'border="{expected***REMOVED***"'
...
def test_to_html(biggie_df_fixture):
    ...
    df.to_html(columns=["B", "A"], formatters={"A": lambda x: f"{x:.1f***REMOVED***"***REMOVED***)
...
def test_to_html_with_no_bold():
    df = DataFrame({"x": np.random.default_rng(2).standard_normal(5)***REMOVED***)
...
def test_to_html_justify(justify, datapath):
    df = DataFrame(
        {"A": [6, 30000, 2], "B": [1, 2, 70000], "C": [223442, 0, 1]***REMOVED***,
        columns=["A", "B", "C"],
    )
    result = df.to_html(justify=justify)
...
class TestHTMLIndex:
    ...
    def df(self):
        index = ["foo", "bar", "baz"]
        df = DataFrame(
            {"A": [1, 2, 3], "B": [1.2, 3.4, 5.6], "C": ["one", "two", np.nan]***REMOVED***,
            columns=["A", "B", "C"],
            index=index,
        )
        return df
...
def test_to_html_no_index_max_rows(datapath):
    # GH 14998
    df = DataFrame({"A": [1, 2, 3, 4]***REMOVED***)
...
def test_to_html_multi_indexes_index_false(datapath):
    # GH 22579
    df = DataFrame(
        {"a": range(10), "b": range(10, 20), "c": range(10, 20), "d": range(10, 20)***REMOVED***
    )
    df.columns = MultiIndex.from_product([["a", "b"], ["c", "d"]])
...
def test_to_html_with_index_names_false():
    # GH 16493
    df = DataFrame({"A": [1, 2]***REMOVED***, index=Index(["a", "b"], name="myindexname"))
...
def test_to_html_with_col_space_units(unit):
    ...
    if isinstance(unit, int):
        unit = str(unit) + "px"
    for h in hdrs:
        expected = f'<th style="min-width: {unit***REMOVED***;">'
...
class TestReprHTML:
    ...
    def test_info_repr_html(self):
        ...
        df = DataFrame({k: np.arange(1, 1 + h) for k in np.arange(w)***REMOVED***)
    ...
    def test_fake_qtconsole_repr_html(self, float_frame):
        ...
        def get_ipython():
            return {"config": {"KernelApp": {"parent_appname": "ipython-qtconsole"***REMOVED******REMOVED******REMOVED***
    ...
    def test_repr_html_long(self):
        with option_context("display.max_rows", 60):
            ...
            h = max_rows - 1
            df = DataFrame({"A": np.arange(1, 1 + h), "B": np.arange(41, 41 + h)***REMOVED***)
            reg_repr = df._repr_html_()
            assert ".." not in reg_repr
            ...
            assert f"{h***REMOVED*** rows " in long_repr
            ...
            df = DataFrame(
                {
                    ...
                    "B": np.arange(41, 41 + h),
                ***REMOVED***
***REMOVED***.set_index("idx")
            reg_repr = df._repr_html_()
            assert ".." not in reg_repr
            assert f"<td>{40 + h***REMOVED***</td>" in reg_repr
...
def test_to_html_na_rep_and_float_format(na_rep, datapath):
    ...
    result = df.to_html(na_rep=na_rep, float_format="{:.2f***REMOVED***".format)
...
def test_to_html_na_rep_non_scalar_data(datapath):
    # GH47103
    df = DataFrame([{"a": 1, "b": [1, 2, 3]***REMOVED***])
...
def test_to_html_float_format_object_col(datapath):
    # GH#40024
    df = DataFrame(data={"x": [1000.0, "test"]***REMOVED***)
    result = df.to_html(float_format=lambda x: f"{x:,.0f***REMOVED***")
...
def test_to_html_tuple_col_with_colspace():
    # GH#53885
    df = DataFrame({("a", "b"): [1], "b": [2]***REMOVED***)
    result = df.to_html(col_space=100)
    expected = (
        '<table border="1" class="dataframe">\n'
        "  <thead>\n"
...
def test_to_html_empty_complex_array():
    # GH#54167
    df = DataFrame({"x": np.array([], dtype="complex")***REMOVED***)
    result = df.to_html(col_space=100)
    expected = (
        '<table border="1" class="dataframe">\n'
        "  <thead>\n"
