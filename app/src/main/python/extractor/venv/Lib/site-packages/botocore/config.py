...
class Config:
    ...
    :param proxies: A dictionary of proxy servers to use by protocol or
        endpoint, e.g.:
        ``{'http': 'foo.bar:3128', 'http://hostname': 'foo.bar:4012'***REMOVED***``.
    ...
    def _record_user_provided_options(self, args, kwargs):
        ...
        for key, value in kwargs.items():
            ...
            # The key must exist in the available options
                ...
                raise TypeError(f"Got unexpected keyword argument '{key***REMOVED***'")

        # The number of args should not be longer than the allowed
        # options
        if len(args) > len(option_order):
            raise TypeError(
                f"Takes at most {len(option_order)***REMOVED*** arguments ({len(args)***REMOVED*** given)"
***REMOVED***
        ...
        for i, arg in enumerate(args):
            ...
            if option_order[i] in user_provided_options:
                raise TypeError(
                    f"Got multiple values for keyword argument '{option_order[i]***REMOVED***'"
    ***REMOVED***
