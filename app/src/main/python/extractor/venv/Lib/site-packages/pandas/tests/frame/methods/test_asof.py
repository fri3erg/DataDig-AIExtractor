...
def date_range_frame():
    ...
    return DataFrame({"A": np.arange(N), "B": np.arange(N)***REMOVED***, index=rng)


class TestFrameAsof:
    def test_basic(self, date_range_frame):
        # Explicitly cast to float to avoid implicit cast when setting np.nan
        df = date_range_frame.astype({"A": "float"***REMOVED***)
        N = 50
    ...
    def test_subset(self, date_range_frame):
        ...
        df = date_range_frame.iloc[:N].copy().astype({"A": "float"***REMOVED***)
