...
class TestSeriesConstructors:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (lambda idx: Series({***REMOVED***, index=idx)),
            (lambda idx: Series((), index=idx)),
            (lambda idx: Series([], index=idx)),
            (lambda idx: Series((_ for _ in []), index=idx)),
            (lambda idx: Series(data=None, index=idx)),
            (lambda idx: Series(data={***REMOVED***, index=idx)),
    ...
    def test_constructor_categorical_with_coercion(self):
        ...
        df = DataFrame({"A": s***REMOVED***)
        result = df["A"]
        tm.assert_series_equal(result, s)
        assert len(df) == len(factor)

        # multiples
        df = DataFrame({"A": s, "B": s, "C": 1***REMOVED***)
        result1 = df["A"]
        result2 = df["B"]
    ...
    def test_constructor_unsigned_dtype_overflow(self, any_unsigned_int_numpy_dtype):
        ...
        if np_version_gt2:
            msg = (
                ...
                f"all be casted to the dtype {any_unsigned_int_numpy_dtype***REMOVED***"
***REMOVED***
        else:
    ...
    def test_constructor_datelike_coercion2(self):
        ...
        df = DataFrame({"wing1": wing1, "wing2": wing2, "mat": mat***REMOVED***, index=belly)
    ...
    def test_constructor_dtype_datetime64_7(self):
        ...
        for dtype in ["s", "D", "ms", "us", "ns"]:
            values1 = dates.view(np.ndarray).astype(f"M8[{dtype***REMOVED***]")
    ...
    def test_construction_to_datetimelike_unit(self, arr_dtype, kind, unit):
        ...
        dtype = f"{kind***REMOVED***8[{unit***REMOVED***]"
        ...
        if unit in ["ns", "us", "ms", "s"]:
            ...
            assert result.dtype == f"{kind***REMOVED***8[s]"
            assert expected.dtype == f"{kind***REMOVED***8[s]"
...
***REMOVED***
    ...
    def test_constructor_dict_nan_key(self, value):
        # GH 18480
        d = {1: "a", value: "b", float("nan"): "c", 4: "d"***REMOVED***
        result = Series(d).sort_values()
        expected = Series(["a", "b", "c", "d"], index=[1, value, np.nan, 4])
        tm.assert_series_equal(result, expected)

        # MultiIndex:
        d = {(1, 1): "a", (2, np.nan): "b", (3, value): "c"***REMOVED***
        result = Series(d).sort_values()
        expected = Series(
    ...
    def test_constructor_mapping(self, non_dict_mapping_subclass):
        # GH 29788
        ndm = non_dict_mapping_subclass({3: "three"***REMOVED***)
        result = Series(ndm)
    ...
    def test_fromDict(self, using_infer_string):
        data = {"a": 0, "b": 1, "c": 2, "d": 3***REMOVED***

        series = Series(data)
        tm.assert_is_sorted(series.index)

        data = {"a": 0, "b": "1", "c": "2", "d": datetime.now()***REMOVED***
        series = Series(data)
    ...
    def test_constructor_name_hashable(self):
        for n in [777, 777.0, "name", datetime(2001, 11, 11), (1,), "\u05D0"]:
            for data in [[1, 2, 3], np.ones(3), {"a": 0, "b": 1***REMOVED***]:
                s = Series(data, name=n)
                assert s.name == n

    def test_constructor_name_unhashable(self):
        msg = r"Series\.name must be a hashable type"
        for n in [["name_list"], np.ones(2), {1: 2***REMOVED***]:
            for data in [["name_list"], np.ones(2), {1: 2***REMOVED***]:
                with pytest.raises(TypeError, match=msg):
                    Series(data, name=n)
    ...
    def test_constructor_cant_cast_datetimelike(self, index):
        ...
        msg = f"Cannot cast {type(index).__name__.rstrip('Index')***REMOVED***.*? to "
    ...
    def test_constructor_data_aware_dtype_naive(self, tz_aware_fixture, pydt):
        ...
        with pytest.raises(ValueError, match=msg):
            Series({0: ts***REMOVED***, dtype="datetime64[ns]")
    ...
    def test_construction_from_ordered_collection(self):
        ...
        result = Series({"a": 1, "b": 2***REMOVED***.keys())
        expected = Series(["a", "b"])
        tm.assert_series_equal(result, expected)

        result = Series({"a": 1, "b": 2***REMOVED***.values())
        expected = Series([1, 2])
        tm.assert_series_equal(result, expected)
    ...
    def test_constructor_ordereddict(self):
        # GH3283
        data = OrderedDict(
            (f"col{i***REMOVED***", np.random.default_rng(2).random()) for i in range(12)
        )
    ...
    def test_constructor_dict_multiindex(self):
        d = {("a", "a"): 0.0, ("b", "a"): 1.0, ("b", "c"): 2.0***REMOVED***
        _d = sorted(d.items())
        result = Series(d)
        expected = Series(
    ...
    def test_constructor_dict_multiindex_reindex_flat(self):
        ...
        data = {("i", "i"): 0, ("i", "j"): 1, ("j", "i"): 2, "j": np.nan***REMOVED***
...
@pytest.mark.parametrize(
    "input_dict,expected",
    [
        ({0: 0***REMOVED***, np.array([[0]], dtype=np.int64)),
        ({"a": "a"***REMOVED***, np.array([["a"]], dtype=object)),
        ({1: 1***REMOVED***, np.array([[1]], dtype=np.int64)),
    ],
)
