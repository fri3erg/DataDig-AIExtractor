...
@pytest.fixture(
    params=[None, {"book": ["category", "title", "author", "year", "price"]***REMOVED***]
)
...
def test_dtype_single_str(parser):
    df_result = read_xml(StringIO(xml_types), dtype={"degrees": "str"***REMOVED***, parser=parser)
    df_iter = read_xml_iterparse(
        xml_types,
        parser=parser,
        dtype={"degrees": "str"***REMOVED***,
...
def test_dtypes_with_names(parser):
    df_result = read_xml(
        ...
        dtype={"Col2": "string", "Col3": "Int64", "Col4": "datetime64[ns]"***REMOVED***,
        parser=parser,
    )
    df_iter = read_xml_iterparse(
        xml_dates,
...
def test_dtype_nullable_int(parser):
    df_result = read_xml(StringIO(xml_types), dtype={"sides": "Int64"***REMOVED***, parser=parser)
    df_iter = read_xml_iterparse(
        xml_types,
        parser=parser,
        dtype={"sides": "Int64"***REMOVED***,
        iterparse={"row": ["shape", "degrees", "sides"]***REMOVED***,
    )
...
def test_dtype_float(parser):
    df_result = read_xml(StringIO(xml_types), dtype={"degrees": "float"***REMOVED***, parser=parser)
    df_iter = read_xml_iterparse(
        xml_types,
        parser=parser,
        dtype={"degrees": "float"***REMOVED***,
        iterparse={"row": ["shape", "degrees", "sides"]***REMOVED***,
    )
...
def test_wrong_dtype(xml_books, parser, iterparse):
    with pytest.raises(
        ...
        read_xml(
            xml_books, dtype={"title": "Int64"***REMOVED***, parser=parser, iterparse=iterparse
        )
...
def test_both_dtype_converters(parser):
    ...
    with tm.assert_produces_warning(ParserWarning, match="Both a converter and dtype"):
        df_result = read_xml(
            StringIO(xml_types),
            dtype={"degrees": "str"***REMOVED***,
            converters={"degrees": str***REMOVED***,
            parser=parser,
        )
        df_iter = read_xml_iterparse(
            xml_types,
...
def test_converters_str(parser):
    df_result = read_xml(
        StringIO(xml_types), converters={"degrees": str***REMOVED***, parser=parser
    )
    df_iter = read_xml_iterparse(
        xml_types,
        parser=parser,
...
def test_converters_date(parser):
    ...
    df_result = read_xml(
        StringIO(xml_dates), converters={"date": convert_to_datetime***REMOVED***, parser=parser
    )
    df_iter = read_xml_iterparse(
        xml_dates,
        parser=parser,
        converters={"date": convert_to_datetime***REMOVED***,
        iterparse={"row": ["shape", "degrees", "sides", "date"]***REMOVED***,
    )
...
def test_wrong_converters_type(xml_books, parser, iterparse):
    with pytest.raises(TypeError, match=("Type converters must be a dict or subclass")):
        read_xml(
            xml_books, converters={"year", str***REMOVED***, parser=parser, iterparse=iterparse
        )
...
def test_callable_func_converters(xml_books, parser, iterparse):
    with pytest.raises(TypeError, match=("'float' object is not callable")):
        read_xml(
            xml_books, converters={"year": float()***REMOVED***, parser=parser, iterparse=iterparse
        )
...
<data>
  ...
  <row>
    ...
    df_result = read_xml(
        StringIO(xml), parse_dates={"date_end": ["year", "month", "day"]***REMOVED***, parser=parser
    )
    df_iter = read_xml_iterparse(
        xml,
        parser=parser,
        parse_dates={"date_end": ["year", "month", "day"]***REMOVED***,
        iterparse={"row": ["shape", "degrees", "sides", "year", "month", "day"]***REMOVED***,
    )
...
def test_wrong_parse_dates_type(xml_books, parser, iterparse):
    with pytest.raises(
        ...
        read_xml(xml_books, parse_dates={"date"***REMOVED***, parser=parser, iterparse=iterparse)
