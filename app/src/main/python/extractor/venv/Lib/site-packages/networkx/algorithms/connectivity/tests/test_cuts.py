...
def test_brandes_erlebach_book():
    ...
    for flow_func in flow_funcs:
        kwargs = {"flow_func": flow_func***REMOVED***
        errmsg = f"Assertion failed in function: {flow_func.__name__***REMOVED***"
        # edge cutsets
        ...
        # node cuts
        assert {6, 7***REMOVED*** == minimum_st_node_cut(G, 1, 11, **kwargs), errmsg
        assert {6, 7***REMOVED*** == nx.minimum_node_cut(G, 1, 11, **kwargs), errmsg
...
def test_white_harary_paper():
    ...
    for flow_func in flow_funcs:
        ...
        # node cuts
        node_cut = nx.minimum_node_cut(G, **kwargs)
        assert {0***REMOVED*** == node_cut, errmsg
        H = G.copy()
        H.remove_nodes_from(node_cut)
        assert not nx.is_connected(H), errmsg
