...
def avoid_duplicate_name(names, value):
    ...
    if match:
        names = u",".join(names)
        sheet_title_regex = re.compile(f'(?P<title>{re.escape(value)***REMOVED***)(?P<count>\\d*),?', re.I)
        ...
        if matches:
            ...
            if counts:
                highest = max(counts)
            value = u"{0***REMOVED***{1***REMOVED***".format(value, highest + 1)
    return value
...
class _WorkbookChild(object):
    ...
    def __repr__(self):
        return '<{0***REMOVED*** "{1***REMOVED***">'.format(self.__class__.__name__, self.title)
    ...
    def title(self, value):
        ...
        if m:
            msg = "Invalid character {0***REMOVED*** found in sheet title".format(m.group(0))
