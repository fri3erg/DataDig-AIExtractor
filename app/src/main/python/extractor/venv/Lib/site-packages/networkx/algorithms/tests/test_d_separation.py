...
def test_markov_condition(graph):
    ...
    for node in graph.nodes:
        ...
        non_descendants = graph.nodes - nx.descendants(graph, node) - {node***REMOVED*** - parents
        assert nx.d_separated(graph, {node***REMOVED***, non_descendants, parents)


def test_path_graph_dsep(path_graph):
    """Example-based test of d-separation for path_graph."""
    assert nx.d_separated(path_graph, {0***REMOVED***, {2***REMOVED***, {1***REMOVED***)
    assert not nx.d_separated(path_graph, {0***REMOVED***, {2***REMOVED***, {***REMOVED***)


def test_fork_graph_dsep(fork_graph):
    """Example-based test of d-separation for fork_graph."""
    assert nx.d_separated(fork_graph, {1***REMOVED***, {2***REMOVED***, {0***REMOVED***)
    assert not nx.d_separated(fork_graph, {1***REMOVED***, {2***REMOVED***, {***REMOVED***)


def test_collider_graph_dsep(collider_graph):
    """Example-based test of d-separation for collider_graph."""
    assert nx.d_separated(collider_graph, {0***REMOVED***, {1***REMOVED***, {***REMOVED***)
    assert not nx.d_separated(collider_graph, {0***REMOVED***, {1***REMOVED***, {2***REMOVED***)
...
def test_naive_bayes_dsep(naive_bayes_graph):
    ...
    for u, v in combinations(range(1, 5), 2):
        assert nx.d_separated(naive_bayes_graph, {u***REMOVED***, {v***REMOVED***, {0***REMOVED***)
        assert not nx.d_separated(naive_bayes_graph, {u***REMOVED***, {v***REMOVED***, {***REMOVED***)


def test_asia_graph_dsep(asia_graph):
    """Example-based test of d-separation for asia_graph."""
    assert nx.d_separated(
        asia_graph, {"asia", "smoking"***REMOVED***, {"dyspnea", "xray"***REMOVED***, {"bronchitis", "either"***REMOVED***
    )
    assert nx.d_separated(
        asia_graph, {"tuberculosis", "cancer"***REMOVED***, {"bronchitis"***REMOVED***, {"smoking", "xray"***REMOVED***
    )
...
def test_undirected_graphs_are_not_supported():
    ...
    with pytest.raises(nx.NetworkXNotImplemented):
        nx.d_separated(g, {0***REMOVED***, {1***REMOVED***, {2***REMOVED***)
    with pytest.raises(nx.NetworkXNotImplemented):
        nx.is_minimal_d_separator(g, {0***REMOVED***, {1***REMOVED***, {2***REMOVED***)
    with pytest.raises(nx.NetworkXNotImplemented):
        nx.minimal_d_separator(g, {0***REMOVED***, {1***REMOVED***)
...
def test_cyclic_graphs_raise_error():
    ...
    with pytest.raises(nx.NetworkXError):
        nx.is_minimal_d_separator(g, 0, 1, {2***REMOVED***)
...
def test_invalid_nodes_raise_error(asia_graph):
    ...
    with pytest.raises(nx.NodeNotFound):
        nx.d_separated(asia_graph, {0***REMOVED***, {1***REMOVED***, {2***REMOVED***)
    with pytest.raises(nx.NodeNotFound):
        nx.is_minimal_d_separator(asia_graph, 0, 1, {2***REMOVED***)
    with pytest.raises(nx.NodeNotFound):
...
def test_minimal_d_separator():
    ...
    assert not nx.d_separated(G, {"B"***REMOVED***, {"E"***REMOVED***, set())
    ...
    G = nx.DiGraph(edge_list)
    assert not nx.d_separated(G, {"A"***REMOVED***, {"C"***REMOVED***, set())
    Zmin = nx.minimal_d_separator(G, "A", "C")

    # the minimal separating set should pass the test for minimality
    assert nx.is_minimal_d_separator(G, "A", "C", Zmin)
    assert Zmin == {"B"***REMOVED***

    Znotmin = Zmin.union({"D"***REMOVED***)
...
def test_minimal_d_separator_checks_dsep():
    ...
    assert not nx.d_separated(g, {"C"***REMOVED***, {"F"***REMOVED***, {"D"***REMOVED***)

    # since {'D'***REMOVED*** and {***REMOVED*** are not d-separators, we return false
    assert not nx.is_minimal_d_separator(g, "C", "F", {"D"***REMOVED***)
    assert not nx.is_minimal_d_separator(g, "C", "F", {***REMOVED***)
