...
class TestReindexSetIndex:
    ...
    def test_setitem_reset_index_dtypes(self):
        ...
        df = DataFrame(columns=["a", "b", "c"]).astype(
            {"a": "datetime64[ns]", "b": np.int64, "c": np.float64***REMOVED***
        )
        df1 = df.set_index(["a"])
        df1["d"] = []
        result = df1.reset_index()
        expected = DataFrame(columns=["a", "b", "c", "d"], index=range(0)).astype(
            {"a": "datetime64[ns]", "b": np.int64, "c": np.float64, "d": np.float64***REMOVED***
        )
    ...
    def test_reindex_timestamp_with_fold(self, timezone, year, month, day, hour):
        ...
        df = (
            DataFrame({"index": [transition_1, transition_2], "vals": ["a", "b"]***REMOVED***)
            .set_index("index")
            .reindex(["1", "2"])
        )
        exp = DataFrame({"index": ["1", "2"], "vals": [np.nan, np.nan]***REMOVED***).set_index(
            "index"
        )
...
class TestDataFrameSelectReindex:
    ...
    def test_reindex_tzaware_fill_value(self):
        ...
        expected = DataFrame({0: [1], 1: [ts]***REMOVED***)
        ...
        expected = DataFrame({0: [1], 1: [per]***REMOVED***)
        tm.assert_frame_equal(res, expected)
        ...
        expected = DataFrame({0: [1], 1: [interval]***REMOVED***)
        expected[1] = expected[1].astype(res.dtypes[1])
        tm.assert_frame_equal(res, expected)
    ...
    def test_reindex_date_fill_value(self):
        ...
        expected = DataFrame(
            {"A": df["A"].tolist() + [fv], "B": df["B"].tolist() + [fv], "C": [fv] * 4***REMOVED***,
            dtype=object,
        )
        tm.assert_frame_equal(res, expected)
        ...
        expected = DataFrame(
            {"A": df["A"].tolist() + [ts], "B": df["B"].tolist() + [ts], "C": [ts] * 4***REMOVED***,
        )
        tm.assert_frame_equal(res, expected)
    ...
    def test_reindex_with_multi_index(self):
        ...
        expected = DataFrame(
            {"a": [0] * 4, "b": new_index, "c": [np.nan, "C", "F", np.nan]***REMOVED***
        ).set_index(["a", "b"])
        tm.assert_frame_equal(expected, reindexed)

        # reindexing with backfilling
        expected = DataFrame(
            {"a": [0] * 4, "b": new_index, "c": ["B", "C", "F", "G"]***REMOVED***
        ).set_index(["a", "b"])
        ...
        expected = DataFrame(
            {"a": [0] * 4, "b": new_index, "c": ["A", "C", "F", "F"]***REMOVED***
        ).set_index(["a", "b"])
    ...
    def test_reindex_methods(self, method, expected_values):
        df = DataFrame({"x": list(range(5))***REMOVED***)
        target = np.array([-0.1, 0.9, 1.1, 1.5])

        expected = DataFrame({"x": expected_values***REMOVED***, index=target)
    ...
    def test_reindex_methods_nearest_special(self):
        ...
        expected = DataFrame({"x": [0, 1, 1, np.nan]***REMOVED***, index=target)
        actual = df.reindex(target, method="nearest", tolerance=0.2)
        tm.assert_frame_equal(expected, actual)

        expected = DataFrame({"x": [0, np.nan, 1, np.nan]***REMOVED***, index=target)
    ...
    def test_reindex_nearest_tz(self, tz_aware_fixture):
        ...
        df = DataFrame({"x": list(range(5))***REMOVED***, index=idx)
    ...
    def test_reindex_frame_add_nat(self):
        ...
        df = DataFrame(
            {"A": np.random.default_rng(2).standard_normal(len(rng)), "B": rng***REMOVED***
        )
    ...
    ***REMOVED***,
    ...
    def test_reindex_sparse(self):
        ...
        df = DataFrame(
            {"A": [0, 1], "B": pd.array([0, 1], dtype=pd.SparseDtype("int64", 0))***REMOVED***
        )
    ...
    def test_reindex_uint_dtypes_fill_value(self, any_unsigned_int_numpy_dtype):
        # GH#48184
        df = DataFrame({"a": [1, 2], "b": [1, 2]***REMOVED***, dtype=any_unsigned_int_numpy_dtype)
        result = df.reindex(columns=list("abcd"), index=[0, 1, 2, 3], fill_value=10)
        expected = DataFrame(
            {"a": [1, 2, 10, 10], "b": [1, 2, 10, 10], "c": 10, "d": 10***REMOVED***,
    ...
    def test_reindex_single_column_ea_index_and_columns(self, any_numeric_ea_dtype):
        # GH#48190
        df = DataFrame({"a": [1, 2]***REMOVED***, dtype=any_numeric_ea_dtype)
        result = df.reindex(columns=list("ab"), index=[0, 1, 2], fill_value=10)
        expected = DataFrame(
            {"a": Series([1, 2, 10], dtype=any_numeric_ea_dtype), "b": 10***REMOVED***
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_reindex_axis_style(self):
        ...
        expected = DataFrame(
            {"A": [1, 2, np.nan], "B": [4, 5, np.nan]***REMOVED***, index=[0, 1, 3]
        )
    ...
    def test_reindex_single_named_indexer(self):
        # https://github.com/pandas-dev/pandas/issues/12392
        df = DataFrame({"A": [1, 2, 3], "B": [1, 2, 3]***REMOVED***)
        result = df.reindex([0, 1], columns=["A"])
        expected = DataFrame({"A": [1, 2]***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_reindex_corner(self, int_frame):
        index = Index(["a", "b", "c"])
        dm = DataFrame({***REMOVED***).reindex(index=[1, 2, 3])
    ...
    def test_reindex_multi_categorical_time(self):
        ...
        df = DataFrame({"a": range(len(midx))***REMOVED***, index=midx)
        df2 = df.iloc[[0, 1, 2, 3, 4, 5, 6, 8]]

        result = df2.reindex(midx)
        expected = DataFrame({"a": [0, 1, 2, 3, 4, 5, 6, np.nan, 8]***REMOVED***, index=midx)
        tm.assert_frame_equal(result, expected)
...
***REMOVED***,
        ...
        expected = DataFrame({"A": [0, 1, np.nan], "B": Series(list("abe"))***REMOVED***).set_index(
            "B"
        )
        tm.assert_frame_equal(result, expected, check_index_type=True)

        result = df.reindex(["a", "b"])
        expected = DataFrame({"A": [0, 1], "B": Series(list("ab"))***REMOVED***).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)

        result = df.reindex(["e"])
        expected = DataFrame({"A": [np.nan], "B": Series(["e"])***REMOVED***).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)

        result = df.reindex(["d"])
        expected = DataFrame({"A": [np.nan], "B": Series(["d"])***REMOVED***).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)
        ...
        expected = DataFrame(
            {"A": [0, np.nan], "B": Series(list("ae")).astype(CategoricalDtype(cats))***REMOVED***
        ).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)

        result = df.reindex(Categorical(["a"], categories=cats))
        expected = DataFrame(
            {"A": [0], "B": Series(list("a")).astype(CategoricalDtype(cats))***REMOVED***
        ).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)
...
***REMOVED***,
        ...
        msg = r"argument {***REMOVED*** is not implemented for CategoricalIndex\.reindex"
    ...
    @pytest.mark.parametrize(
        "kwargs",
        [
            {"method": "pad", "tolerance": timedelta(seconds=9)***REMOVED***,
            {"method": "backfill", "tolerance": timedelta(seconds=9)***REMOVED***,
            {"method": "nearest"***REMOVED***,
            {"method": None***REMOVED***,
        ],
    )
    def test_reindex_empty_frame(self, kwargs):
        ...
        expected = DataFrame({"a": [np.nan] * 3***REMOVED***, index=idx, dtype=object)
        tm.assert_frame_equal(result, expected)
...
***REMOVED***,
    ...
    def test_invalid_method(self):
        df = DataFrame({"A": [1, np.nan, 2]***REMOVED***)
