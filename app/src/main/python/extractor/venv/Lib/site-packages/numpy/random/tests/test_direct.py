...
class Base:
    ...
    def _read_csv(cls, filename):
        with open(filename) as csv:
            ...
            return {'seed': seed, 'data': np.array(data, dtype=cls.dtype)***REMOVED***
    ...
    def test_repr(self):
        ...
        assert f'{id(rs):#x***REMOVED***'.upper().replace('X', 'x') in repr(rs)

    def test_str(self):
        rs = Generator(self.bit_generator(*self.data1['seed']))
        assert 'Generator' in str(rs)
        assert str(self.bit_generator.__name__) in str(rs)
        assert f'{id(rs):#x***REMOVED***'.upper().replace('X', 'x') not in str(rs)
