...
@pytest.mark.parametrize("filename", [None, "test.{ext***REMOVED***"])
...
def test_infer_compression(all_parsers, csv1, buffer, ext):
    # see gh-9770
    parser = all_parsers
    kwargs = {"index_col": 0, "parse_dates": True***REMOVED***
...
def test_compression_utf_encoding(all_parsers, csv_dir_path, utf_value, encoding_fmt):
    ...
    path = os.path.join(csv_dir_path, f"utf{utf_value***REMOVED***_ex_small.zip")
...
def test_invalid_compression(all_parsers, invalid_compression):
    parser = all_parsers
    compress_kwargs = {"compression": invalid_compression***REMOVED***

    msg = f"Unrecognized compression type: {invalid_compression***REMOVED***"
