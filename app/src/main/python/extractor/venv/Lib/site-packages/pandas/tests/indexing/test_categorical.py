...
class TestCategoricalIndex:
    def test_loc_scalar(self, df):
        ...
        expected = DataFrame({"A": [0, 1, 5]***REMOVED***, index=Index(bidx))
    ...
    def test_slicing(self):
        ...
        df = DataFrame({"value": (np.arange(100) + 1).astype("int64")***REMOVED***)
        ...
        expected = DataFrame(
            {"value": np.arange(11, 21).astype("int64")***REMOVED***,
    ...
    def test_slicing_and_getting_ops(self):
        ...
        df = DataFrame({"cats": cats, "values": values***REMOVED***, index=idx)
        ...
        # 2:4,: | "j":"k",:
        exp_df = DataFrame({"cats": cats2, "values": values2***REMOVED***, index=idx2)
    ...
    def test_slicing_doc_examples(self):
        ...
        expected = DataFrame(
            {"cats": Categorical(["a", "b", "b"], categories=["a", "b", "c"])***REMOVED***,
            index=["h", "i", "j"],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_loc_setitem_expansion_label_unused_category(self, df2):
        ...
        expected = DataFrame({"A": [0, 1, 5, 2, 3, 20]***REMOVED***, index=exp_index)
        tm.assert_frame_equal(result, expected)
    ...
    def test_loc_listlike_dtypes(self):
        ...
        df = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]***REMOVED***, index=index)

        # unique slice
        res = df.loc[["a", "b"]]
        exp_index = CategoricalIndex(["a", "b"], categories=index.categories)
        exp = DataFrame({"A": [1, 2], "B": [4, 5]***REMOVED***, index=exp_index)
        ...
        exp = DataFrame({"A": [1, 1, 2], "B": [4, 4, 5]***REMOVED***, index=exp_index)
        tm.assert_frame_equal(res, exp, check_index_type=True)
    ...
    def test_loc_listlike_dtypes_duplicated_categories_and_codes(self):
        ...
        res = df.loc[["a", "b"]]
        exp = DataFrame(
            {"A": [1, 3, 2], "B": [4, 6, 5]***REMOVED***, index=CategoricalIndex(["a", "a", "b"])
        )
        tm.assert_frame_equal(res, exp, check_index_type=True)
        ...
        res = df.loc[["a", "a", "b"]]
        exp = DataFrame(
            {"A": [1, 3, 1, 3, 2], "B": [4, 6, 4, 6, 5]***REMOVED***,
    ...
    def test_loc_listlike_dtypes_unused_category(self):
        ...
        df = DataFrame({"A": [1, 2, 3, 4], "B": [5, 6, 7, 8]***REMOVED***, index=index)

        res = df.loc[["a", "b"]]
        exp = DataFrame(
            {"A": [1, 3, 2], "B": [5, 7, 6]***REMOVED***,
        ...
        res = df.loc[["a", "a", "b"]]
        exp = DataFrame(
            {"A": [1, 3, 1, 3, 2], "B": [5, 7, 5, 7, 6]***REMOVED***,
...
***REMOVED***
    ...
    def test_loc_getitem_with_non_string_categories(self, idx_values, ordered):
        ...
        df = DataFrame({"A": ["foo", "bar", "baz"]***REMOVED***, index=cat_idx)
        ...
        expected = DataFrame({"A": ["qux", "bar", "baz"]***REMOVED***, index=cat_idx)
        tm.assert_frame_equal(result, expected)

        # list assignment
        result = df.copy()
        result.loc[idx_values[:2], "A"] = ["qux", "qux2"]
        expected = DataFrame({"A": ["qux", "qux2", "baz"]***REMOVED***, index=cat_idx)
        tm.assert_frame_equal(result, expected)
