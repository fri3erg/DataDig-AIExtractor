    ...
    def _insert_index(self, data):
        ...
        if idx_nlevels == 1:
            ...
            for i in range(idx_nlevels):
                data.insert(i, f"Index{i***REMOVED***", data.index._get_level_values(i))
...
def main() -> None:
    ...
    df1 = pd.DataFrame({"A": [10, 11, 12], "B": [20, 21, 22], "C": [30, 31, 32]***REMOVED***)
    df2 = pd.DataFrame({"A": [10, 12], "C": [30, 32]***REMOVED***)

    p.plot([df1, df2], pd.concat([df1, df2]), labels=["df1", "df2"], vertical=True)
    plt.show()

    df3 = pd.DataFrame({"X": [10, 12], "Z": [30, 32]***REMOVED***)
    ...
    df3 = pd.DataFrame({"v1": [1, 2, 3, 4, 5, 6], "v2": [5, 6, 7, 8, 9, 10]***REMOVED***, index=idx)
    df3.columns = column
