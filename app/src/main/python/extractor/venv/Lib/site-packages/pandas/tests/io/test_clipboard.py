...
def df(request):
    ...
    if data_type == "delims":
        return DataFrame({"a": ['"a,\t"b|c', "d\tef`"], "b": ["hi'j", "k''lm"]***REMOVED***)
    elif data_type == "utf8":
        return DataFrame({"a": ["µasd", "Ωœ∑`"], "b": ["øπ∆˚¬", "œ∑`®"]***REMOVED***)
    elif data_type == "utf16":
        return DataFrame(
            {"a": ["\U0001f44d\U0001f44d", "\U0001f44d\U0001f44d"], "b": ["abc", "def"]***REMOVED***
        )
    elif data_type == "string":
        return DataFrame(
            np.array([f"i-{i***REMOVED***" for i in range(15)]).reshape(5, 3), columns=list("abc")
        )
    elif data_type == "long":
        ...
        return DataFrame({"en": "in English".split(), "es": "en español".split()***REMOVED***)
...
def test_checked_call_with_bad_call(monkeypatch):
    ...
    msg = f"Error calling {_return_false.__name__***REMOVED*** \\(Window Error\\)"
...
def test_stringify_text(text):
    ...
    if isinstance(text, valid_types):
        ...
        msg = (
            ...
            f"can be copied to the clipboard, not {type(text).__name__***REMOVED***"
        )
...
class TestClipboard:
    ...
    def test_read_clipboard_infer_excel(self, clipboard):
        ...
        clip_kwargs = {"engine": "python"***REMOVED***
    ...
    def test_infer_excel_with_nulls(self, clipboard):
        ...
        df_expected = DataFrame(
            data={"col1": [1, None, 2], "col2": ["red", "blue", "green"]***REMOVED***
        )
...
***REMOVED***,
    ...
    def test_raw_roundtrip(self, data):
        ...
        df = DataFrame({"data": [data]***REMOVED***)
        df.to_clipboard()
        result = read_clipboard()
        tm.assert_frame_equal(df, result)
