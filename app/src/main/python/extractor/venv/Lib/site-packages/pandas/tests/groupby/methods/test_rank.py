...
def test_rank_args(grps, vals, ties_method, ascending, pct, exp):
    ...
    df = DataFrame({"key": key, "val": vals***REMOVED***)
...
def test_rank_resets_each_group(pct, exp):
    df = DataFrame(
        {"key": ["a", "a", "a", "a", "a", "b", "b", "b", "b", "b"], "val": [1] * 10***REMOVED***
    )
...
def test_rank_avg_even_vals(dtype, upper):
    ...
    df = DataFrame({"key": ["a"] * 4, "val": [1] * 4***REMOVED***)
...
def test_rank_object_dtype(ties_method, ascending, na_option, pct, vals):
    df = DataFrame({"key": ["foo"] * 5, "val": vals***REMOVED***)
    mask = df["val"].isna()
    ...
    if mask.any():
        df2 = DataFrame({"key": ["foo"] * 5, "val": [0, np.nan, 2, np.nan, 1]***REMOVED***)
    else:
        df2 = DataFrame({"key": ["foo"] * 5, "val": [0, 0, 2, 0, 1]***REMOVED***)
...
def test_rank_empty_group():
    # see gh-22519
    column = "A"
    df = DataFrame({"A": [0, 1, 0], "B": [1.0, np.nan, 2.0]***REMOVED***)
    ...
    expected = DataFrame({"B": [0.5, np.nan, 1.0]***REMOVED***)
...
def test_rank_zero_div(input_key, input_value, output_value):
    # GH 23666
    df = DataFrame({"A": input_key, "B": input_value***REMOVED***)

    result = df.groupby("A").rank(method="dense", pct=True)
    expected = DataFrame({"B": output_value***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_rank_min_int():
    ...
    expected = DataFrame(
        {"int_col": [1.0, 2.0, 1.0], "datetimelike": [np.nan, 1.0, np.nan]***REMOVED***
    )
...
def test_rank_multiindex():
    ...
    df = concat(
        {
            "a": DataFrame({"col1": [3, 4], "col2": [1, 2]***REMOVED***),
            "b": DataFrame({"col3": [5, 6], "col4": [7, 8]***REMOVED***),
        ***REMOVED***,
        axis=1,
    )
...
def test_groupby_axis0_rank_axis1():
    # GH#41320
    df = DataFrame(
        {0: [1, 3, 5, 7], 1: [2, 4, 6, 8], 2: [1.5, 3.5, 5.5, 7.5]***REMOVED***,
        index=["a", "a", "b", "b"],
    )
...
def test_non_unique_index():
    # GH 16577
    df = DataFrame(
        {"A": [1.0, 2.0, 3.0, np.nan], "value": 1.0***REMOVED***,
...
def test_rank_categorical():
    ...
    df = DataFrame({"col1": [0, 1, 0, 1, 0, 1], "col2": cat, "col3": cat2***REMOVED***)
...
def test_groupby_op_with_nullables(na_option):
    # GH 54206
    df = DataFrame({"x": [None]***REMOVED***, dtype="Float64")
