 ...
 *** it is a list of dictionaries containing `blocks':
     ...
     B['implicit'] --- dictionary {'a':<variable definition>,'b':...***REMOVED*** | None
     ...
     B['use'] --- dictionary of modules used in current block:
         {<modulename>:{['only':<0|1>],['map':{<local_name1>:<use_name1>,...***REMOVED***]***REMOVED******REMOVED***
     ...
     B['f2pyenhancements'] --- optional dictionary
           ...
           ***REMOVED***
     B['entry'] --- dictionary {entryname:argslist,..***REMOVED***
     B['varnames'] --- list of variable names given in the order of reading the
                       Fortran code, useful for derived types.
     B['saved_interface'] --- a string of scanned routine signature, defines explicit interface
 *** Variable definition is a dictionary
     D = B['vars'][<variable name>] =
     {'typespec'[,'attrspec','kindselector','charselector','=','typename']***REMOVED***
     D['typespec'] = 'byte' | 'character' | 'complex' | 'double complex' |
                     'double precision' | 'integer' | 'logical' | 'real' | 'type'
     D['attrspec'] --- list of attributes (e.g. 'dimension(<arrayspec>)',
                       'external','intent(in|out|inout|hide|c|callback|cache|aligned4|aligned8|aligned16)',
                       'optional','required', etc)
     K = D['kindselector'] = {['*','kind']***REMOVED*** (only if D['typespec'] =
                         'complex' | 'integer' | 'logical' | 'real' )
     C = D['charselector'] = {['*','len','kind','f2py_len']***REMOVED***
...
groupcache = None
groupcounter = 0
grouplist = {groupcounter: []***REMOVED***
groupname = ''
include_paths = []
neededmodule = -1
onlyfuncs = []
...
for c in "abcdefghopqrstuvwxyz$_":
    defaultimplicitrules[c] = {'typespec': 'real'***REMOVED***
for c in "ijklmn":
    defaultimplicitrules[c] = {'typespec': 'integer'***REMOVED***
badnames = {***REMOVED***
invbadnames = {***REMOVED***
...
def readfortrancode(ffile, dowithline=show, istop=1):
    ...
    while True:
        try:
            ...
            raise Exception(
                f'readfortrancode: reading {fin.filename()***REMOVED***#{fin.lineno()***REMOVED***'
...
def split_by_unquoted(line, characters):
    ...
    r = re.compile(
        r"\A(?P<before>({single_quoted***REMOVED***|{double_quoted***REMOVED***|{not_quoted***REMOVED***)*)"
        r"(?P<after>{char***REMOVED***.*)\Z".format(
            not_quoted="[^\"'{***REMOVED***]".format(re.escape(characters)),
            char="[{***REMOVED***]".format(re.escape(characters)),
            single_quoted=r"('([^'\\]|(\\.))*')",
            double_quoted=r'("([^"\\]|(\\.))*")'))
    m = r.match(line)
    if m:
...
def crackline(line, reset=0):
    ...
    if reset < 0:
        ...
        groupcache[groupcounter]['vars'] = {***REMOVED***
...
def analyzeline(m, case, line):
    ...
    if expectbegin and case not in ['begin', 'call', 'callfun', 'type'] \
       and not skipemptyends and groupcounter < 1:
        ...
        groupcache[groupcounter] = {***REMOVED***
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        if case in ['call', 'callfun']:
            ...
            if name in groupcache[groupcounter]['interfaced']:
                return
            block = {'call': 'subroutine', 'callfun': 'function'***REMOVED***[case]
        ...
        groupcache[groupcounter]['interfaced'] = []
        groupcache[groupcounter]['vars'] = {***REMOVED***
        groupcache[groupcounter]['entry'] = {***REMOVED***
        # end of creation
        if block == 'type':
        ...
        if block in ['function', 'subroutine']:  # set global attributes
            ...
            if bindcline:
                ...
                if bindcdat:
                    groupcache[groupcounter]['bindlang'] = {name : {***REMOVED******REMOVED***
            ...
            if t:
                if t.get('typespec') == 'real':
                    ...
                    initexpr = initexpr[1:].lower().replace('d', 'e').\
                        replace(',', '+1j*(')
            try:
                v = eval(initexpr, {***REMOVED***, params)
        ...
        if f == 2:
            ...
            if dl.startswith(','):
                dl = dl[1:].strip()
            ll.append([dl, il])
        vars = groupcache[groupcounter].get('vars', {***REMOVED***)
        last_name = None
        for l in ll:
        ...
        if m1:
            ...
            if 'use' not in groupcache[groupcounter]:
                groupcache[groupcounter]['use'] = {***REMOVED***
            name = m1.group('name')
            groupcache[groupcounter]['use'][name] = {***REMOVED***
            isonly = 0
        ...
        if 'f2pyenhancements' not in groupcache[groupcounter]:
            groupcache[groupcounter]['f2pyenhancements'] = {***REMOVED***
    ...
    if 'externals' in block and block['externals']:
        ...
        if interface['vars'] or interface['body']:
            ...
            mblock = {'block': 'python module', 'body': [
                interface], 'vars': {***REMOVED***, 'name': mname, 'interfaced': block['externals']***REMOVED***
            useblock[mname] = {***REMOVED***
        ...
        for e in block['common'][k]:
            ...
            if m:
                ...
                if n in block['vars']:
                    ...
                    if dims:
                        block['vars'][n] = {
                            'attrspec': ['dimension(%s)' % (','.join(dims))]***REMOVED***
                    else:
                        block['vars'][n] = {***REMOVED***
...
def getlincoef(e, xset):  # e = a*x+b ; x in xset
    ...
    >>> getlincoef('2*x + 1', {'x'***REMOVED***)
    (2, 1, 'x')
    >>> getlincoef('3*x + x*2 + 2 + 1', {'x'***REMOVED***)
    (5, 3, 'x')
    >>> getlincoef('0', {'x'***REMOVED***)
    (0, 0, None)
    >>> getlincoef('0*x', {'x'***REMOVED***)
    (0, 0, 'x')
    >>> getlincoef('x*x', {'x'***REMOVED***)
    (None, None, None)

    This can be tricked by sufficiently complex expressions

    >>> getlincoef('(x - 0.5)*(x - 1.5)*(x - 1)*x + 2*x + 3', {'x'***REMOVED***)
    (2.0, 3.0, 'x')
    """
    try:
        c = int(myeval(e, {***REMOVED***, {***REMOVED***))
        return 0, c, None
    except Exception:
        pass
        ...
        if m:
            try:
                ...
                while m1:
                    ...
                    m1 = re_1.match(ee)
                b = myeval(ee, {***REMOVED***, {***REMOVED***)
                m1 = re_1.match(e)
                while m1:
                    ee = '%s(%s)%s' % (
                        m1.group('before'), 1, m1.group('after'))
                    m1 = re_1.match(ee)
                a = myeval(ee, {***REMOVED***, {***REMOVED***) - b
                m1 = re_1.match(e)
                while m1:
                    ee = '%s(%s)%s' % (
                        m1.group('before'), 0.5, m1.group('after'))
                    m1 = re_1.match(ee)
                c = myeval(ee, {***REMOVED***, {***REMOVED***)
...
def get_parameters(vars, global_params={***REMOVED***):
        ...
        if '=' in vars[n]:
            ...
            if isdouble(vars[n]):
                ...
                outmess(f'get_parameters[TODO]: '
                        f'implement evaluation of complex expression {v***REMOVED***\n')
            ...
            try:
                ...
                params[n] = v
                outmess(f'get_parameters: got "{msg***REMOVED***" on {n!r***REMOVED***\n')
            ...
            print(vars[n])
            outmess(f'get_parameters:parameter {n!r***REMOVED*** does not have value?!\n')
    return params
...
def _eval_scalar(value, params):
    ...
    try:
        ...
        value = eval(value, {***REMOVED***, params)
        ...
        if 'attrspec' in block['vars']['']:
            ...
            for n in set(vars) | set(b['name'] for b in block['body']):
                for k in ['public', 'private']:
                    if k in gen:
                        vars[n] = setattrspec(vars.get(n, {***REMOVED***), k)
    svars = []
    args = block['args']
    for a in args:
        try:
                        ...
                        for v, (solver, deps) in coeffs_and_deps.items():
                            ...
                            if solver is not None and v not in all_deps:
                                ...
                                if 'check' not in vars[v]:
                                    ...
                                    vars[v]['check'] = [
                                        f'shape({n***REMOVED***, {i***REMOVED***) == {d***REMOVED***']
                            else:
                            ...
                            if 'kind' in kindselect:
                                try:
                                    kindselect['kind'] = eval(
                                        kindselect['kind'], {***REMOVED***, params)
                                except Exception:
                                    pass
...
def param_eval(v, g_params, params, dimspec=None):
    ...
    if dimspec is None:
        try:
            ...
            p = v
            outmess(f'param_eval: got "{msg***REMOVED***" on {v!r***REMOVED***\n')
        return p

    # This is an array parameter.
    # First, we parse the dimension information
    if len(dimspec) < 2 or dimspec[::len(dimspec)-1] != "()":
        raise ValueError(f'param_eval: dimension {dimspec***REMOVED*** can\'t be parsed')
    ...
    for item in v:
        try:
            ...
            outmess(f'param_eval: got "{msg***REMOVED***" on {item!r***REMOVED***\n')
        v_eval.append(item)
...
def param_parse(d, params):
    ...
    * If the line being analyzed is
    ...
    >>> d = 'pa(1)'
    >>> params = dict(pa={1: 3, 2: 5***REMOVED***)
    >>> param_parse(d, params)
    3
    """
    if "(" in d:
...
def analyzeargs(block):
    ...
    if 'result' in block and block['result'] not in block['vars']:
        block['vars'][block['result']] = {***REMOVED***
    return block
...
def determineexprtype(expr, vars, rules={***REMOVED***):
    if expr in vars:
...
def crackfortran(files):
    ...
    for hook in post_processing_hooks:
        outmess(f'  {hook.__name__***REMOVED***\n', 0)
...
def character_backward_compatibility_hook(item, parents, result,
    ...
    if new_value is not None:
        if new_value != value:
            ...
            outmess(f'character_bc_hook[{parent_key***REMOVED***.{key***REMOVED***]:'
                    f' replaced `{value***REMOVED***` -> `{new_value***REMOVED***`\n', 1)
    ...
    for l in sys.argv[1:]:
        if l == '':
            ...
            try:
                ...
                errmess(f'OSError: {detail!s***REMOVED***\n')
        else:
            funcs.append(l)
