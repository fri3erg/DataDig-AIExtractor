...
EXPECTED_KEYS = {'descr', 'fortran_order', 'shape'***REMOVED***
...
def descr_to_dtype(descr):
    ...
    return numpy.dtype({'names': names, 'formats': formats, 'titles': titles,
                        'offsets': offsets, 'itemsize': offset***REMOVED***)
...
def _wrap_header(header, version):
    ...
    try:
        ...
        msg = "Header length {***REMOVED*** too big for version={***REMOVED***".format(hlen, version)
...
def _read_array_header(fp, version, max_header_size=_MAX_HEADER_SIZE):
    ...
    if hinfo is None:
        raise ValueError("Invalid version {!r***REMOVED***".format(version))
    ...
    if len(header) > max_header_size:
        raise ValueError(
            f"Header info length ({len(header)***REMOVED***) is large and may not be safe "
            "to load securely.\n"
    ...
    try:
        ...
        if version <= (2, 0):
            ...
            try:
                ...
                msg = "Cannot parse header: {!r***REMOVED***"
    ...
    if not isinstance(d, dict):
        msg = "Header is not a dictionary: {!r***REMOVED***"
        raise ValueError(msg.format(d))

    if EXPECTED_KEYS != d.keys():
        keys = sorted(d.keys())
        msg = "Header does not contain the correct keys: {!r***REMOVED***"
        raise ValueError(msg.format(keys))

    # Sanity-check the values.
    if (not isinstance(d['shape'], tuple) or
            not all(isinstance(x, int) for x in d['shape'])):
        msg = "shape is not valid: {!r***REMOVED***"
        raise ValueError(msg.format(d['shape']))
    if not isinstance(d['fortran_order'], bool):
        msg = "fortran_order is not a valid bool: {!r***REMOVED***"
        raise ValueError(msg.format(d['fortran_order']))
    try:
        dtype = descr_to_dtype(d['descr'])
    except TypeError as e:
        msg = "descr is not a valid dtype descriptor: {!r***REMOVED***"
