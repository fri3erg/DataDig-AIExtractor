...
class TestCycles:
    ...
    def test_cycle_basis_ordered(self):
        ...
        perm = {1: 0, 0: 1***REMOVED***  # switch 0 and 1
...
class TestCycleEnumeration:
    ...
    def check_cycle(self, g, c, es, cache, source, original_c, length_bound, chordless):
        if length_bound is not None and len(c) > length_bound:
            raise RuntimeError(
                f"computed cycle {original_c***REMOVED*** exceeds length bound {length_bound***REMOVED***"
***REMOVED***
        if source == "computed":
            if es in cache:
                raise RuntimeError(
                    f"computed cycle {original_c***REMOVED*** has already been found!"
    ***REMOVED***
            else:
            ...
            if es in cache:
                ...
                raise RuntimeError(f"expected cycle {original_c***REMOVED*** was not computed")

        if not all(g.has_edge(*e) for e in es):
            raise RuntimeError(
                f"{source***REMOVED*** claimed cycle {original_c***REMOVED*** is not a cycle of g"
***REMOVED***
        if chordless and len(g.subgraph(c).edges) > len(c):
            raise RuntimeError(f"{source***REMOVED*** cycle {original_c***REMOVED*** is not chordless")
...
***REMOVED***
        ...
        if isinstance(expected_cycles, int):
            if len(cycle_cache) != expected_cycles:
                raise RuntimeError(
                    f"expected {expected_cycles***REMOVED*** cycles, got {len(cycle_cache)***REMOVED***"
    ***REMOVED***
            return
...
***REMOVED***
        ...
        if len(cycle_cache):
            for c in cycle_cache.values():
                raise RuntimeError(
                    f"computed cycle {c***REMOVED*** is valid but not in the expected cycle set!"
    ***REMOVED***
...
***REMOVED***
    ...
    def test_simple_cycles_notable_clique_sequences(self):
        ...
        # A050534: the number of 4-cycles in the complete graph K_{n+1***REMOVED***
        ...
        # A002807: Number of cycles in the complete graph on n nodes K_{n***REMOVED***.
