...
class JSONLoader(BaseLoader):
    ...
    Example:
        [{"text": ...***REMOVED***, {"text": ...***REMOVED***, {"text": ...***REMOVED***] -> schema = .[].text
        {"key": [{"text": ...***REMOVED***, {"text": ...***REMOVED***, {"text": ...***REMOVED***]***REMOVED*** -> schema = .key[].text
        ["", "", ""] -> schema = .[]
    """
...
***REMOVED***
    ...
    def _get_text(self, sample: Any) -> str:
        ...
        if self._text_content and not isinstance(content, str):
            raise ValueError(
                f"Expected page_content is string, got {type(content)***REMOVED*** instead. \
        ...
        if not isinstance(sample, dict):
            raise ValueError(
                f"Expected the jq schema to result in a list of objects (dict), \
                    so sample must be a dict but got `{type(sample)***REMOVED***`"
***REMOVED***

        if sample.get(self._content_key) is None:
            raise ValueError(
                f"Expected the jq schema to result in a list of objects (dict) \
                    with the key `{self._content_key***REMOVED***`"
***REMOVED***
    ...
    def _validate_metadata_func(self, data: Any) -> None:
        ...
        sample = data.first()
        if self._metadata_func is not None:
            sample_metadata = self._metadata_func(sample, {***REMOVED***)
            if not isinstance(sample_metadata, dict):
                raise ValueError(
                    f"Expected the metadata_func to return a dict but got \
                        `{type(sample_metadata)***REMOVED***`"
    ***REMOVED***
