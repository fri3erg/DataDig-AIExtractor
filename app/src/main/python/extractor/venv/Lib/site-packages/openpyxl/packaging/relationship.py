...
class Relationship(Serialisable):
     ...
     ***REMOVED***:
        ...
        """
        if type is not None:
            Type = "{0***REMOVED***/{1***REMOVED***".format(REL_NS, type)
        self.Type = Type
        self.Target = Target
...
class RelationshipList(Serialisable):
    ...
    def append(self, value):
        ...
        values.append(value)
        if not value.Id:
            value.Id = "rId{0***REMOVED***".format((len(values)))
    ...
    def __getitem__(self, key):
        for r in self.Relationship:
            if r.Id == key:
                return r
        raise KeyError("Unknown relationship: {0***REMOVED***".format(key))
    ...
    def to_tree(self):
        ...
        for idx, rel in enumerate(self.Relationship, 1):
            if not rel.Id:
                rel.Id = "rId{0***REMOVED***".format(idx)
...
def get_rels_path(path):
    ...
    filename = posixpath.join(folder, '_rels', '{0***REMOVED***.rels'.format(obj))
    return filename
...
def get_dependents(archive, filename):
    ...
    try:
        ...
        msg = "{0***REMOVED*** contains invalid dependency definitions".format(filename)
        warn(msg)
