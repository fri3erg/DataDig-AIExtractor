...
class LazyMap(AbstractLazySequence):
    ...
    def __init__(self, function, *lists, **config):
        ...
        self._cache = {***REMOVED*** if self._cache_size > 0 else None
...
class Trie(dict):
    ...
    def insert(self, string):
        ...
        >>> trie = Trie(["abc", "def"])
        >>> expected = {'a': {'b': {'c': {True: None***REMOVED******REMOVED******REMOVED***, \
                        'd': {'e': {'f': {True: None***REMOVED******REMOVED******REMOVED******REMOVED***
        >>> trie == expected
        True
