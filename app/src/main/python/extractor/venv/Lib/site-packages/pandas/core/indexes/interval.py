...
def _get_next_label(label):
    ...
    if lib.is_np_dtype(dtype, "mM") or isinstance(dtype, DatetimeTZDtype):
        ...
        raise TypeError(f"cannot determine next label for type {repr(type(label))***REMOVED***")
...
***REMOVED***  # type: ignore[operator]
        ...
        if subtype != key_dtype:
            raise ValueError(
                f"Cannot index an IntervalIndex of subtype {subtype***REMOVED*** with "
                f"values of dtype {key_dtype***REMOVED***"
***REMOVED***
...
def interval_range(
    ...
    if not _is_valid_endpoint(start):
        raise ValueError(f"start must be numeric or datetime-like, got {start***REMOVED***")
    if not _is_valid_endpoint(end):
        raise ValueError(f"end must be numeric or datetime-like, got {end***REMOVED***")
    ...
    if freq is not None and not is_number(freq):
        try:
            ...
            raise ValueError(
                f"freq must be numeric or convertible to DateOffset, got {freq***REMOVED***"
***REMOVED*** from err
