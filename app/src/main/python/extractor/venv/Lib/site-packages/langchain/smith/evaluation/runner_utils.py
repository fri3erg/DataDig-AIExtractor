...
***REMOVED*** from e
        ...
        for example_id, result in self["results"].items():
            ...
            if isinstance(output_, dict):
                output = {f"outputs.{k***REMOVED***": v for k, v in output_.items()***REMOVED***
            elif output_ is None:
                output = {***REMOVED***
            else:
                output = {"output": output_***REMOVED***

            r = {
                **{f"inputs.{k***REMOVED***": v for k, v in result["input"].items()***REMOVED***,
                **output,
            ***REMOVED***
            if "reference" in result:
                if isinstance(result["reference"], dict):
                    r.update(
                        {f"reference.{k***REMOVED***": v for k, v in result["reference"].items()***REMOVED***
        ***REMOVED***
                else:
                    r["reference"] = result["reference"]
            r.update(
                {
                    **{f"feedback.{f.key***REMOVED***": f.score for f in feedback***REMOVED***,
...
class EvalError(dict):
    ...
    def __getattr__(self, name: str) -> Any:
        try:
            ...
            raise AttributeError(f"'EvalError' object has no attribute '{name***REMOVED***'")
    ...
    if isinstance(llm_or_chain_factory, Chain):
        ...
        if llm_or_chain_factory.memory is not None:
            ...
            raise ValueError(
                ...
                f"    new_memory = {memory_class***REMOVED***(...)\n"
                f"    return {chain_class***REMOVED***"
                "(memory=new_memory, ...)\n\n"
                f'run_on_dataset("{dataset_name***REMOVED***", chain_constructor, ...)'
***REMOVED***
        return lambda: chain
        ...
        try:
            ...
            logger.info(f"Wrapping function {sig***REMOVED*** as RunnableLambda.")
    ...
    if "prompt" in inputs:
        if not isinstance(inputs["prompt"], str):
            raise InputFormatError(
                ...
                f" {type(inputs['prompt']).__name__***REMOVED***"
***REMOVED***
        ...
        if not isinstance(inputs["prompts"], list) or not all(
            ...
            raise InputFormatError(
                ...
                f" got {type(inputs['prompts']).__name__***REMOVED***"
***REMOVED***
        ...
        if isinstance(prompt_, str):
            ...
            raise InputFormatError(f"LLM Run expects string prompt input. Got {inputs***REMOVED***")
    else:
        raise InputFormatError(
            f"LLM Run expects 'prompt' or 'prompts' in inputs. Got {inputs***REMOVED***"
        )
    if len(prompts) == 1:
        return prompts[0]
    else:
        raise InputFormatError(
            f"LLM Run expects single prompt input. Got {len(prompts)***REMOVED*** prompts."
        )
...
def _get_messages(inputs: Dict[str, Any]) -> List[BaseMessage]:
    ...
    if "messages" in inputs:
        ...
        raise InputFormatError(
            ...
            f" input keys. Got {inputs***REMOVED***"
        )
    if isinstance(single_input, list) and all(
        ...
        raise InputFormatError(
            ...
            f" 'messages' key input. Got {inputs***REMOVED***"
        )
    if len(raw_messages) == 1:
        ...
        raise InputFormatError(
            ...
            f" input. Got {len(raw_messages)***REMOVED*** messages from inputs {inputs***REMOVED***"
        )
    ...
    if input_mapper:
        ...
        if not isinstance(prompt_input, str) and not (
            ...
            raise InputFormatError(
                ...
                " a list of chat messages."
                f"\nGot: {prompt_input***REMOVED*** of type {type(prompt_input)***REMOVED***."
***REMOVED***
    else:
        try:
            ...
            try:
                ...
                raise InputFormatError(
                    ...
                    f" Got: {first_example.inputs***REMOVED***"
    ...
    if input_mapper:
        ...
        if not isinstance(first_inputs, dict):
            raise InputFormatError(
                ...
                f"\nGot: {first_inputs***REMOVED*** of type {type(first_inputs)***REMOVED***."
***REMOVED***
        if missing_keys:
            raise InputFormatError(
                "Missing keys after loading example using input_mapper."
                f"\nExpected: {chain.input_keys***REMOVED***. Got: {first_inputs.keys()***REMOVED***"
***REMOVED***
    else:
        ...
        if len(first_inputs) == 1 and len(chain.input_keys) == 1:
            ...
            raise InputFormatError(
                ...
                f"Expected: {chain.input_keys***REMOVED***. "
                f"Got: {first_inputs.keys()***REMOVED***"
***REMOVED***
...
def _validate_example_inputs(
    ...
    if isinstance(llm_or_chain_factory, BaseLanguageModel):
        ...
        if isinstance(chain, Chain):
            ...
            logger.debug(f"Skipping input validation for {chain***REMOVED***")
...
def _determine_input_key(
    ...
    if config.input_key:
        ...
        if run_inputs and input_key not in run_inputs:
            logger.warning(
                f"Input key {input_key***REMOVED*** not in chain's specified"
                f" input keys {run_inputs***REMOVED***. Evaluation behavior may be undefined."
***REMOVED***
    elif run_inputs and len(run_inputs) == 1:
        input_key = run_inputs[0]
    elif run_inputs is not None and len(run_inputs) > 1:
        logger.warning(
            f"Chain expects multiple input keys: {run_inputs***REMOVED***,"
...
def _determine_prediction_key(
    ...
    if config.prediction_key:
        ...
        if run_outputs and prediction_key not in run_outputs:
            logger.warning(
                f"Prediction key {prediction_key***REMOVED*** not in chain's specified"
                f" output keys {run_outputs***REMOVED***. Evaluation behavior may be undefined."
***REMOVED***
    elif run_outputs and len(run_outputs) == 1:
        prediction_key = run_outputs[0]
    elif run_outputs is not None and len(run_outputs) > 1:
        logger.warning(
            f"Chain expects multiple output keys: {run_outputs***REMOVED***,"
...
def _determine_reference_key(
    ...
    if config.reference_key:
        ...
        if example_outputs and reference_key not in example_outputs:
            raise ValueError(
                f"Reference key {reference_key***REMOVED*** not in Dataset"
                f" example outputs: {example_outputs***REMOVED***"
***REMOVED***
...
def _construct_run_evaluator(
    ...
    if isinstance(eval_config, (EvaluatorType, str)):
        ...
        kwargs = {"llm": eval_llm, **eval_config.get_kwargs()***REMOVED***
    ...
    if isinstance(evaluator_, StringEvaluator):
        if evaluator_.requires_reference and reference_key is None:
            raise ValueError(
                ...
                f" evaluator of type {eval_type_tag***REMOVED*** with"
                f" dataset with multiple output keys: {example_outputs***REMOVED***."
***REMOVED***
        ...
        raise NotImplementedError(
            f"Run evaluator for {eval_type_tag***REMOVED*** is not implemented."
...
***REMOVED***
            ...
            raise ValueError(
                f"Unsupported custom evaluator: {custom_evaluator***REMOVED***."
...
***REMOVED***
        ...
        logger.warning(
            f"{chain_or_llm***REMOVED*** failed for example {example.id***REMOVED*** "
            f"with inputs {example.inputs***REMOVED***"
            f"\n{repr(e)***REMOVED***"
        )
...
***REMOVED***
        ...
        logger.warning(
            f"{chain_or_llm***REMOVED*** failed for example {example.id***REMOVED*** "
            f"with inputs {example.inputs***REMOVED***"
            f"\nError Type: {error_type***REMOVED***, Message: {e***REMOVED***"
        )
        result = EvalError(Error=e)
    return result
...
def _prepare_eval_run(
    ...
    if not examples:
        raise ValueError(f"Dataset {dataset_name***REMOVED*** has no example rows.")

    try:
        git_info = get_git_info()
        if git_info:
            project_metadata = project_metadata or {***REMOVED***
            project_metadata = {
                **project_metadata,
                "git": git_info,
            ***REMOVED***
        project = client.create_project(
            ...
            project_extra={"tags": tags***REMOVED*** if tags else {***REMOVED***,
...
run_on_dataset(
    ...
    project_name="{project_name***REMOVED*** - {uid***REMOVED***", # Update since {project_name***REMOVED*** already exists
)
"""
        raise ValueError(
            f"Test project {project_name***REMOVED*** already exists. Please use a different name:"
            f"\n\n{example_msg***REMOVED***"
        )
    comparison_url = dataset.url + f"/compare?selectedSessions={project.id***REMOVED***"
    print(
        f"View the evaluation results for project '{project_name***REMOVED***'"
        f" at:\n{comparison_url***REMOVED***\n\n"
        f"View all tests for Dataset {dataset_name***REMOVED*** at:\n{dataset.url***REMOVED***",
        flush=True,
    )
...
class _DatasetRunContainer:
    ...
    def _merge_test_outputs(
        ...
        results: dict = {***REMOVED***
        for example, output in zip(self.examples, batch_results):
            row_result = cast(_RowResult, all_eval_results.get(str(example.id), {***REMOVED***))
    ...
    def _collect_metrics(self) -> Dict[str, _RowResult]:
        ...
        ***REMOVED***
                    ...
                    all_eval_results.setdefault(str(callback.example_id), {***REMOVED***).update(
                        {
    ...
    def finish(self, batch_results: list, verbose: bool = False) -> TestResult:
        ...
        if verbose:
            try:
                ...
                logger.debug(f"Failed to print aggregate feedback: {repr(e)***REMOVED***")
        try:
...
***REMOVED***
            ...
            logger.debug(f"Failed to close project: {repr(e)***REMOVED***")
        return results
    ...
    def prepare(
        ...
        tags = tags or []
        for k, v in (project.metadata.get("git") or {***REMOVED***).items():
            tags.append(f"git:{k***REMOVED***={v***REMOVED***")
...
def _display_aggregate_results(aggregate_results: pd.DataFrame) -> None:
    if _is_jupyter_environment():
        ...
        formatted_string = aggregate_results.to_string(
            float_format=lambda x: f"{x:.2f***REMOVED***", justify="right"
        )
        print("\n Experiment Results:")
...
_INPUT_MAPPER_DEP_WARNING = (
    ...
    "def construct_chain():\n"
    "    my_chain = ...\n"
    "    input_mapper = {'other_key': 'MyOtherInput', 'my_input_key': x***REMOVED***\n"
