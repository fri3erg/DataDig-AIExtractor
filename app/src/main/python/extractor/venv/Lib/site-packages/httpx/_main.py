...
def format_request_headers(request: httpcore.Request, http2: bool = False) -> str:
    ...
    lines = [f"{method***REMOVED*** {target***REMOVED*** {version***REMOVED***"] + [
        f"{name.decode('ascii')***REMOVED***: {value.decode('ascii')***REMOVED***" for name, value in headers
    ]
    return "\n".join(lines)
...
def format_response_headers(
    ...
    lines = [f"{version***REMOVED*** {status***REMOVED*** {reason***REMOVED***"] + [
        f"{name.decode('ascii')***REMOVED***: {value.decode('ascii')***REMOVED***" for name, value in headers
    ]
    return "\n".join(lines)
...
def print_response(response: Response) -> None:
    ...
    if lexer_name:
        ...
        console.print(f"<{len(response.content)***REMOVED*** bytes of binary data>")
            ...
            for item in value:
                if key in ("subject", "issuer"):
                    for sub_item in item:
                        lines.append(f"*     {sub_item[0]***REMOVED***: {sub_item[1]!r***REMOVED***")
                elif isinstance(item, tuple) and len(item) == 2:
                    lines.append(f"*     {item[0]***REMOVED***: {item[1]!r***REMOVED***")
                else:
                    lines.append(f"*     {item!r***REMOVED***")
        else:
            lines.append(f"*   {key***REMOVED***: {value!r***REMOVED***")
    return "\n".join(lines)
...
def trace(
    ...
    if name == "connection.connect_tcp.started" and verbose:
        host = info["host"]
        console.print(f"* Connecting to {host!r***REMOVED***")
    elif name == "connection.connect_tcp.complete" and verbose:
        stream = info["return_value"]
        server_addr = stream.get_extra_info("server_addr")
        console.print(f"* Connected to {server_addr[0]!r***REMOVED*** on port {server_addr[1]***REMOVED***")
        ...
        console.print(f"* SSL established using {version!r***REMOVED*** / {cipher[0]!r***REMOVED***")
        console.print(f"* Selected ALPN protocol: {alpn!r***REMOVED***")
        if server_cert:
...
def download_response(response: Response, download: typing.BinaryIO) -> None:
    ...
    with rich.progress.Progress(
        "[progress.description]{task.description***REMOVED***",
        "[progress.percentage]{task.percentage:>3.0f***REMOVED***%",
        rich.progress.BarColumn(bar_width=None),
        rich.progress.DownloadColumn(),
        rich.progress.TransferSpeedColumn(),
    ) as progress:
        description = f"Downloading [bold]{rich.markup.escape(download.name)***REMOVED***"
    ...
    try:
        with Client(
            ...
            with client.stream(
                ...
                extensions={"trace": functools.partial(trace, verbose=verbose)***REMOVED***,
***REMOVED*** as response:
        ...
        console = rich.console.Console()
        console.print(f"[red]{type(exc).__name__***REMOVED***[/red]: {exc***REMOVED***")
        sys.exit(1)
