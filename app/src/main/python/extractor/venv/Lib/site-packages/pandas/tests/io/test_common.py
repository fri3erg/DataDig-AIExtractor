...
"""
    ...
    def test_stringify_file_and_path_like(self):
        ...
        with tm.ensure_clean() as path:
            with fsspec.open(f"file://{path***REMOVED***", mode="wb") as fsspec_obj:
...
Look,a snake,🐍"""
        expected = pd.DataFrame(
            {"a": ["1", "©", "Look"], "b": ["2", "®", "a snake"], "c": ["3", "®", "🐍"]***REMOVED***
        )
        s = StringIO(data)
    ...
    def test_read_non_existent(self, reader, module, error_class, fn_ext):
        ...
        msg1 = rf"File (b')?.+does_not_exist\.{fn_ext***REMOVED***'? does not exist"
        msg2 = rf"\[Errno 2\] No such file or directory: '.+does_not_exist\.{fn_ext***REMOVED***'"
        msg3 = "Expected object or value"
        msg4 = "path_or_buf needs to be a string file path or file-like"
        msg5 = (
            rf"\[Errno 2\] File .+does_not_exist\.{fn_ext***REMOVED*** does not exist: "
            rf"'.+does_not_exist\.{fn_ext***REMOVED***'"
        )
        msg6 = rf"\[Errno 2\] 没有那个文件或目录: '.+does_not_exist\.{fn_ext***REMOVED***'"
        msg7 = (
            rf"\[Errno 2\] File o directory non esistente: '.+does_not_exist\.{fn_ext***REMOVED***'"
        )
        msg8 = rf"Failed to open local file.+does_not_exist\.{fn_ext***REMOVED***"

        with pytest.raises(
            error_class,
            match=rf"({msg1***REMOVED***|{msg2***REMOVED***|{msg3***REMOVED***|{msg4***REMOVED***|{msg5***REMOVED***|{msg6***REMOVED***|{msg7***REMOVED***|{msg8***REMOVED***)",
        ):
            reader(path)
    ...
    def test_write_missing_parent_directory(self, method, module, error_class, fn_ext):
        ...
        dummy_frame = pd.DataFrame({"a": [1, 2, 3], "b": [2, 3, 4], "c": [3, 4, 5]***REMOVED***)
...
***REMOVED***,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("to_csv", {***REMOVED***, "os"),
            ("to_excel", {"engine": "openpyxl"***REMOVED***, "openpyxl"),
            ("to_feather", {***REMOVED***, "pyarrow"),
            ("to_html", {***REMOVED***, "os"),
            ("to_json", {***REMOVED***, "os"),
            ("to_latex", {***REMOVED***, "os"),
            ("to_pickle", {***REMOVED***, "os"),
            ("to_stata", {"time_stamp": pd.to_datetime("2019-01-01 00:00")***REMOVED***, "os"),
        ],
    )
...
def test_is_fsspec_url():
    ...
    assert not icom.is_fsspec_url("{'url': 'gs://pandas/somethingelse.com'***REMOVED***")
...
def test_codecs_encoding(encoding, format):
    ...
    with tm.ensure_clean() as path:
        with codecs.open(path, mode="w", encoding=encoding) as handle:
            getattr(expected, f"to_{format***REMOVED***")(handle)
...
def test_explicit_encoding(io_class, mode, msg):
    ...
    with io_class() as buffer:
        with pytest.raises(TypeError, match=msg):
            expected.to_csv(buffer, mode=f"w{mode***REMOVED***")
...
def test_encoding_errors(encoding_errors, format):
    ...
    with tm.ensure_clean() as path:
        ...
        if encoding_errors != "replace":
            ...
            expected = pd.DataFrame({decoded: [decoded]***REMOVED***, index=[decoded * 2])
            tm.assert_frame_equal(df, expected)
