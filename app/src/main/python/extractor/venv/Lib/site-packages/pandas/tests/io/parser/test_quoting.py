...
@pytest.mark.parametrize(
    "kwargs,msg",
    [
        ({"quotechar": "foo"***REMOVED***, '"quotechar" must be a(n)? 1-character string'),
        (
            {"quotechar": None, "quoting": csv.QUOTE_MINIMAL***REMOVED***,
            "quotechar must be set if quoting enabled",
        ),
        ({"quotechar": 2***REMOVED***, '"quotechar" must be string( or None)?, not int'),
    ],
)
...
def test_null_quote_char(all_parsers, quoting, quote_char):
    kwargs = {"quotechar": quote_char, "quoting": quoting***REMOVED***
    data = "a,b,c\n1,2,3"
    parser = all_parsers
...
@pytest.mark.parametrize(
    "kwargs,exp_data",
    [
        ({***REMOVED***, [[1, 2, "foo"]]),  # Test default.
        # QUOTE_MINIMAL only applies to CSV writing, so no effect on reading.
        ({"quotechar": '"', "quoting": csv.QUOTE_MINIMAL***REMOVED***, [[1, 2, "foo"]]),
        # QUOTE_MINIMAL only applies to CSV writing, so no effect on reading.
        ({"quotechar": '"', "quoting": csv.QUOTE_ALL***REMOVED***, [[1, 2, "foo"]]),
        # QUOTE_NONE tells the reader to do no special handling
        # of quote characters and leave them alone.
        ({"quotechar": '"', "quoting": csv.QUOTE_NONE***REMOVED***, [[1, 2, '"foo"']]),
        # QUOTE_NONNUMERIC tells the reader to cast
        # all non-quoted fields to float
        ({"quotechar": '"', "quoting": csv.QUOTE_NONNUMERIC***REMOVED***, [[1.0, 2.0, "foo"]]),
    ],
)
@xfail_pyarrow  # ValueError: The 'quoting' option is not supported
