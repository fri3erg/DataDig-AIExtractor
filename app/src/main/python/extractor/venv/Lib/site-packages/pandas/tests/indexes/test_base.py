...
***REMOVED***,  # datetimetz
    ...
    @pytest.mark.parametrize(
        "mapper",
        [Series(["foo", 2.0, "baz"], index=[0, 2, -1]), {0: "foo", 2: 2.0, -1: "baz"***REMOVED***],
    )
    ...
    def test_drop_tuple(self, values, to_drop):
        ...
        msg = rf"\"\[{re.escape(to_drop[1].__repr__())***REMOVED***\] not found in axis\""
        for drop_me in to_drop[1], [to_drop[1]]:
    ...
    @pytest.mark.parametrize("values", [["foo", "bar", "quux"], {"foo", "bar", "quux"***REMOVED***])
    @pytest.mark.parametrize(
        "index,expected",
        [
    ...
    def test_isin_nan_common_float64(self, nulls_fixture, float_numpy_dtype):
        ...
        if nulls_fixture is pd.NaT or nulls_fixture is pd.NA:
            ...
            msg = (
                ...
                f"not {repr(type(nulls_fixture).__name__)***REMOVED***"
***REMOVED***
            with pytest.raises(TypeError, match=msg):
    ...
    def test_isin_level_kwarg_bad_label_raises(self, label, index):
        if isinstance(index, MultiIndex):
            ...
            msg = f"'Level {label***REMOVED*** not found'"
        else:
            index = index.rename("foo")
            msg = rf"Requested level \({label***REMOVED***\) does not match index name \(foo\)"
        with pytest.raises(KeyError, match=msg):
...
***REMOVED***,
    ...
    def test_groupby(self):
        ...
        expected = {1: Index([0, 1]), 2: Index([2, 3, 4])***REMOVED***
    ...
    def test_contains_method_removed(self, index):
        ...
        if isinstance(index, IntervalIndex):
            ...
            msg = f"'{type(index).__name__***REMOVED***' object has no attribute 'contains'"
...
@pytest.mark.parametrize(
    ...
    [
        [Index, {***REMOVED***],
        *[[lambda x: Index(x, dtype=dtyp), {***REMOVED***] for dtyp in tm.ALL_REAL_NUMPY_DTYPES],
        [DatetimeIndex, {***REMOVED***],
        [TimedeltaIndex, {***REMOVED***],
        [PeriodIndex, {"freq": "Y"***REMOVED***],
    ],
)
