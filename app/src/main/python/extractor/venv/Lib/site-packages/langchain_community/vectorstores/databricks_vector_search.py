...
***REMOVED*** from e
        ...
        if self._is_databricks_managed_embeddings():
            ...
            if text_column is not None and text_column != index_source_column:
                raise ValueError(
                    f"text_column '{text_column***REMOVED***' does not match with the "
                    f"source column of the index: '{index_source_column***REMOVED***'."
    ***REMOVED***
            ...
            if index_schema:
                for col in self.columns:
                    if col not in index_schema:
                        raise ValueError(
                            f"column '{col***REMOVED***' is not in the index's schema."
            ***REMOVED***
        ...
        if not self._is_databricks_managed_embeddings():
            ...
            if index_embedding_dimension is not None:
                ...
                if inferred_embedding_dimension != index_embedding_dimension:
                    raise ValueError(
                        f"embedding model's dimension '{inferred_embedding_dimension***REMOVED***' "
                        f"does not match with the index's dimension "
                        f"'{index_embedding_dimension***REMOVED***'."
        ***REMOVED***
        else:
...
***REMOVED***
    ...
    def _op_require_direct_access_index(self, op_name: str) -> None:
        ...
        if not self._is_direct_access_index():
            raise ValueError(f"`{op_name***REMOVED***` is only supported for direct-access index.")

    @staticmethod
    def _require_arg(arg: Any, arg_name: str) -> None:
        """Raise ValueError if the required arg with name `arg_name` is None."""
        if not arg:
            raise ValueError(f"`{arg_name***REMOVED***` is required for this index.")
