...
def ensure_list_vars(arg_vars, variable: str, columns) -> list:
    if arg_vars is not None:
        if not is_list_like(arg_vars):
            ...
            raise ValueError(
                f"{variable***REMOVED*** must be a list of tuples when columns are a MultiIndex"
***REMOVED***
        else:
...
@Appender(_shared_docs["melt"] % {"caller": "pd.melt(df, ", "other": "DataFrame.melt"***REMOVED***)
def melt(
    frame: DataFrame,
    id_vars=None,
    value_vars=None,
    ...
    if value_name in frame.columns:
        raise ValueError(
            f"value_name ({value_name***REMOVED***) cannot match an element in "
    ...
    if id_vars or value_vars:
        ...
        if missing.any():
            ...
            raise KeyError(
                ...
                f"the DataFrame: {missing_labels***REMOVED***"
***REMOVED***
    ...
    if var_name is None:
        if isinstance(frame.columns, MultiIndex):
            if len(frame.columns.names) == len(set(frame.columns.names)):
                ...
                var_name = [f"variable_{i***REMOVED***" for i in range(len(frame.columns.names))]
        else:
            var_name = [
                frame.columns.name if frame.columns.name is not None else "variable"
            ]
    elif is_list_like(var_name):
        raise ValueError(f"{var_name=***REMOVED*** must be a scalar.")
    else:
        var_name = [var_name]
    ...
    mdata: dict[Hashable, AnyArrayLike] = {***REMOVED***
    for col in id_vars:
...
def lreshape(data: DataFrame, groups: dict, dropna: bool = True) -> DataFrame:
    ...
    groups : dict
        {new_name : list_of_columns***REMOVED***.
    ...
    ...                      'year1': [2007, 2007], 'year2': [2008, 2008]***REMOVED***)
    >>> data
       hr1  hr2     team  year1  year2
    0  514  545  Red Sox   2007   2008
    1  573  526  Yankees   2007   2008

    >>> pd.lreshape(data, {'year': ['year1', 'year2'], 'hr': ['hr1', 'hr2']***REMOVED***)
          team  year   hr
    0  Red Sox  2007  514
    1  Yankees  2007  573
    2  Red Sox  2008  545
    ...
    if dropna:
        ...
        for c in pivot_cols:
            mask &= notna(mdata[c])
        if not mask.all():
            mdata = {k: v[mask] for k, v in mdata.items()***REMOVED***
...
def wide_to_long(
    ...
    --------
    >>> np.random.seed(123)
    >>> df = pd.DataFrame({"A1970" : {0 : "a", 1 : "b", 2 : "c"***REMOVED***,
    ...                    "A1980" : {0 : "d", 1 : "e", 2 : "f"***REMOVED***,
    ...                    "B1970" : {0 : 2.5, 1 : 1.2, 2 : .7***REMOVED***,
    ...                    "B1980" : {0 : 3.2, 1 : 1.3, 2 : .1***REMOVED***,
    ...
    >>> w = l.unstack()
    >>> w.columns = w.columns.map('{0[0]***REMOVED***{0[1]***REMOVED***'.format)
    >>> w.reset_index()
       famid  birth  ht1  ht2
    0      1      1  2.8  3.4
    1      1      2  2.9  3.8
    ...
    ...                    'X' : np.random.randint(3, size=3)***REMOVED***)
    >>> df['id'] = df.index
    ...
    def get_var_names(df, stub: str, sep: str, suffix: str):
        regex = rf"^{re.escape(stub)***REMOVED***{re.escape(sep)***REMOVED***{suffix***REMOVED***$"
