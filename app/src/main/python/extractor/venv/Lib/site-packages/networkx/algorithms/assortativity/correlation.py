...
def degree_assortativity_coefficient(G, x="out", y="in", weight=None, nodes=None):
    ...
    if G.is_directed():
        indeg = (
            {d for _, d in G.in_degree(nodes, weight=weight)***REMOVED***
            if "in" in (x, y)
            else set()
        )
        outdeg = (
            {d for _, d in G.out_degree(nodes, weight=weight)***REMOVED***
            if "out" in (x, y)
            else set()
        )
        degrees = set.union(indeg, outdeg)
    else:
        degrees = {d for _, d in G.degree(nodes, weight=weight)***REMOVED***

    mapping = {d: i for i, d, in enumerate(degrees)***REMOVED***
...
def numeric_assortativity_coefficient(G, attribute, nodes=None):
    ...
    if nodes is None:
        nodes = G.nodes
    vals = {G.nodes[n][attribute] for n in nodes***REMOVED***
    mapping = {d: i for i, d, in enumerate(vals)***REMOVED***
