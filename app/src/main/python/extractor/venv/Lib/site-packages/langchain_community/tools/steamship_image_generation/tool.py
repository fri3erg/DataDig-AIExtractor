    ...
    def validate_size(cls, values: Dict) -> Dict:
        if "size" in values:
            ...
            if size not in SUPPORTED_IMAGE_SIZES[model_name]:
                raise RuntimeError(f"size {size***REMOVED*** is not supported by {model_name***REMOVED***")
...
***REMOVED***
    ...
    def _run(
        ...
        image_generator = self.steamship.use_plugin(
            plugin_handle=self.model_name.value, config={"n": 1, "size": self.size***REMOVED***
        )
        ...
        raise RuntimeError(f"[{self.name***REMOVED***] Tool unable to generate image!")
