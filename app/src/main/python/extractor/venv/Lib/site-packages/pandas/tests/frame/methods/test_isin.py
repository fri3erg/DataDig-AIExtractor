...
class TestDataFrameIsIn:
    ...
    def test_isin_empty(self, empty):
        # GH#16991
        df = DataFrame({"A": ["a", "b", "c"], "B": ["a", "e", "f"]***REMOVED***)
    ...
    def test_isin_df(self):
        df1 = DataFrame({"A": [1, 2, 3, 4], "B": [2, np.nan, 4, 4]***REMOVED***)
        df2 = DataFrame({"A": [0, 2, 12, 4], "B": [2, np.nan, 4, 5]***REMOVED***)
    ...
    def test_isin_tuples(self):
        # GH#16394
        df = DataFrame({"A": [1, 2, 3], "B": ["a", "b", "f"]***REMOVED***)
    ...
    def test_isin_dupe_self(self):
        other = DataFrame({"A": [1, 0, 1, 0], "B": [1, 1, 0, 0]***REMOVED***)
    ...
    def test_isin_against_series(self):
        df = DataFrame(
            {"A": [1, 2, 3, 4], "B": [2, np.nan, 4, 4]***REMOVED***, index=["a", "b", "c", "d"]
        )
    ...
    def test_isin_multiIndex(self):
        ...
        df1 = DataFrame({"A": np.ones(12), "B": np.zeros(12)***REMOVED***, index=idx)
        df2 = DataFrame(
            {
    ...
    def test_isin_empty_datetimelike(self):
        # GH#15473
        df1_ts = DataFrame({"date": pd.to_datetime(["2014-01-01", "2014-01-02"])***REMOVED***)
        df1_td = DataFrame({"date": [pd.Timedelta(1, "s"), pd.Timedelta(2, "s")]***REMOVED***)
        df2 = DataFrame({"date": []***REMOVED***)
        df3 = DataFrame()

        expected = DataFrame({"date": [False, False]***REMOVED***)
    ...
    @pytest.mark.parametrize(
        "values",
        [
            DataFrame({"a": [1, 2, 3]***REMOVED***, dtype="category"),
    ...
    def test_isin_category_frame(self, values):
        ...
        expected = DataFrame({"a": [True, True, True], "b": [False, False, False]***REMOVED***)
    ...
    def test_isin_not_lossy(self):
        ...
        df = DataFrame({"a": [val], "b": [1.0]***REMOVED***)
        result = df.isin([val])
        expected = DataFrame({"a": [True], "b": [False]***REMOVED***)
        tm.assert_frame_equal(result, expected)
