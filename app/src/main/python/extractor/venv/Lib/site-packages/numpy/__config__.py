...
def _cleanup(d):
    ...
    if isinstance(d, dict):
        return {k: _cleanup(v) for k, v in d.items() if v and _cleanup(v)***REMOVED***
    else:
        return d
...
CONFIG = _cleanup(
    {
        ...
        ***REMOVED***,
        "Python Information": {
            "path": r"C:\Users\runneradmin\AppData\Local\Temp\cibw-run-frm2g0yv\cp311-win_amd64\build\venv\Scripts\python.exe",
            "version": "3.11",
        ***REMOVED***,
        "SIMD Extensions": {
...
def show(mode=DisplayModes.stdout.value):
    ...
    Parameters
    ----------
    mode : {`'stdout'`, `'dicts'`***REMOVED***, optional.
    ...
    if mode == DisplayModes.stdout.value:
        ...
        raise AttributeError(
            f"Invalid `mode`, use one of: {', '.join([e.value for e in DisplayModes])***REMOVED***"
        )
