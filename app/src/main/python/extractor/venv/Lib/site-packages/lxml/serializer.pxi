...
cdef int _findOutputMethod(method) except -1:
    ...
    raise ValueError(f"unknown output method {method!r***REMOVED***")
...
cdef _tostring(_Element element, encoding, doctype, method,
    ...
    if enchandler is NULL and c_enc is not NULL:
        ...
        raise LookupError, f"unknown encoding: '{encoding***REMOVED***'"
...
cdef _raiseSerialisationError(int error_result):
    ...
    if message is None:
        message = f"unknown error {error_result***REMOVED***"
    ...
    while cur[0] != 0:
        if cur[0] == b'\n':
            ...
            if (l == 1) or (not tree.xmlIsCharQ(val)):
                raise ValueError(f"Invalid character: {val:X***REMOVED***")
...
cdef _FilelikeWriter _create_output_buffer(
    ...
    if enchandler is NULL:
        raise LookupError(
            f"unknown encoding: '{c_enc.decode('UTF-8') if c_enc is not NULL else u''***REMOVED***'")
    try:
        f = _getFSPathOrObject(f)
        if _isString(f):
            filename8 = _encodeFilename(f)
            ...
            raise TypeError(
                f"File or filename expected, got '{python._fqtypename(f).decode('UTF-8')***REMOVED***'")
    except:
        tree.xmlCharEncCloseFunc(enchandler)
        raise
...
cdef _tofilelikeC14N(f, _Element element, bint exclusive, bint with_comments,
    ...
    try:
        ...
        if _isString(f):
            ...
            raise TypeError(f"File or filename expected, got '{python._fqtypename(f).decode('UTF-8')***REMOVED***'")
    finally:
...
cdef class C14NWriterTarget:
    ...
    - *rewrite_prefixes*: set to true to replace namespace prefixes by "n{number***REMOVED***"
    ...
    cdef _resolve_prefix_name(self, prefixed_name):
        ...
        for uri, p in self._iter_namespaces(self._ns_stack):
            if p == prefix:
                return f'{{{uri***REMOVED******REMOVED******REMOVED***{name***REMOVED***'
        raise ValueError(f'Prefix {prefix***REMOVED*** of QName "{prefixed_name***REMOVED***" is not declared in scope')

    cdef _qname(self, qname, uri=None):
        if uri is None:
            uri, tag = qname[1:].rsplit('***REMOVED***', 1) if qname[:1] == '{' else ('', qname)
        else:
            tag = qname
        ...
        for u, prefix in self._iter_namespaces(self._declared_ns_stack):
            if u == uri and prefix not in prefixes_seen:
                return f'{prefix***REMOVED***:{tag***REMOVED***' if prefix else tag, tag, uri
        ...
        if self._rewrite_prefixes:
            if uri in self._prefix_map:
                ...
                prefix = self._prefix_map[uri] = f'n{len(self._prefix_map)***REMOVED***'
            self._declared_ns_stack[-1].append((uri, prefix))
            return f'{prefix***REMOVED***:{tag***REMOVED***', tag, uri
        ...
        raise ValueError(f'Namespace "{uri***REMOVED***" of name "{tag***REMOVED***" is not declared in scope')
    ...
    cdef _start(self, tag, attrs, new_namespaces, qname_text=None):
        if self._exclude_attrs is not None and attrs:
            attrs = {k: v for k, v in attrs.items() if k not in self._exclude_attrs***REMOVED***

        qnames = {tag, *attrs***REMOVED***
        resolved_names = {***REMOVED***
        ...
        parsed_qnames = {n: self._qname(n) for n in sorted(
            qnames, key=lambda n: n.split('***REMOVED***', 1))***REMOVED***
        ...
        space_behaviour = attrs.get('{http://www.w3.org/XML/1998/namespace***REMOVED***space')
        ...
        if attr_list:
            write(''.join([f' {k***REMOVED***="{_escape_attrib_c14n(v)***REMOVED***"' for k, v in attr_list]))
        write('>')
    ...
    def end(self, tag):
        ...
        if self._data:
            self._flush()
        self._write(f'</{self._qname(tag)[0]***REMOVED***>')
    ...
    def comment(self, text):
        ...
        if self._root_done:
            ...
            self._flush()
        self._write(f'<!--{_escape_cdata_c14n(text)***REMOVED***-->')
    ...
    def pi(self, target, data):
        ...
        if self._root_done:
            ...
            self._flush()
        self._write(
            f'<?{target***REMOVED*** {_escape_cdata_c14n(data)***REMOVED***?>' if data else f'<?{target***REMOVED***?>')
        if not self._root_seen:
            self._write('\n')
    ...
    def write(self, *args, bint with_tail=True, bint pretty_print=False, method=None):
        ...
        for content in args:
            if _isString(content):
                ...
                raise TypeError(
                    f"got invalid input value of type {type(content)***REMOVED***, expected string or Element")
            self._handle_error(self._c_out.error)
        if not self._buffered:
            tree.xmlOutputBufferFlush(self._c_out)
            self._handle_error(self._c_out.error)
