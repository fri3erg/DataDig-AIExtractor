...
***REMOVED***
    ...
    def _create_rspace_client(self) -> Any:
        ...
        try:
            ...
            raise Exception(
                f"Unable to initialize client - is url {self.url***REMOVED*** or "
                f"api key  correct?"
***REMOVED***
    ...
    def _get_doc(self, cli: Any, field_content: Any, d_id: Union[str, int]) -> Document:
        ...
        content += f"<h2>{doc['name']***REMOVED***<h2/>"
        for f in doc["fields"]:
            content += f"{f['name']***REMOVED***\n"
            fc = field_content(f["content"])
            content += fc.get_text()
            content += "\n"
        return Document(
            metadata={"source": f"rspace: {doc['name']***REMOVED***-{doc['globalId']***REMOVED***"***REMOVED***,
...
***REMOVED***
    ...
    def _load_pdf(self) -> Iterator[Document]:
        ...
        if ext.lower() == ".pdf":
            outfile = f"{self.global_id***REMOVED***.pdf"
