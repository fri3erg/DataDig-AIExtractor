...
def test_read_jsonl_engine_pyarrow(datapath, engine):
    ...
    expected = DataFrame({"a": [1, 3, 5], "b": [2, 4, 6]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_readjson_chunks_closes(chunksize):
    with tm.ensure_clean("test.json") as path:
        ...
        ), f"didn't close stream with chunksize = {chunksize***REMOVED***"
...
def test_readjson_chunks_multiple_empty_lines(chunksize):
    ...
    {"A":3,"B":6***REMOVED***
    """
    orig = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]***REMOVED***)
    test = read_json(StringIO(j), lines=True, chunksize=chunksize)
    if chunksize is not None:
        with test:
            test = pd.concat(test)
    tm.assert_frame_equal(orig, test, obj=f"chunksize: {chunksize***REMOVED***")
...
def test_readjson_unicode(request, monkeypatch, engine):
    ...
    with tm.ensure_clean("test.json") as path:
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write('{"£©µÀÆÖÞßéöÿ":["АБВГДабвгд가"]***REMOVED***')

        result = read_json(path, engine=engine)
        expected = DataFrame({"£©µÀÆÖÞßéöÿ": ["АБВГДабвгд가"]***REMOVED***)
        tm.assert_frame_equal(result, expected)
...
def test_readjson_nrows(nrows, engine):
    ...
    expected = DataFrame({"a": [1, 3, 5, 7], "b": [2, 4, 6, 8]***REMOVED***).iloc[:nrows]
    tm.assert_frame_equal(result, expected)
...
def test_to_json_append_orient(orient_):
    ...
    df = DataFrame({"col1": [1, 2], "col2": ["a", "b"]***REMOVED***)
    msg = (
...
def test_to_json_append_mode(mode_):
    ...
    df = DataFrame({"col1": [1, 2], "col2": ["a", "b"]***REMOVED***)
    msg = (
        f"mode={mode_***REMOVED*** is not a valid option."
...
def test_to_json_append_output_consistent_columns():
    ...
    df1 = DataFrame({"col1": [1, 2], "col2": ["a", "b"]***REMOVED***)
    df2 = DataFrame({"col1": [3, 4], "col2": ["c", "d"]***REMOVED***)

    expected = DataFrame({"col1": [1, 2, 3, 4], "col2": ["a", "b", "c", "d"]***REMOVED***)
    with tm.ensure_clean("test.json") as path:
...
def test_to_json_append_output_inconsistent_columns():
    ...
    df1 = DataFrame({"col1": [1, 2], "col2": ["a", "b"]***REMOVED***)
    df3 = DataFrame({"col2": ["e", "f"], "col3": ["!", "#"]***REMOVED***)
...
def test_to_json_append_output_different_columns():
    ...
    df2 = DataFrame({"col1": [3, 4], "col2": ["c", "d"]***REMOVED***)
    df3 = DataFrame({"col2": ["e", "f"], "col3": ["!", "#"]***REMOVED***)
    df4 = DataFrame({"col4": [True, False]***REMOVED***)
