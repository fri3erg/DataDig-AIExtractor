...
class _DataEssentialsAndFunctions:
    def __init__(
        ...
        self.node_indices = {u: i for i, u in enumerate(self.node_list)***REMOVED***  # node indices
        self.node_demands = [
        ...
        self.edge_indices = {***REMOVED***  # edge indices
...
@nx._dispatch(node_attrs="demand", edge_attrs={"capacity": float("inf"), "weight": 0***REMOVED***)
def network_simplex(G, demand="demand", capacity="capacity", weight="weight"):
    ...
    24
    >>> flowDict
    {'a': {'b': 4, 'c': 1***REMOVED***, 'd': {***REMOVED***, 'b': {'d': 4***REMOVED***, 'c': {'d': 1***REMOVED******REMOVED***
    ...
    37
    >>> flowDict
    {'p': {'q': 2, 'a': 2***REMOVED***, 'q': {'d': 1***REMOVED***, 'a': {'t': 4***REMOVED***, 'd': {'w': 2***REMOVED***, 't': {'q': 1, 'w': 1***REMOVED***, 'w': {***REMOVED******REMOVED***
    ...
    for u, d in zip(DEAF.node_list, DEAF.node_demands):
        if abs(d) == inf:
            raise nx.NetworkXError(f"node {u!r***REMOVED*** has infinite demand")
    for e, w in zip(DEAF.edge_indices, DEAF.edge_weights):
        if abs(w) == inf:
            raise nx.NetworkXError(f"edge {e!r***REMOVED*** has infinite weight")
    if not multigraph:
    ...
    for e in edges:
        if abs(e[-1].get(weight, 0)) == inf:
            raise nx.NetworkXError(f"edge {e[:-1]!r***REMOVED*** has infinite weight")
    ...
    for e, c in zip(DEAF.edge_indices, DEAF.edge_capacities):
        if c < 0:
            raise nx.NetworkXUnfeasible(f"edge {e!r***REMOVED*** has negative capacity")
    if not multigraph:
        edges = nx.selfloop_edges(G, data=True)
    else:
        edges = nx.selfloop_edges(G, data=True, keys=True)
    for e in edges:
        if e[-1].get(capacity, inf) < 0:
            raise nx.NetworkXUnfeasible(f"edge {e[:-1]!r***REMOVED*** has negative capacity")
...
***REMOVED***
    ...
    flow_dict = {n: {***REMOVED*** for n in DEAF.node_list***REMOVED***
