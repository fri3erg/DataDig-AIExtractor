...
class TestSelection:
    ...
    def test_getitem_from_grouper(self, func):
        # GH 50383
        df = DataFrame({"a": [1, 1, 2], "b": 3, "c": 4, "d": 5***REMOVED***)
        gb = df.groupby(["a", "b"])[["a", "c"]]

        idx = MultiIndex.from_tuples([(1, 3), (2, 3)], names=["a", "b"])
        expected = DataFrame({"a": [2, 2], "c": [8, 4]***REMOVED***, index=idx)
        result = func(gb)
...
class TestGrouping:
    ...
    def test_grouper_creation_bug(self):
        # GH 8795
        df = DataFrame({"A": [0, 0, 1, 1, 2, 2], "B": [1, 2, 3, 4, 5, 6]***REMOVED***)
        g = df.groupby("A")
        expected = g.sum()
    ...
    def test_grouper_creation_bug2(self):
        ...
        df = DataFrame(
            {"A": [0, 0, 0, 1, 1, 1], "B": [1, 1, 2, 2, 3, 3], "C": [1, 2, 3, 4, 5, 6]***REMOVED***
        )
    ...
    def test_grouper_returning_tuples(self, func):
        ...
        df = DataFrame({"X": ["A", "B", "A", "B"], "Y": [1, 4, 3, 2]***REMOVED***)
    ...
    def test_grouper_column_and_index(self):
        ...
        df_multi = DataFrame(
            {"A": np.arange(6), "B": ["one", "one", "two", "two", "one", "one"]***REMOVED***,
            index=idx,
        )
...
***REMOVED***,
    ...
    def test_groupby_dict_mapping(self):
        # GH #679
        s = Series({"T1": 5***REMOVED***)
        result = s.groupby({"T1": "T2"***REMOVED***).agg("sum")
    ...
    def test_multifunc_select_col_integer_cols(self, df):
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            df.groupby(1, as_index=False)[2].agg({"Q": np.mean***REMOVED***)
...
***REMOVED***,
    ...
    def test_groupby_level_index_names(self, axis):
        ...
        df = DataFrame({"exp": ["A"] * 3 + ["B"] * 3, "var1": range(6)***REMOVED***).set_index(
            "exp"
        )
        ...
        msg = f"level name foo is not the name of the {df._get_axis_name(axis)***REMOVED***"
        with pytest.raises(ValueError, match=msg):
            with tm.assert_produces_warning(FutureWarning, match=depr_msg):
    ...
    def test_list_grouper_with_nat(self):
        # GH 14715
        df = DataFrame({"date": date_range("1/1/2011", periods=365, freq="D")***REMOVED***)
        df.iloc[-1] = pd.NaT
        grouper = Grouper(key="date", freq="YS")

        # Grouper in a list grouping
        result = df.groupby([grouper])
        expected = {Timestamp("2011-01-01"): Index(list(range(364)))***REMOVED***
        tm.assert_dict_equal(result.groups, expected)

        # Test case without a list
        result = df.groupby(grouper)
        expected = {Timestamp("2011-01-01"): 365***REMOVED***
        tm.assert_dict_equal(result.groups, expected)
...
class TestGetGroup:
    ...
    ***REMOVED***,
    ...
    def test_get_group_grouped_by_tuple(self):
        ...
        gr = df.groupby("ids")
        expected = DataFrame({"ids": [(1,), (1,)]***REMOVED***, index=[0, 2])
        result = gr.get_group((1,))
        tm.assert_frame_equal(result, expected)

        dt = pd.to_datetime(["2010-01-01", "2010-01-02", "2010-01-01", "2010-01-02"])
        df = DataFrame({"ids": [(x,) for x in dt]***REMOVED***)
        gr = df.groupby("ids")
        result = gr.get_group(("2010-01-01",))
        expected = DataFrame({"ids": [(dt[0],), (dt[0],)]***REMOVED***, index=[0, 2])
        tm.assert_frame_equal(result, expected)
    ...
    ***REMOVED***
    ...
    def test_groupby_with_single_column(self):
        df = DataFrame({"a": list("abssbab")***REMOVED***)
...
class TestIteration:
    ...
    def test_multi_iter_frame(self, three_group):
        ...
        groups = {key: gp for key, gp in grouped***REMOVED***  # noqa: C416
        assert len(groups) == 2
    ...
    def test_groupby_with_small_elem(self):
        ...
        # length=2
        df = DataFrame(
            {"event": ["start", "start"], "change": [1234, 5678]***REMOVED***,
        ...
        df = DataFrame(
            {"event": ["start", "start", "start"], "change": [1234, 5678, 9123]***REMOVED***,
...
def test_grouping_by_key_is_in_axis():
    ...
    df = DataFrame({"a": [1, 1, 2], "b": [1, 1, 2], "c": [3, 4, 5]***REMOVED***).set_index("a")
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = gb.sum()
    expected = DataFrame({"b": [1, 2], "c": [7, 5]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_depr_grouping_attrs(attr):
    ...
    gb = df.groupby("a")
    msg = f"{attr***REMOVED*** is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
