...
_version_error = (
    "Version of given Stata file is {version***REMOVED***. pandas supports importing "
...
filepath_or_buffer : str, path object or file-like object
    ...
    or ``StringIO``.
{_statafile_processing_params1***REMOVED***
{_statafile_processing_params2***REMOVED***
{_chunksize_params***REMOVED***
{_iterator_params***REMOVED***
{_shared_docs["decompression_options"] % "filepath_or_buffer"***REMOVED***
{_shared_docs["storage_options"]***REMOVED***
...
...                     'speed': [350, 18, 361, 15]***REMOVED******REMOVED***)  # doctest: +SKIP
...
{_statafile_processing_params2***REMOVED***
{_chunksize_params***REMOVED***
{_shared_docs["decompression_options"]***REMOVED***
{_shared_docs["storage_options"]***REMOVED***
...
def _stata_elapsed_date_to_datetime_vec(dates: Series, fmt: str) -> Series:
    ...
    # 52nd week may have more than 7 days
        ...
        raise ValueError(f"Date fmt {fmt***REMOVED*** not understood")
...
***REMOVED***
    ...
    if fmt in ["%tc", "tc"]:
        ...
        raise ValueError(f"Format {fmt***REMOVED*** is not a known Stata date format")
...
characters.  Column '{0***REMOVED***' does not satisfy this restriction. Use the
'version=117' parameter to write the newer (Stata 13 and later) format.
"""


precision_loss_doc: Final = """
Column converted from {0***REMOVED*** to {1***REMOVED***, and some data are outside of the lossless
conversion range. This may result in a loss of precision in the saved data.
"""


value_label_mismatch_doc: Final = """
Stata value labels (pandas categories) must be strings. Column {0***REMOVED*** contains
...
***REMOVED***:
            ...
            if np.isinf(data[col]).any():
                raise ValueError(
                    f"Column {col***REMOVED*** contains infinity or -infinity"
    ...
    ***REMOVED***
            ...
            if dtype == np.float32 and value > float32_max:
                ...
                if value > float64_max:
                    raise ValueError(
                        f"Column {col***REMOVED*** has a maximum value ({value***REMOVED***) outside the range "
                        f"supported by Stata ({float64_max***REMOVED***)"
        ***REMOVED***
        if is_nullable_int:
            if orig_missing.any():
...
class StataValueLabel:
    ...
    encoding : {"latin-1", "utf-8"***REMOVED***
...
class StataMissingValue:
    ...
    MISSING_VALUES: dict[float, str] = {***REMOVED***
    ...
    ***REMOVED***
    ...
    def __repr__(self) -> str:
        return f"{type(self)***REMOVED***({self***REMOVED***)"
...
class StataParser:
    def __init__(self) -> None:
        ...
        self.DTYPE_MAP = dict(
            [(i, np.dtype(f"S{i***REMOVED***")) for i in range(1, 245)]
            + [
...
***REMOVED***
    ...
    def _read_uint16(self) -> int:
        return struct.unpack(f"{self._byteorder***REMOVED***H", self._path_or_buf.read(2))[0]

    def _read_uint32(self) -> int:
        return struct.unpack(f"{self._byteorder***REMOVED***I", self._path_or_buf.read(4))[0]

    def _read_uint64(self) -> int:
        return struct.unpack(f"{self._byteorder***REMOVED***Q", self._path_or_buf.read(8))[0]
    ...
    def _read_int16_count(self, count: int) -> tuple[int, ...]:
        return struct.unpack(
            f"{self._byteorder***REMOVED***{'h' * count***REMOVED***",
        ...
        for _ in range(self._nvar):
            ...
            if typ <= 2045:
                ...
                try:
                    ...
                    raise ValueError(f"cannot convert stata types [{typ***REMOVED***]") from err
    ...
    def _read_old_header(self, first_char: bytes) -> None:
        ...
        try:
            ...
            raise ValueError(f"cannot convert stata types [{invalid_types***REMOVED***]") from err
        try:
            self._dtyplist = [self.DTYPE_MAP[typ] for typ in typlist]
        except ValueError as err:
            invalid_dtypes = ",".join([str(x) for x in typlist])
            raise ValueError(f"cannot convert stata dtypes [{invalid_dtypes***REMOVED***]") from err
    ...
    def _setup_dtype(self) -> np.dtype:
        ...
        for i, typ in enumerate(self._typlist):
            if typ in self.NUMPY_TYPE_MAP:
                ...
                dtypes.append((f"s{i***REMOVED***", f"{self._byteorder***REMOVED***{self.NUMPY_TYPE_MAP[typ]***REMOVED***"))
            else:
                dtypes.append((f"s{i***REMOVED***", f"S{typ***REMOVED***"))
    ...
    def _decode(self, s: bytes) -> str:
        ...
        try:
            ...
            encoding = self._encoding
            msg = f"""
One or more strings in the dta file could not be decoded using {encoding***REMOVED***, and
...
***REMOVED***
    ...
    def _read_value_labels(self) -> None:
        ...
        if self._format_version <= 108:
            ...
            self._value_label_dict: dict[str, dict[float, str]] = {***REMOVED***
            return
        ...
        self._value_labels_read = True
        self._value_label_dict = {***REMOVED***
        ...
        while True:
            ...
            off = np.frombuffer(
                self._path_or_buf.read(4 * n), dtype=f"{self._byteorder***REMOVED***i4", count=n
***REMOVED***
            val = np.frombuffer(
                self._path_or_buf.read(4 * n), dtype=f"{self._byteorder***REMOVED***i4", count=n
***REMOVED***
            ...
            self._value_label_dict[labname] = {***REMOVED***
            for i in range(n):
...
***REMOVED***  # set attr instead of set_index to avoid copy
...
***REMOVED***
        ...
        if not self._column_selector_set:
            ...
            if unmatched:
                ...
                raise ValueError(
                    ...
                    f"found in the Stata data set: {joined***REMOVED***"
    ***REMOVED***
    ...
    ***REMOVED***
                ...
                try:
                    ...
                    # GH 25772
                    msg = f"""
Value labels for column {col***REMOVED*** are not unique. These cannot be converted to
pandas categoricals.
...
def _set_endianness(endianness: str) -> str:
    if endianness.lower() in ["<", "little"]:
        ...
        raise ValueError(f"Endianness {endianness***REMOVED*** not understood")
...
def _convert_datetime_to_stata_type(fmt: str) -> np.dtype:
    ...
    if fmt in [
        ...
        raise NotImplementedError(f"Format {fmt***REMOVED*** not implemented")
...
def _maybe_convert_to_int_keys(convert_dates: dict, varlist: list[Hashable]) -> dict:
    ...
    for key in convert_dates:
        ...
        if key in varlist:
            new_dict.update({varlist.index(key): convert_dates[key]***REMOVED***)
        else:
            if not isinstance(key, int):
                raise ValueError("convert_dates key must be a column or an integer")
            new_dict.update({key: convert_dates[key]***REMOVED***)
    return new_dict
...
def _dtype_to_stata_type(dtype: np.dtype, column: Series) -> int:
    ...
    if dtype.type is np.object_:  # try to coerce it to the biggest string
        ...
        raise NotImplementedError(f"Data type {dtype***REMOVED*** not supported.")
...
class StataWriter(StataParser):
    ...
    >>> compression = {{"method": "zip", "archive_name": "data_file.dta"***REMOVED******REMOVED***
    ...
    >>> writer = StataWriter('./date_data_file.dta', data, {{'date' : 'tw'***REMOVED******REMOVED***)
    >>> writer.write_file()
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.data = data
        self._convert_dates = {***REMOVED*** if convert_dates is None else convert_dates
        ...
        self._converted_names: dict[Hashable, str] = {***REMOVED***
        ...
        self._fname = fname
        self.type_converters = {253: np.int32, 252: np.int16, 251: np.int8***REMOVED***
    ...
    def _prepare_non_cat_value_labels(
        ...
        for labname, labels in self._non_cat_value_labels.items():
            if labname in self._converted_names:
                ...
                raise KeyError(
                    f"Can't create value labels for {labname***REMOVED***, it wasn't "
    ...
    ***REMOVED***
            ...
            if not is_numeric_dtype(data[colname].dtype):
                ...
                raise ValueError(
                    f"Can't create value labels for {labname***REMOVED***, value labels "
...
***REMOVED***:
    ...
    def _check_column_names(self, data: DataFrame) -> DataFrame:
        ...
        """
        converted_names: dict[Hashable, str] = {***REMOVED***
        ...
        if converted_names:
            ...
            for orig_name, name in converted_names.items():
                msg = f"{orig_name***REMOVED***   ->   {name***REMOVED***"
        ...
        for i, col in enumerate(self.data):
            ...
            if dtype.type is np.object_:
                ...
                if not ((inferred_dtype == "string") or len(column) == 0):
                    ...
                    raise ValueError(
                        f"""\
Column `{col***REMOVED***` cannot be exported.\n\nOnly string-like object arrays
...
supported types."""
    ...
    ***REMOVED***:
    ...
    def write_file(self) -> None:
        ...
        ...                    ***REMOVED***)
        >>> path = "/My_path/filename.dta"
        >>> labels = {"fully_labelled": {1: "one", 2: "two", 3: "three"***REMOVED***,
        ...           "partially_labelled": {1.0: "one", 2.0: "two"***REMOVED***,
        ...           ***REMOVED***
    ...
    ***REMOVED***:
                    try:
                        ...
                        warnings.warn(
                            f"This save was not successful but {self._fname***REMOVED*** could not "
...
***REMOVED***
        ...
        month_lookup = {i + 1: month for i, month in enumerate(months)***REMOVED***
        ts = (
...
class StataWriter117(StataWriter):
    ...
    Directly write a zip file
    >>> compression = {"method": "zip", "archive_name": "data_file.dta"***REMOVED***
    ...
    def __init__(
        ...
        *,
        ...
        self._map: dict[str, int] = {***REMOVED***
        self._strl_blob = b""
        ...
        for col in self.data:
            if col in self._variable_labels:
                ...
                try:
                    ...
                    raise ValueError(
                        "Variable labels must contain only characters that "
                        f"can be encoded in {self._encoding***REMOVED***"
        ***REMOVED*** from err
