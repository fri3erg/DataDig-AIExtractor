...
"""
    ...
    with pytest.raises(TypeError, match=msg):
        read_fwf(StringIO(data), colspecs={"a": 1***REMOVED***, delimiter=",")
...
def test_fwf_regression():
    ...
    names = ["SST"] + [f"T{z:03d***REMOVED***" for z in tz_list[1:]]
...
""".strip(
    ...
    encoding = "utf8"
    kwargs = {"header": None, "encoding": encoding***REMOVED***
...
@pytest.mark.parametrize("dtype", [{***REMOVED***, {"a": "float64", "b": str, "c": "int32"***REMOVED***])
...
"""
    ...
    expected = DataFrame(
        {"a": [1, 3], "b": [2, 4], "c": [3.2, 5.2]***REMOVED***, columns=["a", "b", "c"]
    )
...
def test_fwf_compression(compression_only, infer, compression_to_extension):
    ...
    kwargs = {"widths": [5, 5], "names": ["one", "two"]***REMOVED***
...
6\t f
    ...
    expected = DataFrame({"a": [1, 3, 5, 6], "b": ["a", "c", "e", "f"]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
9
    ...
    expected_frames = [
        DataFrame({"a": [3, 4]***REMOVED***),
        DataFrame({"a": [5, 7]***REMOVED***, index=[2, 3]),
        DataFrame({"a": [8]***REMOVED***, index=[4]),
    ]
...
def test_names_and_infer_colspecs():
    ...
    expected = DataFrame({"a": [959.0], "b": 22.2***REMOVED***)
    tm.assert_frame_equal(result, expected)
