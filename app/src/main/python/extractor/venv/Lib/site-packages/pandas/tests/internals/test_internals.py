...
def create_block(typestr, placement, item_shape=None, num_offset=0, maker=new_block):
    ...
    if typestr in (
        ...
        values = np.reshape([f"A{i:d***REMOVED***" for i in mat.ravel() + num_offset], shape)
        ...
        assert m is not None, f"incompatible typestr -> {typestr***REMOVED***"
        tz = m.groups()[0]
        ...
        raise ValueError(f'Unsupported typestr: "{typestr***REMOVED***"')
...
class TestBlockManager:
    ...
    def test_interleave(self):
        ...
        for dtype in ["f8", "i8", "object", "bool", "complex", "M8[ns]", "m8[ns]"]:
            mgr = create_mgr(f"a: {dtype***REMOVED***")
            assert mgr.as_array().dtype == dtype
            mgr = create_mgr(f"a: {dtype***REMOVED***; b: {dtype***REMOVED***")
            assert mgr.as_array().dtype == dtype
