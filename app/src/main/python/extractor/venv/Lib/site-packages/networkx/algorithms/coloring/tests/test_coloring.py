...
***REMOVED***
    ...
    def test_cast_no_solo(self):
        ...
        ***REMOVED***
        ...
        F = {0: 0, 1: 0, 2: 2, 3: 2, 4: 2, 5: 3, 6: 3, 7: 3, 8: 1, 9: 1, 10: 1, 11: 1***REMOVED***
    ...
    def test_hard_prob(self):
        ...
        F = {node: node // s for node in range(num_colors * s)***REMOVED***
        F[s - 1] = num_colors - 1
...
def make_params_from_graph(G, F):
    """Returns {N, L, H, C***REMOVED*** from the given graph."""
    num_nodes = len(G)
    L = {u: [] for u in range(num_nodes)***REMOVED***
    for u, v in G.edges:
        L[u].append(v)
        L[v].append(u)
    ...
    return {"N": N, "F": F, "C": C, "H": H, "L": L***REMOVED***
