...
def test_size_axis_1(df, axis_1, by, sort, dropna):
    # GH#45715
    counts = {key: sum(value == key for value in by) for key in dict.fromkeys(by)***REMOVED***
    if dropna:
        counts = {key: value for key, value in counts.items() if key is not None***REMOVED***
...
def test_size_groupby_all_null():
    ...
    df = DataFrame({"A": [None, None]***REMOVED***)  # all-null groups
...
def test_size_strings(dtype):
    # GH#55627
    df = DataFrame({"a": ["a", "a", "b"], "b": "a"***REMOVED***, dtype=dtype)
