...
- the set of states.
- the transition probabilities *a_{ij***REMOVED*** = P(s_t = j | s_{t-1***REMOVED*** = i)*. These
...
***REMOVED***
            for t in range(1, T):
                ...
                p += self._transitions[last_state].logprob(
                    state
    ***REMOVED*** + self._output_logprob(state, sequence[t][_TEXT])
                last_state = state
            return p
        else:
...
***REMOVED***
    ...
    def entropy(self, unlabeled_sequence):
        ...
        *Z = Pr(O) = sum_S Pr(S, O)***REMOVED**** where the summation ranges over all state
        sequences and O is the observation sequence. As such the entropy can
        be re-expressed as::

            H = - sum_S Pr(S | O) log [ Pr(S, O) / Z ]
            = log Z - sum_S Pr(S | O) log Pr(S, 0)
            = log Z - sum_S Pr(S | O) [ log Pr(S_0) + sum_t Pr(S_t | S_{t-1***REMOVED***) + sum_t Pr(O_t | S_t) ]
    ...
    def test(self, test_sequence, verbose=False, **kwargs):
        ...
        if verbose:
            for test_sent, predicted_sent in zip(test_sequence, predicted_sequence):
                print(
                    "Test:",
                    " ".join(f"{token***REMOVED***/{tag***REMOVED***" for (token, tag) in test_sent),
    ***REMOVED***
                print()
                ...
                print(
                    "HMM-tagged:",
                    " ".join(f"{token***REMOVED***/{tag***REMOVED***" for (token, tag) in predicted_sent),
    ***REMOVED***
                print()
                print(
                    "Entropy:",
    ...
    def train_unsupervised(self, unlabeled_sequences, update_outputs=True, **kwargs):
        ...
        if not model:
            ...
            transitions = DictionaryConditionalProbDist(
                {state: RandomProbDist(self._states) for state in self._states***REMOVED***
***REMOVED***
            outputs = DictionaryConditionalProbDist(
                {state: RandomProbDist(self._symbols) for state in self._states***REMOVED***
***REMOVED***
...
***REMOVED***
        ...
        N = len(self._states)
        M = len(self._symbols)
        symbol_numbers = {sym: i for i, sym in enumerate(self._symbols)***REMOVED***
        ...
        while not converged and iteration < max_iterations:
            ...
            for sequence in unlabeled_sequences:
                ...
                (
                    ...
                    seq_B_numer,
                    seq_B_denom,
    ***REMOVED*** = self._baum_welch_step(sequence, model, symbol_numbers)
