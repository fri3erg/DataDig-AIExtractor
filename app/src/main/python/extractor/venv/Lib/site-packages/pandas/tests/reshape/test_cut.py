...
@pytest.mark.parametrize(
    "kwargs,msg",
    [
        ({"duplicates": "drop"***REMOVED***, None),
        ({***REMOVED***, "Bin edges must be unique"),
        ({"duplicates": "raise"***REMOVED***, "Bin edges must be unique"),
        ({"duplicates": "foo"***REMOVED***, "invalid value for 'duplicates' parameter"),
    ],
)
...
def test_datetime_cut(unit, box):
    ...
    data = to_datetime(["2013-01-01", "2013-01-02", "2013-01-03"]).astype(f"M8[{unit***REMOVED***]")
    data = box(data)
...
def test_cut_no_warnings():
    df = DataFrame({"value": np.random.default_rng(2).integers(0, 100, 20)***REMOVED***)
    labels = [f"{i***REMOVED*** - {i + 9***REMOVED***" for i in range(0, 100, 10)]
