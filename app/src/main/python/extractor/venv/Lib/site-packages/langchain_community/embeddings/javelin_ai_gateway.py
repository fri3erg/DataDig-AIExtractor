    ...
    ***REMOVED***
    ...
    def _query(self, texts: List[str]) -> List[List[float]]:
        ...
        for txt in _chunk(texts, 20):
            try:
                resp = self.client.query_route(self.route, query_body={"input": txt***REMOVED***)
                resp_dict = resp.dict()

                embeddings_chunk = resp_dict.get("llm_response", {***REMOVED***).get("data", [])
        ...
        for txt in _chunk(texts, 20):
            try:
                resp = await self.client.aquery_route(
                    self.route, query_body={"input": txt***REMOVED***
    ***REMOVED***
                resp_dict = resp.dict()
