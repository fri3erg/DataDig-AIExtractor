...
***REMOVED***
        ...
        if not isinstance(df, DataFrame):
            raise ValueError(
                f"Expected data_frame to be a PySpark DataFrame, got {type(df)***REMOVED***"
***REMOVED***
        self.df = df
...
***REMOVED*** from e
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        for row in self.rdd_df.toLocalIterator():
            metadata = {self.column_names[i]: row[i] for i in range(len(row))***REMOVED***
    ...
    def load(self) -> List[Document]:
        ...
        if self.df.count() > self.max_num_rows:
            logger.warning(
                f"The number of DataFrame rows is {self.df.count()***REMOVED***, "
                f"but we will only include the amount "
                f"of rows that can reasonably fit in memory: {self.num_rows***REMOVED***."
***REMOVED***
