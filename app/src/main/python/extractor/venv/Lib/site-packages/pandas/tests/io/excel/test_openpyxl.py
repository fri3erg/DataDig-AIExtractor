...
def test_to_excel_styleconverter():
    ...
    hstyle = {
        "font": {"color": "00FF0000", "bold": True***REMOVED***,
        "borders": {"top": "thin", "right": "thin", "bottom": "thin", "left": "thin"***REMOVED***,
        "alignment": {"horizontal": "center", "vertical": "top"***REMOVED***,
        "fill": {"patternType": "solid", "fgColor": {"rgb": "006666FF", "tint": 0.3***REMOVED******REMOVED***,
        "number_format": {"format_code": "0.00"***REMOVED***,
        "protection": {"locked": True, "hidden": False***REMOVED***,
    ***REMOVED***
...
def test_write_cells_merge_styled(ext):
    ...
    sty_b1 = {"font": {"color": "00FF0000"***REMOVED******REMOVED***
    sty_a2 = {"font": {"color": "0000FF00"***REMOVED******REMOVED***
    ...
    sty_merged = {"font": {"color": "000000FF", "bold": True***REMOVED******REMOVED***
...
def test_engine_kwargs_write(ext, iso_dates):
    # GH 42286 GH 43445
    engine_kwargs = {"iso_dates": iso_dates***REMOVED***
...
***REMOVED***,
            ...
            with ExcelWriter(
                f, engine="openpyxl", mode="a", engine_kwargs={"apple_banana": "fruit"***REMOVED***
***REMOVED*** as writer:
...
def test_engine_kwargs_append_data_only(ext, data_only, expected):
    ...
    with tm.ensure_clean(ext) as f:
        ...
        with ExcelWriter(
            f, engine="openpyxl", mode="a", engine_kwargs={"data_only": data_only***REMOVED***
        ) as writer:
        ...
        assert (
            pd.read_excel(
                ...
                sheet_name="Sheet1",
                engine="openpyxl",
                engine_kwargs={"data_only": data_only***REMOVED***,
***REMOVED***.iloc[0, 1]
            == expected
        )
...
def test_engine_kwargs_append_reader(datapath, ext, kwarg_name, kwarg_value):
    ...
    with contextlib.closing(
        OpenpyxlReader(filename, engine_kwargs={kwarg_name: kwarg_value***REMOVED***)
    ) as reader:
...
def test_if_sheet_exists_append_modes(ext, if_sheet_exists, num_sheets, expected):
    # GH 40230
    df1 = DataFrame({"fruit": ["apple", "banana"]***REMOVED***)
    df2 = DataFrame({"fruit": ["pear"]***REMOVED***)
...
def test_append_overlay_startrow_startcol(ext, startrow, startcol, greeting, goodbye):
    df1 = DataFrame({"greeting": ["hello", "world"], "goodbye": ["goodbye", "people"]***REMOVED***)
    df2 = DataFrame(["poop"])
...
***REMOVED***
        ...
        expected = DataFrame({"greeting": greeting, "goodbye": goodbye***REMOVED***)
...
def test_if_sheet_exists_raises(ext, if_sheet_exists, msg):
    # GH 40230
    df = DataFrame({"fruit": ["pear"]***REMOVED***)
    with tm.ensure_clean(ext) as f:
...
def test_to_excel_with_openpyxl_engine(ext):
    ...
    with tm.ensure_clean(ext) as filename:
        df1 = DataFrame({"A": np.linspace(1, 10, 10)***REMOVED***)
        df2 = DataFrame({"B": np.linspace(1, 20, 10)***REMOVED***)
        df = pd.concat([df1, df2], axis=1)
        styled = df.style.map(
            lambda val: f"color: {'red' if val < 0 else 'black'***REMOVED***"
        ).highlight_max()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (2, {"A": [1, 2, 3], "B": [4, 5, 6], "C": [7, 8, 9]***REMOVED***),
    ],
)
@pytest.mark.parametrize(
...
def test_read_with_bad_dimension(
    ...
    path = datapath("io", "data", "excel", f"{filename***REMOVED***{ext***REMOVED***")
...
def test_read_with_empty_trailing_rows(datapath, ext, read_only):
    # GH 39181
    path = datapath("io", "data", "excel", f"empty_trailing_rows{ext***REMOVED***")
    if read_only is None:
...
def test_read_empty_with_blank_row(datapath, ext, read_only):
    ...
    path = datapath("io", "data", "excel", f"empty_with_blank_row{ext***REMOVED***")
    if read_only is None:
        result = pd.read_excel(path)
    else:
        with contextlib.closing(
...
def test_book_and_sheets_consistent(ext):
    ...
    with tm.ensure_clean(ext) as f:
        with ExcelWriter(f, engine="openpyxl") as writer:
            assert writer.sheets == {***REMOVED***
            sheet = writer.book.create_sheet("test_name", 0)
            assert writer.sheets == {"test_name": sheet***REMOVED***


def test_ints_spelled_with_decimals(datapath, ext):
    # GH 46988 - openpyxl returns this sheet with floats
    path = datapath("io", "data", "excel", f"ints_spelled_with_decimals{ext***REMOVED***")
    result = pd.read_excel(path)
...
def test_read_multiindex_header_no_index_names(datapath, ext):
    # GH#47487
    path = datapath("io", "data", "excel", f"multiindex_no_index_names{ext***REMOVED***")
