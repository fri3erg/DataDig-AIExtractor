...
URLS = r"""			# Capture 1: entire matched URL
  ...
  (?:					# One or more:
    [^\s()<>{***REMOVED***\[\]]+			# Run of non-space, non-()<>{***REMOVED***[]
    |					#   or
  ...
  (?:					# End with:
    ...
    \([^\s]+?\)				# balanced parens, non-recursive: (...)
    |					#   or
    [^\s`!()\[\]{***REMOVED***;:'".,<>?«»“”‘’]	# not a space or one of these punct chars
  )
...
# r'\U0001F3F4[\U000E0000-\U000E007E]{5***REMOVED***\U000E007F'
FLAGS = r"""
  (?:
    [\U0001F1E6-\U0001F1FF]{2***REMOVED***  # all enclosed letter pairs
    |
    # English flag
...
HANG_RE = regex.compile(r"([^a-zA-Z0-9])\1{3,***REMOVED***")
...
HANDLES_RE = regex.compile(
    r"(?<![A-Za-z0-9_!@#\$%&*])@"
    r"(([A-Za-z0-9_]){15***REMOVED***(?!@)|([A-Za-z0-9_]){1,14***REMOVED***(?![A-Za-z0-9_]*@))"
)
...
def reduce_lengthening(text):
    ...
    of length 3.
    """
    pattern = regex.compile(r"(.)\1{2,***REMOVED***")
