...
def test_spss_labelled_num(path_klass, datapath):
    ...
    expected = pd.DataFrame({"VAR00002": "This is one"***REMOVED***, index=[0])
    expected["VAR00002"] = pd.Categorical(expected["VAR00002"])
    tm.assert_frame_equal(df, expected)

    df = pd.read_spss(fname, convert_categoricals=False)
    expected = pd.DataFrame({"VAR00002": 1.0***REMOVED***, index=[0])
    tm.assert_frame_equal(df, expected)
...
def test_spss_labelled_num_na(datapath):
    ...
    df = pd.read_spss(fname, convert_categoricals=True)
    expected = pd.DataFrame({"VAR00002": ["This is one", None]***REMOVED***)
    expected["VAR00002"] = pd.Categorical(expected["VAR00002"])
    tm.assert_frame_equal(df, expected)

    df = pd.read_spss(fname, convert_categoricals=False)
    expected = pd.DataFrame({"VAR00002": [1.0, np.nan]***REMOVED***)
    tm.assert_frame_equal(df, expected)
...
def test_spss_labelled_str(datapath):
    ...
    df = pd.read_spss(fname, convert_categoricals=True)
    expected = pd.DataFrame({"gender": ["Male", "Female"]***REMOVED***)
    expected["gender"] = pd.Categorical(expected["gender"])
    tm.assert_frame_equal(df, expected)

    df = pd.read_spss(fname, convert_categoricals=False)
    expected = pd.DataFrame({"gender": ["M", "F"]***REMOVED***)
    tm.assert_frame_equal(df, expected)
...
def test_spss_umlauts(datapath):
    ...
    expected = pd.DataFrame(
        {"var1": ["the ä umlaut", "the ü umlaut", "the ä umlaut", "the ö umlaut"]***REMOVED***
    )
    expected["var1"] = pd.Categorical(expected["var1"])
    tm.assert_frame_equal(df, expected)

    df = pd.read_spss(fname, convert_categoricals=False)
    expected = pd.DataFrame({"var1": [1.0, 2.0, 1.0, 3.0]***REMOVED***)
    tm.assert_frame_equal(df, expected)
...
def test_spss_umlauts_dtype_backend(datapath, dtype_backend):
    ...
    expected = pd.DataFrame({"var1": [1.0, 2.0, 1.0, 3.0]***REMOVED***, dtype="Int64")
...
def test_spss_metadata(datapath):
    ...
    metadata = {
        ...
        "column_labels": [None],
        "column_names_to_labels": {"VAR00002": None***REMOVED***,
        "file_encoding": "UTF-8",
        "number_columns": 1,
        "number_rows": 1,
        "variable_value_labels": {"VAR00002": {1.0: "This is one"***REMOVED******REMOVED***,
        "value_labels": {"labels0": {1.0: "This is one"***REMOVED******REMOVED***,
        "variable_to_label": {"VAR00002": "labels0"***REMOVED***,
        "notes": [],
        "original_variable_types": {"VAR00002": "F8.0"***REMOVED***,
        "readstat_variable_types": {"VAR00002": "double"***REMOVED***,
        "table_name": None,
        "missing_ranges": {***REMOVED***,
        "missing_user_values": {***REMOVED***,
        "variable_storage_width": {"VAR00002": 8***REMOVED***,
        "variable_display_width": {"VAR00002": 8***REMOVED***,
        "variable_alignment": {"VAR00002": "unknown"***REMOVED***,
        "variable_measure": {"VAR00002": "unknown"***REMOVED***,
        "file_label": None,
