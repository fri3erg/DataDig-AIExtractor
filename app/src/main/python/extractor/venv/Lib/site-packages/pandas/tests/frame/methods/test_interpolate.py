...
class TestDataFrameInterpolate:
    ...
    def test_interp_nan_idx(self):
        df = DataFrame({"A": [1, 2, np.nan, 4], "B": [np.nan, 2, 3, 4]***REMOVED***)
        df = df.set_index("A")
        msg = (
    ...
    def test_interp_various(self):
        ...
        df = DataFrame(
            {"A": [1, 2, np.nan, 4, 5, np.nan, 7], "C": [1, 2, 3, 5, 8, 13, 21]***REMOVED***
        )
        df = df.set_index("C")
        expected = df.copy()
    ...
    def test_interp_axis_names(self, axis_name, axis_number):
        ...
        data = {0: [0, np.nan, 6], 1: [1, np.nan, 7], 2: [2, 5, 8]***REMOVED***
    ...
    def test_interp_leading_nans(self, check_scipy):
        df = DataFrame(
            {"A": [np.nan, np.nan, 0.5, 0.25, 0], "B": [np.nan, -3, -3.5, np.nan, -4]***REMOVED***
        )
        result = df.interpolate()
        expected = df.copy()
    ...
    def test_interp_raise_on_all_object_dtype(self):
        # GH 22985
        df = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]***REMOVED***, dtype="object")
        msg = (
            "Cannot interpolate with all object-dtype columns "
            "in the DataFrame. Try setting at least one "
            "column to a numeric dtype."
    ...
    def test_interp_inplace(self, using_copy_on_write):
        df = DataFrame({"a": [1.0, 2.0, np.nan, 4.0]***REMOVED***)
...
***REMOVED***:
    ...
    def test_interp_inplace_row(self):
        # GH 10395
        result = DataFrame(
            {"a": [1.0, 2.0, 3.0, 4.0], "b": [np.nan, 2.0, 3.0, 4.0], "c": [3, 2, 2, 2]***REMOVED***
        )
    ...
    def test_interp_fillna_methods(
        ...
        msg = f"DataFrame.interpolate with method={method***REMOVED*** is deprecated"
        with tm.assert_produces_warning(FutureWarning, match=msg):
    ...
    def test_interpolate_ea(self, any_int_ea_dtype):
        # GH#55347
        df = DataFrame({"a": [1, None, None, None, 3]***REMOVED***, dtype=any_int_ea_dtype)
        orig = df.copy()
        result = df.interpolate(limit=2)
        expected = DataFrame({"a": [1, 1.5, 2.0, None, 3]***REMOVED***, dtype="Float64")
        tm.assert_frame_equal(result, expected)
    ...
    def test_interpolate_ea_float(self, dtype):
        # GH#55347
        df = DataFrame({"a": [1, None, None, None, 3]***REMOVED***, dtype=dtype)
        orig = df.copy()
        result = df.interpolate(limit=2)
        expected = DataFrame({"a": [1, 1.5, 2.0, None, 3]***REMOVED***, dtype=dtype)
        tm.assert_frame_equal(result, expected)
        tm.assert_frame_equal(df, orig)
    ...
    def test_interpolate_arrow(self, dtype):
        ...
        df = DataFrame({"a": [1, None, None, None, 3]***REMOVED***, dtype=dtype + "[pyarrow]")
        result = df.interpolate(limit=2)
        expected = DataFrame({"a": [1, 1.5, 2.0, None, 3]***REMOVED***, dtype="float64[pyarrow]")
        tm.assert_frame_equal(result, expected)
