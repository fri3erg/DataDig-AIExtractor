...
class TestArrowArray(base.ExtensionTests):
    ...
    def test_astype_str(self, data, request):
        ...
        if pa.types.is_binary(pa_dtype):
            request.applymarker(
                pytest.mark.xfail(
                    reason=f"For {pa_dtype***REMOVED*** .astype(str) decodes.",
    ***REMOVED***
***REMOVED***
        elif (
    ...
    def test_from_dtype(self, data, request):
        ...
        if pa.types.is_string(pa_dtype) or pa.types.is_decimal(pa_dtype):
            if pa.types.is_string(pa_dtype):
                ...
                reason = f"pyarrow.type_for_alias cannot infer {pa_dtype***REMOVED***"
...
***REMOVED***
            ...
            request.applymarker(
                pytest.mark.xfail(
                    ...
                    reason=f"pyarrow doesn't support parsing {pa_dtype***REMOVED***",
    ***REMOVED***
***REMOVED***
...
***REMOVED***
        ...
        if pa_version_under13p0 and all_numeric_accumulations != "cumsum":
            ...
            if opt.markexpr and "not slow" in opt.markexpr:
                pytest.skip(
                    f"{all_numeric_accumulations***REMOVED*** not implemented for pyarrow < 9"
    ***REMOVED***
            mark = pytest.mark.xfail(
                reason=f"{all_numeric_accumulations***REMOVED*** not implemented for pyarrow < 9"
***REMOVED***
            request.applymarker(mark)
            ...
            request.applymarker(
                pytest.mark.xfail(
                    reason=f"{all_numeric_accumulations***REMOVED*** not implemented for {pa_type***REMOVED***",
...
***REMOVED***
    ...
    def test_reduce_series_numeric(self, data, all_numeric_reductions, skipna, request):
        ...
        xfail_mark = pytest.mark.xfail(
            raises=TypeError,
            reason=(
                f"{all_numeric_reductions***REMOVED*** is not implemented in "
                f"pyarrow={pa.__version__***REMOVED*** for {pa_dtype***REMOVED***"
***REMOVED***,
        )
        if all_numeric_reductions in {"skew", "kurt"***REMOVED*** and (
    ...
    def test_reduce_series_boolean(
        ...
        xfail_mark = pytest.mark.xfail(
            raises=TypeError,
            reason=(
                f"{all_boolean_reductions***REMOVED*** is not implemented in "
                f"pyarrow={pa.__version__***REMOVED*** for {pa_dtype***REMOVED***"
***REMOVED***,
        )
    ...
    def test_median_not_approximate(self, typ):
        # GH 52679
        result = pd.Series([1, 2], dtype=f"{typ***REMOVED***[pyarrow]").median()
        assert result == 1.5
...
***REMOVED***
            ...
            expected = pd.Index(["C"])
            msg = re.escape(f"agg function failed [how->sum,dtype->{dtype***REMOVED***")
    ...
    def test_get_common_dtype(self, dtype, request):
        ...
        if (
            ...
            request.applymarker(
                pytest.mark.xfail(
                    reason=(
                        f"{pa_dtype***REMOVED*** does not have associated numpy "
    ...
    def test_EA_types(self, engine, data, dtype_backend, request):
        ...
        if pa.types.is_decimal(pa_dtype):
            request.applymarker(
                pytest.mark.xfail(
                    raises=NotImplementedError,
                    reason=f"Parameterized types {pa_dtype***REMOVED*** not supported.",
    ***REMOVED***
***REMOVED***
...
***REMOVED***
        df = pd.DataFrame({"with_dtype": pd.Series(data, dtype=str(data.dtype))***REMOVED***)
        ...
        result = pd.read_csv(
            csv_output,
            dtype={"with_dtype": str(data.dtype)***REMOVED***,
            engine=engine,
    ...
    def test_invert(self, data, request):
        ...
        if not (
            ...
            request.applymarker(
                pytest.mark.xfail(
                    raises=pa.ArrowNotImplementedError,
                    reason=f"pyarrow.compute.invert does support {pa_dtype***REMOVED***",
    ***REMOVED***
***REMOVED***
        ...
        if pa.types.is_unsigned_integer(pa_dtype) and periods == 1:
            request.applymarker(
                pytest.mark.xfail(
                    raises=pa.ArrowInvalid,
                    reason=(
                        f"diff with {pa_dtype***REMOVED*** and periods={periods***REMOVED*** will overflow"
        ***REMOVED***,
    ***REMOVED***
***REMOVED***
...
***REMOVED***:
        ...
        if pa.types.is_duration(pa_expected.type):
            ...
            pa_expected = pa_expected.cast(f"duration[{unit***REMOVED***]")
...
***REMOVED***
            ...
            mark = pytest.mark.xfail(
                raises=TypeError,
                reason=(
                    f"{opname***REMOVED*** not supported between"
                    f"pd.NA and {pa_dtype***REMOVED*** Python scalar"
    ***REMOVED***,
***REMOVED***
            ...
            request.applymarker(
                pytest.mark.xfail(
                    ...
                    reason=(
                        ...
                        f"which raises on overflow for {pa_dtype***REMOVED***"
        ***REMOVED***,
    ***REMOVED***
***REMOVED***
        elif all_arithmetic_operators in ("__rmul__", "__mul__") and (
    ...
    def test_add_series_with_extension_array(self, data, request):
        ...
        if pa_dtype.equals("int8"):
            request.applymarker(
                pytest.mark.xfail(
                    raises=pa.ArrowInvalid,
                    reason=f"raises on overflow for {pa_dtype***REMOVED***",
    ***REMOVED***
***REMOVED***
    ...
    def test_comp_masked_numpy(self, masked_dtype, comparison_op):
        ...
        ser_pa = pd.Series(data, dtype=f"{masked_dtype.lower()***REMOVED***[pyarrow]")
...
def test_quantile(data, interpolation, quantile, request):
    ...
    if (
        ...
        request.applymarker(
            pytest.mark.xfail(
                raises=pa.ArrowNotImplementedError,
                reason=f"quantile not supported by pyarrow for {pa_dtype***REMOVED***",
***REMOVED***
        )
    data = data.take([0, 0, 0])
    ser = pd.Series(data)
    ...
    if pa.types.is_timestamp(pa_dtype) and interpolation not in ["lower", "higher"]:
        ...
        if pa_dtype.tz:
            pd_dtype = f"M8[{pa_dtype.unit***REMOVED***, {pa_dtype.tz***REMOVED***]"
        else:
            pd_dtype = f"M8[{pa_dtype.unit***REMOVED***]"
...
def test_astype_errors_ignore():
    # GH 55399
    expected = pd.DataFrame({"col": [17000000]***REMOVED***, dtype="int32[pyarrow]")
...
def test_from_arrow_respecting_given_dtype():
    ...
    result = date_array.to_pandas(
        types_mapper={pa.date32(): ArrowDtype(pa.date64())***REMOVED***.get
    )
    expected = pd.Series(
...
def test_from_arrow_respecting_given_dtype_unsafe():
    ...
    with pytest.raises(pa.ArrowInvalid, match="Float value 1.5 was truncated"):
        array.to_pandas(types_mapper={pa.float64(): ArrowDtype(pa.int64())***REMOVED***.get)
...
@pytest.mark.parametrize("pat", ["abc", "a[a-z]{2***REMOVED***"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["a[a-z]{1***REMOVED***", False, None, True, [True, None]],
        ["A[a-z]{1***REMOVED***", True, None, True, [False, None]],
    ],
)
...
def test_str_replace_unsupported(arg_name, arg):
    ser = pd.Series(["abc", None], dtype=ArrowDtype(pa.string()))
    kwargs = {"pat": "b", "repl": "x", "regex": True***REMOVED***
    kwargs[arg_name] = arg
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["a[a-z]{1***REMOVED***", False, None, [True, None]],
        ["A[a-z]{1***REMOVED***", True, None, [False, None]],
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["a[a-z]{2***REMOVED***", False, None, [True, True, False, None]],
        ["A[a-z]{1***REMOVED***", True, None, [False, False, False, None]],
        # GH Issue: #56652
...
def test_str_translate():
    ser = pd.Series(["abcba", None], dtype=ArrowDtype(pa.string()))
    result = ser.str.translate({97: "b"***REMOVED***)
...
def test_dt_ceil_year_floor(freq, method):
    ...
    expected = getattr(ser.dt, method)(f"1{freq***REMOVED***").astype(pa_dtype)
    result = getattr(ser.astype(pa_dtype).dt, method)(f"1{freq***REMOVED***")
    tm.assert_series_equal(result, expected)
...
def test_dt_to_pydatetime_date_error(date_type):
    ...
    ser = pd.Series(
        [date(2022, 12, 31)],
        dtype=ArrowDtype(getattr(pa, f"date{date_type***REMOVED***")()),
    )
    msg = "The behavior of ArrowTemporalProperties.to_pydatetime is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
...
def test_concat_null_array():
    df = pd.DataFrame({"a": [None, None]***REMOVED***, dtype=ArrowDtype(pa.null()))
    df2 = pd.DataFrame({"a": [0, 1]***REMOVED***, dtype="int64[pyarrow]")

    result = pd.concat([df, df2], ignore_index=True)
    expected = pd.DataFrame({"a": [None, None, 0, 1]***REMOVED***, dtype="int64[pyarrow]")
    tm.assert_frame_equal(result, expected)
...
def test_groupby_count_return_arrow_dtype(data_missing):
    df = pd.DataFrame({"A": [1, 1], "B": data_missing, "C": data_missing***REMOVED***)
...
def test_duration_fillna_numpy(pa_type):
    ...
    ser2 = pd.Series(np.array([1, 3], dtype=f"m8[{pa_type.unit***REMOVED***]"))
