...
def _three_digit_exp():
    return f"{1.7e8:.4g***REMOVED***" == "1.7e+008"
...
class TestDataFrameToStringFormatters:
    def test_to_string_masked_ea_with_formatter(self):
        ...
        result = df.to_string(formatters=["{:.2f***REMOVED***".format, "{:.2f***REMOVED***".format])
        expected = dedent(
            """\
                  a     b
            0  0.12  1.00
    ...
    def test_to_string_with_formatters(self):
        ...
        formatters = [
            ("int", lambda x: f"0x{x:x***REMOVED***"),
            ("float", lambda x: f"[{x: 4.1f***REMOVED***]"),
            ("object", lambda x: f"-{x!s***REMOVED***-"),
        ]
    ...
    def test_to_string_with_datetime64_monthformatter(self):
        ...
        x = DataFrame({"months": months***REMOVED***)

        def format_func(x):
            return x.strftime("%Y-%m")

        result = x.to_string(formatters={"months": format_func***REMOVED***)
        expected = dedent(
            """\
            months
            0 2016-01
    ...
    def test_to_string_with_datetime64_hourformatter(self):
        x = DataFrame(
            {"hod": to_datetime(["10:10:10.100", "12:12:12.120"], format="%H:%M:%S.%f")***REMOVED***
        )

        def format_func(x):
            return x.strftime("%H:%M")

        result = x.to_string(formatters={"hod": format_func***REMOVED***)
        expected = dedent(
            """\
            hod
            0 10:10
    ...
    def test_to_string_with_formatters_unicode(self):
        df = DataFrame({"c/\u03c3": [1, 2, 3]***REMOVED***)
        result = df.to_string(formatters={"c/\u03c3": str***REMOVED***)
        expected = dedent(
            """\
              c/\u03c3
            0   1
        ...
        def test_to_string_index_formatter(self):
            ...
            rs = df.to_string(formatters={"__index__": lambda x: "abc"[x]***REMOVED***)
...
***REMOVED***
    ...
    def test_no_extra_space(self):
        ...
        col2 = "PANDAS"
        col3 = "to_string"
        expected = f"{col1:<6s***REMOVED*** {col2:<7s***REMOVED*** {col3:<10s***REMOVED***"
        df = DataFrame([{"col1": "TEST", "col2": "PANDAS", "col3": "to_string"***REMOVED***])
        d = {"col1": "{:<6s***REMOVED***".format, "col2": "{:<7s***REMOVED***".format, "col3": "{:<10s***REMOVED***".format***REMOVED***
...
class TestDataFrameToStringColSpace:
    def test_to_string_with_column_specific_col_space_raises(self):
        ...
        msg = "unknown column"
        with pytest.raises(ValueError, match=msg):
            df.to_string(col_space={"a": "foo", "b": 23, "d": 34***REMOVED***)
    ...
    def test_to_string_with_column_specific_col_space(self):
        ...
        result = df.to_string(col_space={"a": 10, "b": 11, "c": 12***REMOVED***)
    ...
    def test_to_string_repr_tuples(self):
        ...
        df = DataFrame({"tups": list(zip(range(10), range(10)))***REMOVED***)
        repr(df)
...
class TestDataFrameToStringHeader:
    ...
    def test_to_string_multindex_header(self):
        # GH#16718
        df = DataFrame({"a": [0], "b": [1], "c": [2], "d": [3]***REMOVED***).set_index(["a", "b"])
...
class TestDataFrameToStringLineWidth:
    ...
    def test_to_string_line_width_no_index(self):
        ...
        df = DataFrame({"x": [11, 22, 33], "y": [4, 5, 6]***REMOVED***)
...
class TestToStringNumericFormatting:
    def test_to_string_float_format_no_fixed_width(self):
        # GH#21625
        df = DataFrame({"x": [0.19999]***REMOVED***)
        expected = "      x\n0 0.200"
        assert df.to_string(float_format="%.3f") == expected

        # GH#22270
        df = DataFrame({"x": [100.0]***REMOVED***)
        expected = "    x\n0 100"
        assert df.to_string(float_format="%.0f") == expected

    def test_to_string_small_float_values(self):
        df = DataFrame({"a": [1.5, 1e-17, -5.5e-7]***REMOVED***)
...
***REMOVED***
    ...
    def test_to_string_int_formatting(self):
        df = DataFrame({"x": [-15, 20, 25, -35]***REMOVED***)
    ...
    def test_to_string_float_formatting(self):
        with option_context(
            ...
            df = DataFrame(
                {"x": [0, 0.25, 3456.000, 12e45, 1.64e6, 1.7e8, 1.253456, np.pi, -1e6]***REMOVED***
***REMOVED***
    ...
    ***REMOVED***
            ...
            df = DataFrame({"x": [3234, 0.253]***REMOVED***)
            df_s = df.to_string()
        ...
        df = DataFrame({"x": [1e9, 0.2512]***REMOVED***)
        df_s = df.to_string()
...
class TestDataFrameToString:
    def test_to_string_decimal(self):
        # GH#23614
        df = DataFrame({"A": [6.0, 3.1, 2.2]***REMOVED***)
        expected = "     A\n0  6,0\n1  3,1\n2  2,2"
    ...
    def test_to_string_with_dict_entries(self):
        df = DataFrame({"A": [{"a": 1, "b": 2***REMOVED***]***REMOVED***)
            ...
            [
                {
                    ...
                    "d": 1,
                ***REMOVED***,
                {"a": "foo", "b": "bar", "c": "stuff", "d": 1***REMOVED***,
            ]
        )
        df.set_index(["a", "b", "c"])
        assert df.to_string() == (
...
***REMOVED***
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({"A": ["a"]***REMOVED***, "A\na"),
            ({"A": ["a", "b"], "B": ["c", "dd"]***REMOVED***, "A  B\na  c\nb dd"),
            ({"A": ["a", 1], "B": ["aa", 1]***REMOVED***, "A  B\na aa\n1  1"),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"col1": [1, 2], "col2": [3, 4]***REMOVED***,
                "   col1  col2\n0     1     3\n1     2     4",
***REMOVED***,
            (
                {"col1": ["Abc", 0.756], "col2": [np.nan, 4.5435]***REMOVED***,
                "    col1    col2\n0    Abc     NaN\n1  0.756  4.5435",
***REMOVED***,
            (
                {"col1": [np.nan, "a"], "col2": [0.009, 3.543], "col3": ["Abc", 23]***REMOVED***,
                "  col1   col2 col3\n0  NaN  0.009  Abc\n1    a  3.543   23",
***REMOVED***,
        ],
    )
...
***REMOVED***,
    ...
    def test_to_string_no_index(self):
        # GH#16839, GH#13032
        df = DataFrame({"x": [11, 22], "y": [33, -44], "z": ["AAA", "   "]***REMOVED***)
    ...
    def test_to_string_unicode_columns(self, float_frame):
        df = DataFrame({"\u03c3": np.arange(10.0)***REMOVED***)
    ...
    def test_to_string_na_rep_and_float_format(self, na_rep):
        ...
        result = df.to_string(na_rep=na_rep, float_format="{:.2f***REMOVED***".format)
        expected = dedent(
            f"""\
               Group  Data
             0     A  1.22
    ...
    def test_to_string_string_dtype(self):
        ...
        df = DataFrame(
            {"x": ["foo", "bar", "baz"], "y": ["a", "b", "c"], "z": [1, 2, 3]***REMOVED***
        )
        df = df.astype(
            {"x": "string[pyarrow]", "y": "string[python]", "z": "int64[pyarrow]"***REMOVED***
        )
    ...
    def test_to_string(self):
        ...
        biggie = DataFrame(
            {
                ...
                "B": Index([f"{i***REMOVED***?!" for i in range(200)]),
            ***REMOVED***,
        )
        ...
        biggie.to_string(columns=["B", "A"], formatters={"A": lambda x: f"{x:.1f***REMOVED***"***REMOVED***)
    ...
    def test_to_string_index_with_nan(self):
        ...
        df = DataFrame(
            {
                "id1": {0: "1a3", 1: "9h4"***REMOVED***,
                "id2": {0: np.nan, 1: "d67"***REMOVED***,
                "id3": {0: "78d", 1: "79d"***REMOVED***,
                "value": {0: 123, 1: 64***REMOVED***,
            ***REMOVED***
        )
        ...
        df = DataFrame(
            {
                "id1": {0: np.nan, 1: "9h4"***REMOVED***,
                "id2": {0: np.nan, 1: "d67"***REMOVED***,
                "id3": {0: np.nan, 1: "79d"***REMOVED***,
                "value": {0: 123, 1: 64***REMOVED***,
            ***REMOVED***
        )
    ...
    def test_unicode_problem_decoding_as_ascii(self):
        df = DataFrame({"c/\u03c3": Series({"test": np.nan***REMOVED***)***REMOVED***)
        str(df.to_string())
    ...
    def test_to_string_repr_unicode(self):
        ...
        df = DataFrame({"unicode": unicode_values***REMOVED***)
        df.to_string(col_space=10, buf=buf)
...
class TestSeriesToString:
    ...
    def test_to_string_float_format(self):
        ...
        res = ser.to_string(float_format=lambda x: f"{x:2.1f***REMOVED***", max_rows=2)
        exp = "0   0.0\n     ..\n9   9.0"
        assert res == exp
    ...
    def test_to_string(self):
        ...
        assert last_line == (f"Freq: B, Name: foo, Length: {len(cp)***REMOVED***, dtype: float64")
