...
class TestRank:
    ...
    ***REMOVED***
    ...
    def test_rank2(self):
        ...
        df = DataFrame({"a": [1e-20, -5, 1e-20 + 1e-40, 10, 1e60, 1e80, 1e-30]***REMOVED***)
        exp = DataFrame({"a": [3.5, 1.0, 3.5, 5.0, 6.0, 7.0, 2.0]***REMOVED***)
...
***REMOVED***
    ...
    def test_rank_2d_tie_methods(self, method, axis, dtype):
        ...
        def _check2d(df, expected, method="average", axis=0):
            exp_df = DataFrame({"A": expected, "B": expected***REMOVED***)
...
***REMOVED***,
    ...
    def test_pct_max_many_rows(self):
        # GH 18271
        df = DataFrame(
            {"A": np.arange(2**24 + 1), "B": np.arange(2**24 + 1, 0, -1)***REMOVED***
        )
...
***REMOVED***,
    ...
    def test_rank_both_inf(self):
        # GH#32593
        df = DataFrame({"a": [-np.inf, 0, np.inf]***REMOVED***)
        expected = DataFrame({"a": [1.0, 2.0, 3.0]***REMOVED***)
        result = df.rank()
        tm.assert_frame_equal(result, expected)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"a": [1, 2, "a"], "b": [4, 5, 6]***REMOVED***,
                DataFrame({"b": [1.0, 2.0, 3.0]***REMOVED***, columns=Index(["b"], dtype=object)),
***REMOVED***,
            ({"a": [1, 2, "a"]***REMOVED***, DataFrame(index=range(3), columns=[])),
        ],
    )
