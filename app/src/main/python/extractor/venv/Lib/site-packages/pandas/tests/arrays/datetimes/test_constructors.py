...
***REMOVED***
    ...
    def test_numpy_datetime_unit(self, unit):
        data = np.array([1, 2, 3], dtype=f"M8[{unit***REMOVED***]")
...
def test_from_arrow_with_different_units_and_timezones_with(
    ...
    expected = DatetimeArray._from_sequence(data, dtype=f"M8[{pa_unit***REMOVED***, UTC]").astype(
        dtype, copy=False
    )
...
def test_from_arrow_from_empty(unit, tz):
    ...
    result = dtype.__from_arrow__(arr)
    expected = DatetimeArray._from_sequence(np.array(data, dtype=f"datetime64[{unit***REMOVED***]"))
