...
def document_model_driven_resource_method(
    ...
    if resource_action_model.resource:
        ...
        return_resource_type = '{***REMOVED***.{***REMOVED***'.format(
            operation_model.service_model.service_name, resource_type
        )

        return_type = f':py:class:`{return_resource_type***REMOVED***`'
        return_description = f'{resource_type***REMOVED*** resource'

        if _method_returns_resource_list(resource_action_model.resource):
            return_type = f'list({return_type***REMOVED***)'
            return_description = f'A list of {resource_type***REMOVED*** resources'

        new_return_section.style.new_line()
        new_return_section.write(f':rtype: {return_type***REMOVED***')
        new_return_section.style.new_line()
        new_return_section.write(f':returns: {return_description***REMOVED***')
        new_return_section.style.new_line()
