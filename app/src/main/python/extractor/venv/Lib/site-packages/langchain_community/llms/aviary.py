...
class AviaryBackend:
    ...
    def __post_init__(self) -> None:
        self.header = {"Authorization": self.bearer***REMOVED***
    ...
    def from_env(cls) -> "AviaryBackend":
        ...
        bearer = f"Bearer {aviary_token***REMOVED***" if aviary_token else ""
...
def get_models() -> List[str]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Error decoding JSON from {request_url***REMOVED***. Text response: {response.text***REMOVED***"
        ) from e
    result = sorted(
...
def get_completions(
    ...
    response = requests.post(
        ...
        json={"prompt": prompt, "use_prompt_format": use_prompt_format***REMOVED***,
        timeout=TIMEOUT,
    )
    try:
        ...
        raise RuntimeError(
            f"Error decoding JSON from {url***REMOVED***. Text response: {response.text***REMOVED***"
        ) from e
...
class Aviary(LLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if model and model not in aviary_models:
            raise ValueError(f"{aviary_url***REMOVED*** does not support model {values['model']***REMOVED***.")
    ...
    def _llm_type(self) -> str:
        """Return type of llm."""
        return f"aviary-{self.model.replace('/', '-')***REMOVED***"
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        """
        kwargs = {"use_prompt_format": self.use_prompt_format***REMOVED***
        if self.version:
