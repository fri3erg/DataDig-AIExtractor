...
def can_cast(from_: Union[Dtype, Array], to: Dtype, /) -> bool:
    ...
    if isinstance(from_, Array):
        ...
        raise TypeError(f"{from_=***REMOVED***, but should be an array_api array or dtype")
    if to not in _all_dtypes:
        raise TypeError(f"{to=***REMOVED***, but should be a dtype")
...
def isdtype(
    ...
    if isinstance(kind, tuple):
        ...
        if kind == 'bool':
            ...
            raise ValueError(f"Unrecognized data type kind: {kind!r***REMOVED***")
    elif kind in _all_dtypes:
        return dtype == kind
    else:
        raise TypeError(f"'kind' must be a dtype, str, or tuple of dtypes and strs, not {type(kind).__name__***REMOVED***")
