...
cdef int _raiseParseError(xmlparser.xmlParserCtxt* ctxt, filename,
    ...
    if filename is not None and \
        ...
        if ctxt.lastError.message is not NULL:
            ...
            message = f"Error reading file '{filename***REMOVED***': {message.strip()***REMOVED***"
        else:
            message = f"Error reading '{filename***REMOVED***'"
        ...
        if ctxt.lastError.line > 0:
            message = f"line {line***REMOVED***: {message***REMOVED***"
    ...
    cdef xmlDoc* _parseUnicodeDoc(self, utext, char* c_filename) except NULL:
        ...
        if python.PyUnicode_IS_READY(utext):
            ...
            if c_kind == 1:
                ...
                assert False, f"Illegal Unicode kind {c_kind***REMOVED***"
        else:
...
cdef _Document _parseDocument(source, _BaseParser parser, base_url):
    ...
    raise TypeError, f"cannot parse from '{python._fqtypename(source).decode('UTF-8')***REMOVED***'"
