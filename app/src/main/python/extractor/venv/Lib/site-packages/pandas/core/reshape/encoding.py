        ...
        def check_len(item, name: str):
            if is_list_like(item):
                if not len(item) == data_to_encode.shape[1]:
                    len_msg = (
                        f"Length of '{name***REMOVED***' ({len(item)***REMOVED***) did not match the "
                        "length of the columns being encoded "
                        f"({data_to_encode.shape[1]***REMOVED***)."
        ***REMOVED***
...
def _get_dummies_1d(
    ...
    if prefix is None:
        ...
        dummy_cols = Index([f"{prefix***REMOVED***{prefix_sep***REMOVED***{level***REMOVED***" for level in levels])
...
def from_dummies(
    ...
    >>> pd.from_dummies(df, sep="_", default_category={"col1": "d", "col2": "e"***REMOVED***)
        col1    col2
    0    a       b
    1    b       a
    2    d       e
    ...
    if not isinstance(data, DataFrame):
        raise TypeError(
            ...
            f"Received 'data' of type: {type(data).__name__***REMOVED***"
        )
    ...
    if col_isna_mask.any():
        raise ValueError(
            ...
            f"'{col_isna_mask.idxmax()***REMOVED***'"
        )
    ...
    if sep is None:
        ...
        for col in data_to_decode.columns:
            ...
            if len(prefix) == len(col):
                raise ValueError(f"Separator not specified for column: {col***REMOVED***")
            variables_slice[prefix].append(col)
    else:
        raise TypeError(
            "Expected 'sep' to be of type 'str' or 'None'; "
            f"Received 'sep' of type: {type(sep).__name__***REMOVED***"
        )
    ...
    if default_category is not None:
        if isinstance(default_category, dict):
            if not len(default_category) == len(variables_slice):
                len_msg = (
                    f"Length of 'default_category' ({len(default_category)***REMOVED***) "
                    f"did not match the length of the columns being encoded "
                    f"({len(variables_slice)***REMOVED***)"
    ***REMOVED***
                raise ValueError(len_msg)
...
***REMOVED***
            ...
            raise TypeError(
                ...
                f"{type(default_category).__name__***REMOVED***"
***REMOVED***
    ...
    for prefix, prefix_slice in variables_slice.items():
        ...
        if any(assigned > 1):
            raise ValueError(
                ...
                f"First instance in row: {assigned.idxmax()***REMOVED***"
***REMOVED***
        if any(assigned == 0):
            if isinstance(default_category, dict):
                ...
                raise ValueError(
                    ...
                    f"First instance in row: {assigned.idxmin()***REMOVED***"
    ***REMOVED***
            data_slice = concat(
