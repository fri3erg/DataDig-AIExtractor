...
class RedisVectorField(RedisField):
    ...
    def uppercase_and_check_dtype(cls, v: str) -> str:
        if v.upper() not in REDIS_VECTOR_DTYPE_MAP:
            raise ValueError(
                f"datatype must be one of {REDIS_VECTOR_DTYPE_MAP.keys()***REMOVED***. Got {v***REMOVED***"
***REMOVED***
        return v.upper()
    ...
    def add_vector_field(self, vector_field: Dict[str, Any]) -> None:
        ...
        if vector_field["algorithm"] == "FLAT":
            ...
            raise ValueError(
                ...
                f"{vector_field['algorithm']***REMOVED***"
***REMOVED***

    def as_dict(self) -> Dict[str, List[Any]]:
        schemas: Dict[str, List[Any]] = {"text": [], "tag": [], "numeric": []***REMOVED***
        ...
        schema: Dict[str, List[Any]] = {***REMOVED***
...
def read_schema(
    ...
    if isinstance(index_schema, dict):
        ...
        if Path(index_schema).resolve().is_file():
            ...
            raise FileNotFoundError(f"index_schema file {index_schema***REMOVED*** does not exist")
    else:
        raise TypeError(
            f"index_schema must be a dict, or path to a yaml file "
            f"Got {type(index_schema)***REMOVED***"
        )
