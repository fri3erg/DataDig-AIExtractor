...
try:
    ...
    class ExceptionGroup(Exception):  # type: ignore[no-redef]  # noqa: N818
        ...
        def __repr__(self) -> str:
            return f"{self.__class__.__name__***REMOVED***({self.message!r***REMOVED***, {self.exceptions!r***REMOVED***)"
...
_EMAIL_TO_RAW_MAPPING = {
    ...
    "version": "version",
***REMOVED***
_RAW_TO_EMAIL_MAPPING = {raw: email for email, raw in _EMAIL_TO_RAW_MAPPING.items()***REMOVED***
...
def parse_email(data: Union[bytes, str]) -> Tuple[RawMetadata, Dict[str, List[str]]]:
    ...
    """
    raw: Dict[str, Union[str, List[str], Dict[str, str]]] = {***REMOVED***
    unparsed: Dict[str, List[str]] = {***REMOVED***
...
class _Validator(Generic[T]):
    ...
    def __get__(self, instance: "Metadata", _owner: Type["Metadata"]) -> T:
        ...
        try:
            converter: Callable[[Any], T] = getattr(self, f"_process_{self.name***REMOVED***")
        except AttributeError:
            pass
        else:
    ...
    def _invalid_metadata(
        ...
        exc = InvalidMetadata(
            self.raw_name, msg.format_map({"field": repr(self.raw_name)***REMOVED***)
        )
        exc.__cause__ = cause
        return exc
    ...
    def _process_metadata_version(self, value: str) -> _MetadataVersion:
        ...
        if value not in _VALID_METADATA_VERSIONS:
            raise self._invalid_metadata(f"{value!r***REMOVED*** is not a valid metadata version")
        return cast(_MetadataVersion, value)

    def _process_name(self, value: str) -> str:
        if not value:
            raise self._invalid_metadata("{field***REMOVED*** is a required field")
        # Validate the name as a side-effect.
        try:
            utils.canonicalize_name(value, validate=True)
        except utils.InvalidName as exc:
            raise self._invalid_metadata(
                f"{value!r***REMOVED*** is invalid for {{field***REMOVED******REMOVED***", cause=exc
***REMOVED***
        else:
            return value
...
***REMOVED***
    ...
    def _process_summary(self, value: str) -> str:
        ...
        if "\n" in value:
            raise self._invalid_metadata("{field***REMOVED*** must be a single line")
        return value

    def _process_description_content_type(self, value: str) -> str:
        content_types = {"text/plain", "text/x-rst", "text/markdown"***REMOVED***
        ...
        if content_type not in content_types or content_type not in value.lower():
            raise self._invalid_metadata(
                f"{{field***REMOVED******REMOVED*** must be one of {list(content_types)***REMOVED***, not {value!r***REMOVED***"
***REMOVED***

        charset = parameters.get("charset", "UTF-8")
        if charset != "UTF-8":
            raise self._invalid_metadata(
                f"{{field***REMOVED******REMOVED*** can only specify the UTF-8 charset, not {list(charset)***REMOVED***"
***REMOVED***

        markdown_variants = {"GFM", "CommonMark"***REMOVED***
        variant = parameters.get("variant", "GFM")  # Use an acceptable default.
        if content_type == "text/markdown" and variant not in markdown_variants:
            raise self._invalid_metadata(
                f"valid Markdown variants for {{field***REMOVED******REMOVED*** are {list(markdown_variants)***REMOVED***, "
                f"not {variant!r***REMOVED***",
***REMOVED***
        return value

    def _process_dynamic(self, value: List[str]) -> List[str]:
        for dynamic_field in map(str.lower, value):
            if dynamic_field in {"name", "version", "metadata-version"***REMOVED***:
                raise self._invalid_metadata(
                    f"{value!r***REMOVED*** is not allowed as a dynamic field"
    ***REMOVED***
            elif dynamic_field not in _EMAIL_TO_RAW_MAPPING:
                raise self._invalid_metadata(f"{value!r***REMOVED*** is not a valid dynamic field")
    ...
    def _process_provides_extra(
        ...
        try:
            ...
            raise self._invalid_metadata(
                f"{name!r***REMOVED*** is invalid for {{field***REMOVED******REMOVED***", cause=exc
***REMOVED***
        else:
...
***REMOVED***
    ...
    def _process_requires_dist(
        ...
        try:
            ...
            raise self._invalid_metadata(f"{req!r***REMOVED*** is invalid for {{field***REMOVED******REMOVED***", cause=exc)
        else:
            return reqs
...
class Metadata:
    ...
    def from_raw(cls, data: RawMetadata, *, validate: bool = True) -> "Metadata":
        ...
        if validate:
            ...
            fields_to_check -= {"metadata_version"***REMOVED***
                ...
                try:
                    if metadata_version:
                        ...
                        try:
                            ...
                            exc = InvalidMetadata(key, f"unrecognized field: {key!r***REMOVED***")
            ...
            ***REMOVED***
                        if field_age > metadata_age:
                            ...
                            exc = InvalidMetadata(
                                field,
                                "{field***REMOVED*** introduced in metadata version "
                                "{field_metadata_version***REMOVED***, not {metadata_version***REMOVED***",
                ***REMOVED***
                            exceptions.append(exc)
                            continue
                    getattr(ins, key)
        ...
        if validate:
            for unparsed_key in unparsed:
                if unparsed_key in _EMAIL_TO_RAW_MAPPING:
                    message = f"{unparsed_key!r***REMOVED*** has invalid data"
                else:
                    message = f"unrecognized field: {unparsed_key!r***REMOVED***"
