...
def asadpour_atsp(G, weight="weight", seed=None, source=None):
    ...
    >>> nx.set_edge_attributes(G, {(0, 1): 2, (1, 2): 2, (2, 0): 2, (0, 2): 1, (2, 1): 1, (1, 0): 1***REMOVED***, "weight")
    ...
    for u, v in z_star:
        if (u, v) not in z_support.edges:
            ...
            z_support.add_edge(u, v, **{weight: edge_weight***REMOVED***)
    ...
    lambda_dict = {(u, v): exp(gamma[(u, v)]) for u, v in z_support.edges()***REMOVED***
    ...
    for u, v, d in minimum_sampled_tree.edges(data=weight):
        if d == G[u][v][weight]:
            t_star.add_edge(u, v, **{weight: d***REMOVED***)
        else:
            t_star.add_edge(v, u, **{weight: d***REMOVED***)

    # Find the node demands needed to neutralize the flow of t_star in G
    node_demands = {n: t_star.out_degree(n) - t_star.in_degree(n) for n in t_star***REMOVED***
...
def held_karp_ascent(G, weight="weight"):
    ...
    def k_pi():
        ...
        # minimum
        min_root = {"node": None, weight: math.inf***REMOVED***
        max_root = {"node": None, weight: -math.inf***REMOVED***
        for u, v, d in G.edges(n, data=True):
            if d[weight] < min_root[weight]:
                min_root = {"node": v, weight: d[weight]***REMOVED***
            if d[weight] > max_root[weight]:
                max_root = {"node": v, weight: d[weight]***REMOVED***
        ...
        for arb in nx.ArborescenceIterator(G_1):
            ...
            for N, deg in arb.in_degree:
                if deg == 0:
                    # root found
                    arb.add_edge(n, N, **{weight: G[n][N][weight]***REMOVED***)
            ...
            for u, v, d in min_edges:
                new_arb = arb.copy()
                new_arb.add_edge(u, v, **{weight: d***REMOVED***)
    ...
    def direction_of_ascent():
        ...
        while True:
            ...
            # 3. If sum of d_i * v_{i, k***REMOVED*** is greater than zero, terminate
            if min_k_d_weight > 0:
                return d, min_k_d
            # 4. d_i = d_i + v_{i, k***REMOVED***
    ...
    def find_epsilon(k, d):
        ...
        for e_u, e_v, e_w in G.edges(data=weight):
            ...
            k.add_edge(e_u, e_v, **{weight: e_w***REMOVED***)
...
***REMOVED***:
                ...
                if d[sub_u] == d[e_u] or sub_w == e_w:
                    ...
                    k.add_edge(sub_u, sub_v, **{weight: sub_w***REMOVED***)
                    continue
    ...
    ***REMOVED***
                ...
                if round(new_q_e, 8) != round(desired_q_e, 8):
                    raise nx.NetworkXError(
                        f"Unable to modify probability for edge ({u***REMOVED***, {v***REMOVED***)"
        ***REMOVED***
            else:
                in_range_count += 1
