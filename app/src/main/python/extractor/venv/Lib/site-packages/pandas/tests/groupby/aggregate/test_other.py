...
def test_agg_period_index():
    ...
    df = DataFrame.from_dict({"s1": s1, "s2": s2***REMOVED***)
...
def test_agg_dict_parameter_cast_result_dtypes():
    ...
    tm.assert_frame_equal(grouped.agg({"time": "first"***REMOVED***), exp)
    ...
    tm.assert_frame_equal(grouped.agg({"time": "last"***REMOVED***), exp)
...
def test_aggregate_float64_no_int64():
    # see gh-11199
    df = DataFrame({"a": [1, 2, 3, 4, 5], "b": [1, 2, 2, 4, 5], "c": [1, 2, 3, 4, 5]***REMOVED***)

    expected = DataFrame({"a": [1, 2.5, 4, 5]***REMOVED***, index=[1, 2, 4, 5])
    expected.index.name = "b"

    result = df.groupby("b")[["a"]].mean()
    tm.assert_frame_equal(result, expected)

    expected = DataFrame({"a": [1, 2.5, 4, 5], "c": [1, 2.5, 4, 5]***REMOVED***, index=[1, 2, 4, 5])
    expected.index.name = "b"
...
def test_aggregate_api_consistency():
    ...
    result = grouped.agg({"C": "mean", "D": "sum"***REMOVED***)
    expected = pd.concat([d_sum, c_mean], axis=1)
    tm.assert_frame_equal(result, expected, check_like=True)

    result = grouped.agg({"C": ["mean", "sum"], "D": ["mean", "sum"]***REMOVED***)
    expected = pd.concat([c_mean, c_sum, d_mean, d_sum], axis=1)
    expected.columns = MultiIndex.from_product([["C", "D"], ["mean", "sum"]])

    msg = r"Column\(s\) \['r', 'r2'\] do not exist"
    with pytest.raises(KeyError, match=msg):
        grouped[["D", "C"]].agg({"r": "sum", "r2": "mean"***REMOVED***)


def test_agg_dict_renaming_deprecation():
    # 15931
    df = DataFrame({"A": [1, 1, 1, 2, 2], "B": range(5), "C": range(5)***REMOVED***)

    msg = r"nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        df.groupby("A").agg(
            {"B": {"foo": ["sum", "max"]***REMOVED***, "C": {"bar": ["count", "min"]***REMOVED******REMOVED***
        )

    msg = r"Column\(s\) \['ma'\] do not exist"
    with pytest.raises(KeyError, match=msg):
        df.groupby("A")[["B", "C"]].agg({"ma": "max"***REMOVED***)

    msg = r"nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        df.groupby("A").B.agg({"foo": "count"***REMOVED***)
...
def test_agg_compat():
    ...
    msg = r"nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        g["D"].agg({"C": ["sum", "std"]***REMOVED***)

    with pytest.raises(SpecificationError, match=msg):
        g["D"].agg({"C": "sum", "D": "std"***REMOVED***)
...
def test_agg_nested_dicts():
    ...
    msg = r"nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        g.aggregate({"r1": {"C": ["mean", "sum"]***REMOVED***, "r2": {"D": ["mean", "sum"]***REMOVED******REMOVED***)

    with pytest.raises(SpecificationError, match=msg):
        g.agg({"C": {"ra": ["mean", "std"]***REMOVED***, "D": {"rb": ["mean", "std"]***REMOVED******REMOVED***)

    # same name as the original column
    # GH9052
    with pytest.raises(SpecificationError, match=msg):
        g["D"].agg({"result1": np.sum, "result2": np.mean***REMOVED***)

    with pytest.raises(SpecificationError, match=msg):
        g["D"].agg({"D": np.sum, "result2": np.mean***REMOVED***)
...
def test_agg_callables():
    # GH 7929
    df = DataFrame({"foo": [1, 2], "bar": [3, 4]***REMOVED***).astype(np.int64)
...
def test_agg_tzaware_non_datetime_result(as_period):
    ...
    df = DataFrame({"a": [0, 0, 1, 1], "b": dti***REMOVED***)
    gb = df.groupby("a")
...
def test_agg_timezone_round_trip():
    ...
    df = DataFrame({"a": 1, "b": [ts + dt.timedelta(minutes=nn) for nn in range(10)]***REMOVED***)
    ...
    dates = [
        pd.Timestamp(f"2016-01-0{i:d***REMOVED*** 12:00:00", tz="US/Pacific") for i in range(1, 5)
    ]
    df = DataFrame({"A": ["a", "b"] * 2, "B": dates***REMOVED***)
...
def test_sum_uint64_overflow():
    ...
    expected = DataFrame(
        {1: [9223372036854775809, 9223372036854775811, 9223372036854775813]***REMOVED***,
        index=index,
        dtype=object,
    )
...
@pytest.mark.parametrize(
    "structure, expected",
    [
        (tuple, DataFrame({"C": {(1, 1): (1, 1, 1), (3, 4): (3, 4, 4)***REMOVED******REMOVED***)),
        (list, DataFrame({"C": {(1, 1): [1, 1, 1], (3, 4): [3, 4, 4]***REMOVED******REMOVED***)),
        (
            lambda x: tuple(x),
            DataFrame({"C": {(1, 1): (1, 1, 1), (3, 4): (3, 4, 4)***REMOVED******REMOVED***),
        ),
        (
            lambda x: list(x),
            DataFrame({"C": {(1, 1): [1, 1, 1], (3, 4): [3, 4, 4]***REMOVED******REMOVED***),
...
def test_agg_structs_dataframe(structure, expected):
    df = DataFrame(
        {"A": [1, 1, 1, 3, 3, 3], "B": [1, 1, 1, 4, 4, 4], "C": [1, 1, 1, 3, 4, 4]***REMOVED***
    )
...
def test_agg_category_nansum(observed):
    categories = ["a", "b", "c"]
    df = DataFrame(
        {"A": pd.Categorical(["a", "a", "b"], categories=categories), "B": [1, 2, 3]***REMOVED***
    )
...
def test_agg_list_like_func():
    # GH 18473
    df = DataFrame({"A": [str(x) for x in range(3)], "B": [str(x) for x in range(3)]***REMOVED***)
    grouped = df.groupby("A", as_index=False, sort=False)
    result = grouped.agg({"B": lambda x: list(x)***REMOVED***)
    expected = DataFrame(
        {"A": [str(x) for x in range(3)], "B": [[str(x)] for x in range(3)]***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
...
def test_agg_lambda_with_timezone():
    ...
    result = df.groupby("tag").agg({"date": lambda e: e.head(1)***REMOVED***)
    expected = DataFrame(
        [pd.Timestamp("2018-01-01", tz="UTC")],
...
def test_groupby_agg_err_catching(err_cls):
    ...
    data = make_data()[:5]
    df = DataFrame(
        {"id1": [0, 0, 0, 1, 1], "id2": [0, 1, 0, 1, 1], "decimals": DecimalArray(data)***REMOVED***
    )
