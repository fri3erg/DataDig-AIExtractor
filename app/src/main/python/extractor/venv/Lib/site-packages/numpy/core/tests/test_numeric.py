...
class TestTypes:
    ...
    def test_promote_types_metadata(self, dtype1, dtype2):
        ...
        """
        metadata1 = {1: 1***REMOVED***
        metadata2 = {2: 2***REMOVED***
...
***REMOVED***
        ...
        amin = data.draw(s | hynp.arrays(dtype=numeric_dtypes,
            shape=in_shapes[0], elements={"allow_nan": False***REMOVED***))
        amax = data.draw(s | hynp.arrays(dtype=numeric_dtypes,
            shape=in_shapes[1], elements={"allow_nan": False***REMOVED***))
...
class TestCreationFuncs:
    ...
    def setup_method(self):
        dtypes = {np.dtype(tp) for tp in itertools.chain(*np.sctypes.values())***REMOVED***
        # void, bytes, str
        variable_sized = {tp for tp in dtypes if tp.str.endswith('0')***REMOVED***
        self.dtypes = sorted(dtypes - variable_sized |
                             {np.dtype(tp.str.replace("0", str(i)))
                              for tp in variable_sized for i in range(1, 10)***REMOVED***,
...
class TestLikeFuncs:
    ...
    def test_dtype_str_bytes(self, likefunc, dtype):
        ...
        kwargs = {'fill_value': ''***REMOVED*** if likefunc == np.full_like else {***REMOVED***
...
class TestBroadcast:
    ...
    def test_broadcast_error_kwargs(self):
        ...
        arrs = [np.empty((5, 6, 7))]
        mit  = np.broadcast(*arrs)
        mit2 = np.broadcast(*arrs, **{***REMOVED***)
        ...
        assert_raises(ValueError, np.broadcast, 1, **{'x': 1***REMOVED***)
