...
def get_standard_colors(
    ...
    color_type : {"default", "random"***REMOVED***, optional
...
def _get_cmap_instance(colormap: str | Colormap) -> Colormap:
    ...
    if isinstance(colormap, str):
        ...
        if colormap is None:
            raise ValueError(f"Colormap {cmap***REMOVED*** is not recognized")
    return colormap
...
def _get_colors_from_color(
    ...
    if len(color) == 0:
        raise ValueError(f"Invalid color argument: {color***REMOVED***")
...
def _gen_list_of_colors_from_iterable(color: Collection[Color]) -> Iterator[Color]:
    ...
    for x in color:
        if _is_single_color(x):
            ...
            raise ValueError(f"Invalid color {x***REMOVED***")
