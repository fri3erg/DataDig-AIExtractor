...
class SeriesFormatter:
    ...
    def _get_footer(self) -> str:
        ...
        if (
            ...
            footer += f"Freq: {index.freqstr***REMOVED***"
        ...
        if self.name is not False and name is not None:
            ...
            footer += f"Name: {series_name***REMOVED***"
        ...
        if self.length is True or (
            ...
            if footer:
                footer += ", "
            footer += f"Length: {len(self.series)***REMOVED***"
        ...
        if self.dtype is not False and self.dtype is not None:
            ...
            if dtype_name:
                if footer:
                    footer += ", "
                footer += f"dtype: {printing.pprint_thing(dtype_name)***REMOVED***"
    ...
    def to_string(self) -> str:
        ...
        if len(series) == 0:
            return f"{type(self.series).__name__***REMOVED***([], {footer***REMOVED***)"
...
class DataFrameFormatter:
    ...
    def dimensions_info(self) -> str:
        return f"\n\n[{len(self.frame)***REMOVED*** rows x {len(self.frame.columns)***REMOVED*** columns]"
    ...
    def _initialize_formatters(
        ...
        if formatters is None:
            ...
            raise ValueError(
                f"Formatters length({len(formatters)***REMOVED***) should match "
                f"DataFrame number of columns({len(self.frame.columns)***REMOVED***)"
***REMOVED***
    ...
    def _initialize_colspace(self, col_space: ColspaceArgType | None) -> ColspaceType:
        ...
        if col_space is None:
            ...
            result = {"": col_space***REMOVED***
            result.update({column: col_space for column in self.frame.columns***REMOVED***)
        elif isinstance(col_space, Mapping):
            for column in col_space.keys():
                if column not in self.frame.columns and column != "":
                    raise ValueError(
                        f"Col_space is defined for an unknown column: {column***REMOVED***"
        ***REMOVED***
            result = col_space
        else:
            if len(self.frame.columns) != len(col_space):
                raise ValueError(
                    f"Col_space length({len(col_space)***REMOVED***) should match "
                    f"DataFrame number of columns({len(self.frame.columns)***REMOVED***)"
    ***REMOVED***
    ...
    ***REMOVED***
        ...
        if is_list_like(self.header):
            ...
            if len(self.header) != len(self.columns):
                raise ValueError(
                    f"Writing {len(self.columns)***REMOVED*** cols "
                    f"but got {len(self.header)***REMOVED*** aliases"
    ***REMOVED***
...
***REMOVED***
    ...
    def _get_formatted_index(self, frame: DataFrame) -> list[str]:
        ...
        col_space = {k: cast(int, v) for k, v in self.col_space.items()***REMOVED***
        index = frame.index
        columns = frame.columns
...
class DataFrameRenderer:
    ...
    def to_html(
        ...
        notebook : {True, False***REMOVED***, optional, default False
        ...
        if self.float_format is None:
            ...
            if float_format is None:
                ...
                float_format = lambda x: _trim_zeros_single_float(
                    f"{x: .{precision:d***REMOVED***f***REMOVED***"
    ***REMOVED***
        else:
            float_format = self.float_format
...
***REMOVED***
        ...
        for i, v in enumerate(vals):
            if (not is_float_type[i] or self.formatter is not None) and leading_space:
                fmt_values.append(f" {_format(v)***REMOVED***")
...
***REMOVED***:
                if not re_isna and not im_isna:
                    ...
                    formatted_lst.append(f"{formatter(real_val)***REMOVED***+{na_rep***REMOVED***j")
                    ...
                    if imag_formatted.startswith("-"):
                        formatted_lst.append(f"{na_rep***REMOVED***{imag_formatted***REMOVED***j")
                    else:
                        formatted_lst.append(f"{na_rep***REMOVED***+{imag_formatted***REMOVED***j")
                else:  # nan+nanj
                    formatted_lst.append(f"{na_rep***REMOVED***+{na_rep***REMOVED***j")
        ...
        if self.float_format is None:
            if self.fixed_width:
                if self.leading_space is True:
                    fmt_str = "{value: .{digits:d***REMOVED***f***REMOVED***"
                else:
                    fmt_str = "{value:.{digits:d***REMOVED***f***REMOVED***"
        ...
        if has_small_values or (too_long and has_large_values):
            if self.leading_space is True:
                fmt_str = "{value: .{digits:d***REMOVED***e***REMOVED***"
            else:
                fmt_str = "{value:.{digits:d***REMOVED***e***REMOVED***"
            float_format = partial(fmt_str.format, digits=self.digits)
            formatted_values = format_values_with(float_format)
...
class _IntArrayFormatter(_GenericArrayFormatter):
    def _format_strings(self) -> list[str]:
        if self.leading_space is False:
            formatter_str = lambda x: f"{x:d***REMOVED***".format(x=x)
        else:
            formatter_str = lambda x: f"{x: d***REMOVED***".format(x=x)
...
def _trim_zeros_complex(str_complexes: ArrayLike, decimal: str = ".") -> list[str]:
    ...
    for x in str_complexes:
        ...
        # The split will give [{"", "-"***REMOVED***, "xxx", "+/-", "xxx", "j", ""]
    ...
    padded = [
        ...
        + imag_pt[0]  # +/-
        + f"{imag_pt[1:]:>{padded_length***REMOVED******REMOVED***"  # complex part (no sign), possibly nan
        + "j"
...
def _trim_zeros_float(
    ...
    """
    trimmed = str_floats
    number_regex = re.compile(rf"^\s*[\+-]?[0-9]+\{decimal***REMOVED***[0-9]*$")
    ...
    ***REMOVED***
    ...
    def __call__(self, num: float) -> str:
        ...
        if self.use_eng_prefix:
            ...
            prefix = f"E-{-int_pow10:02d***REMOVED***"
        else:
            prefix = f"E+{int_pow10:02d***REMOVED***"
        ...
        if self.accuracy is None:  # pragma: no cover
            format_str = "{mant: g***REMOVED***{prefix***REMOVED***"
        else:
            format_str = f"{{mant: .{self.accuracy:d***REMOVED***f***REMOVED******REMOVED***{{prefix***REMOVED******REMOVED***"
