...
def test_complibs(tmp_path, lvl, lib, request):
    ...
    if PY311 and is_platform_linux() and lib == "blosc2" and lvl != 0:
        request.applymarker(
            pytest.mark.xfail(reason=f"Fails for {lib***REMOVED*** on Linux and PY > 3.11")
        )
    df = DataFrame(
    ...
    tmpfile = tmp_path / f"{lvl***REMOVED***_{lib***REMOVED***.h5"
    gname = f"{lvl***REMOVED***_{lib***REMOVED***"
...
def test_encoding(setup_path):
    with ensure_clean_store(setup_path) as store:
        df = DataFrame({"A": "foo", "B": "bar"***REMOVED***, index=range(5))
        df.loc[2, "A"] = np.nan
        df.loc[3, "B"] = np.nan
