...
***REMOVED***
        if not isinstance(typesense_client, Client):
            raise ValueError(
                ...
                f"got {type(typesense_client)***REMOVED***"
***REMOVED***
        self._typesense_client = typesense_client
        self._embedding = embedding
        self._typesense_collection_name = (
            typesense_collection_name or f"langchain-{str(uuid.uuid4())***REMOVED***"
        )
    ...
    def _prep_texts(
        ...
        _metadatas: Iterable[dict] = metadatas or ({***REMOVED*** for _ in texts)
        embedded_texts = self._embedding.embed_documents(list(texts))
        return [
            {"id": _id, "vec": vec, f"{self._text_key***REMOVED***": text, "metadata": metadata***REMOVED***
            for _id, vec, text, metadata in zip(_ids, embedded_texts, texts, _metadatas)
        ]

    def _create_collection(self, num_dim: int) -> None:
        fields = [
            {"name": "vec", "type": "float[]", "num_dim": num_dim***REMOVED***,
            {"name": f"{self._text_key***REMOVED***", "type": "string"***REMOVED***,
            {"name": ".*", "type": "auto"***REMOVED***,
        ]
        self._typesense_client.collections.create(
            {"name": self._typesense_collection_name, "fields": fields***REMOVED***
        )
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        try:
            self._collection.documents.import_(docs, {"action": "upsert"***REMOVED***)
