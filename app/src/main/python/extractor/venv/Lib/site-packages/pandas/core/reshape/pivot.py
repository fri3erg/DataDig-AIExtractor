...
def _add_margins(
    ...
    msg = f'Conflicting name "{margins_name***REMOVED***" in margins'
    ...
    if not values and isinstance(table, ABCSeries):
        ...
        return table._append(table._constructor({key: grand_margin[margins_name]***REMOVED***))
...
def _compute_grand_margin(
    ...
    if values:
        ...
        return {margins_name: aggfunc(data.index)***REMOVED***
...
def crosstab(
    ...
    normalize : bool, {'all', 'index', 'columns'***REMOVED***, or {0,1***REMOVED***, default False
    ...
    ***REMOVED***
    ...
    if values is None:
        df["__dummy__"] = 0
        kwargs = {"aggfunc": len, "fill_value": 0***REMOVED***
    else:
        df["__dummy__"] = values
        kwargs = {"aggfunc": aggfunc***REMOVED***
...
def _normalize(
    ...
    if not isinstance(normalize, (bool, str)):
        axis_subs = {0: "index", 1: "columns"***REMOVED***
        try:
    ...
    if margins is False:
        ...
        ***REMOVED***
        ...
        if (margins_name not in last_ind_or_col) & (margins_name != last_ind_or_col):
            raise ValueError(f"{margins_name***REMOVED*** not in pivoted DataFrame")
    ...
    if names is None:
        ...
        for i, arr in enumerate(arrs):
            if isinstance(arr, ABCSeries) and arr.name is not None:
                ...
                names.append(f"{prefix***REMOVED***_{i***REMOVED***")
    else:
...
def _build_names_mapper(
    ...
    def get_duplicates(names):
        seen: set = set()
        return {name for name in names if name not in seen***REMOVED***
    ...
    rownames_mapper = {
        f"row_{i***REMOVED***": name for i, name in enumerate(rownames) if name in dup_names
    ***REMOVED***
    unique_rownames = [
        f"row_{i***REMOVED***" if name in dup_names else name for i, name in enumerate(rownames)
    ]

    colnames_mapper = {
        f"col_{i***REMOVED***": name for i, name in enumerate(colnames) if name in dup_names
    ***REMOVED***
    unique_colnames = [
        f"col_{i***REMOVED***" if name in dup_names else name for i, name in enumerate(colnames)
    ]
