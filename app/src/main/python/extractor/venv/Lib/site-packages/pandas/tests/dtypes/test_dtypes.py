...
class TestDatetimeTZDtype(Base):
    ...
    def test_parser(self, tz, constructor):
        # pr #11245
        dtz_str = f"{constructor***REMOVED***[ns, {tz***REMOVED***]"
...
class TestIntervalDtype(Base):
    ...
    def test_name_repr(self, subtype):
        ...
        closed = "right" if subtype is not None else None
        dtype = IntervalDtype(subtype, closed=closed)
        expected = f"interval[{subtype***REMOVED***, {closed***REMOVED***]"
...
class TestCategoricalDtypeParametrized:
    ...
    def test_str_vs_repr(self, ordered, using_infer_string):
        ...
        pat = (
            r"CategoricalDtype\(categories=\[.*\], ordered={ordered***REMOVED***, "
            rf"categories_dtype={dtype***REMOVED***\)"
        )
...
def test_is_dtype_no_warning(check):
    data = pd.DataFrame({"A": [1, 2]***REMOVED***)

    warn = None
    msg = f"{check.__name__***REMOVED*** is deprecated"
    if (
...
def test_multi_column_dtype_assignment():
    # GH #27583
    df = pd.DataFrame({"a": [0.0], "b": 0.0***REMOVED***)
    expected = pd.DataFrame({"a": [0], "b": 0***REMOVED***)
