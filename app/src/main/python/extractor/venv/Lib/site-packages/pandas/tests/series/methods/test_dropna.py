...
class TestDropna:
    ...
    def test_datetime64_tz_dropna(self, unit):
        ...
        ser = Series(idx)
        assert ser.dtype == f"datetime64[{unit***REMOVED***, Asia/Tokyo]"
        result = ser.dropna()
        expected = Series(
            [
            ...
            index=[0, 2],
            dtype=f"datetime64[{unit***REMOVED***, Asia/Tokyo]",
        )
        assert result.dtype == f"datetime64[{unit***REMOVED***, Asia/Tokyo]"
        tm.assert_series_equal(result, expected)
