...
class RunnableConfigurableFields(DynamicRunnable[Input, Output]):
    ...
    ***REMOVED***
    ...
    def configurable_fields(
        ...
        return self.default.configurable_fields(**{**self.fields, **kwargs***REMOVED***)

    def _prepare(
        self, config: Optional[RunnableConfig] = None
    ) -> Tuple[Runnable[Input, Output], RunnableConfig]:
        config = ensure_config(config)
        specs_by_id = {spec.id: (key, spec) for key, spec in self.fields.items()***REMOVED***
        configurable_fields = {
            specs_by_id[k][0]: v
            for k, v in config.get("configurable", {***REMOVED***).items()
            if k in specs_by_id and isinstance(specs_by_id[k][1], ConfigurableField)
        ***REMOVED***
        configurable_single_options = {
            k: v.options[(config.get("configurable", {***REMOVED***).get(v.id) or v.default)]
        ...
        ***REMOVED***
        configurable_multi_options = {
            k: [
                v.options[o]
                for o in config.get("configurable", {***REMOVED***).get(v.id, v.default)
            ]
            for k, v in self.fields.items()
        ...
        ***REMOVED***
        ...
        if configurable:
            return (
                self.default.__class__(**{**self.default.__dict__, **configurable***REMOVED***),
                config,
***REMOVED***
        else:
...
class RunnableConfigurableAlternatives(DynamicRunnable[Input, Output]):
    ...
    ***REMOVED***,
            ...
            + (
                [
                    prefix_config_spec(s, f"{self.which.id***REMOVED***=={self.default_key***REMOVED***")
...
***REMOVED***
            ...
            + [
                prefix_config_spec(s, f"{self.which.id***REMOVED***=={alt_key***REMOVED***")
                if self.prefix_keys
                else s
    ...
    def _prepare(
        ...
        config = ensure_config(config)
        which = config.get("configurable", {***REMOVED***).get(self.which.id, self.default_key)
        ...
        if self.prefix_keys:
            config = cast(
                RunnableConfig,
                {
                    **config,
                    "configurable": {
                        _strremoveprefix(k, f"{self.which.id***REMOVED***=={which***REMOVED***/"): v
                        for k, v in config.get("configurable", {***REMOVED***).items()
                    ***REMOVED***,
                ***REMOVED***,
***REMOVED***
        ...
        if which == self.default_key:
            ...
            raise ValueError(f"Unknown alternative: {which***REMOVED***")
...
def prefix_config_spec(
    ...
    return (
        ConfigurableFieldSpec(
            id=f"{prefix***REMOVED***/{spec.id***REMOVED***",
            name=spec.name,
            description=spec.description,
            annotation=spec.annotation,
