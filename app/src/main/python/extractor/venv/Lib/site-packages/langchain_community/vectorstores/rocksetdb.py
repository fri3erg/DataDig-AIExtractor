...
***REMOVED***
    ...
    def similarity_search_by_vector_with_relevance_scores(
        ...
        **kwargs: Any,
        ...
        try:
            query_response = self._client.Queries.query(sql={"query": q_str***REMOVED***)
        ...
        for document in query_response.results:
            ...
            assert isinstance(
                document, dict
***REMOVED***, "document should be of type `dict[str,Any]`. But found: `{***REMOVED***`".format(
                type(document)
***REMOVED***
            for k, v in document.items():
                if k == self._text_key:
                    assert isinstance(v, str), (
                        "page content stored in column `{***REMOVED***` must be of type `str`. "
                        "But found: `{***REMOVED***`"
        ***REMOVED***.format(self._text_key, type(v))
                    page_content = v
                elif k == "dist":
                    assert isinstance(v, float), (
                        "Computed distance between vectors must of type `float`. "
                        "But found {***REMOVED***"
        ***REMOVED***.format(type(v))
                    score = v
...
***REMOVED***
    ...
    def _build_query_sql(
        ...
        distance_str = f"""{distance_func.value***REMOVED***({self._embedding_key***REMOVED***, \
[{q_embedding_str***REMOVED***]) as dist"""
        where_str = f"WHERE {where_str***REMOVED***\n" if where_str else ""
        return f"""\
SELECT * EXCEPT({self._embedding_key***REMOVED***), {distance_str***REMOVED***
FROM {self._workspace***REMOVED***.{self._collection_name***REMOVED***
{where_str***REMOVED***\
ORDER BY dist {distance_func.order_by()***REMOVED***
LIMIT {str(k)***REMOVED***
"""
