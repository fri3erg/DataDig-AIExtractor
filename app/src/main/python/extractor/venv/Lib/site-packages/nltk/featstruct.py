...
class FeatStruct(SubstituteBindingsI):
    ...
    ***REMOVED***:
    ...
    def cyclic(self):
        ...
        """
        return self._find_reentrances({***REMOVED***)[id(self)]
    ...
    def __repr__(self):
        ...
        """
        return self._repr(self._find_reentrances({***REMOVED***), {***REMOVED***)
...
class FeatDict(FeatStruct, dict):
    ...
    def __str__(self):
        ...
        """
        return "\n".join(self._str(self._find_reentrances({***REMOVED***), {***REMOVED***))
    ...
    def _repr(self, reentrances, reentrance_ids):
        ...
        for (fname, fval) in sorted(self.items()):
            ...
            if id(fval) in reentrance_ids:
                segments.append(f"{fname***REMOVED***->({reentrance_ids[id(fval)]***REMOVED***)")
            elif (
...
***REMOVED***:
                ...
                segments.append(f"{fname***REMOVED***={fval.name***REMOVED***")
            elif fval is True:
                segments.append("+%s" % fname)
            elif fval is False:
                segments.append("-%s" % fname)
            elif isinstance(fval, Expression):
                segments.append(f"{fname***REMOVED***=<{fval***REMOVED***>")
            elif not isinstance(fval, FeatStruct):
                segments.append(f"{fname***REMOVED***={repr(fval)***REMOVED***")
            else:
                fval_repr = fval._repr(reentrances, reentrance_ids)
                segments.append(f"{fname***REMOVED***={fval_repr***REMOVED***")
        # If it's reentrant, then add on an identifier tag.
        if reentrances[id(self)]:
            prefix = f"({reentrance_ids[id(self)]***REMOVED***){prefix***REMOVED***"
        return "{***REMOVED***[{***REMOVED***]{***REMOVED***".format(prefix, ", ".join(segments), suffix)
    ...
    def _str(self, reentrances, reentrance_ids):
        ...
        for (fname, fval) in sorted(self.items()):
            ...
            if isinstance(fval, Variable):
                lines.append(f"{fname***REMOVED*** = {fval.name***REMOVED***")

            elif isinstance(fval, Expression):
                lines.append(f"{fname***REMOVED*** = <{fval***REMOVED***>")

            elif isinstance(fval, FeatList):
                fval_repr = fval._repr(reentrances, reentrance_ids)
                lines.append(f"{fname***REMOVED*** = {repr(fval_repr)***REMOVED***")

            elif not isinstance(fval, FeatDict):
                # It's not a nested feature structure -- just print it.
                lines.append(f"{fname***REMOVED*** = {repr(fval)***REMOVED***")

            elif id(fval) in reentrance_ids:
                # It's a feature structure we've seen before -- print
                # the reentrance id.
                lines.append(f"{fname***REMOVED*** -> ({reentrance_ids[id(fval)]***REMOVED***)")
    ...
    ***REMOVED***
        ...
        lines = ["[ {***REMOVED***{***REMOVED*** ]".format(line, " " * (maxlen - len(line))) for line in lines]
...
class FeatList(FeatStruct, list):
    ...
    def _repr(self, reentrances, reentrance_ids):
        ...
        return "{***REMOVED***[{***REMOVED***]".format(prefix, ", ".join(segments))
...
def retract_bindings(fstruct, bindings, fs_class="default"):
    ...
    inv_bindings = {id(val): var for (var, val) in bindings.items()***REMOVED***
...
def rename_variables(
    ...
    the same dictionary:
        ...
        >>> new_vars = {***REMOVED***  # Maps old vars to alpha-renamed vars
...
def _rename_variable(var, used_vars):
    ...
    if not name:
        name = "?"
    while Variable(f"{name***REMOVED***{n***REMOVED***") in used_vars:
        n += 1
    return Variable(f"{name***REMOVED***{n***REMOVED***")
...
***REMOVED***
    ...
    if rename_vars:
        ...
        _rename_variables(fstruct2copy, vars1, vars2, {***REMOVED***, fs_class, set())
...
def _trace_bindings(path, bindings):
    ...
    if len(bindings) > 0:
        ...
        bindstr = "{%s***REMOVED***" % ", ".join(
            f"{var***REMOVED***: {_trace_valrepr(val)***REMOVED***" for (var, val) in binditems
        )
...
class FeatureValueTuple(SubstituteBindingsSequence, tuple):
    ...
    def __repr__(self):  # [xx] really use %s here?
        if len(self) == 0:
            return "()"
        return "(%s)" % ", ".join(f"{b***REMOVED***" for b in self)
...
class FeatureValueSet(SubstituteBindingsSequence, frozenset):
    ...
    def __repr__(self):  # [xx] really use %s here?
        if len(self) == 0:
            return "{/***REMOVED***"  # distinguish from dict.
        # n.b., we sort the string reprs of our elements, to ensure
        # that our own repr is deterministic.
        return "{%s***REMOVED***" % ", ".join(sorted(f"{b***REMOVED***" for b in self))
...
class FeatStructReader:
    def __init__(
        ...
        self._features = {f.name: f for f in features***REMOVED***
    ...
    def fromstring(self, s, fstruct=None):
        ...
        """
        s = s.strip()
        value, position = self.read_partial(s, 0, {***REMOVED***, fstruct)
        if position != len(s):
    ...
    _SYM_CONSTS = {"None": None, "True": True, "False": False***REMOVED***
    ...
    def read_set_value(self, s, position, reentrances, match):
        return self._read_seq_value(
            s, position, reentrances, match, "***REMOVED***", FeatureValueSet, FeatureValueUnion
        )
