...
ll_params = [
    ...
    ((1,), True, "tuple"),
    ((), True, "tuple-empty"),
    ({"a": 1***REMOVED***, True, "dict"),
    ({***REMOVED***, True, "dict-empty"),
    ({"a", 1***REMOVED***, "set", "set"),
    (set(), "set", "set-empty"),
    (frozenset({"a", 1***REMOVED***), "set", "frozenset"),
...
def test_is_list_like_generic():
    ...
    tst = MyDataFrame[int]({"x": [1, 2, 3]***REMOVED***)
...
@pytest.mark.parametrize("ll", [{***REMOVED***, {"A": 1***REMOVED***, Series([1]), collections.defaultdict()])
...
@pytest.mark.parametrize("ll", [(1, 2, 3), "a", Series({"pi": 3.14***REMOVED***)])
...
***REMOVED***,
    ...
    def test_maybe_convert_objects_itemsize(self, data0, data1):
        ...
        expected = np.array(data, dtype=f"{kind***REMOVED***{itemsize***REMOVED***")
        result = lib.maybe_convert_objects(arr)
        tm.assert_numpy_array_equal(result, expected)
...
class TestNumberScalar:
    ...
    def test_is_datetime_dtypes_with_tz(self, tz):
        dtype = f"datetime64[ns, {tz***REMOVED***]"
...
def test_datetimeindex_from_empty_datetime64_array(unit):
    idx = DatetimeIndex(np.array([], dtype=f"datetime64[{unit***REMOVED***]"))
    assert len(idx) == 0


def test_nan_to_nat_conversions():
    df = DataFrame(
        {"A": np.asarray(range(10), dtype="float64"), "B": Timestamp("20010101")***REMOVED***
    )
    df.iloc[3:6, :] = np.nan
    result = df.loc[4, "B"]
