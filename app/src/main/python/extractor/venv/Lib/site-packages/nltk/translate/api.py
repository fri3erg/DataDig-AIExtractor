...
class AlignedSent:
    ...
    def __repr__(self):
        ...
        return f"AlignedSent({words***REMOVED***, {mots***REMOVED***, {self._alignment!r***REMOVED***)"
    ...
    def _to_dot(self):
        ...
        for w in self._words:
            s += f'"{w***REMOVED***_source" [label="{w***REMOVED***"] \n'

        for w in self._mots:
            s += f'"{w***REMOVED***_target" [label="{w***REMOVED***"] \n'

        # Alignment
        for u, v in self._alignment:
            s += f'"{self._words[u]***REMOVED***_source" -- "{self._mots[v]***REMOVED***_target" \n'

        # Connect the source words
        for i in range(len(self._words) - 1):
            s += '"{***REMOVED***_source" -- "{***REMOVED***_source" [style=invis]\n'.format(
                self._words[i],
                self._words[i + 1],
***REMOVED***
        ...
        for i in range(len(self._mots) - 1):
            s += '"{***REMOVED***_target" -- "{***REMOVED***_target" [style=invis]\n'.format(
                self._mots[i],
                self._mots[i + 1],
***REMOVED***

        # Put it in the same rank
        s += "{rank = same; %s***REMOVED***\n" % (" ".join('"%s_source"' % w for w in self._words))
...
***REMOVED***
    ...
    def __str__(self):
        ...
        return f"<AlignedSent: '{source***REMOVED***' -> '{target***REMOVED***'>"
