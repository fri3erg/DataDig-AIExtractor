...
Options:
  ...
  --short-latex    Create 'incomplete' LaTeX document (without commands
                   \\documentclass, \\tableofcontents, and \\begin{{document***REMOVED******REMOVED***,
...
Version:     {f2py_version***REMOVED***
numpy Version: {numpy_version***REMOVED***
    ...
    for l in inputline:
        if l == '':
            ...
            try:
                ...
                errmess(f'OSError: {detail!s***REMOVED***. Skipping file "{l!s***REMOVED***".\n')
        elif f == -1:
            skipfuncs.append(l)
        elif f == 0:
            onlyfuncs.append(l)
...
def buildmodules(lst):
    ...
    modules, mnames, isusedby = [], [], {***REMOVED***
    for item in lst:
            ...
            if 'use' in module:
                for u in module['use'].keys():
                    if u in isusedby and u in mnames:
                        ...
                        outmess(
                            f'\tModule "{name***REMOVED***" uses nonexisting "{u***REMOVED***" '
...
def run_main(comline_list):
    ...
    if args.module_name:
        if "-h" in comline_list:
            modname = (
                args.module_name
***REMOVED***  # Directly use from args when -h is present
        else:
            modname = validate_modulename(
                pyf_files, args.module_name
***REMOVED***  # Validate modname when -h is not present
    ...
    for plist in postlist:
        if plist['block'] == 'python module' and '__user__' in plist['name']:
            if plist['name'] in isusedby:
                # if not quiet:
                outmess(
                    f'Skipping Makefile build for module "{plist["name"]***REMOVED***" '
                    'which is used by {***REMOVED***\n'.format(
                        ','.join(f'"{s***REMOVED***"' for s in isusedby[plist['name']])))
    if 'signsfile' in options:
    ...
    for mn in ret.keys():
        dict_append(ret[mn], {'csrc': fobjcsrc, 'h': fobjhsrc***REMOVED***)
    return ret
...
def run_compile():
    ...
    if backend_key == 'meson':
        if not pyf_files:
            ...
            run_main(f" {' '.join(f2py_flags)***REMOVED*** -m {modulename***REMOVED*** {' '.join(sources)***REMOVED***".split())
        else:
            run_main(f" {' '.join(f2py_flags)***REMOVED*** {' '.join(pyf_files)***REMOVED***".split())
...
def validate_modulename(pyf_files, modulename='untitled'):
    ...
    if pyf_files:
        ...
        if modulename != pyf_modname:
            outmess(
                f"Ignoring -m {modulename***REMOVED***.\n"
                f"{pyff***REMOVED*** defines {pyf_modname***REMOVED*** to be the modulename.\n"
***REMOVED***
