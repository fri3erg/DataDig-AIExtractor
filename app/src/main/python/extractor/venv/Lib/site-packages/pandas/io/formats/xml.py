...
class _BaseXMLFormatter:
    ...
    def _validate_columns(self) -> None:
        ...
        if self.attr_cols and not is_list_like(self.attr_cols):
            raise TypeError(
                f"{type(self.attr_cols).__name__***REMOVED*** is not a valid type for attr_cols"
***REMOVED***

        if self.elem_cols and not is_list_like(self.elem_cols):
            raise TypeError(
                f"{type(self.elem_cols).__name__***REMOVED*** is not a valid type for elem_cols"
***REMOVED***
    ...
    ***REMOVED***
    ...
    def _other_namespaces(self) -> dict:
        ...
        nmsp_dict: dict[str, str] = {***REMOVED***
        if self.namespaces:
            nmsp_dict = {
                f"xmlns{p if p=='' else f':{p***REMOVED***'***REMOVED***": n
    ...
    def _build_attribs(self, d: dict[str, Any], elem_row: Any) -> Any:
        ...
        for col in self.attr_cols:
            ...
            try:
                ...
                raise KeyError(f"no valid column, {col***REMOVED***")
        return elem_row
...
***REMOVED***
        return f"{self.prefix_uri***REMOVED***{flat_col***REMOVED***"
...
class EtreeXMLFormatter(_BaseXMLFormatter):
    ...
    def _build_tree(self) -> bytes:
        ...
        self.root = Element(
            f"{self.prefix_uri***REMOVED***{self.root_name***REMOVED***", attrib=self._other_namespaces()
        )

        for d in self.frame_dicts.values():
            elem_row = SubElement(self.root, f"{self.prefix_uri***REMOVED***{self.row_name***REMOVED***")
...
***REMOVED***
    ...
    def _get_prefix_uri(self) -> str:
        ...
        if self.namespaces:
            ...
            if self.prefix:
                try:
                    uri = f"{{{self.namespaces[self.prefix]***REMOVED******REMOVED******REMOVED***"
                except KeyError:
                    raise KeyError(f"{self.prefix***REMOVED*** is not included in namespaces")
            elif "" in self.namespaces:
                uri = f'{{{self.namespaces[""]***REMOVED******REMOVED******REMOVED***'
            else:
                uri = ""
...
class LxmlXMLFormatter(_BaseXMLFormatter):
    ...
    def _build_tree(self) -> bytes:
        ...
        self.root = Element(f"{self.prefix_uri***REMOVED***{self.root_name***REMOVED***", nsmap=self.namespaces)
