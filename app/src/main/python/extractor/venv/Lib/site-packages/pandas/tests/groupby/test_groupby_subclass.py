...
def test_groupby_preserves_subclass(obj, groupby_func):
    ...
    if isinstance(obj, Series) and groupby_func in {"corrwith"***REMOVED***:
        pytest.skip(f"Not applicable for Series and {groupby_func***REMOVED***")
    ...
    msg = f"{type(grouped).__name__***REMOVED***.fillna is deprecated"
    ...
    slices = {"ngroup", "cumcount", "size"***REMOVED***
...
def test_groupby_preserves_metadata():
    # GH-37343
    custom_df = tm.SubclassedDataFrame({"a": [1, 2, 3], "b": [1, 1, 2], "c": [7, 8, 9]***REMOVED***)
