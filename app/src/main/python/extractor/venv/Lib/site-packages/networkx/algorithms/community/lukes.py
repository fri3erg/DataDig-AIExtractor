    ...
    for x in all_n_attr:
        if not isinstance(x, int):
            raise TypeError(
                ...
                f"values for node_weight ({node_weight***REMOVED***)"
***REMOVED***
    ...
    for lv in leaves:
        t_G.nodes[lv][PKEY] = {***REMOVED***
        slot = safe_G.nodes[lv][node_weight]
        t_G.nodes[lv][PKEY][slot] = [{lv***REMOVED***]
        t_G.nodes[lv][PKEY][0] = [{lv***REMOVED***]

    for inner in [x for x in t_G.nodes if x not in leaves]:
        t_G.nodes[inner][PKEY] = {***REMOVED***
        slot = safe_G.nodes[inner][node_weight]
        t_G.nodes[inner][PKEY][slot] = [{inner***REMOVED***]
