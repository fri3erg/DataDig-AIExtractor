...
class BaseTracer(BaseCallbackHandler, ABC):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.run_map: Dict[str, Run] = {***REMOVED***
    ...
    ***REMOVED***
        ...
        ***REMOVED***
                    ...
                    logger.debug(
                        f"Parent run with UUID {run.parent_run_id***REMOVED*** has no dotted_order."
        ***REMOVED***
            else:
                # Something wrong with tracer, parent run not found
                # Calculate the trace_id and dotted_order server side
                logger.debug(f"Parent run with UUID {run.parent_run_id***REMOVED*** not found.")
        else:
            run.trace_id = run.id
...
***REMOVED***:
    ...
    def _get_execution_order(self, parent_run_id: Optional[str] = None) -> int:
        ...
        if parent_run is None:
            logger.debug(f"Parent run with UUID {parent_run_id***REMOVED*** not found.")
            return 1
        if parent_run.child_execution_order is None:
            raise TracerException(
                f"Parent run with UUID {parent_run_id***REMOVED*** has no child execution order."
***REMOVED***
    ...
    def _get_run(self, run_id: UUID, run_type: Optional[str] = None) -> Run:
        try:
            ...
            raise TracerException(f"No indexed run ID {run_id***REMOVED***.") from exc
        if run_type is not None and run.run_type != run_type:
            raise TracerException(
                f"Found {run.run_type***REMOVED*** run at ID {run_id***REMOVED***, but expected {run_type***REMOVED*** run."
***REMOVED***
        return run
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        if metadata:
            kwargs.update({"metadata": metadata***REMOVED***)
        llm_run = Run(
            id=run_id,
            parent_run_id=parent_run_id,
            serialized=serialized,
            inputs={"prompts": prompts***REMOVED***,
            extra=kwargs,
            events=[{"name": "start", "time": start_time***REMOVED***],
    ...
    def on_llm_new_token(
        ...
        **kwargs: Any,
        ...
        event_kwargs: Dict[str, Any] = {"token": token***REMOVED***
        if chunk:
    ...
    def on_llm_end(self, response: LLMResult, *, run_id: UUID, **kwargs: Any) -> Run:
        ...
        ***REMOVED***
        ...
        llm_run.events.append({"name": "end", "time": llm_run.end_time***REMOVED***)
    ...
    def on_llm_error(
        ...
        **kwargs: Any,
        ...
        llm_run.end_time = datetime.now(timezone.utc)
        llm_run.events.append({"name": "error", "time": llm_run.end_time***REMOVED***)
        self._end_trace(llm_run)
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        chain_run = Run(
            ...
            parent_run_id=parent_run_id,
            serialized=serialized,
            inputs=inputs if isinstance(inputs, dict) else {"input": inputs***REMOVED***,
            extra=kwargs,
            events=[{"name": "start", "time": start_time***REMOVED***],
            start_time=start_time,
            execution_order=execution_order,
    ...
    def on_chain_end(
        ...
        **kwargs: Any,
        ...
        chain_run.outputs = (
            outputs if isinstance(outputs, dict) else {"output": outputs***REMOVED***
        )
        chain_run.end_time = datetime.now(timezone.utc)
        chain_run.events.append({"name": "end", "time": chain_run.end_time***REMOVED***)
        if inputs is not None:
            chain_run.inputs = inputs if isinstance(inputs, dict) else {"input": inputs***REMOVED***
    ...
    def on_chain_error(
        ...
        **kwargs: Any,
        ...
        chain_run.end_time = datetime.now(timezone.utc)
        chain_run.events.append({"name": "error", "time": chain_run.end_time***REMOVED***)
        if inputs is not None:
            chain_run.inputs = inputs if isinstance(inputs, dict) else {"input": inputs***REMOVED***
        self._end_trace(chain_run)
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        tool_run = Run(
            ...
            parent_run_id=parent_run_id,
            serialized=serialized,
            inputs={"input": input_str***REMOVED***,
            extra=kwargs,
            events=[{"name": "start", "time": start_time***REMOVED***],
            start_time=start_time,
            execution_order=execution_order,
    ...
    def on_tool_end(self, output: str, *, run_id: UUID, **kwargs: Any) -> Run:
        ...
        tool_run.outputs = {"output": output***REMOVED***
        tool_run.end_time = datetime.now(timezone.utc)
        tool_run.events.append({"name": "end", "time": tool_run.end_time***REMOVED***)
    ...
    def on_tool_error(
        ...
        **kwargs: Any,
        ...
        tool_run.end_time = datetime.now(timezone.utc)
        tool_run.events.append({"name": "error", "time": tool_run.end_time***REMOVED***)
        self._end_trace(tool_run)
    ...
    def on_retriever_error(
        ...
        **kwargs: Any,
        ...
        retrieval_run.events.append({"name": "error", "time": retrieval_run.end_time***REMOVED***)
    ...
    def on_retriever_end(
        ...
        retrieval_run = self._get_run(run_id, run_type="retriever")
        retrieval_run.outputs = {"documents": documents***REMOVED***
        retrieval_run.end_time = datetime.now(timezone.utc)
        retrieval_run.events.append({"name": "end", "time": retrieval_run.end_time***REMOVED***)
        self._end_trace(retrieval_run)
