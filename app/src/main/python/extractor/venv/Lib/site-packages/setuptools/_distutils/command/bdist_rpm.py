...
***REMOVED***
    ...
    def run(self):  # noqa: C901
        ...
        # list is empty)
        nvr_string = "%{name***REMOVED***-%{version***REMOVED***-%{release***REMOVED***"
        src_rpm = nvr_string + ".src.rpm"
        non_src_rpm = "%{arch***REMOVED***/" + nvr_string + ".%{arch***REMOVED***.rpm"
        q_cmd = r"rpm -q --qf '{***REMOVED*** {***REMOVED***\n' --specfile '{***REMOVED***'".format(
            src_rpm,
            non_src_rpm,
            spec_path,
        )
    ...
    def _make_spec_file(self):  # noqa: C901
        ...
        vendor_hook = subprocess.getoutput('rpm --eval %{__os_install_post***REMOVED***')
        ...
        fixed = "brp-python-bytecompile %{__python***REMOVED*** \\\n"
        ...
        if self.use_bzip2:
            spec_file.append('Source0: %{name***REMOVED***-%{unmangled_version***REMOVED***.tar.bz2')
        else:
            spec_file.append('Source0: %{name***REMOVED***-%{unmangled_version***REMOVED***.tar.gz')

        spec_file.extend(
            [
                'License: ' + (self.distribution.get_license() or "UNKNOWN"),
                'Group: ' + self.group,
                'BuildRoot: %{_tmppath***REMOVED***/%{name***REMOVED***-%{version***REMOVED***-%{release***REMOVED***-buildroot',
                'Prefix: %{_prefix***REMOVED***',
            ]
        )
        ...
        for field in (
            ...
            if isinstance(val, list):
                spec_file.append('{***REMOVED***: {***REMOVED***'.format(field, ' '.join(val)))
            elif val is not None:
                spec_file.append('{***REMOVED***: {***REMOVED***'.format(field, val))
        ...
        def_setup_call = "{***REMOVED*** {***REMOVED***".format(self.python, os.path.basename(sys.argv[0]))
        ...
        script_options = [
            ('prep', 'prep_script', "%setup -n %{name***REMOVED***-%{unmangled_version***REMOVED***"),
