    ...
    **Security Note**: This loader is a crawler that will start crawling
    ...
    def __init__(
        ...
        if any(url.startswith(exclude_dir) for exclude_dir in self.exclude_dirs):
            raise ValueError(
                f"Base url is included in exclude_dirs. Received base_url: {url***REMOVED*** and "
                f"exclude_dirs: {self.exclude_dirs***REMOVED***"
***REMOVED***
    ...
    def _get_child_links_recursive(
        ...
        try:
            ...
            if self.check_response_status and 400 <= response.status_code <= 599:
                raise ValueError(f"Received HTTP status {response.status_code***REMOVED***")
        except Exception as e:
            logger.warning(
                f"Unable to load from {url***REMOVED***. Received error {e***REMOVED*** of type "
                f"{e.__class__.__name__***REMOVED***"
***REMOVED***
            return
        ...
        try:
            async with session.get(url) as response:
                ...
                if self.check_response_status and 400 <= response.status <= 599:
                    raise ValueError(f"Received HTTP status {response.status***REMOVED***")
        except (aiohttp.client_exceptions.InvalidURL, Exception) as e:
            logger.warning(
                f"Unable to load {url***REMOVED***. Received error {e***REMOVED*** of type "
                f"{e.__class__.__name__***REMOVED***"
***REMOVED***
            if close_session:
                await session.close()
