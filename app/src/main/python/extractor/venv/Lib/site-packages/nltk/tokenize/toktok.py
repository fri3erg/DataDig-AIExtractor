...
class ToktokTokenizer(TokenizerI):
    ...
    FUNKY_PUNCT_1 = re.compile(r'([،;؛¿!"\])***REMOVED***»›”؟¡%٪°±©®।॥…])'), r" \1 "
    ...
    MULTI_COMMAS = re.compile(r"(,{2,***REMOVED***)"), r" \1 "
    # Treat continuous dashes as fake en-dash, etc.
    MULTI_DASHES = re.compile(r"(-{2,***REMOVED***)"), r" \1 "
    # Treat multiple periods as a thing (eg. ellipsis)
    MULTI_DOTS = re.compile(r"(\.{2,***REMOVED***)"), r" \1 "

    # This is the \p{Open_Punctuation***REMOVED*** from Perl's perluniprops
    ...
    # This is the \p{Close_Punctuation***REMOVED*** from Perl's perluniprops
    CLOSE_PUNCT = str(
        ")]***REMOVED***\u0f3b\u0f3d\u169c\u2046\u207e\u208e\u232a"
        "\u2769\u276b\u276d\u276f\u2771\u2773\u2775\u27c6"
        "\u27e7\u27e9\u27eb\u27ed\u27ef\u2984\u2986\u2988"
    ...
    OPEN_PUNCT_RE = re.compile(f"([{OPEN_PUNCT***REMOVED***])"), r"\1 "
    # Pad spaces before closing punctuations.
    CLOSE_PUNCT_RE = re.compile(f"([{CLOSE_PUNCT***REMOVED***])"), r"\1 "
    # Pad spaces after currency symbols.
    CURRENCY_SYM_RE = re.compile(f"([{CURRENCY_SYM***REMOVED***])"), r"\1 "

    # Use for tokenizing URL-unfriendly characters: [:/?#]
    URL_FOE_1 = re.compile(r":(?!//)"), r" : "  # in perl s{:(?!//)***REMOVED***{ : ***REMOVED***g;
    URL_FOE_2 = re.compile(r"\?(?!\S)"), r" ? "  # in perl s{\?(?!\S)***REMOVED***{ ? ***REMOVED***g;
    # in perl: m{://***REMOVED*** or m{\S+\.\S+/\S+***REMOVED*** or s{/***REMOVED***{ / ***REMOVED***g;
    URL_FOE_3 = re.compile(r"(:\/\/)[\S+\.\S+\/\S+][\/]"), " / "
    URL_FOE_4 = re.compile(r" /"), r" / "  # s{ /***REMOVED***{ / ***REMOVED***g;
    ...
    ONE_SPACE = re.compile(r" {2,***REMOVED***"), " "
