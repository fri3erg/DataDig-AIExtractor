...
def ensure_config(config: Optional[RunnableConfig] = None) -> RunnableConfig:
    ...
    if var_config := var_child_runnable_config.get():
        empty.update(
            cast(RunnableConfig, {k: v for k, v in var_config.items() if v is not None***REMOVED***)
        )
...
def get_config_list(
    ...
    """
    if length < 0:
        raise ValueError(f"length must be >= 0, but got {length***REMOVED***")
    if isinstance(config, list) and len(config) != length:
        raise ValueError(
            f"config must be a list of the same length as inputs, "
            f"but got {len(config)***REMOVED*** configs for {length***REMOVED*** inputs"
        )
...
def patch_config(
    ...
    *,
    ...
    if configurable is not None:
        config["configurable"] = {**config.get("configurable", {***REMOVED***), **configurable***REMOVED***
    return config
        ...
        for key in config:
            if key == "metadata":
                base[key] = {  # type: ignore
                    **base.get(key, {***REMOVED***),  # type: ignore
                    **(config.get(key) or {***REMOVED***),  # type: ignore
                ***REMOVED***
            elif key == "tags":
