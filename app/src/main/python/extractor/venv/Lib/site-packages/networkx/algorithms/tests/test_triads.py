...
def test_all_triplets():
    ...
    G.add_edges_from(["01", "02", "03", "04", "05", "12", "16", "51", "56", "65"])
    expected = [
        f"{i***REMOVED***,{j***REMOVED***,{k***REMOVED***"
        for i in range(7)
...
def test_triadic_census_short_path_nodelist():
    ...
    expected = {"021C": 1***REMOVED***
    for nl in ["a", "b", "c", "ab", "ac", "bc", "abc"]:
        triad_census = nx.triadic_census(G, nodelist=nl)
        assert expected == {typ: cnt for typ, cnt in triad_census.items() if cnt > 0***REMOVED***
...
def test_triadic_census_tiny_graphs():
    ...
    assert {***REMOVED*** == {typ: cnt for typ, cnt in tc.items() if cnt > 0***REMOVED***
...
def test_triadic_census_selfloops():
    ...
    for n in GG:
        ...
        assert expected == {typ: cnt for typ, cnt in tc.items() if cnt > 0***REMOVED***
...
def test_triadic_census_four_path_nodelist():
    G = nx.path_graph("abcd", create_using=nx.DiGraph)
    expected_end = {"012": 2, "021C": 1***REMOVED***
    expected_mid = {"012": 1, "021C": 2***REMOVED***
    a_triad_census = nx.triadic_census(G, nodelist=["a"])
    assert expected_end == {typ: cnt for typ, cnt in a_triad_census.items() if cnt > 0***REMOVED***
    b_triad_census = nx.triadic_census(G, nodelist=["b"])
    assert expected_mid == {typ: cnt for typ, cnt in b_triad_census.items() if cnt > 0***REMOVED***
    c_triad_census = nx.triadic_census(G, nodelist=["c"])
    assert expected_mid == {typ: cnt for typ, cnt in c_triad_census.items() if cnt > 0***REMOVED***
    d_triad_census = nx.triadic_census(G, nodelist=["d"])
    assert expected_end == {typ: cnt for typ, cnt in d_triad_census.items() if cnt > 0***REMOVED***
...
def test_triadic_census_nodelist():
    ...
    expected = {
        ...
        "021C": 2,
    ***REMOVED***
    actual = {k: 0 for k in expected***REMOVED***
    for node in G.nodes():
...
def test_triadic_census_on_random_graph(N):
    ...
    tc2 = {tt: len(tbt[tt]) for tt in tc1***REMOVED***
    assert tc1 == tc2

    for n in G:
        tc1 = nx.triadic_census(G, nodelist={n***REMOVED***)
        tc2 = {tt: sum(1 for t in tbt.get(tt, []) if n in t) for tt in tc1***REMOVED***
        assert tc1 == tc2
