...
class TestGetDummies:
    @pytest.fixture
    def df(self):
        return DataFrame({"A": ["a", "b", "a"], "B": ["b", "b", "c"], "C": [1, 2, 3]***REMOVED***)
    ...
    def test_get_dummies_basic(self, sparse, dtype):
        ...
        expected = DataFrame(
            {"a": [1, 0, 0], "b": [0, 1, 0], "c": [0, 0, 1]***REMOVED***,
    ...
    def test_get_dummies_basic_types(self, sparse, dtype, using_infer_string):
        ...
        s_series = Series(s_list)
        s_df = DataFrame(
            {"a": [0, 1, 0, 1, 2], "b": ["A", "A", "B", "C", "C"], "c": [2, 3, 3, 3, 2]***REMOVED***
        )
        ...
        if sparse:
            dtype_name = f"Sparse[{self.effective_dtype(dtype).name***REMOVED***, {fill_value***REMOVED***]"
        else:
            dtype_name = self.effective_dtype(dtype).name

        expected = Series({dtype_name: 8***REMOVED***, name="count")
        ...
        expected_counts = {"int64": 1, key: 1***REMOVED***
    ...
    def test_get_dummies_include_na(self, sparse, dtype):
        ...
        exp = DataFrame(
            {"a": [1, 0, 0], "b": [0, 1, 0]***REMOVED***, dtype=self.effective_dtype(dtype)
        )
        if sparse:
            if dtype.kind == "b":
        ...
        exp_na = DataFrame(
            {np.nan: [0, 0, 1], "a": [1, 0, 0], "b": [0, 1, 0]***REMOVED***,
            dtype=self.effective_dtype(dtype),
        )
    ...
    def test_get_dummies_unicode(self, sparse):
        ...
        exp = DataFrame(
            {"letter_e": [True, False, False], f"letter_{eacute***REMOVED***": [False, True, True]***REMOVED***
        )
        if sparse:
            exp = exp.apply(SparseArray, fill_value=False)
        tm.assert_frame_equal(res, exp)
    ...
    def test_dataframe_dummies_all_obj(self, df, sparse):
        ...
        expected = DataFrame(
            {"A_a": [1, 0, 1], "A_b": [0, 1, 0], "B_b": [1, 1, 0], "B_c": [0, 0, 1]***REMOVED***,
            dtype=bool,
        )
        if sparse:
            expected = DataFrame(
...
***REMOVED***
    ...
    def test_dataframe_dummies_string_dtype(self, df, using_infer_string):
        # GH44965
        df = df[["A", "B"]]
        df = df.astype({"A": "object", "B": "string"***REMOVED***)
    ...
    def test_dataframe_dummies_prefix_str(self, df, sparse):
        ...
        expected = expected.astype({"C": np.int64***REMOVED***)
        if sparse:
...
***REMOVED***
    ...
    def test_dataframe_dummies_prefix_sep(self, df, sparse):
        ...
        expected = expected.rename(columns={"B..b": "B__b", "B..c": "B__c"***REMOVED***)
        tm.assert_frame_equal(result, expected)

        result = get_dummies(df, prefix_sep={"A": "..", "B": "__"***REMOVED***, sparse=sparse)
        tm.assert_frame_equal(result, expected)
    ...
    def test_dataframe_dummies_prefix_dict(self, sparse):
        prefixes = {"A": "from_A", "B": "from_B"***REMOVED***
        df = DataFrame({"C": [1, 2, 3], "A": ["a", "b", "a"], "B": ["b", "b", "c"]***REMOVED***)
        result = get_dummies(df, prefix=prefixes, sparse=sparse)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"data": DataFrame({"ä": ["a"]***REMOVED***)***REMOVED***,
                DataFrame({"ä_a": [True]***REMOVED***),
***REMOVED***,
            (
                {"data": DataFrame({"x": ["ä"]***REMOVED***)***REMOVED***,
                DataFrame({"x_ä": [True]***REMOVED***),
***REMOVED***,
            (
                {"data": DataFrame({"x": ["a"]***REMOVED***), "prefix": "ä"***REMOVED***,
                DataFrame({"ä_a": [True]***REMOVED***),
***REMOVED***,
            (
                {"data": DataFrame({"x": ["a"]***REMOVED***), "prefix_sep": "ä"***REMOVED***,
                DataFrame({"xäa": [True]***REMOVED***),
***REMOVED***,
        ],
    )
    ...
    def test_get_dummies_basic_drop_first(self, sparse):
        ...
        expected = DataFrame({"b": [0, 1, 0], "c": [0, 0, 1]***REMOVED***, dtype=bool)
    ...
    def test_get_dummies_basic_drop_first_NA(self, sparse):
        ...
        exp = DataFrame({"b": [0, 1, 0]***REMOVED***, dtype=bool)
        if sparse:
            exp = exp.apply(SparseArray, fill_value=False)
        ...
        exp_na = DataFrame({"b": [0, 1, 0], np.nan: [0, 0, 1]***REMOVED***, dtype=bool).reindex(
            ["b", np.nan], axis=1
        )
        if sparse:
            exp_na = exp_na.apply(SparseArray, fill_value=False)
    ...
    def test_dataframe_dummies_drop_first(self, df, sparse):
        ...
        expected = DataFrame({"A_b": [0, 1, 0], "B_c": [0, 0, 1]***REMOVED***, dtype=bool)
        if sparse:
            expected = expected.apply(SparseArray, fill_value=False)
        tm.assert_frame_equal(result, expected)
    ...
    def test_dataframe_dummies_drop_first_with_categorical(self, df, sparse, dtype):
        ...
        result = get_dummies(df, drop_first=True, sparse=sparse)
        expected = DataFrame(
            {"C": [1, 2, 3], "A_b": [0, 1, 0], "B_c": [0, 0, 1], "cat_y": [0, 1, 1]***REMOVED***
        )
        cols = ["A_b", "B_c", "cat_y"]
    ...
    def test_get_dummies_dont_sparsify_all_columns(self, sparse):
        # GH18914
        df = DataFrame.from_dict({"GDP": [1, 2], "Nation": ["AB", "CD"]***REMOVED***)
    ...
    def test_get_dummies_duplicate_columns(self, df):
        ...
        expected = expected.astype({"A": np.int64***REMOVED***)
    ...
    def test_get_dummies_ea_dtype_series(self, any_numeric_ea_and_arrow_dtype):
        ...
        expected = DataFrame(
            {"a": [1, 0, 0, 1], "b": [0, 1, 0, 0], "c": [0, 0, 1, 0]***REMOVED***,
    ...
    def test_get_dummies_ea_dtype_dataframe(self, any_numeric_ea_and_arrow_dtype):
        # GH#32430
        df = DataFrame({"x": list("abca")***REMOVED***)
        result = get_dummies(df, dtype=any_numeric_ea_and_arrow_dtype)
        expected = DataFrame(
            {"x_a": [1, 0, 0, 1], "x_b": [0, 1, 0, 0], "x_c": [0, 0, 1, 0]***REMOVED***,
            dtype=any_numeric_ea_and_arrow_dtype,
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_get_dummies_ea_dtype(self):
        ...
        for dtype, exp_dtype in [
            ...
            df = DataFrame({"name": Series(["a"], dtype=dtype), "x": 1***REMOVED***)
            result = get_dummies(df)
            expected = DataFrame({"x": 1, "name_a": Series([True], dtype=exp_dtype)***REMOVED***)
            tm.assert_frame_equal(result, expected)

    @td.skip_if_no("pyarrow")
    def test_get_dummies_arrow_dtype(self):
        # GH#56273
        df = DataFrame({"name": Series(["a"], dtype=ArrowDtype(pa.string())), "x": 1***REMOVED***)
        result = get_dummies(df)
        expected = DataFrame({"x": 1, "name_a": Series([True], dtype="bool[pyarrow]")***REMOVED***)
        tm.assert_frame_equal(result, expected)
