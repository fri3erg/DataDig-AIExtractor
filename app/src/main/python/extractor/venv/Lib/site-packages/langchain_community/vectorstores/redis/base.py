...
***REMOVED*** from e
        ...
        try:
            ...
            raise ValueError(f"Redis failed to connect: {e***REMOVED***")

        self.client = redis_client
        self.relevance_score_fn = relevance_score_fn
        self._schema = self._get_schema_with_defaults(index_schema, vector_schema)
        self.key_prefix = key_prefix if key_prefix is not None else f"doc:{index_name***REMOVED***"
...
***REMOVED*** from e
            ...
            if index_schema:
                ...
                if user_schema != generated_schema:
                    logger.warning(
                        ...
                        + "ignore this message.\n"
                        + f"index_schema: {user_schema***REMOVED***\n"
                        + f"generated_schema: {generated_schema***REMOVED***\n"
        ***REMOVED***
            else:
    ...
    ***REMOVED***
        ...
        if not check_index_exists(instance.client, index_name):
            ...
            raise ValueError(
                f"Redis failed to connect: Index {index_name***REMOVED*** does not exist."
***REMOVED***
...
***REMOVED***
        try:
            ...
            raise ValueError(f"Your redis connected error: {e***REMOVED***")
    ...
    def _collect_metadata(self, result: "Document") -> Dict[str, Any]:
        ...
        for key in self._schema.metadata_keys:
            try:
                ...
                logger.warning(
                    f"Metadata key {key***REMOVED*** not found in metadata. "
                    + "Setting to None. \n"
                    + "Metadata fields defined for this instance: "
                    + f"{self._schema.metadata_keys***REMOVED***"
    ***REMOVED***
                meta[key] = None
        return meta
...
***REMOVED*** from e
        ...
        base_query = f"@{vector_key***REMOVED***:[VECTOR_RANGE $distance_threshold $vector]"

        if filter:
            base_query = "(" + base_query + " " + str(filter) + ")"

        query_string = base_query + "=>{$yield_distance_as: distance***REMOVED***"
...
***REMOVED*** from e
        ...
        query_prefix = "*"
        if filter:
            query_prefix = f"{str(filter)***REMOVED***"
        vector_key = self._schema.content_vector_key
        base_query = f"({query_prefix***REMOVED***)=>[KNN {k***REMOVED*** @{vector_key***REMOVED*** $vector AS distance]"
        ...
        ***REMOVED***
        for key, value in kwargs.items():
            if key in deprecated_kwargs:
                raise ValueError(
                    f"Keyword argument '{key***REMOVED***' is deprecated. "
                    f"Please use '{deprecated_kwargs[key]***REMOVED***' instead."
    ***REMOVED***
...
def _generate_field_schema(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ***REMOVED***
    ...
    for key, value in data.items():
        ...
        try:
            int(value)
            result["numeric"].append({"name": key***REMOVED***)
            continue
        ...
        if isinstance(value, (list, tuple)):
            if not value or isinstance(value[0], str):
                result["tag"].append({"name": key***REMOVED***)
            else:
                name = type(value[0]).__name__
                raise ValueError(
                    f"List/tuple values should contain strings: '{key***REMOVED***': {name***REMOVED***"
    ***REMOVED***
            continue

        # Check if value is string before processing further
        if isinstance(value, str):
            result["text"].append({"name": key***REMOVED***)
            continue
        ...
        raise ValueError(
            ...
            + f"for metadata: '{key***REMOVED***': {name***REMOVED***"
        )
...
def _prepare_metadata(metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def raise_error(key: str, value: Any) -> None:
        raise ValueError(
            f"Metadata value for key '{key***REMOVED***' must be a string, int, "
            + f"float, or list of strings. Got {type(value).__name__***REMOVED***"
        )

    clean_meta: Dict[str, Union[str, float, int]] = {***REMOVED***
...
***REMOVED***
            ...
            raise ValueError(f"search_type of {self.search_type***REMOVED*** not allowed.")
        return docs
