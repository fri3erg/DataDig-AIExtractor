...
class BasePage(GenericModel, Generic[_T]):
    ...
    def _info_to_options(self, info: PageInfo) -> FinalRequestOptions:
        ...
        if not isinstance(info.params, NotGiven):
            options.params = {**options.params, **info.params***REMOVED***
            return options
...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def __init__(
        ...
        *,
        ...
        self._custom_headers = custom_headers or {***REMOVED***
        self._custom_query = custom_query or {***REMOVED***
    ...
    def _make_status_error_from_response(
        ...
        if response.is_closed and not response.is_stream_consumed:
            ...
            body = None
            err_msg = f"Error code: {response.status_code***REMOVED***"
        else:
            ...
            try:
                ...
                err_msg = f"Error code: {response.status_code***REMOVED*** - {body***REMOVED***"
            except Exception:
                err_msg = err_text or f"Error code: {response.status_code***REMOVED***"
    ...
    def _build_headers(self, options: FinalRequestOptions) -> httpx.Headers:
        custom_headers = options.headers or {***REMOVED***
    ...
    def _build_request(
        ...
        if options.extra_json is not None:
            if json_data is None:
                ...
                raise RuntimeError(f"Unexpected JSON data type, {type(json_data)***REMOVED***, cannot merge with `extra_body`")
        ...
        if headers.get("Content-Type") == "multipart/form-data":
            ...
            if json_data:
                if not is_dict(json_data):
                    raise TypeError(
                        f"Expected query input to be a dictionary for multipart requests but got {type(json_data)***REMOVED*** instead."
        ***REMOVED***
    ...
    def _serialize_multipartform(self, data: Mapping[object, object]) -> dict[str, object]:
        ...
        serialized: dict[str, object] = {***REMOVED***
        for key, value in items:
            if key in serialized:
                raise ValueError(f"Duplicate key encountered: {key***REMOVED***; This behaviour is not supported")
    ...
    def user_agent(self) -> str:
        return f"{self.__class__.__name__***REMOVED***/Python {self._version***REMOVED***"
    ...
    def _idempotency_key(self) -> str:
        return f"stainless-python-retry-{uuid.uuid4()***REMOVED***"
...
***REMOVED***
        ...
        if inspect.isclass(origin) and issubclass(origin, BaseAPIResponse):
            if not issubclass(origin, APIResponse):
                raise TypeError(f"API Response types must subclass {APIResponse***REMOVED***; Received {origin***REMOVED***")
...
***REMOVED***
    ...
    def get(
        ...
        *,
        cast_to: Type[ResponseT],
        options: RequestOptions = {***REMOVED***,
        stream: Literal[False] = False,
    ) -> ResponseT:
        ...
...
***REMOVED***
        ...
        if inspect.isclass(origin) and issubclass(origin, BaseAPIResponse):
            if not issubclass(origin, AsyncAPIResponse):
                raise TypeError(f"API Response types must subclass {AsyncAPIResponse***REMOVED***; Received {origin***REMOVED***")
...
def make_request_options(
    *,
    ...
    if extra_query is not None:
        options["params"] = {**options.get("params", {***REMOVED***), **extra_query***REMOVED***
...
class OtherPlatform:
    ...
    def __str__(self) -> str:
        return f"Other:{self.name***REMOVED***"
...
def _merge_mappings(
    ...
    """
    merged = {**obj1, **obj2***REMOVED***
    return {key: value for key, value in merged.items() if not isinstance(value, Omit)***REMOVED***
