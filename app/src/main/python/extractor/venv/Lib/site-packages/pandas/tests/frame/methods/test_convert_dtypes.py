...
class TestConvertDtypes:
        ...
        expected = pd.DataFrame(
            {
                ...
                "b": pd.Series(["x", "y", "z"], dtype=f"string[{string_storage***REMOVED***]"),
            ***REMOVED***
        )
    ...
    ***REMOVED***,
    ...
    def test_pyarrow_backend_no_conversion(self):
        # GH#52872
        pytest.importorskip("pyarrow")
        df = pd.DataFrame({"a": [1, 2], "b": 1.5, "c": True, "d": "x"***REMOVED***)
        expected = df.copy()
        result = df.convert_dtypes(
    ...
    def test_convert_dtypes_avoid_block_splitting(self):
        # GH#55341
        df = pd.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": "a"***REMOVED***)
    ...
    def test_convert_dtypes_from_arrow(self):
        ...
        result = df.convert_dtypes()
        expected = df.astype({"a": "string[python]"***REMOVED***)
        tm.assert_frame_equal(result, expected)
