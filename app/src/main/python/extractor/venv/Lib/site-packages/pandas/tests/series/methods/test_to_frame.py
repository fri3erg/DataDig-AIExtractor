...
class TestToFrame:
    ...
    def test_to_frame(self, datetime_series):
        ...
        rs = datetime_series.to_frame()
        xp = DataFrame(
            {"testname": datetime_series.values***REMOVED***, index=datetime_series.index
        )
        tm.assert_frame_equal(rs, xp)

        rs = datetime_series.to_frame(name="testdifferent")
        xp = DataFrame(
            {"testdifferent": datetime_series.values***REMOVED***, index=datetime_series.index
        )
        tm.assert_frame_equal(rs, xp)
    ...
    def test_to_frame_expanddim(self):
        ...
        expected = SubclassedFrame({"X": [1, 2, 3]***REMOVED***)
