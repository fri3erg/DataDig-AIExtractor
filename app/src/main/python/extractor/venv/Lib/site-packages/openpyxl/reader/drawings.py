...
def find_images(archive, path):
    ...
    for rel in drawing._chart_rels:
        try:
            ...
            warn(f"Unable to read chart {rel.id***REMOVED*** from {path***REMOVED*** {e***REMOVED***")
            continue
        chart = read_chart(cs)
    ...
    for rel in drawing._blip_rels:
        ...
        if dep.Type == IMAGE_NS:
            try:
                ...
                msg = "The image {0***REMOVED*** will be removed because it cannot be read".format(dep.target)
                warn(msg)
                continue
            if image.format.upper() == "WMF": # cannot save
                msg = "{0***REMOVED*** image format is not supported so the image is being dropped".format(image.format)
                warn(msg)
                continue
