...
class TestConcatenate:
    ...
    def test_concat_copy(self, using_array_manager, using_copy_on_write):
        ...
        df3 = DataFrame({5: "foo"***REMOVED***, index=range(4))
    ...
    ***REMOVED***
    ...
    def test_with_mixed_tuples(self, sort):
        ...
        df1 = DataFrame({"A": "foo", ("B", 1): "bar"***REMOVED***, index=range(2))
        df2 = DataFrame({"B": "foo", ("B", 1): "bar"***REMOVED***, index=range(2))
    ...
    def test_dtype_coercion(self):
        # 12411
        df = DataFrame({"date": [pd.Timestamp("20130101").tz_localize("UTC"), pd.NaT]***REMOVED***)
    ...
    def test_concat_keys_with_none(self):
        ...
        result = concat({"a": None, "b": df0, "c": df0[:2], "d": df0[:1], "e": df0***REMOVED***)
        expected = concat({"b": df0, "c": df0[:2], "d": df0[:1], "e": df0***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_concat_bug_2972(self):
        ...
        expected = DataFrame({0: ts0, 1: ts1***REMOVED***)
    ...
    def test_concat_bug_3602(self):
        ...
        df2 = DataFrame(
            {"C": [9, 10, 11, 12], "misc": [1, 2, 3, 4], "prc": [6, 6, 6, 6]***REMOVED***
        )
        expected = DataFrame(
            [
                [0, 6, "rrr", 9, 1, 6],
    ...
    def test_concat_iterables(self):
        ...
        class CustomIterator1:
            ...
            def __getitem__(self, index):
                try:
                    return {0: df1, 1: df2***REMOVED***[index]
    ...
    def test_concat_ordered_dict(self):
        ...
        result = concat({"First": Series(range(3)), "Another": Series(range(4))***REMOVED***)
        tm.assert_series_equal(result, expected)
...
def test_concat_empty_and_non_empty_frame_regression():
    ...
    df1 = DataFrame({"foo": [1]***REMOVED***)
    df2 = DataFrame({"foo": []***REMOVED***)
    expected = DataFrame({"foo": [1.0]***REMOVED***)
...
@pytest.mark.parametrize(
    "obj",
    [
        tm.SubclassedDataFrame({"A": np.arange(0, 10)***REMOVED***),
...
def test_concat_frame_axis0_extension_dtypes():
    ...
    df1 = DataFrame({"a": pd.array([1, 2, 3], dtype="Int64")***REMOVED***)
    df2 = DataFrame({"a": np.array([4, 5, 6])***REMOVED***)

    result = concat([df1, df2], ignore_index=True)
    expected = DataFrame({"a": [1, 2, 3, 4, 5, 6]***REMOVED***, dtype="Int64")
    tm.assert_frame_equal(result, expected)

    result = concat([df2, df1], ignore_index=True)
    expected = DataFrame({"a": [4, 5, 6, 1, 2, 3]***REMOVED***, dtype="Int64")
    tm.assert_frame_equal(result, expected)


def test_concat_preserves_extension_int64_dtype():
    # GH 24768
    df_a = DataFrame({"a": [-1]***REMOVED***, dtype="Int64")
    df_b = DataFrame({"b": [1]***REMOVED***, dtype="Int64")
    result = concat([df_a, df_b], ignore_index=True)
    expected = DataFrame({"a": [-1, None], "b": [None, 1]***REMOVED***, dtype="Int64")
    tm.assert_frame_equal(result, expected)
...
def test_concat_repeated_keys(keys, integrity):
    # GH: 20816
    series_list = [Series({"a": 1***REMOVED***), Series({"b": 2***REMOVED***), Series({"c": 3***REMOVED***)]
...
def test_concat_null_object_with_dti():
    ...
    right = DataFrame(data={"C": [0.5274]***REMOVED***, index=dti)

    idx = Index([None], dtype="object", name="Maybe Time (UTC)")
    left = DataFrame(data={"A": [None], "B": [np.nan]***REMOVED***, index=idx)
...
def test_concat_drop_attrs(data):
    # GH#41828
    df1 = data.copy()
    df1.attrs = {1: 1***REMOVED***
    df2 = data.copy()
    df2.attrs = {1: 2***REMOVED***
    df = concat([df1, df2])
...
def test_concat_ignore_empty_object_float(empty_dtype, df_dtype):
    ...
    df = DataFrame({"foo": [1, 2], "bar": [1, 2]***REMOVED***, dtype=df_dtype)
...
def test_concat_ignore_all_na_object_float(empty_dtype, df_dtype):
    df = DataFrame({"foo": [1, 2], "bar": [1, 2]***REMOVED***, dtype=df_dtype)
    empty = DataFrame({"foo": [np.nan], "bar": [np.nan]***REMOVED***, dtype=empty_dtype)
    ...
    expected = DataFrame({"foo": [np.nan, 1, 2], "bar": [np.nan, 1, 2]***REMOVED***, dtype=df_dtype)
    tm.assert_frame_equal(result, expected)
...
def test_concat_ignore_empty_from_reindex():
    ...
    df1 = DataFrame({"a": [1], "b": [pd.Timestamp("2012-01-01")]***REMOVED***)
    df2 = DataFrame({"a": [2]***REMOVED***)
    ...
    expected = df1 = DataFrame({"a": [1, 2], "b": [pd.Timestamp("2012-01-01"), pd.NaT]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_concat_none_with_timezone_timestamp():
    # GH#52093
    df1 = DataFrame([{"A": None***REMOVED***])
    df2 = DataFrame([{"A": pd.Timestamp("1990-12-20 00:00:00+00:00")***REMOVED***])
    msg = "The behavior of DataFrame concatenation with empty or all-NA entries"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = concat([df1, df2], ignore_index=True)
    expected = DataFrame({"A": [None, pd.Timestamp("1990-12-20 00:00:00+00:00")]***REMOVED***)
    tm.assert_frame_equal(result, expected)
