...
def df_duplicates():
    return pd.DataFrame(
        {"a": [1, 2, 3, 4, 4], "b": [1, 1, 1, 1, 1], "c": [0, 1, 2, 5, 4]***REMOVED***,
        index=[0, 0, 1, 1, 1],
    )
    ...
    def test_nlargest_n(self, df_strings, nselect_method, n, order):
        ...
        if "b" in order:
            error_msg = (
                ...
                f"cannot use method '{nselect_method***REMOVED***' with this dtype"
***REMOVED***
    ...
    def test_nlargest_error(self, df_main_dtypes, nselect_method, columns):
        ...
        col = columns[1]
        error_msg = (
            f"Column '{col***REMOVED***' has dtype {df[col].dtype***REMOVED***, "
            f"cannot use method '{nselect_method***REMOVED***' with this dtype"
        )
    ...
    def test_nlargest_all_dtypes(self, df_main_dtypes):
        df = df_main_dtypes
        df.nsmallest(2, list(set(df) - {"category_string", "string"***REMOVED***))
        df.nlargest(2, list(set(df) - {"category_string", "string"***REMOVED***))

    def test_nlargest_duplicates_on_starter_columns(self):
        # regression test for GH#22752

        df = pd.DataFrame({"a": [2, 2, 2, 1, 1, 1], "b": [1, 2, 3, 3, 2, 1]***REMOVED***)

        result = df.nlargest(4, columns=["a", "b"])
        expected = pd.DataFrame(
            {"a": [2, 2, 2, 1], "b": [3, 2, 1, 3]***REMOVED***, index=[2, 1, 0, 3]
        )
        tm.assert_frame_equal(result, expected)

        result = df.nsmallest(4, columns=["a", "b"])
        expected = pd.DataFrame(
            {"a": [1, 1, 1, 2], "b": [1, 2, 3, 1]***REMOVED***, index=[5, 4, 3, 0]
        )
        tm.assert_frame_equal(result, expected)

    def test_nlargest_n_identical_values(self):
        # GH#15297
        df = pd.DataFrame({"a": [1] * 5, "b": [1, 2, 3, 4, 5]***REMOVED***)

        result = df.nlargest(3, "a")
        expected = pd.DataFrame({"a": [1] * 3, "b": [1, 2, 3]***REMOVED***, index=[0, 1, 2])
        tm.assert_frame_equal(result, expected)

        result = df.nsmallest(3, "a")
        expected = pd.DataFrame({"a": [1] * 3, "b": [1, 2, 3]***REMOVED***)
        tm.assert_frame_equal(result, expected)
...
***REMOVED***
    ...
    def test_nlargest_duplicate_keep_all_ties(self):
        # GH#16818
        df = pd.DataFrame(
            {"a": [5, 4, 4, 2, 3, 3, 3, 3], "b": [10, 9, 8, 7, 5, 50, 10, 20]***REMOVED***
        )
        result = df.nlargest(4, "a", keep="all")
        expected = pd.DataFrame(
            {
                "a": {0: 5, 1: 4, 2: 4, 4: 3, 5: 3, 6: 3, 7: 3***REMOVED***,
                "b": {0: 10, 1: 9, 2: 8, 4: 5, 5: 50, 6: 10, 7: 20***REMOVED***,
            ***REMOVED***
        )
        tm.assert_frame_equal(result, expected)
        ...
        expected = pd.DataFrame(
            {
                "a": {3: 2, 4: 3, 5: 3, 6: 3, 7: 3***REMOVED***,
                "b": {3: 7, 4: 5, 5: 50, 6: 10, 7: 20***REMOVED***,
            ***REMOVED***
        )
        tm.assert_frame_equal(result, expected)
