...
class TestResetIndex:
    ...
    def test_set_reset(self):
        ...
        # set/reset
        df = DataFrame({"A": [0, 1, 2]***REMOVED***, index=idx)
    ...
    def test_reset_index_tz(self, tz_aware_fixture):
        ...
        df = DataFrame({"a": range(5), "b": ["A", "B", "C", "D", "E"]***REMOVED***, index=idx)
    ...
    def test_reset_index_with_intervals(self):
        ...
        original = DataFrame({"x": idx, "y": np.arange(10)***REMOVED***)[["x", "y"]]

        result = original.set_index("x")
        expected = DataFrame({"y": np.arange(10)***REMOVED***, index=idx)
        tm.assert_frame_equal(result, expected)
    ...
    def test_reset_index(self, float_frame):
        ...
        stacked = DataFrame({"foo": stacked, "bar": stacked***REMOVED***)
    ...
    def test_reset_index_multiindex_nan(self):
        ...
        df = DataFrame({"A": ["a", "b", "c"], "B": [0, 1, 2], "C": [np.nan, 1.1, 2.2]***REMOVED***)
        rs = df.set_index(["A", "B"]).reset_index()
        tm.assert_frame_equal(rs, df)
...
***REMOVED***
    ...
    ***REMOVED***
    ...
    def test_reset_index_datetime(self, tz_naive_fixture):
        ...
        df = DataFrame(
            {"a": np.arange(5, dtype="int64"), "b": ["A", "B", "C", "D", "E"]***REMOVED***,
            index=idx,
        )
    ...
    ***REMOVED***,
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                [(pd.NaT, 1), (pd.NaT, 2)],
                {"a": [pd.NaT, pd.NaT], "b": [1, 2], "x": [11, 12]***REMOVED***,
***REMOVED***,
            (
                [(pd.NaT, 1), (Timestamp("2020-01-01"), 2)],
                {"a": [pd.NaT, Timestamp("2020-01-01")], "b": [1, 2], "x": [11, 12]***REMOVED***,
***REMOVED***,
            (
                [(pd.NaT, 1), (pd.Timedelta(123, "d"), 2)],
                {"a": [pd.NaT, pd.Timedelta(123, "d")], "b": [1, 2], "x": [11, 12]***REMOVED***,
***REMOVED***,
        ],
    )
    def test_reset_index_nat_multiindex(self, ix_data, exp_data):
        ...
        result = DataFrame({"x": [11, 12]***REMOVED***, index=ix)
    ...
    def test_rest_index_multiindex_categorical_with_missing_values(self, codes):
        ...
        data = {"col": range(len(index))***REMOVED***
...
def test_reset_index_dtypes_on_empty_frame_with_multiindex(
    ...
    if using_infer_string and dtype == object:
        dtype = "string"
    expected = Series({"level_0": np.int64, "level_1": np.float64, "level_2": dtype***REMOVED***)
...
def test_reset_index_empty_frame_with_datetime64_multiindex_from_groupby(
    ...
    df = DataFrame({"c1": [10.0], "c2": ["a"], "c3": dti***REMOVED***)
...
def test_reset_index_multiindex_nat():
    ...
    df = DataFrame({"id": idx, "tstamp": tstamp, "a": list("abc")***REMOVED***)
...
def test_reset_index_rename_multiindex(float_frame):
    ...
    stacked_df = DataFrame({"foo": stacked_df, "bar": stacked_df***REMOVED***)
...
def test_errorreset_index_rename(float_frame):
    # GH 6878
    stacked_df = float_frame.stack(future_stack=True)[::2]
    stacked_df = DataFrame({"first": stacked_df, "second": stacked_df***REMOVED***)

    with pytest.raises(
        ValueError, match="Index names must be str or 1-dimensional list"
    ):
        stacked_df.reset_index(names={"first": "new_first", "second": "new_second"***REMOVED***)
