...
class TestCategoricalConcat:
    ...
    def test_categorical_concat_preserve(self):
        ...
        df2 = DataFrame({"A": a, "B": b.astype(CategoricalDtype(list("cab")))***REMOVED***)
    ...
    def test_categorical_index_preserver(self):
        ...
        df2 = DataFrame(
            {"A": a, "B": b.astype(CategoricalDtype(list("cab")))***REMOVED***
        ).set_index("B")
        ...
        df3 = DataFrame(
            {"A": a, "B": Categorical(b, categories=list("abe"))***REMOVED***
        ).set_index("B")
    ...
    def test_concat_categorical_datetime(self):
        # GH-39443
        df1 = DataFrame(
            {"x": Series(datetime(2021, 1, 1), index=[0], dtype="category")***REMOVED***
        )
        df2 = DataFrame(
            {"x": Series(datetime(2021, 1, 2), index=[1], dtype="category")***REMOVED***
        )

        result = pd.concat([df1, df2])
        expected = DataFrame(
            {"x": Series([datetime(2021, 1, 1), datetime(2021, 1, 2)])***REMOVED***
        )
    ...
    def test_categorical_concat_gh7864(self):
        ...
        df = DataFrame({"id": [1, 2, 3, 4, 5, 6], "raw_grade": list("abbaae")***REMOVED***)
    ...
    def test_categorical_index_upcast(self):
        ...
        a = DataFrame({"foo": [1, 2]***REMOVED***, index=Categorical(["foo", "bar"]))
        b = DataFrame({"foo": [4, 3]***REMOVED***, index=Categorical(["baz", "bar"]))

        res = pd.concat([a, b])
        exp = DataFrame({"foo": [1, 2, 4, 3]***REMOVED***, index=["foo", "bar", "baz", "bar"])
    ...
    def test_categorical_missing_from_one_frame(self):
        # GH 25412
        df1 = DataFrame({"f1": [1, 2, 3]***REMOVED***)
        df2 = DataFrame({"f1": [2, 3, 1], "f2": Series([4, 4, 4]).astype("category")***REMOVED***)
    ...
    def test_concat_categorical_same_categories_different_order(self):
        ...
        df1 = DataFrame({"A": [1, 2]***REMOVED***, index=c1)
        df2 = DataFrame({"A": [3, 4]***REMOVED***, index=c2)

        result = pd.concat((df1, df2))
        expected = DataFrame({"A": [1, 2, 3, 4]***REMOVED***, index=c3)
        tm.assert_frame_equal(result, expected)
