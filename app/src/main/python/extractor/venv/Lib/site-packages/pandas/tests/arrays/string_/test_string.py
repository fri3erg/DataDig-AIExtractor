...
def test_repr(dtype):
    df = pd.DataFrame({"A": pd.array(["a", pd.NA, "b"], dtype=dtype)***REMOVED***)
    if dtype.storage == "pyarrow_numpy":
        expected = "     A\n0    a\n1  NaN\n2    b"
    else:
        expected = "      A\n0     a\n1  <NA>\n2     b"
...
def test_comparison_methods_scalar(comparison_op, dtype):
    op_name = f"__{comparison_op.__name__***REMOVED***__"
...
def test_comparison_methods_scalar_not_string(comparison_op, dtype):
    ...
    if dtype.storage == "pyarrow_numpy":
        ...
        ***REMOVED***[op_name]
        ...
        expected_data = {"__eq__": [False, None, False], "__ne__": [True, None, True]***REMOVED***[
            op_name
        ]
        expected_dtype = "boolean[pyarrow]" if dtype.storage == "pyarrow" else "boolean"
...
***REMOVED***
    ...
    expected = df.astype(f"string[{string_storage2***REMOVED***]")
    tm.assert_frame_equal(result, expected)
...
def test_memory_usage(dtype, arrow_string_storage):
    ...
    if dtype.storage in arrow_string_storage:
        pytest.skip(f"not applicable for {dtype.storage***REMOVED***")
