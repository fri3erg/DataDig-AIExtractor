...
def test_error():
    df = pd.DataFrame(
        {"A": pd.Series([[0, 1, 2], np.nan, [], (3, 4)], index=list("abcd")), "B": 1***REMOVED***
    )
    with pytest.raises(
...
def test_basic(scalar):
    df = pd.DataFrame(
        {scalar: pd.Series([[0, 1, 2], np.nan, [], (3, 4)], index=list("abcd")), "B": 1***REMOVED***
    )
...
def test_multi_index_rows():
    df = pd.DataFrame(
        {"A": np.array([[0, 1, 2], np.nan, [], (3, 4)], dtype=object), "B": 1***REMOVED***,
...
def test_multi_index_columns():
    df = pd.DataFrame(
        {("A", 1): np.array([[0, 1, 2], np.nan, [], (3, 4)], dtype=object), ("A", 2): 1***REMOVED***
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"col1": [[1, 2], [3, 4]], "col2": ["foo", "bar"]***REMOVED***,
            [0, 0],
            {"col1": [1, 2, 3, 4], "col2": ["foo", "foo", "bar", "bar"]***REMOVED***,
            [0, 0, 0, 0],
        ),
        (
            {"col1": [[1, 2], [3, 4]], "col2": ["foo", "bar"]***REMOVED***,
...
def test_ignore_index():
    # GH 34932
    df = pd.DataFrame({"id": range(0, 20, 10), "values": [list("ab"), list("cd")]***REMOVED***)
    result = df.explode("values", ignore_index=True)
    expected = pd.DataFrame(
        {"id": [0, 0, 10, 10], "values": list("abcd")***REMOVED***, index=[0, 1, 2, 3]
    )
    tm.assert_frame_equal(result, expected)
...
def test_explode_sets():
    ...
    df = pd.DataFrame({"a": [{"x", "y"***REMOVED***], "b": [1]***REMOVED***, index=[1])
    result = df.explode(column="a").sort_values(by="a")
    expected = pd.DataFrame({"a": ["x", "y"], "b": [1, 1]***REMOVED***, index=[1, 1])
    tm.assert_frame_equal(result, expected)
