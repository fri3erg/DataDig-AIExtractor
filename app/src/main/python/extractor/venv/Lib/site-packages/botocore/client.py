    ...
    ***REMOVED***
    ...
    def _register_legacy_retries(self, client):
        ...
        retry_config = self._retry_config_translator.build_retry_config(
            endpoint_prefix,
            original_config.get('retry', {***REMOVED***),
            original_config.get('definitions', {***REMOVED***),
            retries,
        )
        ...
        client.meta.events.register(
            f"needs-retry.{service_event_name***REMOVED***", handler, unique_id=unique_id
        )
...
***REMOVED***
    ...
    def _create_api_method(
        ...
        def _api_call(self, *args, **kwargs):
            ...
            if args:
                raise TypeError(
                    f"{py_operation_name***REMOVED***() only accepts keyword arguments."
    ***REMOVED***
...
class ClientEndpointBridge:
    ...
    DEFAULT_ENDPOINT = '{service***REMOVED***.{region***REMOVED***.amazonaws.com'
...
***REMOVED***
    ...
    def _is_s3_dualstack_mode(self, service_name):
        ...
        if self.scoped_config is not None:
            enabled = self.scoped_config.get('s3', {***REMOVED***).get(
                'use_dualstack_endpoint'
***REMOVED***
...
***REMOVED***
        logger.debug(
            f'Assuming an endpoint for {service_name***REMOVED***, {region_name***REMOVED***: {endpoint_url***REMOVED***'
        )
        # We still want to allow the user to provide an explicit version.
        signature_version = self._resolve_signature_version(
            service_name, {'signatureVersions': ['v4']***REMOVED***
        )
        signing_name = self._resolve_signing_name(service_name, resolved={***REMOVED***)
        return self._create_result(
            service_name=service_name,
            region_name=region_name,
    ...
    def _make_url(self, hostname, is_secure, supported_protocols):
        if is_secure and 'https' in supported_protocols:
            ...
            scheme = 'http'
        return f'{scheme***REMOVED***://{hostname***REMOVED***'
...
***REMOVED***
    ...
    def __getattr__(self, item):
        ...
        event_name = f'getattr.{service_id***REMOVED***.{item***REMOVED***'
        ...
        raise AttributeError(
            f"'{self.__class__.__name__***REMOVED***' object has no attribute '{item***REMOVED***'"
        )
    ...
    def _register_handlers(self):
        ...
        self.meta.events.register(
            f"request-created.{service_id***REMOVED***", self._request_signer.handler
        )
...
***REMOVED***
        ...
        ***REMOVED***
        ...
        service_id = self._service_model.service_id.hyphenize()
        handler, event_response = self.meta.events.emit_until_response(
            'before-call.{service_id***REMOVED***.{operation_name***REMOVED***'.format(
...
***REMOVED***
        ...
        self.meta.events.emit(
            'after-call.{service_id***REMOVED***.{operation_name***REMOVED***'.format(
                service_id=service_id, operation_name=operation_name
***REMOVED***,
            http_response=http,
        ...
        if http.status_code >= 300:
            error_info = parsed_response.get("Error", {***REMOVED***)
...
***REMOVED***
    ...
    def _make_request(self, operation_model, request_dict, request_context):
        try:
            ...
            self.meta.events.emit(
                'after-call-error.{service_id***REMOVED***.{operation_name***REMOVED***'.format(
                    service_id=self._service_model.service_id.hyphenize(),
...
***REMOVED***
    ...
    def _emit_api_params(self, api_params, operation_model, context):
        ...
        responses = self.meta.events.emit(
            f'provide-client-params.{service_id***REMOVED***.{operation_name***REMOVED***',
            params=api_params,
            model=operation_model,
            context=context,
        )
        ...
        self.meta.events.emit(
            f'before-parameter-build.{service_id***REMOVED***.{operation_name***REMOVED***',
            params=api_params,
            model=operation_model,
            context=context,
        )
...
***REMOVED***
            paginator_class_name = (
                f"{service_module_name***REMOVED***.Paginator.{actual_operation_name***REMOVED***"
***REMOVED***

            # Create the new paginator class
            documented_paginator_cls = type(
                paginator_class_name, (Paginator,), {'paginate': paginate***REMOVED***
***REMOVED***
...
***REMOVED***
    ...
    ***REMOVED***['pagination']
                ...
                self._cache['page_config'] = {***REMOVED***
        actual_operation_name = self._PY_TO_OP_NAME[operation_name]
    ...
    ***REMOVED***
                ...
                self._cache['waiter_config'] = {***REMOVED***
