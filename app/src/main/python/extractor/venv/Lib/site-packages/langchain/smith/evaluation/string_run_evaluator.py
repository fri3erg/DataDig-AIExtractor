...
class StringRunMapper(Serializable):
    ...
    def __call__(self, run: Run) -> Dict[str, str]:
        """Maps the Run to a dictionary."""
        if not run.outputs:
            raise ValueError(f"Run {run.id***REMOVED*** has no outputs to evaluate.")
        return self.map(run)
...
class LLMStringRunMapper(StringRunMapper):
    ...
    def serialize_chat_messages(self, messages: List[Dict]) -> str:
        ...
        if isinstance(messages, list) and messages:
            if isinstance(messages[0], dict):
                ...
                raise ValueError(f"Could not extract messages to evaluate {messages***REMOVED***")
    ...
    def map(self, run: Run) -> Dict[str, str]:
        ...
        if run.run_type != "llm":
            ...
            if run.error:
                raise ValueError(
                    f"Cannot evaluate errored LLM run {run.id***REMOVED***: {run.error***REMOVED***"
    ***REMOVED***
            else:
                raise ValueError(
                    f"Run {run.id***REMOVED*** has no outputs. Cannot evaluate this run."
    ***REMOVED***
        else:
            try:
                ...
                raise ValueError(
                    f"Could not parse LM input from run inputs {run.inputs***REMOVED***"
    ***REMOVED*** from e
            try:
                output_ = self.serialize_outputs(run.outputs)
            except Exception as e:
                raise ValueError(
                    f"Could not parse LM prediction from run outputs {run.outputs***REMOVED***"
    ***REMOVED*** from e
            return {"input": inputs, "prediction": output_***REMOVED***
...
class ChainStringRunMapper(StringRunMapper):
    ...
    def _get_key(self, source: Dict, key: Optional[str], which: str) -> str:
        if key is not None:
            ...
            raise ValueError(
                f"Could not map run {which***REMOVED*** with multiple keys: "
                f"{source***REMOVED***\nPlease manually specify a {which***REMOVED***_key"
***REMOVED***
    ...
    def map(self, run: Run) -> Dict[str, str]:
        ...
        if not run.outputs:
            raise ValueError(
                f"Run with ID {run.id***REMOVED*** lacks outputs required for evaluation."
                " Ensure the Run has valid outputs."
***REMOVED***
        if self.input_key is not None and self.input_key not in run.inputs:
            raise ValueError(
                f"Run with ID {run.id***REMOVED*** is missing the expected input key"
                f" '{self.input_key***REMOVED***'.\nAvailable input keys in this Run"
                f"  are: {run.inputs.keys()***REMOVED***.\nAdjust the evaluator's"
...
***REMOVED***
            ...
            raise ValueError(
                f"Run with ID {run.id***REMOVED*** doesn't have the expected prediction key"
                f" '{self.prediction_key***REMOVED***'. Available prediction keys in this Run are:"
                f" {available_keys***REMOVED***. Adjust the evaluator's prediction_key or"
...
class ToolStringRunMapper(StringRunMapper):
    ...
    def map(self, run: Run) -> Dict[str, str]:
        if not run.outputs:
            raise ValueError(f"Run {run.id***REMOVED*** has no outputs to evaluate.")
        return {"input": run.inputs["input"], "prediction": run.outputs["output"]***REMOVED***
...
class StringExampleMapper(Serializable):
    ...
    def map(self, example: Example) -> Dict[str, str]:
        ...
        if not example.outputs:
            raise ValueError(
                f"Example {example.id***REMOVED*** has no outputs to use as a reference."
***REMOVED***
        if self.reference_key is None:
            if len(example.outputs) > 1:
                raise ValueError(
                    f"Example {example.id***REMOVED*** has multiple outputs, so you must"
    ...
    ***REMOVED***
            ...
            raise ValueError(
                f"Example {example.id***REMOVED*** does not have reference key"
                f" {self.reference_key***REMOVED***."
***REMOVED***
        else:
    ...
    def __call__(self, example: Example) -> Dict[str, str]:
        ...
        if not example.outputs:
            raise ValueError(
                f"Example {example.id***REMOVED*** has no outputs to use as areference label."
***REMOVED***
...
class StringRunEvaluatorChain(Chain, RunEvaluator):
    ...
    def _prepare_input(self, inputs: Dict[str, Any]) -> Dict[str, str]:
        ...
        if example and self.example_mapper and self.string_evaluator.requires_reference:
            ...
            raise ValueError(
                f"Evaluator {self.name***REMOVED*** requires an reference"
                " example from the dataset,"
                f" but none was provided for run {run.id***REMOVED***."
***REMOVED***
    ...
    def _prepare_output(self, output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"feedback": evaluation_result***REMOVED***
    ...
    def evaluate_run(
        ...
        """Evaluate an example."""
        try:
            result = self({"run": run, "example": example***REMOVED***, include_run_info=True)
            return self._prepare_evaluator_output(result)
        except Exception as e:
            return EvaluationResult(
                key=self.string_evaluator.evaluation_name,
                comment=f"Error evaluating run {run.id***REMOVED***: {e***REMOVED***",
    ...
    async def aevaluate_run(
        ...
        try:
            result = await self.acall(
                {"run": run, "example": example***REMOVED***, include_run_info=True
***REMOVED***
            return self._prepare_evaluator_output(result)
        except Exception as e:
            return EvaluationResult(
...
***REMOVED***
            ...
            raise ValueError(
                f"Unsupported run type {run_type***REMOVED***. Expected one of 'llm' or 'chain'."
***REMOVED***
        ...
        if (
            ...
            raise ValueError(
                f"Evaluator {evaluator.evaluation_name***REMOVED*** requires a reference"
