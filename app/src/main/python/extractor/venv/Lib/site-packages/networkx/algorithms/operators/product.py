...
def _dict_product(d1, d2):
    return {k: (d1.get(k), d2.get(k)) for k in set(d1) | set(d2)***REMOVED***
...
def power(G, k):
    ...
    *Graph Theory* by Bondy and Murty [1]_.
    ...
    for n in G:
        seen = {***REMOVED***  # level (number of hops) when seen in BFS
        level = 1  # the current level
        nextlevel = G[n]
        while nextlevel:
            thislevel = nextlevel  # advance to next level
            nextlevel = {***REMOVED***  # and start a new list (fringe)
            for v in thislevel:
