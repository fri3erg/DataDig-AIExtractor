    ...
    <xsl:template name="iso:exslt.add.imports">
      ...
      <xsl:choose>
        <xsl:when test="contains($imports, ';')">
          <axsl:import href="{ substring-before($imports, ';') ***REMOVED***"/>
          <xsl:call-template name="iso:exslt.add.imports">
            <xsl:with-param name="imports"  select="substring-after($imports, ';')"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:when test="$imports">
          <axsl:import href="{ $imports ***REMOVED***"/>
        </xsl:when>
      </xsl:choose>
    </xsl:template>
...
<!-- ============================================================== -->
	...
	<xsl:template  match="xsl:key" mode="do-keys" >
	     ...
	     <xsl:choose>
	       ...
	       <xsl:choose>
	       	<xsl:when test="@path">
				<axsl:key match="{../@context***REMOVED***" name="{@name***REMOVED***" use="{@path***REMOVED***"/>
			</xsl:when>
			<xsl:otherwise>
							<axsl:key match="{../@context***REMOVED***" name="{@name***REMOVED***" use="{@use***REMOVED***"/>
			</xsl:otherwise>
			</xsl:choose>	
		</xsl:when>
		<xsl:otherwise>
   ...
   <xsl:template match="iso:include" mode="do-all-patterns">
       ...
       <!-- lets at the top-level are implemented as parameters -->
       	...
       	<xsl:choose>
       		<xsl:when test="parent::iso:schema">
	      		 ...
	      		 <axsl:param name="{@name***REMOVED***" select="{@value***REMOVED***">
       		...
       		<xsl:otherwise>
				<axsl:variable name="{@name***REMOVED***" select="{@value***REMOVED***"/>
			</xsl:otherwise>
		</xsl:choose>
	...
	or (../iso:phase[@id= $phase]/iso:active[@pattern= current()/@id])">
		...
		<xsl:choose>
		  <xsl:when test="$select-contexts='key'">
		    <axsl:apply-templates select="key('M','M{count(preceding-sibling::*)***REMOVED***')" mode="M{count(preceding-sibling::*)***REMOVED***"/>
		  </xsl:when>
		  <xsl:when test="$select-contexts='//'">
		    <axsl:apply-templates mode="M{count(preceding-sibling::*)***REMOVED***">
		  ...
		  <xsl:otherwise>
		    <axsl:apply-templates select="/" mode="M{count(preceding-sibling::*)***REMOVED***"/>
		  </xsl:otherwise>
		</xsl:choose>
        </xsl:if>
	</xsl:template>
	...
	<xsl:template match="iso:pattern[not(@abstract='true')]">
      ...
      <xsl:if test="($phase = '#ALL') 
		...
		<xsl:if test="not($select-contexts)">
		  <axsl:template match="text()" priority="-1" mode="M{count(preceding-sibling::*)***REMOVED***">
		    <!-- strip characters -->
		  </axsl:template>
		  ...
		  <axsl:template match="@*|node()"
				 priority="-2"
				 mode="M{ count(preceding-sibling::*) ***REMOVED***">
		    <axsl:apply-templates select="{$context-xpath***REMOVED***" mode="M{count(preceding-sibling::*)***REMOVED***"/>
		  </axsl:template>
		</xsl:if>
      </xsl:if>
	</xsl:template>
	...
	<xsl:template match="iso:rule[not(@abstract='true')] ">
		...
		<xsl:if test="$select-contexts='key'">
		    <axsl:key name="M"
			      match="{@context***REMOVED***" 
			      use="'M{count(../preceding-sibling::*)***REMOVED***'"/>
		</xsl:if>
   
	
<!-- DPC priorities count up from 1000 not down from 4000 (templates in same priority order as before) -->
		<axsl:template match="{@context***REMOVED***"
		priority="{1000 + count(following-sibling::*)***REMOVED***" mode="M{count(../preceding-sibling::*)***REMOVED***">
...
<!--xsl:template name="handle-namespace" version="2.0">
   <xsl:namespace name="{@prefix***REMOVED***" select="@uri">
</xsl:template-->
       ...
       <xsl:choose>
        ...
        <xsl:when test="function-available('exsl:node-set')">
           <xsl:variable name="ns-dummy-elements">
             <xsl:element name="{@prefix***REMOVED***:dummy" namespace="{@uri***REMOVED***"/>
           </xsl:variable>
    	...
    	<xsl:when test="function-available('msxsl:node-set')">
      		<xsl:variable name="ns-dummy-elements">
        		<xsl:element name="{ $prefix ***REMOVED***:e" namespace="{ $uri ***REMOVED***"/>
      		</xsl:variable>
        ...
        <xsl:otherwise>
          <xsl:attribute name="{concat(@prefix,':dummy-for-xmlns')***REMOVED***" namespace="{@uri***REMOVED***" />
...
<!-- ============================================================== -->
	...
	<xsl:template name="process-name">
		...
		<!-- We generate too much whitespace rather than risking concatenation -->
		<axsl:text> </axsl:text>
		<axsl:value-of select="{$name***REMOVED***"/>
		<axsl:text> </axsl:text>
	...
	<xsl:template name="process-value-of">
	    ...
	    <!-- We generate too much whitespace rather than risking concatenation -->
		<axsl:text> </axsl:text>
		<axsl:value-of select="{$select***REMOVED***"/>
		<axsl:text> </axsl:text>
	</xsl:template>
