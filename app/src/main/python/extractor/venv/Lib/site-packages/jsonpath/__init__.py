...
def create_logger(name: str = None, level: Union[int, str] = logging.INFO):
    ...
    formater = logging.Formatter(
        f"%(asctime)s-%(levelname)s-[{name***REMOVED***] %(message)s", datefmt="[%Y-%m-%d %H:%M:%S]"
    )
...
class JSONPath:
    ...
    ***REMOVED***
    ...
    def __init__(self, expr: str):
        ...
        logger.debug(f"segments  : {self.segments***REMOVED***")
    ...
    def parse(self, obj, result_type="VALUE", eval_func=eval):
        ...
        if result_type not in JSONPath.RESULT_TYPE:
            raise ValueError(
                f"result_type must be one of {tuple(JSONPath.RESULT_TYPE.keys())***REMOVED***"
***REMOVED***
    ...
    def _parse_expr(self, expr):
        logger.debug(f"before expr : {expr***REMOVED***")
        ...
        # split
        expr = JSONPath.REP_DOUBLEDOT.sub(f"{JSONPath.SEP***REMOVED***..{JSONPath.SEP***REMOVED***", expr)
        ...
        logger.debug(f"after expr  : {expr***REMOVED***")
        return expr
    ...
    def _traverse(f, obj, i: int, path: str, *args):
        if isinstance(obj, list):
            for idx, v in enumerate(obj):
                f(v, i, f"{path***REMOVED***{JSONPath.SEP***REMOVED***{idx***REMOVED***", *args)
        elif isinstance(obj, dict):
            for k, v in obj.items():
                f(v, i, f"{path***REMOVED***{JSONPath.SEP***REMOVED***{k***REMOVED***", *args)
    ...
    ***REMOVED***
    ...
    def _filter(self, obj, i: int, path: str, step: str):
        r = False
        try:
            r = self.eval_func(step, None, {"__obj": obj***REMOVED***)
    ...
    def _trace(self, obj, i: int, path):
        ...
        if i >= self.lpath:
            ...
            logger.debug(f"path: {path***REMOVED*** | value: {obj***REMOVED***")
            return
        ...
        if isinstance(obj, list) and step.isdigit():
            ikey = int(step)
            if ikey < len(obj):
                self._trace(obj[ikey], i + 1, f"{path***REMOVED***{JSONPath.SEP***REMOVED***{step***REMOVED***")
            return

        # get value from dict
        if isinstance(obj, dict) and step in obj:
            self._trace(obj[step], i + 1, f"{path***REMOVED***{JSONPath.SEP***REMOVED***{step***REMOVED***")
            return

        # slice
        if isinstance(obj, list) and JSONPath.REP_SLICE_CONTENT.fullmatch(step):
            obj = list(enumerate(obj))
            vals = self.eval_func(f"obj[{step***REMOVED***]")
            for idx, v in vals:
                self._trace(v, i + 1, f"{path***REMOVED***{JSONPath.SEP***REMOVED***{idx***REMOVED***")
            return
        ...
        if isinstance(obj, dict) and JSONPath.REP_SELECT_CONTENT.fullmatch(step):
            for k in step.split(","):
                if k in obj:
                    self._trace(obj[k], i + 1, f"{path***REMOVED***{JSONPath.SEP***REMOVED***{k***REMOVED***")
            return
        ...
        if step.startswith("/(") and step.endswith(")"):
            if isinstance(obj, list):
                ...
                self._sorter(obj, step[2:-1])
                for k, v in obj:
                    self._trace(v, i + 1, f"{path***REMOVED***{JSONPath.SEP***REMOVED***{k***REMOVED***")
            else:
