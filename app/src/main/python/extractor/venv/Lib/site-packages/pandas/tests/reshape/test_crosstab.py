...
class TestCrosstab:
    ...
    ***REMOVED***
    ...
    def test_crosstab_pass_values(self):
        ...
        df = DataFrame({"foo": a, "bar": b, "baz": c, "values": values***REMOVED***)
    ...
    def test_margin_dropna(self):
        ...
        df = DataFrame({"a": [1, 2, 2, 2, 2, np.nan], "b": [3, 3, 4, 4, 4, 4]***REMOVED***)
    ...
    def test_margin_dropna2(self):
        df = DataFrame(
            {"a": [1, np.nan, np.nan, np.nan, 2, np.nan], "b": [3, np.nan, 4, 4, 4, 4]***REMOVED***
        )
        actual = crosstab(df.a, df.b, margins=True, dropna=True)
    ...
    def test_margin_dropna3(self):
        df = DataFrame(
            {"a": [1, np.nan, np.nan, np.nan, np.nan, 2], "b": [3, 3, 4, 4, 4, 4]***REMOVED***
        )
        actual = crosstab(df.a, df.b, margins=True, dropna=True)
        expected = DataFrame([[1, 0, 1], [0, 1, 1], [1, 1, 2]])
        expected.index = Index([1.0, 2.0, "All"], name="a")
    ...
    def test_crosstab_normalize(self):
        # Issue 12578
        df = DataFrame(
            {"a": [1, 2, 2, 2, 2], "b": [3, 3, 4, 4, 4], "c": [1, 1, np.nan, 1, 1]***REMOVED***
        )
    ...
    def test_crosstab_normalize_arrays(self):
        ...
        df = DataFrame(
            {"a": [1, 2, 2, 2, 2], "b": [3, 3, 4, 4, 4], "c": [0, 4, np.nan, 3, 3]***REMOVED***
        )
...
***REMOVED***
    ...
    def test_crosstab_unsorted_order(self):
        df = DataFrame({"b": [3, 1, 2], "a": [5, 4, 6]***REMOVED***, index=["C", "A", "B"])
...
***REMOVED***,
    ...
    def test_margin_support_Float(self):
        ...
        df = DataFrame(
            {"A": [1, 2, 2, 1], "B": [3, 3, 4, 5], "C": [-1.0, 10.0, 1.0, 10.0]***REMOVED***,
            dtype="Float64",
        )
        result = crosstab(
            df["A"],
