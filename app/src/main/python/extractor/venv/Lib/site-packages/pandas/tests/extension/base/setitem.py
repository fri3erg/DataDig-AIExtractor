...
class BaseSetitemTests:
    ...
    def test_setitem_sequence_mismatched_length_raises(self, data, as_array):
        ...
        xpr = "cannot set using a {***REMOVED*** indexer with a different length"
    ...
    def test_setitem_loc_scalar_mixed(self, data):
        df = pd.DataFrame({"A": np.arange(len(data)), "B": data***REMOVED***)
        df.loc[0, "B"] = data[1]
        assert df.loc[0, "B"] == data[1]

    def test_setitem_loc_scalar_single(self, data):
        df = pd.DataFrame({"B": data***REMOVED***)
        df.loc[10, "B"] = data[1]
        assert df.loc[10, "B"] == data[1]

    def test_setitem_loc_scalar_multiple_homogoneous(self, data):
        df = pd.DataFrame({"A": data, "B": data***REMOVED***)
        df.loc[10, "B"] = data[1]
        assert df.loc[10, "B"] == data[1]
...
***REMOVED***,
    ...
    def test_setitem_expand_columns(self, data):
        ...
        result["B"] = data
        expected = pd.DataFrame({"A": data, "B": data***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_setitem_frame_invalid_length(self, data):
        df = pd.DataFrame({"A": [1] * len(data)***REMOVED***)
        xpr = (
            rf"Length of values \({len(data[:5])***REMOVED***\) "
    ...
    def test_setitem_with_expansion_dataframe_column(self, data, full_indexer):
        ...
        df = expected = pd.DataFrame({0: pd.Series(data)***REMOVED***)
    ...
    def test_setitem_with_expansion_row(self, data, na_value):
        df = pd.DataFrame({"data": data[:1]***REMOVED***)

        df.loc[1, "data"] = data[1]
        expected = pd.DataFrame({"data": data[:2]***REMOVED***)
        tm.assert_frame_equal(df, expected)

        # https://github.com/pandas-dev/pandas/issues/47284
        df.loc[2, "data"] = na_value
        expected = pd.DataFrame(
            {"data": pd.Series([data[0], data[1], na_value], dtype=data.dtype)***REMOVED***
        )
        tm.assert_frame_equal(df, expected)
