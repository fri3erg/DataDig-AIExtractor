...
class TestTreeLCA:
    ...
    def setup_class(cls):
        ...
        gold = {(n, n): n for n in cls.DG***REMOVED***
        gold.update({(0, i): 0 for i in range(1, 7)***REMOVED***)
        gold.update(
            {
                (1, 2): 0,
                (1, 3): 1,
    ...
    def test_tree_all_pairs_lca_disconnected_nodes(self):
        G = nx.DiGraph()
        G.add_node(1)
        assert {(1, 1): 1***REMOVED*** == dict(tree_all_pairs_lca(G))

        G.add_node(0)
        assert {(1, 1): 1***REMOVED*** == dict(tree_all_pairs_lca(G, 1))
        assert {(0, 0): 0***REMOVED*** == dict(tree_all_pairs_lca(G, 0))
