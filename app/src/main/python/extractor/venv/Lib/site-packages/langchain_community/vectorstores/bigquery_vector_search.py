...
***REMOVED***
        ...
        self._full_table_id = (
            f"{self.project_id***REMOVED***." f"{self.dataset_name***REMOVED***." f"{self.table_name***REMOVED***"
        )
    ...
    def _initialize_table(self) -> Any:
        ...
        columns = {c.name: c for c in schema***REMOVED***
...
***REMOVED***
            ...
            raise ValueError(f"Column {self.doc_id_field***REMOVED*** must be of " "STRING type")
...
***REMOVED***
            ...
            raise ValueError(
                f"Column {self.metadata_field***REMOVED*** must be of STRING or JSON type"
***REMOVED***
...
***REMOVED***
            ...
            raise ValueError(f"Column {self.content_field***REMOVED*** must be of " "STRING type")
...
***REMOVED***
            ...
            raise ValueError(
                f"Column {self.text_embedding_field***REMOVED*** must be of " "ARRAY<FLOAT64> type"
***REMOVED***
        if changed_schema:
    ...
    def _initialize_vector_index(self) -> Any:
        ...
        with _vector_table_lock:
            ...
            check_query = (
                f"SELECT 1 FROM `{self.project_id***REMOVED***.{self.dataset_name***REMOVED***"
                ".INFORMATION_SCHEMA.VECTOR_INDEXES` WHERE"
                f" table_name = '{self.table_name***REMOVED***'"
***REMOVED***
...
***REMOVED***
    ...
    def _create_index(self):
        ...
        # Default to EUCLIDEAN_DISTANCE
            ...
            distance_type = "EUCLIDEAN"
        index_name = f"{self.table_name***REMOVED***_langchain_index"
        try:
            sql = f"""
                CREATE VECTOR INDEX IF NOT EXISTS
                `{index_name***REMOVED***`
                ON `{self.full_table_id***REMOVED***`({self.text_embedding_field***REMOVED***)
                OPTIONS(distance_type="{distance_type***REMOVED***", index_type="IVF")
            """
    ...
    def add_texts_with_embeddings(
        ...
        ***REMOVED***
        ...
        metadatas = [m if m is not None else {***REMOVED*** for m in metadatas + add_meta]
...
***REMOVED***
            id_expr = f"{self.doc_id_field***REMOVED*** IN UNNEST(@ids)"
        else:
            job_config = None
            id_expr = "TRUE"
        if filter:
            ...
            for i in filter.items():
                if isinstance(i[1], float):
                    expr = (
                        "ABS(CAST(JSON_VALUE("
                        f"`{self.metadata_field***REMOVED***`,'$.{i[0]***REMOVED***') "
                        f"AS FLOAT64) - {i[1]***REMOVED***) "
                        f"<= {sys.float_info.epsilon***REMOVED***"
        ***REMOVED***
                else:
                    val = str(i[1]).replace('"', '\\"')
                    expr = (
                        f"JSON_VALUE(`{self.metadata_field***REMOVED***`,'$.{i[0]***REMOVED***')" f' = "{val***REMOVED***"'
        ***REMOVED***
                filter_expressions.append(expr)
            filter_expression_str = " AND ".join(filter_expressions)
            where_filter_expr = f" AND ({filter_expression_str***REMOVED***)"
        else:
            where_filter_expr = ""

        job = self.bq_client.query(
            f"""
                    SELECT * FROM `{self.full_table_id***REMOVED***` WHERE {id_expr***REMOVED***
                    {where_filter_expr***REMOVED***
                    """,
    ...
    def delete(self, ids: Optional[List[str]] = None, **kwargs: Any) -> Optional[bool]:
        ...
        self.bq_client.query(
            f"""
                    DELETE FROM `{self.full_table_id***REMOVED***` WHERE {self.doc_id_field***REMOVED***
                    IN UNNEST(@ids)
                    """,
            job_config=job_config,
        ).result()
        ...
        if filter:
            ...
            for i in filter.items():
                if isinstance(i[1], float):
                    expr = (
                        "ABS(CAST(JSON_VALUE("
                        f"base.`{self.metadata_field***REMOVED***`,'$.{i[0]***REMOVED***') "
                        f"AS FLOAT64) - {i[1]***REMOVED***) "
                        f"<= {sys.float_info.epsilon***REMOVED***"
        ***REMOVED***
                else:
                    val = str(i[1]).replace('"', '\\"')
                    expr = (
                        f"JSON_VALUE(base.`{self.metadata_field***REMOVED***`,'$.{i[0]***REMOVED***')"
                        f' = "{val***REMOVED***"'
        ***REMOVED***
                filter_expressions.append(expr)
            filter_expression_str = " AND ".join(filter_expressions)
            filter_expr += f" AND ({filter_expression_str***REMOVED***)"
        ...
        # Default to EUCLIDEAN_DISTANCE
            ...
            distance_type = "EUCLIDEAN"
        if brute_force:
            options_string = ",options => '{\"use_brute_force\":true***REMOVED***'"
    ...
    ***REMOVED***
            options_string = (
                ...
                f"{fraction_lists_to_search***REMOVED******REMOVED******REMOVED***'"
***REMOVED***
        else:
            options_string = ""
        query = f"""
            ...
            FROM VECTOR_SEARCH(
                TABLE `{self.full_table_id***REMOVED***`,
                "{self.text_embedding_field***REMOVED***",
                (SELECT @v AS {self.text_embedding_field***REMOVED***),
                distance_type => "{distance_type***REMOVED***",
                top_k => {k***REMOVED***
                {options_string***REMOVED***
***REMOVED***
            WHERE {filter_expr***REMOVED***
            LIMIT {k***REMOVED***
        """
    ...
    def _select_relevance_score_fn(self) -> Callable[[float], float]:
        if self.distance_strategy == DistanceStrategy.COSINE:
            ...
            raise ValueError(
                ...
                f"for `{self.distance_strategy***REMOVED***` distance."
***REMOVED***
