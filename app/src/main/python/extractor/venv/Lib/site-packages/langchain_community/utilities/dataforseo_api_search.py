...
class DataForSeoAPIWrapper(BaseModel):
    ...
    params: dict = Field(default={***REMOVED***)
    ...
    def _prepare_request(self, keyword: str) -> dict:
        ...
        cred = base64.b64encode(
            f"{self.api_login***REMOVED***:{self.api_password***REMOVED***".encode("utf-8")
        ).decode("utf-8")
        headers = {"Authorization": f"Basic {cred***REMOVED***", "Content-Type": "application/json"***REMOVED***
        obj = {"keyword": quote(keyword)***REMOVED***
        obj = {**obj, **self.default_params, **self.params***REMOVED***
        data = [obj]
        _url = (
            f"https://api.dataforseo.com/v3/serp/{obj['se_name']***REMOVED***"
            f"/{obj['se_type']***REMOVED***/live/advanced"
        )
        return {
            "url": _url,
            "headers": headers,
    ...
    def _check_response(self, response: dict) -> dict:
        ...
        if response.get("status_code") != 20000:
            raise ValueError(
                f"Got error from DataForSEO SERP API: {response.get('status_message')***REMOVED***"
***REMOVED***
        return response
