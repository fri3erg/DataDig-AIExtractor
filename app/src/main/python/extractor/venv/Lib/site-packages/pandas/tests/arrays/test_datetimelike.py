...
class SharedTests:
    ...
    def test_take_fill_raises(self, fill_value, arr1d):
        msg = f"value should be a '{arr1d._scalar_type.__name__***REMOVED***' or 'NaT'. Got"
    ...
    def test_unbox_scalar(self, arr1d):
        ...
        msg = f"'value' should be a {self.scalar_type.__name__***REMOVED***."
    ...
    def test_searchsorted_castable_strings(self, arr1d, box, string_storage):
        ...
        with pytest.raises(
            TypeError,
            match=re.escape(
                f"value should be a '{arr1d._scalar_type.__name__***REMOVED***', 'NaT', "
...
***REMOVED***:
    ...
    def test_repr_2d(self, arr1d):
        ...
        if isinstance(arr2d, TimedeltaArray):
            expected = (
                f"<{type(arr2d).__name__***REMOVED***>\n"
                "[\n"
                f"['{arr1d[0]._repr_base()***REMOVED***'],\n"
                f"['{arr1d[1]._repr_base()***REMOVED***'],\n"
                f"['{arr1d[2]._repr_base()***REMOVED***']\n"
                "]\n"
                f"Shape: (3, 1), dtype: {arr1d.dtype***REMOVED***"
***REMOVED***
        else:
            expected = (
                f"<{type(arr2d).__name__***REMOVED***>\n"
...
***REMOVED***
    ...
    def test_setitem_numeric_raises(self, arr1d, box):
        ...
        msg = (
            f"value should be a '{arr1d._scalar_type.__name__***REMOVED***', "
...
class TestTimedeltaArray(SharedTests):
    ...
    def test_take_fill_valid(self, timedelta_index, fixed_now_ts):
        ...
        value = fixed_now_ts
        msg = f"value should be a '{arr._scalar_type.__name__***REMOVED***' or 'NaT'. Got"
        with pytest.raises(TypeError, match=msg):
...
def test_from_pandas_array(dtype):
    ...
    cls = {"M8[ns]": DatetimeArray, "m8[ns]": TimedeltaArray***REMOVED***[dtype]

    depr_msg = f"{cls.__name__***REMOVED***.__init__ is deprecated"
    ...
    func = {"M8[ns]": pd.to_datetime, "m8[ns]": pd.to_timedelta***REMOVED***[dtype]
    result = func(arr).array
    expected = func(data).array
    tm.assert_equal(result, expected)

    # Let's check the Indexes while we're here
    idx_cls = {"M8[ns]": DatetimeIndex, "m8[ns]": TimedeltaIndex***REMOVED***[dtype]
    result = idx_cls(arr)
