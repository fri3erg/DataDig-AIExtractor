...
.. sourcecode:: pycon+sql
    ...
    {printsql***REMOVED***INSERT INTO my_table (id, data) VALUES (%(id)s, %(data)s)
...
* When using :paramref:`_postgresql.Insert.on_conflict_do_update.index_elements` to
  ...
  .. sourcecode:: pycon+sql
    ...
    ... )
    >>> print(stmt)
    {printsql***REMOVED***INSERT INTO my_table (data, user_email)
...
.. sourcecode:: pycon+sql
    ...
    ... )
    >>> print(do_update_stmt)
    {printsql***REMOVED***INSERT INTO my_table (id, data, author)
...
keyword argument::
    ...
    Index('my_index', my_table.c.data, postgresql_with={"fillfactor": 50***REMOVED***)
...
* ``json_each()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> stmt = select(func.json_each('{"a":"foo", "b":"bar"***REMOVED***').table_valued("key", "value"))
    >>> print(stmt)
    {printsql***REMOVED***SELECT anon_1.key, anon_1.value
...
* ``json_populate_record()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    ... )
    >>> print(stmt)
    {printsql***REMOVED***SELECT x.a, x.b
...
* ``json_to_record()`` - this form uses a PostgreSQL specific form of derived
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> stmt = select(
    ...     func.json_to_record('{"a":1,"b":[1,2,3],"c":"bar"***REMOVED***').table_valued(
    ...         column("a", Integer), column("b", Text), column("d", Text),
    ...     ).render_derived(name="x", with_types=True)
    ... )
    >>> print(stmt)
    {printsql***REMOVED***SELECT x.a, x.b, x.d
...
* ``WITH ORDINALITY`` - part of the SQL standard, ``WITH ORDINALITY`` adds an
  ...
  .. sourcecode:: pycon+sql
    ...
    ... )
    >>> print(stmt)
    {printsql***REMOVED***SELECT anon_1.value, anon_1.ordinality
...
* ``unnest()`` - in order to generate a PostgreSQL ARRAY literal, the
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> print(stmt)
    {printsql***REMOVED***SELECT anon_1
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> print(stmt)
    {printsql***REMOVED***SELECT unnested_value
...
.. sourcecode:: pycon+sql
    ...
    ... )
    >>> print(stmt)
    {printsql***REMOVED***SELECT t.id, t.fk
    FROM t
...
.. sourcecode:: pycon+sql
    ...
    >>> print(stmt)
    {printsql***REMOVED***SELECT row_to_json(a) AS row_to_json_1
    FROM a
    ...
    def _assert_pg_ts_ext(self, element, **kw):
        if not isinstance(element, _regconfig_fn):
            ...
            raise exc.CompileError(
                f'Can\'t compile "{element.name***REMOVED***()" full text search '
                ...
                f'"sqlalchemy.func.{element.name***REMOVED***()" to ensure registration '
                f"of the correct argument and return types."
***REMOVED***

        return f"{element.name***REMOVED***{self.function_argspec(element, **kw)***REMOVED***"
...
***REMOVED***
            options.append(f"CHECK ({check***REMOVED***)")

        return (
            f"CREATE DOMAIN {self.preparer.format_type(domain)***REMOVED*** AS "
            f"{self.type_compiler.process(domain.data_type)***REMOVED*** "
            f"{' '.join(options)***REMOVED***"
        )

    def visit_drop_domain_type(self, drop, **kw):
        domain = drop.element
        return f"DROP DOMAIN {self.preparer.format_type(domain)***REMOVED***"
...
***REMOVED***
    ...
    def _can_comment_on_constraint(self, ddl_instance):
        ...
        if constraint.name is None:
            raise exc.CompileError(
                f"Can't emit COMMENT ON for constraint {constraint!r***REMOVED***: "
                "it has no name"
***REMOVED***
...
class PGTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_FLOAT(self, type_, **kw):
        if not type_.precision:
            ...
            return "FLOAT(%(precision)s)" % {"precision": type_.precision***REMOVED***
...
***REMOVED***
    ...
    def format_type(self, type_, use_schema=True):
        if not type_.name:
            raise exc.CompileError(
                f"PostgreSQL {type_.__class__.__name__***REMOVED*** type requires a name."
***REMOVED***
        ...
        if (
            ...
            name = f"{self.quote_schema(effective_schema)***REMOVED***.{name***REMOVED***"
        return name
...
class PGDialect(default.DefaultDialect):
    ...
    construct_arguments = [
        ...
        (
            ...
            {"nulls_not_distinct": None***REMOVED***,
        ),
    ]
    ...
    def set_isolation_level(self, dbapi_connection, level):
        ...
        cursor.execute(
            ...
            f"ISOLATION LEVEL {level***REMOVED***"
        )
    ...
    ***REMOVED***
        ...
        if ports_str:
            try:
                ...
                raise exc.ArgumentError(
                    f"Received non-integer port arguments: {ports_str***REMOVED***"
    ***REMOVED*** from None
...
***REMOVED***,
    ...
    def has_table(self, connection, table_name, schema=None, **kw):
        ...
        return bool(connection.scalar(query, {"table_name": table_name***REMOVED***))
...
***REMOVED***,
        ...
        if table_oid is None:
            raise exc.NoSuchTableError(
                f"{schema***REMOVED***.{table_name***REMOVED***" if schema else table_name
***REMOVED***
        return table_oid
...
***REMOVED***
        ...
        if res is None:
            raise exc.NoSuchTableError(
                f"{schema***REMOVED***.{view_name***REMOVED***" if schema else view_name
***REMOVED***
        else:
            return res
    ...
    def _value_or_raise(self, data, table, schema):
        try:
            ...
            raise exc.NoSuchTableError(
                f"{schema***REMOVED***.{table***REMOVED***" if schema else table
***REMOVED*** from None

    def _prepare_filter_names(self, filter_names):
        if filter_names:
            return True, {"filter_names": filter_names***REMOVED***
        else:
...
***REMOVED***
    ...
    ***REMOVED***.add_columns(
        ...
        ***REMOVED***.label("indnullsnotdistinct")
    ***REMOVED***
            else:
    ...
    def _reflect_constraint(
        ...
        while batches:
            ...
            result = connection.execute(
                ...
                {"oids": [r[0] for r in batch], "contype": contype***REMOVED***,
***REMOVED***
...
***REMOVED***
    ...
    def _fk_regex_pattern(self):
        ...
        return re.compile(
            r"FOREIGN KEY \((.*?)\) "
            rf"REFERENCES (?:({qtoken***REMOVED***)\.)?({qtoken***REMOVED***)\(((?:{qtoken***REMOVED***(?: *, *)?)+)\)"  # noqa: E501
...
***REMOVED***
    ...
    def get_multi_indexes(
        ...
        while batches:
            ...
            result = connection.execute(
                self._index_query, {"oids": [r[0] for r in batch]***REMOVED***
***REMOVED***.mappings()
            ...
            ***REMOVED***
                    ...
                    index = {"name": index_name, "unique": row["indisunique"]***REMOVED***
...
***REMOVED***
    ...
    def get_multi_table_comment(
        ...
        return (
            (
                (schema, table),
                {"text": comment***REMOVED*** if comment is not None else default(),
***REMOVED***
...
***REMOVED***
    ...
    ***REMOVED***.sub(r"\1", m.group(1))
            ...
            ***REMOVED***
            if m and m.group(2):
                entry["dialect_options"] = {"not_valid": True***REMOVED***
...
***REMOVED***
    ...
    ***REMOVED***
                for name, def_ in sorted_constraints:
                    ...
                    check = def_[7:-1]
                    constraints.append({"name": name, "check": check***REMOVED***)
