...
_msg_validate_usecols_names = (
    "Usecols do not match columns, columns expected but not found: {0***REMOVED***"
)
...
def test_usecols_index_col_false(all_parsers, data):
    ...
    parser = all_parsers
    usecols = ["a", "c", "d"]
    expected = DataFrame({"a": [1, 5], "c": [3, 7], "d": [4, 8]***REMOVED***)
...
def test_usecols_index_col_conflict(all_parsers, usecols, index_col, request):
    ...
    expected = DataFrame({"c": [1, 2]***REMOVED***, index=Index(["a", "b"], name="b"))
...
def test_usecols_index_col_conflict2(all_parsers):
    ...
    expected = DataFrame({"b": ["a", "b"], "c": [1, 2], "d": ("one", "two")***REMOVED***)
...
def test_usecols_implicit_index_col(all_parsers):
    ...
    expected = DataFrame({"a": ["apple", "orange"], "b": ["bat", "cow"]***REMOVED***, index=[4, 8])
    tm.assert_frame_equal(result, expected)
...
"""
    ...
    expected = DataFrame({"b": [2], "d": [4]***REMOVED***, index=Index([3], name="c"))
    tm.assert_frame_equal(result, expected)
...
"""
    ...
    expected = DataFrame({"b": [2], "c": [3]***REMOVED***, index=Index([4], name="d"))
    tm.assert_frame_equal(result, expected)
            ...
            DataFrame(
                {
                    ...
                    ***REMOVED***,
                    "bBb": {0: 8, 1: 2, 2: 7***REMOVED***,
                    "ddd": {0: "a", 1: "b", 2: "a"***REMOVED***,
                ***REMOVED***
***REMOVED***,
        ),
...
def test_incomplete_first_row(all_parsers, usecols):
    ...
    parser = all_parsers
    names = ["a", "b", "c"]
    expected = DataFrame({"a": [1, 1], "c": [np.nan, 3]***REMOVED***)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["a", "b", "c", "d"],
            {***REMOVED***,
            DataFrame({"a": [1, 5], "b": [2, 6], "c": [3, 7], "d": [4, 8]***REMOVED***),
            None,
        ),
        (
            ["a", "b", "c", "f"],
        ...
        (["a", "b", "f"], {***REMOVED***, None, _msg_validate_usecols_names.format(r"\['f'\]")),
        (
            ["a", "b", "f", "g"],
            {***REMOVED***,
            None,
        ...
        (
            None,
            {"header": 0, "names": ["A", "B", "C", "D"]***REMOVED***,
            DataFrame({"A": [1, 5], "B": [2, 6], "C": [3, 7], "D": [4, 8]***REMOVED***),
            None,
        ),
        (
...
def test_usecols_subset_names_mismatch_orig_columns(all_parsers, usecols, request):
    ...
    expected = DataFrame({"A": [1, 5], "C": [3, 7]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_usecols_additional_columns(all_parsers):
    ...
    expected = DataFrame({"a": ["x"], "b": "y"***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_usecols_additional_columns_integer_columns(all_parsers):
    ...
    expected = DataFrame({"0": ["x"], "1": "y"***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
"""
    result = parser.read_csv(
        StringIO(data),
        usecols=["col1", "col2"],
        dtype={"col1": "string", "col2": "uint8", "col3": "string"***REMOVED***,
    )
    expected = DataFrame(
        {"col1": array(["a", "b"]), "col2": np.array([1, 2], dtype="uint8")***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
