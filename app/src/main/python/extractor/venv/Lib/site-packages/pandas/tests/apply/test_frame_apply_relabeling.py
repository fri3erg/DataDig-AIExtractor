...
def test_agg_relabel():
    # GH 26513
    df = pd.DataFrame({"A": [1, 2, 1, 2], "B": [1, 2, 3, 4], "C": [3, 4, 5, 6]***REMOVED***)

    # simplest case with one column, one func
    result = df.agg(foo=("B", "sum"))
    expected = pd.DataFrame({"B": [10]***REMOVED***, index=pd.Index(["foo"]))
    tm.assert_frame_equal(result, expected)

    # test on same column with different methods
    result = df.agg(foo=("B", "sum"), bar=("B", "min"))
    expected = pd.DataFrame({"B": [10, 1]***REMOVED***, index=pd.Index(["foo", "bar"]))
...
def test_agg_relabel_partial_functions():
    ...
    expected = pd.DataFrame(
        {"A": [1.5, 1.5, 1.0]***REMOVED***, index=pd.Index(["foo", "bar", "cat"])
    )
    tm.assert_frame_equal(result, expected)
...
def test_agg_namedtuple():
    # GH 26513
    df = pd.DataFrame({"A": [0, 1], "B": [1, 2]***REMOVED***)
    result = df.agg(
    ...
    expected = pd.DataFrame(
        {"B": [3, 1, 2, 2]***REMOVED***, index=pd.Index(["foo", "bar", "cat", "fft"])
    )
    tm.assert_frame_equal(result, expected)
    ...
    expected = pd.DataFrame(
        {"A": [0.0, np.nan, 1.0], "B": [np.nan, 2.0, np.nan]***REMOVED***,
