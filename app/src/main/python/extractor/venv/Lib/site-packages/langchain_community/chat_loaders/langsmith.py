...
class LangSmithRunChatLoader(BaseChatLoader):
    ...
    def _get_messages_from_llm_run(llm_run: "Run") -> ChatSession:
        ...
        if llm_run.run_type != "llm":
            raise ValueError(f"Expected run of type llm. Got: {llm_run.run_type***REMOVED***")
        if "messages" not in llm_run.inputs:
            raise ValueError(f"Run has no 'messages' inputs. Got {llm_run.inputs***REMOVED***")
    ...
    def _get_functions_from_llm_run(llm_run: "Run") -> Optional[List[Dict]]:
        ...
        if llm_run.run_type != "llm":
            raise ValueError(f"Expected run of type llm. Got: {llm_run.run_type***REMOVED***")
        return (llm_run.extra or {***REMOVED***).get("invocation_params", {***REMOVED***).get("functions")
    ...
    def lazy_load(self) -> Iterator[ChatSession]:
        ...
        for run_obj in self.runs:
            try:
                ...
                logger.warning(f"Could not load run {run_obj***REMOVED***: {repr(e)***REMOVED***")
                continue
