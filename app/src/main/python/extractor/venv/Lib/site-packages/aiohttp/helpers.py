...
CHAR = {chr(i) for i in range(0, 128)***REMOVED***
CTL = {chr(i) for i in range(0, 32)***REMOVED*** | {
    chr(127),
***REMOVED***
SEPARATORS = {
    "(",
...
***REMOVED***.decode(encoding)
    ...
    def encode(self) -> str:
        """Encode credentials."""
        creds = (f"{self.login***REMOVED***:{self.password***REMOVED***").encode(self.encoding)
...
def basicauth_from_netrc(netrc_obj: Optional[netrc.netrc], host: str) -> BasicAuth:
    ...
    if auth_from_netrc is None:
        raise LookupError(f"No entry for {host!s***REMOVED*** found in the `.netrc` file.")
...
def proxies_from_env() -> Dict[str, ProxyInfo]:
    ...
    ***REMOVED***
    ...
    stripped = {k: strip_auth_from_url(v) for k, v in proxy_urls.items()***REMOVED***
    ret = {***REMOVED***
...
def get_env_proxy_for_url(url: URL) -> Tuple[URL, Optional[BasicAuth]]:
    ...
    if url.host is not None and proxy_bypass(url.host):
        raise LookupError(f"Proxying is disallowed for `{url.host!r***REMOVED***`")

    proxies_in_env = proxies_from_env()
    try:
        proxy_info = proxies_in_env[url.scheme]
    except KeyError:
        raise LookupError(f"No proxies found for `{url!s***REMOVED***` in the env")
    else:
...
QCONTENT = {chr(i) for i in range(0x20, 0x7F)***REMOVED*** | {"\t"***REMOVED***
...
def quoted_string(content: str) -> str:
    ...
    if not (QCONTENT > set(content)):
        raise ValueError(f"bad content for quoted-string {content!r***REMOVED***")
...
def content_disposition_header(
    ...
    if not disptype or not (TOKEN > set(disptype)):
        raise ValueError("bad content disposition type {!r***REMOVED***" "".format(disptype))
    ...
    if params:
        ...
        for key, val in params.items():
            if not key or not (TOKEN > set(key)):
                raise ValueError(
                    "bad content disposition parameter" " {!r***REMOVED***={!r***REMOVED***".format(key, val)
    ***REMOVED***
            if quote_fields:
...
_ipv4_pattern = (
    r"^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3***REMOVED***"
    r"(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$"
)
_ipv6_pattern = (
    r"^(?:(?:(?:[A-F0-9]{1,4***REMOVED***:){6***REMOVED***|(?=(?:[A-F0-9]{0,4***REMOVED***:){0,6***REMOVED***"
    r"(?:[0-9]{1,3***REMOVED***\.){3***REMOVED***[0-9]{1,3***REMOVED***$)(([0-9A-F]{1,4***REMOVED***:){0,5***REMOVED***|:)"
    r"((:[0-9A-F]{1,4***REMOVED***){1,5***REMOVED***:|:)|::(?:[A-F0-9]{1,4***REMOVED***:){5***REMOVED***)"
    r"(?:(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])\.){3***REMOVED***"
    r"(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])|(?:[A-F0-9]{1,4***REMOVED***:){7***REMOVED***"
    r"[A-F0-9]{1,4***REMOVED***|(?=(?:[A-F0-9]{0,4***REMOVED***:){0,7***REMOVED***[A-F0-9]{0,4***REMOVED***$)"
    r"(([0-9A-F]{1,4***REMOVED***:){1,7***REMOVED***|:)((:[0-9A-F]{1,4***REMOVED***){1,7***REMOVED***|:)|(?:[A-F0-9]{1,4***REMOVED***:){7***REMOVED***"
    r":|:(:[A-F0-9]{1,4***REMOVED***){7***REMOVED***)$"
)
...
def _is_ip_address(
    ...
    if isinstance(host, str):
        ...
        raise TypeError(f"{host***REMOVED*** [{type(host)***REMOVED***] is not a str or bytes")
    ...
    def __repr__(self) -> str:
        ...
        if t is None:
            ...
            if t.__module__ == "builtins":
                ...
                t_repr = f"{t.__module__***REMOVED***.{t.__qualname__***REMOVED***"
        else:
            t_repr = repr(t)
        return f"<AppKey({self._name***REMOVED***, type={t_repr***REMOVED***)>"
...
class ChainMapProxy(Mapping[Union[str, AppKey[Any]], Any]):
    ...
    def __init_subclass__(cls) -> None:
        raise TypeError(
            "Inheritance class {***REMOVED*** from ChainMapProxy "
    ...
    def __iter__(self) -> Iterator[Union[str, AppKey[Any]]]:
        d: Dict[Union[str, AppKey[Any]], Any] = {***REMOVED***
    ...
    def __repr__(self) -> str:
        ...
        return f"ChainMapProxy({content***REMOVED***)"


# https://tools.ietf.org/html/rfc7232#section-2.3
_ETAGC = r"[!\x23-\x7E\x80-\xff]+"
_ETAGC_RE = re.compile(_ETAGC)
_QUOTED_ETAG = rf'(W/)?"({_ETAGC***REMOVED***)"'
QUOTED_ETAG_RE = re.compile(_QUOTED_ETAG)
LIST_QUOTED_ETAG_RE = re.compile(rf"({_QUOTED_ETAG***REMOVED***)(?:\s*,\s*|$)|(.)")
...
def validate_etag_value(value: str) -> None:
    if value != ETAG_ANY and not _ETAGC_RE.fullmatch(value):
        raise ValueError(
            f"Value {value!r***REMOVED*** is not a valid etag. Maybe it contains '\"'?"
        )
...
def status_code_must_be_empty_body(code: int) -> bool:
    ...
    # https://datatracker.ietf.org/doc/html/rfc9112#section-6.3-2.1
    return code in {204, 304***REMOVED*** or 100 <= code < 200
