...
class Apply(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        *,
        ...
        self.args = args or ()
        self.kwargs = kwargs or {***REMOVED***

        self.engine = engine
        self.engine_kwargs = {***REMOVED*** if engine_kwargs is None else engine_kwargs

        if result_type not in [None, "reduce", "broadcast", "expand"]:
            raise ValueError(
                "invalid value for result_type, must be one "
                "of {None, 'reduce', 'broadcast', 'expand'***REMOVED***"
***REMOVED***
    ...
    def transform(self) -> DataFrame | Series:
        ...
        if is_list_like(func) and not is_dict_like(func):
            ...
            if is_series:
                func = {com.get_callable_name(v) or v: v for v in func***REMOVED***
            else:
                func = {col: func for col in obj***REMOVED***
    ...
    def transform_dict_like(self, func) -> DataFrame:
        ...
        results: dict[Hashable, DataFrame | Series] = {***REMOVED***
    ...
    ***REMOVED*** from err
    ...
    def wrap_results_dict_like(
        ...
        if all(is_ndframe):
            ...
            result = concat(
                {k: results[k] for k in keys_to_use***REMOVED***,
                axis=axis,
                keys=keys_to_use,
***REMOVED***
        elif any(is_ndframe):
...
***REMOVED***:
                raise ValueError(f"Operation {func***REMOVED*** does not support axis=1")
...
***REMOVED***
    ...
    def normalize_dictlike_arg(
        ...
        if obj.ndim != 1:
            ...
            if len(cols) > 0:
                raise KeyError(f"Column(s) {list(cols)***REMOVED*** do not exist")

        aggregator_types = (list, tuple, dict)

        # if we have a dict of any non-scalars
        # eg. {'A' : ['mean']***REMOVED***, normalize all to
        # be list-likes
        # Cannot use func.values() because arg may be a Series
        if any(isinstance(x, aggregator_types) for _, x in func.items()):
            new_func: AggFuncTypeDict = {***REMOVED***
    ...
    def _apply_str(self, obj, func: str, *args, **kwargs):
        ...
        if hasattr(obj, func):
            ...
            msg = f"'{func***REMOVED***' is not a valid function for '{type(obj).__name__***REMOVED***' object"
...
class NDFrameApply(Apply):
    ...
    def agg_or_apply_list_like(
        ...
        if op_name == "apply":
            if isinstance(self, FrameApply):
                ...
                by_row = False
            kwargs = {**kwargs, "by_row": by_row***REMOVED***
    ...
    def agg_or_apply_dict_like(
        ...
        if op_name == "apply":
            by_row = "_compat" if self.by_row else False
            kwargs.update({"by_row": by_row***REMOVED***)
...
class FrameApply(NDFrameApply):
    ...
    def __init__(
        ...
        *,
        ...
        if by_row is not False and by_row != "compat":
            raise ValueError(f"by_row={by_row***REMOVED*** not allowed")
        super().__init__(
            obj,
            func,
            raw,
    ...
    def validate_values_for_numba(self):
        ...
        for colname, dtype in self.obj.dtypes.items():
            if not is_numeric_dtype(dtype):
                raise ValueError(
                    f"Column {colname***REMOVED*** must have a numeric dtype. "
                    f"Found '{dtype***REMOVED***' instead"
    ***REMOVED***
            if is_extension_array_dtype(dtype):
                raise ValueError(
                    f"Column {colname***REMOVED*** is backed by an extension array, "
    ...
    ***REMOVED***
    ...
    def apply_raw(self, engine="python", engine_kwargs=None):
        ...
        if engine == "numba":
            engine_kwargs = {***REMOVED*** if engine_kwargs is None else engine_kwargs
    ...
    def agg(self):
        ...
        if result is None:
            ...
            try:
                ...
                msg = (
                    f"using {func***REMOVED*** in {type(obj).__name__***REMOVED***.agg cannot aggregate and "
                    f"has been deprecated. Use {type(obj).__name__***REMOVED***.transform to "
...
class GroupByApply(Apply):
    ...
    def agg_or_apply_list_like(
        ...
        kwargs = self.kwargs
        if op_name == "apply":
            kwargs = {**kwargs, "by_row": False***REMOVED***
    ...
    def agg_or_apply_dict_like(
        ...
        if is_groupby:
            ...
            kwargs.update({"engine": engine, "engine_kwargs": engine_kwargs***REMOVED***)
...
def reconstruct_func(
    ...
    and str/Callable/list of them (e.g. {'A': 'min'***REMOVED***, or {'A': [np.min, lambda x: x]***REMOVED***)
    ...
    func: agg function (e.g. 'min' or Callable) or list of agg functions
        (e.g. ['min', np.max]) or dictionary (e.g. {'A': ['min', np.max]***REMOVED***).
    **kwargs: dict, kwargs used in is_multi_agg_with_relabel and
    ...
    Examples
    --------
    >>> reconstruct_func(None, **{"foo": ("col", "min")***REMOVED***)
    (True, defaultdict(<class 'list'>, {'col': ['min']***REMOVED***), ('foo',), array([0]))
...
def normalize_keyword_aggregation(
    ...
    Examples
    --------
    >>> normalize_keyword_aggregation({"output": ("input", "sum")***REMOVED***)
    (defaultdict(<class 'list'>, {'input': ['sum']***REMOVED***), ('output',), array([0]))
    """
...
def _make_unique_kwarg_list(
    ...
    """
    return [
        (pair[0], f"{pair[1]***REMOVED***_{seq[:i].count(pair)***REMOVED***") if seq.count(pair) > 1 else pair
...
def relabel_result(
    ...
    >>> result = pd.DataFrame(
    ...     {"A": [np.nan, 2, np.nan], "C": [6, np.nan, np.nan], "B": [np.nan, 4, 2.5]***REMOVED***,
    ...     index=["max", "mean", "min"]
    ... )
    >>> funcs = {"A": ["max"], "C": ["max"], "B": ["mean", "min"]***REMOVED***
    ...
    reordered_result_in_dict: dict[Hashable, Series] = {***REMOVED***
    idx = 0
...
def _managle_lambda_list(aggfuncs: Sequence[Any]) -> Sequence[Any]:
    ...
    for aggfunc in aggfuncs:
        if com.get_callable_name(aggfunc) == "<lambda>":
            ...
            aggfunc.__name__ = f"<lambda_{i***REMOVED***>"
            i += 1
...
def validate_func_kwargs(
    ...
    Examples
    --------
    >>> validate_func_kwargs({'one': 'min', 'two': 'max'***REMOVED***)
    (['one', 'two'], ['min', 'max'])
    """
    tuple_given_message = "func is expected but received {***REMOVED*** in **kwargs."
    columns = list(kwargs)
    func = []
...
def warn_alias_replacement(
    ...
    if alias.startswith("np."):
        ...
        full_alias = f"{type(obj).__name__***REMOVED***.{alias***REMOVED***"
        alias = f'"{alias***REMOVED***"'
    warnings.warn(
        f"The provided callable {func***REMOVED*** is currently using "
        f"{full_alias***REMOVED***. In a future version of pandas, "
        f"the provided callable will be used directly. To keep current "
        f"behavior pass the string {alias***REMOVED*** instead.",
