...
class MetadataEndpoint(BaseEndpoint):
   ...
   """
    ...
    def __init__(self, endpoints, claims={***REMOVED***, raise_errors=True):
    ...
    def validate_metadata(self, array, key, is_required=False, is_list=False, is_url=False, is_issuer=False):
        ...
        if key not in array:
            if is_required:
                raise ValueError("key {***REMOVED*** is a mandatory metadata.".format(key))

        elif is_issuer:
            if not utils.is_secure_transport(array[key]):
                raise ValueError("key {***REMOVED***: {***REMOVED*** must be an HTTPS URL".format(key, array[key]))
            if "?" in array[key] or "&" in array[key] or "#" in array[key]:
                raise ValueError("key {***REMOVED***: {***REMOVED*** must not contain query or fragment components".format(key, array[key]))

        elif is_url:
            if not array[key].startswith("http"):
                raise ValueError("key {***REMOVED***: {***REMOVED*** must be an URL".format(key, array[key]))

        elif is_list:
            if not isinstance(array[key], list):
                raise ValueError("key {***REMOVED***: {***REMOVED*** must be an Array".format(key, array[key]))
            for elem in array[key]:
                if not isinstance(elem, str):
                    raise ValueError("array {***REMOVED***: {***REMOVED*** must contains only string (not {***REMOVED***)".format(key, array[key], elem))
