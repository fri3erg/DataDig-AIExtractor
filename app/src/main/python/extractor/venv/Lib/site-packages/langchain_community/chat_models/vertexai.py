...
def _parse_chat_history(history: List[BaseMessage]) -> _ChatHistory:
    ...
    for i, message in enumerate(history):
        ...
        if i == 0 and isinstance(message, SystemMessage):
            ...
            raise ValueError(
                f"Unexpected message with type {type(message)***REMOVED*** at the position {i***REMOVED***."
***REMOVED***
...
def _is_url(s: str) -> bool:
    try:
        ...
        logger.debug(f"Unable to parse URL: {e***REMOVED***")
        return False
...
def _parse_chat_history_gemini(
    ...
    def _convert_to_prompt(part: Union[str, Dict]) -> Part:
        ...
        if not isinstance(part, Dict):
            raise ValueError(
                f"Message's content is expected to be a dict, got {type(part)***REMOVED***!"
***REMOVED***
        if part["type"] == "text":
            ...
            if path.startswith("gs://"):
                ...
                try:
                    encoded = re.search(r"data:image/\w{2,4***REMOVED***;base64,(.*)", path).group(
                        1
        ***REMOVED***
...
def _parse_examples(examples: List[BaseMessage]) -> List["InputOutputTextPair"]:
    ...
    if len(examples) % 2 != 0:
        raise ValueError(
            f"Expect examples to have an even amount of messages, got {len(examples)***REMOVED***."
        )
    example_pairs = []
    input_text = None
        ...
        if i % 2 == 0:
            if not isinstance(example, HumanMessage):
                raise ValueError(
                    ...
                    f"{type(example)***REMOVED*** for the {i***REMOVED***th message."
    ***REMOVED***
...
def _get_question(messages: List[BaseMessage]) -> HumanMessage:
    ...
    if not isinstance(question, HumanMessage):
        raise ValueError(
            f"Last message in the list should be from human, got {question.type***REMOVED***."
        )
    return question
