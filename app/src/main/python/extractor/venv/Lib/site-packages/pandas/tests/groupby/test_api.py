...
def test_tab_completion(multiindex_dataframe_random_data):
    ...
    results = {v for v in dir(grp) if not v.startswith("_")***REMOVED***
    expected = {
        "A",
        "B",
        "C",
...
def test_all_methods_categorized(multiindex_dataframe_random_data):
    ...
    names = {_ for _ in dir(grp) if not _.startswith("_")***REMOVED*** - set(
...
def test_frame_consistency(groupby_func):
    ...
    if groupby_func in ("any", "all"):
        exclude_expected = {"kwargs", "bool_only", "axis"***REMOVED***
    elif groupby_func in ("count",):
        exclude_expected = {"numeric_only", "axis"***REMOVED***
    elif groupby_func in ("nunique",):
        exclude_expected = {"axis"***REMOVED***
    elif groupby_func in ("max", "min"):
        exclude_expected = {"axis", "kwargs", "skipna"***REMOVED***
        exclude_result = {"min_count", "engine", "engine_kwargs"***REMOVED***
    elif groupby_func in ("mean", "std", "sum", "var"):
        exclude_expected = {"axis", "kwargs", "skipna"***REMOVED***
        exclude_result = {"engine", "engine_kwargs"***REMOVED***
    elif groupby_func in ("median", "prod", "sem"):
        exclude_expected = {"axis", "kwargs", "skipna"***REMOVED***
    elif groupby_func in ("backfill", "bfill", "ffill", "pad"):
        exclude_expected = {"downcast", "inplace", "axis", "limit_area"***REMOVED***
    elif groupby_func in ("cummax", "cummin"):
        exclude_expected = {"skipna", "args"***REMOVED***
        exclude_result = {"numeric_only"***REMOVED***
    elif groupby_func in ("cumprod", "cumsum"):
        exclude_expected = {"skipna"***REMOVED***
    elif groupby_func in ("pct_change",):
        exclude_expected = {"kwargs"***REMOVED***
        exclude_result = {"axis"***REMOVED***
    elif groupby_func in ("rank",):
        exclude_expected = {"numeric_only"***REMOVED***
    elif groupby_func in ("quantile",):
        exclude_expected = {"method", "axis"***REMOVED***
...
def test_series_consistency(request, groupby_func):
    ...
    if groupby_func in ("any", "all"):
        ...
        exclude_expected = {"args", "kwargs"***REMOVED***
    elif groupby_func in ("quantile",):
        exclude_result = {"numeric_only"***REMOVED***
