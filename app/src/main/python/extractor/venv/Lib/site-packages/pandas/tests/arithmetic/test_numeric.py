...
class TestDivisionByZero:
    ...
    def test_div_negative_zero(self, zero, numeric_idx, op):
        ...
        if numeric_idx.dtype == np.uint64:
            pytest.skip(f"Div by negative 0 not relevant for {numeric_idx.dtype***REMOVED***")
        idx = numeric_idx - 3
...
***REMOVED***
    ...
    def test_df_div_zero_df(self):
        ...
        df = pd.DataFrame({"first": [3, 4, 5, 8], "second": [0, 0, 0, 3]***REMOVED***)
        result = df / df

        first = Series([1.0, 1.0, 1.0, 1.0])
        second = Series([np.nan, np.nan, np.nan, 1])
        expected = pd.DataFrame({"first": first, "second": second***REMOVED***)
    ...
    def test_df_mod_zero_df(self, using_array_manager):
        ...
        df = pd.DataFrame({"first": [3, 4, 5, 8], "second": [0, 0, 0, 3]***REMOVED***, copy=False)
...
class TestMultiplicationDivision:
    ...
    def test_modulo2(self):
        with np.errstate(all="ignore"):
            # GH#3590, modulo as ints
            p = pd.DataFrame({"first": [3, 4, 5, 8], "second": [0, 0, 0, 3]***REMOVED***)
...
***REMOVED***,
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                pd.DataFrame({"x": [1, 2, 3]***REMOVED***, index=list("ABC")),
                pd.DataFrame({"x": [2, 2, 2]***REMOVED***, index=list("ABD")),
                pd.DataFrame({"x": [3.0, 4.0, np.nan, np.nan]***REMOVED***, index=list("ABCD")),
***REMOVED***,
            (
                pd.DataFrame({"x": [1, 2, 3]***REMOVED***, index=list("ABC")),
                pd.DataFrame({"x": [2, 2, 2, 2]***REMOVED***, index=list("ABCD")),
                pd.DataFrame({"x": [3, 4, 5, np.nan]***REMOVED***, index=list("ABCD")),
***REMOVED***,
        ],
    )
    ...
    def test_series_frame_radd_bug(self, fixed_now_ts):
        ...
        frame = pd.DataFrame({"vals": vals***REMOVED***)
        result = "foo_" + frame
        expected = pd.DataFrame({"vals": vals.map(lambda x: "foo_" + x)***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_frame_operators_none_to_nan(self):
        df = pd.DataFrame({"a": ["a", None, "b"]***REMOVED***)
        tm.assert_frame_equal(df + df, pd.DataFrame({"a": ["aa", np.nan, "bb"]***REMOVED***))
...
class TestUFuncCompat:
    ...
    def test_ufunc_compat(self, holder, dtype):
        ...
        if holder is RangeIndex:
            if dtype != np.int64:
                pytest.skip(f"dtype {dtype***REMOVED*** not relevant for RangeIndex")
...
def test_fill_value_inf_masking():
    ...
    df = pd.DataFrame({"A": [0, 1, 2], "B": [1.1, None, 1.1]***REMOVED***)

    other = pd.DataFrame({"A": [1.1, 1.2, 1.3]***REMOVED***, index=[0, 2, 3])

    result = df.rfloordiv(other, fill_value=1)

    expected = pd.DataFrame(
        {"A": [np.inf, 1.0, 0.0, 1.0], "B": [0.0, np.nan, 0.0, np.nan]***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
...
def test_sub_multiindex_swapped_levels():
    # GH 9952
    df = pd.DataFrame(
        {"a": np.random.default_rng(2).standard_normal(6)***REMOVED***,
