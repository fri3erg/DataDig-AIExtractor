...
def test_groupby_cumprod():
    # GH 4095
    df = DataFrame({"key": ["b"] * 10, "value": 2***REMOVED***)
    ...
    df = DataFrame({"key": ["b"] * 100, "value": 2***REMOVED***)
...
def test_groupby_cumprod_overflow():
    ...
    df = DataFrame({"key": ["b"] * 4, "value": 100_000***REMOVED***)
    actual = df.groupby("key")["value"].cumprod()
    expected = Series(
...
def test_groupby_cumprod_nan_influences_other_columns():
    ...
    expected = DataFrame({"b": [1, np.nan, np.nan], "c": [1, 2, 6.0]***REMOVED***)
...
def test_cummin(dtypes_for_minmax):
    ...
    # GH 15048
    base_df = DataFrame({"A": [1, 1, 1, 1, 2, 2, 2, 2], "B": [3, 4, 3, 2, 2, 3, 2, 1]***REMOVED***)
    expected_mins = [3, 3, 3, 2, 2, 2, 2, 1]

    df = base_df.astype(dtype)

    expected = DataFrame({"B": expected_mins***REMOVED***).astype(dtype)
    ...
    base_df = base_df.astype({"B": "float"***REMOVED***)
    base_df.loc[[0, 2, 4, 6], "B"] = np.nan
    expected = DataFrame({"B": [np.nan, 4, np.nan, 2, np.nan, 3, np.nan, 1]***REMOVED***)
    ...
    # GH 15561
    df = DataFrame({"a": [1], "b": pd.to_datetime(["2001"])***REMOVED***)
    ...
    # GH 15635
    df = DataFrame({"a": [1, 2, 1], "b": [1, 2, 2]***REMOVED***)
    result = df.groupby("a").b.cummin()
...
def test_cummin_max_all_nan_column(method, dtype):
    base_df = DataFrame({"A": [1, 1, 1, 1, 2, 2, 2, 2], "B": [np.nan] * 8***REMOVED***)
    base_df["B"] = base_df["B"].astype(dtype)
    grouped = base_df.groupby("A")

    expected = DataFrame({"B": [np.nan] * 8***REMOVED***, dtype=dtype)
...
def test_cummax(dtypes_for_minmax):
    ...
    expected = DataFrame({"B": expected_maxs***REMOVED***).astype(dtype)
    ...
    base_df = base_df.astype({"B": "float"***REMOVED***)
    base_df.loc[[0, 2, 4, 6], "B"] = np.nan
    expected = DataFrame({"B": [np.nan, 4, np.nan, 4, np.nan, 3, np.nan, 3]***REMOVED***)
    ...
    # GH 15635
    df = DataFrame({"a": [1, 2, 1], "b": [2, 1, 1]***REMOVED***)
    result = df.groupby("a").b.cummax()
...
def test_cummax_i8_at_implementation_bound():
    ...
    df = DataFrame({"A": 1, "B": ser, "C": ser._values.view("M8[ns]")***REMOVED***)
    gb = df.groupby("A")
...
def test_cummin_max_skipna(method, dtype, groups, expected_data):
    # GH-34047
    df = DataFrame({"a": Series([1, None, 2], dtype=dtype)***REMOVED***)
    orig = df.copy()
...
def test_cummin_max_skipna_multiple_cols(method):
    ...
    df = DataFrame({"a": [np.nan, 2.0, 2.0], "b": [2.0, 2.0, 2.0]***REMOVED***)
    gb = df.groupby([1, 1, 1])[["a", "b"]]

    result = getattr(gb, method)(skipna=False)
    expected = DataFrame({"a": [np.nan, np.nan, np.nan], "b": [2.0, 2.0, 2.0]***REMOVED***)
...
def test_numpy_compat(func):
    # see gh-12811
    df = DataFrame({"A": [1, 2, 1], "B": [1, 2, 3]***REMOVED***)
    g = df.groupby("A")
...
def test_nullable_int_not_cast_as_float(method, dtype, val):
    data = [val, pd.NA]
    df = DataFrame({"grp": [1, 1], "b": data***REMOVED***, dtype=dtype)
    grouped = df.groupby("grp")

    result = grouped.transform(method)
    expected = DataFrame({"b": data***REMOVED***, dtype=dtype)
