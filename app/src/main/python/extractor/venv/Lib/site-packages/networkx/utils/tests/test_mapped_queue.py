...
class TestMappedQueue:
    ...
    def _check_map(self, q):
        assert q.position == {elt: pos for pos, elt in enumerate(q.heap)***REMOVED***

    def _make_mapped_queue(self, h):
        q = MappedQueue()
        q.heap = h
        q.position = {elt: pos for pos, elt in enumerate(h)***REMOVED***
        return q
...
class TestMappedDict(TestMappedQueue):
    def _make_mapped_queue(self, h):
        priority_dict = {elt: elt for elt in h***REMOVED***
        return MappedQueue(priority_dict)

    def test_init(self):
        d = {5: 0, 4: 1, "a": 2, 2: 3, 1: 4***REMOVED***
        q = MappedQueue(d)
        assert q.position == d

    def test_ties(self):
        d = {5: 0, 4: 1, 3: 2, 2: 3, 1: 4***REMOVED***
        q = MappedQueue(d)
        assert q.position == {elt: pos for pos, elt in enumerate(q.heap)***REMOVED***
