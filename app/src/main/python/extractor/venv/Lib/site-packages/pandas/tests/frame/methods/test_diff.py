...
class TestDataFrameDiff:
    ...
    def test_diff_int_dtype(self):
        ...
        rs = DataFrame({"s": ser***REMOVED***).diff()
        assert rs.s[1] == 1
    ...
    def test_diff_axis1_nonconsolidated(self):
        # GH#10907
        df = DataFrame({"y": Series([2]), "z": Series([3])***REMOVED***)
        df.insert(0, "x", 1)
        result = df.diff(axis=1)
        expected = DataFrame({"x": np.nan, "y": Series(1), "z": Series(1)***REMOVED***)
    ...
    def test_diff_timedelta64_with_nat(self):
        ...
        expected = DataFrame({0: df[0], 1: [pd.NaT, pd.Timedelta(2), pd.Timedelta(2)]***REMOVED***)
    ...
    def test_diff_axis1_mixed_dtypes(self):
        ...
        df = DataFrame({"A": range(3), "B": 2 * np.arange(3, dtype=np.float64)***REMOVED***)

        expected = DataFrame({"A": [np.nan, np.nan, np.nan], "B": df["B"] / 2***REMOVED***)
        ...
        df = DataFrame(
            {"a": np.arange(3, dtype="float32"), "b": np.arange(3, dtype="float64")***REMOVED***
        )
        result = df.diff(axis=1)
        expected = DataFrame({"a": df["a"] * np.nan, "b": df["b"] * 0***REMOVED***)
        tm.assert_frame_equal(result, expected)
    ...
    def test_diff_axis1_mixed_dtypes_negative_periods(self):
        ...
        expected = DataFrame({"A": -1.0 * df["A"], "B": df["B"] * np.nan***REMOVED***)
