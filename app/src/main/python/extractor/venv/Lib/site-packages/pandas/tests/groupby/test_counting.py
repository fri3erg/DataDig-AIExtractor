...
class TestCounting:
    ...
    def test_ngroup(self):
        df = DataFrame({"A": list("aaaba")***REMOVED***)
        g = df.groupby("A")
        sg = g.A
    ...
    def test_ngroup_distinct(self):
        df = DataFrame({"A": list("abcde")***REMOVED***)
        g = df.groupby("A")
        sg = g.A
    ...
    def test_ngroup_dupe_index(self):
        df = DataFrame({"A": list("aaaba")***REMOVED***, index=[0] * 5)
        g = df.groupby("A")
        sg = g.A
    ...
    def test_ngroup_mi(self):
        mi = MultiIndex.from_tuples([[0, 1], [1, 2], [2, 2], [2, 2], [1, 0]])
        df = DataFrame({"A": list("aaaba")***REMOVED***, index=mi)
        g = df.groupby("A")
        sg = g.A
    ...
    def test_ngroup_respects_groupby_order(self, sort):
        df = DataFrame({"a": np.random.default_rng(2).choice(list("abcdef"), 100)***REMOVED***)
    ...
    @pytest.mark.parametrize(
        "datetimelike",
        [
            [Timestamp(f"2016-05-{i:02d***REMOVED*** 20:09:25+00:00") for i in range(1, 4)],
            [Timestamp(f"2016-05-{i:02d***REMOVED*** 20:09:25") for i in range(1, 4)],
            [Timestamp(f"2016-05-{i:02d***REMOVED*** 20:09:25", tz="UTC") for i in range(1, 4)],
    ...
    def test_count_with_datetimelike(self, datetimelike):
        ...
        df = DataFrame({"x": ["a", "a", "b"], "y": datetimelike***REMOVED***)
        res = df.groupby("x").count()
        expected = DataFrame({"y": [2, 1]***REMOVED***, index=["a", "b"])
        expected.index.name = "x"
        tm.assert_frame_equal(expected, res)

    def test_count_with_only_nans_in_first_group(self):
        # GH21956
        df = DataFrame({"A": [np.nan, np.nan], "B": ["a", "b"], "C": [1, 2]***REMOVED***)
    ...
    def test_count_groupby_column_with_nan_in_groupby_column(self):
        ...
        df = DataFrame({"A": [1, 1, 1, 1, 1], "B": [5, 4, np.nan, 3, 0]***REMOVED***)
        res = df.groupby(["B"]).count()
        expected = DataFrame(
            index=Index([0.0, 3.0, 4.0, 5.0], name="B"), data={"A": [1, 1, 1, 1]***REMOVED***
        )
        tm.assert_frame_equal(expected, res)
...
def test_groupby_timedelta_cython_count():
    df = DataFrame(
        {"g": list("ab" * 2), "delta": np.arange(4).astype("timedelta64[ns]")***REMOVED***
    )
...
def test_count_object():
    df = DataFrame({"a": ["a"] * 3 + ["b"] * 3, "c": [2] * 3 + [3] * 3***REMOVED***)
    result = df.groupby("c").a.count()
    expected = Series([3, 3], index=Index([2, 3], name="c"), name="a")
    tm.assert_series_equal(result, expected)

    df = DataFrame({"a": ["a", np.nan, np.nan] + ["b"] * 3, "c": [2] * 3 + [3] * 3***REMOVED***)
    result = df.groupby("c").a.count()
...
def test_lower_int_prec_count():
    ...
    expected = DataFrame(
        {"a": [2, 2], "b": [2, 2], "c": [2, 2]***REMOVED***, index=Index(list("ab"), name="grp")
    )
    tm.assert_frame_equal(result, expected)
...
def test_count_uses_size_on_exception():
    ...
    df = DataFrame({"a": [RaisingObject() for _ in range(4)], "grp": list("ab" * 2)***REMOVED***)
    result = df.groupby("grp").count()
    expected = DataFrame({"a": [2, 2]***REMOVED***, index=Index(list("ab"), name="grp"))
    tm.assert_frame_equal(result, expected)
...
def test_count_arrow_string_array(any_string_dtype):
    ...
    df = DataFrame(
        {"a": [1, 2, 3], "b": Series(["a", "b", "a"], dtype=any_string_dtype)***REMOVED***
    )
    result = df.groupby("a").count()
    expected = DataFrame({"b": 1***REMOVED***, index=Index([1, 2, 3], name="a"))
    tm.assert_frame_equal(result, expected)
