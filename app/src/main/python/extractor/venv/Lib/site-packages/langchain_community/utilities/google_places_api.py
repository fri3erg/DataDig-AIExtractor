...
***REMOVED***
    ...
    def run(self, query: str) -> str:
        ...
        return "\n".join([f"{i+1***REMOVED***. {item***REMOVED***" for i, item in enumerate(places)])
    ...
    def fetch_place_details(self, place_id: str) -> Optional[str]:
        try:
            ...
            logging.error(f"An Error occurred while fetching place details: {e***REMOVED***")
            return None

    def format_place_details(self, place_details: Dict[str, Any]) -> Optional[str]:
        try:
            name = place_details.get("result", {***REMOVED***).get("name", "Unknown")
            address = place_details.get("result", {***REMOVED***).get(
                "formatted_address", "Unknown"
***REMOVED***
            phone_number = place_details.get("result", {***REMOVED***).get(
                "formatted_phone_number", "Unknown"
***REMOVED***
            website = place_details.get("result", {***REMOVED***).get("website", "Unknown")
            place_id = place_details.get("result", {***REMOVED***).get("place_id", "Unknown")

            formatted_details = (
                f"{name***REMOVED***\nAddress: {address***REMOVED***\n"
                f"Google place ID: {place_id***REMOVED***\n"
                f"Phone: {phone_number***REMOVED***\nWebsite: {website***REMOVED***\n\n"
***REMOVED***
            return formatted_details
        except Exception as e:
            logging.error(f"An error occurred while formatting place details: {e***REMOVED***")
            return None
