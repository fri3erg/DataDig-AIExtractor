...
@pytest.mark.parametrize(
    "replace_kwargs",
    [
        {"to_replace": {"a": 1, "b": 4***REMOVED***, "value": -1***REMOVED***,
        # Test CoW splits blocks to avoid copying unchanged columns
        {"to_replace": {"a": 1***REMOVED***, "value": -1***REMOVED***,
        {"to_replace": {"b": 4***REMOVED***, "value": -1***REMOVED***,
        {"to_replace": {"b": {4: 1***REMOVED******REMOVED******REMOVED***,
        # TODO: Add these in a further optimization
        # We would need to see which columns got replaced in the mask
        # which could be expensive
        # {"to_replace": {"b": 1***REMOVED******REMOVED***,
        # 1
    ],
)
def test_replace(using_copy_on_write, replace_kwargs):
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": ["foo", "bar", "baz"]***REMOVED***)
    df_orig = df.copy()
...
def test_replace_regex_inplace_refs(using_copy_on_write, warn_copy_on_write):
    df = DataFrame({"a": ["aaa", "bbb"]***REMOVED***)
    df_orig = df.copy()
    view = df[:]
    arr = get_array(df, "a")
...
def test_replace_mask_all_false_second_block(using_copy_on_write):
    df = DataFrame({"a": [1.5, 2, 3], "b": 100.5, "c": 1, "d": 2***REMOVED***)
    df_orig = df.copy()
...
def test_replace_coerce_single_column(using_copy_on_write, using_array_manager):
    df = DataFrame({"a": [1.5, 2, 3], "b": 100.5***REMOVED***)
    df_orig = df.copy()
...
def test_replace_list_categorical(using_copy_on_write):
    df = DataFrame({"a": ["a", "b", "c"]***REMOVED***, dtype="category")
    arr = get_array(df, "a")
    msg = (
...
def test_replace_inplace(using_copy_on_write, to_replace):
    df = DataFrame({"a": [1.5, 2, 3]***REMOVED***)
...
def test_replace_categorical_inplace_reference(using_copy_on_write, val, to_replace):
    df = DataFrame({"a": Categorical([1, 2, 3])***REMOVED***)
    df_orig = df.copy()
    arr_a = get_array(df, "a")
    view = df[:]
    msg = (
...
def test_replace_categorical_inplace(using_copy_on_write, val):
    ...
    expected = DataFrame({"a": Categorical([val, 2, 3])***REMOVED***)
...
def test_replace_columnwise_no_op_inplace(using_copy_on_write):
    ...
    view = df[:]
    df_orig = df.copy()
    df.replace({"a": 10***REMOVED***, 100, inplace=True)
    if using_copy_on_write:
...
def test_replace_columnwise_no_op(using_copy_on_write):
    df = DataFrame({"a": [1, 2, 3], "b": [1, 2, 3]***REMOVED***)
    df_orig = df.copy()
    df2 = df.replace({"a": 10***REMOVED***, 100)
    if using_copy_on_write:
        assert np.shares_memory(get_array(df2, "a"), get_array(df, "a"))
    df2.iloc[0, 0] = 100
    tm.assert_frame_equal(df, df_orig)
...
def test_replace_chained_assignment(using_copy_on_write):
    df = DataFrame({"a": [1, np.nan, 2], "b": 1***REMOVED***)
    df_orig = df.copy()
    if using_copy_on_write:
