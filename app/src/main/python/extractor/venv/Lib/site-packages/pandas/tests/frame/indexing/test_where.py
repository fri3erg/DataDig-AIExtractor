...
class TestDataFrameIndexingWhere:
    ...
    def test_where_upcasting(self):
        ...
        #
        # expected = Series({ 'float32' : 1, 'float64' : 3 ***REMOVED***)
    ...
    def test_where_series_slicing(self):
        ...
        df = DataFrame({"a": range(3), "b": range(4, 7)***REMOVED***)
    ...
    def test_where_array_like(self, klass):
        ...
        expected = DataFrame({"a": [np.nan, 2, 3]***REMOVED***)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            DataFrame({"a": [2, 5, 7], "b": [4, 8, 9]***REMOVED***),
            [
    ...
    def test_where_invalid_input_multiple(self, cond):
        # see gh-15414: only boolean arrays accepted
        df = DataFrame({"a": [1, 2, 3], "b": [2, 2, 2]***REMOVED***)
        msg = "Boolean array expected for the condition"
    ...
    def test_where_bug(self):
        # see gh-2793
        df = DataFrame(
            {"a": [1.0, 2.0, 3.0, 4.0], "b": [4.0, 3.0, 2.0, 1.0]***REMOVED***, dtype="float64"
        )
        expected = DataFrame(
            {"a": [np.nan, np.nan, 3.0, 4.0], "b": [4.0, 3.0, np.nan, np.nan]***REMOVED***,
            dtype="float64",
        )
    ...
    def test_where_bug_mixed(self, any_signed_int_numpy_dtype):
        ...
        expected = DataFrame(
            {"a": [-1, -1, 3, 4], "b": [4.0, 3.0, -1, -1]***REMOVED***,
        ).astype({"a": any_signed_int_numpy_dtype, "b": "float64"***REMOVED***)
    ...
    def test_where_bug_transposition(self):
        # see gh-7506
        a = DataFrame({0: [1, 2], 1: [3, 4], 2: [5, 6]***REMOVED***)
        b = DataFrame({0: [np.nan, 8], 1: [9, np.nan], 2: [np.nan, np.nan]***REMOVED***)
        ...
        a = DataFrame({0: [4, 6], 1: [1, 0]***REMOVED***)
        b = DataFrame({0: [np.nan, 3], 1: [3, np.nan]***REMOVED***)
        do_not_replace = b.isna() | (a > b)
    ...
    def test_where_none(self):
        ...
        df = DataFrame({"series": Series(range(10))***REMOVED***).astype(float)
        df[df > 7] = None
        expected = DataFrame(
            {"series": Series([0, 1, 2, 3, 4, 5, 6, 7, np.nan, np.nan])***REMOVED***
        )
        ...
        df = DataFrame(
            [
                {"A": 1, "B": np.nan, "C": "Test"***REMOVED***,
                {"A": np.nan, "B": "Test", "C": np.nan***REMOVED***,
            ]
        )
    ...
    ***REMOVED***,
    ...
    @pytest.mark.parametrize("kwargs", [{***REMOVED***, {"other": None***REMOVED***])
    def test_df_where_with_category(self, kwargs):
        ...
        expected = DataFrame({"A": A, "B": B, "C": C***REMOVED***)
    ...
    def test_where_categorical_filtering(self):
        ...
        expected = df.copy().astype({"a": "float"***REMOVED***)
    ...
    def test_where_ea_other(self):
        ...
        expected = DataFrame({"A": [1, np.nan, 3], "B": [4, np.nan, 6]***REMOVED***)
        tm.assert_frame_equal(result, expected)

        ser2 = Series(arr[:2], index=["A", "B"])
        expected = DataFrame({"A": [1, 7, 3], "B": [4, np.nan, 6]***REMOVED***)
...
def test_where_int_downcasting_deprecated():
    ...
    expected = DataFrame({0: arr[:, 0], 1: np.array([2**17] * 3, dtype=np.int32)***REMOVED***)
...
def test_where_bool_comparison():
    # GH 10336
    df_mask = DataFrame(
        {"AAA": [True] * 4, "BBB": [False] * 4, "CCC": [True, False, True, False]***REMOVED***
    )
...
def test_where_columns_casting():
    ...
    df = DataFrame({"a": [1.0, 2.0], "b": [3, np.nan]***REMOVED***)
    expected = df.copy()
...
def test_where_nullable_invalid_na(frame_or_series, any_numeric_ea_dtype):
    ...
    msg = r"Invalid value '.*' for dtype (U?Int|Float)\d{1,2***REMOVED***"
...
def test_where_dt64_2d():
    ...
    expected = DataFrame({"A": other[:, 0], "B": dta[:, 1]***REMOVED***)
    with tm.assert_produces_warning(
        FutureWarning, match="Setting an item of incompatible dtype"
    ):
...
def test_where_producing_ea_cond_for_np_dtype():
    # GH#44014
    df = DataFrame({"a": Series([1, pd.NA, 2], dtype="Int64"), "b": [1, 2, 3]***REMOVED***)
    result = df.where(lambda x: x.apply(lambda y: y > 1, axis=1))
    expected = DataFrame(
        {"a": Series([pd.NA, pd.NA, 2], dtype="Int64"), "b": [np.nan, 2, 3]***REMOVED***
    )
    tm.assert_frame_equal(result, expected)
...
def test_where_inplace_no_other():
    # GH#51685
    df = DataFrame({"a": [1.0, 2.0], "b": ["x", "y"]***REMOVED***)
    cond = DataFrame({"a": [True, False], "b": [False, True]***REMOVED***)
    df.where(cond, inplace=True)
    expected = DataFrame({"a": [1, np.nan], "b": [np.nan, "y"]***REMOVED***)
    tm.assert_frame_equal(df, expected)
