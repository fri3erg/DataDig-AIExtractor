...
Testing treebank's detokenizer
    ...
    >>> detokenizer.detokenize(word_tokenize(s))
    'Hello (world)'
    >>> s = '<A sentence> with (many) [kinds] of {parentheses***REMOVED***. "Sometimes it\'s inside (quotes)". ("Sometimes the otherway around").'
    >>> detokenizer.detokenize(word_tokenize(s))
    '<A sentence> with (many) [kinds] of {parentheses***REMOVED***. "Sometimes it\'s inside (quotes)". ("Sometimes the otherway around").'
