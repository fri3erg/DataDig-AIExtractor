        ...
        for branch in branches[:-1]:
            if not isinstance(branch, (tuple, list)):  # type: ignore[arg-type]
                raise TypeError(
                    ...
                    f"tuples or lists, not {type(branch)***REMOVED***"
    ***REMOVED***

            if not len(branch) == 2:
                raise ValueError(
                    f"RunnableBranch branches must be "
                    f"tuples or lists of length 2, not {len(branch)***REMOVED***"
    ***REMOVED***
        ...
        try:
            for idx, branch in enumerate(self.branches):
                ...
                expression_value = condition.invoke(
                    ...
                    config=patch_config(
                        config,
                        callbacks=run_manager.get_child(tag=f"condition:{idx + 1***REMOVED***"),
        ***REMOVED***,
    ***REMOVED***
                ...
                if expression_value:
                    output = runnable.invoke(
                        ...
                        config=patch_config(
                            config,
                            callbacks=run_manager.get_child(tag=f"branch:{idx + 1***REMOVED***"),
            ***REMOVED***,
                        **kwargs,
        ***REMOVED***
                    break
