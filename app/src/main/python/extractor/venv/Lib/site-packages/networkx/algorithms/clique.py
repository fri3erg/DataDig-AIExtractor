...
def enumerate_all_cliques(G):
    ...
    for u in G:
        ...
        nbrs[u] = {v for v in G[u] if v not in index***REMOVED***
...
def find_cliques(G, nodes=None):
    ...
    >>> pprint({n: sum(1 for c in nx.find_cliques(G) if n in c) for n in G***REMOVED***)
    {0: 13,
     1: 6,
     2: 7,
     3: 3,
    ...
    adj = {u: {v for v in G[u] if v != u***REMOVED*** for u in G***REMOVED***
    ...
    for node in Q:
        if node not in cand:
            raise ValueError(f"The given `nodes` {nodes***REMOVED*** do not form a clique")
        cand &= adj[node]
...
def make_max_clique_graph(G, create_using=None):
    ...
    This function behaves like the following code::
        ...
        G = nx.relabel_nodes(G, {-v: v - 1 for v in G***REMOVED***)
...
def node_clique_number(G, nodes=None, cliques=None, separate_nodes=False):
    ...
    if nodes is None:
        return size_for_n
    return {n: size_for_n[n] for n in nodes***REMOVED***
...
class MaxWeightClique:
    ...
    def __init__(self, G, weight):
        ...
        if weight is None:
            self.node_weights = {v: 1 for v in G.nodes()***REMOVED***
        else:
            for v in G.nodes():
                if weight not in G.nodes[v]:
                    errmsg = f"Node {v!r***REMOVED*** does not have the requested weight field."
                    raise KeyError(errmsg)
                if not isinstance(G.nodes[v][weight], int):
                    errmsg = f"The {weight!r***REMOVED*** field of node {v!r***REMOVED*** is not an integer."
                    raise ValueError(errmsg)
            self.node_weights = {v: G.nodes[v][weight] for v in G.nodes()***REMOVED***
    ...
    def find_branching_nodes(self, P, target):
        ...
        residual_wt = {v: self.node_weights[v] for v in P***REMOVED***
        total_wt = 0
        P = P[:]
        while P:
