...
def mapdomain(x, old, new):
    ...
    .. math::
        m = \\frac{new[1]-new[0]***REMOVED***{old[1]-old[0]***REMOVED***
...
def _vander_nd(vander_fs, points, degrees):
    ...
    .. math::
        W[i_0, \ldots, i_M, j_0, \ldots, j_N] = \prod_{k=0***REMOVED***^N{V_k(x_k)[i_0, \ldots, i_M, j_k]***REMOVED***

    where

    .. math::
        N &= \texttt{len(points)***REMOVED*** = \texttt{len(degrees)***REMOVED*** = \texttt{len(vander\_fs)***REMOVED*** \\
        M &= \texttt{points[k].ndim***REMOVED*** \\
        V_k &= \texttt{vander\_fs[k]***REMOVED*** \\
        x_k &= \texttt{points[k]***REMOVED*** \\
        0 \le j_k &\le \texttt{degrees[k]***REMOVED***

    Expanding the one-dimensional :math:`V_k` functions gives:

    .. math::
        W[i_0, \ldots, i_M, j_0, \ldots, j_N] = \prod_{k=0***REMOVED***^N{B_{k, j_k***REMOVED***(x_k[i_0, \ldots, i_M])***REMOVED***

    where :math:`B_{k,m***REMOVED***` is the m'th basis of the polynomial construction used along
    dimension :math:`k`. For a regular polynomial, :math:`B_{k, m***REMOVED***(x) = P_m(x) = x^m`.
    ...
    if n_dims != len(points):
        raise ValueError(
            f"Expected {n_dims***REMOVED*** dimensions of sample points, got {len(points)***REMOVED***")
    if n_dims != len(degrees):
        raise ValueError(
            f"Expected {n_dims***REMOVED*** dimensions of degrees, got {len(degrees)***REMOVED***")
    if n_dims == 0:
...
def _deprecate_as_int(x, desc):
    ...
    try:
        ...
        try:
            ...
            if ix == x:
                warnings.warn(
                    f"In future, this will raise TypeError, as {desc***REMOVED*** will "
    ...
    ***REMOVED***
        ...
        raise TypeError(f"{desc***REMOVED*** must be an integer") from e
