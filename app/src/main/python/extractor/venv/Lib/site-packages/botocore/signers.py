    ...
    * before-sign: Allows mutating the request before signing.
    ...
    def sign(
        ...
        self._event_emitter.emit(
            'before-sign.{***REMOVED***.{***REMOVED***'.format(
    ...
    ***REMOVED***
    ...
    def _choose_signer(self, operation_name, signing_type, context):
        ...
        ***REMOVED***
        ...
        signing = context.get('signing', {***REMOVED***)
        ...
        handler, response = self._event_emitter.emit_until_response(
            'choose-signer.{***REMOVED***.{***REMOVED***'.format(
                self._service_id.hyphenize(), operation_name
***REMOVED***,
            signing_name=signing_name,
            region_name=region_name,
...
class CloudFrontSigner:
    ...
    def generate_presigned_url(self, url, date_less_than=None, policy=None):
        ...
        params.extend(
            [
                f"Signature={self._url_b64encode(signature).decode('utf8')***REMOVED***",
                f"Key-Pair-Id={self.key_id***REMOVED***",
            ]
        )
    ...
    def build_policy(
        ...
        condition = OrderedDict({"DateLessThan": {"AWS:EpochTime": moment***REMOVED******REMOVED***)
        if ip_address:
            if '/' not in ip_address:
                ip_address += '/32'
            condition["IpAddress"] = {"AWS:SourceIp": ip_address***REMOVED***
        if date_greater_than:
            moment = int(datetime2timestamp(date_greater_than))
            condition["DateGreaterThan"] = {"AWS:EpochTime": moment***REMOVED***
        ordered_payload = [('Resource', resource), ('Condition', condition)]
        custom_policy = {"Statement": [OrderedDict(ordered_payload)]***REMOVED***
...
def generate_db_auth_token(self, DBHostname, Port, DBUsername, Region=None):
    ...
    ***REMOVED***
    ...
    scheme = 'https://'
    endpoint_url = f'{scheme***REMOVED***{DBHostname***REMOVED***:{Port***REMOVED***'
...
class S3PostPresigner:
    ...
    def generate_presigned_post(
        ...
        :returns: A dictionary with two elements: ``url`` and ``fields``.
            ...
            {'url': 'https://mybucket.s3.amazonaws.com
             'fields': {'acl': 'public-read',
                        ...
                        'policy': 'mybase64 encoded policy'***REMOVED***
            ***REMOVED***
        """
        if fields is None:
            fields = {***REMOVED***
        ...
        return {'url': request.url, 'fields': fields***REMOVED***
...
def generate_presigned_post(
    ...
    :type Key: string
    :param Key: Key name, optionally add ${filename***REMOVED*** to the end to
    ...
    ***REMOVED***
    ...
    params = self._emit_api_params(
        api_params={'Bucket': bucket***REMOVED***,
        operation_model=operation_model,
        context=context,
    )
    bucket_is_arn = ArnParser.is_arn(params.get('Bucket', ''))
    ...
    conditions.append({'bucket': bucket***REMOVED***)

    # If the key ends with filename, the only constraint that can be
    # imposed is if it starts with the specified prefix.
    if key.endswith('${filename***REMOVED***'):
        conditions.append(["starts-with", '$key', key[: -len('${filename***REMOVED***')]])
    else:
        conditions.append({'key': key***REMOVED***)
