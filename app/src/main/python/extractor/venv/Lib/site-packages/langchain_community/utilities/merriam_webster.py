...
class MerriamWebsterAPIWrapper(BaseModel):
    ...
    def run(self, query: str) -> str:
        ...
        request_url = (
            f"{MERRIAM_WEBSTER_API_URL***REMOVED***/{quoted_query***REMOVED***"
            f"?key={self.merriam_webster_api_key***REMOVED***"
        )
    ...
    def _format_response(self, query: str, response: requests.Response) -> str:
        ...
        if not content:
            return f"No Merriam-Webster definition was found for query '{query***REMOVED***'."

        if isinstance(content[0], str):
            result = f"No Merriam-Webster definition was found for query '{query***REMOVED***'.\n"
            if len(content) > 1:
                alternatives = [f"{i + 1***REMOVED***. {content[i]***REMOVED***" for i in range(len(content))]
                result += "You can try one of the following alternative queries:\n\n"
                result += "\n".join(alternatives)
            else:
                result += f"Did you mean '{content[0]***REMOVED***'?"
        else:
    ...
    def _format_definitions(self, query: str, definitions: List[Dict]) -> str:
        ...
        if len(formatted_definitions) == 1:
            return f"Definition of '{query***REMOVED***':\n" f"{formatted_definitions[0]***REMOVED***"

        result = f"Definitions of '{query***REMOVED***':\n\n"
        for i, formatted_definition in enumerate(formatted_definitions, 1):
            result += f"{i***REMOVED***. {formatted_definition***REMOVED***\n"
    ...
    def _format_definition(self, definition: Dict) -> Iterator[str]:
        ...
        if "shortdef" in definition:
            for short_def in definition["shortdef"]:
                yield f"{headword***REMOVED***, {functional_label***REMOVED***: {short_def***REMOVED***"
        else:
            yield f"{headword***REMOVED***, {functional_label***REMOVED***"
