...
def gpd_style_subclass_df():
    ...
    return SubclassedDataFrame({"a": [1, 2, 3]***REMOVED***)
...
class TestDataFrameSubclassing:
    def test_frame_subclassing_and_slicing(self):
        ...
        data = {"col1": range(10), "col2": range(10)***REMOVED***
    ...
    def test_dataframe_metadata(self):
        df = tm.SubclassedDataFrame(
            {"X": [1, 2, 3], "Y": [1, 2, 3]***REMOVED***, index=["a", "b", "c"]
        )
        df.testattr = "XXX"
    ...
    def test_indexing_sliced(self):
        # GH 11559
        df = tm.SubclassedDataFrame(
            {"X": [1, 2, 3], "Y": [4, 5, 6], "Z": [7, 8, 9]***REMOVED***, index=["a", "b", "c"]
        )
        res = df.loc[:, "X"]
    ...
    def test_subclass_align(self):
        ...
        df1 = tm.SubclassedDataFrame(
            {"a": [1, 3, 5], "b": [1, 3, 5]***REMOVED***, index=list("ACE")
        )
        df2 = tm.SubclassedDataFrame(
            {"c": [1, 2, 4], "d": [1, 2, 4]***REMOVED***, index=list("ABD")
        )

        res1, res2 = df1.align(df2, axis=0)
        exp1 = tm.SubclassedDataFrame(
            {"a": [1, np.nan, 3, np.nan, 5], "b": [1, np.nan, 3, np.nan, 5]***REMOVED***,
            index=list("ABCDE"),
        )
        exp2 = tm.SubclassedDataFrame(
            {"c": [1, 2, np.nan, 4, np.nan], "d": [1, 2, np.nan, 4, np.nan]***REMOVED***,
            index=list("ABCDE"),
        )
    ...
    def test_subclass_align_combinations(self):
        # GH 12983
        df = tm.SubclassedDataFrame({"a": [1, 3, 5], "b": [1, 3, 5]***REMOVED***, index=list("ACE"))
    ...
    def test_subclass_iterrows(self):
        # GH 13977
        df = tm.SubclassedDataFrame({"a": [1]***REMOVED***)
...
***REMOVED***,
    ...
    def test_subclassed_reductions(self, all_reductions):
        ...
        df = tm.SubclassedDataFrame({"A": [1, 2, 3], "B": [4, 5, 6], "C": [7, 8, 9]***REMOVED***)
    ...
    def test_subclassed_count(self):
        ...
        df = tm.SubclassedDataFrame({"A": [1, 0, 3], "B": [0, 5, 6], "C": [7, 8, 0]***REMOVED***)
        result = df.count()
        assert isinstance(result, tm.SubclassedSeries)
...
***REMOVED***,
    ...
    def test_isin(self):
        df = tm.SubclassedDataFrame(
            {"num_legs": [2, 4], "num_wings": [2, 0]***REMOVED***, index=["falcon", "dog"]
        )
        result = df.isin([0, 2])
        assert isinstance(result, tm.SubclassedDataFrame)
    ...
    def test_astype_preserves_subclass(self):
        ...
        result = df.astype({"A": np.int64, "B": np.int32, "C": np.float64***REMOVED***)
        assert isinstance(result, tm.SubclassedDataFrame)

    def test_equals_subclass(self):
        # https://github.com/pandas-dev/pandas/pull/34402
        # allow subclass in both directions
        df1 = DataFrame({"a": [1, 2, 3]***REMOVED***)
        df2 = tm.SubclassedDataFrame({"a": [1, 2, 3]***REMOVED***)
        assert df1.equals(df2)
        assert df2.equals(df1)

    def test_replace_list_method(self):
        # https://github.com/pandas-dev/pandas/pull/46018
        df = tm.SubclassedDataFrame({"A": [0, 1, 2]***REMOVED***)
        ...
        expected = tm.SubclassedDataFrame({"A": [0, 0, 0]***REMOVED***)
        assert isinstance(result, tm.SubclassedDataFrame)
        tm.assert_frame_equal(result, expected)
...
class TestSubclassWithoutConstructor:
    ...
    def test_groupby(self):
        df = SimpleDataFrameSubClass(DataFrame({"a": [1, 2, 3]***REMOVED***))
