...
def _timedelta_as_isostr(td: timedelta) -> str:
    ...
    if hours or minutes or seconds:
        ...
        if bigger_exists:
            time_str += "{:02***REMOVED***H".format(hours)

        # Minutes
        bigger_exists = bigger_exists or minutes
        if bigger_exists:
            time_str += "{:02***REMOVED***M".format(minutes)

        # Seconds
        try:
            if seconds.is_integer():
                seconds_string = "{:02***REMOVED***".format(int(seconds))
            else:
            ...
            seconds_string = "{:02***REMOVED***".format(seconds)

        time_str += "{***REMOVED***S".format(seconds_string)
    else:
        time_str = ""
    ...
    def default(self, o):  # pylint: disable=too-many-return-statements
        if _is_model(o):
            if self.exclude_readonly:
                ...
                return {k: v for k, v in o.items() if k not in readonly_props***REMOVED***
            return dict(o.items())
        try:
...
_VALID_DATE = re.compile(r"\d{4***REMOVED***[-]\d{2***REMOVED***[-]\d{2***REMOVED***T\d{2***REMOVED***:\d{2***REMOVED***:\d{2***REMOVED***" + r"\.?\d*Z?[-+]?[\d{2***REMOVED***]?:?[\d{2***REMOVED***]?")
_VALID_RFC7231 = re.compile(
    r"(Mon|Tue|Wed|Thu|Fri|Sat|Sun),\s\d{2***REMOVED***\s"
    r"(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s\d{4***REMOVED***\s\d{2***REMOVED***:\d{2***REMOVED***:\d{2***REMOVED***\sGMT"
)
...
def _get_model(module_name: str, model_name: str):
    models = {k: v for k, v in sys.modules[module_name].__dict__.items() if isinstance(v, type)***REMOVED***
    module_end = module_name.rsplit(".", 1)[0]
    models.update({k: v for k, v in sys.modules[module_end].__dict__.items() if isinstance(v, type)***REMOVED***)
...
def _serialize(o, format: typing.Optional[str] = None):  # pylint: disable=too-many-return-statements
    ...
    if isinstance(o, dict):
        return {k: _serialize(v, format) for k, v in o.items()***REMOVED***
    if isinstance(o, set):
        return {_serialize(x, format) for x in o***REMOVED***
...
class Model(_MyMutableMapping):
    ...
    def __init__(self, *args: typing.Any, **kwargs: typing.Any) -> None:
        ...
        if len(args) > 1:
            raise TypeError(f"{class_name***REMOVED***.__init__() takes 2 positional arguments but {len(args) + 1***REMOVED*** were given")
        dict_to_pass = {
        ...
        ***REMOVED***
        if args:
            dict_to_pass.update(
                {k: _create_value(_get_rest_field(self._attr_to_rest_field, k), v) for k, v in args[0].items()***REMOVED***
***REMOVED***
        else:
            non_attr_kwargs = [k for k in kwargs if k not in self._attr_to_rest_field]
            if non_attr_kwargs:
                # actual type errors only throw the first wrong keyword arg they see, so following that.
                raise TypeError(f"{class_name***REMOVED***.__init__() got an unexpected keyword argument '{non_attr_kwargs[0]***REMOVED***'")
            dict_to_pass.update(
                {
...
***REMOVED***:  # pylint: disable=protected-access
    ...
    def _as_dict_value(v: typing.Any, exclude_readonly: bool = False) -> typing.Any:
        ...
        if isinstance(v, dict):
            return {dk: Model._as_dict_value(dv, exclude_readonly=exclude_readonly) for dk, dv in v.items()***REMOVED***
...
def _get_deserialize_callable_from_annotation(  # pylint: disable=R0911, R0915, R0912
    ...
    try:
        if sys.version_info >= (3, 8):
            from typing import (
                Literal,
***REMOVED***  # pylint: disable=no-name-in-module, ungrouped-imports
        else:
