...
class FlatLayoutPackageFinder(PEP420PackageFinder):
    ...
    DEFAULT_EXCLUDE = tuple(chain_iter((p, f"{p***REMOVED***.*") for p in _EXCLUDE))
...
class ConfigDiscovery:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        log.debug(f"`explicit-layout` detected -- analysing {package_dir***REMOVED***")
        pkgs = chain_iter(
        ...
        log.debug(f"discovered packages -- {self.dist.packages***REMOVED***")
        return True
    ...
    def _analyse_src_layout(self) -> bool:
        ...
        log.debug(f"`src-layout` detected -- analysing {src_dir***REMOVED***")
        ...
        log.debug(f"discovered packages -- {self.dist.packages***REMOVED***")
        log.debug(f"discovered py_modules -- {self.dist.py_modules***REMOVED***")
        return True
    ...
    def _analyse_flat_layout(self) -> bool:
        ...
        """
        log.debug(f"`flat-layout` detected -- analysing {self._root_dir***REMOVED***")
    ...
    def _ensure_no_accidental_inclusion(self, detected: List[str], kind: str):
        if len(detected) > 1:
            ...
            msg = f"""Multiple top-level {kind***REMOVED*** discovered in a flat-layout: {detected***REMOVED***.

            To avoid accidental inclusion of unwanted files or directories,
            setuptools will not proceed with this build.

            If you are trying to create a single distribution with multiple {kind***REMOVED***
    ...
    def _find_name_single_package_or_module(self) -> Optional[str]:
        ...
        for field in ('packages', 'py_modules'):
            ...
            if items and len(items) == 1:
                log.debug(f"Single module/package detected, name: {items[0]***REMOVED***")
                return items[0]
    ...
    def _find_name_from_packages(self) -> Optional[str]:
        ...
        if parent_pkg:
            log.debug(f"Common parent package detected, name: {parent_pkg***REMOVED***")
            return parent_pkg
...
def remove_nested_packages(packages: List[str]) -> List[str]:
    ...
    for i, name in enumerate(reversed(pkgs)):
        if any(name.startswith(f"{other***REMOVED***.") for other in top_level):
...
def find_parent_package(
    ...
    for i, name in enumerate(packages):
        if not all(n.startswith(f"{name***REMOVED***.") for n in packages[i+1:]):
...
def find_package_path(
    ...
    >>> path = find_package_path("my.pkg", {"": "root/is/nested"***REMOVED***, ".")
    >>> path.replace(os.sep, "/")
    './root/is/nested/my/pkg'

    >>> path = find_package_path("my.pkg", {"my": "root/is/nested"***REMOVED***, ".")
    >>> path.replace(os.sep, "/")
    './root/is/nested/pkg'

    >>> path = find_package_path("my.pkg", {"my.pkg": "root/is/nested"***REMOVED***, ".")
    >>> path.replace(os.sep, "/")
    './root/is/nested'

    >>> path = find_package_path("other.pkg", {"my.pkg": "root/is/nested"***REMOVED***, ".")
    >>> path.replace(os.sep, "/")
    './other/pkg'
    """
    parts = name.split(".")
...
def construct_package_dir(packages: List[str], package_path: _Path) -> Dict[str, str]:
    ...
    return {pkg: "/".join([*prefix, *pkg.split(".")]) for pkg in parent_pkgs***REMOVED***
