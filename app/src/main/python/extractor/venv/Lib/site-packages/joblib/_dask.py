...
def _make_tasks_summary(tasks):
    ...
    unique_funcs = {func for func, args, kwargs in tasks***REMOVED***
...
class Batch:
    ...
    def __repr__(self):
        descr = f"batch_of_{self._funcname***REMOVED***_{self._num_tasks***REMOVED***_calls"
        if self._mixed:
            descr = "mixed_" + descr
        return descr
...
class DaskDistributedBackend(AutoBatchingMixin, ParallelBackendBase):
    ...
    def __init__(self, scheduler_host=None, scatter=None,
        ...
        if scatter is not None and len(scatter) > 0:
            ...
            self.data_futures = {id(x): f for x, f in zip(scatter, scattered)***REMOVED***
        else:
            self._scatter = []
            self.data_futures = {***REMOVED***
...
***REMOVED***
            ...
            error_msg = (
                "DaskDistributedBackend has no worker after {***REMOVED*** seconds. "
                "Make sure that workers are started and can properly connect "
                "to the scheduler and increase the joblib/dask connection "
                "timeout with:\n\n"
                "parallel_config(backend='dask', wait_for_workers_timeout={***REMOVED***)"
***REMOVED***.format(self.wait_for_workers_timeout,
    ...
    def apply_async(self, func, callback=None):
        ...
        async def f(func, callback):
            ...
            key = f'{repr(batch)***REMOVED***-{uuid4().hex***REMOVED***'
