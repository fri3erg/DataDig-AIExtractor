...
***REMOVED***
    ...
    def _text_analysis(self, text: str) -> Dict:
        poller = self.text_analytics_client.begin_analyze_healthcare_entities(
            [{"id": "1", "language": "en", "text": text***REMOVED***]
        )
        ...
        if docs is not None:
            res_dict["entities"] = [
                f"{x.text***REMOVED*** is a healthcare entity of type {x.category***REMOVED***"
                for y in docs
                for x in y.entities
            ]
...
***REMOVED***
    ...
    def _run(
        ...
        try:
            ...
            raise RuntimeError(
                f"Error while running AzureCogsTextAnalyticsHealthTool: {e***REMOVED***"
***REMOVED***
