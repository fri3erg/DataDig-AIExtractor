...
class TestSetOps:
    ...
    def test_ediff1d_forbidden_type_casts(self, ary, prepend, append, expected):
        ...
        msg = 'dtype of `{***REMOVED***` must be compatible'.format(expected)
    ...
    def test_isin(self, kind):
        ...
        def _isin_slow(a, b):
            ...
            return a in b
        isin_slow = np.vectorize(_isin_slow, otypes=[bool], excluded={1***REMOVED***)
        ...
        for dtype in [bool, np.int64, np.float64]:
            ...
            if dtype in {np.int64, np.float64***REMOVED***:
                ar = np.array([10, 20, 30], dtype=dtype)
            elif dtype in {bool***REMOVED***:
