...
def dedensify(G, threshold, prefix=None, copy=True):
    ...
    high_degree_nodes = {n for n, d in degrees if d > threshold***REMOVED***
    ...
    for node in G:
        ...
        if high_degree_neighbors:
            if high_degree_neighbors in auxiliary:
                ...
                auxiliary[high_degree_neighbors] = {node***REMOVED***
...
def _snap_build_graph(
    ...
    """
    output = G.__class__()
    node_label_lookup = {***REMOVED***
    for index, group_id in enumerate(groups):
        group_set = groups[group_id]
        supernode = f"{prefix***REMOVED***{index***REMOVED***"
        ...
        for other_group, group_edge_types in neighbor_info[
            ...
            if group_edge_types:
                ...
                if output.is_multigraph():
                    ...
                    superedge_attributes = {superedge_attribute: edge_types***REMOVED***
...
def _snap_eligible_group(G, groups, group_lookup, edge_types):
    ...
    """
    neighbor_info = {node: {gid: Counter() for gid in groups***REMOVED*** for node in group_lookup***REMOVED***
    for group_id in groups:
        current_group = groups[group_id]

        # build neighbor_info for nodes in group
        for node in current_group:
            neighbor_info[node] = {group_id: Counter() for group_id in groups***REMOVED***
...
def snap_aggregation(
    ...
    have edges with nodes in groups {A, C***REMOVED***, but some also have edges with other
    nodes in B, then group B is not homogeneous and needs to be split into
    groups have edges with {A, C***REMOVED*** and a group of nodes having
    edges with {A, B, C***REMOVED***.  This way, viewers of the summary graph can
