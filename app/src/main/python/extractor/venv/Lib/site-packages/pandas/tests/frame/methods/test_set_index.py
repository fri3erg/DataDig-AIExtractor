...
class TestSetIndex:
    ...
    def test_set_index_empty_column(self):
        ...
        df = DataFrame(
            [
                {"a": 1, "p": 0***REMOVED***,
                {"a": 2, "m": 10***REMOVED***,
                {"a": 3, "m": 11, "p": 20***REMOVED***,
                {"a": 4, "m": 12, "p": 21***REMOVED***,
            ],
            columns=["a", "m", "p", "x"],
        )
    ...
    def test_set_index_empty_dataframe(self):
        # GH#38419
        df1 = DataFrame(
            {"a": Series(dtype="datetime64[ns]"), "b": Series(dtype="int64"), "c": []***REMOVED***
        )
    ...
    def test_set_index_dst(self):
        ...
        df = DataFrame(data={"a": [0, 1, 2], "b": [3, 4, 5]***REMOVED***, index=di).reset_index()
        # single level
        res = df.set_index("index")
        exp = DataFrame(
            data={"a": [0, 1, 2], "b": [3, 4, 5]***REMOVED***,
        ...
        exp = DataFrame({"b": [3, 4, 5]***REMOVED***, index=exp_index)
        tm.assert_frame_equal(res, exp)
    ...
    def test_set_index_names(self):
        df = DataFrame(
            ...
            index=Index([f"i-{i***REMOVED***" for i in range(10)], dtype=object),
        )
        df.index.name = "name"
    ...
    def test_set_index_after_mutation(self):
        # GH#1590
        df = DataFrame({"val": [0, 1, 2], "key": ["a", "b", "c"]***REMOVED***)
        expected = DataFrame({"val": [1, 2]***REMOVED***, Index(["b", "c"], name="key"))
...
***REMOVED***
    ...
    def test_construction_with_categorical_index(self):
        ...
        # with Categorical
        df = DataFrame(
            {"A": np.random.default_rng(2).standard_normal(10), "B": ci.values***REMOVED***
        )
        idf = df.set_index("B")
        tm.assert_index_equal(idf.index, ci)

        # from a CategoricalIndex
        df = DataFrame({"A": np.random.default_rng(2).standard_normal(10), "B": ci***REMOVED***)
        idf = df.set_index("B")
        tm.assert_index_equal(idf.index, ci)
    ...
    def test_set_index_custom_label_type(self):
        ...
        class Thing:
            ...
            def __str__(self) -> str:
                return f"<Thing {repr(self.name)***REMOVED***>"
        ...
        df = DataFrame({thing1: [0, 1], thing2: [2, 3]***REMOVED***)
        expected = DataFrame({thing1: [0, 1]***REMOVED***, index=Index([2, 3], name=thing2))
    ...
    def test_set_index_custom_label_hashable_iterable(self):
        ...
        class Thing(frozenset):
            ...
            def __repr__(self) -> str:
                ...
                return f"frozenset({{{joined_reprs***REMOVED******REMOVED******REMOVED***)"
        ...
        # missing key
        thing3 = Thing(["Three", "pink"])
        msg = r"frozenset\(\{'Three', 'pink'\***REMOVED***\)"
        with pytest.raises(KeyError, match=msg):
            # missing label directly
            df.set_index(thing3)
