...
def _get_news_api(llm: BaseLanguageModel, **kwargs: Any) -> BaseTool:
    ...
    chain = APIChain.from_llm_and_api_docs(
        llm,
        news_docs.NEWS_DOCS,
        headers={"X-Api-Key": news_api_key***REMOVED***,
...
def _get_tmdb_api(llm: BaseLanguageModel, **kwargs: Any) -> BaseTool:
    ...
    chain = APIChain.from_llm_and_api_docs(
        llm,
        tmdb_docs.TMDB_DOCS,
        headers={"Authorization": f"Bearer {tmdb_bearer_token***REMOVED***"***REMOVED***,
...
def _get_podcast_api(llm: BaseLanguageModel, **kwargs: Any) -> BaseTool:
    ...
    chain = APIChain.from_llm_and_api_docs(
        ...
        headers={"X-ListenAPI-Key": listen_api_key***REMOVED***,
...
def _get_searx_search_results_json(**kwargs: Any) -> BaseTool:
    wrapper_kwargs = {k: v for k, v in kwargs.items() if k != "num_results"***REMOVED***
...
***REMOVED***
        if name == "requests_all":
            ...
            if llm is None:
                raise ValueError(f"Tool {name***REMOVED*** requires an LLM to be provided")
            ...
            if missing_keys:
                raise ValueError(
                    f"Tool {name***REMOVED*** requires some parameters that were not "
                    f"provided: {missing_keys***REMOVED***"
    ***REMOVED***
            sub_kwargs = {k: kwargs[k] for k in extra_keys***REMOVED***
            tool = _get_llm_tool_func(llm=llm, **sub_kwargs)
            tools.append(tool)
        elif name in _EXTRA_OPTIONAL_TOOLS:
            _get_tool_func, extra_keys = _EXTRA_OPTIONAL_TOOLS[name]
            sub_kwargs = {k: kwargs[k] for k in extra_keys if k in kwargs***REMOVED***
            tool = _get_tool_func(**sub_kwargs)
            tools.append(tool)
        else:
            raise ValueError(f"Got unknown tool {name***REMOVED***")
    if callbacks is not None:
        for tool in tools:
