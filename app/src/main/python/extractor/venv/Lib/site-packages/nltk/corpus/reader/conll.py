...
class ConllCorpusReader(CorpusReader):
    ...
    def __init__(
        ...
        self._colmap = {c: i for (i, c) in enumerate(columntypes)***REMOVED***
...
***REMOVED***
    ...
    def _get_parsed_sent(self, grid, pos_in_tree, tagset=None):
        ...
        for (word, pos_tag, parse_tag) in zip(words, pos_tags, parse_tags):
            ...
            treestr += f"{left***REMOVED*** ({pos_tag***REMOVED*** {word***REMOVED***) {right***REMOVED***"
        try:
            tree = self._tree_class.fromstring(treestr)
        except (ValueError, IndexError):
            tree = self._tree_class.fromstring(f"({self._root_label***REMOVED*** {treestr***REMOVED***)")
...
class ConllSRLInstance:
    ...
    def pprint(self):
        ...
        hdr = f"SRL for {verbstr!r***REMOVED*** (stem={self.verb_stem!r***REMOVED***):\n"
        s = ""
...
class ConllSRLInstanceList(list):
        ...
        for i in range(len(words)):
            ...
            for inst in self:
                ...
                for (start, end), argid in inst.tagged_spans:
                    if i == start:
                        argstr = f"({argid***REMOVED***{argstr***REMOVED***"
                    if i == (end - 1):
                        argstr += ")"
                s += "%-12s " % argstr
            s += "\n"
    ...
    def _tree2conll(self, tree, wordnum, words, pos, synt):
        ...
        if len(tree) == 1 and isinstance(tree[0], str):
            ...
            synt[wordnum] = f"({tree.label()***REMOVED***{synt[wordnum]***REMOVED***"
            for child in tree:
