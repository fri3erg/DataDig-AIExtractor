...
def test_weight_key():
    ...
    impossible = {(3, 6), (3, 9)***REMOVED***
    rng = random.Random(0)
    avail_uv = list(set(complement_edges(G)) - impossible)
    avail = [(u, v, {"cost": rng.random()***REMOVED***) for u, v in avail_uv]
...
def test_tarjan():
    ...
    print(f"aug_edges = {aug_edges!r***REMOVED***")
        ...
        try:
            ...
            if avail is None:
                ...
                assert n_nodes <= k, (
                    ...
                    f"Got |V|={n_nodes***REMOVED*** and k={k***REMOVED***"
    ***REMOVED***
            else:
...
***REMOVED***
            ...
            if avail_dict is None:
                assert set(partial_edges) == set(
                    complement_edges(G)
    ***REMOVED***, "unweighted partial solutions should be the complement"
    ...
    ***REMOVED***, "adding more edges should not increase k-conn"
        ...
        if not infeasible and orig_k < k:
            assert info["aug_k"] >= k, f"connectivity should increase to k={k***REMOVED*** or more"
        ...
        info["failed"] = True
        print(f"edges = {list(G.edges())***REMOVED***")
        print(f"nodes = {list(G.nodes())***REMOVED***")
        print(f"aug_edges = {list(aug_edges)***REMOVED***")
        print(f"info  = {info***REMOVED***")
        raise
    else:
        if verbose:
            print(f"info  = {info***REMOVED***")
...
def _check_augmentations(G, avail=None, max_k=None, weight=None, verbose=False):
    ...
    avail_uniform = {e: 1 for e in complement_edges(G)***REMOVED***

    if verbose:
        print("\n=== CHECK_AUGMENTATION ===")
        print(f"G.number_of_nodes = {G.number_of_nodes()!r***REMOVED***")
        print(f"G.number_of_edges = {G.number_of_edges()!r***REMOVED***")
        print(f"max_k = {max_k!r***REMOVED***")
        print(f"max_aug_k = {max_aug_k!r***REMOVED***")
        print(f"orig_k = {orig_k!r***REMOVED***")

    # check augmentation for multiple values of k
    for k in range(1, max_k + 1):
        if verbose:
            print("---------------")
            print(f"Checking k = {k***REMOVED***")
