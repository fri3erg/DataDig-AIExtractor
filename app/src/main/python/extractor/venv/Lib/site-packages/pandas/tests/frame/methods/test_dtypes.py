...
class TestDataFrameDataTypes:
    def test_empty_frame_dtypes(self):
        ...
        df = DataFrame({"a": 1, "b": True, "c": 1.0***REMOVED***, index=[1, 2, 3])
        ex_dtypes = Series({"a": np.int64, "b": np.bool_, "c": np.float64***REMOVED***)
    ...
    def test_dtypes_are_correct_after_column_slice(self):
        ...
        tm.assert_series_equal(
            df.dtypes,
            Series({"a": np.float64, "b": np.float64, "c": np.float64***REMOVED***),
        )
        tm.assert_series_equal(df.iloc[:, 2:].dtypes, Series({"c": np.float64***REMOVED***))
        tm.assert_series_equal(
            df.dtypes,
            Series({"a": np.float64, "b": np.float64, "c": np.float64***REMOVED***),
        )
    ...
    def test_dtypes_are_correct_after_groupby_last(self, data):
        # GH46409
        df = DataFrame(
            {"id": [1, 2, 3, 4], "test": [True, pd.NA, data, False]***REMOVED***
        ).convert_dtypes()
    ...
    def test_dtypes_gh8722(self, float_string_frame):
        ...
        expected = Series(
            {k: v.dtype for k, v in float_string_frame.items()***REMOVED***, index=result.index
        )
        tm.assert_series_equal(result, expected)
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            with option_context("use_inf_as_na", True):
                df = DataFrame([[1]])
                result = df.dtypes
                tm.assert_series_equal(result, Series({0: np.dtype("int64")***REMOVED***))
