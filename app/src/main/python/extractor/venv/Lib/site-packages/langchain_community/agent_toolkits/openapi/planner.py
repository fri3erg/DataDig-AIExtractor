...
def _create_api_planner_tool(
    ...
    endpoint_descriptions = [
        f"{name***REMOVED*** {description***REMOVED***" for name, description, _ in api_spec.endpoints
    ]
    prompt = PromptTemplate(
        template=API_PLANNER_PROMPT,
        input_variables=["query"],
        partial_variables={"endpoints": "- " + "- ".join(endpoint_descriptions)***REMOVED***,
    )
...
def _create_api_controller_agent(
    ...
    prompt = PromptTemplate(
        ...
        partial_variables={
            ...
            "tool_descriptions": "\n".join(
                [f"{tool.name***REMOVED***: {tool.description***REMOVED***" for tool in tools]
***REMOVED***,
        ***REMOVED***,
    )
    agent = ZeroShotAgent(
...
def _create_api_controller_tool(
    ...
    def _create_and_run_api_controller_agent(plan_str: str) -> str:
        ...
        endpoint_names = [
            "{method***REMOVED*** {route***REMOVED***".format(method=method, route=route.split("?")[0])
        ...
        for endpoint_name in endpoint_names:
            ...
            for name, _, docs in api_spec.endpoints:
                regex_name = re.compile(re.sub("\{.*?\***REMOVED***", ".*", name))
                if regex_name.match(endpoint_name):
                    found_match = True
                    docs_str += f"== Docs for {endpoint_name***REMOVED*** == \n{yaml.dump(docs)***REMOVED***\n"
            if not found_match:
                raise ValueError(f"{endpoint_name***REMOVED*** endpoint does not exist.")
...
***REMOVED***,
    ...
    return AgentExecutor.from_agent_and_tools(
        ...
        verbose=verbose,
        **(agent_executor_kwargs or {***REMOVED***),
    )
