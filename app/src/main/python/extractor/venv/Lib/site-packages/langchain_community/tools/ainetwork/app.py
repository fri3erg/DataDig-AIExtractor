        ...
        try:
            if type is AppOperationType.SET_ADMIN:
                ...
                res = await self.interface.db.ref(
                    f"/manage_app/{appName***REMOVED***/create/{getTimestamp()***REMOVED***"
    ***REMOVED***.setValue(
                    transactionInput=ValueOnlyTransactionInput(
                        value={"admin": {address: True for address in address***REMOVED******REMOVED***
        ***REMOVED***
    ***REMOVED***
            elif type is AppOperationType.GET_CONFIG:
                res = await self.interface.db.ref(
                    f"/manage_app/{appName***REMOVED***/config"
    ***REMOVED***.getValue()
            else:
                raise ValueError(f"Unsupported 'type': {type***REMOVED***.")
            return json.dumps(res, ensure_ascii=False)
        except Exception as e:
            return f"{builtins.type(e).__name__***REMOVED***: {str(e)***REMOVED***"
