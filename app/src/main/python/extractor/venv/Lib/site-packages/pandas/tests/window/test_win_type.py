...
def test_agg_function_support(arg):
    pytest.importorskip("scipy")
    df = DataFrame({"A": np.arange(5)***REMOVED***)
    roll = df.rolling(2, win_type="triang")

    msg = f"'{arg***REMOVED***' is not a valid function for 'Window' object"
...
def test_win_type_not_implemented():
    ...
    df = DataFrame({"values": range(2)***REMOVED***)
...
def test_cmov_window_special(win_types_special, step):
    ...
    pytest.importorskip("scipy")
    kwds = {
        "kaiser": {"beta": 1.0***REMOVED***,
        "gaussian": {"std": 1.0***REMOVED***,
        "general_gaussian": {"p": 2.0, "sig": 2.0***REMOVED***,
        "exponential": {"tau": 10***REMOVED***,
    ***REMOVED***
...
def test_rolling_center_axis_1():
    pytest.importorskip("scipy")
    df = DataFrame(
        {"a": [1, 1, 0, 0, 0, 1], "b": [1, 0, 0, 1, 0, 0], "c": [1, 0, 0, 1, 0, 1]***REMOVED***
    )
    ...
    expected = DataFrame(
        {"a": [np.nan] * 6, "b": [3.0, 1.0, 0.0, 2.0, 0.0, 2.0], "c": [np.nan] * 6***REMOVED***
    )
