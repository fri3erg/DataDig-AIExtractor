    ...
    def __init__(
        ...
        *,
        ...
        if self.on is None:
            ...
            raise ValueError(
                f"invalid on specified as {self.on***REMOVED***, "
    ...
    def _validate(self) -> None:
        ...
        if self.min_periods is not None:
            ...
            if is_integer(self.window) and self.min_periods > self.window:
                raise ValueError(
                    f"min_periods {self.min_periods***REMOVED*** must be <= window {self.window***REMOVED***"
    ***REMOVED***
        ...
        if not isinstance(self.obj, (ABCSeries, ABCDataFrame)):
            raise TypeError(f"invalid type: {type(self)***REMOVED***")
...
***REMOVED***.parameters.keys()
            expected_signature = inspect.signature(
                BaseIndexer().get_window_bounds
***REMOVED***.parameters.keys()
            if get_window_bounds_signature != expected_signature:
                raise ValueError(
                    f"{type(self.window).__name__***REMOVED*** does not implement "
    ...
    ***REMOVED***
    ...
    def _check_window_bounds(
        ...
        if len(start) != len(end):
            raise ValueError(
                f"start ({len(start)***REMOVED***) and end ({len(end)***REMOVED***) bounds must be the "
                f"same length"
***REMOVED***
        if len(start) != (num_vals + (self.step or 1) - 1) // (self.step or 1):
            raise ValueError(
                f"start and end bounds ({len(start)***REMOVED***) must be the same length "
                f"as the object ({num_vals***REMOVED***) divided by the step ({self.step***REMOVED***) "
    ...
    def _validate_numeric_only(self, name: str, numeric_only: bool) -> None:
        ...
        if (
            ...
            raise NotImplementedError(
                f"{type(self).__name__***REMOVED***.{name***REMOVED*** does not implement numeric_only"
***REMOVED***
    ...
    def _gotitem(self, key, ndim, subset=None):
        ...
        kwargs = {attr: getattr(self, attr) for attr in self._attributes***REMOVED***
    ...
    def __repr__(self) -> str:
        ...
        """
        attrs_list = (
            f"{attr_name***REMOVED***={getattr(self, attr_name)***REMOVED***"
            for attr_name in self._attributes
            if getattr(self, attr_name, None) is not None and attr_name[0] != "_"
        )
        attrs = ",".join(attrs_list)
        return f"{type(self).__name__***REMOVED*** [{attrs***REMOVED***]"
    ...
    def _prep_values(self, values: ArrayLike) -> np.ndarray:
        ...
        if needs_i8_conversion(values.dtype):
            raise NotImplementedError(
                f"ops for {type(self).__name__***REMOVED*** for this "
                f"dtype {values.dtype***REMOVED*** are not implemented"
***REMOVED***
        ...
        try:
            ...
            raise TypeError(f"cannot handle this type -> {values.dtype***REMOVED***") from err
    ...
    def _apply_columnwise(
        ...
        for i, arr in enumerate(obj._iter_column_arrays()):
            ...
            try:
                ...
                raise DataError(
                    f"Cannot aggregate non-numeric type: {arr.dtype***REMOVED***"
    ***REMOVED*** from err
...
class Window(BaseWindow):
    ...
    Examples
    --------
    >>> df = pd.DataFrame({'B': [0, 1, 2, np.nan, 4]***REMOVED***)
    >>> df
         B
    0  0.0
    1  1.0
    ...
    **window**
    ...
    >>> df_time = pd.DataFrame({'B': [0, 1, 2, np.nan, 4]***REMOVED***,
    ...
    **on**
    ...
    def _validate(self):
        ...
        if not isinstance(self.win_type, str):
            raise ValueError(f"Invalid win_type {self.win_type***REMOVED***")
...
***REMOVED***
    ...
    ***REMOVED***
    ...
    @doc(
        ...
        Examples
        --------
        >>> df = pd.DataFrame({"A": [1, 2, 3], "B": [4, 5, 6], "C": [7, 8, 9]***REMOVED***)
        >>> df
           A  B  C
        0  1  4  7
        1  2  5  8
...
class Rolling(RollingAndExpandingMixin):
    ...
    def _validate(self):
        ...
        if (
            ...
            try:
                ...
                raise ValueError(
                    f"passed window {self.window***REMOVED*** is not "
    ...
    ***REMOVED***
    ...
    def _raise_monotonic_error(self, msg: str):
        ...
        if on is None:
            if self.axis == 0:
                ...
                on = "column"
        raise ValueError(f"{on***REMOVED*** {msg***REMOVED***")
    ...
    @doc(
        ...
        >>> df.rolling(2).agg({"A": "sum", "B": "min"***REMOVED***)
             A    B
        0  NaN  NaN
        1  3.0  4.0
        2  5.0  5.0
    ...
    @doc(
        ...
        >>> df = pd.DataFrame({{"A": s, "B": s ** 2***REMOVED******REMOVED***)
        >>> df
           A   B
        0  1   1
        1  2   4
    ...
    @doc(
        ...
        >>> arr = [1, 2, 3, 4, 999]
        >>> import scipy.stats
        >>> print(f"{{scipy.stats.kurtosis(arr[:-1], bias=False):.6f***REMOVED******REMOVED***")
        -1.200000
        >>> print(f"{{scipy.stats.kurtosis(arr[1:], bias=False):.6f***REMOVED******REMOVED***")
        3.999946
        >>> s = pd.Series(arr)
    ...
    @doc(
        ...
        interpolation : {{'linear', 'lower', 'higher', 'midpoint', 'nearest'***REMOVED******REMOVED***
    ...
    @doc(
        ...
        dedent(
            """
        method : {{'average', 'min', 'max'***REMOVED******REMOVED***, default 'average'
...
***REMOVED***:
                ...
                raise ValueError(
                    f"Each group within {on***REMOVED*** must be monotonic. "
                    f"Sort the values in {on***REMOVED*** first."
    ***REMOVED***
