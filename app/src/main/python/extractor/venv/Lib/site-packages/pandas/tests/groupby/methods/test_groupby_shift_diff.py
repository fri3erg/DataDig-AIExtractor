...
def test_group_shift_lose_timezone():
    ...
    df = DataFrame({"a": [1, 1], "date": now_dt***REMOVED***)
    result = df.groupby("a").shift(0).iloc[0]
    expected = Series({"date": now_dt***REMOVED***, name=result.name)
    tm.assert_series_equal(result, expected)


def test_group_diff_real_series(any_real_numpy_dtype):
    df = DataFrame(
        {"a": [1, 2, 3, 3, 2], "b": [1, 2, 3, 4, 5]***REMOVED***,
...
def test_group_diff_datetimelike(data, unit):
    df = DataFrame({"a": [1, 2, 2], "b": data***REMOVED***)
...
def test_group_diff_bool():
    df = DataFrame({"a": [1, 2, 3, 3, 2], "b": [True, True, False, False, True]***REMOVED***)
    result = df.groupby("a")["b"].diff()
...
def test_group_diff_object_raises(object_dtype):
    df = DataFrame(
        {"a": ["foo", "bar", "bar"], "b": ["baz", "foo", "foo"]***REMOVED***, dtype=object_dtype
    )
...
def test_shift_periods_freq():
    # GH 54093
    data = {"a": [1, 2, 3, 4, 5, 6], "b": [0, 0, 0, 1, 1, 1]***REMOVED***
...
def test_group_shift_with_multiple_periods():
    ...
    expected_df = DataFrame(
        {"a_0": [1, 2, 3, 3, 2], "a_1": [np.nan, 1.0, np.nan, 3.0, 2.0]***REMOVED***
    )
...
def test_group_shift_with_multiple_periods_and_freq():
    # GH#44424
    df = DataFrame(
        {"a": [1, 2, 3, 4, 5], "b": [True, True, False, False, True]***REMOVED***,
...
def test_group_shift_with_multiple_periods_and_fill_value():
    ...
    expected_df = DataFrame(
        {"a_0": [1, 2, 3, 4, 5], "a_1": [-1, 1, -1, 3, 2]***REMOVED***,
    )
    tm.assert_frame_equal(shifted_df, expected_df)
