...
class CometCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def _init_resp(self) -> Dict:
        return {k: None for k in self.callback_columns***REMOVED***
    ...
    def on_llm_start(
        ...
        metadata.update({"action": "on_llm_start"***REMOVED***)
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        metadata = self._init_resp()
        metadata.update({"action": "on_llm_end"***REMOVED***)
        metadata.update(flatten_dict(response.llm_output or {***REMOVED***))
        metadata.update(self.get_custom_callback_meta())
    ...
    def on_chain_start(
        ...
        for chain_input_key, chain_input_val in inputs.items():
            if isinstance(chain_input_val, str):
                ...
                input_resp.update({chain_input_key: chain_input_val***REMOVED***)
                self.action_records.append(input_resp)

            else:
                self.comet_ml.LOGGER.warning(
                    f"Unexpected data format provided! "
                    f"Input Value for {chain_input_key***REMOVED*** will not be logged"
    ***REMOVED***
    ...
    def on_chain_end(self, outputs: Dict[str, Any], **kwargs: Any) -> None:
        ...
        resp = self._init_resp()
        resp.update({"action": "on_chain_end"***REMOVED***)
        resp.update(self.get_custom_callback_meta())
        ...
        for chain_output_key, chain_output_val in outputs.items():
            if isinstance(chain_output_val, str):
                ...
                output_resp.update({chain_output_key: chain_output_val***REMOVED***)
                self.action_records.append(output_resp)
            else:
                self.comet_ml.LOGGER.warning(
                    f"Unexpected data format provided! "
                    f"Output Value for {chain_output_key***REMOVED*** will not be logged"
    ***REMOVED***
    ...
    def on_tool_start(
        ...
        resp = self._init_resp()
        resp.update({"action": "on_tool_start"***REMOVED***)
        resp.update(flatten_dict(serialized))
        resp.update(self.get_custom_callback_meta())
        if self.stream_logs:
            self._log_stream(input_str, resp, self.step)

        resp.update({"input_str": input_str***REMOVED***)
        self.action_records.append(resp)
    ...
    def on_tool_end(self, output: str, **kwargs: Any) -> None:
        ...
        resp = self._init_resp()
        resp.update({"action": "on_tool_end"***REMOVED***)
        resp.update(self.get_custom_callback_meta())
        if self.stream_logs:
            self._log_stream(output, resp, self.step)

        resp.update({"output": output***REMOVED***)
        self.action_records.append(resp)
    ...
    def on_text(self, text: str, **kwargs: Any) -> None:
        ...
        resp = self._init_resp()
        resp.update({"action": "on_text"***REMOVED***)
        resp.update(self.get_custom_callback_meta())
        if self.stream_logs:
            self._log_stream(text, resp, self.step)

        resp.update({"text": text***REMOVED***)
        self.action_records.append(resp)
    ...
    def on_agent_finish(self, finish: AgentFinish, **kwargs: Any) -> None:
        ...
        resp.update({"action": "on_agent_finish", "log": log***REMOVED***)
        resp.update(self.get_custom_callback_meta())
        if self.stream_logs:
            self._log_stream(output, resp, self.step)
    ...
    def on_agent_action(self, action: AgentAction, **kwargs: Any) -> Any:
        ...
        resp = self._init_resp()
        resp.update({"action": "on_agent_action", "log": log, "tool": tool***REMOVED***)
        resp.update(self.get_custom_callback_meta())
        if self.stream_logs:
            self._log_stream(tool_input, resp, self.step)

        resp.update({"tool_input": tool_input***REMOVED***)
        self.action_records.append(resp)
        ...
        if langchain_asset:
            try:
                ...
                self.comet_ml.LOGGER.error(
                    ...
                    exc_info=True,
                    extra={"show_traceback": True***REMOVED***,
    ***REMOVED***
        ...
        try:
            ...
            if hasattr(langchain_asset, "save_agent"):
                ...
                self.comet_ml.LOGGER.error(
                    ...
                    f"for {langchain_asset.__class__.__name__***REMOVED***"
    ***REMOVED***
...
***REMOVED***
        ...
        try:
            metadata = {"langchain_version": str(langchain_community.__version__)***REMOVED***
            ...
            for visualization in self.visualizations:
                try:
                    html = spacy.displacy.render(
                        sentence_spans,
                        style=visualization,
                        options={"compact": True***REMOVED***,
                        jupyter=False,
                        page=True,
        ***REMOVED***
                    self.experiment.log_asset_data(
                        html,
                        name=f"langchain-viz-{visualization***REMOVED***-{idx***REMOVED***.html",
                        metadata={"prompt": prompt***REMOVED***,
                        step=idx,
        ***REMOVED***
                except Exception as e:
                    self.comet_ml.LOGGER.warning(
                        e, exc_info=True, extra={"show_traceback": True***REMOVED***
        ***REMOVED***
