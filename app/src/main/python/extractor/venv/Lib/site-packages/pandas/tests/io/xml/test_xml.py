...
df_kml = DataFrame(
    {
        ...
        ***REMOVED***,
        "styleUrl": {
            ...
            4: "#LineStyle01",
        ***REMOVED***,
        "extrude": {0: 0, 1: 0, 2: 0, 3: 0, 4: 0***REMOVED***,
        "altitudeMode": {
            0: "clampedToGround",
            1: "clampedToGround",
            2: "clampedToGround",
...
def test_parser_consistency_file(xml_books):
    ...
    df_iter_lxml = read_xml(
        xml_books,
        parser="lxml",
        iterparse={"book": ["category", "title", "year", "author", "price"]***REMOVED***,
    )
...
def test_parser_consistency_url(parser, httpserver):
    ...
    df_iter = read_xml(
        ...
        parser=parser,
        iterparse={"row": ["shape", "degrees", "sides"]***REMOVED***,
    )
...
def test_string_charset(parser):
    ...
    df_expected = DataFrame({"c1": 1, "c2": 2***REMOVED***, index=[0])
...
def test_whitespace(parser):
    ...
    df_iter = read_xml_iterparse(
        xml,
        parser=parser,
        iterparse={"row": ["sides", "shape", "degrees"]***REMOVED***,
        dtype="string",
    )
...
def test_default_namespace(parser):
    df_nmsp = read_xml(
        ...
        xpath=".//ns:row",
        namespaces={"ns": "http://example.com"***REMOVED***,
        parser=parser,
    )
...
def test_prefix_namespace(parser):
    df_nmsp = read_xml(
        ...
        xpath=".//doc:row",
        namespaces={"doc": "http://example.com"***REMOVED***,
        parser=parser,
    )
    df_iter = read_xml_iterparse(
        xml_prefix_nmsp, parser=parser, iterparse={"row": ["shape", "degrees", "sides"]***REMOVED***
    )
...
def test_none_namespace_prefix(key):
    ...
    with pytest.raises(
        ...
        read_xml(
            StringIO(xml_default_nmsp),
            xpath=".//kml:Placemark",
            namespaces={key: "http://www.opengis.net/kml/2.2"***REMOVED***,
            parser="lxml",
        )
...
def test_file_elems_and_attrs(xml_books, parser):
    ...
    df_iter = read_xml(
        xml_books,
        parser=parser,
        iterparse={"book": ["category", "title", "author", "year", "price"]***REMOVED***,
    )
    df_expected = DataFrame(
        {
            "category": ["cooking", "children", "web"],
...
def test_file_only_attrs(xml_books, parser):
    ...
    df_iter = read_xml(xml_books, parser=parser, iterparse={"book": ["category"]***REMOVED***)
    df_expected = DataFrame({"category": ["cooking", "children", "web"]***REMOVED***)
...
def test_file_only_elems(xml_books, parser):
    ...
    df_iter = read_xml(
        xml_books,
        parser=parser,
        iterparse={"book": ["title", "author", "year", "price"]***REMOVED***,
    )
    df_expected = DataFrame(
        {
            "title": ["Everyday Italian", "Harry Potter", "Learning XML"],
...
<TrainSchedule>
    ...
    df_iter_lx = read_xml_iterparse(xml, iterparse={"station": ["Name", "coords"]***REMOVED***)
    df_iter_et = read_xml_iterparse(
        xml, parser="etree", iterparse={"station": ["Name", "coords"]***REMOVED***
    )
...
<shapes>
  ...
  <shape type="3D">
    ...
    df_iter = read_xml_iterparse(
        xml,
        parser=parser,
        iterparse={"shape": ["type", "name", "type"]***REMOVED***,
        names=["type_dim", "shape", "type_edge"],
    )
...
<shapes>
  ...
  <shape>
    ...
    df_iter = read_xml_iterparse(
        xml,
        parser=parser,
        iterparse={"shape": ["name", "family"]***REMOVED***,
        names=["name", "group"],
    )
...
<shapes>
  ...
  <shape>
    ...
    df_iter = read_xml_iterparse(
        xml,
        parser=parser,
        iterparse={"shape": ["value", "value", "value", "value"]***REMOVED***,
        names=["name", "family", "degrees", "sides"],
    )
...
def test_parser_consistency_with_encoding(xml_baby_names):
    ...
    df_iter_lxml = read_xml(
        ...
        parser="lxml",
        encoding="ISO-8859-1",
        iterparse={"row": ["rank", "malename", "femalename"]***REMOVED***,
    )
    df_iter_etree = read_xml(
        xml_baby_names,
        parser="etree",
...
"""
    ...
    expected = DataFrame({"a": ["c"]***REMOVED***)
...
def test_stylesheet_file(kml_cta_rail_lines, xsl_flatten_doc):
    ...
    df_style = read_xml(
        kml_cta_rail_lines,
        xpath=".//k:Placemark",
        namespaces={"k": "http://www.opengis.net/kml/2.2"***REMOVED***,
    ...
    with open(xml_file, encoding="utf-8") as f:
        ...
        with pytest.raises(
            ...
            read_xml(
                httpserver.url,
                parser=parser,
                iterparse={"row": ["shape", "degrees", "sides", "date"]***REMOVED***,
***REMOVED***
...
def test_wrong_dict_value(xml_books, parser):
    with pytest.raises(
        ...
        read_xml(xml_books, parser=parser, iterparse={"book": "category"***REMOVED***)
...
<!-- comment after root -->"""
    ...
    df_iter = read_xml_iterparse(
        xml, parser=parser, iterparse={"shape": ["name", "type"]***REMOVED***
    )
...
def test_no_result(xml_books, parser):
    with pytest.raises(
        ...
        read_xml(
            xml_books,
            parser=parser,
            iterparse={"node": ["attr1", "elem1", "elem2", "elem3"]***REMOVED***,
        )
...
def test_empty_data(xml_books, parser):
    with pytest.raises(EmptyDataError, match="No columns to parse from file"):
        read_xml(
            xml_books,
            parser=parser,
            iterparse={"book": ["attr1", "elem1", "elem2", "elem3"]***REMOVED***,
        )
...
def test_wrong_compression(parser, compression, compression_only):
    ...
    if actual_compression == attempted_compression:
        pytest.skip(f"{actual_compression***REMOVED*** == {attempted_compression***REMOVED***")
...
def test_s3_parser_consistency(s3_public_bucket_with_data, s3so):
    ...
    pytest.importorskip("lxml")
    s3 = f"s3://{s3_public_bucket_with_data.name***REMOVED***/books.xml"
