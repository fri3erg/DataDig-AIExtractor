...
def test_value_counts_null(null_obj, index_or_series_obj):
    ...
    if not allow_na_ops(obj):
        ...
        pytest.skip(f"MultiIndex can't hold '{null_obj***REMOVED***'")
...
def test_value_counts_bins(index_or_series, using_infer_string):
    ...
    exp1 = Series({Interval(0.997, 3.0): 4***REMOVED***, name="count")
    tm.assert_series_equal(res1, exp1)
    res1n = s1.value_counts(bins=1, normalize=True)
    exp1n = Series({Interval(0.997, 3.0): 1.0***REMOVED***, name="proportion")
    ...
    s = klass({***REMOVED***) if klass is dict else klass({***REMOVED***, dtype=object)
...
***REMOVED***.as_unit(unit),
    ...
    assert result.index.dtype == f"datetime64[{unit***REMOVED***]"
    ...
    assert s.dtype == f"datetime64[{unit***REMOVED***]"
    unique = s.unique()
    assert unique.dtype == f"datetime64[{unit***REMOVED***]"
...
def test_value_counts_timedelta64(index_or_series, unit):
    ...
    tdvals = np.zeros(6, dtype=f"m8[{unit***REMOVED***]") + day
    ...
    td2 = day + np.zeros(6, dtype=f"m8[{unit***REMOVED***]")
