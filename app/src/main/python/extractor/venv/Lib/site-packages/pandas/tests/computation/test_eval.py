        ...
        pytest.param(
            ...
            marks=[
                pytest.mark.skipif(
                    ...
                    reason=f"numexpr enabled->{USE_NUMEXPR***REMOVED***, "
                    f"installed->{NUMEXPR_INSTALLED***REMOVED***",
    ***REMOVED***,
...
def idx_func_dict():
    return {
        ...
        "s": lambda n: Index([f"{i***REMOVED***_{chr(i)***REMOVED***" for i in range(97, 97 + n)]),
    ...
    def test_complex_cmp_ops(self, cmp1, cmp2, binop, lhs, rhs, engine, parser):
        if parser == "python" and binop in ["and", "or"]:
            ...
            with pytest.raises(NotImplementedError, match=msg):
                ex = f"(lhs {cmp1***REMOVED*** rhs) {binop***REMOVED*** (lhs {cmp2***REMOVED*** rhs)"
        ...
        if cmp_op in ("in", "not in") and not is_list_like(rhs):
            with pytest.raises(TypeError, match=msg):
                pd.eval(
                    ...
                    engine=engine,
                    parser=parser,
                    local_dict={"lhs": lhs, "rhs": rhs***REMOVED***,
    ***REMOVED***
        else:
...
***REMOVED***
    ...
    ***REMOVED***
    ...
    def test_chained_cmp_op(self, cmp1, cmp2, lhs, midhs, rhs, engine, parser):
        mid = midhs
        if parser == "python":
            ex1 = f"lhs {cmp1***REMOVED*** mid {cmp2***REMOVED*** rhs"
            msg = "'BoolOp' nodes are not implemented"
            with pytest.raises(NotImplementedError, match=msg):
        ...
        if lhs_new is not None and rhs_new is not None:
            ex1 = f"lhs {cmp1***REMOVED*** mid {cmp2***REMOVED*** rhs"
            ex2 = f"lhs {cmp1***REMOVED*** mid and mid {cmp2***REMOVED*** rhs"
            ex3 = f"(lhs {cmp1***REMOVED*** mid) & (mid {cmp2***REMOVED*** rhs)"
    ...
    def test_binary_arith_ops(self, arith1, lhs, rhs, engine, parser):
        ex = f"lhs {arith1***REMOVED*** rhs"
        result = pd.eval(ex, engine=engine, parser=parser)
        expected = _eval_single_bin(lhs, arith1, rhs, engine)

        tm.assert_almost_equal(result, expected)
        ex = f"lhs {arith1***REMOVED*** rhs {arith1***REMOVED*** rhs"
        result = pd.eval(ex, engine=engine, parser=parser)
        ...
        try:
            ...
            if engine == "numexpr":
                ...
                expected = ne.evaluate(f"nlhs {arith1***REMOVED*** ghs")
                ...
                expected = eval(f"nlhs {arith1***REMOVED*** ghs")
                tm.assert_almost_equal(result, expected)
...
***REMOVED***
    ...
    ***REMOVED***
    ...
    def test_frame_invert(self, engine, parser):
        ...
        # object raises
        lhs = DataFrame(
            {"b": ["a", 1, 2.0], "c": np.random.default_rng(2).standard_normal(3) > 0.5***REMOVED***
        )
        if engine == "numexpr":
    ...
    def test_float_comparison_bin_op(self, dtype, expr):
        # GH 16363
        df = DataFrame({"x": np.array([0], dtype=dtype)***REMOVED***)
        res = df.eval(expr)
        assert res.values == np.array([False])

    def test_unary_in_function(self):
        # GH 46471
        df = DataFrame({"x": [0, 1, np.nan]***REMOVED***)
    ...
    def test_float_truncation(self, engine, parser):
        ...
        df = DataFrame({"A": [1000000000.0009, 1000000000.0011, 1000000000.0015]***REMOVED***)
        cutoff = 1000000000.0006
        result = df.query(f"A < {cutoff:.4f***REMOVED***")
        assert result.empty
        ...
        result = df.query(f"A == {exact:.4f***REMOVED***")
        expected = df.loc[[1], :]
        tm.assert_frame_equal(expected, result)
    ...
    def test_and_logic_string_match(self):
        # GH 25823
        event = Series({"a": "hello"***REMOVED***)
        assert pd.eval(f"{event.str.match('hello').a***REMOVED***")
        assert pd.eval(f"{event.str.match('hello').a and event.str.match('hello').a***REMOVED***")
...
class TestTypeCasting:
    ...
    def test_binop_typecasting(self, engine, parser, op, dt, left_right):
        ...
        s = f"{left***REMOVED*** {op***REMOVED*** {right***REMOVED***"
...
class TestAlignment:
    ...
    def test_basic_series_frame_alignment(
        ...
        if (
            ...
            reason = (
                f"Flaky column ordering when engine={engine***REMOVED***, "
                f"parser={parser***REMOVED***, index_name={index_name***REMOVED***, "
                f"r_idx_type={r_idx_type***REMOVED***, c_idx_type={c_idx_type***REMOVED***"
***REMOVED***
    ...
    def test_series_frame_commutativity(
        ...
        lhs = f"s {op***REMOVED*** df"
        rhs = f"df {op***REMOVED*** s"
        if should_warn(df.index, s.index):
            with tm.assert_produces_warning(RuntimeWarning):
        ...
        with tm.assert_produces_warning(wrn) as w:
            ...
            if not is_python_engine:
                ...
                expected = (
                    ...
                    f"by more than {logged:.4g***REMOVED***; performance may suffer."
    ***REMOVED***
                assert msg == expected
...
class TestOperations:
    ...
    def test_simple_arith_ops(self, engine, parser):
        ...
        for op in ops:
            ex = f"1 {op***REMOVED*** 1"
            ex2 = f"x {op***REMOVED*** 1"
            ex3 = f"1 {op***REMOVED*** (x + 1)"
            ...
            if op in ("in", "not in"):
                ...
                y = self.eval(ex2, local_dict={"x": x***REMOVED***, engine=engine, parser=parser)
                assert y == expec

                expec = _eval_single_bin(1, op, x + 1, engine)
                y = self.eval(ex3, local_dict={"x": x***REMOVED***, engine=engine, parser=parser)
                assert y == expec
    ...
    def test_simple_bool_ops(self, rhs, lhs, op):
        ex = f"{lhs***REMOVED*** {op***REMOVED*** {rhs***REMOVED***"
    ...
    def test_4d_ndarray_fails(self):
        ...
        with pytest.raises(NotImplementedError, match=msg):
            self.eval("x + y", local_dict={"x": x, "y": y***REMOVED***)
    ...
    def test_single_variable(self):
        ...
        df2 = self.eval("df", local_dict={"df": df***REMOVED***)
    ...
    def test_lhs_expression_subscript(self):
        df = DataFrame(np.random.default_rng(2).standard_normal((5, 3)))
        result = self.eval("(df + 1)[df > 2]", local_dict={"df": df***REMOVED***)
        expected = (df + 1)[df > 2]
    ...
    def test_attr_expression(self):
        ...
        for e, expec in zip(exprs, expecs):
            tm.assert_series_equal(expec, self.eval(e, local_dict={"df": df***REMOVED***))
    ...
    def test_assignment_fails(self):
        ...
        with pytest.raises(ValueError, match=msg):
            self.eval(expr1, local_dict={"df": df, "df2": df2***REMOVED***)
    ...
    def test_assignment_explicit(self):
        ...
        # explicit targets
        self.eval("c = df.a + df.b", local_dict={"df": df***REMOVED***, target=df, inplace=True)
        expected = df.copy()
        expected["c"] = expected["a"] + expected["b"]
        tm.assert_frame_equal(df, expected)
    ...
    def test_column_in(self):
        # GH 11235
        df = DataFrame({"a": [11], "b": [-32]***REMOVED***)
...
***REMOVED***
    ...
    @pytest.mark.parametrize("target", [1, "cat", [1, 2], np.array([]), (1, 3), {1: 2***REMOVED***])
    ...
    def test_basic_period_index_boolean_expression(self):
        ...
        e = df < 2
        r = self.eval("df < 2", local_dict={"df": df***REMOVED***)
        x = df < 2
    ...
    def test_basic_period_index_subscript_expression(self):
        ...
        r = self.eval("df[df < 2 + 3]", local_dict={"df": df***REMOVED***)
        e = df[df < 2 + 3]
        tm.assert_frame_equal(r, e)
    ...
    def test_nested_period_index_subscript_expression(self):
        ...
        r = self.eval("df[df[df < 2] < 2] + df * 2", local_dict={"df": df***REMOVED***)
        e = df[df[df < 2] < 2] + df * 2
        tm.assert_frame_equal(r, e)
...
***REMOVED***
    ...
    def test_fails_ampersand_pipe(self, char, engine, parser):
        df = DataFrame(np.random.default_rng(2).standard_normal((5, 3)))  # noqa: F841
        ex = f"(df + 2)[df > 1] > 0 {char***REMOVED*** (df > 0)"
        if parser == "python":
...
class TestMath:
    ...
    def test_unary_functions(self, fn):
        df = DataFrame({"a": np.random.default_rng(2).standard_normal(10)***REMOVED***)
        a = df.a
    ...
    def test_result_types(self, dtype, expect_dtype, engine, parser):
        ...
        df = DataFrame(
            {"a": np.random.default_rng(2).standard_normal(10).astype(dtype)***REMOVED***
        )
...
class TestScope:
    ...
    def test_empty_locals(self, engine, parser):
        ...
        with pytest.raises(UndefinedVariableError, match=msg):
            pd.eval("x + 1", engine=engine, parser=parser, local_dict={***REMOVED***)
    ...
    def test_empty_globals(self, engine, parser):
        ...
        e = "_var_s * 2"
        with pytest.raises(UndefinedVariableError, match=msg):
            pd.eval(e, engine=engine, parser=parser, global_dict={***REMOVED***)
...
def test_invalid_engine():
    ...
    with pytest.raises(KeyError, match=msg):
        pd.eval("x + y", local_dict={"x": 1, "y": 2***REMOVED***, engine="asdf")
...
def test_numexpr_option_incompatible_op():
    ...
    with pd.option_context("compute.use_numexpr", False):
        df = DataFrame(
            {"A": [True, False, True, False, None, None], "B": [1, 2, 3, 4, 5, 6]***REMOVED***
        )
        result = df.query("A.isnull()")
        expected = DataFrame({"A": [None, None], "B": [5, 6]***REMOVED***, index=[4, 5])
        tm.assert_frame_equal(result, expected)
...
def test_invalid_parser():
    ...
    with pytest.raises(KeyError, match=msg):
        pd.eval("x + y", local_dict={"x": 1, "y": 2***REMOVED***, parser="asdf")
...
def test_bool_ops_fails_on_scalars(lhs, cmp, rhs, engine, parser):
    ...
    ***REMOVED***
    ...
    ex1 = f"lhs {cmp***REMOVED*** mid {cmp***REMOVED*** rhs"
    ex2 = f"lhs {cmp***REMOVED*** mid and mid {cmp***REMOVED*** rhs"
    ex3 = f"(lhs {cmp***REMOVED*** mid) & (mid {cmp***REMOVED*** rhs)"
    for ex in (ex1, ex2, ex3):
        msg = "cannot evaluate scalar only bool ops|'BoolOp' nodes are not"
        with pytest.raises(NotImplementedError, match=msg):
            pd.eval(ex, engine=engine, parser=parser)
...
def test_equals_various(other):
    df = DataFrame({"A": ["a", "b", "c"]***REMOVED***, dtype=object)
    result = df.eval(f"A == {other***REMOVED***")
...
def test_query_token(engine, column):
    ...
    query_string = f"`{column***REMOVED***` > 5"
...
def test_eval_no_support_column_name(request, column):
    ...
    if column in ["True", "False", "inf", "Inf"]:
        request.applymarker(
            pytest.mark.xfail(
                raises=KeyError,
                reason=f"GH 47859 DataFrame eval not supported with {column***REMOVED***",
***REMOVED***
        )
    ...
    result = df.query(f"{column***REMOVED***>6")
...
def test_set_inplace(using_copy_on_write, warn_copy_on_write):
    ...
    df = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6], "C": [7, 8, 9]***REMOVED***)
    result_view = df[:]
    ser = df["A"]
    with tm.assert_cow_warning(warn_copy_on_write):
        df.eval("A = B + C", inplace=True)
    expected = DataFrame({"A": [11, 13, 15], "B": [4, 5, 6], "C": [7, 8, 9]***REMOVED***)
    tm.assert_frame_equal(df, expected)
