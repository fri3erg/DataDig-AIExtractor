...
_CLEANUP_FUNCS = {"folder": shutil.rmtree, "file": os.unlink***REMOVED***
...
class ResourceTracker:
    ...
    ***REMOVED***
            ...
            cmd = f"from {main.__module__***REMOVED*** import main; main({r***REMOVED***, {VERBOSE***REMOVED***)"
            try:
                fds_to_pass.append(r)
                # process will out live us, so no need to wait on pid
                exe = spawn.get_executable()
                args = [exe, *util._args_from_interpreter_flags(), "-c", cmd]
                util.debug(f"launching resource tracker: {args***REMOVED***")
    ...
    def _send(self, cmd, name, rtype):
        ...
        msg = f"{cmd***REMOVED***:{name***REMOVED***:{rtype***REMOVED***\n".encode("ascii")
...
def main(fd, verbose=0):
    ...
    registry = {rtype: {***REMOVED*** for rtype in _CLEANUP_FUNCS.keys()***REMOVED***
    try:
        ...
        ***REMOVED***
                    ...
                    if rtype not in _CLEANUP_FUNCS:
                        raise ValueError(
                            f"Cannot register {name***REMOVED*** for automatic cleanup: "
                            f"unknown resource type ({rtype***REMOVED***). Resource type "
                            "should be one of the following: "
                            f"{list(_CLEANUP_FUNCS.keys())***REMOVED***"
            ***REMOVED***
                    ...
                    if cmd == "REGISTER":
                        ...
                        if verbose:
                            util.debug(
                                ...
                                f"{rtype***REMOVED*** {name***REMOVED*** "
                                f"(current {registry[rtype][name]***REMOVED***)"
                ***REMOVED***
                    elif cmd == "UNREGISTER":
                        del registry[rtype][name]
                        if verbose:
                            util.debug(
                                f"[ResourceTracker] unregister {name***REMOVED*** {rtype***REMOVED***: "
                                f"registry({len(registry)***REMOVED***)"
                ***REMOVED***
                ...
                ***REMOVED***
                        ...
                        if registry[rtype][name] == 0:
                            ...
                            try:
                                if verbose:
                                    util.debug(
                                        f"[ResourceTracker] unlink {name***REMOVED***"
                        ***REMOVED***
                                _CLEANUP_FUNCS[rtype](name)
                            except Exception as e:
                                warnings.warn(
                                    f"resource_tracker: {name***REMOVED***: {e!r***REMOVED***"
                    ***REMOVED***

                    else:
                        raise RuntimeError(f"unrecognized command {cmd!r***REMOVED***")
                except BaseException:
                    try:
            ...
            if rtype_registry:
                try:
                    warnings.warn(
                        ...
                        f"{len(rtype_registry)***REMOVED*** leaked {rtype***REMOVED*** objects to "
        ...
        ***REMOVED***
            ...
            for name in rtype_registry:
                ...
                try:
                    _CLEANUP_FUNCS[rtype](name)
                    if verbose:
                        util.debug(f"[ResourceTracker] unlink {name***REMOVED***")
                except Exception as e:
                    warnings.warn(f"resource_tracker: {name***REMOVED***: {e!r***REMOVED***")
...
def spawnv_passfds(path, args, passfds):
    ...
    if sys.platform != "win32":
        ...
        cmd = " ".join(f'"{x***REMOVED***"' for x in args)
        try:
