...
def _get_client_creator(session, region_name):
    def client_creator(service_name, **kwargs):
        create_client_kwargs = {'region_name': region_name***REMOVED***
...
***REMOVED***
        ...
        if version != 1:
            raise CredentialRetrievalError(
                provider=self.METHOD,
                error_msg=(
                    f"Unsupported version '{version***REMOVED***' for credential process "
...
***REMOVED***
        try:
            ...
            ***REMOVED***
            ...
            raise CredentialRetrievalError(
                provider=self.METHOD,
                error_msg=f"Missing required key in response: {e***REMOVED***",
***REMOVED***
    ...
    def _credential_process(self):
        ...
        profile_config = self._loaded_config.get('profiles', {***REMOVED***).get(
            self._profile_name, {***REMOVED***
        )
...
class AssumeRoleProvider(CredentialProvider):
    ...
    def load(self):
        self._loaded_config = self._load_config()
        profiles = self._loaded_config.get('profiles', {***REMOVED***)
        profile = profiles.get(self._profile_name, {***REMOVED***)
...
***REMOVED***
    ...
    def _validate_credential_source(self, parent_profile, credential_source):
        if self._credential_sourcer is None:
            raise InvalidConfigError(
                error_msg=(
                    f"The credential_source \"{credential_source***REMOVED***\" is specified "
                    f"in profile \"{parent_profile***REMOVED***\", "
...
***REMOVED***
        if not self._credential_sourcer.is_supported(credential_source):
            raise InvalidConfigError(
                error_msg=(
                    f"The credential source \"{credential_source***REMOVED***\" referenced "
                    f"in profile \"{parent_profile***REMOVED***\" is not valid."
    ***REMOVED***
***REMOVED***
    ...
    def _validate_source_profile(
        ...
        if source_profile_name not in profiles:
            raise InvalidConfigError(
                error_msg=(
                    f"The source_profile \"{source_profile_name***REMOVED***\" referenced in "
                    f"the profile \"{parent_profile_name***REMOVED***\" does not exist."
    ***REMOVED***
***REMOVED***
    ...
    ***REMOVED***
    ...
    def _get_profile_config(self, key):
        if self._profile_config is None:
            ...
            profiles = loaded_config.get('profiles', {***REMOVED***)
            self._profile_config = profiles.get(self._profile_name, {***REMOVED***)
...
class ContainerProvider(CredentialProvider):
    ...
    def _build_headers(self):
        ...
        if auth_token is not None:
            ...
            return {'Authorization': auth_token***REMOVED***
...
class SSOProvider(CredentialProvider):
    ...
    def _load_sso_config(self):
        ...
        profile_config = profiles.get(self._profile_name, {***REMOVED***)
        sso_sessions = loaded_config.get('sso_sessions', {***REMOVED***)
...
***REMOVED***
    ...
    def _resolve_sso_session_reference(self, profile_config, sso_sessions):
        ...
        if sso_session_name not in sso_sessions:
            error_msg = f'The specified sso-session does not exist: "{sso_session_name***REMOVED***"'
            raise InvalidConfigError(error_msg=error_msg)
        ...
        for config_var, val in session.items():
            ...
            if config.get(config_var, val) != val:
                error_msg = (
                    f"The value for {config_var***REMOVED*** is inconsistent between "
                    f"profile ({config[config_var]***REMOVED***) and sso-session ({val***REMOVED***)."
    ***REMOVED***
                raise InvalidConfigError(error_msg=error_msg)
