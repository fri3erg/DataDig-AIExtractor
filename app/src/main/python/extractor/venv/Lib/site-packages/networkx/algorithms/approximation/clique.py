...
def clique_removal(G):
    ...
    ({0, 2, 4, 6, 9***REMOVED***, [{0, 1***REMOVED***, {2, 3***REMOVED***, {4, 5***REMOVED***, {6, 7***REMOVED***, {8, 9***REMOVED***])
...
def large_clique_size(G):
    ...
    *d* is the maximum degree.
    ...
    def _clique_heuristic(G, U, size, best_size):
        ...
        u = max(U, key=degrees)
        U.remove(u)
        N_prime = {v for v in G[u] if degrees[v] >= best_size***REMOVED***
        return _clique_heuristic(G, U & N_prime, size + 1, best_size)

    best_size = 0
    nodes = (u for u in G if degrees[u] >= best_size)
    for u in nodes:
        neighbors = {v for v in G[u] if degrees[v] >= best_size***REMOVED***
