...
def is_global_accesspoint(context):
    ...
    s3_accesspoint = context.get('s3_accesspoint', {***REMOVED***)
...
***REMOVED***
    ...
    def _construct_url(self, path):
        ...
        if self._base_url and not self._base_url.endswith('/'):
            sep = '/'
        return f'{self._base_url***REMOVED***{sep***REMOVED***{path***REMOVED***'
...
***REMOVED***
    ...
    ***REMOVED***
                logger.info(
                    ...
                    f"the next {refresh_interval_with_jitter/60:.0f***REMOVED*** minutes."
    ***REMOVED***
        except ValueError:
            logger.debug(
                f"Unable to parse expiry_time in {credentials['expiry_time']***REMOVED***"
***REMOVED***
        ...
        if isinstance(value, list):
            for element in value:
                encoded_pairs.append(
                    f'{percent_encode(key)***REMOVED***={percent_encode(element)***REMOVED***'
    ***REMOVED***
        else:
            encoded_pairs.append(
                f'{percent_encode(key)***REMOVED***={percent_encode(value)***REMOVED***'
***REMOVED***
...
def _parse_timestamp_with_tzinfo(value, tzinfo):
    ...
    try:
        ...
        return dateutil.parser.parse(value, tzinfos={'GMT': tzutc()***REMOVED***)
    except (TypeError, ValueError) as e:
        raise ValueError(f'Invalid timestamp "{value***REMOVED***": {e***REMOVED***')
...
def is_valid_ipv6_endpoint_url(endpoint_url):
    if UNSAFE_URL_CHARS.intersection(endpoint_url):
        return False
    hostname = f'[{urlparse(endpoint_url).hostname***REMOVED***]'
...
def is_valid_endpoint_url(endpoint_url):
    ...
    if hostname[-1] == ".":
        hostname = hostname[:-1]
    allowed = re.compile(
        r"^((?!-)[A-Z\d-]{1,63***REMOVED***(?<!-)\.)*((?!-)[A-Z\d-]{1,63***REMOVED***(?<!-))$",
        re.IGNORECASE,
    )
...
def validate_region_name(region_name):
    ...
    if region_name is None:
        return
    valid_host_label = re.compile(r'^(?![0-9]+$)(?!-)[a-zA-Z0-9-]{,63***REMOVED***(?<!-)$')
...
def _get_new_endpoint(original_endpoint, new_endpoint, use_new_scheme=True):
    ...
    logger.debug(f'Updating URI from {original_endpoint***REMOVED*** to {final_endpoint***REMOVED***')
...
class S3ExpressIdentityResolver:
    ...
    def inject_signing_cache_key(self, params, context, **kwargs):
        if 'Bucket' in params:
            context['S3Express'] = {'bucket_name': params['Bucket']***REMOVED***

    def apply_signing_cache_key(self, params, context, **kwargs):
        endpoint_properties = context.get('endpoint_properties', {***REMOVED***)
        backend = endpoint_properties.get('backend', None)

        # Add cache key if Bucket supplied for s3express request
        bucket_name = context.get('S3Express', {***REMOVED***).get('bucket_name')
        if backend == 'S3Express' and bucket_name is not None:
            context.setdefault('signing', {***REMOVED***)
    ...
    def resolve_s3express_identity(
        ...
        **kwargs,
        ...
        signing_context = request.context.get('signing', {***REMOVED***)
        ...
        if signing_name == 's3express' and signature_version.startswith(
            ...
            if 'cache_key' not in signing_context:
                signing_context['cache_key'] = (
                    request.context.get('s3_redirect', {***REMOVED***)
                    .get('params', {***REMOVED***)
                    .get('Bucket')
    ***REMOVED***
...
class S3RegionRedirectorv2:
    ...
    def redirect_from_error(self, request_dict, response, operation, **kwargs):
        ...
        redirect_ctx = request_dict.get('context', {***REMOVED***).get('s3_redirect', {***REMOVED***)
...
***REMOVED***
        ...
        error = response[1].get('Error', {***REMOVED***)
        error_code = error.get('Code')
        response_metadata = response[1].get('ResponseMetadata', {***REMOVED***)
        ...
        is_special_head_bucket = (
            ...
            in response_metadata.get('HTTPHeaders', {***REMOVED***)
        )
...
***REMOVED***
        ...
        if auth_schemes is not None:
            ...
            request_dict['context']['signing'] = {
                **request_dict['context'].get('signing', {***REMOVED***),
                **signing_context,
            ***REMOVED***
    ...
    def get_bucket_region(self, bucket, response):
        ...
        region = service_response.get('Error', {***REMOVED***).get('Region', None)
...
class S3RegionRedirector:
    ...
    def redirect_from_error(self, request_dict, response, operation, **kwargs):
        ...
        if self._is_s3_accesspoint(request_dict.get('context', {***REMOVED***)):
            logger.debug(
                'S3 request was previously to an accesspoint, not redirecting.'
***REMOVED***
            return

        if request_dict.get('context', {***REMOVED***).get('s3_redirected'):
            logger.debug(
                'S3 request was previously redirected, not redirecting.'
***REMOVED***
            return
...
***REMOVED***
    ...
    def set_request_url(self, params, context, **kwargs):
        endpoint = context.get('signing', {***REMOVED***).get('endpoint', None)
    ...
    def redirect_from_cache(self, params, context, **kwargs):
        ...
        if signing_context is not None:
            ...
            context['signing'] = {'bucket': bucket***REMOVED***
...
class S3ArnParamHandler:
    ...
    _OUTPOST_RESOURCE_REGEX = re.compile(
        r'^(?P<outpost_name>[a-zA-Z0-9\-]{1,63***REMOVED***)[/:]accesspoint[/:]'
        r'(?P<accesspoint_name>[a-zA-Z0-9\-]{1,63***REMOVED***$)'
    )
...
***REMOVED***
        ...
        new_endpoint = 'https://{host_prefix***REMOVED***{hostname***REMOVED***'.format(
    ...
    def _validate_fips_supported(self, request):
        ...
        if 'fips' in request.context['s3_accesspoint']['region']:
            raise UnsupportedS3AccesspointConfigurationError(
                msg={'Invalid ARN, FIPS region not allowed in ARN.'***REMOVED***
***REMOVED***
...
***REMOVED***
        ...
        logger.debug(
            f'Updating URI from {request.url***REMOVED*** to {accesspoint_endpoint***REMOVED***'
        )
...
***REMOVED***
    ...
    def _get_accesspoint_netloc(self, request_context, region_name):
        ...
        accesspoint_netloc_components = [
            '{***REMOVED***-{***REMOVED***'.format(s3_accesspoint['name'], s3_accesspoint['account']),
        ]
...
***REMOVED***
    ...
    def _override_signing_name(self, context, signing_name):
        signing_context = context.get('signing', {***REMOVED***)
...
class S3ControlEndpointSetter:
    _DEFAULT_PARTITION = 'aws'
    _DEFAULT_DNS_SUFFIX = 'amazonaws.com'
    _HOST_LABEL_REGEX = re.compile(r'^[a-zA-Z0-9\-]{1,63***REMOVED***$')
        ...
        if not self._s3_config.get('use_arn_region', False):
            ...
            if arn_region != self._region:
                error_msg = (
                    ...
                    'to use "%s"'
    ***REMOVED*** % (arn_region, self._region)
...
***REMOVED***
        ...
        logger.debug(
            f'Updating URI from {request.url***REMOVED*** to {arn_details_endpoint***REMOVED***'
        )
...
***REMOVED***
    ...
    def _raise_for_accelerate_endpoint(self, context):
        s3_config = context['client_config'].s3 or {***REMOVED***
...
***REMOVED***
    ...
    def _validate_allowed_url(self, full_url):
        ...
        if not is_whitelisted_host:
            raise ValueError(
                f"Unsupported host '{parsed.hostname***REMOVED***'.  Can only retrieve metadata "
                f"from a loopback address or one of these hosts: {', '.join(self._ALLOWED_HOSTS)***REMOVED***"
***REMOVED***
    ...
    def _get_response(self, full_url, headers, timeout):
        try:
            ...
            if response.status_code != 200:
                raise MetadataRetrievalError(
                    error_msg=(
                        f"Received non 200 response {response.status_code***REMOVED*** "
                        f"from container metadata: {response_text***REMOVED***"
        ***REMOVED***
    ***REMOVED***
            try:
            ...
            error_msg = (
                "Received error when attempting to retrieve "
                f"container metadata: {e***REMOVED***"
***REMOVED***
            raise MetadataRetrievalError(error_msg=error_msg)

    def full_url(self, relative_uri):
        return f'http://{self.IP_ADDRESS***REMOVED***{relative_uri***REMOVED***'
...
def _is_s3express_request(params):
    endpoint_properties = params.get('context', {***REMOVED***).get(
        'endpoint_properties', {***REMOVED***
    )
...
def conditionally_enable_crc32(params, **kwargs):
    checksum_context = params.get('context', {***REMOVED***).get('checksum', {***REMOVED***)
...
class SSOTokenLoader:
    ...
    def __call__(self, start_url, session_name=None):
        cache_key = self._generate_cache_key(start_url, session_name)
        logger.debug(f'Checking for cached token at: {cache_key***REMOVED***')
        if cache_key not in self._cache:
            name = start_url
            if session_name is not None:
                name = session_name
            error_msg = f'Token for {name***REMOVED*** does not exist'
        ...
        if 'accessToken' not in token or 'expiresAt' not in token:
            error_msg = f'Token for {start_url***REMOVED*** is invalid'
            raise SSOTokenLoadError(error_msg=error_msg)
        return token
...
class EventbridgeSignerSetter:
    ...
    def set_endpoint_url(self, params, context, **kwargs):
        if 'eventbridge_endpoint' in context:
            ...
            logger.debug(f"Rewriting URL from {params['url']***REMOVED*** to {endpoint***REMOVED***")
            params['url'] = endpoint
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        if self._endpoint_url is None:
            ...
            parts = urlparse(f'https://{endpoint***REMOVED***')
...
***REMOVED***
    ...
    def _get_global_endpoint(self, endpoint, endpoint_variant_tags=None):
        ...
        return f"https://{endpoint***REMOVED***.endpoint.events.{dns_suffix***REMOVED***/"
    ...
    def __setitem__(self, cache_key, value):
        ...
        try:
            ...
            raise ValueError(
                ...
                f"JSON serializable: {value***REMOVED***"
***REMOVED***
...
SERVICE_NAME_ALIASES = {'runtime.sagemaker': 'sagemaker-runtime'***REMOVED***
