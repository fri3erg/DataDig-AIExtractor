...
def set_engine(engine, ext):
    ...
    """
    option_name = f"io.excel.{ext.strip('.')***REMOVED***.writer"
...
class TestRoundTrip:
    @pytest.mark.parametrize(
        "header,expected",
        [(None, DataFrame([np.nan] * 4)), (0, DataFrame({"Unnamed: 0": [np.nan] * 3***REMOVED***))],
    )
            ...
            expected = DataFrame(
                {
                    ...
                    ("Two", "X"): {0: 3***REMOVED***,
                    ("Two", "Y"): {0: 7***REMOVED***,
                    ("Zero", "Unnamed: 4_level_1"): {0: 0***REMOVED***,
                ***REMOVED***
***REMOVED***
...
***REMOVED***
            ...
            expected = DataFrame(
                {
                    ("Beg", "Unnamed: 1_level_1"): {0: 0***REMOVED***,
                    ("Middle", "x"): {0: 1***REMOVED***,
                    ("Tail", "X"): {0: 3***REMOVED***,
                    ("Tail", "Y"): {0: 7***REMOVED***,
                ***REMOVED***
        ...
        with tm.ensure_clean(ext) as pth:
            ...
            if c_idx_levels == 1:
                ...
                columns = MultiIndex.from_arrays(
                    ...
                    names=[f"{c_idx_names***REMOVED***-{i***REMOVED***" for i in range(c_idx_levels)],
    ***REMOVED***
            if r_idx_levels == 1:
                ...
                index = MultiIndex.from_arrays(
                    ...
                    names=[f"{r_idx_names***REMOVED***-{i***REMOVED***" for i in range(r_idx_levels)],
    ***REMOVED***
            df = DataFrame(
                1.1 * np.ones((5, 5)),
                columns=columns,
...
***REMOVED***
    ...
    def test_read_excel_parse_dates(self, ext):
        ...
        df = DataFrame(
            {"col": [1, 2, 3], "date_strings": date_range("2012-01-01", periods=3)***REMOVED***
        )
        df2 = df.copy()
...
***REMOVED***
    ...
    def test_excel_date_datetime_format(self, ext, path):
        ...
        df_expected = df_expected.astype(f"M8[{unit***REMOVED***]")
...
***REMOVED***
    ...
    def test_to_excel_multiindex_no_write_index(self, path):
        ...
        frame1 = DataFrame({"a": [10, 20], "b": [30, 40], "c": [50, 60]***REMOVED***)
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        if c_idx_nlevels == 1:
            columns = Index([f"a-{i***REMOVED***" for i in range(ncols)], dtype=object)
        else:
            columns = MultiIndex.from_arrays(
                [range(ncols) for _ in range(c_idx_nlevels)],
                names=[f"i-{i***REMOVED***" for i in range(c_idx_nlevels)],
***REMOVED***
        if r_idx_nlevels == 1:
            index = Index([f"b-{i***REMOVED***" for i in range(nrows)], dtype=object)
        else:
            index = MultiIndex.from_arrays(
                [range(nrows) for _ in range(r_idx_nlevels)],
                names=[f"j-{i***REMOVED***" for i in range(r_idx_nlevels)],
***REMOVED***
...
***REMOVED***
    ...
    def test_swapped_columns(self, path):
        # Test for issue #5427.
        write_frame = DataFrame({"A": [1, 1, 1], "B": [2, 2, 2]***REMOVED***)
    ...
    def test_write_subset_columns(self, path, to_excel_index, read_excel_index_col):
        # GH 31677
        write_frame = DataFrame({"A": [1, 1, 1], "B": [2, 2, 2], "C": [3, 3, 3]***REMOVED***)
    ...
    def test_comment_arg(self, path):
        ...
        df = DataFrame({"A": ["one", "#one", "one"], "B": ["two", "two", "#two"]***REMOVED***)
    ...
    def test_comment_used(self, path):
        ...
        expected = DataFrame({"A": ["one", None, "one"], "B": ["two", None, None]***REMOVED***)
    ...
    def test_comment_empty_line(self, path):
        ...
        expected = DataFrame({"a": [1], "b": [2]***REMOVED***)
    ...
    def test_datetimes(self, path):
        ...
        write_frame = DataFrame({"A": datetimes***REMOVED***)
        write_frame.to_excel(path, sheet_name="Sheet1")
        read_frame = pd.read_excel(path, sheet_name="Sheet1", header=0)

        expected = write_frame.astype(f"M8[{unit***REMOVED***]")
    ...
    def test_engine_kwargs(self, engine, path):
        # GH#52368
        df = DataFrame([{"A": 1, "B": 2***REMOVED***, {"A": 3, "B": 4***REMOVED***])
...
***REMOVED***
    ...
    def test_render_as_column_name(self, path):
        # see gh-34331
        df = DataFrame({"render": [1, 2], "data": [3, 4]***REMOVED***)
        df.to_excel(path, sheet_name="Sheet1")
    ...
    def test_true_and_false_value_options(self, path):
        ...
        with option_context("future.no_silent_downcasting", True):
            expected = df.replace({"foo": True, "bar": False***REMOVED***).astype("bool")
    ...
    def test_path_path_lib(self, engine, ext):
        df = DataFrame(
            ...
            columns=Index(list("ABCD")),
            index=Index([f"i-{i***REMOVED***" for i in range(30)], dtype=object),
        )
        writer = partial(df.to_excel, engine=engine)

        reader = partial(pd.read_excel, index_col=0)
        result = tm.round_trip_pathlib(writer, reader, path=f"foo{ext***REMOVED***")
        tm.assert_frame_equal(result, df)
    ...
    def test_path_local_path(self, engine, ext):
        df = DataFrame(
            1.1 * np.arange(120).reshape((30, 4)),
            columns=Index(list("ABCD")),
            index=Index([f"i-{i***REMOVED***" for i in range(30)]),
        )
        writer = partial(df.to_excel, engine=engine)

        reader = partial(pd.read_excel, index_col=0)
        result = tm.round_trip_localpath(writer, reader, path=f"foo{ext***REMOVED***")
        tm.assert_frame_equal(result, df)
    ...
    def test_excel_duplicate_columns_with_names(self, path):
        # GH#39695
        df = DataFrame({"A": [0, 1], "B": [10, 11]***REMOVED***)
...
def test_subclass_attr(klass):
    ...
    attrs_base = {name for name in dir(ExcelWriter) if not name.startswith("_")***REMOVED***
    attrs_klass = {name for name in dir(klass) if not name.startswith("_")***REMOVED***
