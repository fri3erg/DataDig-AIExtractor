...
def test_conv_read_write():
    with tm.ensure_clean() as path:
        ...
        o = Series(range(10), dtype="float64", index=[f"i_{i***REMOVED***" for i in range(10)])
        ...
        # table
        df = DataFrame({"A": range(5), "B": range(5)***REMOVED***)
...
def test_long_strings(setup_path):
    # GH6166
    data = ["a" * 50] * 10
    df = DataFrame({"a": data***REMOVED***, index=data)
...
def test_api_invalid(tmp_path, setup_path):
    ...
    path = ""
    msg = f"File {path***REMOVED*** does not exist"
...
def test_table_values_dtypes_roundtrip(setup_path):
    with ensure_clean_store(setup_path) as store:
        df1 = DataFrame({"a": [1, 2, 3]***REMOVED***, dtype="f8")
        store.append("df_f8", df1)
        tm.assert_series_equal(df1.dtypes, store["df_f8"].dtypes)

        df2 = DataFrame({"a": [1, 2, 3]***REMOVED***, dtype="i8")
...
def test_series(setup_path):
    s = Series(range(10), dtype="float64", index=[f"i_{i***REMOVED***" for i in range(10)])
...
def test_unicode_longer_encoded(setup_path):
    # GH 11234
    char = "\u0394"
    df = DataFrame({"A": [char]***REMOVED***)
    with ensure_clean_store(setup_path) as store:
        store.put("df", df, format="table", encoding="utf-8")
        result = store.get("df")
        tm.assert_frame_equal(result, df)

    df = DataFrame({"A": ["a", char], "B": ["b", "b"]***REMOVED***)
    with ensure_clean_store(setup_path) as store:
        store.put("df", df, format="table", encoding="utf-8")
        result = store.get("df")
        tm.assert_frame_equal(result, df)
...
def test_store_datetime_mixed(setup_path):
    df = DataFrame({"a": [1, 2, 3], "b": [1.0, 2.0, 3.0], "c": ["a", "b", "c"]***REMOVED***)
    ts = Series(
        np.arange(10, dtype=np.float64), index=date_range("2020-01-01", periods=10)
    )
    df["d"] = ts.index[:3]
...
def test_round_trip_equals(tmp_path, setup_path):
    # GH 9330
    df = DataFrame({"B": [1, 2], "A": ["x", "y"]***REMOVED***)
