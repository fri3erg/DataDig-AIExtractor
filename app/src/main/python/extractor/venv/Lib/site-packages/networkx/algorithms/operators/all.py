...
def intersection_all(graphs):
    ...
    >>> new_node_attr = {n: min(*(anyG.nodes[n].get('capacity', float('inf')) for anyG in [g, h])) for n in gh***REMOVED***
    >>> nx.set_node_attributes(gh, new_node_attr, 'new_capacity')
    >>> gh.nodes(data=True)
    NodeDataView({0: {'new_capacity': 2***REMOVED***, 1: {'new_capacity': 3***REMOVED******REMOVED***)
