...
class TestDegreeCentrality:
    ...
    def test_degree_centrality_3(self):
        ...
        ***REMOVED***
        for n, dc in d.items():
            assert exact[n] == pytest.approx(float(f"{dc:.3f***REMOVED***"), abs=1e-7)
    ...
    def test_small_graph_centrality(self):
        ...
        assert {***REMOVED*** == nx.degree_centrality(G)
        assert {***REMOVED*** == nx.out_degree_centrality(G)
        assert {***REMOVED*** == nx.in_degree_centrality(G)

        G = nx.empty_graph(1, create_using=nx.DiGraph)
        assert {0: 1***REMOVED*** == nx.degree_centrality(G)
        assert {0: 1***REMOVED*** == nx.out_degree_centrality(G)
        assert {0: 1***REMOVED*** == nx.in_degree_centrality(G)
