...
class Downloader:
    ...
    ***REMOVED***[27:]
                print("  [{***REMOVED***] {***REMOVED*** {***REMOVED***".format(prefix, info.id.ljust(20, "."), name))
    ...
    def incr_download(self, info_or_id, download_dir=None, force=False):
        ...
        try:
            ...
            yield ErrorMessage(None, f"Error loading {info_or_id***REMOVED***: {e***REMOVED***")
            return
        ...
        for collection in self._collections.values():
            for i, child_id in enumerate(collection.children):
                if child_id in self._packages:
                    ...
                    print(
                        "removing collection member with no package: {***REMOVED***".format(
                            child_id
            ***REMOVED***
        ***REMOVED***
...
***REMOVED***
    ...
    def _simple_interactive_update(self):
        ...
        ***REMOVED***[27:]
                    print("  [ ] {***REMOVED*** {***REMOVED***".format(pid.ljust(20, "."), name))
                print()
            ...
            if user_input == "s":
                ...
                if new_url in ("", "x", "q", "X", "Q"):
                    ...
                    try:
                        ...
                        print(f"Error reading <{new_url!r***REMOVED***>:\n  {e***REMOVED***")
            elif user_input == "m":
                break
...
class DownloaderGUI:
    ...
    COLUMN_WEIGHTS = {"": 0, "Name": 5, "Size": 0, "Status": 0***REMOVED***
    ...
    ***REMOVED***
    ...
    def _log(self, msg):
        self._log_messages.append(
            "{***REMOVED*** {***REMOVED***{***REMOVED***".format(time.ctime(), " | " * self._log_indent, msg)
        )
...
def build_index(root, base_url):
    ...
    for pkg_xml, zf, subdir in _find_packages(os.path.join(root, "packages")):
        ...
        url = f"{base_url***REMOVED***/{subdir***REMOVED***/{os.path.split(zf.filename)[1]***REMOVED***"
...
def _check_package(pkg_xml, zipfilename, zf):
    ...
    if pkg_xml.get("id") != uid:
        raise ValueError(
            "package identifier mismatch ({***REMOVED*** vs {***REMOVED***)".format(pkg_xml.get("id"), uid)
        )
        ...
        for filename in files:
            if filename.endswith(".xml"):
                ...
                try:
                    ...
                    raise ValueError(f"Error reading file {zipfilename!r***REMOVED***!\n{e***REMOVED***") from e
                try:
                    pkg_xml = ElementTree.parse(xmlfilename).getroot()
                except Exception as e:
                    raise ValueError(f"Error reading file {xmlfilename!r***REMOVED***!\n{e***REMOVED***") from e
    ...
    ***REMOVED***:
        ...
        ***REMOVED***
                ...
                if not os.path.exists(xmlfilename):
                    warnings.warn(
                        f"{filename***REMOVED*** exists, but {resourcename + '.xml'***REMOVED*** cannot be found! "
                        f"This could mean that {resourcename***REMOVED*** can not be downloaded.",
                        stacklevel=2,
        ***REMOVED***
