...
Feature value tuples are written with parentheses:
    ...
    >>> fs1
    [x=(?x, ?y)]
    >>> fs1.substitute_bindings({Variable('?x'): 1, Variable('?y'): 2***REMOVED***)
    [x=(1, 2)]

Feature sets are written with braces:

    >>> fs1 = FeatStruct('[x={?x, ?y***REMOVED***]')
    >>> fs1
    [x={?x, ?y***REMOVED***]
    >>> fs1.substitute_bindings({Variable('?x'): 1, Variable('?y'): 2***REMOVED***)
    [x={1, 2***REMOVED***]
...
`update()`:
    ...
    >>> fs2.update({'a':'A', 'b':'B'***REMOVED***, c='C')
    >>> fs2
...
The parser also includes support for reading sets and tuples.
    ...
    >>> FeatStruct('[x={1,2,2,2***REMOVED***, y={/***REMOVED***]')
    [x={1, 2***REMOVED***, y={/***REMOVED***]
    >>> FeatStruct('[x=(1,2,2,2), y=()]')
    [x=(1, 2, 2, 2), y=()]
    >>> print(FeatStruct('[x=(1,[z=(1,2,?x)],?z,{/***REMOVED***)]'))
    [ x = (1, [ z = (1, 2, ?x) ], ?z, {/***REMOVED***) ]

Note that we can't put a featstruct inside a tuple, because doing so
would hash it, and it's not frozen yet:

    >>> print(FeatStruct('[x={[]***REMOVED***]'))
    Traceback (most recent call last):
      . . .
    TypeError: FeatStructs must be frozen before they can be hashed.

There's a special syntax for taking the union of sets: "{...+...***REMOVED***".
The elements should only be variables or sets.

    >>> FeatStruct('[x={?a+?b+{1,2,3***REMOVED******REMOVED***]')
    [x={?a+?b+{1, 2, 3***REMOVED******REMOVED***]
...
Mixing dicts and lists is fine:
    ...
    ...               width=1)
    [{'q': 5, 'x': 1, 'y': {'z': 2***REMOVED******REMOVED***, 3]
...
-------------------
    ...
    [ b -> (1)                  ]
    >>> pprint(bindings)
    {Variable('?x'): [c=[d=1]], Variable('?y'): [d=1]***REMOVED***
    >>> retract_bindings(fs3, bindings)
    [a=?x, b=?x]
    >>> pprint(bindings)
    {Variable('?x'): [c=?y], Variable('?y'): [d=1]***REMOVED***
