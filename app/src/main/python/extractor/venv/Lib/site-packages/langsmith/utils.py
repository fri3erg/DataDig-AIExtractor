        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if invalid_groups:
                ...
                raise ValueError(
                    ...
                    f" {', '.join(invalid_group_names)***REMOVED***"
    ***REMOVED***
...
def _get_message_type(message: Mapping[str, Any]) -> str:
    ...
    if "lc" in message:
        if "id" not in message:
            raise ValueError(
                f"Unexpected format for serialized message: {message***REMOVED***"
...
***REMOVED***
        ...
        if "type" not in message:
            raise ValueError(
                f"Unexpected format for stored message: {message***REMOVED***"
...
def _convert_message(message: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {"type": message_type, "data": message_data***REMOVED***
...
def get_messages_from_inputs(inputs: Mapping[str, Any]) -> List[Dict[str, Any]]:
    ...
    raise ValueError(f"Could not find message(s) in run with inputs {inputs***REMOVED***.")


def get_message_generation_from_outputs(outputs: Mapping[str, Any]) -> Dict[str, Any]:
    if "generations" not in outputs:
        raise ValueError(f"No generations found in in run with output: {outputs***REMOVED***.")
    generations = outputs["generations"]
    if len(generations) != 1:
        raise ValueError(
            "Chat examples expect exactly one generation."
            f" Found {len(generations)***REMOVED*** generations: {generations***REMOVED***."
        )
    first_generation = generations[0]
    if "message" not in first_generation:
        raise ValueError(
            f"Unexpected format for generation: {first_generation***REMOVED***."
...
def get_prompt_from_inputs(inputs: Mapping[str, Any]) -> str:
    ...
    if "prompts" in inputs:
        ...
        if len(prompts) == 1:
            return prompts[0]
        raise ValueError(
            f"Multiple prompts in run with inputs {inputs***REMOVED***."
            " Please create example manually."
        )
    raise ValueError(f"Could not find prompt in run with inputs {inputs***REMOVED***.")
...
def get_llm_generation_from_outputs(outputs: Mapping[str, Any]) -> str:
    ...
    generations = outputs["generations"]
    if len(generations) != 1:
        raise ValueError(f"Multiple generations in run: {generations***REMOVED***")
    first_generation = generations[0]
    if "text" not in first_generation:
        raise ValueError(f"No text in generation: {first_generation***REMOVED***")
...
def convert_langchain_message(message: ls_schemas.BaseMessageLike) -> dict:
    ...
    converted: Dict[str, Any] = {
        "type": message.type,
        "data": {"content": message.content***REMOVED***,
    ***REMOVED***
    # Check for presence of keys in additional_kwargs
    if message.additional_kwargs and len(message.additional_kwargs) > 0:
        converted["data"]["additional_kwargs"] = {**message.additional_kwargs***REMOVED***
    return converted
