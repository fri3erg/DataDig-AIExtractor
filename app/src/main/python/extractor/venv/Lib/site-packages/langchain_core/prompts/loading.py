...
def load_prompt_from_config(config: dict) -> BasePromptTemplate:
    ...
    if config_type not in type_to_loader_dict:
        raise ValueError(f"Loading {config_type***REMOVED*** prompt not supported")
...
def _load_template(var_name: str, config: dict) -> dict:
    ...
    if f"{var_name***REMOVED***_path" in config:
        # If it does, make sure template variable doesn't also exist.
        if var_name in config:
            raise ValueError(
                f"Both `{var_name***REMOVED***_path` and `{var_name***REMOVED***` cannot be provided."
***REMOVED***
        # Pop the template path from the config.
        template_path = Path(config.pop(f"{var_name***REMOVED***_path"))
        # Load the template.
...
def _load_output_parser(config: dict) -> dict:
    ...
    if "output_parser" in config and config["output_parser"]:
        ...
        if output_parser_type == "default":
            ...
            raise ValueError(f"Unsupported output parser {output_parser_type***REMOVED***")
...
def _load_prompt(config: dict) -> PromptTemplate:
    ...
    if template_format == "jinja2":
        ...
        raise ValueError(
            f"Loading templates with '{template_format***REMOVED***' format is no longer supported "
...
def load_prompt(path: Union[str, Path]) -> BasePromptTemplate:
    ...
    if hub_result := try_load_from_hub(
        path, _load_prompt_from_file, "prompts", {"py", "json", "yaml"***REMOVED***
    ):
        return hub_result
    else:
...
def _load_prompt_from_file(file: Union[str, Path]) -> BasePromptTemplate:
    ...
    if file_path.suffix == ".json":
        ...
        raise ValueError(f"Got unsupported file type {file_path.suffix***REMOVED***")
