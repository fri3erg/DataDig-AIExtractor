...
class DiscourseTester:
    ...
    def __init__(self, input, reading_command=None, background=None):
        ...
        """
        self._input = input
        self._sentences = {"s%s" % i: sent for i, sent in enumerate(input)***REMOVED***
        self._models = None
        self._readings = {***REMOVED***
        self._reading_command = (
            reading_command if reading_command else CfgReadingCommand()
        )
        self._threads = {***REMOVED***
        self._filtered_threads = {***REMOVED***
    ...
    def sentences(self):
        ...
        for id in sorted(self._sentences):
            print(f"{id***REMOVED***: {self._sentences[id]***REMOVED***")
    ...
    def add_sentence(self, sentence, informchk=False, consistchk=False):
        ...
        self._sentences = {"s%s" % i: sent for i, sent in enumerate(self._input)***REMOVED***
...
***REMOVED***
    ...
    def _construct_readings(self):
        ...
        for sid in sorted(self._sentences):
            ...
            self._readings[sid] = {
                f"{sid***REMOVED***-r{rid***REMOVED***": reading.simplify()
    ...
    def _construct_threads(self):
        ...
        self._threads = {"d%s" % tid: thread for tid, thread in enumerate(thread_list)***REMOVED***
        ...
        if sentence is not None:
            ...
            for sid in sorted(self._readings):
                ...
                for rid in sorted(self._readings[sid]):
                    ...
                    print(f"{rid***REMOVED***: {lf.normalize()***REMOVED***")
...
***REMOVED***
    ...
    def models(self, thread_id=None, show=True, verbose=False):
        ...
        self._construct_readings()
        self._construct_threads()
        threads = {thread_id: self._threads[thread_id]***REMOVED*** if thread_id else self._threads
        ...
        for (tid, modelfound) in self._check_consistency(
            ...
            if not modelfound:
                print(f"Inconsistent discourse: {tid***REMOVED*** {idlist***REMOVED***:")
                for rid, reading in self.expand_threads(tid):
                    print(f"    {rid***REMOVED***: {reading.normalize()***REMOVED***")
                print()
            else:
                print(f"Consistent discourse: {tid***REMOVED*** {idlist***REMOVED***:")
                for rid, reading in self.expand_threads(tid):
                    print(f"    {rid***REMOVED***: {reading.normalize()***REMOVED***")
                print()
