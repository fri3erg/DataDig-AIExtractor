...
class MachArLike:
    ...
    def smallest_subnormal(self):
        ...
        if self.ftype(0) == value:
            warnings.warn(
                'The value of the smallest subnormal for {***REMOVED*** type '
...
_title_fmt = 'numpy {***REMOVED*** precision floating point number'
_MACHAR_PARAMS = {
    ntypes.double: dict(
        itype = ntypes.int64,
        fmt = '%24.16e',
    ...
    ntypes.half: dict(
        itype = ntypes.int16,
        fmt = '%12.5e',
        title = _title_fmt.format('half'))***REMOVED***
...
def _get_machar(ftype):
    ...
    if ma_like is None and len(key) == 16:
        ...
        _kt = {k[:10]: v for k, v in _KNOWN_TYPES.items() if len(k) == 16***REMOVED***
        ma_like = _kt.get(key[:10])
    if ma_like is not None:
        return ma_like
    # Fall back to parameter discovery
    warnings.warn(
        f'Signature {key***REMOVED*** for {ftype***REMOVED*** does not match any known type: '
