...
class EtherscanLoader(BaseLoader):
    ...
    def __init__(
        ...
        if not re.match(r"^0x[a-fA-F0-9]{40***REMOVED***$", self.account_address):
            raise ValueError(f"Invalid contract address {self.account_address***REMOVED***")
        if filter not in [
            "normal_transaction",
            ...
            raise ValueError(f"Invalid filter {filter***REMOVED***")
    ...
    def getNormTx(self) -> List[Document]:
        url = (
            f"https://api.etherscan.io/api?module=account&action=txlist&address={self.account_address***REMOVED***"
            f"&startblock={self.start_block***REMOVED***&endblock={self.end_block***REMOVED***&page={self.page***REMOVED***"
            f"&offset={self.offset***REMOVED***&sort={self.sort***REMOVED***&apikey={self.api_key***REMOVED***"
        )
        try:
        ...
        for item in items:
            content = str(item)
            metadata = {"from": item["from"], "tx_hash": item["hash"], "to": item["to"]***REMOVED***
    ...
    def getEthBalance(self) -> List[Document]:
        url = (
            ...
            f"&address={self.account_address***REMOVED***&tag=latest&apikey={self.api_key***REMOVED***"
        )
    ...
    def getInternalTx(self) -> List[Document]:
        url = (
            ...
            f"&address={self.account_address***REMOVED***&startblock={self.start_block***REMOVED***"
            f"&endblock={self.end_block***REMOVED***&page={self.page***REMOVED***&offset={self.offset***REMOVED***"
            f"&sort={self.sort***REMOVED***&apikey={self.api_key***REMOVED***"
        )
