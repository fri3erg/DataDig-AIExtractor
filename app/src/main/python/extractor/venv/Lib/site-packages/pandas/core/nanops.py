    ...
    def __call__(self, f: F) -> F:
        ...
        def _f(*args, **kwargs):
            ...
            if any(self.check(obj) for obj in obj_iter):
                ...
                raise TypeError(
                    f"reduction operation '{f_name***REMOVED***' not allowed for this dtype"
    ***REMOVED***
            try:
    ...
    if values.dtype.kind != "f":
        if values.dtype == object:
            ...
            if inferred in ["string", "mixed"]:
                raise TypeError(f"Cannot convert {values***REMOVED*** to numeric")
        try:
...
def _nanminmax(meth, fill_value_typ):
    @bottleneck_switch(name=f"nan{meth***REMOVED***")
    @_datetimelike_compat
    def reduction(
        values: np.ndarray,
        *,
...
def get_corr_func(
    ...
    raise ValueError(
        f"Unknown method '{method***REMOVED***', expected one of "
    ...
    if isinstance(x, np.ndarray):
        if x.dtype.kind in "biu":
            ...
            if inferred in ["string", "mixed"]:
                ...
                raise TypeError(f"Could not convert {x***REMOVED*** to numeric")
            try:
                ...
                try:
                    ...
                    raise TypeError(f"Could not convert {x***REMOVED*** to numeric") from err
            else:
        ...
        if isinstance(x, str):
            # GH#44008, GH#36703 avoid casting e.g. strings to numeric
            raise TypeError(f"Could not convert string '{x***REMOVED***' to numeric")
        try:
            x = float(x)
        except (TypeError, ValueError):
            # e.g. "1+1j" or "foo"
...
def na_accum_func(values: ArrayLike, accum_func, *, skipna: bool) -> ArrayLike:
    ...
    accum_func : {np.cumprod, np.maximum.accumulate, np.cumsum, np.minimum.accumulate***REMOVED***
    skipna : bool
