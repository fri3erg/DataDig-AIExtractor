...
def test_path__partition_provided_as_dict_of_lists():
    G = nx.path_graph(6)
    partition = {0: [0, 1], 2: [2, 3], 4: [4, 5]***REMOVED***
    M = nx.quotient_graph(G, partition, relabel=True)
    assert nodes_equal(M, [0, 1, 2])
    assert edges_equal(M.edges(), [(0, 1), (1, 2)])
    for n in M:
...
def test_overlapping_blocks():
    with pytest.raises(nx.NetworkXException):
        G = nx.path_graph(6)
        partition = [{0, 1, 2***REMOVED***, {2, 3***REMOVED***, {4, 5***REMOVED***]
...
def test_node_attributes():
    ...
    expected = nx.complete_graph(3)
    expected = nx.relabel_nodes(expected, {1: 2, 2: 3***REMOVED***)
    expected.add_edge(0, 0)
    cdict = {1: {"baz": "xyzzy"***REMOVED******REMOVED***
    expected.nodes[0].update({"foo": "bar", "contraction": cdict***REMOVED***)
    assert nx.is_isomorphic(actual, expected)
