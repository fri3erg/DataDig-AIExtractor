...
10|13|10.
"""
    expected = DataFrame({"A": [1, 10], "B": [2334, 13], "C": [5, 10.0]***REMOVED***)
...
"""
    parser = all_parsers
    expected = DataFrame({"A": ["a", "b", "c"], "B": [1, 3, 4], "C": [2, 4, 5]***REMOVED***)
    result = parser.read_csv(StringIO(data))
    tm.assert_frame_equal(result, expected)
...
7,8,9,"""
    ...
    expected = DataFrame({"A": [1, 4, 7], "B": [2, 5, 8], "C": [3, 6, 9]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
8,9,10"""
    ...
    if usecols is None:
        ...
        expected = DataFrame({"a": [0, 3, 8], "b": [1, 4, 9]***REMOVED***)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("", {***REMOVED***, None),
        ("", {"usecols": ["X"]***REMOVED***, None),
        (
            ",,",
            {"names": ["Dummy", "X", "Dummy_2"], "usecols": ["X"]***REMOVED***,
        ...
        (
            {
                ...
                "skip_blank_lines": True,
            ***REMOVED***,
            DataFrame({"A": [1.0, 5.1], "B": [2.0, np.nan], "C": [4.0, 10]***REMOVED***),
        ),
    ],
)
...
b\n"""
    ...
    expected = DataFrame({"MyColumn": list("abab")***REMOVED***)
    depr_msg = "The 'delim_whitespace' keyword in pd.read_csv is deprecated"
...
def test_read_csv_and_table_sys_setprofile(all_parsers, read_func):
    ...
    expected = DataFrame({"a b": ["0 1"]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
***REMOVED***:
    ...
    expected = DataFrame({"a": [1, 4], "b": [-2, 5], "c": [-3, 6]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_encoding_surrogatepass(all_parsers):
    ...
    expected = DataFrame({decoded: [decoded]***REMOVED***, index=[decoded * 2])
...
def test_malformed_second_line(all_parsers):
    ...
    expected = DataFrame({"a": ["b"]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_short_single_line(all_parsers):
    ...
    expected = DataFrame({"a": [1], "b": [2], "c": [np.nan]***REMOVED***)
    tm.assert_frame_equal(result, expected)
...
def test_short_multi_line(all_parsers):
    ...
    data = "1,2\n1,2"
    result = parser.read_csv(StringIO(data), header=None, names=columns)
    expected = DataFrame({"a": [1, 1], "b": [2, 2], "c": [np.nan, np.nan]***REMOVED***)
    tm.assert_frame_equal(result, expected)
