...
def test_map_compat():
    ...
    result = s.map({True: "foo", False: "bar"***REMOVED***)
    expected = Series(["foo", "foo", "bar"], index=[1, 2, 3])
    tm.assert_series_equal(result, expected)


def test_map_int():
    left = Series({"a": 1.0, "b": 2.0, "c": 3.0, "d": 4***REMOVED***)
    right = Series({1: 11, 2: 22, 3: 33***REMOVED***)
...
def test_map_dict_with_tuple_keys():
    ...
    """
    # GH 18496
    df = DataFrame({"a": [(1,), (2,), (3, 4), (5, 6)]***REMOVED***)
    label_mappings = {(1,): "A", (2,): "B", (3, 4): "A", (5, 6): "B"***REMOVED***
...
def test_map_dict_na_key():
    ...
    result = s.map({1: "a", 2: "b", np.nan: "c"***REMOVED***)
    tm.assert_series_equal(result, expected)
...
def test_map_defaultdict_na_key(na_action):
    # GH 48813
    s = Series([1, 2, np.nan])
    default_map = defaultdict(lambda: "missing", {1: "a", 2: "b", np.nan: "c"***REMOVED***)
    result = s.map(default_map, na_action=na_action)
    expected = Series({0: "a", 1: "b", 2: "c" if na_action is None else np.nan***REMOVED***)
    tm.assert_series_equal(result, expected)
...
def test_map_defaultdict_missing_key(na_action):
    # GH 48813
    s = Series([1, 2, np.nan])
    default_map = defaultdict(lambda: "missing", {1: "a", 2: "b", 3: "c"***REMOVED***)
    result = s.map(default_map, na_action=na_action)
    expected = Series({0: "a", 1: "b", 2: "missing" if na_action is None else np.nan***REMOVED***)
    tm.assert_series_equal(result, expected)
...
def test_map_dict_ignore_na(arg_func):
    # GH#47527
    mapping = arg_func({1: 10, np.nan: 42***REMOVED***)
...
def test_map_defaultdict_ignore_na():
    # GH#47527
    mapping = defaultdict(int, {1: 10, np.nan: 42***REMOVED***)
    ser = Series([1, np.nan, 2])
...
def test_map_categorical_na_ignore(na_action, expected):
    ...
    ser = Series(values)
    result = ser.map({1: 10, np.nan: 42***REMOVED***, na_action=na_action)
    tm.assert_series_equal(result, expected)
...
def test_map_dict_subclass_with_missing():
    ...
    s = Series([1, 2, 3])
    dictionary = DictWithMissing({3: "three"***REMOVED***)
...
def test_map_dict_subclass_without_missing():
    ...
    s = Series([1, 2, 3])
    dictionary = DictWithoutMissing({3: "three"***REMOVED***)
    result = s.map(dictionary)
...
def test_map_abc_mapping(non_dict_mapping_subclass):
    ...
    s = Series([1, 2, 3])
    not_a_dictionary = non_dict_mapping_subclass({3: "three"***REMOVED***)
...
def test_map_abc_mapping_with_missing(non_dict_mapping_subclass):
    ...
    s = Series([1, 2, 3])
    not_a_dictionary = NonDictMappingWithMissing({3: "three"***REMOVED***)
    result = s.map(not_a_dictionary)
...
def test_map_box_dt64(unit):
    ...
    assert ser.dtype == f"datetime64[{unit***REMOVED***]"
    # boxed value must be Timestamp instance
    res = ser.map(lambda x: f"{type(x).__name__***REMOVED***_{x.day***REMOVED***_{x.tz***REMOVED***")
...
def test_map_box_dt64tz(unit):
    ...
    ser = Series(vals).dt.as_unit(unit)
    assert ser.dtype == f"datetime64[{unit***REMOVED***, US/Eastern]"
    res = ser.map(lambda x: f"{type(x).__name__***REMOVED***_{x.day***REMOVED***_{x.tz***REMOVED***")
...
def test_map_box_td64(unit):
    ...
    ser = Series(vals).dt.as_unit(unit)
    assert ser.dtype == f"timedelta64[{unit***REMOVED***]"
    res = ser.map(lambda x: f"{type(x).__name__***REMOVED***_{x.days***REMOVED***")
...
def test_map_box_period():
    ...
    res = ser.map(lambda x: f"{type(x).__name__***REMOVED***_{x.freqstr***REMOVED***")
...
@pytest.mark.parametrize(
    "vals,mapping,exp",
    [
        (list("abc"), {np.nan: "not NaN"***REMOVED***, [np.nan] * 3 + ["not NaN"]),
        (list("abc"), {"a": "a letter"***REMOVED***, ["a letter"] + [np.nan] * 3),
        (list(range(3)), {0: 42***REMOVED***, [42] + [np.nan] * 3),
    ],
)
def test_map_missing_mixed(vals, mapping, exp, using_infer_string):
    ...
    exp = Series(exp)
    if using_infer_string and mapping == {np.nan: "not NaN"***REMOVED***:
        exp.iloc[-1] = np.nan
    tm.assert_series_equal(result, exp)
...
def test_map_float_to_string_precision():
    ...
    expected = {0: "0.3333333333333333"***REMOVED***
