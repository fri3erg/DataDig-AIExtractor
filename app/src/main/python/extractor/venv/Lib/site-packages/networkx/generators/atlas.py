...
#:             f.write(bytes(f'GRAPH {i***REMOVED***\n', encoding='utf-8'))
#:             f.write(bytes(f'NODES {len(G)***REMOVED***\n', encoding='utf-8'))
    ...
    with gzip.open(ATLAS_FILE, "rb") as f:
        ...
        while line and line.startswith(b"GRAPH"):
            ...
            while line and not line.startswith(b"GRAPH"):
                ...
                line = f.readline()
            G = nx.Graph()
            G.name = f"G{graph_index***REMOVED***"
...
def graph_atlas(i):
    ...
    if not (0 <= i < NUM_GRAPHS):
        raise ValueError(f"index must be between 0 and {NUM_GRAPHS***REMOVED***")
