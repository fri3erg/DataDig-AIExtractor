...
def deprecate(
    ...
    msg : str
        ...
        Default is '{name***REMOVED*** is deprecated. Use {alt_name***REMOVED*** instead.'
    """
    alt_name = alt_name or alternative.__name__
    klass = klass or FutureWarning
    warning_msg = msg or f"{name***REMOVED*** is deprecated, use {alt_name***REMOVED*** instead."
    ...
    msg = msg or f"Use `{alt_name***REMOVED***` instead."
    doc_error_msg = (
...
def deprecate_kwarg(
    ...
    >>> @deprecate_kwarg('old', 'new', {'yes': True, 'no': False***REMOVED***)
    ... def f(new=False):
        ...
        def wrapper(*args, **kwargs) -> Callable[..., Any]:
            ...
            if old_arg_value is not None:
                if new_arg_name is None:
                    msg = (
                        f"the {repr(old_arg_name)***REMOVED*** keyword is deprecated and "
                        "will be removed in a future version. Please take "
                        f"steps to stop the use of {repr(old_arg_name)***REMOVED***"
        ***REMOVED***
                    ...
                    msg = (
                        f"the {old_arg_name***REMOVED***={repr(old_arg_value)***REMOVED*** keyword is "
                        "deprecated, use "
                        f"{new_arg_name***REMOVED***={repr(new_arg_value)***REMOVED*** instead."
        ***REMOVED***
                else:
                    new_arg_value = old_arg_value
                    msg = (
                        f"the {repr(old_arg_name)***REMOVED*** keyword is deprecated, "
                        f"use {repr(new_arg_name)***REMOVED*** instead."
        ***REMOVED***

                warnings.warn(msg, FutureWarning, stacklevel=stacklevel)
                if kwargs.get(new_arg_name) is not None:
                    msg = (
                        f"Can only specify {repr(old_arg_name)***REMOVED*** "
                        f"or {repr(new_arg_name)***REMOVED***, not both."
        ***REMOVED***
                    raise TypeError(msg)
...
def _format_argument_list(allow_args: list[str]) -> str:
    ...
    if not allow_args:
        ...
        return f" except for the argument '{allow_args[0]***REMOVED***'"
    else:
        last = allow_args[-1]
        args = ", ".join(["'" + x + "'" for x in allow_args[:-1]])
        return f" except for the arguments {args***REMOVED*** and '{last***REMOVED***'"
...
def future_version_msg(version: str | None) -> str:
    ...
    if version is None:
        ...
        return f"Starting with pandas version {version***REMOVED***"
...
***REMOVED***
        ...
        msg = (
            f"{future_version_msg(version)***REMOVED*** all arguments of "
            f"{name or func.__qualname__***REMOVED***{{arguments***REMOVED******REMOVED*** will be keyword-only."
        )
