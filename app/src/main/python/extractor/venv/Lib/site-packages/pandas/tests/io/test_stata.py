...
class TestStata:
    ...
    def test_read_index_col_none(self, version):
        df = DataFrame({"a": range(5), "b": ["b1", "b2", "b3", "b4", "b5"]***REMOVED***)
        # GH 7369, make sure can read a 0-obs dta file
        with tm.ensure_clean() as path:
    ...
    def test_read_dta1(self, file, datapath):
        file = datapath("io", "data", "stata", f"{file***REMOVED***.dta")
...
***REMOVED***
    ...
    def test_read_write_dta10(self, version):
        ...
        with tm.ensure_clean() as path:
            original.to_stata(path, convert_dates={"datetime": "tc"***REMOVED***, version=version)
            written_and_read_again = self.read_dta(path)
...
***REMOVED***
    ...
    def test_read_write_dta13(self):
        ...
        original = DataFrame({"int16": s1, "int32": s2, "int64": s3***REMOVED***)
        original.index.name = "index"
    ...
    def test_read_write_reread_dta14(self, file, parsed_114, version, datapath):
        ...
        with tm.ensure_clean() as path:
            parsed_114.to_stata(path, convert_dates={"date_td": "td"***REMOVED***, version=version)
            written_and_read_again = self.read_dta(path)
...
***REMOVED***
    ...
    def test_nan_to_missing_value(self, version):
        ...
        s1[::2] = np.nan
        s2[1::2] = np.nan
        original = DataFrame({"s1": s1, "s2": s2***REMOVED***)
        original.index.name = "index"
    ...
    def test_large_value_conversion(self):
        ...
        original = DataFrame({"s0": s0, "s1": s1, "s2": s2, "s3": s3***REMOVED***)
        original.index.name = "index"
        with tm.ensure_clean() as path:
    ...
    def test_dates_invalid_column(self):
        ...
        with tm.ensure_clean() as path:
            with tm.assert_produces_warning(InvalidColumnName):
                original.to_stata(path, convert_dates={0: "tc"***REMOVED***)
    ...
    def test_value_labels_old_format(self, datapath):
        ...
        with StataReader(dpath) as reader:
            assert reader.value_labels() == {***REMOVED***

    def test_date_export_formats(self):
        columns = ["tc", "td", "tw", "tm", "tq", "th", "ty"]
        conversions = {c: c for c in columns***REMOVED***
    ...
    def test_bool_uint(self, byteorder, version):
        ...
        original = DataFrame(
            {"s0": s0, "s1": s1, "s2": s2, "s3": s3, "s4": s4, "s5": s5, "s6": s6***REMOVED***
        )
        original.index.name = "index"
        expected = original.copy()
...
***REMOVED***
    ...
    def test_missing_value_conversion(self, file, datapath):
        ...
        parsed = read_stata(
            datapath("io", "data", "stata", f"{file***REMOVED***.dta"), convert_missing=True
        )
        tm.assert_frame_equal(parsed, expected)
    ...
    def test_big_dates(self, datapath):
        ...
        date_conversion = {c: c[-2:] for c in columns***REMOVED***
        # {c : c[-2:] for c in columns***REMOVED***
        with tm.ensure_clean() as path:
...
***REMOVED***
    ...
    ***REMOVED***
    ...
    def test_categorical_sorting(self, file, datapath):
        parsed = read_stata(datapath("io", "data", "stata", f"{file***REMOVED***.dta"))
    ...
    def test_read_chunks_117(
        ...
        fname = datapath("io", "data", "stata", f"{file***REMOVED***.dta")
    ...
    ***REMOVED***
    ...
    def test_write_variable_labels(self, version, mixed_frame):
        ...
        variable_labels = {"a": "City Rank", "b": "City Exponent", "c": "City"***REMOVED***
        with tm.ensure_clean() as path:
    ...
    def test_invalid_variable_labels(self, version, mixed_frame):
        mixed_frame.index.name = "index"
        variable_labels = {"a": "very long" * 10, "b": "City Exponent", "c": "City"***REMOVED***
        with tm.ensure_clean() as path:
    ...
    ***REMOVED***
    ...
    def test_default_date_conversion(self):
        ...
        with tm.ensure_clean() as path:
            ...
            original.to_stata(path, write_index=False, convert_dates={"dates": "tc"***REMOVED***)
            ...
            original.to_stata(path, write_index=False, convert_dates={dates_idx: "tc"***REMOVED***)
            direct = read_stata(path, convert_dates=True)
            tm.assert_frame_equal(reread, direct)

    def test_unsupported_type(self):
        original = DataFrame({"a": [1 + 2j, 2 + 4j]***REMOVED***)
    ...
    def test_unsupported_datetype(self):
        ...
        with pytest.raises(NotImplementedError, match=msg):
            with tm.ensure_clean() as path:
                original.to_stata(path, convert_dates={"dates": "tC"***REMOVED***)
...
***REMOVED***
    ...
    def test_inf(self, infval):
        # GH 45350
        df = DataFrame({"WithoutInf": [0.0, 1.0], "WithInf": [2.0, infval]***REMOVED***)
        msg = (
    ...
    def test_path_pathlib(self):
        df = DataFrame(
            ...
            index=pd.Index([f"i-{i***REMOVED***" for i in range(30)], dtype=object),
        )
        df.index.name = "index"
    ...
    def test_value_labels_iterator(self, write_index):
        ...
        assert value_labels == {"A": {0: "A", 1: "B", 2: "C", 3: "E"***REMOVED******REMOVED***
    ...
    def test_writer_117(self):
        ...
        with tm.ensure_clean() as path:
            original.to_stata(
                path,
                convert_dates={"datetime": "tc"***REMOVED***,
...
***REMOVED***
    ...
    def test_invalid_date_conversion(self):
        ...
        with tm.ensure_clean() as path:
            ...
            with pytest.raises(ValueError, match=msg):
                original.to_stata(path, convert_dates={"wrong_name": "tc"***REMOVED***)
    ...
    def test_mixed_string_strl(self):
        # GH 23633
        output = [{"mixed": "string" * 500, "number": 0***REMOVED***, {"mixed": None, "number": 1***REMOVED***]
...
***REMOVED***
    ...
    def test_all_none_exception(self, version):
        output = [{"none": "none", "number": 0***REMOVED***, {"none": None, "number": 1***REMOVED***]
        output = DataFrame(output)
        output["none"] = None
        with tm.ensure_clean() as path:
...
***REMOVED***
            with pytest.raises(UnicodeEncodeError, match=f"{msg1***REMOVED***|{msg2***REMOVED***"):
                df.to_stata(path)
...
the string values returned are correct."""
    ...
    def test_utf8_writer(self, version):
        ...
        ***REMOVED***
        ...
        value_labels = {"β": {1: "label", 2: "æøå", 3: "ŋot valid latin-1"***REMOVED******REMOVED***
...
def test_backward_compat(version, datapath):
    ...
    old = os.path.join(data_base, f"stata-compat-{version***REMOVED***.dta")
...
def test_compression(compression, version, use_dict, infer, compression_to_extension):
    ...
    if compression:
        ...
        file_name += f".{file_ext***REMOVED***"
    compression_arg = compression
    if infer:
        compression_arg = "infer"
    if use_dict:
        compression_arg = {"method": compression***REMOVED***
...
def test_compression_dict(method, file_ext):
    file_name = f"test.{file_ext***REMOVED***"
    ...
    df.index.name = "index"
    with tm.ensure_clean(file_name) as path:
        compression = {"method": method, "archive_name": archive_name***REMOVED***
...
def test_chunked_categorical(version):
    df = DataFrame({"cats": Series(["a", "b", "a", "b", "c"], dtype="category")***REMOVED***)
    df.index.name = "index"
...
def test_iterator_value_labels():
    ...
    df = DataFrame({f"col{k***REMOVED***": pd.Categorical(values, ordered=True) for k in range(2)***REMOVED***)
    with tm.ensure_clean() as path:
        df.to_stata(path, write_index=False)
...
def test_non_categorical_value_labels():
    ...
    with tm.ensure_clean() as path:
        value_labels = {
            "fully_labelled": {1: "one", 2: "two", 3: "three"***REMOVED***,
            "partially_labelled": {1.0: "one", 2.0: "two"***REMOVED***,
        ***REMOVED***
        expected = {**value_labels, "Z": {0: "j", 1: "k", 2: "l"***REMOVED******REMOVED***
        ...
        with pytest.raises(KeyError, match=msg):
            value_labels = {"notY": {7: "label1", 8: "label2"***REMOVED******REMOVED***
        ...
        with pytest.raises(ValueError, match=msg):
            value_labels = {"Z": {1: "a", 2: "k", 3: "j", 4: "i"***REMOVED******REMOVED***
            StataWriter(path, data, value_labels=value_labels)
...
def test_non_categorical_value_label_name_conversion():
    ...
    value_labels = {
        "invalid~!": {1: "label1", 2: "label2"***REMOVED***,
        "6_invalid": {1: "label1", 2: "label2"***REMOVED***,
        "invalid_name_longer_than_32_characters": {8: "eight", 9: "nine"***REMOVED***,
        "aggregate": {5: "five"***REMOVED***,
        (1, 2): {3: "three"***REMOVED***,
    ***REMOVED***

    expected = {
        "invalid__": {1: "label1", 2: "label2"***REMOVED***,
        "_6_invalid": {1: "label1", 2: "label2"***REMOVED***,
        "invalid_name_longer_than_32_char": {8: "eight", 9: "nine"***REMOVED***,
        "_aggregate": {5: "five"***REMOVED***,
        "_1__2_": {3: "three"***REMOVED***,
    ***REMOVED***
...
def test_non_categorical_value_label_convert_categoricals_error():
    ...
    value_labels = {
        "repeated_labels": {10: "Ten", 20: "More than ten", 40: "More than ten"***REMOVED***
    ***REMOVED***
...
def test_empty_frame():
    ...
    df = DataFrame(data={"a": range(3), "b": [1.0, 2.0, 3.0]***REMOVED***).head(0)
    with tm.ensure_clean() as path:
        ...
        dtypes = Series({"a": np.dtype("int32"), "b": np.dtype("float64")***REMOVED***)
