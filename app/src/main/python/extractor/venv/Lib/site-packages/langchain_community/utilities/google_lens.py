...
class GoogleLensAPIWrapper(BaseModel):
    ...
    def run(self, query: str) -> str:
        ...
        params = {
            ...
            "url": query,
        ***REMOVED***
        queryURL = f"https://serpapi.com/search?engine={params['engine']***REMOVED***&api_key={params['api_key']***REMOVED***&url={params['url']***REMOVED***"
        ...
        if len(responseValue["knowledge_graph"]) > 0:
            ...
            xs += f"Subject:{subject['title']***REMOVED***({subject['subtitle']***REMOVED***)\n"
            xs += f"Link to subject:{subject['link']***REMOVED***\n\n"
        xs += "Related Images:\n\n"
        for image in responseValue["visual_matches"]:
            xs += f"Title: {image['title']***REMOVED***\n"
            xs += f"Source({image['source']***REMOVED***): {image['link']***REMOVED***\n"
            xs += f"Image: {image['thumbnail']***REMOVED***\n\n"
        xs += (
            "Reverse Image Search"
            + f"Link: {responseValue['reverse_image_search']['link']***REMOVED***\n"
        )
        print(xs)
