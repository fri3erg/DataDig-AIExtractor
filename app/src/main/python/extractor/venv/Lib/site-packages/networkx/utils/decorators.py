...
def not_implemented_for(*graph_types):
    ...
    if not set(graph_types) < {"directed", "undirected", "multigraph", "graph"***REMOVED***:
        raise KeyError(
            "use one or more of directed, undirected, multigraph, graph.  "
            f"You used {graph_types***REMOVED***"
        )
    ...
    errmsg = f"not implemented for {' '.join(graph_types)***REMOVED*** type"
...
def nodes_or_number(which_args):
    ...
    def _nodes_or_number(n):
        try:
            ...
            if n < 0:
                raise nx.NetworkXError(f"Negative number of nodes not valid: {n***REMOVED***")
        return (n, nodes)
...
class argmap:
    ...
    *args : iterable of (int, str or tuple)
    ...
    The decorator (named `convert_to`) would be used like::
        ...
        def show_me_the_money(name, income):
            print(f"{name***REMOVED*** : {income***REMOVED***")
    ...
    **Try Finally**
    ...
    def __call__(self, f):
        ...
        func.__kwdefaults__.update(f.__kwdefaults__ or {***REMOVED***)
    ...
    def _name(cls, f):
        ...
        return f"argmap_{fname***REMOVED***_{cls._count()***REMOVED***"
    ...
    def compile(self, f):
        ...
        call = f"{sig.call_sig.format(wrapped_name)***REMOVED***#"
        mut_args = f"{sig.args***REMOVED*** = list({sig.args***REMOVED***)" if mutable_args else ""
        body = argmap._indent(sig.def_sig, mut_args, mapblock, call, finallys)
        code = "\n".join(body)

        locl = {***REMOVED***
        globl = dict(functions.values())
        filename = f"{self.__class__***REMOVED*** compilation {self._count()***REMOVED***"
...
***REMOVED*** = f.__argmap__.assemble(f.__wrapped__)
        ...
        def get_name(arg, first=True):
            ...
            if isinstance(arg, tuple):
                ...
                return name if first else f"({name***REMOVED***)"
            if arg in applied:
                raise nx.NetworkXError(f"argument {arg***REMOVED*** is specified multiple times")
            applied.add(arg)
            if arg in sig.names:
                ...
                if sig.kwargs is None:
                    raise nx.NetworkXError(
                        f"name {arg***REMOVED*** is not a named parameter and this function doesn't have kwargs"
        ***REMOVED***
                return f"{sig.kwargs***REMOVED***[{arg!r***REMOVED***]"
            else:
                if sig.args is None:
                    raise nx.NetworkXError(
                        f"index {arg***REMOVED*** not a parameter index and this function doesn't have args"
        ***REMOVED***
                mutable_args = True
                return f"{sig.args***REMOVED***[{arg - sig.n_positional***REMOVED***]"
        ...
        if self._finally:
            ...
            for a in self._args:
                ...
                mapblock.append(f"{name***REMOVED***, {final***REMOVED*** = {fname***REMOVED***({name***REMOVED***)")
                mapblock.append("try:")
                finallys = ["finally:", f"{final***REMOVED***()#", "#", finallys]
        else:
            mapblock.extend(
                f"{name***REMOVED*** = {fname***REMOVED***({name***REMOVED***)" for name in map(get_name, self._args)
***REMOVED***
    ...
    def signature(cls, f):
        ...
        for i, param in enumerate(sig.parameters.values()):
            ...
            if kind == param.KEYWORD_ONLY:
                call_sig.append(f"{name***REMOVED*** = {name***REMOVED***")
            else:
                npos += count
        ...
        fname = cls._name(f)
        def_sig = f'def {fname***REMOVED***({", ".join(def_sig)***REMOVED***):'

        call_sig = f"return {{***REMOVED******REMOVED***({', '.join(call_sig)***REMOVED***)"
    ...
    def _indent(*lines):
        ...
        *lines : lists and/or strings
        ...
        for line in argmap._flatten(lines, set()):
            yield f"{argmap._tabs[:depth]***REMOVED***{line***REMOVED***"
            depth += (line[-1:] == ":") - (line[-1:] == "#")
    ...
    * will issue a warning when passed as a positional argument.
    ...
    def _inner_deprecate_positional_args(f):
        ...
        def inner_f(*args, **kwargs):
            ...
            # extra_args > 0
            args_msg = [
                f"{name***REMOVED***={arg***REMOVED***"
                for name, arg in zip(kwonly_args[:extra_args], args[-extra_args:])
            ]
            args_msg = ", ".join(args_msg)
            warnings.warn(
                (
                    f"Pass {args_msg***REMOVED*** as keyword args. From NetworkX version "
                    f"{version***REMOVED*** passing these as positional arguments "
