...
def data_for_twos(dtype):
    if dtype.kind == "O":
        pytest.skip(f"{dtype***REMOVED*** is not a numeric dtype")
    arr = np.ones(100) * 2
        ...
        if data.dtype.numpy_dtype == "object":
            request.applymarker(
                pytest.mark.xfail(
                    ...
                    f"NumPy name: {data.dtype.numpy_dtype***REMOVED***"
    ***REMOVED***
***REMOVED***
    ...
    ***REMOVED***
    ...
    def test_setitem_with_expansion_dataframe_column(self, data, full_indexer):
        ...
        df = expected = pd.DataFrame({"data": pd.Series(data)***REMOVED***)
        ...
        if data.dtype.numpy_dtype != object:
            if not isinstance(key, slice) or key != slice(None):
                expected = pd.DataFrame({"data": data.to_numpy()***REMOVED***)
