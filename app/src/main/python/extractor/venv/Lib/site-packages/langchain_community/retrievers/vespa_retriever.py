...
class VespaRetriever(BaseRetriever):
    ...
    def _query(self, body: Dict) -> List[Document]:
        ...
        if not str(response.status_code).startswith("2"):
            raise RuntimeError(
                "Could not retrieve data from Vespa. Error code: {***REMOVED***".format(
...
***REMOVED***
        ...
        for child in response.hits:
            ...
            if self.metadata_fields == "*":
                ...
                metadata = {mf: child["fields"].get(mf) for mf in self.metadata_fields***REMOVED***
    ...
    def get_relevant_documents_with_filter(
        ...
        body = self.body.copy()
        _filter = f" and {_filter***REMOVED***" if _filter else ""
...
***REMOVED***
            ...
            _filter = f" and {_filter***REMOVED***" if _filter else ""
            yql = f"select {_fields***REMOVED*** from sources {_sources***REMOVED*** where userQuery(){_filter***REMOVED***"
        body["yql"] = yql
        if k:
            body["hits"] = k
        return cls(
