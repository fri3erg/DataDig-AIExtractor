...
class TestNonNano:
    ...
    def dtype(self, unit, tz_naive_fixture):
        tz = tz_naive_fixture
        if tz is None:
            return np.dtype(f"datetime64[{unit***REMOVED***]")
        else:
    ...
    def dta_dti(self, unit, dtype):
        ...
        if tz is None:
            arr = np.asarray(dti).astype(f"M8[{unit***REMOVED***]")
        else:
...
***REMOVED***
    ...
    def test_non_nano(self, unit, dtype):
        arr = np.arange(5, dtype=np.int64).view(f"M8[{unit***REMOVED***]")
        dta = DatetimeArray._simple_new(arr, dtype=dtype)
    ...
    def test_normalize(self, unit):
        ...
        exp = np.asarray(dti.normalize()).astype(f"M8[{unit***REMOVED***]")
    ...
    def test_repr(self, dta_dti, unit):
        ...
        assert repr(dta) == repr(dti._data).replace("[ns", f"[{unit***REMOVED***")
