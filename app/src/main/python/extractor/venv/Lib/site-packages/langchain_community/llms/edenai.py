...
class EdenAI(LLM):
    ...
    passed in with the format params={model_param: value, ...***REMOVED***
    ...
    def build_extra(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field_name in list(values):
            if field_name not in all_required_field_names:
                ...
                logger.warning(
                    f"""{field_name***REMOVED*** was transferred to model_kwargs.
    ...
    ***REMOVED***
    ...
    def get_user_agent() -> str:
        ...
        return f"langchain/{__version__***REMOVED***"
...
***REMOVED***
        ...
        url = f"{self.base_url***REMOVED***/{self.feature***REMOVED***/{self.subfeature***REMOVED***"
        headers = {
            "Authorization": f"Bearer {self.edenai_api_key***REMOVED***",
        ...
        ***REMOVED***
        ...
        payload = {k: v for k, v in payload.items() if v is not None***REMOVED***

        if self.model is not None:
            payload["settings"] = {self.provider: self.model***REMOVED***

        request = Requests(headers=headers)
        response = request.post(url=url, data=payload)

        if response.status_code >= 500:
            raise Exception(f"EdenAI Server: Error {response.status_code***REMOVED***")
        elif response.status_code >= 400:
            raise ValueError(f"EdenAI received an invalid payload: {response.text***REMOVED***")
        elif response.status_code != 200:
            raise Exception(
                f"EdenAI returned an unexpected response with status "
                f"{response.status_code***REMOVED***: {response.text***REMOVED***"
***REMOVED***

        data = response.json()
        provider_response = data[self.provider]
        if provider_response.get("status") == "fail":
            err_msg = provider_response.get("error", {***REMOVED***).get("message")
        ...
        async with ClientSession() as session:
            async with session.post(url, json=payload, headers=headers) as response:
                if response.status >= 500:
                    raise Exception(f"EdenAI Server: Error {response.status***REMOVED***")
                elif response.status >= 400:
                    raise ValueError(
                        f"EdenAI received an invalid payload: {response.text***REMOVED***"
        ***REMOVED***
                elif response.status != 200:
                    raise Exception(
                        f"EdenAI returned an unexpected response with status "
                        f"{response.status***REMOVED***: {response.text***REMOVED***"
        ***REMOVED***
