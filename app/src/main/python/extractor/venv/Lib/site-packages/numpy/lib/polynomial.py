...
def poly(seq_of_zeros):
    ...
    matrix **A** is given by
        ...
        :math:`p_a(t) = \\mathrm{det***REMOVED***(t\\, \\mathbf{I***REMOVED*** - \\mathbf{A***REMOVED***)`,
...
def polyint(p, m=1, k=None):
    ...
    :math:`\\frac{d^m***REMOVED***{dx^m***REMOVED***P(x) = p(x)` and is defined up to `m - 1`
    integration constants `k`. The constants determine the low-order
    polynomial part

    .. math:: \\frac{k_{m-1***REMOVED******REMOVED***{0!***REMOVED*** x^0 + \\ldots + \\frac{k_0***REMOVED***{(m-1)!***REMOVED***x^{m-1***REMOVED***

    of `P` so that :math:`P^{(j)***REMOVED***(0) = k_{m-j-1***REMOVED***`.
...
def polyfit(x, y, deg, rcond=None, full=False, w=None, cov=False):
    ...
    .. math::
        E = \\sum_{j=0***REMOVED***^k |p(x_j) - y_j|^2
...
def polydiv(u, v):
    ...
    Examples
    --------
    .. math:: \\frac{3x^2 + 5x + 2***REMOVED***{2x + 1***REMOVED*** = 1.5x + 1.75, remainder 0.25
