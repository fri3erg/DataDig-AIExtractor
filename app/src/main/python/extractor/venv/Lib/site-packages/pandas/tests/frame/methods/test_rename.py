...
class TestRename:
    ...
    def test_rename(self, float_frame):
        mapping = {"A": "a", "B": "b", "C": "c", "D": "d"***REMOVED***
        ...
        # index
        data = {"A": {"foo": 0, "bar": 1***REMOVED******REMOVED***

        df = DataFrame(data)
        renamed = df.rename(index={"foo": "bar", "bar": "foo"***REMOVED***)
        ...
        # partial columns
        renamed = float_frame.rename(columns={"C": "foo", "D": "bar"***REMOVED***)
        tm.assert_index_equal(renamed.columns, Index(["A", "B", "foo", "bar"]))

        # other axis
        renamed = float_frame.T.rename(index={"C": "foo", "D": "bar"***REMOVED***)
        tm.assert_index_equal(renamed.index, Index(["A", "B", "foo", "bar"]))

        # index with name
        index = Index(["foo", "bar"], name="name")
        renamer = DataFrame(data, index=index)
        renamed = renamer.rename(index={"foo": "bar", "bar": "foo"***REMOVED***)
    ...
    @pytest.mark.parametrize(
        "args,kwargs",
        [
            ((ChainMap({"A": "a"***REMOVED***, {"B": "b"***REMOVED***),), {"axis": "columns"***REMOVED***),
            ((), {"columns": ChainMap({"A": "a"***REMOVED***, {"B": "b"***REMOVED***)***REMOVED***),
        ],
    )
    def test_rename_chainmap(self, args, kwargs):
        ...
        df = DataFrame({"A": colAData, "B": colBdata***REMOVED***)
        result = df.rename(*args, **kwargs)

        expected = DataFrame({"a": colAData, "b": colBdata***REMOVED***)
    ...
    def test_rename_multiindex(self):
        ...
        renamed = df.rename(
            index={"foo1": "foo3", "bar2": "bar3"***REMOVED***,
            columns={"fizz1": "fizz3", "buzz2": "buzz3"***REMOVED***,
        )
        ...
        renamed = df.rename(columns={"fizz1": "fizz3", "buzz2": "buzz3"***REMOVED***, level=0)
        tm.assert_index_equal(renamed.columns, new_columns)
        renamed = df.rename(columns={"fizz1": "fizz3", "buzz2": "buzz3"***REMOVED***, level="fizz")
        tm.assert_index_equal(renamed.columns, new_columns)

        new_columns = MultiIndex.from_tuples(
            [("fizz1", "buzz1"), ("fizz2", "buzz3")], names=["fizz", "buzz"]
        )
        renamed = df.rename(columns={"fizz1": "fizz3", "buzz2": "buzz3"***REMOVED***, level=1)
        tm.assert_index_equal(renamed.columns, new_columns)
        renamed = df.rename(columns={"fizz1": "fizz3", "buzz2": "buzz3"***REMOVED***, level="buzz")
        tm.assert_index_equal(renamed.columns, new_columns)
        ...
        renamed = df.rename(index={"foo1": "foo3", "bar2": "bar3"***REMOVED***, level=0)
        tm.assert_index_equal(renamed.index, new_index)

    def test_rename_nocopy(self, float_frame, using_copy_on_write, warn_copy_on_write):
        renamed = float_frame.rename(columns={"C": "foo"***REMOVED***, copy=False)
    ...
    def test_rename_inplace(self, float_frame):
        float_frame.rename(columns={"C": "foo"***REMOVED***)
        assert "C" in float_frame
        assert "foo" not in float_frame

        c_values = float_frame["C"]
        float_frame = float_frame.copy()
        return_value = float_frame.rename(columns={"C": "foo"***REMOVED***, inplace=True)
    ...
    def test_rename_bug(self):
        ...
        df = DataFrame({0: ["foo", "bar"], 1: ["bah", "bas"], 2: [1, 2]***REMOVED***)
        df = df.rename(columns={0: "a"***REMOVED***)
        df = df.rename(columns={1: "b"***REMOVED***)
        df = df.set_index(["a", "b"])
...
***REMOVED***,
    ...
    def test_rename_bug2(self):
        ...
        df = df.rename({(1, 1): (5, 4)***REMOVED***, axis="index")
        expected = DataFrame(
    ...
    def test_rename_errors_raises(self):
        ...
        with pytest.raises(KeyError, match="'E'] not found in axis"):
            df.rename(columns={"A": "a", "E": "e"***REMOVED***, errors="raise")

    @pytest.mark.parametrize(
        "mapper, errors, expected_columns",
        [
            ({"A": "a", "E": "e"***REMOVED***, "ignore", ["a", "B", "C", "D"]),
            ({"A": "a"***REMOVED***, "raise", ["a", "B", "C", "D"]),
    ...
    def test_rename_axis_style(self):
        ...
        df = DataFrame({"A": [1, 2], "B": [1, 2]***REMOVED***, index=["X", "Y"])
        expected = DataFrame({"a": [1, 2], "b": [1, 2]***REMOVED***, index=["X", "Y"])
        ...
        result = df.rename({"A": "a", "B": "b"***REMOVED***, axis=1)
        tm.assert_frame_equal(result, expected)

        result = df.rename({"A": "a", "B": "b"***REMOVED***, axis="columns")
        tm.assert_frame_equal(result, expected)
        ...
        result = df.rename({"X": "x", "Y": "y"***REMOVED***, axis=0)
        tm.assert_frame_equal(result, expected)

        result = df.rename({"X": "x", "Y": "y"***REMOVED***, axis="index")
        tm.assert_frame_equal(result, expected)
    ...
    def test_rename_mapper_multi(self):
        df = DataFrame({"A": ["a", "b"], "B": ["c", "d"], "C": [1, 2]***REMOVED***).set_index(
            ["A", "B"]
        )
    ...
    def test_rename_axis_style_raises(self):
        # see gh-12392
        df = DataFrame({"A": [1, 2], "B": [1, 2]***REMOVED***, index=["0", "1"])
    ...
    def test_rename_mapper_and_positional_arguments_raises(self):
        ...
        with pytest.raises(TypeError, match=msg):
            df.rename({***REMOVED***, index={***REMOVED***)

        with pytest.raises(TypeError, match=msg):
            df.rename({***REMOVED***, columns={***REMOVED***)

        with pytest.raises(TypeError, match=msg):
            df.rename({***REMOVED***, columns={***REMOVED***, index={***REMOVED***)

    def test_rename_with_duplicate_columns(self):
        # GH#4403
        df4 = DataFrame(
            {"RT": [0.0454], "TClose": [22.02], "TExg": [0.0422]***REMOVED***,
            index=MultiIndex.from_tuples(
...
***REMOVED***,
        ...
        result = k.rename(columns={"TClose_x": "TClose", "TClose_y": "QT_Close"***REMOVED***)
    ...
    def test_rename_boolean_index(self):
        ...
        mapper = {0: "foo", 1: "bar", 2: "bah"***REMOVED***
