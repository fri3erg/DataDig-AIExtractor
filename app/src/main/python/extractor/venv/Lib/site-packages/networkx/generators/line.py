...
def line_graph(G, create_using=None):
    ...
    >>> G = nx.path_graph(4)
    >>> G.add_edges_from((u, v, {"tot": u+v***REMOVED***) for u, v in G.edges)
    >>> G.edges(data=True)
    EdgeDataView([(0, 1, {'tot': 1***REMOVED***), (1, 2, {'tot': 3***REMOVED***), (2, 3, {'tot': 5***REMOVED***)])
    >>> H = nx.line_graph(G)
    >>> H.add_nodes_from((node, G.edges[node]) for node in H)
    >>> H.nodes(data=True)
    NodeDataView({(0, 1): {'tot': 1***REMOVED***, (2, 3): {'tot': 5***REMOVED***, (1, 2): {'tot': 3***REMOVED******REMOVED***)
    ...
    *Self-loops in undirected graphs*
    ...
    written as a set `\{u, v\***REMOVED***`.  Its line graph `L` has the edges of `G` as
    its nodes. If `x` and `y` are two nodes in `L`, then `\{x, y\***REMOVED***` is an edge
...
def _lg_undirected(G, selfloops=False, create_using=None):
    ...
    the edge {u,v***REMOVED*** is connected to every node corresponding to an edge that
    involves u or v.
    ...
    node_index = {n: i for i, n in enumerate(G)***REMOVED***
    ...
    for u in G:
        ...
        # Decide on representation of {u, v***REMOVED*** as (u, v) or (v, u) depending on node_index.
...
def inverse_line_graph(G):
    ...
    References
    ----------
    .. [1] Roussopoulos, N.D. , "A max {m, n***REMOVED*** algorithm for determining the graph H from
    ...
    P_count = {u: 0 for u in G.nodes***REMOVED***
    for p in P:
        for u in p:
            P_count[u] += 1
...
def _triangles(G, e):
    ...
    u, v = e
    if u not in G:
        raise nx.NetworkXError(f"Vertex {u***REMOVED*** not in graph")
    if v not in G[u]:
        raise nx.NetworkXError(f"Edge ({u***REMOVED***, {v***REMOVED***) not in graph")
    triangle_list = []
    for x in G[u]:
        if x in G[v]:
...
def _odd_triangle(G, T):
    ...
    for e in list(combinations(T, 2)):
        if e[0] not in G[e[1]]:
            raise nx.NetworkXError(f"Edge ({e[0]***REMOVED***, {e[1]***REMOVED***) not in graph")
...
def _select_starting_cell(G, starting_edge=None):
    ...
    if starting_edge is None:
        ...
        if e[0] not in G.nodes():
            raise nx.NetworkXError(f"Vertex {e[0]***REMOVED*** not in graph")
        if e[1] not in G[e[0]]:
            msg = f"starting_edge ({e[0]***REMOVED***, {e[1]***REMOVED***) is not in the Graph"
            raise nx.NetworkXError(msg)
