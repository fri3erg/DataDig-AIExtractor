...
def _handle_date_column(
    ...
    if isinstance(format, dict):
        ...
        # {"errors": "coerce"***REMOVED*** or {"dayfirst": True***REMOVED***
...
def read_sql_table(
    ...
    parse_dates : list or dict, default None
        ...
        - Dict of ``{column_name: format string***REMOVED***`` where format string is
          strftime compatible in case of parsing string times or is one of
          (D, s, ns, ms, us) in case of parsing integer timestamps.
        - Dict of ``{column_name: arg dict***REMOVED***``, where the arg dict corresponds
    ...
    with pandasSQL_builder(con, schema=schema, need_transaction=True) as pandas_sql:
        if not pandas_sql.has_table(table_name):
            raise ValueError(f"Table {table_name***REMOVED*** not found")
    ...
    if table is not None:
        ...
        raise ValueError(f"Table {table_name***REMOVED*** not found", con)
...
def read_sql_query(
    ...
    params : list, tuple or mapping, optional, default: None
        ...
        Eg. for psycopg2, uses %(name)s so use params={'name' : 'value'***REMOVED***.
    parse_dates : list or dict, default: None
        - List of column names to parse as dates.
        - Dict of ``{column_name: format string***REMOVED***`` where format string is
          strftime compatible in case of parsing string times, or is one of
    ...
    dtype : Type name or dict of columns
        ...
        {'a': np.float64, 'b': np.int32, 'c': 'Int64'***REMOVED***.
...
def read_sql(
    ...
    ...             conn,
    ...             parse_dates={"date_column": {"format": "%d/%m/%y"***REMOVED******REMOVED***)
       int_column date_column
    0           0  2012-11-10
    1           1  2010-11-12
...
def to_sql(
    ...
    **engine_kwargs,
    ...
    engine : {'auto', 'sqlalchemy'***REMOVED***, default 'auto'
    ...
    **engine_kwargs
    ...
    if if_exists not in ("fail", "replace", "append"):
        raise ValueError(f"'{if_exists***REMOVED***' is not valid for if_exists")
...
class SQLTable(PandasObject):
    ...
    def __init__(
        ...
        if self.table is None:
            raise ValueError(f"Could not init table '{name***REMOVED***'")
    ...
    def create(self) -> None:
        if self.exists():
            if self.if_exists == "fail":
                raise ValueError(f"Table '{self.name***REMOVED***' already exists.")
            if self.if_exists == "replace":
                ...
                raise ValueError(f"'{self.if_exists***REMOVED***' is not valid for if_exists")
        else:
            self._execute_create()
    ...
    def insert_data(self) -> tuple[list[str], list[np.ndarray]]:
        if self.index is not None:
            ...
            try:
                ...
                raise ValueError(f"duplicate name in index/columns: {err***REMOVED***") from err
        else:
            temp = self.frame
    ...
    def insert(
        ...
        if method is None:
            ...
            raise ValueError(f"Invalid parameter `method`: {method***REMOVED***")
            ...
            if index_label is not None:
                ...
                if len(index_label) != nlevels:
                    raise ValueError(
                        ...
                        f"levels, which is {nlevels***REMOVED***"
        ***REMOVED***
                return index_label
...
***REMOVED***:
    ...
    ***REMOVED***:
    ...
    def _sqlalchemy_type(self, col: Index | Series):
        dtype: DtypeArg = self.dtype or {***REMOVED***
...
***REMOVED***|inf can not be used with MySQL"""
    ...
    def read_table(
        ...
        parse_dates : list or dict, default: None
            ...
            - Dict of ``{column_name: format string***REMOVED***`` where format string is
              strftime compatible in case of parsing string times, or is one of
              (D, s, ns, ms, us) in case of parsing integer timestamps.
            - Dict of ``{column_name: arg***REMOVED***``, where the arg corresponds
              to the keyword arguments of :func:`pandas.to_datetime`.
              Especially useful with databases without native Datetime support,
              such as SQLite.
        columns : list, default: None
    ...
    def read_query(
        ...
        parse_dates : list or dict, default: None
            ...
            - Dict of ``{column_name: format string***REMOVED***`` where format string is
              strftime compatible in case of parsing string times, or is one of
              (D, s, ns, ms, us) in case of parsing integer timestamps.
            - Dict of ``{column_name: arg dict***REMOVED***``, where the arg dict
    ...
    def prep_table(
        ...
        if dtype:
            if not is_dict_like(dtype):
                ...
                dtype = {col_name: dtype for col_name in frame***REMOVED***  # type: ignore[misc]
            else:
                dtype = cast(dict, dtype)
            ...
            for col, my_type in dtype.items():
                if isinstance(my_type, type) and issubclass(my_type, TypeEngine):
                    ...
                    raise ValueError(f"The type of {col***REMOVED*** is not a SQLAlchemy type")
    ...
    def check_case_sensitive(
        ...
        if not name.isdigit() and not name.islower():
            ...
            if name not in table_names:
                msg = (
                    f"The provided table name '{name***REMOVED***' is not found exactly as "
    ...
    ***REMOVED***
    ...
    def to_sql(
        ...
        **engine_kwargs,
        ...
        method : {None', 'multi', callable***REMOVED***, default None
            Controls the SQL insertion clause used:
    ...
    def execute(self, sql: str | Select | TextClause, params=None):
        ...
        try:
            ...
            try:
                ...
                ex = DatabaseError(
                    f"Execution failed on sql: {sql***REMOVED***\n{exc***REMOVED***\nunable to rollback"
    ***REMOVED***
                raise ex from inner_exc

            ex = DatabaseError(f"Execution failed on sql '{sql***REMOVED***': {exc***REMOVED***")
            raise ex from exc
...
***REMOVED***
        ...
        if columns:
            ...
            select_list = ", ".join(f'"{x***REMOVED***"' for x in to_select)
        else:
            select_list = "*"
        if schema:
            stmt = f"SELECT {select_list***REMOVED*** FROM {schema***REMOVED***.{table_name***REMOVED***"
        else:
            stmt = f"SELECT {select_list***REMOVED*** FROM {table_name***REMOVED***"
...
***REMOVED***
        ...
        if schema:
            table_name = f"{schema***REMOVED***.{name***REMOVED***"
        else:
            table_name = name
        ...
        if self.has_table(name, schema):
            if if_exists == "fail":
                raise ValueError(f"Table '{table_name***REMOVED***' already exists.")
            elif if_exists == "replace":
                with self.con.cursor() as cur:
                    cur.execute(f"DROP TABLE {table_name***REMOVED***")
...
def _get_unicode_name(name: object):
    try:
        ...
        raise ValueError(f"Cannot convert identifier to UTF-8: '{name***REMOVED***'") from err
    return uname
...
class SQLiteTable(SQLTable):
    ...
    def _register_date_adapters(self) -> None:
        ...
        def _adapt_time(t) -> str:
            ...
            return f"{t.hour:02d***REMOVED***:{t.minute:02d***REMOVED***:{t.second:02d***REMOVED***.{t.microsecond:06d***REMOVED***"
    ...
    def insert_statement(self, *, num_rows: int) -> str:
        ...
        wildcards = ",".join([f"({row_wildcards***REMOVED***)" for _ in range(num_rows)])
        insert_statement = (
            f"INSERT INTO {escape(self.name)***REMOVED*** ({col_names***REMOVED***) VALUES {wildcards***REMOVED***"
        )
    ...
    def _create_table_setup(self):
        ...
        if self.keys is not None and len(self.keys):
            ...
            create_tbl_stmts.append(
                f"CONSTRAINT {self.name***REMOVED***_pk PRIMARY KEY ({cnames_br***REMOVED***)"
***REMOVED***
        if self.schema:
...
***REMOVED***
        ...
        if dtype:
            ...
            for col, my_type in dtype.items():
                if not isinstance(my_type, str):
                    raise ValueError(f"{col***REMOVED*** ({my_type***REMOVED***) not a string")
    ...
    def drop_table(self, name: str, schema: str | None = None) -> None:
        drop_sql = f"DROP TABLE {_get_valid_sqlite_name(name)***REMOVED***"
