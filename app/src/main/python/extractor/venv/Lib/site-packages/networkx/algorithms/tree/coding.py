...
def to_nested_tuple(T, root, canonical_form=False):
    ...
    def _make_tuple(T, root, _parent):
        ...
        children = set(T[root]) - {_parent***REMOVED***
        if len(children) == 0:
            return ()
    ...
    if root not in T:
        raise nx.NodeNotFound(f"Graph {T***REMOVED*** contains no node {root***REMOVED***")
...
def from_nested_tuple(sequence, sensible_relabeling=False):
    ...
    if sensible_relabeling:
        ...
        labels = {v: i for i, v in enumerate(bfs_nodes)***REMOVED***
...
def to_prufer_sequence(T):
    ...
    *n* - 1, inclusive. The tree corresponding to a given Prüfer
    ...
    KeyError
        If the set of nodes in `T` is not {0, …, *n* - 1***REMOVED***.
    ...
    if set(T) != set(range(n)):
        raise KeyError("tree must have node labels {0, ..., n - 1***REMOVED***")
    ...
    for v in sequence:
        ...
        if v < 0 or v > n - 1:
            raise nx.NetworkXError(
                f"Invalid Prufer sequence: Values must be between 0 and {n-1***REMOVED***, got {v***REMOVED***"
***REMOVED***
        T.add_edge(u, v)
        not_orphaned.add(u)
        degree[v] -= 1
