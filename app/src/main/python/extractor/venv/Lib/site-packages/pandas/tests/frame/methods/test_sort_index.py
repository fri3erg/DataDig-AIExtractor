...
***REMOVED***,
    ...
    ***REMOVED***,
    ...
    def test_sort_index_nan(self):
        ...
        expected = DataFrame(
            {"A": [4, 1, 2, np.nan, 1, 6, 8], "B": [5, 9, np.nan, 5, 2, 5, 4]***REMOVED***,
        ...
        expected = DataFrame(
            {"A": [8, 6, 1, np.nan, 2, 1, 4], "B": [4, 5, 2, 5, np.nan, 9, 5]***REMOVED***,
        ...
        expected = DataFrame(
            {"A": [4, 8, 6, 1, np.nan, 2, 1], "B": [5, 4, 5, 2, 5, np.nan, 9]***REMOVED***,
    ...
    def test_sort_index_multi_index(self):
        ...
        df = DataFrame(
            {"a": [3, 1, 2], "b": [0, 0, 0], "c": [0, 1, 2], "d": list("abc")***REMOVED***
        )
        result = df.set_index(list("abc")).sort_index(level=list("ba"))

        expected = DataFrame(
            {"a": [1, 2, 3], "b": [0, 0, 0], "c": [1, 2, 0], "d": list("bca")***REMOVED***
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({"A": [1, 2, 3]***REMOVED***, {"A": [2, 3, 1]***REMOVED***, False, True, [0, 1, 2]),
            ({"A": [1, 2, 3]***REMOVED***, {"A": [1, 3, 2]***REMOVED***, True, True, [0, 1, 2]),
            ({"A": [1, 2, 3]***REMOVED***, {"A": [2, 3, 1]***REMOVED***, False, False, [5, 3, 2]),
            ({"A": [1, 2, 3]***REMOVED***, {"A": [1, 3, 2]***REMOVED***, True, False, [2, 3, 5]),
        ],
    )
    ...
    def test_respect_ignore_index(self, inplace, ignore_index):
        # GH 43591
        df = DataFrame({"a": [1, 2, 3]***REMOVED***, index=RangeIndex(4, -1, -2))
        result = df.sort_index(
        ...
        if inplace:
            result = df
        if ignore_index:
            expected = DataFrame({"a": [1, 2, 3]***REMOVED***)
        else:
            expected = DataFrame({"a": [1, 2, 3]***REMOVED***, index=RangeIndex(4, -1, -2))
...
***REMOVED***,
    ...
    def test_sort_index_with_categories(self, categories):
        # GH#23452
        df = DataFrame(
            {"foo": range(len(categories))***REMOVED***,
...
***REMOVED***,
        ...
        result = df.sort_index()
        expected = DataFrame(
            {"foo": reversed(range(len(categories)))***REMOVED***,
            index=CategoricalIndex(
...
class TestDataFrameSortIndexKey:
    def test_sort_multi_index_key(self):
        ...
        result = df.sort_index(level=list("ac"), key=lambda x: -x)
        expected = DataFrame(
            {"a": [3, 2, 1], "b": [0, 0, 0], "c": [0, 2, 1], "d": list("acb")***REMOVED***
        ).set_index(list("abc"))
    ...
    def test_sort_multi_index_key_str(self):
        # GH 25775, testing that sorting by index works with a multi-index.
        df = DataFrame(
            {"a": ["B", "a", "C"], "b": [0, 1, 0], "c": list("abc"), "d": [0, 1, 2]***REMOVED***
        ).set_index(list("abc"))

        result = df.sort_index(level="a", key=lambda x: x.str.lower())

        expected = DataFrame(
            {"a": ["a", "B", "C"], "b": [1, 0, 0], "c": list("bac"), "d": [1, 0, 2]***REMOVED***
        ).set_index(list("abc"))
        tm.assert_frame_equal(result, expected)
    ...
    def test_sort_index_multiindex_sort_remaining(self, ascending):
        # GH #24247
        df = DataFrame(
            {"A": [1, 2, 3, 4, 5], "B": [10, 20, 30, 40, 50]***REMOVED***,
            index=MultiIndex.from_tuples(
                [("a", "x"), ("a", "y"), ("b", "x"), ("b", "y"), ("c", "x")]
***REMOVED***,
        )
        ...
        if ascending:
            expected = DataFrame(
                {"A": [1, 3, 5, 2, 4], "B": [10, 30, 50, 20, 40]***REMOVED***,
                index=MultiIndex.from_tuples(
                    [("a", "x"), ("b", "x"), ("c", "x"), ("a", "y"), ("b", "y")]
    ***REMOVED***,
***REMOVED***
            ...
            expected = DataFrame(
                {"A": [2, 4, 1, 3, 5], "B": [20, 40, 10, 30, 50]***REMOVED***,
                index=MultiIndex.from_tuples(
                    [("a", "y"), ("b", "y"), ("a", "x"), ("b", "x"), ("c", "x")]
    ***REMOVED***,
***REMOVED***
...
def test_sort_index_with_sliced_multiindex():
    ...
    df = DataFrame({"x": range(len(mi))***REMOVED***, index=mi)
    result = df.iloc[0:6].sort_index()

    expected = DataFrame(
        {"x": [0, 1, 2, 5, 3, 4]***REMOVED***,
        index=MultiIndex.from_tuples(
            [
                ("a", "10"),
                ("a", "18"),
