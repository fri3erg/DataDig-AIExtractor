...
@nx._dispatch(node_attrs={"time": None, "weight": 1***REMOVED***)
def cd_index(G, node, time_delta, *, time="time", weight=None):
    ...
    >>> G = nx.DiGraph()
    >>> nodes = {
    ...     1: {"time": datetime(2015, 1, 1)***REMOVED***,
    ...     2: {"time": datetime(2012, 1, 1), 'weight': 4***REMOVED***,
    ...     3: {"time": datetime(2010, 1, 1)***REMOVED***,
    ...     4: {"time": datetime(2008, 1, 1)***REMOVED***,
    ...     5: {"time": datetime(2014, 1, 1)***REMOVED***
    ... ***REMOVED***
    ...
    >>> node_times = {1: 2015, 2: 2012, 3: 2010, 4: 2008, 5: 2014***REMOVED***
    ...
    .. math::
        CD_{t***REMOVED***=\frac{1***REMOVED***{n_{t***REMOVED******REMOVED***\sum_{i=1***REMOVED***^{n***REMOVED***\frac{-2f_{it***REMOVED***b_{it***REMOVED***+f_{it***REMOVED******REMOVED***{w_{it***REMOVED******REMOVED***,

    where `f_{it***REMOVED***` equals 1 if `i` cites the focal patent else 0, `b_{it***REMOVED***` equals
    1 if `i` cites any of the focal patents successors else 0, `n_{t***REMOVED***` is the number
    of forward citations in `i` and `w_{it***REMOVED***` is a matrix of weight for patent `i`
    at time `t`.
    ...
    try:
        ...
        pred = {i for i in G.pred[node] if G.nodes[i][time] <= target_date***REMOVED***
    except:
    ...
    n = len(pred.union(*(G.pred[s].keys() - {node***REMOVED*** for s in G[node])))
    if n == 0:
