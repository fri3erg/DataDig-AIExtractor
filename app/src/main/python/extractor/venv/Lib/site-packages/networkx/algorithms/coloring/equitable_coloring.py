...
def pad_graph(G, num_colors):
    ...
    if n_ != s * (r + 1):
        ...
        K = nx.relabel_nodes(nx.complete_graph(p), {idx: idx + n_ for idx in range(p)***REMOVED***)
...
def equitable_color(G, num_colors):
    ...
    nodes_to_int = {***REMOVED***
    int_to_nodes = {***REMOVED***
    ...
    if r_ >= num_colors:
        raise nx.NetworkXAlgorithmError(
            f"Graph has maximum degree {r_***REMOVED***, needs "
            f"{r_ + 1***REMOVED*** (> {num_colors***REMOVED***) colors for guaranteed coloring."
        )
    ...
    # L = {node: list(G.neighbors(node)) for node in G.nodes***REMOVED***
    L_ = {node: [] for node in G.nodes***REMOVED***

    # Arbitrary equitable allocation of colors to nodes.
    F = {node: idx % num_colors for idx, node in enumerate(G.nodes)***REMOVED***
    ...
    return {int_to_nodes[x]: F[x] for x in int_to_nodes***REMOVED***
