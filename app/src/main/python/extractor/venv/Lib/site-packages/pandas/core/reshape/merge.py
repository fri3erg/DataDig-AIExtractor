...
def _cross_merge(
    ...
    cross_col = f"_cross_{uuid.uuid4()***REMOVED***"
    left = left.assign(**{cross_col: 1***REMOVED***)
    right = right.assign(**{cross_col: 1***REMOVED***)
...
def merge_ordered(
    ...
    fill_method : {'ffill', None***REMOVED***, default None
    ...
    how : {'left', 'right', 'outer', 'inner'***REMOVED***, default 'outer'
    ...
    >>> df2 = pd.DataFrame({"key": ["b", "c", "d"], "rvalue": [1, 2, 3]***REMOVED***)
    >>> df2
      key  rvalue
    0   b       1
    1   c       2
    ...
    if left_by is not None:
        ...
        if len(check) != 0:
            raise KeyError(f"{check***REMOVED*** not found in left columns")
        ...
        if len(check) != 0:
            raise KeyError(f"{check***REMOVED*** not found in right columns")
...
def merge_asof(
    ...
    >>> left = pd.DataFrame({"left_val": ["a", "b", "c"]***REMOVED***, index=[1, 5, 10])
    >>> left
       left_val
    1         a
    5         b
    ...
    >>> right = pd.DataFrame({"right_val": [1, 2, 3, 6, 7]***REMOVED***, index=[1, 2, 3, 6, 7])
    >>> right
       right_val
    1          1
    2          2
...
class _MergeOperation:
    ...
    def __init__(
        ...
        if not is_bool(left_index):
            raise ValueError(
                f"left_index parameter must be of type bool, not {type(left_index)***REMOVED***"
***REMOVED***
        if not is_bool(right_index):
            raise ValueError(
                f"right_index parameter must be of type bool, not {type(right_index)***REMOVED***"
***REMOVED***
        ...
        if _left.columns.nlevels != _right.columns.nlevels:
            msg = (
                ...
                f"({_left.columns.nlevels***REMOVED*** levels on the left, "
                f"{_right.columns.nlevels***REMOVED*** on the right)"
***REMOVED***
        ...
        for i in ["_left_indicator", "_right_indicator"]:
            if i in columns:
                raise ValueError(
                    ...
                    f"data contains a column named {i***REMOVED***"
    ***REMOVED***
...
***REMOVED***:
    ...
    def _maybe_add_join_keys(
        ...
        ***REMOVED***
                    ...
                    result.insert(i, name or f"key_{i***REMOVED***", key_col)
...
***REMOVED***
            ...
            msg = (
                f"You are trying to merge on {lk.dtype***REMOVED*** and {rk.dtype***REMOVED*** columns "
                f"for key '{name***REMOVED***'. If you wish to proceed you should use pd.concat"
***REMOVED***
...
***REMOVED*** or (lk.dtype.kind == "M" and rk.dtype.kind == "M"):
            ...
            if self.left_index and self.right_index:
                ...
                if len(common_cols) == 0:
                    raise MergeError(
                        ...
                        f"Merge options: left_on={left_on***REMOVED***, "
                        f"right_on={right_on***REMOVED***, "
                        f"left_index={self.left_index***REMOVED***, "
                        f"right_index={self.right_index***REMOVED***"
        ***REMOVED***
                if (
                    not left_cols.join(common_cols, how="inner").is_unique
                    or not right_cols.join(common_cols, how="inner").is_unique
    ***REMOVED***:
                    raise MergeError(f"Data columns not unique: {repr(common_cols)***REMOVED***")
    ...
    ***REMOVED***
            ...
            raise ValueError(
                f'"{validate***REMOVED***" is not a valid argument. '
                "Valid arguments are:\n"
                '- "1:1"\n'
                '- "1:m"\n'
                '- "m:1"\n'
...
def get_join_indexers(
    ...
    how : {'inner', 'outer', 'left', 'right'***REMOVED***, default 'inner'
...
def _asof_by_function(direction: str):
    name = f"asof_join_{direction***REMOVED***_on_X_by_Y"
...
class _AsOfMerge(_OrderedMerge):
    ...
    def __init__(
        ...
        if self.direction not in ["backward", "forward", "nearest"]:
            raise MergeError(f"direction invalid: {self.direction***REMOVED***")

        # validate allow_exact_matches
        if not is_bool(self.allow_exact_matches):
            msg = (
                "allow_exact_matches must be boolean, "
                f"passed {self.allow_exact_matches***REMOVED***"
***REMOVED***
            raise MergeError(msg)
    ...
    ***REMOVED***
        ...
        if (
            ...
            raise MergeError(
                f"Incompatible merge dtype, {repr(ro_dtype)***REMOVED*** and "
                f"{repr(lo_dtype)***REMOVED***, both sides must have numeric dtype"
***REMOVED***
    ...
    ***REMOVED***:
                    ...
                    # for them here.
                    msg = (
                        f"incompatible merge keys [{i***REMOVED***] {repr(left.dtype)***REMOVED*** and "
                        f"{repr(right.dtype)***REMOVED***, both sides category, but not equal ones"
        ***REMOVED***
                else:
                    msg = (
                        f"incompatible merge keys [{i***REMOVED***] {repr(left.dtype)***REMOVED*** and "
                        f"{repr(right.dtype)***REMOVED***, must be the same type"
        ***REMOVED***
                raise MergeError(msg)
    ...
    def _validate_tolerance(self, left_join_keys: list[ArrayLike]) -> None:
        ...
        if self.tolerance is not None:
            ...
            msg = (
                f"incompatible tolerance {self.tolerance***REMOVED***, must be compat "
                f"with type {repr(lt.dtype)***REMOVED***"
***REMOVED***
...
***REMOVED***:
    ...
    def _convert_values_for_libjoin(
        ...
        if not Index(values).is_monotonic_increasing:
            if isna(values).any():
                raise ValueError(f"Merge keys contain null values on {side***REMOVED*** side")
            raise ValueError(f"{side***REMOVED*** keys must be sorted")
...
def _factorize_keys(
    ...
    `sort=True`, the encoding will be `{0: 'a', 1: 'b', 2: 'c'***REMOVED***`:

    >>> pd.core.reshape.merge._factorize_keys(lk, rk)
    (array([0, 2, 1]), array([0, 2]), 3)

    With the `sort=False`, the encoding will correspond to the order
    in which the unique elements first appear: `{0: 'a', 1: 'c', 2: 'b'***REMOVED***`:
...
def _validate_operand(obj: DataFrame | Series) -> DataFrame:
    if isinstance(obj, ABCDataFrame):
        ...
        raise TypeError(
            f"Can only merge Series or DataFrame objects, a {type(obj)***REMOVED*** was passed"
        )
...
def _items_overlap_with_suffix(
    ...
    if not is_list_like(suffixes, allow_sets=False) or isinstance(suffixes, dict):
        raise TypeError(
            f"Passing 'suffixes' as a {type(suffixes)***REMOVED***, is not supported. "
    ...
    if not lsuffix and not rsuffix:
        raise ValueError(f"columns overlap but no suffix specified: {to_rename***REMOVED***")
    ...
    def renamer(x, suffix: str | None):
        ...
        if x in to_rename and suffix is not None:
            return f"{x***REMOVED***{suffix***REMOVED***"
        return x
    ...
    if dups:
        raise MergeError(
            f"Passing 'suffixes' which cause duplicate columns {set(dups)***REMOVED*** is "
            f"not allowed.",
        )
