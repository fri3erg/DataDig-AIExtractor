...
# Cache for lookfor: {id(module): {name: (docstring, kind, index), ...***REMOVED***...***REMOVED***
...
def _lookfor_generate_cache(module, import_modules, regenerate):
    ...
    Returns
    -------
    cache : dict {obj_full_name: (docstring, kind, index), ...***REMOVED***
...
def safe_eval(source):
    ...
    >>> np.safe_eval('[1, 2, 3]')
    [1, 2, 3]
    >>> np.safe_eval('{"foo": ("bar", 10.0)***REMOVED***')
    {'foo': ('bar', 10.0)***REMOVED***
...
def _opt_info():
    ...
    for feature in __cpu_dispatch__:
        if __cpu_features__[feature]:
            enabled_features += f" {feature***REMOVED****"
        else:
            enabled_features += f" {feature***REMOVED***?"
