...
***REMOVED***
    ...
    if tz_parsed is not None:
        ...
        dt64_values = result.view(f"M8[{dtype.unit***REMOVED***]")
...
def _array_strptime_with_fallback(
    ...
    if tz_out is not None:
        ...
        unit = np.datetime_data(result.dtype)[0]
        res = Index(result, dtype=f"M8[{unit***REMOVED***, UTC]", name=name)
        return res
...
def _to_datetime_with_unit(arg, unit, name, utc: bool, errors: str) -> Index:
    ...
    if isinstance(arg, IntegerArray):
        arr = arg.astype(f"datetime64[{unit***REMOVED***]")
        tz_parsed = None
    else:
        arg = np.asarray(arg)
        ...
        if arg.dtype.kind in "iu":
            ...
            arr = arg.astype(f"datetime64[{unit***REMOVED***]", copy=False)
            try:
            ...
            ***REMOVED***
                    raise OutOfBoundsDatetime(
                        f"cannot convert input with unit '{unit***REMOVED***'"
        ***REMOVED***
...
***REMOVED*** from err
        ...
        if np.any(arg > j_max) or np.any(arg < j_min):
            raise OutOfBoundsDatetime(
                f"{original***REMOVED*** is Out of Bounds for origin='julian'"
***REMOVED***
    else:
        # arg must be numeric
        if not (
            ...
            raise ValueError(
                f"'{arg***REMOVED***' is not compatible with origin='{origin***REMOVED***'; "
...
***REMOVED***
        ...
        try:
            ...
            raise OutOfBoundsDatetime(f"origin {origin***REMOVED*** is Out of Bounds") from err
...
***REMOVED*** from err
        ...
        if offset.tz is not None:
            raise ValueError(f"origin offset {offset***REMOVED*** must be tz-naive")
...
def to_datetime(
    ...
    errors : {'ignore', 'raise', 'coerce'***REMOVED***, default 'raise'
    ...
    **Timezones and time offsets**
    ...
    DatetimeIndex(['2018-10-26 12:00:00+00:00', '2020-01-01 18:00:00+00:00'],
                  dtype='datetime64[ns, UTC]', freq=None)
    """
    if exact is not lib.no_default and format in {"mixed", "ISO8601"***REMOVED***:
...
def _assemble_from_unit_mappings(arg, errors: DateTimeErrorChoices, utc: bool):
    ...
    unit = {k: f(k) for k in arg.keys()***REMOVED***
    unit_rev = {v: k for k, v in unit.items()***REMOVED***
    ...
    if len(req):
        ...
        raise ValueError(
            ...
            f"[year, month, day] be specified: [{_required***REMOVED***] is missing"
        )
    ...
    if len(excess):
        _excess = ",".join(excess)
        raise ValueError(
            f"extra keys have been passed to the datetime assemblage: [{_excess***REMOVED***]"
        )
    ...
    try:
        ...
        raise ValueError(f"cannot assemble the datetimes: {err***REMOVED***") from err
    ...
    for u in units:
        ...
        if value is not None and value in arg:
            try:
                ...
                raise ValueError(
                    f"cannot assemble the datetimes [{value***REMOVED***]: {err***REMOVED***"
    ***REMOVED*** from err
    return values
