...
def subgraph_centrality_exp(G):
    ...
    >>> print([f"{node***REMOVED*** {sc[node]:0.2f***REMOVED***" for node in sorted(sc)])
...
def subgraph_centrality(G):
    ...
    .. math::
       ...
       SC(u)=\sum_{j=1***REMOVED***^{N***REMOVED***(v_{j***REMOVED***^{u***REMOVED***)^2 e^{\lambda_{j***REMOVED******REMOVED***,
...
def communicability_betweenness_centrality(G):
    ...
    .. math::
         ...
         \omega_{r***REMOVED*** = \frac{1***REMOVED***{C***REMOVED***\sum_{p***REMOVED***\sum_{q***REMOVED***\frac{G_{prq***REMOVED******REMOVED***{G_{pq***REMOVED******REMOVED***,
         p\neq q, q\neq r,

    where
    `G_{prq***REMOVED***=(e^{A***REMOVED***_{pq***REMOVED*** - (e^{A+E(r)***REMOVED***)_{pq***REMOVED***`  is the number of walks
    involving node r,
    `G_{pq***REMOVED***=(e^{A***REMOVED***)_{pq***REMOVED***` is the number of closed walks starting
    at node `p` and ending at node `q`,
    and `C=(n-1)^{2***REMOVED***-(n-1)` is a normalization factor equal to the
    number of terms in the sum.

    The resulting `\omega_{r***REMOVED***` takes values between zero and one.
    ...
    >>> print([f"{node***REMOVED*** {cbc[node]:0.2f***REMOVED***" for node in sorted(cbc)])
...
def estrada_index(G):
    ...
    `\lambda_{1***REMOVED***\leq\lambda_{2***REMOVED***\leq\cdots\lambda_{n***REMOVED***`
    be a non-increasing ordering of the eigenvalues of its adjacency
    matrix `A`. The Estrada index is ([1]_, [2]_)

    .. math::
        EE(G)=\sum_{j=1***REMOVED***^n e^{\lambda _j***REMOVED***.
