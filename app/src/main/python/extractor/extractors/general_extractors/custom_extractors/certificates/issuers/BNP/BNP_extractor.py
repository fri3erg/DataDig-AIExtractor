...
from .cleaning_bnp import check_for, regex_callable, regex_cleaning
from extractors.general_extractors.custom_extractors.certificates.issuers.BNP.tags_bnp import (
    InformazioniBaseBNP,
...
from extractors.general_extractors.utils import extract_regex_text, is_in_text
from extractors.models import Models
from extractors.general_extractors.utils import extract_between
from .config_bnp import prompts, word_representation
from .tags_bnp import NA, TabellaAllegatoPremioBNP, TabellaAllegatoScadenzaBNP, TabellaFirstInfoBNP, TabellaMainInfoBNP
from .....config.json_config.json_bnp import renaming
from .....config.json_config.json_kid import renaming as kid_renaming
...
class BNPDerivatiKidExtractor(DerivatiKidExtractor):
    ...
    def get_tables(self):
        ...
        try:
            ...
            performance_table, _ = self._extract_table(
                word_representation[self.language]["performance"], black_list_pages=[0]
***REMOVED***
    ...
    def extract_basic_info(self):
        ...
        """
        try:
            extraction = {"description": extract_between(self.text[0].page_content[:800], "Prodotto", "codice")***REMOVED***
            extraction.update(
                dict(
                    llm_extraction_and_tag(
                        [self.text[1]], prompts[self.language]["general_info_bnp"], InformazioniBaseBNP, self.file_id
        ***REMOVED***
    ***REMOVED***
***REMOVED***
        ...
        try:
            ...
            ***REMOVED***
            str_to_check = {
                ...
                "leva_airbag": NA,
            ***REMOVED***
            ret = extract_regex_text(regex_callable, check_for, self.text[0], booleans_to_check, str_to_check)
            ...
            ret = {**booleans_to_check, **str_to_check***REMOVED***
    ...
    def extract_main_info(self):
        ...
        extraction = self.extract_from_multiple_tables(
            pages_for_main, [(prompts[self.language]["main_info_bnp"], TabellaMainInfoBNP)], complex=True
        )
    ...
    def extract_allegato(self):
        ...
        try:
            if len(self.di_tables_pages) > 3:
                ...
                extraction = self.extract_from_multiple_tables(
                    list(range(3, len(self.di_tables_pages))),
                    [
                        (prompts[self.language]["allegato_bnp_premio"], TabellaAllegatoPremioBNP),
                        (prompts[self.language]["allegato_bnp_scadenza"], TabellaAllegatoScadenzaBNP),
                    ],
                    complex=True,
    ***REMOVED***
                # Define error list
    ...
    ***REMOVED***
            ...
            ***REMOVED***
            ...
            print(f"extract_allegato error: {error***REMOVED***")
    ...
    def extract_sottostanti(self):
        ...
        sottostante, _ = self._extract_table_only_header(
            word_representation[self.language]["sottostante_bnp"], pages_to_check=[0, 1]
        )

        extraction = general_table_inspection(sottostante, TabellaSottostanteBNP, self.file_id)
        ...
        if len(self.di_tables_pages) > 3 and (
            ...
            sottostante, _ = self._extract_table_only_header(
                word_representation[self.language]["sottostante_bnp"], list(range(3, len(self.di_tables_pages)))
***REMOVED***
    ...
    def extract_entryexit_management_costs(self):
        ...
        extraction = self.extract_from_multiple_tables([1, 2], [TabellaCostiGestione, TabellaCostiIngresso])
        # extraction = clean_response_regex( "main_info", self.language, extraction)
    ...
    def extract_first_info(self):
        ...
        try:
            extraction = llm_extraction_and_tag(
                [self.text[0]], prompts[self.language]["first_info_bnp"], TabellaFirstInfoBNP, self.file_id
***REMOVED***
    ...
    def _write_to_excel(self, complete, complete2, allegato, sottostanti, api_costs, filename):
        ...
        with pd.ExcelWriter(
            "results\\13febbraio\\resultsmarco_{***REMOVED***.xlsx".format(os.path.basename(self.file_id)),
            engine="xlsxwriter",
        ) as excel_writer:
...
***REMOVED***
    ...
    def process(self):
        ...
        try:
            ...
            functions_parameters = {
                "allegato": {"function": self.extract_allegato***REMOVED***,
            ...
            complete = self.raccorda(
                {
                    ...
                    **dict(clean_response_regex(regex_cleaning[self.language]["bnp_main"], main_info)),
                    **dict(first_info),
                ***REMOVED***,
                renaming,
                keep=True,
